...
function normalizePublicContract(
...
function main(): void {
	...
	if (CHECK_ONLY) {
		...
		assertDeepEqual(
			"catalog_manifest_drift:template_content_mismatch",
			template,
			manifest,
		);

		const generatedRaw = JSON.parse(
			readFileSync(generatedPublicCatalogPath, "utf8"),
		) as Record<string, unknown>;
		const generatedCounts = generatedRaw.counts as
			| Record<string, unknown>
			| undefined;
		if (
			generatedCounts?.skillCount !== expectedCounts.skillCount ||
			generatedCounts?.promptCount !== expectedCounts.promptCount
		) {
		...
		assertDeepEqual(
			...
			normalizePublicContract(
				publicContract as unknown as Record<string, unknown>,
			),
		);
