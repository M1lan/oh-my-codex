...
describe("prompt inventory", () => {
	it("counts prompt surfaces, absolute directives, markers, and duplicate fragments", async () => {
		...
		try {
			...
			await mkdir(join(root, "docs", "prompt-guidance-fragments"), {
				recursive: true,
			});
			...
			const repeated =
				"AUTO-CONTINUE for clear, already-requested, low-risk, reversible local work with evidence.";
			await writeFile(
				join(root, "AGENTS.md"),
			...
			await writeFile(
				join(root, "skills", "worker", "SKILL.md"),
				"# Worker\nALWAYS claim tasks.\n",
			);
			await writeFile(
				join(root, "docs", "prompt-guidance-contract.md"),
				"# Contract\n",
			);
			await writeFile(join(root, "docs", "guidance-schema.md"), "# Schema\n");
			await writeFile(
				join(root, "docs", "prompt-guidance-fragments", "core.md"),
				"fragment\n",
			);
			await writeFile(
				join(root, "src", "hooks", "prompt-guidance-contract.ts"),
				"export {};\n",
			);
			await writeFile(
				join(root, "src", "config", "generator.ts"),
				"export {};\n",
			);
			...
			assert.equal(
				report.surfaces.find(
					(surface) => surface.path === "templates/AGENTS.md",
				)?.markers["<!-- OMX:RUNTIME:START -->"],
				1,
			);
