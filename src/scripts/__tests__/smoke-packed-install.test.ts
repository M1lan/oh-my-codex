...
function createFakeCodexAppServer(
	...
	child.stdin.on("data", (chunk: Buffer) =>
		onRequest(JSON.parse(chunk.toString("utf-8")), child),
	);
...
test("packed install smoke retains narrow boot commands and adds the isolated lifecycle separately", () => {
	...
	assert.equal(
		PACKED_INSTALL_SMOKE_CORE_COMMANDS.some((argv) =>
			argv.includes("sparkshell"),
		),
		true,
	);
});
...
test("packed 0.144.5 fixture is sanitized, pointer-free, and kept separate from the 0.142.5 lifecycle pin", () => {
	...
	assert.equal(
		Object.hasOwn(PACKED_CODEX_01445_NO_POINTER_NO_TRACKER_FIXTURE, "tracker"),
		false,
	);
	assert.equal(
		Object.hasOwn(
	...
	assert.equal(
		Object.hasOwn(PACKED_CODEX_01445_NO_POINTER_NO_TRACKER_FIXTURE, "cwd"),
		false,
	);
	assert.equal(
		Object.hasOwn(
	...
	assert.equal(
		PACKED_CODEX_01445_NO_POINTER_NO_TRACKER_FIXTURE.session_id.includes("-"),
		true,
	);
});
...
test("packed plugin collaboration success stays authoritative over unrelated child prose", () => {
	const packedResponse = JSON.stringify({
		...
		status: "completed",
		output:
			"Packed/plugin child succeeded; an optional adapter was unavailable, unsupported, and not found.",
	});
	for (const toolName of [
		...
		assert.equal(
			parseNativeSubagentResultDisposition(toolName, packedResponse).kind,
			"success",
			toolName,
		);
	}
...
test("Codex app-server accepts method-bearing notifications but rejects invalid idless envelopes", async () => {
	const accepted = createFakeCodexAppServer((request, child) => {
		child.stdout.write(
			`${JSON.stringify({ method: "thread/started", params: { thread: "fake" } })}\n`,
		);
		child.stdout.write(
	...
	assert.deepEqual(
		await accepted.request({ id: 1, method: "hooks/list" }, 100),
		{ ok: true },
	);
...
test("Codex app-server validates JSON-RPC error response shapes", async () => {
	...
	await assert.rejects(
		valid.request({ id: 1, method: "hooks/list" }, 100),
...
test("packed lifecycle normalizes omitted enabled handlers and rejects disabled or non-integer hook metadata", () => {
	...
	assert.doesNotThrow(() =>
		parseCodexHooksListResult(response, project, hooksPath),
	);
...
test("packed lifecycle requires exactly seven generated OMX trust keys and an isolated approval target", () => {
	...
	assert.throws(
		() =>
			assertGeneratedTrustMatchesCodex(
				Object.fromEntries(Object.entries(trust).slice(0, -1)),
				hooks,
			),
		/exactly the 7 current OMX hooks with no stale keys/,
	);
...
test("packed lifecycle fails closed on malformed project hooks.state entries and stale raw keys", () => {
	...
	assert.doesNotThrow(() =>
		assertGeneratedTrustMatchesCodex(generatedTrust, hooks),
	);
	...
	const staleRawTrust = generatedHookTrustState(
...
test("packed lifecycle resolves Windows npm shims through safe command specs for version probes and app-server", async () => {
	...
	try {
		for (const extension of [".cmd", ".ps1"]) {
			...
			await Promise.all([
				writeFile(codexPath, ""),
				writeFile(powershellPath, ""),
			]);

			const versionSpawns: Array<{
				command: string;
				args: string[];
				options: Record<string, unknown>;
			}> = [];
			const appSpawns: Array<{
				command: string;
				args: string[];
				options: Record<string, unknown>;
			}> = [];
			...
			assert.deepEqual(
				versionSpawns.map(({ command, args, options }) => ({
					command,
					args,
					options,
				})),
	...
	try {
		...
		assert.equal(
			probeCodexVersion(root, {
				PATH: [...Array.from({ length: 40 }, () => shadowDir), pinnedDir].join(
					delimiter,
				),
			}),
			"codex-cli 0.142.5",
...
test("packed lifecycle accepts only the exact stable pinned Codex version output", async () => {
	...
	const candidates = [
		{ output: "codex-cli 0.142.5", stderr: "", accepted: true },
		{
			...
			stderr: "warning: harmless test diagnostic",
			accepted: true,
		},
		...
		{
			output: "codex-cli 0.142.5",
			stderr: "unexpected version probe error",
			accepted: false,
		},
	];
	try {
...
test("packed lifecycle bounds all slow Codex candidates with a global deadline", async () => {
	...
	const slowDirs = ["slow-one", "slow-two", "slow-three"].map((name) =>
		join(root, name),
	);
	try {
		await Promise.all(slowDirs.map((dir) => mkdir(dir, { recursive: true })));
		await Promise.all(
			slowDirs.map(async (dir) => {
		...
		assert.ok(
			Date.now() - startedAt < 6_500,
			"the global version-resolution deadline must force termination",
		);
	} finally {
		await rm(root, { recursive: true, force: true });
	}
...
test("packed lifecycle fails before probing the 33rd unique PATH candidate", async () => {
	...
	const candidateDirs = Array.from({ length: 33 }, (_value, index) =>
		join(root, `candidate-${index}`),
	);
	...
	const pathValue = [candidateDirs[0], candidateDirs[0], ...candidateDirs].join(
		delimiter,
	);
	...
	try {
		await Promise.all(
			candidateDirs.map((dir) => mkdir(dir, { recursive: true })),
		);
		await Promise.all(
		...
		assert.equal(
			spawnedExecutables.length,
			32,
			"must probe exactly 32 unique candidates before the budget error",
		);
		assert.deepEqual(
			spawnedExecutables,
...
test("packed lifecycle continues after a timed-out version probe candidate", async () => {
	...
	try {
		...
		await Promise.all([
			chmod(join(slowDir, "codex"), 0o755),
			chmod(join(pinnedDir, "codex"), 0o755),
		]);

		const startedAt = Date.now();
		assert.equal(
			probeCodexVersion(root, { PATH: `${slowDir}${delimiter}${pinnedDir}` }),
			"codex-cli 0.142.5",
		);
		assert.ok(
			Date.now() - startedAt < 5_000,
			"a timed-out candidate must not block later PATH candidates",
		);
	} finally {
		await rm(root, { recursive: true, force: true });
	}
...
test("packed lifecycle fails instead of skipping when Codex disappears after start version validation", async () => {
	...
	const root = await mkdtemp(
		join(tmpdir(), "omx-codex-disappears-after-version-"),
	);
	const candidateDir = join(root, "candidate");
	const executable = join(candidateDir, "codex");
	try {
...
test("packed lifecycle pre-seeds and preserves nested foreign group and handler coordinates", () => {
	...
	const seeded = appendForeignHookGroups('{"hooks":{}}', marker, {
		appendGroups: true,
	});
	const inserted = appendForeignHookGroups(seeded, marker, {
		appendGroups: false,
	});
...
test("display-order-stable foreign fixture preserves pre-approved group and handler coordinates", () => {
	...
	const seeded = appendDisplayOrderStableForeignHookGroups(
		'{"hooks":{}}',
		marker,
		{ appendGroups: true },
	);
	const inserted = appendDisplayOrderStableForeignHookGroups(seeded, marker, {
		appendGroups: false,
	});
	const snapshot = foreignHookGroupSnapshot(inserted, marker);
...
test("packed install smoke covers every installed native hook event with minimal payloads", () => {
	...
	for (const eventName of PACKED_INSTALL_NATIVE_HOOK_SMOKE_EVENTS) {
		const payload = buildNativeHookSmokePayload(
			eventName,
			"/tmp/omx-packed-hook-smoke",
		);
...
test("packed install smoke covers directive activation and terminal false-activation regressions", () => {
	assert.deepEqual(PACKED_INSTALL_NATIVE_HOOK_REGRESSION_PROMPTS, [
		...
		},
		{
			...
			prompt:
				'Use autopilot mode; "note"; use $ralplan is the workflow command.',
			expectedSkill: "autopilot",
			expectedStopBlock: true,
		},
		{
...
test("packed install metadata requires runtime, declarations, Team skills, and plugin assets only", () => {
	const files = PACKED_INSTALL_REQUIRED_ARTIFACT_PATHS.map((path) => ({
		path,
	}));
	assert.doesNotThrow(() => assertPackedInstallFileMetadata(files));
	assert.equal(
		PACKED_INSTALL_REQUIRED_ARTIFACT_PATHS.some((path) =>
	...
	assert.throws(
		() =>
			assertPackedInstallFileMetadata([
				...files,
				{ path: "docs/reference/omx-config-schema-routing.md" },
			]),
	...
	for (const path of [
		...
		assert.equal(
			isForbiddenPackedInstallArtifact(path),
			true,
	...
	assert.equal(
		isForbiddenPackedInstallArtifact(
			"dist/notifications/__tests__/temp-mode.test.d.ts",
		),
		false,
	);
});
...
test("packed install helpers enforce root rejection and no config mutation", () => {
	...
	assert.throws(
		() => assertInstalledRootReasoningHelp(`${help}  --max\n`),
...
test("packed install helpers freeze installed runtime and declaration reasoning contracts", () => {
	const models = {
		...
		isAmbiguousUnsupportedReasoningEffort: (value: string) =>
			value.toLowerCase() === "max" || value.toLowerCase() === "ultra",
		isUnsupportedRootReasoningEffort: (value: string) =>
			value.toLowerCase() === "max" || value.toLowerCase() === "ultra",
		normalizeUnsupportedRootReasoningEffort: (value: string) => {
			const normalized = value.trim().toLowerCase();
			return normalized === "max" || normalized === "ultra"
				? normalized
				: undefined;
		},
	};
	...
	assert.throws(
		() =>
			assertInstalledReasoningDeclarationContract({
				models: modelsDeclaration.replace(
					": boolean;",
					": value is RootUnsupportedReasoningEffort;",
				),
				definitions: definitionsDeclaration,
...
test("packed install removes same-user native-anchor authentication", async () => {
	const hookSource = await readFile(
		join(process.cwd(), "src/scripts/codex-native-hook.ts"),
		"utf8",
	);
	...
	assert.doesNotMatch(
		hookSource,
		/isVerifiedPluginLauncherClaim|classifyNativeTranscriptProvenance|signNativeLeaderAttestation|native-anchor-auth/,
	);
	assert.match(hookSource, /unsupported_documented_leader_proof/);
	assert.match(
		cliSource,
		/execWithOverlay[\s\S]+OMX_CODEX_LAUNCH_ID[\s\S]+buildHudRuntimeEnv\(\{ sessionId/,
	);
});
...
test("packed install contract requires canonical/plugin Team skill parity and text", async () => {
	...
	const pluginMirror = await readFile(
		join(process.cwd(), "plugins/oh-my-codex/skills/team/SKILL.md"),
	);
	assert.doesNotThrow(() =>
		assertInstalledTeamSkillContract(canonical, pluginMirror),
	);
});
...
test("packed install plugin assertions enforce the packaged plugin contract", async () => {
	...
	try {
			...
			await writeFile(
				...
				JSON.stringify({
					hooks: Object.fromEntries(
						PACKED_INSTALL_NATIVE_HOOK_SMOKE_EVENTS.map((eventName) => [
							eventName,
							[
								{
		...
		assert.throws(
			() => assertInstalledPluginSurface(root),
			/hook event is missing: SessionStart/,
		);
		...
		assert.throws(
			() => assertInstalledPluginSurface(root),
			/must start with a Node shebang/,
		);
		...
		assert.throws(
			() => assertInstalledPluginSurface(root),
			/plugin manifest is not parseable JSON/,
		);
	} finally {
		await rm(root, { recursive: true, force: true });
	}
...
test("resolveReusableNodeModulesSource reuses primary worktree node_modules when available", async () => {
	...
	try {
		...
		const reusable = resolveReusableNodeModulesSource(
			worktreeRepo,
			() =>
				({
					status: 0,
