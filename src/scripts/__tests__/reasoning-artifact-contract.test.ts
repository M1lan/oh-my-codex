...
test("compiled reasoning artifacts preserve exact surface-specific max contracts", async () => {
	...
	for (const relativePath of requiredArtifacts) {
		assert.equal(
			...
			true,
			`missing compiled artifact: ${relativePath}`,
		);
	}
	...
	const models = await import(
		pathToFileURL(join(dist, "config/models.js")).href
	);
	assert.deepEqual(models.CANONICAL_REASONING_EFFORTS, [
		"low",
		"medium",
		"high",
		"xhigh",
	]);
	assert.deepEqual(models.AMBIGUOUS_UNSUPPORTED_REASONING_EFFORTS, [
		"max",
		"ultra",
	]);
	assert.deepEqual(models.PER_AGENT_REASONING_EFFORTS, [
		"low",
		"medium",
		"high",
		"xhigh",
	...
	assert.deepEqual(models.ROOT_REASONING_EFFORTS, [
		"low",
		"medium",
		"high",
		"xhigh",
	...
	assert.equal(
		models.normalizeUnsupportedRootReasoningEffort("Ultra"),
		"ultra",
	);
	assert.equal(
		models.normalizeUnsupportedRootReasoningEffort("xhigh"),
		undefined,
	);

	const nativeConfig = await import(
		pathToFileURL(join(dist, "agents/native-config.js")).href
	);
	const definitions = await import(
		pathToFileURL(join(dist, "agents/definitions.js")).href
	);
	...
	const team = await import(
		pathToFileURL(join(dist, "team/model-contract.js")).href
	);
	const maxArgs = team.resolveTeamWorkerLaunchArgs({
	...
	const pluginSkill = readFileSync(
		join(root, "plugins/oh-my-codex/skills/team/SKILL.md"),
	);
...
test("compiled root reasoning keeps four-value help and rejects max/ultra without mutation", () => {
	...
	try {
		...
		const help = spawnSync(process.execPath, [omxPath, "reasoning"], {
			encoding: "utf-8",
			env,
		});
		assert.equal(help.status, 0, help.stderr);
		assertInstalledRootReasoningHelp(help.stdout);
		const max = spawnSync(process.execPath, [omxPath, "reasoning", "max"], {
			encoding: "utf-8",
			env,
		});
		assertInstalledRootReasoningRejection(
		...
		const ultra = spawnSync(process.execPath, [omxPath, "reasoning", "ultra"], {
			encoding: "utf-8",
			env,
		});
		assertInstalledRootReasoningRejection(
