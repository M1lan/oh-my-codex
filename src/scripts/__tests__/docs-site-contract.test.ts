...
describe("public docs site contract", () => {
		...
		for (const page of docsPages) {
			...
			for (const href of hrefs) {
				assert.ok(
					...
					`${page} should use a repo-owned relative stylesheet, got ${href}`,
				);
				const resolved = join(root, dirname(page), href);
				assert.ok(
					existsSync(resolved),
					`${page} stylesheet must exist: ${href}`,
				);
			}
		}
	});
	...
	it("defines CSS for the layout classes used by the static docs pages", () => {
		...
		assert.match(
			css,
			/(^|\n)\.grid\s*,\s*\n\.card-grid\s*\{/,
			"docs/style.css must style .grid cards used by docs pages",
		);
		for (const page of docsPages) {
			const content = read(page);
			if (content.includes('class="grid"')) {
				assert.match(
					css,
					/(^|\n)\.grid\s*,/,
					`${page} uses .grid, so docs/style.css must style it`,
				);
			}
		}
	});
	...
	it("documents research workflow boundaries on the public skills page", () => {
		...
		assert.match(
			content,
			/\$best-practice-research[\s\S]*ordinary pre-planning wrapper/,
		);
		assert.match(
			content,
			/\$autoresearch[\s\S]*bounded validator-gated research deliverable/,
		);
		assert.match(content, /\$autoresearch-goal[\s\S]*Codex goal-mode version/);
		assert.match(
			content,
			/Autoresearch findings gathered before planning should feed into <code>\$ralplan<\/code> as evidence/,
		);
	});
});
