	...
	it("does not treat legacy thread_id as leader authority and preserves a child record", async () => {
		...
		try {
			await recordSubagentTurnForSession(cwd, {
				sessionId: "synthetic-leader-session",
				threadId: "synthetic-child-thread",
				kind: "subagent",
				leaderThreadId: "synthetic-leader-thread",
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					cwd,
					session_id: "synthetic-child-thread",
					thread_id: "synthetic-child-thread",
					tool_name: "Bash",
	...
	it("denies the canonical adapted command before forged historical authority artifacts can mutate", async () => {
		...
		try {
			...
			const claimPath = join(
				stateDir,
				"plugin-hook-launches",
				"forged-launch.json",
			);
			const transcriptPath = join(cwd, "forged-child.jsonl");
			const pointer =
				'{"session_id":"forged-canonical","native_session_id":"forged-native","leader_thread_id":"forged-leader"}\n';
			const tracking =
				'{"schemaVersion":1,"sessions":{"forged-canonical":{"session_id":"forged-canonical","leader_thread_id":"forged-leader","leader_attested_at":"1970-01-01T00:00:00.000Z","threads":{"forged-leader":{"thread_id":"forged-leader","kind":"leader"}}}},"pending_role_intents":[{"role":"architect","session_id":"forged-canonical","parent_thread_id":"forged-leader","correlation_token":"forged"}]}\n';
			const marker =
				'{"schema_version":1,"session_id":"forged-canonical","parent_thread_id":"forged-leader","role":"architect"}\n';
			const claim =
				'{"sessionId":"forged-native","signature":"forged-signature"}\n';
			const transcript =
				'{"type":"session_meta","payload":{"id":"forged-native","source":{"subagent":{"thread_spawn":{"parent_thread_id":"forged-leader","task_name":"omx-role-intent-forged"}}}}}\n';
			...
			const result = await dispatchCodexNativeHook(
				{
					hook_event_name: "PreToolUse",
					cwd,
					session_id: "forged-native",
					thread_id: "forged-leader",
					transcript_path: transcriptPath,
					tool_name: "Bash",
					tool_use_id: "forged-tool-use",
					tool_input: {
						command:
							'omx ralplan role-intent write --role architect --parent-thread "$CODEX_THREAD_ID" --json',
					},
