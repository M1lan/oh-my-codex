...
describe("hook-derived-watcher", () => {
	it("uses offset-bounded rollout reads instead of re-reading whole tracked files", async () => {
		const source = await readFile(
			new URL("../hook-derived-watcher.js", import.meta.url),
			"utf-8",
		);
	...
	it("stores watcher state and logs under boxed runtime root when OMX_ROOT is set", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(
					join(boxedRoot, ".omx", "state", "hook-derived-watcher-state.json"),
				),
				true,
			);
			assert.equal(
				existsSync(
					join(cwd, ".omx", "state", "hook-derived-watcher-state.json"),
				),
				false,
			);
			...
			assert.equal(
				logNames.some((name) => name.startsWith("hook-derived-watcher-")),
				true,
			);
		} finally {
...
`,
			...
			const rolloutPath = join(
				todaySessionDir(homeDir),
				"rollout-hook-derived-array.jsonl",
			);
			await writeFile(
				rolloutPath,
				[
			...
			assert.equal(
				...
				true,
				"expected needs-input hook log to be written",
			);
			...
			assert.equal(
				events[0].parser_reason,
				"assistant_message_heuristic_question",
			);
			assert.match(
				String((events[0].context as Record<string, unknown>)?.preview ?? ""),
...
`,
			...
			const rolloutPath = join(
				todaySessionDir(homeDir),
				"rollout-hook-derived-utf8.jsonl",
			);
			await writeFile(
				rolloutPath,
				`${JSON.stringify({
			...
			const watcherStatePath = join(
				...
				".omx",
				"state",
				"hook-derived-watcher-state.json",
			);
			await waitFor(async () => {
				try {
			...
			assert.equal(
				existsSync(hookLogPath),
				false,
				"incomplete UTF-8 and JSON line should not dispatch",
			);
