...
describe("notify-hook tmux injection canonical skill gating", () => {
	it("reads canonical skill-active state from authoritative team state root", async () => {
		...
		try {
			...
			await mkdir(join(teamStateRoot, "sessions", sessionId), {
				recursive: true,
			});
			await writeFile(
				join(teamStateRoot, "session.json"),
				JSON.stringify(
					{ session_id: sessionId, cwd: join(wd, "source-repo") },
					null,
					2,
				),
				"utf-8",
	...
	it("treats missing session canonical state as session-scoped inactive instead of root fallback", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-notify-tmux-missing-canonical-"),
		);
		try {
			...
			const visible = await readVisibleAllowedModes(wd, stateDir, {}, [
				"ralplan",
			]);
