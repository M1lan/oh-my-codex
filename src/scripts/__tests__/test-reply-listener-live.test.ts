...
test("runReplyListenerLiveSmoke exercises Discord and Telegram send + cleanup requests", async () => {
	...
	const fetchImpl: typeof fetch = async (input, init) => {
		...
		if (
			url ===
			"https://discord.com/api/v10/channels/channel-1/messages/discord-message-1"
		) {
	...
	assert.ok(
		logs.some((entry) =>
			entry.includes("Discord probe message sent: discord-message-1"),
		),
	);
	assert.ok(
		logs.some((entry) => entry.includes("Telegram probe message sent: 42")),
	);
});
