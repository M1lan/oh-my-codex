...
PY`,
	...
	it("includes blocking state source and canonical agreement in Stop diagnostics", async () => {
		...
		try {
			...
			assert.equal(
				output.statePath,
				".omx/state/sessions/sess-autopilot-diagnostic/autopilot-state.json",
			);
...
`,
	...
	it("adds resume-by-id instructions for persisted subagents on SessionStart resume", async () => {
		...
		try {
			...
			await writeSessionStart(cwd, sessionId, {
				nativeSessionId: "codex-leader-reopen",
				pid: process.pid,
			});
			...
			assert.match(
				additionalContext,
				/resume_agent\("thread-architect-reopen"\)/,
			);
			assert.match(
				additionalContext,
				/role: architect; lane: plan-review; scope: SessionStart reopen; status: available/,
			);
			...
			const tracking = JSON.parse(
				...
				sessions?: Record<
					string,
					{ threads?: Record<string, { resume_requested_at?: string }> }
				>;
			};
			assert.match(
				tracking.sessions?.[sessionId]?.threads?.["thread-architect-reopen"]
					?.resume_requested_at ?? "",
				/^\d{4}-\d{2}-\d{2}T/,
			);
		} finally {
	...
	it("does not suggest duplicate same-role subagent spawns when reopen ids exist", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-no-duplicates-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			const sessionId = "omx-reuse-session";
			await writeSessionStart(cwd, sessionId, {
				nativeSessionId: "codex-leader-reuse",
				pid: process.pid,
			});
			await writeJson(join(stateDir, "subagent-tracking.json"), {
				schemaVersion: 1,
			...
			assert.match(
				additionalContext,
				/avoid duplicate same-type subagent spawns/,
			);
			assert.match(
				additionalContext,
				/do not spawn a new agent solely because reopen failed/,
			);
	...
	it("surfaces clear warnings for unavailable persisted subagents without spawning replacements", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-warning-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			const sessionId = "omx-warning-session";
			await writeSessionStart(cwd, sessionId, {
				nativeSessionId: "codex-leader-warning",
				pid: process.pid,
			});
			await writeJson(join(stateDir, "subagent-tracking.json"), {
				schemaVersion: 1,
			...
			assert.match(
				additionalContext,
				/No compatible saved subagent id is currently marked reopenable/,
			);
			assert.match(additionalContext, /Reopen warnings:/);
			assert.match(additionalContext, /thread-executor-unavailable/);
			assert.match(
				additionalContext,
				/last failure: Codex reported missing thread id/,
			);
			assert.doesNotMatch(
				additionalContext,
				/resume_agent\("thread-executor-unavailable"\)/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("suppresses child-agent SessionStart hook dispatch at minimal verbosity", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-session-minimal-"),
		);
		const originalCodexHome = process.env.CODEX_HOME;
		try {
			process.env.CODEX_HOME = join(cwd, "codex-home");
	...
	it("allows explicit child-agent lifecycle hook dispatch when includeChildAgents is enabled", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-session-include-"),
		);
		const originalCodexHome = process.env.CODEX_HOME;
		try {
			process.env.CODEX_HOME = join(cwd, "codex-home");
	...
	it("allows child-agent lifecycle hook dispatch at agent verbosity", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-session-agent-"),
		);
		const originalCodexHome = process.env.CODEX_HOME;
		try {
			process.env.CODEX_HOME = join(cwd, "codex-home");
			...
			const transcriptPath = join(
				cwd,
				"agent-verbosity-subagent-rollout.jsonl",
			);
			await writeFile(
				transcriptPath,
				`${JSON.stringify({
	...
	it("suppresses child-agent SessionStart and Stop before the canonical leader session is reconciled (#2831)", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-no-canonical-"),
		);
		const originalCodexHome = process.env.CODEX_HOME;
		try {
			process.env.CODEX_HOME = join(cwd, "codex-home");
	...
	it("preserves root/leader SessionStart dispatch at session verbosity (#2831)", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-root-session-start-preserved-"),
		);
		const originalCodexHome = process.env.CODEX_HOME;
		try {
			process.env.CODEX_HOME = join(cwd, "codex-home");
	...
	it("keeps a self-parented native role thread as subagent evidence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-self-parented-subagent-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.equal(
				tracking.sessions?.[canonicalSessionId]?.threads?.[nativeRoleThreadId]
					?.kind,
				"subagent",
			);
			assert.equal(
	...
	it("prefers the OMX owner session id when a native new session revives HUD", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-hud-owner-session-revive-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("falls back to the canonical session id for malformed HUD owner ids", async () => {
		for (const [index, invalidOwnerSessionId] of [
			"codex-native-hud-owner",
			"omx-../../stale",
		].entries()) {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-native-hook-hud-invalid-owner-revive-"),
			);
			try {
				const stateDir = join(cwd, ".omx", "state");
	...
	it("includes repo-local .omx project-memory during SessionStart when OMX_ROOT is boxed", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-session-boxed-memory-"),
		);
		const boxedRoot = await mkdtemp(
			join(tmpdir(), "omx-native-hook-boxed-root-"),
		);
		...
		try {
			...
			await writeJson(join(cwd, ".omx", "project-memory.json"), {
				...
				directives: [
					{
						directive:
							"Prefer repo-local .omx project memory over boxed runtime fallback.",
						priority: "high",
					},
				],
			});
			...
			assert.match(
				additionalContext,
				/SessionStart should load CLI-written project memory/,
			);
			assert.match(
				additionalContext,
				/Prefer repo-local \.omx project memory over boxed runtime fallback\./,
			...
			assert.doesNotMatch(
				additionalContext,
				/Boxed runtime memory should not win/,
			);
	...
	it("allows same-session/current-thread Ralph Stop continuation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-stop-current-session-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeJson(
				join(stateDir, "sessions", "omx-current-ralph", "ralph-state.json"),
				{
					active: true,
					mode: "ralph",
					current_phase: "executing",
	...
	it("does not resume stale different-session global Ralph Stop state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-stop-stale-global-"),
		);
		try {
	...
	it("does not resume global Ralph Stop state for low-overlap unrelated tasks", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-stop-low-overlap-"),
		);
		try {
			await writeJson(join(cwd, ".omx", "state", "ralph-state.json"), {
				active: true,
				...
				current_phase: "executing",
				task_description:
					"Refactor documentation navigation and README examples",
			});
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					cwd,
					last_user_message:
						"continue auditing billing webhooks and payment retries",
				},
				{ cwd },
			);
	...
	it("allows explicit current user continuation for live-risk Ralph tasks when owner context is current", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-stop-live-current-"),
		);
		try {
			await writeJson(join(cwd, ".omx", "state", "ralph-state.json"), {
				active: true,
				...
				task_description:
					"Deploy billing migration rollback guard for production payments",
			});
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					last_user_message:
						"continue the current ralph deploy billing migration rollback guard",
				},
				{ cwd },
			);
	...
	it("allows current-owner global Ralph Stop continuation with generic current-task wording", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-stop-current-owner-generic-"),
		);
		try {
			await writeJson(join(cwd, ".omx", "state", "ralph-state.json"), {
				active: true,
	...
	it("allows same-thread global Ralph Stop continuation without payload text for non-live tasks", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-stop-same-thread-no-text-"),
		);
		try {
			await writeJson(join(cwd, ".omx", "state", "ralph-state.json"), {
				active: true,
	...
	it("blocks no-text global Ralph Stop continuation for live-risk tasks", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-stop-live-no-text-"),
		);
		try {
			await writeJson(join(cwd, ".omx", "state", "ralph-state.json"), {
				active: true,
	...
	it("blocks no-text global Ralph Stop continuation for issue 2974 operational live-risk tasks", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-stop-issue-2974-live-no-text-"),
		);
		try {
			await writeJson(join(cwd, ".omx", "state", "ralph-state.json"), {
				active: true,
				...
				current_phase: "executing",
				task_description:
					"Implement Telegram assistant GPT switch plan on VPS service with cron restart send notify workflow",
			});
	...
	it("injects deep-interview config overrides into UserPromptSubmit developer context", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-deep-interview-config-"),
		);
		try {
			await mkdir(join(cwd, ".omx", "state"), { recursive: true });
			await writeFile(
...
`,
	...
	it("proves UserPromptSubmit context changes before and after adding deep-interview config", async () => {
		...
		try {
			...
			assert.doesNotMatch(
				beforeOutput,
				/Deep-interview config override active/,
			);
...
`,
			...
			assert.equal(
				after.skillState?.deep_interview_config?.profile,
				"standard",
			);
...
`,
			...
			assert.equal(
				result.skillState?.deep_interview_config?.enableChallengeModes,
				false,
			);
			assert.match(serializedOutput, /Deep-interview config override active/);
...
`,
			...
			assert.equal(continued.skillState?.skill, "deep-interview");
			assert.equal(
				continued.skillState?.deep_interview_config?.profile,
				"deep",
			);
			assert.match(serializedOutput, /Deep-interview config override active/);
			assert.match(serializedOutput, /profile=deep/);
	...
	it("keeps the documented deep-interview Suggested Config reflected in UserPromptSubmit context", async () => {
		...
		try {
			...
			assert.equal(
				result.skillState?.deep_interview_config?.profile,
				"standard",
			);
...
`,
	...
	it("nudges next-goal prompts when a completed Codex goal cleanup step remains", async () => {
		...
		try {
			...
			assert.match(
				output,
				/hooks only nudge and must not mutate Codex goal state/,
			);
	...
	it("does not block ordinary Stop after completed Ultragoal cleanup remains", async () => {
		...
		try {
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					last_assistant_message:
						"Implemented the requested fix and verified the focused tests.",
				},
				{ cwd },
			);
			...
			assert.notEqual(
				...
				"completed_codex_goal_cleanup_required",
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					last_assistant_message:
						"Starting another ultragoal now; create_goal payload follows after /goal clear already cleared native state.",
				},
				{ cwd },
			);
	...
	it("does not inject Conductor guidance into typed agent-role autopilot continuations", async () => {
		...
		try {
			...
			assert.doesNotMatch(
				message,
				/Golden Rule: When the Main agent is acting in Conductor mode/,
			);
	...
	it("lets the current canonical leader boundary beat stale global subagent tracking with a distinct prompt thread id", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", staleSessionId, "autopilot-state.json"),
				),
				false,
			);
		} finally {
	...
	it("injects autopilot ralplan consensus gate guidance on prompt activation", async () => {
		...
		try {
			...
			assert.match(
				message,
				/Do not advance from deep-interview to ralplan merely because the first question was answered/,
			);
			assert.match(
				message,
				/Planner output has been reviewed sequentially by Architect and then Critic/,
			);
			assert.match(
				message,
			...
			assert.match(
				snapshot,
				/activation prompt \/ task seed: \$autopilot implement issue #2430/,
			);
			assert.match(
				snapshot,
				/scope note: this seed captures the Autopilot activation prompt/,
			);
			assert.match(
				snapshot,
				/constraints: follow deep-interview -> ralplan -> ultragoal -> code-review -> ultraqa/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
			...
			const prose = await dispatchCodexNativeHook(
				{
					...
					prompt:
						"Please add a subgoal for docs later; this is normal prose, not a directive.",
				},
				{ cwd },
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					prompt: `OMX_ULTRAGOAL_STEER: ${JSON.stringify({
						...
						objective:
							"Verify UserPromptSubmit bounded steering bridge with tests.",
					})}`,
				},
				{ cwd },
			);
...
${JSON.stringify({
	...
	objective:
		"Verify repeated UserPromptSubmit steering does not duplicate goals.",
})}
\`\`\``;
			await dispatchCodexNativeHook(
				{
			...
			const plan = await readUltragoalPlan(cwd);
			assert.equal(
				plan.goals.filter((goal) => goal.title === "Deduped bridge regression")
					.length,
				1,
			);
			const ledger = await readFile(
	...
	it("keeps omx question answers on the active autopilot skill so the interview chain guidance is injected", async () => {
		...
		try {
			...
			assert.match(
				message,
				/readiness gate remains unresolved and the answer would materially change execution/,
			);
			assert.match(
				message,
				/Do not advance from deep-interview to ralplan merely because the first question was answered/,
	...
	it("keeps deep-interview bridge guidance on marked question answers with workflow-like tokens", async () => {
		...
		try {
			...
			assert.doesNotMatch(
				message,
				/detected workflow keyword "\$ralplan" -> ralplan/,
			);
			assert.equal(existsSync(join(sessionDir, "ralplan-state.json")), false);
		} finally {
			await rm(cwd, { recursive: true, force: true });
...
`,
		...
		try {
			...
			await writeJson(join(sessionDir, "deep-interview-state.json"), {
				...
				deep_interview_gate: {
					status: "complete",
					rationale:
						"Requirements are clarified and ready for ralplan consensus.",
				},
			});
			await writeJson(join(sessionDir, "skill-active-state.json"), {
				active: true,
	...
	it("preserves prompt-submit HUD reconciliation for native subagents even with worker pane env", async () => {
		...
		try {
			...
			await initTeamState(
				teamName,
				"preserve subagent HUD reconcile",
				"executor",
				1,
				cwd,
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					prompt:
						"Review the worker patch literally; do not activate $ralplan.",
				},
				{
					cwd,
					reconcileHudForPromptSubmitFn: async (hookCwd, deps = {}) => {
...
`,
		...
		try {
			...
			await writeJson(join(sessionDir, "deep-interview-state.json"), {
				...
				rounds: [
					{
						answer:
							"Implement by editing src/hooks/keyword-detector.ts and add tests.",
					},
				],
			});
		...
		try {
			...
			await writeJson(join(sessionDir, "deep-interview-state.json"), {
				...
				rounds: [
					{
						answer:
							"Use CLI wrapper normalization for protected planning state commands.",
					},
				],
			});
			...
			assert.equal(
				(blockedTmpScriptWrite.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			assert.match(
			...
			assert.equal(
				(blockedTmpScriptExecution.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			assert.match(
			...
			assert.equal(
				(
					blockedTmpInterpreterExecution.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(blockedTmpTsxExecution.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			assert.match(
			...
			for (const [toolUseId, command] of [
				...
				assert.equal(
					(blockedTmpTransport.outputJson as { decision?: string } | null)
						?.decision,
					"block",
					command,
				);
			...
			const deepInterviewRalplanHandoffState = JSON.stringify({
				...
				state: {
					deep_interview_gate: {
						status: "complete",
						rationale:
							"Requirements are clarified and ready for ralplan handoff.",
					},
				},
			});
			...
			assert.equal(
				(blockedTmpOnlyHandoffShape.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			assert.match(
				String(
					(blockedTmpOnlyHandoffShape.outputJson as { reason?: string } | null)
						?.reason ?? "",
				),
				/handoff|Bash write intent|deep-interview/i,
			);
			...
			assert.equal(
				(
					blockedHandoffWithSameCommandArtifactExecution.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.match(
				...
				/same-command|Bash write intent|handoff/i,
			);

			const blockedHandoffWithPythonSameCommandArtifactExecution =
				await preToolUse(
					{
						hook_event_name: "PreToolUse",
						cwd,
			...
			assert.equal(
				(
					blockedHandoffWithPythonSameCommandArtifactExecution.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			for (const [toolUseId, artifactDir, scriptName, executionLine] of [
				...
				assert.equal(
					(
						blockedCwdRelativeArtifactExecution.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			assert.equal(
				(
					blockedHandoffWithImplementationWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedHandoffWithUnredirectedSourceMutation.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			const allowedReadOnlyEditors = await preToolUse(
				{
					...
					tool_input: {
						command:
							"sed -n '1,20p' src/runtime.ts; perl -ne 'print if $. < 3' src/runtime.ts",
					},
				},
				{ cwd },
			);
			...
			assert.equal(
				(
					blockedCombinedSedSourceEdit.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			try {
				...
				const allowedRuntimeStateWrite = await dispatchCodexNativeHook(
					{
						...
						tool_input: {
							file_path: join(runtimeSessionDir, "deep-interview-state.json"),
							content: "{}\n",
						},
					},
					{ cwd },
				...
				assert.equal(
					(
						blockedRuntimeStatePeerWrite.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			assert.equal(
				(blockedStateCliMutation.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedMcpStateMutation.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			const standaloneAutopilotRalplanHandoffPayload = {
				...
				state: {
					deep_interview_gate: {
						status: "complete",
						rationale:
							"The bounded PMO catalog task is clear and ready for ralplan handoff.",
						handoff_summary:
							"Create the root-level pmo catalog from the captured spec.",
					},
				},
			};
			...
			await mkdir(dirname(standaloneAutopilotRalplanHandoffInputFile), {
				recursive: true,
			});
			await writeJson(
				standaloneAutopilotRalplanHandoffInputFile,
				standaloneAutopilotRalplanHandoffPayload,
			);

			const blockedStandaloneAutopilotRalplanHandoffWithoutEvidence =
				await preToolUse(
					{
						hook_event_name: "PreToolUse",
						cwd,
			...
			assert.equal(
				(
					blockedStandaloneAutopilotRalplanHandoffWithoutEvidence.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			const allowedStandaloneAutopilotRalplanHandoffWithEvidence =
				await preToolUse(
					{
						hook_event_name: "PreToolUse",
						cwd,
			...
			assert.equal(
				(blockedStateDeactivation.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);

			const blockedStateInputFile = join(cwd, "blocked-state-input.json");
			await writeJson(blockedStateInputFile, {
				mode: "deep-interview",
				active: false,
			});
			...
			assert.equal(
				(
					blockedStateDeactivationFile.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(blockedModeFlagDeactivation.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);

			const conflictingModeFlagPayload = join(
				cwd,
				"conflicting-mode-flag-state.json",
			);
			await writeJson(conflictingModeFlagPayload, {
				mode: "ralph",
				active: false,
			});
			...
			assert.equal(
				(
					blockedModeFlagFileDeactivation.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedRepeatedModeFlagDeactivation.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedRepeatedInputDeactivation.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			const repeatedInputFileBlocked = join(
				cwd,
				"repeated-input-file-blocked.json",
			);
			await writeJson(repeatedInputFileSafe, {
				mode: "deep-interview",
				current_phase: "intent-first",
				active: true,
			});
			await writeJson(repeatedInputFileBlocked, {
				mode: "deep-interview",
				active: false,
			});
			...
			assert.equal(
				(
					blockedRepeatedInputFileDeactivation.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			const terminalCurrentPhaseAliases = [
				"finish",
				"finished",
				"complete",
				"completed",
			...
			for (const alias of terminalCurrentPhaseAliases) {
				...
				assert.equal(
					(
						blockedStateWriteCurrentPhaseAlias.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			const blockedCurrentPhaseAliasInputFile = join(
				cwd,
				"blocked-current-phase-alias-input-file.json",
			);
			await writeJson(blockedCurrentPhaseAliasInputFile, {
				mode: "deep-interview",
				current_phase: "done",
			});
			...
			assert.equal(
				(
					blockedStateWriteCurrentPhaseAliasFile.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			for (const [
				index,
				aliasPayload,
			] of terminalOutcomeAliasPayloads.entries()) {
				...
				assert.equal(
					(
						blockedCamelCaseOutcomeAlias.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			const blockedCamelCaseOutcomeInputFile = join(
				cwd,
				"blocked-camel-case-outcome-input-file.json",
			);
			await writeJson(blockedCamelCaseOutcomeInputFile, {
				mode: "deep-interview",
			...
			assert.equal(
				(
					blockedStateWriteCamelCaseOutcomeFile.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(blockedCrossModeActivation.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);

			const cwdRelativeInputFileRootSafe = join(
				cwd,
				"cwd-relative-input-file-safe.json",
			);
			const cwdRelativeInputFileSubdir = join(
				cwd,
				"cwd-relative-input-file-subdir",
			);
			await mkdir(cwdRelativeInputFileSubdir, { recursive: true });
			await writeJson(cwdRelativeInputFileRootSafe, {
				mode: "deep-interview",
				active: true,
			});
			await writeJson(join(cwdRelativeInputFileSubdir, "payload.json"), {
				mode: "deep-interview",
				active: false,
			});
			...
			assert.equal(
				(
					blockedCwdRelativeInputFileAfterCd.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			await writeJson(blockedRewrittenInputFile, {
				mode: "deep-interview",
				active: true,
			});
			...
			assert.equal(
				(
					blockedRewrittenInputFileMutation.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedNestedStateDeactivation.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(blockedMultipleStateWrites.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			const blockedFileWriteWithLaterSafeDecoy = join(
				cwd,
				"blocked-file-before-later-decoy.json",
			);
			await writeJson(blockedFileWriteWithLaterSafeDecoy, {
				mode: "deep-interview",
				active: false,
			});
			...
			assert.equal(
				(blockedSegmentedStateWrite.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedNestedMcpStateWrite.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			for (const alias of terminalCurrentPhaseAliases) {
				...
				assert.equal(
					(
						blockedMcpCurrentPhaseAlias.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			for (const [
				...
				assert.equal(
					(
						blockedMcpCamelCaseOutcomeAlias.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			assert.equal(
				(blockedMcpStateClear.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedStateClear.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);

			const stateDeactivationInput =
			...
			for (const [name, command] of cliWrapperPlanningDeactivationCommands) {
				...
				assert.equal(
					(
						blockedCliWrapperPlanningDeactivation.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			for (const [name, command] of safeCliWrapperStateWriteCommands) {
				...
				assert.equal(
					allowedCliWrapperStateWrite.outputJson,
					null,
					`${command} should defer to backend validation`,
				);
			}
			...
			await writeJson(safeArtifactInputFile, {
				mode: "deep-interview",
				active: true,
				current_phase: "intent-first",
			});
			...
			for (const [name, command] of safeCliWrapperInputFileStateWriteCommands) {
				...
				assert.equal(
					allowedCliWrapperInputFileStateWrite.outputJson,
					null,
					`${command} should defer to backend validation`,
				);
			}
			...
			const envChdirRelativeInputFileSubdir = join(
				cwd,
				"env-chdir-input-file-subdir",
			);
			await mkdir(envChdirRelativeInputFileSubdir, { recursive: true });
			await writeJson(envChdirRelativeInputFileRoot, {
				mode: "deep-interview",
				active: true,
				current_phase: "intent-first",
			});
			...
			assert.equal(
				(
					blockedEnvChdirRelativeInputFileWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedEnvChdirLongFlagRelativeInputFileWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			const pnpmChdirRelativeInputFileSubdir = join(
				cwd,
				"pnpm-chdir-input-file-subdir",
			);
			await mkdir(pnpmChdirRelativeInputFileSubdir, { recursive: true });
			await writeJson(join(cwd, "payload.json"), {
				mode: "deep-interview",
				active: true,
				current_phase: "intent-first",
			});
			...
			assert.equal(
				(
					blockedPnpmChdirRelativeInputFileWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			await writeJson(rewrittenArtifactInputFile, {
				mode: "deep-interview",
				active: true,
			});
			...
			for (const [
				name,
				command,
			] of blockedNestedArtifactInputFileRewriteBeforeStateWriteCommands) {
				const blockedNestedArtifactInputFileRewriteBeforeStateWrite =
					await preToolUse(
						{
							hook_event_name: "PreToolUse",
							cwd,
				...
				assert.equal(
					(
						blockedNestedArtifactInputFileRewriteBeforeStateWrite.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			const blockedArtifactInputFileRewriteBeforeStateWrite = await preToolUse(
				{
					...
					tool_name: "Bash",
					tool_use_id:
						"tool-di-state-cli-artifact-input-file-rewrite-before-write",
					tool_input: {
						command:
			...
			assert.equal(
				(
					blockedArtifactInputFileRewriteBeforeStateWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			await writeJson(repeatedArtifactInputFile, {
				mode: "deep-interview",
				active: true,
			});
			const blockedRepeatedArtifactInputFileRewriteBeforeSecondStateWrite =
				await preToolUse(
					{
						hook_event_name: "PreToolUse",
						cwd,
			...
			assert.equal(
				(
					blockedRepeatedArtifactInputFileRewriteBeforeSecondStateWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(blockedChainedWrite.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			for (const [name, command] of blockedSourceGeneratedScriptCommands) {
				...
				assert.equal(
					(
						blockedSourceGeneratedScript.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			assert.equal(
				(
					blockedUnquotedHeredocSubstitution.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedUnquotedHeredocHyphenDelimiter.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedEscapedNewlineStateClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedEscapedNewlineStateWriteInput.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			const escapedNewlineInputFile = join(
				cwd,
				"deep-interview-escaped-newline-state.json",
			);
			await writeJson(escapedNewlineInputFile, {
				mode: "ralplan",
				active: false,
			});
			...
			assert.equal(
				(
					blockedEscapedNewlineStateWriteInputFile.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(blockedShellStdinHeredoc.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedShellStdinHereString.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedShellStdinPipe.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedHeredocPipeToShell.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			for (const [index, command] of [
				...
				assert.equal(
					(
						blockedCompactShellStdinPipe.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			assert.equal(
				(blockedEnvDispatcherShell.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(
					blockedEnvSplitStringStateClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedEnvSplitStringShellStateClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedEnvSplitStringStateWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedEnvSplitStringEqualsStateClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedEnvSplitStringAttachedStateClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedEnvSplitStringPrefixedOptionsClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedEnvSplitStringPrefixedOptionsSplitClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(blockedShellStdinStderrPipe.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			for (const [index, command] of [
				...
				assert.equal(
					(
						blockedCompactShellStdinStderrPipe.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			assert.equal(
				(
					blockedDotProcessSubstitution.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedBashProcessSubstitution.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedCatProcessSubstitution.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedDiffProcessSubstitution.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedQuotedCommandSubstitutionClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedQuotedCommandSubstitutionWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedQuotedBacktickSubstitutionClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedRawNestedCommandSubstitutionClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedRawNestedEvalSubstitutionClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedRawNestedBacktickClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedBacktickSubstitutionWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(blockedNestedBashClear.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedNestedShWrite.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(
					blockedPathQualifiedBashClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(blockedLoginBashClear.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedDashShellClear.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedRcfileBashClear.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(
					blockedShellOptionValueBeforeCommandString.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(blockedEvalStateClear.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(
					blockedDynamicEvalStateClear.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedDynamicShellStateWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedDynamicTopLevelPayloadCommand.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(blockedExecPayloadCommand.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(
					blockedCommandPayloadCommand.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedExecDashDashPayloadCommand.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedExecOptionPayloadCommand.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedCommandDashDashPayloadCommand.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedCommandOptionPayloadCommand.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedDynamicShellSubstitutionPayload.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			assert.equal(
				(
					blockedDynamicEvalBacktickPayload.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			const blockedBash = await preToolUse(
				{
					...
					tool_input: {
						command:
							"cat > src/scripts/__tests__/codex-native-hook.test.ts <<'EOF'\nexport const x = 1;\nEOF",
					},
				},
				{ cwd },
			);
			assert.equal(
				(blockedBash.outputJson as { decision?: string } | null)?.decision,
				"block",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("allows complete ralplan terminal state writes while blocking partial deactivation writes", async () => {
		...
		try {
			...
			await writeJson(allowedPayload, {
				mode: "ralplan",
				current_phase: "critic-review",
				active: true,
			});
			...
			assert.equal(
				(blocked.outputJson as { decision?: string } | null)?.decision,
				"block",
			);
			...
			const mismatchedTerminalPayload = join(
				cwd,
				"ralplan-terminal-state-mismatched.json",
			);
			...
			assert.equal(
				(
					blockedMismatchedTerminalFile.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			for (const alias of terminalCurrentPhaseAliases) {
				const blockedAliasPayload = join(
					cwd,
					`ralplan-blocked-state-${alias}.json`,
				);
				await writeJson(blockedAliasPayload, {
					mode: "ralplan",
					current_phase: alias,
				});
				...
				assert.equal(
					(blockedAliasFile.outputJson as { decision?: string } | null)
						?.decision,
					"block",
			...
			assert.equal(
				(blockedModeFlagTerminal.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(
					blockedTerminalThenImplementationWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			for (const [toolUseId, suffix] of [
				...
				[
					"tool-ralplan-state-terminal-command-substitution",
					"$(touch src/leak.ts)",
				],
				[
					"tool-ralplan-state-terminal-backtick-substitution",
					"`touch src/leak.ts`",
				],
				[
					"tool-ralplan-state-terminal-process-substitution",
					"<(touch src/leak.ts)",
				],
			] as const) {
				...
				assert.equal(
					(blockedTerminalThenSuffix.outputJson as { decision?: string } | null)
						?.decision,
					"block",
			...
			for (const alias of terminalCurrentPhaseAliases) {
				...
				assert.equal(
					(blockedModeFlagAlias.outputJson as { decision?: string } | null)
						?.decision,
					"block",
			...
			for (const [
				...
				assert.equal(
					(
						blockedModeFlagOutcomeAlias.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			const blockedOutcomeAliasPayload = join(
				cwd,
				"ralplan-blocked-state-camel-terminal-outcome.json",
			);
			await writeJson(blockedOutcomeAliasPayload, {
				mode: "ralplan",
			...
			assert.equal(
				(blockedOutcomeAliasFile.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedRalplanMcpStateWrite.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(
					blockedNestedRalplanMcpStateWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			for (const alias of terminalCurrentPhaseAliases) {
				...
				assert.equal(
					(
						blockedNestedRalplanMcpAlias.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			for (const [
				...
				assert.equal(
					(
						blockedNestedRalplanMcpOutcomeAlias.outputJson as {
							decision?: string;
						} | null
					)?.decision,
					"block",
			...
			assert.equal(
				(blockedRalplanMcpStateClear.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
		} finally {
	...
	it("blocks MCP state_clear for standalone protected planning modes", async () => {
		...
		try {
			...
			await writeJson(join(stateDir, "skill-active-state.json"), {
				...
				phase: "planning",
				active_skills: [
					{ skill: "deep-interview", phase: "planning", active: true },
				],
			});
			...
			assert.equal(
				(blockedDeepInterviewClear.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedRalplanClear.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
		} finally {
	...
	it("allows read-only diagnostics mentioning apply_patch while deep-interview blocks real apply_patch invocations", async () => {
		...
		try {
			...
			const allowedGrep = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command: 'grep -n "apply_patch" dist/scripts/codex-native-hook.js',
					},
				},
				{ cwd },
			);
			...
			const allowedSubshellGrep = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'(grep -n "apply_patch" dist/scripts/codex-native-hook.js)',
					},
				},
				{ cwd },
			);
			...
			assert.equal(
				(blockedApplyPatch.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(
					blockedEnvAssignmentApplyPatch.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			const blockedEnvWrapperApplyPatch = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							"env FOO=bar apply_patch <<'EOF'\n*** Begin Patch\n*** Add File: src/leak.ts\n+export const x = 1;\n*** End Patch\nEOF",
					},
				},
				{ cwd },
			);
			assert.equal(
				(blockedEnvWrapperApplyPatch.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			const blockedRealApplyPatchForms: Array<{ id: string; command: string }> =
				[
					{
						id: "tool-di-apply-patch-env-i",
						command: `env -i apply_patch <<'EOF'${heredocBody}`,
					},
					{
						id: "tool-di-apply-patch-env-unset",
						command: `env -u FOO apply_patch <<'EOF'${heredocBody}`,
					},
					{
						id: "tool-di-apply-patch-env-i-assignment",
						command: `env -i FOO=bar apply_patch <<'EOF'${heredocBody}`,
					},
					{
						id: "tool-di-apply-patch-assignment-env",
						command: `FOO=bar env apply_patch <<'EOF'${heredocBody}`,
					},
					{
						id: "tool-di-apply-patch-exec-env-assignment",
						command: `exec env FOO=bar apply_patch <<'EOF'${heredocBody}`,
					},
					{
						id: "tool-di-apply-patch-absolute-path",
						command: `/usr/bin/apply_patch <<'EOF'${heredocBody}`,
					},
					{
						id: "tool-di-apply-patch-relative-path",
						command: `./apply_patch <<'EOF'${heredocBody}`,
					},
					{
						id: "tool-di-apply-patch-subshell",
						command: `(apply_patch <<'EOF'${heredocBody}\n)`,
					},
					{
						id: "tool-di-apply-patch-subshell-spaced",
						command: `( apply_patch <<'EOF'${heredocBody}\n)`,
					},
					{
						id: "tool-di-apply-patch-double-subshell",
						command: `((apply_patch <<'EOF'${heredocBody}\n))`,
					},
					{
						id: "tool-di-apply-patch-pipe-subshell",
						command: `true | (apply_patch <<'EOF'${heredocBody}\n)`,
					},
					{
						id: "tool-di-apply-patch-subshell-env",
						command: `(env apply_patch <<'EOF'${heredocBody}\n)`,
					},
					{
						id: "tool-di-apply-patch-command-substitution",
						command: `x=$(apply_patch <<'EOF'${heredocBody}\n)`,
					},
					{
						id: "tool-di-apply-patch-brace-group",
						command: `{ apply_patch <<'EOF'${heredocBody}\n}`,
					},
					// Command substitution runs even inside double quotes, so quoting the
					// already-blocked `$(…)` / `` `…` `` form must not bypass the guard.
					{
						id: "tool-di-apply-patch-quoted-command-substitution",
						command: `echo "$(apply_patch <<'EOF'${heredocBody}\n)"`,
					},
					{
						id: "tool-di-apply-patch-quoted-backtick",
						command: `echo "\`apply_patch <<'EOF'${heredocBody}\`"`,
					},
					{
						id: "tool-di-apply-patch-quoted-command-substitution-prefixed",
						command: `echo "patched: $(apply_patch <<'EOF'${heredocBody}\n) done"`,
					},
				];
		...
		try {
			...
			for (const [index, command] of blockedCommands.entries()) {
				const blocked = await preToolUse(
					`tool-di-transport-bypass-block-${index}`,
					command,
				);
				assert.equal(
					(blocked.outputJson as { decision?: string } | null)?.decision,
					"block",
					`expected transport guard to block: ${command}`,
				);
			}
			...
			for (const [index, command] of allowedCommands.entries()) {
				const allowed = await preToolUse(
					`tool-di-transport-bypass-allow-${index}`,
					command,
				);
				assert.equal(
					allowed.outputJson,
					null,
					`expected safe transport form to remain allowed: ${command}`,
				);
			}
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("allows deep-interview same-command literal variable redirects to artifacts while blocking variable redirects outside them", async () => {
		...
		try {
			...
			assert.equal(
				(blockedVarRedirect.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(
					blockedUnresolvedVarRedirect.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
	...
	it("allows deep-interview apply_patch artifact writes from freeform patch text while blocking outside paths", async () => {
		...
		try {
			...
			assert.equal(
				(blockedProtectedStatePatch.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedOutsidePath.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			assert.match(
				String(
					(blockedOutsidePath.outputJson as { reason?: string } | null)
						?.reason ?? "",
				),
				/Deep-interview is active/,
			);
			...
			assert.equal(
				(blockedMixedPaths.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			...
			assert.equal(
				(blockedUnparseablePatch.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
		} finally {
		...
		try {
			...
			for (const [index, command] of readOnlyCommands.entries()) {
				...
				assert.equal(
					result.outputJson,
					null,
					`read-only command should be allowed: ${command}`,
				);
			}
			...
			for (const path of [
				...
				const writeResult = await preToolUse(
					"Write",
					`tool-ralplan-write-${path}`,
					{ file_path: path, content: "ok" },
				);
				assert.equal(
					writeResult.outputJson,
					null,
					`Write should be allowed for ${path}`,
				);
			}
			...
			for (const protectedPath of [
				...
				const protectedResult = await preToolUse(
					"Write",
					`tool-ralplan-protected-${protectedPath}`,
					{
				...
				assert.equal(
					(protectedResult.outputJson as { decision?: string } | null)
						?.decision,
					"block",
			...
			const allowedPatchAdd = await preToolUse(
				"apply_patch",
			...
			const allowedPatchUpdate = await preToolUse(
				"ApplyPatch",
			...
			const allowedRedirect = await preToolUse(
				"Bash",
			...
			const allowedCombinedSedArtifact = await preToolUse(
				"Bash",
			...
			for (const [toolUseId, command] of [
				...
				const blockedTmpStdin = await preToolUse("Bash", toolUseId, {
					command,
				});
				assert.equal(
					(blockedTmpStdin.outputJson as { decision?: string } | null)
						?.decision,
					"block",
					command,
				);
			...
			const allowedReadOnlyEditors = await preToolUse(
				"Bash",
				"tool-ralplan-read-only-editors-allow",
				{
					command:
						"sed -n '1,20p' src/runtime.ts; perl -ne 'print if $. < 3' src/runtime.ts",
				},
			...
			const blockedCombinedSedSource = await preToolUse(
				"Bash",
				"tool-ralplan-sed-combined-source-block",
				{
					command: "sed -Ei 's/old/new/' src/runtime.ts",
				},
			);
			assert.equal(
				(blockedCombinedSedSource.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			assert.match(
				String(
					(blockedCombinedSedSource.outputJson as { reason?: string } | null)
						?.reason ?? "",
				),
				/src\/runtime\.ts/,
			);

			const blockedRedirect = await preToolUse(
				"Bash",
				"tool-ralplan-redirect-block",
				{
					command: "printf 'bad' > src/implementation.ts",
				},
			);
			assert.equal(
				(blockedRedirect.outputJson as { decision?: string } | null)?.decision,
				"block",
			);
			const redirectReason = String(
				(blockedRedirect.outputJson as { reason?: string } | null)?.reason ??
					"",
			);
			...
			assert.equal(
				(blockedEdit.outputJson as { decision?: string } | null)?.decision,
				"block",
			);
			const editReason = String(
				(blockedEdit.outputJson as { reason?: string } | null)?.reason ?? "",
			);
			assert.match(editReason, /Edit path/);
			assert.match(editReason, /src\/implementation\.ts/);

			const blockedMixedPatch = await preToolUse(
				"apply_patch",
			...
			assert.equal(
				(blockedMixedPatch.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			const mixedReason = String(
				(blockedMixedPatch.outputJson as { reason?: string } | null)?.reason ??
					"",
			);
			assert.match(mixedReason, /apply_patch target/);
			assert.match(mixedReason, /src\/leak\.ts/);

			const blockedUnparseablePatch = await preToolUse(
				"apply_patch",
				"tool-ralplan-patch-unparseable",
				{
			...
			assert.match(
				String(
					(blockedUnparseablePatch.outputJson as { reason?: string } | null)
						?.reason ?? "",
				),
				/apply_patch target extraction failed/,
			);

			const blockedUnresolvedRedirect = await preToolUse(
				"Bash",
				"tool-ralplan-redirect-unresolved",
				{
					command: "cat > \"$PLAN_PATH\" <<'EOF'\ncontent\nEOF",
				},
			);
			assert.equal(
				(blockedUnresolvedRedirect.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			const unresolvedReason = String(
				(blockedUnresolvedRedirect.outputJson as { reason?: string } | null)
					?.reason ?? "",
			);
			assert.match(unresolvedReason, /unresolved Bash write target/);
			assert.match(unresolvedReason, /\$PLAN_PATH/);

			const blockedTraversal = await preToolUse(
				"Write",
			...
			assert.equal(
				(blockedTraversal.outputJson as { decision?: string } | null)?.decision,
				"block",
			);
			assert.match(
				String(
			...
			assert.equal(
				(blockedAbsolute.outputJson as { decision?: string } | null)?.decision,
				"block",
			);
			assert.match(
				String(
					(blockedAbsolute.outputJson as { reason?: string } | null)?.reason ??
						"",
				),
				/\/tmp\/leak\.ts/,
			);
	...
	it("allows Autopilot ralplan planning artifacts while blocking implementation writes", async () => {
		...
		try {
			...
			const allowedIssueReadOnlyBash = await preToolUse(
				"Bash",
				"tool-autopilot-ralplan-read-only-bash",
				{
			...
			for (const command of [
				...
				const blockedDestructiveGit = await preToolUse(
					"Bash",
					`tool-autopilot-ralplan-${command.replace(/[^a-z0-9]+/gi, "-")}`,
					{
						command,
					},
				);
				assert.equal(
					(blockedDestructiveGit.outputJson as { decision?: string } | null)
						?.decision,
					"block",
			...
			const allowedPythonPlanWrite = await preToolUse(
				"Bash",
				"tool-autopilot-ralplan-python-plan-write",
				{
					command: `python3 - <<'PY'
from pathlib import Path
Path('.omx/plans').mkdir(parents=True, exist_ok=True)
...
PY`,
			...
			const blockedPythonAllowedMkdirDynamicSourceWrite = await preToolUse(
				"Bash",
				"tool-autopilot-ralplan-python-allowed-mkdir-dynamic-source-write",
				{
					command: `python3 - <<'PY'
from pathlib import Path
Path('.omx/plans').mkdir(parents=True, exist_ok=True)
...
PY`,
			...
			const blockedPythonSourceWrite = await preToolUse(
				"Bash",
				"tool-autopilot-ralplan-python-source-write",
				{
					command: `python3 - <<'PY'
from pathlib import Path
Path('src/generated.ts').write_text('implementation')
PY`,
			...
			assert.equal(
				(blockedPythonSourceWrite.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			assert.match(
				String(
					(blockedPythonSourceWrite.outputJson as { reason?: string } | null)
						?.reason ?? "",
				),
				/src\/generated\.ts/,
			);

			const blockedPythonMixedWrite = await preToolUse(
				"Bash",
				"tool-autopilot-ralplan-python-mixed-write",
				{
					command: `python3 - <<'PY'
from pathlib import Path
Path('.omx/plans/rebase-pr3010-ultragoal-fix-plan.md').write_text('planning text')
...
PY`,
			...
			assert.equal(
				(blockedPythonMixedWrite.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			assert.match(
				String(
					(blockedPythonMixedWrite.outputJson as { reason?: string } | null)
						?.reason ?? "",
				),
				/src\/generated/,
			);
			...
			assert.equal(
				(blockedImplementationEdit.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
			assert.match(
				String(
					(blockedImplementationEdit.outputJson as { reason?: string } | null)
						?.reason ?? "",
				),
			...
			assert.match(
				...
				/not under allowed planning artifact paths/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("allows Ralplan Beads tracker metadata writes during planning", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-ralplan-beads-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			const allowedEdit = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						file_path: ".beads/tasks/issue.json",
						old_string: "old",
						new_string: "new",
					},
				},
			...
			for (const [index, command] of allowedCommands.entries()) {
				...
				assert.equal(
					result.outputJson,
					null,
					`expected Beads metadata command to be allowed: ${command}`,
				);
			}
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("blocks unsafe Beads tracker targets and mixed implementation writes during Autopilot ralplan planning", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-autopilot-beads-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			const sessionDir = join(
			...
			assert.equal(
				(
					mentionWithImplementationWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
	...
	it("allows Autopilot rework implementation writes while ralplan remains guarded", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-autopilot-rework-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeJson(join(sessionDir, "autopilot-state.json"), {
				...
				review_verdict: {
					...
					findings: ["src/implementation.ts"],
				},
				session_id: "sess-autopilot-rework",
				thread_id: "thread-autopilot-rework",
			});
	...
	it("blocks typed agent-role-only implementation writes under active ralplan", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-typed-agent-role-ralplan-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("allows typed agent-role implementation writes under active ralplan when trusted provenance is present", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-trusted-typed-agent-role-ralplan-",
			),
		);
					...
					[sessionId]: {
						...
						threads: {
							[leaderThreadId]: {
								thread_id: leaderThreadId,
								kind: "leader",
								first_seen_at: "2026-04-19T00:00:00.000Z",
								last_seen_at: "2026-04-19T00:10:00.000Z",
								turn_count: 1,
							},
							[childThreadId]: {
								...
								kind: "subagent",
								mode: "executor",
								first_seen_at: "2026-04-19T00:01:00.000Z",
								last_seen_at: "2026-04-19T00:10:00.000Z",
								turn_count: 1,
								leader_thread_id: leaderThreadId,
							},
	...
	it("blocks typed thread-spawn provenance when the parent belongs to another leader session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-stale-typed-thread-spawn-"),
		);
		const originalOmxRoot = process.env.OMX_ROOT;
		const originalOmxStateRoot = process.env.OMX_STATE_ROOT;
		const originalOmxTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
	...
	it("allows null-device fd redirects while deep-interview blocks real Bash writes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-deep-interview-null-redirect-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("allows implementation tools after an explicit deep-interview handoff deactivates the mode", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-deep-interview-handoff-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await dispatchCodexNativeHook(
				{
					...
					session_id: "sess-di-handoff",
					prompt:
						"$ralph implement the clarified spec in src/implementation.ts",
				},
				{ cwd },
			);
	...
	it("records native subagent capacity exhaustion from spawn_agent PostToolUse output", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-capacity-record-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
			...
			const blocker = JSON.parse(
				await readFile(
					join(cwd, ".omx", "state", "native-subagent-capacity-blocker.json"),
					"utf-8",
				),
			) as Record<string, unknown>;
			...
			assert.ok(
				Date.parse(String(blocker.expires_at)) >
					Date.parse(String(blocker.observed_at)),
			);
			assert.equal(
				existsSync(join(cwd, ".omx", "state", "native-subagent-support.json")),
				false,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("records unsupported native subagent support blocker from spawn_agent PostToolUse output", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-support-record-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("blocks close_agent cleanup after recent native subagent capacity exhaustion", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-capacity-close-block-"),
		);
		try {
			await dispatchCodexNativeHook(
				{
	...
	it("emits hook-specific deny PreToolUse CLI stdout for close_agent capacity blocks", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-cli-subagent-capacity-close-block-"),
		);
		try {
			parseSingleJsonStdout(
				runNativeHookCli(
					{
						hook_event_name: "PostToolUse",
						cwd,
			...
			assert.match(
				String(hookSpecificOutput.permissionDecisionReason ?? ""),
				/Native subagent capacity was exhausted recently/,
			);
			assert.match(
				String(hookSpecificOutput.additionalContext ?? ""),
				/agent thread limit reached/,
			...
			assert.match(
				String(hookSpecificOutput.additionalContext ?? ""),
				/Do not call multi_agent_v1\.close_agent/,
			);
			assert.equal(output.decision, undefined);
			assert.equal(output.reason, undefined);
			assert.equal(output.systemMessage, undefined);
	...
	it("blocks parallel close_agent cleanup after recent native subagent capacity exhaustion", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-capacity-parallel-close-block-"),
		);
		try {
			await dispatchCodexNativeHook(
				{
						...
						tool_uses: [
							{
								...
								parameters: { target: "stale-1" },
							},
							{
								recipient_name: "multi_agent_v1.close_agent",
								parameters: { target: "stale-2" },
							},
						],
					},
				},
			...
			assert.match(
				JSON.stringify(result.outputJson),
				/do not batch close_agent through multi_tool_use\.parallel/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("does not block close_agent without a recent native capacity blocker", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-capacity-close-no-blocker-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("does not block close_agent when the native capacity blocker is expired", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-capacity-close-expired-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'OMX_LORE_COMMIT_GUARD=1 git commit -m "quick hack fallback if it fails"',
					},
				},
				{ cwd },
			);
	...
	it("blocks PreToolUse git commit when process env explicitly enables the Lore commit guard", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-lore-env-enabled-"),
		);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'OMX_LORE_COMMIT_GUARD=1 git commit -m "fix: conventional"',
					},
				},
				{ cwd },
			);
	...
	it("allows inline disabled guard to override an enabled process env", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-commit-lore-inline-override-disabled-",
			),
		);
		const original = process.env.OMX_LORE_COMMIT_GUARD;
		try {
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'OMX_LORE_COMMIT_GUARD=1 HUSKY=0 git commit -m "fix tests"',
					},
				},
				{ cwd },
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'OMX_LORE_COMMIT_GUARD=1 git -c core.editor=true commit -m "fix tests"',
					},
				},
				{ cwd },
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'env OMX_LORE_COMMIT_GUARD=1 git.exe commit -m "fix tests"',
					},
				},
				{ cwd },
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command: 'OMX_LORE_COMMIT_GUARD=1 git.exe commit -m "fix tests"',
					},
				},
				{ cwd },
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'env -i PATH=/usr/bin OMX_LORE_COMMIT_GUARD=1 git.exe commit -m "fix tests"',
					},
				},
				{ cwd },
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'env -u FOO OMX_LORE_COMMIT_GUARD=1 git.exe commit -m "fix tests"',
					},
				},
				{ cwd },
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'OMX_LORE_COMMIT_GUARD=1 "C:/Program Files/Git/cmd/git.exe" commit -m "fix tests"',
					},
				},
				{ cwd },
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'OMX_LORE_COMMIT_GUARD=1 /usr/bin/git commit -m "fix tests"',
					},
				},
				{ cwd },
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							"OMX_LORE_COMMIT_GUARD=1 git commit -F .git/COMMIT_EDITMSG",
					},
				},
				{ cwd },
			);
	...
	it("does not treat non-MCP source output containing detector constants as MCP transport death", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-read-mcp-source-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("does not treat non-MCP docs stdout mentioning closed MCP transport as transport death", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-docs-mcp-log-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_response: JSON.stringify({
						stdout:
							"Troubleshooting note: MCP transport closed after the server disconnected in an old log.",
						stderr: "",
					}),
				},
				{ cwd },
	...
	it("does not MCP-block non-MCP command output with unrelated stderr and MCP transport stdout", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-nonmcp-mixed-output-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("still blocks MCP-like raw transport failures", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-mcp-raw-transport-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("does not block ordinary non-zero grep output in PostToolUse", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-grep-nonzero-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_response:
						'{"exit_code":1,"stdout":"src/example.ts:TODO","stderr":""}',
				},
				{ cwd },
			);
	...
	it("does not block ordinary non-zero diagnostic output in PostToolUse", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-diagnostic-nonzero-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_response:
						'{"exit_code":1,"stdout":"searched 10 files","stderr":""}',
				},
				{ cwd },
			);
	...
	it("treats wrapped gh pr checks output as reviewable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-gh-wrapped-"),
		);
		try {
			for (const command of [
				...
				const result = await dispatchCodexNativeHook(
					{
						...
						tool_input: { command },
						tool_response:
							'{"exit_code":8,"stdout":"build pending","stderr":""}',
					},
					{ cwd },
				);
	...
	it("does not treat heredoc gh pr checks text as a reviewable command", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-gh-heredoc-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("does not treat echoed gh pr checks text as a reviewable command", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-gh-echo-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("suppresses parent Autopilot Stop continuation in side conversations", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-autopilot-side-conversation-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					transcript_path: transcriptPath,
					last_assistant_message:
						"Waiting for a new side-conversation question.",
				},
				{ cwd },
			);
	...
	it("requires Autopilot code review after a compact-boundary Stop exemption", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-autopilot-review-compact-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("allows Stop when terminal Autopilot run-state shadows stale session ralplan state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-autopilot-terminal-run-state-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("still blocks Stop while Autopilot ralplan state is genuinely non-terminal", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-autopilot-active-ralplan-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("steers worker Stop leader nudge directly when leader pane is busy", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-busy-leader-"),
		);
		const prevTeamWorker = process.env.OMX_TEAM_WORKER;
		const prevTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		const prevPath = process.env.PATH;
		try {
			...
			await writeFile(
				join(fakeBinDir, "tmux"),
				buildWorkerStopFakeTmux(tmuxLogPath, { busyLeader: true }),
			);
			await chmod(join(fakeBinDir, "tmux"), 0o755);
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.equal(
				submits.length,
				2,
				"busy worker-stop nudge should submit directly as steering, not queue via Tab",
			);
			const nudgeState = JSON.parse(
				await readFile(join(workerDir, "worker-stop-nudge.json"), "utf-8"),
			);
	...
	it("dedupes allowed worker Stop leader nudges across workers in the same team window", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-team-dedupe-"),
		);
		const prevPath = process.env.PATH;
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			const stopNudges =
				tmuxLog.match(
					/send-keys -t %42 -l \[OMX\] worker-\d+ native Stop allowed/g,
				) || [];
			assert.equal(
				stopNudges.length,
				1,
				"same-team workers should share one leader nudge cooldown window",
			);
			const teamNudgeState = JSON.parse(
				await readFile(join(teamDir, "worker-stop-nudge.json"), "utf-8"),
			);
	...
	it("serializes concurrent allowed worker Stop leader nudges with a team lock", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-concurrent-dedupe-"),
		);
		const prevPath = process.env.PATH;
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeFile(
				join(fakeBinDir, "tmux"),
				buildWorkerStopFakeTmux(tmuxLogPath, { sendDelayMs: 100 }),
			);
			await chmod(join(fakeBinDir, "tmux"), 0o755);
			await writeJson(join(teamDir, "manifest.v2.json"), {
				name: teamName,
			...
			assert.equal(
				results.filter((result) => result.result === "sent").length,
				1,
			);
			assert.equal(
				results.filter(
					(result) => result.result === "suppressed_team_lock_held",
				).length,
				1,
			);
			const tmuxLog = await readFile(tmuxLogPath, "utf-8");
			...
			assert.equal(
				stopNudges.length,
				1,
				"concurrent same-team workers should emit only one leader nudge",
			);
	...
	it("skips worker Stop leader nudge when team state is missing or shut down", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-missing-team-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			const logsDir = join(cwd, ".omx", "logs");
			...
			assert.equal(
				existsSync(
					join(stateDir, "team", "removed-team", "worker-stop-nudge.json"),
				),
				false,
			);

			await writeJson(
				join(stateDir, "team", "shutdown-team", "shutdown.json"),
				{
			...
			assert.equal(
				existsSync(
					join(stateDir, "team", "shutdown-team", "worker-stop-nudge.json"),
				),
				false,
			);
			const deliveryLogPath = join(
				logsDir,
				`team-delivery-${new Date().toISOString().split("T")[0]}.jsonl`,
			);
			const deliveryEvents = (await readFile(deliveryLogPath, "utf-8"))
				.trim()
				.split("\n")
				.map((line) => JSON.parse(line));
			const suppressedEvents = deliveryEvents.filter(
				(event) => event.reason === "team_state_gone_or_shutdown",
			);
			assert.equal(
				suppressedEvents.length,
				2,
				"late closed-team Stop nudges should be diagnostics, not queued prompts",
			);
			assert.equal(
				suppressedEvents.every(
					(event) =>
						event.result === "suppressed" && event.transport === "none",
				),
				true,
			);
		} finally {
	...
	it("does not treat old visible worker Stop transcript as pending queue state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-queue-dedupe-"),
		);
		const prevPath = process.env.PATH;
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.match(
				tmuxLog,
				/send-keys -t %42 -l \[OMX\] worker-2 native Stop allowed/,
			);
			assert.doesNotMatch(tmuxLog, /send-keys -t %42 Tab/);
			const teamNudgeState = JSON.parse(
				await readFile(join(teamDir, "worker-stop-nudge.json"), "utf-8"),
	...
	it("reports deferred when non-teardown persistence failure prevents worker Stop nudge cooldown state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-persist-fail-"),
		);
		const prevPath = process.env.PATH;
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.equal(
				existsSync(
					join(teamDir, "workers", "worker-1", "worker-stop-nudge.json"),
				),
				false,
			);
			const tmuxLog = await readFile(tmuxLogPath, "utf-8");
			...
			const deliveryEvents = (await readFile(deliveryLogPath, "utf-8"))
				...
				.split("\n")
				.map((line) => JSON.parse(line));
			const deferredEvent = deliveryEvents.find(
				(event) =>
					event.event === "nudge_triggered" && event.result === "deferred",
			);
			assert.equal(deferredEvent?.team, teamName);
			assert.equal(deferredEvent?.from_worker, "worker-1");
			assert.match(
				String(deferredEvent?.reason || ""),
				/EEXIST|ENOTDIR|not a directory|file already exists/,
			);
		} finally {
			if (typeof prevPath === "string") process.env.PATH = prevPath;
			else delete process.env.PATH;
	...
	it("does not recreate team state when teardown removes it during worker Stop delivery", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-teardown-race-"),
		);
		const prevPath = process.env.PATH;
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeFile(
				join(fakeBinDir, "tmux"),
				buildWorkerStopFakeTmux(tmuxLogPath, { removePathOnSend: teamDir }),
			);
			await chmod(join(fakeBinDir, "tmux"), 0o755);
			process.env.PATH = `${fakeBinDir}:${prevPath || ""}`;
			...
			assert.equal(
				existsSync(teamDir),
				false,
				"worker Stop delivery must not recreate removed team state",
			);
			const tmuxLog = await readFile(tmuxLogPath, "utf-8");
			assert.match(
				tmuxLog,
	...
	it("does not recreate team state when teardown removes it before deferred worker Stop recording", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-deferred-teardown-"),
		);
		const prevPath = process.env.PATH;
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.equal(
				existsSync(teamDir),
				false,
				"deferred worker Stop recording must not recreate removed team state",
			);
			const tmuxLog = await readFile(tmuxLogPath, "utf-8");
			assert.doesNotMatch(
				tmuxLog,
	...
	it("does not block Stop from canonical team state owned by another thread", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-canonical-other-thread-"),
		);
		try {
			await initTeamState(
	...
	it("blocks Stop from canonical team state owned by the current thread", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-canonical-current-thread-"),
		);
		try {
			await initTeamState(
				...
				{
					...process.env,
					OMX_SESSION_ID: "sess-stop-team-canonical-current-thread",
				},
			);
			const manifestPath = join(
				cwd,
		...
		try {
			...
			await initTeamState(
				...
				undefined,
				{
					...
					OMX_TEAM_STATE_ROOT: sharedRoot,
				},
			);
	...
	it("does not block Stop after owner-session ralplan CLI completion writes to native canonical state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralplan-owner-alias-complete-"),
		);
		...
		try {
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", ownerSessionId, "ralplan-state.json"),
				),
				false,
			);
			const result = await dispatchCodexNativeHook(
	...
	it("does not block when canonical root ralplan state is inactive but session ralplan state is stale active", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-stop-stale-session-ralplan-root-inactive-",
			),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("keeps blocking current session ralplan when root inactive ralplan state belongs to another session", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-stop-session-ralplan-root-other-session-",
			),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("keeps blocking current session ralplan when root inactive ralplan state is unscoped", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-session-ralplan-root-unscoped-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("keeps blocking current session ralplan when unscoped root completion lacks a plan artifact", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-stop-session-ralplan-root-no-plan-artifact-",
			),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("does not loop when newer unscoped root ralplan complete state shadows stale session planning", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-stop-stale-session-ralplan-newer-root-complete-",
			),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeFile(
				join(cwd, ".omx", "plans", "prd-issue-2923.md"),
				"# PRD\n",
			);
			await writeFile(
				join(cwd, ".omx", "plans", "test-spec-issue-2923.md"),
				"# Test spec\n",
			);
			await writeJson(join(stateDir, "skill-active-state.json"), {
				active: false,
	...
	it("keeps blocking current session ralplan when newer unscoped root completion belongs to another worktree", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-stop-ralplan-cross-worktree-root-complete-",
			),
		);
		try {
			const otherWorktree = join(
				tmpdir(),
				"omx-native-hook-other-worktree-2925",
			);
			const stateDir = join(cwd, ".omx", "state");
	...
	it("keeps blocking current session ralplan when newer unscoped root completion lacks project context", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralplan-root-complete-no-cwd-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("does not block stale session ralplan when root ralplan is terminal and another root skill is active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-stale-ralplan-other-root-skill-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("keeps blocking session ralplan when canonical root state is not inactive", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-session-ralplan-root-active-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("does not report ralplan subagent waiting when notify-fallback already recorded completion", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-skill-subagent-complete-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			const now = new Date().toISOString();
			await mkdir(
				join(stateDir, "sessions", "sess-stop-skill-subagent-complete"),
				{ recursive: true },
			);
			await writeJson(join(stateDir, "session.json"), {
	...
	it("blocks Codex App Stop when Ralph is marked complete without completion-audit evidence", async () => {
		...
		try {
			...
			assert.equal(
				repeat.outputJson?.stopReason,
				"ralph_completion_audit_missing_completion_audit",
			);
			assert.doesNotMatch(
				String(repeat.outputJson?.reason),
				/Ralph is still active/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("allows Stop from stale orphaned session-scoped Ralph starting iteration zero state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-stale-orphan-starting-ralph-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					last_assistant_message:
						"APPROVE: read-only verifier evidence is fresh and sufficient.",
				},
				{ cwd },
			);
	...
	it("retires prompt-seeded Ralph starting state when canonical Ralph already completed with audit", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralph-shadowed-starting-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeJson(
				join(stateDir, "sessions", nativeSessionId, "ralph-state.json"),
				{
					active: true,
					mode: "ralph",
					current_phase: "starting",
			...
			assert.equal(
				retiredState.stop_reason,
				"shadowed_by_completed_canonical_ralph",
			);
			assert.equal(
				retiredState.shadowed_by_completed_canonical_ralph.session_id,
				canonicalSessionId,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("does not retire prompt-seeded Ralph starting state from a completed canonical Ralph owned by another thread", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralph-shadowed-thread-mismatch-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			const nativeSessionId = "native-hook-seed";
	...
	it("blocks implementation writes while ralplan is active without execution handoff", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralplan-pretool-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks implementation writes while Autopilot is supervising deep-interview without a persisted phase transition", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-deep-interview-pretool-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks implementation writes while Autopilot is supervising ralplan without handoff", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-ralplan-pretool-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			for (const command of [
				...
				assert.equal(
					(writeIntentResult.outputJson as { decision?: string } | null)
						?.decision,
					"block",
					command,
				);
				assert.match(
					String(
						(writeIntentResult.outputJson as { reason?: string } | null)
							?.reason ?? "",
					),
	...
	it("allows Autopilot planning handoffs with active:true state writes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-planning-state-write-allow-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.equal(
				(blockedPlanningDeactivation.outputJson as { decision?: string } | null)
					?.decision,
				"block",
			);
		} finally {
	...
	it("blocks implementation writes when Autopilot ralplan is visible only in skill-active phase", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-skill-ralplan-pretool-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("ignores stale Autopilot ralplan skill mirrors after detail state leaves planning", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-stale-ralplan-mirror-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("allows explicit blank Autopilot detail phase to use a ralplan skill mirror", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-blank-phase-mirror-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("does not block implementation writes from Autopilot ralplan detail state without canonical skill state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-ralplan-no-canonical-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks implementation writes when terminal Autopilot run-state shadows stale supervised ralplan state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-ralplan-terminal-pretool-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.match(
				JSON.stringify(result.outputJson),
				/(?:Ralplan|Autopilot planning) is active \(phase: ralplan\)/,
			);
			assert.match(
				JSON.stringify(result.outputJson),
				/implementation\/write tools are blocked/,
	...
	it("blocks bash implementation writes while Autopilot is supervising ralplan without handoff", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-ralplan-pretool-bash-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks implementation writes when ralplan and Autopilot ralplan are both active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralplan-autopilot-mixed-planning-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks implementation writes while Autopilot is supervising replan without handoff", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-replan-pretool-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks implementation writes when native Codex id maps to OMX Autopilot ralplan state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-ralplan-native-map-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks bash implementation writes when native Codex id maps to OMX Autopilot ralplan state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-ralplan-native-map-bash-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks standalone ralplan writes when native Codex id maps to OMX session state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralplan-native-map-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks deep-interview writes when native Codex id maps to OMX session state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-deep-interview-native-map-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							"cat <<'EOF' > .omx/plans/prd-native-map.md\nplanning\nEOF",
					},
				},
				{ cwd },
			);
	...
	it("blocks mapped implementation writes when explicit ultragoal conductor handoff is active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralplan-native-map-handoff-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks mapped implementation writes when terminal Autopilot run-state shadows stale supervised ralplan state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-ralplan-native-map-terminal-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("fails closed for implementation writes when a different live root session owns active ralplan state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralplan-live-root-conflict-"),
		);
		const ownerCwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralplan-live-root-owner-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.equal(
				(
					blockedOmittedSessionTerminalWrite.outputJson as {
						decision?: string;
					} | null
				)?.decision,
				"block",
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							"cat <<'EOF' > .omx/plans/live-root-owner.md\nplanning\nEOF",
					},
				},
				{ cwd },
			);
	...
	it("blocks mapped Autopilot ralplan writes from the authoritative team state root", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-ralplan-team-root-"),
		);
		const teamStateRoot = await mkdtemp(
			join(tmpdir(), "omx-native-hook-team-root-"),
		);
		const previousTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		try {
			process.env.OMX_TEAM_STATE_ROOT = teamStateRoot;
			...
			assert.equal(
				existsSync(join(cwd, ".omx", "state", "session.json")),
				false,
			);
		} finally {
			if (typeof previousTeamStateRoot === "string")
	...
	it("does not block unrelated native Codex ids from the authoritative team state root", async () => {
		...
		const teamStateRoot = await mkdtemp(
			join(tmpdir(), "omx-native-hook-team-root-unrelated-"),
		);
		const previousTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		try {
			process.env.OMX_TEAM_STATE_ROOT = teamStateRoot;
	...
	it("blocks bash implementation writes while ralplan is active without execution handoff", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralplan-pretool-bash-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							"cat <<'EOF' > .omx/plans/prd-issue-2603.md\nplanning\nEOF",
					},
				},
				{ cwd },
			);
	...
	it("allows implementation writes when an explicit execution handoff is active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralplan-pretool-handoff-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks mapped native-session Main-root conductor writes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-conductor-native-map-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks standalone ultragoal conductor writes without requiring ralplan active_skills", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ultragoal-standalone-conductor-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks unsupported active conductor source edits with native delegation recovery guidance", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-unsupported-conductor-source-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks Main-root ralplan writes even when payload has only a typed agent_role", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralplan-agent-role-main-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.match(
				String(result.outputJson?.reason ?? ""),
				/Ralplan is active \(phase: planning\)/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("blocks Main-root ralph conductor source and planning artifact writes while allowing .omx workflow state writes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-conductor-write-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.match(
				String(blocked.outputJson?.reason ?? ""),
				/ralph phase: executing/,
			);
			...
			for (const [toolName, filePath] of [
				...
				assert.equal(
					planningArtifactWrite.outputJson?.decision,
					"block",
					`${toolName} ${filePath}`,
				);
				assert.match(
					String(planningArtifactWrite.outputJson?.reason ?? ""),
					/plan\/code writes are blocked/,
				);
			}
			...
			const allowed = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						file_path:
							".omx/state/sessions/sess-ralph-conductor-write/ralph-state.json",
					},
				},
				{ cwd },
			);
			...
			const protectedRawState = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						file_path:
							".omx/state/sessions/sess-ralph-conductor-write/autopilot-state.json",
					},
				},
				{ cwd },
			);
	...
	it("blocks unsafe dynamic nested shell writes in conductor mode", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-dynamic-shell-write-block-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_name: "Bash",
					tool_input: {
						command: `bash -lc 'cp "$SOURCE_FILE" src/runtime.ts'`,
					},
				},
				{ cwd },
			);
	...
	it("blocks common Bash file mutations in Main-root conductor states unless they target workflow metadata", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-conductor-bash-mutations-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("allows autopilot rework implementation writes while conductor phases stay guarded", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-autopilot-rework-write-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks Main-root team conductor writes from root team state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-root-team-conductor-write-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks Main-root ralph conductor source writes while allowing .omx workflow state writes", async () => {
		...
		try {
			...
			const nativeMappedCwd = await mkdtemp(
				join(tmpdir(), "omx-native-hook-ralph-conductor-native-write-"),
			);
			try {
				...
				assert.match(
					String(nativeMappedBlocked.outputJson?.reason ?? ""),
					/ralph phase: executing/,
				);
			} finally {
	...
	it("allows conductor sed/perl metadata edits while blocking non-metadata targets", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-conductor-sed-perl-metadata-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			for (const command of blockedCommands) {
				...
				assert.match(
					...
					/Bash .* target .*not workflow state\/ledger\/mailbox\/handoff metadata|target <unresolved>/,
				);
			}
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("does not block Stop from root team state without team_name when no session is known", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-root-team-no-session-no-name-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("does not block Stop from root team state without team_name for a foreign session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-root-team-foreign-no-name-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
	...
	it("does not block Stop from another thread's stale root team state when no scoped team state exists", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-stale-root-team-thread-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
			...
			await writeJson(
				join(stateDir, "team", "stale-root-thread-team", "phase.json"),
				{
					current_phase: "team-exec",
					max_fix_attempts: 3,
					current_fix_attempt: 0,
	...
	it("does not block Stop from root team state with matching session but missing thread ownership", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-root-team-missing-thread-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
			...
			await writeJson(
				join(stateDir, "team", "root-missing-thread-team", "phase.json"),
				{
					current_phase: "team-exec",
					max_fix_attempts: 3,
					current_fix_attempt: 0,
	...
	it("does not block Stop from root team state when canonical phase is missing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-root-team-missing-phase-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
			...
			await mkdir(join(stateDir, "team", "root-missing-phase-team"), {
				recursive: true,
			});
			await writeJson(join(stateDir, "session.json"), {
				session_id: "sess-current",
	...
	it("does not block Stop from session-scoped team state owned by another thread", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-scoped-team-other-thread-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
			...
			await writeJson(
				join(stateDir, "team", "scoped-other-thread-team", "phase.json"),
				{
					current_phase: "team-exec",
					max_fix_attempts: 3,
					current_fix_attempt: 0,
	...
	it("blocks Stop from session-scoped team state owned by the current session and thread", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-scoped-team-current-thread-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
			...
			await writeJson(
				join(stateDir, "team", "scoped-current-team", "phase.json"),
				{
					current_phase: "team-exec",
					max_fix_attempts: 3,
					current_fix_attempt: 0,
