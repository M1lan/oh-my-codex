...
describe("generate-release-body", () => {
	it("preserves custom sections while refreshing contributors and compare metadata from git", async () => {
		...
		try {
			...
			git(root, [
				...
				"add",
				"origin",
				"https://github.com/example/oh-my-codex.git",
			]);
			...
			git(root, ["commit", "-m", "alice change"], {
			...
			const generated = await readFile(
				join(root, "RELEASE_BODY.generated.md"),
				"utf-8",
			);
			...
			assert.match(
				generated,
				/\*\*Full Changelog\*\*: \[`v0\.12\.0\.\.\.v0\.13\.0`\]\(https:\/\/github\.com\/example\/oh-my-codex\/compare\/v0\.12\.0\.\.\.v0\.13\.0\)/,
			);
		} finally {
	...
	it("prefers GitHub contributor handles when compare metadata is available", async () => {
		...
		try {
			...
			assert.match(
				generated,
				/Thanks to \[@alice\]\(https:\/\/github\.com\/alice\) and \[@bob\]\(https:\/\/github\.com\/bob\) for contributing to this release\./,
			);
		} finally {
			await rm(root, { recursive: true, force: true });
		}
	...
	it("skips off-ancestry semver-previous tags when auto-resolving the compare base", async () => {
		const root = await mkdtemp(
			join(tmpdir(), "omx-generate-release-body-off-ancestry-"),
		);
		try {
			git(root, ["init"]);
			git(root, ["config", "user.name", "Release Bot"]);
	...
	it("rejects missing or inverted compare refs before rendering a compare link", async () => {
		const root = await mkdtemp(
			join(tmpdir(), "omx-generate-release-body-range-"),
		);
		try {
			git(root, ["init"]);
			git(root, ["config", "user.name", "Release Bot"]);
	...
	it("fails validation when the template is missing required metadata anchors", async () => {
		const root = await mkdtemp(
			join(tmpdir(), "omx-generate-release-body-invalid-"),
		);
		try {
			git(root, ["init"]);
			git(root, ["config", "user.name", "Release Bot"]);
