...
function parseArgs(argv: string[]): SmokeArguments {
	...
	assert(
		...
		"--package-root must be an absolute installed package path",
	);
	assert(
		/(?:^|[\\/])node_modules[\\/]oh-my-codex$/.test(packageRoot),
		"--package-root must name the installed oh-my-codex package",
	);
	assert(
		/^[0-9a-f]{40}$/i.test(testedSha),
		"--tested-sha must be a 40-character SHA",
	);
	assert(
		/^[0-9a-f]{64}$/i.test(tarballSha256),
		"--tarball-sha256 must be a SHA-256 digest",
	);
	assert(
		existsSync(join(packageRoot, "package.json")),
		"installed package.json is missing",
	);
	assert(
		existsSync(join(packageRoot, "dist", "cli", "omx.js")),
		"installed CLI is missing",
	);
	return { packageRoot, testedSha, tarballSha256 };
}

async function importInstalled<T>(
	packageRoot: string,
...
async function withWorkspace<T>(
	name: string,
	run: (cwd: string, home: string) => Promise<T>,
): Promise<T> {
	...
	await writeFile(
		join(cwd, ".omx", "managed"),
		"issue #3138 installed smoke fixture",
	);
	try {
...
async function writeModeState(
	...
	await mkdir(join(cwd, ".omx", "state", "sessions", sessionId), {
		recursive: true,
	});
	await writeFile(
		path,
...
async function writeRalplanConsensusEvidence(
	cwd: string,
	sessionId: string,
): Promise<Record<string, unknown>> {
	const now = "2026-07-14T00:00:00.000Z";
	await writeFile(
		join(cwd, ".omx", "state", "subagent-tracking.json"),
		JSON.stringify(
			{
				schemaVersion: 1,
				sessions: {
...
async function scenarioMissingOwnerAliasActivation(
	packageRoot: string,
): Promise<void> {
	await withWorkspace("missing-owner-alias", async (cwd, home) => {
		...
		await withEnvironment(
			...
			async () => {
				...
				const pointer = await readJson<Record<string, unknown>>(
					join(cwd, ".omx", "state", "session.json"),
				);
				const canonicalSessionId = String(pointer.session_id || "");
				assert(
					canonicalSessionId.length > 0,
					"native SessionStart did not establish a canonical pointer",
				);
				assert(
					pointer.owner_omx_session_id === ownerSessionId,
					"native SessionStart did not bind the verified owner alias",
				);
				assert(
					start.outputJson?.decision !== "block",
					"native SessionStart unexpectedly blocked alias activation",
				);
				...
				assert(
					prompt.outputJson?.decision !== "block",
					"owner alias prompt activation unexpectedly blocked",
				);
				assert(
					existsSync(statePath(cwd, canonicalSessionId, "ralph")),
					"owner alias activation did not write canonical Ralph state",
				);
				assert(
					!existsSync(join(cwd, ".omx", "state", "sessions", ownerSessionId)),
					"owner alias activation created a split owner scope",
				);
			},
		);
	});
...
async function scenarioUnmatchedWrite(packageRoot: string): Promise<void> {
	await withWorkspace("unmatched-write", async (cwd, home) => {
		...
		await writeCanonicalPointer(
			packageRoot,
			cwd,
			home,
			canonicalSessionId,
			"native-unmatched-write",
		);
		...
		assert(
			result.status !== 0,
			"unmatched owner state write unexpectedly succeeded",
		);
		assert(
			!existsSync(statePath(cwd, canonicalSessionId, "ralph")),
			"unmatched owner state write mutated canonical state",
		);
		assert(
			!existsSync(join(cwd, ".omx", "state", "sessions", ownerSessionId)),
			"unmatched owner state write created an owner scope",
		);
	});
}
...
async function scenarioOrdinaryClear(packageRoot: string): Promise<void> {
	await withWorkspace("ordinary-clear", async (cwd, home) => {
		...
		await writeCanonicalPointer(
			...
			home,
			canonicalSessionId,
			"native-unmatched-clear",
		);
		await writeModeState(cwd, canonicalSessionId, "ralph", {
			current_phase: "executing",
		});
		const before = await readFile(
			statePath(cwd, canonicalSessionId, "ralph"),
			"utf-8",
		);
		const result = runInstalledCli(
			packageRoot,
		...
		assert(
			result.status !== 0,
			"unmatched owner ordinary clear unexpectedly succeeded",
		);
		assert(
			(await readFile(statePath(cwd, canonicalSessionId, "ralph"), "utf-8")) ===
				before,
			"unmatched owner ordinary clear mutated canonical state",
		);
		assert(
			!existsSync(join(cwd, ".omx", "state", "sessions", ownerSessionId)),
			"ordinary clear created an owner scope",
		);
	});
}
...
async function scenarioCancel(packageRoot: string): Promise<void> {
	await withWorkspace("cancel", async (cwd, home) => {
		...
		assert(
			(await readFile(statePath(cwd, canonicalSessionId, "ralph"), "utf-8")) ===
				before,
			"unmatched owner cancel mutated canonical state",
		);
		assert(
			!existsSync(join(cwd, ".omx", "state", "sessions", ownerSessionId)),
			"cancel created an owner scope",
		);
	});
}
...
async function scenarioCancelForce(packageRoot: string): Promise<void> {
	await withWorkspace("cancel-force", async (cwd, home) => {
		...
		assert(
			(await readFile(statePath(cwd, canonicalSessionId, "ralph"), "utf-8")) ===
				before,
			"unmatched owner force cancel mutated canonical state",
		);
		assert(
			!existsSync(join(cwd, ".omx", "state", "sessions", ownerSessionId)),
			"force cancel created an owner scope",
		);
	});
}

async function scenarioOwnerTerminalNativeStop(
	packageRoot: string,
): Promise<void> {
	await withWorkspace("owner-terminal-stop", async (cwd, home) => {
		...
		const consensusGate = await writeRalplanConsensusEvidence(
			cwd,
			canonicalSessionId,
		);
		...
		const terminal = runInstalledCli(
			packageRoot,
			cwd,
			[
				"state",
		...
		assert(
			terminal.status === 0,
			terminal.stderr || terminal.stdout || "owner terminal state write failed",
		);
		const terminalState = await readJson<Record<string, unknown>>(
			statePath(cwd, canonicalSessionId, "ralplan"),
		);
		assert(
			terminalState.active === false &&
				terminalState.current_phase === "complete",
			"owner terminal write did not update canonical state",
		);
		assert(
			!existsSync(join(cwd, ".omx", "state", "sessions", ownerSessionId)),
			"owner terminal write created a split owner scope",
		);
		...
		await withEnvironment(env, async () => {
			...
			assert(
				stop.outputJson?.decision !== "block",
				"native Stop continued a terminal canonical Ralph plan",
			);
			assert(
				!String(stop.outputJson?.stopReason || "").startsWith("skill_ralplan_"),
				"native Stop reported a stale Ralph-plan continuation",
			);
			assert(
				!String(stop.outputJson?.reason || "").includes(
					"continue_from_artifact",
				),
				"native Stop reported an artifact continuation",
			);
		});
	});
}
...
async function scenarioAllSessionsClear(packageRoot: string): Promise<void> {
	await withWorkspace("all-sessions-clear", async (cwd, home) => {
		...
		await writeModeState(cwd, otherSessionId, "ralph", {
			current_phase: "executing",
		});
		const result = runInstalledCli(
			packageRoot,
			...
			[
				...
				"clear",
				"--input",
				JSON.stringify({ mode: "ralph", all_sessions: true }),
				"--json",
			],
			cleanEnv(home, cwd, {
				OMX_SESSION_ID: ownerSessionId,
		...
		assert(
			result.status === 0,
			result.stderr || result.stdout || "all-sessions clear failed",
		);
		assert(
			!existsSync(statePath(cwd, canonicalSessionId, "ralph")),
			"all-sessions clear left canonical Ralph state",
		);
		assert(
			!existsSync(statePath(cwd, otherSessionId, "ralph")),
			"all-sessions clear left existing secondary Ralph state",
		);
		assert(
			!existsSync(join(cwd, ".omx", "state", "sessions", ownerSessionId)),
			"all-sessions clear initialized an owner scope",
		);
	});
}
...
async function withMockedTelegramDelivery<T>(
	run: (deliveries: Array<Record<string, unknown>>) => Promise<T>,
): Promise<T> {
	...
	globalThis.fetch = (async (
		...
		return new Response(
			JSON.stringify({
				ok: true,
				result: { message_id: `issue3138-${deliveries.length}` },
			}),
			{
				status: 200,
...
async function scenarioManagedUnmatchedIdleDeliveryNoReceipts(
	packageRoot: string,
): Promise<void> {
	await withWorkspace("managed-unmatched-idle", async (cwd, home) => {
		...
		await withEnvironment(
			...
			async () => {
				...
				await withMockedTelegramDelivery(async (deliveries) => {
					const result = await notifications.notifyLifecycle(
						"session-idle",
						{
						...
						undefined,
						{ persistScopedReceipts: false },
					);
					assert(
						result?.anySuccess === true,
						"managed unmatched idle delivery did not succeed",
					);
					assert(
						result.results.some((entry) => entry.messageId === "issue3138-1"),
						"managed unmatched idle delivery omitted the provider message ID",
					);
					assert(
						deliveries.length === 1,
						"managed unmatched idle delivery did not reach the provider",
					);
				});
			},
		);
		assert(
			...
			"managed unmatched idle wrote an idle receipt",
		);
		assert(
			!existsSync(
				join(
			...
			"managed unmatched idle wrote a lifecycle receipt",
		);
		assert(
			!existsSync(join(home, ".omx", "state", "reply-session-registry.jsonl")),
			"managed unmatched idle wrote a reply mapping",
		);
		assert(
			!existsSync(join(home, ".omx", "state", "reply-session-registry.lock")),
			"managed unmatched idle left a reply registry lock",
		);
	});
}

async function scenarioRejectedRootNoLaunch(
	packageRoot: string,
): Promise<void> {
	await withWorkspace("rejected-root", async (cwd, home) => {
		const outsideRoot = await mkdtemp(
			join(tmpdir(), "omx-issue-3138-rejected-root-"),
		);
		const binDir = join(cwd, "bin");
		...
		try {
			...
			assert(
				result.status !== 0,
				"rejected root direct launch unexpectedly succeeded",
			);
			assert(
				/session_pointer_context_failure|outside allowed roots/i.test(
					`${result.stdout}\n${result.stderr}`,
				),
				"rejected root did not report a typed context abort",
			);
			const invocationText = existsSync(invocationLog)
				? await readFile(invocationLog, "utf-8")
				: "";
			assert(!/^codex\b/m.test(invocationText), "rejected root launched Codex");
			assert(
				!/^tmux (?:new-session|set-option|split-window|send-keys)\b/m.test(
					invocationText,
				),
				"rejected root mutated tmux state",
			);
			assert(
				!existsSync(join(cwd, ".omx", "state", "session.json")),
				"rejected root committed a session pointer",
			);
			assert(
				!existsSync(join(cwd, ".omx", "runtime")),
				"rejected root created post-launch runtime artifacts",
			);
		} finally {
...
async function scenarioManagedUnmatchedLifecycleDeliveryNoReceipts(
	packageRoot: string,
): Promise<void> {
	await withWorkspace("managed-unmatched-lifecycle", async (cwd, home) => {
		...
		await writeNotificationConfig(home);
		await withEnvironment(
			cleanEnv(home, cwd, { OMX_SESSION_ID: ownerSessionId }),
			async () => {
				const notifications = await importInstalled<{
					notifyLifecycle: (
						event: string,
				...
				await withMockedTelegramDelivery(async (deliveries) => {
					const first = await notifications.notifyLifecycle(
						"session-start",
						{
							sessionId: ownerSessionId,
							projectPath: cwd,
					...
					const second = await notifications.notifyLifecycle(
						"session-start",
						{
							sessionId: ownerSessionId,
							projectPath: cwd,
					...
					assert(
						first?.anySuccess === true && second?.anySuccess === true,
						"managed unmatched lifecycle delivery did not succeed",
					);
					assert(
						deliveries.length === 2,
						"managed unmatched lifecycle delivery was unexpectedly deduped",
					);
				});
			},
		);
		assert(
			...
			"managed unmatched lifecycle wrote a lifecycle receipt",
		);
	});
}

async function scenarioExplicitPayloadSessionIsolation(
	packageRoot: string,
): Promise<void> {
	await withWorkspace("explicit-payload-isolation", async (cwd, home) => {
		...
		await writeModeState(cwd, firstSessionId, "ralph", {
			marker: "first-before",
			owner_codex_session_id: firstSessionId,
		});
		await writeModeState(cwd, secondSessionId, "ralph", {
			marker: "second-before",
			owner_codex_session_id: secondSessionId,
		});
		...
		await withEnvironment(cleanEnv(home, cwd), async () => {
			...
			const secondBeforeFirst = await readFile(
				statePath(cwd, secondSessionId, "ralph"),
				"utf-8",
			);
			const first = await native.dispatchCodexNativeHook(
				{
					...
					thread_id: "explicit-first",
					turn_id: "explicit-first-turn",
					cwd,
			...
			assert(
				first.outputJson?.decision !== "block",
				"first explicit payload prompt was blocked",
			);
			assert(
				(await readFile(statePath(cwd, secondSessionId, "ralph"), "utf-8")) ===
					secondBeforeFirst,
				"first explicit payload mutated second scoped state",
			);

			const firstBeforeSecond = await readFile(
				statePath(cwd, firstSessionId, "ralph"),
				"utf-8",
			);
			const second = await native.dispatchCodexNativeHook(
				{
					...
					thread_id: "explicit-second",
					turn_id: "explicit-second-turn",
					cwd,
			...
			assert(
				second.outputJson?.decision !== "block",
				"second explicit payload prompt was blocked",
			);
			assert(
				(await readFile(statePath(cwd, firstSessionId, "ralph"), "utf-8")) ===
					firstBeforeSecond,
				"second explicit payload mutated first scoped state",
			);
		});
		assert(
			(await readFile(pointerPath, "utf8")) === pointerBefore,
			"explicit payload prompts rewrote the selected pointer",
		);
	});
}

async function scenarioNotifyExistingForkPreservesPayloadOwner(
	packageRoot: string,
): Promise<void> {
	await withWorkspace("notify-existing-fork-owner", async (cwd, home) => {
		...
		await mkdir(join(cwd, ".omx", "state", "sessions", forkSessionId), {
			recursive: true,
		});
		...
		await writeFile(
			canonicalHudPath,
			JSON.stringify({ turn_count: 7, marker: "canonical-source" }, null, 2),
		);
		...
		assert(
			pointer.native_session_id === payloadSessionId,
			"notify fork setup did not bind payload owner P",
		);
		const result = runInstalledScript(
			packageRoot,
			cwd,
			...
			[
				JSON.stringify({
					...
					thread_id: "notify-fork-thread",
					turn_id: "notify-fork-turn",
					input_messages: ["$ralph continue"],
					last_assistant_message: "fork storage receipt",
				}),
			],
		...
		assert(
			result.status === 0,
			result.stderr || result.stdout || "installed notify fork route failed",
		);
		const forkDir = join(cwd, ".omx", "state", "sessions", forkSessionId);
		const skillState = await readJson<Record<string, unknown>>(
			join(forkDir, "skill-active-state.json"),
		);
		const ralphState = await readJson<Record<string, unknown>>(
			join(forkDir, "ralph-state.json"),
		);
		assert(
			skillState.owner_codex_session_id === payloadSessionId,
			"notify fork skill state did not retain payload owner P",
		);
		assert(
			ralphState.owner_codex_session_id === payloadSessionId,
			"notify fork Ralph seed did not retain payload owner P",
		);
		const second = runInstalledScript(
			packageRoot,
			cwd,
			"dist/scripts/notify-hook.js",
			[
				JSON.stringify({
					...
					type: "agent-turn-complete",
					thread_id: "notify-fork-thread",
					turn_id: "notify-fork-turn-2",
					input_messages: [],
					last_assistant_message: "fork lifecycle continuation",
				}),
			],
			cleanEnv(home, cwd, { OMX_SESSION_ID: forkSessionId }),
		);
		assert(
			...
			second.stderr ||
				second.stdout ||
				"installed notify fork continuation failed",
		);
		const ralphAfterSecond = await readJson<Record<string, unknown>>(
			join(forkDir, "ralph-state.json"),
		);
		assert(
			ralphAfterSecond.owner_codex_session_id === payloadSessionId,
			"notify fork lifecycle sync lost payload owner P",
		);
		assert(
			existsSync(
				join(cwd, ".omx", "state", "sessions", forkSessionId, "hud-state.json"),
			),
			"notify fork route did not mutate existing fork storage",
		);
		assert(
			existsSync(
				join(
			...
			"notify fork route did not persist its fork receipt",
		);
		assert(
			(await readFile(canonicalHudPath, "utf-8")) === canonicalHudBefore,
			"notify fork route mutated canonical source storage",
		);
		assert(
			!existsSync(
				join(
			...
			"notify fork route created a canonical receipt",
		);
		assert(
			!existsSync(join(cwd, ".omx", "state", "sessions", payloadSessionId)),
			"notify fork route created payload owner storage",
		);
		assert(
			(await readFile(pointerPath, "utf-8")) === pointerBefore,
			"notify fork route mutated the payload owner pointer",
		);
	});
}
...
async function main(): Promise<void> {
	...
	assert(
		scenarios.length === MANDATORY_SCENARIOS.length,
		"installed smoke omitted a mandatory scenario",
	);
	assert(
		new Set(scenarios.map((scenario) => scenario.name)).size ===
			MANDATORY_SCENARIOS.length,
		"installed smoke duplicated a mandatory scenario",
	);
	const report = {
		schema: SCHEMA,
