...
describe("notify-hook state I/O session authority", () => {
	it("uses an explicit session id before the current session pointer", async () => {
		...
		try {
			...
			await mkdir(join(stateDir, "sessions", "sess-explicit"), {
				recursive: true,
			});
			await writeFile(
			...
			await writeScopedJson(stateDir, "hud-state.json", "sess-explicit", {
				turn_count: 3,
			});
	...
	it("resolves current session from authoritative team state root without cwd inference", async () => {
		...
		try {
			...
			await mkdir(join(teamStateRoot, "sessions", "sess-team-root"), {
				recursive: true,
			});
			await writeFile(
				join(teamStateRoot, "session.json"),
				JSON.stringify(
					{ session_id: "sess-team-root", cwd: join(wd, "source-repo") },
					null,
					2,
				),
				"utf-8",
			...
			assert.equal(
				await resolveScopedStateDir(teamStateRoot),
				join(teamStateRoot, "sessions", "sess-team-root"),
			);
			const value = await readScopedJsonIfExists(
				teamStateRoot,
				"hud-state.json",
	...
	it("prefers OMX_SESSION_ID over stale session.json for notify state writes", async () => {
		...
		try {
			...
			await writeFile(
				join(stateDir, "session.json"),
				JSON.stringify(
					{ session_id: "sess-stale", cwd: join(wd, "..", "other-worktree") },
					null,
					2,
				),
				"utf-8",
			...
			assert.equal(
				...
				join(stateDir, "sessions", "sess-env"),
			);

			await writeScopedJson(stateDir, "hud-state.json", undefined, {
				turn_count: 7,
			});
			const value = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-env", "hud-state.json"),
					"utf-8",
				),
			) as { turn_count?: unknown };
	...
	it("maps native Codex session aliases to the canonical OMX session", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-notify-state-io-native-alias-"),
		);
		...
		try {
			...
			assert.equal(
				await resolveScopedStateDir(stateDir),
				join(stateDir, "sessions", "omx-canonical"),
			);
			...
			const value = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "omx-canonical", "hud-state.json"),
					"utf-8",
				),
			) as { turn_count?: unknown };
	...
	it("maps the recorded OMX owner alias to the canonical session for implicit and explicit writes", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-notify-state-io-owner-alias-"),
		);
		const previousOmxSessionId = process.env.OMX_SESSION_ID;
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			await writeScopedJson(stateDir, "hud-state.json", ownerSessionId, {
				turn_count: 12,
			});
			const value = JSON.parse(
				await readFile(
	...
	it("uses the immutable target scope without consulting a stale environment session", async () => {
		...
		try {
			...
			await mkdir(join(stateDir, "sessions", "fork-existing"), {
				recursive: true,
			});
			...
			await writeScopedJsonAtScope(stateDir, "hud-state.json", scope, {
				turn_count: 1,
			});
			assert.deepEqual(
				await readScopedJsonAtScope(stateDir, "hud-state.json", scope, null),
				{ turn_count: 1 },
			);
			assert.equal(
				existsSync(join(stateDir, "sessions", "stale-singleton")),
				false,
			);
		} finally {
			if (typeof previousOmxSessionId === "string")
