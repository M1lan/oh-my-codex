...
describe("run-test-files diagnostics", () => {
	it("applies a bounded node --test timeout so hanging tests fail with file context", () => {
		...
		try {
			...
			assert.match(
				result.stderr,
				/node --test did not exit normally|runner timeout 750ms/,
			);
			assert.match(
				`${result.stdout}\n${result.stderr}`,
				/hang\.test\.js|never resolves|cancelled/i,
			);
		} finally {
	...
	it("can force-exit Node test runner after successful CI tests to avoid leaked-handle hangs", () => {
		...
		try {
			...
			const withoutForceExit = runCompiledRunner(
				wd,
				{ OMX_NODE_TEST_RUNNER_TIMEOUT_MS: "750" },
				2_000,
			);
			...
			assert.match(
				withoutForceExit.stderr,
				/did not exit normally|runner timeout 750ms/,
			);
			...
			assert.equal(
				withForceExit.status,
				0,
				withForceExit.stderr || withForceExit.stdout,
			);
