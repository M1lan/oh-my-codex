			...
			assert.equal(
				(
					JSON.parse(await readFile(path, "utf8")) as {
						sessions: Record<
							string,
							{ threads: Record<string, { provenance_kind?: string }> }
						>;
					}
				).sessions["synthetic-session"].threads["synthetic-child"]
					.provenance_kind,
				"adapted",
			);
			await writeSubagentTrackingState(cwd, state);
			assert.equal(
				JSON.parse(await readFile(path, "utf8")).sessions["synthetic-session"]
					.threads["synthetic-child"].provenance_kind,
				"adapted",
			);
		});
	});
