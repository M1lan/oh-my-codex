...
const manifestPath =
	process.env.OMX_SPARKSHELL_MANIFEST ?? join(nativeRoot, "Cargo.toml");
const binaryName =
...
if (!existsSync(manifestPath)) {
	console.error(
		`omx sparkshell build: missing Rust manifest at ${manifestPath}`,
	);
	process.exit(1);
}
...
if (result.error) {
	console.error(
		`omx sparkshell build: failed to launch cargo: ${result.error.message}`,
	);
	process.exit(1);
}
...
if (!existsSync(releaseBinaryPath)) {
	console.error(
		`omx sparkshell build: expected release binary at ${releaseBinaryPath}`,
	);
	process.exit(1);
}
...
console.log(
	`omx sparkshell build: staged native binary at ${packagedBinaryPath}`,
);
