...
export function normalizePostToolUsePayload(
	...
	const stdoutText =
		safeString(parsedToolResponse?.stdout).trim() || rawToolResponseText;
...
function structuredMcpTransportText(
...
export function commandInvokesApplyPatch(command: string): boolean {
	const tokens = tokenizeShellCommandWithBoundaries(
		removeHereDocBodies(command),
	);
	...
	for (
		...
		while (advanced && index < commandEnd) {
			...
			while (
				index < commandEnd &&
				APPLY_PATCH_COMMAND_WRAPPER_TOKENS.has(
					(tokens[index]?.value ?? "").toLowerCase(),
				)
			) {
				index += 1;
				advanced = true;
		...
		if (
			index < commandEnd &&
			shellCommandBasename(tokens[index]?.value ?? "") === "apply_patch"
		)
			return true;
		if (commandEnd <= commandStart) break;
		commandStart = commandEnd - 1;
...
function buildEffectiveLoreCommitGuardEnv(
	...
	if (
		...
		const configuredValue =
			readConfiguredLoreCommitGuardValue(effectiveEnvironment);
...
function buildGitCommitEnforcementOutput(
	...
	if (!isLoreCommitGuardEnabled(buildEffectiveLoreCommitGuardEnv(parsed)))
		return null;
...
function commandInvokesOmxQuestion(command: string): boolean {
	const tokens =
		tokenizeShellCommandWithBoundaries(removeHereDocBodies(command))?.map(
			(token) => ({ ...token, value: token.value.toLowerCase() }),
		) ?? [];
...
function isReviewableGhPrChecksTokens(tokens: string[]): boolean {
	...
	return (
		tokens[subcommandIndex] === "pr" && tokens[subcommandIndex + 1] === "checks"
	);
}

function isReviewableNonZeroBashCommand(command: string): boolean {
	const boundaryTokens = tokenizeShellCommandWithBoundaries(
		removeHereDocBodies(command),
	);
	if (!boundaryTokens) return false;
	for (
		let commandStart = 0;
		commandStart < boundaryTokens.length;
		commandStart = nextCommandStart(boundaryTokens, commandStart)
	) {
		const commandEnd = nextCommandStart(boundaryTokens, commandStart);
		const commandTokens = boundaryTokens
