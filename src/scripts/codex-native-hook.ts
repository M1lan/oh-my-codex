...
function readNativeSubagentSessionStartMetadata(
	...
	try {
		...
		const agentNickname = safeString(
			threadSpawn.agent_nickname ?? payload.agent_nickname,
		).trim();
		const agentRole = safeString(
			threadSpawn.agent_role ?? payload.agent_role,
		).trim();
		return {
			parentThreadId,
...
function looksLikeNewGoalPrompt(text: string): boolean {
	return /(?:\b(?:start|create|begin|new|another)\b.{0,80}\b(?:goal|ultragoal|performance[-\s]goal|autoresearch[-\s]goal)\b|\b(?:goal|ultragoal|performance[-\s]goal|autoresearch[-\s]goal)\b.{0,80}\b(?:start|create|begin|new|another)\b)/i.test(
		text,
	);
}
...
function commandHasDeepInterviewWriteIntent(command: string): boolean {
	return (
		...
		/\b(?:git\s+(?:checkout|switch|restore|reset|apply|am|merge|rebase)|npm\s+(?:install|i|ci)|pnpm\s+(?:install|i)|yarn\s+(?:install|add))\b/.test(
			command,
		)
	);
}
...
async function readActiveDeepInterviewStateForPreToolUse(
	...
	if (!modeStateMatchesSkillStopContext(autopilotState, cwd, sessionId))
		return null;
	const terminalAutopilotRunState = await readCanonicalTerminalRunStateForStop(
		cwd,
		sessionId,
		"autopilot",
	);
