...
interface NativeHookDispatchOptions {
	...
	sessionStartOptions?: Pick<
		SessionStartOptions,
		"platform" | "regularFileSync"
	>;
...
async function resolveVerifiedOwnerOmxSessionId(): Promise<string | undefined> {
	...
	return tmuxEvidenceBindsCandidate(evidence, candidate)
		? candidate
		: undefined;
}
...
function selectAuthoritativeTaskName(
	...
	for (const obj of [threadSpawn, subagent, payload]) {
		if (obj && typeof obj === "object" && "task_name" in obj) {
			return {
				present: true,
				value: (obj as Record<string, unknown>).task_name,
			};
		}
	}
...
function readNativeSubagentSessionStartMetadata(
	...
	try {
		...
		const authoritativeTaskName = selectAuthoritativeTaskName(
			threadSpawn,
			subagent,
			payload,
		);
...
async function readNativePromptThreadFacts(
	...
	const currentTargetAnchored = Boolean(
		nativeSessionId && currentNativeIds.has(nativeSessionId),
	);
	const trackingState = await readSubagentTrackingState(cwd).catch(() => null);
	if (!trackingState)
		return currentTargetAnchored
			? { currentTargetAnchored: true, rootOrDrift: true }
			: undefined;
	const roots = Object.values(trackingState.sessions).flatMap((session) =>
		candidateIds.some((candidate) =>
			isTrustedSubagentThread(session, candidate),
		)
			? [session.session_id]
			: [],
	);
	return roots.length > 0 || currentTargetAnchored
		? {
				trackerRootOwnerSessionIds: roots,
				currentTargetAnchored,
				rootOrDrift: currentTargetAnchored,
			}
		: undefined;
}
...
function isNativeOutsideTmuxUserPrompt(
	cwd: string,
	payload: CodexHookPayload,
	sessionId?: string,
): boolean {
...
function readTeamWorkerEnvironment(): {
	teamName: string;
	workerName: string;
} | null {
	const internalWorker = parseTeamWorkerEnv(
		safeString(process.env.OMX_TEAM_INTERNAL_WORKER),
	);
	const externalWorker = parseTeamWorkerEnv(
		safeString(process.env.OMX_TEAM_WORKER),
	);
	if (
		internalWorker &&
		externalWorker &&
		internalWorker.workerName !== externalWorker.workerName
	)
		return null;
...
async function hasAuthoritativeTeamWorkerContext(
	...
	const stateRoot = await resolveWorkerTeamStateRootPath(
		cwd,
		workerContext,
		process.env,
	).catch(() => null);
	...
	const identity = await readJsonIfExists(
	...
	const canonicalLeaderCwd = resolve(
		safeString(process.env.OMX_TEAM_LEADER_CWD).trim() || cwd,
	);
	...
	const matchingWorker = (
		...
		return (
			workers
				...
				.find(
					(candidate) =>
						safeString(candidate.name).trim() === workerContext.workerName,
				) ?? null
		);
	};
	...
	if (safeString(identity.name).trim() !== workerContext.workerName)
		return false;
	...
	if (
		!pathMatches(identity.worktree_path ?? identity.working_dir, canonicalCwd)
	)
		return false;
	...
	if (
		safeString(manifest.leader_pane_id).trim() !==
...
function payloadAliasValues(
	payload: CodexHookPayload,
	keys: readonly string[],
): string[] {
	return [
		...new Set(
			keys.map((key) => safeString(payload[key]).trim()).filter(Boolean),
		),
	];
}

function payloadHasConflictingIdentityAliases(
	payload: CodexHookPayload,
): boolean {
	const sessionAliases = payloadAliasValues(payload, [
		"session_id",
		"sessionId",
	]);
	...
	return (
		sessionAliases.length > 1 ||
		threadAliases.length > 1 ||
		agentAliases.length > 1 ||
		actors.length > 1 ||
		(ownerThreads.length > 0 &&
			actors.some((actor) => ownerThreads.some((owner) => owner !== actor)))
	);
}

function payloadHasOwnerIdentityClaim(payload: CodexHookPayload): boolean {
	return (
		payloadAliasValues(payload, [
...
function resolveConductorPolicyRoot(
	stateDir: string,
	fallbackCwd: string,
	...
	try {
		...
		const rootSession = readJsonSyncIfExists(
			join(canonicalStateDir, "session.json"),
		);
		const recordedCwd = safeString(
			rootSession?.cwd ?? rootSession?.workingDirectory,
		).trim();
		if (recordedCwd) {
			...
			return {
				...
				statePresent,
				externalStateRoot:
					canonicalStateDir !== join(canonicalRecordedCwd, ".omx", "state"),
			};
		}
		if (canonicalStateDir === join(canonicalFallback, ".omx", "state")) {
			return {
				cwd: canonicalFallback,
				valid: true,
				statePresent,
				externalStateRoot: false,
			};
		}
	} catch {
	...
	return {
		...
		valid: !statePresent,
		statePresent,
		externalStateRoot: statePresent,
	};
}
...
function isTypedAgentRolePayload(
	payload: CodexHookPayload,
	cwd: string,
): boolean {
	...
	return (
		agentRole !== "" &&
		resolveInstalledRoleName(agentRole, undefined, cwd) !== null
	);
}
...
function buildNativeUnknownRolePreToolUseOutput(
	...
	if (
		!requestedRole ||
		resolveInstalledRoleName(requestedRole, undefined, cwd) !== null
	)
		return null;
	return {
		decision: "block",
		...
		JSON.stringify(
			{
				...
				evidence: summarizeNativeSubagentSupportFailure(
					nativeSubagentResultDisposition(payload).evidenceSummary,
				),
				observed_at: nowIso,
				cwd,
			},
...
async function recordNativeSubagentCapacityBlocker(
	...
	const blocker: NativeSubagentCapacityBlocker = {
		...
		...(safeString(payload.tool_name).trim()
			? { tool_name: safeString(payload.tool_name).trim() }
			: {}),
		error_summary: summarizeCapacityFailure(
			nativeSubagentResultDisposition(payload).evidenceSummary,
		),
		observed_at: new Date(nowMs).toISOString(),
		expires_at: new Date(
...
function isProtectedPlanningStatePath(relativePath: string): boolean {
	...
	const components = relativePath
		.split("/")
		.map(normalizeProtectedPlanningStateFileName);
...
function isRawProtectedPlanningStateCandidate(
	stateDir: string,
	cwd: string,
	rawPath: string,
): boolean {
	try {
		...
		while (true) {
			try {
				...
				const relativePath = relative(canonicalStateDir, canonicalPath).replace(
					/\\/g,
					"/",
				);
...
function isAllowedPlanningArtifactPath(
	...
	if (!relativePath || conductorPathnameExpansionIsAmbiguous(relativePath))
		return false;
...
function isAllowedRalplanArtifactPath(
	cwd: string,
	rawPath: string,
	authoritativeSessionId?: string,
): boolean {
...
function extractCommandLiteralAssignments(
	...
	for (const name of [...assignments.keys()]) {
		...
		const mutationPatterns = [
			...
			new RegExp(
				`\\b(?:declare|typeset|local|export)\\b[^;\\n]*\\b${escapedName}(?:\\[[^\\]]+\\])?(?:\\b|\\s*(?:\\+?=))`,
			),
		...
		if (
			/\beval\b|\|/.test(remaining) ||
			mutationPatterns.some((pattern) => pattern.test(remaining))
		) {
			assignments.delete(name);
		}
	}
	...
	for (const name of [...assignments.keys()]) {
		const escapedName = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
		const boundedCatRedirect = new RegExp(
			`^cat\\s+>\\s+"\\$(?:\\{${escapedName}\\}|${escapedName})"\\s+<<-?\\s*['"][^'"\\s]+['"]\\s*$`,
		);
...
function decodeStaticRedirectShellWord(raw: string): string | null {
	...
	if (simpleVariable && !raw.startsWith("'"))
		return safeString(simpleVariable[1]);
	let quote: "'" | '"' | null = null;
	let decoded = "";
...
function extractDeepInterviewCommandRedirectTargets(command: string): string[] {
	...
	const commandOutsideHeredocBodies = maskQuotedRedirectMetacharsForCommandScan(
		stripHeredocBodiesForCommandScan(command),
	);
	for (const match of commandOutsideHeredocBodies.matchAll(
		/(?:^|[^<>])(?:[0-9]*)(?:<>|>>|>\||>&|>)\s*((?:"[^"]*"|'[^']*'|\\.|[^\s&|;<>])+)/g,
	)) {
		const candidate = decodeStaticRedirectShellWord(
			safeString(match[1]).trim(),
		);
		...
		if (candidate === null) {
			...
			candidate !== "-" &&
			!/^\d+$/.test(candidate) &&
			!isNullDeviceRedirectTarget(candidate)
		) {
...
function conductorRedirectTargetIsSafe(
	executionCwd: string,
	target: string,
	policyCwd = executionCwd,
): boolean {
	if (
		shellWordMayProduceWgetOptions(target) ||
		!isAllowedConductorMetadataExecutionPath(executionCwd, policyCwd, target)
	)
		return false;
	try {
		const entry = lstatSync(
			isAbsolute(target) ? resolve(target) : resolve(executionCwd, target),
		);
...
function conductorCommandMayMutatePathResolution(command: string): boolean {
	...
	return new RegExp(
		`(?:^|[\\s;|&(){}])(?:${resolutionVariable}\\s*\\+?=|(?:export|readonly|declare|typeset|local|env)\\b[^;|&(){}\\n]*${resolutionVariable}\\s*\\+?=|unset\\s+(?:-[A-Za-z]+\\s+)*${resolutionVariable}(?:\\s|$)|printf\\s+(?:-[A-Za-z]+\\s+)*-v\\s+${resolutionVariable}(?:\\s|$)|read\\s+(?:-[A-Za-z]+(?:\\s+\\S+)?\\s+)*${resolutionVariable}(?:\\s|$))`,
	).test(source);
}

function conductorCommandMayChangeProducerResolution(
	command: string,
	depth = 0,
): boolean {
	for (const segment of splitShellCommandSegments(
		...
		if (
			new Set([
				...
				"builtin",
				"command",
			]).has(commandNameFromShellWord(words[commandIndex] ?? ""))
		)
			return true;
	}
	const invokedFunctionBodies =
		extractInvokedShellFunctionBodiesForStateScan(command);
	if (invokedFunctionBodies.length > 0) {
		if (depth >= CONDUCTOR_BASH_MAX_NESTING_DEPTH) return true;
		if (
			invokedFunctionBodies.some(
				(body) =>
...
function conductorRedirectProducerMayBeShadowed(
	command: string,
	commandName: string,
): boolean {
	if (safeString(process.env[`BASH_FUNC_${commandName}%%`]).trim() !== "")
		return true;
	...
	for (let index = 0; index < source.length; index += 1) {
		...
		const bodyEnd = findShellFunctionBodyEnd(
			source,
			definition.openBraceIndex,
			definition.bodyOpenChar,
		);
		if (bodyEnd < 0) return true;
		index = bodyEnd;
	}
	return new RegExp(`(?:^|[;\\n])\\s*alias\\s+${commandName}(?:=|\\s|$)`).test(
		source,
	);
}

function conductorHeredocRedirectHasBoundedProducer(
	command: string,
	fullCommand: string,
): boolean {
	...
	if (
		openers.length !== 1 ||
		!openers[0]?.quoted ||
		extractDeepInterviewCommandRedirectTargets(command).length !== 1
	)
		return false;
	...
	if (
		commandNameFromShellWord(words[commandIndex] ?? "") !== "cat" ||
		conductorRedirectProducerMayBeShadowed(fullCommand, "cat")
	)
		return false;
...
function conductorMetadataRedirectsHaveBoundedProducers(
	...
	if (openers.length > 0) {
		...
		if (
			...
			bodies.some(
				(body) =>
					Buffer.byteLength(body, "utf-8") > MAX_CONDUCTOR_METADATA_COPY_BYTES,
			)
		)
			return false;
	}
	for (const segment of splitShellCommandSegments(
		...
		if (extractDeepInterviewCommandRedirectTargets(segment).length === 0)
			continue;
		if (extractShellHeredocOpeners(segment).length > 0) {
			if (!conductorHeredocRedirectHasBoundedProducer(segment, command))
				return false;
			continue;
		}
		const words = tokenizeConductorShellWords(segment);
		...
		if (
			!new Set([":", "echo", "printf"]).has(commandName) ||
			conductorRedirectProducerMayBeShadowed(command, commandName)
		)
			return false;
		...
		if (
			operands.some(
				(operand) =>
					/[$`]/.test(operand) ||
					conductorPathnameExpansionIsAmbiguous(shellWordLiteral(operand)),
			)
		)
			return false;
		if (
			commandName === "printf" &&
			operands.some((operand) => shellWordLiteral(operand).includes("%"))
		)
			return false;
		if (
			Buffer.byteLength(operands.map(shellWordLiteral).join(" "), "utf-8") >
			MAX_CONDUCTOR_METADATA_COPY_BYTES
		)
			return false;
	}
	return true;
...
function conductorStaticTruncateSize(
	words: string[],
	commandIndex: number,
): number | null {
	...
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (
			...
			parsed > MAX_CONDUCTOR_METADATA_COPY_BYTES
		)
			return null;
		size = parsed;
	}
...
function conductorMetadataWriteSizesStayBounded(
	cwd: string,
	command: string,
): boolean {
	...
	const keyFor = (target: string): string | null => {
		if (
			!target ||
			shellWordMayProduceWgetOptions(target) ||
			conductorPathnameExpansionIsAmbiguous(target)
		)
			return null;
		try {
	...
	for (const segment of splitShellCommandSegments(
		...
		if (commandNameFromShellWord(words[commandIndex] ?? "") === "truncate") {
			const targets = collectConductorBoundedTruncateTargets(
				words,
				commandIndex,
			);
		...
		const redirectScanSegment =
			maskQuotedRedirectMetacharsForCommandScan(segment);
		for (const match of redirectScanSegment.matchAll(
			/(?:^|[^<>])(?:[0-9]*)(<>|>>|>\||>&|>)\s*(["']?)([^\s&|;<>]+)\2/g,
		)) {
...
function commandExecutesUninspectedScript(
	...
	const runtimeFunctionScan = collectConductorStaticNestedBashExecutions(
		command,
		cwd,
		inheritedShellFunctions,
	);
	for (const segment of [stripHeredocBodiesForCommandScan(command)]) {
		...
		for (let index = 0; index < words.length; index += 1) {
			...
			if (
				commandStart &&
				(isEnvironmentAssignmentWord(word) ||
					CONDUCTOR_BASH_COMPOUND_SYNTAX_WORDS.has(word))
			)
				continue;
			...
			while (!visitedIndexes.has(commandIndex)) {
				...
				const commandName = commandNameFromShellWord(words[commandIndex] ?? "");
				const operandIndex = findConductorWrapperOperandIndex(
					commandName,
					words,
					commandIndex + 1,
				);
			...
			const functionBodies =
				runtimeFunctionScan.functions.get(commandWord) ?? [];
			if (
				functionBodies.some(isConductorFunctionBody) &&
				!isConductorShellFunctionDefinitionInvocation(words, commandIndex)
			) {
				if (depth >= CONDUCTOR_BASH_MAX_NESTING_DEPTH) return true;
				if (
					functionBodies.some(
						(body) =>
							isConductorFunctionBody(body) &&
							commandExecutesUninspectedScript(
								body,
								cwd,
								runtimeFunctionScan.functions,
								depth + 1,
							),
					)
				)
			...
			if (commandName === "source" || commandName === ".") {
				...
				if (firstInterpreterScriptOperands(words, commandIndex).length > 0)
					return true;
			} else if (
				/\.(?:ba|z|k)?sh$|\.(?:[cm]?js|[cm]?ts|py|rb|pl|php|lua)$/i.test(
					commandWord,
				)
			) {
				return true;
	...
	if (
		...
		runtimeFunctionScan.executions.length > 0
	)
		return true;
	return runtimeFunctionScan.executions.some((nested) =>
		commandExecutesUninspectedScript(
			nested.command,
			cwd,
			nested.functions,
...
function commandHasDeepInterviewWriteIntent(
	command: string,
	depth = 0,
	cwd = process.cwd(),
): boolean {
	return (
		...
		/\b(?:python3?|perl|ruby)\b[\s\S]{0,260}\b(?:writeFileSync|writeFile|write_text|open\([^)]*["']w|File\.write|Path\()/.test(
			command,
		) ||
		...
		(depth < CONDUCTOR_BASH_MAX_NESTING_DEPTH &&
			(extractNestedShellCommandStringsForStateScan(command).some((nested) =>
				commandHasDeepInterviewWriteIntent(nested, depth + 1, cwd),
			) ||
				extractNestedCommandSubstitutionStringsForStateScan(command).some(
					(nested) =>
						commandHasDeepInterviewWriteIntent(nested, depth + 1, cwd),
				)))
	);
...
type PreToolUseMutationTransport =
	| "read-only"
	| "bash"
	| "path"
	| "state"
...
const READ_ONLY_PRETOOLUSE_MCP_CONTRACT = {
	...
	trace: ["mcp__omx_trace__trace_timeline", "mcp__omx_trace__trace_summary"],
	codeIntel: [
...
function classifyPreToolUseMutationTransport(
	...
	if (toolName === "Bash") {
		...
		return commandHasDeepInterviewWriteIntent(command, 0, cwd) ||
	...
	if (
		READ_ONLY_PRETOOLUSE_TOOL_NAMES.has(toolName) ||
		READ_ONLY_PRETOOLUSE_MCP_TOOL_NAMES.has(toolName)
	) {
		return "read-only";
	}
	if (
...
function extractDeepInterviewCommandWriteTargets(
	command: string,
	cwd = process.cwd(),
	rootCwd = cwd,
): string[] {
	...
	for (const nestedCommand of extractNestedShellCommandStringsForStateScan(
		...
		targets.push(
			...extractDeepInterviewCommandWriteTargets(nestedCommand, cwd),
		);
	}
...
function isUnresolvedVariableTarget(target: string): boolean {
	...
	return (
		target === "<unresolved-redirect-target>" ||
...
function normalizeSameCommandScriptTarget(
	...
	const trimmed = resolveCommandRedirectTarget(rawPath.trim(), assignments);
...
function conductorInvocationUsesEnvCwdChangingWrapper(
	words: string[],
	commandStartIndex: number,
	commandIndex: number,
): boolean {
	return words
		.slice(commandStartIndex, commandIndex)
		.some(isEnvCwdChangingOption);
}
...
function isShellFunctionInvokedLater(
	...
	for (const segment of splitShellCommandSegments(command)) {
		...
		const commandStarts = new Set(collectShellCommandStartIndexes(words));
		if (
			[...commandStarts].some((startIndex) =>
				isShellFunctionInvokedFromWords(words, startIndex, functionName),
			)
		)
			return true;
	}
...
function firstShellScriptOperands(
	...
	for (let index = shellWordIndex + 1; index < words.length; index += 1) {
		...
		if (isShellOptionWithSeparateValue(word)) {
			...
			if ((word === "--init-file" || word === "--rcfile") && value)
				operands.push(value);
			index += 1;
			continue;
		}
...
function pythonCommandUsesIsolatedStartup(
	words: string[],
	commandIndex: number,
): boolean {
	for (let index = commandIndex + 1; index < words.length; index += 1) {
...
function isConductorWritableMetadataRuntimeCwd(cwd: string): boolean {
	...
	return CONDUCTOR_ORCHESTRATION_METADATA_PREFIXES.some(
		(prefix) =>
...
function pythonCwdHasLoadableStartupCandidate(cwd: string): boolean {
	...
	for (const entry of entries) {
		for (const moduleName of startupModules) {
			...
			if (
				new RegExp(`^${escapedModuleName}\\.(?:py|pyc|pyo)$`, "i").test(
					entry.name,
				) ||
				new RegExp(
					`^${escapedModuleName}(?:\\.[A-Za-z0-9_-]+)*\\.(?:so|pyd|dll|dylib)$`,
					"i",
				).test(entry.name)
			)
				return true;
	...
	try {
		...
		return startupModules.some((moduleName) => {
			...
			return cachedEntries.some((entry) =>
				new RegExp(
					`^${escapedModuleName}(?:\\.[A-Za-z0-9_-]+)*\\.(?:py[co]|so|pyd|dll|dylib)$`,
					"i",
				).test(entry),
			);
		});
...
function pythonInvocationHomeHasLoadableStartupCandidate(
	words: string[],
	commandIndex: number,
	isolated: boolean,
): boolean {
	...
	for (
		...
		index >= 0 && !isShellCommandSeparatorAt(words, index);
		index -= 1
	) {
	...
	try {
		...
		for (const entry of readdirSync(userLib, { withFileTypes: true })) {
			...
			if (
				...
				pythonCwdHasLoadableStartupCandidate(sitePackages)
			)
				return true;
		}
		return false;
...
const CONDUCTOR_SAFE_PYTHON_X_OPTIONS = new Set([
	"dev",
	"utf8",
...
function pythonCommandHasOnlySafeOptions(
	words: string[],
	commandIndex: number,
): boolean {
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (word.startsWith("-W") || word.startsWith("--check-hash-based-pycs="))
			continue;
		if (word === "-I") continue;
		if (/^-[A-Za-z]+$/.test(word)) {
...
function pythonCommandHasLiteralReviewedReadOnlyInlineSource(
	words: string[],
	commandIndex: number,
): boolean {
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (
			!rawSource ||
			isShellCommandTerminatorOrGroupClose(rawSource) ||
			shellWordMayProduceWgetOptions(rawSource)
		)
			return false;
...
function pythonInvocationHasUnsafeRuntimeEnvironment(
	words: string[],
	commandIndex: number,
): boolean {
	...
	const unsafeNames = new Set(
		[...CONDUCTOR_PYTHON_DANGEROUS_ENVIRONMENT_NAMES].filter(
			(name) => safeString(process.env[name]).trim() !== "",
		),
	);
	const applyRange = (start: number, end: number): void => {
		for (let index = start; index < end; index += 1) {
			...
			if (
				assignment &&
				CONDUCTOR_PYTHON_DANGEROUS_ENVIRONMENT_NAMES.has(assignment.name)
			) {
			...
			if (commandNameFromShellWord(words[index] ?? "") === "env") {
				for (let cursor = index + 1; cursor < end; cursor += 1) {
					...
					if (option === "-u" || option === "--unset") {
						...
						if (!name) break;
						if (CONDUCTOR_PYTHON_DANGEROUS_ENVIRONMENT_NAMES.has(name))
							unsafeNames.delete(name);
						cursor += 1;
						continue;
					}
			...
			for (index += 1; index < end; index += 1) {
				...
				if (
					!operand.startsWith("-") &&
					CONDUCTOR_PYTHON_DANGEROUS_ENVIRONMENT_NAMES.has(operand)
				)
					unsafeNames.delete(operand);
			}
		}
	};
		...
		for (const segment of splitShellCommandSegments(normalizedCommand)) {
			...
			for (let index = 0; index < words.length; index += 1) {
				...
				for (const operand of operands) {
					...
					if (!relativePath && /[$`]/.test(operand))
						return "<unresolved-script-target>";
					if (
						relativePath &&
		...
		for (const segment of splitShellCommandSegments(normalizedCommand)) {
			...
			for (let index = 0; index < words.length; index += 1) {
				...
				for (const rawOperand of rawOperands) {
					const operand = normalizeSameCommandScriptTarget(
						operandCwd,
						rawOperand,
						assignments,
					);
				...
				if (isNestedShellCommandWord(word)) {
					for (
						...
						const sourcesZero =
							/(?:^|[;&|]\s*|\s)(?:source|\.)\s+['"]?\$0/.test(nestedSource);
						const sourcesArguments =
							/(?:^|[;&|]\s*|\s)(?:source|\.)\s+['"]?\$@/.test(nestedSource);
						if (!sourcesZero && !sourcesArguments) break;
						if (sourcesZero) {
							const zeroTarget = normalizeSameCommandScriptTarget(
								operandCwd,
								words[optionIndex + 2] ?? "",
								assignments,
							);
						...
						if (sourcesArguments) {
							for (
								let argumentIndex = optionIndex + 3;
								argumentIndex < words.length;
								argumentIndex += 1
							) {
								const positionalTarget = normalizeSameCommandScriptTarget(
									operandCwd,
									words[argumentIndex] ?? "",
									assignments,
								);
								if (positionalTarget && writtenTargets.has(positionalTarget))
									return true;
							}
						}
						break;
...
function decodeAnsiCShellEscape(
	segment: string,
	startIndex: number,
	...
	if (escape === "x") {
		const digits =
			/^([0-9A-Fa-f]{1,2})/.exec(segment.slice(startIndex + 2))?.[1] ?? "";
		return digits
			? decodeDigits(digits, 16, startIndex + 1 + digits.length)
			: null;
	}
	if (/^[0-7]$/.test(escape)) {
		const digits =
			/^([0-7]{1,3})/.exec(segment.slice(startIndex + 1))?.[1] ?? "";
...
function tokenizeShellWords(segment: string): string[] {
	...
	const pushCurrent = (): void => {
		if (wordStarted) {
			...
			words.push(
				wordQuoted && reserved.has(current)
					? `${CONDUCTOR_QUOTED_RESERVED_WORD_PREFIX}${current}`
					: current,
			);
		}
		current = "";
	...
	for (let index = 0; index < segment.length; index += 1) {
		...
		if (quote !== "'" && quote !== "$'" && char === "$") {
			...
			const legacyArithmeticEnd = findShellLegacyArithmeticExpansionEnd(
				segment,
				index,
			);
			const parameterEnd =
...
function omxStateTransportHasUnsafeRuntimeWrapper(command: string): boolean {
	...
	const unsafeEnvironmentNames = new Set([
		"NODE_OPTIONS",
		...CONDUCTOR_NODE_OUTPUT_ENVIRONMENT_NAMES,
	]);
	if (
		[...unsafeEnvironmentNames].some(
			(name) => safeString(process.env[name]).trim() !== "",
		)
	)
		return true;
	...
	if (
		rawWords.some(
			(word) =>
				isEnvironmentAssignmentWord(word) &&
				unsafeEnvironmentNames.has(shellAssignmentName(word)),
		)
	)
		return true;
	for (let passes = 0; passes < 8; passes += 1) {
		...
		const runtimeIndex = words.findIndex((word) => {
			...
			return (
				isNodeInterpreterCommandWord(commandName) ||
				commandName === "bun" ||
				commandName === "tsx"
			);
		});
...
function resolveConductorTimeLongOption(
	word: string,
): { option: string; inlineValue: string | undefined } | null {
	if (!word.startsWith("--")) return null;
	...
	return {
		option: matches[0] ?? "",
		inlineValue: separator < 0 ? undefined : word.slice(separator + 1),
	};
}
...
function parseConductorTimeInvocation(
	words: string[],
	startIndex: number,
): ConductorTimeInvocation | null {
	...
	for (let index = startIndex; index < words.length; index += 1) {
		...
		if (isShellAssignmentWord(option)) continue;
		if (option === "--") {
			const dispatchIndex = skipShellCommandPositionPrefixWords(
				words,
				index + 1,
			);
			return dispatchIndex < words.length
				? { dispatchIndex, outputTarget }
				: null;
		}
		if (option.startsWith("--")) {
			...
			if (
				timeOption.option === "--output" ||
				timeOption.option === "--format"
			) {
				const value =
					timeOption.inlineValue ?? shellWordLiteral(words[index + 1] ?? "");
				if (
					!value ||
					isShellCommandTerminatorOrGroupClose(value) ||
					shellWordMayProduceWgetOptions(value)
				)
					return null;
...
function findXargsDispatchOperandIndex(
	...
	for (let index = startIndex; index < words.length; index += 1) {
		...
		if (isShellAssignmentWord(option)) continue;
		if (option.startsWith("-")) {
			const optionValueWordCount = conductorXargsOptionValueWordCount(
				words,
				index,
			);
...
function collectShellCasePhases(
	words: string[],
): Array<"pattern" | "body" | null> {
	const phases: Array<"pattern" | "body" | null> = Array.from(
		{ length: words.length },
		() => null,
	);
...
function isCasePatternPipe(words: string[], pipeIndex: number): boolean {
	return (
		words[pipeIndex] === "|" &&
		collectShellCasePhases(words)[pipeIndex] === "pattern"
	);
}
...
function isShellCommandSeparatorAt(words: string[], index: number): boolean {
	const word = words[index] ?? "";
	return (
		isShellCommandSeparator(word) &&
		!(word === "|" && isCasePatternPipe(words, index))
	);
}
...
function collectShellCommandStartIndexes(words: string[]): number[] {
	...
	for (let index = 0; index < words.length; index += 1) {
		...
		if (phase === "body" && (word === ";;" || word === ";&" || word === ";;&"))
			continue;
		if (
			isShellCommandSeparatorAt(words, index) ||
			word === "(" ||
			word === "{" ||
			commandIntroducers.has(word)
		)
			starts.add(index + 1);
	}
	return [...starts]
		.filter((index) => index < words.length && phases[index] !== "pattern")
...
function isStructuredUltragoalSteeringShellCommand(command: string): boolean {
	if (
		...
		tokenizeConductorShellWords(command).some((word) =>
			["for", "while", "until", "select", "read"].includes(
				shellWordLiteral(word),
			),
		)
	)
		return false;
...
function nestedBashLastpipeEnabled(
	words: string[],
	shellIndex: number,
	commandStringIndex: number,
): boolean {
	let enabled = false;
...
function isDynamicNestedCommandString(command: string): boolean {
	return (
		hasUnresolvedShellArithmeticExpansion(command) ||
...
function splitShellCommandSegments(command: string): string[] {
	...
	for (let index = 0; index < command.length; index += 1) {
		...
		if (char === "$") {
			const expansionEnd =
				findShellArithmeticExpansionEnd(command, index) ??
...
function parseShellAssignmentWord(
	word: string,
): { name: string; append: boolean; value: string } | null {
	const match = /^([A-Za-z_][A-Za-z0-9_]*)(\+?=)([\s\S]*)$/.exec(
		shellWordLiteral(word),
	);
	return match
		? { name: match[1] ?? "", append: match[2] === "+=", value: match[3] ?? "" }
		: null;
}
...
function decodeShellHeredocAnsiEscape(
	line: string,
	startIndex: number,
): { value: string; endIndex: number } | null {
...
function parseShellHeredocDelimiter(
	line: string,
	startIndex: number,
): Omit<ShellHeredocOpener, "stripLeadingTabs"> | null {
	let index = startIndex;
	...
	return delimiter && !quote && !delimiter.includes("\0")
		? { delimiter, quoted }
		: null;
}

function findShellArithmeticExpansionEnd(
	line: string,
	startIndex: number,
): number | null {
...
function findShellLegacyArithmeticExpansionEnd(
	line: string,
	startIndex: number,
): number | null {
...
function findConductorBareArithmeticCommandEnd(
	line: string,
	startIndex: number,
): number | null {
	if (line.slice(startIndex, startIndex + 2) !== "((") return null;
...
function hasUnsafeConductorArithmeticCommand(command: string): boolean {
	...
	for (let index = 0; index < command.length; index += 1) {
		...
		if (
			command.slice(index, index + 3) === "let" &&
			(index === 0 || /[\s;&|()]/.test(command[index - 1] ?? "")) &&
			/(?:\s|$)/.test(command[index + 3] ?? "")
		)
			return true;
		if (
			char !== "(" ||
...
function findShellParameterExpansionEnd(
	line: string,
	startIndex: number,
): number | null {
	if (line.slice(startIndex, startIndex + 2) !== "${") return null;
...
function staticExpansionAssignmentMarker(
	raw: string,
	kind: "ARITH" | "PARAMETER",
): string | null {
	const expression =
		kind === "ARITH"
	...
	for (const match of expression.matchAll(assignmentPattern)) {
		...
		const mode =
			operator === "+=" ? "APPEND" : operator === "=" ? "SET" : "DYNAMIC";
		const value =
			mode === "DYNAMIC" || /[$`]/.test(rawValue) || !rawValue
				? "DYNAMIC"
				: rawValue;
	...
	if (kind === "ARITH" && /(?:=|\+\+|--)/.test(expression)) {
		for (const name of CONDUCTOR_SHELL_BINDING_NAMES) {
			if (
				...
				!markers.some((marker) => marker.includes(`_ASSIGN_${name}_`))
			) {
	...
	if (kind === "PARAMETER" && markers.length === 0) {
		...
		if (parameter) {
			...
			markers.push(
				`$CONDUCTOR_PARAMETER_ASSIGN_${name}_SET_${/[$`]/.test(value) || !value ? "DYNAMIC" : value}`,
			);
		}
	}
...
function maskShellNonCommandExpansionsForConductorScan(
	command: string,
): string {
	let result = "";
	let quote: "'" | '"' | "$'" | null = null;
	for (let index = 0; index < command.length; index += 1) {
		...
		const arithmeticEnd =
			char === "$" ? findShellArithmeticExpansionEnd(command, index) : null;
		const legacyArithmeticEnd =
			char === "$"
				? findShellLegacyArithmeticExpansionEnd(command, index)
				: null;
		const parameterEnd =
...
function isShellHeredocTerminator(
	line: string,
	opener: ShellHeredocOpener,
): boolean {
	const rawCandidate = opener.stripLeadingTabs
		? line.replace(/^\t+/, "")
		: line;
...
function stripHeredocBodiesForCommandScan(command: string): string {
	...
	for (let index = 0; index < lines.length; index += 1) {
		...
		for (const opener of openers) {
			...
			while (
				index < lines.length &&
				!isShellHeredocTerminator(lines[index] ?? "", opener)
			) {
				kept.push("");
				index += 1;
			}
...
function isStandaloneParsedOmxStateWriteTransport(
	...
	if (stateWriteOperations.length !== 1 || stateWriteOperations[0]?.nested)
		return false;
	const stateWriteOperation = stateWriteOperations[0];
	...
	if (
		...
		return false;
	const usesInputFile =
		readStateWriteFlagValue(stateWriteOperation.args, "--input-file") !==
		undefined;
	if (usesInputFile && splitStateScanSegments(canonicalCommand).length !== 1)
		return false;
	...
	if (classifyConductorExecutableRuntime(canonicalCommand, 0, cwd) !== null)
		return false;
	const mutations = extractConductorBashMutations(command, cwd);
	return (
		mutations.length === 1 &&
		mutations[0]?.mainRootStructuredStateWrite === true
	);
}
...
function isAllowedDeepInterviewRalplanHandoffCommand(
	...
	return targets.every((target) =>
		isAllowedDeepInterviewArtifactPath(cwd, target, authoritativeSessionId),
	);
}
...
function suppliedSessionAliasesMatch(
	payload: Record<string, unknown>,
	sessionId: string,
): boolean {
	return [
...
function hasOnlyFinishedExplicitOutcomes(
	payload: Record<string, unknown>,
): boolean {
	const values = [
...
function isAllowedDeepInterviewTerminalStateWriteCommand(
	...
	if (omxStateTransportHasUnsafeRuntimeWrapper(command)) return false;
	const rawWords = tokenizeShellWords(
		normalizeShellLineContinuations(command).trim(),
	);
	...
	if (
		!operation ||
		operation.nested ||
		operation.commandPrefix.trim() ||
		hasPriorExecutableCommand(operation.prefix)
	)
		return false;
	const inlineInputCount = operation.args.filter(
		(arg) => arg === "--input" || arg.startsWith("--input="),
	).length;
	if (
		inlineInputCount !== 1 ||
		operation.args.some(
			(arg) => arg === "--input-file" || arg.startsWith("--input-file="),
		)
	)
		return false;
	...
	return isCompleteDeepInterviewTerminalWritePayload(
		rawPayload,
		activeState,
		sessionId,
		cwd,
...
function isAllowedRalplanTerminalStateWriteCommand(
	...
	const payload = readStateWriteInputPayload(cwd, canonicalCommand, command);
	return payload
		? isCompleteRalplanTerminalWritePayload(
				payload,
				activeState,
				sessionId,
				cwd,
...
function isAllowedDeepInterviewBashWrite(
	...
	const hasUnsafeRuntimeStateWrite = (words: string[]): boolean => {
		...
		const writeWordIndex =
			stateWordIndex >= 0 ? words.indexOf("write", stateWordIndex + 1) : -1;
		if (writeWordIndex <= stateWordIndex) return false;
		const hasRuntimeBeforeState = words
			.slice(0, stateWordIndex)
			.some((word) => {
				const base = shellWordBaseName(word);
				return (
					isNodeInterpreterCommandWord(base) || base === "bun" || base === "tsx"
				);
			});
		...
		try {
			...
			payload = parsed
				? normalizeStateWriteClassificationPayload(
						modeOverride ? { ...parsed, mode: modeOverride } : parsed,
					)
				: null;
		} catch {
			payload = null;
		...
		return (
			(!payload && stateWriteOperations.length === 0) ||
			Boolean(
				payload &&
	...
	if (isAllowedDeepInterviewRalplanHandoffCommand(cwd, command, sessionId))
		return true;
	const rawCommand = normalizeShellLineContinuations(command).trim();
	if (
		[rawCommand, canonicalizeOmxStateTransportCommand(rawCommand)].some(
			(candidate) => hasUnsafeRuntimeStateWrite(tokenizeShellWords(candidate)),
		)
	)
		return false;
	...
	if (commandEndsPlanningPhase(cwd, command)) {
		return activeState
			? isAllowedDeepInterviewTerminalStateWriteCommand(
					cwd,
					command,
					activeState,
					sessionId,
	...
	if (
		extractDeepInterviewCommandRedirectTargets(command).length > 0 &&
		!conductorMetadataRedirectsHaveBoundedProducers(command)
	)
		return false;
	if (
		extractDeepInterviewCommandRedirectTargets(command).length > 0 &&
		conductorCommandMayMutatePathResolution(command)
	)
		return false;
	if (
		extractDeepInterviewCommandRedirectTargets(command).length > 0 &&
		extractConductorBashMutations(command, cwd).some(
			(mutation) =>
				mutation.targets.length === 0 &&
				!mutation.mainRootStructuredStateWrite &&
				!mutation.mainRootStructuredOrchestrationMutation,
		)
	)
		return false;
	...
	if (
		targets.some(
			(target) => !isAllowedDeepInterviewArtifactPath(cwd, target, sessionId),
		)
	)
		return false;
	return (
		...
		targets.every((target) =>
			isAllowedDeepInterviewArtifactPath(cwd, target, sessionId),
		)
	);
}
...
async function readActiveDeepInterviewStateForPreToolUse(
	...
	if (hasActiveDeepInterviewSkill && modeState?.active === true)
		return modeState;
...
function isAllowedRalplanBashWrite(
	...
	const hasAllowedTargets =
		targets.length > 0 &&
		targets.every((target) =>
			isAllowedRalplanArtifactPath(cwd, target, sessionId),
		);
	if (sourcesFileWrittenEarlierInSameCommand(cwd, command)) return false;
	if (
		extractDeepInterviewCommandRedirectTargets(command).length > 0 &&
	...
	if (
		targets.some(
			(target) => !isAllowedRalplanArtifactPath(cwd, target, sessionId),
		)
	)
		return false;
...
function buildRalplanBashBlockedDetail(
	cwd: string,
	command: string,
	authoritativeSessionId: string,
): string {
	...
	const blockedTarget = targets.find(
		(target) =>
			!isAllowedRalplanArtifactPath(cwd, target, authoritativeSessionId),
	);
...
function buildTeamWorkerProtectedStateDeny(
	modeLabel: string,
	phase: string,
	toolName: string,
): Record<string, unknown> {
	return {
		...
		hookSpecificOutput: {
			hookEventName: "PreToolUse",
			additionalContext:
				"Team-worker product-write authority never includes clearing or rewriting protected Deep-interview, Ralplan, or Conductor state.",
		},
	};
}

function buildPlanningStateScopeDeny(
	modeLabel: string,
	phase: string,
): Record<string, unknown> {
	return {
		...
		hookSpecificOutput: {
			hookEventName: "PreToolUse",
			additionalContext:
				"Structured planning state writes fail closed unless the operation payload is bound to the current canonical session and policy cwd.",
		},
	};
}
...
function teamWorkerMutationTargetsProtectedWorkflowState(
	...
	const targetIsProtectedOrAliased = (target: string): boolean => {
		if (isRawProtectedPlanningStateCandidate(stateDir, cwd, target))
			return true;
		const relativeTarget = normalizePlanningArtifactRelativePath(cwd, target);
		return (
			relativeTarget === null || conductorPathTraversesLink(cwd, relativeTarget)
		);
	};
	...
	if (
		...
		return true;
	if (mutationTransport === "unknown" || mutationTransport === "state")
		return true;
	if (toolName === "Bash") {
		if (
			collectOmxStateCommandOperations(command, "write").length > 0 ||
			findUnquotedOmxStateCommandIndexes(command, "clear").length > 0
		)
			return true;
		let effectiveCwd = cwd;
		...
		for (const segment of splitShellCommandSegments(
			...
			const commandName =
				commandIndex === null
					? ""
					: basename(words[commandIndex] ?? "").toLowerCase();
			const protectedMutatorCommands = new Set([
				"patch",
				"ed",
				"sponge",
				"setfacl",
			...
			if (
				...
				words
					...
					.some((word) =>
						protectedMutatorCommands.has(
							basename(shellWordLiteral(word) ?? "").toLowerCase(),
						),
					)
			)
				return true;
			if (protectedMutatorCommands.has(commandName)) {
				...
				if (!target || target.startsWith("-") || /[$`*?\[\]{}]/.test(target))
					return true;
				if (
					targetIsProtectedOrAliased(
						isAbsolute(target) ? target : resolve(effectiveCwd, target),
					)
				)
					return true;
				continue;
			...
			if (
				...
				const expectedLength =
					commandName === "popd" ? commandIndex! + 1 : commandIndex! + 2;
				if (words.length !== expectedLength) return true;
				let changedCwd: string | null = null;
				if (commandName === "popd") {
					changedCwd = directoryStack.pop() ?? null;
				} else {
					changedCwd = resolveSimpleCdCommandCwd(effectiveCwd, [
						"cd",
						words[commandIndex! + 1] ?? "",
					]);
					if (commandName === "pushd" && changedCwd !== null)
						directoryStack.push(effectiveCwd);
				}
			...
			const segmentWrites = [
				...
				...extractConductorInterpreterWrites(segment).flatMap(
					(write) => write.targets,
				),
				...extractConductorBashMutations(
					segment,
					effectiveCwd,
					policyCwd,
				).flatMap((mutation) => mutation.targets),
			];
			const segmentHasWriteIntent =
				commandHasDeepInterviewWriteIntent(segment, 0, effectiveCwd) ||
			...
			for (const target of segmentWrites) {
				const effectiveTarget = isAbsolute(target)
					? target
	...
	return candidates.length === 0 || candidates.some(targetIsProtectedOrAliased);
}
...
async function buildRalplanPreToolUseBoundaryOutput(
	...
	if (
		toolName === "mcp__omx_state__state_write" &&
		!directConductorStateWritePayloadHasExactSchema(payload, cwd, sessionId)
	) {
		return buildPlanningStateScopeDeny(
			safeString(activeState.mode).trim().toLowerCase() === "autopilot"
				? "Autopilot planning"
				: "Ralplan",
			formatPhase(
	...
	if (toolName === "Bash") {
		...
		if (toolPathCandidates.length === 0) {
			...
			const blockedPath = toolPathCandidates.find(
				(candidate) => !isAllowedRalplanArtifactPath(cwd, candidate, sessionId),
			);
...
function buildRawProtectedWorkflowStatePathOutput(
	...
	const toolName = safeString(payload.tool_name).trim();
	if (classifyPreToolUseMutationTransport(payload, toolName, cwd) !== "path")
		return null;
	const candidates = collectImplementationToolPathCandidates(
		payload,
		toolName,
	...
	const protectedPath = candidates.find((candidate) =>
		isRawProtectedPlanningStateCandidate(stateDir, cwd, candidate),
	);
	...
	return {
		...
		hookSpecificOutput: {
			hookEventName: "PreToolUse",
			additionalContext:
				"Gate-bearing workflow state is protected from raw Write/Edit-style mutations. Use validated structured state transport instead.",
		},
	};
}
...
async function buildDeepInterviewPreToolUseBoundaryOutput(
	...
	if (toolName === "Bash") {
		blocked = !isAllowedDeepInterviewBashWrite(
			cwd,
			command,
			activeState,
			sessionId,
		...
		if (blocked) {
			blockedDetail = buildDeepInterviewBashBlockedDetail(
				cwd,
				command,
				sessionId,
			);
		...
		blocked =
			...
			!candidates.every((candidate) =>
				isAllowedDeepInterviewArtifactPath(cwd, candidate, sessionId),
			);
		if (blocked) {
	...
	if (toolName === "Bash") {
		...
		blocked =
			...
			toolPathCandidates.some(
				(candidate) =>
					!isAllowedRalplanArtifactPath(cwd, candidate, rootSessionId),
			);
	} else if (mutationTransport === "unknown") {
		blocked = true;
	}
...
function hasSubagentThreadSpawnProvenance(payload: CodexHookPayload): boolean {
	...
	return Boolean(
		subagent &&
			...
			Object.prototype.hasOwnProperty.call(subagent, "thread_spawn"),
	);
}
...
async function resolvePreToolUseWriteActor(
	...
	if (payloadHasConflictingIdentityAliases(payload))
		return "provenance-conflict";
	const unofficialAgentId = safeString(payload.agentId).trim();
	if (unofficialAgentId && !safeString(payload.agent_id).trim())
		return "provenance-conflict";
	const unofficialThreadId = safeString(payload.threadId).trim();
	if (unofficialThreadId && !safeString(payload.thread_id).trim())
		return "provenance-conflict";
	const trackingState = await readSubagentTrackingState(cwd).catch(() => null);
	...
	if (leaderAnchors.has(payloadAgentId) || leaderAnchors.has(payloadThreadId))
		return "main-root";
	const rootSessionState = await readRootSessionStateFromStateDir(
		stateDir,
	).catch(() => null);
	const payloadSessionId = readPayloadSessionId(payload);
	const leaderNativeSessionId =
		rootSessionState &&
		payloadMatchesSessionPointer(sessionId, rootSessionState)
	...
	if (
		...
		payloadSessionId === leaderNativeSessionId
	)
		return "main-root";
	...
	if (!payloadAgentId && isTypedAgentRolePayload(payload, cwd))
		return "native-child";
	if (payloadAgentId || payloadThreadId) return "native-child";
	return (await hasAuthoritativeTeamWorkerContext(cwd))
		? "team-worker"
		: "native-child";
}
...
function conductorPathTraversesLink(
	cwd: string,
	relativePath: string,
): boolean {
	...
	for (const segment of relativePath.split("/").filter(Boolean)) {
		...
		try {
			...
			if (entry.isSymbolicLink() || (!entry.isDirectory() && entry.nlink > 1))
				return true;
		} catch (error) {
...
function isAllowedConductorMetadataExecutionPath(
	...
	const normalized = normalizeConductorMutationTargets(
		[rawPath],
		executionCwd,
		policyCwd,
	);
...
function isAllowedConductorMetadataSourcePath(
	cwd: string,
	rawPath: string,
): boolean {
	const relativePath = normalizeRepoRelativePath(cwd, rawPath);
...
const NODE_FS_MODULE_NAMES = new Set([
	"fs",
	"node:fs",
	"fs/promises",
	"node:fs/promises",
]);
const NODE_MUTATION_CAPABLE_MODULE_NAMES = new Set([
	"child_process",
	"node:child_process",
]);
...
function isNodeMutationCapableModuleName(moduleName: string | null): boolean {
	return (
		moduleName !== null && NODE_MUTATION_CAPABLE_MODULE_NAMES.has(moduleName)
	);
}
...
function nodeCreateReadStreamHasReadOnlyOptions(
	script: string,
	mask: string,
	openIndex: number,
): boolean {
	...
	const args = splitJavaScriptCallArguments(
		script,
		mask,
		openIndex,
		closeIndex,
...
function maskJavaScriptStringsAndComments(script: string): {
	mask: string;
	valid: boolean;
} {
	const chars = [...script];
	...
	for (let index = 0; index < chars.length; index += 1) {
		...
		if (templateExpressionDepths.length > 0) {
			...
			if (char === "{")
				templateExpressionDepths[depthIndex] =
					(templateExpressionDepths[depthIndex] ?? 0) + 1;
			else if (char === "}") {
		...
		const followsPostfixUpdate =
			previousPreviousSignificant + previousSignificant === "++" ||
			previousPreviousSignificant + previousSignificant === "--";
		if (
			char === "/" &&
			!followsPostfixUpdate &&
			(!previousSignificant ||
...
function parseStaticJavaScriptStringAt(
	script: string,
	startIndex: number,
): { value: string; endIndex: number } | null {
	let index = startIndex;
	...
	for (; index < script.length; index += 1) {
		...
		const value = parseStaticJavaScriptString(
			script.slice(literalStart, index + 1),
		);
...
function findMatchingJavaScriptDelimiter(
	mask: string,
	openIndex: number,
	open: string,
	close: string,
...
function splitJavaScriptCallArguments(
	script: string,
	mask: string,
	openIndex: number,
	closeIndex: number,
...
function parseConductorSingleSubstitutionProgram(
	script: string,
): ConductorSubstitutionProgram | null {
...
function perlInPlaceSubstitutionIsPositivelySafe(
	substitution: ConductorSubstitutionProgram,
): boolean {
	return (
		!substitution.flags.includes("e") &&
...
function isPositivelyClassifiedPerlCommand(
	words: string[],
	commandIndex: number,
): boolean {
	let inPlace = false;
	...
	if (inPlace) {
		...
		return (
			substitution !== null &&
			perlInPlaceSubstitutionIsPositivelySafe(substitution)
		);
	}
	return /^print(?:\s+\$_)?\s*;?$/.test(source.trim());
}

function packageManagerInvokesScriptRuntime(
	words: string[],
	commandIndex: number,
): boolean {
	for (let index = commandIndex + 1; index < words.length; index += 1) {
...
function commandDefinesConductorCommandNotFoundHandler(
	command: string,
): boolean {
	const source = stripHeredocBodiesForCommandScan(command);
	for (let index = 0; index < source.length; index += 1) {
...
function commandHasUnsafeConductorShellState(
	command: string,
	cwd = process.cwd(),
): boolean {
	...
	if (
		...
		return true;
	const words = tokenizeConductorShellWords(
		stripHeredocBodiesForCommandScan(command),
	);
	...
	for (const commandStart of collectShellCommandStartIndexes(words)) {
		const directCommandIndex = skipShellCommandPositionPrefixWords(
			words,
			commandStart,
		);
		...
		if (directCommandWord.includes("/") && isAbsolute(directCommandWord)) {
			...
			if (!conductorSlashCommandIsTrusted(directCommandWord, commandState, cwd))
				return true;
		}
		for (let index = commandStart; index < directCommandIndex; index += 1) {
			...
			if (
				allexport &&
				assignment &&
				conductorShellStateNameIsSensitive(assignment.name)
			)
				return true;
		}

		const commandIndex =
			findWrappedCommandPositionIndex(words, commandStart) ??
			directCommandIndex;
		const commandName = commandNameFromShellWord(words[commandIndex] ?? "");
		...
		if (declaration) {
			if (
				rawOperands.some(
					(operand) =>
						!parseShellAssignmentWord(operand) && /[$`]/.test(operand),
				)
			)
				return true;
			if (
				...
				return true;
			if (operands.some((operand) => /^[+-][A-Za-z]*[iaA]/.test(operand)))
				return true;
			if (commandName === "local" && operands.includes("-")) return true;
			const declaresNameref = operands.some((operand) =>
				/^[+-][A-Za-z]*n[A-Za-z]*$/.test(operand),
			);
			if (declaresNameref) {
				for (const operand of operands.filter(
					(candidate) =>
					...
					if (
						!assignment ||
						!/^[A-Za-z_][A-Za-z0-9_]*$/.test(assignment.value)
					) {
			...
			if (allexport) {
				for (const operand of operands) {
					const assignment = parseShellAssignmentWord(operand);
					if (assignment && conductorShellStateNameIsSensitive(assignment.name))
						return true;
				}
			}
			continue;
		...
		if (
			unresolvedNameref &&
			!shellBuiltins.has(commandName) &&
			!CONDUCTOR_BASH_COMPOUND_SYNTAX_WORDS.has(commandName)
		)
			return true;
	}
...
function commandConfiguresRuntimeEnvironment(
	command: string,
	names: Set<string>,
	depth = 0,
): boolean {
	for (const segment of splitShellCommandSegments(
		...
		const exportsVariables =
			...
			((commandName === "declare" || commandName === "typeset") &&
				words.slice(commandIndex + 1).some((word) => /^-[^-]*x/.test(word)));
		if (
			exportsVariables &&
			words.slice(commandIndex + 1).some((word) => /[$`]/.test(word))
		)
			return true;
		if (
			exportsVariables &&
			words
				.slice(commandIndex + 1)
				.some((word) => names.has(word.split("=", 1)[0] ?? ""))
		)
			return true;
	}
	...
	if (depth >= CONDUCTOR_BASH_MAX_NESTING_DEPTH) return true;
	return functionBodies.some((body) =>
		commandConfiguresRuntimeEnvironment(body, names, depth + 1),
	);
}
...
function isConductorDynamicLoaderEnvironmentName(name: string): boolean {
	return (
		CONDUCTOR_DYNAMIC_LOADER_ENVIRONMENT_NAMES.has(name) ||
		/^LD_[A-Z0-9_]+$/.test(name) ||
		/^DYLD_[A-Z0-9_]+$/.test(name) ||
		name === "LIBPATH" ||
		name === "SHLIB_PATH"
...
function commandHasAmbiguousDynamicLoaderControlFlow(command: string): boolean {
	const mentionsLoaderState =
		/\b(?:LD_[A-Z0-9_]+|DYLD_[A-Z0-9_]+|LIBPATH|SHLIB_PATH|loader)\b/.test(
			command,
		);
	if (!mentionsLoaderState) return false;
	return /&&|\|\||\b(?:if|elif|case|for|while|until|select|function)\b|(?:^|[;|&()]\s*)[A-Za-z_][A-Za-z0-9_]*\s*\(\s*\)\s*\{/.test(
		command,
	);
}

function commandHasUnsafeDynamicLoaderEnvironment(
	command: string,
	depth = 0,
): boolean {
	if (
		/\bcommand\s+(?:env\s+)?(?:LD_[A-Z0-9_]+|DYLD_[A-Z0-9_]+|LIBPATH|SHLIB_PATH)=/.test(
			command,
		)
	)
		return true;
	if (
		/(?:^|[;|&()]\s*)(?:LD_[A-Z0-9_]+|DYLD_[A-Z0-9_]+|LIBPATH|SHLIB_PATH)=[^\s;|&()]+\s+(?:\/[^\s;|&()]+\/)?env(?:\s|$)/.test(
			command,
		)
	)
		return true;
	if (
		/\bcommand\s+export\s+[^;\n]*(?:LD_[A-Z0-9_]+|DYLD_[A-Z0-9_]+|LIBPATH|SHLIB_PATH)\b/.test(
			command,
		)
	)
		return true;
	...
	if (
		...
		return true;
	for (const words of splitShellCommandSegments(
		stripHeredocBodiesForCommandScan(command),
	).map(tokenizeConductorShellWords)) {
		for (const word of words) {
	...
	const resolveName = (name: string): string | null =>
		aliases.has(name) ? (aliases.get(name) ?? null) : name;
	const assign = (
		name: string,
		value: string | undefined,
		destinationValues = values,
		destinationExported = exported,
		forceExport = false,
	): boolean => {
	...
	const environmentIsConfigured = (
		candidateValues: ReadonlyMap<string, string | undefined>,
		candidateExported: ReadonlySet<string>,
	): boolean =>
		[...candidateExported].some(
			(name) =>
				isConductorDynamicLoaderEnvironmentName(name) &&
				safeString(candidateValues.get(name)).trim() !== "",
		);
	const shellBuiltins = new Set([
		":",
		"declare",
	...
	for (const segment of splitShellCommandSegments(
		...
		if (
			words.some(
				(word) =>
					/[$`]/.test(word) &&
					/(?:LD_|DYLD_|LIBPATH|SHLIB_PATH|loader)/.test(word),
			)
		)
			return true;
		let commandIndex = 0;
		...
		if (!commandName) {
			for (const assignment of prefixAssignments)
				if (assign(assignment.name, assignment.value)) return true;
			continue;
		}
		...
		const declaration =
			...
			commandName === "typeset" ||
			commandName === "local";
		const declaresNameref =
			declaration && operands.some((operand) => /^-[A-Za-z]*n/.test(operand));
		const declaresExport =
			(declaration &&
				operands.some((operand) => /^-[A-Za-z]*x/.test(operand))) ||
			commandName === "export";
		if (declaresNameref) {
			for (const operand of operands.filter(
				...
				if (!assignment || !/^[A-Za-z_][A-Za-z0-9_]*$/.test(assignment.value))
					aliases.set(assignment?.name ?? operand, null);
		...
		if (commandName === "unset") {
			const removesNameref = operands.some(
				(operand) => operand === "-n" || /^-[A-Za-z]*n/.test(operand),
			);
			for (const operand of operands.filter(
		...
		if (commandName === "export" || commandName === "readonly" || declaration) {
			const unexports =
				commandName === "export" &&
				operands.some((operand) => /^-.*n/.test(operand));
			for (const operand of operands.filter(
				(operand) => !/^[-+]/.test(operand),
			)) {
				const assignment = parseShellAssignmentWord(operand);
		...
		if (commandName === "env") {
			...
			if (clears) {
				...
				for (let index = commandIndex + 1; index < cursor; index += 1) {
					const assignment = parseShellAssignmentWord(
						shellWordLiteral(words[index] ?? ""),
					);
					if (
						assignment &&
		...
		if (
			!shellBuiltins.has(executedCommandName) &&
			environmentIsConfigured(executionValues, executionExported)
		)
			return true;
	}
	const functionBodies = extractInvokedShellFunctionBodiesForStateScan(command);
	...
	if (depth >= CONDUCTOR_BASH_MAX_NESTING_DEPTH) return true;
	return functionBodies.some((body) =>
		commandHasUnsafeDynamicLoaderEnvironment(body, depth + 1),
	);
}
...
function ghCommandPath(
	...
	for (
		let index = commandIndex + 1;
		index < words.length && operands.length < 2;
		index += 1
	) {
		const word = words[index] ?? "";
		if (!word || isShellCommandSeparator(word)) break;
...
function ghInvocationHasUnsafeHelperEnvironment(
	words: string[],
	commandIndex: number,
): boolean {
	...
	for (const rawWord of words.slice(
		clearBoundary === null ? commandStartIndex : clearBoundary + 1,
		commandIndex,
	)) {
		const assignment = parseShellAssignmentWord(rawWord);
		if (
			assignment &&
			CONDUCTOR_GH_HELPER_ENVIRONMENT_NAMES.has(assignment.name)
		)
			return true;
	}
	return false;
...
function parseConductorStaticGhApiInvocation(
	words: string[],
	commandIndex: number,
): ConductorStaticGhApiInvocation | null {
	const args = collectConductorInvocationWords(words, commandIndex);
	...
	const applyOption = (name: string, value: string): boolean => {
		if (name === "--method" || name === "X") {
			...
			if (
				!new Set(["GET", "POST", "PUT", "PATCH", "DELETE"]).has(
					normalizedMethod,
				)
			)
				return false;
		...
		if (name === "H") {
			const override =
				/^x-http-method-override\s*:\s*(GET|POST|PUT|PATCH|DELETE)\s*$/i.exec(
					value,
				);
	...
	for (let index = 1; index < args.length; index += 1) {
		const rawWord = args[index] ?? "";
		const word = shellWordLiteral(rawWord);
		if (!word || word === "--" || ghStaticValue(rawWord, true) === null)
			return null;
		if (word.startsWith("--")) {
	...
	return endpoint
		? { mutationIntent: method !== "GET" || hasRequestBody }
		: null;
}

function ghApiHasStaticRemoteEndpoint(
	words: string[],
	commandIndex: number,
): boolean {
...
function ghIssueCreateHasStaticRemoteArguments(
	words: string[],
	commandIndex: number,
): boolean {
	const args = collectConductorInvocationWords(words, commandIndex);
	if (
		shellWordLiteral(args[0] ?? "") !== "issue" ||
		shellWordLiteral(args[1] ?? "") !== "create"
	)
		return false;
	...
	return (
		values.has("--title") && (values.has("--body") || values.has("--body-file"))
	);
}

function isPositivelyClassifiedGhCommand(
	words: string[],
	commandIndex: number,
): boolean {
	if (ghInvocationHasUnsafeHelperEnvironment(words, commandIndex)) return false;
	return (
		ghApiHasStaticRemoteEndpoint(words, commandIndex) ||
		ghIssueCreateHasStaticRemoteArguments(words, commandIndex)
	);
}

function ghCommandUsesOnlyRemoteOptions(
	words: string[],
	commandIndex: number,
): boolean {
...
function ghCommandHasMutationIntent(
	words: string[],
	commandIndex: number,
): boolean {
	...
	if (command === "api")
		return (
			parseConductorStaticGhApiInvocation(words, commandIndex)
				?.mutationIntent ?? true
		);
...
function collectShellCliInvocations(words: string[]): ShellCliInvocation[] {
	...
	const commandStarts = [
		...
		...words.flatMap((word, index) =>
			isShellCommandSeparator(word) ? [index + 1] : [],
		),
	];
	for (const startIndex of commandStarts) {
		...
		if (commandIndex >= 0 && commandIndex < words.length)
			invocations.push({ commandIndex, argumentProducingWrapper });
	}
	return invocations;
}

function commandHasGhMutationIntent(command: string): boolean {
	return splitShellCommandSegments(
		stripHeredocBodiesForCommandScan(command),
	).some((segment) => {
		const words = tokenizeShellWords(segment);
		return collectShellCliInvocations(words).some(
			({ commandIndex, argumentProducingWrapper }) =>
				commandNameFromShellWord(words[commandIndex] ?? "") === "gh" &&
				(argumentProducingWrapper ||
					ghCommandHasMutationIntent(words, commandIndex)),
		);
	});
}

function omxCliInvocationHasMutationIntent(
	words: string[],
	commandIndex: number,
): boolean {
	...
	const boundedWords =
		separatorIndex >= 0
			? invocationWords.slice(0, separatorIndex)
			: invocationWords;
	if (
		boundedWords.some(
			(word) =>
	...
	if (
		!commandName ||
		["help", "read", "status", "version"].includes(commandName)
	)
		return false;
	if (commandName === "state" && ["read", "status"].includes(subcommand))
		return false;
	if (
		["deep-interview", "ralplan", "ralph", "team", "ultragoal"].includes(
			commandName,
		) &&
		["read", "status"].includes(subcommand)
	)
		return false;
	return true;
}
		...
		return collectShellCliInvocations(words).some(
			({ commandIndex, argumentProducingWrapper }) => {
				...
				return (
					(commandName === "omx" || commandName === "gjc") &&
					(argumentProducingWrapper ||
						omxCliInvocationHasMutationIntent(words, commandIndex))
				);
			},
		);
	});
...
function commandHasNestedCliMutationIntent(
	command: string,
	depth = 0,
): boolean {
	const normalizedCommand = stripHeredocBodiesForCommandScan(
	...
	if (
		commandHasGhMutationIntent(normalizedCommand) ||
		commandHasOmxCliMutationIntent(normalizedCommand)
	)
		return true;
	...
	return nestedCommands.some((nestedCommand) =>
		commandHasNestedCliMutationIntent(nestedCommand, depth + 1),
	);
}

function isPositivelyClassifiedUniqCommand(
	words: string[],
	commandIndex: number,
	...
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (!optionsTerminated && word.startsWith("-")) {
			if (
				...
				continue;
			if (
				/^-(?:f|s|w)[1-9][0-9]*$/.test(word) ||
				/^(?:--skip-fields|--skip-chars|--check-chars)=[1-9][0-9]*$/.test(word)
			)
				continue;
			if (
				new Set([
...
function isPositivelyClassifiedRgCommand(
	words: string[],
	commandIndex: number,
): boolean {
	...
	if (
		words
			...
			.some((word) => shellAssignmentName(word) === "RIPGREP_CONFIG_PATH")
	)
		return false;
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (!word || isDynamicNestedCommandString(word) || /[$`]/.test(raw))
			return false;
		if (!optionsTerminated && word === "--") {
			optionsTerminated = true;
			continue;
		...
		if (word.startsWith("--")) {
			...
			const value = word.startsWith(`${name}=`)
				? word.slice(name.length + 1)
				: shellWordLiteral(args[index + 1] ?? "");
			if (!value || isDynamicNestedCommandString(value) || /[$`]/.test(value))
				return false;
		...
		if (
			/^-[A-Za-z]+$/.test(word) &&
			[...word.slice(1)].every((option) => safeShortFlags.has(option))
		)
			continue;
	...
	return (
		!existsSync(join(process.cwd(), ".ripgreprc")) ||
		args.some((word) => shellWordLiteral(word) === "--no-config")
	);
}

function isPositivelyClassifiedSortCommand(
	words: string[],
	commandIndex: number,
): boolean {
	...
	const safeShortOptions = new Set([
		"b",
		"d",
		"f",
		"g",
	...
	for (const rawWord of collectConductorInvocationWords(words, commandIndex)) {
		const word = shellWordLiteral(rawWord);
		if (!word || isDynamicNestedCommandString(word) || /[$`]/.test(word))
			return false;
		if (word === "--") continue;
		if (word.startsWith("--")) {
		...
		if (
			word.startsWith("-") &&
			word !== "-" &&
			![...word.slice(1)].every((option) => safeShortOptions.has(option))
		)
			return false;
	}
	return true;
...
function isPositivelyClassifiedSedCommand(
	words: string[],
	commandIndex: number,
): boolean {
	...
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (
			word.startsWith("--") &&
			!new Set(["--in-place", "--expression", "--file"]).has(
				word.split("=", 1)[0] ?? "",
			)
		)
			return false;
	...
	return (
		scripts.length > 0 &&
		(!sawInPlace || scripts.length === 1) &&
		scripts.every((script) => sedScriptIsPositivelyReadOnly(script))
	);
}
...
function commandConfiguresNodeOptions(command: string, depth = 0): boolean {
	for (const segment of splitShellCommandSegments(
		...
		let leadingIndex = 0;
		while (
			leadingIndex < words.length &&
			(isEnvironmentAssignmentWord(words[leadingIndex] ?? "") ||
				CONDUCTOR_BASH_COMPOUND_SYNTAX_WORDS.has(words[leadingIndex] ?? ""))
		) {
			const word = words[leadingIndex] ?? "";
			if (
				isEnvironmentAssignmentWord(word) &&
				shellAssignmentName(word) === "NODE_OPTIONS"
			)
				return true;
			leadingIndex += 1;
		}
		...
		while (commandIndex >= 0) {
			...
			const wrappedIndex = findConductorWrapperOperandIndex(
		...
		if (words.slice(commandIndex + 1).some((word) => /[$`]/.test(word)))
			return true;
		for (let index = commandIndex + 1; index < words.length; index += 1) {
			const word = words[index] ?? "";
			if (
	...
	if (functionBodies.length > 0) {
		if (depth >= CONDUCTOR_BASH_MAX_NESTING_DEPTH) return true;
		if (
			functionBodies.some((body) =>
				commandConfiguresNodeOptions(body, depth + 1),
			)
		)
			return true;
	}
...
function nodeCommandHasPreloadExecution(
	words: string[],
	commandIndex: number,
): boolean {
	const preloadPattern =
		/(?:^|\s)(?:-r|--require|--import|--loader|--experimental-loader)(?:\s|=|$)/;
	...
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (
			/^(?:-r.+|--(?:require|import|loader|experimental-loader)=.+)$/.test(word)
		)
			return true;
	}
	return false;
...
function gitCommandHasUnsafeRuntimeEnvironment(
	words: string[],
	commandIndex: number,
): boolean {
	...
	return words.slice(0, commandIndex).some((word) => {
		const assignment = parseShellAssignmentWord(word);
		return (
			assignment !== null && gitRuntimeEnvironmentIsUnsafe(assignment.name)
		);
	});
}
...
function conductorRuntimeEnvironmentNameIsSensitive(name: string): boolean {
	return (
		...
		new Set([
			...
			"ENV",
			"ZDOTDIR",
			...CONDUCTOR_GH_HELPER_ENVIRONMENT_NAMES,
		]).has(name) ||
		/^RSYNC_[A-Z0-9_]+$/.test(name) ||
		isConductorDynamicLoaderEnvironmentName(name) ||
	...
	for (const segment of splitShellCommandSegments(
		...
		for (let index = 0; index < words.length; index += 1) {
			...
			if (commandName === "printf") {
				...
				const valueIndex = operands.findIndex(
					(operand) => shellWordLiteral(operand) === "-v",
				);
				if (valueIndex >= 0 && isSensitiveTarget(operands[valueIndex + 1]))
					return true;
				if (
					operands.some((operand) => /^-v[^-]/.test(shellWordLiteral(operand)))
				)
					return true;
...
function nodeCommandHasUnsafeRuntimeOutput(
	...
	if (
		...
		(persistentOutputEnvironmentConfigured ||
			...
			[...CONDUCTOR_NODE_OUTPUT_ENVIRONMENT_NAMES].some(
				(name) => safeString(process.env[name]).trim() !== "",
			))
	)
		return true;
	for (const word of words.slice(
		...
		if (
			CONDUCTOR_NODE_OUTPUT_ENVIRONMENT_NAMES.has(assignment.name) ||
			assignment.name === "NODE_OPTIONS"
		)
			return true;
	}
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (
			...
			const value = words[index + 1] ?? "";
			if (!value || isShellCommandSeparator(value) || /[$`]/.test(value))
				return true;
			index += 1;
			continue;
		}
		if (
			/^(?:-e|-p).+/.test(word) ||
			/^(?:--(?:eval|print|input-type|conditions)=).+/.test(word)
		)
			continue;
...
function collectConductorCoprocCompoundBodies(
	command: string,
): ConductorCoprocCompoundBody[] {
	...
	for (let index = 0; index < command.length; index += 1) {
		...
		if (quote || command.slice(index, index + "coproc".length) !== "coproc")
			continue;
		if (
			/[_A-Za-z0-9]/.test(command[index - 1] ?? "") ||
			/[_A-Za-z0-9]/.test(command[index + "coproc".length] ?? "")
		)
			continue;
		...
		bodies.push({
			...
			body: command.slice(cursor + 1, end),
		});
		index = end;
	}
	return bodies;
...
function stripConductorCoprocCompoundBodiesForRuntimeInspection(
	command: string,
): { command: string; bodies: string[] } {
	...
	return {
		command: `${stripped}${command.slice(cursor)}`,
		bodies: regions.map((region) => region.body),
	};
}

function nestedShellHasUnsafeStartup(
	words: string[],
	commandIndex: number,
	...
	for (let index = commandStartIndex; index < commandIndex; index += 1) {
		...
		if (wrapper === "exec" && (option === "-l" || /^-[^-]*l/.test(option)))
			login = true;
		const argv0Value =
			wrapper === "exec"
				? option === "-a"
					? shellWordLiteral(words[index + 2] ?? "")
					: /^-a(.+)$/.exec(option)?.[1]
				: wrapper === "env"
					? option === "-a" || option === "--argv0"
						? shellWordLiteral(words[index + 2] ?? "")
						: (/^(?:-a|--argv0)=(.+)$/.exec(option)?.[1] ??
							/^-a(.+)$/.exec(option)?.[1])
					: undefined;
...
function mergeConductorRuntimeExecutionInspection(
	...
	target.uninspectedPackageScriptRuntimeCount +=
		source.uninspectedPackageScriptRuntimeCount;
...
function isConductorShellFunctionDefinitionInvocation(
	words: string[],
	commandIndex: number,
): boolean {
	return (
		...
		words[commandIndex + 3] === "{"
	);
}

function staticLastpipeShoptSetting(
	words: string[],
	commandIndex: number,
): boolean | null {
	const operands = collectConductorInvocationWords(words, commandIndex).map(
		shellWordLiteral,
	);
...
function inspectConductorRuntimeExecutions(
	command: string,
	cwd?: string,
	depth = 0,
	inheritedShellFunctions: ReadonlyMap<string, string[]> = new Map(),
): ConductorRuntimeExecutionInspection {
	...
	const coprocCompounds =
		stripConductorCoprocCompoundBodiesForRuntimeInspection(command);
	const topLevelCommand = coprocCompounds.command;
	const unsafeDynamicLoaderEnvironment =
		commandHasUnsafeDynamicLoaderEnvironment(topLevelCommand);
	...
	for (const body of coprocCompounds.bodies) {
		if (depth >= CONDUCTOR_BASH_MAX_NESTING_DEPTH) {
			...
			mergeConductorRuntimeExecutionInspection(
				inspection,
				inspectConductorRuntimeExecutions(
					body,
					cwd,
					depth + 1,
					new Map(functionBindings),
				),
			);
		}
	}
	...
	const commandSetsPerlStartup =
		inheritedRuntimeEnvironmentConfigured(perlStartupNames) ||
		commandConfiguresRuntimeEnvironment(topLevelCommand, perlStartupNames);
	const shellStartupNames = new Set(["BASH_ENV", "ENV", "ZDOTDIR"]);
	const commandSetsShellStartup =
		inheritedRuntimeEnvironmentConfigured(shellStartupNames) ||
		commandConfiguresRuntimeEnvironment(topLevelCommand, shellStartupNames);
	const gitHelperNames = new Set(["GIT_EXTERNAL_DIFF"]);
	const commandSetsGitHelper =
		inheritedRuntimeEnvironmentConfigured(gitHelperNames) ||
		commandConfiguresRuntimeEnvironment(topLevelCommand, gitHelperNames);
	...
	for (const segment of [stripHeredocBodiesForCommandScan(topLevelCommand)]) {
		const segmentHeredocCount = segment
			.split("\n")
		...
		for (let index = 0; index < words.length; index += 1) {
			...
			if (isEnvironmentAssignmentWord(word)) {
				...
				if (assignment && isShellCommandSeparatorAt(words, index + 1)) {
					applyConductorAssignment(runtimeShellState, assignment, {
						persistent: true,
					});
				}
				continue;
			...
			while (!visitedWrapperIndexes.has(commandIndex)) {
				visitedWrapperIndexes.add(commandIndex);
				const wrappedCommandName = commandNameFromShellWord(
					words[commandIndex] ?? "",
				);
				if (CONDUCTOR_BASH_EXTERNAL_DISPATCH_WRAPPERS.has(wrappedCommandName))
					usedExternalDispatchWrapper = true;
			...
			if (
				commandIndex >= 0 &&
				!conductorWrapperLayersAreTrusted(
					words,
					index,
					commandIndex,
					runtimeShellState,
			...
			if (
				argumentProducingWrapper &&
				!new Set([":", "echo", "false", "printf", "true"]).has(commandName)
			) {
				inspection.uninspectedOtherRuntimeCount += 1;
			...
			const pathOverride = words
				.slice(index, Math.max(index, commandIndex))
				.some(
					(candidate) =>
						isEnvironmentAssignmentWord(candidate) &&
						shellAssignmentName(candidate) === "PATH",
				);
			const pathCommandUntrusted =
				/[\\/]/.test(commandWord) &&
				!conductorSlashCommandIsTrusted(
					commandWord,
					runtimeShellState,
					runtimeCwd,
				);
			if (commandWord && (pathCommandUntrusted || pathOverride)) {
				inspection.uninspectedOtherRuntimeCount += 1;
				inspection.uninspectedCommandNames.push(
					commandName || "path-dispatched-command",
				);
				commandStart = false;
				continue;
			}
			const invokesDefinedShellFunction =
				!usedExternalDispatchWrapper &&
				/^[A-Za-z_][\w]*$/.test(commandWord) &&
				definedShellFunctionNames.has(commandWord);
			if (
			...
			if (["node", "node.exe", "nodejs", "nodejs.exe"].includes(commandName)) {
				const commandStartIndex = (() => {
					...
					while (
						start > 0 &&
						!isShellCommandSeparatorAt(words, start - 1) &&
						!isShellGroupingSyntaxWord(words[start - 1] ?? "")
					)
						start -= 1;
					return start;
				})();
				...
				if (
					...
					nodeCommandHasUnsafeRuntimeOutput(
						words,
						commandStartIndex,
						commandIndex,
						commandSetsNodeOutputEnvironment,
					)
				)
					inspection.uninspectedNodeRuntimeCount += 1;
				let foundInlineEval = false;
				for (
					let argIndex = commandIndex + 1;
					...
					if (
						...
						if (
							script &&
							!isShellCommandSeparator(script) &&
							!runtimeOptionIsInlineCode(script)
						) {
					...
					if (attachedShortPrefix) {
						inspection.nodeInlineEvalScripts.push(
							arg.slice(attachedShortPrefix.length),
						);
						foundInlineEval = true;
						continue;
					}
				...
				const safePythonOptions = pythonCommandHasOnlySafeOptions(
					words,
					commandIndex,
				);
				const isolatedPythonStartup = pythonCommandUsesIsolatedStartup(
					words,
					commandIndex,
				);
				...
				if (pythonSource === null) {
					const heredocBody =
						segmentHeredocCount === 1
							? heredocBodies[heredocBodyIndex]
							: undefined;
				...
				if (pythonSource === null) {
					...
					const pythonMetadataCwd =
						pythonStartupCwd !== null &&
						isConductorWritableMetadataRuntimeCwd(pythonStartupCwd);
					const pythonCwdStartup =
						pythonStartupCwd === null ||
						pythonCwdHasLoadableStartupCandidate(pythonStartupCwd) ||
						pythonInvocationHomeHasLoadableStartupCandidate(
							words,
							commandIndex,
							isolatedPythonStartup,
						) ||
						(pythonMetadataCwd && !isolatedPythonStartup);
					const reviewedReadOnlyInlineSource =
						inlineSource &&
						pythonCommandHasLiteralReviewedReadOnlyInlineSource(
							words,
							commandIndex,
						);
					const modeledMetadataWrite =
						pythonStartupCwd !== null &&
						isPositivelyModeledPythonMetadataWriteSource(
							pythonSource,
							pythonStartupCwd,
						);
					if (
				...
				if (
					!isPositivelyClassifiedUniqCommand(
						words,
						commandIndex,
						stateHasPosixlyCorrect(runtimeShellState) ||
							commandHasPosixlyCorrectPrefix(words, commandIndex),
					)
				) {
					inspection.uninspectedOtherRuntimeCount += 1;
					inspection.uninspectedCommandNames.push(commandName);
				...
				if (
					commandSetsPerlStartup ||
					!isPositivelyClassifiedPerlCommand(words, commandIndex)
				) {
					inspection.uninspectedOtherRuntimeCount += 1;
					inspection.uninspectedCommandNames.push(commandName);
				}
				...
				const nestedIndex = findShellCommandStringArgIndex(
					words,
					commandIndex + 1,
				);
				if (
					commandSetsShellStartup ||
					nestedShellHasUnsafeStartup(words, commandIndex, index) ||
					(nestedIndex === null &&
						firstInterpreterScriptOperands(words, commandIndex).length === 0)
				) {
					inspection.uninspectedOtherRuntimeCount += 1;
					inspection.uninspectedCommandNames.push(commandName);
				}
...
function extractNodeFsSemanticMutations(
	command: string,
	...
	for (const script of extractNodeInlineEvalScripts(command)) {
		...
		if (
			/\b(?:__proto__|_load)\b|\bmodule\s*\.\s*constructor\b|\bModule\s*\.\s*_load\b|\b(?:globalThis\s*\.\s*)?process\s*\.\s*(?:binding|dlopen)\b/.test(
				mask,
			)
		)
			pushAmbiguous();
		for (const match of mask.matchAll(
			/\bObject\s*\.\s*getOwnPropertyDescriptor\s*\(/g,
		)) {
			...
			if (
				(receiver === "process" || receiver === "globalThis.process") &&
		...
		for (const match of mask.matchAll(/\[/g)) {
			...
			if (!/[A-Za-z0-9_$)\]]\s*(?:\?\.)?\s*$/.test(mask.slice(0, openIndex)))
				continue;
			const closeIndex = findMatchingJavaScriptDelimiter(
				mask,
				openIndex,
				"[",
				"]",
			...
			const computedMember = parseStaticJavaScriptString(
				script.slice(openIndex + 1, closeIndex),
			);
			const receiver =
				/([A-Za-z_$][\w$]*)\s*$/.exec(mask.slice(0, openIndex))?.[1] ?? "";
		...
		for (const match of mask.matchAll(
			/\b(?:module|(?:globalThis\s*\.\s*)?process(?:\s*\.\s*mainModule)?)\s*\[/g,
		)) {
			const openIndex = (match.index ?? 0) + match[0].lastIndexOf("[");
			const closeIndex = findMatchingJavaScriptDelimiter(
				mask,
		...
		const registerDestructuredBindings = (
			entries: string,
			separator: ":" | "as",
		): void => {
			for (const rawEntry of entries.split(",")) {
				...
				const parts =
					separator === ":" ? entry.split(":") : entry.split(/\s+as\s+/);
				...
				if (
					NODE_FS_MUTATION_METHODS.has(methodName) ||
					NODE_FS_CONDITIONALLY_READ_ONLY_METHODS.has(methodName)
				)
					directFunctionBindings.set(localName, methodName);
				else if (!NODE_FS_READ_ONLY_METHODS.has(methodName)) pushAmbiguous();
			}
		};
		const loaderPattern =
			"(?:process\\.mainModule\\.require|module\\.require|require|import|(?:globalThis\\.)?process\\.getBuiltinModule)";
		const objectBindingPattern = new RegExp(
			`\\b(?:const|let|var)\\s+([A-Za-z_$][\\w$]*)\\s*=\\s*(?:await\\s+)?${loaderPattern}\\s*\\(`,
			"g",
		);
		for (const match of mask.matchAll(objectBindingPattern)) {
			...
			const moduleName = parseStaticJavaScriptString(
				script.slice(openIndex + 1, closeIndex),
			);
			if (isNodeFsModuleName(moduleName)) {
				...
				moduleName === null ||
				isNodeMutationCapableModuleName(moduleName)
			)
				pushAmbiguous();
		}
		const destructuredPattern = new RegExp(
			`\\b(?:const|let|var)\\s*\\{([^}]*)\\}\\s*=\\s*(?:await\\s+)?${loaderPattern}\\s*\\(`,
			"g",
		);
		...
		for (const match of mask.matchAll(
			...
			const moduleLiteral = parseStaticJavaScriptStringAt(
				script,
		...
		for (const match of mask.matchAll(/\bimport\s*\{([^}]*)\}\s*from\b/g)) {
			...
			const moduleName = moduleLiteral?.value ?? null;
			if (isNodeFsModuleName(moduleName))
				registerDestructuredBindings(safeString(match[1]), "as");
			else if (isNodeMutationCapableModuleName(moduleName)) pushAmbiguous();
		}

		const registerAccess = (
			method: string | null,
			callOpenIndex: number | null,
		): void => {
			if (method === null) {
				pushAmbiguous();
				return;
			...
			if (
				NODE_FS_CONDITIONALLY_READ_ONLY_METHODS.has(method) &&
				callOpenIndex !== null
			) {
		...
		const inspectAccessTail = (tailStart: number): boolean => {
			...
			const dotMatch =
				/^\s*(?:\.\s*promises\s*)?\.\s*([A-Za-z_$][\w$]*)(\s*\()?/.exec(tail);
			if (dotMatch) {
				const method = safeString(dotMatch[1]);
				const callOpenIndex = dotMatch[2]
					? tailStart + dotMatch[0].lastIndexOf("(")
					: null;
			...
			const method = parseStaticJavaScriptString(
				script.slice(openIndex + 1, closeIndex),
			);
			const callMatch = /^\s*\(/.exec(mask.slice(closeIndex + 1));
			registerAccess(
				method,
				callMatch ? closeIndex + 1 + callMatch[0].lastIndexOf("(") : null,
			);
			return true;
		};
		...
		for (const match of mask.matchAll(directLoaderPattern)) {
			...
			if (isNodeFsModuleName(moduleName)) {
				...
				if (!handled && !recognizedLoaderCloseIndexes.has(closeIndex))
					pushAmbiguous();
			} else if (
				moduleName === null ||
				isNodeMutationCapableModuleName(moduleName)
		...
		for (const binding of fsBindings) {
			...
			const accessPattern = new RegExp(
				`\\b${escaped}\\s*(?:\\.\\s*promises\\s*)?(?:\\?\\.|\\.|\\[)`,
				"g",
			);
			for (const match of mask.matchAll(accessPattern)) {
				const accessStart = match.index ?? 0;
				inspectAccessTail(
					accessStart + safeString(match[0]).indexOf(binding) + binding.length,
				);
			}
			...
			for (const match of mask.matchAll(referencePattern)) {
				...
				const isDeclaration =
					/(?:\bconst|\blet|\bvar)\s+$/.test(before) && /^\s*=/.test(tail);
				const isStaticImport =
					/\bimport\s+(?:\*\s+as\s+)?$/.test(before) && /^\s+from\b/.test(tail);
				if (isDeclaration || isStaticImport || /^\s*(?:\?\.|\.|\[)/.test(tail))
					continue;
				pushAmbiguous();
			}
		}
		for (const [binding, method] of directFunctionBindings) {
			...
			for (const match of mask.matchAll(referencePattern)) {
				...
				const before = mask.slice(0, referenceIndex);
				const tail = mask.slice(afterBinding);
				const isDestructuredDeclaration =
					/(?:\bconst|\blet|\bvar)\s*\{[^{}]*$/.test(before) &&
					/^[^{}]*(?:,|\})/.test(tail);
				const isStaticImport =
					/\bimport\s*\{[^{}]*$/.test(before) && /^[^{}]*(?:,|\})/.test(tail);
		...
		for (const { method, openIndex } of calls.values()) {
			...
			if (
				...
				isReadOnlyNodeOpenFlags(parseStaticJavaScriptString(args[1] ?? ""))
			)
				continue;
			if (
				method === "createReadStream" &&
				nodeCreateReadStreamHasReadOnlyOptions(script, mask, openIndex)
			)
				continue;
			const targetIndexes = NODE_FS_TWO_TARGET_MUTATION_METHODS.has(method)
				? [0, 1]
				: [0];
			const targets = targetIndexes.map((index) =>
				parseStaticJavaScriptString(args[index] ?? ""),
			);
			writes.push({
				runtime: "node",
...
function isPositivelyReadOnlyNodeInlineEval(script: string): boolean {
	...
	if (
		/\b(?:eval|Function|createRequire|global|globalThis|process|child_process)\b/.test(
			mask,
		)
	)
		return false;
	...
	for (const match of mask.matchAll(/\bReflect\s*\.\s*get\s*\(/g)) {
		const openIndex = (match.index ?? 0) + match[0].lastIndexOf("(");
		const closeIndex = findMatchingJavaScriptParen(mask, openIndex);
		if (closeIndex < 0 || /^\s*\(/.test(mask.slice(closeIndex + 1)))
			return false;
		const args = splitJavaScriptCallArguments(
			script,
			mask,
		...
		if (
			!/^(?:\{|\[)/.test(receiver) ||
			parseStaticJavaScriptString(args[1] ?? "") === null ||
			args.length !== 2
		)
			return false;
	}
	...
	for (const match of mask.matchAll(
		...
		if (mask[targetStart - 1] === ".") {
			...
			const fsMethod = target.startsWith("promises.")
				? target.slice("promises.".length)
				: target;
			const directFsReceiver =
				/\brequire\s*\(\s*(["'])(?:node:)?fs(?:\/promises)?\1\s*\)\s*(?:\.\s*promises\s*)?\.\s*$/.test(
					before,
				);
			const directPathReceiver =
				/\brequire\s*\(\s*(["'])(?:node:)?path\1\s*\)\s*\.\s*$/.test(before);
			const directRegexReceiver = /\/(?:\\.|[^/\n])+\/[A-Za-z]*\s*\.\s*$/.test(
				before,
			);
			if (directFsReceiver && fsMethod === "createReadStream") {
				const openIndex = targetStart + match[0].lastIndexOf("(");
				if (nodeCreateReadStreamHasReadOnlyOptions(script, mask, openIndex))
					continue;
				return false;
			}
			if (
				directFsReceiver &&
				(NODE_FS_READ_ONLY_METHODS.has(fsMethod) ||
					fsMethod === "open" ||
					fsMethod === "openSync")
			)
				continue;
		...
		if (
			memberMatch &&
			(NODE_FS_READ_ONLY_METHODS.has(safeString(memberMatch[1])) ||
				memberMatch[1] === "open" ||
				memberMatch[1] === "openSync")
		)
			continue;
		return false;
	}
...
function maskPythonStringsAndComments(source: string): {
	mask: string;
	valid: boolean;
} {
	const chars = [...source];
...
function isPositivelyModeledPythonSource(source: string): boolean {
	...
	for (const match of lexical.mask.matchAll(/\bopen\s*\(/g)) {
		...
		if (!rawMode) continue;
		const modeExpression = rawMode.startsWith("mode=")
			? rawMode.slice("mode=".length).trim()
			: rawMode;
	...
	for (const match of lexical.mask.matchAll(
		/\b([A-Za-z_][\w]*(?:\s*\.\s*[A-Za-z_][\w]*)*)\s*\(/g,
	)) {
...
function isPositivelyReadOnlyPythonInlineSource(source: string): boolean {
	...
	if (!lexical.valid) return false;
	if (
		/\b(?:from|import|__import__|eval|exec|compile|breakpoint|globals|locals|getattr|setattr|delattr|open|Path|json|shutil)\b/.test(
			lexical.mask,
		)
	)
		return false;
...
function classifyConductorExecutableRuntime(
	...
	const inspection = inspectConductorRuntimeExecutions(
		command,
		runtimeCwd,
		depth,
		inheritedShellFunctions,
	...
	if (
		inspection.nodeInlineEvalScripts.some(
			(script) => !isPositivelyReadOnlyNodeInlineEval(script),
		)
	) {
		return "Bash Node runtime source is not positively classified as read-only";
	}
	if (depth < CONDUCTOR_BASH_MAX_NESTING_DEPTH) {
		const inspectedNestedShellCommands = new Set(
			runtimeFunctionScan.executions.map((nested) => nested.command),
		);
		for (const nested of extractNestedShellCommandStringsForStateScan(
			command,
		)) {
			if (inspectedNestedShellCommands.has(nested)) continue;
			const nestedBlockedDetail = classifyConductorExecutableRuntime(
				nested,
				depth + 1,
				runtimeCwd,
				runtimeFunctionScan.functions,
		...
		for (const nested of extractNestedCommandSubstitutionStringsForStateScan(
			command,
		)) {
			const nestedBlockedDetail = classifyConductorExecutableRuntime(
				nested,
...
function extractConductorInterpreterWrites(
	...
	for (const match of scanCommand.matchAll(
		/\bpython3?\b[\s\S]{0,520}\bos\s*\.\s*(?:remove|unlink|rmdir|removedirs|mkdir|makedirs|chmod|chown|truncate)\s*\(\s*(["'])([^"']+)\1/g,
	)) {
		const target = pythonTarget(match[2]);
		writes.push({ runtime: "python", ...target });
	...
	const pythonOsSingleTargetMutationCalls = hasPythonRuntime
		? [
				...scanCommand.matchAll(
					/\bos\s*\.\s*(?:remove|unlink|rmdir|removedirs|mkdir|makedirs|chmod|chown|truncate)\s*\(/g,
				),
			].length
		: 0;
	if (
...
function conductorMetadataCopySourceIsFiniteRegular(
	cwd: string,
	rawPath: string,
): boolean {
	if (
		!rawPath ||
		isUnresolvedVariableTarget(rawPath) ||
		conductorPathnameExpansionIsAmbiguous(rawPath)
	)
		return false;
	try {
		const entry = lstatSync(
		...
		return (
			...
			entry.nlink === 1 &&
			entry.size <= MAX_CONDUCTOR_METADATA_COPY_BYTES
		);
	} catch {
		return false;
	}
...
function isPositivelyModeledPythonMetadataWriteSource(
	source: string,
	cwd: string,
): boolean {
	const lexical = maskPythonStringsAndComments(source);
		...
		if (statement) {
			if (statement === "import shutil") {
				...
				if (
					openIndex < lineStart ||
					closeIndex < 0 ||
					lexical.mask.slice(closeIndex + 1, lineStart + rawLine.length).trim()
				)
					return false;
				const args = splitJavaScriptCallArguments(
					source,
...
const CONDUCTOR_SHELL_BUILTIN_WRAPPERS = new Set([
	"command",
	"exec",
	"time",
	"coproc",
...
function conductorWrapperLayersAreTrusted(
	...
	let wrapperIndex = skipShellCommandPositionPrefixWords(
		words,
		commandStartIndex,
	);
	...
	while (wrapperIndex < commandIndex) {
		...
		if (
			...
			operandIndex === null ||
			operandIndex <= wrapperIndex
		)
			return false;
		const shellBuiltinWrapper =
			CONDUCTOR_SHELL_BUILTIN_WRAPPERS.has(wrapperName) &&
		...
		if (shellBuiltinWrapper && !requiresExecutableIdentity) {
			...
			const trusted = wrapperWord.includes("/")
				...
				: !conductorCommandPathMayResolveRepositoryExecutable(
						words,
						commandStartIndex,
						wrapperIndex,
						state,
...
function collectConductorTimeOutputTargets(
	words: string[],
	commandStartIndex: number,
	...
	while (wrapperIndex < words.length) {
		...
		if (wrapperName === "time") {
			...
			return invocation.outputTarget === undefined
				? []
				: [invocation.outputTarget];
		}
		const operandIndex = findConductorWrapperOperandIndex(
			wrapperName,
			words,
...
function isConductorDestinationOnlyMutationCommand(
	...
	return (
		commandName === "cp" || commandName === "install" || commandName === "ln"
	);
}
...
function collectConductorSafeReferenceControlTargets(
	...
	for (let index = 0; index < invocationWords.length; index += 1) {
		...
		if (!optionsTerminated && word === "--reference") {
			...
			if (!value || shellWordMayProduceWgetOptions(value) || reference !== null)
				return null;
			reference = value;
			index += 1;
			continue;
	...
	if (targets.some((target) => !isAllowedConductorMetadataPath(cwd, target)))
		return null;
	for (const path of [reference, ...targets]) {
		try {
			const effectivePath = isAbsolute(path)
				? resolve(path)
				: resolve(cwd, path);
...
function conductorMutationUsesUnsafeOption(
	commandName: string,
	words: string[],
	commandIndex: number,
): boolean {
	...
	for (const rawWord of collectConductorInvocationWords(words, commandIndex)) {
		...
		if (
			!permittedShort.has(word) &&
			!(word.startsWith("-t") && word.length > 2)
		)
			return true;
	}
	return false;
...
function shellWordMayProduceWgetOptions(word: string): boolean {
	if (isDynamicNestedCommandString(word)) return true;
	const staticExpansionMarkersRemoved = word.replace(
		/\$CONDUCTOR_(?:ARITH|PARAMETER)_ASSIGN_[A-Za-z_][A-Za-z0-9_]*_(?:SET|APPEND|DYNAMIC)_[A-Za-z0-9_./-]+/g,
		"",
	);
	if (
		/(?:^|[^\\])\$(?:\{?[A-Za-z_][A-Za-z0-9_]*\}?|[0-9@*#?$!_-])/.test(
			staticExpansionMarkersRemoved,
		)
	)
		return true;
...
function parseConductorWgetStartupOptions(
	words: string[],
	commandIndex: number,
): ConductorWgetStartupOptions | null {
	const args = collectConductorInvocationWords(words, commandIndex);
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (
			!word ||
			shellWordMayProduceWgetOptions(word) ||
			word === "--" ||
			!word.startsWith("-")
		)
			break;
...
function wgetStartupOptionIsEffective(
	...
	return (
		options !== null &&
		(option === "--no-config" ? options.noConfig : options.noHsts)
	);
}
...
const CONDUCTOR_WGET_UNSAFE_SHORT_OPTIONS = new Set([
	"r",
	"m",
	"p",
	"k",
...
function wgetInvocationHasUnsafeFiniteTransferMode(
	words: string[],
	commandIndex: number,
): boolean {
...
function downloaderHasUnsafeTlsKeyLogEnvironment(
	...
	let unsafe =
		clearBoundary === null &&
		inherited.exported &&
		(inherited.value === CONDUCTOR_UNKNOWN_SHELL_BINDING ||
			safeString(inherited.value).trim() !== "");
	for (
		let index = clearBoundary === null ? commandStartIndex : clearBoundary + 1;
		index < commandIndex;
		index += 1
	) {
		const word = words[index] ?? "";
		...
		if (commandName === "unset") {
			if (
				collectConductorInvocationWords(words, index).some(
					(operand) => shellWordLiteral(operand) === "SSLKEYLOGFILE",
				)
			)
				unsafe = false;
			continue;
...
function parseConductorWgetExecuteTargets(
	value: string,
):
	| { kind: "output" | "log" | "directory" | "auxiliary"; target: string }[]
	| null {
	...
	if (name === "outputdocument")
		return target ? [{ kind: "output", target }] : null;
	if (name === "dirprefix" || name === "directoryprefix")
		return target ? [{ kind: "directory", target }] : null;
	if (new Set(["logfile", "outputfile", "appendoutput"]).has(name))
		return target ? [{ kind: "log", target }] : null;
	if (
		new Set([
			"savecookies",
			"warcfile",
...
function parseConductorWgetShortOptionCluster(
	...
	| {
			kind: "output" | "log" | "directory" | "execute";
			value: string | null;
			consumeNext: boolean;
	  }
	| "safe"
...
function wgetShortClusterConsumesNextValue(word: string): boolean {
	...
	const valueOptions = new Set("tTUiIXDARwWQBl");
	for (let index = 0; index < options.length; index += 1) {
		if (valueOptions.has(options[index] ?? ""))
...
function curlUrlGlobCaptureValues(url: string): string[][] | null {
	...
	for (let index = 0; index < url.length; index += 1) {
		...
		if (
			values.length === 0 ||
			cardinality >
				Math.floor(CONDUCTOR_CURL_GLOB_CARDINALITY_CEILING / values.length)
		)
			return null;
...
const CONDUCTOR_CURL_SHORT_VALUE_OPTIONS = new Set([
	"A",
	"b",
	"c",
	"C",
...
function collectConductorStaticReadOnlyDownloadUrls(
	...
	for (let index = startIndex; index < endIndex; index += 1) {
		...
		if (!optionsTerminated && commandName === "curl" && word === "--next")
			return null;
		if (!optionsTerminated && word.startsWith("--")) {
			const separator = word.indexOf("=");
			const option = separator < 0 ? word : word.slice(0, separator);
			const longValueOptions =
				commandName === "curl"
					? CONDUCTOR_CURL_LONG_VALUE_OPTIONS
					: CONDUCTOR_WGET_LONG_VALUE_OPTIONS;
			const standaloneOptions =
				commandName === "curl"
					? CONDUCTOR_CURL_LONG_STANDALONE_OPTIONS
					: CONDUCTOR_WGET_LONG_STANDALONE_OPTIONS;
			if (
				commandName === "wget" &&
				(option === "--no-config" || option === "--spider")
			)
				continue;
			if (commandName === "wget" && option === "--input-file") return null;
			if (!longValueOptions.has(option) && !standaloneOptions.has(option))
				return null;
		...
		if (!optionsTerminated && word.startsWith("-")) {
			if (commandName === "wget") {
				...
				const cluster = parseConductorWgetShortOptionCluster(
					word,
					words[index + 1],
				);
	...
	if (
		commandName === "curl" &&
		/[\[\]{}]/.test(url) &&
		curlUrlGlobCaptureValues(url) === null
	)
		return null;
...
function staticConductorDownloadLeaf(
	...
	const url = collectConductorStaticReadOnlyDownloadUrls(
		commandName,
		words,
		startIndex,
		endIndex,
...
function curlTransferEndIndex(words: string[], startIndex: number): number {
	for (let index = startIndex; index < words.length; index += 1) {
		if (
			words[index] === "--next" ||
			isShellCommandTerminatorOrGroupClose(words[index] ?? "")
		)
			return index;
	}
	return words.length;
...
function curlOutputTargetTemplateLeaves(
	...
	if (
		captures === null ||
		placeholders.some((placeholder) => captures.length < placeholder)
	)
		return null;
	const combinations = captures.reduce<string[][]>(
		(product, values) =>
	...
	for (const values of combinations) {
		const leaf = target.replace(
			/#([1-9])/g,
			(_match, rawIndex: string) => values[Number(rawIndex) - 1] ?? "",
		);
...
function curlCommandHasUnsafeShellExpandedWord(command: string): boolean {
	const unescaped = stripHeredocBodiesForCommandScan(command).replace(
		/\\./g,
		"",
	);
	for (const match of unescaped.matchAll(/\bcurl\b([^;\n]*)/g)) {
		const args = match[1] ?? "";
		if (
			/(?:^|\s)(?:-o|--output(?:=|\s+)|--output-dir(?:=|\s+))(?!['"])[^\s]*[*?{\[]/.test(
				args,
			)
		)
			return true;
...
function conductorCurlOutputLeavesAreSafe(
	targets: string[],
	cwd: string,
): boolean {
	return targets.every((target) => {
...
function curlHeaderOverridesRequestMethod(value: string): boolean {
	return /^\s*x-(?:http-)?method(?:-override)?\s*:/i.test(
		shellWordLiteral(value),
	);
}
...
function curlShortOptionClusterContains(
	word: string,
	controls: ReadonlySet<string>,
): boolean {
	return (
		/^-[^-]+$/.test(word) &&
		[...word.slice(1)].some((option) => controls.has(option))
	);
}
...
function curlRequestOptionMutates(word: string): boolean {
	return (
		/^--(?:data(?:-ascii|-binary|-raw|-urlencode)?|form(?:-string)?|json|upload-file|post(?:301|302|303)?)(?:=|$)/.test(
			word,
...
function curlQuoteControlIsUnsafe(word: string): boolean {
	return (
		/^(?:--(?:quote|prequote|postquote)(?:=|$))/.test(word) ||
...
function curlUnknownRequestControlIsUnsafe(word: string): boolean {
	return (
		/^--(?:request|method|http-method)(?:=|-|$)/.test(word) &&
		!/^--request(?:=|$)/.test(word)
	);
}

function curlRemoteHeaderNameIsUnsafe(word: string): boolean {
	return /^--remote-header-name(?:=|$)/.test(word) || /^-[^-]*J/.test(word);
}
...
function curlCommandDisablesStartupConfiguration(
	words: string[],
	commandIndex: number,
): boolean {
	...
	for (let index = 0; index < options.length; index += 1) {
		...
		if (option === "w") {
			...
			return {
				...
				sawSink:
					directTargets === null ||
					directTargets.some((target) => target !== "-"),
			};
		}
		if (valueOptions.has(option))
			return {
				targets: [],
				directTargets: [],
				consumeNext: options.length === index + 1,
				unresolved: false,
				sawSink: false,
			};
		return {
...
function collectConductorDownloaderOutputTargets(
	...
	options: {
		posixlyCorrect?: boolean;
		startupConfigurationUnresolved?: boolean;
		startupIsolationVerified?: boolean;
		cwd?: string;
	} = {},
): {
	sawOutputFlag: boolean;
	targets: string[];
	unresolvedWgetTarget: boolean;
} {
	const targets: string[] = [];
	...
	const applyWgetExecuteTargets = (value: string): void => {
		...
		for (const executeTarget of executeTargets) {
			if (executeTarget.kind === "output")
				wgetOutputDocumentTarget = executeTarget.target;
			else if (executeTarget.kind === "log")
				wgetLogTarget = executeTarget.target;
			else if (executeTarget.kind === "directory")
				wgetDirectoryPrefixTarget = executeTarget.target;
	...
	const curlStartupConfigurationUnresolved =
		commandName === "curl" &&
		!curlCommandDisablesStartupConfiguration(words, commandIndex);
	...
	const finalizeCurlTransfer = (): void => {
		if (
			collectConductorStaticReadOnlyDownloadUrls(
				"curl",
				words,
				curlTransferStartIndex,
				curlTransferEndIndex(words, curlTransferStartIndex),
			) === null
		) {
		...
		if (sawCurlRemoteName) {
			sawOutputFlag = true;
			const leaf = staticConductorDownloadLeaf(
				"curl",
				words,
				curlTransferStartIndex,
				curlTransferEndIndex(words, curlTransferStartIndex),
		...
		if (explicitCurlTargets.length > 0) {
			...
			for (const target of effectiveTargets) {
				const leaves = curlOutputTargetTemplateLeaves(
					target,
					words,
					curlTransferStartIndex,
				);
			...
			if (
				...
				!conductorCurlOutputLeavesAreSafe(expandedTargets, commandCwd)
			) {
				curlOutputTemplateUnresolved = true;
			}
	...
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (commandName === "curl") {
			...
			const curlWriteOut = !curlOptionsTerminated
				? /^--write-out(?:=(.*))?$/.exec(word)
				: null;
			if (curlWriteOut) {
				const value = shellWordLiteral(
					curlWriteOut[1] ?? words[index + 1] ?? "",
				);
				...
				if (expandedTargets.some((target) => target !== "-"))
					sawOutputFlag = true;
			...
			if (!curlOptionsTerminated) {
				...
				if (
					curlQuoteControlIsUnsafe(word) ||
					curlUnknownRequestControlIsUnsafe(word) ||
					curlRequestOptionMutates(word)
				) {
					return { sawOutputFlag: true, targets, unresolvedWgetTarget: true };
				}
				...
				if (longHeader || shortHeader) {
					...
					if (
						!header ||
						shellWordLiteral(header).startsWith("@") ||
						shellWordMayProduceWgetOptions(header) ||
						curlHeaderOverridesRequestMethod(header)
					) {
						return { sawOutputFlag: true, targets, unresolvedWgetTarget: true };
					}
				...
				const shortRequestOffset = /^-[^-]*X/.test(word)
					? word.slice(1).indexOf("X")
					: -1;
				const shortRequest =
					shortRequestOffset >= 0
						? word.slice(shortRequestOffset + 2) || words[index + 1]
						: undefined;
				const request = longRequest
					? (longRequest[1] ?? words[index + 1])
					: shortRequest;
				if (longRequest || shortRequestOffset >= 0) {
					if (
						!request ||
						shellWordMayProduceWgetOptions(request) ||
						!curlRequestMethodIsReadOnly(shellWordLiteral(request))
					) {
						return { sawOutputFlag: true, targets, unresolvedWgetTarget: true };
					}
			...
			const shortCluster = parseConductorCurlShortOptionCluster(
				word,
				words[index + 1],
			);
			if (shortCluster) {
				...
				if (
					shortCluster.unresolved ||
					(shortCluster.consumeNext &&
						(words[index + 1] === undefined ||
							isShellCommandTerminatorOrGroupClose(words[index + 1] ?? "")))
				) {
				...
				for (const target of shortCluster.directTargets) {
					...
					if (
						leaves === null ||
						!conductorCurlOutputLeavesAreSafe(leaves, commandCwd)
					) {
						return { sawOutputFlag, targets, unresolvedWgetTarget: true };
					}
			...
			const curlAuxiliary =
				/^(?:--trace-ascii|--trace|--dump-header|--cookie-jar|--libcurl|--etag-save|--stderr|--alt-svc|--hsts|--ssl-sessions|--write-out-file)(?:=(.*))?$/.exec(
					word,
				);
			if (curlAuxiliary) {
				sawOutputFlag = true;
				const value = curlAuxiliary[1] ?? words[index + 1];
				if (!value || isShellCommandTerminatorOrGroupClose(value))
					return { sawOutputFlag, targets, unresolvedWgetTarget: true };
				pushTarget(value);
			...
			const option = word.split("=", 1)[0] ?? "";
			const standaloneCurlLongOptions = new Set([
				"--create-dirs",
				"--disable",
				"--fail",
				"--get",
			...
			if (
				...
				(!exactCurlLongOptions.has(option) ||
					...
					(!standaloneCurlLongOptions.has(option) &&
						!word.includes("=") &&
						(!words[index + 1] ||
							isShellCommandTerminatorOrGroupClose(words[index + 1] ?? ""))) ||
		...
		if (commandName === "wget" && /^-[^-]*[Vh?]/.test(word))
		...
		if (commandName === "wget" && word.startsWith("--")) {
			...
			const option =
				optionSeparator < 0 ? word : word.slice(0, optionSeparator);
			const inlineValue =
				optionSeparator < 0 ? undefined : word.slice(optionSeparator + 1);
			...
			if (option === "--method") {
				...
				if (
					!value ||
					shellWordMayProduceWgetOptions(value) ||
					!curlRequestMethodIsReadOnly(shellWordLiteral(value))
				) {
					sawUnresolvedWgetOutputTarget = true;
					sawOutputFlag = true;
				}
			...
			if (option === "--header") {
				...
				if (
					!value ||
					shellWordMayProduceWgetOptions(value) ||
					curlHeaderOverridesRequestMethod(value)
				) {
					sawUnresolvedWgetOutputTarget = true;
					sawOutputFlag = true;
				}
		...
		const inlineWgetBodyOutput =
			commandName === "wget" ? word.match(/^--output-document=(.+)$/) : null;
		const inlineWgetLogOutput =
			commandName === "wget"
				? word.match(/^--(?:output-file|append-output)=(.+)$/)
				: null;
		const inlineWgetAuxiliaryOutput =
			commandName === "wget"
				? word.match(
						/^--(?:save-cookies|warc-file|rejected-log|hsts-file)=(.+)$/,
					)
				: null;
		...
		if (commandName === "wget" && word === "-qO") {
			...
			if (
				nextWord === undefined ||
				isShellCommandTerminatorOrGroupClose(nextWord)
			)
				sawUnresolvedWgetOutputTarget = true;
			else {
		...
		if (
			...
			if (word.startsWith("-O"))
				wgetOutputDocumentTarget = word.slice(2).trim() || null;
			else wgetLogTarget = word.slice(2).trim() || null;
			continue;
		}

		const curlOutputFlag =
			commandName === "curl" &&
			(word === "-o" || word === "--output" || word === "--append-output");
		const wgetOutputDocumentFlag =
			commandName === "wget" && (word === "-O" || word === "--output-document");
		const wgetDirectoryPrefixFlag =
			commandName === "wget" &&
			(word === "-P" || word === "--directory-prefix");
		const wgetLogOutputFlag =
			commandName === "wget" &&
			(word === "-o" ||
				word === "-a" ||
				word === "--output-file" ||
				word === "--append-output");
		const wgetAuxiliaryOutputFlag =
			commandName === "wget" &&
			(word === "--save-cookies" ||
				word === "--warc-file" ||
				word === "--rejected-log" ||
		...
		if (
			!curlOutputFlag &&
			!wgetOutputDocumentFlag &&
			!wgetDirectoryPrefixFlag &&
			!wgetLogOutputFlag &&
			!wgetAuxiliaryOutputFlag
		) {
		...
		if (
			...
			if (wgetOutputDocumentFlag)
				wgetOutputDocumentTarget = nextWord.trim() || null;
			else if (wgetDirectoryPrefixFlag)
				wgetDirectoryPrefixTarget = nextWord.trim() || null;
			else if (wgetLogOutputFlag) wgetLogTarget = nextWord.trim() || null;
			else if (wgetAuxiliaryOutputFlag)
				wgetAuxiliaryTargets.push(nextWord.trim());
	...
	let unresolvedWgetTarget =
		commandName === "wget" &&
		(sawUnmodeledWgetOption ||
			sawUnresolvedWgetOutputTarget ||
			options.startupConfigurationUnresolved === true);
	if (commandName === "wget" && !unresolvedWgetTarget) {
		const leaf =
			wgetDirectoryPrefixTarget !== undefined
				? staticConductorDownloadLeaf(
						"wget",
						words,
						commandIndex + 1,
						words.length,
		...
		if (wgetDirectoryPrefixTarget !== undefined) {
			if (wgetDirectoryPrefixTarget === null || !leaf)
		...
		if (wgetOutputDocumentTarget !== undefined) {
			...
			else if (wgetOutputDocumentTarget !== "-")
				targets.push(wgetOutputDocumentTarget);
	...
	if (
		commandName === "wget" &&
		sawWgetStdoutOrSpider &&
		!options.startupIsolationVerified
	)
		unresolvedWgetTarget = true;
	if (
		commandName === "curl" &&
...
function conductorSedInPlaceBackupSuffixIsUnsafe(rawWord: string): boolean {
	...
	if (word.startsWith("--in-place="))
		return word.slice("--in-place=".length) !== "";
...
function conductorSedHasUnsafeOption(
	words: string[],
	commandIndex: number,
): boolean {
	for (const rawWord of collectConductorInvocationWords(words, commandIndex)) {
		...
		if (!word.startsWith("--")) continue;
		const option = word.split("=", 1)[0] ?? "";
		if (!new Set(["--in-place", "--expression", "--file"]).has(option))
			return true;
	}
	return false;
}

function conductorEditorHasInPlaceOption(
	commandName: string,
	words: string[],
	commandIndex: number,
): boolean {
	return collectConductorInvocationWords(words, commandIndex).some(
		(rawWord) => {
			const word = shellWordLiteral(rawWord);
...
const CONDUCTOR_XARGS_SHORT_STANDALONE_OPTIONS = new Set([
	"0",
	"o",
	"p",
	"r",
...
function resolveConductorXargsLongOption(
	option: string,
): "required" | "optional" | "standalone" | null {
	if (CONDUCTOR_XARGS_LONG_OPTIONS_WITH_REQUIRED_VALUES.has(option))
		return "required";
	if (CONDUCTOR_XARGS_LONG_OPTIONS_WITH_OPTIONAL_ATTACHED_VALUES.has(option))
		return "optional";
	...
	if (CONDUCTOR_XARGS_LONG_OPTIONS_WITH_REQUIRED_VALUES.has(match))
		return "required";
	if (CONDUCTOR_XARGS_LONG_OPTIONS_WITH_OPTIONAL_ATTACHED_VALUES.has(match))
		return "optional";
	return "standalone";
}
...
function conductorXargsOptionValueWordCount(
	...
	if (word.startsWith("--")) {
		...
		if (optionKind === "standalone")
			return inlineValue === undefined ? 0 : null;
...
function conductorMutationOptionValueWordCount(
	commandName: string,
	option: string,
): number {
	if (option === "--target-directory" || option === "-t")
		return commandUsesTargetDirectoryOption(commandName) ? 1 : 0;
	if (
		option === "-if" ||
		option === "if" ||
		option === "-of" ||
		...
		return commandName === "dd" ? 1 : 0;
	if (option === "-m" || option === "--mode")
		return commandName === "install" || commandName === "mkdir" ? 1 : 0;
	if (
		option === "-o" ||
		option === "--owner" ||
		option === "-g" ||
		...
		return commandName === "install" || commandName === "chown" ? 1 : 0;
	if (option === "--reference")
		return new Set(["chmod", "chown", "chgrp", "cp", "install", "touch"]).has(
			commandName,
		)
			? 1
			: 0;
	if (option === "--suffix")
		return new Set(["cp", "mv", "ln", "install"]).has(commandName) ? 1 : 0;
	if (option === "-S") return commandName === "ln" ? 1 : 0;
	if (option === "--preserve" || option === "--size")
		return commandName === "install" ? 1 : 0;
	return 0;
}

function conductorLnUsesSymbolicTarget(
	words: string[],
	commandIndex: number,
): boolean {
	let optionsTerminated = false;
...
function conductorCpUsesUnsafeAliasOrPathShapingOption(
	words: string[],
	commandIndex: number,
): boolean {
	let optionsTerminated = false;
...
function conductorHardLinkSourcesAreSafe(
	...
	if (
		!destinations.some((destination) =>
			isAllowedConductorMetadataPath(cwd, destination),
		)
	)
		return true;
...
function collectConductorFiniteDestinationLeaves(
	...
	const staticPath = (path: string): boolean =>
		Boolean(path) && !/[$`\0]/.test(path) && !/[?*{}\[\]]/.test(path);
	const sourceLeaf = (source: string): string | null => {
		if (!staticPath(source)) return null;
		const leaf =
			source.replace(/\\/g, "/").split("/").filter(Boolean).at(-1) ?? "";
		if (!leaf || leaf === "." || leaf === "..") return null;
		try {
			const entry = lstatSync(
				isAbsolute(source) ? resolve(source) : resolve(cwd, source),
			);
			if (entry.isDirectory() || entry.isSymbolicLink() || entry.nlink > 1)
				return null;
		} catch (error) {
	...
	const resolveDirectory = (directory: string): string | null => {
		...
		try {
			const entry = lstatSync(
				isAbsolute(directory) ? resolve(directory) : resolve(cwd, directory),
			);
	...
	if (sawTargetDirectory) {
		if (targetDirectoryTargets.length !== 1 || positionalTargets.length === 0)
			return null;
		const directory = resolveDirectory(targetDirectoryTargets[0] ?? "");
		if (!directory) return null;
		const leaves = positionalTargets.map(sourceLeaf);
		return leaves.some((leaf) => leaf === null)
			? null
			: leaves.map((leaf) => join(directory, leaf ?? ""));
	}
	...
	if (
		sources.some((source) => sourceLeaf(source) === null) ||
		!staticPath(destination)
	)
		return null;
	try {
		const entry = lstatSync(
		...
		if (!entry.isDirectory())
			return sources.length === 1 ? [destination] : null;
...
function collectConductorBoundedDdTargets(
	words: string[],
	commandIndex: number,
	cwd: string,
): string[] | null {
	...
	for (const rawWord of collectConductorInvocationWords(words, commandIndex)) {
		...
		if (
			!value ||
			((name === "if" || name === "of") &&
				conductorPathnameExpansionIsAmbiguous(value))
		)
			return null;
		if (name === "if") {
	...
	if (input === null || output === null || blockSize === null || count === null)
		return null;
	if (
		blockSize > MAX_CONDUCTOR_METADATA_COPY_BYTES ||
		count > Math.floor(MAX_CONDUCTOR_METADATA_COPY_BYTES / blockSize)
	)
		return null;
	return conductorMetadataCopySourceIsFiniteRegular(cwd, input)
		? [output]
		: null;
}

function collectConductorBoundedTruncateTargets(
	words: string[],
	commandIndex: number,
): string[] | null {
	const targets: string[] = [];
	...
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (size !== null || !sizeValue || !/^(?:0|[1-9][0-9]*)$/.test(sizeValue))
			return null;
		const parsedSize = Number(sizeValue);
		if (
			!Number.isSafeInteger(parsedSize) ||
			parsedSize > MAX_CONDUCTOR_METADATA_COPY_BYTES
		)
			return null;
		size = parsedSize;
	}
...
function collectConductorMutationCommandTargets(
	...
	if (commandName === "dd")
		return collectConductorBoundedDdTargets(words, commandIndex, cwd);
	if (commandName === "truncate")
		return collectConductorBoundedTruncateTargets(words, commandIndex);
	if (isConductorReferenceTargetModeCommand(commandName)) {
		return collectConductorSafeReferenceControlTargets(
			commandName,
			words,
			commandIndex,
			cwd,
	...
	if (
		commandName === "cp" &&
		conductorCpUsesUnsafeAliasOrPathShapingOption(words, commandIndex)
	)
		return null;
	if (conductorMutationUsesUnsafeOption(commandName, words, commandIndex))
		return null;
		...
		if (!optionsTerminated && word.startsWith("--")) {
			...
			if (option === "--reference") {
				...
				const referenceInput =
					inlineValue ?? shellWordLiteral(words[index + 1] ?? "");
				if (
					!referenceInput ||
					isShellCommandTerminatorOrGroupClose(referenceInput)
				)
					return null;
				referenceMode = true;
				if (inlineValue === undefined) index += 1;
			...
			if (inlineValue === undefined)
				index += conductorMutationOptionValueWordCount(commandName, option);
	...
	if (commandName === "cp" || commandName === "install") {
		const copySources = sawTargetDirectory
			? positionalTargets
			: positionalTargets.slice(0, -1);
		if (
			copySources.length === 0 ||
			copySources.some(
				(source) => !conductorMetadataCopySourceIsFiniteRegular(cwd, source),
			)
		)
			return null;
	}
	if (new Set(["cp", "install", "ln"]).has(commandName)) {
		if (commandName === "ln") {
			...
			const hardLinkSources = sawTargetDirectory
				? positionalTargets
				: positionalTargets.slice(0, -1);
			if (
				finiteDestinationLeaves === null ||
				!conductorHardLinkSourcesAreSafe(
					hardLinkSources,
					finiteDestinationLeaves,
					cwd,
				)
			)
				return null;
		}
		return finiteDestinationLeaves === null
			? null
			: [...targets, ...finiteDestinationLeaves];
	}
	if (sawTargetDirectory) {
	...
	if (commandName === "ln")
		return [...targets, ...targetDirectoryTargets, ...positionalTargets];
...
function conductorCommandMayCreateFilesystemAlias(
	commandName: string,
	words: string[],
	commandIndex: number,
): boolean {
...
const CONDUCTOR_SHELL_BINDING_NAMES = [
	"POSIXLY_CORRECT",
	"WGETRC",
	"HOME",
	"CDPATH",
...
interface ShellPosixState {
	...
	posixMode: boolean;
	physicalCwd: boolean;
	functionShellOptionSnapshot?: Pick<
		ShellPosixState,
		| "lastpipe"
		| "jobControl"
		| "jobControlMayBeDisabled"
		| "allexport"
		| "posixMode"
		| "physicalCwd"
	>;
...
const CONDUCTOR_SAFE_SHORT_SHELL_OPTION_LETTERS = new Set([
	"a",
	"b",
	"e",
	"f",
...
const CONDUCTOR_NESTED_SHELL_INVOCATION_OPTION_LETTERS = new Set([
	"c",
	"i",
	"l",
	"s",
...
function applyConductorShortShellOptionCluster(
	...
	for (const letter of option.slice(1)) {
		if (
			!CONDUCTOR_SAFE_SHORT_SHELL_OPTION_LETTERS.has(letter) &&
			!(
				allowNestedShellInvocationLetters &&
				CONDUCTOR_NESTED_SHELL_INVOCATION_OPTION_LETTERS.has(letter)
			)
		) {
...
function isConductorSecuritySensitiveEnvironmentName(name: string): boolean {
	return (
		/^RSYNC_[A-Z0-9_]+$/.test(name) || /^(?:OMX|GJC)_[A-Z0-9_]+$/.test(name)
	);
}
...
function inheritedConductorSecurityEnvironment(): Map<string, string> {
	return new Map(
		Object.entries(process.env)
			.filter(
				([name, value]) =>
					isConductorSecuritySensitiveEnvironmentName(name) &&
					safeString(value).trim() !== "",
			)
...
function parseConductorShellOptions(value: string | undefined): {
	known: boolean;
	posix: boolean;
	allexport: boolean;
	physical: boolean;
...
function parseConductorBashOptions(value: string | undefined): {
	known: boolean;
	lastpipe: boolean;
} {
	const options = safeString(value).split(":").filter(Boolean);
...
function inheritedConductorShellOptions(): {
	known: boolean;
	posix: boolean;
	allexport: boolean;
	physical: boolean;
...
function tokenizeConductorShellWords(command: string): string[] {
	...
	return tokenizeShellWords(
		maskShellNonCommandExpansionsForConductorScan(normalized),
	);
}
...
function findConductorBraceGroupEnd(
	words: string[],
...
function isConductorBraceGroupIsolated(
	words: string[],
	openingBraceIndex: number,
...
function isInsideIsolatedConductorBraceGroup(
	words: string[],
	commandStartIndex: number,
): boolean {
	const braces: number[] = [];
	...
	for (const openingBraceIndex of braces) {
		const closingBraceIndex = findConductorBraceGroupEnd(
			words,
			openingBraceIndex,
		);
		if (
			closingBraceIndex === null ||
			isConductorBraceGroupIsolated(words, openingBraceIndex, closingBraceIndex)
		)
			return true;
	}
	return false;
}
function isInvocationIsolated(
	words: string[],
	commandStartIndex: number,
	commandIndex: number,
): boolean {
	...
	if (
		groupingDepth > 0 ||
		isInsideIsolatedConductorBraceGroup(words, commandStartIndex)
	)
		return true;

	if (commandNameFromShellWord(words[commandIndex] ?? "") === "coproc")
		return true;
	for (let index = commandStartIndex; index < words.length; index += 1) {
		const word = words[index] ?? "";
		if (word === "&") return true;
		if (word === ";" || word === "&&" || word === "||") break;
		if ((word === "|" || word === "|&") && !isCasePatternPipe(words, index))
			return true;
	}
...
function isConductorPipelineMember(
	words: string[],
	commandStartIndex: number,
): boolean {
	for (let index = commandStartIndex - 1; index >= 0; index -= 1) {
...
function isConductorFinalPipelineMember(
	words: string[],
	commandStartIndex: number,
): boolean {
	let groupingDepth = 0;
...
function findConductorParentIsolationScope(
	words: string[],
	commandStartIndex: number,
	scope: number,
): number | null {
	...
	for (let index = 0; index <= commandStartIndex; index += 1) {
		if (isShellGroupingOpen(words, index) || words[index] === "{")
			groupingOpens.push(index);
...
function findConductorIsolationScope(
	words: string[],
	commandStartIndex: number,
): number | null {
	if (isConductorPipelineMember(words, commandStartIndex))
	...
	return isInvocationIsolated(words, commandStartIndex, commandStartIndex)
		? commandStartIndex
		: null;
}

function staticConductorIfCondition(
	words: string[],
	ifIndex: number,
	...
	if (
		...
		functions.has(condition)
	)
		return null;
	return condition === "true";
}

function isInsideCertainlySkippedConductorIfBranch(
	words: string[],
	commandStartIndex: number,
	functions: ReadonlyMap<string, string[]>,
): boolean {
	const branches: Array<{
		...
		phase: "condition" | "then" | "else";
	}> = [];
	for (let index = 0; index < commandStartIndex; index += 1) {
		const word = words[index] ?? "";
		if (word === "if") {
			branches.push({
				condition: staticConductorIfCondition(words, index, functions),
				phase: "condition",
			});
			continue;
		}
...
function isConductorCommandCertainlySkipped(
	words: string[],
	commandStartIndex: number,
	functions: ReadonlyMap<string, string[]>,
): boolean {
	if (
		isInsideCertainlySkippedConductorIfBranch(
			words,
			commandStartIndex,
			functions,
		)
	...
	while (
		previousStart >= 0 &&
		!isShellCommandSeparatorAt(words, previousStart) &&
		words[previousStart] !== "(" &&
		words[previousStart] !== "{"
	)
		previousStart -= 1;
	...
	return (
		(separator === "&&" && previousWord === "false") ||
		(separator === "||" && previousWord === "true")
	);
}

function collectConductorInvocationWords(
	words: string[],
	commandIndex: number,
): string[] {
...
const CONDUCTOR_KNOWN_OMX_RUNTIME_ENVIRONMENT_NAMES = new Set([
	...
	"OMX_TEAM_LEADER_CWD",
	"OMX_NOTIFY_HOOK_TRUSTED_MANAGED_CWD",
...
const CONDUCTOR_BENIGN_ORCHESTRATION_RUNTIME_ENVIRONMENT_NAMES = new Set([
	...
	"OMX_OPENCLAW_DEBUG",
	"OMX_TEST_RELAX_TMUX_TIMEOUT",
]);

function conductorOrchestrationRuntimeEnvironmentNameIsPermitted(
	name: string,
): boolean {
	return (
		CONDUCTOR_KNOWN_OMX_RUNTIME_ENVIRONMENT_NAMES.has(name) ||
...
function hasSafeConductorOrchestrationRuntimeEnvironment(
	...
	if (clearBoundary === null) {
		for (const [name, value] of Object.entries(process.env)) {
			if (!/^(?:OMX|GJC)_/.test(name) || safeString(value).trim() === "")
				continue;
			if (!conductorOrchestrationRuntimeEnvironmentNameIsPermitted(name))
				return false;
	...
	for (
		let index = clearBoundary === null ? 0 : clearBoundary + 1;
		index < commandIndex;
		index += 1
	) {
		const word = words[index] ?? "";
		...
		if (assignment && /^(?:OMX|GJC)_/.test(assignment.name)) {
			if (
				!conductorOrchestrationRuntimeEnvironmentNameIsPermitted(
					assignment.name,
				) ||
				assignment.append ||
				/[$`]/.test(assignment.value)
	...
	try {
		...
		if (realpathSync(resolve(rootCwd, rootValue)) !== canonicalRoot)
			return false;
		const expectedStateRoot = realpathSync(
			join(canonicalRoot, ".omx", "state"),
		);
		for (const name of ["OMX_STATE_ROOT", "OMX_TEAM_STATE_ROOT"] as const) {
			const value = environment.get(name);
			if (value && realpathSync(resolve(rootCwd, value)) !== expectedStateRoot)
				return false;
		}
	} catch {
		return false;
...
function hasCanonicalInheritedConductorOrchestrationRoots(
	...
	if (
		nestedExecEnvironmentClearBoundary(
			words,
			commandStartIndex,
			commandIndex,
		) !== null
	...
	try {
		...
		for (const [name, expectedPath] of [
			...
			if (value && realpathSync(resolve(rootCwd, value)) !== expectedPath)
				return false;
		}
	} catch {
		return false;
...
function commandHasUnsafeConductorOrchestrationPrefixEnvironment(
	command: string,
	rootCwd: string,
): boolean {
	...
	return splitShellCommandSegments(command).some((segment) => {
		...
		return collectShellCliInvocations(words).some(({ commandIndex }) => {
			...
			for (let index = 0; index < commandIndex; index += 1) {
				...
				if (
					assignment.name === "OMX_ROOT" &&
					value &&
					resolve(rootCwd, value) !== canonicalRoot
				)
					return true;
				if (
					(assignment.name === "OMX_STATE_ROOT" ||
						assignment.name === "OMX_TEAM_STATE_ROOT") &&
					value &&
...
function conductorStateWritePayloadHasExactSchema(
	payload: Record<string, unknown>,
): boolean {
	for (const [key, value] of Object.entries(payload)) {
		if (key === "state") {
			const nested = safeObject(value);
			if (!nested || !conductorStateWritePayloadHasExactSchema(nested))
				return false;
			continue;
		}
		...
		if (
			key === "mode" &&
			(typeof value !== "string" || !safeString(value).trim())
		)
			return false;
	}
	return Object.keys(payload).length > 0;
}
function isStaticallyValidatedOmxStateWriteInvocation(
	words: string[],
	commandIndex: number,
): boolean {
	const args = readOmxStateCommandArgsFromWords(
		[
			words[commandIndex] ?? "",
			...collectConductorInvocationWords(words, commandIndex),
		],
		"write",
	);
...
function hasExactConductorOrchestrationOptionSchema(
	commandName: string,
	words: string[],
	commandIndex: number,
): boolean {
	...
	for (let index = 2; index < args.length; index += 1) {
		...
		if (
			...
			!option.startsWith("--") ||
			option === "--" ||
			!conductorOrchestrationWordIsStatic(raw)
		)
			return false;
		if (option === "--json") {
		...
		const rawValue = option.startsWith(`${name}=`)
	...
	return (
		status === undefined ||
		new Set(["complete", "blocked", "failed", "in_progress"]).has(status)
	);
}
...
function isStaticallyRecognizedConductorOrchestrationMutation(
	...
	if (
		...
		argumentProducing ||
		(!hasExactConductorOrchestrationOptionSchema(
			commandName,
			words,
			commandIndex,
		) &&
	...
	if (commandName === "gh") {
		return (
			isPositivelyClassifiedGhCommand(words, commandIndex) &&
			!argumentProducing &&
			ghCommandHasMutationIntent(words, commandIndex)
		);
	}
	if (commandName !== "omx" && commandName !== "gjc") return false;
	return hasExactConductorOrchestrationOptionSchema(
		commandName,
		words,
		commandIndex,
	);
...
function findConductorIsolatedInvocationBoundary(
	words: string[],
	commandStartIndex: number,
): number {
	const groupingOpens: number[] = [];
	...
	if (enclosingGroupingOpen !== undefined) {
		let groupingDepth = 1;
		for (
			let index = enclosingGroupingOpen + 1;
			index < words.length;
			index += 1
		) {
			if (isShellGroupingOpen(words, index)) groupingDepth += 1;
...
function collectConductorFunctionBindingsReachableAfter(
	...
	for (const commandStartIndex of collectShellCommandStartIndexes(words)) {
		...
		const commandIndex = skipShellCommandPositionPrefixWords(
			words,
			commandStartIndex,
		);
		const commandWord = words[commandIndex] ?? "";
...
function copyConductorShellBinding(
	binding: ConductorShellBinding,
): ConductorShellBinding {
	return {
		...binding,
...
function cloneConductorShellBinding(
	binding: ConductorShellBinding,
): ConductorShellBinding {
	return {
		...binding,
		dirty: false,
		outer: binding.outer
			? cloneConductorShellBinding(binding.outer)
			: undefined,
	};
}
...
function cloneShellPosixState(state: ShellPosixState): ShellPosixState {
	return {
		bindings: new Map(
			[...state.bindings].map(([name, binding]) => [
				name,
		...
		directoryStack:
			state.directoryStack === null ? null : [...state.directoryStack],
		...
		functionShellOptionSnapshot: state.functionShellOptionSnapshot
...
function replaceConductorShellState(
	target: ShellPosixState,
	source: ShellPosixState,
): void {
	target.bindings = source.bindings;
	target.securityEnvironment = new Map(source.securityEnvironment);
	target.dirtySecurityEnvironmentNames = new Set(
		source.dirtySecurityEnvironmentNames,
	);
	...
	target.directoryStack =
		source.directoryStack === null ? null : [...source.directoryStack];
	...
	target.functionShellOptionSnapshot = source.functionShellOptionSnapshot
		? { ...source.functionShellOptionSnapshot }
		: undefined;
	target.filesystemAliasMayExist = source.filesystemAliasMayExist;
	target.invalidatedStaticDirectories = new Set(
		source.invalidatedStaticDirectories,
	);
	target.pathUsesSystemDefaultWhenUnset = source.pathUsesSystemDefaultWhenUnset;
}

function joinConductorShellStateAlternatives(
	candidates: ShellPosixState[],
): ShellPosixState {
	const cloneForJoin = (candidate: ShellPosixState): ShellPosixState => {
		...
		cloned.bindings = new Map(
			[...candidate.bindings].map(([name, binding]) => [
				name,
...
function isConductorShellBindingName(
	name: string,
): name is ConductorShellBindingName {
	return (CONDUCTOR_SHELL_BINDING_NAMES as readonly string[]).includes(name);
}

function getConductorShellBinding(
	state: ShellPosixState,
...
function setConductorShellBinding(
	...
	if (update.local === true && !previous.local)
		next.outer = copyConductorShellBinding(previous);
	state.bindings.set(name, {
		...next,
		dirty: previous.dirty || (persistent && !previous.local && !update.local),
	});
}

function beginConductorFunctionLocalBinding(
	state: ShellPosixState,
	name: ConductorShellBindingName,
): void {
...
function getConductorGlobalShellBinding(
	binding: ConductorShellBinding,
): ConductorShellBinding {
	return binding.local
		? getConductorGlobalShellBinding(
				binding.outer ?? {
					value: undefined,
					exported: false,
					readonly: false,
...
function commandMayBeConditionallyExecuted(
	words: string[],
	commandStartIndex: number,
): boolean {
	let ifDepth = 0;
...
function joinConductorShellStates(
	baseline: ShellPosixState,
	candidate: ShellPosixState,
): void {
	for (const name of CONDUCTOR_SHELL_BINDING_NAMES) {
		...
		candidate.bindings.set(name, {
			...
			readonlyPossible:
				prior.readonly ||
				next.readonly ||
				prior.readonlyPossible ||
				next.readonlyPossible,
			local: prior.local && next.local,
			dirty: prior.dirty || next.dirty,
		});
	}
	for (const name of candidate.functionLocalBindings) {
		if (!baseline.functionLocalBindings.has(name))
			candidate.functionLocalBindings.delete(name);
	}
	candidate.securityEnvironmentUnresolved ||=
		baseline.securityEnvironmentUnresolved;
	...
	for (const name of new Set([
		...baseline.securityEnvironment.keys(),
		...candidate.securityEnvironment.keys(),
	])) {
	...
	if (
		shellOptionSnapshotKeys.some(
			(key) =>
				baseline.functionShellOptionSnapshot?.[key] !==
				candidate.functionShellOptionSnapshot?.[key],
		)
	) {
	...
	candidate.bashoptsLastpipe =
		baseline.bashoptsLastpipe || candidate.bashoptsLastpipe;
	candidate.bashoptsExported =
		baseline.bashoptsExported || candidate.bashoptsExported;
	candidate.allexport = baseline.allexport || candidate.allexport;
	candidate.shellOptionsKnown =
		baseline.shellOptionsKnown && candidate.shellOptionsKnown;
	...
	candidate.filesystemAliasMayExist =
		baseline.filesystemAliasMayExist || candidate.filesystemAliasMayExist;
	candidate.pathUsesSystemDefaultWhenUnset =
		baseline.pathUsesSystemDefaultWhenUnset &&
		candidate.pathUsesSystemDefaultWhenUnset;
	...
	candidate.jobControlMayBeDisabled =
		baseline.jobControlMayBeDisabled || candidate.jobControlMayBeDisabled;
	candidate.effectiveCwd =
		baseline.effectiveCwd === candidate.effectiveCwd
			? baseline.effectiveCwd
			: null;
	candidate.directoryStack =
		baseline.directoryStack !== null &&
		candidate.directoryStack !== null &&
		baseline.directoryStack.length === candidate.directoryStack.length &&
		baseline.directoryStack.every(
			(directory, index) => directory === candidate.directoryStack?.[index],
		)
			? [...baseline.directoryStack]
			: null;
	const aliasNames = new Set([
		...baseline.aliases.keys(),
		...candidate.aliases.keys(),
	]);
	for (const name of aliasNames) {
		...
		if (
			...
			!baseline.aliases.has(name) ||
			!candidate.aliases.has(name)
		)
			candidate.aliases.set(name, null);
		if (baseline.globalAliases.has(name) && candidate.globalAliases.has(name))
			candidate.globalAliases.add(name);
...
function resolveConductorCommandIndex(
	words: string[],
	startIndex: number,
): ConductorCommandInvocation {
	let commandIndex = skipShellCommandPositionPrefixWords(words, startIndex);
	...
	while (commandIndex < words.length) {
		...
		if (
			...
			childDispatch = true;
		if (commandName !== "time" && commandName !== "coproc")
			functionLookupAllowed = false;
...
function commandPrefixConfiguresPosixlyCorrect(
	words: string[],
	commandStartIndex: number,
	commandIndex: number,
): boolean {
	return words
		.slice(commandStartIndex, commandIndex)
		.some(wordConfiguresPosixlyCorrect);
}

function commandHasPosixlyCorrectPrefix(
	words: string[],
	commandIndex: number,
): boolean {
...
function declarationExportMode(
	commandName: string,
	words: string[],
	commandIndex: number,
): boolean | undefined {
	...
	let exported: boolean | undefined =
		commandName === "export" ? true : undefined;
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		const word = words[index];
		if (word === undefined || isShellCommandTerminatorOrGroupClose(word)) break;
		if (word.startsWith("+")) {
		...
		if (word.startsWith("-")) {
			if (commandName === "export" && word.slice(1).includes("n"))
				exported = false;
...
function declarationReadonlyMode(
	commandName: string,
	words: string[],
	commandIndex: number,
): boolean | undefined {
	...
	let readonly: boolean | undefined =
		commandName === "readonly" ? true : undefined;
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		const word = words[index] ?? "";
...
function resolveConductorShellAssignmentValue(
	...
	if (previous.value === CONDUCTOR_UNKNOWN_SHELL_BINDING)
		return CONDUCTOR_UNKNOWN_SHELL_BINDING;
...
function applyConductorSecurityEnvironmentAssignment(
	...
	const value =
		assignment.append || /[$`]/.test(assignment.value)
			? CONDUCTOR_UNKNOWN_SHELL_BINDING
			: assignment.value;
	...
	if (options.persistent)
		state.dirtySecurityEnvironmentNames.add(assignment.name);
}

function unsetConductorSecurityEnvironment(
	state: ShellPosixState,
	name: string,
	persistent: boolean,
): void {
...
function applyConductorAssignment(
	state: ShellPosixState,
	assignment: { name: string; append: boolean; value: string },
	options: {
		exported?: boolean;
	...
	if (assignment.name === "BASHOPTS") {
		...
		const value = resolveConductorShellAssignmentValue(
			{
				value: current,
				exported: state.bashoptsExported,
				readonly: false,
				local: false,
				dirty: false,
			},
...
function applyCommandPrefixEnvironment(
	words: string[],
	commandStartIndex: number,
	commandIndex: number,
	state: ShellPosixState,
...
function applyStandaloneConductorAssignments(
	words: string[],
	commandStartIndex: number,
	commandIndex: number,
	state: ShellPosixState,
	...
	for (const word of words.slice(commandStartIndex, commandIndex)) {
		...
		applyConductorAssignment(state, assignment, {
			...
			persistent: !getConductorShellBinding(
				state,
				assignment.name as ConductorShellBindingName,
			).local,
		});
		if (!assignment.append && !/[$`]/.test(assignment.value)) {
			(state.staticVariables ??= new Map<string, string>()).set(
				assignment.name,
				assignment.value,
			);
		} else {
...
function classifyConductorStaticDirectory(
	path: string,
): "accessible" | "unknown" {
	try {
...
function isConductorStaticDirectoryInvalidated(
	state: ShellPosixState,
	path: string,
): boolean {
	return [...state.invalidatedStaticDirectories].some(
		(invalidated) => path === invalidated || path.startsWith(`${invalidated}/`),
	);
}

function invalidateConductorStaticDirectoryProofs(
	state: ShellPosixState,
	targets: string[],
	cwd: string,
): void {
	for (const target of targets) {
		...
		try {
			state.invalidatedStaticDirectories.add(
				isAbsolute(target) ? resolve(target) : resolve(cwd, target),
			);
		} catch {
			markConductorCwdUnresolved(state);
...
function classifyConductorStaticDirectoryForState(
	state: ShellPosixState,
	path: string,
): "accessible" | "unknown" {
...
function resolveConductorStaticDirectoryChange(
	...
	const resolveCandidate = (
		...
		if (
			classifyConductorStaticDirectoryForState(state, candidate) !==
			"accessible"
		)
			return null;
	...
	try {
		...
		if (!resolvesThroughCdpath || !cdpath)
...
function applyShellCwdStateEffect(
	words: string[],
	commandIndex: number,
	state: ShellPosixState,
): void {
	...
	const unsafeOption = operands.some(
		(operand) =>
			operand.startsWith("-") &&
			operand !== "-P" &&
			operand !== "-L" &&
	...
	const nonOptionOperands = operands.filter(
		(operand) => !operand.startsWith("-"),
	);
	if (commandName === "pushd" && nonOptionOperands.length === 0) {
		...
		state.directoryStack = [
			state.directoryStack[1] ?? "",
			state.directoryStack[0] ?? "",
			...state.directoryStack.slice(2),
		];
		state.effectiveCwd = state.directoryStack[0] ?? null;
		return;
	}
	...
	const physicalCwd =
		operands.includes("-P") || (!operands.includes("-L") && state.physicalCwd);
	const directoryChange = resolveConductorStaticDirectoryChange(
		state,
		nonOptionOperands[0],
		physicalCwd,
	);
	...
	if (commandName === "pushd") {
		if (
			...
			state.directoryStack === null ||
			state.directoryStack[0] !== state.effectiveCwd ||
			state.directoryStack.length >= 16
		) {
			markConductorCwdUnresolved(state);
			return;
		}
...
function applyShellCwdStateEffectWithPrefix(
	...
	state.directoryStack =
		commandState.directoryStack === null
			? null
			: [...commandState.directoryStack];
}
...
function applyPosixSpecialBuiltinPrefixAssignments(
	...
	if (!state.posixMode || !CONDUCTOR_POSIX_SPECIAL_BUILTINS.has(commandName))
		return;
	for (const word of words.slice(commandStartIndex, commandIndex)) {
		const assignment = parseShellAssignmentWord(word);
...
function applyStaticExpansionStateEffects(
	words: string[],
	commandStartIndex: number,
	state: ShellPosixState,
): void {
	for (let index = commandStartIndex; index < words.length; index += 1) {
		const word = words[index] ?? "";
		if (isShellCommandTerminatorOrGroupClose(word)) break;
		for (const match of word.matchAll(
			/\$CONDUCTOR_(ARITH|PARAMETER)_ASSIGN_([A-Za-z_][A-Za-z0-9_]*)_(SET|APPEND|DYNAMIC)_([A-Za-z0-9_./-]+)/g,
		)) {
			const kind = match[1] ?? "";
			const name = match[2] ?? "";
			...
			const value =
				match[4] === "DYNAMIC"
					...
					: (match[4] ?? "");
			const baseline =
				kind === "PARAMETER" ? cloneShellPosixState(state) : null;
			applyConductorAssignment(
				state,
				{ name, append: mode === "APPEND", value },
				{
					persistent: isConductorShellBindingName(name)
						? !getConductorShellBinding(state, name).local
						: true,
				},
			);
...
function applyConductorTrackedVariableWrite(
	state: ShellPosixState,
	name: string,
	value: string,
): void {
...
function resolveConductorStaticVariableName(
	state: ShellPosixState,
	word: string,
): string | null {
	const literal = shellWordLiteral(word);
	...
	const value = literal.replace(
		...
		(_match, bracedName, bareName) => {
			const staticValue = state.staticVariables?.get(bracedName ?? bareName);
			const bindingValue =
				staticValue ??
				getConductorShellBinding(state, bracedName ?? bareName).value;
			if (
				bindingValue === undefined ||
				bindingValue === CONDUCTOR_UNKNOWN_SHELL_BINDING ||
				/[$`]/.test(bindingValue)
			) {
				unresolved = true;
				return "";
...
function applyConductorTrackedVariableWriter(
	words: string[],
	commandIndex: number,
	state: ShellPosixState,
): void {
	...
	if (commandName === "printf") {
		...
		const value =
			format === "%s" && values.length === 1 && !/[$`]/.test(values[0] ?? "")
				? (values[0] ?? "")
	...
	if (commandName === "read") {
		...
		if (names.some((name) => !isConductorStaticVariableName(name)))
			markConductorTrackedBindingsUnresolved(state);
		else
			for (const name of names)
				applyConductorTrackedVariableWrite(
					state,
					name,
					CONDUCTOR_UNKNOWN_SHELL_BINDING,
				);
	...
	if (commandName === "wait") {
		...
		for (let index = 0; index < operands.length; index += 1) {
			...
			if (optionsTerminated || !operand.startsWith("-") || operand === "-")
				continue;
			if (operand === "-p") {
			...
			if (
				targetName !== null ||
				cluster.indexOf("p", pIndex + 1) >= 0 ||
				!/^[nf]*$/.test(cluster.slice(0, pIndex))
			) {
				markConductorTrackedBindingsUnresolved(state);
				return;
			}
...
function applyConductorLoopVariableWriter(
	words: string[],
	commandIndex: number,
	state: ShellPosixState,
): void {
...
function resolveConductorCommandPathState(
	...
	for (const word of words.slice(
		...
		if (assignment?.name === "PATH") {
			applyConductorAssignment(commandState, assignment, {
				exported: true,
				local: false,
				persistent: true,
			});
...
function conductorValidatedWorkspaceNpmBinDirectory(
	rootCwd: string,
): string | null {
	try {
		const workspaceRoot = realpathSync(resolve(rootCwd));
		const binDirectory = realpathSync(
		...
		if (!lstatSync(omxCandidate).isSymbolicLink() || knownOmxCli === null)
			return null;
...
function conductorWorkspaceNpmBinPathMayResolveRepositoryExecutable(
	...
	if (binDirectory !== conductorValidatedWorkspaceNpmBinDirectory(rootCwd))
		return null;
	const candidate = join(binDirectory, commandName);
	try {
		if (!existsSync(candidate)) return null;
		accessSync(candidate, fsConstants.X_OK);
		return !conductorWorkspacePackageCliCandidateIsTrusted(
			commandName,
			candidate,
			binDirectory,
			rootCwd,
...
function conductorExecutableHasTrustedCurrentNodeRuntimeIdentity(
	commandName: string,
	commandPath: string,
): boolean {
...
function conductorExecutableHasTrustedIdentity(
	...
	if (
		conductorExecutableHasTrustedCurrentNodeRuntimeIdentity(
			commandName,
			commandPath,
		)
	)
		return true;
	if (!conductorExecutableHasTrustedSystemIdentity(commandPath, rootCwd))
		return false;
	if (depth >= CONDUCTOR_BASH_MAX_NESTING_DEPTH) return false;
	...
	seen.add(canonical);
	return conductorTrustedScriptInterpreterIsSafe(
		canonical,
		state,
		rootCwd,
		depth,
...
function conductorExecutableHasTrustedSystemIdentity(
	commandPath: string,
	rootCwd: string,
): boolean {
	try {
...
function conductorResolvePathInterpreter(
	commandName: string,
	state: ShellPosixState,
): string | null {
...
function conductorTrustedScriptInterpreterIsSafe(
	...
	if (shellWordBaseName(interpreter) === "env") {
		if (
			...
			!conductorExecutableHasTrustedSystemIdentity(interpreter, rootCwd)
		)
			return false;
		let environmentWords = shebangWords;
		if (
			environmentWords[0] === "-S" ||
			environmentWords[0] === "--split-string"
		) {
			environmentWords = tokenizeShellWords(
				environmentWords.slice(1).join(" "),
			);
		}
		...
		return (
			targetPath !== null &&
			conductorExecutableHasTrustedIdentity(
				target,
				targetPath,
				rootCwd,
				state,
...
function conductorPathMayResolveRepositoryExecutable(
	...
	for (const entry of path.split(":")) {
		...
		const workspaceNpmBinPathSafety =
			conductorWorkspaceNpmBinPathMayResolveRepositoryExecutable(
				commandName,
				canonical,
				rootCwd,
			);
...
function conductorDeclaredPackageCliPath(
	packageRoot: string,
	commandName: string,
): string | null {
	try {
		const canonicalPackageRoot = realpathSync(packageRoot);
		const packageManifest = safeObject(
			JSON.parse(
				readFileSync(join(canonicalPackageRoot, "package.json"), "utf-8"),
			),
		);
		const packageBin = safeObject(packageManifest.bin);
		const declaredTarget = safeString(
			packageBin[commandName === "gjc" ? "omx" : commandName],
		).trim();
		if (
		...
		if (
			target === canonicalPackageRoot ||
			!target.startsWith(`${canonicalPackageRoot}/`)
		)
			return null;
		const canonicalTarget = realpathSync(target);
		return canonicalTarget === canonicalPackageRoot ||
			!canonicalTarget.startsWith(`${canonicalPackageRoot}/`)
			? null
			: canonicalTarget;
	} catch {
		return null;
...
function conductorWorkspacePackageCliCandidateIsTrusted(
	...
	try {
		const workspaceRoot = realpathSync(resolve(rootCwd));
		if (
			realpathSync(binDirectory) !==
			realpathSync(join(workspaceRoot, "node_modules", ".bin"))
		)
			return false;
...
function conductorPackageCliNodeInterpreterIsTrusted(
	nodeCandidate: string,
	rootCwd: string,
): boolean {
	return (
		conductorExecutableHasTrustedSystemIdentity(nodeCandidate, rootCwd) ||
		conductorExecutableHasTrustedCurrentNodeRuntimeIdentity(
			"node",
			nodeCandidate,
		)
...
function conductorPackageCliHasTrustedNodeInterpreter(
	candidate: string,
	state: ShellPosixState,
	rootCwd: string,
): boolean {
	try {
		const firstLine =
			readFileSync(realpathSync(candidate), "utf-8").split(/\r?\n/, 1)[0] ?? "";
	...
	for (const entry of path.split(":")) {
		...
		try {
			...
			const trustedCli =
				...
				realpathSync(candidate) === expectedCandidate;
			return (
				trustedCli &&
				conductorPackageCliHasTrustedNodeInterpreter(candidate, state, rootCwd)
			);
		} catch {
			return false;
		}
...
function conductorSlashCommandIsTrusted(
	commandWord: string,
	state: ShellPosixState,
	rootCwd: string,
): boolean {
	...
	return conductorExecutableHasTrustedIdentity(
		commandNameFromShellWord(commandWord),
		commandWord,
		rootCwd,
		state,
	);
...
function conductorCommandPathMayResolveRepositoryExecutable(
	...
	return conductorPathMayResolveRepositoryExecutable(
		resolveConductorCommandPathState(
			words,
			commandStartIndex,
			commandIndex,
			state,
...
function applyShellPosixStateEffect(
	words: string[],
	commandIndex: number,
	state: ShellPosixState,
	inFunction: boolean,
	...
	if (
		declarationTargetsFunctions(operands) &&
		(commandName === "readonly" ||
			commandName === "declare" ||
			commandName === "typeset" ||
			commandName === "unset" ||
	...
	if (!inFunction) {
		...
		for (const assignment of assignments) {
			if (!assignment.append && !/[$`]/.test(assignment.value))
				staticVariables.set(assignment.name, assignment.value);
	...
	if (
		...
		[...sensitiveDeclarationNames].some(
			isConductorSecuritySensitiveEnvironmentName,
		)
	)
		state.securityEnvironmentUnresolved = true;
	const declarationQuery = operands.some((word) =>
		/^-[A-Za-z]*[pF]/.test(word),
	);
	...
	const supportsNamerefMode =
		commandName === "declare" ||
		commandName === "typeset" ||
		commandName === "local";
	const namerefMode =
		supportsNamerefMode && operands.some((word) => /^-[A-Za-z]*n/.test(word));
	const clearNamerefMode =
		supportsNamerefMode && operands.some((word) => /^\+[A-Za-z]*n/.test(word));
	if (namerefMode || clearNamerefMode) {
		...
		for (const assignment of assignments) {
			...
			const target = isConductorShellBindingName(assignment.value)
				? assignment.value
				: null;
			if (isConductorSecuritySensitiveEnvironmentName(assignment.value))
				state.securityEnvironmentUnresolved = true;
		...
		for (const name of operands.filter(
			(word) =>
				!word.startsWith("-") &&
				!word.startsWith("+") &&
				!parseShellAssignmentWord(word),
		)) {
			if (clearNamerefMode) {
	...
	for (const assignment of assignments) {
		...
		applyConductorAssignment(state, assignment, {
			exported: exported ?? state.bashoptsExported,
			persistent: true,
		});
	}
		...
		for (const operand of operands.filter((word) => !word.startsWith("-"))) {
			...
			if (binding.readonlyPossible) {
				...
				setConductorShellBinding(
					state,
					targetName,
					{ value: undefined, exported: false },
					!binding.local,
				);
			}
		}
	...
	const localBinding = (name: ConductorShellBindingName): void => {
		if (localDeclaration && inFunction)
			beginConductorFunctionLocalBinding(state, name);
	};
	...
	for (const operand of operands) {
		...
		const name =
			commandName === "export" || commandName === "readonly"
		...
		if (name.startsWith("-") || name.startsWith("+") || declaredNames.has(name))
			continue;
		if (isConductorSecuritySensitiveEnvironmentName(name)) {
			if (!state.securityEnvironment.has(name))
				state.securityEnvironment.set(name, CONDUCTOR_UNKNOWN_SHELL_BINDING);
		...
		if (global) {
			if (readonly !== undefined)
				setGlobalConductorShellBinding(state, name, {
					readonly,
					readonlyPossible: false,
				});
			if (exported !== undefined)
				setGlobalConductorShellBinding(state, name, { exported });
			continue;
		}
		if (readonly !== undefined) {
			setConductorShellBinding(
				state,
				name,
				{ readonly, readonlyPossible: false, local },
				persistent,
			);
		}
		if (exported !== undefined) {
		...
		if (localDeclaration) {
			if (inheritLocal)
				setConductorShellBinding(state, name, { local: true }, false);
			else if (readonly === undefined && exported === undefined)
				setConductorShellBinding(
					state,
					name,
...
function normalizeWgetMutationTargets(
	targets: string[],
	effectiveCwd: string,
	rootCwd: string,
): string[] | null {
	...
	for (const target of targets) {
		if (
			isUnresolvedVariableTarget(target) ||
			/[`$]/.test(target) ||
			conductorPathnameExpansionIsAmbiguous(target)
		)
			return null;
		try {
			const absoluteTarget = isAbsolute(target)
				...
				: resolve(effectiveCwd, target);
			normalized.push(
				relative(rootCwd, absoluteTarget).replace(/\\/g, "/") || ".",
			);
		} catch {
			return null;
		}
...
function normalizeConductorMutationTargets(
	targets: string[],
	effectiveCwd: string,
	rootCwd: string,
): string[] | null {
...
function applyPersistentFunctionEffects(
	caller: ShellPosixState,
	callee: ShellPosixState,
	baseline: ShellPosixState,
): void {
	for (const name of CONDUCTOR_SHELL_BINDING_NAMES) {
		...
		if (callee.functionLocalBindings.has(name)) {
			caller.bindings.set(
				name,
				copyConductorShellBinding(after.outer ?? before),
			);
			continue;
		}
	...
	caller.jobControlMayBeDisabled =
		restoredShellOptions?.jobControlMayBeDisabled ??
		callee.jobControlMayBeDisabled;
	...
	caller.directoryStack =
		callee.directoryStack === null ? null : [...callee.directoryStack];
	...
	for (const name of new Set([
		...
		if (!baseline.aliases.has(name) && !callee.globalAliases.has(name))
			continue;
		if (
			baseline.aliases.get(name) === callee.aliases.get(name) &&
			baseline.aliases.has(name) === callee.aliases.has(name)
		)
			continue;
		if (callee.aliases.has(name))
			caller.aliases.set(name, callee.aliases.get(name) ?? null);
...
function joinConductorFunctionMaps(
	candidates: ReadonlyArray<ReadonlyMap<string, string[]>>,
): Map<string, string[]> {
	...
	for (const name of names) {
		...
		for (const candidate of candidates) {
			for (const body of candidate.get(name) ?? [
				CONDUCTOR_UNBOUND_FUNCTION_BODY,
			])
				bodies.add(body);
		}
...
function replaceConductorFunctionMap(
...
function applyNestedBashShoptOptions(
	words: string[],
	commandIndex: number,
	child: ShellPosixState,
): void {
	...
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (shoptEnabled !== null) {
			if (
				!/^[A-Za-z_][A-Za-z0-9_]*$/.test(shoptName) ||
				shoptName !== "lastpipe"
			) {
				child.shellOptionsKnown = false;
			} else {
...
function createNestedExecChildState(
	...
	applyNestedShellEnvironment(
		words,
		clearBoundary === null ? commandStartIndex : clearBoundary + 1,
		commandIndex,
		child,
	);
	const nestedPosixMode = nestedShellEnablesPosix(
		words,
		commandStartIndex,
		commandIndex,
	);
...
function applyNestedShellEnvironment(
	words: string[],
	commandStartIndex: number,
	commandIndex: number,
	child: ShellPosixState,
	...
	for (let index = commandStartIndex; index < commandIndex; index += 1) {
		...
		if (assignment?.name === "SHELLOPTS") {
			...
			child.shellOptionsKnown &&=
				options.known && !assignment.append && !/[$`]/.test(assignment.value);
		...
		if (assignment) {
			applyConductorAssignment(child, assignment, {
				exported: true,
				local: false,
				persistent: false,
			});
...
function nestedShellEnablesPosix(
	words: string[],
	commandStartIndex: number,
	commandIndex: number,
): boolean | null {
	...
	const effectiveName =
		(argv0 ?? commandNameFromShellWord(words[commandIndex] ?? ""))
			.split(/[\\/]/)
			.pop()
			?.toLowerCase() ?? "";
	if (new Set(["sh", "dash", "ash", "ksh", "mksh"]).has(effectiveName))
		return true;
	...
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (option === "-o" && shellWordLiteral(words[index + 1] ?? "") === "posix")
			return true;
		if (option === "-O" || option === "+O") {
			index += 1;
			continue;
...
function nestedShellUsesLastpipe(
	words: string[],
	commandStartIndex: number,
	commandIndex: number,
	commandStringIndex: number,
	...
	if (commandNameFromShellWord(words[commandIndex] ?? "") !== "bash")
		return false;
	const prefixHasLastpipe = words
		.slice(commandStartIndex, commandIndex)
		.some((word) => {
			const assignment = parseShellAssignmentWord(word);
			return (
				assignment?.name === "BASHOPTS" &&
				(assignment.append ||
					/[$`]/.test(assignment.value) ||
					assignment.value.split(":").includes("lastpipe"))
			);
		});
	return (
		nestedBashLastpipeEnabled(words, commandIndex, commandStringIndex) ||
		prefixHasLastpipe ||
		state.lastpipe ||
		state.bashoptsLastpipe
	);
...
function isConductorFunctionBody(body: string): boolean {
	return (
		body !== CONDUCTOR_UNBOUND_FUNCTION_BODY &&
...
function inheritedConductorFunctionBindings(): Map<string, string[]> {
	...
	for (const [environmentName, value] of Object.entries(process.env)) {
		const name = /^BASH_FUNC_([A-Za-z_][A-Za-z0-9_]*)%%$/.exec(
			environmentName,
		)?.[1];
		if (!name) continue;
		...
		functions.set(
			...
			body && body.length <= 8192 && !/\$\(|[<>]\(/.test(body)
				? [body, CONDUCTOR_EXPORTED_FUNCTION_BODY]
				: [
						CONDUCTOR_UNINSPECTED_FUNCTION_BODY,
						CONDUCTOR_EXPORTED_FUNCTION_BODY,
					],
		);
	}
	return functions;
}
function nestedExecEnvironmentClearBoundary(
	words: string[],
	commandStartIndex: number,
	commandIndex: number,
): number | null {
...
function nestedExecClearsEnvironment(
	words: string[],
	commandStartIndex: number,
	commandIndex: number,
): boolean {
...
function createNestedExecChildFunctions(
	...
	if (
		commandNameFromShellWord(words[commandIndex] ?? "") !== "bash" ||
		nestedExecClearsEnvironment(words, commandStartIndex, commandIndex)
	)
		return new Map();
		...
		const unsetName =
			word === "-u" || word === "--unset"
				...
				: word.startsWith("--unset=")
					? word.slice("--unset=".length)
					: "";
		const functionName = /^BASH_FUNC_([A-Za-z_][A-Za-z0-9_]*)%%$/.exec(
			unsetName,
		)?.[1];
	...
	for (const [name, bodies] of parent) {
		if (
			unsetFunctionNames.has(name) ||
			!bodies.includes(CONDUCTOR_EXPORTED_FUNCTION_BODY)
		)
			continue;
...
function applyShellFunctionBindingEffect(
	words: string[],
	commandIndex: number,
	...
	const commandName = commandNameFromShellWord(words[commandIndex] ?? "");
	if (
		!new Set(["unset", "readonly", "export", "declare", "typeset"]).has(
			commandName,
		)
	)
		return;
	...
	const functionNames = operands.filter(
		(word) => !word.startsWith("-") && !word.startsWith("+"),
	);
	const negated = operands.some(
		(word) => word.startsWith("-") && word.includes("n"),
	);
	const exported =
		commandName === "export" ||
		operands.some((word) => word.startsWith("-") && word.includes("x"));
	const readonly =
		commandName === "readonly" ||
		operands.some((word) => word.startsWith("-") && word.includes("r"));
...
function scanConductorShellSegment(
	...
	const isolatedContexts = new Map<
		number,
		{ state: ShellPosixState; functions: Map<string, string[]> }
	>();
	const resolvingIsolationScopes = new Set<number>();
	const getIsolatedContext = (
		scope: number,
		commandStartIndex: number,
	): { state: ShellPosixState; functions: Map<string, string[]> } => {
		...
		const parentScope = findConductorParentIsolationScope(
			words,
			commandStartIndex,
			scope,
		);
		const parent =
			parentScope === null || parentScope === scope
				? { state, functions }
				: getIsolatedContext(parentScope, commandStartIndex);
		const context = {
			state: cloneShellPosixState(parent.state),
	...
	for (const commandStartIndex of collectShellCommandStartIndexes(words)) {
		...
		if (CONDUCTOR_BASH_COMPOUND_SYNTAX_WORDS.has(commandStartWord)) {
			if (commandStartWord === "for" || commandStartWord === "select")
				applyConductorLoopVariableWriter(words, commandStartIndex, state);
			continue;
		}
		const initialCommandIndex = skipShellCommandPositionPrefixWords(
			words,
			commandStartIndex,
		);
		if (
			initialCommandIndex >= words.length ||
			isShellCommandTerminatorOrGroupClose(words[initialCommandIndex] ?? "")
		) {
			const standaloneIsolated =
				segment.isolated ||
				isInvocationIsolated(words, commandStartIndex, commandStartIndex);
			const standaloneScope = standaloneIsolated
				? findConductorIsolationScope(words, commandStartIndex)
				: null;
			const standaloneContext =
				standaloneScope === null
					? null
					: getIsolatedContext(standaloneScope, commandStartIndex);
			applyStandaloneConductorAssignments(
				words,
				commandStartIndex,
				initialCommandIndex,
				standaloneContext?.state ?? state,
			);
			continue;
		}
		const initialCommandName = commandNameFromShellWord(
			words[initialCommandIndex] ?? "",
		);
		if (
			!initialCommandName ||
			CONDUCTOR_BASH_COMPOUND_SYNTAX_WORDS.has(initialCommandName)
		)
			continue;
		const directFunctionBodies = functions.get(initialCommandName);
		const preliminaryTimeOutputTargets =
			directFunctionBodies === undefined
		...
		const invocation = directFunctionBodies
			? {
					index: initialCommandIndex,
					unresolved: false,
					functionLookupAllowed: true,
					argumentProducing: false,
					childDispatch: false,
				}
		...
		const commandName = commandNameFromShellWord(words[commandIndex] ?? "");
		const initiallyIsolated =
			segment.isolated ||
			isInvocationIsolated(words, commandStartIndex, initialCommandIndex);
		const provisionalScope = initiallyIsolated
			? findConductorIsolationScope(words, commandStartIndex)
			: null;
		const parentScope =
			provisionalScope === null
				? null
				: findConductorParentIsolationScope(
						words,
						commandStartIndex,
						provisionalScope,
					);
		...
		const lastpipeFinalMember =
			optionState.lastpipe &&
			!optionState.jobControl &&
			optionState.jobControlMayBeDisabled &&
			isConductorFinalPipelineMember(words, commandStartIndex);
		const isolated = initiallyIsolated && !lastpipeFinalMember;
		const scope = isolated ? provisionalScope : null;
		const isolatedContext =
			scope === null
				? inheritedContext
				: getIsolatedContext(scope, commandStartIndex);
		const activeState = isolatedContext?.state ?? state;
		const activeFunctions = isolatedContext?.functions ?? functions;
		if (
			isConductorCommandCertainlySkipped(
				words,
				commandStartIndex,
				activeFunctions,
			)
			...
			continue;
		if (
			preliminaryTimeOutputTargets !== undefined &&
			preliminaryTimeOutputTargets.length > 0
		) {
			const timeExecutionContext =
				activeState.effectiveCwd === null
					? null
					: resolveWrappedCommandExecutionContext(
							words,
							activeState.effectiveCwd,
							commandStartIndex,
						);
			...
			const normalizedTimeTargets = normalizeConductorMutationTargets(
				preliminaryTimeOutputTargets,
				timeExecutionContext.cwd,
				rootCwd,
			);
			mutations.push({ command: "time", targets: normalizedTimeTargets ?? [] });
		}
		const functionStateBefore =
			!invocation.childDispatch &&
			commandMayBeConditionallyExecuted(words, commandStartIndex)
				? new Map(activeFunctions)
				: null;
		const variableStateBefore =
			!invocation.childDispatch &&
			commandMayBeConditionallyExecuted(words, commandStartIndex)
		...
		if (!invocation.childDispatch)
			applyConductorTrackedVariableWriter(words, commandIndex, activeState);
		if (variableStateBefore)
			joinConductorShellStates(variableStateBefore, activeState);
		if (!invocation.childDispatch)
			applyShellFunctionBindingEffect(words, commandIndex, activeFunctions);
		const childIsolatedFunctionBindings =
			isolatedFunctionBindings ??
			(isolated
				...
				: null);
		const functionBodies = invocation.functionLookupAllowed
			? activeFunctions.get(commandName)
			: undefined;
		...
		if (functionBodies !== undefined) {
			...
			const hasUnboundAlternative = functionBodies.includes(
				CONDUCTOR_UNBOUND_FUNCTION_BODY,
			);
			if (depth >= CONDUCTOR_BASH_MAX_NESTING_DEPTH) {
				mutations.push({ command: commandName, targets: [] });
			...
			for (const body of functionBodies.filter(isConductorFunctionBody)) {
				...
				scanConductorShellSource(
					...
					functionState,
					functionMap,
					functionState.effectiveCwd ?? cwd,
					true,
					depth + 1,
					mutations,
					childIsolatedFunctionBindings,
					rootCwd,
					true,
					onStaticNestedBashExecution,
				);
				const projectedState = cloneShellPosixState(baselineState);
				applyPersistentFunctionEffects(
					projectedState,
					functionState,
					baselineState,
				);
			...
			if (hasUnboundAlternative) {
				...
				scanConductorShellSegment(
					{ command: words.join(" "), isolated: segment.isolated },
					fallbackState,
					fallbackFunctions,
					childIsolatedFunctionBindings,
					cwd,
			...
			replaceConductorShellState(
				activeState,
				joinConductorShellStateAlternatives(candidateStates),
			);
			replaceConductorFunctionMap(
				activeFunctions,
				joinConductorFunctionMaps(candidateFunctions),
			);
			continue;
		}
		...
		const omxGjcInheritedRootsAreCanonical =
			isOmxGjcCommand &&
			hasCanonicalInheritedConductorOrchestrationRoots(
				words,
				commandStartIndex,
				commandIndex,
				rootCwd,
			);
		const omxGjcPrefixEnvironmentIsSafe =
			!isOmxGjcCommand ||
			!commandHasUnsafeConductorOrchestrationPrefixEnvironment(
				words.join(" "),
				rootCwd,
			);
		if (
			...
			(!omxGjcPrefixEnvironmentIsSafe ||
				!hasSafeConductorOrchestrationRuntimeEnvironment(
					words,
					commandStartIndex,
					commandIndex,
					rootCwd,
		...
		const commandPathMayResolveRepositoryExecutable =
			commandIsBare &&
			(!CONDUCTOR_PATH_INDEPENDENT_BUILTINS.has(commandName) ||
				invocation.childDispatch) &&
			conductorCommandPathMayResolveRepositoryExecutable(
				words,
				commandStartIndex,
		...
		const trustedOmxGjcPackageCliPath =
			isOmxGjcCommand &&
			commandIsBare &&
			conductorCommandResolvesTrustedPackageCli(
				words,
				commandStartIndex,
				commandIndex,
				activeState,
		...
		const bareCommandPathIsSafe =
			commandIsBare && !commandPathMayResolveRepositoryExecutable;
		...
		const omxGjcStateWriteDestinationIsCanonical =
			activeState.effectiveCwd !== null &&
			resolve(activeState.effectiveCwd) === resolve(rootCwd) &&
			!conductorInvocationUsesEnvCwdChangingWrapper(
				words,
				commandStartIndex,
				commandIndex,
			);
		const mainRootStructuredStateWrite =
			omxGjcInheritedRootsAreCanonical &&
			omxGjcPrefixEnvironmentIsSafe &&
		...
		const mainRootStructuredOrchestrationMutation =
			isStaticallyRecognizedConductorOrchestrationMutation(
				commandName,
				words,
				commandIndex,
			...
			(commandName === "omx" || commandName === "gjc"
				? omxGjcInheritedRootsAreCanonical &&
					omxGjcPathIsSafeForStaticOrchestration
				: commandName === "gh"
					? commandIsBare
						? bareCommandPathIsSafe
						: conductorSlashCommandIsTrusted(commandWord, activeState, rootCwd)
					: false);
		const cliMutationIntent =
			commandName === "gh"
				? ghCommandHasMutationIntent(words, commandIndex) ||
					(invocation.argumentProducing &&
						isPositivelyClassifiedGhCommand(words, commandIndex))
		...
		if (isNestedShellCommandWord(commandName)) {
			...
			if (commandStringIndex !== null) {
				...
				if (!nestedCommand || executionContext === null) {
					...
					const childState = createNestedExecChildState(
						activeState,
						words,
						commandStartIndex,
						commandIndex,
					...
					childState.lastpipe = nestedShellUsesLastpipe(
						words,
						commandStartIndex,
						commandIndex,
						commandStringIndex,
					...
					const childFunctions = createNestedExecChildFunctions(
						activeFunctions,
						words,
						commandStartIndex,
						commandIndex,
					...
					activeState.filesystemAliasMayExist ||=
						childState.filesystemAliasMayExist;
					for (const directory of childState.invalidatedStaticDirectories)
						activeState.invalidatedStaticDirectories.add(directory);
				}
				continue;
			}
		}
		...
		const stateBefore =
			...
			commandMayBeConditionallyExecuted(words, commandStartIndex)
				...
				: null;
		if (!invocation.childDispatch) {
			applyPosixSpecialBuiltinPrefixAssignments(
				words,
				commandStartIndex,
				commandIndex,
				commandName,
			...
			if (CONDUCTOR_BASH_MODELED_CURRENT_SHELL_BUILTINS.has(commandName)) {
				applyShellCwdStateEffectWithPrefix(
					words,
					commandStartIndex,
					commandIndex,
					activeState,
		...
		if (functionStateBefore)
			replaceConductorFunctionMap(
				activeFunctions,
				joinConductorFunctionMaps([functionStateBefore, activeFunctions]),
			);
		const executionContext =
			activeState.effectiveCwd === null
				? null
		...
		if (invocation.argumentProducing) {
			if (!CONDUCTOR_PATH_INDEPENDENT_BUILTINS.has(commandName))
				mutations.push({ command: commandName || "xargs", targets: [] });
			continue;
		}
		...
		if (CONDUCTOR_BASH_DOWNLOADER_COMMANDS.has(commandName)) {
			const downloaderTargets = collectConductorDownloaderOutputTargets(
				...
				commandIndex,
				{
					posixlyCorrect:
						commandName === "wget" &&
						(stateHasPosixlyCorrect(activeState) ||
							commandPrefixConfiguresPosixlyCorrect(
								words,
								commandStartIndex,
								commandIndex,
							)),
					startupConfigurationUnresolved:
						commandName === "wget" &&
						wgetStartupConfigurationIsUnresolved(
							words,
							commandStartIndex,
							commandIndex,
							activeState,
					...
					startupIsolationVerified:
						commandName === "wget" &&
						wgetStartupOptionIsEffective(
							words,
							commandIndex,
							"--no-config",
							stateHasPosixlyCorrect(activeState) ||
			...
			const wgetArgvIsDynamic = collectConductorInvocationWords(
				words,
				commandIndex,
			).some(shellWordMayProduceWgetOptions);
			const wgetPosixlyCorrect =
				stateHasPosixlyCorrect(activeState) ||
				commandPrefixConfiguresPosixlyCorrect(
					words,
					commandStartIndex,
			...
			const wgetInvocationWords =
				commandName === "wget"
					? collectConductorInvocationWords(words, commandIndex).map(
							shellWordLiteral,
						)
					: [];
			const wgetIsExplicitNonNetworkInfoMode =
				commandName === "wget" &&
				wgetInvocationWords.some(
					(word) => word === "-V" || word === "--version" || word === "--help",
				) &&
				wgetInvocationWords.every((word) =>
					new Set([
						"--no-config",
						"--no-hsts",
						"-V",
			...
			if (
				...
				downloaderHasUnsafeTlsKeyLogEnvironment(
					words,
					commandStartIndex,
					commandIndex,
					activeState,
			...
			if (commandName === "wget") {
				if (downloaderTargets.unresolvedWgetTarget)
					mutations.push({ command: commandName, targets: [] });
				if (downloaderTargets.targets.length > 0) {
					const targets = normalizeConductorMutationTargets(
						downloaderTargets.targets,
				...
				downloaderTargets.unresolvedWgetTarget ||
				downloaderTargets.sawOutputFlag
			) {
				const targets = downloaderTargets.unresolvedWgetTarget
					? null
					: normalizeConductorMutationTargets(
							downloaderTargets.targets,
							executionContext?.cwd ?? cwd,
							rootCwd,
						);
		...
		if (CONDUCTOR_BASH_MUTATION_COMMANDS.has(commandName)) {
			...
			const rsyncMetadataControl =
				commandName === "rsync" &&
				isConductorSafeRsyncMetadataControl(
					words,
					commandIndex,
					executionContext?.cwd ?? cwd,
					rootCwd,
			...
			if (targets === null) {
				if (
					...
					((commandName === "sed" || commandName === "perl") &&
						conductorEditorHasInPlaceOption(commandName, words, commandIndex))
				)
					mutations.push({ command: commandName, targets: [] });
			} else {
				const normalizedTargets = normalizeConductorMutationTargets(
					targets,
					executionContext?.cwd ?? cwd,
					rootCwd,
				);
				mutations.push({
					...
					nativeChildMetadataControl:
						isNativeChildSafeConductorReferenceControl(
							words,
							commandIndex,
							executionContext?.cwd ?? cwd,
						) ||
						(rsyncMetadataControl &&
							isNativeChildSafeConductorRsyncMetadataControl(
								words,
								commandIndex,
								executionContext?.cwd ?? cwd,
								rootCwd,
				...
				if (
					conductorCommandInvalidatesStaticDirectoryProof(
						commandName,
						words,
						commandIndex,
					)
				) {
					invalidateConductorStaticDirectoryProofs(
						activeState,
						targets,
						executionContext?.cwd ?? cwd,
					);
					if (activeState !== state) {
						for (const directory of activeState.invalidatedStaticDirectories)
							state.invalidatedStaticDirectories.add(directory);
					}
				}
				if (
					conductorCommandMayCreateFilesystemAlias(
						commandName,
						words,
						commandIndex,
					)
		...
		for (const context of isolatedContexts.values()) {
			...
			for (const directory of context.state.invalidatedStaticDirectories)
				state.invalidatedStaticDirectories.add(directory);
		}
	}
}
...
function isConductorIsolatingOperatorAt(
	source: string,
	index: number,
): boolean {
	let cursor = index;
...
function conductorShellCommandStartBefore(
	source: string,
	index: number,
): number {
...
function findConductorIsolatedFunctionDefinitionRegion(
	...
	for (let index = 0; index < definitionStart; index += 1) {
		...
		if (expectedOpenChar && groupings.at(-1)?.openChar === expectedOpenChar)
			groupings.pop();
	}

	for (const grouping of groupings) {
		const groupingEnd = findShellFunctionBodyEnd(
			source,
			grouping.openIndex,
			grouping.openChar,
		);
		...
		const prefix = source.slice(
			conductorShellCommandStartBefore(source, grouping.openIndex),
			grouping.openIndex,
		);
		...
		if (
			grouping.openChar === "(" ||
			coprocGroup ||
			isConductorIsolatingOperatorAt(source, groupingEnd + 1)
		) {
			return {
	...
	const definitionPrefix = source.slice(
		conductorShellCommandStartBefore(source, definitionStart),
		definitionStart,
	);
	if (
		!tokenizeConductorShellWords(definitionPrefix).includes("coproc") &&
		!isConductorIsolatingOperatorAt(source, bodyEnd + 1)
	)
		return null;
	return {
...
function stripProcessSubstitutionBodiesForConductorScan(
	command: string,
): string {
	let result = "";
	let quote: "'" | '"' | null = null;
...
function stripCommandSubstitutionBodiesForConductorScan(
	command: string,
): string {
	let result = "";
	let quote: "'" | '"' | null = null;
...
function collectProcessSubstitutionExpansionPrefix(
	command: string,
...
function isInsideConductorSubstitutionGrouping(
	command: string,
	substitutionIndex: number,
): boolean {
	let quote: "'" | '"' | "$'" | null = null;
...
function isInsideConductorShellFunctionDefinition(
	command: string,
	index: number,
): boolean {
...
function collectLexicalProcessSubstitutions(
	command: string,
): Array<{ prefix: string; body: string; index: number; grouped: boolean }> {
	...
	for (let index = 0; index < command.length; index += 1) {
		...
		if (body && !isInsideConductorShellFunctionDefinition(command, index)) {
			substitutions.push({
				prefix: collectProcessSubstitutionExpansionPrefix(command, index),
				body,
				index,
				grouped: isInsideConductorSubstitutionGrouping(command, index),
			});
		}
		index = end;
	}
...
function collectLexicalCommandSubstitutions(
	command: string,
): Array<{ prefix: string; body: string; index: number; grouped: boolean }> {
	const substitutions: Array<{
		prefix: string;
...
function scanConductorShellSource(
	...
	const structural = maskShellNonCommandExpansionsForConductorScan(
		stripShellCommentsForConductorScan(
			stripHeredocBodiesForCommandScan(
				normalizeShellLineContinuations(command),
			),
		),
	);
	const normalized = stripCommandSubstitutionBodiesForConductorScan(
		stripProcessSubstitutionBodiesForConductorScan(structural),
	);
	if (depth >= CONDUCTOR_BASH_MAX_NESTING_DEPTH) {
		if (normalized.trim())
			mutations.push({ command: "nested-shell", targets: [] });
		return;
	}
	...
	for (let index = 0; index < structural.length; index += 1) {
		...
		const isolatedRegion = findConductorIsolatedFunctionDefinitionRegion(
			structural,
			definition.startIndex,
			bodyEnd,
		);
		if (isolatedRegion && isolatedRegion.start >= unscannedStart) {
			for (const segment of splitConductorShellSegments(
				stripCommandSubstitutionBodiesForConductorScan(
					stripProcessSubstitutionBodiesForConductorScan(
						structural.slice(unscannedStart, isolatedRegion.start),
					),
				),
			)) {
				scanConductorShellSegment(
		...
		for (const segment of splitConductorShellSegments(
			stripCommandSubstitutionBodiesForConductorScan(
				stripProcessSubstitutionBodiesForConductorScan(
					structural.slice(unscannedStart, definition.startIndex),
				),
			),
		)) {
			scanConductorShellSegment(
		...
		const conditional =
			functionDefinitionMayBeConditional(
				structural.slice(0, definition.startIndex),
			) || /^\s*&/.test(structural.slice(bodyEnd + 1));
		if (isolatedFunctionBindings?.has(definition.name))
			mutations.push({ command: "function", targets: [] });
		const attributes = priorBodies.filter(
			(priorBody) => !isConductorFunctionBody(priorBody),
		);
		const hasReadonlyAttribute = attributes.includes(
			CONDUCTOR_READONLY_FUNCTION_BODY,
		);
		functions.set(
			definition.name,
			conditional
	...
	for (const segment of splitConductorShellSegments(
		stripCommandSubstitutionBodiesForConductorScan(
			stripProcessSubstitutionBodiesForConductorScan(
				structural.slice(unscannedStart),
			),
		),
	)) {
		scanConductorShellSegment(
...
function extractConductorBashMutations(
	...
	if (inheritedConductorBashStartupIsUnsafe())
		return [{ command: "BASH_ENV", targets: [] }];
	if (hasConductorPromptParameterTransform(command))
		return [{ command: "prompt", targets: [] }];
	if (hasUnresolvedShellArithmeticExpansion(command))
		return [{ command: "arithmetic", targets: [] }];
	if (curlCommandHasUnsafeShellExpandedWord(command))
		return [{ command: "curl", targets: [] }];
	...
	const state: ShellPosixState = {
		...
		jobControl: false,
		jobControlMayBeDisabled: true,
		bashoptsLastpipe: safeString(process.env.BASHOPTS)
			.split(":")
			.includes("lastpipe"),
		functionLocalBindings: new Set(),
		posixMode:
			safeString(process.env.POSIXLY_CORRECT).length > 0 ||
			inheritedConductorShellOptions().posix,
	...
	scanConductorShellSource(
		command,
		state,
		inheritedConductorFunctionBindings(),
		cwd,
		false,
		0,
		mutations,
	...
	if (state.securityEnvironmentUnresolved)
		mutations.push({ command: "environment", targets: [] });
	return mutations;
}

function rsyncInvocationHasUnsafeRuntimeEnvironment(
	words: string[],
	commandIndex: number,
	state?: ShellPosixState,
): boolean {
	...
	if (clearBoundary === null) {
		for (const [name, value] of Object.entries(process.env)) {
			if (/^RSYNC_[A-Z0-9_]+$/.test(name))
				values.set(name, safeString(value).trim() !== "");
		}
	}
	for (
		let index = clearBoundary === null ? 0 : clearBoundary + 1;
		...
		for (const operand of collectConductorInvocationWords(words, index)) {
			...
			if (declaration && /^RSYNC_[A-Z0-9_]+$/.test(declaration.name)) {
				...
				values.set(
					shellWordLiteral(operand),
					values.get(shellWordLiteral(operand)) ?? true,
				);
			}
		}
	}
	if (clearBoundary === null && state?.securityEnvironmentUnresolved)
		return true;
	if (clearBoundary === null) {
		for (const [name, value] of state?.securityEnvironment ?? []) {
			...
			values.set(
				name,
				value === CONDUCTOR_UNKNOWN_SHELL_BINDING || value.trim() !== "",
			);
		}
	}
...
function isNativeChildSafeConductorMetadataTarget(
	cwd: string,
	target: string,
): boolean {
	...
	if (
		/(^|\/)(?:session(?:s)?|mode|skill(?:s)?|team|config|manifest|actor|run)(?:[._/-]|$)/i.test(
			normalized,
		)
	)
		return false;
	return (
		normalized.startsWith(".omx/state/inbox/") ||
...
function collectConductorRsyncStaticTransferPaths(
	...
	for (let index = commandIndex + 1; index < words.length; index += 1) {
		...
		if (
			!word ||
			isShellCommandTerminatorOrGroupClose(word) ||
			shellWordMayProduceWgetOptions(word)
		)
			return null;
		if (!optionsTerminated && word === "--") {
		...
		if (!optionsTerminated && word.startsWith("-") && word !== "-") {
			if (
				!/^-[-A-Za-z0-9]+$/.test(word) ||
				[...word.slice(1)].some((flag) => !noArgumentFlags.has(flag))
			)
				return null;
			continue;
		}
...
function conductorMetadataOperandIdentity(
	cwd: string,
	rawPath: string,
): string | null {
...
function conductorRsyncOperandsArePairwiseDistinct(
	cwd: string,
	transfer: { source: string; destination: string; logFile?: string },
): boolean {
	const identities = [
		transfer.source,
		transfer.destination,
		...(transfer.logFile === undefined ? [] : [transfer.logFile]),
	].map((path) => conductorMetadataOperandIdentity(cwd, path));
	return (
		identities.every((identity): identity is string => identity !== null) &&
...
function isConductorSafeRsyncMetadataControl(
	...
	if (rsyncInvocationHasUnsafeRuntimeEnvironment(words, commandIndex, state))
		return false;
	const transfer = collectConductorRsyncStaticTransferPaths(
		words,
		commandIndex,
	);
	...
	const normalizedDestination = normalizeConductorMutationTargets(
		[transfer.destination],
		cwd,
		rootCwd,
	);
	...
	if (transfer.logFile !== undefined) {
		const normalizedLog = normalizeConductorMutationTargets(
			[transfer.logFile],
			cwd,
			rootCwd,
		);
	...
	if (!conductorMetadataCopySourceIsFiniteRegular(cwd, transfer.source))
		return false;
	try {
		const destinationPath = isAbsolute(transfer.destination)
			? resolve(transfer.destination)
			: resolve(cwd, transfer.destination);
...
function isNativeChildSafeConductorRsyncMetadataControl(
	...
	if (
		!transfer ||
		!conductorMetadataCopySourceIsFiniteRegular(cwd, transfer.source)
	)
		return false;
	if (!conductorRsyncOperandsArePairwiseDistinct(cwd, transfer)) return false;
	const targets = [
	...
	return (
		...
		normalized.every((target) =>
			isNativeChildSafeConductorMetadataTarget(rootCwd, target),
		)
	);
}

function isNativeChildSafeConductorReferenceControl(
	words: string[],
	commandIndex: number,
	cwd: string,
): boolean {
	...
	return (
		targets !== null &&
		targets.every((target) =>
			isNativeChildSafeConductorMetadataTarget(cwd, target),
		)
	);
}
...
function collectConductorStaticNestedBashExecutions(
	...
	functions: Map<string, string[]>;
	executions: ConductorStaticNestedBashExecution[];
} {
	...
	scanConductorShellSource(
		...
		cwd,
		true,
		(nestedCommand, childFunctions, childCwd) =>
			executions.push({
				command: nestedCommand,
				functions: new Map(childFunctions),
				cwd: childCwd,
			}),
	);
...
function collectStaticNestedShellTargetAliases(
	cwd: string,
	command: string,
): Map<string, string[]> {
	...
	for (let index = 0; index < words.length; index += 1) {
		...
		while (
			commandStart > 0 &&
			!isShellCommandSeparatorAt(words, commandStart - 1)
		)
			commandStart -= 1;
		const context = resolveWrappedCommandExecutionContext(
			words,
			cwd,
			commandStart,
		);
		if (context === null) continue;
		for (const rawTarget of extractDeepInterviewCommandWriteTargets(
			nestedCommand,
		)) {
			const normalized = normalizeConductorMutationTargets(
				[rawTarget],
...
function conductorStateWriteTransportIsBoundToActiveSession(
	...
	const canonicalCommand = canonicalizeOmxStateTransportCommand(command);
	const hasStateWrite =
		collectOmxStateCommandOperations(canonicalCommand, "write").length > 0 ||
		/\b(?:omx|gjc)\s+state\s+write\b/.test(
			stripHeredocBodiesForCommandScan(command),
		);
	if (!hasStateWrite) return true;
	if (!authoritativeSessionId) return false;
	const statePayload = readStateWriteInputPayload(
		cwd,
		canonicalCommand,
		command,
	);
	...
	const inheritedSessionSelectors = [
		...new Set(
			["OMX_SESSION_ID", "GJC_SESSION_ID"]
				.map((name) => safeString(process.env[name]).trim())
				.filter(Boolean),
		),
	];
	const inheritedSelectorsAreCanonical =
		inheritedSessionSelectors.length === 1 &&
		inheritedSessionSelectors[0] === authoritativeSessionId;
	for (const segment of splitShellCommandSegments(
		stripHeredocBodiesForCommandScan(command),
	)) {
		const segmentHasStateWrite = /\b(?:omx|gjc)\s+state\s+write\b/.test(
			segment,
		);
		const clearsInheritedEnvironment =
			/\b(?:env\s+(?:(?:-[A-Za-z]*i[A-Za-z]*|--ignore-environment)\s+|--ignore-environment\s+)|exec\s+-[A-Za-z]*c[A-Za-z]*)/.test(
				segment,
			);
		const words = tokenizeConductorShellWords(segment);
		const explicitSelectors = words.flatMap((word) => {
			const assignment = parseShellAssignmentWord(word);
			return assignment &&
				(assignment.name === "OMX_SESSION_ID" ||
					assignment.name === "GJC_SESSION_ID")
				? [assignment]
				: [];
		});
		const explicitSelectorsAreCanonical =
			explicitSelectors.length > 0 &&
			explicitSelectors.every(
				(assignment) =>
					!assignment.append &&
					!/[$`]/.test(assignment.value) &&
					assignment.value === authoritativeSessionId,
			);
		if (
		...
		if (commandNameFromShellWord(words[commandIndex] ?? "") === "unset") {
			if (
				collectConductorInvocationWords(words, commandIndex).some((operand) => {
					const name = shellWordLiteral(operand);
		...
		for (let index = 0; index < words.length; index += 1) {
			...
			const unsetName = word.startsWith("--unset=")
				...
				: /^-u.+/.test(word)
					? word.slice(2)
					: shellWordLiteral(words[index + 1] ?? "");
			if (unsetName === "OMX_SESSION_ID" || unsetName === "GJC_SESSION_ID")
				return false;
		}
	}
	return true;
...
function evaluateConductorBashWrite(
	...
	if (
		authoritativeSessionId &&
		!conductorStateWriteTransportIsBoundToActiveSession(
			commandWithHeredocBodies,
			authoritativeSessionId,
			policyCwd,
		)
		...
		return {
			allowed: false,
			blockedDetail:
				"Bash structured state writes must remain bound to the active Conductor session",
		};
	}
	if (depth > CONDUCTOR_BASH_MAX_NESTING_DEPTH) {
		return {
	...
	if (hasUnresolvedShellArithmeticExpansion(normalizedCommand)) {
		return {
			allowed: false,
			blockedDetail:
				"Bash arithmetic expansion is not statically numeric and cannot be validated for Main-root Conductor writes",
		};
	}
	if (commandHasUnsafeConductorShellState(normalizedCommand, cwd)) {
		return {
			allowed: false,
			blockedDetail:
				"Bash nameref or allexport shell state cannot be statically validated for Main-root Conductor writes",
		};
	}
	...
	const redirectTargets = extractDeepInterviewCommandRedirectTargets(
		commandWithHeredocBodies,
	);
	if (
		!conductorMetadataRedirectsHaveBoundedProducers(commandWithHeredocBodies)
	) {
		return {
			allowed: false,
			blockedDetail:
				"Bash metadata redirects require a statically bounded producer",
		};
	}
	if (
		redirectTargets.some(
			(target) => !conductorRedirectTargetIsSafe(cwd, target, policyCwd),
		)
	) {
		return {
			allowed: false,
			blockedDetail:
				"Bash redirect target is not a static workflow metadata leaf",
		};
	}
	if (!conductorMetadataWriteSizesStayBounded(cwd, commandWithHeredocBodies)) {
		return {
			allowed: false,
			blockedDetail:
				"Bash metadata writes exceed the bounded per-leaf size limit",
		};
	}
	...
	function isExactConductorMetadataRoot(cwd: string, target: string): boolean {
		...
		return Boolean(
			relativeTarget &&
				CONDUCTOR_ORCHESTRATION_METADATA_PREFIXES.includes(
					relativeTarget as (typeof CONDUCTOR_ORCHESTRATION_METADATA_PREFIXES)[number],
				),
		);
	}

	const shellMutations = extractConductorBashMutations(
		normalizedCommand,
		cwd,
		policyCwd,
	);
	...
	if (editorTargets.length > 0) {
		const blockedTarget = editorTargets.find(
			(target) =>
				!isAllowedConductorMetadataExecutionPath(cwd, policyCwd, target),
		);
		if (blockedTarget) {
			return {
				allowed: false,
	...
	const runtimeBlockedDetail = classifyConductorExecutableRuntime(
		commandWithHeredocBodies,
		0,
		cwd,
	);
	if (runtimeBlockedDetail)
		return { allowed: false, blockedDetail: runtimeBlockedDetail };
	...
	const nestedTargetAliases = collectStaticNestedShellTargetAliases(
		cwd,
		commandWithHeredocBodies,
	);
	...
	if (
		...
		targets.length === 0 &&
		shellMutations.length === 0 &&
		/\b(?:export\s+(?:-[A-Za-z]*f[A-Za-z]*|--functions?)|(?:declare|typeset)\s+-[A-Za-z]*f[A-Za-z]*)\b/.test(
			normalizedCommand,
		)
	)
		return { allowed: true };
	...
	const createsMetadataDirectories =
		/\binstall\b/.test(normalizedCommand) &&
		/(?:^|\s)-[^\s]*d|(?:^|\s)--directory(?:\s|$)/.test(normalizedCommand);
	const blockedTarget = targets.find((target) => {
		const isNormalizedShellTarget = shellMutations.some((mutation) =>
			mutation.targets.includes(target),
		);
		const matchingShellMutations = shellMutations.filter((mutation) =>
			mutation.targets.includes(target),
		);
		if (
			target === ".omx/state" &&
			...
			return false;
		if (
			isNormalizedShellTarget &&
			isAllowedConductorMetadataPath(policyCwd, target)
		)
			return false;
		if (isAllowedConductorMetadataExecutionPath(cwd, policyCwd, target))
			return false;
		if (
			createsMetadataDirectories &&
			isExactConductorMetadataRoot(policyCwd, target)
		)
			return false;
		return !(nestedTargetAliases.get(target) ?? []).some((normalized) =>
			isAllowedConductorMetadataPath(policyCwd, normalized),
		);
	});
	if (blockedTarget) {
...
function directConductorStateWritePayloadHasExactSchema(
	payload: CodexHookPayload,
	policyCwd: string,
	canonicalSessionId: string,
): boolean {
	...
	while (nestedState) {
		if (!suppliedSessionAliasesMatch(nestedState, canonicalSessionId))
			return false;
		...
		const nestedWorkingDirectory = safeString(
			nestedState.workingDirectory,
		).trim();
		if (
			nestedWorkingDirectory &&
			resolve(nestedWorkingDirectory) !== resolve(policyCwd)
		)
			return false;
		const nestedMode = safeString(nestedState.mode).trim();
		if (nestedMode && nestedMode !== safeString(input.mode).trim())
			return false;
		nestedState =
			nestedState.state === undefined ? null : safeObject(nestedState.state);
	}
	if (
		safeString(input.workingDirectory).trim() === "" ||
		resolve(safeString(input.workingDirectory)) !== resolve(policyCwd)
	)
		return false;
	return true;
}
...
function conductorStatePayloadPreservesActiveGuard(
	...
	const phase = safeString(input.current_phase ?? input.currentPhase)
		.trim()
		.toLowerCase();
...
export async function buildConductorPreToolUseWriteGuardOutput(
	...
	const activeState = await readActiveConductorStateForPreToolUse(
		payload,
		policyCwd,
		stateDir,
		resolvedSessionId,
	...
	const writeActor = await resolvePreToolUseWriteActor(
		payload,
		cwd,
		stateDir,
		sessionId,
	...
	if (writeActor === "provenance-conflict") {
		return buildConductorSessionProvenanceDeny(
			activeState,
	...
	if (toolName === "Bash") {
		...
		const bashEvaluation = evaluateConductorBashWrite(
			cwd,
			command,
			0,
			sessionId,
		...
		const bashStateOperations = collectOmxStateCommandOperations(
			canonicalStateCommand,
			"write",
		);
		if (bashStateOperations.length > 0) {
			const bashStatePayload = readStateWriteInputPayload(
				policyCwd,
				canonicalStateCommand,
				command,
			);
			if (
				!isStandaloneParsedOmxStateWriteTransport(
					policyCwd,
					command,
					sessionId,
				) ||
				!conductorStatePayloadPreservesActiveGuard(
					bashStatePayload,
					activeState,
				)
			) {
				blocked = true;
				blockedDetail =
					"Bash state writes must preserve the canonical active Conductor guard";
			}
		}
		const safeExportedFunctionRead =
			!blocked &&
			shellMutations.length === 0 &&
			/\b(?:export\s+(?:-[A-Za-z]*f[A-Za-z]*|--functions?)|(?:declare|typeset)\s+-[A-Za-z]*f[A-Za-z]*)\b/.test(
				command,
		...
		nativeChildMutationAttempt =
			(mutationTransport === "bash" || shellMutations.length > 0) &&
			!safeExportedFunctionRead;
		if (blocked)
			blockedDetail =
				bashEvaluation.blockedDetail ??
				buildConductorBashBlockedDetail(cwd, command);
		...
		if (toolName === "mcp__omx_state__state_clear") {
			blocked = true;
			blockedDetail =
				"Structured state_clear is not authorized while a Conductor workflow is active";
		} else if (
			toolName === "mcp__omx_state__state_write" &&
			(!directConductorStateWritePayloadHasExactSchema(
				payload,
				policyCwd,
				sessionId,
			) ||
			...
			blocked = true;
			blockedDetail =
				"Structured state writes must preserve the canonical active Conductor guard";
		}
	...
	const teamWorkerProtectedStateTarget =
		teamWorkerMutationTargetsProtectedWorkflowState(
			payload,
			toolName,
			command,
	...
	if (writeActor === "team-worker" && !teamWorkerProtectedStateTarget)
		return null;
	if (
		writeActor === "team-worker" &&
		teamWorkerProtectedStateTarget &&
		!blocked
	) {
		blocked = true;
		blockedDetail =
			"Bash targets protected workflow state outside authorized Team-worker scope";
	}
	if (
		!blocked &&
		(writeActor !== "native-child" || !nativeChildMutationAttempt)
	)
		return null;
...
async function preflightNativePromptTarget(
	...
	try {
		...
		return preflightSelectedTargetOwner(
			context,
			[{ ownerCodexSessionId: {} }],
			"native",
	...
	for (const filename of filenames) {
		if (
			!filename.endsWith("-state.json") &&
			filename !== SKILL_ACTIVE_STATE_FILE
		)
			continue;
		try {
			const value = JSON.parse(
				await readFile(join(targetDir, filename), "utf8"),
			) as unknown;
...
export async function dispatchCodexNativeHook(
	...
	if (
		...
		const denial = evaluateCodex01445PreToolUse(payload, {
			resolveInstalledRoleName: (role) =>
				resolveInstalledRoleName(role, undefined, cwd),
		});
		if (denial) {
			return {
				hookEventName,
	...
	let promptTurnContext: ResolvedPromptTurnContext | null =
		hookEventName === "UserPromptSubmit"
			? evaluateResolvedPromptTurn({
					producer: "native",
					...
					threadFacts: await readNativePromptThreadFacts(
						cwd,
						nativeSessionId,
						threadId,
						currentSessionState,
	...
	if (promptTurnContext) {
		promptTurnContext = await preflightNativePromptTarget(
			stateDir,
			promptTurnContext,
		);
	}
	...
	let allowImplicitSessionSideEffects =
		pointer.status === "usable" ||
		pointer.status === "absent" ||
		promptTurnContext?.status === "authorized";
	let stopAuthorizationFailure: { stopReason: string; reason: string } | null =
	...
	if (
		...
		canonicalSessionId =
			normalizeSessionId(readPayloadSessionId(payload)) ?? "";
		allowImplicitSessionSideEffects = true;
		stopAuthorizationFailure = null;
	...
	if (hookEventName === "UserPromptSubmit") {
		...
		if (promptTurnContext?.status === "authorized") {
			...
			if (rawHookSessionId && !normalizedHookSessionId) {
				...
				try {
					const writableScope = await resolveWritableStateScope(
						cwd,
						explicitHookSessionId,
					);
		...
		if (
			...
			skillState =
				...
				(await recordSkillActivation({
					...
					allowSecondaryTeam: !isNativeOutsideTmuxUserPrompt(
						cwd,
						payload,
						sessionIdForState || undefined,
					),
		...
		if (
			allowImplicitSessionSideEffects &&
			allowPromptGlobalSideEffects &&
			!skipHudReconcileForDoctorSmoke &&
			!skipHudReconcileForTeamWorkerPane
		) {
	...
	if (hookEventName === "PreCompact") {
		...
		const identitylessTeamWorkerContext =
		...
		const payloadPolicyCwd =
			rootSessionId !== "" && rootSessionId !== payloadSessionId
				? cwd
				: policyCwd;
		const foreignRootNativeChildDeny =
		...
		if (
			...
			const canonicalPlanningState = sessionBinding.canonicalSessionId
				...
				: null;
			const canonicalPlanningGuard: ActiveConductorState | null =
				canonicalPlanningState
					? {
							mode:
								safeString(canonicalPlanningState.mode).trim().toLowerCase() ||
								"planning",
							phase:
								safeString(
									canonicalPlanningState.current_phase ??
										canonicalPlanningState.currentPhase,
								).trim() || "active",
						}
					: null;
			const payloadScopedConductorState =
				!sessionBinding.valid &&
				payloadSessionId &&
				policyRoot.externalStateRoot
					...
					: null;
			const payloadScopedPlanningState =
				!sessionBinding.valid &&
				payloadSessionId &&
				policyRoot.externalStateRoot
					? ((await readActiveDeepInterviewStateForPreToolUse(
					...
					: null;
			const payloadScopedPlanningGuard: ActiveConductorState | null =
				payloadScopedPlanningState
					? {
							mode:
								safeString(payloadScopedPlanningState.mode)
									.trim()
									.toLowerCase() || "planning",
							phase:
								safeString(
									payloadScopedPlanningState.current_phase ??
										payloadScopedPlanningState.currentPhase,
								).trim() || "active",
						}
					: null;
			const guardedConductorState =
				activeConductorState ??
				payloadScopedConductorState ??
				payloadScopedPlanningGuard ??
				canonicalPlanningGuard;
			const preservesIdentitylessTeamWorkerExemption =
			...
			if (
				...
				!sessionBinding.valid &&
				mutationTransport !== "read-only" &&
				(!preservesIdentitylessTeamWorkerExemption ||
					canonicalPlanningGuard !== null)
			) {
				...
				const nativeChildDeny =
					hasExplicitNativeIdentity &&
					!payloadHasConflictingIdentityAliases(payload)
						? await buildConductorPreToolUseWriteGuardOutput(
								payload,
								cwd,
				...
				const preToolUseSessionId =
					sessionBinding.valid || preservesIdentitylessTeamWorkerExemption
						? sessionBinding.canonicalSessionId
						: "";
				outputJson =
					buildNativeUnknownRolePreToolUseOutput(payload, policyCwd) ??
		...
		if (allowImplicitSessionSideEffects) {
			...
			const failure = stopAuthorizationFailure ?? {
				stopReason: "session_pointer_unusable",
				reason:
					"OMX cannot authorize Stop without a writable session authority.",
			};
			outputJson = {
				decision: "block",
...
function buildUnparseablePreToolUseDenyOutput(
	reason: string,
