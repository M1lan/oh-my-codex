...
function usage(): never {
	console.error(
		"Usage: node scripts/generate-release-body.mjs --template <path> --out <path> [--current-tag <tag>] [--previous-tag <tag>] [--repo <owner/name>]",
	);
	process.exit(1);
}
...
function runGit(args: string[], cwd: string, allowFailure = false): string {
	...
	if (result.status !== 0) {
		...
		throw new Error(
			`git ${args.join(" ")} failed: ${(result.stderr || result.stdout || "").trim()}`.trim(),
		);
	}
...
export function resolveCurrentTag(cwd: string, explicit?: string): string {
	...
	throw new Error(
		"unable to determine current release tag; pass --current-tag or set GITHUB_REF_NAME",
	);
}

function isAncestorTag(
	cwd: string,
	...
	const result = spawnSync(
		"git",
		["merge-base", "--is-ancestor", possibleAncestor, currentTag],
		{
			cwd,
			encoding: "utf-8",
			stdio: "pipe",
...
function verifyGitCommitRef(cwd: string, ref: string, label: string): void {
	if (
		!runGit(["rev-parse", "--verify", "--quiet", `${ref}^{commit}`], cwd, true)
	) {
		throw new Error(
			`unable to verify ${label} ref for release compare: ${ref}`,
		);
	}
}

export function verifyCompareRange(
	cwd: string,
	currentTag: string,
	previousTag?: string,
): void {
	...
	const mergeBase = spawnSync(
		"git",
		["merge-base", "--is-ancestor", previousTag, currentTag],
		{
			cwd,
			encoding: "utf-8",
			stdio: "pipe",
	...
	if (mergeBase.status !== 0) {
		throw new Error(
			`invalid release compare range: ${previousTag} is not an ancestor of ${currentTag}`,
		);
	}
}
...
export function buildFullChangelogLine(
	repo: string,
	currentTag: string,
	previousTag?: string,
): string {
	if (!repo) {
		throw new Error(
			"unable to determine GitHub repository; pass --repo or set GITHUB_REPOSITORY",
		);
	}
	if (previousTag) {
...
export function replaceFullChangelogLine(
	markdown: string,
...
export function getGitContributors(
	cwd: string,
	currentTag: string,
	previousTag?: string,
): Contributor[] {
	...
	return normalizeContributors(
		shortlog
			.split(/\r?\n/)
			.map((line) => parseShortlogLine(line))
...
export async function getGitHubCompareContributors(
	...
	const response = await fetchImpl(
		`https://api.github.com/repos/${repo}/compare/${previousTag}...${currentTag}`,
		{
			headers: {
...
export async function resolveContributors(options: {
	...
	if (repo && previousTag && githubToken) {
		try {
			return await getGitHubCompareContributors(
				repo,
				currentTag,
				previousTag,
				githubToken,
			...
			console.error(
				`[generate-release-body] falling back to git shortlog: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
...
export async function generateReleaseBody(
	options: GenerateReleaseBodyOptions,
): Promise<string> {
	...
	const repo =
		...
		resolveRepositoryFromRemote(cwd);
	...
	markdown = replaceSectionBody(
		markdown,
		"Contributors",
		renderContributorsSection(contributors),
	);
	markdown = replaceFullChangelogLine(
		markdown,
		buildFullChangelogLine(repo || "", currentTag, previousTag),
	);
