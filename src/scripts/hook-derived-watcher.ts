...
const pollMs = Math.max(
	250,
	asNumber(
		argValue("--poll-ms", process.env.OMX_HOOK_DERIVED_POLL_MS || "800"),
		800,
	),
);
const maxFileAgeMs = Math.max(
	10_000,
	asNumber(
		argValue(
			"--file-age-ms",
			process.env.OMX_HOOK_DERIVED_FILE_AGE_MS || "90000",
		),
		90000,
	),
);
...
const logPath = join(
	logsDir,
	`hook-derived-watcher-${new Date().toISOString().split("T")[0]}.jsonl`,
);
...
function collectMessageTextFragments(
	value: unknown,
	fragments: string[],
): void {
...
function shouldTrackSessionMeta(
	line: string,
...
function inferDerivedEvent(
	...
	const turnId = safeString(
		(payload as Record<string, unknown>).turn_id ||
			(parsed as Record<string, unknown>).turn_id ||
		...
		return {
			...
			context: {
				...
				tool_name: safeString(
					payload.tool_name || payload.tool || payload.name,
				),
			},
		};
	}
	...
	if (payloadType === "assistant_message") {
		...
		const looksLikeQuestion =
			/\?|\b(can you|could you|please provide|need input|what should)/i.test(
				message,
			);
...
function updateTurnState(
	...
	const payloadType = safeString(
		(parsed.payload as Record<string, unknown>).type,
	);
	if (payloadType === "task_started") {
		meta.currentTurnId = safeString(
...
function inferOperationalCall(
	...
	if (payload.type === "function_call" && payload.name === "exec_command") {
		const { command, workdir } = parseExecCommandArgs(
...
async function dispatchDerivedEvent(
	...
	try {
		...
		await dispatchHookEvent(
			event as unknown as Parameters<typeof dispatchHookEvent>[0],
			{
				cwd,
...
async function dispatchOperationalEvent(
	input: OperationalEventInput,
): Promise<void> {
	try {
		const { buildDerivedHookEvent } = await import(
...
async function processLine(meta: FileMeta, line: string): Promise<void> {
	...
	if (operational?.phase === "start") {
		...
		if (operational.kind === "test") {
			if (
				operational.result?.success !== true &&
			...
			await dispatchOperationalEvent({
				...
				confidence: 0.95,
				extra: {
					...(operational.result?.exit_code !== undefined
						? { exit_code: operational.result.exit_code }
						: {}),
				},
			});
		}
		if (
			operational.kind === "pr-create" &&
			operational.result?.success !== false &&
			(operational.result?.pr_number !== undefined ||
				operational.result?.pr_url)
		) {
			await dispatchOperationalEvent({
				event: "pr-created",
				workdir: operational.workdir,
...
async function writeState(): Promise<void> {
	...
	const tracked = Array.from(fileState.values()).reduce(
