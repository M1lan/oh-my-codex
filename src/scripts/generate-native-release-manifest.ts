...
function usage(): never {
	console.error(
		"Usage: node scripts/generate-native-release-manifest.mjs --plan <path> --artifacts-dir <dir> --out <path> --release-base-url <url> [--require-products a,b]",
	);
	process.exit(1);
}
...
function checksum(raw: string, artifact: string): string {
	...
	if (!/^[a-f0-9]{64}$/i.test(value))
		throw new Error(`invalid SHA-256 sidecar for ${artifact}`);
...
const requestedProducts = (arg("--require-products") || "")
	.split(",")
...
if (!/^v[^\s/]+$/.test(plan.announcement_tag))
	throw new Error("announcement tag must be a v-prefixed release version");
const version = plan.announcement_tag.slice(1);
if (
	!Array.isArray(plan.releases) ||
	plan.releases.length === 0 ||
	plan.releases.some((release) => release.app_version !== version)
) {
	throw new Error(
		`release versions must exactly match announcement tag ${plan.announcement_tag}`,
	);
}
...
for (const artifact of Object.values(plan.artifacts)) {
	...
	if (
		!Array.isArray(artifact.target_triples) ||
	...
	if (!/\.(?:tar\.xz|tar\.gz|zip)$/i.test(artifact.name))
		throw new Error(`unsupported native archive format: ${artifact.name}`);
	const executable =
		artifact.assets?.filter((asset) => asset.kind === "executable") ?? [];
	if (executable.length !== 1)
		throw new Error(`expected exactly one executable for ${artifact.name}`);
	...
	if (statSync(archivePath).size <= 0 || statSync(checksumPath).size <= 0)
		throw new Error(`empty artifact file for ${artifact.name}`);
	const expectedSha256 = checksum(
		readFileSync(checksumPath, "utf-8"),
		artifact.name,
	);
	if (sha256(archivePath) !== expectedSha256)
	...
	const release = plan.releases.find(
		(item) =>
			item.app_name === binary.name ||
			item.app_name === binary.id?.split("-exe-")[0],
	);
	if (!release || release.app_version !== version)
		throw new Error(`missing coherent release metadata for ${artifact.name}`);
	assets.push({
...
const manifest = {
	...
	assets: assets.sort((a, b) => {
		const libcRank = (asset: NativeReleaseAssetPolicyInput): number =>
			asset.libc === "musl" ? 0 : asset.libc === "glibc" ? 1 : 2;
		return (
...
if (manifest.assets.length === 0)
	throw new Error("release manifest must declare at least one native archive");
validateNativeReleaseManifest(manifest);
if (manifest.tag !== `v${manifest.version}`)
	throw new Error("manifest tag/version mismatch");
