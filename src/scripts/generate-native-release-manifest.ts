...
function mapTriple(triple: string): TripleMapping | undefined {
	switch (triple) {
		case "x86_64-unknown-linux-gnu":
			return { platform: "linux", arch: "x64", libc: "glibc" };
		case "aarch64-unknown-linux-gnu":
			return { platform: "linux", arch: "arm64", libc: "glibc" };
		case "x86_64-unknown-linux-musl":
			return { platform: "linux", arch: "x64", libc: "musl" };
		case "aarch64-unknown-linux-musl":
			return { platform: "linux", arch: "arm64", libc: "musl" };
...
const requireProducts = (arg("--require-products") || "")
	.split(",")
...
for (const artifact of Object.values(plan.artifacts)) {
	...
	if (!mapped) continue;
	const executable = (artifact.assets || []).find(
		(asset) => asset.kind === "executable",
	);
	...
	const release = plan.releases.find(
		(item) =>
			item.app_name === executable.name ||
			item.app_name === executable.id?.split("-exe-")[0],
	);
	const version =
		release?.app_version || plan.announcement_tag.replace(/^v/, "");
	assets.push({
...
const manifest = {
	...
	assets: assets.sort((a, b) => {
		const keyCompare = `${a.product}-${a.platform}-${a.arch}`.localeCompare(
			`${b.product}-${b.platform}-${b.arch}`,
		);
		if (keyCompare !== 0) return keyCompare;
		const libcOrder: Record<string, number> = { musl: 0, glibc: 1 };
		const libcCompare =
			(libcOrder[a.libc ?? ""] ?? 2) - (libcOrder[b.libc ?? ""] ?? 2);
