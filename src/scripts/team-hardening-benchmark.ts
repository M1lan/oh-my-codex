	...
	for (let i = 0; i < iterations; i += 1) {
		...
		try {
			...
			const task = await createTask(
				"team-bench",
				{ subject: "recover", description: "d", status: "pending" },
				cwd,
			);
			...
			const current = JSON.parse(await readFile(taskPath, "utf-8")) as Record<
				string,
				unknown
			>;
			(current.claim as Record<string, unknown>).leased_until = new Date(
				Date.now() - 1000,
			).toISOString();
...
async function benchDirtyWorktree(iterations = 10): Promise<number[]> {
	...
	for (let i = 0; i < iterations; i += 1) {
		...
		try {
			...
			await writeFile(
				join(first.worktreePath, "DIRTY.txt"),
				"dirty\n",
				"utf-8",
			);
			const start = performance.now();
...
function summarize(name: string, values: number[]): void {
	...
	console.log(
		`${name}: n=${sorted.length} avg_ms=${avg.toFixed(2)} median_ms=${median.toFixed(2)} p95_ms=${p95.toFixed(2)}`,
	);
}
