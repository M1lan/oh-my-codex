...
export async function runReplyListenerLiveSmoke(
	...
	if (!discordSendResponse.ok) {
		throw new Error(
			`Discord live smoke failed: HTTP ${discordSendResponse.status}`,
		);
	}
	const discordPayload = await parseResponseJson(
		discordSendResponse,
		"Discord sendMessage",
	);
	const discordMessageId =
		typeof discordPayload.id === "string" && discordPayload.id.trim()
			? discordPayload.id
			: null;
	if (!discordMessageId) {
	...
	if (!telegramSendResponse.ok) {
		throw new Error(
			`Telegram live smoke failed: HTTP ${telegramSendResponse.status}`,
		);
	}
	const telegramPayload = await parseResponseJson(
		telegramSendResponse,
		"Telegram sendMessage",
	);
	const telegramResult = requireJsonObject(
		telegramPayload.result,
		"Telegram sendMessage.result",
	);
	const telegramMessageId =
		typeof telegramResult.message_id === "number" ||
		typeof telegramResult.message_id === "string"
...
export async function main(): Promise<void> {
	...
	if (!resolution.enabled) {
		console.log(
			`reply-listener live smoke: SKIP (${LIVE_ENABLE_ENV}=1 to enable)`,
		);
		return;
	}
	if (!resolution.config) {
		console.log(
			`reply-listener live smoke: SKIP (missing env: ${resolution.missing.join(", ")})`,
		);
		return;
	}
...
if (isMain) {
	main().catch((error) => {
		console.error(
			`reply-listener live smoke: FAIL\n${error instanceof Error ? error.message : String(error)}`,
		);
		process.exit(1);
	});
}
