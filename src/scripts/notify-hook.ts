...
function isExplicitAutopilotActivationText(text: string): boolean {
	return (
		/(?:^|[^\w])\$autopilot\b/i.test(text) ||
		/^\s*\/autopilot\b/i.test(text) ||
		/^\s*(?:please\s+)?autopilot(?:\s+(?:this|mode|workflow|skill|loop|now))?\s*[.!]?\s*$/i.test(
			text,
		) ||
		/\b(?:use|run|start|enable|launch|invoke|activate|resume|continue)\s+(?:the\s+)?autopilot(?:\s+(?:mode|workflow|skill|loop|now))?\s*[.!]?\s*$/i.test(
			text,
		) ||
...
async function main() {
	...
	const payloadSessionId = safeString(
		payload.session_id || payload["session-id"] || "",
	);
	const payloadThreadId = safeString(
		payload["thread-id"] || payload.thread_id || "",
	);
	...
	try {
		...
		if (turnId) {
			...
			const dedupeStatePath = await getScopedStatePath(
				stateDir,
	...
	if (!isTeamWorker) {
		try {
			...
			const hudStatePath = await getScopedStatePath(
				stateDir,
				"hud-state.json",
				scopedSessionId,
			);
			let hudState = await readScopedJsonIfExists(
				stateDir,
				"hud-state.json",
				scopedSessionId,
				{
	...
	try {
		const { detectKeywords, recordSkillActivation } = await import(
			"../hooks/keyword-detector.js"
		);
		if (latestUserInput) {
			const activationSessionId = getEffectiveSessionId();
			const isAutopilotActivation =
				detectKeywords(latestUserInput).some(
					(match) => match.skill === "autopilot",
				) || isExplicitAutopilotActivationText(latestUserInput);
			const suppressTerminalReplay =
	...
	if (!isTeamWorker) {
		try {
			...
			if (csResult.triggered) {
				const managedSession = await isManagedOmxSession(cwd, payload, {
