...
async function readJsonFileIfObject(
	path: string,
...
function hasOmxRuntimeStateMarker(
...
async function hasManagedTeamStateTree(cwd: string): Promise<boolean> {
	...
	for (const entry of entries) {
		...
		if (
			existsSync(join(teamDir, "manifest.v2.json")) ||
			existsSync(join(teamDir, "config.json"))
		) {
			return true;
		}
	}
...
async function isOmxManagedCwd(cwd: string): Promise<boolean> {
	const trustedInternalCwd = safeString(
	...
	const teamState = await readJsonFileIfObject(
		join(cwd, ".omx", "state", "team-state.json"),
	);
	if (hasOmxRuntimeStateMarker(teamState)) return true;
	const hudState = await readJsonFileIfObject(
		join(cwd, ".omx", "state", "hud-state.json"),
	);
	if (
		hudState &&
		(typeof hudState.last_turn_at === "string" ||
			typeof hudState.turn_count === "number")
	)
		return true;
	if (await hasManagedTeamStateTree(cwd)) return true;
	const teamWorkerEnv = safeString(
...
function classifyIdleNotificationPhase(
	message: unknown,
): "idle" | "progress" | "finished" | "failed" {
...
function looksLikeAutopilotTerminalHandoff(text: string): boolean {
	return (
		/\bAutopilot complete\b/i.test(text) ||
...
function terminalStateMatchesNotifyTurn(
	state: Record<string, unknown>,
	payload: Record<string, unknown>,
): boolean {
	const payloadTurnId = safeString(
		payload["turn-id"] || payload.turn_id || "",
	).trim();
	const stateTurnId = safeString(state.turn_id || state.turnId || "").trim();
	const payloadThreadId = safeString(
		payload["thread-id"] || payload.thread_id || "",
	).trim();
	...
	if (payloadTurnId || stateTurnId) {
		if (!payloadTurnId || !stateTurnId || payloadTurnId !== stateTurnId)
			return false;
		return (
			!payloadThreadId || !stateThreadId || payloadThreadId === stateThreadId
		);
	}
...
async function hasTerminalAutopilotStateForNotifyTurn(
	...
	return (
		isTerminalModeStateObject(state, "autopilot") &&
...
async function shouldSuppressAutopilotTerminalReplayActivation(
	...
	if (
		!isTurnCompletePayload(payload) &&
		!isNotifyFallbackTaskCompletePayload(payload)
	)
		return false;
	...
	if (
		!looksLikeAutopilotTerminalHandoff(lastAssistantMessage) &&
		!isNotifyFallbackTaskCompletePayload(payload)
	)
		return false;
...
export interface NotifySkillActivationDependencies {
	...
	readonly recordSkillActivation?: (
...
function boundedNotifySkillActivationContext(value: unknown): string | null {
	try {
		const context = safeString(value)
			.trim()
			.slice(0, NOTIFY_SKILL_ACTIVATION_CONTEXT_MAX_LENGTH);
		return context || null;
	} catch {
		return null;
	}
...
export async function recordNotifySkillActivation(
	...
	const classification = (
		dependencies.classifyKeywordInput ?? classifyKeywordInput
	)(input.text);
	...
	const terminalAutopilotReplay =
		await shouldSuppressAutopilotTerminalReplayActivation(
			input.stateDir,
			input.payload,
			runtimeMatches.some((match) => match.skill === "autopilot"),
			input.sessionId || "",
		);
	if (terminalAutopilotReplay && runtimeMatches[0]?.skill === "autopilot")
		return null;
...
function buildIdleNotificationFingerprint(
	payload: Record<string, unknown>,
): string {
	const lastAssistantMessage = safeString(
		payload["last-assistant-message"] || payload.last_assistant_message || "",
...
function isTurnCompletePayload(payload: Record<string, unknown>): boolean {
	const type = safeString(payload.type || "")
		...
		.toLowerCase();
	return (
		type === "" || type === "agent-turn-complete" || type === "turn-complete"
	);
}

function isNotifyFallbackTaskCompletePayload(
	payload: Record<string, unknown>,
): boolean {
...
async function preflightLeaderNotifyTarget(
	...
	const probePath = await getScopedStatePathAtScope(
		stateDir,
		"prompt-provenance-probe.json",
		scope,
	);
	...
	for (const filename of filenames) {
		if (
			...
			filename !== "skill-active-state.json"
		)
			continue;
		try {
...
async function resolveLeaderNotifyWriteDecision(
	...
	const preflightedContext = await preflightLeaderNotifyTarget(
		stateDir,
		context,
	);
	if (preflightedContext.status !== "authorized")
		return { context: preflightedContext, pointerContext };
	...
	return {
		...
		scope: {
			...
			allowedStorageSessionIds:
				preflightedContext.authorization.allowedStorageSessionIds,
		},
	};
}
...
async function dispatchTurnCompleteHookEvents(
	...
	try {
		const { buildNativeHookEvent, buildDerivedHookEvent } = await import(
			"../hooks/extensibility/events.js"
		);
		const { dispatchHookEvent } = await import(
			"../hooks/extensibility/dispatcher.js"
		);
		const threadIdForHooks = safeString(
			payload["thread-id"] || payload.thread_id || "",
		);
		const turnIdForHooks = safeString(
			payload["turn-id"] || payload.turn_id || "",
		);
		const modeForHooks = safeString(payload.mode || "");
		const outputPreview = safeString(
			payload["last-assistant-message"] || payload.last_assistant_message || "",
		).slice(0, 400);
		const event = buildNativeHookEvent(
			"turn-complete",
			{
		...
		for (const signal of deriveAssistantSignalEvents(outputPreview)) {
			const derivedEvent = buildDerivedHookEvent(
				signal.event,
				buildOperationalContext({
					cwd,
...
async function main() {
	...
	const latestUserInput = safeString(
		inputMessages.length > 0 ? inputMessages[inputMessages.length - 1] : "",
	);
	const isTurnComplete = isTurnCompletePayload(payload);
	const isNotifyFallbackTaskComplete =
		isNotifyFallbackTaskCompletePayload(payload);

	// Team worker detection via environment variable
	const teamWorkerEnv =
		process.env.OMX_TEAM_INTERNAL_WORKER || process.env.OMX_TEAM_WORKER; // e.g., "fix-ts/worker-1"
	const parsedTeamWorker = parseTeamWorkerEnv(teamWorkerEnv);
	const isTeamWorker = !!parsedTeamWorker;

	const resolvedWorkerStateDir =
		isTeamWorker && parsedTeamWorker
	...
	const leaderWriteDecision = isTeamWorker
		? null
		: await resolveLeaderNotifyWriteDecision(
				cwd,
				stateDir,
				rawPayloadSessionId,
			);
	...
	if (
		!isTeamWorker &&
		leaderWriteDecision?.context.status === "suppressed-target-child"
	)
		return;
	const leaderAuthorization =
		leaderWriteDecision?.context.status === "authorized"
			? leaderWriteDecision.context.authorization
			: null;
	const canWriteLeaderScopedState = Boolean(
		leaderWriteDecision?.scope && leaderAuthorization,
	);
	...
	if (isTeamWorker && workerStateRootResolved) {
		...
		currentOmxSessionId =
			(await readCurrentSessionId(stateDir).catch(() => "")) || "";
	}
		...
		try {
			...
			if (turnId) {
				...
				const dedupeStatePath = scope
					? await getScopedStatePathAtScope(
							stateDir,
							"notify-hook-state.json",
							scope,
						)
					: await getScopedStatePath(
							stateDir,
							"notify-hook-state.json",
							dedupeSessionId,
						);
				const dedupeState = normalizeNotifyState(
					scope
						? await readScopedJsonAtScope(
								stateDir,
								"notify-hook-state.json",
								scope,
								null,
							)
						: await readScopedJsonIfExists(
								stateDir,
								"notify-hook-state.json",
								dedupeSessionId,
								null,
				...
				dedupeState.recent_turns = pruneRecentTurns(
					dedupeState.recent_turns,
					now,
				);
	...
	if (!isTeamWorker && canWriteLeaderScopedState) {
		try {
			...
			if (getEffectiveSessionId() && threadId) {
				const { recordSubagentTurnForSession } = await import(
	...
	if (isTeamWorker && !workerStateRootResolved) {
		...
		const explicitWorkerStateRoot = safeString(
			process.env.OMX_TEAM_STATE_ROOT || "",
		).trim();
		const autoNudgeStateDir = explicitWorkerStateRoot
			? resolve(cwd, explicitWorkerStateRoot)
			: "";
		if (autoNudgeStateDir && existsSync(autoNudgeStateDir)) {
			try {
				await maybeAutoNudge({
					cwd,
					stateDir: autoNudgeStateDir,
					logsDir,
					payload,
				});
			} catch {
	...
	if (!isTeamWorker && canWriteLeaderScopedState) {
		try {
			...
			if (
				resumeResult.currentOmxSessionId &&
				resumeResult.currentOmxSessionId ===
					leaderAuthorization!.targetSessionId
			) {
	...
	if (!isTeamWorker && canWriteLeaderScopedState) {
		try {
			const scopedDirs = await getScopedStateDirsForCurrentSession(
				stateDir,
					...
					if (state.active) {
						...
						if (
							...
							if (isActiveRalphProgress) {
								...
								state.max_iterations_auto_expand_count =
									(asNumber(state.max_iterations_auto_expand_count) || 0) + 1;
								...
								if (
									...
									!["cancelled", "failed", "complete"].includes(
										state.current_phase,
									)
								) {
									state.current_phase = "complete";
								...
								if (
									typeof state.stop_reason !== "string" ||
									!state.stop_reason
								) {
	...
	if (!isTeamWorker) {
		...
		try {
			...
			if (existsSync(metricsPath)) {
				metrics = {
					...metrics,
					...JSON.parse(await readFile(metricsPath, "utf-8")),
				};
			}
			...
			if (tokenUsage) {
				if (tokenUsage.input !== null) {
					if (tokenUsage.inputCumulative) {
						...
						metrics.session_input_tokens =
							(metrics.session_input_tokens || 0) + tokenUsage.input;
					}
				}
				if (tokenUsage.output !== null) {
					if (tokenUsage.outputCumulative) {
						...
						metrics.session_output_tokens =
							(metrics.session_output_tokens || 0) + tokenUsage.output;
					}
				}
				if (tokenUsage.total !== null) {
					if (tokenUsage.totalCumulative) {
						...
						metrics.session_total_tokens =
							(metrics.session_total_tokens || 0) + tokenUsage.total;
					}
				} else {
					metrics.session_total_tokens =
						(metrics.session_input_tokens || 0) +
						(metrics.session_output_tokens || 0);
				}
			} else {
				metrics.session_total_tokens =
					(metrics.session_input_tokens || 0) +
			...
			if (quotaUsage) {
				if (quotaUsage.fiveHourLimitPct !== null)
					(metrics as any).five_hour_limit_pct = quotaUsage.fiveHourLimitPct;
				if (quotaUsage.weeklyLimitPct !== null)
					(metrics as any).weekly_limit_pct = quotaUsage.weeklyLimitPct;
			}
	...
	if (!isTeamWorker) {
		try {
			...
			preComputedLeaderStale = await isLeaderStale(
				stateDir,
				stalenessMs,
				Date.now(),
			);
	...
	if (!isTeamWorker && canWriteLeaderScopedState) {
		try {
			const scope = leaderWriteDecision?.scope;
			if (!scope) return;
			const hudStatePath = await getScopedStatePathAtScope(
				stateDir,
				"hud-state.json",
				scope,
			);
			let hudState = await readScopedJsonAtScope(
				stateDir,
				"hud-state.json",
				scope,
				{
	...
	if (isTeamWorker) {
		try {
			if (parsedTeamWorker) {
				const { teamName: twTeamName, workerName: twWorkerName } =
					parsedTeamWorker;
	...
	let skillSyncResult: Awaited<
		ReturnType<typeof syncSkillStateFromTurn>
	> | null = null;
	...
	if (isTeamWorker || canWriteLeaderScopedState) {
						...
						recordSkillActivation({
							...
							...(!isTeamWorker && leaderWriteDecision
								? {
										...
										onProvenanceRejected: async (
											diagnostic: import("../hooks/prompt-session-provenance.js").PromptDiagnosticDescriptor,
										) => {
											await appendPromptSessionProvenanceRejection(
												leaderWriteDecision.pointerContext,
												diagnostic,
		...
		try {
			skillSyncResult = await syncSkillStateFromTurn(
				stateDir,
				payload,
				!isTeamWorker && leaderAuthorization
					? leaderAuthorization.targetSessionId
					: "",
	...
	const deepInterviewInputLockActive = await isDeepInterviewInputLockActive(
		stateDir,
		effectiveSessionId,
	);
	...
	if (!isTeamWorker && canWriteLeaderScopedState) {
		try {
			await handleTmuxInjection({
				payload,
				cwd,
				stateDir,
				logsDir,
				context: leaderWriteDecision!.context,
			});
		} catch {
			// Non-critical
		}
	...
	if (!isTeamWorker) {
		try {
			...
			const shouldDispatchSessionIdleHookEvent =
				canWriteLeaderScopedState &&
				notifySessionId &&
				shouldSendSessionIdleHookEvent(
					stateDir,
			...
			if (shouldNotifyLifecycle || shouldDispatchSessionIdleHookEvent) {
				if (shouldNotifyLifecycle) {
					const idleResult = await notifyLifecycle(
						"session-idle",
						{
	...
	if (
		(isTeamWorker || canWriteLeaderScopedState) &&
		(!deepInterviewStateActive || deepInterviewInputLockActive)
	) {
		try {
			await maybeAutoNudge({
				cwd,
	...
	if (!isTeamWorker) {
		try {
			const { maybePersistVisualVerdict } = await import(
				"./notify-hook/visual-verdict.js"
			);
	...
	if (!isTeamWorker && canWriteLeaderScopedState) {
		try {
			const { processCodeSimplifier } = await import(
				"../hooks/code-simplifier/index.js"
			);
			const csResult = processCodeSimplifier(cwd, stateDir);
			if (csResult.triggered) {
				const managedSession = await isManagedOmxSessionAtPromptContext(
					cwd,
				...
				if (!managedSession) {
					...
					const csPaneId = await resolveNudgePaneTarget(
						stateDir,
						cwd,
						payload,
						leaderWriteDecision!.context,
