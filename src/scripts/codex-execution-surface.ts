...
function readPersistedSessionStateSync(
	cwd: string,
...
export function resolveCodexExecutionSurface(
	...
	const payloadSessionId = safeString(
		options.payload?.session_id ?? options.payload?.sessionId,
	).trim();
	const payloadSource = safeString(options.payload?.source)
		.trim()
		.toLowerCase();
	const persistedSession = readPersistedSessionStateSync(cwd);
	const persistedNativeSessionId = safeString(
		persistedSession?.native_session_id,
	).trim();
	const explicitCliSource =
		payloadSource === "cli" ||
		payloadSource === "shell" ||
		payloadSource === "terminal";
	const explicitNativeSource =
		payloadSource === "native" ||
		payloadSource === "codex-app" ||
		payloadSource === "app";
	const launcher: CodexLauncherKind =
		!explicitCliSource &&
		(explicitNativeSource ||
			(options.hookEventName === "SessionStart" &&
