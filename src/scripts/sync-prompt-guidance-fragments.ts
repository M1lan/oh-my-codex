...
async function main(): Promise<void> {
	const op = (
		await read("docs/prompt-guidance-fragments/core-operating-principles.md")
	).trim();
	const sr = (
		await read("docs/prompt-guidance-fragments/leader-specialist-routing.md")
	).trim();
	const vs = (
		await read(
	...
	const exC = (
		await read("docs/prompt-guidance-fragments/executor-constraints.md")
	).trim();
	const exO = (
		await read("docs/prompt-guidance-fragments/executor-output.md")
	).trim();
	const plC = (
		await read("docs/prompt-guidance-fragments/planner-constraints.md")
	).trim();
	const plI = (
		await read("docs/prompt-guidance-fragments/planner-investigation.md")
	).trim();
	const plO = (
		await read("docs/prompt-guidance-fragments/planner-output.md")
	).trim();
	const vfC = (
		await read("docs/prompt-guidance-fragments/verifier-constraints.md")
	).trim();
	const vfI = (
		await read("docs/prompt-guidance-fragments/verifier-investigation.md")
	).trim();
