...
function isActiveRalphCandidate(
...
function resolveActiveStateStaleThresholdMs(
	env: NodeJS.ProcessEnv = process.env,
): number {
	return (
		parsePositiveInteger(env.OMX_RALPH_ACTIVE_STATE_STALE_MS) ??
...
function stateActivityTimestampMs(
	state: Record<string, unknown>,
...
async function scanMatchingRalphCandidates(
	...
	for (const entry of entries) {
		if (
			...
			entry.name === currentOmxSessionId ||
			!allowedStorageSessionIds.includes(entry.name)
		)
			continue;
		...
		if (ownerThreadId && payloadThreadId && ownerThreadId !== payloadThreadId)
			continue;
...
export async function reconcileRalphSessionResume({
	...
	const lockedResult = await withRalphResumeLock(stateDir, async () => {
		...
		if (
			!SESSION_ID_PATTERN.test(currentOmxSessionId) ||
			!authorization.allowedStorageSessionIds.includes(currentOmxSessionId)
		) {
			return {
				currentOmxSessionId: "",
				resumed: false,
		...
		const currentOwnerCodexSessionId = safeString(
			currentRalphState?.owner_codex_session_id,
		).trim();
		if (
			currentRalphState &&
			currentOwnerCodexSessionId &&
			currentOwnerCodexSessionId !== authorization.ownerCodexSessionId
		) {
			return {
				currentOmxSessionId,
				resumed: false,
		...
		if (currentRalphState && currentRalphState.active === true) {
			const freshness = await readRalphStateFreshness(
				currentRalphPath,
				currentRalphState,
				env,
			);
			if (freshness.stale) {
				await markRalphStateAbandoned(
					currentRalphPath,
					currentRalphState,
					freshness,
				);
			...
			if (
				safeString(updated.owner_omx_session_id).trim() !== currentOmxSessionId
			) {
			...
			if (
				...
				safeString(updated.owner_codex_thread_id).trim() !==
					normalizedPayloadThreadId
			) {
		...
		if (currentRalphExists) {
			return {
				...
				resumed: false,
				updatedCurrentOwner: false,
				reason: currentRalphState
					? "current_ralph_present"
					: "current_ralph_unreadable",
				targetPath: currentRalphPath,
			};
		}
		...
		if (candidates.length !== 1) {
			return {
				...
				reason:
					candidates.length === 0
						? abandonedCount > 0
							? "matching_prior_ralph_abandoned_stale"
							: "no_matching_prior_ralph"
