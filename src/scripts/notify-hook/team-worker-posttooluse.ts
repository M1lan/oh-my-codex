...
import {
	...
	type TeamOperationalCommitKind,
} from "../../team/commit-hygiene.js";
...
const PROTECTED_PATH_PREFIXES = [".omx/state/", ".omx/logs/", ".omx/reports/"];
const PROTECTED_PATH_SUFFIXES = [".pid", ".lock", ".tmp"];
const PROTECTED_PATH_EXACT = new Set(["AGENTS.md"]);
...
function readToolUseId(payload: JsonRecord): string | undefined {
	const value = safeString(
		payload.tool_use_id ?? payload.toolUseId ?? payload.id,
	).trim();
...
async function gitMaybe(
	...
	try {
		...
		return {
			ok: false,
			stderr: safeString(err.stderr) || safeString(err.message),
		};
	}
}
...
function parsePorcelainPaths(status: string): string[] {
	return status
		...
		.map((line) => {
			...
			const renameTarget = raw.includes(" -> ")
				? raw.split(" -> ").at(-1)!
				: raw;
...
function isProtectedCheckpointPath(path: string): boolean {
	...
	if (
		normalized.includes("/posttooluse-dedupe.json") ||
		normalized.endsWith("/posttooluse-dedupe.json")
	)
		return true;
...
function readLeaderArtifactCwd(
...
async function readLeaderHeadObserved(
	config: JsonRecord | null,
	fallbackCwd: string,
): Promise<string | null> {
	const leaderCwd = safeString(config?.leader_cwd ?? config?.leaderCwd).trim();
	if (leaderCwd) {
		const head = await gitMaybe(resolve(fallbackCwd, leaderCwd), [
			"rev-parse",
			"--verify",
			"HEAD",
		]);
	...
	const value = safeString(
		config?.leader_head ?? config?.leaderHead ?? config?.leader_head_observed,
	).trim();
	return value || null;
}

async function readTeamPhaseTerminal(
	stateRoot: string,
	teamName: string,
): Promise<boolean> {
	const phase = await readJsonIfExists(
		join(stateRoot, "team", teamName, "phase.json"),
	);
	const current = safeString(phase?.current_phase ?? phase?.phase).trim();
	return (
		current === "complete" || current === "failed" || current === "cancelled"
	);
}
...
async function writeHeartbeat(
	...
	await writeFile(
		...
		JSON.stringify({ last_turn_at: nowIso, source: "posttooluse" }, null, 2),
	);
}
...
async function readDedupeMarker(path: string): Promise<DedupeMarker> {
	...
	const entries = Array.isArray(parsed?.entries)
		? parsed.entries.filter(
				(entry): entry is DedupeMarker["entries"][number] =>
					!!entry &&
					typeof entry === "object" &&
					typeof (entry as JsonRecord).dedupe_key === "string",
			)
		: [];
	return {
		version: 1,
...
async function writeDedupeMarker(
	path: string,
	params: {
		dedupeKey: string;
		createdAt: string;
	...
	const keys = marker.keys.includes(params.dedupeKey)
		? marker.keys
		: [...marker.keys, params.dedupeKey].slice(-100);
	const entries = [
...
async function classifyCleanScaffolding(
	cwd: string,
	leaderHeadObserved: string | null,
	workerHeadAfter: string | null,
): Promise<{ status: PostToolUseStatus; reason: string }> {
	...
	const leaderAncestor = await gitMaybe(cwd, [
		"merge-base",
		"--is-ancestor",
		leaderHeadObserved,
		workerHeadAfter,
	...
	const workerAncestor = await gitMaybe(cwd, [
		"merge-base",
		"--is-ancestor",
		workerHeadAfter,
		leaderHeadObserved,
	...
	if (workerAncestor.ok)
		return {
			status: "skipped",
			reason: "worker_behind_leader_defer_to_leader_integration",
		};

	return {
		status: "skipped",
		reason: "non_fast_forward_defer_to_leader_integration",
	};
}

async function unstageCheckpointablePaths(
	cwd: string,
	paths: string[],
): Promise<void> {
...
async function checkpointIfEligible(
	cwd: string,
	workerName: string,
): Promise<{
	status: PostToolUseStatus;
	...
	if (inProgress) {
		return {
			status: inProgress === "not_git_repository" ? "skipped" : "skipped",
			reason: inProgress,
			checkpointCommit: null,
			workerHeadAfter: await gitHead(cwd),
		};
	}
	...
	const checkpointable = paths.filter(
		(path) => !isProtectedCheckpointPath(path),
	);
	if (checkpointable.length === 0) {
		const onlyRuntimeArtifacts = paths.every((path) => {
			...
			return (
				...
				normalized.startsWith(".omx/logs/") ||
				normalized.startsWith(".omx/reports/")
			);
		});
		return onlyRuntimeArtifacts
			...
			: {
					status: "skipped",
					reason: "only_protected_paths_changed",
					checkpointCommit: null,
					workerHeadAfter: await gitHead(cwd),
				};
	}
	...
	if (!addResult.ok) {
		...
		return {
			status: "skipped",
			reason: `git_add_failed:${addResult.stderr.slice(0, 120)}`,
			checkpointCommit: null,
			workerHeadAfter: await gitHead(cwd),
		};
	}

	const commitResult = await gitMaybe(cwd, [
		"commit",
		"--no-verify",
		"-m",
		`omx(team): auto-checkpoint ${workerName}`,
	]);
	if (!commitResult.ok) {
		await unstageCheckpointablePaths(cwd, checkpointable);
		const reason = commitResult.stderr.includes("conflict")
			? "git_commit_conflict"
			: `git_commit_failed:${commitResult.stderr.slice(0, 120)}`;
		return {
			status: reason.includes("conflict") ? "conflict" : "skipped",
			reason,
			checkpointCommit: null,
			workerHeadAfter: await gitHead(cwd),
		};
...
export async function handleTeamWorkerPostToolUseSuccess(
	...
	try {
		...
		const parsedWorker = parseTeamWorkerEnv(
			env.OMX_TEAM_INTERNAL_WORKER ?? env.OMX_TEAM_WORKER,
		);
		if (!parsedWorker)
			return {
				handled: false,
				status: "skipped",
				reason: "missing_worker_env",
				operationKinds: [],
			};

		const resolvedStateRoot = await resolveWorkerTeamStateRoot(
			cwd,
			parsedWorker,
			env,
		);
		...
		await appendLedger({
			...
			detail: checkpoint.reason
				? `posttooluse:${checkpoint.reason}`
				: "posttooluse",
		});

		const cleanScaffolding = await classifyCleanScaffolding(
			cwd,
			leaderHeadObserved,
			workerHeadAfter,
		);
