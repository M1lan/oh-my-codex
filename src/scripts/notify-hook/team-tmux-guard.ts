...
function explicitPaneIdentity(value: any): {
	provided: boolean;
	paneId: string;
} {
	...
	return {
		provided: paneId !== "",
		paneId: EXACT_PANE_ID_RE.test(paneId) ? paneId : "",
	};
}
	...
	if (
		...
		return {
			...
			proof: {
				status: "unavailable",
				paneId: normalizeExactPaneId(exactPaneId),
				reason: !expectedOwner
					? "missing_expected_pane_owner"
					: "missing_expected_pane_pid",
			},
		};
	}
...
function exactPaneUnavailableResult(
	target: string,
	paneProof: any,
	extra: any = {},
): any {
...
export async function evaluatePaneInjectionReadiness(
	paneTarget: any,
	{
	...
	const normalizedRequireObservableState =
		typeof requireCaptureEvidence === "boolean"
			? requireCaptureEvidence
			: requireObservableState;
	...
	const verifyExplicitPane = async () => {
		...
		exactPaneProof = paneProof.proof || null;
		if (paneProof.ok && typeof paneProof.proof?.pid === "number")
			pinnedPanePid ??= paneProof.proof.pid;
		return paneProof;
	};
	...
	const exactPaneFailure = (paneProof: any, paneCapture = "") =>
		exactPaneUnavailableResult(target, paneProof, {
			paneCurrentCommand,
			paneCapture,
	...
	if (exactPaneIdentityProvided) {
		const read = await readExplicitPane([
			"display-message",
			"-p",
			"-t",
			target,
		...
		if (!read.ok) {
			...
			return buildReadinessResult(
				false,
				PANE_READINESS_UNVERIFIED_REASON,
				"",
...

	{
		...
		if (!read.ok) {
			...
			paneRunningShell =
				requireRunningAgent && isPaneRunningShell(paneCurrentCommand);
		}
	}

	{
		const read = await readExplicitPane(
			buildCapturePaneArgv(target, captureLines),
		);
		if (!read.ok) {
			if (read.paneProof) return exactPaneFailure(read.paneProof);
			if (exactPaneIdentityProvided) {
			...
			return buildReadinessResult(
				...
				"ok",
				"",
				paneCurrentCommand ? "command_only" : "none",
			);
		}
		const paneCapture = safeString(read.result?.stdout);
		const hasCaptureEvidence = paneCapture.trim() !== "";
		if (hasCaptureEvidence) {
			const paneShowsLiveAgent =
				paneLooksReady(paneCapture) || paneHasActiveTask(paneCapture);
		...
		if (
			normalizedRequireObservableState &&
			!hasCaptureEvidence &&
			!paneCurrentCommand
		) {
			return buildReadinessResult(
...
export async function sendPaneInput({
	...
	let pinnedPanePid =
		typeof expectedPanePid === "number" ? expectedPanePid : undefined;
	const verifyExplicitPane = async () => {
		const paneProof = await verifyExactPaneLive(
			exactPaneIdentity,
			pinnedPanePid,
		);
		exactPaneProof = paneProof.proof || null;
	...
	try {
		const startCommandResult = await runProcess(
			"tmux",
			["display-message", "-p", "-t", target, "#{pane_start_command}"],
			3000,
		);
		if (
			/\bomx\b.*\bhud\b.*--watch/i.test(safeString(startCommandResult.stdout))
		) {
			return {
				ok: false,
				sent: false,
	...
	try {
		if (typePrompt) {
			...
			let verifiedBuffer;
			try {
				verifiedBuffer = await runProcess(
					"tmux",
					pasteArgv.showBufferArgv,
					3000,
				);
			} catch (error) {
				return {
			...
			if (!clearProof.ok)
				return exactPaneUnavailableResult(target, clearProof, { argv });
			try {
			...
			if (!pasteProof.ok)
				return exactPaneUnavailableResult(target, pasteProof, { argv });
			try {
		...
		if (queueFirstSubmit && argv.submitArgv.length > 0) {
			...
			if (!queueProof.ok)
				return exactPaneUnavailableResult(target, queueProof, { argv });
		...
		for (const submit of argv.submitArgv) {
			...
			if (!submitProof.ok)
				return exactPaneUnavailableResult(target, submitProof, { argv });
...
export async function queuePaneInput({
	...
	let expectedPanePid =
		typeof sendResult.exactPaneProof?.pid === "number"
			? sendResult.exactPaneProof.pid
			: undefined;
	const verifyExplicitPane = async () => {
		const paneProof = await verifyExactPaneLive(
			exactPaneIdentity,
		...
		exactPaneProof = paneProof.proof || null;
		if (paneProof.ok && typeof paneProof.proof?.pid === "number")
			expectedPanePid ??= paneProof.proof.pid;
		return paneProof;
	};
	const submitArgv = [
...
export async function checkPaneReadyForTeamSendKeys(
	paneTarget: any,
	exactPaneId: any,
): Promise<any> {
