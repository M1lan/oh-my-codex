...
async function initWorkerFixture(): Promise<{
	...
	await writeFile(
		join(stateRoot, "team", "demo-team", "phase.json"),
		JSON.stringify({ current_phase: "team-exec" }, null, 2),
		"utf-8",
	);
	await writeFile(
		join(stateRoot, "team", "demo-team", "config.json"),
		JSON.stringify({ leader_cwd: cwd }, null, 2),
		"utf-8",
	);
	return {
		cwd,
...
describe("handleTeamWorkerPostToolUseSuccess", () => {
	it("creates a safe worker checkpoint, ledger entries, leader signal, and dedupe marker", async () => {
		...
		const events = await readTeamEvents("demo-team", fixture.cwd, {
	...
	it("does not checkpoint its own runtime report artifacts on a repeated PostToolUse turn", async () => {
		...
		const first = await handleTeamWorkerPostToolUseSuccess(
			successPayload,
			fixture.cwd,
			fixture.env,
		);
		...
		const second = await handleTeamWorkerPostToolUseSuccess(
