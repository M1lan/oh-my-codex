...
export async function readNotifySessionMetadata(
	baseStateDir: string,
): Promise<SessionMetadata> {
	return readSessionMetadataFromBaseStateDir(baseStateDir);
}

function readOmxSessionIdFromEnvironment(
...
export async function hasExistingScopedSessionDir(
	baseStateDir: string,
	sessionId: string,
): Promise<boolean> {
	try {
		return (
			await stat(join(baseStateDir, "sessions", sessionId))
		).isDirectory();
	} catch {
		return false;
	}
...
async function resolveBaseScopedStateDirs(
	...
	const scopedDir = await resolveBaseScopedStateDir(
		baseStateDir,
		explicitSessionId,
	);
...
export async function getScopedStatePath(
	...
	return join(
		await resolveScopedStateDir(baseStateDir, explicitSessionId),
		fileName,
	);
}
...
export async function writeScopedJson(
	...
	const targetPath = await getScopedStatePath(
		baseStateDir,
		fileName,
		explicitSessionId,
	);
...
export async function readScopedJsonAtScope(
	...
	return readJsonIfExists(
		await getScopedStatePathAtScope(baseStateDir, fileName, scope),
		fallback,
	);
}
...
export async function writeScopedJsonAtScope(
	...
	const targetPath = await getScopedStatePathAtScope(
		baseStateDir,
		fileName,
		scope,
	);
...
export function normalizeTmuxState(raw: any): any {
	...
	return {
		...
		pane_counts:
			raw.pane_counts && typeof raw.pane_counts === "object"
				? raw.pane_counts
				: {},
		session_counts:
			raw.session_counts && typeof raw.session_counts === "object"
				? raw.session_counts
				: {},
		recent_keys:
			raw.recent_keys && typeof raw.recent_keys === "object"
				? raw.recent_keys
				: {},
...
export function normalizeNotifyState(raw: any): any {
	...
	return {
		recent_turns:
			raw.recent_turns && typeof raw.recent_turns === "object"
				? raw.recent_turns
				: {},
		last_event_at: safeString(raw.last_event_at),
	};
...
export function pruneRecentTurns(
	recentTurns: any,
	now: number,
...
export function pruneRecentKeys(
	recentKeys: any,
	now: number,
): Record<string, number> {
	const pruned: Record<string, number> = {};
