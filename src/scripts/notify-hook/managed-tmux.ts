...
export async function resolveManagedSessionContext(
	...
	try {
		...
		const currentTmuxPaneTarget = safeString(
			paneTarget || process.env.TMUX_PANE || "",
		).trim();
		const currentTmuxPaneInstanceId = currentTmuxPaneTarget
			? await readTmuxPaneInstanceId(currentTmuxPaneTarget)
			: "";
		if (
		...
		const currentTmuxInstanceId = currentTmuxSessionName
