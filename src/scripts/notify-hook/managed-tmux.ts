...
export function buildExpectedManagedTmuxSessionName(
	cwd: string,
	sessionId: string,
): string {
...
function readNativeSessionId(sessionState: {
	...
	return safeString(
		sessionState.native_session_id || sessionState.codex_session_id || "",
	).trim();
}

function readAuthoritativeTmuxSessionName(sessionState: {
	tmux_session_name?: unknown;
	tmuxSessionName?: unknown;
}): string {
	return safeString(
		sessionState.tmux_session_name || sessionState.tmuxSessionName || "",
	).trim();
}
...
async function probeTmuxOption(
	targetValue: string,
	optionName: string,
	{ pane = false } = {},
): Promise<TmuxOptionProbe> {
...
async function readTmuxSessionInstanceId(
...
export async function probeActualTmuxInstanceEvidence(
	paneTarget?: string,
): Promise<ActualTmuxInstanceEvidence> {
	const resolvedPaneTarget = safeString(
	...
	if (resolvedPaneTarget) {
		try {
			const result = await runProcess(
				"tmux",
				["display-message", "-p", "-t", resolvedPaneTarget, "#S"],
				2000,
			);
	...
	const paneProbe = resolvedPaneTarget
		? await probeTmuxOption(resolvedPaneTarget, OMX_PANE_INSTANCE_OPTION, {
				pane: true,
			})
...
export async function resolveTmuxSessionForInstance(
	instanceId: string,
): Promise<string> {
	...
	try {
		const result = await runProcess(
			"tmux",
			["list-sessions", "-F", `#{session_name}\t#{${OMX_INSTANCE_OPTION}}`],
			2000,
		);
		const rows = safeString(result.stdout)
			.split("\n")
			.map((line) => line.trim())
			.filter(Boolean);
...
function processHasAncestorPid(
	targetPid: number,
	currentPid = process.pid,
): boolean {
	if (!Number.isInteger(targetPid) || targetPid <= 1) return false;
	let pid =
		Number.isInteger(currentPid) && currentPid > 1 ? currentPid : process.pid;
...
export async function resolveManagedSessionContext(
	...
	payload: any,
	{
		allowTeamWorker = true,
		paneTarget = "",
	}: { allowTeamWorker?: boolean; paneTarget?: string } = {},
): Promise<any> {
	if (
		allowTeamWorker &&
	...
	try {
		...
		const allowedInvocationIds = new Set(
			[canonicalSessionId, nativeSessionId].filter(Boolean),
		);
		...
		const authoritativeSessionCwd =
			safeString(sessionState.cwd || cwd).trim() || cwd;
		const authoritativeTmuxSessionName =
			readAuthoritativeTmuxSessionName(sessionState);
		const expectedTmuxSessionName =
			authoritativeTmuxSessionName ||
		...
		const currentTmuxSessionName =
			evidence.sessionName || readCurrentTmuxSessionName();
		...
		if (
			...
			currentTmuxPaneInstanceId !== invocationSessionId
		) {
			return {
				managed: false,
		...
		const currentTmuxInstanceId =
			evidence.sessionName === currentTmuxSessionName
		...
		if (
			...
			currentTmuxInstanceId !== invocationSessionId
		) {
			return {
				managed: false,
		...
		if (currentTmuxInstanceId === invocationSessionId) {
			if (currentTmuxPaneTarget)
				warnPaneInstanceFallback(currentTmuxPaneTarget);
			return {
				managed: true,
		...
		const taggedTmuxSessionName =
			await resolveTmuxSessionForInstance(invocationSessionId);
		...
		if (
			currentTmuxSessionName &&
			currentTmuxSessionName === expectedTmuxSessionName
		) {
			return {
				managed: true,
		...
		if (processHasAncestorPid(sessionState.pid)) {
			return {
				managed: true,
				reason: currentTmuxSessionName
					? "pid_ancestry_match_tmux_mismatch"
					: "pid_ancestry_match",
				invocationSessionId,
				canonicalSessionId,
				nativeSessionId,
...
export async function isManagedOmxSession(
	cwd: string,
	payload: any,
...
export async function isManagedOmxSessionAtPromptContext(
	...
	return isManagedOmxSession(
		cwd,
		{ session_id: context.authorization.ownerCodexSessionId },
		options,
	);
}

export async function verifyManagedPaneTargetAtPromptContext(
	paneId: string,
	cwd: string,
	context: ResolvedPromptTurnContext,
): Promise<any> {
	if (context.status !== "authorized")
		return {
			ok: false,
			reason: "prompt_context_unauthorized",
			paneTarget: safeString(paneId).trim(),
		};
	return verifyManagedPaneTarget(
		paneId,
		cwd,
		{ session_id: context.authorization.ownerCodexSessionId },
...
export async function resolveManagedCurrentPaneAtPromptContext(
	cwd: string,
	context: ResolvedPromptTurnContext,
): Promise<string> {
	if (context.status !== "authorized") return "";
	return resolveManagedCurrentPane(
		cwd,
		{ session_id: context.authorization.ownerCodexSessionId },
		{ allowTeamWorker: false },
	);
...
export async function resolveManagedSessionPaneAtPromptContext(
	cwd: string,
	context: ResolvedPromptTurnContext,
): Promise<string> {
	if (context.status !== "authorized") return "";
	return resolveManagedSessionPane(cwd, {
		session_id: context.authorization.ownerCodexSessionId,
	});
}

export async function resolveManagedPaneFromAnchorAtPromptContext(
	anchorPane: string,
	cwd: string,
	context: ResolvedPromptTurnContext,
): Promise<string> {
	if (context.status !== "authorized") return "";
	return resolveManagedPaneFromAnchor(
		anchorPane,
		cwd,
		{ session_id: context.authorization.ownerCodexSessionId },
		{ allowTeamWorker: false },
...
export async function verifyManagedPaneTarget(
	paneId: string,
	cwd: string,
	payload: any,
	{ allowTeamWorker = true } = {},
	...
	const managedContext = await resolveManagedSessionContext(cwd, payload, {
		allowTeamWorker,
		paneTarget,
	});
	if (!managedContext.managed) {
		return {
			ok: false,
			reason: managedContext.reason || "unmanaged_session",
			paneTarget,
			managedContext,
		};
	}
	...
	const expectedSession = safeString(
		managedContext.expectedTmuxSessionName,
	).trim();
	if (!expectedSession) {
		return {
			ok: false,
			reason: "missing_expected_tmux_session",
			paneTarget,
			managedContext,
		};
	}
	...
	try {
		let paneSessionName = safeString(
			managedContext.currentTmuxSessionName,
		).trim();
		if (!paneSessionName) {
		...
		if (!paneSessionName) {
			return {
				ok: false,
				reason: "pane_session_missing",
				paneTarget,
				managedContext,
			};
		}
		const paneInstanceId =
			safeString(managedContext.currentTmuxPaneInstanceId).trim() ||
		...
		if (
			paneInstanceId &&
			paneInstanceId !== managedContext.invocationSessionId
		) {
			return {
				ok: false,
				reason: "pane_instance_mismatch",
		...
		if (paneSessionName !== expectedSession) {
			const taggedSession = safeString(
				managedContext.taggedTmuxSessionName,
			).trim();
			if (taggedSession && paneSessionName === taggedSession) {
				return {
					...
					paneInstanceWarning: paneInstanceId
						? ""
						: "missing_pane_instance_tag_session_fallback",
					managedContext,
				};
			}
			return {
				ok: false,
				reason: "pane_not_managed_session",
				paneTarget,
				paneSessionName,
				managedContext,
			};
		...
		return {
			ok: false,
			reason: "pane_session_lookup_failed",
			paneTarget,
			managedContext,
		};
	}
...
async function readManagedPaneCommandState(paneTarget: string): Promise<{
	...
	try {
		const [currentResult, startResult] = await Promise.all([
			runProcess(
				"tmux",
				["display-message", "-p", "-t", paneTarget, "#{pane_current_command}"],
				2000,
			),
			runProcess(
				"tmux",
				["display-message", "-p", "-t", paneTarget, "#{pane_start_command}"],
				2000,
			),
		]);
		return {
...
function paneLooksLikeManagedAgent({
	currentCommand,
	startCommand,
}: {
	currentCommand: string;
...
function paneLooksLikeRetainableManagedAnchor({
	currentCommand,
	startCommand,
}: {
	currentCommand: string;
	...
	if (
		(currentCommand === "node" || currentCommand === "npx") &&
		startCommand.includes("codex")
	)
		return true;
	return false;
}

function paneLooksLikeDetachedManagedWrapperFallback({
	currentCommand,
	startCommand,
}: {
	currentCommand: string;
...
function selectManagedSessionPane(
	...
	const nonHudRows = rows.filter(
		(row) => !/\bomx\b.*\bhud\b.*--watch/i.test(row.startCommand),
	);
	const canonicalRows = nonHudRows.filter((row) =>
		paneLooksLikeRetainableManagedAnchor(row),
	);
	...
	const wrapperFallbackRows = nonHudRows.filter((row) =>
		paneLooksLikeDetachedManagedWrapperFallback(row),
	);
...
export async function resolveManagedCurrentPane(
	cwd: string,
	payload: any,
	{ allowTeamWorker = false } = {},
): Promise<string> {
	...
	const verdict = await verifyManagedPaneTarget(paneTarget, cwd, payload, {
		allowTeamWorker,
	});
...
export async function resolveManagedSessionPane(
	cwd: string,
	payload: any,
): Promise<string> {
	const managedContext = await resolveManagedSessionContext(cwd, payload, {
	...
	try {
		...
		return selectManagedSessionPane(
			parseManagedSessionPaneRows(panesResult.stdout),
		);
	} catch {
		// best effort only
	}
...
export async function resolveManagedPaneFromAnchor(
	anchorPane: string,
	cwd: string,
	payload: any,
	{ allowTeamWorker = false } = {},
	...
	try {
		const sessionName = safeString(
			verdict.paneSessionName ||
				verdict.managedContext?.expectedTmuxSessionName,
		).trim();
		...
		const selectedPane = selectManagedSessionPane(
			parseManagedSessionPaneRows(panesResult.stdout),
			{
				allowWrapperFallback:
					paneLooksLikeDetachedManagedWrapperFallback(commandState),
			},
		);
