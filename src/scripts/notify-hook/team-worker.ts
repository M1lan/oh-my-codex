...
export async function readTeamWorkersForIdleCheck(stateDir, teamName) {
	...
	for (const srcPath of candidatePaths) {
		try {
			...
			const tmuxPaneOwnerId = safeString(
				parsed.tmux_pane_owner_id || "",
			).trim();
			const result = {
				workers,
				tmuxSession,
...
async function checkLeaderPaneReadyForWorkerStateReminder(
	paneTarget,
	exactPaneId,
	expectedPanePid,
	expectedPaneOwnerId,
...
export async function maybeNotifyLeaderAllWorkersIdle({
	...
	const resolvedLeaderPaneId = await resolveCanonicalLeaderPaneId(
		tmuxSession,
		leaderPaneId,
	);
	const canonicalLeaderPaneId =
		resolvedLeaderPaneId &&
		resolvedLeaderPaneId !== normalizeExactPaneId(hudPaneId)
			? resolvedLeaderPaneId
			: "";
