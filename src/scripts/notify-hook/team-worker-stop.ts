	...
	for (let attempt = 0; attempt < 2; attempt += 1) {
		try {
			...
			if (error?.code === "ENOENT")
				return { acquired: false, reason: TEAM_SHUTDOWN_NO_INJECTION_REASON };
...
export async function maybeNudgeLeaderForAllowedWorkerStop({
	...
	if (!teamName || !workerName || !stateDir)
		return { ok: false, result: "unresolved" };
	...
	try {
		...
		const leaderPanePid =
			Number.isInteger(teamInfo.leaderPanePid) &&
			Number(teamInfo.leaderPanePid) > 0
				? Number(teamInfo.leaderPanePid)
				: undefined;
		const leaderPaneOwnerId = safeString(teamInfo.tmuxPaneOwnerId).trim();
		const resolvedLeaderPaneId =
			await resolveCanonicalLeaderPaneId(leaderPaneId);
		const tmuxTarget =
			resolvedLeaderPaneId &&
			resolvedLeaderPaneId !== normalizeExactPaneId(teamInfo.hudPaneId)
				? resolvedLeaderPaneId
				: "";
		...
		const wroteWorkerState = await writeStopNudgeStateIfTeamExists(
			teamDir,
			statePath,
			deliveryState,
		);
		const wroteTeamState = wroteWorkerState
			? await writeStopNudgeStateIfTeamExists(
					teamDir,
					teamStatePath,
					deliveryState,
				)
