...
import {
	...
	mapPaneInjectionReadinessReason,
	sendPaneInput,
...
import {
	...
	DEFAULT_MARKER,
	tmuxHookExplicitlyDisablesInjection,
...
export const DEFAULT_AUTO_NUDGE_RESPONSE =
	"continue with the current task only if it is already authorized";
const DEEP_INTERVIEW_ERROR_PATTERNS = [
	" error",
	" failed",
	" failure",
	" exception",
...
const DEEP_INTERVIEW_SUCCESS_PATTERNS = [
	"interview completed",
	"interview complete",
	"interview finished",
	"final summary ready",
...
function buildBlockedAutoApprovalMatcher(blockedInputs) {
	const normalizedBlockedInputs = blockedInputs
		.map((entry) => normalizeBlockedAutoApprovalInput(entry))
		.filter(Boolean);
	return {
		exactMatches: new Set(normalizedBlockedInputs),
		prefixedMatches: normalizedBlockedInputs.filter((entry) =>
			DEEP_INTERVIEW_BLOCKED_APPROVAL_PREFIXES.has(entry),
		),
		blockedTokenSet: new Set(
			normalizedBlockedInputs.flatMap((entry) =>
...
export function isBlockedAutoApprovalInput(
	text,
	blockedInputs = DEEP_INTERVIEW_BLOCKED_APPROVAL_INPUTS,
) {
	const normalized = normalizeBlockedAutoApprovalInput(text);
	if (!normalized) return false;
	const { exactMatches, prefixedMatches, blockedTokenSet } =
		buildBlockedAutoApprovalMatcher(blockedInputs);
...
function isDeepInterviewAbortInput(text) {
	return DEEP_INTERVIEW_ABORT_INPUTS.has(
		normalizeBlockedAutoApprovalInput(text),
	);
}
...
export function isDeepInterviewAutoApprovalLocked(skillState) {
	return Boolean(
		skillState &&
			...
			(safeString(skillState.input_lock.scope) === "" ||
				skillState.input_lock.scope === "deep-interview-auto-approval") &&
			skillState.input_lock.active === true,
	);
}

export function inferDeepInterviewReleaseReason({
	skillState,
	latestUserInput = "",
	lastMessage = "",
}) {
	...
	if (
		isDeepInterviewAbortInput(latestUserInput) ||
		hasAnySubstring(lastMessage, DEEP_INTERVIEW_ABORT_PATTERNS)
	) {
		return "abort";
	}
	if (
		hasAnySubstring(
			` ${safeString(lastMessage).toLowerCase()}`,
			DEEP_INTERVIEW_ERROR_PATTERNS,
		)
	) {
		return "error";
	}
...
async function loadSkillActiveState(stateDir, sessionId) {
	const raw = await readScopedJsonIfExists(
		stateDir,
...
export async function syncSkillStateFromTurn(
	stateDir,
	payload,
	invocationSessionIdOverride = "",
	authorization: PromptMutationAuthorization | null = null,
) {
	...
	const invocationSessionId =
		invocationSessionIdOverride || resolveInvocationSessionId(payload);
	...
	if (authorization) {
		const allowedOwners = Array.isArray(
	...
	const explicitDeepInterviewSuccess =
		skillState.skill === "deep-interview" &&
		looksLikeDeepInterviewSuccess(lastMessage);
	const nextPhase =
		skillState.skill === "deep-interview" &&
	...
	if (skillState.skill === "autoresearch") {
		...
		if (completion.complete) {
			...
			skillState.phase =
				previousPhase === "completing" ? "reviewing" : previousPhase;
	...
	skillState.updated_at = nowIso;
	releaseReason = inferDeepInterviewReleaseReason({
		skillState,
		latestUserInput,
		lastMessage,
	});
	...
	if (
		skillState.skill === "deep-interview" ||
		previousSkillState?.skill === "deep-interview"
	) {
...
export async function resolveAutoNudgeSignature(
	stateDir,
	payload,
	lastMessage = "",
) {
	...
	const hudTurnCount = Number.isFinite(hudState?.turn_count)
		? hudState.turn_count
		: null;
	const hudMessage = normalizeAutoNudgeSignatureText(
...
function latestUserInputFromPayload(payload) {
	const inputMessages =
		payload["input-messages"] || payload.input_messages || [];
...
export function normalizeAutoNudgeSignatureText(text) {
	...
	if (detectStallPattern(normalized, DEFAULT_STALL_PATTERNS)) {
		...
		semantic = semantic
			.replace(
				/\b(?:please|just|simply|the|a|an|this|that|these|those|for|from|here|there|now|then|when|you|me|i|can|could|will|would|should|shall|to|with|on|if|also|know)\b/g,
				" ",
			)
			.replace(/\s+/g, " ")
			.trim();
		return semantic.includes("proceed_intent")
			? "stall:proceed_intent"
			: `stall:${semantic || "detected"}`;
	}
...
function normalizePatternList(patterns) {
	return patterns
		.map((pattern) => normalizeStallDetectionText(pattern))
		.filter(Boolean);
}
...
function usesDefaultStallPatterns(patterns) {
	...
	return (
		normalizedPatterns.length === normalizedDefaults.length &&
		normalizedPatterns.every(
			(pattern, index) => pattern === normalizedDefaults[index],
		)
	);
}
...
function looksLikePlanningOnlyContinuation(normalizedText) {
	return matchesNormalizedPatterns(
		normalizedText,
		normalizePatternList(PLANNING_ONLY_STALL_PATTERNS),
	);
}
...
function looksLikePermissionSeekingContinuation(normalizedText) {
	return matchesNormalizedPatterns(
		normalizedText,
		normalizePatternList(PERMISSION_SEEKING_STALL_PATTERNS),
	);
}
...
export function normalizeAutoNudgeConfig(raw) {
	...
	return {
		...
		ttlMs:
			...
			raw.ttlMs <= 10 * 60_000
				? raw.ttlMs
				: typeof raw.cooldownMs === "number" &&
						raw.cooldownMs >= 0 &&
						raw.cooldownMs <= 10 * 60_000
					? raw.cooldownMs
...
export function resolveEffectiveAutoNudgeResponse(response) {
	...
	return isBlockedAutoApprovalInput(normalized)
...
async function localTmuxInjectionDisabled(cwd) {
	...
	const raw = await readJsonIfExists(
		join(normalizedCwd, ".omx", "tmux-hook.json"),
		null,
	);
	return tmuxHookExplicitlyDisablesInjection(raw);
}

function detectStallPatternWithOptions(
	text,
	patterns,
	currentPhase = "",
	options = {},
	...
	if (
		options.allowPermissionSeeking !== true &&
		looksLikePermissionSeekingContinuation(normalized)
	)
		return false;
...
export function detectNativeStopStallPattern(
	text,
	patterns,
	currentPhase = "",
) {
...
async function resolveTeamWorkerNudgeBinding(stateDir: string): Promise<{
	...
	const match = /^([A-Za-z0-9][A-Za-z0-9_-]*)\/(worker-[1-9]\d*)$/.exec(
		workerContext,
	);
	...
	try {
		...
		if (
			safeString(raw?.name).trim() !== teamName ||
			!Array.isArray(raw?.workers)
		)
			return null;
		const worker = raw.workers.find(
			(entry: any) => safeString(entry?.name).trim() === workerName,
		);
		...
		const paneOwnerId =
			typeof raw?.tmux_pane_owner_id === "string"
				? raw.tmux_pane_owner_id.trim()
				: "";
		if (
			!/^%\d+$/.test(paneId) ||
...
export async function resolveNudgePaneTarget(
	stateDir: any,
	cwd = "",
	payload: any = undefined,
	context: ResolvedPromptTurnContext | null = null,
) {
		...
		for (const f of files) {
			...
			try {
				...
				const managedPane = context
					? await resolveManagedPaneFromAnchorAtPromptContext(
							anchoredPane,
							cwd,
							context,
						)
					: await resolveManagedPaneFromAnchor(anchoredPane, cwd, payload, {
							allowTeamWorker,
						});
				if (managedPane) return managedPane;
				if (allowTeamWorker && !context) {
					const verdict = await verifyManagedPaneTarget(
						anchoredPane,
						cwd,
						payload,
						{ allowTeamWorker },
...
export async function maybeAutoNudge({
	cwd,
	stateDir,
	logsDir,
	payload,
	...
	const managedSession = context
		? await isManagedOmxSessionAtPromptContext(cwd, context, {
	...
	const { invocationSessionId, skillState, releaseReason } =
		syncResult ||
		(context?.status === "authorized"
			? {
					invocationSessionId: context.authorization.targetSessionId,
					skillState: null,
					releaseReason: null,
				}
	...
	try {
		const nudgeStatePath = await getScopedStatePath(
			stateDir,
			"auto-nudge-state.json",
			invocationSessionId,
		);
		let nudgeState = await readScopedJsonIfExists(
			stateDir,
			"auto-nudge-state.json",
			invocationSessionId,
			null,
		...
		let detected = detectStallPattern(
			lastMessage,
			config.patterns,
			skillState?.phase,
		);
		...
		if (!detected && paneId) {
			...
			detected = detectStallPattern(
				filterCapturedTestLines(captured),
				config.patterns,
				skillState?.phase,
			);
			source = "capture-pane";
		...
		const signatureSourceText =
			source === "capture-pane" ? captured : lastMessage;
		...
		const semanticSignature =
			normalizeAutoNudgeSignatureText(signatureSourceText);
		...
		const isFallbackWatcherSource =
			sourceName === "notify-fallback-watcher-stall";
		...
		const pinnedPaneOptions = {
			...teamPaneOptions,
			expectedPanePid: readinessPanePid,
		};

		const blockedAutoApproval =
			isDeepInterviewAutoApprovalLocked(skillState) &&
			!releaseReason &&
		...
		if (blockedAutoApproval) {
			const blockedMessage =
				skillState.input_lock?.message || DEEP_INTERVIEW_INPUT_LOCK_MESSAGE;
			await logTmuxHookEvent(logsDir, {
				timestamp: new Date().toISOString(),
		...
		if (
			...
			releaseDeepInterviewInputLock(
				skillState,
				releaseReason,
				new Date().toISOString(),
			);
			await persistSkillActiveState(
				stateDir,
				invocationSessionId,
				skillState,
			).catch(() => {});
