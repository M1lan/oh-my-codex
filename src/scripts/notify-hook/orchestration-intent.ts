...
export function buildOrchestrationIntentTag(intent) {
	...
	return normalizedIntent
		? `${ORCHESTRATION_INTENT_TAG_PREFIX}${normalizedIntent}]`
		: "";
}
...
export function classifyLeaderActionState({
	...
	const blockedWaitingOnLeader =
		allWorkersIdle && blocked > 0 && pending === 0 && inProgress === 0;
	const terminalWaitingOnLeader =
		allWorkersIdle && tasksComplete && workerPanesAlive;
...
export function resolveAllWorkersIdleIntent(
