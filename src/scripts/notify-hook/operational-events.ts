...
function buildTmuxSessionName(cwd: any, sessionId: any): string {
	...
	const sessionToken = sanitizeTmuxToken(
		safeString(sessionId).replace(/^omx-/, ""),
	);
...
export function resolveOperationalSessionName(
	cwd: any,
	sessionId = "",
	sessionName = "",
): string | undefined {
...
export function readRepositoryMetadata(cwd: any): any {
	...
	const repoPath =
		gitValue(worktreePath, ["rev-parse", "--show-toplevel"]) || worktreePath;
	const branch =
		gitValue(worktreePath, ["rev-parse", "--abbrev-ref", "HEAD"]) || undefined;
	return {
		repo_path: repoPath,
...
export function extractErrorSummary(
	...
	const preferred = [...lines]
		.reverse()
		.find((line: string) =>
			/(error|failed|exception|invalid|timed out|timeout)/i.test(line),
		);
...
export function parseExecCommandArgs(rawArguments: any): {
	command: string;
	workdir: string;
} {
	try {
...
export function buildOperationalContext({
	...
	const resolvedSessionName = resolveOperationalSessionName(
		cwd,
		sessionId,
		sessionName,
	);
	...
	return {
		...
		...(detectedPrInfo.pr_number !== undefined
			? { pr_number: detectedPrInfo.pr_number }
			: {}),
