...
function runtimeExec(command, stateDir, team) {
	...
	try {
		...
		execFileSync(
			binaryPath,
			["exec", JSON.stringify(command), `--state-dir=${stateDir}`],
			{
				timeout: 5000,
...
function recordBridgeFallback({
	...
	try {
		process.emitWarning(
			`[omx] team-dispatch bridge fallback: ${event.bridge_operation} -> ${event.fallback_target}: ${event.reason}`,
			{
	...
	return parsed.records
		.map((record) => {
			...
			return {
				...
				worker_index:
					typeof metadata.worker_index === "number"
						? metadata.worker_index
						: undefined,
				pane_id: safeString(metadata.pane_id).trim() || undefined,
				trigger_message:
					safeString(metadata.trigger_message).trim() ||
					safeString(record.reason).trim() ||
					safeString(record.request_id).trim(),
				message_id: safeString(metadata.message_id).trim() || undefined,
				inbox_correlation_key:
					safeString(metadata.inbox_correlation_key).trim() || undefined,
				transport_preference:
					safeString(metadata.transport_preference).trim() ||
				...
				attempt_count: Number.isFinite(metadata.attempt_count)
					? Number(metadata.attempt_count)
					: 0,
				created_at:
					safeString(record.created_at).trim() || new Date().toISOString(),
				updated_at:
...
function dispatchRequestLeaseDir(teamDirPath, requestId) {
	return join(
		teamDirPath,
		"dispatch",
		`.processing-${safeString(requestId).trim()}`,
	);
}
...
function serializeDispatchRequestRecord(request) {
	return {
		...
		created_at:
			safeString(request.created_at).trim() || new Date().toISOString(),
		...
		metadata: {
			...
			worker_index: Number.isFinite(request.worker_index)
			...
			inbox_correlation_key:
				safeString(request.inbox_correlation_key).trim() || undefined,
			transport_preference:
				safeString(request.transport_preference).trim() ||
				"hook_preferred_with_fallback",
			fallback_allowed:
				typeof request.fallback_allowed === "boolean"
					? request.fallback_allowed
					: true,
			attempt_count: Number.isFinite(request.attempt_count)
...
async function writeBridgeDispatchCompat(stateDir, teamName, requests) {
	...
	const records = [
		...otherTeams,
		...requests.map(serializeDispatchRequestRecord),
	];
...
function explicitPaneIdentity(value) {
	...
	return {
		provided: rawPaneId !== "",
		paneId: normalizeExactPaneId(rawPaneId),
		rawPaneId,
	};
}
...
function resolveAddressedWorker(request, config) {
	...
	if (Number.isFinite(request?.worker_index)) {
		const worker = workers.find(
			(candidate) => Number(candidate?.index) === Number(request.worker_index),
		);
	...
	return (
		workers.find(
			(candidate) => safeString(candidate?.name).trim() === recipient,
		) || null
	);
}
...
function resolveConfiguredPaneIdentity(request, config) {
	if (request.to_worker === "leader-fixed") {
		return {
			source: "leader_pane_id",
			expectedPanePid: positivePanePid(config?.leader_pane_pid),
			expectedPaneOwnerId: safeString(config?.tmux_pane_owner_id).trim(),
			...explicitPaneIdentity(config?.leader_pane_id),
		};
	}
	const worker = resolveAddressedWorker(request, config);
	const expectedPanePid = positivePanePid(worker?.pid);
	return {
		source: "worker_pane_id",
		expectedPanePid,
		expectedPaneOwnerId: safeString(config?.tmux_pane_owner_id).trim(),
		...explicitPaneIdentity(worker?.pane_id),
	};
}
...
function resolveDispatchTarget(request, config) {
	...
	if (requestPane.provided) {
		if (!requestPane.paneId) {
			return {
				failure: "invalid_explicit_pane",
				paneId: requestPane.paneId,
				paneSource: "request_pane_id",
				rawPaneId: requestPane.rawPaneId,
			};
		}
		if (requestPane.paneId === hudPaneId) {
			return {
				failure: "hud_pane_target",
				paneId: requestPane.paneId,
				paneSource: "request_pane_id",
			};
		}
		if (configuredPane.provided) {
			if (!configuredPane.paneId) {
				return {
					failure: "invalid_explicit_pane",
					paneId: configuredPane.paneId,
					paneSource: configuredPane.source,
					rawPaneId: configuredPane.rawPaneId,
				};
			}
		...
		if (!positivePanePid(configuredPane.expectedPanePid)) {
			return {
				failure: "missing_exact_pane_pid",
				paneId: requestPane.paneId,
				paneSource: configuredPane.source,
			};
		}
		if (!configuredPane.expectedPaneOwnerId) {
			return {
				failure: "missing_expected_pane_owner",
				paneId: requestPane.paneId,
				paneSource: configuredPane.source,
			};
		}
		return {
			target: { type: "pane", value: requestPane.paneId },
			exactPaneId: requestPane.paneId,
			expectedPanePid: configuredPane.expectedPanePid,
			expectedPaneOwnerId: configuredPane.expectedPaneOwnerId,
			expectedHudPaneId: hudPaneId,
			source: "request_pane_id",
			reason: "explicit_request_pane_id",
		};
	}
	if (configuredPane.provided) {
		if (!configuredPane.paneId) {
		...
		return {
			target: { type: "pane", value: configuredPane.paneId },
			exactPaneId: configuredPane.paneId,
			expectedPanePid: configuredPane.expectedPanePid,
			expectedPaneOwnerId: configuredPane.expectedPaneOwnerId,
			expectedHudPaneId: hudPaneId,
			source: configuredPane.source,
			reason: configuredPane.source,
		};
	}
	return { target: null };
}
...
async function finalizeClaimedDispatchRequest({
	...
	await withDispatchLock(teamDirPath, async () => {
		...
		const index = requests.findIndex(
			(entry) =>
				safeString(entry?.request_id).trim() === claim.request.request_id,
		);
		if (index < 0) return;
		...
		if (result.ok) {
			...
			if (
				...
				runtimeExec(
					{
						command: "MarkFailed",
						request_id: request.request_id,
						reason: "unconfirmed_after_max_retries",
					},
					stateDir,
					teamName,
				);
				...
				if (request.kind === "mailbox" && request.message_id) {
					runtimeExec(
						{ command: "MarkMailboxNotified", message_id: request.message_id },
						stateDir,
						teamName,
					);
			...
			await emitOperationalHookEvent(
				cwd,
...
function resolveWorkerCliForRequest(request, config) {
	...
	if (idx !== null) {
		const worker = workers.find(
			(candidate) => Number(candidate?.index) === idx,
		);
...
function capturedPaneContainsTrigger(captured, trigger) {
	...
	const normalizeForDraftMatch = (value) =>
		normalizeTmuxCapture(value).replace(/-\s+/g, "-");
	return normalizeForDraftMatch(captured).includes(
		normalizeForDraftMatch(trigger),
	);
}
...
async function injectDispatchRequest(request, config, cwd, stateDir) {
	...
	if (dispatchTarget.failure) {
		if (
			dispatchTarget.failure !== "exact_pane_mismatch" &&
			dispatchTarget.failure !== "invalid_explicit_pane"
		) {
			return {
				ok: false,
	...
	if (!resolution.paneTarget) {
		return {
			ok: false,
			reason: `target_resolution_failed:${resolution.reason}`,
		};
	}

	const exactPaneId =
		dispatchTarget.exactPaneId || normalizeExactPaneId(resolution.paneTarget);
	...
	const paneGuard = await evaluatePaneInjectionReadiness(
		resolution.paneTarget,
		{
	...
	const verifyExplicitPane = async () => {
		const paneProof = await verifyExactPaneOwnerLive(
			exactPaneId,
			dispatchTarget.expectedPanePid,
			dispatchTarget.expectedPaneOwnerId,
		);
	...
	if (attemptCountAtStart >= 1) {
		...
		try {
			...
			const preCapture = await runProcess(
				"tmux",
				buildJoinedCapturePaneArgv(resolution.paneTarget, 8),
				2000,
			);
	...
	if (shouldTypePrompt && attemptCountAtStart >= 1) {
		...
		await runProcess(
			"tmux",
			["send-keys", "-t", resolution.paneTarget, "C-u"],
			1000,
		).catch(() => {});
		...
		if (narrowCap) {
			...
			try {
				...
				if (triggerInNarrow || triggerNearTail) {
					...
					if (
						!retrySend.ok &&
						retrySend.reason === EXACT_PANE_UNAVAILABLE_REASON
					) {
						return exactPaneFailure(
							{ reason: retrySend.reason, proof: retrySend.exactPaneProof },
							true,
						);
					}
				...
				if (paneHasActiveTask(wideCap.stdout)) {
					runtimeExec(
						{ command: "MarkDelivered", request_id: request.request_id },
						stateDir,
						request.team_name,
					);
					return {
...
async function updateMailboxNotified(
	...
	return await withMailboxLock(teamDirPath, workerName, async () => {
		const mailbox = await readJson(mailboxPath, {
			worker: workerName,
			messages: [],
		});
		if (!mailbox || !Array.isArray(mailbox.messages)) return false;
		const msg = mailbox.messages.find(
...
function buildDispatchAttemptEvidence(result, fallback = {}) {
	return {
		...
		pane_source:
			safeString(result?.pane_source || fallback.pane_source || "").trim() ||
			null,
		readiness_evidence:
			safeString(
				result?.readiness_evidence || fallback.readiness_evidence || "",
			).trim() || null,
		pane_current_command:
			safeString(
				result?.pane_current_command || fallback.pane_current_command || "",
			).trim() || null,
		exact_pane_proof:
			result?.exact_pane_proof ?? fallback.exact_pane_proof ?? null,
		tmux_injection_attempted:
			typeof result?.tmux_injection_attempted === "boolean"
				? result.tmux_injection_attempted
				: typeof fallback.tmux_injection_attempted === "boolean"
					? fallback.tmux_injection_attempted
					: null,
	};
}
...
export async function drainPendingTeamDispatch({
	...
	for (const teamName of teams) {
		...
		const claims = [];
		await withDispatchLock(teamDirPath, async () => {
			const bridgeRequests = await readBridgeDispatchRequests(
				stateDir,
				teamName,
			);
			const usingLegacyRequests = bridgeRequests === null;
			...
			for (const request of requests) {
				...
				if (
					...
					!resolveLeaderPaneId(config) &&
					!explicitPaneIdentity(request.pane_id).provided
				) {
					const nowIso = new Date().toISOString();
					const alreadyDeferred =
						safeString(request.last_reason).trim() ===
				...
				const lease = await tryAcquireDispatchRequestLease(
					teamDirPath,
					request.request_id,
				);
				if (!lease) {
