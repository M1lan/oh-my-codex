...
async function resolvePaneCwdMismatch(
	paneId: string,
	expectedCwd: any,
): Promise<any | null> {
	if (!expectedCwd) return null;
	try {
		const paneCwdResult = await runProcess("tmux", [
			"display-message",
			"-p",
			"-t",
			paneId,
...
async function finalizeResolvedPane(
	paneId: string,
	reason: string,
	expectedCwd: any,
): Promise<any> {
	...
	let sessionName = "";
	try {
		const currentSession = await runProcess("tmux", [
			"display-message",
			"-p",
			"-t",
			paneId,
...
async function resolveCanonicalPaneFromPaneTarget(
	paneTarget: any,
	expectedCwd: any,
): Promise<any> {
	const paneResult = await runProcess("tmux", [
		"display-message",
		"-p",
		"-t",
		paneTarget,
	...
	const paneId =
		safeString(paneResult.stdout).trim() ||
		(safeString(paneTarget)
			.trim()
			.match(/^%\d+$/)
			? safeString(paneTarget).trim()
			: "");
	if (!paneId) return { paneTarget: null, reason: "target_not_found" };

	let startCommand = "";
	try {
		const startResult = await runProcess("tmux", [
			"display-message",
			"-p",
			"-t",
			paneId,
	...
	let sessionName = "";
	try {
		const sessionResult = await runProcess("tmux", [
			"display-message",
			"-p",
			"-t",
			paneId,
	...
	return finalizeResolvedPane(
		healedPaneId,
		"healed_hud_pane_target",
		expectedCwd,
	);
}
...
function modeStateMatchesInvocationOwner(
	modeState: any,
	payload: any,
	managedContext: any,
): { ok: true } | { ok: false; reason: string } {
	const invocationSessionId = resolveInvocationSessionId(payload);
	const canonicalSessionId = safeString(
		managedContext?.canonicalSessionId ||
			managedContext?.sessionState?.session_id,
	).trim();
	const nativeSessionId = safeString(
		managedContext?.nativeSessionId ||
			managedContext?.sessionState?.native_session_id ||
			managedContext?.sessionState?.codex_session_id,
	).trim();
	const allowedSessionIds = new Set(
		[invocationSessionId, canonicalSessionId, nativeSessionId].filter(Boolean),
	);
	...
	if (
		!ownerOmxSessionId &&
		stateSessionId &&
		!allowedSessionIds.has(stateSessionId)
	) {
		return { ok: false, reason: "mode_session_mismatch" };
	}

	const ownerCodexSessionId = safeString(
		modeState?.owner_codex_session_id || modeState?.codex_session_id,
	).trim();
...
async function validateResolvedInjectionOwnership({
	...
	{ ok: true } | { ok: false; reason: string; managedContext?: any }
> {
	const ownership = await verifyManagedPaneTarget(paneTarget, cwd, payload, {
		allowTeamWorker: false,
	});
	if (!ownership.ok) {
		return {
			ok: false,
			reason: ownership.reason || "pane_not_managed_session",
			managedContext: ownership.managedContext,
		};
	}

	const modeOwner = modeStateMatchesInvocationOwner(
		modeState,
		payload,
		ownership.managedContext,
	);
	if (!modeOwner.ok)
		return { ...modeOwner, managedContext: ownership.managedContext };
	...
	const expectedWindowId = safeString(
		modeState?.tmux_window_id || modeState?.tmuxWindowId,
	).trim();
	if (!expectedWindowId) {
		return { ok: true };
	}
	...
	try {
		const windowResult = await runProcess(
			"tmux",
			["display-message", "-p", "-t", paneTarget, "#{window_id}"],
			2000,
		);
...
export async function readVisibleAllowedModes(
	...
	if (candidateSessionIds.length === 0) {
		const rootCanonicalState = await readVisibleSkillActiveStateForStateDir(
			stateDir,
		).catch(() => null);
...
export async function resolveSessionToPane(
	sessionName: any,
): Promise<string | null> {
	...
	const nonHudRows = rows.filter(
		(row: any) => !isHudPaneStartCommand(row.startCommand),
	);
	const canonicalRows = nonHudRows.filter((row: any) =>
...
export async function resolvePaneTarget(
	...
	const requiresManagedOwnership =
		safeString(cwd).trim() !== "" &&
		safeString(
			payload?.session_id ||
				payload?.["session-id"] ||
	...
	const managedContext = requiresManagedOwnership
		? await resolveManagedSessionContext(cwd, payload, {
				allowTeamWorker: false,
			})
		: {
				managed: false,
	...
	const taggedSessionTarget = safeString(
	...
	if (taggedSessionTarget) {
		try {
			...
			if (paneId) {
				const resolved = await finalizeResolvedPane(
					paneId,
					"managed_instance_target",
					expectedCwd,
				);
				if (!resolved.paneTarget) return resolved;
				const ownership = await verifyManagedPaneTarget(
					resolved.paneTarget,
					cwd,
					payload,
					{ allowTeamWorker: false },
		...
		try {
			...
			if (resolved.paneTarget) {
				const ownership = requiresManagedOwnership
					? await verifyManagedPaneTarget(resolved.paneTarget, cwd, payload, {
							allowTeamWorker: false,
						})
					: { ok: true };
				if (ownership.ok) {
	...
	try {
		if (!requiresManagedOwnership)
			return {
				paneTarget: null,
				reason: "target_session_requires_managed_context",
			};
		const explicitSessionTarget = safeString(target.value).trim();
		const expectedSessionTarget = safeString(
			managedContext.taggedTmuxSessionName ||
				managedContext.expectedTmuxSessionName,
		).trim();
		const sessionIdTarget = safeString(
			managedContext.invocationSessionId,
		).trim();
		const stateSessionTarget = safeString(
			managedContext.sessionState?.session_id,
		).trim();
		const nativeSessionTarget = safeString(
			managedContext.nativeSessionId,
		).trim();
		const canonicalSessionTarget = safeString(
			managedContext.canonicalSessionId,
		).trim();
...
export async function handleTmuxInjection({
	...
	const ownershipPayload =
		context?.status === "authorized"
			? {
					...
					"session-id": context.authorization.ownerCodexSessionId,
				}
			: payload;
	const now = Date.now();
	...
	const assistantMessage = safeString(
	...
	const canonicalModeState = await readVisibleAllowedModes(
		cwd,
		stateDir,
		ownershipPayload,
	...
	if (
		...
		canonicalModeState.activeSkillCount > 0 &&
	...
	const scanActiveModeStateDirs = async (
		dirs: string[],
		...
		for (const scopedDir of dirs) {
			...
			for (const file of files) {
				...
				if (parsed && parsed.active) {
					...
					if (
						canonicalModeState.allowedSet &&
						!canonicalModeState.allowedSet.has(modeName)
					)
						continue;
	...
	try {
		const scopedDirs = await getScopedStateDirsForCurrentSession(
			...
			undefined,
			{
				includeRootFallback:
					!canonicalModeState.canonicalPresent &&
					!canonicalModeState.sessionScoped,
			},
		);
	...
	const mode = canonicalModeState.canonicalPresent
		? canonicalModeState.preferredMode
		: preferredModePane?.mode ||
			pickActiveMode(activeModes, config.allowed_modes);
	const modeState =
		preferredModePane?.state || (mode ? activeModeStates[mode] || {} : {});
	const modePane =
		preferredModePane?.pane || safeString(modeState.tmux_pane_id || "");
	...
	const { renderPrompt, injectLanguageReminder } = await import(
		"./payload-parser.js"
	);
	const prompt = injectLanguageReminder(
		renderPrompt(config.prompt_template, {
			mode: mode || "unknown",
			threadId,
			turnId,
			timestamp: nowIso,
	...
	let resolution = preferredModePane
		? await resolvePaneTarget(
				{ type: "pane", value: preferredModePane.pane },
				cwd,
				preferredModePane.pane,
				cwd,
				ownershipPayload,
			)
		: preferredPaneTarget
			? await resolvePaneTarget(
					{ type: "pane", value: preferredPaneTarget },
					cwd,
					"",
					cwd,
					ownershipPayload,
	...
	if (!resolution.paneTarget && preferredPaneTarget) {
		resolution = await resolvePaneTarget(
			config.target,
			cwd,
			modePane,
			cwd,
	...
	if (
		resolution.healTarget &&
		config.target &&
		(config.target.type !== "pane" ||
			safeString(config.target.value).trim() !== paneTarget)
	) {
		try {
			const healed = {
	...
	const updateStateForAttempt = (success: boolean, reason: string) => {
		...
		if (success) {
			...
			state.pane_counts =
				state.pane_counts && typeof state.pane_counts === "object"
					? state.pane_counts
					: {};
			state.pane_counts[paneTarget] =
				(asNumber(state.pane_counts[paneTarget]) ?? 0) + 1;
	...
	try {
		...
		if (!paneGuard.ok) {
			...
			await logTmuxHookEvent(logsDir, {
				...
				pane_capture_excerpt: paneGuard.paneCapture
					? paneGuard.paneCapture.slice(-200)
					: undefined,
			});
			return;
		}
