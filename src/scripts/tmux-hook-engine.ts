...
export function normalizeTmuxHookConfig(raw: any): any {
	...
	const allowedModes = Array.isArray(raw.allowed_modes)
		? raw.allowed_modes.filter(
				(mode: any) => typeof mode === "string" && mode.trim() !== "",
			)
		: [];
	...
	const logLevel =
		raw.log_level === "error" || raw.log_level === "debug"
			? raw.log_level
			: "info";
	...
	return {
		...
		target: targetIsValid
			? { type: raw.target.type, value: raw.target.value }
			: null,
		allowed_modes:
			allowedModes.length > 0 ? allowedModes : DEFAULT_ALLOWED_MODES,
...
export function pickActiveMode(
	...
	const activeSet = new Set(
		(activeModes || []).filter((mode: any) => typeof mode === "string"),
	);
...
export function evaluateInjectionGuards({
	...
	const recentKeys =
		state.recent_keys && typeof state.recent_keys === "object"
			? state.recent_keys
			: {};
	...
	if (!skipQuotaChecks) {
		...
		const legacySessionCounts =
			state.session_counts && typeof state.session_counts === "object"
				? state.session_counts
				: {};
		const paneKeyNorm =
			typeof paneKey === "string" && paneKey.trim() !== "" ? paneKey : "";
		const sessionKeyNorm =
		...
		const lastInjectionTs =
			typeof state.last_injection_ts === "number" ? state.last_injection_ts : 0;
		if (
			config.cooldown_ms > 0 &&
			lastInjectionTs > 0 &&
			now - lastInjectionTs < config.cooldown_ms
		) {
...
export function resolveCodexPane(): string {
	...
	try {
		const cmd = execFileSync(
			tmuxCommand,
			["display-message", "-t", envPane, "-p", "#{pane_current_command}"],
			{
				encoding: "utf-8",
				timeout: 2000,
			...
			.trim()
			.toLowerCase();
		const startCmd = execFileSync(
			tmuxCommand,
			["display-message", "-t", envPane, "-p", "#{pane_start_command}"],
			{
				encoding: "utf-8",
				timeout: 2000,
				windowsHide: process.platform === "win32",
...
export function paneLooksReady(captured: any): boolean {
	...
	const hasCodexWelcomePrompt = lines.some((line) =>
		/\bhow can i help(?: you)?\b/i.test(line),
	);
	if (hasCodexWelcomePrompt) return true;

	return lines.some((line) =>
		/^\s*(?:[›>❯]\s*)?[A-Z][A-Z0-9]+-\d+\s+only(?:\s*(?:…|\.{3}))?\s*$/iu.test(
			line,
		),
	);
}
...
export function buildCapturePaneArgv(
	paneTarget: any,
	tailLines = 80,
): string[] {
...
export function buildSendKeysArgv({
	...
	const pressCountRaw = Number.isFinite(submitKeyPresses)
		? Math.floor(submitKeyPresses)
		: 2;
