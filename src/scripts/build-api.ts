...
const manifestPath =
	process.env.OMX_API_MANIFEST ?? join(nativeRoot, "Cargo.toml");
...
if (result.error) {
	console.error(
		`omx api build: failed to launch cargo: ${result.error.message}`,
	);
	process.exit(1);
}
...
if (!existsSync(releaseBinaryPath)) {
	console.error(
		`omx api build: expected release binary at ${releaseBinaryPath}`,
	);
	process.exit(1);
}
