...
function archiveContainsBinary(members: string[], binaryPath: string): boolean {
	return (
		members.includes(binaryPath) ||
		members.some((member) => member.endsWith(`/${binaryPath}`)) ||
		members.some((member) =>
			member.replace(/\\/g, "/").endsWith(`/${binaryPath.replace(/\\/g, "/")}`),
		)
	);
}
...
const manifest = JSON.parse(readFileSync(resolve(manifestPath!), "utf-8")) as {
	assets: ManifestAsset[];
};
const byName = new Map(
	walk(resolve(artifactsDir!)).map((file) => [file.split("/").pop()!, file]),
);
...
for (const asset of manifest.assets) {
	...
	if (
		...
		statSync(archivePath).size !== asset.size
	) {
	...
	if (list.status !== 0)
		throw new Error(
			`unable to inspect archive ${asset.archive}: ${list.stderr || list.stdout}`,
		);
	const members = String(list.stdout || "")
		.split(/\r?\n/)
		.filter(Boolean);
