...
function usage(): never {
	console.error(
		"Usage: node scripts/verify-native-release-assets.mjs --manifest <path> --artifacts-dir <dir>",
	);
	process.exit(1);
}
...
) as NativeReleaseManifestPolicyInput;
...
for (const asset of manifest.assets) {
	if (!/\.(?:tar\.xz|tar\.gz|zip)$/i.test(asset.archive))
		throw new Error(`unsupported native archive format: ${asset.archive}`);
	if (manifestArchives.has(asset.archive))
		throw new Error(`duplicate manifest archive: ${asset.archive}`);
	...
	if (
		...
		expectedSize <= 0 ||
		statSync(archivePath).size !== expectedSize
	) {
		throw new Error(`size mismatch for ${asset.archive}`);
	}
	if (
		!/^[a-f0-9]{64}$/i.test(asset.sha256) ||
		sha256(archivePath) !== asset.sha256.toLowerCase()
	) {
...
const archiveFiles = [...byName.keys()].filter((name) =>
	/\.(?:tar\.xz|tar\.gz|zip)$/i.test(name),
);
for (const archive of archiveFiles) {
	if (!manifestArchives.has(archive))
		throw new Error(`archive is not declared by manifest: ${archive}`);
}
console.log(
	`[native-release-assets] verified ${manifest.assets.length} assets`,
);
