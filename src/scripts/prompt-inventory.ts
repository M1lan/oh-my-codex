...
const ABSOLUTE_DIRECTIVE_PATTERN =
	/\b(MUST(?:\s+NOT)?|DO NOT|DON'T|NEVER|ALWAYS|REQUIRED|REQUIRE|ONLY|STOP|ASK only|AUTO-CONTINUE|KEEP GOING)\b/i;
...
function countAbsoluteDirectives(text: string): number {
	return text
		.split(/\r?\n/)
		.filter((line) => ABSOLUTE_DIRECTIVE_PATTERN.test(line)).length;
}
...
function inventorySurface(root: string, path: string): PromptSurfaceInventory {
	...
	const markers = Object.fromEntries(
		MARKERS.map((marker) => [marker, countOccurrences(text, marker)]),
	);
	return {
		path,
...
function duplicateFragmentFamilies(
	root: string,
	paths: string[],
): DuplicateFragmentFamily[] {
...
export function buildPromptInventory(
	root = process.cwd(),
	generatedAt = new Date().toISOString(),
): PromptInventoryReport {
	...
	return {
		...
		totals: {
			...
			approximateTokens: surfaces.reduce(
				(sum, surface) => sum + surface.approximateTokens,
				0,
			),
			absoluteDirectiveCount: surfaces.reduce(
				(sum, surface) => sum + surface.absoluteDirectiveCount,
				0,
			),
		},
		surfaces,
...
export function renderPromptInventoryMarkdown(
	...
	const duplicates =
		report.duplicateFragmentFamilies.length === 0
			? ["- None detected."]
			: report.duplicateFragmentFamilies.map(
					(family) =>
						`- ${family.count} files: ${family.text}\n  - ${family.paths.join(", ")}`,
				);
