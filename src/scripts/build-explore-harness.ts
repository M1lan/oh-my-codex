...
const build = spawnSync(
	"cargo",
	["build", "--release", "-p", "omx-explore-harness"],
	{
		cwd: root,
		encoding: "utf-8",
...
if (build.error) {
	console.error(
		`[build-explore-harness] failed to launch cargo: ${build.error.message}`,
	);
	process.exit(1);
}
if (build.status !== 0) {
...
if (!existsSync(sourcePath)) {
	console.error(
		`[build-explore-harness] expected built binary at ${sourcePath}`,
	);
	process.exit(1);
}
