...
const testTimeoutMs = parseTimeoutMs(
	process.env.OMX_NODE_TEST_TIMEOUT_MS,
	DEFAULT_TEST_TIMEOUT_MS,
);
const runnerTimeoutMs = parseTimeoutMs(
	process.env.OMX_NODE_TEST_RUNNER_TIMEOUT_MS,
	DEFAULT_RUNNER_TIMEOUT_MS,
);
const forceExitGraceMs = parseTimeoutMs(
	process.env.OMX_NODE_TEST_FORCE_EXIT_GRACE_MS,
...
console.error(
	`[run-test-files] running ${files.length} test file(s) from ${targets.join(", ")}${
		testTimeoutMs > 0
			? ` with per-test timeout ${testTimeoutMs}ms`
			: " with per-test timeout disabled"
	}${testConcurrency ? `, test concurrency ${testConcurrency}` : ", default test concurrency"}${
		forceExit
			? `, force exit enabled with ${forceExitGraceMs}ms completion grace`
			: ", force exit disabled"
...
function reportAbnormalExit(
	...
	if (errorMessage) {
		console.error(
			`[run-test-files] node --test error for ${file}: ${errorMessage}`,
		);
	}
	console.error(
...
function runFileWithCompletionForceExit(file: string): Promise<TestRunResult> {
	return new Promise((resolveRun) => {
		...
		function finish(
			...
			console.error(
				`[run-test-files] ${file}: ${reason}; status ${status ?? "unknown"}`,
			);
			if (terminate) {
			...
			resolveRun({
				...
				abnormal:
					status === null ||
					signal !== null ||
					reason.startsWith("node --test failed to spawn"),
			});
		}
		...
		function parseTapLine(line: string): void {
			...
			if (summary) {
				...
				if ((summary[1] === "fail" || summary[1] === "cancelled") && count > 0)
					markFailure();
				return;
			}
			...
			if (plan && Number(plan[1]) === lastTapOk && !sawFailure) {
				armCompletionTimer(
					`force-exit completion grace elapsed after TAP plan ${line}`,
				);
				return;
			}
			...
			if (/^# duration_ms /.test(line) && sawCleanTapSummary()) {
				...
				armCompletionTimer(
					"force-exit completion grace elapsed after clean TAP summary",
				);
			}
		}
		...
		child.stdout?.on("data", (chunk: Buffer) =>
			handleOutput(chunk, process.stdout, true),
		);
		child.stderr?.on("data", (chunk: Buffer) =>
			handleOutput(chunk, process.stderr, false),
		);
		...
		child.on("exit", (status, signal) => {
			...
			if (typeof status === "number") {
				finish(
					status,
					null,
					`node --test exited normally${completedFromSummary ? " after clean TAP summary" : ""}`,
					false,
				);
				return;
			}
...
async function main(): Promise<void> {
	...
	for (const file of files) {
		const result = forceExit
			? await runFileWithCompletionForceExit(file)
			: runFileSync(file);
	...
	if (failedFiles.length > 0 || runnerTimedOut) {
		const failureSummary =
			failedFiles.length === 0 && runnerTimedOut
		...
		console.error(`[run-test-files] ${failureSummary}:`);
