...
const FLATTENED_NATIVE_COLLABORATION_TOOL_NAMES: ReadonlyMap<string, string> =
	new Map([
...
export function isNativeSubagentSpawnToolName(name: string): boolean {
	...
	return (
		NATIVE_SUBAGENT_SPAWN_TOOL_PATTERN.test(canonical) || canonical === "task"
	);
}
...
export function isNativeSubagentResultToolName(name: string): boolean {
	const canonical = canonicalizeNativeCollaborationToolName(name);
	return (
		NATIVE_SUBAGENT_RESULT_TOOL_PATTERN.test(canonical) || canonical === "task"
	);
}
