...
export function parseRoleIntentCorrelationToken(
	taskName: unknown,
): string | undefined {
	if (typeof taskName !== "string") return undefined;
	if (!taskName.startsWith(ROLE_INTENT_SPAWN_TASK_NAME_PREFIX))
		return undefined;
	const correlationToken = taskName.slice(
		ROLE_INTENT_SPAWN_TASK_NAME_PREFIX.length,
	);
	return ROLE_INTENT_CORRELATION_TOKEN_PATTERN.test(correlationToken)
		? correlationToken
		: undefined;
}
...
export type NativeSubagentSupportStatus =
	| "supported"
	| "unsupported"
	| "unknown"
	| "role_routing_unavailable";
...
export type NativeSubagentResultDisposition =
	...
	| {
			kind: "unsupported";
			reason: Exclude<
				NativeSubagentUnsupportedReason,
				"agent_thread_limit_reached"
			>;
			evidenceSummary: string;
	  }
...
export const NATIVE_SUBAGENT_SUPPORT_BLOCKER_FILE =
	"native-subagent-support.json";
...
function nativeSubagentFailureDisposition(
	evidence: string,
): NativeSubagentResultDisposition | null {
...
export function parseNativeSubagentResultDisposition(
	...
	const evidenceSummary = stringifySupportValue(value)
		.replace(/\s+/g, " ")
		.trim();
	...
	if (record) {
		...
		const explicitFailure =
			...
			(record.error !== undefined &&
				record.error !== null &&
				record.error !== "") ||
			["error", "failed", "failure", "unsupported", "unavailable"].includes(
				status,
			);
		...
		if (explicitFailure) {
			return (
				nativeSubagentFailureDisposition(evidenceSummary) ?? {
					kind: "unknown",
					evidenceSummary,
				}
			);
	...
	if (toolName && !isNativeSubagentSpawnToolName(toolName))
		return { kind: "unknown", evidenceSummary };
...
function isNativeSubagentUnsupportedReason(
	value: unknown,
): value is NativeSubagentUnsupportedReason {
	return (
		typeof value === "string" &&
		(NATIVE_SUBAGENT_SUPPORT_BLOCKER_REASONS as readonly string[]).includes(
			value,
		)
	);
}

function isNativeSubagentSupportEvidenceSource(
	value: unknown,
): value is NativeSubagentSupportEvidenceSource {
	return (
		typeof value === "string" &&
		[
			"hook_payload_capability",
...
function blockerMatchesScope(
	...
	if (
		blockerCwd &&
		(!input.cwd ||
			canonicalizeOriginCwd(blockerCwd) !== canonicalizeOriginCwd(input.cwd))
	)
		return false;
	const blockerSessionId = supportString(
		blocker.session_id ?? blocker.sessionId,
	);
	if (
		blockerSessionId &&
		(!input.sessionId || blockerSessionId !== input.sessionId)
	)
		return false;
	return true;
}

function unsupportedEvidenceMatchesScope(
	...
	return (
		isNativeSubagentSupportEvidenceSource(source) &&
...
function supportEvidenceFromBlocker(
	...
	if (reason === "agent_thread_limit_reached") {
		...
		return {
			...
			source,
			...(supportString(
				blocker.error_summary ?? blocker.evidenceSummary ?? blocker.evidence,
			)
				? {
						evidenceSummary: supportString(
				...
				: {}),
			...(supportString(blocker.observed_at ?? blocker.observedAt)
				? {
						observedAt: supportString(
							blocker.observed_at ?? blocker.observedAt,
						),
					}
				: {}),
			...(expiresAt ? { expiresAt } : {}),
	...
	return {
		...
		...(supportString(
			...
			: {}),
		...(supportString(blocker.observed_at ?? blocker.observedAt)
			? { observedAt: supportString(blocker.observed_at ?? blocker.observedAt) }
			: {}),
		...(supportString(blocker.expires_at ?? blocker.expiresAt)
			? { expiresAt: supportString(blocker.expires_at ?? blocker.expiresAt) }
			: {}),
	};
}
...
function roleRoutingUnavailableEvidenceFromMarker(
	...
	if (
		...
		!blockerMatchesScope(record, input)
	)
		return null;
	...
	return {
		...
		...(supportString(record.evidence ?? record.evidenceSummary)
			? {
					evidenceSummary: supportString(
						record.evidence ?? record.evidenceSummary,
					),
				}
			: {}),
		observedAt,
...
function capabilityStatusFromRecord(
	record: Record<string, unknown> | null,
): NativeSubagentSupportStatus | null {
	if (!record) return null;
	const nativeSubagents = supportBoolean(
		record.native_subagents ?? record.nativeSubagents,
	);
	const multiAgent = supportBoolean(
		record.multi_agent_v1 ?? record.multiAgentV1,
	);
...
function reasonFromCapabilityRecord(
	record: Record<string, unknown> | null,
): NativeSubagentUnsupportedReason {
	const nativeSubagents = supportBoolean(
		record?.native_subagents ?? record?.nativeSubagents,
	);
	return nativeSubagents === false
		? "native_subagents_unsupported"
		: "multi_agent_v1_unavailable";
}
...
const NATIVE_SUBAGENT_RESULT_TOOL_PATTERN =
	/(?:^|\.)(?:spawn_agent|list_agents|followup_task|wait_agent)$/;
...
function availableToolsEvidence(
	payload: Record<string, unknown> | null,
): NativeSubagentSupportEvidence | null {
	const tools = supportArray(
		payload?.available_tools ?? payload?.availableTools ?? payload?.tools,
	);
	...
	const names = tools
		.map((tool) =>
			typeof tool === "string"
				? tool
				: supportString(
						supportRecord(tool)?.name ??
							supportRecord(tool)?.tool_name ??
							supportRecord(tool)?.toolName,
					),
		)
		.filter(Boolean);
...
export function resolveNativeSubagentSupportStatus(
	...
	const supportBlockerEvidence = supportEvidenceFromBlocker(
		input.persistedSupportBlocker,
		"persisted_support_blocker",
		input,
	);
	...
	const explicitCapability =
		supportRecord(payload?.omx_runtime_capabilities) ??
	...
	if (explicitStatus === "unsupported") {
		return {
			...
			evidenceSummary:
				"payload capability reports native subagents or multi_agent_v1 unavailable",
		};
	}
	...
	const roleRoutingMarkerEvidence = roleRoutingUnavailableEvidenceFromMarker(
		input.persistedRoleRoutingMarker,
		input,
	);
	...
	const capacityBlockerEvidence = supportEvidenceFromBlocker(
		input.persistedCapacityBlocker,
		"capacity_blocker",
		input,
	);
...
export function buildUnsupportedNativeSubagentGuidance(
	evidence: NativeSubagentSupportEvidence,
): string {
...
export function buildRoleRoutingUnavailableGuidance(
	evidence: NativeSubagentSupportEvidence,
): string {
	const reason = evidence.reason ? ` Reason: ${evidence.reason}.` : "";
	const summary = evidence.evidenceSummary
...
export function classifyConductorArtifactKind(
	relativePath: string,
	...
	if (
		CONDUCTOR_ORCHESTRATION_METADATA_PREFIXES.some(
	...
	if (
		CONDUCTOR_SUBSTANTIVE_DELIVERABLE_PREFIXES.some(
			(prefix) => normalized === prefix || normalized.startsWith(`${prefix}/`),
		)
	) {
...
export function actionKindForConductorArtifact(
	artifactKind: ConductorArtifactKind,
): ConductorActionKind {
	switch (artifactKind) {
...
export function authorizeConductorAction(
	input: ConductorAuthorizationInput,
): ConductorAuthorizationDecision {
	if (input.actionKind === "read-only") {
		return {
			allowed: true,
			reason: "read-only actions are outside the write guard",
		};
	}
	if (
		input.laneKind === "typed-subagent" ||
		input.laneKind === "team-worker" ||
		input.laneKind === "performer-carveout"
	) {
		return {
			allowed: true,
			reason:
				"delegated performer lanes are outside Main-root Conductor write restrictions",
		};
	}
	...
	if (
		...
		(input.artifactKind === "orchestration-metadata" ||
			input.artifactKind === "transport" ||
			input.artifactKind === "ledger")
	) {
		return {
			allowed: true,
			reason:
				"Main-root Conductor may write orchestration metadata, transport, and ledger artifacts",
		};
	}
	if (input.actionKind === "substantive-deliverable-write") {
		return {
			allowed: false,
			reason:
				"Main-root Conductor must delegate substantive plan/spec/interview/review/QA deliverables",
		};
	}
	if (input.actionKind === "implementation-mutation") {
		return {
			allowed: false,
			reason:
				"Main-root Conductor must delegate source/package/git implementation mutations",
		};
	}
	return {
		allowed: false,
		reason: "Main-root Conductor write target is unclassified and fails closed",
	};
}
