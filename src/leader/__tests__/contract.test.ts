...
describe("leader conductor contract", () => {
	...
	it("exports the exact canonical conductor block without ledger/reuse guidance", () => {
		...
		assert.doesNotMatch(
			LEADER_CONDUCTOR_BLOCK,
			/resume_agent|subagent-tracking|Silver Rule/,
		);
	});
	...
	it("classifies and authorizes Conductor writes by phase/lane/action/artifact, not path alone", () => {
		const stateLedger = classifyConductorArtifactKind(
			".omx/state/sessions/sess-1/subagent-tracking.json",
		);
		assert.equal(stateLedger, "ledger");
		assert.equal(
			classifyConductorArtifactKind(".omx/state/subagent-tracking.json"),
			"ledger",
		);
		assert.equal(
			classifyConductorArtifactKind("src/subagent-tracking.json"),
			"implementation-source-package-git",
		);
		assert.equal(
			classifyConductorArtifactKind("subagent-tracking.json"),
			"implementation-source-package-git",
		);
		assert.equal(
			classifyConductorArtifactKind(".omx/plans/subagent-tracking.json"),
		...
		const plan = classifyConductorArtifactKind(
			".omx/plans/conductor-main-root-orchestration-fix.md",
		);
		...
		const source = classifyConductorArtifactKind(
	...
	it("resolves native subagent support from explicit runtime evidence only", () => {
		...
		const incompleteInventory = resolveNativeSubagentSupportStatus({
	...
	it("resolves role routing unavailability from explicit capability and scoped marker evidence", () => {
		...
		assert.equal(
			isUnsupportedNativeSubagentEvidence(capabilityEvidence),
			false,
		);
	...
	it("recognizes namespaced collaboration spawn tools and rejects near-miss names (#3119)", () => {
		...
		for (const toolName of [
			...
			assert.equal(
				resolveNativeSubagentSupportStatus({
					payload: { available_tools: ["Read", toolName] },
				}).status,
				"supported",
				toolName,
			);
		...
		assert.equal(
			resolveNativeSubagentSupportStatus({
				payload: { available_tools: [{ name: "collaboration.spawn_agent" }] },
			}).status,
			"supported",
		);
	...
	it("parses structured collaboration dispositions without child-output prose poisoning", () => {
		for (const toolName of [
			...
			const disposition = parseNativeSubagentResultDisposition(toolName, {
				...
				status: "completed",
				output:
					"Child report: the optional plugin is unavailable, unsupported, and not found.",
			});
			assert.equal(disposition.kind, "success", toolName);
		}
		assert.equal(
			parseNativeSubagentResultDisposition("collaboration.wait_agent", {
				success: true,
				status: "completed",
				error: null,
	...
	it("handles contradictory structured result fields deterministically", () => {
		...
		assert.equal(
			parseNativeSubagentResultDisposition("collaboration.followup_task", {
				success: true,
				status: "failed",
	...
	it("limits legacy prose classification to spawn results", () => {
		...
		for (const toolName of [
			...
			assert.equal(
				parseNativeSubagentResultDisposition(
					toolName,
					"successful child output says a dependency is unavailable",
				).kind,
				"unknown",
				toolName,
			);
	...
	it("ignores structured failure prose from provably unrelated tools", () => {
		assert.equal(
			parseNativeSubagentResultDisposition("Bash", {
				success: false,
				status: "failed",
	...
	it("recognizes scoped unsupported native blocker evidence and renders blocker guidance", () => {
		...
		assert.match(
			buildUnsupportedNativeSubagentGuidance(evidence),
		...
		assert.match(
			LEADER_CONDUCTOR_UNSUPPORTED_NATIVE_DEGRADE_BLOCK,
			/permission for Main-root source\/package\/git edits/,
		);
	});
	...
	it("builds and parses App-compatible native role-intent task names", () => {
		...
		assert.equal(
			isAppCompatibleSpawnTaskName("omx_role_intent_DEADBEEF"),
			false,
		);
		assert.equal(
			isAppCompatibleSpawnTaskName("omx_role_intent_dead-beef"),
			false,
		);
		assert.equal(
			isAppCompatibleSpawnTaskName("omx_role_intent_dead:beef"),
		...
		assert.throws(
			() => buildRoleIntentSpawnTaskName("abc_def"),
			/Invalid role-intent correlation token/,
		);

		assert.equal(
			parseRoleIntentCorrelationToken("omx_role_intent_a3118"),
			"a3118",
		);
		assert.equal(
			parseRoleIntentCorrelationToken(["omx_role_intent_a3118"]),
		...
		assert.equal(
			parseRoleIntentCorrelationToken({
				toString: () => "omx_role_intent_a3118",
			}),
			undefined,
		);
		assert.equal(
		...
		assert.equal(
			parseRoleIntentCorrelationToken("omx-role-intent:deadbeef"),
			undefined,
		);
		assert.equal(
			parseRoleIntentCorrelationToken("omx_role_intent_abc_def"),
			undefined,
		);
		assert.equal(
			parseRoleIntentCorrelationToken("omx_role_intent_DEADBEEF"),
			undefined,
		);
		...
		const generatedTaskName = buildRoleIntentSpawnTaskName(
	...
	it("canonicalizes symlinked and nonexistent-leaf origins while rejecting ELOOP identities", async () => {
		const root = await mkdtemp(
			join(tmpdir(), "omx-contract-canonical-origin-"),
		);
