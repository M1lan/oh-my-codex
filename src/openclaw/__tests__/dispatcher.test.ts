...
describe("interpolateInstruction", () => {
	it("replaces known variables", () => {
		const result = interpolateInstruction("Session {{sessionId}} ended", {
			sessionId: "abc",
		});
	...
	it("replaces undefined variables with empty string", () => {
		const result = interpolateInstruction("{{sessionId}}", {
			sessionId: undefined,
		});
		assert.equal(result, "");
	});
	...
	it("replaces undefined tmuxSession with empty string", () => {
		const result = interpolateInstruction("tmux:{{tmuxSession}}", {
...
describe("interpolateInstruction - reply context variables", () => {
	it("replaces {{replyChannel}} variable", () => {
		const result = interpolateInstruction("channel: {{replyChannel}}", {
	...
	it("replaces {{replyTarget}} variable", () => {
		const result = interpolateInstruction("to: {{replyTarget}}", {
			replyTarget: "@bot",
		});
	...
	it("replaces {{replyThread}} variable", () => {
		const result = interpolateInstruction("thread: {{replyThread}}", {
...
describe("isCommandGateway", () => {
	it("returns true for command gateway", () => {
		assert.equal(
			isCommandGateway({ type: "command", command: "notify-send test" }),
			true,
		);
	});
	...
	it("returns false for http gateway with explicit type", () => {
		assert.equal(
			isCommandGateway({ type: "http", url: "https://example.com" }),
			false,
		);
	});
});
	...
	it("kills shell-command descendants on timeout to avoid bash process leaks", async () => {
      ...
      `;
			...
			const result = await wakeCommandGateway(
				...
				{
					type: "command",
					command: `${process.execPath} ${scriptFile}; true`,
					timeout: 500,
				},
				{},
			);
