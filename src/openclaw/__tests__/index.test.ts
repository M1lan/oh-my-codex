...
describe("wakeOpenClaw", () => {
	...
	it("returns null and does not throw on invalid HTTP URL", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				enabled: true,
				gateways: {
					gw: { type: "http", url: "http://bad-remote.example.com/hook" },
				},
				hooks: {
					"session-start": {
						gateway: "gw",
		...
		for (const eventName of ["stop", "session-end"] as const) {
			...
			writeFileSync(
				...
				JSON.stringify({
					enabled: true,
					gateways: {
						gw: { type: "http", url: `http://127.0.0.1:${port}/hook` },
					},
					hooks: {
			...
			const result = await wakeOpenClaw(eventName, {
	...
	it("context.replyChannel takes precedence over env var", async () => {
		...
		assert.equal(
			parsed.channel,
			"ctx-channel",
			"context value should win over env var",
		);
	...
	it("includes text field as alias of instruction in HTTP payload", async () => {
		...
		assert.equal(
			parsed.text,
			"do the thing",
			"text should be an alias of instruction",
		);
		assert.equal(
			parsed.text,
			parsed.instruction,
			"text and instruction must be identical",
		);
	});
