...
function auditLogPath(cwd: string, nowIso: string): string {
	return join(
		...
		".omx",
		"logs",
		`exec-followups-${nowIso.slice(0, 10)}.jsonl`,
	);
}
...
async function quarantineCorruptQueue(
	...
	const recovered: ExecFollowupQueue = {
		version: 1,
		session_id: sessionId,
		records: [],
	};
...
async function readQueue(
	...
	return {
		...
		records: records.filter(
			(record): record is ExecFollowupRecord =>
...
export async function injectExecFollowup(
	...
	const canonicalSessionId =
		activeUsable &&
		(active.session_id === sessionId || active.native_session_id === sessionId)
			? active.session_id
			: sessionId;
...
export function formatInjectExecFollowupSuccess(
	result: InjectExecFollowupResult,
): string {
	return [
...
export async function execInjectCommand(
	args: string[],
	cwd = process.cwd(),
): Promise<void> {
