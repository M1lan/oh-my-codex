...
function evidenceText(value: JsonObject, keys: string[]): string {
	return keys
		.map((key) => nonEmptyString(value[key]).toLowerCase())
		.filter(Boolean)
		.join("\n");
}
...
function hasCodeReviewLocator(value: JsonObject): boolean {
	...
	return (
		reviewUrl.length > 0 ||
		hasAnyStringField(value, ["thread_id", "agent_id", "tool_call_id", "url"])
	);
}
...
function hasUltraqaLocator(value: JsonObject): boolean {
	...
	if (artifactPath)
		return looksLikeUltraqaEvidence(value) || /\bqa\b/.test(artifactPath);
	return (
		qaUrl.length > 0 ||
		urlLooksLikeCi(url) ||
		hasAnyStringField(value, ["tool_call_id", "thread_id"])
	);
}
...
export function hasCleanCodeReviewEvidence(value: unknown): boolean {
	...
	if (nonEmptyString(verdict.recommendation).toUpperCase() !== "APPROVE")
		return false;
	if (nonEmptyString(verdict.architectural_status).toUpperCase() !== "CLEAR")
		return false;
...
export function hasCleanUltraqaEvidence(value: unknown): boolean {
	...
	if (verdict.skipped === true) {
		return (
			(nonEmptyString(verdict.reason).length > 0 ||
				nonEmptyString(verdict.skip_reason).length > 0) &&
...
export function hasCleanAutopilotReviewAndQaEvidence(
	state: JsonObject,
): boolean {
	const review = objectRecord(stateField(state, "review_verdict"));
	const qa = objectRecord(stateField(state, "qa_verdict"));
	if (!hasCleanCodeReviewEvidence(review) || !hasCleanUltraqaEvidence(qa))
		return false;
...
export function validateAutopilotCompletionTransition(
	...
	if (
		...
		successfulTerminal &&
		!hasCleanAutopilotReviewAndQaEvidence(nextState)
	) {
