...
describe("autopilot deep-interview gate execution contract", () => {
	it("falls back to persisted contract state when nextState is partial", async () => {
		...
		assert.equal(
			decision.reason,
			"record-backed deep-interview completion gate",
		);
	});
			...
			currentState: {
				...
				state: {
					...
					handoff_artifacts: {
						deep_interview: {
							summary:
								"Handoff-local malformed contract must fail before direct fallback.",
							execution_contract_required: true,
							execution_contract: [],
						},
					},
			...
			currentState: {
				...
				state: {
					...
					handoff_artifacts: {
						deep_interview: {
							summary:
								"Marker is canonical; contract came from a compatibility location.",
							execution_contract_required: true,
						},
					},
				},
