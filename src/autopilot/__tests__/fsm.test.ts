...
describe("autopilot supervisor FSM helpers", () => {
	it("normalizes only known Autopilot runtime phases", () => {
		...
		assert.equal(
			normalizeAutopilotPhase("waiting_for_user"),
			"waiting-for-user",
		);
	...
	it("derives child-stage labels only from Autopilot supervisor state", () => {
		...
		assert.equal(
			isAutopilotSupervisingChild(autopilotState, "deep-interview"),
			true,
		);
		assert.equal(deriveAutopilotChildPhase(autopilotState), "deep-interview");
		assert.equal(
			deriveAutopilotStageLabel(autopilotState),
	...
	it("maps waiting-for-user back to the supervised deep-interview child stage", () => {
		...
		assert.equal(
			isAutopilotSupervisingChild(waitingState, "deep-interview"),
			true,
		);
		assert.equal(
			deriveAutopilotStageLabel(waitingState),
			"autopilot:deep-interview",
		);
	});
