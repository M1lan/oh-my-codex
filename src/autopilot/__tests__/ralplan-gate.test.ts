...
describe("autopilot ralplan gate", () => {
	...
	it("rejects missing tracker-backed native evidence by default for autopilot ralplan to ultragoal", () => {
		...
		assert.match(
			decision.reason,
			/tracker-backed native architect and critic lanes/,
		);
	});
	...
	it("rejects invalid next-state complete consensus before falling back to older valid current state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-next-invalid-terminal-"),
		);
		...
		try {
						...
						sessions: {
							[sessionId]: {
								...
								threads: {
									...
									"thread-architect-old": {
										thread_id: "thread-architect-old",
										kind: "subagent",
										first_seen_at: "2026-06-12T09:59:30.000Z",
										last_seen_at: "2026-06-12T09:59:30.000Z",
									...
									"thread-critic-old": {
										...
										kind: "subagent",
										first_seen_at: "2026-06-12T10:00:00.000Z",
			...
			assert.equal(
				...
				"next-autopilot-state:handoff_artifacts",
			);
			assert.equal(
				decision.evidence?.blockedReason,
				"non_approving_ralplan_consensus_review",
			);
			assert.match(
				buildAutopilotRalplanUltragoalGateError(decision),
	...
	it("rejects ordered invalid next-state direct consensus before older valid current state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-next-invalid-direct-"),
		);
	...
	it("rejects stale nested ralplan handoff consensus when parent state returned to ralplan", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-stale-nested-"),
		);
	...
	it("rejects stale raw handoff consensus when parent state returned to ralplan", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-stale-raw-handoff-"),
		);
	...
	it("accepts fresh next-state consensus over stale invalid current-state consensus", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-fresh-next-valid-"),
		);
	...
	it("accepts tracker-backed native reviews without duplicated session, tracker, or artifact fields", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-tracker-resolved-"),
		);
	...
	it("rejects unsupported native evidence even with tracker-backed native consensus", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-unsupported-veto-"),
		);
	...
	it("rejects handoff-root unsupported native evidence even with tracker-backed native consensus", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-unsupported-root-veto-"),
		);
	...
	it("rejects omitted review session ids when no transition session context exists", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-no-session-context-"),
		);
		const trackingPath = subagentTrackingPath(cwd);
		try {
			await mkdir(join(trackingPath, ".."), { recursive: true });
	...
	it("rejects architect and critic reviews that reuse the same native tracker thread", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-duplicate-thread-"),
		);
		...
		try {
			...
			assert.match(
				buildAutopilotRalplanUltragoalGateError(decision),
				/distinct native subagent tracker threads/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("rejects tracker-backed native reviews composed from different sessions", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-cross-session-"),
		);
		const trackingPath = subagentTrackingPath(cwd);
		try {
			await mkdir(join(trackingPath, ".."), { recursive: true });
			...
			assert.match(
				buildAutopilotRalplanUltragoalGateError(decision),
				/same native subagent tracker session/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("rejects stale review session ids even when transition session context exists", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-live-session-mismatch-"),
		);
		...
		try {
			...
			assert.match(
				buildAutopilotRalplanUltragoalGateError(decision),
				/critic review session_id=sess-stale-critic does not match/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("rejects tracker-backed native reviews whose subagent threads are not completed", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-incomplete-thread-"),
		);
		...
		try {
			...
			assert.match(
				buildAutopilotRalplanUltragoalGateError(decision),
				/architect tracker thread thread-architect is not completed/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("explains tracker-backed native review schema and observed missing session values", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-diagnostic-missing-session-"),
		);
		...
		try {
			...
			assert.equal(decision.allowed, false);
			assert.equal(
				decision.evidence?.diagnostic?.current_session_id,
				sessionId,
			);
			assert.equal(
				decision.evidence?.diagnostic?.architect.session_found,
				false,
			);
			assert.equal(
				decision.evidence?.diagnostic?.architect.thread_found,
				false,
			);
			...
			assert.match(
				error,
				/current_session_id: sess-autopilot-diagnostic-missing-session/,
			);
			assert.match(
				error,
				/architect thread_id: thread-architect found: no kind=missing completed=no/,
			);
			assert.match(
				error,
				/session_id: sess-autopilot-diagnostic-missing-session session_found=no/,
			);
			assert.match(
				error,
				/Re-run typed native ralplan Architect\/Critic reviews/,
			);
	...
	it("explains observed tracker thread kind and completion checks", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-diagnostic-thread-values-"),
		);
		...
		try {
			...
			assert.match(
				error,
				/architect thread_id: thread-architect found: yes kind=leader completed=no/,
			);
			assert.match(
				error,
				/critic thread_id: thread-critic found: yes kind=subagent completed=yes/,
			);
	...
	it("rejects native review evidence from the session leader even when malformed tracking marks it as subagent", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-leader-spoof-"),
		);
		...
		try {
			...
			assert.match(
				buildAutopilotRalplanUltragoalGateError(decision),
				/architect tracker thread thread-leader is the session leader/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("accepts fresh native review evidence when tracker leader id aliases a subagent lane", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-fresh-subagent-alias-"),
		);
	...
	it("rejects native review evidence from the current native session leader", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autopilot-ralplan-native-leader-"),
		);
