...
function lifecycleConsensus(
	reviewCycle: number,
	criticVerdict: "approve" | "iterate" = "approve",
) {
	return {
		complete: true,
		sequence: ["architect-review", "critic-review"],
...
describe("autopilot ralplan gate", () => {
	it("fails closed when locally supplied lifecycle reviews and a receipt-shaped artifact claim approval", () => {
		...
		assert.deepEqual(evidence.blockedDetails, [
	...
	it("keeps a newer invalid next-state lifecycle record ahead of older current-state reviews", () => {
		...
		assert.deepEqual(decision.evidence?.blockedDetails, [
			"official host consensus receipt verifier is unavailable",
		]);
	});
});
