...
async function readDeepInterviewState(
...
function handoffArtifacts(
	...
	return (
		safeObject(state?.handoff_artifacts) ??
		safeObject(nestedState(state)?.handoff_artifacts)
	);
}
...
function deepInterviewGate(
	state: JsonObject | null | undefined,
): JsonObject | null {
	return (
		safeObject(state?.deep_interview_gate) ??
		safeObject(nestedState(state)?.deep_interview_gate)
	);
}

function executionContractRequiredMarker(
	state: JsonObject | null | undefined,
): boolean {
	const nested = nestedState(state);
	const handoff = safeObject(deepInterviewHandoff(state));
	return (
		executionContractRequiredValue(state) ||
...
function executionContractRequiredValue(
	state: JsonObject | null | undefined,
): boolean {
	return (
		state?.execution_contract_required === true ||
		state?.executionContractRequired === true
	);
}
...
function executionContractBoolean(
	contract: JsonObject,
	snakeKey: string,
	camelKey: string,
): boolean | undefined {
...
function executionContractStride(contract: JsonObject): string {
	return (
		executionContractValue(contract, "execution_stride", "executionStride") ||
		safeString(contract.stride)
	);
}

function executionContractCandidates(
	state: JsonObject | null | undefined,
): unknown[] {
...
function isExecutionContractPlaceholder(contract: JsonObject): boolean {
	return (
		executionContractStride(contract).length === 0 &&
		executionContractValue(contract, "completion_unit", "completionUnit")
			.length === 0 &&
		executionContractValue(contract, "stop_condition", "stopCondition")
			.length === 0
	);
}
...
function isValidExecutionContract(contract: JsonObject): boolean {
	...
	const selectedBy = executionContractValue(
		contract,
		"selected_by",
		"selectedBy",
	);
	...
	return (
		executionContractBoolean(
			contract,
			"allow_task_shrink",
			"allowTaskShrink",
		) === expected.allowTaskShrink &&
		executionContractValue(
			contract,
			"acceptance_coverage_scope",
			"acceptanceCoverageScope",
		) === expected.acceptanceCoverageScope &&
		executionContractValue(contract, "shrink_policy", "shrinkPolicy") ===
			expected.shrinkPolicy
	);
}

function executionContractStatusForState(
	state: JsonObject | null | undefined,
...
function requiresExecutionContract(
	...
	return allCandidateStates(input, deepState).some((state) =>
		executionContractRequiredMarker(state),
	);
}
...
function questionEnforcement(
	state: JsonObject | null | undefined,
): DeepInterviewQuestionEnforcementState | undefined {
	return safeObject(state?.question_enforcement) as unknown as
...
function autopilotQuestionEnforcement(
	...
	const wait =
		safeObject(state?.deep_interview_question) ??
		safeObject(nestedState(state)?.deep_interview_question);
	...
	if (status === "waiting-for-user") {
		return {
			...
			requested_at:
				safeString(wait.requested_at) || safeString(wait.updated_at),
		};
	}
	if (status === "satisfied") {
		return {
			...
			satisfied_at:
				safeString(wait.satisfied_at) ||
				safeString(wait.resolved_at) ||
				undefined,
		};
	}
	if (status === "cleared") {
...
function firstGate(
	...
	return (
		gates.find((gate) => isSkipGate(gate, input.sessionId)) ??
...
function isCompletionGate(
	...
	return (
		(status === "complete" || gate.complete === true) &&
...
function isSkipGate(gate: JsonObject, sessionId?: string): boolean {
	const status = normalizeStatus(gate.status);
	const reason =
		safeString(gate.reason) ||
		safeString(gate.skip_reason) ||
		safeString(gate.rationale);
	const timestamp =
		safeString(gate.skipped_at) ||
		safeString(gate.timestamp) ||
		safeString(gate.updated_at);
	const source = safeString(gate.source);
	const gateSession = safeString(gate.session_id);
	const userAuthorized =
		gate.skip_authorized_by_user === true || gate.authorized_by_user === true;
...
function questionEnforcementKey(
...
function collectResultingQuestionEnforcements(
	...
	if (!input.nextState)
		return collectQuestionEnforcementsFromStates([
			input.currentState,
			deepState,
		]);

	const resulting = collectQuestionEnforcementsFromStates([
		input.nextState,
		deepState,
	]);
	...
	for (const current of collectQuestionEnforcementsFromStates([
		input.currentState,
	])) {
...
function hasPendingQuestion(
	...
	return enforcements.some(
		(enforcement) => normalizeStatus(enforcement.status) === "pending",
	);
}

function hasDeniedClearedQuestion(
	enforcements: readonly DeepInterviewQuestionEnforcementState[],
): boolean {
	return enforcements.some(
		(enforcement) =>
...
async function allSatisfiedQuestionsHaveAnsweredRecords(
	...
	for (const enforcement of enforcements) {
		...
		if (!(await satisfiedQuestionHasAnsweredRecord(input, enforcement)))
			return false;
	}
	return true;
}
...
export async function canAdvanceAutopilotDeepInterviewToRalplan(
	...
	if (hasPendingQuestion(enforcements)) {
		return {
			allowed: false,
			reason:
				"deep-interview question obligation is still pending; ralplan handoff requires completion or explicit user-authorized skip",
		};
	}
	if (hasDeniedClearedQuestion(enforcements)) {
		return {
			allowed: false,
			reason:
				"cleared deep-interview question obligations with handoff/error are not completion evidence",
		};
	}
	...
	if (requiresExecutionContract(input, deepState, gate)) {
		const executionContractStatus = executionContractStatusForHandoff(
			input,
			deepState,
		);
		if (executionContractStatus !== "valid") {
			return {
				allowed: false,
				reason:
					"missing valid execution_contract for deep-interview ralplan handoff",
				evidence: {
					gate_status: gate.status,
					execution_contract_required: true,
					execution_contract_status: executionContractStatus,
				},
			};
		}
	}
	...
	if (!isCompletionGate(gate, input, deepState)) {
		return {
			allowed: false,
			reason:
				"deep-interview gate is not complete/skipped with required rationale",
	...
	if (!(await allSatisfiedQuestionsHaveAnsweredRecords(input, enforcements))) {
		return {
			allowed: false,
			reason:
				"satisfied deep-interview question obligation lacks same-session answered omx question record",
		};
	}
