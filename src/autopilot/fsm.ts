...
function normalizePhaseText(value: unknown): string {
	...
	if (
		normalized === "fix" ||
		normalized === "review-fix" ||
		normalized === "implementation-fix"
	)
		return "rework";
	return normalized;
}

export function isAutopilotChildPhase(
	value: unknown,
): value is AutopilotChildPhase {
	return (AUTOPILOT_CHILD_PHASES as readonly string[]).includes(
		normalizePhaseText(value),
	);
}

export function normalizeAutopilotPhase(
	value: unknown,
): AutopilotRuntimePhase | null {
...
function readWaitingPreviousPhase(
	state: Record<string, unknown>,
): AutopilotChildPhase | null {
	...
	return previousPhase && isAutopilotChildPhase(previousPhase)
		? previousPhase
		: null;
}
...
export function deriveAutopilotChildPhase(
	state: unknown,
): AutopilotChildPhase | null {
...
export function isAutopilotSupervising(state: unknown): boolean {
	const candidate = safeObject(state);
	return (
		isActiveAutopilotState(candidate) &&
		deriveAutopilotChildPhase(candidate) !== null
	);
}
...
export function isAutopilotSupervisingChild(
	...
	return (
		isAutopilotSupervising(state) && deriveAutopilotChildPhase(state) === child
	);
}

export function deriveAutopilotStageLabel(
	state: unknown,
): AutopilotStageLabel | null {
