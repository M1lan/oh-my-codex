...
export interface LaunchDirectSessionOptions
	extends BuildDirectSessionArgsOptions {
	cwd: string;
	prompt?: string;
	omxCommand?: string;
...
export function resolveVscodeLaunchEnv(
	options: ResolveVscodeLaunchEnvOptions,
): VscodeLaunchEnvResolution {
	const cwd = normalizeCwd(options.cwd);
	const platform = options.platform ?? process.platform;
	const baseEnv = options.baseEnv ?? process.env;
	const pathKey = resolvePathKey(baseEnv);
	const currentPathEntries = String(
		baseEnv[pathKey] ?? baseEnv.PATH ?? baseEnv.Path ?? "",
	)
		.split(delimiter)
		.filter(Boolean);
...
export function buildDirectSessionArgs(
	options: BuildDirectSessionArgsOptions = {},
): string[] {
...
export function launchRequiresDangerousApproval(
...
export function launchDirectSession(
	...
	const sessionId =
		options.sessionId ?? deps.randomId?.() ?? defaultSessionId(now);
	...
	if (dangerousArgs.length > 0 && options.dangerousApproved !== true) {
		throw new Error(
			`dangerous_launch_requires_approval:${dangerousArgs.join(",")}`,
		);
	}

	const logPath = options.logPath
		? resolve(options.logPath)
		: defaultLogPath(cwd, sessionId);
	...
	const launched = spawnPlatformCommand(
		command,
		args,
		spawnOptions,
		platform,
	...
	writeLog(
		logStream,
		`[${startedAt}] $ ${launched.spec.command} ${launched.spec.args.join(" ")}\n`,
	);

	child.stdout?.on("data", (chunk: Buffer | string) =>
		writeLog(logStream, String(chunk)),
	);
...
export function runOmxCatalogCommand(
	...
	const { spec, result } = spawnPlatformCommandSync(
		command,
		options.args,
		{
			cwd,
