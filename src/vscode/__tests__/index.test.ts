...
describe("VSCode direct session API", () => {
	it("builds direct launch args without preserving conflicting tmux policy flags", () => {
		assert.deepEqual(
			buildDirectSessionArgs({
				codexArgs: ["--tmux", "--model", "gpt-5", "--direct"],
			}),
	...
	it("spawns omx in direct mode with VSCode session environment and log path", async () => {
		await withTempDir(async (cwd) => {
			...
			assert.equal(
				(capturedOptions?.env as NodeJS.ProcessEnv).OMX_LAUNCH_POLICY,
				"direct",
			);
			assert.equal(
				(capturedOptions?.env as NodeJS.ProcessEnv).OMX_VSCODE_SESSION_ID,
				"vscode-test-session",
			);
			assert.equal(
				(capturedOptions?.env as NodeJS.ProcessEnv).OMX_VSCODE_LOG_PATH,
				handle.log_path,
			);
	...
	it("builds VSCode launch env through the shared platform command resolver", async () => {
		await withTempDir(async (cwd) => {
			...
			assert.equal(
				resolution.pathEntries.filter((entry) => entry === binDir).length,
				1,
			);
			assert.ok(resolution.pathEntries.includes(join(cwd, "dist", "cli")));
			assert.ok(
				resolution.pathEntries.includes(join(cwd, "node_modules", ".bin")),
			);
		});
	});
