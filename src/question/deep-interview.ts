...
export function isPendingDeepInterviewQuestionEnforcement(
	enforcement:
		| Partial<DeepInterviewQuestionEnforcementState>
		| null
		| undefined,
): enforcement is DeepInterviewQuestionEnforcementState {
	return (
		safeString(enforcement?.obligation_id).trim() !== "" &&
...
function isSameSessionQuestionRecord(
...
function isAnsweredDeepInterviewRecordForObligation(
	...
	if (
		requestedAtMs !== null &&
		(createdAtMs === null || createdAtMs < requestedAtMs)
	) {
		return false;
	}
...
async function findAnsweredDeepInterviewRecordForObligation(
	...
	if (exactQuestionId) {
		...
		if (
			isAnsweredDeepInterviewRecordForObligation(
				exactRecord,
				sessionId,
				enforcement,
			)
	...
	for (const entry of entries) {
		...
		const record = await readQuestionRecord(
			join(getQuestionStateDir(cwd, sessionId), entry),
		);
		if (
			isAnsweredDeepInterviewRecordForObligation(record, sessionId, enforcement)
		) {
			...
			? {
					...
					...(nextEnforcement.status === "pending"
						? {
								...
								active: false,
								completed_at:
									safeString(state.completed_at) || new Date().toISOString(),
							}
						: {}),
				}
			: {}),
...
export async function runDeepInterviewQuestion(
	...
	const sessionId =
		safeString(input.session_id).trim() || (await readCurrentSessionId(cwd));
	const obligation = createDeepInterviewQuestionObligation();
	const existingAutopilotWait =
