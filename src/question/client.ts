...
export type OmxQuestionPayload =
	| OmxQuestionSuccessPayload
	| OmxQuestionErrorPayload;
...
export async function runOmxQuestion(
	input:
		| (Partial<QuestionInput> & { question: string })
		| {
				questions: Array<Partial<QuestionInput> & { question: string }>;
				header?: string;
				source?: string;
				session_id?: string;
		  },
	...
	if (!omxBin) {
		throw new OmxQuestionError(
			"question_cli_not_found",
			"Could not resolve the omx CLI entrypoint for blocking question execution.",
		);
	}
	...
	const payload = parseQuestionStdout(
		result.stdout,
		result.stderr,
		result.code,
	);
