	...
	return [
		{
			...
			type:
				record.type ??
				(record.multi_select ? "multi-answerable" : "single-answerable"),
		},
	];
}
...
function getOptionEntries(
	question: Pick<
		NormalizedQuestionItem,
		"options" | "allow_other" | "other_label"
	>,
): QuestionOptionEntry[] {
...
function buildAnswer(
	...
	if (isMultiAnswerableQuestion(question)) {
		const values = resolvedOtherText
			? [...selected_values, resolvedOtherText]
			: selected_values;
		const labels = includesOther
...
function supportsInteractiveArrowUi(
	input: QuestionUiInput,
	output: QuestionUiOutput,
): boolean {
	return Boolean(
...
export function applyInteractiveSelectionKey(
	...
	if (optionCount === 0)
		throw new Error(
			"Interactive question UI requires at least one selectable option.",
		);
		...
		if (explicitIndex < optionCount) {
			return {
				...
				state: {
					...
					selectedIndices: isMultiAnswerableQuestion(question)
						? toggleSelection(state.selectedIndices, explicitIndex)
	...
	if (key.name === "space") {
		if (!isMultiAnswerableQuestion(question))
			return { submit: true, state: { ...state, error: undefined } };
		return {
			submit: false,
			state: {
				...state,
				selectedIndices: toggleSelection(
					state.selectedIndices,
					state.cursorIndex,
				),
				error: undefined,
...
export function renderInteractiveQuestionFrame(
	...
	if (record.header || question.header)
		lines.push(record.header ?? question.header ?? "");
	lines.push(question.question);

	optionEntries.forEach((entry, index) => {
		const isActive = state.cursorIndex === index;
		const isChecked = isMultiAnswerableQuestion(question)
		...
		lines.push(
			entry.description
				? `${cursor} ${box} ${entry.label} — ${entry.description}`
				: `${cursor} ${box} ${entry.label}`,
		);
	});
...
export function createInitialQuestionWizardState(
	record: QuestionRecord,
): WizardState {
...
function isQuestionSelectionValid(
	...
	return (
		!isMultiAnswerableQuestion(question) || state.selectedIndices.length > 0
	);
}
...
function advanceWizard(
	...
	if (
		!isQuestionSelectionValid(
			current,
			state.selections[state.currentQuestionIndex]!,
		)
	) {
		const selections = state.selections.map((item, index) =>
			index === state.currentQuestionIndex
				? {
						...item,
						error: "Select one or more options with Space before continuing.",
					}
				: item,
		);
	...
	if (state.currentQuestionIndex >= questions.length - 1)
		return { ...state, mode: "review", error: undefined };
	return {
		...state,
...
function questionNeedsOtherTextNow(
	question: NormalizedQuestionItem,
	selectionState: InteractiveSelectionState,
...
function syncOtherTexts(
	...
	return recordQuestions(record).map((question, index) =>
		clearStaleOtherText(
			...
			state.otherTexts[index],
		),
	);
}

export function applyQuestionWizardKey(
	record: QuestionRecord,
	state: WizardState,
	key: KeyLike,
): WizardUpdate {
		...
		if (key.name === "left" || key.name === "backspace")
			return {
				...
				state: {
					...state,
					mode: "answering",
					currentQuestionIndex: recordQuestions(record).length - 1,
				},
			};
		if (key.name === "return" || key.name === "enter")
			return { submit: true, state };
		return { submit: false, state };
	}
	...
	if (key.name === "right") {
		if (
			questionNeedsOtherTextNow(
				current,
				currentSelection,
				state.otherTexts[state.currentQuestionIndex],
			)
		) {
			return {
				submit: false,
				state,
				needsOtherText: state.currentQuestionIndex,
			};
		}
	...
	const update = applyInteractiveSelectionKey(
		{ ...record, ...current, questions: [current] },
		currentSelection,
		key,
	);
	const nextSelections = state.selections.map((item, index) =>
		index === state.currentQuestionIndex ? update.state : item,
	);
	const nextOtherTexts = state.otherTexts.map((value, index) =>
		index === state.currentQuestionIndex
			? clearStaleOtherText(current, update.state, value)
			: value,
	);
	...
	if (
		questionNeedsOtherTextNow(
			current,
			update.state,
			nextOtherTexts[state.currentQuestionIndex],
		)
	) {
		return {
			submit: false,
...
function selectedNumbersForQuestion(
	question: NormalizedQuestionItem,
	state: InteractiveSelectionState,
): number[] {
	if (isMultiAnswerableQuestion(question))
		return state.selectedIndices.map((index) => index + 1);
...
function buildAnswerEntries(
	...
	return recordQuestions(record).map((question, index) => ({
		...
		answer: buildAnswer(
			question,
			selectedNumbersForQuestion(question, state.selections[index]!),
			otherTexts[index],
		),
	}));
}
	...
	return selections
		.map((selection) => {
			if (selection === otherIndex && question.allow_other) {
				return otherText
					? `${question.other_label}: ${otherText}`
...
export function renderQuestionWizardFrame(
	record: QuestionRecord,
	state: WizardState,
): string {
	const questions = recordQuestions(record);
	if (state.mode === "review") {
		...
		questions.forEach((question, index) => {
			...
			lines.push(
				`   ${formatSelectedLabels(question, state.selections[index]!, state.otherTexts[index])}`,
			);
		});
	...
	if (question.header && question.header !== record.header)
	...
	optionEntries.forEach((entry, index) => {
		...
		const isChecked = isMultiAnswerableQuestion(question)
			? selection.selectedIndices.includes(index)
			: isActive;
		const cursor = isActive ? "›" : " ";
		const box = `[${isChecked ? "x" : " "}]`;
		lines.push(
	...
	lines.push(
		isMultiAnswerableQuestion(question)
			? "↑↓ move · Space toggle · Enter/→ next · ← back"
			: "↑↓ move · Enter/→ next · ← back",
	);
	if (selection.error || state.error)
		lines.push(selection.error ?? state.error ?? "");
	return `${lines.join("\n")}\n`;
}
...
export async function promptForSelectionsWithArrows(
	...
	if (!supportsInteractiveArrowUi(input, output))
		throw new Error(
			"Interactive arrow UI requires TTY stdin/stdout with raw-mode support.",
		);
	...
	return new Promise<number[]>((resolve, reject) => {
		...
		const onKeypress = (_: string, key: KeyLike) => {
			...
			finish(
				isMultiAnswerableQuestion(question)
					? state.selectedIndices.map((index) => index + 1)
					: [state.cursorIndex + 1],
			);
		};
...
async function promptForAnswersWithArrows(
	record: QuestionRecord,
	deps: QuestionUiDeps = {},
	...
	while (true) {
		...
		const nextOtherTexts = state.otherTexts.map((value, index) =>
			index === segment.needsOtherText ? text : value,
		);
...
function runWizardSegment(
	record: QuestionRecord,
	initialState: WizardState,
	deps: QuestionUiDeps,
): Promise<WizardSegmentResult> {
	const input = deps.input ?? defaultInput;
	const output = deps.output ?? defaultOutput;
	return new Promise<WizardSegmentResult>((resolve, reject) => {
		...
		const settle = (result: WizardSegmentResult) => {
			if (finished) return;
			finished = true;
			cleanup();
			output.write("\n");
		...
		const render = () => {
			output.write("\u001b[H\u001b[J");
			output.write("\u001b[?25l");
			output.write(renderQuestionWizardFrame(record, state));
		};
		const onKeypress = (_: string, key: KeyLike) => {
			if (key.ctrl && key.name === "c") {
				fail(new Error("Question UI cancelled by user."));
			...
			if (update.needsOtherText !== undefined) {
				settle({
					kind: "needs-other-text",
					state,
					needsOtherText: update.needsOtherText,
				});
				return;
			}
...
async function promptForQuestionWithNumbers(
	question: NormalizedQuestionItem,
	deps: QuestionUiDeps = {},
): Promise<number[]> {
	const input = deps.input ?? defaultInput;
	...
	try {
		...
		const optionCount =
			question.options.length + (question.allow_other ? 1 : 0);
		const prompt = isMultiAnswerableQuestion(question)
			? "Choose one or more options by number (comma-separated): "
			: "Choose an option by number: ";
		let selections: number[] | null = null;
		while (!selections) {
			selections = parseSelection(
...
async function promptForAnswersWithNumbers(
	record: QuestionRecord,
	deps: QuestionUiDeps = {},
): Promise<QuestionAnswerEntry[]> {
	...
	for (const [index, question] of recordQuestions(record).entries()) {
		...
		if (
			...
			selections.includes(question.options.length + 1)
		)
			otherText = await promptForOtherText(question.other_label, deps);
		entries.push({
			question_id: question.id,
			index,
			answer: buildAnswer(question, selections, otherText),
		});
	}
	return entries;
}
	...
	try {
		...
		if (questions.length === 1) {
			...
			if (
				...
				otherText = await promptForOtherText(question.other_label, {
					input,
					output,
				});
			}
