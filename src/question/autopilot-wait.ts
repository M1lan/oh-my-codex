...
export function resolveAutopilotQuestionWaitLockTimeoutMs(
	...
	if (raw === undefined || raw === "")
		return DEFAULT_AUTOPILOT_QUESTION_WAIT_LOCK_TIMEOUT_MS;
...
function isPendingAutopilotQuestionWait(
	wait: Record<string, unknown>,
): boolean {
	return (
		safeString(wait.status) === "waiting_for_user" &&
...
async function readAutopilotState(
	...
	try {
		return JSON.parse(await readFile(statePath, "utf-8")) as Record<
			string,
			unknown
		>;
	} catch {
...
async function maybeRecoverStaleAutopilotQuestionWaitLock(
	lockDir: string,
): Promise<boolean> {
	try {
...
export async function claimAutopilotDeepInterviewQuestionWaiting(
	...
	return await withAutopilotQuestionWaitLock(
		...
		async () => {
			...
			if (
				!state ||
				safeString(state.mode) !== "autopilot" ||
				state.active !== true
			) {
				return "not_applicable";
			}

			if (!isAutopilotSupervisingChild(state, "deep-interview"))
				return "not_applicable";
			const currentPhase =
				normalizeAutopilotPhase(state.current_phase) || "deep-interview";

			const nestedState = safeObject(state.state);
			if (
				isPendingAutopilotQuestionWait(
					safeObject(nestedState.deep_interview_question),
				)
			) {
				return "blocked";
			}
...
export async function resolveAutopilotDeepInterviewQuestionWaiting(
	...
	options: {
		questionId?: string;
		clearReason?: "handoff" | "abort" | "error";
		now?: Date;
	} = {},
): Promise<boolean> {
	...
	if (
		typeof previousLifecycleOutcome === "string" &&
		previousLifecycleOutcome.trim()
	) {
