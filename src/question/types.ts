...
export type QuestionRendererKind =
	| "tmux-pane"
	| "tmux-session"
	| "inline-tty"
	| "windows-console";
...
function normalizeOption(raw: unknown, index: number): QuestionOption {
	...
	const value =
		safeString((raw as Record<string, unknown>).value).trim() || label;
	const description =
		safeString((raw as Record<string, unknown>).description).trim() ||
		undefined;
	if (!label)
...
export function getNormalizedQuestionType(input: {
	...
	return (
		input.type ??
		(input.multi_select === true ? "multi-answerable" : "single-answerable")
	);
}
...
function normalizeQuestionItem(
	...
	const header =
		safeString(input.header).trim() ||
		(index === 0 ? inheritedHeader : undefined);
	...
	if (rawOptions.length === 0 && !allow_other) {
		throw new Error(
			`questions[${index}].options must be a non-empty array unless allow_other is true`,
		);
	}
	if (parsedType === "single-answerable" && rawMultiSelect === true) {
		throw new Error(
			`questions[${index}] type=single-answerable conflicts with multi_select=true`,
		);
	}
...
function normalizeLegacyQuestion(
	...
	return normalizeQuestionItem(
		{ ...raw, id: safeString(raw.id).trim() || "q-1", header },
		0,
		header,
	);
}
...
export function normalizeQuestionInput(raw: unknown): QuestionInput {
	...
	const questions = rawQuestions
		? rawQuestions.map((item, index) =>
				normalizeQuestionItem(item, index, header),
			)
	...
	for (const question of questions) {
		if (seenIds.has(question.id))
			throw new Error(`questions id must be unique: ${question.id}`);
