...
export type QuestionRendererStrategy =
	| "inside-tmux"
	| "detached-tmux"
	| "inline-tty"
	| "windows-console"
...
export type SpawnDetachedRenderer = (
	command: string,
	args: string[],
...
function hasExplicitQuestionPaneTarget(env: NodeJS.ProcessEnv): boolean {
	return isPaneId(
		safeString(env.OMX_QUESTION_RETURN_PANE || env.OMX_LEADER_PANE_ID).trim(),
	);
}
...
export function resolveQuestionRendererStrategy(
	...
	if (
		options?.cwd &&
		readPersistedQuestionReturnTarget(options.cwd, options.sessionId)
	)
		return "inside-tmux";
...
function wrappedDisplayLineCount(
	value: string | undefined,
	paneWidth: number,
): number {
	if (!value) return 0;
	const safeWidth =
		Number.isFinite(paneWidth) && paneWidth > 0 ? Math.floor(paneWidth) : 80;
	return value
		.split("\n")
		.reduce(
			(count, line) =>
				count + Math.max(1, Math.ceil(displayWidth(line) / safeWidth)),
			0,
		);
}
...
export function estimateQuestionRenderFootprint(
	...
	for (let index = 0; index < questions.length; index += 1) {
		const frame = renderQuestionWizardFrame(record, {
			...wizardState,
			currentQuestionIndex: index,
		});
		maxFootprint = Math.max(
			maxFootprint,
			wrappedDisplayLineCount(frame, paneWidth),
		);
	}
	const reviewFrame = renderQuestionWizardFrame(record, {
		...wizardState,
		mode: "review" as const,
	});
	maxFootprint = Math.max(
		maxFootprint,
		wrappedDisplayLineCount(reviewFrame, paneWidth),
	);
	return maxFootprint;
}

export function computeAdaptiveQuestionPaneHeight(
	...
	const minLarge = Math.min(
		Math.max(18, Math.floor(safeAvailable * 0.6)),
		maxHeight,
	);
	const requested =
		Number.isFinite(estimatedContentLines) && estimatedContentLines > 0
			? Math.ceil(estimatedContentLines)
			: 24;
	return Math.min(
		Math.max(Math.max(requested, minLarge), Math.min(8, maxHeight)),
		maxHeight,
	);
}

export function shouldOpenQuestionInNewWindow(
	availableHeight: number,
	estimatedRenderFootprint: number,
): boolean {
	const safeAvailable =
		Number.isFinite(availableHeight) && availableHeight > 0
			? Math.floor(availableHeight)
	...
	const requested =
		Number.isFinite(estimatedRenderFootprint) && estimatedRenderFootprint > 0
			? Math.ceil(estimatedRenderFootprint)
			: 24;
	return requested > maxSplitHeight;
}

function readQuestionRecordForSizing(
	recordPath: string,
): QuestionRecord | null {
	return readJsonFileIfExists(recordPath) as QuestionRecord | null;
}

function recordQuestionsForSizing(
...
function resolveAvailablePaneWidth(
	...
	if (target) {
		try {
			const parsed = Number.parseInt(
				execTmux(["display-message", "-p", "-t", target, format]).trim(),
				10,
			);
			if (Number.isFinite(parsed) && parsed > 0)
				return { width: parsed, probeFailed: false };
		} catch {
	...
	try {
		const parsed = Number.parseInt(
			execTmux(["display-message", "-p", format]).trim(),
			10,
		);
		if (Number.isFinite(parsed) && parsed > 0)
			return { width: parsed, probeFailed: false };
...
function launchQuestionPane(
	...
	if (!paneId)
		throw new Error("Failed to create tmux question renderer container.");
	...
	if (options.underCmux) {
		...
		const envPrefix =
			envEntries.length > 0
				...
				: "";
		return [
			envPrefix + command.map((token) => shellEscapeSingle(token)).join(" "),
		];
	}
...
function buildWindowsConsoleStartCommand(
	command: string,
	args: string[],
): string {
	return [
...
function resolveReturnTarget(options: {
	...
	const explicitPane = safeString(
		env.OMX_QUESTION_RETURN_PANE || env.OMX_LEADER_PANE_ID,
	).trim();
	...
	const persistedPane = readPersistedQuestionReturnTarget(
		options.cwd,
		options.sessionId,
	);
...
export function isWindowsConsoleRendererAlive(
...
export function closeQuestionRenderer(
	...
	try {
		...
		if (
			renderer.renderer === "tmux-session" &&
			renderer.target !== "test-noop-renderer" &&
			safeString(renderer.target).trim()
		) {
...
export function formatQuestionAnswerForInjection(
	answer: QuestionAnswer,
): string {
	const prefix = "[omx question answered]";
	if (answer.kind === "other") {
		return sanitizeReplyInput(
			`${prefix} ${answer.other_text ?? String(answer.value)}`,
		);
	}
...
export function formatQuestionAnswersForInjection(
	answers: Array<{ question_id: string; answer: QuestionAnswer }>,
): string {
	const prefix = "[omx question answered]";
	const body = answers
		.map((entry) => {
			const value = Array.isArray(entry.answer.value)
				? entry.answer.value.join(", ")
...
export function injectQuestionAnswerToPane(
	...
	for (const [index, argv] of argvs.entries()) {
		...
		sleepImpl(
			index === 0 ? QUESTION_TEXT_SETTLE_MS : QUESTION_SUBMIT_REPEAT_DELAY_MS,
		);
	}
	return true;
}
...
export function injectQuestionAnswersToPane(
	...
	if (answers.length === 1)
		return injectQuestionAnswerToPane(
			paneId,
			answers[0]!.answer,
			execTmux,
			sleepImpl,
...
function getQuestionStateDirForRenderer(
	cwd: string,
	sessionId?: string,
): string {
...
function writeQuestionRecordSync(
	recordPath: string,
	record: QuestionRecord,
): void {
	...
	for (const item of live) {
		writeQuestionRecordSync(item.recordPath, {
			...
			error: {
				...
				message:
					"Question was superseded by a newer omx question launch for the same session.",
				at: nowIso,
			},
		});
...
export function launchQuestionRenderer(
	...
	const strategy =
		deps.strategy ??
		resolveQuestionRendererStrategy(options.env ?? process.env, undefined, {
			cwd: options.cwd,
	...
	const spawnDetachedRenderer =
		deps.spawnDetachedRenderer ?? defaultSpawnDetachedRenderer;
	...
	if (strategy === "inside-tmux") {
		...
		const attachedCheckTarget = safeString(env.TMUX).trim()
			...
			: undefined;
		if (
			safeString(env.TMUX).trim() &&
			!isCurrentTmuxSessionAttached(execTmux, env, attachedCheckTarget)
		) {
			throw new Error(
		...
		const sizingTarget =
			returnTarget || safeString(env.TMUX_PANE).trim() || undefined;
		const availableWidthInfo = resolveAvailablePaneWidth(
			execTmux,
			sizingTarget,
		);
	...
	if (strategy === "windows-console") {
		const uiArgs = resolveQuestionUiProcessArgs(options.recordPath, {
			cwd: options.cwd,
			env: options.env,
		});
		const child = spawnDetachedRenderer(
			...
			[
				...
				"/s",
				"/c",
				buildWindowsConsoleStartCommand(process.execPath, uiArgs),
			],
			{
				cwd: options.cwd,
				env: buildQuestionUiProcessEnv(env, {
					sessionId: options.sessionId,
					returnTarget,
				}),
				detached: true,
		...
		return {
			...
			...(returnTarget
				? { return_target: returnTarget, return_transport: "tmux-send-keys" }
				: {}),
		};
	}
	...
	if (strategy === "detached-tmux") {
		...
		const baseName =
			basename(options.recordPath, ".json")
