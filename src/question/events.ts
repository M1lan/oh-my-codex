...
async function maybeRecoverStaleQuestionEventLock(
	lockDir: string,
): Promise<boolean> {
	try {
	...
	while (true) {
		try {
			...
			if (Date.now() > deadline) {
				throw new Error(
					`Timed out acquiring question event lock for ${eventsPath}`,
				);
			}
			await sleep(25);
		}
...
function parseQuestionEventLines(
	contents: string,
	options: { type?: QuestionEventType } = {},
): QuestionEventRecord[] {
...
async function readAllQuestionEventsFromPath(
	path: string,
	options: { type?: QuestionEventType } = {},
...
export function resolveQuestionRunId(
	env: NodeJS.ProcessEnv = process.env,
): string | undefined {
	return (
		safeString(env.OMX_RUN_ID) ||
		safeString(env.OMX_RUN_ID_OVERRIDE) ||
		safeString(env.OMX_CURRENT_RUN_ID) ||
		undefined
	);
}
...
export function summarizeQuestionContext(record: QuestionRecord): string {
	...
	if (questions.length > 1)
		parts.push(`${questions.length} structured questions`);
...
export async function appendQuestionAnsweredEventOnce(
	...
	const path = getQuestionEventsPath(cwd);
	return await withQuestionEventLock(
		path,
		`answered-${record.question_id}`,
		async () => {
			const existing = (
				await readAllQuestionEventsFromPath(path, { type: "question-answered" })
			).find((event) => event.question_id === record.question_id);
			if (existing) return { event: existing, appended: false };
			const event = await appendQuestionEvent(
				cwd,
				"question-answered",
				record,
				options,
...
export async function readQuestionEvents(
	cwd: string,
	options: { limit?: number; type?: QuestionEventType } = {},
): Promise<QuestionEventRecord[]> {
	const path = getQuestionEventsPath(cwd);
	const limit = Math.max(1, Math.min(options.limit ?? 100, 1000));
	const events = await readAllQuestionEventsFromPath(path, {
		type: options.type,
	});
...
export function normalizeSubmittedAnswers(
				...
				? [
						{
							...
							index: 0,
							answer: (raw as { answer: unknown }).answer,
						},
					]
				: [];

	if (rawAnswers.length === 0)
		throw new Error("answer payload must include answer or answers[]");
	...
	return rawAnswers.map((entry, index) => {
		...
		const questionId =
			safeString(object.question_id) ||
			(record.questions?.[index]?.id ?? (index === 0 ? "q-1" : ""));
		if (!questionId || !validQuestionIds.has(questionId))
			throw new Error(
				`answers[${index}].question_id is unknown for this question: ${questionId || "<missing>"}`,
			);
		if (seen.has(questionId))
			throw new Error(`answers question_id must be unique: ${questionId}`);
		seen.add(questionId);
		const answer = object.answer;
		if (!answer || typeof answer !== "object" || Array.isArray(answer))
			throw new Error(`answers[${index}].answer must be an object`);
		const answerObject = answer as Record<string, unknown>;
		const kind = safeString(answerObject.kind);
		if (!["option", "other", "multi"].includes(kind))
			throw new Error(
				`answers[${index}].answer.kind must be option, other, or multi`,
			);
		if (
			!Array.isArray(answerObject.selected_labels) ||
			!Array.isArray(answerObject.selected_values)
		) {
			throw new Error(
				`answers[${index}].answer must include selected_labels[] and selected_values[]`,
			);
		}
		return {
