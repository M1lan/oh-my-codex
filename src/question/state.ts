...
export type CloseQuestionRenderer = (
...
export function getQuestionStateDirForStateDir(
	stateDir: string,
	sessionId?: string,
): string {
	const scopedStateDir = sessionId
		? join(stateDir, "sessions", sessionId)
		: stateDir;
	return join(scopedStateDir, QUESTION_NAMESPACE);
}

export function getQuestionRecordPath(
	cwd: string,
	questionId: string,
	sessionId?: string,
): string {
...
export function getQuestionRecordPathForStateDir(
	stateDir: string,
	questionId: string,
	sessionId?: string,
): string {
	return join(
		getQuestionStateDirForStateDir(stateDir, sessionId),
		`${questionId}.json`,
	);
}
...
export async function markQuestionPrompting(
	...
	return await withQuestionSubmitLock(recordPath, async () => {
		...
		if (rendererToClose)
			maybeCloseQuestionRenderer(
				rendererToClose,
				options.closeQuestionRenderer,
			);
		return updated;
	});
}
...
export async function markQuestionAnswered(
	...
	const updated = await withQuestionSubmitLock(recordPath, () =>
		persistQuestionAnswered(recordPath, answerOrAnswers),
	);
...
async function persistQuestionAnswered(
	...
	const updated = await updateQuestionRecord(recordPath, (record) => {
		const answers = Array.isArray(answerOrAnswers)
			...
			: [
					{
						question_id: record.questions?.[0]?.id ?? "q-1",
						index: 0,
...
function maybeInjectQuestionAnswersToReturnPane(
	...
	if (typeof returnTarget !== "string" || !/^%\d+$/.test(returnTarget.trim()))
		return false;
...
function isValidQuestionId(questionId: string): boolean {
	return (
		/^question-[A-Za-z0-9_.-]+$/.test(questionId) && !questionId.includes("..")
	);
}

async function listQuestionRecordsInDir(
	dir: string,
	...
	for (const entry of entries) {
		...
		if (record?.kind === "omx.question/v1")
...
async function maybeRecoverStaleQuestionLock(
	lockDir: string,
): Promise<boolean> {
	try {
...
function assertNoDuplicateValues(values: string[], path: string): void {
	...
	for (const value of values) {
		if (seen.has(value))
			throw new Error(`${path} must not contain duplicate values: ${value}`);
		seen.add(value);
	}
}
...
function assertSelectedLabelsMatch(
	...
	const mismatchIndex = selectedLabels.findIndex(
...
function questionForAnswer(
	...
	const question = (record.questions ?? []).find(
		(item) => item.id === entry.question_id,
	);
...
function validateAnswerAgainstQuestion(
	...
	const { answer } = entry;
	const selectedValues = validateStringArray(
		answer.selected_values,
		`answers[${entry.index}].answer.selected_values`,
	);
	const selectedLabels = validateStringArray(
	...
	const hasOtherText =
		typeof answer.other_text === "string" &&
		answer.other_text.trim().length > 0;
	const otherText = hasOtherText ? answer.other_text!.trim() : undefined;
	const outOfSchemaValues = selectedValues.filter(
	...
	if (answer.kind === "other") {
		...
		if (
			...
			selectedValues[0] !== otherText ||
			answer.value !== otherText
		) {
			throw new Error(
				`answers[${entry.index}].answer other value must match selected_values[0] and other_text`,
			);
		}
	...
	if (answer.kind === "option") {
		...
		if (answer.value !== selectedValue) {
			throw new Error(
				`answers[${entry.index}].answer.value must match selected_values[0]`,
			);
		}
		assertSelectedLabelsMatch(
			selectedLabels,
	...
	if (!multi)
		throw new Error(
			`answers[${entry.index}].answer.kind=multi is only valid for multi-answerable questions`,
		);
	if (!Array.isArray(answer.value))
		throw new Error(
			`answers[${entry.index}].answer.value must be an array for multi answers`,
		);
	if (
		answer.value.length !== selectedValues.length ||
		answer.value.some((value, index) => value !== selectedValues[index])
	) {
		throw new Error(
			`answers[${entry.index}].answer.value must match selected_values`,
		);
	}
	if (outOfSchemaValues.length > 0) {
		if (!question.allow_other) {
			throw new Error(
				`answers[${entry.index}].answer selected value is not in the option schema: ${outOfSchemaValues[0]}`,
			);
		}
		if (
			!otherText ||
			outOfSchemaValues.length !== 1 ||
			outOfSchemaValues[0] !== otherText
		) {
			throw new Error(
				`answers[${entry.index}].answer out-of-schema selection must match a single other_text value`,
			);
		}
	}
	assertSelectedLabelsMatch(
...
function validateSubmittedAnswersAgainstSchema(
	record: QuestionRecord,
	answers: QuestionAnswerEntry[],
): void {
	const expectedQuestionIds = new Set(
		(record.questions ?? []).map((question) => question.id),
	);
	if (
		expectedQuestionIds.size > 0 &&
		answers.length !== expectedQuestionIds.size
	) {
		throw new Error(
			`answer payload must include exactly one answer for each question (${expectedQuestionIds.size})`,
		);
	}
...
export async function listQuestionRecords(
	...
	const records = (
		await Promise.all(dirs.map((dir) => listQuestionRecordsInDir(dir)))
	).flat();
	const filtered = records.filter(({ record }) => {
		if (!options.status) return true;
		if (options.status === "open")
			return record.status === "pending" || record.status === "prompting";
...
export async function submitQuestionAnswerById(
	...
	const recordPath = getQuestionRecordPath(
		cwd,
	...
	const result = await withQuestionSubmitLock(recordPath, async () => {
		...
		if (!current)
			throw new QuestionSubmitError(
				"question_unknown",
				`Unknown question id: ${normalizedQuestionId}`,
			);
...
export async function waitForQuestionTerminalState(
	...
	while (true) {
		...
		if (
			...
			throw new Error(
				`Timed out waiting for question answer after ${timeoutMs}ms`,
			);
		}
