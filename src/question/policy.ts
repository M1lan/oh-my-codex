import {
	...
	type NotifyCanonicalActiveTeam,
} from "../scripts/notify-hook/active-team.js";
...
export async function evaluateQuestionPolicy(
	...
	const sessionId =
		options.explicitSessionId || (await readCurrentSessionId(options.cwd));
	...
	if (hasWorkerContext(env)) {
		return {
			...
			code: "worker_blocked",
			message:
				"omx question is unavailable for OMX team workers; only non-team leader sessions may ask user questions.",
	...
	const [activeModes, skillState, activeTeams] = await Promise.all([
		...
		sessionId
			? listNotifyCanonicalActiveTeams(options.cwd, sessionId)
			: Promise.resolve([]),
	]);

	const activeSkills = listActiveSkills(skillState ?? {}).map(
		(entry) => entry.skill,
	);

	if (activeTeams.length > 0) {
		const summary = activeTeams
			.map((team) => `${team.teamName} (${team.phase})`)
			.join(", ");
		return {
			allowed: false,
			sessionId,
	...
	const blockedModes = activeModes.filter((mode) =>
		BLOCKED_EXECUTION_SKILLS.has(mode),
	);
	const blockedSkills = activeSkills.filter((skill) =>
		BLOCKED_EXECUTION_SKILLS.has(skill),
	);
	...
	if (blocked.length > 0) {
		...
		if (
			...
			const canStartAutopilotQuestion =
