...
describe("question ui arrow navigation", () => {
	it("moves single-select cursor with up/down arrows and submits current selection on Enter", () => {
		...
		const submit = applyInteractiveSelectionKey(record, state, {
			name: "enter",
		});
	...
	it("toggles multi-select choices with Space and requires a choice before Enter", () => {
		...
		state = update.state;
		state = applyInteractiveSelectionKey(record, state, {
			name: "space",
		}).state;
	...
	it("renders option descriptions inline next to each option to keep the frame compact", () => {
				...
				options: [
					{
						label: "Alpha",
						value: "alpha",
						description: "First choice explanation",
					},
					{
						label: "Beta",
						value: "beta",
						description: "Second choice explanation",
					},
				],
			}),
			{
		...
		assert.doesNotMatch(
			frame,
			/Alpha\n\s+First choice/,
			"description must NOT be rendered on a separate line below the option",
		);
	});
	...
	it("collects arrow-based selection in interactive mode", async () => {
		...
		const promise = promptForSelectionsWithArrows(makeRecord(), {
			input,
			output,
		});
					...
					options: [
						{
							...
							value: "ralplan",
							description:
								"Need architecture and test-shape review before execution",
						},
						{
							label: "Execute directly",
							value: "autopilot",
							description:
								"Requirements are already explicit enough for planning plus execution",
						},
					],
					allow_other: false,
					other_label: "Other",
	...
	it("persists multi-answerable checkbox selections without return-pane injection", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-question-ui-multi-env-return-"),
		);
		try {
			const { recordPath } = await createQuestionRecord(
				cwd,
...
describe("question ui batch wizard", () => {
	it("signals needsOtherText before advancing when Other is picked, then renders the inline text into review", () => {
		...
		const firstAdvance = applyQuestionWizardKey(record, state, {
			name: "enter",
		});
		assert.equal(
			firstAdvance.needsOtherText,
			0,
			"wizard must request inline Other text before advancing",
		);
		assert.equal(firstAdvance.submit, false);
		assert.equal(
			firstAdvance.state.mode,
			"answering",
			"wizard must stay in answering mode until Other text is collected",
		);

		state = { ...firstAdvance.state, otherTexts: ["my custom answer"] };

		const secondAdvance = applyQuestionWizardKey(record, state, {
			name: "enter",
		});
		assert.equal(
			secondAdvance.needsOtherText,
			undefined,
			"wizard must not re-prompt once Other text is stored",
		);
		assert.equal(
			secondAdvance.state.mode,
			"review",
			"wizard advances to review after inline text is captured",
		);
		assert.match(
			renderQuestionWizardFrame(record, secondAdvance.state),
			/Custom: my custom answer/,
		);
	});
	...
	it("clears stored Other text when the user navigates back and changes the selection away from Other", () => {
		...
		assert.equal(
			state.otherTexts[0],
			undefined,
			"wizard must drop stale Other text once the user cursors away from Other",
		);
	});
	...
	it("submits a batch after navigating back and editing an earlier answer", async () => {
		...
		try {
			...
			assert.equal(loaded?.status, "answered");
			assert.deepEqual(
				loaded?.answers?.map((entry) => entry.answer.value),
				["b", "d"],
			);
