...
describe("evaluateQuestionPolicy", { concurrency: false }, () => {
	it("allows non-team leader sessions with no blocked modes", {
		concurrency: false,
	}, async () => {
	...
	it("blocks worker contexts immediately", { concurrency: false }, async () => {
		...
		const result = await evaluateQuestionPolicy({
			cwd,
			explicitSessionId: "sess-1",
			env: { ...process.env, OMX_TEAM_WORKER: "demo/worker-1" },
		});
	...
	it("blocks canonical active team ownership for the current session", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		...
		await writeFile(
			join(cwd, ".omx", "state", "session.json"),
			JSON.stringify({ session_id: "sess-team" }),
		);
		const result = await evaluateQuestionPolicy({
			cwd,
			explicitSessionId: "sess-team",
			env: { ...process.env, OMX_TEAM_WORKER: "" },
		});
		assert.equal(result.allowed, false);
	...
	it("blocks active execution-like workflows for the current session", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		...
		await writeFile(
			...
			JSON.stringify({ mode: "ralph", active: true }),
		);
		await writeFile(
			join(cwd, ".omx", "state", "session.json"),
			JSON.stringify({ session_id: "sess-ralph" }),
		);
		const result = await evaluateQuestionPolicy({
			cwd,
			explicitSessionId: "sess-ralph",
			env: { ...process.env, OMX_TEAM_WORKER: "" },
		});
		assert.equal(result.allowed, false);
	...
	it("does not falsely block from another session team state", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		...
		const result = await evaluateQuestionPolicy({
			cwd,
			explicitSessionId: "sess-main",
			env: { ...process.env, OMX_TEAM_WORKER: "" },
		});
		assert.equal(result.allowed, true);
	});

	it("allows deep-interview state when no execution-like workflow is active", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		...
		await writeFile(
			...
			JSON.stringify({ mode: "deep-interview", active: true }),
		);
		const result = await evaluateQuestionPolicy({
			cwd,
			explicitSessionId: "sess-di",
			env: { ...process.env, OMX_TEAM_WORKER: "" },
		});
		assert.equal(result.allowed, true);
...
describe("evaluateQuestionPolicy autopilot deep-interview wait", {
	concurrency: false,
}, () => {
	it("allows a deep-interview question to start while autopilot is in its deep-interview phase", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
	...
	it("blocks duplicate deep-interview questions while autopilot is already waiting", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		...
		await writeFile(
			join(cwd, ".omx", "state", "session.json"),
			JSON.stringify({ session_id: "sess-auto" }),
		);
	...
	it("rejects malformed autopilot waits that are not for the deep-interview child phase", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		const sessionDir = join(
