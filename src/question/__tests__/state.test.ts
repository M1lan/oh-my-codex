...
describe("question state", () => {
	it("creates records under session-scoped question state and reads them back", async () => {
		...
		assert.equal(
			recordPath,
			getQuestionRecordPath(cwd, record.question_id, "sess-1"),
		);
	...
	it("emits a structured creation event with correlation metadata", async () => {
		...
		const event = (await readQuestionEvents(cwd)).find(
			(item) => item.question_id === record.question_id,
		);
		...
		assert.equal(
			event?.option_schema?.[0]?.options[0]?.description,
			"Alpha lane",
		);
	...
	it("submits bounded answers by id and rejects duplicate stale or unknown submissions", async () => {
		...
		const submitted = await submitQuestionAnswerById(
			cwd,
			record.question_id,
			{
				answer: {
		...
		await assert.rejects(
			...
			(error) =>
				error instanceof QuestionSubmitError &&
				error.code === "question_not_open",
		);
	...
	it("rejects submitted answers that do not match the prompt option schema", async () => {
		...
		await assert.rejects(
			...
			(error) =>
				error instanceof QuestionSubmitError &&
				error.code === "question_invalid_answer",
		);
		...
		const loaded = await readQuestionRecord(
			getQuestionRecordPath(cwd, record.question_id, "sess-invalid"),
		);
	...
	it("rejects other answers whose selected labels do not exactly match the other label", async () => {
		...
		const loaded = await readQuestionRecord(
			getQuestionRecordPath(cwd, record.question_id, "sess-other-labels"),
		);
		assert.equal(loaded?.status, "pending");
	});
	...
	it("rejects multi answers whose selected labels do not match selected values", async () => {
		...
		const loaded = await readQuestionRecord(
			getQuestionRecordPath(cwd, record.question_id, "sess-multi-labels"),
		);
		assert.equal(loaded?.status, "pending");
	});
	...
	it("dedupes answered lifecycle events when a waiting command observes an externally submitted answer", async () => {
		...
		const duplicateAppend = await appendQuestionAnsweredEventOnce(
			cwd,
			submitted.record,
			{
				recordPath,
		...
		const answeredEvents = (await readQuestionEvents(cwd)).filter(
			(event) =>
				event.type === "question-answered" &&
				event.question_id === record.question_id,
		);
	...
	it("serializes concurrent duplicate submissions so only one answer is accepted", async () => {
		...
		const results = await Promise.allSettled([
			submitQuestionAnswerById(cwd, record.question_id, payload, {
				sessionId: "sess-race",
			}),
			submitQuestionAnswerById(cwd, record.question_id, payload, {
				sessionId: "sess-race",
		...
		assert.equal(
			(rejected?.reason as QuestionSubmitError).code,
			"question_not_open",
		);
		const answeredEvents = (await readQuestionEvents(cwd)).filter(
			(event) =>
	...
	it("preserves original run_id correlation on later answered and error events", async () => {
		...
		await submitQuestionAnswerById(
			cwd,
			record.question_id,
			{
				answer: {
		...
		const answerEvent = (await readQuestionEvents(cwd)).find(
			(event) =>
				event.type === "question-answered" &&
				event.question_id === record.question_id,
		);
		...
		const { recordPath: errorPath } = await createQuestionRecord(
			cwd,
			{
				question: "Pick one",
				options: [{ label: "A", value: "a" }],
		...
		await appendQuestionEvent(cwd, "question-error", errorRecord, {
			recordPath: errorPath,
		});
		const errorEvent = (await readQuestionEvents(cwd)).find(
			(event) =>
				event.type === "question-error" &&
				event.question_id === errorRecord.question_id,
		);
	...
	it("waits for terminal answered state and returns free-text other values exactly", async () => {
		...
		const waiter = waitForQuestionTerminalState(recordPath, {
			pollIntervalMs: 10,
			timeoutMs: 2000,
		});
		setTimeout(() => {
	...
	it("persists explicit terminal errors after prompting begins", async () => {
		...
		assert.match(
			...
			/pane %42 disappeared immediately after launch/,
		);
	});
		...
		const record = await markQuestionAnswered(
			...
			},
			{
				injectAnswersToPane: (paneId, answers) => {
					injected.push({
						paneId,
						values: answers.map((entry) => entry.answer.value),
					});
					return true;
				},
