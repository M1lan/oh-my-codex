...
describe("resolveQuestionRendererStrategy", () => {
	it("prefers inside-tmux when TMUX is present", () => {
		assert.equal(
			resolveQuestionRendererStrategy(
				{ TMUX: "/tmp/tmux-demo" } as NodeJS.ProcessEnv,
				"/usr/bin/tmux",
			),
			"inside-tmux",
		);
	...
	it("supports explicit host-pane bridge hints when TMUX is absent", () => {
		assert.equal(
			resolveQuestionRendererStrategy(
				{ OMX_QUESTION_RETURN_PANE: "%77" } as NodeJS.ProcessEnv,
				"/usr/bin/tmux",
			),
			"inside-tmux",
		);
		assert.equal(
			resolveQuestionRendererStrategy(
				{ OMX_LEADER_PANE_ID: "%88" } as NodeJS.ProcessEnv,
				"/usr/bin/tmux",
			),
			"inside-tmux",
		);
	...
	it("rejects malformed explicit host-pane bridge hints", () => {
		assert.equal(
			resolveQuestionRendererStrategy(
				{ OMX_QUESTION_RETURN_PANE: "not-a-pane" } as NodeJS.ProcessEnv,
				"/usr/bin/tmux",
			),
			"unsupported",
		);
	...
	it("uses noop test renderer override when requested", () => {
		assert.equal(
			resolveQuestionRendererStrategy(
				{ OMX_QUESTION_TEST_RENDERER: "noop" } as NodeJS.ProcessEnv,
				"/usr/bin/tmux",
			),
			"test-noop",
		);
...
describe("question renderer cleanup", () => {
	...
	it("ignores invalid, noop, and Windows process renderers during cleanup", () => {
		const calls: string[][] = [];
		assert.equal(
			closeQuestionRenderer(undefined, (args) => {
				calls.push(args);
				return "";
			}),
			false,
...
describe("question window topology selection", () => {
	...
	it("counts wrapped question text more conservatively in narrow panes", () => {
		...
		assert.ok(
			estimateQuestionRenderFootprint(record, 20) >
				estimateQuestionRenderFootprint(record, 80),
		);
	});
	...
	it("sizes multi-question records from the largest visible screen rather than summing every question", () => {
		...
		const multiFootprint = estimateQuestionRenderFootprint(
			multiQuestionRecord,
			20,
		);
	...
	it("includes the review screen when sizing multi-question records", () => {
		...
		assert.ok(
			estimateQuestionRenderFootprint(reviewScreenRecord, 80) >
				estimateQuestionRenderFootprint(singleScreenRecord, 80),
		);
	});
...
describe("launchQuestionRenderer", () => {
	...
	it("opens an interactive foreground split when already inside an attached tmux session", () => {
		...
		assert.equal(
			splitCall[splitCall.length - 5]?.endsWith("/dist/cli/omx.js"),
			true,
		);
		...
		assert.ok(
			splitCall.includes("OMX_QUESTION_RETURN_TRANSPORT=tmux-send-keys"),
		);
		assert.ok(
			calls.some(
				(call) =>
					call.join(" ") === "list-panes -t %42 -F #{pane_dead}\t#{pane_id}",
			),
		);
	});
	...
	it("opens a new tmux window when the current pane is too short for the question frame", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-question-renderer-new-window-"),
		);
		try {
			const stateDir = join(
				cwd,
				...
				JSON.stringify(
					{
						...
						status: "pending",
						question:
							"Round 1 | Target: definition-boundary | Ambiguity: 42%\n\nСамая важная неоднозначность: что именно считать системной метрикой скорости для TTS / Image / Voice, чтобы прогресс-бар и карточки не врали оператору?",
						options: [
							{
								label: "Stage timings",
								value: "stage-timings",
								description:
									"Считать отдельно реальные этапы: TTS synthesis, image/still generation, voice/video final generation; для каждого нужны stage timestamps/metadata.",
							},
							{
								label: "Wall-clock by artifact",
								value: "wall-clock-by-artifact",
								description:
									"Брать общий wall-clock от createdAt до completed и нормализовать по типу результата: 1 image / N sec, 1s video / N sec.",
							},
							{
								label: "Hybrid recommended",
								value: "hybrid",
								description:
									"Сначала использовать wall-clock fallback, но добавлять stage timings для новых генераций, когда этапы можно инструментировать.",
							},
						],
						allow_other: true,
						other_label: "Other",
				...
				},
				{
					...
					execTmux: (args) => {
						...
						if (
							...
							args.includes("#{session_attached}")
						)
							return "1\n";
						if (
							args[0] === "display-message" &&
							args.includes("#{pane_height}")
						)
							return "5\n";
			...
			assert.deepEqual(newWindowCall.slice(targetIndex, targetIndex + 2), [
				"-t",
				"$1",
			]);
			assert.equal(
				calls.some((call) => call[0] === "split-window"),
				false,
			);
			assert.equal(
				calls.some(
					(call) =>
						call[0] === "display-message" && call.includes("#{session_id}"),
				),
				true,
			);
		} finally {
	...
	it("opens a new tmux window when wrapped content would exceed the split budget in a narrow pane", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-question-renderer-wrapped-window-"),
		);
		try {
			const stateDir = join(
				cwd,
	...
	it("falls back to the default tmux width when the width probe fails", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-question-renderer-width-fallback-"),
		);
		try {
			const stateDir = join(
				cwd,
			...
			assert.equal(
				calls.some(
					(call) =>
						call[0] === "display-message" && call.includes("#{pane_height}"),
				),
				false,
			);
			assert.equal(
	...
	it("targets the explicit leader pane even when the caller is already inside tmux", () => {
		...
		const result = launchQuestionRenderer(
			{
				cwd: "/repo",
				recordPath:
					"/repo/.omx/state/sessions/s1/questions/question-leader.json",
				sessionId: "s1",
				env: {
					TMUX: "/tmp/tmux-demo",
					TMUX_PANE: "%22",
		...
		const splitCall = calls.find((call) => call[0] === "split-window");
		assert.ok(splitCall);
		assert.deepEqual(splitCall.slice(0, 6), [
			"split-window",
			"-v",
			"-l",
			"24",
	...
	it("fails closed before splitting when inside a detached tmux session", () => {
				...
				launchQuestionRenderer(
					{
						cwd: "/repo",
						recordPath:
							"/repo/.omx/state/sessions/s1/questions/question-detached.json",
						sessionId: "s1",
						env: {
							TMUX: "/tmp/tmux-demo",
							TMUX_PANE: "%11",
		...
		assert.deepEqual(calls, [
			["display-message", "-p", "-t", "%11", "#{session_attached}"],
		]);
	});
	...
	it("targets an explicit host pane when launching from a container without TMUX", () => {
		...
		const result = launchQuestionRenderer(
			{
				cwd: "/repo",
				recordPath:
					"/repo/.omx/state/sessions/s1/questions/question-bridge.json",
				sessionId: "s1",
				nowIso: "2026-04-19T00:00:00.000Z",
		...
		assert.ok(
			calls.some(
				(call) =>
					call.join(" ") === "list-panes -t %78 -F #{pane_dead}\t#{pane_id}",
			),
		);
	});
	...
	it("opens a visible Windows psmux shell pane and injects the question UI command literally", () => {
		...
		assert.equal(
			tmuxCalls.some((call) => call[0] === "split-window"),
			false,
		);
		...
		const literalSend = tmuxCalls.find(
			(call) => call[0] === "send-keys" && call.includes("-l"),
		);
		assert.ok(literalSend);
		assert.deepEqual(literalSend.slice(0, 6), [
			"send-keys",
			"-t",
			"%55",
			"-l",
		...
		assert.match(
			literalSend[5] ?? "",
			/^set "OMX_SESSION_ID=s1" && set "OMX_QUESTION_RETURN_TARGET=%%44" && set "OMX_QUESTION_RETURN_TRANSPORT=tmux-send-keys" && /,
		);
		assert.match(
			literalSend[5] ?? "",
			/"question" "--ui" "--state-path" "\/repo\/\.omx\/state\/sessions\/s1\/questions\/question-bridge\.json"$/,
		);
		assert.deepEqual(
			tmuxCalls.filter(
				(call) => call[0] === "send-keys" && call.includes("C-m"),
			),
			[["send-keys", "-t", "%55", "C-m"]],
		);
	});
		...
		const result = launchQuestionRenderer(
			{
				cwd: "C:/repo",
				recordPath:
					"C:/repo/.omx/state/sessions/s1/questions/question-bridge.json",
				sessionId: "s1",
				nowIso: "2026-04-24T00:00:00.000Z",
		...
		const result = launchQuestionRenderer(
			{
				cwd: "/repo",
				recordPath:
					"/repo/.omx/state/sessions/s1/questions/question-inline.json",
				sessionId: "s1",
	...
	it("prefers session-scoped persisted panes over root workflow fallback panes", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-question-renderer-precedence-"),
		);
		try {
			...
			writeFileSync(
				join(rootStateDir, "team-state.json"),
				JSON.stringify(
					{
						active: true,
						mode: "team",
	...
	it("passes direct tmux argv so non-POSIX default shells do not parse a shell string", () => {
		...
		assert.ok(splitCall);
		assert.equal(
			splitCall.some((part) => /question --ui --state-path/.test(part)),
			false,
		);
		assert.equal(
			splitCall.some((part) => /^'.*'$/.test(part)),
			false,
		);
		assert.equal(splitCall[splitCall.length - 6], process.execPath);
		assert.equal(
	...
	it("uses detached sessions outside tmux", () => {
		...
		assert.equal(
			calls[0]?.[calls[0]!.length - 5]?.endsWith("/dist/cli/omx.js"),
			true,
		);
...
describe("question answer injection", () => {
	...
	it("injects the answered text back into the requester pane and submits with isolated double C-m", () => {
		...
		assert.deepEqual(
			calls,
			buildSendPaneArgvs("%11", "[omx question answered] proceed", true),
		);
		assert.deepEqual(sleeps, [120, 100]);
		assert.equal(
			calls.some((argv) => argv.includes("Enter")),
			false,
		);
	});
	...
	it("injects all batch answers back into the requester pane", () => {
		...
		assert.deepEqual(
			...
			buildSendPaneArgvs(
				"%11",
				"[omx question answered] first: a; second: d",
				true,
			),
		);
		assert.deepEqual(sleeps, [120, 100]);
...
describe("question renderer in-flight dedupe", () => {
		...
		try {
			...
			writeQuestionRecord(join(dir, "question-live.json"), {
				...
				renderer: {
					renderer: "tmux-pane",
					target: "%41",
					launched_at: "2026-05-27T00:00:01.000Z",
				},
			});
			writeQuestionRecord(join(dir, "question-dead.json"), {
				...
				renderer: {
					renderer: "tmux-pane",
					target: "%42",
					launched_at: "2026-05-27T00:00:02.000Z",
				},
			});
			writeQuestionRecord(join(dir, "question-answered.json"), {
				...
				renderer: {
					renderer: "tmux-pane",
					target: "%43",
					launched_at: "2026-05-27T00:00:03.000Z",
				},
			});
			...
			assert.deepEqual(
				live.map((item) => item.record.question_id),
				["question-live"],
			);
		} finally {
			rmSync(cwd, { recursive: true, force: true });
	...
	it("marks prior live prompting panes superseded and kills them before a new tmux split", () => {
		...
		try {
			...
			const result = launchQuestionRenderer(
				...
				},
				{
					...
					execTmux: (args) => {
						...
						if (args[0] === "list-panes" && args[2] === "%44")
							return "0\t%44\n";
						return "";
					},
					sleepSync: () => {},
			...
			const prior = JSON.parse(readFileSync(priorPath, "utf-8")) as {
				status: string;
				error?: { code?: string };
				updated_at?: string;
			};
			...
			const killIndex = calls.findIndex(
				(call) => call.join(" ") === "kill-pane -t %41",
			);
	...
	it("does not supersede answered records when launching a replacement renderer", () => {
		...
		try {
			...
			const superseded = supersedeLiveQuestionsForSession(
				cwd,
				"sess-dedupe",
				(args) => {
			...
			const answered = JSON.parse(readFileSync(answeredPath, "utf-8")) as {
				status: string;
			};
...
describe("buildQuestionUiTmuxArgs", () => {
	...
	it("delivers env via a single shell-neutral env-prefixed command and never emits a bare -e under cmux", () => {
		...
		assert.ok(
			command.includes("OMX_QUESTION_RETURN_TRANSPORT='tmux-send-keys'"),
		);
	...
	it("single-quotes values containing =, % and spaces so they survive the cmux shell command", () => {
		...
		assert.ok(
			command.includes(`OMX_QUESTION_RETURN_TARGET='${trickyReturnTarget}'`),
		);
		assert.equal(command.includes(" -e "), false);
	});
