...
const originalQuestionWaitLockTimeout =
	process.env.OMX_AUTOPILOT_QUESTION_WAIT_LOCK_TIMEOUT_MS;
...
async function makeRepo(): Promise<string> {
	...
	await mkdir(join(cwd, ".omx", "state", "sessions", "sess-di"), {
		recursive: true,
	});
	await writeFile(
...
after(async () => {
	...
	if (originalQuestionWaitLockTimeout === undefined)
		delete process.env.OMX_AUTOPILOT_QUESTION_WAIT_LOCK_TIMEOUT_MS;
	else
		process.env.OMX_AUTOPILOT_QUESTION_WAIT_LOCK_TIMEOUT_MS =
			originalQuestionWaitLockTimeout;
	await Promise.all(
...
describe("runDeepInterviewQuestion", { concurrency: false }, () => {
	it("tracks a pending obligation before omx question returns and satisfies it afterward", {
		concurrency: false,
	}, async () => {
		...
		const runner: OmxQuestionProcessRunner = async () => {
			...
			assert.equal(
				inFlightState.question_enforcement?.lifecycle_outcome,
				"askuserQuestion",
			);
		...
		assert.equal(
			finalState.question_enforcement?.lifecycle_outcome,
			"askuserQuestion",
		);
	...
	it("clears the pending obligation when omx question fails after being attempted", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		const statePath = join(
					...
					runner: async () => ({
						...
						stdout: JSON.stringify({
							...
							error: {
								code: "team_blocked",
								message:
									"omx question is unavailable while this session owns active team mode.",
							},
						}),
						stderr: "",
					}),
	...
	it("clears the pending obligation when question renderer launch fails", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		const statePath = join(
	...
	it("reconciles a pending obligation from an already-answered same-session question record", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		const statePath = join(
		...
		assert.equal(
			reconciled?.question_enforcement?.question_id,
			"question-answered",
		);
		assert.equal(
			reconciled?.question_enforcement?.satisfied_at,
			"2026-04-19T00:00:21.000Z",
		);
		...
		assert.equal(finalState.question_enforcement?.status, "satisfied");
		assert.equal(
			finalState.question_enforcement?.question_id,
			"question-answered",
		);
		assert.equal(
			finalState.question_enforcement?.satisfied_at,
			"2026-04-19T00:00:21.000Z",
		);
		assert.equal(finalState.lifecycle_outcome, undefined);
		assert.equal(finalState.run_outcome, undefined);
...
describe("runDeepInterviewQuestion autopilot wait bridge", {
	concurrency: false,
}, () => {
	it("does not overwrite an already pending Autopilot deep-interview question", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		...
		const started = await markAutopilotDeepInterviewQuestionWaiting(
			cwd,
			"sess-di",
			{
		...
		assert.equal(
			autopilotState.state?.deep_interview_question?.requested_at,
			"2026-04-19T00:00:00.000Z",
		);
	});

	it("serializes concurrent Autopilot deep-interview question ownership claims", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		const sessionDir = join(cwd, ".omx", "state", "sessions", "sess-di");
		...
		const results = await Promise.all(
			obligations.map((obligation) =>
				markAutopilotDeepInterviewQuestionWaiting(cwd, "sess-di", obligation),
			),
		);
		...
		assert.equal(
			autopilotState.state?.deep_interview_question?.status,
			"waiting_for_user",
		);
		assert.equal(
			autopilotState.state?.deep_interview_question?.obligation_id,
	...
	it("blocks instead of prompting when the Autopilot wait claim cannot acquire its lock", {
		concurrency: false,
	}, async () => {
		...
		try {
			...
			await assert.rejects(
				...
				(error: unknown) =>
					error instanceof OmxQuestionError &&
					error.code === "active_execution_mode_blocked",
			);

			assert.equal(runnerCalled, false);
			assert.equal(
				await readAutopilotDeepInterviewQuestionWaitState(cwd, "sess-di"),
				null,
			);
	...
	it("fails terminally instead of replacing another pending Autopilot question owner", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		const sessionDir = join(cwd, ".omx", "state", "sessions", "sess-di");
		...
		assert.equal(
			deepInterviewState.question_enforcement?.obligation_id,
			"obligation-original",
		);
		assert.equal(deepInterviewState.question_enforcement?.status, "pending");
	});

	it("persists readable autopilot waiting-for-user state while omx question is in flight and restores it after answer", {
		concurrency: false,
	}, async () => {
		const cwd = await makeRepo();
		const sessionDir = join(cwd, ".omx", "state", "sessions", "sess-di");
		...
		const runner: OmxQuestionProcessRunner = async (
			...
			const waitState = await readAutopilotDeepInterviewQuestionWaitState(
				cwd,
				"sess-di",
			);
			assert.ok(waitState);
			...
			const autopilotState = JSON.parse(
				...
				lifecycle_outcome?: string;
				state?: {
					deep_interview_question?: { status?: string; obligation_id?: string };
				};
			};
		...
		const finalAutopilot = JSON.parse(
			await readFile(autopilotPath, "utf-8"),
		) as {
			active?: boolean;
			current_phase?: string;
		...
		assert.equal(
			finalAutopilot.state?.deep_interview_question?.status,
			"satisfied",
		);
		assert.equal(
			finalAutopilot.state?.deep_interview_question?.question_id,
			"question-autopilot-1",
		);
