		...
		return {
			frontmatter: {
				...
				category: (fm.category ||
					"reference") as WikiPageFrontmatter["category"],
...
function safeWikiPath(wikiDir: string, filename: string): string | null {
	...
	if (
		resolved !== resolvedWikiDir &&
		!resolved.startsWith(`${resolvedWikiDir}${sep}`)
	) {
		return null;
	}
	return filePath;
...
export function writePageUnsafe(
	root: string,
	page: WikiPage,
		...
		for (const page of byCategory.get(category) ?? []) {
			...
			const summary =
				summaryLine.length > 80
					? `${summaryLine.slice(0, 77)}...`
					: summaryLine;
			lines.push(
				`- [${page.frontmatter.title}](${page.filename}) — ${summary}`,
			);
		}
		lines.push("");
	}
