...
export function onPreCompact(data: { cwd?: string }): {
	...
	try {
		...
		const categories = [
			...new Set(allPages.map((page) => page.frontmatter.category)),
		];
		const latestUpdate =
			allPages
...
export function onPostCompact(data: { cwd?: string }): {
	additionalContext?: string;
} {
	try {
		const root = data.cwd || process.cwd();
...
function feedProjectMemory(root: string): void {
	try {
		...
		const parsed = JSON.parse(
			readFileSync(projectMemoryPath, "utf8"),
		...
		const existingUpdated = existing
			? new Date(existing.frontmatter.updated).getTime()
			: 0;
		...
		if (Array.isArray(parsed.directives) && parsed.directives.length > 0) {
			...
			for (const directive of parsed.directives.slice(0, 20)) {
				const content =
					typeof (directive as { directive?: unknown }).directive === "string"
