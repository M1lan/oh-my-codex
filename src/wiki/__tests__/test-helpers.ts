...
function matchesArrayContaining(
	actual: unknown,
	expected: ArrayContaining,
): boolean {
	if (!Array.isArray(actual)) return false;
	return expected.expected.every((item) =>
	...
	return {
		...
		toEqual(expected: unknown) {
			if (
				...
				(expected as Partial<ArrayContaining>).__matcher === ARRAY_CONTAINING
			) {
