...
describe("Wiki lifecycle hooks", () => {
	...
	it("respects autoCapture=false from project .omx-config.json", () => {
		...
		expect(
			wikiEntries.filter((entry: string) => entry.startsWith("session-log-")),
		).toHaveLength(0);
	...
	it("refreshes the wiki index after session-end capture", () => {
		...
		expect(
			onSessionEnd({ cwd: tempDir, session_id: "session-abcdefgh" }),
		).toEqual({ continue: true });
	...
	it("summarizes legacy fallback on session start without creating canonical wiki files", () => {
		...
		fs.writeFileSync(
			path.join(tempDir, ".omx", "project-memory.json"),
			JSON.stringify({
				techStack:
					"Do not sync this into canonical storage during legacy fallback.",
			}),
		);
		fs.writeFileSync(
