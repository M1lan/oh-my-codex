...
test("rejects absolute completion-audit paths", async () => {
	...
	try {
		const result = evaluateRalphCompletionAuditEvidence(
			{ completion_audit_path: "/etc/hosts" },
			cwd,
		);
...
test("rejects non-JSON completion-audit artifacts", async () => {
	...
	try {
		await writeFile(
			join(cwd, "audit.md"),
			"passed with checklist and tests",
			"utf-8",
		);

		const result = evaluateRalphCompletionAuditEvidence(
			{ completion_audit_path: "audit.md" },
			cwd,
		);
...
test("rejects completion-audit symlinks that resolve outside the workspace", async () => {
	...
	const outside = await mkdtemp(
		join(tmpdir(), "omx-ralph-audit-symlink-outside-"),
	);
	try {
		await writeFile(
			join(outside, "audit.json"),
		...
		await writeFile(
			...
			JSON.stringify({
				...
				verification_evidence: [
					"node --test dist/ralph/__tests__/completion-audit.test.js",
				],
			}),
			"utf-8",
		);
