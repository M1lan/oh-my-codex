...
describe("ensureCanonicalRalphArtifacts", () => {
	it("keeps canonical files authoritative when they already exist", async () => {
		...
		try {
			...
			await writeFile(
				canonicalProgress,
				JSON.stringify({ canonical: true }, null, 2),
			);
			await writeFile(
				join(cwd, ".omx", "prd.json"),
				JSON.stringify({ project: "legacy-project" }),
			);
	...
	it("migrates legacy PRD/progress files one-way when canonical artifacts are absent", async () => {
		...
		try {
				...
				JSON.stringify(
					{
						...
						userStories: [
							{ id: "US-1", title: "Story", acceptanceCriteria: ["A", "B"] },
						],
					},
					null,
					2,
			...
			const canonicalProgress = JSON.parse(
				await readFile(result.canonicalProgressPath, "utf-8"),
			);
			...
			assert.equal(
				await readFile(legacyProgressPath, "utf-8"),
				legacyProgressBefore,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("records visual feedback with numeric and qualitative guidance for the next iteration", async () => {
		...
		try {
			...
			const progress = JSON.parse(
				await readFile(artifacts.canonicalProgressPath, "utf-8"),
			);
			...
			assert.equal(
				progress.visual_feedback[0].qualitative_feedback.summary,
				"Layout is close but CTA still misaligned.",
			);
			assert.equal(
				Array.isArray(
			...
			assert.equal(
				progress.visual_feedback[0].qualitative_feedback.next_actions.length >
					0,
				true,
			);
			assert.equal(
				progress.visual_feedback[0].next_actions.length <=
