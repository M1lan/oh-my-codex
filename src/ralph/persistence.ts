...
async function readCanonicalProgressLedger(
	...
	try {
		const parsed = JSON.parse(
			await readFile(canonicalProgressPath, "utf-8"),
		) as RalphProgressLedger;
		...
		return {
			...parsed,
			schema_version:
				typeof parsed.schema_version === "number" ? parsed.schema_version : 2,
			entries,
			visual_feedback,
			created_at:
...
async function writeMigrationMarker(
	...
	if (existsSync(markerPath)) {
		try {
			existing = JSON.parse(await readFile(markerPath, "utf-8")) as Record<
				string,
				unknown
			>;
		} catch {
...
async function migrateLegacyPrdIfNeeded(
	...
	while (existsSync(canonicalPrdPath)) {
		canonicalPrdPath = join(
			plansDir,
			`prd-${timestamp}-${baseSlug}-${counter}.md`,
		);
		counter += 1;
	}
...
export async function recordRalphVisualFeedback(
	...
	const canonicalProgressPath = resolveRalphProgressPath(
		cwd,
		sessionId,
		baseStateDir,
	);
...
export async function ensureCanonicalRalphArtifacts(
	...
	const canonicalProgressPath = join(
		getStateDir(cwd, sessionId),
		"ralph-progress.json",
	);
	...
	const migratedPrdResult = await migrateLegacyPrdIfNeeded(
		cwd,
		canonicalPrdFiles.at(-1),
	);
