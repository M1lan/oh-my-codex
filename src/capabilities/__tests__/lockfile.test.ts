...
describe("capabilities lockfile", () => {
	it("uses a repo-visible default lock path", () => {
		assert.equal(
			defaultCapabilitiesLockfilePath("/repo"),
			join("/repo", DEFAULT_CAPABILITIES_LOCKFILE),
		);
	});
	...
	it("checks a freshly written lockfile successfully", async () => {
		await withTempDir(async (dir) => {
			...
			await writeCapabilitiesLockfile(
				path,
				await buildCapabilitiesLockfile({ cwd: dir }),
			);
	...
	it("validates successful observations", async () => {
		await withTempDir(async (dir) => {
					...
					observations: [
						{
							...
							tool_calls: [
								{ name: "project_memory_write", arguments: { memory: {} } },
							],
						},
						{
			...
			const result = await checkCapabilitiesPreflight({
				cwd: dir,
				observationsPath: "observations.json",
		...
		await withTempDir(async (dir) => {
			...
			await writeFile(
				...
				JSON.stringify({
					...
					observations: [
						{
							fixture_id: "project-memory-write-required-arg",
							tool_calls: [{ name: "project_memory_write", arguments: {} }],
						},
						{
							fixture_id: "project-memory-read-known-tool",
							tool_calls: [{ name: "project_memory_read", arguments: {} }],
			...
			assert.ok(
				result.failures.some(
					(failure) => failure.code === "missing_required_arg",
				),
			);
		});
	});
	...
	it("fails hallucinated tool observations", async () => {
		await withTempDir(async (dir) => {
			...
			await writeFile(
				...
				JSON.stringify({
					...
					observations: [
						...
						{
							fixture_id: "project-memory-read-known-tool",
							tool_calls: [{ name: "imaginary_tool", arguments: {} }],
						},
						{ fixture_id: "tool-restraint-no-call", tool_calls: [] },
					],
				}),
			...
			assert.equal(result.ok, false);
			assert.ok(
				result.failures.some((failure) => failure.code === "hallucinated_tool"),
			);
		});
	});
	...
	it("fails tool-restraint observations that call a tool", async () => {
		await withTempDir(async (dir) => {
			...
			assert.ok(
				result.failures.some(
					(failure) => failure.code === "unexpected_tool_call",
				),
			);
		});
	});
	...
	it("changes configured tool digest when lockfile tool schema drifts", async () => {
		await withTempDir(async (dir) => {
			...
			assert.ok(
				result.failures.some(
					(failure) => failure.code === "configured_tool_surface_mismatch",
				),
			);
