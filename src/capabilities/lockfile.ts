...
export async function buildCapabilitiesLockfile(
	options: CapabilityCheckOptions = {},
): Promise<CapabilitiesLockfile> {
	...
	const fixtures = buildFixtureContracts(
		toolSurface.tools.filter((tool) => tool.enabled).map((tool) => tool.name),
	);
	return {
...
export async function writeCapabilitiesLockfile(
	path: string,
...
export async function checkCapabilitiesPreflight(
	options: CapabilityCheckOptions = {},
): Promise<CapabilityCheckResult> {
	const cwd = options.cwd ?? process.cwd();
	const lockfile = options.lockfilePath
		? resolve(cwd, options.lockfilePath)
		: defaultCapabilitiesLockfilePath(cwd);
	...
	try {
		...
		return emptyResult(
			...
			code === "lockfile_missing"
				? "Capabilities lockfile is missing."
				: "Capabilities lockfile is invalid JSON.",
		);
	}
	if (
		expected.version !== CAPABILITIES_LOCKFILE_VERSION ||
		expected.kind !== "omx_capabilities_lock"
	) {
		return emptyResult(
			lockfile,
			"lockfile_unsupported_version",
			"Capabilities lockfile version is unsupported.",
		);
	}

	const current = await buildCapabilitiesLockfile({
		cwd,
	...
	compareDigest(
		failures,
		"configured_tool_surface_mismatch",
		"Configured tool surface changed.",
		expected.surfaces.configured_tools.digest,
		current.surfaces.configured_tools.digest,
	);
	compareDigest(
		failures,
		"skill_surface_mismatch",
		"Skill surface changed.",
		expected.surfaces.skills.digest,
		current.surfaces.skills.digest,
	);
	compareDigest(
		failures,
		"agent_surface_mismatch",
		"Agent surface changed.",
		expected.surfaces.agents.digest,
		current.surfaces.agents.digest,
	);
	compareDigest(
		failures,
		"fixture_contract_mismatch",
		"Fixture contracts changed.",
		expected.surfaces.fixtures.digest,
		current.surfaces.fixtures.digest,
	);

	for (const server of current.surfaces.configured_tools.external_servers) {
		const warning = {
			code: "external_schema_unavailable" as const,
			server: server.name,
			message: `External MCP server '${server.name}' has no schema snapshot.`,
		};
		if (options.strictExternalSchemas)
			failures.push({ ...warning, code: "external_schema_unavailable" });
	...
	if (options.requireObservations && !options.observationsPath) {
		failures.push({
			...
			message: "Capability observations are required.",
		});
	}
	if (options.observationsPath) {
		await validateObservations(
			resolve(cwd, options.observationsPath),
			expected,
			failures,
		);
	}
...
async function collectConfiguredToolSurface(
	cwd: string,
	codexHome?: string,
): Promise<CapabilitiesLockfile["surfaces"]["configured_tools"]> {
	const priorEnv = new Map<string, string | undefined>();
	const disabledFirstPartyServers = FIRST_PARTY_SERVERS.filter(
		(server) => process.env[FIRST_PARTY_DISABLE_ENV[server]] === "1",
	);
	...
	try {
		...
		const tools = Object.entries(builders)
			.flatMap(([server, build]) =>
				build().map((tool) => ({
					...normalizeTool(tool),
					server,
					enabled: !disabled.includes(
						server as (typeof FIRST_PARTY_SERVERS)[number],
					),
				})),
			)
			.sort((a, b) =>
		...
		const digestInput = {
			tools,
			disabled_first_party_servers: disabled,
...
async function collectExternalMcpServers(
	cwd: string,
	codexHome?: string,
): Promise<ExternalMcpServerContract[]> {
	...
	for (const configPath of configPaths) {
		...
		try {
			...
			for (const [name, config] of Object.entries(rawServers as JsonObject)) {
				...
				servers.set(name, {
					name,
					config_digest: sha256(canonicalStringify(config)),
					schema_status: "unavailable",
				});
			}
		} catch {
...
function buildFixtureContracts(allowedTools: string[]): FixtureContract[] {
	...
	const writeTool = choose(
		"project_memory_write",
		sortedAllowed[0] ?? "project_memory_write",
	);
	const readTool = choose(
		"project_memory_read",
...
async function validateObservations(
	...
	try {
		...
		) as CapabilityObservationsFile;
	} catch {
		failures.push({
			code: "observations_invalid_json",
			message: "Capability observations are invalid JSON.",
			path,
		});
		return;
	}
	if (
		parsed.version !== 1 ||
		parsed.kind !== "omx_capability_observations" ||
		!Array.isArray(parsed.observations)
	) {
		failures.push({
			code: "observations_invalid_json",
			message: "Capability observations have an unsupported shape.",
			path,
		});
		return;
	}
	const fixtures = new Map(
		lockfile.surfaces.fixtures.fixtures.map((fixture) => [fixture.id, fixture]),
	);
	const seen = new Set<string>();
	const availableTools = new Map(
		lockfile.surfaces.configured_tools.tools
	...
	for (const observation of parsed.observations) {
		...
		if (!fixture) {
			failures.push({
				...
				message: `Unknown fixture '${observation.fixture_id}'.`,
		...
		if (seen.has(observation.fixture_id)) {
			failures.push({
				...
				message: `Duplicate observation for fixture '${observation.fixture_id}'.`,
				fixture_id: observation.fixture_id,
			});
			continue;
		}
	...
	for (const fixture of fixtures.values()) {
		if (fixture.required && !seen.has(fixture.id)) {
			failures.push({
				code: "required_observation_missing",
				message: `Required observation '${fixture.id}' is missing.`,
...
function validateObservation(
	...
	availableTools: Map<string, ToolContract>,
	failures: CapabilityFailure[],
): void {
	const calls = Array.isArray(observation.tool_calls)
		? observation.tool_calls
		: [];
	if (fixture.no_tool_calls && calls.length > 0) {
		failures.push({
			code: "unexpected_tool_call",
			message: `Fixture '${fixture.id}' requires tool restraint.`,
			fixture_id: fixture.id,
		});
	}
	for (const call of calls) {
		...
		if (!tool) {
			failures.push({
				...
				message: `Tool '${call.name}' is not in the configured surface.`,
				fixture_id: fixture.id,
				tool: call.name,
			});
			continue;
		...
		if (!fixture.allowed_tools.includes(call.name)) {
			failures.push({
				...
				message: `Tool '${call.name}' is not allowed for fixture '${fixture.id}'.`,
				fixture_id: fixture.id,
				tool: call.name,
			});
		}
		if (fixture.expected_tool && call.name !== fixture.expected_tool) {
			failures.push({
				...
				message: `Expected '${fixture.expected_tool}' but observed '${call.name}'.`,
				fixture_id: fixture.id,
				tool: call.name,
			});
		}
...
function validateRequiredArgs(
	...
	if (!args || typeof args !== "object" || Array.isArray(args)) {
		if (required.length > 0) {
			failures.push({
				...
				message: `Tool '${tool.name}' is missing required arguments: ${required.join(", ")}.`,
			...
			failures.push({
				code: "arg_schema_invalid",
				message: `Tool '${tool.name}' arguments must be an object.`,
				fixture_id: fixtureId,
				tool: tool.name,
			});
		}
	...
	for (const name of required) {
		if (!Object.prototype.hasOwnProperty.call(args, name)) {
			failures.push({
				code: "missing_required_arg",
				message: `Tool '${tool.name}' is missing required argument '${name}'.`,
				fixture_id: fixtureId,
				tool: tool.name,
			});
		}
...
function validatePropertyTypes(
	...
	for (const [name, rawProperty] of Object.entries(
		...
		if (
			...
			isJsonSchemaType(args[name], expectedType)
		)
			continue;
		failures.push({
			code: "arg_schema_invalid",
			message: `Tool '${tool.name}' argument '${name}' must be ${expectedType}.`,
			fixture_id: fixtureId,
			tool: tool.name,
		});
	}
