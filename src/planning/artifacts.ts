...
function selectPlanningArtifactsBase(
	...
	const requestedPrdPath =
		prdPath == null ? null : resolveRequestedPrdPath(artifacts, prdPath);
	const selectedPrdPath =
		prdPath == null
	...
	return {
		...
		testSpecPaths: selectMatchingTestSpecsForPrd(
			selectedPrdPath,
			artifacts.testSpecPaths,
		),
		deepInterviewSpecPaths: selectDeepInterviewSpecPathsForSlug(
...
function resolveRequestedPrdPath(
	...
	const canonicalByResolvedPath = new Map(
		artifacts.prdPaths.map((artifactPath) => [
...
function boundedRepositoryContextSummary(
	sourcePath: string,
	content: string,
	...
	const limitedLines = normalizedLines.slice(
		0,
		APPROVED_REPOSITORY_CONTEXT_MAX_LINES,
	);
...
function extractApprovedRepositoryContextSection(
	...
	const headingIndex = lines.findIndex((line) =>
		/^#{1,6}\s+Approved Repository Context Summary\s*$/i.test(line.trim()),
	);
...
function readApprovedRepositoryContextSummary(
	...
	if (existsSync(sidecarPath)) {
		try {
			const sidecar = boundedRepositoryContextSummary(
				sidecarPath,
...
function readApprovedPlanText(
	...
	try {
		...
		const planSlug = artifactPathSuffix(
			latestPrdPath,
			/^prd-(?<slug>.*)\.md$/i,
		);
...
export function readLatestPlanningArtifacts(
	cwd: string,
): LatestPlanningArtifactSelection {
...
function extractTeamDagMarkdownHandoff(content: string): string | null {
	...
	while (searchFrom < content.length) {
		const headingIndex = content
			.toLowerCase()
			.indexOf("team dag handoff", searchFrom);
		if (headingIndex < 0) return null;
...
export function readTeamDagArtifactResolution(
	cwd: string,
): TeamDagArtifactResolution {
	...
	if (artifacts.prdPaths.length === 0 || artifacts.testSpecPaths.length === 0) {
		return {
			...
			prdPath: null,
			planSlug: null,
			warnings: ["planning_incomplete"],
		};
	}

	const selection = selectPlanningArtifactsBase(artifacts);
	const prdPath = selection.prdPath;
	const planSlug = prdPath
		? artifactPathSuffix(prdPath, /^prd-(?<slug>.*)\.md$/i)
		: null;
	if (!prdPath || !planSlug) {
		return {
			source: "none",
	...
	if (selection.testSpecPaths.length === 0) {
		return {
			...
			prdPath,
			planSlug,
			warnings: ["missing_matching_test_spec"],
		};
	}
	...
	try {
		...
		if (markdownHandoff) {
			return {
				source: "markdown-handoff",
				prdPath,
				planSlug,
				content: markdownHandoff,
				warnings: [],
			};
		}
	} catch {
...
const TEAM_LAUNCH_HINT_PATTERN_SOURCE = String.raw`(?<command>(?:omx\s+team|\$team)\s+(?<ralph>ralph\s+)?(?<count>\d+)(?::(?<role>[a-z][a-z0-9-]*))?\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'))`;
const RALPH_LAUNCH_HINT_PATTERN_SOURCE = String.raw`(?<command>(?:omx\s+ralph|\$ralph)\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'))`;
	...
	if (normalizedCommand) {
		const exactMatches = matches.flatMap((match) => {
			...
			const task = match.groups?.task
				? decodeApprovedExecutionQuotedValue(match.groups.task)
				: null;
			if (!task) {
				return [];
			}
...
function buildRequestedTeamLaunchSignatureMatchFilter(
	...
	return (match: RegExpMatchArray, _task: string) =>
		matchesRequestedTeamLaunchSignature(match, options);
}
...
function readApprovedExecutionLaunchHintOutcomeForPrdPath(
	...
	const approvedPlan = readApprovedPlanText(
		cwd,
		{ ...options, prdPath },
		artifacts,
	);
	...
	if (mode === "team") {
		...
		return {
			...
			hint: {
				mode,
				command:
					normalizeLaunchHintCommandFromMatch(mode, selected.match) ??
					selected.match.groups.command,
				task: selected.task,
				workerCount,
...
function isApprovedExecutionLaunchHintReady(
...
function resolveOlderReusableSameLineageHint(
	...
	const orderedPrdPaths = [...artifacts.prdPaths].sort(
		comparePlanningArtifactPaths,
	);
	...
	for (let index = latestIndex - 1; index >= 0; index -= 1) {
		...
		const outcome = readApprovedExecutionLaunchHintOutcomeForPrdPath(
			...
			mode === "team"
				? (match: RegExpMatchArray, _task: string) =>
						sameTeamLaunchSignatureMatch(anchorHint, match)
				: undefined,
			artifacts,
		);
...
export function readApprovedExecutionLaunchHintOutcome(
	...
	for (const prdPath of orderedPrdPathsNewestFirst(artifacts.prdPaths)) {
		...
		const teamLineageMatchFilter =
			...
			teamLineageAnchor
				? (match: RegExpMatchArray, _task: string) =>
						sameTeamLaunchSignatureMatch(teamLineageAnchor, match)
				: undefined);
		const outcome = readApprovedExecutionLaunchHintOutcomeForPrdPath(
			cwd,
			mode,
...
export function readApprovedExecutionLaunchHint(
	...
	if (
		...
		!isApprovedExecutionLaunchHintReady(outcome.hint)
	) {
		return null;
	}
	return outcome.hint;
