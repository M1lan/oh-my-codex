...
describe("planning artifacts", () => {
	...
	it("round-trips single-quoted approved execution tasks with only escaped apostrophes normalized", () => {
		...
		assert.equal(
			decodeApprovedExecutionQuotedValue(
				encodeApprovedExecutionTask(task, "single"),
			),
			task,
		);
	});
	...
	it("round-trips double-quoted approved execution tasks without applying JSON escapes", () => {
		...
		assert.equal(
			decodeApprovedExecutionQuotedValue(
				encodeApprovedExecutionTask(task, "double"),
			),
			task,
		);
		...
		assert.equal(
			decodeApprovedExecutionQuotedValue(
				encodeApprovedExecutionTask(literalEscapeTask, "double"),
			),
			literalEscapeTask,
		);
	});
	...
	it("resolves matching Team DAG sidecar before markdown handoff", async () => {
		...
		await writeFile(
			join(plansDir, "team-dag-repo-aware.json"),
			'{"source":"sidecar"}\n',
		);
	...
	it("does not approve latest PRD launch hints without a matching test spec slug", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-other.md"),
			"# Other Test Spec\n",
		);
	...
	it("prefers timestamped PRD/test-spec pairs while keeping legacy artifacts compatible", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-legacy.md"),
			"# Legacy Test Spec\n",
		);
		await writeFile(
		...
		await writeFile(
			join(plansDir, "test-spec-20260427T153100Z-alpha.md"),
			"# Alpha Timestamped Test Spec\n",
		);
		await writeFile(
			join(specsDir, "deep-interview-alpha.md"),
			"# Alpha Legacy Deep Interview\n",
		);
		await writeFile(
			join(specsDir, "deep-interview-20260427T153100Z-alpha.md"),
			"# Alpha Timestamped Deep Interview\n",
		);
		await writeFile(
			join(specsDir, "deep-interview-autoresearch-20260427T153100Z-alpha.md"),
			"# Autoresearch Draft\n",
		);
	...
	it("parses $ralph aliases with single-quoted task text for approved launch hints", async () => {
		...
		await writeFile(
			join(specsDir, "deep-interview-issue-1072.md"),
			"# Deep Interview Spec\n",
		);
		...
		assert.deepEqual(hint?.testSpecPaths, [
			join(plansDir, "test-spec-issue-1072.md"),
		]);
		assert.deepEqual(hint?.deepInterviewSpecPaths, [
			join(specsDir, "deep-interview-issue-1072.md"),
		]);
	...
	it("parses $team aliases with single-quoted task text for approved launch hints", async () => {
		...
		await writeFile(
			join(specsDir, "deep-interview-issue-1142.md"),
			"# Deep Interview Spec\n",
		);
		...
		assert.equal(
			hint?.command,
			"$team ralph 4:debugger 'Execute approved issue 1142 plan'",
		);
		...
		assert.deepEqual(hint?.testSpecPaths, [
			join(plansDir, "test-spec-issue-1142.md"),
		]);
		assert.deepEqual(hint?.deepInterviewSpecPaths, [
			join(specsDir, "deep-interview-issue-1142.md"),
		]);
	...
	it("binds approved team handoff context to the selected PRD slug in multi-plan repos", async () => {
		...
		await writeFile(
			join(specsDir, "deep-interview-zeta.md"),
			"# Zeta Deep Interview\n",
		);
	...
	it("does not bind requested PRD aliases that do not resolve to a discovered canonical PRD", async () => {
		...
		for (const prdPath of rejectedAliases) {
			assert.equal(
				readApprovedExecutionLaunchHintOutcome(tempDir, "ralph", { prdPath })
					.status,
				"absent",
			);
		}
	...
	it("fails closed for bare Ralph lookups when a single plan lists multiple Ralph launch hints", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-issue-909-bare.md"),
			"# Test Spec\n",
		);
	...
	it("ignores Ralph launch hints that appear only inside indented code blocks", async () => {
		...
		await writeFile(
			...
			["# PRD", "", `    omx ralph ${JSON.stringify(task)}`].join("\n"),
		);
		await writeFile(
			join(plansDir, "test-spec-hidden-indented-ralph.md"),
			"# Test Spec\n",
		);
	...
	it("does not let Ralph launch hints span hidden markdown gaps", async () => {
		...
		const scenarios = [
			...
			{
				name: "indented",
				buildHiddenLines: (task: string) => [`    ${JSON.stringify(task)}`],
			},
		] as const;
		...
		for (const scenario of scenarios) {
			...
			await writeFile(
				join(plansDir, `test-spec-hidden-gap-${scenario.name}-ralph.md`),
				"# Test Spec\n",
			);
	...
	it("normalizes wrapped linked-Ralph team launch hints for exact command matching", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-wrapped-visible-team-linked-ralph.md"),
			"# Test Spec\n",
		);
	...
	it("does not normalize whitespace that changes the quoted task payload", async () => {
		...
		const exactCommand = ['omx ralph "Execute embedded', 'newline task"'].join(
			"\n",
		);
		...
		await writeFile(
			join(plansDir, "test-spec-exact-command-embedded-newline-task.md"),
			"# Test Spec\n",
		);

		const exactOutcome = readApprovedExecutionLaunchHintOutcome(
			tempDir,
			"ralph",
	...
	it("ignores Team launch hints that appear only inside fenced code blocks", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-hidden-fenced-team.md"),
			"# Test Spec\n",
		);
	...
	it("ignores Ralph launch hints that appear only inside nested commented blocks", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-hidden-commented-ralph.md"),
			"# Test Spec\n",
		);
	...
	it("fails closed when a single plan repeats the same team task in multiple launch hints", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-issue-910-duplicate.md"),
			"# Test Spec\n",
		);
	...
	it("uses the requested team launch signature to disambiguate same-task launch hints", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-issue-910-signature.md"),
			"# Test Spec\n",
		);
		...
		assert.equal(
			...
			`omx team 2:executor ${JSON.stringify(sharedTask)}`,
		);
	...
	it("resolves Team launch hints when PRD recommends Team plus Ultragoal and separates Ralph follow-up", async () => {
		const plansDir = join(tempDir, ".omx", "plans");
		const sharedTask =
			"Implement durable parallel delivery with Team and Ultragoal";
		await mkdir(plansDir, { recursive: true });
		await writeFile(
		...
		await writeFile(
			join(plansDir, "test-spec-ultragoal-team-handoff.md"),
			"# Test Spec\n",
		);
		...
		if (outcome.status !== "resolved") {
			throw new Error(
				"expected Team + Ultragoal PRD to resolve a team launch hint",
			);
		}
		assert.equal(outcome.hint.linkedRalph, false);
	...
	it("keeps same-task team launch-hint selection ambiguous when the full signature repeats", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-issue-910-same-signature.md"),
			"# Test Spec\n",
		);
	...
	it("rehydrates the exact team launch hint by command when one PRD repeats the same task", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-issue-910-command.md"),
			"# Test Spec\n",
		);
	...
	it("fails closed for bare team lookups when a single plan lists multiple team launch hints", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-issue-910-bare.md"),
			"# Test Spec\n",
		);
	...
	it("attaches bounded approved repository context from a matching latest-plan sidecar", async () => {
		...
		await writeFile(
			...
			"Key files: src/planning/artifacts.ts\n".repeat(120),
		);
		...
		assert.match(
			hint.repositoryContextSummary.content,
			/Key files: src\/planning\/artifacts\.ts/,
		);
	...
	it("prefers exact timestamped repository context sidecars for timestamped PRDs", async () => {
		...
		await writeFile(
			join(plansDir, "repo-context-alpha.md"),
			"stale alpha context\n",
		);
		await writeFile(
	...
	it("falls back to an inline approved repository context section when no sidecar exists", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-inline.md"),
			"# Inline Test Spec\n",
		);
	...
	it("surfaces deep-interview specs for downstream traceability", async () => {
		...
		await writeFile(
			join(specsDir, "deep-interview-issue-827.md"),
			"# Deep Interview Spec\n",
		);
	...
	it("loads a matching Team DAG sidecar for the latest PRD slug", async () => {
		...
		await writeFile(
			join(plansDir, "team-dag-alpha.json"),
			JSON.stringify({
				schema_version: 1,
				nodes: [
					{
						...
						description: "Implement alpha DAG",
					},
				],
			}),
		);
	...
	it("does not overmatch sidecars for a different slug prefix", async () => {
		...
		await writeFile(
			join(plansDir, "team-dag-foobar.json"),
			JSON.stringify({
				schema_version: 1,
				nodes: [
					{
						id: "wrong",
						subject: "Wrong slug",
						description: "Must not match foo",
					},
				],
			}),
		);
	...
	it("prefers sidecar DAG over embedded PRD Team DAG Handoff block", async () => {
		...
		await writeFile(
			join(plansDir, "team-dag-beta.json"),
			JSON.stringify({
				schema_version: 1,
				nodes: [
					{
						id: "sidecar",
						subject: "Sidecar wins",
						description: "Sidecar DAG",
					},
				],
			}),
		);
	...
	it("reports multiple matching sidecars and chooses the lexicographically latest", async () => {
		...
		await writeFile(
			join(plansDir, "team-dag-gamma-a.json"),
			JSON.stringify({
				schema_version: 1,
		...
		await writeFile(
			join(plansDir, "team-dag-gamma-z.json"),
			JSON.stringify({
				schema_version: 1,
	...
	it("does not load a Team DAG handoff when the latest PRD lacks a matching test spec", async () => {
		...
		await writeFile(
			join(plansDir, "team-dag-epsilon.json"),
			JSON.stringify({
				schema_version: 1,
				nodes: [
					{
						id: "impl",
						subject: "Implement epsilon",
						description: "Implement epsilon DAG",
					},
				],
			}),
		);
	...
	it("rejects a Team DAG sidecar whose declared plan_slug does not match the latest PRD", async () => {
		...
		await writeFile(
			join(plansDir, "team-dag-zeta.json"),
			JSON.stringify({
				schema_version: 1,
				plan_slug: "other",
				nodes: [
					{
						id: "impl",
						subject: "Implement zeta",
						description: "Implement zeta DAG",
					},
				],
			}),
		);
