...
describe("approved execution lifecycle baseline matrix", () => {
	...
	it("keeps approved execution absent when the PRD has no matching test-spec baseline", async () => {
		...
		assert.equal(
			readApprovedExecutionLaunchHintOutcome(tempDir, "team").status,
			"absent",
		);
		assert.equal(
			readApprovedExecutionLaunchHintOutcome(tempDir, "ralph").status,
			"absent",
		);
		...
		const result = await createTeamExecStage().run(
			makeCtx({ artifacts: { ralplan: { latestPlanPath: fixture.prdPath } } }),
		);
	...
	it("reuses approved execution only with an explicit PRD and matching test-spec baseline", async () => {
		...
		if (teamOutcome.status !== "resolved")
			throw new Error("expected ready team hint");
		...
		assert.match(
			buildApprovedTeamHandoffSection(teamOutcome.hint) ?? "",
			/approved plan/i,
		);
		assert.match(
			buildApprovedTeamHandoffSection(teamOutcome.hint) ?? "",
			/matching test specs|Test specs/,
		);

		const ralphOutcome = readApprovedExecutionLaunchHintOutcome(
			tempDir,
			"ralph",
		);
		assert.equal(ralphOutcome.status, "resolved");
		if (ralphOutcome.status !== "resolved")
			throw new Error("expected ready ralph hint");
	...
	it("adds leader-owned Ultragoal checkpoint context to approved Team handoffs without breaking launch hint selection", async () => {
		...
		const teamTask =
			"Use Team + Ultragoal together for G001-team-runtime-bridge";
		await writeFile(
			prdPath,
			[
		...
		const teamOutcome = readApprovedExecutionLaunchHintOutcome(
			tempDir,
			"team",
			{
				prdPath,
		...
		assert.equal(teamOutcome.status, "resolved");
		if (teamOutcome.status !== "resolved")
			throw new Error("expected ultragoal team hint");
		...
		assert.match(
			...
			/verified \.omx\/ultragoal\/goals\.json context/,
		);
		...
		assert.match(
			section,
			/No checkpoint command is emitted from approved-plan hints/i,
		);
	...
	it("keeps same-task team selector outcomes fail-closed when launch hints are ambiguous", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-ambiguous-team.md"),
			"# ambiguous team test spec\n",
		);
