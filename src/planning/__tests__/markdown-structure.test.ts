...
function collectMatchesFromMarkdownModel(content: string): string[] {
	...
	for (const line of lines) {
		...
		matches.push(
			...Array.from(line.matchAll(MATCH_TOKEN_PATTERN), (match) => match[0]),
		);
	}
...
describe("collectMarkdownVisibleMatches", () => {
	it("collects matches from normal markdown lines", () => {
		const matches = collectMarkdownVisibleMatches(
			["# PRD", "", "match-alpha", "", "match-beta"].join("\n"),
			MATCH_TOKEN_PATTERN,
		);
	...
	it("preserves multiline matches across contiguous visible markdown while blocking hidden-gap counterfactuals", () => {
		const cases = [
			{
				...
				content: ["match-alpha", "match-beta"].join("\n"),
			...
			{
				name: "indented-gap",
				content: ["match-alpha", "    match-hidden", "match-beta"].join("\n"),
				expected: [],
			},
		] as const;
		...
		for (const { name, content, expected } of cases) {
			assert.deepEqual(
				collectMarkdownVisibleMatches(content, MULTILINE_MATCH_PATTERN).map(
					(match) => match[0],
				),
				expected,
				name,
	...
	it("ignores matches inside backtick fenced code blocks with info strings", () => {
		const matches = collectMarkdownVisibleMatches(
			["# PRD", "", "```sh", "match-hidden", "```", "", "match-visible"].join(
				"\n",
			),
			MATCH_TOKEN_PATTERN,
		);
	...
	it("ignores matches inside four-space indented code blocks", () => {
		const matches = collectMarkdownVisibleMatches(
			["# PRD", "", "    match-hidden", "", "match-visible"].join("\n"),
			MATCH_TOKEN_PATTERN,
		);
	...
	it("keeps three-space fences distinct from four-space indented fence lookalikes", () => {
		...
		assert.deepEqual(
			fencedMatches.map((match) => match[0]),
			["match-visible"],
		);
		assert.deepEqual(
			indentedMatches.map((match) => match[0]),
			["match-visible"],
		);
	});
	...
	it("keeps exact closing fences distinct from trailing-text fence lookalikes", () => {
		const closedMatches = collectMarkdownVisibleMatches(
			["# PRD", "", "```md", "match-hidden-a", "```", "match-visible"].join(
				"\n",
			),
			MATCH_TOKEN_PATTERN,
		);
		...
		assert.deepEqual(
			closedMatches.map((match) => match[0]),
			["match-visible"],
		);
		assert.deepEqual(
			stillOpenMatches.map((match) => match[0]),
			["match-visible"],
		);
	});
	...
	it("keeps matching close characters distinct from different-character fence lookalikes", () => {
		...
		assert.deepEqual(
			mismatchedMatches.map((match) => match[0]),
			["match-visible"],
		);
	});
			...
			for (const second of generatedLinePatterns) {
				for (const third of generatedLinePatterns) {
					for (const fourth of generatedLinePatterns) {
						...
						const actualMatches = collectMarkdownVisibleMatches(
							content,
							MATCH_TOKEN_PATTERN,
						).map((match) => match[0]);
						assert.deepEqual(
