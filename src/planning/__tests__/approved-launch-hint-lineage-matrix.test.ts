...
const RALPH_QUERY_KINDS: readonly RalphQueryKind[] = [
	"bare",
	"task",
	"command",
] as const;
...
function ralphVisibleHints(state: RalphPlanState): HintModel[] {
	switch (state) {
		...
		case "nonready":
			return [
				{ command: RALPH_COMMAND, task: RALPH_SHARED_TASK, ready: false },
			];
		case "ambiguous":
			return [
				{ command: RALPH_COMMAND, task: RALPH_SHARED_TASK, ready: true },
				{
					command: RALPH_DUPLICATE_COMMAND,
					task: RALPH_SHARED_TASK,
					ready: true,
				},
			];
	}
	...
	switch (state) {
		...
		case "readyAA":
			return [
				...
				{
					command: TEAM_COMMANDS.ADuplicate,
					task: TEAM_SHARED_TASK,
					ready: true,
					signature: "A",
				},
...
function resolveExpectedRalphSelection(
	...
	if (!query.task && !query.command) {
		...
		const latestSelection = selectUniqueHint(
			reusableRalphHints(states[latestIndex]!),
			query,
		);
		...
		return latestSelection.status === "unique"
			? {
					status: "resolved",
					stem: stems[latestIndex]!,
					hint: latestSelection.hint,
				}
			: { status: "absent" };
	}
	...
	for (let index = states.length - 1; index >= 0; index -= 1) {
		const selection = selectUniqueHint(
			reusableRalphHints(states[index]!),
			query,
		);
...
async function writeRalphPlanState(
	...
	await writePlanFiles(
		...
		stem,
		lines,
		state === "hidden" || state === "ready" || state === "ambiguous",
	);
}
...
async function writeRalphScenario(
	...
	await Promise.all(
		states.map((state, index) =>
			writeRalphPlanState(cwd, STEMS[index]!, state),
		),
	);
}
...
async function writeTeamScenario(
	...
	await Promise.all(
		states.map((state, index) => writeTeamPlanState(cwd, STEMS[index]!, state)),
	);
}
...
function assertExpectedSelection(
	...
	assert.equal(
		outcome.status,
		expected.status,
		`${label}: unexpected outcome status`,
	);
		...
		for (const queryKind of RALPH_QUERY_KINDS) {
			for (const olderState of RALPH_PLAN_STATES) {
				for (const latestState of RALPH_PLAN_STATES) {
					const cwd = join(
						tempDir,
						`ralph-${queryKind}-${olderState}-${latestState}`,
					);
					const states = [olderState, latestState] as const;
					await writeRalphScenario(cwd, states);
					const expected = resolveExpectedRalphSelection(
						stems,
						states,
						ralphQueryModel(queryKind),
					);
		...
		for (const queryKind of TEAM_QUERY_KINDS) {
			for (const olderState of TEAM_PLAN_STATES) {
				for (const latestState of TEAM_PLAN_STATES) {
					const cwd = join(
						tempDir,
						`team-${queryKind}-${olderState}-${latestState}`,
					);
					const states = [olderState, latestState] as const;
					await writeTeamScenario(cwd, states);
					const expected = resolveExpectedTeamSelection(
						stems,
						states,
						teamQueryModel(queryKind),
					);
					assertExpectedSelection(
						cwd,
