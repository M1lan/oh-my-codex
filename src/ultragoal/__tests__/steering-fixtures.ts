...
const baseGoals = [
	goal(
		...
		"Core steering model",
		"Implement bounded dynamic steering for .omx/ultragoal ledger state.",
	),
	goal(
		"G002-cli-bridge",
		"CLI bridge",
		"Expose structured steering through the ultragoal CLI.",
	),
	goal(
		"G003-hook-bridge",
		"Hook bridge",
		"Bridge explicit UserPromptSubmit steering directives.",
	),
];
...
export const steeringFixtures: SteeringFixture[] = [
	{
		case: "add",
		description:
			"accepted add_subgoal appends a schedule-eligible pending goal and keeps the aggregate Codex objective immutable",
		...
		proposal: {
			...
			rationale:
				"The new subgoal preserves the original objective while making audit evidence explicit.",
	...
	},
	{
		case: "split",
		description:
			"accepted split_subgoal preserves the original goal, marks it superseded, and schedules replacement children",
		before: plan(baseGoals),
		proposal: {
			kind: "split_subgoal",
			source: "finding",
			...
			rationale:
				"Splitting reduces coupling without deleting or weakening G001-core-steering-model.",
	...
	},
	{
		case: "superseded",
		description:
			"superseded original does not block final-candidate detection once replacement children are complete",
		before: plan([
			{
				...baseGoals[0]!,
				...
				supersededBy: [
					...
					"G005-core-steering-scheduler",
				],
			},
			{ ...baseGoals[1]!, status: "complete" },
			{ ...baseGoals[2]!, status: "complete" },
			{
				...goal("G004-core-steering-schema", "Core steering schema"),
				status: "complete",
			},
		...
		proposal: {
			...
			targetGoalIds: ["G001-core-steering-model"],
			evidence:
				"Replacement child G004 is complete and G005 is the only unresolved replacement.",
			rationale:
				"The superseded parent should stay audit-visible but not remain independently schedulable.",
	...
	},
	{
		...
		description:
			"blocked-without-replacement is skipped by scheduling but still blocks completion and final-candidate detection",
		before: plan([
			{
				...baseGoals[0]!,
				steeringStatus: "blocked",
				blockedReason:
					"External API behavior is unknown until upstream issue is resolved.",
			},
			{ ...baseGoals[1]!, status: "complete" },
			{ ...baseGoals[2]!, status: "complete" },
		]),
		proposal: {
			...
			targetGoalIds: ["G001-core-steering-model"],
			evidence:
				"No safe replacement exists yet for the blocked core steering slice.",
			rationale:
				"The blocked goal should not churn through retries but must prevent final completion.",
	...
	},
	{
		...
		description:
			"blocked goal stops blocking when later replacement children supersede it",
		before: plan([
			{
				...baseGoals[0]!,
				steeringStatus: "blocked",
				blockedReason: "Original implementation path is unsafe.",
			},
			{ ...baseGoals[1]!, status: "complete" },
			{ ...baseGoals[2]!, status: "complete" },
		]),
		proposal: {
			...
			after: [
				{
					title: "Core steering replacement",
					objective: "Implement the safer replacement path for core steering.",
				},
			],
			evidence:
				"A safer replacement path is now evidence-backed by the spike result.",
			rationale:
				"Superseding the blocked original keeps the audit trail while unblocking completion through replacement work.",
	...
	},
	{
		case: "reorder",
		description:
			"accepted reorder_pending changes only pending order and keeps completed/in-progress history stable",
		before: plan([
		...
		proposal: {
			...
			evidence:
				"Tests should land before hook integration so hook behavior can be verified.",
			rationale:
				"Only pending goals are reordered; completed evidence remains immutable.",
	...
	},
	{
		case: "revise",
		description:
			"accepted revise_pending_wording updates pending wording without changing completion state",
		before: plan(baseGoals),
		proposal: {
			...
			objective:
				"Expose only structured steering directives through the ultragoal CLI.",
			evidence: "Prompt-submit parser must stay bounded and structured.",
			rationale:
				"The revised wording narrows ambiguity but preserves the CLI bridge deliverable.",
	...
	},
	{
		case: "annotate",
		description:
			"accepted annotate_ledger records evidence without changing plan scheduling",
		before: plan(baseGoals),
		proposal: {
			kind: "annotate_ledger",
			source: "finding",
			...
			evidence:
				"Normal prose mentioning ultragoal should not trigger steering; only structured directives should.",
			rationale:
				"The finding clarifies hook test scope without mutating subgoals.",
	...
	},
	{
		case: "reject",
		description:
			"rejected steering records a structured rejection when a proposal tries to weaken protected aggregate state",
		before: plan(baseGoals),
		proposal: {
			kind: "revise_pending_wording",
			source: "cli",
