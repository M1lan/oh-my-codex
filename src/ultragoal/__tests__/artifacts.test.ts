...
import {
	steeringFixtures,
	type SteeringFixtureProposal,
} from "./steering-fixtures.js";
...
function cleanQualityGate(): object {
	return {
		...
		codeReview: {
			...
			independentReview: {
				codeReviewer: {
					...
					evidence: "code-reviewer subagent returned APPROVE",
				},
				architect: {
					agentRole: "architect",
					evidence: "architect subagent returned CLEAR",
				},
			},
		},
		architectureInvariantGate: {
			...
			invariants: [],
			evidence:
				"architect verified no additional architecture invariants were declared in the brief",
		},
	};
}
...
async function writeAggregateFixturePlan(
	cwd: string,
	...
	await writeFile(
		...
		"aggregate terminalization fixture\n",
	);
	await writeFile(
...
async function writeFixturePlan(
	cwd: string,
	plan: UltragoalPlan,
): Promise<void> {
	await mkdir(join(cwd, ".omx/ultragoal"), { recursive: true });
	await writeFile(
		join(cwd, ".omx/ultragoal/brief.md"),
		"G001-core-steering-model fixture for .omx/ultragoal steering behavior.\n",
	);
	await writeFile(
		join(cwd, ".omx/ultragoal/goals.json"),
...
function asChildGoals(
	after: unknown,
): Array<{ title: string; objective: string }> | undefined {
	...
	switch (proposal.kind) {
		...
		case "revise_pending_wording":
			return {
				...common,
				objective: proposal.objective,
				directiveText: proposal.forbidden
					? "attempt to skip verification, weaken quality gates, and auto-complete protected aggregate state"
					: undefined,
		...
		case "mark_blocked_superseded": {
			...
			return {
				...common,
				childGoals,
				blockedReason: childGoals
					? undefined
					: "Evidence-backed blocker has no safe replacement yet.",
			};
		}
	}
}
...
describe("ultragoal artifacts", () => {
	...
	it("derives story goals without queuing nested criteria or plain-label checklist items", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.deepEqual(
				plan.goals.map((goal) => goal.title),
				["Ship parser fix", "Add coverage"],
			);
			assert.match(
				plan.goals[0]?.objective ?? "",
				/Preserve parent story objective detail/,
			);
			assert.doesNotMatch(
				plan.goals[1]?.objective ?? "",
				/Run tests|Run lint|Parent stories only/,
			);
		});
	});
	...
	it("does not fall back to checklist bullets when a brief has only non-story sections", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.doesNotMatch(
				plan.goals[0]?.id ?? "",
				/run-tests|run-lint|parent-stories/,
			);
		});
	});
	...
	it("does not atomize RALPLAN review and consensus sections into pseudo-goals", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.doesNotMatch(
				plan.goals[0]?.id ?? "",
				/g104|verdict|review-artifact|consensus-status/,
			);
		});
	});
	...
	it("fails closed for broad implicit plan-like markdown without compact stories", async () => {
		await withTempRepo(async (cwd) => {
			const broadBrief = [
				...
				...Array.from(
					{ length: 21 },
					(_, index) => `- Review or handoff detail ${index + 1}`,
				),
			].join("\n");
	...
	it("emits final-story handoffs that block missing independent review before update_goal", async () => {
		await withTempRepo(async (cwd) => {
			...
			const started = await startNextUltragoal(cwd);
			const aggregateInstruction = buildCodexGoalInstruction(
				started.goal!,
				started.plan,
			);
			...
			assert.match(
				...
				/independentReview evidence from both code-reviewer and architect subagents/,
			);
			assert.match(
				aggregateInstruction,
				/independent delegation is unavailable\/skipped\/failed, do not call update_goal/,
			);
			assert.match(
				aggregateInstruction,
				/APPROVE \+ CLEAR \+ independent code-reviewer and architect subagent evidence/,
			);
			assert.match(
				aggregateInstruction,
				new RegExp(escapeRegExp(LEADER_CONDUCTOR_BLOCK)),
			...
			const perStoryInstruction = buildCodexGoalInstruction(
				perStory.goal!,
				perStory.plan,
			);
			...
			const unsupportedInstruction = buildCodexGoalInstruction(
				perStory.goal!,
				perStory.plan,
			...
			assert.match(
				unsupportedInstruction,
				/Native independent review unavailable/,
			);
		});
	});
	...
	it("reconciles completed task-scoped Codex proof to finish exploded aggregate ultragoal bookkeeping", async () => {
		await withTempRepo(async (cwd) => {
			const taskObjective =
				"Fix the mismatch between Codex immutable completed goal snapshots and OMX ultragoal checkpoint reconciliation.";
			await createUltragoalPlan(cwd, {
				brief: taskObjective,
			...
			const reconciled = await checkpointUltragoal(cwd, {
				...
				status: "complete",
				evidence:
					"Actual planned work done for .omx/ultragoal/goals.json G001-micro-goal-1; validation complete; reviews clean.",
			...
			assert.equal(
				reconciled.goals.filter((candidate) => candidate.status === "complete")
					.length,
				1,
			);
			assert.equal(reconciled.goals[0]?.status, "complete");
			assert.equal(
				reconciled.goals[0]?.completedAt,
			...
			assert.match(
				reconciled.aggregateCompletion?.evidence ?? "",
				/planned work done/,
			);
			...
			assert.match(
				ledger,
				/checkpointed active microgoal row was reconciled to complete/,
			);
			assert.equal(
			...
			await assert.rejects(
				() =>
					checkpointUltragoal(cwd, {
						...
						status: "complete",
						evidence:
							"Actual planned work done for .omx/ultragoal/goals.json G001-first; validation complete; reviews clean.",
						codexGoal: {
							goal: {
								objective: "Unrelated completed task",
								status: "complete",
							},
						},
						qualityGate: cleanQualityGate(),
				...
				() =>
					checkpointUltragoal(cwd, {
						...
						codexGoal: {
							goal: {
								objective:
									"Audit .omx/ultragoal/goals.json for a different unrelated task",
								status: "complete",
							},
						},
						qualityGate: cleanQualityGate(),
	...
	it("fails closed for task-scoped aggregate completion on a non-active microgoal id", async () => {
		await withTempRepo(async (cwd) => {
			...
			await assert.rejects(
				() =>
					checkpointUltragoal(cwd, {
						...
						status: "complete",
						evidence:
							"Actual planned work done for .omx/ultragoal/goals.json G002-second; validation complete; reviews clean.",
						codexGoal: {
							goal: { objective: taskObjective, status: "complete" },
						},
						qualityGate: cleanQualityGate(),
			...
			assert.equal(
				plan.goals.find((goal) => goal.id === "G001-first")?.status,
				"in_progress",
			);
			assert.equal(
				plan.goals.find((goal) => goal.id === "G002-second")?.status,
				"pending",
			);
	...
	it("requires aggregate Codex goal completion only for the final story", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.equal(
				midRunPlan.goals.find((goal) => goal.id === "G001-first")?.status,
				"complete",
			);
			assert.equal(
				midRunPlan.goals.find((goal) => goal.id === "G002-second")?.status,
				"pending",
			);
			...
			const plan = await readUltragoalPlan(cwd);
			assert.equal(
				plan.goals.find((goal) => goal.id === second.goal!.id)?.status,
				"complete",
			);
			assert.equal(
	...
	it("appends goals without changing the stored aggregate objective", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.doesNotMatch(
				added.plan.codexObjective ?? "",
				/G002-resolve-final-code-review-blockers/,
			);
	...
	it("migrates legacy enumerated aggregate objectives to the pointer contract", async () => {
		await withTempRepo(async (cwd) => {
			...
			const legacyObjective =
				"Complete all ultragoal stories in .omx/ultragoal/goals.json: G001-first First; G002-second Second";
			await writeFile(
				join(cwd, ".omx/ultragoal/goals.json"),
				`${JSON.stringify(
					{
	...
	it("accepts migrated legacy aggregate objective aliases for active Codex snapshots", async () => {
		await withTempRepo(async (cwd) => {
			...
			const checkpointed = await checkpointUltragoal(cwd, {
				...
				status: "complete",
				evidence:
					"legacy active Codex objective alias still represents the migrated aggregate run.",
				codexGoal: { goal: { objective: legacyObjective, status: "active" } },
			});

			assert.equal(checkpointed.goals[0]?.status, "complete");
			assert.equal(
				checkpointed.codexObjective,
				ULTRAGOAL_AGGREGATE_CODEX_OBJECTIVE,
			);
	...
	it("applies steering idempotently and keeps split replacements schedulable", async () => {
		await withTempRepo(async (cwd) => {
			await createUltragoalPlan(cwd, {
				...
				goals: [
					{
						title: "Core steering model",
						objective: "Implement bounded dynamic steering.",
					},
					{
						title: "CLI bridge",
						objective: "Expose structured steering through the CLI.",
					},
					{
						title: "Hook bridge",
						objective: "Bridge explicit steering directives.",
					},
				],
			});
			...
			const firstSteer = await steerUltragoal(cwd, {
				...
				childGoals: [
					{
						title: "Core steering schema",
						objective: "Add steering proposal and audit schema.",
					},
					{
						title: "Core steering scheduler semantics",
						objective:
							"Make superseded and blocked metadata affect scheduling and completion.",
					},
				],
				evidence:
					"Implementation findings show schema and scheduler invariants should be isolated.",
				rationale:
					"Splitting reduces coupling without deleting or weakening the original goal.",
			...
			assert.equal(
				firstSteer.plan.goals.find(
					(goal) => goal.id === "G001-core-steering-model",
				)?.steeringStatus,
				"superseded",
			);
			assert.equal(
				firstSteer.plan.goals.find(
					(goal) => goal.id === "G004-core-steering-schema",
				)?.supersedes?.[0],
				"G001-core-steering-model",
			);
			assert.equal(
				firstSteer.plan.goals.find(
					(goal) => goal.id === "G005-core-steering-scheduler-semantics",
				)?.supersedes?.[0],
				"G001-core-steering-model",
			);
			assert.equal(
				firstSteer.plan.goals.filter(
					(goal) => goal.steeringStatus === "superseded",
				).length,
				1,
			);
	...
	it("records final aggregate review blockers atomically and starts the blocker next", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.equal(
				result.addedGoal.resolvesReviewBlockedGoalId,
				result.blockedGoal.id,
			);
	...
	it("reconciles a review-blocked final story when the appended resolver completes with a clean quality gate", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.equal(
				isFinalRunCompletionCandidate(resolver.plan, resolver.goal!),
				true,
			);
			...
			const parent = completed.goals.find(
				(goal) => goal.id === blocked.blockedGoal.id,
			);
			...
			assert.equal(
				parent?.reviewBlockerResolution?.resolverGoalId,
				blocked.addedGoal.id,
			);
	...
	it("does not terminalize through a one-way designated resolver pointer", async () => {
		await withTempRepo(async (cwd) => {
			...
			const resolverGoal = tampered.goals.find(
				(goal) => goal.id === resolver.goal!.id,
			)!;
			delete resolverGoal.resolvesReviewBlockedGoalId;
			await writeFile(planPath, `${JSON.stringify(tampered, null, 2)}\n`);

			const parentBeforeCheckpoint = tampered.goals.find(
				(goal) => goal.id === blocked.blockedGoal.id,
			);
			assert.equal(isFinalRunCompletionCandidate(tampered, resolverGoal), true);
			assert.equal(
				parentBeforeCheckpoint?.reviewBlockerResolution?.resolverGoalId,
				resolverGoal.id,
			);
			...
			const completedResolver = completed.goals.find(
				(goal) => goal.id === resolverGoal.id,
			);
			const summary = summarizeUltragoalPlan(completed);
	...
	it("does not terminalize when two unresolved review-blocked parents point at one resolver", async () => {
		await withTempRepo(async (cwd) => {
			...
			const originalParent = tampered.goals.find(
				(goal) => goal.id === blocked.blockedGoal.id,
			)!;
			tampered.goals.push({
				...originalParent,
			...
			assert.equal(
				tampered.goals.filter((goal) => goal.status === "review_blocked")
					.length,
				2,
			);
			assert.equal(isFinalRunCompletionCandidate(tampered, resolverGoal), true);
			...
			const originalParentAfter = completed.goals.find(
				(goal) => goal.id === originalParent.id,
			);
			const secondParentAfter = completed.goals.find(
				(goal) => goal.id === "G999-forged-second-parent",
			);
			const completedResolver = completed.goals.find(
				(goal) => goal.id === resolverGoal.id,
			);
			...
			assert.equal(
				originalParentAfter?.reviewBlockerResolution?.status,
				"complete",
			);
			assert.equal(secondParentAfter?.status, "review_blocked");
			assert.equal(
				secondParentAfter?.reviewBlockerResolution?.status,
				"pending",
			);
			assert.equal(completed.aggregateCompletion, undefined);
			assert.equal(summary.reviewBlocked, 1);
	...
	it("does not terminalize when a review-blocked parent names a resolver whose back-pointer targets another parent", async () => {
		await withTempRepo(async (cwd) => {
			...
			await writeAggregateFixturePlan(cwd, "G004-final", [
				...
				aggregateFixtureGoal("G003-other", "complete", {
					completedAt: "2026-01-01T00:00:00.000Z",
					evidence: "other done",
				}),
				aggregateFixtureGoal("G004-final", "in_progress", {
			...
			assert.equal(
				isFinalRunCompletionCandidate(
					tampered,
					tampered.goals.find((goal) => goal.id === "G004-final")!,
				),
				false,
			);
			...
			await assert.rejects(
				() =>
					checkpointUltragoal(cwd, {
						...
						status: "complete",
						evidence:
							"G004-final completed planned work for .omx/ultragoal/goals.json; validation complete; reviews clean.",
						codexGoal: { goal: { objective, status: "complete" } },
						qualityGate: cleanQualityGate(),
					}),
			...
			const plan = await readUltragoalPlan(cwd);
			assert.equal(
				plan.goals.find((goal) => goal.id === "G001-parent")?.status,
				"review_blocked",
			);
			assert.equal(plan.aggregateCompletion, undefined);
	...
	it("does not terminalize when a falsely-resolved parent coexists with a legitimate designated resolver", async () => {
		await withTempRepo(async (cwd) => {
			...
			await assert.rejects(
				() =>
					checkpointUltragoal(cwd, {
						...
						status: "complete",
						evidence:
							"G004-resolver-b completed planned work for .omx/ultragoal/goals.json; validation complete; reviews clean.",
						codexGoal: { goal: { objective, status: "complete" } },
						qualityGate: cleanQualityGate(),
					}),
				/not complete|objective mismatch|Completed task-scoped aggregate reconciliation/,
			...
			const plan = await readUltragoalPlan(cwd);
			assert.equal(
				plan.goals.find((goal) => goal.id === "G001-parent-a")?.status,
				"review_blocked",
			);
			assert.equal(plan.aggregateCompletion, undefined);
			assert.equal(summarizeUltragoalPlan(plan).aggregateComplete, false);
	...
	it("does not treat a self-referential review-blocker resolver as a designated resolver", async () => {
		await withTempRepo(async (cwd) => {
			...
			const completed = await checkpointUltragoal(cwd, {
				...
				status: "complete",
				evidence:
					"G001-self completed planned work for .omx/ultragoal/goals.json; validation complete; reviews clean.",
				codexGoal: { goal: { objective, status: "complete" } },
				qualityGate: cleanQualityGate(),
			});
	...
	it("does not terminalize when the final aggregate candidate is steering-blocked", async () => {
		await withTempRepo(async (cwd) => {
			...
			await writeAggregateFixturePlan(cwd, "G002-target", [
				aggregateFixtureGoal("G001-first", "complete", {
					completedAt: "2026-01-01T00:00:00.000Z",
					evidence: "first done",
				}),
				aggregateFixtureGoal("G002-target", "in_progress", {
					startedAt: "2026-01-01T00:00:00.000Z",
			...
			assert.equal(
				isFinalRunCompletionCandidate(
					tampered,
					tampered.goals.find((goal) => goal.id === "G002-target")!,
				),
				true,
			);
			...
			const completed = await checkpointUltragoal(cwd, {
				...
				status: "complete",
				evidence:
					"G002-target completed planned work for .omx/ultragoal/goals.json; validation complete; reviews clean.",
				codexGoal: { goal: { objective, status: "complete" } },
				qualityGate: cleanQualityGate(),
			});

			assert.equal(
				completed.goals.find((goal) => goal.id === "G002-target")
					?.steeringStatus,
				"blocked",
			);
			assert.equal(completed.aggregateCompletion, undefined);
	...
	it("does not terminalize when duplicate goal ids hide an unresolved row", async () => {
		await withTempRepo(async (cwd) => {
			...
			await writeAggregateFixturePlan(cwd, "G002-dup", [
				...
				aggregateFixtureGoal("G002-dup", "in_progress", {
					startedAt: "2026-01-01T00:00:00.000Z",
				}),
			...
			assert.equal(
				isFinalRunCompletionCandidate(
					tampered,
					tampered.goals.find((goal) => goal.id === "G002-dup")!,
				),
				true,
			);
			...
			const completed = await checkpointUltragoal(cwd, {
				...
				status: "complete",
				evidence:
					"G002-dup completed planned work for .omx/ultragoal/goals.json; validation complete; reviews clean.",
				codexGoal: { goal: { objective, status: "complete" } },
				qualityGate: cleanQualityGate(),
			});
			...
			assert.equal(
				completed.goals.filter(
					(goal) => goal.id === "G002-dup" && goal.status === "pending",
				).length,
				1,
			);
			assert.equal(completed.aggregateCompletion, undefined);
	...
	it("does not terminalize a final aggregate candidate that is not the active in-progress goal", async () => {
		...
		for (const unowned of [
			...
			await withTempRepo(async (cwd) => {
				...
				assert.equal(
					summarizeUltragoalPlan(completed).aggregateComplete,
					false,
					unowned,
				);
				const ledger = await readFile(
	...
	it("pins allowActiveFinalCodexGoal as a required negative condition for terminal aggregate completion", async () => {
		await withTempRepo(async (cwd) => {
			...
			const completed = await checkpointUltragoal(cwd, {
				...
				status: "complete",
				evidence:
					"final story completed while the aggregate Codex goal stays active",
				codexGoal: {
					goal: { objective: aggregateObjective, status: "active" },
				},
			...
			assert.equal(
				completed.goals.find((goal) => goal.id === second.goal!.id)?.status,
				"complete",
			);
			assert.equal(completed.aggregateCompletion, undefined);
			assert.equal(summarizeUltragoalPlan(completed).aggregateComplete, false);
	...
	it("does not terminalize an in-progress final candidate whose activeGoalId pointer does not match", async () => {
		...
		for (const activePointer of ["G001-first", undefined] as const) {
			await withTempRepo(async (cwd) => {
				...
				const tampered = await readUltragoalPlan(cwd);
				const target = tampered.goals.find(
					(goal) => goal.id === "G002-target",
				)!;
	...
	it("freezes a completed aggregate plan against post-terminal failure checkpoints and goal additions", async () => {
		await withTempRepo(async (cwd) => {
			...
			const terminal = await checkpointUltragoal(cwd, {
				...
				codexGoal: {
					goal: { objective: started.plan.codexObjective!, status: "complete" },
				},
				qualityGate: cleanQualityGate(),
			});
			assert.equal(terminal.aggregateCompletion?.status, "complete");
			...
			await assert.rejects(
				() =>
					addUltragoalGoal(cwd, {
						title: "Post terminal",
						objective: "Added after completion.",
					}),
	...
	it("does not reconcile a review-blocked parent from a non-designated resolver goal", async () => {
		await withTempRepo(async (cwd) => {
			...
			const completed = await checkpointUltragoal(cwd, {
				...
				status: "complete",
				evidence:
					"forged resolver completed with tests but is not the designated review blocker resolver",
				codexGoal: { goal: { objective, status: "active" } },
			});
			const parent = completed.goals.find(
				(goal) => goal.id === blocked.blockedGoal.id,
	...
	it("fails closed when a forged non-designated resolver presents completed task-scoped aggregate proof", async () => {
		await withTempRepo(async (cwd) => {
			const taskObjective =
				"Fix review-blocked ultragoal resolver reconciliation tracked in .omx/ultragoal/goals.json without allowing forged aggregate completion.";
			await createUltragoalPlan(cwd, {
				brief: taskObjective,
				goals: [{ title: "Final", objective: "Complete final milestone." }],
			});
			...
			await assert.rejects(
				() =>
					checkpointUltragoal(cwd, {
						...
						status: "complete",
						evidence:
							"G999-forged-resolver completed planned work for .omx/ultragoal/goals.json; passed tests; final quality gate clean.",
						codexGoal: {
							goal: { objective: taskObjective, status: "complete" },
						},
						qualityGate: cleanQualityGate(),
			...
			const plan = await readUltragoalPlan(cwd);
			const parent = plan.goals.find(
				(goal) => goal.id === blocked.blockedGoal.id,
			);
			const designatedResolver = plan.goals.find(
				(goal) => goal.id === blocked.addedGoal.id,
			);
			const forgedResolver = plan.goals.find(
				(goal) => goal.id === "G999-forged-resolver",
			);
			const summary = summarizeUltragoalPlan(plan);
					...
					checkpointUltragoal(cwd, {
						...
						qualityGate: {
							...
							codeReview: {
								...
								architectStatus: "CLEAR",
								evidence:
									"same execution lane self-reviewed and approved without spawning review subagents",
							},
						},
					}),
					...
					checkpointUltragoal(cwd, {
						...
						qualityGate: {
							...
							nativeSubagentSupport: {
								...
								evidenceSummary:
									"native subagents are disabled in this runtime",
							},
						},
					}),
				/independent review unavailable|self-approving/i,
						...
						qualityGate: {
							...
							codeReview: {
								...
								independentReview: {
									codeReviewer: {
										agentRole: "executor",
										evidence: "authoring lane approved its own change",
									},
									architect: {
										agentRole: "architect",
										evidence: "architect subagent returned CLEAR",
			...
			await checkpointUltragoal(cwd, {
				...
				qualityGate: {
					...
					architectureInvariantGate: {
						...
						evidence:
							"all declared invariants have implementation, test, and review proof",
						invariants: [
							{
								...
								implementationEvidence:
									"parser changes stayed inside src/parser without scheduler coupling",
								testEvidence: "parser boundary regression test passed",
								reviewEvidence:
									"architect review confirmed parser boundary remained intact",
							},
							{
								...
								status: "proved",
								implementationEvidence:
									"implementation reused the existing scheduler entrypoint",
								testEvidence: "scheduler singleton regression passed",
								reviewEvidence:
									"architect review confirmed no duplicate scheduler path",
							},
						],
					},
				},
	...
	it("requires final architecture invariant proof from accepted steering annotations", async () => {
		await withTempRepo(async (cwd) => {
			await createUltragoalPlan(cwd, {
				...
				goals: [
					{
						title: "Audit steering invariant",
						objective: "Accept steering invariant annotation.",
					},
			...
			await steerUltragoal(cwd, {
				...
				source: "finding",
				evidence:
					"Reviewer finding. Architecture invariant: Ledger entries remain append-only",
				rationale:
					"Non-negotiable architecture invariant: Ledger entries remain append-only",
			});
			await checkpointUltragoal(cwd, {
				goalId: first.goal!.id,
				status: "complete",
				...
				codexGoal: {
					goal: { objective: first.plan.codexObjective!, status: "active" },
				},
				allowActiveFinalCodexGoal: true,
			});
			...
			await checkpointUltragoal(cwd, {
				...
				qualityGate: {
					...
					architectureInvariantGate: {
						...
						sourceArtifacts: [".omx/ultragoal/ledger.jsonl"],
						evidence:
							"accepted steering invariant has implementation, test, and review proof",
						invariants: [
							{
								invariant: "Ledger entries remain append-only",
								source:
									".omx/ultragoal/ledger.jsonl#steering-3-inline-architecture-invariant",
								status: "proved",
								implementationEvidence:
									"appendLedger only appends JSONL records",
								testEvidence: "ledger append-only regression passed",
								reviewEvidence:
									"architect review confirmed ledger mutation remains append-only",
							},
						],
					},
				},
					...
					checkpointUltragoal(cwd, {
						...
						qualityGate: {
							...
							architectureInvariantGate: {
								...
								evidence:
									"decorative label claims the invariant came from the brief",
								invariants: [
									{
										invariant: "Preserve the existing parser boundary.",
										...
										reviewEvidence:
											"architect review confirmed parser boundary",
									},
								],
							},
						},
					...
					checkpointUltragoal(cwd, {
						...
						qualityGate: {
							...
							architectureInvariantGate: {
								...
								evidence:
									"source artifact is listed but record source is decorative",
								invariants: [
									{
										invariant: "Preserve the existing parser boundary.",
										source: "review-note: brief architecture invariant",
	...
	it("blocks final completion when architecture invariants are unproved or carry blockers", async () => {
		await withTempRepo(async (cwd) => {
								...
								invariants: [
									{
										...
										status: "blocked",
										implementationEvidence:
											"mutation path still rewrites prior entries",
			...
			const result = await recordFinalReviewBlockers(cwd, {
				...
				objective:
					"Prove ledger append-only behavior and rerun final quality gates.",
				evidence:
					"architectureInvariantGate found unproved invariant: Ledger entries remain append-only.",
				codexGoal: { goal: { objective, status: "active" } },
			});
			assert.equal(result.blockedGoal.status, "review_blocked");
		...
		await withTempRepo(async (cwd) => {
			await createUltragoalPlan(cwd, {
				...
				goals: [
					...
					{
						title: "Second",
						objective: "Complete second milestone with tests.",
					},
				],
			});

			const added = await steerUltragoal(
				cwd,
				{
					kind: "add_subgoal",
					source: "cli",
					...
					rationale:
						"Add a bounded follow-up without weakening the aggregate objective.",
					title: "Add migration regression test",
					objective:
						"Add migration regression coverage and keep all existing quality gates.",
			...
			assert.equal(
				added.plan.goals.at(-1)?.id,
				"G003-add-migration-regression-test",
			);
			assert.equal((added.audit.before as UltragoalPlan).goals.length, 2);
			assert.equal(
				(added.audit.after as { id?: string }).id,
				"G003-add-migration-regression-test",
			);

			const split = await steerUltragoal(
				cwd,
				{
					kind: "split_subgoal",
					source: "finding",
					...
					evidence:
						"Implementation evidence shows the second milestone has two independent safety checks.",
					rationale:
						"Split the pending work into narrower goals while preserving verification burden.",
					childGoals: [
						{
							title: "Second parser coverage",
							objective:
								"Complete parser coverage for the second milestone with tests.",
						},
						{
							title: "Second CLI coverage",
							objective:
								"Complete CLI coverage for the second milestone with tests.",
						},
					],
				},
			...
			const superseded = split.plan.goals.find(
				(goal) => goal.id === "G002-second",
			);
			assert.equal(superseded?.steeringStatus, "superseded");
			assert.deepEqual(superseded?.supersededBy, [
				"G004-second-parser-coverage",
				"G005-second-cli-coverage",
			]);
			assert.equal(
				split.plan.goals.find(
					(goal) => goal.id === "G004-second-parser-coverage",
				)?.supersedes?.[0],
				"G002-second",
			);

			const revised = await steerUltragoal(
				cwd,
				{
					...
					evidence:
						"Prompt-submit clarified the regression target after the goal was added.",
					rationale:
						"Clarify wording only; do not change acceptance or verification gates.",
					revisedTitle: "Add ledger migration regression test",
					revisedObjective:
						"Add ledger migration regression coverage and keep all existing quality gates.",
			...
			assert.equal(
				revised.plan.goals.find(
					(goal) => goal.id === "G003-add-migration-regression-test",
				)?.title,
				"Add ledger migration regression test",
			);

			const reordered = await steerUltragoal(
				cwd,
				{
					kind: "reorder_pending",
					source: "cli",
					evidence:
						"Dependency analysis shows parser coverage should run before the original first milestone.",
					rationale:
						"Reorder pending stories only; status and quality gates are unchanged.",
			...
			const next = await startNextUltragoal(cwd, {
				now: new Date("2026-05-04T10:14:00Z"),
			});
			assert.equal(next.goal?.id, "G004-second-parser-coverage");

			const annotated = await steerUltragoal(
				cwd,
				{
					kind: "annotate_ledger",
					source: "finding",
					evidence:
						"A reviewer recorded why parser coverage was scheduled first.",
	...
	it("rejects weakening steering and records rejected audit evidence", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.equal(
				unchanged.goals[0]?.objective,
				"Complete first milestone with tests.",
			);
			const ledger = await readFile(
				join(cwd, ".omx/ultragoal/ledger.jsonl"),
				"utf-8",
	...
	it("rejects unknown steering mutation kinds before audit acceptance", async () => {
		await withTempRepo(async (cwd) => {
			...
			const proposal = {
				...
				source: "cli",
				evidence:
					"A stale proposal path supplied a non-allowlisted mutation kind.",
				rationale:
					"The core validator must fail closed even if the CLI parser is bypassed.",
			...
			assert.equal(invariant.accepted, false);
			assert.match(
				invariant.rejectedReasons.join("\n"),
	...
	it("dedupes steering by ledger idempotency key without duplicating child goals", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.equal(
				second.plan.goals.filter((goal) => goal.title === "Add regression")
					.length,
				1,
			);
	...
	it("skips superseded and blocked goals for scheduling while blocked goals prevent completion", async () => {
		await withTempRepo(async (cwd) => {
			...
			await steerUltragoal(cwd, {
				...
				targetGoalIds: ["G002-second"],
				evidence:
					"Second milestone is better represented as replacement child work.",
				rationale:
					"Supersede with a replacement goal that preserves the acceptance criteria.",
				childGoals: [
					{
						title: "Replacement second",
						objective: "Complete replacement second milestone with tests.",
					},
				],
			});
	...
	it("clears the active goal when mark_blocked_superseded supersedes the running goal", async () => {
		await withTempRepo(async (cwd) => {
			...
			const result = await steerUltragoal(cwd, {
				...
				targetGoalIds: ["G001-first"],
				evidence:
					"The active goal should be split into narrower replacement work.",
				rationale:
					"Supersede the active goal and keep the audit trail durable.",
				childGoals: [
					{
						title: "Replacement first part A",
						objective: "Complete replacement first part A with tests.",
					},
					{
						title: "Replacement first part B",
						objective: "Complete replacement first part B with tests.",
					},
				],
			});
			...
			assert.equal(result.plan.activeGoalId, undefined);
			assert.equal(
				result.plan.goals.find((goal) => goal.id === "G001-first")
					?.steeringStatus,
				"superseded",
			);
			assert.deepEqual(
				result.plan.goals
					.filter((goal) => goal.supersedes?.includes("G001-first"))
	...
	it("rejects invalid steering source and malformed superseded replacement children with audit evidence", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.match(
				invalidSource.rejectedReasons.join(" | "),
				/Invalid steering source/,
			);
			...
			const malformedReplacement = await steerUltragoal(cwd, {
				...
				rationale:
					"Malformed children should be rejected and audited instead of throwing.",
				childGoals: [{ title: "", objective: "Replacement objective." }],
			});
			assert.equal(malformedReplacement.accepted, false);
			assert.match(
				malformedReplacement.rejectedReasons.join(" | "),
				/replacement children require title and objective/,
			);
			...
			assert.match(
				nullReplacement.rejectedReasons.join(" | "),
				/replacement children require title and objective/,
			);
			...
			const weakenedSplitChild = await steerUltragoal(cwd, {
				...
				childGoals: [
					{
						title: "Shortcut child",
						objective:
							"Skip tests and remove verification for faster completion.",
					},
				],
			});
			assert.equal(weakenedSplitChild.accepted, false);
			assert.match(
				weakenedSplitChild.rejectedReasons.join(" | "),
				/must not weaken completion/,
			);
			assert.equal(
				weakenedSplitChild.audit.invariant.noEasierCompletion,
				false,
			);
				...
				childGoals: [
					{
						...
						objective: "Bypass review and omit quality gate evidence.",
					},
				],
			});
			assert.equal(weakenedSupersedeChild.accepted, false);
			assert.match(
				weakenedSupersedeChild.rejectedReasons.join(" | "),
				/must not weaken completion/,
			);
			assert.equal(
				weakenedSupersedeChild.audit.invariant.noEasierCompletion,
				false,
			);
		...
		for (const fixture of steeringFixtures) {
			await withTempRepo(async (cwd) => {
				...
				const result = await steerUltragoal(
					cwd,
					toSteeringProposal(fixture.proposal),
					{
				...
				assert.equal(
					result.audit.kind,
					fixture.expected.mutationKind,
					fixture.case,
				);
				assert.equal(
					result.audit.evidence,
					fixture.proposal.evidence,
					fixture.case,
				);
				assert.equal(
					result.audit.rationale,
					fixture.proposal.rationale,
					fixture.case,
				);
				assert.equal(result.audit.before !== undefined, true, fixture.case);
				assert.equal(
					isUltragoalDone(result.plan),
					fixture.expected.isDoneAfterMutation,
					fixture.case,
				);
				...
				assert.match(
					ledger,
					new RegExp(`"event":"${fixture.expected.ledgerEvent}"`),
					fixture.case,
				);
				assert.match(
					ledger,
					new RegExp(`"kind":"${fixture.expected.mutationKind}"`),
					fixture.case,
				);
				...
				if (!fixture.expected.accepted) {
					...
					assert.equal(
						result.audit.invariant.noEasierCompletion,
						false,
						fixture.case,
					);
					assert.match(
						result.rejectedReasons.join(" | "),
						/weaken completion|quality gates|tests|reviews/i,
						fixture.case,
					);
					assert.ok(fixture.proposal.forbidden?.codexObjective, fixture.case);
					assert.ok(
						fixture.proposal.forbidden?.aggregateCompletion,
						fixture.case,
					);
					assert.deepEqual(
						await readUltragoalPlan(cwd),
						JSON.parse(JSON.stringify(fixture.before)),
						fixture.case,
					);
					return;
				}
				...
				if (fixture.expected.summaryDelta?.superseded !== undefined) {
					assert.equal(
						summary.superseded,
						fixture.expected.summaryDelta.superseded,
						fixture.case,
					);
				}
				if (fixture.expected.summaryDelta?.steeringBlocked !== undefined) {
					const beforeBlocked = fixture.before.goals.filter(
						(goal) => goal.steeringStatus === "blocked",
					).length;
					assert.equal(
						summary.steeringBlocked,
						beforeBlocked + fixture.expected.summaryDelta.steeringBlocked,
						fixture.case,
					);
				}

				if (fixture.case === "split") {
					const parent = result.plan.goals.find(
						(goal) => goal.id === "G001-core-steering-model",
					);
					assert.equal(parent?.steeringStatus, "superseded");
					assert.deepEqual(parent?.supersededBy, [
						"G004-core-steering-schema",
						"G005-core-steering-scheduler-semantics",
					]);
				}
				if (fixture.case === "blocked-with-replacement") {
					...
					assert.equal(parent?.steeringStatus, "superseded");
					assert.deepEqual(parent?.supersededBy, [
						"G004-core-steering-replacement",
					]);
				}
				...
				if (fixture.case === "revise") {
					const revised = result.plan.goals.find(
						(goal) => goal.id === "G002-cli-bridge",
					);
				...
				if (fixture.case === "annotate") {
					assert.deepEqual(
						result.plan.goals,
						fixture.before.goals,
						fixture.case,
					);
				}

				const next = await startNextUltragoal(cwd, {
					now: new Date("2026-05-19T04:21:00.000Z"),
				});
				assert.equal(
					next.goal?.id,
					fixture.expected.scheduleStartsGoalId,
					fixture.case,
				);
				if (fixture.expected.finalCandidateForGoalId) {
					assert.equal(
						next.goal?.id,
						fixture.expected.finalCandidateForGoalId,
						fixture.case,
					);
				}
			});
		...
		await withTempRepo(async (cwd) => {
			...
			await assert.rejects(
				...
				(error: unknown) => {
					...
					assert.match(
						String(error),
						/unavailable get_goal error JSON or path/,
					);
	...
	it("records unavailable get_goal DB/schema errors as non-terminal blocked audit checkpoints", async () => {
		await withTempRepo(async (cwd) => {
			...
			const plan = await checkpointUltragoal(cwd, {
				...
				status: "blocked",
				evidence:
					"get_goal unavailable due to Codex DB/schema/context error; safe recovery requires a working Codex goal context",
	...
	it("records a safe blocker when the aggregate Codex goal is already complete while a microgoal remains in progress", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.match(
				plan.goals[0]?.failureReason ?? "",
				/aggregate Codex goal already complete/,
			);
			const ledger = await readFile(
				join(cwd, ".omx/ultragoal/ledger.jsonl"),
				"utf-8",
			...
			await assert.rejects(
				() =>
					checkpointUltragoal(cwd, {
						...
						codexGoal: {
							goal: { objective: "Different blocked work", status: "blocked" },
						},
					}),
				/objective mismatch/,
			);
	...
	it("steers a split pending goal through superseded lifecycle without weakening completion gates", async () => {
		await withTempRepo(async (cwd) => {
			await createUltragoalPlan(cwd, {
				brief:
					"G001-core-steering-model .omx/ultragoal split lifecycle coverage",
				codexGoalMode: "per_story",
				goals: [
					{
						title: "Original",
						objective: "Implement the original broad steering objective.",
					},
				],
			});
			...
			const split = await steerUltragoal(cwd, {
				...
				evidence:
					"G001-core-steering-model review found .omx/ultragoal needs smaller replacement children.",
				rationale:
					"Split preserves the original objective while scheduling verifiable child goals.",
				after: {
					children: [
						{
							title: "Child A",
							objective: "Implement child A steering support.",
						},
						{
							title: "Child B",
							objective: "Implement child B steering support.",
						},
					],
				},
			...
			assert.deepEqual(split.plan.goals[0]?.supersededBy, [
				"G002-child-a",
				"G003-child-b",
			]);
			assert.equal(
				split.plan.goals.some((goal) => goal.id === "G001-original"),
				true,
			);
			...
			await checkpointUltragoal(cwd, {
				...
				status: "complete",
				evidence:
					"child A tests passed for .omx/ultragoal G001-core-steering-model",
				codexGoal: {
					goal: { objective: first.goal!.objective, status: "complete" },
				},
			});
			...
			assert.equal(
				isFinalRunCompletionCandidate(second.plan, second.goal!),
				true,
			);
			...
			const done = await checkpointUltragoal(cwd, {
				...
				status: "complete",
				evidence:
					"child B tests passed for .omx/ultragoal G001-core-steering-model",
				codexGoal: {
					goal: { objective: second.goal!.objective, status: "complete" },
				},
				qualityGate: cleanQualityGate(),
			});
	...
	it("skips blocked-without-replacement steering while keeping completion blocked", async () => {
		await withTempRepo(async (cwd) => {
			await createUltragoalPlan(cwd, {
				brief:
					"G001-core-steering-model .omx/ultragoal blocked lifecycle coverage",
				codexGoalMode: "per_story",
				goals: [
					{
						title: "Blocked",
						objective: "Investigate blocked steering dependency.",
					},
			...
			const blocked = await steerUltragoal(cwd, {
				...
				evidence:
					"G001-core-steering-model evidence names .omx/ultragoal blocker without replacement.",
				rationale:
					"Avoid retry churn while preserving the unresolved blocker for final completion.",
			});
			...
			const afterNext = await checkpointUltragoal(cwd, {
				...
				status: "complete",
				evidence:
					"independent tests passed for .omx/ultragoal G001-core-steering-model",
				codexGoal: {
					goal: { objective: next.goal!.objective, status: "complete" },
				},
			});
	...
	it("rejects protected steering payloads and records a rejected audit without mutation", async () => {
		await withTempRepo(async (cwd) => {
			...
			const rejected = await steerUltragoal(cwd, {
				...
				after: {
					objective: "new wording",
					codexObjective: "weakened end goal",
				} as never,
			});
	...
	it("dedupes accepted steering by idempotency key", async () => {
		await withTempRepo(async (cwd) => {
			...
			const proposal = {
				...
				evidence:
					"prompt-submit evidence for .omx/ultragoal G001-core-steering-model",
			...
			const plan = await readUltragoalPlan(cwd);
			assert.equal(
				plan.goals.filter((goal) => goal.title === "Follow-up").length,
				1,
			);
			const ledger = await readFile(
				join(cwd, ".omx/ultragoal/ledger.jsonl"),
	...
	it("preserves root-mode bootstrap when OMX_SESSION_ID is absent", async () => {
		await withTempRepo(async (cwd) => {
			...
			try {
				...
				assert.equal(
					existsSync(join(cwd, ".omx/ultragoal/ledger.jsonl")),
					true,
				);
			} finally {
				if (typeof previousEnv === "string")
					process.env.OMX_SESSION_ID = previousEnv;
		...
		await withTempRepo(async (cwd) => {
			...
			try {
				...
				await writeFile(
					...
					JSON.stringify({ session_id: "sess-live", cwd }),
				);
	...
	it("blocks durable ultragoal mutations while the selected pointer is stale-dead and no exact session is bound", async () => {
		await withTempRepo(async (cwd) => {
			...
			try {
				...
				await writeFile(
					join(stateDir, "session.json"),
					JSON.stringify({ session_id: "sess-stale-dead", cwd, pid: 8388607 }),
				);
				...
				assert.equal(
					existsSync(join(cwd, ".omx", "ultragoal", "goals.json")),
					false,
				);
			} finally {
				if (typeof previousEnv === "string")
					...
					[
						...
						() =>
							steerUltragoal(cwd, {
								...
								rationale:
									"bounded explicit directive requires one follow-up only",
	...
	it("propagates non-compatibility resolver failures instead of permitting durable mutation", async () => {
		await withTempRepo(async (cwd) => {
			const allowedRoot = await mkdtemp(
				join(tmpdir(), "omx-ultragoal-allowlist-"),
			);
			...
			try {
				...
				if (typeof prevAllowlist === "string")
					process.env.OMX_MCP_WORKDIR_ROOTS = prevAllowlist;
	...
	it("refuses read-path legacy objective migration while writable lifecycle authority is unrestored", async () => {
		await withTempRepo(async (cwd) => {
			...
			try {
				...
				await mkdir(join(cwd, ".omx/ultragoal"), { recursive: true });
				const legacyObjective =
					"Complete all ultragoal stories in .omx/ultragoal/goals.json: G001-first First";
	...
	it("T16 compares the full writable authority token around ultragoal mutation locks", async () => {
		await withTempRepo(async (cwd) => {
			...
			try {
				...
				await addUltragoalGoal(cwd, {
					title: "Stable compatibility authority",
					objective: "Mutation proceeds.",
				});

				await writeFile(pointerPath, pointer("sess-compat"));
				await addUltragoalGoal(cwd, {
					title: "Stable resolved authority",
					objective: "Mutation proceeds.",
				});

				for (const [kind, beforeLock, change] of [
					[
						"resolved to different live session",
						async () => writeFile(pointerPath, pointer("sess-compat")),
						async () => {
							process.env.OMX_SESSION_ID = "sess-replacement";
							await writeFile(pointerPath, pointer("sess-replacement"));
						},
					],
					[
						"resolved to compatibility",
						async () => writeFile(pointerPath, pointer("sess-compat")),
						async () => rm(pointerPath),
					],
					[
						"compatibility to resolved",
						async () => rm(pointerPath, { force: true }),
						async () => writeFile(pointerPath, pointer("sess-compat")),
					],
				] as const) {
					process.env.OMX_SESSION_ID = "sess-compat";
	...
	describe("canonical state paths in nested projects", () => {
		async function withNestedRepo<T>(
			run: (paths: { repo: string; subproject: string }) => Promise<T>,
		): Promise<T> {
		...
		it("binds the aggregate objective to the subtree state root", async () => {
			await withNestedRepo(async ({ subproject }) => {
				const plan = await createUltragoalPlan(subproject, {
					brief: "Ship the nested feature",
				});

				assert.equal(plan.statePathPrefix, "subproject");
				assert.match(
					plan.codexObjective ?? "",
					/subproject\/\.omx\/ultragoal\/goals\.json/,
				);
				assert.match(
					plan.codexObjective ?? "",
					/subproject\/\.omx\/ultragoal\/ledger\.jsonl/,
				);
				assert.notEqual(
					plan.codexObjective,
					ULTRAGOAL_AGGREGATE_CODEX_OBJECTIVE,
		...
		it("keeps repository-root launches byte-identical to the root objective", async () => {
			await withNestedRepo(async ({ repo }) => {
				const plan = await createUltragoalPlan(repo, {
					brief: "Ship the root feature",
					force: true,
				});
		...
		it("renders the aggregate handoff header with canonical state paths", async () => {
			await withNestedRepo(async ({ subproject }) => {
				await createUltragoalPlan(subproject, {
					brief: "Ship the nested feature",
				});
				...
				assert.match(
					instruction,
					/Plan: subproject\/\.omx\/ultragoal\/goals\.json/,
				);
				assert.match(
					instruction,
					/Ledger: subproject\/\.omx\/ultragoal\/ledger\.jsonl/,
				);
			});
		});
		...
		it("reconciles the prefixed objective and rejects the ambiguous bare objective", async () => {
			await withNestedRepo(async ({ subproject }) => {
				const created = await createUltragoalPlan(subproject, {
					brief: "Ship the nested feature",
				});
				const started = await startNextUltragoal(subproject);
				...
				const checkpointed = await checkpointUltragoal(subproject, {
					...
					evidence: "tests passed",
					codexGoal: {
						goal: { objective: created.codexObjective, status: "complete" },
					},
					qualityGate: cleanQualityGate(),
				});
				assert.equal(checkpointed.goals[0]?.status, "complete");
		...
		it("migrates a pre-existing nested plan and leaves root plans untouched", async () => {
			await withNestedRepo(async ({ repo, subproject }) => {
				...
				assert.match(
					migrated.codexObjective ?? "",
					/subproject\/\.omx\/ultragoal\/goals\.json/,
				);
				assert.deepEqual(migrated.codexObjectiveAliases, [
					ULTRAGOAL_AGGREGATE_CODEX_OBJECTIVE,
				]);
				const ledger = await readFile(
					join(subproject, ".omx/ultragoal/ledger.jsonl"),
					"utf-8",
				);
				assert.match(ledger, /"event":"aggregate_objective_migrated"/);
				...
				const rootPlan = await createUltragoalPlan(repo, {
					brief: "Ship the root feature",
					force: true,
				});
				const rootLedgerBefore = await readFile(
					join(repo, ".omx/ultragoal/ledger.jsonl"),
					"utf-8",
				);
				const reread = await readUltragoalPlan(repo);
				const rootLedgerAfter = await readFile(
					join(repo, ".omx/ultragoal/ledger.jsonl"),
					"utf-8",
				);
	...
	it("allows durable Ultragoal mutations for an exact-match identity-indeterminate pointer and rejects mismatches", async () => {
		await withTempRepo(async (cwd) => {
			...
			try {
				...
				const plan = await createUltragoalPlan(cwd, {
					brief: "- Ship the indeterminate recovery",
				});
				assert.equal(plan.goals.length, 1);
				assert.equal(
					existsSync(join(cwd, ".omx", "ultragoal", "goals.json")),
				...
				await assert.rejects(
					() =>
						addUltragoalGoal(cwd, {
							...
							objective: "Foreign session mutation.",
						}),
					(error: unknown) => {
						assert.match(String(error), /writable lifecycle authority/);
						return true;
