...
function compatibleCodexObjectives(plan: UltragoalPlan): string[] {
	return (plan.codexObjectiveAliases ?? []).filter((objective) =>
...
export async function readUltragoalPlan(cwd: string): Promise<UltragoalPlan> {
	...
	if (
		codexGoalMode(parsed) === "aggregate" &&
		isLegacyEnumeratedAggregateObjective(parsed.codexObjective)
	) {
	...
	return withUltragoalMutationLock(cwd, async () => {
		...
		if (options.status === "blocked") {
			...
			if (!snapshot?.available) {
				throw new UltragoalError(
					"Blocked ultragoal checkpoints require either a get_goal snapshot for the completed legacy Codex goal that blocked create_goal, or an unavailable get_goal error JSON for a Codex DB/schema/context failure; pass --codex-goal-json.",
				);
			}
