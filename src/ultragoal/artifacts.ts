...
export type UltragoalWritableAuthority =
	| {
			kind: "resolved";
			source: ResolvedStateScope["source"];
...
function normalizeIndentedAtxStorySectionLabel(
	value: string,
): string | undefined {
...
function selectedItemObjective(
	...
	for (
		let lineIndex = parent.lineIndex + 1;
		lineIndex < (nextParentLineIndex ?? lines.length);
		lineIndex += 1
	) {
		const line = lines[lineIndex] ?? "";
		...
		if (
			...
			sectionLooksNonStory(normalizeSectionLabel(line))
		)
			break;
		if (/^\s*(?:[-*+]\s+|\d+[.)]\s+)/.test(line) && indent <= parent.indent)
			break;
...
function topLevelStoryItems(
	items: readonly MarkdownListItem[],
): MarkdownListItem[] {
	const storyItems = items.filter(
		(item) => !sectionLooksNonStory(item.section),
	);
	if (storyItems.length === 0) return [];
	const storySectionItems = storyItems.filter((item) =>
		sectionLooksStory(item.section),
	);
	const candidates =
...
function objectivesHaveConservativeSpecificTokenOverlap(
	actual: string,
	brief: string,
): boolean {
	...
	const shared = [...actualTokens].filter((token) =>
		briefTokens.has(token),
	).length;
...
function classifyExternalAuthorizationBlocker(
	evidence: string | undefined,
): ExternalAuthorizationBlocker | null {
	...
	if (mentionsGhcr) {
		const has401 =
			/\b(401|unauthorized|anonymous pull|authentication required)\b/.test(
				normalized,
			);
		const has403 = /\b(403|forbidden|read packages|package api)\b/.test(
			normalized,
		);
		const status =
			[
				has401 ? "HTTP_401_ANONYMOUS" : null,
				has403 ? "HTTP_403_NO_READ_PACKAGES" : null,
			]
...
function sameBlockerOccurrences(
	entries: readonly UltragoalLedgerEntry[],
	goalId: string,
	signature: string,
): number {
	return entries.filter(
		(entry) =>
			entry.goalId === goalId &&
			(entry.event === "goal_failed" ||
				entry.event === "goal_needs_user_decision") &&
...
async function snapshotObjectiveMapsToUltragoalPlan(
	...
	try {
		...
		return (
			brief.includes(actual) ||
			actual.includes(brief) ||
			objectivesHaveConservativeSpecificTokenOverlap(actual, brief)
		);
	} catch {
		return false;
	}
...
function hasReciprocalReviewBlockerResolver(
	goal: UltragoalItem,
	plan: UltragoalPlan,
): boolean {
	const resolverId = goal.reviewBlockerResolution?.resolverGoalId;
	if (!resolverId) return false;
	const resolvers = plan.goals.filter(
		(candidate) => candidate.id === resolverId,
	);
...
function hasUniqueGoalIds(plan: UltragoalPlan): boolean {
	return (
		new Set(plan.goals.map((candidate) => candidate.id)).size ===
		plan.goals.length
	);
}
...
function canPersistNormalFinalAggregateCompletion(
	...
	return canUseCleanFinalResolverPathForReviewBlockedParent(
		plan,
		goal,
		finalRunCheckpoint,
		allowActiveFinalCodexGoal,
...
function evidenceDescribesCompletedAggregateMicrogoalLoop(
	evidence: string | undefined,
): boolean {
	const normalized = normalizeObjective(evidence ?? "").toLowerCase();
	return (
		normalized.includes("aggregate codex goal") &&
		/\bcomplete(?:d)?\b/.test(normalized) &&
		normalized.includes("microgoal") &&
		/\b(?:unreconcilable|mismatch|loop|already complete|already completed|blocks?)\b/.test(
			normalized,
		)
	);
}
...
function isSafeCompletedAggregateBlockerSnapshot(
	...
	return [
		expectedCodexObjective(plan, goal),
...
function isScheduleEligibleGoal(goal: UltragoalItem): boolean {
	return (
		goal.steeringStatus !== "superseded" && goal.steeringStatus !== "blocked"
	);
}

export const ULTRAGOAL_AGGREGATE_CODEX_OBJECTIVE =
	buildAggregateCodexObjective("");
...
function isLegacyEnumeratedAggregateObjective(
	...
	return (
		objective.startsWith(
			`Complete all ultragoal stories in ${ULTRAGOAL_DIR}/${ULTRAGOAL_GOALS}: `,
		) ||
		objective ===
...
function compatibleCodexObjectives(plan: UltragoalPlan): string[] {
	return (plan.codexObjectiveAliases ?? []).filter(
		(objective) =>
			isLegacyEnumeratedAggregateObjective(objective) ||
			objective === ULTRAGOAL_AGGREGATE_CODEX_OBJECTIVE,
	);
}

function expectedCodexObjective(
	plan: UltragoalPlan,
	goal: UltragoalItem,
): string {
	return codexGoalMode(plan) === "aggregate"
		? (plan.codexObjective ??
				aggregateCodexObjective(plan.statePathPrefix ?? ""))
		: goal.objective;
}

function isSupersededResolved(
	goal: UltragoalItem,
	plan: UltragoalPlan,
): boolean {
...
export function deriveGoalCandidates(
	...
	const listGoals = parentItems
		...
		.filter(
			(objective, index, all) =>
				all.findIndex((candidate) => candidate === objective) === index,
		);
	const bulletGoals =
			...
			: lines
					...
					.filter(
						({ original, cleaned }, index, all) =>
							...
							all.findIndex((candidate) => candidate.cleaned === cleaned) ===
								index,
					)
					.map(({ cleaned }) => cleaned);
...
function writableAuthorityEquals(
	...
	const resolvedAfterLock = afterLock as Extract<
		UltragoalWritableAuthority,
		{ kind: "resolved" }
	>;
	return (
		beforeLock.source === resolvedAfterLock.source &&
		beforeLock.sessionId === resolvedAfterLock.sessionId &&
		beforeLock.stateDir === resolvedAfterLock.stateDir
	);
}

function describeWritableAuthority(
	authority: UltragoalWritableAuthority,
): string {
...
async function withUltragoalMutationLock<T>(
	...
	const beforeLock = await assertUltragoalWritableLifecycleAuthority(
		cwd,
		options,
	);
	...
	for (let attempt = 0; attempt < 100; attempt += 1) {
		try {
			...
			await handle.writeFile(
				JSON.stringify({ pid: process.pid, createdAt: iso() }),
			);
			break;
		} catch (error) {
	...
	try {
		...
		const afterLock = await assertUltragoalWritableLifecycleAuthority(
			cwd,
			options,
		);
 ...
 */
export async function readUltragoalPlanSnapshot(
	cwd: string,
): Promise<UltragoalPlan> {
	return readUltragoalPlanFile(cwd);
}

function requiresCanonicalStatePathMigration(
	plan: UltragoalPlan,
	statePathPrefix: string,
): boolean {
	return (
		statePathPrefix !== "" &&
		plan.codexObjective === ULTRAGOAL_AGGREGATE_CODEX_OBJECTIVE
	);
}

function requiresAggregateObjectiveMigration(
	plan: UltragoalPlan,
	statePathPrefix: string,
): boolean {
	if (codexGoalMode(plan) !== "aggregate") return false;
...
async function migrateAggregateObjectiveUnderLock(
	...
	if (!requiresAggregateObjectiveMigration(parsed, statePathPrefix))
		return parsed;
	const canonicalStatePathMigration = requiresCanonicalStatePathMigration(
		parsed,
		statePathPrefix,
	);
	...
	parsed.codexObjectiveAliases = Array.from(
		new Set(
			[...(parsed.codexObjectiveAliases ?? []), previousObjective].filter(
...
async function readUltragoalPlanUnderLock(cwd: string): Promise<UltragoalPlan> {
	return migrateAggregateObjectiveUnderLock(
		cwd,
		await readUltragoalPlanFile(cwd),
	);
}
...
export async function readUltragoalPlan(cwd: string): Promise<UltragoalPlan> {
	...
	if (
		!requiresAggregateObjectiveMigration(parsed, ultragoalStatePathPrefix(cwd))
	)
		return parsed;
	...
	return withUltragoalMutationLock(cwd, async () =>
		readUltragoalPlanUnderLock(cwd),
	);
}
...
export async function createUltragoalPlan(
	...
	return withUltragoalMutationLock(
		cwd,
		async () => {
			...
			const candidates = sourceGoals.map(
				(goal, index): UltragoalItem => ({
					id: normalizeGoalId(
						goal.title ??
							titleFromObjective(goal.objective, `Goal ${index + 1}`),
						index,
					),
					title:
						goal.title ??
...
export function summarizeUltragoalPlan(plan: UltragoalPlan): {
	...
	return {
		...
		superseded: plan.goals.filter(
			(goal) => goal.steeringStatus === "superseded",
		).length,
		steeringBlocked: plan.goals.filter(
...
export function parseUltragoalSteeringDirective(
	raw: string,
	...
	try {
		...
		if (
			!ULTRAGOAL_STEERING_MUTATION_KINDS.includes(
				parsed.kind as UltragoalSteeringMutationKind,
			)
		)
			return null;
		if (
			!ULTRAGOAL_STEERING_SOURCES.includes(
				parsed.source as UltragoalSteeringSource,
			)
		)
			return null;
		return parsed;
...
function mentionsWeakenedCompletion(
	...
	return (
		...
		/\b(test|tests|verification|review|quality gate|complete|completion)\b/.test(
			normalized,
		)
	);
}
...
function rawChildGoalsFromProposal(
	proposal: UltragoalSteeringProposal,
): unknown[] {
...
function isValidSteeringChildGoal(
	value: unknown,
): value is UltragoalSteeringChildGoal {
...
function revisedTitleFromProposal(
	proposal: UltragoalSteeringProposal,
): string | undefined {
...
function revisedObjectiveFromProposal(
	proposal: UltragoalSteeringProposal,
): string | undefined {
...
export function validateUltragoalSteeringProposal(
	...
	const evidenceBackedNecessity =
		Boolean(proposal.evidence?.trim()) && Boolean(proposal.rationale?.trim());
	if (!ULTRAGOAL_STEERING_MUTATION_KINDS.includes(proposal.kind))
		rejectedReasons.push(
			`Invalid steering mutation kind: ${String(proposal.kind)}.`,
		);
	if (!ULTRAGOAL_STEERING_SOURCES.includes(proposal.source))
		rejectedReasons.push(
			`Invalid steering source: ${String(proposal.source)}.`,
		);
	...
	if (plan.aggregateCompletion?.status === "complete")
		rejectedReasons.push(
			"Cannot steer an already completed aggregate ultragoal plan.",
		);
	...
	try {
		...
		rejectedReasons.push(
			message.replace(/^Unknown ultragoal id:/, "unknown ultragoal id:"),
		);
	}
	...
	if (proposal.kind === "split_subgoal") {
		...
		if (rawChildren.some((child) => !isValidSteeringChildGoal(child)))
			rejectedReasons.push(
				"split_subgoal children require title and objective.",
			);
	}
	if (proposal.kind === "mark_blocked_superseded") {
		...
		if (rawChildren.some((child) => !isValidSteeringChildGoal(child)))
			rejectedReasons.push(
				"mark_blocked_superseded replacement children require title and objective.",
			);
	}
	if (proposal.kind === "reorder_pending") {
		...
		if (requested.length === 0)
			rejectedReasons.push(
				"reorder_pending requires at least one pending goal id.",
			);
		if (new Set(requested).size !== requested.length)
			rejectedReasons.push("duplicate goal id in pendingOrder.");
		if (requested.some((id) => !pending.includes(id)))
			rejectedReasons.push(
				"pendingOrder contains non-pending or unknown goal.",
			);
	}
	if (proposal.kind === "revise_pending_wording") {
		if (
			!revisedTitleFromProposal(proposal)?.trim() &&
			!revisedObjectiveFromProposal(proposal)?.trim()
		)
			rejectedReasons.push(
	...
	const noEasierCompletion = !mentionsWeakenedCompletion(
		protectedIntentText(proposal),
	);
	return {
...
async function readSteeringLedgerEntries(
	cwd: string,
): Promise<UltragoalLedgerEntry[]> {
	try {
...
function moveGoalsAfterTarget(
	...
	plan.goals.splice(
		targetIndex >= 0 ? targetIndex + 1 : plan.goals.length,
		0,
		...moved,
	);
}
...
function applySteeringMutation(
	...
	if (proposal.kind === "split_subgoal") {
		...
		return {
			before,
			after: {
				target: cloneForAudit(target),
				children: cloneForAudit(children),
			},
		};
	}
	if (proposal.kind === "reorder_pending") {
	...
	if (proposal.kind === "annotate_ledger") {
		return {
			before: undefined,
			after: { evidence: proposal.evidence, rationale: proposal.rationale },
		};
	}
	if (proposal.kind === "mark_blocked_superseded") {
		const before = cloneForAudit(target);
		...
		if (children.length > 0) {
			...
			return {
				...
				after: {
					target: cloneForAudit(target),
					children: cloneForAudit(replacements),
				},
			};
		}
...
export async function steerUltragoal(
	...
	return withUltragoalMutationLock(cwd, async () => {
		...
		if (invariant.accepted) {
			try {
				...
				const rejectedReasons = [
					...invariant.rejectedReasons,
					`Steering mutation failed: ${message}`,
				];
				invariant = {
					...invariant,
					accepted: false,
		...
		return {
			plan,
			accepted: invariant.accepted,
			audit,
			rejectedReasons: invariant.rejectedReasons,
			deduped: false,
		};
	});
}
...
function requiredInvariantSourceKey(
	invariant: RequiredArchitectureInvariant,
): string {
	return `${normalizeInvariantText(invariant.invariant)}\u0000${invariant.sourceArtifact}`;
}

function uniqueRequiredArchitectureInvariants(
	invariants: readonly RequiredArchitectureInvariant[],
): RequiredArchitectureInvariant[] {
...
function sourceReferencesAnyArtifact(
	source: string,
	artifacts: readonly string[],
): boolean {
	return artifacts.some((artifact) =>
		sourceReferencesArtifact(source, artifact),
	);
}
...
function invariantFromInlineDeclaration(line: string): string | undefined {
	const trimmed = cleanLine(line)
		...
		.trim();
	const match =
		/\b(?:(?:non-negotiable|required)\s+)?(?:architecture|architectural|domain)\s+(?:invariants?|constraints?|non-negotiables?)\s*:\s*(.+)$/i.exec(
			trimmed,
		) ??
		/\bnon-negotiables?\s+(?:architecture|architectural|domain)\s+(?:invariants?|constraints?)\s*:\s*(.+)$/i.exec(
			trimmed,
		);
...
function extractArchitectureInvariantsFromArtifact(
	text: string,
	sourceArtifact: string,
	sourcePrefix?: string,
): RequiredArchitectureInvariant[] {
	...
	for (const line of lines) {
		...
		if (heading) {
			...
			inInvariantSection =
				/\b(?:architecture|architectural|domain|non-negotiable)\b/.test(
					normalizedLabel,
				) &&
				/\binvariants?\b|\bconstraints?\b|\bnon-negotiables?\b/.test(
					normalizedLabel,
				);
		...
		if (inline) {
			invariants.push({
				invariant: inline,
				sourceArtifact,
				source: `${sourceArtifact}#${sourcePrefix ?? "inline-architecture-invariant"}`,
			});
			continue;
		}
		...
		invariants.push({
			invariant: item,
			sourceArtifact,
			source: `${sourceArtifact}#${sourcePrefix ? `${sourcePrefix}-${sectionSlug}` : sectionSlug}`,
		});
	}
	return uniqueRequiredArchitectureInvariants(
		invariants
			.map((item) => ({ ...item, invariant: item.invariant.trim() }))
			.filter((item) => item.invariant),
	);
}

function extractArchitectureInvariantsFromBrief(
	brief: string,
): RequiredArchitectureInvariant[] {
	return extractArchitectureInvariantsFromArtifact(
		brief,
		`${ULTRAGOAL_DIR}/${ULTRAGOAL_BRIEF}`,
	);
}

function extractArchitectureInvariantsFromAcceptedSteering(
	entries: readonly UltragoalLedgerEntry[],
): RequiredArchitectureInvariant[] {
	const invariants: RequiredArchitectureInvariant[] = [];
	for (const [index, entry] of entries.entries()) {
		if (
			entry.event !== "steering_accepted" ||
			!entry.steering?.invariant.accepted
		)
			continue;
		...
		for (const text of texts) {
			invariants.push(
				...extractArchitectureInvariantsFromArtifact(
					text,
					`${ULTRAGOAL_DIR}/${ULTRAGOAL_LEDGER}`,
					sourcePrefix,
				),
			);
		}
...
async function collectRequiredArchitectureInvariants(
	cwd: string,
): Promise<RequiredArchitectureInvariant[]> {
	const briefInvariants = extractArchitectureInvariantsFromBrief(
		await readFile(ultragoalBriefPath(cwd), "utf-8"),
	);
	const steeringInvariants = extractArchitectureInvariantsFromAcceptedSteering(
		await readSteeringLedgerEntries(cwd),
	);
	return uniqueRequiredArchitectureInvariants([
		...briefInvariants,
		...steeringInvariants,
...
function validateArchitectureInvariantGate(
	gate: Partial<UltragoalQualityGate>,
	requiredInvariants: readonly RequiredArchitectureInvariant[],
): void {
	...
	const sourceArtifacts = invariantGate.sourceArtifacts.map((source) =>
		assertNonEmpty(source, "architectureInvariantGate.sourceArtifacts[]"),
	);
	for (const required of requiredInvariants) {
		if (
			!sourceArtifacts.some((source) =>
				sourceReferencesArtifact(source, required.sourceArtifact),
			)
		) {
			throw new UltragoalError(
	...
	if (!Array.isArray(invariantGate.invariants)) {
		throw new UltragoalError(
			"Final architecture-invariant gate requires architectureInvariantGate.invariants.",
		);
	}
	...
	for (const invariant of invariantGate.invariants) {
		if (!invariant || typeof invariant !== "object")
			throw new UltragoalError(
				"Final architecture-invariant gate invariants must be objects.",
			);
		const record =
			invariant as Partial<UltragoalArchitectureInvariantEvidence> & {
				blockers?: unknown;
			};
		const text = assertNonEmpty(
			record.invariant,
			"architectureInvariantGate.invariants[].invariant",
		);
		const source = assertNonEmpty(
			record.source,
			"architectureInvariantGate.invariants[].source",
		);
		if (!sourceReferencesAnyArtifact(source, sourceArtifacts)) {
			throw new UltragoalError(
				`Final architecture invariant "${text}" source must reference one of architectureInvariantGate.sourceArtifacts; decorative provenance labels are not sufficient.`,
			);
		}
		if (record.status !== "proved")
			throw new UltragoalError(
				`Final architecture invariant "${text}" is not proved; record blocker-resolution work before final completion.`,
			);
		if (record.blockers !== undefined)
			throw new UltragoalError(
				`Final architecture invariant "${text}" has blockers; record blocker-resolution work before final completion.`,
			);
		assertNonEmpty(
			record.implementationEvidence,
			"architectureInvariantGate.invariants[].implementationEvidence",
		);
		assertNonEmpty(
			record.testEvidence,
			"architectureInvariantGate.invariants[].testEvidence",
		);
		assertNonEmpty(
			record.reviewEvidence,
			"architectureInvariantGate.invariants[].reviewEvidence",
		);
	...
	for (const required of requiredInvariants) {
		const matches =
			provided.get(normalizeInvariantText(required.invariant)) ?? [];
		if (matches.length === 0) {
			throw new UltragoalError(
				`Final architecture-invariant gate is missing proof for required invariant from ${required.sourceArtifact}: ${required.invariant}`,
			);
		}
		if (
			!matches.some((record) =>
				sourceReferencesArtifact(record.source, required.sourceArtifact),
			)
		) {
			throw new UltragoalError(
				`Final architecture-invariant gate proof for required invariant must reference ${required.sourceArtifact}: ${required.invariant}`,
			);
		}
	}
}
...
function validateQualityGate(
	value: unknown,
	requiredInvariants: readonly RequiredArchitectureInvariant[] = [],
): UltragoalQualityGate {
	if (!value || typeof value !== "object") {
		throw new UltragoalError(
			"Final ultragoal completion requires --quality-gate-json with ai-slop-cleaner, verification, code-review, and architecture-invariant evidence.",
		);
	}
	...
	const independentReview = (
		review as Partial<UltragoalQualityGate["codeReview"]>
	).independentReview;
	...
	assertNonEmpty(
		...
		"codeReview.independentReview.codeReviewer.evidence",
	);
	...
	assertNonEmpty(
		architect.evidence,
		"codeReview.independentReview.architect.evidence",
	);
...
export async function startNextUltragoal(
	...
	return withUltragoalMutationLock(cwd, async () => {
		...
		const existing = plan.goals.find(
			(goal) => goal.status === "in_progress" && isScheduleEligibleGoal(goal),
		);
		if (existing) {
			await appendLedger(cwd, {
				ts: now,
		...
		let next = plan.goals.find(
			(goal) => goal.status === "pending" && isScheduleEligible(goal),
		);
...
export async function checkpointUltragoal(
	...
	return withUltragoalMutationLock(cwd, async () => {
		...
		const goal = plan.goals.find(
			(candidate) => candidate.id === options.goalId,
		);
		if (!goal)
		...
		if (options.status === "blocked") {
			...
			if (snapshot.status === "blocked") {
				...
				await appendLedger(cwd, {
					...
					codexGoal: options.codexGoal,
					message:
						"Native Codex goal status is blocked for the matching objective; recorded a non-terminal goal_blocked checkpoint. Continue work without treating this as complete or failed; re-check get_goal when the blocker clears.",
				});
				return plan;
			}
			...
			const safeCompletedAggregateBlocker =
				isSafeCompletedAggregateBlockerSnapshot(
					plan,
					goal,
					snapshot,
					options.evidence,
			...
			goal.updatedAt = now;
			if (safeCompletedAggregateBlocker)
				goal.failureReason = assertNonEmpty(options.evidence, "--evidence");
			plan.activeGoalId = goal.id;
			plan.updatedAt = now;
			await writePlan(cwd, plan);
		...
		if (options.status === "complete") {
			...
			const reconciliation = reconcileCodexGoalSnapshot(snapshot, {
				expectedObjective,
				acceptedObjectives: aggregateMode
					? compatibleCodexObjectives(plan)
					: undefined,
				allowedStatuses: aggregateMode
					? finalRunCheckpoint && !options.allowActiveFinalCodexGoal
						? ["complete"]
						: ["active"]
					: ["complete"],
				...
				if (completedTaskScopedAggregateSnapshot) {
					...
					const remediation =
						...
						normalizeObjective(reconciliation.snapshot.objective ?? "") !==
							...
							: reconciliation.snapshot.unavailableReason ===
									"db_schema_context_error"
		...
		const requiredArchitectureInvariants =
			options.status === "complete" &&
		...
		const qualityGate =
			...
			(aggregateCompletion !== undefined ||
				...
				? validateQualityGate(
						options.qualityGate,
						requiredArchitectureInvariants,
					)
				: undefined;
		...
		if (options.status === "complete") {
			...
			const previousEntries = blocker
				? await readSteeringLedgerEntries(cwd)
				: [];
			const occurrenceCount = blocker
				? sameBlockerOccurrences(previousEntries, goal.id, blocker.signature) +
					1
				: 0;
...
function buildPerStoryCodexGoalInstruction(
	...
	return [
		...
		finalStory
			...
			: null,
		"- If blocked on a matching native Codex goal status, record a non-terminal blocker with: omx ultragoal checkpoint --goal-id " +
			goal.id +
			' --status blocked --evidence "<blocker evidence>" --codex-goal-json "<matching blocked get_goal JSON or path>". If failed, checkpoint with --status failed and the failure evidence; rerun complete-goals --retry-failed to resume.',
		"",
		"create_goal payload:",
