...
describe("buildSessionFrictionReport", () => {
			...
			await writeRollout(
				...
				[
					...
					},
					{
						...
						payload: {
							type: "user_message",
							message: "secret prompt must not be emitted sk-test-secret",
						},
					},
					{
						type: "response_item",
	...
	it("defaults to current-project recent sessions and safe source labels", async () => {
		...
		const otherCwd = await mkdtemp(
			join(tmpdir(), "omx-session-friction-other-"),
		);
		const codexHomeDir = join(cwd, ".codex-home");
		try {
			await writeRollout(
			...
			assert.deepEqual(
				report.sessions.map((session) => session.session_id),
				["current-session"],
			);
