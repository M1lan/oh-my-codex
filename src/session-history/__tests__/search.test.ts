...
describe("searchSessionHistory", () => {
	it("returns structured matches with snippets from rollout transcripts", async () => {
		...
		try {
				...
				[
					...
					},
					{
						...
						payload: {
							type: "user_message",
							message:
								"Please investigate the worker inbox path issue in team mode.",
						},
					},
				],
			);
			...
			assert.equal(
				...
				"sessions/2026/03/10/rollout-2026-03-10T12-00-00-session-a.jsonl",
			);
		} finally {
				...
				[
					...
					},
					{
						...
						payload: {
							...
							output:
								"all_workers_idle fired after startup evidence was missing",
						},
					},
				],
			);
				...
				[
					...
					},
					{
						...
						payload: {
							type: "agent_message",
							message:
								"all_workers_idle should remain searchable in older sessions too",
						},
					},
				],
			);
	...
	it("discovers generated project runtime Codex homes alongside the default home", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-search-project-runtime-"),
		);
		const defaultCodexHome = join(cwd, "default-codex-home");
		const projectRuntimeHome = join(
			cwd,
			".omx",
			"runtime",
			"codex-home",
		...
		try {
				...
				[
					...
					},
					{
						...
						payload: {
							type: "user_message",
							message:
								"project runtime discovery should include default history",
						},
					},
				],
			);
				...
				[
					...
					},
					{
						...
						payload: {
							type: "user_message",
							message:
								"project runtime discovery should include generated history",
						},
					},
				],
			);
			...
			try {
				...
				assert.deepEqual(
					report.results.map((result) => result.session_id).sort(),
					["default-session", "runtime-session"],
				);
				assert.equal(report.sources.length, 2);
				assert.ok(
					report.sources.some(
						(source) => source.codex_home === defaultCodexHome,
					),
				);
				assert.ok(
					report.sources.some(
						(source) => source.codex_home === projectRuntimeHome,
					),
				);
			} finally {
	...
	it("ignores non-generated runtime homes under the project runtime root", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-search-non-generated-"),
		);
		const defaultCodexHome = join(cwd, "default-codex-home");
		const generatedRuntimeHome = join(
			cwd,
			".omx",
			"runtime",
			"codex-home",
		...
		try {
			...
			await writeRollout(
				...
				[
					...
					},
					{
						...
						payload: {
							type: "user_message",
							message: "generated runtime only",
						},
					},
				],
			);
			...
			try {
				...
				assert.ok(
					report.sources.some(
						(source) => source.codex_home === generatedRuntimeHome,
					),
				);
				assert.ok(
					!report.sources.some(
						(source) => source.codex_home === manualRuntimeHome,
					),
				);
			} finally {
				if (previousCodexHome === undefined) delete process.env.CODEX_HOME;
	...
	it("searches runtime homes even when default results fill the limit", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-search-limit-runtime-"),
		);
		const defaultCodexHome = join(cwd, "default-codex-home");
			...
			await writeRollout(
				...
				[
					...
					},
					{
						...
						payload: {
							type: "user_message",
							message: "limit saturation match",
						},
					},
				],
			);
	...
	it("honors an explicit Codex home escape hatch", async () => {
		...
		try {
			await writeRollout(
				...
				[
					...
					},
					{
						...
						payload: {
							type: "user_message",
							message: "escape hatch target default",
						},
					},
				],
			);
			await writeRollout(
				...
				[
					...
					},
					{
						...
						payload: {
							type: "user_message",
							message: "escape hatch target explicit",
						},
					},
				],
			);
			...
			assert.deepEqual(
				report.sources.map((source) => source.codex_home),
				[explicitCodexHome],
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
