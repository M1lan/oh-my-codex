...
async function normalizeExistingCodexHomeDirs(
	candidates: Array<string | ResolvedSessionSearchCodexHome>,
): Promise<ResolvedSessionSearchCodexHome[]> {
	...
	for (const candidate of candidates) {
		...
		dirs.push({
			dir: absolute,
			publicLabel:
				typeof candidate === "string" ? absolute : candidate.publicLabel,
		});
	}
	return dirs;
}

async function resolveSessionSearchCodexHomeSources(
	options: Pick<
...
export async function resolveSessionSearchCodexHomeDirs(
	...
	return (await resolveSessionSearchCodexHomeSources(options)).map(
		(source) => source.dir,
	);
}
...
export function parseSinceSpec(
	...
	throw new Error(
		`Invalid --since value "${value}". Use formats like 7d, 24h, or 2026-03-10.`,
	);
}
	...
	if (topType === "response_item") {
		...
		if (payloadType === "message") {
			...
			if (text)
				texts.push({
					text,
					recordType: `response_item:${payloadType}:${role}`,
				});
			return texts;
		}
...
async function searchRolloutFile(
	filePath: string,
	options: Required<
		Pick<SessionSearchOptions, "query" | "context" | "caseSensitive">
	> & {
		limit: number;
		session?: string;
		...
		for await (const line of reader) {
			...
			if (lineNumber === 1) {
				...
				const fallbackSessionId =
					filePath.split("rollout-")[1]?.replace(/\.jsonl$/, "") ?? filePath;
				if (!meta) {
				...
				const sessionTimestamp = meta.timestamp
					...
					: Number.NaN;
				if (
					options.sinceCutoff != null &&
					!Number.isNaN(sessionTimestamp) &&
					sessionTimestamp < options.sinceCutoff
				) {
					skipFile = true;
					break;
				}
				if (
					!matchesFilter(meta.sessionId, options.session, options.caseSensitive)
				) {
					skipFile = true;
					break;
				}
				if (
					!matchesFilter(meta.cwd, options.projectFilter, options.caseSensitive)
				) {
					skipFile = true;
					break;
				}
			...
			for (const candidate of extractSearchableTexts(parsed, line)) {
				...
				results.push({
					...
					transcript_path: join(
						options.codexHomeDisplay,
						relative(options.codexHomeDir, filePath),
					),
...
export async function searchSessionHistory(
	...
	const limit =
		clampInteger(options.limit ?? DEFAULT_LIMIT, DEFAULT_LIMIT, MAX_LIMIT) ||
		DEFAULT_LIMIT;
	const context =
		clampInteger(
	...
	for (const codexHomeSource of codexHomeSources) {
		...
		sources.push({
			codex_home: codexHomeSource.publicLabel,
			searched_files: sourceSearch.searchedFiles,
		});
		for (const sessionId of sourceSearch.matchedSessions)
			matchedSessions.add(sessionId);
	...
	const limitedMatchedSessions = new Set(
		limitedResults.map((result) => result.session_id),
	);
