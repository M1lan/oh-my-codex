...
function matchesFilter(
	...
	return normalizeDarwinPathAlias(value)
		.toLowerCase()
		.includes(normalizeDarwinPathAlias(filter).toLowerCase());
}
...
async function normalizeExistingCodexHomeDirs(
	candidates: Array<string | ResolvedCodexHomeSource>,
): Promise<ResolvedCodexHomeSource[]> {
	...
	for (const candidate of candidates) {
		...
		dirs.push({
			dir: absolute,
			publicLabel:
				typeof candidate === "string" ? "codex-home" : candidate.publicLabel,
		});
	}
	return dirs;
}

async function resolveCodexHomeSources(
	options: Pick<
		SessionFrictionOptions,
		"cwd" | "codexHomeDir" | "codexHomeDirs"
	>,
): Promise<ResolvedCodexHomeSource[]> {
	...
	if (options.codexHomeDir) {
		return normalizeExistingCodexHomeDirs([
			{ dir: options.codexHomeDir, publicLabel: "explicit-codex-home" },
		]);
	}
	...
	return normalizeExistingCodexHomeDirs([
		...
		...projectHomes.map((home) => ({
			dir: home.path,
			publicLabel:
				home.source === "madmax-run"
					? (home.publicLabel ?? "madmax:runtime-codex-home")
...
function buildRisks(input: {
	...
	if (input.bytes >= DANGER_BYTES || input.records >= DANGER_RECORDS) {
		risks.push({
			code: "context_bloat_high",
			severity: "high",
			message:
				"Transcript metadata suggests a very large continuation context.",
		});
	} else if (input.bytes >= WARN_BYTES || input.records >= WARN_RECORDS) {
		risks.push({
			code: "context_bloat_watch",
			severity: "warn",
			message: "Transcript metadata suggests growing continuation context.",
		});
	}
	if (input.idleMinutes != null && input.idleMinutes >= STALE_MINUTES) {
		risks.push({
			code: "stale_session",
			severity: "warn",
			message: "Last observed activity is more than 24 hours old.",
		});
	}
	if (input.ageMinutes != null && input.ageMinutes >= OLD_SESSION_MINUTES) {
		risks.push({
			...
			severity: "warn",
			message: "Session began more than 7 days ago.",
		});
	}
	if (input.maxGapMinutes != null && input.maxGapMinutes >= 120) {
		risks.push({
			...
			severity: "info",
			message:
				"Session contains idle gaps over 2 hours, which can make continuation state stale.",
		});
	}
	if (
		input.toolCalls >= 100 ||
		(input.userTurns > 0 && input.toolCalls / input.userTurns >= 10)
	) {
		risks.push({
			code: "tool_heavy",
			severity: "warn",
			message: "Tool-call volume is high relative to user turns.",
		});
	}
	if (input.malformedRecords > 0) {
		risks.push({
			code: "parse_gaps",
			severity: "info",
			message: "Some transcript records could not be parsed as JSON.",
		});
	}
	if (risks.length === 0) {
		risks.push({
			code: "no_obvious_friction",
			severity: "info",
			message:
				"No obvious size, staleness, or tool-volume risk crossed default thresholds.",
		});
	}
	return risks;
}
...
async function inspectRolloutFile(
	filePath: string,
	source: ResolvedCodexHomeSource,
	options: {
		now: number;
...
): Promise<SessionFrictionSessionReport | null> {
		...
		for await (const line of reader) {
			...
			if (records === 1) {
				meta = extractSessionMeta(parsed) ?? {
					sessionId:
						filePath.split("rollout-")[1]?.replace(/\.jsonl$/, "") ??
	...
	const ageMinutes =
		startedAt != null && !Number.isNaN(startedAt)
			? Math.max(0, roundMinutes(options.now - startedAt))
			: null;
	const idleMinutes =
		lastActivity != null && !Number.isNaN(lastActivity)
			? Math.max(0, roundMinutes(options.now - lastActivity))
			: null;
	...
	return {
		...
		context_growth: {
			...
			tool_call_ratio:
				userTurns > 0 ? roundOne(toolCalls / userTurns) : toolCalls,
		},
		idle_gaps: {
...
export async function buildSessionFrictionReport(
	...
	const projectFilter = normalizeProjectFilter(
		options.project ?? "current",
		cwd,
	);
	const sources = await resolveCodexHomeSources({
		cwd,
		codexHomeDir: options.codexHomeDir,
		codexHomeDirs: options.codexHomeDirs,
	});
	...
	for (const source of sources) {
		...
		const files = (await listRolloutFiles(rolloutRoot)).sort((a, b) =>
			b.localeCompare(a),
		);
		let sourceScanned = 0;
