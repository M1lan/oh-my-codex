...
function authorityRevisionOf(state: SubagentTrackingState): string {
	const sessions = Object.keys(state.sessions)
		...
		.map((sessionId) => {
			...
			const threads = Object.keys(session.threads)
				.sort()
				.map((threadId) => {
...
export function resolveInstalledRoleName(
	...
	for (const agentsDir of [
		...
		projectCodexAgentsDir(projectRootOverride),
	]) {
		try {
		...
		for (const [threadId, rawThread] of Object.entries(
			...
			threads[normalizedThreadId] = {
				...
				candidate.resume_failure_reason.trim().length > 0
					...
					: {}),
				...(typeof candidate.direct_child_root_id === "string" &&
				candidate.direct_child_root_id.trim().length > 0
					? { direct_child_root_id: candidate.direct_child_root_id.trim() }
					: {}),
				...(typeof candidate.direct_child_parent_id === "string" &&
				candidate.direct_child_parent_id.trim().length > 0
					? { direct_child_parent_id: candidate.direct_child_parent_id.trim() }
					: {}),
				...(candidate.reopen_authority_revoked === true
					? { reopen_authority_revoked: true }
					: {}),
				...(typeof candidate.reopen_authority_conflict_reason === "string" &&
				candidate.reopen_authority_conflict_reason.trim()
					? {
							reopen_authority_conflict_reason:
								candidate.reopen_authority_conflict_reason.trim(),
						}
					: {}),
				...(typeof candidate.reopen_authority_conflict_at === "string" &&
				candidate.reopen_authority_conflict_at.trim()
					? {
...
function parseStrictSubagentTrackingState(
	raw: string,
): SubagentTrackingState | null {
	let parsed: unknown;
	try {
	...
	if (
		candidate.schemaVersion !== SUBAGENT_TRACKING_SCHEMA_VERSION ||
		!candidate.sessions ||
	...
	for (const [sessionId, sessionValue] of Object.entries(
		candidate.sessions as Record<string, unknown>,
	)) {
		if (
			!sessionId ||
			sessionId.trim() !== sessionId ||
			normalizedSessionIds.has(sessionId.trim())
		)
			return null;
		...
		if (
			!session.threads ||
			typeof session.threads !== "object" ||
			Array.isArray(session.threads)
		)
			return null;
		if (
			session.leader_thread_id !== undefined &&
			(typeof session.leader_thread_id !== "string" ||
				!session.leader_thread_id.trim() ||
				session.leader_thread_id.trim() !== session.leader_thread_id)
		)
			return null;
		const normalizedThreadIds = new Set<string>();
		for (const [threadId, threadValue] of Object.entries(
			session.threads as Record<string, unknown>,
		)) {
			if (
				!threadId ||
				threadId.trim() !== threadId ||
				normalizedThreadIds.has(threadId.trim())
			)
				return null;
			...
			if (
				...
				(thread.kind !== "leader" && thread.kind !== "subagent")
			)
				return null;
			if (
				typeof thread.first_seen_at !== "string" ||
				!thread.first_seen_at.trim()
			)
				return null;
			if (
				typeof thread.last_seen_at !== "string" ||
				!thread.last_seen_at.trim()
			)
				return null;
			if (
				typeof thread.turn_count !== "number" ||
				!Number.isFinite(thread.turn_count) ||
				thread.turn_count < 1
			)
				return null;
			...
			if (
				...
				normalizeSubagentStatus(thread.status) === undefined
			)
				return null;
			if (
				hasAuthorityEvidence &&
				thread.provenance_kind !== undefined &&
				thread.provenance_kind !== NATIVE_SUBAGENT_PROVENANCE
			)
				return null;
			for (const field of [
				"direct_child_root_id",
				"direct_child_parent_id",
				"reopen_authority_conflict_reason",
				"reopen_authority_conflict_at",
			] as const) {
				if (
					thread[field] !== undefined &&
					(typeof thread[field] !== "string" ||
						!(thread[field] as string).trim() ||
						(thread[field] as string).trim() !== thread[field])
				)
					return null;
			}
			if (
				thread.reopen_authority_revoked !== undefined &&
				typeof thread.reopen_authority_revoked !== "boolean"
			)
				return null;
			if (
				(thread.direct_child_root_id === undefined) !==
...
function readSubagentTrackingStateStrictSync(
	cwd: string,
): { ok: true; state: SubagentTrackingState } | { ok: false } {
	const path = subagentTrackingPath(cwd);
	if (!existsSync(path))
		return { ok: true, state: createSubagentTrackingState() };
	try {
...
function threadIsTrackedAsSubagent(
	...
	return (
		...
		Object.values(state.sessions).some((session) =>
			isTrustedSubagentThread(session, id),
		)
	);
}

export function hasLeaderSubagentCollision(
	state: SubagentTrackingState,
	leaderThreadId: string,
): boolean {
...
export async function writeSubagentTrackingState(
	...
	if (!parseStrictSubagentTrackingState(proposedRaw))
	...
	return withCrossProcessFileLockSync(path, (lock) => {
		if (existsSync(path)) {
			...
			proposedAuthorityRevision !==
			authorityRevisionOf(createSubagentTrackingState())
		) {
			...
			const mintsAuthority = Object.values(normalized.sessions).some(
				(session) =>
					Object.values(session.threads).some(
						(thread) =>
							thread.provenance_kind === NATIVE_SUBAGENT_PROVENANCE ||
							thread.direct_child_root_id !== undefined ||
							thread.direct_child_parent_id !== undefined ||
	...
	const nextThread: TrackedSubagentThread = {
		...
		...(input.resumeFailureReason?.trim()
			...
			: existingThread?.resume_failure_reason
				...
				: {}),
		...(existingThread?.direct_child_root_id
			? { direct_child_root_id: existingThread.direct_child_root_id }
			: {}),
		...(existingThread?.direct_child_parent_id
			? { direct_child_parent_id: existingThread.direct_child_parent_id }
			: {}),
		...(existingThread?.reopen_authority_revoked
			? { reopen_authority_revoked: true }
			: {}),
		...(existingThread?.reopen_authority_conflict_reason
			? {
					reopen_authority_conflict_reason:
						existingThread.reopen_authority_conflict_reason,
				}
			: {}),
		...(existingThread?.reopen_authority_conflict_at
			? {
					reopen_authority_conflict_at:
						existingThread.reopen_authority_conflict_at,
				}
...
export function recordNativeSubagentAuthorityObservation(
	...
	const requestedSessionIds = [
		...new Set(
			observation.sessionIds.map((value) => value.trim()).filter(Boolean),
		),
	];
	...
	const implicatedChildThreadIds = [
		...new Set(
			[
				childThreadId,
				...(observation.implicatedChildThreadIds ?? []).map((value) =>
					value.trim(),
				),
			].filter(Boolean),
		),
	...
	return withCrossProcessFileLockSync(subagentTrackingPath(cwd), (context) => {
		...
		const existingSessionIds = Object.entries(next.sessions)
			.filter(([, session]) =>
				implicatedChildThreadIds.some((id) =>
					Boolean(
						session.threads[id]?.direct_child_root_id ||
							session.threads[id]?.reopen_authority_revoked,
					),
				),
			)
			.map(([sessionId]) => sessionId);
		const sessionIds = [
			...new Set([...requestedSessionIds, ...existingSessionIds]),
		];
		const existingBindings = Object.values(next.sessions)
			.map((session) => session.threads[childThreadId])
			.filter((thread): thread is TrackedSubagentThread =>
				Boolean(thread?.direct_child_root_id),
			);
		const authorityEvidence =
			observation.authorityEvidence ??
			(rootNativeSessionId ? "valid" : "absent");
		const globalConflict =
			(authorityEvidence === "untrusted" && existingBindings.length > 0) ||
		...
		if (authorityEvidence === "untrusted") {
			for (const session of Object.values(next.sessions)) {
				for (const implicatedId of implicatedChildThreadIds) {
					...
					if (
						!implicatedThread?.direct_child_root_id &&
						!implicatedThread?.reopen_authority_revoked
					)
						continue;
					implicatedThread.reopen_authority_revoked = true;
					implicatedThread.reopen_authority_conflict_reason =
						"identity_untrusted";
 ...
 */
export function fenceNativeSubagentAuthorities(
	cwd: string,
	...
	try {
		return withCrossProcessFileLockSync(path, (lock) => {
			...
			if (merged.length > AUTHORITY_FENCE_MAX_IDS) {
				...
				lock.assertOwnership();
				lock.publish(
					`${JSON.stringify({ ids: "all", reason: "fence_capacity_exceeded", fenced_at: new Date().toISOString() }, null, 2)}\n`,
				);
				return true;
			}
			mkdirSync(dirname(path), { recursive: true });
			lock.assertOwnership();
			lock.publish(
				`${JSON.stringify({ ids: merged, reason, fenced_at: new Date().toISOString() }, null, 2)}\n`,
			);
			return true;
		});
	} catch {
 ...
 * durable on disk.
 */
export function forceGlobalAuthorityDenial(
	cwd: string,
	reason: string,
): boolean {
	const path = authorityFencePath(cwd);
...
function clearAuthorityFence(cwd: string, childThreadIds: string[]): void {
	...
	const cleared = new Set(
		childThreadIds.map((value) => value.trim()).filter(Boolean),
	);
	...
	try {
		withCrossProcessFileLockSync(path, (lock) => {
			...
			lock.publish(
				`${JSON.stringify({ ids: remaining, reason: "partial_clear", fenced_at: new Date().toISOString() }, null, 2)}\n`,
			);
		});
	} catch {
...
export function revokeNativeSubagentAuthorities(
	...
	const ids = [
		...new Set(childThreadIds.map((value) => value.trim()).filter(Boolean)),
	];
	return withCrossProcessFileLockSync(subagentTrackingPath(cwd), (context) => {
		const strict = readSubagentTrackingStateStrictSync(cwd);
		if (!strict.ok)
		...
		for (const session of Object.values(next.sessions)) {
			for (const id of ids) {
				...
				if (!thread?.direct_child_root_id && !thread?.reopen_authority_revoked)
					continue;
				thread.reopen_authority_revoked = true;
...
function persistedReopenLegacyNotice(
...
function repairPersistedRootIdentityInState(
	...
	for (const correlatedSession of Object.values(state.sessions)) {
		...
		const correlated = partitionCorrelatesToRoot(
			correlatedSession,
			sessionId,
			rootNativeSessionId,
		);
		for (const rootId of new Set([sessionId, rootNativeSessionId])) {
			...
			const ambiguousCanonicalCollision =
				rootId === sessionId && sessionId !== rootNativeSessionId;
			if (ambiguousCanonicalCollision) {
				...
				if (carriesAuthority && rootThread.reopen_authority_revoked !== true) {
					...
					rootThread.reopen_authority_conflict_reason =
						"canonical_root_collision";
					rootThread.reopen_authority_conflict_at = repairTimestamp;
					sessionChanged = true;
				}
				if (
					rootThread.resume_requested_at !== undefined ||
		...
		if (
			correlatedSession.threads[rootNativeSessionId] &&
...
export function repairPersistedRootIdentity(
	...
	return withCrossProcessFileLockSync(path, (lock) => {
		...
		const changed = repairPersistedRootIdentityInState(
			state,
			sessionId,
			rootNativeSessionId,
...
export function quarantinePersistedRootAuthority(
	...
	return withCrossProcessFileLockSync(path, (lock) => {
		...
		for (const session of Object.values(state.sessions)) {
			if (!partitionCorrelatesToRoot(session, sessionId, rootNativeSessionId))
				continue;
			...
			if (carriesAuthority && rootThread.reopen_authority_revoked !== true) {
				rootThread.reopen_authority_revoked = true;
				rootThread.reopen_authority_conflict_reason =
					"contradictory_root_child_evidence";
...
export function consumeDirectChildReopenContext(
	...
	return withCrossProcessFileLockSync(path, (lock) => {
		...
		const state = parseStrictSubagentTrackingState(raw);
		if (!state)
			return persistedReopenAuthorityWarning("malformed_tracker_state");
		...
		if (!fence.ok)
			return persistedReopenAuthorityWarning("unreadable_authority_fence");
		...
		const savedSubagents = Object.values(session.threads)
			.filter((thread) => {
				...
				return Object.values(state.sessions).every((correlationSession) => {
					...
					if (
						view.provenance_kind === DESCRIPTIVE_ADAPTED_PROVENANCE &&
						view.kind === "subagent" &&
						correlationSession.leader_thread_id !== thread.thread_id
					)
						return true;
					return (
						correlationSession.leader_thread_id !== thread.thread_id &&
		...
		if (!savedSubagents.length) {
			...
			const everyRelevantViewIsLegacy =
				excludedIds.size > 0 &&
				Object.values(state.sessions).every((correlationSession) => {
					if (
						correlationSession.leader_thread_id &&
						excludedIds.has(correlationSession.leader_thread_id)
					)
						return false;
		...
		const reopenTargets = savedSubagents.filter(
			(entry) => entry.status !== "unavailable",
		);
		const unavailableTargets = savedSubagents.filter(
			(entry) => entry.status === "unavailable",
		);
		const failedTargets = savedSubagents.filter(
			(entry) => entry.resumeFailedAt || entry.resumeFailureReason,
		);
		const now = new Date().toISOString();
		for (const target of reopenTargets) {
			session.threads[target.threadId] = {
				...session.threads[target.threadId]!,
		...
		if (reopenTargets.length) {
			...
			lines.push(
				...reopenTargets
					...
					.map(
						(entry) =>
							`  - resume_agent(${JSON.stringify(entry.agentId)})${formatReopenMetadata(entry)}`,
					),
			);
		...
		if (warnings.length) {
			...
			lines.push(
				...warnings
					.slice(0, 8)
					.map((entry) => `  - ${entry.agentId}${formatReopenMetadata(entry)}`),
			);
			if (warnings.length > 8)
				lines.push(
					`  - ... ${warnings.length - 8} more warning(s) omitted from this compact SessionStart context.`,
				);
		}
		return lines.join("\n");
	});
