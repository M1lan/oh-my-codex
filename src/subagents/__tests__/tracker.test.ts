...
function spawnAuthorityCompetitor(
	cwd: string,
	operation: "reopen" | "revoke",
	readyPath: string,
	resultPath: string,
) {
	return spawn(
		process.execPath,
		["--input-type=module", "--eval", AUTHORITY_COMPETITOR_SOURCE],
		{
			env: {
				...process.env,
...
function spawnCrossProcessLockHolder(
	...
	return spawn(
		process.execPath,
		["--input-type=module", "--eval", CROSS_PROCESS_LOCK_HOLDER_SOURCE],
		{
			env: {
				...process.env,
				OMX_TRACKER_MODULE_URL: new URL("../tracker.js", import.meta.url).href,
				...
				...(publicationPath
					? { OMX_LOCK_PUBLICATION_PATH: publicationPath }
					: {}),
			},
		},
	);
...
async function waitForChildExit(
	...
	await new Promise<void>((resolve, reject) => {
		...
		child.once("exit", (code) =>
			code === 0
				? resolve()
				: reject(new Error(`Lock-owner child exited with code ${code}`)),
		);
	});
}
...
describe("subagents/tracker", () => {
	...
	it("authorizes only attested direct children for persisted reopen", () => {
		...
		try {
			...
			const context =
				consumeDirectChildReopenContext(cwd, {
					sessionId: "root",
					rootNativeSessionId: "root",
					source: "resume",
				}) ?? "";
			assert.match(context, /resume_agent\("direct"\)/);
			assert.doesNotMatch(
				context,
				/resume_agent\("root"\)|resume_agent\("nested"\)|resume_agent\("revoked"\)/,
			);
			const persisted = JSON.parse(readFileSync(path, "utf8"));
			assert.match(
				persisted.sessions.root.threads.direct.resume_requested_at,
				/^\d{4}-\d{2}-\d{2}T/,
			);
			assert.equal(
				persisted.sessions.root.threads.root.resume_requested_at,
				undefined,
			);
		} finally {
	...
	it("atomically revokes conflicting direct-child authority across correlation records", () => {
		...
		try {
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root-a"],
				childThreadId: "child",
				parentThreadId: "root-a",
				rootNativeSessionId: "root-a",
			});
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root-b"],
				childThreadId: "child",
				parentThreadId: "root-b",
				rootNativeSessionId: "root-b",
			});
			const state = JSON.parse(readFileSync(subagentTrackingPath(cwd), "utf8"));
			assert.equal(
				state.sessions.canonical.threads.child.reopen_authority_revoked,
				true,
			);
			assert.equal(
				state.sessions["root-a"].threads.child.reopen_authority_revoked,
				true,
			);
			assert.equal(
				state.sessions["root-b"].threads.child.reopen_authority_revoked,
				true,
			);
			assert.equal(
				state.sessions.canonical.threads.child.reopen_authority_conflict_reason,
				"root_or_parent_mismatch",
			);
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root-a"],
				childThreadId: "child",
			...
			const repaired = JSON.parse(
				readFileSync(subagentTrackingPath(cwd), "utf8"),
			);
			assert.equal(
				repaired.sessions.canonical.threads.child.reopen_authority_revoked,
				undefined,
			);
			assert.equal(
				repaired.sessions["root-a"].threads.child.reopen_authority_revoked,
				undefined,
			);
			assert.match(
				consumeDirectChildReopenContext(cwd, {
	...
	it("denies a leader-colliding attested child without bookkeeping", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-leader-collision-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
			...
			assert.equal(
				state.sessions.root.threads.child.resume_requested_at,
				undefined,
			);
		} finally {
			rmSync(cwd, { recursive: true, force: true });
	...
	it("denies malformed valid-JSON authority evidence without rewriting bytes", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-malformed-authority-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
					...
					root: {
						...
						threads: {
							child: {
								...
								direct_child_root_id: "root",
								direct_child_parent_id: "root",
								reopen_authority_revoked: "false",
								status: "available",
								first_seen_at: "2026-07-23T00:00:00.000Z",
								last_seen_at: "2026-07-23T00:00:00.000Z",
								turn_count: 1,
	...
	it("denies malformed status authority evidence without rewriting bytes", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-malformed-status-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
	...
	it("denies an unattested descriptive correlation view", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-partial-correlation-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
	...
	it("denies cross-partition authority disagreement", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-partition-disagreement-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
					...
					sessions: {
						...
						correlation: {
							...
							threads: {
								child: {
									...thread,
									direct_child_root_id: "foreign",
									direct_child_parent_id: "foreign",
								},
							},
						},
					},
	...
	it("revokes cached authority after an identity-untrusted reobservation", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-untrusted-reobserve-"),
		);
		try {
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root"],
			...
			assert.equal(
				state.sessions.root.threads.child.reopen_authority_revoked,
				true,
			);
			assert.equal(
				consumeDirectChildReopenContext(cwd, {
	...
	it("ignores a same-child adapted legacy peer when validating a newly attested child", () => {
		...
		try {
			...
			const persisted = JSON.parse(readFileSync(path, "utf8"));
			assert.equal(
				persisted.sessions.correlation.threads.child.provenance_kind,
				DESCRIPTIVE_ADAPTED_PROVENANCE,
			);
			assert.equal(
				persisted.sessions.correlation.threads.child.direct_child_root_id,
				undefined,
			);
		} finally {
			rmSync(cwd, { recursive: true, force: true });
	...
	it("denies a canonical-key-colliding attested child without destructive reclassification", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-canonical-collision-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
			...
			const persisted = JSON.parse(readFileSync(path, "utf8"));
			assert.equal(
				persisted.sessions.canonical.threads.canonical.kind,
				"subagent",
			);
			assert.equal(
				persisted.sessions.canonical.threads.canonical.reopen_authority_revoked,
				true,
			);
			assert.equal(
				persisted.sessions.canonical.threads.canonical
					.reopen_authority_conflict_reason,
				"canonical_root_collision",
			);
			assert.equal(
				persisted.sessions.canonical.threads.canonical.resume_requested_at,
				undefined,
			);
		} finally {
			rmSync(cwd, { recursive: true, force: true });
	...
	it("keeps a legacy plain canonical-key collision as ambiguous without revocation or reclassification", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-legacy-collision-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
	...
	it("denies an adapted peer view colliding with the session leader boundary", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-adapted-leader-boundary-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
	...
	it("rejects malformed proposed state in the generic writer even without an existing tracker", async () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-malformed-proposed-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			const malformedProposed = {
				schemaVersion: 1,
				sessions: {
					root: {
						session_id: "root",
			...
			await assert.rejects(
				writeSubagentTrackingState(cwd, malformedProposed as never),
			);
			assert.equal(existsSync(path), false);
			const validState = recordSubagentTurn(createSubagentTrackingState(), {
				sessionId: "root",
				threadId: "child",
				kind: "subagent",
			});
	...
	it("refuses a stale generic write that would roll back a newer authority revocation", async () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-stale-generic-write-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			recordNativeSubagentAuthorityObservation(cwd, {
			...
			assert.equal(
				JSON.parse(readFileSync(path, "utf8")).sessions.canonical.threads.child
					.reopen_authority_revoked,
				true,
			);
			// Publishing the stale snapshot must not resurrect the revoked authority.
			await assert.rejects(
				writeSubagentTrackingState(cwd, stale),
				/Stale subagent tracker authority state/,
			);
			const persisted = JSON.parse(readFileSync(path, "utf8"));
			assert.equal(
				persisted.sessions.canonical.threads.child.reopen_authority_revoked,
				true,
			);
			assert.equal(
				consumeDirectChildReopenContext(cwd, {
	...
	it("still allows a descriptive generic write when authority has not moved", async () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-descriptive-write-ok-"),
		);
		try {
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root"],
			...
			const descriptive = recordSubagentTurn(snapshot, {
				sessionId: "canonical",
				threadId: "child",
				kind: "subagent",
			});
	...
	it("leaves an uncorrelated partition untouched when a thread id collides with the canonical key", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-uncorrelated-collision-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
						...
						unrelated: {
							...
							threads: {
								canonical: {
									...
									resume_requested_at: "2026-07-23T00:02:00.000Z",
									resume_completed_at: "2026-07-23T00:03:00.000Z",
									status: "available",
									first_seen_at: "2026-07-23T00:00:00.000Z",
									last_seen_at: "2026-07-23T00:00:00.000Z",
									turn_count: 1,
			...
			assert.equal(
				foreignChild.resume_requested_at,
				"2026-07-23T00:02:00.000Z",
			);
			assert.equal(
				foreignChild.resume_completed_at,
	...
	it("still quarantines a canonical-key collision inside a correlated partition", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-correlated-collision-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
			...
			const collided = JSON.parse(readFileSync(path, "utf8")).sessions
				.correlated.threads.canonical;
			assert.equal(collided.kind, "subagent");
			assert.equal(collided.reopen_authority_revoked, true);
			assert.equal(
				collided.reopen_authority_conflict_reason,
				"canonical_root_collision",
			);
	...
	it("keeps authority denied when a revocation could not be published", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-authority-fence-"),
		);
		try {
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root"],
			...
			assert.equal(
				JSON.parse(readFileSync(subagentTrackingPath(cwd), "utf8")).sessions
					.canonical.threads.child.reopen_authority_revoked,
				undefined,
			);
			assert.equal(
				consumeDirectChildReopenContext(cwd, {
	...
	it("fails closed for every id when the authority fence is unreadable", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-fence-unreadable-"),
		);
		try {
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root"],
			...
			writeFileSync(
				`${subagentTrackingPath(cwd)}.authority-fence.json`,
				"{ not json",
			);
			assert.match(
				consumeDirectChildReopenContext(cwd, {
	...
	it("refuses a fresh generic write that would grant or remove reopen authority", async () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-fresh-generic-authority-"),
		);
		try {
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root"],
			...
			delete fresh.sessions.canonical!.threads.child!
				.reopen_authority_conflict_reason;
			delete fresh.sessions.canonical!.threads.child!
				.reopen_authority_conflict_at;
			await assert.rejects(
				writeSubagentTrackingState(cwd, fresh),
				/Stale subagent tracker authority state/,
			);
			assert.equal(
				consumeDirectChildReopenContext(cwd, {
			...
			const plain = recordSubagentTurn(granting, {
				sessionId: "canonical",
				threadId: "newcomer",
				kind: "subagent",
			});
			plain.sessions.canonical!.threads.newcomer!.provenance_kind =
				NATIVE_SUBAGENT_PROVENANCE;
			plain.sessions.canonical!.threads.newcomer!.direct_child_root_id = "root";
			plain.sessions.canonical!.threads.newcomer!.direct_child_parent_id =
				"root";
			await assert.rejects(
				writeSubagentTrackingState(cwd, plain),
				/Stale subagent tracker authority state/,
			);
			assert.equal(
				consumeDirectChildReopenContext(cwd, {
	...
	it("refuses a generic write that would mint authority into a fresh tracker", async () => {
		...
		try {
			const minted = recordSubagentTurn(createSubagentTrackingState(), {
				sessionId: "canonical",
				threadId: "child",
				kind: "subagent",
			});
			minted.sessions.canonical!.threads.child!.provenance_kind =
				NATIVE_SUBAGENT_PROVENANCE;
			...
			await assert.rejects(
				writeSubagentTrackingState(cwd, minted),
				/Stale subagent tracker authority state/,
			);
	...
	it("lets a fresh generic write set a descriptive leader_thread_id but never mint authority", async () => {
		...
		try {
			...
			const descriptive = recordSubagentTurn(createSubagentTrackingState(), {
				sessionId: "canonical",
				threadId: "child",
				kind: "subagent",
	...
	it("denies every reopen target after a forced global authority denial", () => {
		...
		try {
			...
			assert.equal(
				forceGlobalAuthorityDenial(cwd, "identity_untrusted_denial_escalated"),
				true,
			);
			assert.match(
				consumeDirectChildReopenContext(cwd, {
	...
	it("refuses a generic write that would roll an authorized child back to available", async () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-status-rollback-"),
		);
		try {
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root"],
			...
			await assert.rejects(
				writeSubagentTrackingState(cwd, snapshot),
				/Stale subagent tracker authority state/,
			);
		} finally {
			rmSync(cwd, { recursive: true, force: true });
	...
	it("never destroys an unreadable global fence during an unrelated revocation clear", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-fence-preserved-"),
		);
		try {
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root"],
	...
	it("does not weaken an unreadable global fence when a later fence records more ids", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-fence-no-weaken-"),
		);
		try {
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root"],
	...
	it("clears only fully reconciled fence ids and keeps the rest denied", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-fence-partial-clear-"),
		);
		try {
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root"],
	...
	it("repairs a root inversion even when the canonical tracker partition is missing", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-native-only-repair-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
			writeFileSync(
				...
				JSON.stringify({
					...
					sessions: {
						root: {
							session_id: "root",
							leader_thread_id: "turn-like-foreign",
							updated_at: "2026-07-23T00:00:00.000Z",
							threads: {
								root: {
									thread_id: "root",
	...
	it("repairs a root inversion through the decoupled repair entrypoint", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-decoupled-repair-"),
		);
		try {
			const path = subagentTrackingPath(cwd);
			mkdirSync(join(cwd, ".omx", "state"), { recursive: true });
			...
			assert.equal(
				repairPersistedRootIdentity(cwd, {
					sessionId: "canonical",
					rootNativeSessionId: "root",
				}),
				true,
	...
	it("surfaces an explicit notice when only legacy non-authoritative records are excluded", () => {
		...
		try {
			...
			const persisted = JSON.parse(readFileSync(path, "utf8"));
			assert.equal(
				persisted.sessions.canonical.threads.legacy.resume_requested_at,
				undefined,
			);
		} finally {
			rmSync(cwd, { recursive: true, force: true });
		...
		try {
			...
			for (let index = 0; index < 14; index += 1)
				threads[`child-${index}`] = {
					...
					status: "available",
					first_seen_at: "2026-07-23T00:00:00.000Z",
					last_seen_at: `2026-07-23T00:${String(index).padStart(2, "0")}:00.000Z`,
					turn_count: 1,
				};
			for (let index = 0; index < 10; index += 1)
				threads[`warning-${index}`] = {
					thread_id: `warning-${index}`,
					kind: "subagent",
					provenance_kind: NATIVE_SUBAGENT_PROVENANCE,
					direct_child_root_id: "root",
					direct_child_parent_id: "root",
	...
	it("revokes direct-child authority when an authoritative observation moves it under a nested parent", () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-direct-to-nested-"),
		);
		try {
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "root"],
			...
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "nested-parent"],
				childThreadId: "child",
			...
			assert.equal(
				state.sessions["nested-parent"].threads.child.reopen_authority_revoked,
				true,
			);
			assert.equal(
				consumeDirectChildReopenContext(cwd, {
	...
	it("linearizes revocation before reopen as a denied operation", () => {
		...
		try {
			...
			recordNativeSubagentAuthorityObservation(cwd, {
				sessionIds: ["canonical", "other-root"],
				childThreadId: "child",
				parentThreadId: "other-root",
				rootNativeSessionId: "other-root",
			});
			assert.equal(
				consumeDirectChildReopenContext(cwd, {
					sessionId: "canonical",
			...
			const state = JSON.parse(readFileSync(subagentTrackingPath(cwd), "utf8"));
			assert.equal(
				state.sessions.canonical.threads.child.resume_requested_at,
				undefined,
			);
		} finally {
			rmSync(cwd, { recursive: true, force: true });
	...
	it("linearizes reopen before a later revocation and denies subsequent reopen", () => {
		...
		try {
			...
			const requestedAt = JSON.parse(
				readFileSync(subagentTrackingPath(cwd), "utf8"),
			).sessions.canonical.threads.child.resume_requested_at;
			...
			assert.equal(
				revoked.sessions.canonical.threads.child.resume_requested_at,
				requestedAt,
			);
			assert.equal(
				revoked.sessions.canonical.threads.child.reopen_authority_revoked,
				true,
			);
		} finally {
			rmSync(cwd, { recursive: true, force: true });
	...
	it("serializes a queued reopen behind revocation publication", async () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-contended-revoke-first-"),
		);
		...
		try {
			...
			__setCrossProcessPublishBarrierForTest(() => {
				competitor = spawnAuthorityCompetitor(
					cwd,
					"reopen",
					readyPath,
					resultPath,
	...
	it("serializes queued revocation behind completed reopen publication", async () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-3284-tracker-contended-reopen-first-"),
		);
	...
	it("never grants fresh authority from absent or untrusted evidence", () => {
		for (const authorityEvidence of ["absent", "untrusted"] as const) {
			const cwd = mkdtempSync(
				join(tmpdir(), `omx-3284-tracker-${authorityEvidence}-fresh-`),
			);
			try {
				recordNativeSubagentAuthorityObservation(cwd, {
					sessionIds: ["canonical", "root"],
				...
				assert.equal(
					state.sessions.canonical.threads.child.direct_child_root_id,
					undefined,
				);
				const context =
					consumeDirectChildReopenContext(cwd, {
				...
				assert.equal(
					JSON.parse(readFileSync(subagentTrackingPath(cwd), "utf8")).sessions
						.canonical.threads.child.resume_requested_at,
					undefined,
				);
			} finally {
				rmSync(cwd, { recursive: true, force: true });
	...
	it("can record an explicitly spawned subagent as subagent even when it is the first seen thread", () => {
		...
		assert.equal(
			state.sessions["sess-ralplan"]?.threads["thread-architect"]?.kind,
			"subagent",
		);
		assert.equal(
			state.sessions["sess-ralplan"]?.threads["thread-critic"]?.kind,
			"subagent",
		);
		assert.deepEqual(summary?.allSubagentThreadIds, [
			"thread-architect",
			"thread-critic",
		]);
	});
	...
	it("keeps an explicitly spawned first-seen subagent as subagent after a generic follow-up turn", () => {
		...
		assert.equal(
			state.sessions["sess-ralplan"]?.threads["thread-architect"]?.turn_count,
			2,
		);
		assert.equal(
			state.sessions["sess-ralplan"]?.threads["thread-architect"]?.last_turn_id,
			"turn-after-session-start",
		);
	});
	...
	it("does not promote a known subagent when it becomes an immediate parent", () => {
		...
		assert.equal(
			state.sessions["sess-ralplan"]?.threads["thread-researcher"]?.kind,
			"subagent",
		);
		assert.deepEqual(summary?.allSubagentThreadIds, [
			"thread-architect",
	...
	it("does not downgrade a known leader when later native metadata claims the same thread as subagent", () => {
		...
		assert.equal(
			state.sessions["sess-ralplan"]?.leader_thread_id,
			"thread-leader",
		);
		assert.equal(
			state.sessions["sess-ralplan"]?.threads["thread-leader"]?.kind,
			"leader",
		);
	});
	...
	it("records role and lane metadata for restart resume/reuse summaries", () => {
		...
		assert.equal(
			state.sessions["sess-conductor"]?.threads["thread-executor"]?.role,
			"executor",
		);
		assert.equal(
			state.sessions["sess-conductor"]?.threads["thread-executor"]?.lane_id,
			"implementation-fix",
		);
		assert.equal(
			state.sessions["sess-conductor"]?.threads["thread-executor"]
				?.provenance_kind,
			NATIVE_SUBAGENT_PROVENANCE,
		);
	});
	...
	it("builds a reusable ledger that preserves unavailable status and handoff summaries", () => {
		...
		state = recordSubagentTurn(state, {
			...
			leaderThreadId: "thread-leader",
			lastHandoffSummary:
				"architect reviewed v1 and requested reuse of the same lane",
			status: "available",
		});
		state = recordSubagentTurn(state, {
			sessionId: "sess-conductor",
		...
		assert.equal(
			ledger?.savedSubagents.find(
				(entry) => entry.agentId === "thread-architect",
			)?.status,
			"available",
		);
		assert.equal(
		...
		assert.equal(
			ledger?.savedSubagents.find((entry) => entry.agentId === "thread-critic")
				?.status,
			"unavailable",
		);
		assert.equal(
			ledger?.savedSubagents.find((entry) => entry.agentId === "thread-critic")
				?.lastHandoffSummary,
			"critic paused pending planner response",
		);
		assert.deepEqual(
	...
	it("preserves explicit closed ledger status so older available lanes win reuse selection", () => {
		...
		assert.equal(
			ledger.savedSubagents.find(
				(entry) => entry.agentId === "thread-closed-recent",
			)?.status,
			"closed",
		);
		assert.deepEqual(
	...
	it("does not let a stale owner release a successor lock", async () => {
		...
		try {
			withCrossProcessFileLockSync(resourcePath, () => {
				...
				successor = spawnCrossProcessLockHolder(
					resourcePath,
					successorReadyPath,
					successorReleasePath,
				);
				waitForFileSync(successorReadyPath);
			});

			assert.ok(successor);
			const successorClaim = JSON.parse(readFileSync(lockPath, "utf-8")) as {
				token?: unknown;
				pid?: unknown;
			};
	...
	it("removes displaced quarantine and release artifacts when a replacement wins restoration", async () => {
		...
		try {
			...
			assert.throws(
				() =>
					withCrossProcessFileLockSync(resourcePath, () => "must not acquire", {
						maxAttempts: 1,
						retryMs: 1,
					}),
			...
			assert.deepEqual(
				readdirSync(cwd).filter(
					(entry) =>
						entry.endsWith(".quarantine") || entry.endsWith(".release"),
				),
				[],
			);
			...
			assert.equal(
				withCrossProcessFileLockSync(resourcePath, () => "released"),
				"released",
			);
			assert.deepEqual(
				readdirSync(cwd).filter(
	...
	it("sweeps only lease-aged parseable lock displacement artifacts", async () => {
		...
		try {
			...
			assert.equal(
				withCrossProcessFileLockSync(resourcePath, () => "swept"),
				"swept",
			);
	...
	it("bounds the displacement-artifact sweep by a fixed per-acquisition cap, oldest-first", async () => {
		...
		try {
			...
			assert.equal(
				withCrossProcessFileLockSync(resourcePath, () => "first"),
				"first",
			);
			const remainingAfterFirst = artifactPaths.filter((path) =>
				existsSync(path),
			);
			...
			assert.deepEqual(
				remainingAfterFirst,
				artifactPaths.slice(CROSS_PROCESS_LOCK_ARTIFACT_SWEEP_CAP),
			);
			// A subsequent acquisition drains the remaining backlog.
			assert.equal(
				withCrossProcessFileLockSync(resourcePath, () => "second"),
				"second",
			);
			assert.deepEqual(
				artifactPaths.filter((path) => existsSync(path)),
				[],
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("treats an already-removed displaced release artifact as a clean terminal cleanup", async () => {
		...
		try {
			...
			__setCrossProcessQuarantineBarrierForTest(
				(_lockPath, quarantinedPath) => {
					rmSync(quarantinedPath, { force: true });
				},
			);
			assert.equal(
				withCrossProcessFileLockSync(resourcePath, () => "published"),
				"published",
			);
	...
	it("reclaims a dead-owner cross-process lock", async () => {
		...
		try {
			...
			assert.equal(
				withCrossProcessFileLockSync(resourcePath, () => "dead lock recovered"),
				"dead lock recovered",
			);
			assert.equal(existsSync(crossProcessLockPath(resourcePath)), false);
		} finally {
	...
	it("does not steal a live different-pid lock within its lease", async () => {
		...
		const resourcePath = join(cwd, "lock-resource");
		const lockPath = crossProcessLockPath(resourcePath);
		const liveOwner = spawn(process.execPath, [
			"--eval",
			"setInterval(() => {}, 1_000)",
		]);
		...
		try {
			...
			const claim = crossProcessLockClaim(
				"live-owner-token",
				liveOwner.pid,
				Date.now(),
			);
	...
	it("recovers despite an abandoned legacy recovery guard", async () => {
		...
		try {
			...
			assert.equal(
				withCrossProcessFileLockSync(resourcePath, () => "guard ignored"),
				"guard ignored",
			);
			assert.equal(existsSync(lockPath), false);
		} finally {
	...
	it("does not use local pid liveness to reclaim a remote claim before its lease", async () => {
		...
		try {
			...
			assert.ok(deadOwnerPid);
			const freshRemoteClaim = crossProcessLockClaim(
				"fresh-remote-token",
				deadOwnerPid,
				Date.now(),
				"remote-test-host",
			...
			assert.throws(
				() =>
					withCrossProcessFileLockSync(resourcePath, () => "must not run", {
						maxAttempts: 2,
						retryMs: 1,
					}),
				/Timed out waiting for cross-process lock/,
	...
	it("uses the lease fallback for legacy same-host claims without a process identity", async () => {
		...
		try {
			...
			const freshLegacyClaim = crossProcessLockClaim(
				"fresh-legacy-token",
				process.pid,
				Date.now(),
			);
	...
	it("uses the shared recoverable lock for role-routing markers", async () => {
		const baseStateDir = await mkdtemp(
			join(tmpdir(), "omx-role-routing-marker-"),
		);
		const nowMs = Date.now();
		try {
			await writeFile(
				crossProcessLockPath(
					join(baseStateDir, NATIVE_SUBAGENT_ROLE_ROUTING_MARKER_FILE),
				),
				crossProcessLockClaim(
					"expired-marker-token",
					process.pid,
					nowMs - CROSS_PROCESS_LOCK_LEASE_MS - 1,
					"remote-test-host",
				),
			);
	...
	it("reclaims a same-host claim after a reboot changes the boot id", async () => {
		...
		try {
			...
			assert.equal(
				withCrossProcessFileLockSync(resourcePath, () => "reboot recovered", {
					maxAttempts: 1,
					retryMs: 1,
				}),
				"reboot recovered",
	...
	it("fences a stalled role-routing marker writer after a successor publishes and leaves no temp artifacts", async () => {
		...
		try {
			...
			assert.throws(
				() => writeRoleRoutingMarker(baseStateDir, markerFor("sess-stalled")),
				CrossProcessLockLostError,
			);
			assert.equal(barrierRuns, 1);
			assert.equal(
			...
			assert.deepEqual(
				readdirSync(baseStateDir).filter(
					(entry) => entry.includes(".stage.") || entry.endsWith(".tmp"),
				),
				[],
			);
		} finally {
