...
const credentialDigest = (value: string) =>
	createHash("sha256").update(value).digest("hex");
const canonicalCorrelationToken = (value: string) =>
	credentialDigest(value).slice(0, 32);
...
type PendingRoleIntentWorkerInput =
	| PendingRoleIntentRecordWorkerInput
	| PendingRoleIntentConsumeWorkerInput
	| PendingRoleIntentRecordTurnWorkerInput;
...
type PendingRoleIntentWorkerResult =
	| { ok: boolean; reason?: string }
	| { role: string; provenanceKind: string }
	| null;
...
function spawnPendingRoleIntentWorker(
	startSignal: string,
	input: PendingRoleIntentWorkerInput,
): PendingRoleIntentWorker {
	const child = spawn(
		process.execPath,
		["--input-type=module", "--eval", PENDING_ROLE_INTENT_WORKER_SOURCE],
		{
			env: {
				...process.env,
	...
	if (!child.stdout || !child.stderr) {
		throw new Error(
			"Pending role intent test worker did not expose output streams",
		);
	}
	...
	const resultPromise = new Promise<PendingRoleIntentWorkerResult>(
		(resolve, reject) => {
	...
	child.once("close", (code) => {
		if (!ready) {
			rejectReady(
				new Error(
					`Pending role intent test worker exited before ready: ${stderr || `code ${code}`}`,
				),
			);
		}
		if (code !== 0) {
			rejectResult(
				new Error(
					`Pending role intent test worker failed: ${stderr || `code ${code}`}`,
				),
			);
			return;
		}
		try {
			const resultJson = stdout.slice(
				stdout.indexOf("ready\n") + "ready\n".length,
			);
...
async function runConcurrentPendingRoleIntentWorkers(
	cwd: string,
	inputs: [PendingRoleIntentWorkerInput, PendingRoleIntentWorkerInput],
): Promise<[PendingRoleIntentWorkerResult, PendingRoleIntentWorkerResult]> {
	const startSignal = join(cwd, "pending-role-intent-workers-start");
	const workers = inputs.map((input) =>
		spawnPendingRoleIntentWorker(startSignal, input),
	);
	await Promise.all(workers.map((worker) => worker.ready));
	await writeFile(startSignal, "start\n");
	return Promise.all(workers.map((worker) => worker.result)) as Promise<
		[PendingRoleIntentWorkerResult, PendingRoleIntentWorkerResult]
	>;
}

function isSuccessfulRoleIntentRecord(
	result: PendingRoleIntentWorkerResult,
): result is { ok: true; reason?: string } {
	return result !== null && "ok" in result && result.ok === true;
}

function isSingleFlightConflict(
	result: PendingRoleIntentWorkerResult,
): result is { ok: false; reason: "single_flight_conflict" } {
	return (
		result !== null &&
		"ok" in result &&
		result.ok === false &&
		result.reason === "single_flight_conflict"
	);
}

function isConsumedRoleIntent(
	result: PendingRoleIntentWorkerResult,
): result is { role: string; provenanceKind: string } {
...
describe("subagents/tracker", () => {
	...
	it("rejects parser-invalid tokens before persistence and round-trips valid tokens", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-subagent-tracker-token-grammar-"),
		);
		try {
			assert.deepEqual(
			...
			assert.equal(
				(await readSubagentTrackingState(cwd)).pending_role_intents[0]
					?.correlation_token,
				canonicalCorrelationToken("abc123"),
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("preserves expired foreign-origin journals through successful pending-role lifecycle writes", async () => {
		const sharedRoot = await mkdtemp(
			join(tmpdir(), "omx-tracker-shared-writeback-"),
		);
		...
		try {
			...
			if (!foreignResult.ok)
				throw new Error("Expected the foreign role intent to record.");
			const foreignIntent = foreignResult.intent;
			const assertForeignRetained = async () => {
				const retained = (
					await readSubagentTrackingState(cwdB)
				).pending_role_intents.find(
					(intent) =>
						intent.correlation_token === foreignIntent.correlation_token,
				);
			...
			const binding = bindPendingRoleIntentUnderLock(
				cwdB,
				{
					sessionId: "session-bound",
					parentThreadId: "parent-bound",
			...
			assert.equal(
				completeAdaptedRoleBinding(cwdB, {
					sessionId: "session-bound",
					parentThreadId: "parent-bound",
					correlationToken: canonicalCorrelationToken("tokenbound"),
	...
	it("binds a pending role intent only when its correlation token matches", async () => {
		...
		let bindCount = 0;
		const bind = (
			state: ReturnType<typeof createSubagentTrackingState>,
		...
		try {
			...
			assert.equal(
				bindPendingRoleIntentUnderLock(
					...
					{
						...input,
						correlationToken: canonicalCorrelationToken("wrongtoken"),
					},
					bind,
				),
				null,
			...
			assert.equal(
				(await readSubagentTrackingState(cwd)).pending_role_intents[0]
					?.correlation_token,
				canonicalCorrelationToken("expectedtoken"),
			);
			const firstBinding = bindPendingRoleIntentUnderLock(
				cwd,
				{
					...input,
					correlationToken: canonicalCorrelationToken("expectedtoken"),
			...
			const retainedIntent = (await readSubagentTrackingState(cwd))
				.pending_role_intents[0];
			assert.equal(retainedIntent?.binding_state, "bound");
			assert.equal(
				retainedIntent?.binding_claimant_token,
				firstBinding?.claimantToken,
			);
			const replay = bindPendingRoleIntentUnderLock(
				cwd,
				{
					...input,
					correlationToken: canonicalCorrelationToken("expectedtoken"),
			...
			assert.equal(
				completeAdaptedRoleBinding(cwd, {
					...input,
					correlationToken: canonicalCorrelationToken("expectedtoken"),
					claimantToken: replay?.claimantToken,
				}),
				"claimant_mismatch",
			);
			assert.equal(
	...
	it("stamps an owned already-bound cwd-default legacy journal on bind replay without disclosing its claimant", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-subagent-tracker-bound-legacy-"),
		);
		const previousOmxRoot = process.env.OMX_ROOT;
		const previousTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		const previousStateRoot = process.env.OMX_STATE_ROOT;
	...
	it("does not allow a shared-root workspace to claim another originless legacy journal", async () => {
		const sharedRoot = await mkdtemp(
			join(tmpdir(), "omx-subagent-tracker-shared-legacy-"),
		);
		const cwdA = join(sharedRoot, "workspace-a");
		const cwdB = join(sharedRoot, "workspace-b");
		const previousOmxRoot = process.env.OMX_ROOT;
		...
		try {
			...
			assert.deepEqual(
				(await readSubagentTrackingState(cwdA)).pending_role_intents,
				[legacyIntent],
			);
		} finally {
			if (previousOmxRoot === undefined) delete process.env.OMX_ROOT;
	...
	it("selects exact-origin duplicate journals deterministically and consumes every own duplicate", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-subagent-tracker-duplicate-origin-"),
		);
		const previousOmxRoot = process.env.OMX_ROOT;
		const previousTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		const previousStateRoot = process.env.OMX_STATE_ROOT;
		...
		try {
			...
			for (const duplicateOrder of [
				...
				assert.deepEqual(
					(await readSubagentTrackingState(cwd)).pending_role_intents,
					[],
				);
			}
			...
			for (const duplicateOrder of [
				...
				assert.ok(binding?.claimantToken);
				assert.deepEqual(
					(await readSubagentTrackingState(cwd)).pending_role_intents.map(
						(intent) => intent.role,
					),
					["critic"],
				);
			...
			for (const duplicateOrder of [
				[legacyBound, exactBound],
				[exactBound, legacyBound],
			]) {
				await writeJournals(duplicateOrder);
				assert.equal(
					completeAdaptedRoleBinding(cwd, {
			...
			for (const { name, bound, pending } of mixedStates) {
				for (const invocationOrder of [
					["bind", "consume"],
					["consume", "bind"],
				] as const) {
					for (const journalOrder of [
						[bound, pending],
						[pending, bound],
					]) {
	...
	it("completes retained adapted bindings only for the claimant that began them", async () => {
		...
		try {
			...
			assert.equal(
				completeAdaptedRoleBinding(cwd, {
					...input,
					claimantToken: canonicalClaimantToken("wrong-claimant"),
				}),
				"claimant_mismatch",
			);
			assert.equal(
	...
	it("requires a claimant-less bound journal to receive its durable correlation token before completion", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-subagent-tracker-claimant-less-"),
		);
		const nowMs = Date.now();
		const intent = {
			role: "architect",
	...
	it("serializes lifecycle tracking writes with pending role intent records and consumes", async () => {
		...
		try {
			...
			assert.equal(
				recorded.sessions[sessionId]?.threads["thread-lifecycle-before-consume"]
					?.kind,
				"leader",
			);
			...
			assert.equal(
				consumed.sessions[sessionId]?.threads["thread-lifecycle-before-consume"]
					?.kind,
				"leader",
			);
			assert.equal(
				consumed.sessions[sessionId]?.threads["thread-lifecycle-after-consume"]
					?.kind,
				"subagent",
			);
		} finally {
	...
	it("retains invalid durable credentials and rejects whitespace callers without fallthrough", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-subagent-tracker-credentials-"),
		);
		const nowMs = Date.now();
		const scope = {
			session_id: "credential-session",
			parent_thread_id: "credential-parent",
		};
		const base = {
			role: "architect",
			...scope,
		...
		try {
					...
					pending_role_intents: [
						{
							...base,
							correlation_token: "",
							binding_claimant_token: canonicalClaimantToken("claimant"),
						},
						{
							...base,
							correlation_token: canonicalCorrelationToken("validtoken"),
							binding_claimant_token: canonicalClaimantToken("claimant"),
						},
					],
				})}\n`,
			...
			assert.equal(
				retained[0]?.binding_claimant_token,
				canonicalClaimantToken("claimant"),
			);
			assert.equal(
				completeAdaptedRoleBinding(cwd, {
					sessionId: scope.session_id,
					parentThreadId: scope.parent_thread_id,
					correlationToken: canonicalCorrelationToken("validtoken"),
					claimantToken: canonicalClaimantToken("claimant"),
					nowMs,
				}),
				"claimant_mismatch",
			);
			...
			assert.equal(
				completeAdaptedRoleBinding(cwd, {
					sessionId: scope.session_id,
					parentThreadId: scope.parent_thread_id,
					correlationToken: " validtoken ",
					claimantToken: " claimant ",
					nowMs,
				}),
				"claimant_mismatch",
			);
	...
	it("rejects noncanonical caller correlation credentials without changing a retained intent", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-subagent-tracker-caller-credential-"),
		);
		const correlationToken = canonicalCorrelationToken("caller-correlation");
		const scope = {
			sessionId: "caller-session",
			parentThreadId: "caller-parent",
			correlationToken,
			nowMs: Date.now(),
		};
		try {
			assert.equal(
				recordPendingRoleIntent(cwd, { role: "architect", ...scope }).ok,
				true,
			);
			for (const invalid of ["", " ", null, 1, true, [], {}]) {
				assert.equal(
					bindPendingRoleIntentUnderLock(
						cwd,
						{ ...scope, correlationToken: invalid as string },
						(state) => state,
					),
					null,
				);
				assert.equal(
					consumePendingRoleIntent(cwd, {
						...scope,
						correlationToken: invalid as string,
					}),
					null,
				);
				assert.equal(
	...
	it("rejects every malformed completion credential without rewriting durable bound journals", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-subagent-tracker-completion-credentials-"),
		);
		const nowMs = Date.now();
		const correlationToken = canonicalCorrelationToken(
			"completion-correlation",
		);
		const claimantToken = canonicalClaimantToken("completion-claimant");
		const scope = {
			sessionId: "completion-session",
			parentThreadId: "completion-parent",
			nowMs,
		};
		const base = {
			role: "architect",
		...
		const assertRetainedMismatch = async (
			...
			assert.equal(
				completeAdaptedRoleBinding(cwd, input),
				"claimant_mismatch",
				label,
			);
			assert.equal(
				readFileSync(subagentTrackingPath(cwd), "utf-8"),
				raw,
				label,
			);
		};
		try {
			...
			const invalidCorrelations: Array<[string, unknown]> = [
				...
				["empty", ""],
				["ascii-whitespace", " "],
				["tab-cr-lf-wrapped", `\t${correlationToken}\r\n`],
				["uppercase", correlationToken.toUpperCase()],
				["wrong-length", correlationToken.slice(1)],
				["non-hex", "g".repeat(32)],
				["wrong-canonical", canonicalCorrelationToken("other-correlation")],
				["null", null],
				["number", 1],
				["boolean", true],
				["array", []],
			...
			for (const [name, value] of invalidCorrelations) {
				const input = {
					...scope,
					correlationToken: value as unknown as string,
					claimantToken,
				};
				await assertRetainedMismatch(
					`claimant-less correlation ${name}`,
					claimantLess,
					input,
				);
				await assertRetainedMismatch(
					`claimed correlation ${name}`,
					claimed,
					input,
				);
			}
			const invalidClaimants: Array<[string, unknown]> = [
				...
				["empty", ""],
				["ascii-whitespace", " "],
				["tab-cr-lf-wrapped", `\t${claimantToken}\r\n`],
				["uppercase", claimantToken.toUpperCase()],
				["wrong-length", claimantToken.slice(1)],
				["non-hex", "x".repeat(36)],
				["wrong-canonical", canonicalClaimantToken("other-claimant")],
				["null", null],
				["number", 1],
				["boolean", true],
				["array", []],
			...
			for (const [name, value] of invalidClaimants) {
				await assertRetainedMismatch(`claimed claimant ${name}`, claimed, {
					...scope,
					correlationToken,
					claimantToken: value as unknown as string,
				});
			}
			await writeJournal(claimantLess);
			assert.equal(
				completeAdaptedRoleBinding(cwd, { ...scope, correlationToken }),
				"completed",
			);
			assert.deepEqual(
				(await readSubagentTrackingState(cwd)).pending_role_intents,
			...
			for (const persistedCorrelation of ["", " ", null, 1, true, [], {}]) {
				await assertRetainedMismatch(
					"claimed persisted correlation",
					{ ...claimed, correlation_token: persistedCorrelation },
					{ ...scope, correlationToken, claimantToken },
				);
				await assertRetainedMismatch(
					"claimant-less persisted correlation",
					{ ...claimantLess, correlation_token: persistedCorrelation },
					{ ...scope, correlationToken },
				);
			}
			for (const persistedClaimant of ["", " ", null, 1, true, [], {}]) {
				await assertRetainedMismatch(
					"claimed persisted claimant",
					{ ...claimed, binding_claimant_token: persistedClaimant },
					{ ...scope, correlationToken, claimantToken },
				);
			}
		} finally {
	...
	it("rejects NUL and ELOOP origins before creating tracking artifacts", async () => {
		const parent = await mkdtemp(
			join(tmpdir(), "omx-subagent-tracker-origin-"),
		);
		const stateRoot = await mkdtemp(
			join(tmpdir(), "omx-subagent-tracker-origin-state-"),
		);
		...
		try {
			...
			for (const cwd of [`${parent}\u0000suffix`, loop]) {
				assert.deepEqual(
					recordPendingRoleIntent(cwd, {
						...
						parentThreadId: "origin-parent",
						correlationToken: canonicalCorrelationToken("origintoken"),
					}),
			...
			const recorded = recordPendingRoleIntent(validCwd, {
				role: "architect",
				sessionId: "valid-origin-session",
				parentThreadId: "valid-origin-parent",
				correlationToken: canonicalCorrelationToken("validorigintoken"),
			});
			assert.equal(recorded.ok, true);
			assert.equal(
				recorded.ok && recorded.intent.origin_cwd,
				canonicalizeOriginCwd(validCwd),
			);
			const state = await readSubagentTrackingState(validCwd);
			assert.deepEqual(
				state.pending_role_intents,
				recorded.ok ? [recorded.intent] : [],
			);
