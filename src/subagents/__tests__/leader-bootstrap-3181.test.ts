...
describe("#3194 ordinary tracker behavior", () => {
	it("excludes a child thread across all sessions without promoting legacy thread identity to leader authority", async () => {
		await withCwd(async (cwd) => {
			await recordSubagentTurnForSession(cwd, {
				...
				kind: "leader",
				leaderThreadId: "synthetic-thread",
			...
			await recordSubagentTurnForSession(cwd, {
				...
				threadId: "synthetic-thread",
				kind: "subagent",
				leaderThreadId: "synthetic-other-leader",
				timestamp: new Date(0).toISOString(),
			});
			const state = await readSubagentTrackingState(cwd);
			assert.equal(
				isTrustedSubagentThread(
					state.sessions["synthetic-session-a"],
					"synthetic-thread",
				),
				false,
			);
			assert.equal(
				isTrustedSubagentThread(
					state.sessions["synthetic-session-b"],
					"synthetic-thread",
				),
				true,
			);
			assert.equal(
				state.sessions["synthetic-session-a"]?.leader_thread_id,
				"synthetic-thread",
			);
		});
	});
			...
			await writeFile(
				...
				JSON.stringify({
					...
					pending_role_intents: [
						{ role: "architect", session_id: "native-session" },
					],
				}),
			);

			const session = (await readSubagentTrackingState(cwd)).sessions[
				"native-session"
			];
			assert.equal(session?.threads["native-child"]?.role, "architect");
			assert.equal(
				session?.threads["native-child"]?.provenance_kind,
				"native_subagent",
			);
	...
	it("preserves ordinary Architect then Critic turn ordering", () => {
		...
		const critic = recordSubagentTurn(architect, {
			sessionId: "synthetic-session",
			threadId: "synthetic-critic",
			kind: "subagent",
			role: "critic",
			timestamp: new Date(1).toISOString(),
		});
		assert.equal(
			critic.sessions["synthetic-session"]?.threads["synthetic-architect"]
				?.role,
			"architect",
		);
		assert.equal(
			critic.sessions["synthetic-session"]?.threads["synthetic-critic"]?.role,
			"critic",
		);
	});
	...
	it("strictly rejects corrupt tracker data without overwriting it", async () => {
		await withCwd(async (cwd) => {
			...
			assert.deepEqual(await readSubagentTrackingStateStrict(cwd), {
				ok: false,
			});
