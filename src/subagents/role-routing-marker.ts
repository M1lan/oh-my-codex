...
export const NATIVE_SUBAGENT_ROLE_ROUTING_MARKER_FILE =
	"native-subagent-role-routing.json";
...
function readMarkerStore(baseStateDir: string): RoleRoutingMarkerStore {
	...
	try {
		...
		) as Partial<RoleRoutingMarkerStore>;
		if (
			parsed.schema_version !== ROLE_ROUTING_MARKER_SCHEMA_VERSION ||
		...
		return {
			...
			markers: parsed.markers
				...
				.filter(
					(marker): marker is RoleRoutingUnavailableMarker => marker !== null,
				),
		};
	} catch {
		return { schema_version: ROLE_ROUTING_MARKER_SCHEMA_VERSION, markers: [] };
...
function isExpired(
	marker: RoleRoutingUnavailableMarker,
	nowMs: number,
): boolean {
...
export function writeRoleRoutingMarker(
	baseStateDir: string,
	marker: RoleRoutingUnavailableMarker,
): void {
	...
	if (!normalizedMarker) {
		throw new Error(
			"Role-routing marker requires a session_id plus valid observed_at and expires_at timestamps.",
		);
	}
	const parentThreadId = normalizedMarker.parent_thread_id ?? "";

	withCrossProcessFileLockSync(
		markerStorePath(baseStateDir),
		(context) => {
			...
			const markers = store.markers.filter((candidate) => {
				...
				return (
					candidate.session_id !== normalizedMarker.session_id ||
...
export function readRoleRoutingMarker(
	...
	const nowMs =
		typeof scope.nowMs === "number" && Number.isFinite(scope.nowMs)
			? scope.nowMs
			: Date.now();

	return (
		readMarkerStore(baseStateDir)
			.markers.filter(
				(marker) =>
