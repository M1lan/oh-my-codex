...
export function parseTmuxPaneSnapshot(output: string): TmuxPaneSnapshot[] {
	return output
		...
		.map((line) => {
			...
			const currentPath = hasCurrentPathColumn
				? (payloadParts.at(-1) ?? "")
				: "";
			const startCommandParts = hasCurrentPathColumn
				? payloadParts.slice(0, -1)
				: payloadParts;
...
export function readHudPaneOwner(pane: TmuxPaneSnapshot): HudPaneOwner {
	...
	return {
		...
		leaderPaneId: parseShellEnvAssignment(
			command,
...
function hasHudPaneOwnerMetadata(pane: TmuxPaneSnapshot): boolean {
	...
	return (
		parseShellEnvAssignment(command, OMX_TMUX_HUD_OWNER_ENV) === "1" ||
...
export function hudPaneMatchesOwner(
	...
	const wantedLeaderPaneId =
		typeof owner.leaderPaneId === "string" ? owner.leaderPaneId.trim() : "";
...
function buildLegacyHudResizeHookName(
	sessionId: string,
	windowId: string,
): string {
	return [
...
export function parseHudResizeHookContext(
	...
	if (
		...
		!isTmuxPaneId(normalizedLeaderPaneId)
	)
		return null;
...
function buildHudHookUnregisterCommand(
	tmuxBin: string,
	context: HudResizeHookContext,
	tmuxEnv?: string,
): string {
	const unregisterResize = buildNestedTmuxCommand(
		tmuxBin,
		["set-hook", "-u", "-t", context.sessionId, context.hookSlot],
		tmuxEnv,
	);
	const unregisterLayout = buildNestedTmuxCommand(
		tmuxBin,
		["set-hook", "-u", "-t", context.sessionId, context.layoutHookSlot],
		tmuxEnv,
	);
...
function buildHudLayoutReconcileHookCommand(
	...
	const leaderAlive = buildNestedTmuxCommand(
		tmuxBin,
		["display-message", "-p", "-t", leaderPaneId, "#{pane_id}"],
		env.TMUX,
	);
	const unregister = buildHudHookUnregisterCommand(tmuxBin, context, env.TMUX);
	const lockName = `${context.hookName}_layout`;
	const lock = buildNestedTmuxCommand(
		tmuxBin,
		["wait-for", "-L", lockName],
		env.TMUX,
	);
	const unlock = buildNestedTmuxCommand(
		tmuxBin,
		["wait-for", "-U", lockName],
		env.TMUX,
	);
...
function unregisterLegacyHudResizeHook(
	...
	const legacyHookSlot = buildHudResizeHookSlot(
		buildLegacyHudResizeHookName(context.sessionId, context.windowId),
	);
...
export function buildHudRuntimeEnv(
	...
	const leaderPaneId =
		typeof input.leaderPaneId === "string" ? input.leaderPaneId.trim() : "";
...
export function buildHudWatchCommand(
	...
	const safePreset =
		preset === "minimal" || preset === "focused" || preset === "full"
			? ` --preset=${preset}`
			: "";
...
export function readActiveTmuxPaneId(
	...
	try {
		return parsePaneIdFromTmuxOutput(
