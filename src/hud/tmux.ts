...
export type HudRuntimeRootSource =
	| "team-env"
	| "omx-root-env"
	| "omx-state-root-env"
	| "cwd-default";
...
export function parseCanonicalTmuxPaneId(
...
function canonicalizePaneSnapshotBatch(
...
function defaultExecTmuxSync(args: string[]): string {
	try {
		...
		if (
			maybeSpawnError.status === 0 &&
			typeof maybeSpawnError.stdout === "string"
		) {
...
function parseNonNegativeInteger(value: string | undefined): number | null {
	...
	return Number.isSafeInteger(parsed) && parsed <= 4_294_967_295
		? parsed
		: null;
}
...
export function parseTmuxPaneSnapshot(
	...
	for (const line of lines) {
		...
		const commandParts = hasIncarnationFields
			? payloadParts.slice(0, -2)
			: payloadParts;
		const hasCurrentPathColumn = commandParts.length >= 2;
		const currentPath = hasCurrentPathColumn ? (commandParts.at(-1) ?? "") : "";
		const startCommandParts = hasCurrentPathColumn
		...
		panes.push({
			...
			...(hasIncarnationFields
				? {
						paneDead: incarnationParts[0] === "1",
						panePid: incarnationParts[1]!,
					}
				: {}),
			...(hasGeometry
				? {
...
function parsePosixHudEnvAssignments(command: string): PosixHudEnvAssignments {
	...
	const inspect = (tokens: string[]): boolean => {
		...
		for (let index = 0; index < tokens.length; index += 1) {
			...
			if (
				assignment &&
				isHudOwnerEnvKey(assignment[1]!) &&
				(!commandStarted || acceptsEnvAssignments)
			) {
...
function parseShellEnvAssignment(
	command: string,
	key: string,
): string | undefined {
...
function containsPowerShellHudOwnerReferenceOutsideQuotes(
	command: string,
): boolean {
	...
	for (let index = 0; index < command.length; index += 1) {
		...
		if (command.slice(index, index + 5).toLowerCase() === "$env:") {
			const match = /^\$env:([A-Za-z_][A-Za-z0-9_]*)/i.exec(
				command.slice(index),
			);
...
function unwrapKnownPowerShellCommand(
	command: string,
): { command: string; remainder: string } | null {
	const match =
		/^\s*powershell(?:\.exe)?\s+-NoLogo\s+-NoExit\s+-Command\s+'((?:''|[^'])*)'(.*)$/i.exec(
			command,
		);
...
function parsePowerShellHudEnvPrefix(command: string): PowerShellHudEnvPrefix {
	...
	const values = new Map<string, string[]>();
	if (!present)
		return { present: false, valid: false, values, remainder: command };

	const prefixMatch =
		/^\s*((?:\$env:[A-Za-z_][A-Za-z0-9_]*\s*=\s*'(?:''|[^'])*'\s*;\s*)+)&(?:\s|$)/i.exec(
			source,
		);
	if (!prefixMatch)
		return { present: true, valid: false, values, remainder: command };
...
function hasHudOwnerMetadataAttempt(command: string): boolean {
	return (
		parsePowerShellHudEnvPrefix(command).present ||
		parsePosixHudEnvAssignments(command).attempted
	);
}
...
function parseHudEnvAssignment(
	command: string,
	key: string,
): string | undefined {
	...
	if (powerShellPrefix.present)
		return powerShellPrefix.valid
			? powerShellPrefix.values.get(key.toUpperCase())?.length === 1
				? powerShellPrefix.values.get(key.toUpperCase())![0]
				: undefined
			: undefined;
...
function hasAmbiguousHudOwnerMetadata(command: string): boolean {
	...
	if (powerShellPrefix.present) {
		if (
			...
			posix.values.size > 0 ||
			containsPowerShellHudOwnerReferenceOutsideQuotes(
				powerShellPrefix.remainder,
			) ||
		...
		return HUD_OWNER_ENV_KEYS.some((key) => {
			...
			return (
				values.length > 1 ||
				(values.length === 1 && isInvalidHudOwnerValue(key, values[0]!))
			);
		});
	}
...
export function readHudPaneOwner(pane: TmuxPaneSnapshot): HudPaneOwner {
	...
	const rawLeaderPaneId = parseHudEnvAssignment(
		command,
		OMX_TMUX_HUD_LEADER_PANE_ENV,
	);
	if (
		hasAmbiguousHudOwnerMetadata(command) ||
		(rawLeaderPaneId !== undefined &&
			!parseCanonicalTmuxPaneId(rawLeaderPaneId))
	) {
		return { sessionId: undefined, leaderPaneId: undefined };
	}
	return {
		sessionId: parseHudEnvAssignment(command, "OMX_SESSION_ID"),
		leaderPaneId: rawLeaderPaneId
			? (parseCanonicalTmuxPaneId(rawLeaderPaneId) ?? undefined)
			: undefined,
	};
}
...
function hasPowerShellEnvironmentAssignmentPrefix(command: string): boolean {
	return /^\s*(?:\$env:[A-Za-z_][A-Za-z0-9_]*\s*=\s*(?:'(?:''|[^'])*'|"(?:`.|[^"])*")\s*;\s*)+&(?:\s|$)/i.test(
		command,
	);
}
...
export function findLegacyFocusedHudWatchPaneIds(
	...
	const canonicalPanes = canonicalizePaneSnapshotBatch(panes);
	const canonicalCurrentPaneId = currentPaneId
		? parseCanonicalTmuxPaneId(currentPaneId)
		: null;
...
export function hudPaneMatchesOwner(
	...
	if (!parseCanonicalTmuxPaneId(pane.paneId) || !isHudWatchPane(pane))
		return false;
	const wantedSessionIds = [
		...
		.filter(
			(sessionId, index, sessionIds) =>
				sessionId !== "" && sessionIds.indexOf(sessionId) === index,
		);
	const rawWantedLeaderPaneId =
		typeof owner.leaderPaneId === "string" ? owner.leaderPaneId : "";
	const wantedLeaderPaneId = rawWantedLeaderPaneId
		? parseCanonicalTmuxPaneId(rawWantedLeaderPaneId)
		: null;
	...
	if (
		hasAmbiguousHudOwnerMetadata(`${pane.startCommand} ${pane.currentCommand}`)
	)
		return false;

	const paneOwner = readHudPaneOwner(pane);
	const sessionMatches =
		wantsSession && wantedSessionIds.includes(paneOwner.sessionId ?? "");
	const leaderPaneMatches =
		wantsLeaderPane && paneOwner.leaderPaneId === wantedLeaderPaneId;
	const hasLeaderTag =
		paneOwner.leaderPaneId !== undefined && paneOwner.leaderPaneId !== "";
...
function isDoctorSmokeSessionId(sessionId: string | undefined): boolean {
	return /^(?:doctor-smoke|omx-doctor-[a-z0-9-]+-smoke)$/i.test(
		sessionId ?? "",
	);
}

function shouldReapDeletedCwdHudPane(
	pane: TmuxPaneSnapshot,
...
export function reapDeadHudPanes(
	...
	const canonicalPanes = canonicalizePaneSnapshotBatch(panes);
	if (!canonicalPanes)
		return { reaped: [], preserved: panes.map((pane) => pane.paneId) };
	const livePaneIds = new Set(canonicalPanes.map((pane) => pane.paneId));
	const isLivePane =
		opts.isLivePane ?? ((paneId: string) => livePaneIds.has(paneId));
	...
	for (const pane of canonicalPanes) {
		...
		if (
			shouldReapDeletedCwdHudPane(pane, isLivePane) &&
...
export function buildHudResizeHookName(
	sessionId: string,
	windowId: string,
	leaderPaneId: string,
): string {
...
function hudHookIdentityOption(hookSlot: string): string {
	const match = /^(client-resized|window-layout-changed)\[([0-9]+)\]$/.exec(
		hookSlot,
	);
...
function buildHudHookRegistrationSuffix(
	...
	return [
		...
		"-t",
		context.sessionId,
		hudHookIdentityOption(hookSlot),
		hudHookIdentityToken(context.hookName, hookSlot),
	];
}
...
function readHudResizeHookPaneIncarnations(
	...
	try {
		const lines = parseExactTmuxAuthorityLines(
			execTmuxSync([
				"list-panes",
				"-a",
				"-F",
		...
		for (const line of lines) {
			...
			if (!match || !paneId || paneId !== match[1] || seen.has(paneId))
				return null;
			seen.add(paneId);
...
export function parseHudResizeHookContext(
	...
	incarnations: { leaderPanePid: string; hudPanePid: string } = {
		leaderPanePid: "1",
		hudPanePid: "1",
	},
	...
	if (
		...
		return null;
	const hookName = buildHudResizeHookName(
		sessionId,
		windowId,
		normalizedLeaderPaneId,
	);
...
export function readHudResizeHookContext(
	...
	const incarnations = readHudResizeHookPaneIncarnations(
		canonicalHudPaneId,
...
function buildNestedTmuxCommand(
	tmuxBin: string,
	args: string[],
	tmuxEnv?: string,
): string {
	const command = [tmuxBin, ...args]
		.map((part) => shellEscapeSingle(part))
		.join(" ");
...
function buildHudHookIncarnationCondition(
	paneId: string,
	panePid: string,
): string {
...
function buildAtomicHudHookCommand(
	...
	const hudConditional = [
		...
		"-t",
		context.hudPaneId,
		quoteHudHookShellArgument(
			buildHudHookIncarnationCondition(context.hudPaneId, context.hudPanePid),
		),
	...
	const args = [
		...
		"-F",
		"-t",
		context.leaderPaneId,
		buildHudHookIncarnationCondition(
			context.leaderPaneId,
			context.leaderPanePid,
		),
		hudConditional,
		unregister,
	];
...
function buildAtomicHudResizeCommand(
	...
	return buildAtomicHudHookCommand(
		tmuxBin,
		context,
		`resize-pane -t ${hudPaneId} -y ${height}`,
		tmuxEnv,
	);
}
...
function buildHudResizeHookCommand(
	...
	const atomicResize = buildAtomicHudResizeCommand(
		tmuxBin,
		hudPaneId,
		height,
		context,
...
function buildEnvPrefix(env: Record<string, string | undefined>): string {
	const assignments = Object.entries(env)
		.map(
			([key, value]) => [key, typeof value === "string" ? value : ""] as const,
		)
...
export function writeHudWatchCommand(options: HudCommandWriterOptions): string {
	...
	if (platform === "win32") {
		...
		const invocation = [
			"&",
			quotePowerShell(options.nodeCommand),
			quotePowerShell(options.omxEntry),
			"hud",
			"--watch",
	...
	const assignments = Object.entries(options.runtimeEnv)
		.map(
			([key, value]) =>
				`${key}=${key === OMX_TMUX_HUD_OWNER_ENV && !options.quoteOwnerValue ? value : shellEscapeSingle(value)}`,
		)
		.join(" ");
	const nodeCommand =
		options.nodeCommand === "node"
			? "node"
			: shellEscapeSingle(options.nodeCommand);
	return `exec ${assignments ? `env ${assignments} ` : ""}${nodeCommand} ${shellEscapeSingle(options.omxEntry)} hud --watch${presetArgument}`;
}
export function buildHudRuntimeEnv(
	input: HudRuntimeEnvInput = {},
): HudRuntimeEnvOutput {
	const sessionId =
		typeof input.sessionId === "string" ? input.sessionId.trim() : "";
	const rawLeaderPaneId =
		typeof input.leaderPaneId === "string" ? input.leaderPaneId : "";
	const leaderPaneId = rawLeaderPaneId
		? parseCanonicalTmuxPaneId(rawLeaderPaneId)
		: null;
	...
	if (input.rootSource === "team-env" && input.omxTeamStateRoot?.trim()) {
		...
		input.rootSource === "omx-state-root-env" &&
		input.omxStateRoot?.trim()
	) {
...
export function buildHudWatchCommand(
	...
	rootEnv?: Pick<
		HudRuntimeEnvInput,
		"omxStateRoot" | "omxTeamStateRoot" | "rootSource"
	>,
...
export function listCurrentWindowPanes(
	...
	const targetArgs = canonicalCurrentPaneId
		? ["-t", canonicalCurrentPaneId]
		: [];
	try {
		const paneIds = parseCanonicalPaneIdSnapshot(
			execTmuxSync(["list-panes", ...targetArgs, "-F", "#{pane_id}"]),
		);
		if (!paneIds) return [];
...
export function readActiveTmuxPaneId(
	...
	try {
		const paneId = parseExactTmuxAuthorityScalar(
			execTmuxSync(["display-message", "-p", "#{pane_id}"]),
		);
...
export function listCurrentWindowHudPaneIds(
	...
	return findHudWatchPaneIds(
		listCurrentWindowPanes(execTmuxSync, currentPaneId),
		currentPaneId,
		owner,
	);
}
...
export function readCurrentWindowSize(
	...
	const canonicalCurrentPaneId = currentPaneId
		? parseCanonicalTmuxPaneId(currentPaneId)
		: null;
	if (currentPaneId && !canonicalCurrentPaneId)
...
function readHudSplitSourceAuthority(
	...
	try {
		const source = parseExactTmuxAuthorityScalar(
			execTmuxSync([
				"display-message",
				"-p",
...
function readCanonicalPaneIdSnapshot(
	execTmuxSync: TmuxExecSync,
	targetPaneId?: string,
	global = false,
): Set<string> | null {
	try {
		return parseCanonicalPaneIdSnapshot(
			execTmuxSync([
				"list-panes",
...
function deriveSingleNewPane(
	before: ReadonlySet<string>,
	after: ReadonlySet<string>,
): string | null {
	if (
		after.size !== before.size + 1 ||
		![...before].every((paneId) => after.has(paneId))
	)
		return null;
...
function writeHudSplitOperationMarkedCommand(
	command: string,
	marker: string,
): string {
	if (process.platform === "win32")
...
function hasHudSplitOperationMarker(
	startCommand: string,
	marker: string,
): boolean {
...
function findHudSplitOperationMarkerPaneId(
	marker: string,
	execTmuxSync: TmuxExecSync,
): string | null {
	try {
...
function recoverHudSplitPaneId(
	...
	for (let attempt = 0; attempt < 3; attempt += 1) {
		const globalAfter = readCanonicalPaneIdSnapshot(
			execTmuxSync,
			undefined,
			true,
		);
		const targetAfter = targetPaneId
			? readCanonicalPaneIdSnapshot(execTmuxSync, targetPaneId)
			: null;
		const globalCandidate = globalAfter
			? deriveSingleNewPane(globalBefore, globalAfter)
			: null;
		const targetCandidate = targetPaneId
			? targetAfter
				? deriveSingleNewPane(targetBefore!, targetAfter)
				: null
			: globalCandidate;
		const markerCandidate = findHudSplitOperationMarkerPaneId(
			marker,
			execTmuxSync,
		);
		if (
			globalCandidate &&
			globalCandidate === targetCandidate &&
			markerCandidate === globalCandidate
		)
			return globalCandidate;
		if (markerCandidate && !globalBefore.has(markerCandidate))
...
function readTmuxOptionExactly(
	execTmuxSync: TmuxExecSync,
	option: string,
): string | null {
	try {
		return parseExactTmuxAuthorityScalar(
			execTmuxSync(["show-options", "-g", "-v", option]),
		);
	} catch {
		return null;
	}
...
function hasHudSplitAuthority(
	paneId: string,
	execTmuxSync: TmuxExecSync = defaultExecTmuxSync,
): boolean {
	const authority = hudSplitAuthorities.get(paneId);
	if (!authority) return false;
	const globalPanes = readCanonicalPaneIdSnapshot(
		execTmuxSync,
		undefined,
		true,
	);
	...
	if (authority.targetPaneId) {
		const targetPanes = readCanonicalPaneIdSnapshot(
			execTmuxSync,
			authority.targetPaneId,
		);
	...
	return (
		findHudSplitOperationMarkerPaneId(
			authority.operationMarker,
			execTmuxSync,
		) === paneId &&
		readHudPaneIncarnation(paneId, execTmuxSync)?.panePid ===
			authority.panePid &&
		location?.sessionId === authority.sessionId &&
		location?.windowId === authority.windowId &&
		readTmuxOptionExactly(execTmuxSync, authority.proofOption) ===
			authority.proofValue
	);
}

function readHudPaneIncarnation(
	paneId: string,
	execTmuxSync: TmuxExecSync,
): { paneDead: boolean; panePid: string } | null {
	try {
		const lines = parseExactTmuxAuthorityLines(
			execTmuxSync([
				"list-panes",
		...
		for (const line of lines) {
			...
			if (
				!match ||
				!observedPaneId ||
				observedPaneId !== match[1] ||
				seenPaneIds.has(observedPaneId)
			)
				return null;
			seenPaneIds.add(observedPaneId);
			if (match[2] === "1") continue;
			if (!/^[1-9][0-9]*$/.test(match[3])) return null;
			if (observedPaneId === paneId)
				incarnation = { paneDead: false, panePid: match[3]! };
		}
		return incarnation;
	} catch {
		return null;
...
function isPaneLiveInStrictGlobalProbe(
	paneId: string,
	expectedPid: string | undefined,
	execTmuxSync: TmuxExecSync,
): boolean {
	...
	return Boolean(
		incarnation &&
			...
			(!expectedPid || incarnation.panePid === expectedPid),
	);
}

/** Freshly validates retained split proof and sustained global liveness before a HUD mutation sink. */
export function verifyHudWatchPaneAuthority(
	paneId: string,
	execTmuxSync: TmuxExecSync = defaultExecTmuxSync,
): boolean {
	const canonicalPaneId = parseCanonicalTmuxPaneId(paneId);
	if (!canonicalPaneId || !hasHudSplitAuthority(canonicalPaneId, execTmuxSync))
		return false;
	for (let attempt = 0; attempt < 3; attempt += 1) {
		if (attempt > 0) sleepSync(100);
		if (
			!hasHudSplitAuthority(canonicalPaneId, execTmuxSync) ||
			!isPaneLiveInStrictGlobalProbe(
				canonicalPaneId,
				hudSplitAuthorities.get(canonicalPaneId)?.panePid,
				execTmuxSync,
			)
		)
			return false;
...
export function rollbackHudWatchPaneAuthority(
	paneId: string,
	execTmuxSync: TmuxExecSync = defaultExecTmuxSync,
): boolean {
	const canonicalPaneId = parseCanonicalTmuxPaneId(paneId);
	const authority = canonicalPaneId
		? hudSplitAuthorities.get(canonicalPaneId)
		: undefined;
	if (!canonicalPaneId || !authority) return false;
	const killed = mutateHudWatchPaneIfCurrent(
		canonicalPaneId,
		authority.panePid,
		`kill-pane -t ${canonicalPaneId}`,
		execTmuxSync,
	);
...
function rollbackRecoveredHudSplitPane(
	...
	const pidCondition =
		panePid && /^[1-9][0-9]*$/.test(panePid)
			? `#{&&:#{==:#{pane_pid},${panePid}},`
			: "";
	const sessionCondition =
		sessionId && /^\$[0-9]+$/.test(sessionId)
			? `#{&&:#{==:#{session_id},${sessionId}},`
			: "";
	const windowCondition =
		windowId && /^@[0-9]+$/.test(windowId)
			? `#{&&:#{==:#{window_id},${windowId}},`
			: "";
...
export function createHudWatchPane(
	...
	const canonicalTargetPaneId = options.targetPaneId
		? parseCanonicalTmuxPaneId(options.targetPaneId)
		: null;
	if (options.targetPaneId && !canonicalTargetPaneId) return null;
	const sourceAuthority = readHudSplitSourceAuthority(
		canonicalTargetPaneId,
		execTmuxSync,
	);
	if (!sourceAuthority) return null;
	const sourcePaneId = sourceAuthority.paneId;
	const globalBefore = readCanonicalPaneIdSnapshot(
		execTmuxSync,
		undefined,
		true,
	);
	...
	const args = [
		...
		"-c",
		shellEscapeSingle(cwd),
		writeHudSplitOperationMarkedCommand(hudCmd, operationMarker),
	];
	...
	try {
		...
		if (readTmuxOptionExactly(execTmuxSync, proofOption) !== provisionalProof)
			return null;
		...
		const incarnation = paneId
			? readHudPaneIncarnation(paneId, execTmuxSync)
			: null;
		const location = paneId
			? readHudPaneSessionAndWindow(paneId, execTmuxSync)
			: null;
		if (
			!paneId ||
			!incarnation ||
...
function readHudPaneSessionAndWindow(
	...
	try {
		const value = parseExactTmuxAuthorityScalar(
			execTmuxSync([
				"display-message",
				"-p",
				"-t",
...
export function mutateHudWatchPaneIfCurrent(
	...
	if (
		...
		!/^(?:kill-pane|resize-pane) -t %(?:0|[1-9][0-9]*)(?: -y [1-9][0-9]*)?$/.test(
			mutation,
		)
	)
		return false;
...
export function resizeTmuxPaneIfCurrent(
	...
	const height =
		Number.isFinite(heightLines) && heightLines > 0
			? Math.floor(heightLines)
			: HUD_TMUX_HEIGHT_LINES;
	return mutateTmuxPaneIfCurrent(
		canonicalPaneId,
		expectedPanePid,
		`resize-pane -t ${canonicalPaneId} -y ${height}`,
		execTmuxSync,
	);
}
...
export function registerHudResizeHook(
	...
	const options =
		typeof optionsOrExecTmuxSync === "function" ? {} : optionsOrExecTmuxSync;
	const execTmuxSync =
		typeof optionsOrExecTmuxSync === "function"
			? optionsOrExecTmuxSync
			: (maybeExecTmuxSync ?? defaultExecTmuxSync);
	const canonicalHudPaneId = parseCanonicalTmuxPaneId(hudPaneId);
	const canonicalLeaderPaneId = parseCanonicalTmuxPaneId(leaderPaneId);
	if (!canonicalHudPaneId || !canonicalLeaderPaneId) return false;
	const context = readHudResizeHookContext(
		canonicalHudPaneId,
		canonicalLeaderPaneId,
		execTmuxSync,
	);
	...
	const resizeCmd = shellEscapeSingle(
		buildHudResizeHookCommand(
			tmuxBin,
			canonicalHudPaneId,
			height,
			context,
	...
	try {
		execTmuxSync([
			...
			context.sessionId,
			context.hookSlot,
			`run-shell -b ${resizeCmd}`,
			...buildHudHookRegistrationSuffix(context, context.hookSlot),
		]);
	...
	if (omxBin) {
		try {
			...
			execTmuxSync([
				...
				`run-shell -b ${reconcileCmd}`,
				...buildHudHookRegistrationSuffix(context, context.layoutHookSlot),
			]);
		} catch {
...
export function unregisterHudResizeHook(
	...
	try {
		execTmuxSync(
			buildGuardedHudHookUnregisterArgs(context, context.layoutHookSlot),
		);
	} catch {
		ok = false;
	}
