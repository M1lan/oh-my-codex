...
import {
	...
	HUD_TMUX_ULTRAGOAL_HEIGHT_LINES,
} from "./constants.js";
...
function isAutopilotLateGateSource(
	ctx: HudRenderContext,
	stage: "code-review" | "ultraqa",
): boolean {
	return normalizeHudPhase(ctx.autopilot?.current_phase) === stage;
}

function hasAutopilotLateGateReplacement(
	ctx: HudRenderContext,
	phase: string,
): boolean {
	const normalized = normalizeHudPhase(phase);
	if (normalized === "code-review") {
		return (
			ctx.codeReview?.source === "autopilot" &&
			isAutopilotLateGateSource(ctx, "code-review")
		);
	}
	if (normalized === "ultraqa") {
		return (
			ctx.ultraqa?.source === "autopilot" &&
			isAutopilotLateGateSource(ctx, "ultraqa")
		);
	}
	return false;
}
...
function renderAutopilot(ctx: HudRenderContext): string | null {
	...
	if (
		isLateAutopilotHudPhase(phase) &&
		hasAutopilotLateGateReplacement(ctx, phase)
	)
		return null;
...
function renderStaleAutopilot(ctx: HudRenderContext): string | null {
	...
	const phase =
		sanitizeDynamicText(ctx.staleAutopilot.current_phase || "active") ||
		"active";
...
function renderRalplan(ctx: HudRenderContext): string | null {
	...
	const phase =
		sanitizeDynamicText(ctx.ralplan.current_phase || "active") || "active";
...
function renderDeepInterview(ctx: HudRenderContext): string | null {
	...
	const phase =
		sanitizeDynamicText(ctx.deepInterview.current_phase || "active") ||
		"active";
...
function renderAutoresearch(ctx: HudRenderContext): string | null {
	...
	const phase =
		sanitizeDynamicText(ctx.autoresearch.current_phase || "active") || "active";
...
function renderCodeReview(ctx: HudRenderContext): string | null {
	...
	if (
		...
		return null;
	const phase =
		sanitizeDynamicText(ctx.codeReview.current_phase || "active") || "active";
...
function renderUltraqa(ctx: HudRenderContext): string | null {
	...
	if (
		...
		return null;
	const phase =
		sanitizeDynamicText(ctx.ultraqa.current_phase || "active") || "active";
...
function truncateDynamicText(value: string, maxLength: number): string {
	...
	return normalizeTrailingEllipsis(
		`${value.slice(0, maxLength - 1).trimEnd()}…`,
	);
}

export function getHudRenderMaxLines(
	ctx: Pick<HudRenderContext, "ultragoal">,
): number {
	return ctx.ultragoal?.active
		? HUD_TMUX_ULTRAGOAL_HEIGHT_LINES
		: HUD_TMUX_HEIGHT_LINES;
}

function clampHudMaxLines(
	ctx: Pick<HudRenderContext, "ultragoal">,
	maxLines: number | undefined,
): number {
	const adaptiveMaxLines = getHudRenderMaxLines(ctx);
	if (!Number.isFinite(maxLines ?? Number.NaN) || (maxLines ?? 0) <= 0)
		return adaptiveMaxLines;
...
function renderUltragoal(ctx: HudRenderContext): string | null {
	...
	if (!Number.isFinite(total) || total <= 0 || !Number.isFinite(complete)) {
		const phase =
			sanitizeDynamicText(
				ctx.ultragoal.current_phase || ctx.ultragoal.status || "active",
			) || "active";
	...
	const progress = cyan(
		`ultragoal ${complete}/${total}${teamSummary ? ` + ${teamSummary}` : ""}`,
	);
	const activeGoal =
		ctx.ultragoal.activeGoal ?? ctx.ultragoal.ongoingGoals?.[0];
	const formatGoal = (goal: typeof activeGoal, titleLength: number): string => {
		if (!goal) return "";
		const id = goal.id ? sanitizeDynamicText(goal.id) : "";
		const title = goal.title
			? truncateDynamicText(sanitizeDynamicText(goal.title), titleLength)
			: "";
	...
	const ongoingGoals = [activeSummary].filter(Boolean);
	const rawObjective = activeGoal?.objective
		? sanitizeDynamicText(activeGoal.objective)
		: "";
	const objective =
		ongoingGoals.length === 0 && rawObjective
			? truncateDynamicText(rawObjective, 96)
			: "";
...
function ellipsizeSegment(segment: string, maxWidth: number): string {
	...
	if (maxWidth <= 4)
		return `${sliceAnsiVisible(segment, 0, Math.max(0, maxWidth - 1))}…`;
...
function wrapHudParts(
	...
	const maxWidth =
		Number.isFinite(options.maxWidth) && (options.maxWidth ?? 0) > 0
			? Math.max(12, Math.floor(options.maxWidth ?? 0))
			: Infinity;
	const maxLines =
		Number.isFinite(options.maxLines) && (options.maxLines ?? 0) > 0
	...
	for (const part of parts) {
		...
		currentLine =
			visibleLength(overflowCandidate) <= maxWidth
				? overflowCandidate
