...
describe("buildTmuxSplitArgs – shell injection hardening", () => {
	...
	it("omxBin with single quote is properly escaped in command string", () => {
		...
		assert.equal(
			cmd,
			`exec env OMX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/tmp/it'\\''s/omx.js' hud --watch`,
		);
	});
	...
	it("omxBin with $() is neutralised by single-quote wrapping", () => {
		...
		assert.equal(
			cmd,
			`exec env OMX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/tmp/$(id)/omx.js' hud --watch`,
		);
	});
	...
	it("omxBin with backticks is neutralised by single-quote wrapping", () => {
		...
		assert.equal(
			cmd,
			`exec env OMX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/tmp/\`whoami\`/omx.js' hud --watch`,
		);
	});
	...
	it("omxBin with ';command' breakout attempt is neutralised", () => {
		...
		assert.equal(
			escapeCount,
			2,
			`Expected 2 escape sequences, got ${escapeCount}`,
		);
	});
	...
	it("invalid preset is dropped (defense in depth)", () => {
		...
		assert.equal(
			cmd,
			`exec env OMX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/usr/bin/omx.js' hud --watch`,
		);
		assert.ok(!cmd.includes("--preset="));
	});
	...
	it("prepends OMX_SESSION_ID when provided", () => {
		...
		assert.equal(
			cmd,
			`exec env OMX_SESSION_ID='sess-managed' OMX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/usr/bin/omx.js' hud --watch --preset=focused`,
		);
	});
	...
	it("omits OMX_ROOT when unset while preserving existing OMX_SESSION_ID behavior", () => {
		...
		assert.equal(
			cmd,
			`exec env OMX_SESSION_ID='sess-managed' OMX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/usr/bin/omx.js' hud --watch`,
		);
