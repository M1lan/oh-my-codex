...
async function withWindowsPlatform(
...
function initGitRepo(cwd: string): void {
	...
	execFileSync("git", ["checkout", "-b", "safe-branch"], {
		cwd,
		stdio: "ignore",
	});
...
async function createWorktreePointerFixture(
	cwd: string,
...
describe("buildGitBranchLabel", () => {
	it("keeps the branch when origin lookup fails", () => {
		...
		assert.equal(
			buildGitBranchLabel("/repo", undefined, gitRunner),
			"fix/hud-regression",
		);
	});
	...
	it("falls back to the first resolvable remote when origin is absent", () => {
		const gitRunner = gitRunnerFromMap({
			...
			"git remote get-url upstream":
				"https://github.com/acme/upstream-repo.git",
		});
	...
	it("does not execute shell metacharacters from config.git.remoteName in the non-Windows fallback path", {
	...
	it("keeps the worktree root for --show-toplevel fallback on Windows worktrees", async () => {
		await withTempRepo("omx-hud-worktree-top-", async (cwd) => {
			...
			await withWindowsPlatform(() => {
				assert.equal(
					buildGitBranchLabel(cwd),
					`${basename(cwd)}/worktree-branch`,
				);
			});
		});
	});
...
describe("readUltragoalState", { concurrency: false }, () => {
	...
	it("keeps HUD active when aggregate completion exists but repo-native ultragoal work is still running", async () => {
		await withTempRepo("omx-hud-ultragoal-aggregate-active-", async (cwd) => {
			...
			await writeFile(
				...
				JSON.stringify({
					...
					aggregateCompletion: {
						...
						evidence:
							"task-scoped Codex aggregate completed before microgoal ledger reconciliation finished",
					},
					goals: [
						{
							id: "G001-done",
						...
						},
						{
							id: "G002-running",
							title: "Running",
							objective: "Finish active repo-native work",
							status: "in_progress",
						},
						{
							id: "G003-pending",
			...
			assert.deepEqual(
				state?.ongoingGoals?.map((goal) => goal.id),
				["G002-running", "G003-pending"],
			);
		});
	});
	...
	it("treats superseded pending ultragoal goals as terminal for HUD activity", async () => {
		await withTempRepo(
			"omx-hud-ultragoal-superseded-terminal-",
			async (cwd) => {
				const ultragoalDir = join(cwd, ".omx", "ultragoal");
				await mkdir(ultragoalDir, { recursive: true });
				await writeFile(
						...
						goals: [
							{
								id: "G001-done",
								title: "Done",
								objective: "Completed accepted replacement",
								status: "complete",
							},
							{
								id: "G002-old-pending",
								title: "Old pending",
								objective:
									"Superseded work that should not remain active in HUD",
								status: "pending",
								steeringStatus: "superseded",
								supersededBy: ["G001-done"],
							},
						],
					}),
				);
	...
	it("keeps superseded ultragoal goals active until replacements are declared", async () => {
		await withTempRepo(
			"omx-hud-ultragoal-superseded-without-replacements-",
			async (cwd) => {
				const ultragoalDir = join(cwd, ".omx", "ultragoal");
				await mkdir(ultragoalDir, { recursive: true });
				await writeFile(
					...
					JSON.stringify({
						...
						goals: [
							...
							},
							{
								id: "G002-old-pending",
								title: "Old pending",
								objective: "Superseded work without an auditable replacement",
								status: "pending",
								steeringStatus: "superseded",
							},
						],
	...
	it("keeps superseded ultragoal goals active until every replacement is resolved", async () => {
		await withTempRepo(
			"omx-hud-ultragoal-superseded-unresolved-replacement-",
			async (cwd) => {
				const ultragoalDir = join(cwd, ".omx", "ultragoal");
				await mkdir(ultragoalDir, { recursive: true });
				await writeFile(
					...
					JSON.stringify({
						...
						goals: [
							...
							},
							{
								id: "G002-old-pending",
								title: "Old pending",
								objective: "Superseded work with unfinished replacements",
								status: "pending",
								steeringStatus: "superseded",
								supersededBy: ["G003-real-pending"],
							},
							{
								id: "G003-real-pending",
								title: "Real pending",
								objective: "Finish replacement work",
								status: "pending",
								supersedes: ["G002-old-pending"],
							},
						],
					}),
				);
				...
				assert.deepEqual(
					state?.ongoingGoals?.map((goal) => goal.id),
					["G002-old-pending", "G003-real-pending"],
				);
			},
		);
	});
	...
	it("falls through resolved superseded activeGoalId to genuinely active pending ultragoal goals", async () => {
		await withTempRepo(
			"omx-hud-ultragoal-superseded-with-active-pending-",
			async (cwd) => {
				const ultragoalDir = join(cwd, ".omx", "ultragoal");
				await mkdir(ultragoalDir, { recursive: true });
				await writeFile(
					...
					JSON.stringify({
						...
						goals: [
							...
							},
							{
								...
								status: "pending",
								steeringStatus: "superseded",
								supersededBy: ["G003-replacement-done"],
							},
							{
								id: "G003-replacement-done",
								title: "Replacement done",
								objective: "Completed replacement work",
								status: "complete",
								supersedes: ["G002-old-pending"],
							},
							{
								id: "G004-real-pending",
								title: "Real pending",
								objective: "Still-active follow-up work",
								status: "pending",
							},
						],
					}),
	...
	it("shows active ultragoal plus the next three pending goals", async () => {
		await withTempRepo("omx-hud-ultragoal-next-pending-", async (cwd) => {
					...
					goals: [
						{
							id: "G001-done",
							title: "Done",
							objective: "Complete old work",
							status: "complete",
						},
						{
							id: "G002-previous-pending",
							title: "Previous pending",
							objective: "Do previous pending",
							status: "pending",
						},
						{
							id: "G003-running",
							title: "Running one",
							objective: "Do running one",
							status: "in_progress",
						},
						{
							id: "G004-active",
							title: "Active selected",
							objective: "Do active selected",
							status: "in_progress",
						},
						{
							id: "G005-pending",
							title: "Pending two",
							objective: "Do pending two",
							status: "pending",
						},
						{
							id: "G006-blocked",
						...
						},
						{
							id: "G007-pending",
							title: "Pending three",
							objective: "Do pending three",
							status: "pending",
						},
						{
							id: "G008-pending",
							title: "Pending four",
							objective: "Do pending four",
							status: "pending",
						},
						{
							id: "G009-pending",
							title: "Hidden pending five",
							objective: "Do pending five",
							status: "pending",
						},
					],
				}),
			...
			assert.deepEqual(
				state?.ongoingGoals?.map((goal) => goal.id),
				["G004-active", "G005-pending", "G007-pending", "G008-pending"],
			);
			assert.deepEqual(
				state?.nextGoals?.map((goal) => goal.id),
				["G005-pending", "G007-pending", "G008-pending"],
			);
		});
	});
...
describe("readRalphState scope precedence", () => {
	it("prefers session-scoped Ralph state when session.json points to a session", async () => {
		await withTempRepo("omx-hud-ralph-session-", async (cwd) => {
			...
			await writeFile(
				join(rootStateDir, "session.json"),
				JSON.stringify({ session_id: sessionId }),
			);
			await writeFile(
				join(rootStateDir, "ralph-state.json"),
				JSON.stringify({ active: true, iteration: 9, max_iterations: 10 }),
			);
			await writeFile(
				join(sessionStateDir, "ralph-state.json"),
				JSON.stringify({ active: true, iteration: 2, max_iterations: 10 }),
			);
	...
	it("does not fall back to root Ralph state when current session has no Ralph state file", async () => {
		await withTempRepo("omx-hud-ralph-fallback-", async (cwd) => {
			...
			await mkdir(join(rootStateDir, "sessions", sessionId), {
				recursive: true,
			});
			await writeFile(
				join(rootStateDir, "session.json"),
			...
			await writeFile(
				join(rootStateDir, "ralph-state.json"),
				JSON.stringify({ active: true, iteration: 4, max_iterations: 10 }),
			);
	...
	it("treats session-scoped inactive Ralph state as authoritative over active root fallback", async () => {
		await withTempRepo("omx-hud-ralph-authority-", async (cwd) => {
			...
			await writeFile(
				join(rootStateDir, "ralph-state.json"),
				JSON.stringify({ active: true, iteration: 8, max_iterations: 10 }),
			);
			await writeFile(
				join(sessionStateDir, "ralph-state.json"),
	...
	it("does not treat another session-scoped Ralph state as active for the current session", async () => {
		await withTempRepo("omx-hud-ralph-other-session-", async (cwd) => {
			...
			await mkdir(join(rootStateDir, "sessions", currentSessionId), {
				recursive: true,
			});
			await mkdir(join(rootStateDir, "sessions", otherSessionId), {
				recursive: true,
			});
			await writeFile(
				join(rootStateDir, "session.json"),
			...
			await writeFile(
				join(rootStateDir, "sessions", otherSessionId, "ralph-state.json"),
				JSON.stringify({
					active: true,
					iteration: 7,
					max_iterations: 10,
...
describe("additional HUD mode state readers", () => {
	it("reads active ralplan state", async () => {
		await withTempRepo("omx-hud-ralplan-", async (cwd) => {
			await writeModeState(cwd, "ralplan", {
				active: true,
	...
	it("prefers session-scoped ralplan state over root fallback", async () => {
		await withTempRepo("omx-hud-ralplan-session-", async (cwd) => {
			...
			await writeFile(
				join(rootStateDir, "ralplan-state.json"),
				JSON.stringify({ active: true, current_phase: "draft", iteration: 9 }),
			);
			await writeFile(
	...
	it("reads deep-interview input lock from nested state payload", async () => {
		await withTempRepo("omx-hud-interview-", async (cwd) => {
			await writeModeState(cwd, "deep-interview", {
				active: true,
	...
	it("reads active autoresearch state", async () => {
		await withTempRepo("omx-hud-autoresearch-", async (cwd) => {
			await writeModeState(cwd, "autoresearch", {
				active: true,
				current_phase: "running",
			});
			assert.deepEqual(await readAutoresearchState(cwd), {
				active: true,
				current_phase: "running",
			});
		});
	...
	it("reads active ultraqa state", async () => {
		await withTempRepo("omx-hud-ultraqa-", async (cwd) => {
			await writeModeState(cwd, "ultraqa", {
				active: true,
				current_phase: "diagnose",
			});
			assert.deepEqual(await readUltraqaState(cwd), {
				active: true,
				current_phase: "diagnose",
			});
		});
	...
	it("reads hud notify state from the current session scope", async () => {
		await withTempRepo("omx-hud-notify-session-", async (cwd) => {
			...
			await writeFile(
				join(rootStateDir, "hud-state.json"),
				JSON.stringify({ last_turn_at: "root", turn_count: 99 }),
			);
			await writeFile(
				join(sessionStateDir, "hud-state.json"),
				JSON.stringify({ last_turn_at: "session", turn_count: 2 }),
			);
	...
	it("keeps hud notify pinned to the canonical OMX session when session metadata also carries a native session id", async () => {
		await withTempRepo("omx-hud-notify-native-meta-", async (cwd) => {
			...
			await writeFile(
				...
				JSON.stringify({ last_turn_at: "canonical", turn_count: 3 }),
			);
			await writeFile(
				join(nativeDir, "hud-state.json"),
				JSON.stringify({ last_turn_at: "native", turn_count: 99 }),
			);
	...
	it("prefers OMX_SESSION_ID over stale session.json for hud notify state", async () => {
		await withTempRepo("omx-hud-notify-env-session-", async (cwd) => {
			...
			await writeFile(
				join(activeDir, "hud-state.json"),
				JSON.stringify({ last_turn_at: "active", turn_count: 5 }),
			);
			await writeFile(
				join(staleDir, "hud-state.json"),
				JSON.stringify({ last_turn_at: "stale", turn_count: 1 }),
			);
...
describe("readAllState canonical skill precedence", () => {
	...
	it("keeps session-scoped ralplan phase authoritative over stale canonical autopilot phase", async () => {
		await withTempRepo("omx-hud-ralplan-session-authority-", async (cwd) => {
			...
			assert.equal(
				stripSgr(renderHud(state, "focused")).includes("autopilot:code-review"),
				true,
			);
		});
	});
	...
	it("surfaces code-review from canonical skill-active without detail state", async () => {
		await withTempRepo("omx-hud-canonical-code-review-", async (cwd) => {
			...
			const state = await readAllState(cwd);
			assert.deepEqual(state.codeReview, {
				active: true,
	...
	it("derives late-gate HUD statuses from active Autopilot child phases", async () => {
		await withTempRepo("omx-hud-autopilot-late-gates-", async (cwd) => {
			...
			assert.deepEqual(codeReviewState.codeReview, {
				active: true,
			...
			assert.deepEqual(ultraqaState.ultraqa, {
				active: true,
				current_phase: "autopilot",
				source: "autopilot",
			});
	...
	it("suppresses stale root late-gate detail without session authority", async () => {
		await withTempRepo("omx-hud-late-gate-stale-root-", async (cwd) => {
			...
			await writeFile(
				join(rootStateDir, "code-review-state.json"),
				JSON.stringify({ active: true, current_phase: "stale-root" }),
			);
			await writeFile(
				join(rootStateDir, "ultraqa-state.json"),
				JSON.stringify({ active: true, current_phase: "stale-root" }),
			);
			await writeFile(
				join(sessionDir, "skill-active-state.json"),
	...
	it("surfaces live code-reviewer subagent evidence when canonical autopilot state is inactive", async () => {
		await withTempRepo(
			"omx-hud-inactive-autopilot-live-review-",
			async (cwd) => {
				const rootStateDir = join(cwd, ".omx", "state");
				...
				assert.equal(
					stripSgr(renderHud(state, "focused")).includes(
	...
	it("does not surface completed code-reviewer subagent history over inactive canonical state", async () => {
		await withTempRepo(
			"omx-hud-inactive-autopilot-completed-review-",
			async (cwd) => {
				const rootStateDir = join(cwd, ".omx", "state");
					...
					goals: [
						{
							id: "G001-inspect",
							title: "Inspect HUD",
							objective: "Inspect combined state",
							status: "complete",
						},
						{
							id: "G002-team-hud",
							title: "Patch team HUD",
							objective: "Fix duplicate team and ultragoal summaries",
							status: "in_progress",
						},
					],
				}),
	...
	it("does not surface root autopilot detail when a session exists but has no session canonical or detail state", async () => {
		await withTempRepo(
			"omx-hud-root-mirror-autopilot-session-missing-",
			async (cwd) => {
				const rootStateDir = join(cwd, ".omx", "state");
				...
				await writeFile(
					join(rootStateDir, "skill-active-state.json"),
					JSON.stringify({
						active: true,
						skill: "autopilot",
				...
				await writeFile(
					join(rootStateDir, "autopilot-state.json"),
					JSON.stringify({
						active: true,
						mode: "autopilot",
	...
	it("reports stale current-autopilot when authoritative HUD state is inactive", async () => {
		await withTempRepo("omx-hud-current-autopilot-stale-", async (cwd) => {
			...
			await writeFile(
				join(rootStateDir, "current-autopilot.json"),
				JSON.stringify({
					active: true,
					current_phase: "complete",
					session_id: sessionId,
	...
	it("prefers authoritative active autopilot over stale current-autopilot mirror", async () => {
		await withTempRepo(
			"omx-hud-current-autopilot-authoritative-",
			async (cwd) => {
				const rootStateDir = join(cwd, ".omx", "state");
	...
	it("binds canonical HUD state to OMX_SESSION_ID instead of stale session.json/root fallback", async () => {
		await withTempRepo("omx-hud-canonical-env-session-", async (cwd) => {
			...
			await writeFile(
				join(activeDir, "skill-active-state.json"),
				JSON.stringify({
					active: true,
					skill: "team",
					phase: "running",
			...
			await writeFile(
				join(activeDir, "team-state.json"),
				JSON.stringify({
					active: true,
	...
	it("uses OMX_TEAM_STATE_ROOT session.json for session-scoped canonical HUD state without OMX_SESSION_ID", async () => {
		await withTempRepo(
			"omx-hud-canonical-team-root-session-json-",
			async (cwd) => {
				const teamStateRoot = join(cwd, "team-state-root");
				...
				await writeFile(
					join(sourceStateDir, "session.json"),
					JSON.stringify({
	...
	it("does not let source-root session.json suppress authoritative team-root HUD fallback", async () => {
		await withTempRepo(
			"omx-hud-canonical-team-root-ignore-source-session-",
			async (cwd) => {
				const teamStateRoot = join(cwd, "team-state-root");
				await mkdir(teamStateRoot, { recursive: true });
				await writeFile(
					join(teamStateRoot, "skill-active-state.json"),
					JSON.stringify({
						active: true,
						skill: "ralplan",
						phase: "planning",
				...
				await writeFile(
					join(teamStateRoot, "ralplan-state.json"),
					JSON.stringify({
						active: true,
						current_phase: "planning",
					}),
				...
				const sourceStateDir = join(cwd, ".omx", "state");
				await mkdir(join(sourceStateDir, "sessions", "sess-source-current"), {
					recursive: true,
				});
				await writeFile(
					join(sourceStateDir, "session.json"),
