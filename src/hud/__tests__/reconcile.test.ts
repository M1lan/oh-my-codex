...
describe("reconcileHudForPromptSubmit", () => {
	...
	it("recreates a missing HUD in explicit OMX-owned tmux with a session id", async () => {
		...
		assert.match(
			created[0]?.cmd || "",
			/exec .*\/repo\/dist\/cli\/omx\.js' hud --watch/,
		);
	...
	it("reaps orphaned same-session HUD panes whose leader pane was destroyed, then recreates a single HUD", async () => {
		...
		assert.match(
			created[0]?.cmd || "",
			new RegExp(`${OMX_TMUX_HUD_LEADER_PANE_ENV}='%33'`),
		);
	});
	...
	it("reaps orphaned HUD panes tagged with an equivalent native session id", async () => {
		...
		const result = await reconcileHudForPromptSubmit(repoCwd, {
			env: {
				TMUX: "1",
				TMUX_PANE: "%1",
				OMX_SESSION_ID: "codex-native-uuid",
				[OMX_TMUX_HUD_OWNER_ENV]: "1",
			},
	...
	it("reaps a stale same-leader HUD even when a current-session HUD already exists", async () => {
		...
		assert.deepEqual(resized, [
			{ paneId: "%2", heightLines: HUD_TMUX_HEIGHT_LINES },
		]);
	});
	...
	it("collapses same-owner HUD panes that appear during the create race window", async () => {
		...
		assert.deepEqual(resized, [
			{ paneId: "%9", heightLines: HUD_TMUX_HEIGHT_LINES },
		]);
	});
	...
	it("keeps an observed same-owner HUD when the returned create pane is absent from the post-create scan", async () => {
		...
		assert.deepEqual(killed, []);
		assert.deepEqual(resized, [
			{ paneId: "%8", heightLines: HUD_TMUX_HEIGHT_LINES },
		]);
	});
		...
		const result = await reconcileHudForPromptSubmit(repoCwd, {
			...
			listCurrentWindowPanes: () => [
				{ paneId: "%1", currentCommand: "codex", startCommand: "codex" },
				{
					paneId: "%2",
					currentCommand: "node",
					startCommand: "node /tmp/bin/omx.js hud --watch --preset=focused",
				},
				{
					paneId: "%3",
					currentCommand: "node",
				...
				},
				{
					paneId: "%4",
					currentCommand: "node",
					startCommand: "node /tmp/bin/omx.js hud --watch --preset=minimal",
				},
				{
					paneId: "%5",
					currentCommand: "node",
					startCommand: "node /tmp/bin/omx.js hud --tmux --preset=focused",
				},
			],
			killTmuxPane: (paneId) => {
				killed.push(paneId);
	...
	it("does not resize, kill, or reuse another active leader session HUD in the same tmux window", async () => {
		...
		assert.match(
			created[0]?.cmd || "",
			new RegExp(`${OMX_TMUX_HUD_LEADER_PANE_ENV}='%3'`),
		);
	});
	...
	it("deduplicates same-leader node HUD panes while preserving active ultragoal height despite empty mode state", async () => {
		...
		const result = await reconcileHudForPromptSubmit(repoCwd, {
			...
			listCurrentWindowPanes: () => [
				{ paneId: "%1", currentCommand: "zsh", startCommand: "zsh" },
				{
					paneId: "%2",
					currentCommand: "node",
					startCommand: `exec env OMX_SESSION_ID='sess-a' ${OMX_TMUX_HUD_LEADER_PANE_ENV}='%1' /node /omx.js hud --watch`,
				},
				{
					paneId: "%3",
					currentCommand: "node",
		...
		assert.deepEqual(killed, ["%3"]);
		assert.deepEqual(resized, [
			{ paneId: "%2", heightLines: HUD_TMUX_ULTRAGOAL_HEIGHT_LINES },
		]);
		assert.deepEqual(created, []);
	});
		...
		const result = await reconcileHudForPromptSubmit(repoCwd, {
			...
			listCurrentWindowPanes: () => [
				{ paneId: "%1", currentCommand: "codex", startCommand: "codex" },
				{
					paneId: "%2",
					currentCommand: "node",
					startCommand: `env OMX_TMUX_HUD_OWNER='1' ${OMX_TMUX_HUD_LEADER_PANE_ENV}='%1' node omx hud --watch`,
				},
				{
					paneId: "%3",
					currentCommand: "node",
			...
			listCurrentWindowPanes: () => [
				{
					...
					paneTop: 0,
					paneWidth: 160,
					paneHeight: 50 - HUD_TMUX_HEIGHT_LINES,
					paneBottom: 49 - HUD_TMUX_HEIGHT_LINES,
					windowWidth: 160,
					windowHeight: 50,
				},
				{
	...
	it("keeps a geometrically healthy duplicate HUD pane instead of preserving a malformed first duplicate", async () => {
		...
		assert.deepEqual(killed, ["%2"]);
		assert.deepEqual(resized, [
			{ paneId: "%3", heightLines: HUD_TMUX_HEIGHT_LINES },
		]);
		assert.deepEqual(created, []);
	});
	...
	it("keeps the resize hook on the reused duplicate keeper pane", async () => {
		...
		await reconcileHudForPromptSubmit(repoCwd, {
			...
			registerHudResizeHook: (hudPaneId, leaderPaneId) => {
				registered.push({ hudPaneId, leaderPaneId });
				return true;
			},
			resolveOmxCliEntryPath: () => "/repo/dist/cli/omx.js",
		});
...
if [[ "$cmd" == "list-panes" ]]; then
  printf '%%1\\037codex\\0370\\0370\\037160\\03724\\03723\\037160\\037${crampedHeight}\\037codex\\037${repoCwd}\\0370\\0371001\n'
fi
`,
			);
			await chmod(tmuxBin, 0o755);
			process.env.PATH = `${fakeBinDir}:${originalPath ?? ""}`;
			process.env.OMX_MUX_BINARY = tmuxBin;
