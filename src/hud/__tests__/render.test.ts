...
describe("renderHud – ralplan", () => {
	...
	it("renders iteration display when ralplan iteration is present", () => {
		const ctx = {
			...emptyCtx(),
			ralplan: { active: true, iteration: 2, planning_complete: false },
		};
		const result = renderHud(ctx, "focused");
...
describe("renderHud – code-review", () => {
	it("renders code-review with the current phase", () => {
		const ctx = {
			...emptyCtx(),
			codeReview: { active: true, current_phase: "running" },
		};
		const result = renderHud(ctx, "focused");
	...
	it("keeps canonical code-review distinct from an autopilot late phase", () => {
		const ctx = {
			...
			codeReview: {
				...
				source: "canonical-skill" as const,
			},
		};
		const result = stripSgr(renderHud(ctx, "focused"));
		assert.ok(result.includes("autopilot:code-review"));
...
describe("renderHud – ultragoal", () => {
	it("uses a distinct accent for the current ultragoal only", () => {
		...
		assert.ok(
			result.includes("\x1b[35mG001-active: Active goal highlight\x1b[0m"),
		);
	...
	it("preserves the current ultragoal accent under constrained-width truncation", () => {
		...
		assert.ok(
			...
			"active goal accent should survive width truncation",
		);
	...
	it("renders active ultragoal progress and title in English", () => {
		...
		assert.ok(
			result.includes("ultragoal 2/5 ▶ G003-tests: HUD progress display"),
		);
		assert.ok(
			!result.includes("objective: show active ultragoal objective in OMX HUD"),
		);
	...
	it("combines active ultragoal and team into one non-duplicated focused summary", () => {
		...
		const result = stripSgr(
			renderHud(ctx, "focused", { maxWidth: 220, maxLines: 3 }),
		);
		...
		assert.ok(
			result.includes(
				"ultragoal 1/4 + team:4 workers ▶ G002-team-hud: Fix combined HUD rendering",
			),
		);
	...
	it("truncates long ultragoal objectives", () => {
		const longObjective = "show active ultragoal objective in OMX HUD ".repeat(
			8,
		);
		const ctx = {
			...emptyCtx(),
	...
	it("keeps long ultragoal title/objective summaries compact without duplicate ellipses", () => {
		const repeatedText =
			"Build example component with helper, install example package and app package on the target";
		const ctx = {
			...emptyCtx(),
			ultragoal: {
				active: true,
		...
		const result = stripSgr(
			renderHud(ctx, "focused", { maxWidth: 120, maxLines: 3 }),
		);

		assert.ok(
			result.includes(
				"ultragoal 0/6 ▶ G004-example-long-goal-id: Build example component with helper…",
			),
		);
	...
	it("renders active ultragoal and omits lower-priority pending items to protect compactness", () => {
		const ctx = {
			...
			ultragoal: {
				...
				activeGoal: {
					...
					objective:
						"show three active workflow statuses in the OMX HUD without clipping",
					status: "in_progress",
					index: 2,
				},
				nextGoals: [
		...
		const result = stripSgr(
			renderHud(ctx, "focused", { maxWidth: 260, maxLines: 6 }),
		);
		...
		assert.ok(
			!result.includes("G005-docs: Document compact summary (pending)"),
		);
		assert.ok(result.split("\n").length <= 3);
	});
	...
	it("renders fewer pending ultragoal items without empty separators", () => {
		...
		const result = stripSgr(
			renderHud(ctx, "focused", { maxWidth: 180, maxLines: 3 }),
		);
...
describe("renderHud – wrapping", () => {
	it("wraps long HUD output across multiple lines when width is constrained", () => {
		...
		const result = stripSgr(
			renderHud(ctx, "focused", { maxWidth: 32, maxLines: 5 }),
		);
		assert.ok(result.includes("\n"));
		assert.ok(result.split("\n").length <= 5);
	});
	...
	it("caps wrapped HUD output at the adaptive no-ultragoal line count", () => {
		...
		const result = stripSgr(
			renderHud(ctx, "full", { maxWidth: 22, maxLines: 3 }),
		);
