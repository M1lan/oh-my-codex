...
describe("HUD resize hook helpers", () => {
	...
	it("parses hook context with exact leader and HUD pane incarnations", () => {
		...
		assert.equal(
			parseHudResizeHookContext("$7; touch /tmp/owned\t@3\n", "%1"),
			null,
		);
		assert.equal(
			parseHudResizeHookContext("$7\t@3$(touch /tmp/owned)\n", "%1"),
			null,
		);
		assert.equal(
			parseHudResizeHookContext("$7\t@3\n", "%1; touch /tmp/owned"),
			null,
		);
	});
	...
	it("guards registered hooks against recycled leader or HUD pane IDs", () => {
		const calls: string[][] = [];
		assert.equal(
			registerHudResizeHook("%9", "%1", 3, (args) => {
				calls.push(args);
				return hookAuthority(args) ?? "";
			}),
			true,
	...
	it("rejects a recycled HUD split source before creating a pane", () => {
		...
		const result = createHudWatchPane(
			"/repo",
	...
	it("reports layout-hook installation failure without undoing the installed resize hook", () => {
		...
		assert.ok(
			calls.some(
				(args) =>
					args[0] === "set-hook" &&
					args[3] === buildHudResizeHookSlot("omx_hud_resize_7_3_1"),
			),
		);
		assert.ok(
			calls.some(
				(args) => args[0] === "set-hook" && args[3] === layoutHookSlot,
			),
		);
	});
	...
	it("unregisters leader-scoped hooks through guarded session transactions", () => {
		const calls: string[][] = [];
		assert.equal(
			unregisterHudResizeHook("%1", (args) => {
				calls.push(args);
				return hookAuthority(args) ?? "";
			}),
			true,
		...
		assert.deepEqual(
			guarded.map((args) => args[3]),
			["$7", "$7"],
		);

		const expectedIdentity = (
			hookSlot: string,
		): { option: string; predicate: string } => {
			...
			return {
				option,
				predicate: `#{==:${option},omx-${(hash >>> 0).toString(16)}}`,
			};
		};
	...
	it("attempts layout-hook cleanup after resize-hook cleanup failure", () => {
		...
		const result = unregisterHudResizeHook("%1", (args) => {
			calls.push(args);
			if (args[0] === "if-shell" && (args[5] ?? "").includes(resizeSlot))
				throw new Error("resize hook rejected");
			return hookAuthority(args) ?? "";
		});
		assert.equal(result, false);
		assert.ok(
	...
	it("uses distinct slots across windows and leaders while retaining a leader slot across HUD recreation", () => {
		...
		const registeredResizeSlots = registered.filter((args) =>
			args[3]?.startsWith("client-resized["),
		);
		assert.notEqual(
			registeredResizeSlots[0]?.[3],
			registeredResizeSlots[1]?.[3],
		);
		...
		const recreatedResizeSlots = recreated.filter((args) =>
			args[3]?.startsWith("client-resized["),
		);
...
describe("HUD pane ownership helpers", () => {
	...
	it("reads session and leader ownership from env-prefixed HUD commands", () => {
		...
		assert.equal(
			hudPaneMatchesOwner(pane!, { sessionId: "sess-a", leaderPaneId: "%1" }),
			true,
		);
		assert.equal(
			hudPaneMatchesOwner(pane!, { sessionId: "sess-b", leaderPaneId: "%2" }),
			false,
		);
	});
	...
	it("preserves tab-containing start commands when reading the optional cwd column", () => {
		const [pane] = parseTmuxPaneSnapshot(
			"%9\tnode\tnode\t/omx.js hud --watch\t/tmp/repo",
		);
	...
	it("does not owner-match untagged HUD panes when an owner scope is requested", () => {
		const panes = parseTmuxPaneSnapshot(
			["%1\tcodex\tcodex", "%2\tnode\tnode /tmp/bin/omx.js hud --watch"].join(
				"\n",
			),
		);
	...
	it("finds one same-session HUD pane when TMUX_PANE is unavailable", () => {
			...
			return (
				[
					...
					[
						...
						"160",
						"50",
						`exec env OMX_SESSION_ID='sess-a' ${OMX_TMUX_HUD_LEADER_PANE_ENV}='%1' /node /omx.js hud --watch`,
						"/repo",
						"0",
						"102",
					].join("\x1f"),
		...
		assert.deepEqual(
			listCurrentWindowHudPaneIds(undefined, execTmuxSync, {
				sessionId: "sess-a",
			}),
			["%2"],
		);
...
describe("dead HUD pane reaper", () => {
	it("ignores team ACK commands that mention HUD preserve repro text", () => {
		...
		const result = reapDeadHudPanes(panes, {
			killPane: () => {
				throw new Error(
					"team ACK command should not be classified as a HUD watch pane",
				);
			},
		});
	...
	it("kills untagged HUD panes whose tmux cwd has been deleted", () => {
		const deletedPath = join(
			tmpdir(),
			`omx-doctor-native-hook-dist-${process.pid}-${Date.now()} (deleted)`,
		);
	...
	it("kills deleted-cwd doctor-smoke HUD panes even when an old owner tag points at a live leader", () => {
		const deletedPath = join(
			tmpdir(),
			`omx-doctor-plugin-hook-${process.pid}-${Date.now()} (deleted)`,
		);
		rmSync(deletedPath, { recursive: true, force: true });
		const panes = parseTmuxPaneSnapshot(
			[
	...
	it("kills doctor-smoke HUD panes even if a literal deleted-marker cwd was materialized", () => {
		const parent = mkdtempSync(
			join(tmpdir(), "omx-doctor-plugin-hook-live-marker-"),
		);
	...
	it("preserves non-doctor deleted-cwd HUD panes while their leader is still live", () => {
		const deletedPath = join(
			tmpdir(),
			`omx-live-leader-deleted-cwd-${process.pid}-${Date.now()} (deleted)`,
		);
		rmSync(deletedPath, { recursive: true, force: true });
		const panes = parseTmuxPaneSnapshot(
			[
		...
		const result = reapDeadHudPanes(panes, {
			killPane: () => {
				throw new Error(
					"live leader HUD with stale launch cwd should not be killed",
				);
			},
		});
	...
	it("kills deleted-cwd HUD panes when their owner leader is no longer live", () => {
		const deletedPath = join(
			tmpdir(),
			`omx-dead-leader-deleted-cwd-${process.pid}-${Date.now()} (deleted)`,
		);
		rmSync(deletedPath, { recursive: true, force: true });
		const panes = parseTmuxPaneSnapshot(
			[
	...
	it("preserves HUD panes in an existing cwd whose name ends with the deleted marker text", () => {
		...
		try {
			const result = reapDeadHudPanes(panes, {
				killPane: () => {
					throw new Error(
						"live cwd with literal marker suffix should not be killed",
					);
				},
			});
	...
	it("preserves live deleted-marker cwd paths containing tabs from the tmux list separator", () => {
		...
		try {
			const result = reapDeadHudPanes(panes, {
				killPane: () => {
					throw new Error(
						"live tab cwd with literal marker suffix should not be killed",
					);
				},
			});
	...
	it("preserves deleted-cwd panes with misleading HUD text but no OMX owner metadata", () => {
		const deletedPath = join(
			tmpdir(),
			`omx-misleading-hud-text-${process.pid}-${Date.now()} (deleted)`,
		);
		rmSync(deletedPath, { recursive: true, force: true });
		const panes = parseTmuxPaneSnapshot(
			[
