...
describe("runWatchMode", () => {
	it("resolves a live cwd when the HUD launch path was reused by another run", () => {
		const resolved = resolveHudWatchCwd("/home/tools/calc", {
			...
			readProcCwd: () =>
				"/home/tools/calc.noninteractive-aborted-20260527T233204Z",
			realpath: (path) => {
				if (path === "/home/tools/calc") return "/dev/inode/new-calc-run";
				if (
					path === "/home/tools/calc.noninteractive-aborted-20260527T233204Z"
				) {
	...
	it("does not select a proc deleted-cwd marker over the safer launch cwd", () => {
		const resolved = resolveHudWatchCwd("/tmp/omx-doctor-plugin-hook-smoke", {
			...
			realpath: (path) => {
				if (path === "/tmp/omx-doctor-plugin-hook-smoke")
					return "/dev/inode/reused-safe-path";
	...
	it("follows a live cwd whose literal name ends with the deleted marker text", () => {
		const resolved = resolveHudWatchCwd("/tmp/stale-launch", {
			...
			realpath: (path) => {
				...
				if (path === "/tmp/live workspace (deleted)")
					return "/dev/inode/live-literal-marker";
				return path;
			},
		});
	...
	it("reads HUD state from the resolved live cwd on every watch frame", async () => {
		...
		const promise = runWatchMode("/home/tools/calc", WATCH_FLAGS, {
			...
			renderHudFn: () => "frame",
			runAuthorityTickFn: async ({ cwd }) => {
				seenAuthorityCwds.push(cwd);
			},
			writeStdout: () => {},
			writeStderr: () => {},
		...
		assert.deepEqual(seenConfigCwds, [
			"/home/tools/calc.noninteractive-aborted-20260527T233204Z",
		]);
		assert.deepEqual(seenStateCwds, [
			"/home/tools/calc.noninteractive-aborted-20260527T233204Z",
		]);
	...
	it("restores cursor and clears interval on SIGINT", async () => {
		...
		const promise = runWatchMode("/tmp", WATCH_FLAGS, {
			...
			readAllStateFn: async (_cwd, config) => ({
				...emptyCtx(),
				gitBranch: config?.git.display ?? null,
			}),
		...
		assert.ok(
			writes.some((chunk) => chunk.includes("\x1b[?25l")),
			"cursor should be hidden in watch mode",
		);
		assert.ok(
			writes.some((chunk) => chunk.includes("\x1b[?25h\x1b[2J\x1b[H")),
			"cursor should be restored on SIGINT",
		);
	...
	it("coalesces ticks so slow renders do not overlap", async () => {
		...
		await withTimeout(
			...
			"first render should start before exercising queued ticks",
		);
		...
		await withTimeout(
			secondReadStarted.promise,
			"queued rerender should start after first render is released",
		);
		...
		assert.equal(
			callCount,
			2,
			"multiple overlapping ticks should collapse to one queued rerender",
		);
	});
	...
	it("does not write an extra terminal row beyond the rendered watch frame", async () => {
		...
		assert.ok(
			writes.some((chunk) => chunk.includes("line-one\nline-two\x1b[K\x1b[J")),
		);
		assert.ok(
			!writes.some((chunk) => chunk.includes("line-two\x1b[K\n\x1b[J")),
		);
	});
	...
	it("resizes an OMX-owned running HUD pane when the adaptive budget changes", async () => {
		...
		await withTimeout(
			secondReadStarted.promise,
			"second render should observe active ultragoal",
		);
		sigintHandler?.();
		await promise;
	...
	it("keeps rendering when the authority tick fails after a frame", async () => {
		...
		await withTimeout(
			firstAuthorityAttempted.promise,
			"first authority tick should run",
		);
		await flush();
		assert.equal(process.exitCode, undefined);
		assert.ok(
			errors.some((line) =>
				line.includes("HUD watch authority tick failed: dist is rebuilding"),
			),
		);
		...
		await withTimeout(
			secondReadStarted.promise,
			"watch should render again after authority tick failure",
		);
		sigintHandler?.();
		await promise;
	...
	it("handles render failures gracefully and restores terminal state", async () => {
		...
		assert.equal(process.exitCode, 1);
		assert.ok(
			errors.some((line) => line.includes("HUD watch render failed: boom")),
		);
...
`,
		...
		const previousEnv = {
			...
			OMX_MUX_BINARY: process.env.OMX_MUX_BINARY,
		...
		try {
			...
			assert.equal(
				logs.some((line) => line.includes("Removed duplicate HUD panes")),
				false,
			);
		} finally {
...
`,
		...
		try {
			...
			assert.match(tmuxLog, /split-window -v -l 2 -t %1/);
			assert.equal(
				logs.some((line) => line.includes("Reused existing HUD pane")),
				false,
			);
		} finally {
			console.log = previousLog;
