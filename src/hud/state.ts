...
function normalizeUltragoalGoal(raw: unknown): NormalizedUltragoalGoal | null {
	...
	return {
		...
		status,
		steeringStatus,
		supersededBy: Array.isArray(goal.supersededBy)
			? goal.supersededBy
					.map(sanitizeOptionalString)
...
function isSupersededUltragoalGoalResolved(
	goal: NormalizedUltragoalGoal,
	goals: NormalizedUltragoalGoal[],
): boolean {
	...
	return goal.supersededBy.every((id) => {
		...
		return (
			replacement !== undefined && isResolvedUltragoalStatus(replacement.status)
		);
	});
}

function isNonBlockingSupersededUltragoalGoal(
	goal: NormalizedUltragoalGoal,
	goals: NormalizedUltragoalGoal[],
): boolean {
	return isSupersededUltragoalGoalResolved(goal, goals);
}
function isHudCompletionBlockingUltragoalGoal(
	goal: NormalizedUltragoalGoal,
	goals: NormalizedUltragoalGoal[],
): boolean {
...
function isHudUnresolvedUltragoalGoal(
	goal: NormalizedUltragoalGoal,
	goals: NormalizedUltragoalGoal[],
): boolean {
...
export async function readUltragoalState(
	...
	const pending_goals = goals.filter(
		(goal) =>
			goal.status === "pending" &&
			!isNonBlockingSupersededUltragoalGoal(goal, goals),
	).length;
	...
	const failed_goals = goals.filter(
		(goal) =>
			goal.status === "failed" &&
			!isNonBlockingSupersededUltragoalGoal(goal, goals),
	).length;
	...
	const activeGoal =
		...
		goals.find(
			(goal) =>
				isHudUnresolvedUltragoalGoal(goal, goals) &&
				ULTRAGOAL_ACTIVE_STATUSES.has(goal.status),
		) ??
		goals.find(
			(goal) =>
				isHudUnresolvedUltragoalGoal(goal, goals) &&
				ULTRAGOAL_UNRESOLVED_STATUSES.has(goal.status),
		);
