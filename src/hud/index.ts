...
export function resolveHudWatchCwd(
	...
	const realpath =
		deps.realpath ?? ((path: string) => realpathSync.native(path));
	...
	const liveMarkerMayBeProcDeleted =
		isDeletedCwdMarkerText(livePath) &&
		!isDeletedCwdMarkerText(launchPath) &&
		processCwd !== livePath;
	if (liveMarkerMayBeProcDeleted) {
		const processReal = processCwd
			? safeCallString(() => realpath(processCwd))
			: null;
		const markerReal = safeCallString(() => realpath(livePath));
		if (!processReal || !markerReal || processReal !== markerReal)
			return launchPath;
	}
...
function reconcileRunningHudPaneHeight(
	...
	dependencies: Pick<
		RunWatchModeDependencies,
		"env" | "resizeTmuxPaneFn" | "registerHudResizeHookFn"
	>,
): void {
	if (
	...
	const leaderPaneId =
		dependencies.env[OMX_TMUX_HUD_LEADER_PANE_ENV]?.trim() || undefined;
	if (dependencies.resizeTmuxPaneFn(hudPaneId, desiredHeight) && leaderPaneId) {
		dependencies.registerHudResizeHookFn(
			hudPaneId,
			leaderPaneId,
			desiredHeight,
		);
...
export async function runWatchMode(
	...
	const dependencies: RunWatchModeDependencies = {
		...
		resolveWatchCwdFn:
			deps.resolveWatchCwdFn ?? ((launchCwd) => resolveHudWatchCwd(launchCwd)),
		...
		runAuthorityTickFn:
			deps.runAuthorityTickFn ??
			(async ({ cwd: authorityCwd }) => {
				await runHudAuthorityTick({ cwd: authorityCwd });
			}),
		resizeTmuxPaneFn: deps.resizeTmuxPaneFn ?? resizeTmuxPane,
		registerHudResizeHookFn:
			deps.registerHudResizeHookFn ?? registerHudResizeHook,
		writeStdout:
		...
		clearIntervalFn:
			...
			((timer: ReturnType<typeof setInterval>) => clearInterval(timer)),
	};
		...
		try {
			...
			try {
				...
				dependencies.writeStderr(
					`HUD watch authority tick failed: ${message}\n`,
				);
			}
		} catch (error) {
...
export async function hudCommand(
	...
	if (args.includes("--reconcile-tmux")) {
		const result = await (
			deps.reconcileHudForPromptSubmit ?? reconcileHudForPromptSubmit
		)(cwd);
...
export function buildTmuxSplitArgs(
	...
	const envAssignments = Object.entries(
		...
		([key, value]) =>
			`${key}=${key === OMX_TMUX_HUD_OWNER_ENV ? value : shellEscape(value)}`,
	);
	const envPrefix =
		envAssignments.length > 0 ? `env ${envAssignments.join(" ")} ` : "";
	const cmd = `exec ${envPrefix}${shellEscape(process.execPath)} ${shellEscape(omxBin)} hud --watch${presetArg}`;
	const height =
		Number.isFinite(heightLines) && (heightLines ?? 0) > 0
...
async function launchTmuxPane(cwd: string, flags: HudFlags): Promise<void> {
	...
	const existingHudPaneIds =
		leaderPaneId || sessionId
			? listCurrentWindowHudPaneIds(
					leaderPaneId,
					undefined,
	...
	const args = buildTmuxSplitArgs(
		...
		getHudRenderMaxLines(ctx),
		{
			...
			rootSource: process.env.OMX_TEAM_STATE_ROOT
				? "team-env"
				: process.env.OMX_ROOT
					? "omx-root-env"
	...
	try {
		...
		execFileSync(resolveTmuxBinaryForPlatform() || "tmux", args, {
			stdio: "inherit",
		});
		console.log(
			"HUD launched in tmux pane below. Close with: Ctrl+C in that pane, or `tmux kill-pane -t bottom`",
		);
	} catch {
