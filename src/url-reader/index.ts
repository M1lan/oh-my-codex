...
async function prepareSafeFetchTarget(
	...
	const validation = await validateAndResolveSafeUrl(
		url,
		options,
		redirect,
		inputUrl,
	...
	try {
		...
		return {
			blocked: blockedResult(inputUrl, "dns-resolution-failed", {
				...
				message:
					"Blocked URL read because the hostname could not be resolved safely.",
			}),
		};
	}
	...
	if (addresses.length === 0) {
		return {
			blocked: blockedResult(inputUrl, "dns-resolution-empty", {
				name: "DnsResolutionError",
				message:
					"Blocked URL read because the hostname did not resolve to an address.",
			}),
		};
	}
...
async function resultFromResponse(
	...
	const redirected =
		safeRedirected || response.redirected || urlsDiffer(inputUrl, response.url);
