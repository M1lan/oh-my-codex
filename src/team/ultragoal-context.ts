...
export function normalizeUltragoalTeamContext(
	value: unknown,
): UltragoalTeamContext | null {
...
export async function resolveLeaderOwnedUltragoalContext(
	cwd: string,
	...
	throw new Error(
		`invalid_ultragoal_team_context:${outcome.warning?.message ?? outcome.status}`,
	);
}
...
function warning(
	code: UltragoalContextResolutionStatus,
	message: string,
): UltragoalContextResolution {
	return { status: code, context: null, warning: { code, message } };
}

export async function resolveLeaderOwnedUltragoalContextOutcome(
	cwd: string,
	...
	try {
		const parsed = JSON.parse(await readFile(goalsJsonPath, "utf-8")) as Record<
			string,
			unknown
		>;
		const activeGoalId =
			typeof parsed.activeGoalId === "string" ? parsed.activeGoalId.trim() : "";
		if (activeGoalId === "") {
		...
		const activeGoal = goals.find(
			(goal) =>
				goal &&
				typeof goal === "object" &&
				(goal as Record<string, unknown>).id === activeGoalId,
		...
		const activeGoalTitle =
			typeof activeGoal?.title === "string" && activeGoal.title.trim() !== ""
...
export async function readPersistedTeamUltragoalContext(
	...
	try {
		return normalizeUltragoalTeamContext(
