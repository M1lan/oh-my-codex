...
function isNarrowTask(text: string): boolean {
	return (
		...
		(SIMPLE_SCOPE_PATTERNS.some((pattern) => pattern.test(text)) ||
			!/\b(search|debug|investigate|refactor|migration|review|test)\b/i.test(
				text,
			))
	);
}
...
function roleAwareSubtaskCandidates(
	task: Pick<TeamTask, "subject" | "description" | "role">,
): string[] {
	...
	if (
		/\bdebug|root[-\s]?cause|flaky|failure|regression|bug|investigat/i.test(
			text,
		)
	) {
		candidates.push(
			"Debug/root-cause probe: trace likely failure paths and summarize evidence.",
		);
	}
	if (/\bsearch|map|find references|repo[-\s]?wide|investigat/i.test(text)) {
		candidates.push(
			"Repository map probe: find relevant files, symbols, and ownership boundaries.",
		);
	}
	...
	if (/\brefactor|cleanup|deslop|simplif|migrat|upgrade|port/i.test(text)) {
		candidates.push(
			"Change-slice probe: isolate safe implementation slices and migration hazards.",
		);
	}

	if (candidates.length === 0) {
		candidates.push(
			"Context probe: map the relevant code paths and summarize recommended next steps.",
		);
	}
