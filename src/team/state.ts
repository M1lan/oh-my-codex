...
export function setWriteAtomicPlatformForTests(
...
export type TransitionTaskResult =
	...
	| {
			...
			error:
				...
				| "missing_coordination_compliance_evidence";
	  };
...
function clampDispatchAckTimeoutMs(raw: unknown): number {
	...
	return Math.max(
		...
		Math.min(MAX_DISPATCH_ACK_TIMEOUT_MS, floored),
	);
}
...
export function normalizeTeamPolicy(
	...
	const base = defaultPolicy(
		...
		defaults.worker_launch_mode,
	);
	const dispatchMode =
		policy?.dispatch_mode === "transport_direct"
			? "transport_direct"
			: "hook_preferred_with_fallback";
	...
	return {
		worker_launch_mode:
			policy?.worker_launch_mode === "prompt"
				? "prompt"
				: base.worker_launch_mode,
		display_mode:
			policy?.display_mode === "split_pane" ? "split_pane" : base.display_mode,
		dispatch_mode: dispatchMode,
		dispatch_ack_timeout_ms: clampDispatchAckTimeoutMs(
...
export function normalizeTeamGovernance(
	...
	return {
		...
		cleanup_requires_all_workers_inactive:
			source?.cleanup_requires_all_workers_inactive !== false,
	};
}
...
function parseOptionalBoolean(raw: string | null): boolean | null {
	...
	if (
		["1", "true", "yes", "on", "enabled", "allow", "allowed"].includes(
			normalized,
		)
	)
		return true;
	if (
		["0", "false", "no", "off", "disabled", "deny", "denied"].includes(
			normalized,
		)
	)
		return false;
...
function resolveDisplayModeFromEnv(
	env: NodeJS.ProcessEnv,
): TeamPolicy["display_mode"] {
...
function resolveWorkerLaunchModeFromEnv(
	env: NodeJS.ProcessEnv,
): TeamPolicy["worker_launch_mode"] {
...
function resolvePermissionsSnapshot(
	env: NodeJS.ProcessEnv,
): PermissionsSnapshot {
	...
	const sandboxMode = readEnvValue(env, [
		"OMX_SANDBOX_MODE",
		"CODEX_SANDBOX_MODE",
		"SANDBOX_MODE",
	]);
...
async function resolveLeaderSessionId(
	...
	const fromEnv = readEnvValue(env, [
		"OMX_SESSION_ID",
		"CODEX_SESSION_ID",
		"SESSION_ID",
	]);
...
function normalizeTask(task: TeamTask): TeamTaskV2 {
	const normalizedCoordination = normalizeTeamTaskCoordinationPlanForStorage(
		task.coordination,
	);
...
function isTeamTask(value: unknown): value is TeamTask {
	...
	if (
		...
		!TEAM_TASK_STATUSES.includes(v.status as TeamTaskStatus)
	)
		return false;
...
function isTeamManifestV2(value: unknown): value is TeamManifestV2 {
	...
	if (
		!(
			typeof v.leader_pane_pid === "number" ||
			v.leader_pane_pid === null ||
			v.leader_pane_pid === undefined
		)
	)
		return false;
	if (
		!(
			typeof v.hud_pane_pid === "number" ||
			v.hud_pane_pid === null ||
			v.hud_pane_pid === undefined
		)
	)
		return false;
	if (!(typeof v.resize_hook_name === "string" || v.resize_hook_name === null))
		return false;
	if (
		!(typeof v.resize_hook_target === "string" || v.resize_hook_target === null)
	)
		return false;
...
async function validateMembershipTransactionFiles(
	...
	if (
		canonicalTeamRoot !== canonicalStateRoot &&
		!canonicalTeamRoot.startsWith(`${canonicalStateRoot}${sep}`)
	) {
		throw new Error(
			`Membership transaction team root escapes canonical state root for ${teamName}`,
		);
	}
	const canonicalTasksDir = await realpath(
		resolve(teamDir(teamName, cwd), "tasks"),
	);
	if (!canonicalTasksDir.startsWith(`${canonicalTeamRoot}${sep}`)) {
		throw new Error(
			`Membership transaction tasks root escapes canonical team root for ${teamName}`,
		);
	}
	...
	for (const file of files) {
		...
		if (resolvedPath === expectedConfigPath) {
			...
			if (dirname(resolvedPath) !== expectedTasksDir) {
				throw new Error(
					`Membership transaction path escapes expected team files for ${teamName}`,
				);
			}
			const match = basename(resolvedPath).match(
				/^task-([A-Za-z0-9_-]+)\.json$/,
			);
			if (
				!match ||
				resolve(taskFilePath(teamName, match[1]!, cwd)) !== resolvedPath
			) {
				throw new Error(
					`Invalid membership transaction task path for ${teamName}`,
				);
			}
		...
		if (canonicalParent !== expectedParent) {
			throw new Error(
				`Membership transaction path traverses a symlink for ${teamName}`,
			);
		}
	}
	if (configCount !== 1) {
		throw new Error(
			`Membership transaction must contain exactly one config path for ${teamName}`,
		);
	}
}
...
export async function recoverTeamMembershipTaskTransaction(
	...
	try {
		...
		) as MembershipTransactionJournal;
	} catch (error) {
	...
	await applyMembershipTransactionFiles(
		journal.files,
...
export async function finalizeTeamMembershipTaskTransaction(
	teamName: string,
	cwd: string,
): Promise<void> {
	const journalPath = membershipTransactionPath(teamName, cwd);
	...
	if (
		...
		throw new Error(
			`Cannot finalize non-committed membership transaction for ${teamName}`,
		);
	}
	await validateMembershipTransactionFiles(teamName, cwd, journal.files);
...
export async function commitTeamMembershipTaskTransaction(
	...
	if (
		!taskMembershipBarrierContext
			.getStore()
			?.has(taskMembershipBarrierKey(teamName, cwd))
	) {
	...
	if (transaction.baseGeneration !== currentGeneration) {
		throw new Error(
			`team_membership_stale_generation:${teamName}:${transaction.baseGeneration}:${currentGeneration}`,
		);
	}
	...
	const nextManifest =
		transaction.manifest?.newBytes === null
			? null
			: transaction.manifest
				? {
						...(JSON.parse(transaction.manifest.newBytes) as TeamManifestV2),
						config_generation: acceptedGeneration,
					}
				: undefined;
	...
	if (transaction.manifest) {
		files.push({
			...
			newBytes:
				nextManifest === null ? null : JSON.stringify(nextManifest, null, 2),
		});
	}
	const journalPath = membershipTransactionPath(teamName, cwd);
	...
	try {
		if (
			transaction.interruptAfterFirstTaskWrite &&
		...
		if (
			...
			transaction.failRollbackPersistenceAfter
		) {
			...
			throw new Error(
				"injected_scale_down_failure:rollback-persistence-failure",
			);
		}
		journal.phase = "committed";
		await writeAtomic(journalPath, JSON.stringify(journal, null, 2));
				...
				? files.findIndex(
						(file) =>
							...
							(target === "config"
								...
								: teamManifestV2Path(teamName, cwd)),
					)
				: 0;
...
export async function initTeamState(
	...
	if (maxWorkers > ABSOLUTE_MAX_WORKERS) {
		throw new Error(
			`maxWorkers (${maxWorkers}) exceeds ABSOLUTE_MAX_WORKERS (${ABSOLUTE_MAX_WORKERS})`,
		);
	}

	if (workerCount > maxWorkers) {
		throw new Error(
			`workerCount (${workerCount}) exceeds maxWorkers (${maxWorkers})`,
		);
	}
	...
	await writeAtomic(
		join(dispatchRoot, "requests.json"),
	...
	const leaderWorkerId =
		readEnvValue(env, ["OMX_TEAM_WORKER"]) ?? "leader-fixed";
...
async function writeConfig(cfg: TeamConfig, cwd: string): Promise<void> {
	...
	const files: MembershipTransactionFile[] = [
		{
			path: configPath,
			oldBytes: oldConfigBytes,
			newBytes: JSON.stringify(normalized, null, 2),
		},
		{
			path: manifestPath,
			oldBytes: await readFile(manifestPath, "utf8"),
			newBytes: JSON.stringify(merged, null, 2),
		},
	];
...
function teamConfigFromManifest(manifest: TeamManifestV2): TeamConfig {
	const normalizedPolicy = normalizeTeamPolicy(manifest.policy, {
		display_mode:
			manifest.policy?.display_mode === "split_pane" ? "split_pane" : "auto",
		worker_launch_mode:
	...
	return {
		...
		tmux_pane_owner_id:
			typeof manifest.tmux_pane_owner_id === "string" &&
...
function normalizeTeamConfig(config: TeamConfig): TeamConfig {
	const workerLaunchMode =
		config.worker_launch_mode === "prompt" ? "prompt" : "interactive";
	const configGeneration =
		typeof config.config_generation === "number" &&
		Number.isSafeInteger(config.config_generation) &&
	...
	return {
		...
		leader_pane_pid:
			...
			Number.isSafeInteger(config.leader_pane_pid) &&
...
async function readTeamManifestV2Raw(
	teamName: string,
	cwd: string,
	...
	try {
		...
		const tmuxPaneOwnerId =
			typeof parsedManifest.tmux_pane_owner_id === "string" &&
		...
		return {
			...
			policy: normalizeTeamPolicy(parsedManifest.policy, {
				display_mode:
					parsedManifest.policy?.display_mode === "split_pane"
						? "split_pane"
						: "auto",
				worker_launch_mode:
					parsedManifest.policy?.worker_launch_mode === "prompt"
						? "prompt"
						: "interactive",
			}),
			governance: normalizeTeamGovernance(
				parsedManifest.governance,
				parsedManifest.policy,
			),
			team_decomposition:
...
export async function readTeamManifestV2(
	teamName: string,
	cwd: string,
): Promise<TeamManifestV2 | null> {
...
async function computeNextTaskIdFromDisk(
	teamName: string,
	cwd: string,
): Promise<number> {
...
async function readConfigGenerationRaw(
	teamName: string,
	cwd: string,
): Promise<number | null> {
	try {
		const raw = JSON.parse(
			await readFile(teamConfigPath(teamName, cwd), "utf8"),
		) as { config_generation?: unknown };
		return typeof raw.config_generation === "number" &&
			Number.isSafeInteger(raw.config_generation) &&
			raw.config_generation >= 0
...
// Read team config
async function readTeamConfigRaw(
	teamName: string,
	cwd: string,
	...
	if (v2)
		return {
			...teamConfigFromManifest(v2),
			config_generation: (await readConfigGenerationRaw(teamName, cwd)) ?? 0,
		};
...
export async function withScalingLock<T>(
	...
	return await withScalingLockImpl(
		teamName,
		cwd,
		LOCK_STALE_MS,
...
export async function createTask(
	...
	return withTeamTaskBarrier(teamName, cwd, async () => {
		...
		if (
			!hasValidNextTaskId(cfg.next_task_id) ||
			nextNumericFromDisk > nextNumeric
		) {
		...
		await writeAtomic(
			taskFilePath(teamName, nextId, cwd),
	...
	return await withTeamTaskBarrier(teamName, cwd, async () => {
		...
		const lock = await withTaskClaimLock(teamName, taskId, cwd, async () => {
			...
			if (
				...
				!["pending", "blocked", "in_progress", "completed", "failed"].includes(
					updates.status,
				)
			) {
...
export async function transitionTaskStatus(
	...
	return await withTeamTaskBarrier(teamName, cwd, async () => {
		await recoverTeamMembershipTaskTransaction(teamName, cwd);
		return await transitionTaskStatusImpl(
			taskId,
			from,
			to,
			claimToken,
...
async function readMailbox(
	...
	if (isBridgeEnabled()) {
		try {
			...
			const compat = bridge.readCompatFile<{ records?: unknown[] }>(
				"mailbox.json",
			);
			if (compat) {
		...
		try {
			...
			if (compat) {
				...
				return bridge
					.readDispatchRecords()
					.map((record) =>
						normalizeBridgeDispatchRecord(teamName, record, nowIso),
					)
...
function isStrictLegacyDispatchRequestForRollback(
	...
	return (
		...
		[
			...
			"transport_direct",
			"prompt_stdin",
		].includes(String(request.transport_preference)) &&
		typeof request.fallback_allowed === "boolean" &&
		["pending", "notified", "delivered", "failed"].includes(
...
async function readLegacyDispatchRequestsForRollback(
	teamName: string,
	cwd: string,
): Promise<TeamDispatchRequest[]> {
	const path = dispatchRequestsPath(teamName, cwd);
	try {
		...
		if (
			!Array.isArray(raw) ||
			raw.some(
				(entry) => !isStrictLegacyDispatchRequestForRollback(entry, teamName),
			)
		) {
			throw new Error(
...
async function writeDispatchRequests(
	...
	await writeAtomic(
		dispatchRequestsPath(teamName, cwd),
		JSON.stringify(requests, null, 2),
	);
	await writeBridgeDispatchCompat(teamName, requests, cwd);
}

function serializeDispatchRequestToBridgeRecord(
...
async function writeBridgeDispatchCompat(
	...
	const existing = getDefaultBridge(stateDir).readCompatFile<{
	...
	const records = [
		...otherRecords,
		...requests.map(serializeDispatchRequestToBridgeRecord),
	];
...
async function withDispatchLock<T>(
	...
	return await withDispatchLockImpl(
		teamName,
		cwd,
		teamDir,
		dispatchLockDir,
...
export async function removeDispatchRequestsForWorkers(
	...
	await withDispatchLock(teamName, cwd, async () => {
		...
		if (isBridgeEnabled()) {
			...
			const scopedAuthoritativeIds = bridge
				.readDispatchRecordsStrict()
				.filter((record) => {
					const metadataTeam =
						typeof record.metadata?.team_name === "string"
							? record.metadata.team_name
							: "";
					return metadataTeam === teamName && names.has(record.target);
				})
				.map((record) => record.request_id);
			removedRequestIds = [
				...new Set([...legacyRequestIds, ...scopedAuthoritativeIds]),
			];
			...
			if (
				...
				removedRequestIds.some(
					(requestId) => !removal.request_ids.includes(requestId),
				)
			) {
				throw new Error(
					`authoritative_dispatch_rollback_command_failed:${[...names].join(",")}`,
				);
			}
			...
			const verificationSnapshot = bridge.execCommand({
				command: "CaptureSnapshot",
			});
			if (verificationSnapshot.event !== "SnapshotCaptured") {
				throw new Error(
					`authoritative_dispatch_rollback_verification_failed:${[...names].join(",")}`,
				);
			}
			const remainingRemovedRecord = bridge
				.readDispatchRecordsStrict()
		...
		const retained = requests.filter(
			(request) => !names.has(request.to_worker),
		);
		await writeAtomic(
			dispatchRequestsPath(teamName, cwd),
...
export async function readShutdownAck(
	...
	const ackPath = join(
		workerDir(teamName, workerName, cwd),
		"shutdown-ack.json",
	);
	try {
...
function normalizeTeamLeaderAttentionState(
	...
	return {
		team_name:
			typeof parsed.team_name === "string" && parsed.team_name.trim() !== ""
				? parsed.team_name
				: teamName,
		updated_at:
		...
		leader_attention_reason:
			typeof parsed.leader_attention_reason === "string" &&
			parsed.leader_attention_reason.trim() !== ""
		...
		leader_session_id:
			typeof parsed.leader_session_id === "string" &&
			parsed.leader_session_id.trim() !== ""
				? parsed.leader_session_id
				: null,
		leader_session_stopped_at:
			typeof parsed.leader_session_stopped_at === "string" &&
			parsed.leader_session_stopped_at.trim() !== ""
				? parsed.leader_session_stopped_at
				: null,
		unread_leader_message_count:
			typeof parsed.unread_leader_message_count === "number" &&
			Number.isFinite(parsed.unread_leader_message_count)
				? parsed.unread_leader_message_count
				: 0,
		work_remaining: parsed.work_remaining === true,
		stalled_for_ms:
			typeof parsed.stalled_for_ms === "number" &&
			Number.isFinite(parsed.stalled_for_ms)
...
export async function writeMonitorSnapshot(
	...
	await writeMonitorSnapshotImpl(
		teamName,
		snapshot,
		cwd,
		monitorSnapshotPath,
...
export async function readTeamLeaderAttention(
	...
	try {
		return normalizeTeamLeaderAttentionState(
			teamName,
...
> {
	...
	const workerNames =
		...
		Object.keys(snapshot?.workerStateByName ?? {});
	...
	const unreadLeaderMessageCount = mailbox.filter((message) => {
		const deliveredAt =
			typeof message.delivered_at === "string"
...
export async function markTeamLeaderSessionStopped(
	...
	return await markTeamLeaderStopObserved(
		teamName,
		cwd,
		leaderSessionId,
		nowIso,
...
export async function markOwnedTeamsLeaderSessionStopped(
	...
	return await markOwnedTeamsLeaderStopObserved(
		cwd,
		leaderSessionId,
		nowIso,
		"native_session_end",
...
export async function markOwnedTeamsLeaderStopObserved(
	...
	for (const entry of entries) {
		...
		await markTeamLeaderStopObserved(
			teamName,
			cwd,
			leaderSessionId,
			nowIso,
...
export async function saveTeamConfig(
	...
	await withTeamTaskBarrier(config.name, cwd, async () => {
		...
		if (expectedGeneration !== currentGeneration) {
			throw new Error(
				`team_config_stale_generation:${config.name}:${expectedGeneration}:${currentGeneration}`,
			);
		}
