 ...
 */
async function tryReadGitValue(
	cwd: string,
	args: string[],
): Promise<string | null> {
			...
			if (gitLayout) {
				...
				if (cmd.startsWith("rev-parse --git-path logs/refs/heads/")) {
					...
					if (
						...
						!candidate.startsWith(resolvedBase + sep)
					)
						return null;
					return candidate;
				}
...
async function tryReadGitValueExec(
	cwd: string,
	args: string[],
): Promise<string | null> {
	try {
...
export async function readBranchGitActivityMsForPath(
	cwd: string,
): Promise<number> {
	const gitDir = await tryReadGitValue(cwd, ["rev-parse", "--git-dir"]);
	if (!gitDir) return Number.NaN;

	const branch = await tryReadGitValue(cwd, [
		"symbolic-ref",
		"--quiet",
		"--short",
		"HEAD",
	]);
	const headLogPath = await tryReadGitValue(cwd, [
		"rev-parse",
		"--git-path",
		"logs/HEAD",
	]);
	const branchLogPath = branch
		? await tryReadGitValue(cwd, [
				"rev-parse",
				"--git-path",
				`logs/refs/heads/${branch}`,
			])
		: null;
	const headCommitEpoch = await tryReadGitValue(cwd, [
		"show",
		"-s",
		"--format=%ct",
		"HEAD",
	...
	const headCommitMs = headCommitEpoch
		? parseEpochSecondsMs(headCommitEpoch)
		: Number.NaN;

	const candidates = [headLogMs, branchLogMs, headCommitMs].filter((ms) =>
		Number.isFinite(ms),
	);
	return candidates.length > 0 ? Math.max(...candidates) : Number.NaN;
}

async function readLeaderBranchGitActivityMs(
	stateDir: string,
): Promise<number> {
...
export async function readLeaderRuntimeSignalStatuses(
	...
	const signals: Array<{
		...
		{ source: "leader_runtime_activity", at: leaderActivity?.last_activity_at },
		{
			...
			at: Number.isFinite(leaderGitActivityMs)
				? new Date(leaderGitActivityMs).toISOString()
				: null,
	...
	return signals.map(({ source, at, ms: providedMs }) => {
		const ms = Number.isFinite(providedMs)
			? Number(providedMs)
			: parseIsoMs(at);
...
export async function readLatestLeaderActivityMsFromStateDir(
	stateDir: string,
): Promise<number> {
	const statuses = await readLeaderRuntimeSignalStatuses(
		stateDir,
		Number.MAX_SAFE_INTEGER,
		Date.now(),
	);
	const validMs = statuses
		.filter((status) => status.valid)
		.map((status) => status.ms);
	return validMs.length > 0 ? Math.max(...validMs) : Number.NaN;
}

export async function isLeaderRuntimeStale(
	stateDir: string,
	thresholdMs: number,
	nowMs: number,
): Promise<boolean> {
