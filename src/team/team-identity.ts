...
export class TeamLookupAmbiguityError extends Error {
	...
	constructor(input: string, candidates: TeamLookupCandidate[]) {
		super(
			`ambiguous_team_name:${input}:${candidates.map((c) => c.teamName).join(",")}`,
		);
...
export function resolveTeamIdentityScope(
...
export function buildInternalTeamName(
	displayName: string,
	scope: TeamIdentityScope,
): string {
	const base = sanitizeBase(displayName);
	const identity =
		scope.sessionId ||
		(scope.tmuxTarget || scope.paneId
			? `${scope.tmuxTarget}:${scope.paneId}`
			: "") ||
		scope.runId;
	const suffix = shortHash(identity || randomUUID());
	const prefix =
		base.slice(0, Math.max(1, 30 - suffix.length - 1)).replace(/-$/, "") ||
		"team";
...
function candidateFromDir(
	...
	const leader =
		source.leader &&
		typeof source.leader === "object" &&
		!Array.isArray(source.leader)
	...
	return {
		...
		displayName,
		requestedName:
			str(source.requested_name) || str(config?.requested_name) || displayName,
...
function teamLookupRoots(
	...
	const explicit =
		typeof env.OMX_TEAM_STATE_ROOT === "string" ? env.OMX_TEAM_STATE_ROOT : "";
	addStateRoot(explicit);
	addStateRoot(join(resolve(cwd), ".omx", "state"));
	return roots;
}

export function listTeamLookupCandidates(
	cwd: string,
	env: NodeJS.ProcessEnv = process.env,
): TeamLookupCandidate[] {
...
function matchingCurrentLeader(
	candidate: TeamLookupCandidate,
	scope: TeamIdentityScope,
): boolean {
	return (
		(Boolean(scope.sessionId) &&
			candidate.leaderSessionId === scope.sessionId) ||
		(Boolean(scope.paneId) && candidate.leaderPaneId === scope.paneId)
	);
}

function selectLatestTerminalCandidate(
	candidates: TeamLookupCandidate[],
): TeamLookupCandidate | null {
	...
	const ranked = terminal
		.map((candidate) => ({
			candidate,
			time: Date.parse(candidate.phaseUpdatedAt),
		}))
		.filter((entry) => Number.isFinite(entry.time))
		.sort((a, b) => b.time - a.time);
	if (ranked.length === 0) return terminal.length === 1 ? terminal[0] : null;
	if (ranked.length === 1 || ranked[0].time > ranked[1].time)
...
export function resolveTeamNameForCurrentContext(
	...
	const current = lookupCandidates.filter((candidate) =>
		matchingCurrentLeader(candidate, scope),
	);
	...
	throw new TeamLookupAmbiguityError(
		input,
		current.length > 1 ? current : lookupCandidates,
	);
}
