...
function parsePaneLivenessLine(
	line: string,
...
async function readPaneLivenessByIdAsync(
	paneId: string,
...
export function buildReconcileHudResizeArgs(
	...
	return [
		...
		buildBestEffortShellCommand(
			buildNestedTmuxShellCommand(
				buildHudResizeCommand(hudPaneId, heightLines),
			),
		),
	];
}
				...
				if (id.startsWith("%")) {
					...
					if (isNativeWindows()) {
						...
						const registerHook = runTmux(
							buildRegisterResizeHookArgs(hookTarget, hookName, hudPaneId),
						);
						...
						const delayed = runTmux(
							buildScheduleDelayedHudResizeArgs(hudPaneId),
						);
						if (!delayed.ok) {
							console.warn(
...
export async function waitForWorkerReadyAsync(
	...
	const check = async (): Promise<boolean> => {
		...
		const scrollbackResult = await runTmuxAsync(
			sharedBuildCapturePaneArgv(target, 80),
		);
		if (!scrollbackResult.ok) return false;
		return paneLooksReady(scrollbackResult.stdout);
	};
