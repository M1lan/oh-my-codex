...
import {
	readExactPaneProof,
	readExactPaneProofsSync,
	readExactPaneProofSync,
	type ExactPaneProof,
} from "./exact-pane.js";
...
export class CreateTeamSessionPartialError extends Error {
	constructor(
		...
		readonly proofUnavailable: Array<
			Extract<ExactPaneProof, { status: "unavailable" }>
		>,
...
class ExactPaneProofUnavailableError extends Error {
	constructor(
		readonly proof: Extract<ExactPaneProof, { status: "unavailable" }>,
	) {
...
export function parseSplitWindowPaneId(
	stdout: string,
	...
	const match =
		expectedReceipt === undefined
			? /^(%\d+)(?:\r?\n)?$/.exec(stdout)
			: /^(%\d+)\t([^\r\n\t]+)(?:\r?\n)?$/.exec(stdout);
	if (!match || (expectedReceipt !== undefined && match[2] !== expectedReceipt))
		return null;
	return match[1];
}
...
function captureSourcePaneAuthority(
	paneId: string,
	expectedTeamPaneOwnerId?: string,
): SourcePaneAuthority {
	...
	if (proof.status !== "live" || proof.pid !== parsedPid)
		throw new Error(`tmux pane identity changed: ${target}`);
	const ownerResult = runTmuxStructured([
		"show-option",
		"-qv",
		"-p",
		"-t",
	...
	if (!ownerResult.ok)
		throw new Error(`failed to capture tmux pane owner: ${ownerResult.stderr}`);
	...
	if (
		expectedTeamPaneOwnerId !== undefined &&
		teamPaneOwnerId !== expectedTeamPaneOwnerId
	) {
...
function sourceAuthorityPredicate(source: SourcePaneAuthority): string {
	return [
		...
		...(source.teamPaneOwnerId
			? [`#{==:#{${OMX_TEAM_PANE_OWNER_OPTION}},${source.teamPaneOwnerId}}`]
			: []),
...
function bindSplitReceiptToPaneCommand(
	command: string,
	receipt: string,
): string {
...
function runSourceAuthorizedTmux(
	source: SourcePaneAuthority,
	effect: string,
	receipt: string = sourceTransactionReceipt(),
): string {
	...
	if (result.stdout !== receipt)
		throw new Error("tmux source authority changed before effect");
	return receipt;
}
...
function runSourceAuthorizedSplit(
	...
	const beforePaneIds = new Set(
		beforeTopology.panes.map((pane) => pane.paneId),
	);
	...
	if (!paneId) {
		...
		const newlyObservedPaneIds = afterTopology.error
			...
			: afterTopology.panes
					.filter(
						(pane) =>
							!beforePaneIds.has(pane.paneId) &&
							pane.startCommand.includes(receipt),
					)
					.map((pane) => pane.paneId);
		throw new AmbiguousSplitWindowOutputError(
			newlyObservedPaneIds,
			afterTopology.error,
		);
	}
	return paneId;
...
function restoredHudCleanupDebtPath(
	cwd: string,
	stateRoot?: string | null,
): string {
...
function parseRestoredHudCleanupDebtSync(
	cwd: string,
	stateRoot?: string | null,
): { path: string; debt: RestoredHudCleanupDebt } | null {
	const debtPath = restoredHudCleanupDebtPath(cwd, stateRoot);
	...
	if (
		debt.schema_version !== 1 ||
		debt.operation !== "restored_hud_cleanup" ||
		!hasExplicitWorkerPaneId(debt.pane_id) ||
		!hasExplicitWorkerPaneId(debt.leader_pane_id) ||
		!Number.isSafeInteger(debt.leader_pane_pid) ||
		debt.leader_pane_pid <= 0 ||
		!(
			typeof debt.leader_pane_owner_id === "string" ||
			debt.leader_pane_owner_id === null
		) ||
		debt.hud_owner_leader_pane_id !== debt.leader_pane_id ||
		(debt.pane_pid !== null &&
			(!Number.isSafeInteger(debt.pane_pid) || debt.pane_pid <= 0))
	) {
		throw new Error("restored_hud_cleanup_debt_malformed");
	}
 ...
 */
export function reconcileRestoredHudCleanupDebtSync(
	cwd: string,
	stateRoot?: string | null,
): void {
	...
	const hudMatches =
		!topology.error &&
		topology.panes.filter(
			(pane) =>
				pane.paneId === proof.paneId &&
				hudPaneMatchesOwner(pane, {
					leaderPaneId: debt.hud_owner_leader_pane_id,
				}),
		).length === 1;
	if (!hudMatches)
		throw new Error(`restored_hud_cleanup_debt_unresolved:${debt.pane_id}`);
	killExactPaneSync(proof.paneId, debt.pane_pid, () => {
		requireLiveTeamOwnedPaneSync(
			debt.leader_pane_id,
			debt.leader_pane_pid,
			debt.leader_pane_owner_id!,
		);
 ...
 */
function requireLiveExactPaneSync(
	paneId: string,
	expectedPid?: number,
): string {
...
function killExactPaneSync(
	...
	if (
		finalProof.pid !== proof.pid ||
		(expectedPid !== undefined && finalProof.pid !== expectedPid)
	) {
		throw new Error(`tmux pane identity changed: ${paneId}`);
	}
...
export function tagPaneTeamOwner(
	paneTarget: string,
	teamOwnerId: string,
	...
	if (!result.ok) {
		throw new Error(
			`failed to tag tmux pane ${provenTarget}: ${result.stderr}`,
		);
	}
}
...
export function hasCurrentTmuxClientContext(): boolean {
	...
	return Boolean(
		sessionName && windowIndex && detectedLeaderPaneId.startsWith("%"),
	);
}
...
function requireFrozenWindowTopologySync(
	...
	if (
		actualPaneIds.size !== expectedPanePids.size ||
	...
	for (const [index, [paneId, expectedPanePid]] of [
		...expectedPanePids.entries(),
	].entries()) {
	...
	if (finalTopology.error)
		throw new Error(
			`failed to read tmux pane topology: ${finalTopology.error}`,
		);
	const finalPaneIds = new Set(finalTopology.panes.map((pane) => pane.paneId));
	if (
		finalPaneIds.size !== expectedPanePids.size ||
...
function waitForPaneToRemainPresent(
	...
	while (Date.now() - startedAt <= timeoutMs) {
		...
		sleepFractionalSeconds(
			Math.max(0, Math.min(TMUX_PANE_STABILITY_POLL_MS, remaining)) / 1000,
		);
	}
...
function uniqueRestoreCwdCandidates(
	...
	for (const candidate of candidates) {
		const normalized =
			typeof candidate.rawPath === "string" ? candidate.rawPath.trim() : "";
...
function shouldAttemptRestoreCwdCandidate(
	candidate: RestoreCwdCandidate,
): boolean {
	if (
		candidate.source === "live" &&
		isMsysOrGitBash() &&
		isMsysDriveSlashPath(candidate.rawPath)
	) {
		return true;
	}
...
function hasExplicitWorkerPaneId(
	workerPaneId: string | undefined,
): workerPaneId is string {
...
function readExactWorkerPaneLivenessProofSync(
	paneId: string,
): ExactWorkerPaneLivenessProof {
	if (!/^%[0-9]+$/.test(paneId))
		return { status: "unavailable", paneId, reason: "invalid_pane_id" };
	const result = runTmuxStructured([
		"display-message",
		"-p",
		"-t",
	...
	if (!result.ok) {
		...
		return {
			status: "unavailable",
			paneId,
			reason: `query_failed:${result.stderr || "unknown"}`,
		};
	}
	...
	if (lines.length !== 1)
		return {
			status: "unavailable",
			paneId,
			reason: "malformed_exact_pane_result",
		};
	...
	if (actualPaneId !== paneId) {
		return {
			status: "unavailable",
			paneId,
			reason: `pane_id_changed:expected=${paneId}:actual=${actualPaneId}`,
		};
	}
...
function requireExactWorkerPaneLivenessIdentity(
	...
	if (
		...
		proof.reason.startsWith("pane_id_changed:")
	) {
		throw new Error(
			`tmux worker pane identity changed: ${paneId}: ${proof.reason}`,
		);
	}
	if (proof.status !== "live") return proof;
	if (typeof expectedPanePid === "number" && proof.pid !== expectedPanePid) {
		throw new Error(
			`tmux worker pane PID changed: ${paneId}: expected ${expectedPanePid}, got ${proof.pid}`,
		);
	}
	const expectedOwner =
		typeof expectedTeamOwnerId === "string" ? expectedTeamOwnerId.trim() : "";
	if (expectedOwner && proof.ownerId !== expectedOwner) {
		throw new Error(
			`tmux worker pane incarnation changed: ${paneId}: expected ${expectedOwner}, got ${proof.ownerId || "missing"}`,
		);
	}
	return proof;
}
...
function createPinnedWorkerPaneTargetResolverSync(
	...
	if (!hasExplicitWorkerPaneId(workerPaneId)) {
		return () =>
			resolveWorkerPaneTargetSync(sessionName, workerIndex, workerPaneId);
	}

	const pinnedPid = expectedPanePid;
	const expectedOwner =
		typeof expectedTeamOwnerId === "string" ? expectedTeamOwnerId.trim() : "";
	const canonicalHudPaneId =
		typeof hudPaneId === "string" ? hudPaneId.trim() : "";
	return () => {
		if (
			typeof pinnedPid !== "number" ||
			!Number.isSafeInteger(pinnedPid) ||
			pinnedPid <= 0
		)
			return null;
...
function createPinnedWorkerPaneTargetResolver(
	...
	if (!hasExplicitWorkerPaneId(workerPaneId)) {
		return () =>
			resolveWorkerPaneTargetAsync(sessionName, workerIndex, workerPaneId);
	}
...
async function requireAsyncPaneTarget(
...
async function captureVisiblePaneAsync(
	resolveTarget: AsyncPaneTargetResolver,
): Promise<string> {
	const target = await resolveTarget();
	if (!target) return "";
...
async function isWorkerAliveAsync(
	...
	if (hasExplicitWorkerPaneId(workerPaneId)) {
		const resolver =
			resolveTarget ??
			createPinnedWorkerPaneTargetResolver(
				sessionName,
				workerIndex,
				workerPaneId,
				expectedPanePid,
...
function formatHudEnvAssignments(
	...
	const assignments = [
		...
		leaderPaneId
			? `${OMX_TMUX_HUD_LEADER_PANE_ENV}=${shellQuoteSingle(leaderPaneId)}`
			: "",
...
function resolveNativeWindowsPowerShellPath(
	env: NodeJS.ProcessEnv = process.env,
): string {
	...
	const resolvedFromPath = resolveCommandPathForPlatform(
		"powershell",
		process.platform,
		env,
	);
...
function buildHudResizeArgs(
	...
	return [
		"resize-pane",
		"-t",
		buildHudPaneTarget(hudPaneId),
		"-y",
		String(resolveHudHeightLines(heightLines)),
	];
}
...
function buildAuthoritativeHudResizeShellCommand(
	...
	effectCommand: string = buildNestedTmuxShellCommand(
		buildHudResizeCommand(buildHudPaneTarget(hudPaneId), heightLines),
	),
): string {
	const target = buildHudPaneTarget(hudPaneId);
	const snapshot = buildNestedTmuxShellCommand(
...
export function buildRegisterResizeHookArgs(
	...
	const resizeCommand = buildBestEffortShellCommand(
		buildAuthoritativeHudResizeShellCommand(
			hudPaneId,
			heightLines,
			expectedPanePid,
			expectedPaneOwnerId,
...
export function buildRegisterClientAttachedReconcileArgs(
	...
	const unregisterCommand = buildBestEffortShellCommand(
		buildAuthoritativeHudResizeShellCommand(
			hudPaneId,
			heightLines,
			expectedPanePid,
	...
	const oneShotCommand = shellQuoteSingle(
		`${resizeCommand}; ${unregisterCommand}`,
	);
	return [
		"set-hook",
		"-t",
...
export function buildScheduleDelayedHudResizeArgs(
	...
	return [
		"run-shell",
		"-b",
		`sleep ${delay}; ${buildBestEffortShellCommand(buildAuthoritativeHudResizeShellCommand(hudPaneId, heightLines, expectedPanePid, expectedPaneOwnerId))}`,
	];
}
...
function redrawLeaderPaneAfterTeamLayout(
	...
	const provenTarget = requireLiveTeamOwnedPaneSync(
		source.paneId,
		source.panePid,
		expectedTeamOwnerId,
	);
	if (provenTarget !== source.paneId)
		throw new Error(`tmux pane identity changed: ${source.paneId}`);
...
function resolveSupportedShellAffinity(
...
function resolveShellFromCandidates(
	paths: string[],
	rcFile: string,
): WorkerLaunchSpec | null {
...
function buildWorkerLaunchSpec(
	...
	const bashSpec = resolveShellFromCandidates(
...
function hasModelInstructionsOverride(args: readonly string[]): boolean {
	return someConfigOverrideBeforeEndOfOptions(
		args,
		isModelInstructionsOverride,
	);
}
...
export function resolveTeamWorkerCliForResolvedLaunchArgs(
	...
	if (
		...
		throw new Error(
			`workerIndex must be within 1..${workerCount} (got ${workerIndex})`,
		);
	}
	...
	const entry =
		entries.length === 1 ? (entries[0] as string) : entries[workerIndex - 1];
...
export function assertTeamWorkerCliPolicyCompatibility(
	workerCli: TeamWorkerCli,
	launchArgs: string[],
): void {
	...
	if (
		...
		(policy === "direct-policy" || policy === "mixed-policy")
	) {
		throw new Error(
...
export function assertTeamWorkerLaunchPolicyInvariant(
	workerCli: TeamWorkerCli,
	launchArgs: string[],
): void {
	...
	if (
		workerCli === "codex" &&
		classifyTeamWorkerLaunchPolicy(launchArgs) === "mixed-policy"
	) {
...
export function translateWorkerLaunchArgsForCli(
	...
	void args;
	return shouldGrantExecutionBypassForRole(workerRole)
		? [CLAUDE_SKIP_PERMISSIONS_FLAG]
		: [];
}
...
export function trustWorkerMiseConfigIfAvailable(workerCwd: string): boolean {
	...
	if (result.error || result.status !== 0) {
		const reason =
			...
			`mise exited ${result.status}`;
		console.warn(
			`[omx] mise trust failed for team worker config ${miseConfigPath}: ${reason}; continuing.`,
		);
		return false;
	}
	return true;
...
function appendTeamWorkerMcpDisableOverrides(
	args: string[],
	env: NodeJS.ProcessEnv,
): void {
	...
	for (const server of TEAM_WORKER_DISABLED_OMX_MCP_SERVERS) {
		...
		args.splice(
			endOfOptionsIndex < 0 ? args.length : endOfOptionsIndex,
			0,
			CONFIG_FLAG,
			`${key}=false`,
		);
...
const CODEX_BYPASS_MDM_REJECTED_MARKER =
...
export function classifyCodexBypassMdmIncompatibility(
	...
	if (
		workerCli !== "codex" ||
		!finalArgs ||
		!parseTeamWorkerLaunchArgs([...finalArgs], "final tmux worker arguments", {
			directPolicyMode: "ignore",
		}).wantsBypass
	)
		return null;
...
function buildAttachedTmuxWorkerStartupPlan(
	...
	if (processSpec.workerCli === "codex")
		appendTeamWorkerMcpDisableOverrides(startupArgs, {
			...process.env,
			...extraEnv,
		});
	return Object.freeze({
		processSpec,
		startupArgs: Object.freeze(startupArgs),
	});
}

function insertArgsBeforeEndOfOptions(
	args: string[],
	...
	return [
		...args.slice(0, endOfOptionsIndex),
		...insertedArgs,
		...args.slice(endOfOptionsIndex),
	];
}

function insertCanonicalCodexBypassBeforeEndOfOptions(
	args: string[],
): string[] {
	const endOfOptionsIndex = args.indexOf("--");
	const preMarkerArgs =
		endOfOptionsIndex < 0 ? args : args.slice(0, endOfOptionsIndex);
	const postMarkerArgs =
		endOfOptionsIndex < 0 ? [] : args.slice(endOfOptionsIndex);
	return [
		...preMarkerArgs.filter(
			(arg) => arg !== CODEX_BYPASS_FLAG && arg !== MADMAX_FLAG,
		),
		CODEX_BYPASS_FLAG,
		...postMarkerArgs,
	];
...
function resolveWorkerLaunchArgs(
	...
	const ambientWantsBypass = parseTeamWorkerLaunchArgs(
		process.argv,
		"ambient process arguments",
		{ directPolicyMode: "ignore" },
	).wantsBypass;
	...
	if (
		...
		merged = insertArgsBeforeEndOfOptions(merged, [
			CONFIG_FLAG,
			buildModelInstructionsOverride(cwd, env),
		]);
	}
	return merged;
}
...
export function buildWorkerStartupCommand(
	...
	const resolvedLeaderNodePath =
		processSpec.env[OMX_LEADER_NODE_PATH_ENV]?.trim() ||
		resolveLeaderNodePath();
	...
	if (isNativeWindows()) {
		...
		const invocation = [
			...
			...startupArgs.map(quotePowerShellArg),
		].join(" ");
	...
	const pathPrefix = leaderNodeDir
		? `export PATH=${shellQuoteSingle(leaderNodeDir)}:$PATH; `
		: "";
	const quotedArgs = startupArgs
		.map((arg) => shellQuoteSingle(translatePathForMsys(arg)))
		.join(" ");
	const quotedCommand = shellQuoteSingle(
		translatePathForMsys(processSpec.command),
	);
	const cliInvocation =
		quotedArgs.length > 0
			? `exec ${quotedCommand} ${quotedArgs}`
			: `exec ${quotedCommand}`;
...
export function writeWorkerStartupScriptCommand(
	...
	const startupPlan =
		attachedPlan ??
		buildAttachedTmuxWorkerStartupPlan(
			teamName,
			workerIndex,
			launchArgs,
			cwd,
	...
	writeFileSync(
		scriptPath,
		buildWorkerStartupScriptContent(
			startupPlan.processSpec,
			startupEnv,
			[...startupPlan.startupArgs],
			cwd,
			extraEnv,
		),
		"utf-8",
...
function buildWorkerProcessLaunchSpecForMode(
	...
	const effectiveCliLaunchArgs =
		...
		launchPolicy === "none"
			...
			: cliLaunchArgs;
	const workerCodexHomeOverride =
		typeof effectiveEnv.CODEX_HOME === "string"
			? effectiveEnv.CODEX_HOME.trim()
			: undefined;
	const workerSqliteHomeOverride =
		typeof effectiveEnv[CODEX_SQLITE_HOME_ENV] === "string"
			? effectiveEnv[CODEX_SQLITE_HOME_ENV].trim()
			: undefined;
	const providerLookupCodexHome = workerCodexHomeOverride
		? isAbsolute(workerCodexHomeOverride)
			? workerCodexHomeOverride
			: resolve(cwd, workerCodexHomeOverride)
		: undefined;

	const resolvedCliPath = resolveAbsoluteBinaryPath(workerCli);
	const shouldUseNativeWindowsLaunchSpec =
		process.platform === "win32" &&
		(mode === "direct-spawn" ||
			!isMsysOrGitBash(effectiveEnv, process.platform));

	const platformSpec = shouldUseNativeWindowsLaunchSpec
		? buildPlatformCommandSpec(
				workerCli,
				effectiveCliLaunchArgs,
				process.platform,
				effectiveEnv,
			)
		: {
	...
	const displayTeamName =
		typeof extraEnv.OMX_TEAM_DISPLAY_NAME === "string"
...
export function createTeamSession(
	...
	const workerCliPlan = Array.from(
		{ length: workerCount },
		(_, index) =>
			workerStartups[index]?.workerCli ?? defaultWorkerCliPlan[index]!,
	);
	for (const workerCli of new Set(workerCliPlan)) {
		assertTeamWorkerCliBinaryAvailable(workerCli);
	}
	const workerLaunchPolicyPlan = Array.from(
		{ length: workerCount },
		(_, index) => {
			...
			assertTeamWorkerLaunchPolicyInvariant(
				workerCliPlan[index]!,
				effectiveLaunchArgs,
			);
	...
	const partialWorkerPaneIdsByIndex: Array<string | null> = Array.from(
		{ length: workerCount },
		() => null,
	);
	const partialWorkerPanePidsByIndex: Array<number | null> = Array.from(
		{ length: workerCount },
		() => null,
	);
	...
	try {
		...
		const teamPaneOwnerId = (
			options.teamPaneOwnerId ?? `team:${safeTeamName}`
		).trim();
		partialTeamPaneOwnerId = teamPaneOwnerId;
		const paneListResult = listPanesResult(teamTarget);
		if (paneListResult.error)
			throw new Error(
				`failed to read tmux pane topology: ${paneListResult.error}`,
			);
		const leaderPaneId = chooseTeamLeaderPaneId(
		...
		if (
			leaderSource.sessionName !== sessionName ||
			leaderSource.windowIndex !== windowIndex ||
			leaderSource.panePid !== leaderProof.pid
		) {
		...
		const initialHudPaneIds = findHudWatchPaneIds(
			paneListResult.panes,
			leaderPaneId,
			{ leaderPaneId },
		);
		const initialWindowPanePids = new Map<string, number>([
			[leaderPaneId, leaderPanePid],
		]);
		...
		const exactProveAndTagReconciledPane = (paneId: string): number | null => {
			...
			if (paneSource.panePid !== proof.pid)
				throw new Error(`tmux pane identity changed: ${paneId}`);
			if (
				paneSource.teamPaneOwnerId &&
				paneSource.teamPaneOwnerId !== teamPaneOwnerId
			) {
		...
		const workerStartupArgs: Array<ReadonlyArray<string>> = Array.from(
			{ length: workerCount },
			() => Object.freeze([]),
		);
		const workerPanePidsByIndex: Array<number | null> = Array.from(
			{ length: workerCount },
			() => null,
		);
		let rightStackRootPaneId: string | null = null;
		let rightStackRootPanePid: number | null = null;
		const frozenWindowPanePids = new Map<string, number>([
			[leaderPaneId, leaderPanePid],
		]);
		const frozenWindowPaneOwners = new Map<string, string>([
			[leaderPaneId, teamPaneOwnerId],
		]);
		...
		for (let i = 1; i <= workerCount; i++) {
			...
			const launchArgsForWorker = [
				...(workerLaunchPolicyPlan[i - 1] ?? workerLaunchArgs),
			];
			const attachedStartupPlan = buildAttachedTmuxWorkerStartupPlan(
				safeTeamName,
				i,
				launchArgsForWorker,
				workerCwd,
			...
			const splitSource = captureSourcePaneAuthority(
				splitTarget,
				teamPaneOwnerId,
			);
			let paneId: string;
			try {
				paneId = runSourceAuthorizedSplit(
					splitSource,
					(receipt) =>
						`split-window ${splitDirection} -t ${splitTarget} -d -P -F '#{pane_id}' -c ${shellQuoteSingle(tmuxWorkerCwd)} ${shellQuoteSingle(bindSplitReceiptToPaneCommand(cmd, receipt))}`,
				);
			} catch (error) {
				if (error instanceof AmbiguousSplitWindowOutputError) {
					...
					for (const candidatePaneId of error.newlyObservedPaneIds) {
						...
						if (isUnassignedAmbiguity)
							partialStartupCleanupPanePids.set(candidatePaneId, null);
					}
					...
					for (const candidatePaneId of error.newlyObservedPaneIds) {
						const reconciledPid =
							exactProveAndTagReconciledPane(candidatePaneId);
						if (reconciledPid === null) {
							rollbackPanes.delete(candidatePaneId);
							partialWorkerPaneIds.splice(
								partialWorkerPaneIds.lastIndexOf(candidatePaneId),
								1,
							);
			...
			if (
				isNativeWindows() &&
				!waitForPaneToRemainPresent(teamTarget, paneId)
			) {
				throw new Error(
					`worker pane ${i} did not remain present after tmux split-window returned ${paneId}`,
				);
			}
			const paneSource = captureSourcePaneAuthority(paneId);
			if (paneSource.panePid !== panePid)
				throw new Error(`tmux pane identity changed: ${paneId}`);
			if (ownerSessionId) {
				runSourceAuthorizedTmux(
					paneSource,
		...
		requireFrozenWindowTopologySync(
			teamTarget,
			frozenWindowPanePids,
			frozenWindowPaneOwners,
		);
		runSourceAuthorizedTmux(
			leaderSource,
			`select-layout -t ${leaderSource.windowId} main-vertical`,
		);
		...
		if (windowWidthResult.ok) {
			...
			if (Number.isFinite(width) && width >= 40) {
				...
				runSourceAuthorizedTmux(
					leaderSource,
					`set-window-option -t ${leaderSource.windowId} main-pane-width ${half}`,
				);
				requireFrozenWindowTopologySync(
					teamTarget,
					frozenWindowPanePids,
		...
		if (canRecreateTeamHud && omxEntry) {
			...
			const hudSource = captureSourcePaneAuthority(
				hudSplitTarget,
				teamPaneOwnerId,
			);
			let id: string;
			try {
				id = runSourceAuthorizedSplit(
					hudSource,
					(receipt) =>
						`split-window -v -f -l ${HUD_TMUX_TEAM_HEIGHT_LINES} -t ${hudSplitTarget} -d -P -F '#{pane_id}' -c ${shellQuoteSingle(hudCwd)} ${shellQuoteSingle(bindSplitReceiptToPaneCommand(hudCmd, receipt))}`,
				);
			} catch (error) {
				if (error instanceof AmbiguousSplitWindowOutputError) {
					const isUnassignedAmbiguity = error.newlyObservedPaneIds.length > 1;
			...
			}
			{
				...
				if (isNativeWindows() && !waitForPaneToRemainPresent(teamTarget, id)) {
					throw new Error(
						`HUD pane did not remain present after tmux split-window returned ${id}`,
					);
				}
				const hudSource = captureSourcePaneAuthority(id);
				if (hudSource.panePid !== hudPanePid)
					throw new Error(`tmux pane identity changed: ${id}`);
				if (ownerSessionId) {
					runSourceAuthorizedTmux(
						hudSource,
				...
				if (isNativeWindows()) {
					const provenHudPaneId = requireLiveTeamOwnedPaneSync(
						hudPaneId,
						hudPanePid,
						teamPaneOwnerId,
					);
					...
					if (!delayed.ok) {
						console.warn(
							`[omx] tmux delayed HUD resize unavailable for ${hudPaneId}: ${delayed.stderr}; continuing.`,
						);
					}
					requireLiveTeamOwnedPaneSync(hudPaneId, hudPanePid, teamPaneOwnerId);
					const reconcile = runTmux(
					...
					if (!reconcile.ok) {
						console.warn(
							`[omx] tmux HUD resize reconcile unavailable for ${hudPaneId}: ${reconcile.stderr}; continuing.`,
						);
					}
				}
			}
		...
		const focusedLeader = requireLiveTeamOwnedPaneSync(
			leaderPaneId,
			leaderPanePid,
			teamPaneOwnerId,
		);
		if (focusedLeader !== leaderSource.paneId)
			throw new Error(`tmux pane identity changed: ${leaderPaneId}`);
		...
		if (
			error instanceof AmbiguousSplitWindowOutputError &&
		...
		const unregisterAuthorizedHook = (
			...
			try {
				...
				cleanupErrors.push(
					`failed to unregister tmux ${label} hook ${hook.name}: ${unregistered.stderr}`,
				);
			} catch (cleanupError) {
				cleanupErrors.push(
					`unable to authorize tmux ${label} hook cleanup ${hook.name}: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`,
				);
			}
			return false;
		};
		...
		const unresolvedWorkerPaneIds = partialWorkerPaneIds.filter((paneId) =>
			unresolvedPaneIds.has(paneId),
		);
		const unresolvedWorkerPaneIdsByIndex = partialWorkerPaneIdsByIndex.map(
			(paneId) => (paneId && unresolvedPaneIds.has(paneId) ? paneId : null),
		);
		const unresolvedWorkerPanePidsByIndex = partialWorkerPanePidsByIndex.map(
			(panePid, index) =>
				partialWorkerPaneIdsByIndex[index] &&
				unresolvedPaneIds.has(partialWorkerPaneIdsByIndex[index]!)
					? panePid
					: null,
		);
		const unresolvedStartupCleanupPanes = [
			...partialStartupCleanupPanePids.entries(),
		]
			.filter(([paneId]) => unresolvedPaneIds.has(paneId))
			.map(([paneId, panePid]) => ({ paneId, panePid }));

		const unresolvedHudPaneId =
			partialHudPaneId &&
			(unresolvedPaneIds.has(partialHudPaneId) ||
				...
				? partialHudPaneId
				: null;
		const hasRecoverablePartialArtifact =
			unresolvedPaneIds.size > 0 ||
			registeredResizeHook !== null ||
			registeredClientAttachedHook !== null ||
			(error instanceof AmbiguousSplitWindowOutputError &&
...
export function restoreStandaloneHudPane(
	...
	const leaderPanePid = (() => {
		...
		if (
			options.expectedLeaderPanePid !== undefined &&
			proof.pid !== options.expectedLeaderPanePid
		) {
	...
	const requireAuthorizedLeaderPane = (): string => {
		...
		return requireLiveExactPaneSync(
			normalizedLeaderPaneId,
			options.expectedLeaderPanePid ?? leaderPanePid,
		);
	};
	...
	const beforeSplitPaneIds = new Set(
		paneListResult.panes.map((pane) => pane.paneId),
	);
	...
	if (!paneId) {
		...
		const newlyObservedPaneIds = afterTopology.panes
			.filter(
				(pane) =>
					!beforeSplitPaneIds.has(pane.paneId) &&
					pane.startCommand.includes(splitReceipt),
			)
			.map((pane) => pane.paneId);
		if (newlyObservedPaneIds.length === 1) {
			const reconciledPaneId = newlyObservedPaneIds[0]!;
			persistRestoredHudCleanupDebtSync(
				cwd,
				{
					schema_version: 1,
					operation: "restored_hud_cleanup",
					...
					leader_pane_owner_id:
						options.expectedLeaderPaneOwnerId?.trim() || null,
			...
			if (reconciledProof.status === "gone")
				throw new Error(`tmux pane is not proven live: ${reconciledPaneId}`);
			persistRestoredHudCleanupDebtSync(
				cwd,
				{
					schema_version: 1,
...
export type StartupDirectTriggerSafety =
	...
	| {
			...
			reason:
				...
				| "bootstrapping"
				| "not_agent_viewport"
				| typeof CODEX_BYPASS_MDM_INCOMPATIBLE_REASON;
	  };
...
export function evaluateStartupDirectTriggerSafetyCapture(
	...
	const incompatibleReason = classifyCodexBypassMdmIncompatibility(
		captured,
		workerCli,
		finalArgs,
	);
...
export async function evaluateStartupDirectTriggerSafety(
	...
	const target = await resolveWorkerPaneTargetAsync(
		sessionName,
		workerIndex,
		workerPaneId,
	);
	...
	return evaluateStartupDirectTriggerSafetyCapture(
		result.stdout,
		workerCli,
		finalArgs,
	);
...
async function dismissClaudeBypassPermissionsPromptIfPresentAsync(
	...
	if (!literalTarget) return false;
	const literalSend = await runTmuxAsync([
		"send-keys",
		"-t",
		literalTarget,
		"-l",
	...
	if (!submitTarget) return false;
	const submitSend = await runTmuxAsync([
		"send-keys",
		"-t",
		submitTarget,
		"C-m",
...
export function classifyWorkerStartupInjectSafety(
	captured: string,
): WorkerStartupInjectSafety {
...
export async function checkWorkerStartupInjectSafety(
	...
	| { safe: false; reason: Exclude<WorkerStartupInjectSafety, "safe"> }
> {
	const resolveTarget = (): Promise<string | null> =>
		resolveWorkerPaneTargetAsync(sessionName, workerIndex, workerPaneId);
...
async function sendLiteralTextOrThrow(
	resolveTarget: AsyncPaneTargetResolver,
	text: string,
): Promise<void> {
	const target = await requireAsyncPaneTarget(resolveTarget);
	const send = await runTmuxAsync([
		"send-keys",
		"-t",
		target,
		"-l",
...
function paneHasQueuedCodexSubmission(
...
export function waitForWorkerReady(
	...
	const resolveTarget = createPinnedWorkerPaneTargetResolverSync(
		sessionName,
		workerIndex,
		workerPaneId,
		expectedPanePid,
	...
	const check = (): boolean => {
		...
		if (!result.ok) return false;
		if (
			dismissClaudeBypassPermissionsPromptIfPresent(
				resolveTarget,
				result.stdout,
			)
		) {
		...
		const scrollbackResult = runTmux(
			sharedBuildCapturePaneArgv(scrollbackTarget, 80),
		);
...
export async function waitForWorkerReadyDetailedAsync(
	...
	incompatibility?: {
		...
		finalArgs?: readonly string[];
		onIncompatible: (
			reason: typeof CODEX_BYPASS_MDM_INCOMPATIBLE_REASON,
		) => void;
	},
): Promise<boolean> {
	const initialBackoffMs = 150;
	...
	const check = async (): Promise<boolean> => {
		...
		if (!target) return false;
		const result = await runTmuxAsync(
			sharedBuildVisibleCapturePaneArgv(target),
		);
		if (!result.ok) return false;
		const incompatibleReason = classifyCodexBypassMdmIncompatibility(
			result.stdout,
			incompatibility?.workerCli,
			incompatibility?.finalArgs,
		);
		...
		if (
			await dismissClaudeBypassPermissionsPromptIfPresentAsync(
				resolveTarget,
				result.stdout,
			)
		) {
		...
		if (!scrollbackTarget) return false;
		const scrollbackResult = await runTmuxAsync(
			sharedBuildCapturePaneArgv(scrollbackTarget, 80),
		);
		if (!scrollbackResult.ok) return false;
		return paneLooksReady(scrollbackResult.stdout);
...
export async function waitForWorkerReadyAsync(
	...
	return waitForWorkerReadyDetailedAsync(
		sessionName,
		workerIndex,
		timeoutMs,
		workerPaneId,
...
function normalizeWorkerTriggerForDraftMatch(
...
export async function sendToWorker(
	...
	if (verifyCapture) {
		...
		if (
			!normalizeWorkerTriggerForDraftMatch(verifyCapture).includes(
				normalizeWorkerTriggerForDraftMatch(text),
			) &&
		...
		if (
			normalizeWorkerTriggerForDraftMatch(finalCapture).includes(
				normalizeWorkerTriggerForDraftMatch(text),
			) &&
...
export function getWorkerPanePid(
	...
	if (hasExplicitWorkerPaneId(workerPaneId)) {
		const proof = requireExactWorkerPaneLivenessIdentity(
			workerPaneId,
			expectedPanePid,
			expectedTeamOwnerId,
			hudPaneId,
...
export function isWorkerAlive(
	...
	if (hasExplicitWorkerPaneId(workerPaneId)) {
		...
		if (
			...
			return true;
		if (
			typeof expectedTeamOwnerId !== "string" ||
...
export function isWorkerPaneOpen(
	...
	if (hasExplicitWorkerPaneId(workerPaneId)) {
		return (
			createPinnedWorkerPaneTargetResolverSync(
				sessionName,
				workerIndex,
				workerPaneId,
				expectedPanePid,
...
export function killWorkerByPaneId(
	...
	const expectedOwner =
		typeof expectedTeamOwnerId === "string" ? expectedTeamOwnerId.trim() : "";
	if (
		!hasExplicitWorkerPaneId(workerPaneId) ||
		typeof expectedPanePid !== "number" ||
		!Number.isSafeInteger(expectedPanePid) ||
		expectedPanePid <= 0 ||
		!expectedOwner ||
...
export function paneHasOmxInstanceTag(
	...
	const result = runTmux([
		...
		"-t",
		normalizedPaneId,
		OMX_PANE_INSTANCE_OPTION,
	]);
	if (!result.ok) return false;
	return result.stdout.trim() === expectedInstanceId;
}

export function paneHasOmxTeamOwnerTag(
	paneId: string | null | undefined,
	teamOwnerId: string | null | undefined,
): boolean {
	const expectedTeamOwnerId =
		typeof teamOwnerId === "string" ? teamOwnerId.trim() : "";
...
export function readPaneTeamOwnerTag(
	paneId: string | null | undefined,
): string | null {
	const result = readPaneTeamOwnerTagResult(paneId);
...
export function readPaneTeamOwnerTagResult(
	paneId: string | null | undefined,
): PaneTeamOwnerTagReadResult {
	const normalizedPaneId = normalizePaneTarget(paneId);
	if (!normalizedPaneId)
	...
	return {
		status: "error",
		error: stderr || `tmux show-option exited ${result.status ?? "unknown"}`,
	};
}
...
export interface PaneTeardownOptions {
	...
	authorizePaneKill?: (
		paneId: string,
		proof: Extract<ExactPaneProof, { status: "live" }>,
	) => boolean;
}
...
export function resolveSharedSessionShutdownTopology(
	...
	const leaderOwnedHudPaneIds = resolvedLeaderPaneId
		? panes
				.filter((pane) => pane.paneId !== resolvedLeaderPaneId)
				.filter((pane) =>
					hudPaneMatchesOwner(pane, { leaderPaneId: resolvedLeaderPaneId }),
				)
				.map((pane) => pane.paneId)
				.filter((paneId) => paneId.startsWith("%"))
		: [];
...
function chooseSharedSessionShutdownLeaderPaneId(
	...
	if (
		preferred &&
		!isHudWatchPane(preferred) &&
		!teamWorkerPaneIds.has(preferred.paneId)
	) {
...
function paneLooksLikeTeamWorkerPane(
	pane: TmuxPaneInfo,
	teamName: string,
): boolean {
	const command =
		`${pane.startCommand || ""} ${pane.currentCommand || ""}`.replace(
			/\\/g,
			"/",
		);
	if (!command.trim() || !teamName) return false;
	if (
		command.includes(`/team/${teamName}/runtime/worker-`) &&
		command.includes("-startup.sh")
	) {
		return true;
	}
	const commandVariants = [
		command,
		...decodePowerShellEncodedCommands(command),
	];
	return commandVariants.some(
		(candidate) =>
			commandHasTeamWorkerEnvMarker(
				candidate,
...
function commandHasTeamWorkerEnvMarker(
	command: string,
	envName: string,
	teamName: string,
): boolean {
	...
	const powerShellAssignment = new RegExp(
		`(?:^|;)\\s*\\$env:${key}\\s*=\\s*["']?${workerValue}`,
		"gi",
	);
	return (
...
function shellTailInvokesWorkerCli(tail: string): boolean {
	...
	const directCliPattern = new RegExp(
		`^(?:exec\\s+)?${WORKER_CLI_TOKEN_PATTERN}(?:[\\s;"'\`]|$)`,
		"i",
	);
	...
	for (const match of directTail.matchAll(shellCommandPattern)) {
		...
		const execCliPattern = new RegExp(
			`(?:^|[\\s;&|])exec\\s+${WORKER_CLI_TOKEN_PATTERN}(?:[\\s;"'\`]|$)`,
			"i",
		);
...
function powerShellTailInvokesWorkerCli(tail: string): boolean {
	return /(?:^|[\s;&|"'`\/\\])(?:codex|claude|gemini)(?:\.(?:js|mjs|cjs|cmd|exe|bat|ps1))?(?:[\s;"'`]|$)/i.test(
		tail,
	);
}

function hasSafeShellEnvAssignmentContext(
	command: string,
	matchIndex: number,
): boolean {
...
function decodePowerShellEncodedCommands(command: string): string[] {
	...
	const encodedCommandPattern =
		/(?:^|\s)-(?:EncodedCommand|enc|e)(?:\s+|:)([A-Za-z0-9+/=]+)/gi;
...
export async function teardownWorkerPanes(
	...
	for (const paneId of killablePaneIds) {
		...
		if (
			options.authorizePaneKill &&
			!options.authorizePaneKill(proof.paneId, proof)
		) {
			summary.proofUnavailable.push({
				status: "unavailable",
				paneId: proof.paneId,
...
export function queryDetachedSessionLeaderBinding(
	...
	const result = runTmuxStructured([
		...
		"-a",
		"-F",
		"#{pane_id}\t#{pane_dead}\t#{pane_pid}\t#{session_name}\t#{session_id}\t#{session_created}",
	]);
	if (!result.ok) return null;
	...
	for (const line of result.stdout.split("\n").filter(Boolean)) {
		...
		if (
			!/^%[0-9]+$/.test(currentPaneId) ||
			seenPaneIds.has(currentPaneId) ||
			(dead !== "0" && dead !== "1") ||
			!/^[0-9]+$/.test(panePid) ||
			!Number.isSafeInteger(Number(panePid)) ||
			Number(panePid) <= 0 ||
			!currentSessionName ||
			!/^\$[0-9]+$/.test(sessionId) ||
			!/^[0-9]+$/.test(sessionCreated)
		...
		if (
			...
			sessionCreated !== expected.sessionCreated
		)
			return null;
		binding = {
			...
			incarnation: { sessionId, sessionCreated },
		};
	}
	return binding;
}
...
function parseDetachedSessionIncarnation(
	stdout: string,
	sessionName: string,
): DetachedSessionIncarnation | null | "malformed" {
...
export function queryDetachedTeamSession(
	...
	const sessions = runTmuxStructured([
		"list-sessions",
		"-F",
	...
	const incarnation = parseDetachedSessionIncarnation(
		sessions.stdout,
		sessionName,
	);
	...
	if (
		expected &&
		(incarnation.sessionId !== expected.sessionId ||
			incarnation.sessionCreated !== expected.sessionCreated)
	) {
 ...
 */
function buildDetachedSessionDestroyArgs(
	...
	return [
		...
		leaderPaneId,
		predicate,
		`kill-session -t ${expected.sessionId}`,
		'run-shell "exit 1"',
	];
}
...
export function requestDetachedTeamSessionDestroy(
	...
	if (
		!queryDetachedSessionLeaderBinding(
			expected.leaderPaneId,
			expected.leaderPanePid,
			sessionName,
			incarnation,
		)
	)
...
export function destroyTeamSession(sessionName: string): boolean {
	...
	return !parseTeamSessionNames(sessions.stdout).includes(
