...
async function withMailboxCompatHoleRuntime<T>(
	cwd: string,
	fn: () => Promise<T>,
): Promise<T> {
...
async function markTeamSessionAuthoritativelyAbsent(
	teamName: string,
	cwd: string,
): Promise<void> {
...
describe("buildLegacyTeamDeprecationHint", () => {
	it("produces CLI hint with resolved operation name", () => {
		const hint = buildLegacyTeamDeprecationHint("team_send_message", {
			team_name: "alpha",
		});
	...
	it("falls back to generic hint for unresolvable legacy name", () => {
		const hint = buildLegacyTeamDeprecationHint("team_nonexistent", {
			foo: "bar",
		});
...
describe("validateCommonFields", () => {
	...
	it("resolves a long display team_name before applying internal key validation", async () => {
		...
		try {
			await initTeamState(
				"long-display-aaaaaaaa",
				"test task",
				"executor",
				1,
				cwd,
				undefined,
				{ OMX_SESSION_ID: "session-long" },
				{
					display_name: "this-is-a-long-display-name-that-exceeds-thirty-chars",
					requested_name:
						"this-is-a-long-display-name-that-exceeds-thirty-chars",
					identity_source: "env-session",
				},
			);
			const previousSessionId = process.env.OMX_SESSION_ID;
...
describe("executeTeamApiOperation: send-message", () => {
	it("sends a message successfully and enqueues mailbox dispatch delivery", async () => {
		...
		try {
			...
			const parsedRequests = await listDispatchRequests("msg-team", cwd, {
				kind: "mailbox",
			});
			const mailboxRequest = parsedRequests.find(
				(request) => request.message_id === messageId,
			);
			assert.ok(
				mailboxRequest,
				"send-message should enqueue a mailbox dispatch request",
			);
	...
	it("returns the persisted leader mailbox message when hook-targeted sends are deduped from a worker worktree", async () => {
		...
		const workerCwd = await mkdtemp(
			join(tmpdir(), "omx-interop-send-worktree-"),
		);
		...
		try {
			...
			const workerMessages = (mailbox.messages ?? []).filter(
				(message) =>
					message.from_worker === "worker-1" &&
					message.to_worker === "leader-fixed",
			);
			assert.equal(
				workerMessages.length,
				1,
				"deduped leader sends should not append duplicate worker mailbox rows",
			);
			...
			const createdEvents = deliveryLog.filter(
				(entry) =>
					entry.event === "mailbox_created" &&
					entry.from_worker === "worker-1" &&
			...
			assert.equal(
				...
				1,
				"deduped leader sends should only emit one mailbox_created event",
			);
		} finally {
	...
	it("returns the persisted mailbox message when bridge compat omits the mailbox row under an explicit shared state root", async () => {
		...
		try {
			...
			await initTeamState(
				teamName,
				"shared state root mailbox fallback",
				"executor",
				2,
				leaderCwd,
			);
...
describe("executeTeamApiOperation: mailbox-mark-delivered", () => {
	it("marks a message delivered after sending and promotes matching dispatch receipt", async () => {
		...
		try {
			...
			await mkdir(
				join(cwd, ".omx", "state", "team", "mark-dlv", "mailbox", "worker-2"),
				{ recursive: true },
			);
			...
			const dispatch = await enqueueDispatchRequest(
				"mark-dlv",
				{
					kind: "mailbox",
					to_worker: "worker-2",
			...
			assert.equal(
				result.data.dispatch_request_id,
				dispatch.request.request_id,
			);
			assert.equal(result.data.dispatch_updated, true);

			const updatedDispatch = await readDispatchRequest(
				"mark-dlv",
				dispatch.request.request_id,
				cwd,
			);
	...
	it("keeps one mailbox wake pending until every queued message is acknowledged", async () => {
		...
		try {
			...
			const wake = await enqueueDispatchRequest(
				"mark-dlv-coalesced",
				{
					kind: "mailbox",
					to_worker: "worker-2",
			...
			const alternateWake = await enqueueDispatchRequest(
				"mark-dlv-coalesced",
				{
					kind: "mailbox",
					to_worker: "worker-2",
			...
			const firstAck = await executeTeamApiOperation(
				"mailbox-mark-delivered",
				{
					team_name: "mark-dlv-coalesced",
					worker: "worker-2",
					message_id: firstId,
				},
				cwd,
			...
			if (!firstAck.ok)
				throw new Error("expected first acknowledgement to succeed");
			...
			const finalAck = await executeTeamApiOperation(
				"mailbox-mark-delivered",
				{
					team_name: "mark-dlv-coalesced",
					worker: "worker-2",
			...
			if (!finalAck.ok)
				throw new Error("expected final acknowledgement to succeed");
			...
			assert.equal(
				(
					await readDispatchRequest(
						"mark-dlv-coalesced",
						alternateWake.request.request_id,
						cwd,
					)
				)?.status,
				"delivered",
	...
	it("marks leader-fixed mailbox delivery from a worker worktree and resolves the matching dispatch receipt", async () => {
		...
		const workerCwd = await mkdtemp(
			join(tmpdir(), "omx-interop-mark-dlv-worktree-"),
		);
		const prevTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		delete process.env.OMX_TEAM_STATE_ROOT;
		...
		try {
			await initTeamState(
				teamName,
				"leader mailbox delivery receipt",
				"executor",
				2,
				repoCwd,
			);
			...
			assert.ok(
				messageId,
				"leader mailbox message should include a message_id",
			);

			const pendingRequests = await listDispatchRequests(teamName, repoCwd, {
				kind: "mailbox",
				to_worker: "leader-fixed",
			});
			const pendingDispatch = pendingRequests.find(
				(request) => request.message_id === messageId,
			);
			assert.ok(
				pendingDispatch,
				"leader mailbox send should enqueue a matching dispatch request",
			);
			...
			if (!result.ok)
				throw new Error(
					"expected successful leader mailbox-mark-delivered result",
				);
			assert.equal(result.data.updated, true);
			assert.equal(
				result.data.dispatch_request_id,
				pendingDispatch?.request_id,
			);
			assert.equal(result.data.dispatch_updated, true);
			...
			assert.equal(updatedDispatch?.status, "delivered");
			assert.ok(
				updatedDispatch?.delivered_at,
				"leader dispatch receipt should record delivered_at",
			);

			const leaderMailbox = JSON.parse(
				await readFile(
					join(
						repoCwd,
						".omx",
			...
			const deliveredMessage = (leaderMailbox.messages ?? []).find(
...
describe("executeTeamApiOperation: mailbox-mark-notified", () => {
	it("marks a message notified after sending", async () => {
		...
		try {
			// Ensure the worker-2 mailbox directory exists so sendDirectMessage can write
			await mkdir(
				join(cwd, ".omx", "state", "team", "mark-ntf", "mailbox", "worker-2"),
				{ recursive: true },
			);
			const sendResult = await executeTeamApiOperation(
				"send-message",
...
describe("executeTeamApiOperation: list-tasks", () => {
	it("lists tasks for a team", async () => {
		...
		try {
			await createTask(
				"list-tsk",
				{ subject: "T1", description: "D1", status: "pending" },
				cwd,
			);
			const result = await executeTeamApiOperation(
				"list-tasks",
	...
	it("resolves display team names from OMX_TEAM_STATE_ROOT when API calls run outside the leader cwd", async () => {
		const { cwd: leaderCwd, cleanup } = await setupDisplayTeam(
			"api-root-display-11111111",
			"api-root-display",
			"session-api-root-display",
		);
		const workerCwd = await mkdtemp(
			join(tmpdir(), "omx-interop-api-root-worker-"),
		);
		try {
			process.env.OMX_TEAM_STATE_ROOT = join(leaderCwd, ".omx", "state");
			await createTask(
				"api-root-display-11111111",
				{ subject: "From shared root", description: "D", status: "pending" },
				leaderCwd,
			);
			const event = await appendTeamEvent(
				"api-root-display-11111111",
				{
					type: "task_completed",
					worker: "worker-1",
			...
			assert.equal(
				(listResult.data.tasks as Array<{ subject?: string }>)[0]?.subject,
				"From shared root",
			);

			const eventsResult = await executeTeamApiOperation(
				"read-events",
				{
					team_name: "api-root-display",
					type: "task_completed",
			...
			assert.equal(
				(eventsResult.data.events as Array<{ event_id?: string }>)[0]?.event_id,
				event.event_id,
			);

			const idleResult = await executeTeamApiOperation(
				"read-idle-state",
				{ team_name: "api-root-display" },
				workerCwd,
			);
	...
	it("prefers OMX_TEAM_STATE_ROOT over manifest metadata when resolving the team working directory", async () => {
		...
		try {
			...
			await createTask(
				teamName,
				{ subject: "From env root", description: "A lane", status: "pending" },
				cwdA,
			);
			await createTask(
				teamName,
...
describe("executeTeamApiOperation: update-task", () => {
	it("updates task subject and description", async () => {
		...
		try {
			const task = await createTask(
				"upd-tsk",
				{ subject: "Old", description: "Old desc", status: "pending" },
				cwd,
			);
			const result = await executeTeamApiOperation(
				"update-task",
	...
	it("rejects lifecycle fields (status, owner, result, error)", async () => {
		...
		try {
			const task = await createTask(
				"upd-tsk-lc",
				{ subject: "X", description: "Y", status: "pending" },
				cwd,
			);
			const result = await executeTeamApiOperation(
				"update-task",
				{
					team_name: "upd-tsk-lc",
					task_id: task.id,
					status: "completed",
				},
				cwd,
	...
	it("rejects non-boolean requires_code_change", async () => {
		...
		try {
			...
			if (!result.ok)
				assert.match(
					result.error.message,
					/requires_code_change must be a boolean/,
				);
		} finally {
			await cleanup();
		}
...
describe("executeTeamApiOperation: claim-task", () => {
	it("claims a task successfully", async () => {
		...
		try {
			const task = await createTask(
				"claim-tsk",
				{ subject: "Claim me", description: "D", status: "pending" },
				cwd,
			);
			const result = await executeTeamApiOperation(
				"claim-task",
...
describe("executeTeamApiOperation: transition-task-status", () => {
	...
	it("persists optional result and error payloads", async () => {
		...
		try {
			...
			const claimCompleted = await executeTeamApiOperation(
				"claim-task",
				{
			...
			const completedTransition = await executeTeamApiOperation(
				"transition-task-status",
				{
					team_name: "transition-payload",
					task_id: completedTask.id,
			...
			const claimFailed = await executeTeamApiOperation(
				"claim-task",
				{
					team_name: "transition-payload",
			...
			const failedTransition = await executeTeamApiOperation(
				"transition-task-status",
				{
					team_name: "transition-payload",
					task_id: failedTask.id,
	...
	it("rejects non-string result and error payloads", async () => {
		const badResult = await executeTeamApiOperation(
			"transition-task-status",
			{
				team_name: "x",
				task_id: "1",
		...
		const badError = await executeTeamApiOperation(
			"transition-task-status",
			{
				team_name: "x",
				task_id: "1",
...
describe("executeTeamApiOperation: read-manifest", () => {
	it("returns manifest_not_found when manifest does not exist", async () => {
		...
		try {
			...
			assert.ok(
				result.ok === true ||
					(result.ok === false && result.error.code === "manifest_not_found"),
			);
		} finally {
			await cleanup();
		}
...
describe("executeTeamApiOperation: read-events", () => {
	it("returns canonical filtered events", async () => {
		...
		try {
			const first = await appendTeamEvent(
				"evt-read",
				{
					type: "task_completed",
					worker: "worker-2",
			...
			const second = await appendTeamEvent(
				"evt-read",
				{
					type: "worker_idle",
					worker: "worker-1",
	...
	it("resolves display team names before reading events", async () => {
		const { cwd, cleanup } = await setupDisplayTeam(
			"evt-display-11111111",
			"evt-display",
			"session-events-display",
		);
		try {
			const event = await appendTeamEvent(
				"evt-display-11111111",
...
describe("executeTeamApiOperation: await-event", () => {
	it("waits for the next matching event", async () => {
		...
		try {
			const waitPromise = executeTeamApiOperation(
				"await-event",
				{
					team_name: "evt-await",
					worker: "worker-1",
			...
			if (result.ok) {
				...
				const event = result.data.event as {
					type?: string;
					worker?: string;
					task_id?: string;
				} | null;
	...
	it("resolves display team names before awaiting events", async () => {
		const { cwd, cleanup } = await setupDisplayTeam(
			"evt-await-display-111111",
			"evt-await-display",
			"session-await-display",
		);
		try {
			const waitPromise = executeTeamApiOperation(
				"await-event",
...
describe("executeTeamApiOperation: read-idle-state", () => {
	it("returns structured idle state from summary, snapshot, and recent events", async () => {
		...
		try {
			...
			const allIdleEvent = await appendTeamEvent(
				"idle-state-team",
				{
			...
			if (result.ok) {
				...
				const byWorker = result.data.last_idle_transition_by_worker as Record<
					string,
					{ event_id?: string; source_type?: string } | null
				>;
				assert.equal(byWorker["worker-1"]?.source_type, "worker_idle");
				assert.equal(byWorker["worker-2"], null);
				const lastAllIdle = result.data.last_all_workers_idle_event as {
					event_id?: string;
					type?: string;
					worker_count?: number;
				} | null;
	...
	it("resolves display team names before reading idle state", async () => {
		const { cwd, cleanup } = await setupDisplayTeam(
			"idle-display-11111111",
			"idle-display",
			"session-idle-display",
		);
		try {
			await writeMonitorSnapshot(
				"idle-display-11111111",
...
describe("executeTeamApiOperation: read-stall-state", () => {
	it("returns structured stall state from authoritative leader attention state", async () => {
		...
		try {
			...
			const primed = await executeTeamApiOperation(
				"get-summary",
				{
			...
			if (result.ok) {
				...
				assert.equal(result.data.all_workers_idle, true);
				assert.match(
					(result.data.reasons as string[]).join(" "),
					/workers_non_reporting:worker-1/,
				);
				assert.match(
					(result.data.reasons as string[]).join(" "),
					/leader_attention_pending:leader_session_stopped/,
				);
				const attention = result.data.leader_attention_state as {
	...
	it("resolves display team names before reading stall state", async () => {
		const { cwd, cleanup } = await setupDisplayTeam(
			"stall-display-1111111",
			"stall-display",
			"session-stall-display",
		);
		try {
	...
	it("uses unread leader mailbox state even without recent nudge events", async () => {
		...
		try {
			...
			if (result.ok) {
				...
				assert.match(
					(result.data.reasons as string[]).join(" "),
					/leader_attention_pending:unread_leader_mailbox/,
				);
			}
		} finally {
			await cleanup();
	...
	it("reads leader runtime activity from boxed OMX_ROOT state consistently with writer path", async () => {
		...
		try {
			...
			await writeFile(
				join(box, ".omx", "state", "leader-runtime-activity.json"),
				JSON.stringify(
					{
	...
	it("suppresses stale native-stop attention after newer leader activity", async () => {
		...
		try {
			...
			await writeFile(
				join(cwd, ".omx", "state", "leader-runtime-activity.json"),
				JSON.stringify(
					{
						last_activity_at: "2026-03-10T10:06:00.000Z",
						last_source: "team_status",
	...
	it("marks only active leader-owned teams as stopped on session end", async () => {
		...
		try {
			...
			const updatedTeams = await markOwnedTeamsLeaderSessionStopped(
				cwd,
				"leader-session-1",
			...
			assert.equal(
				ownedAttention?.leader_attention_reason,
				"stale_leader_with_messages",
			);
			assert.deepEqual(ownedAttention?.attention_reasons, [
				"stale_leader_with_messages",
				"leader_session_stopped",
			]);
	...
	it("marks only active leader-owned teams from the native stop hook", async () => {
		...
		try {
			...
			const updatedTeams = await markOwnedTeamsLeaderStopObserved(
				cwd,
				"leader-session-stop",
				"2026-03-10T10:15:00.000Z",
			);
			...
			const ownedAttention = await readTeamLeaderAttention(
				"owned-stop-team",
				cwd,
			);
...
describe("executeTeamApiOperation: cleanup", () => {
	...
	it("requires confirm_issues for failed-task cleanup on normal teams", async () => {
		...
		try {
			...
			const summary = await executeTeamApiOperation(
				"get-summary",
				{
					team_name: "cleanup-gate",
				},
...
describe("executeTeamApiOperation: orphan-cleanup", () => {
	...
	it("resolves display team names before orphan cleanup", async () => {
		const { cwd, cleanup } = await setupDisplayTeam(
			"orphan-display-111111",
			"orphan-display",
			"session-orphan-display",
		);
		try {
			const result = await executeTeamApiOperation(
				"orphan-cleanup",
...
describe("executeTeamApiOperation: read-task-approval", () => {
	it("reads task approval (null when not set)", async () => {
		...
		try {
			const task = await createTask(
				"rd-appr",
				{ subject: "A", description: "B", status: "pending" },
				cwd,
			);
			const result = await executeTeamApiOperation(
				"read-task-approval",
...
describe("executeTeamApiOperation: error handling", () => {
	it("wraps thrown errors in an error envelope", async () => {
		...
		try {
			await mkdir(join(cwd, ".omx", "state", "team", "err-team"), {
				recursive: true,
			});
			await writeFile(
				join(cwd, ".omx", "state", "team", "err-team", "config.json"),
				"{}",
				"utf8",
			);
