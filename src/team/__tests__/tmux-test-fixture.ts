...
function snapshotTmuxEnv(
	source: NodeJS.ProcessEnv = process.env,
): TmuxEnvSnapshot {
	return {
		TMUX: typeof source.TMUX === "string" ? source.TMUX : undefined,
		TMUX_PANE:
			typeof source.TMUX_PANE === "string" ? source.TMUX_PANE : undefined,
		OMX_MUX_BINARY:
			typeof source.OMX_MUX_BINARY === "string"
				? source.OMX_MUX_BINARY
				: undefined,
	};
}
...
function applyTmuxEnv(snapshot: TmuxEnvSnapshot): void {
	...
	if (typeof snapshot.OMX_MUX_BINARY === "string")
		process.env.OMX_MUX_BINARY = snapshot.OMX_MUX_BINARY;
...
function runTmuxResult(
	...
	const env =
		options.env ??
		(options.ignoreTmuxEnv
			? { ...process.env, TMUX: undefined, TMUX_PANE: undefined }
			: process.env);
	const argv = [
...
export function isRealTmuxAvailable(): boolean {
	...
	if (result.status !== 0) {
		throw new Error(
			(result.stderr || "").trim() || `tmux exited ${result.status}`,
		);
	}
	return true;
}
...
export function buildPtyScriptCommand(
	command: string,
...
export async function withTempTmuxSession<T>(
	optionsOrFn:
		| TempTmuxSessionOptions
		| ((fixture: TempTmuxSessionFixture) => Promise<T> | T),
	...
	const serverName = options.useAmbientServer
		? ""
		: uniqueTmuxIdentifier("omx-fixture");
	const serverKind: TempTmuxSessionFixture["serverKind"] =
		options.useAmbientServer ? "ambient" : "synthetic";
	const tmuxOptions = {
		ignoreTmuxEnv: true,
	...
	process.env.OMX_MUX_BINARY = tmuxExecutable;
	const createPathShim = async (
		directory: string,
		commandLogPath?: string,
	): Promise<string> => {
		if (serverKind !== "synthetic")
			throw new Error("private tmux PATH shim requires a synthetic server");
		const shimPath = join(directory, "tmux");
		const logCommand = commandLogPath
		...
		process.env.OMX_MUX_BINARY = shimPath;
		runTmux(
			[
				"set-environment",
				"-g",
	...
	const triggerClientResize = (
		...
		if (result.status !== 124) {
			throw new Error(
				`client resize trigger result: ${JSON.stringify({ status: result.status, stdout: result.stdout || "", stderr: result.stderr || "", error: "" })}`,
			);
		}
	};
	...
	const created = runTmux(
		...
		options.serverLog
			? { ...tmuxOptions, verbose: true, cwd: fixtureCwd }
			: tmuxOptions,
	);
	...
	const serverLogPath = options.serverLog
		? join(
				fixtureCwd,
				`tmux-server-${runTmux(["display-message", "-p", "#{pid}"], tmuxOptions)}.log`,
			)
		: null;

	const socketPath = runTmux(
	...
	const fixture: TempTmuxSessionFixture = {
		...
		sessionExists: (targetSessionName = sessionName) =>
			tmuxSessionExists(targetSessionName, serverName || undefined),
		...
		readServerLog: async () => {
			...
			return readFile(serverLogPath, "utf-8").catch(
