...
describe("team commit hygiene vocabulary", () => {
	it("includes canonical operation and status vocabulary in structured context", () => {
		...
		assert.match(
			context.vocabulary.operational_commit_kinds.find(
				(term) => term.value === "auto_checkpoint",
			)?.description ?? "",
			/worker-local checkpoint commit/i,
		);
		assert.match(
			context.vocabulary.operational_commit_statuses.find(
				(term) => term.value === "conflict",
			)?.description ?? "",
			/reconciliation/i,
		);
	});
	...
	it("renders the vocabulary before runtime ledger details in markdown", () => {
		...
		assert.match(
			markdown,
			/`integration_cherry_pick` \(integration cherry-pick\)/,
		);
		assert.match(markdown, /### Operational commit statuses/);
		assert.match(markdown, /`noop` \(no-op\)/);
		assert.ok(
			markdown.indexOf("## Commit Hygiene Vocabulary") <
				markdown.indexOf("## Runtime Operational Ledger"),
