...
async function initCommittedGitRepo(cwd: string): Promise<void> {
	...
	execFileSync("git", ["config", "user.name", "OMX Test"], {
		cwd,
		stdio: "pipe",
	});
	execFileSync("git", ["config", "user.email", "omx@example.com"], {
		cwd,
		stdio: "pipe",
	});
...
function workerStartupScriptPath(
	cwd: string,
	teamName: string,
	workerName: string,
): string {
...
const BLOCKED_SCALE_UP_APPROVED_BINDING_STATES: readonly BlockedScaleUpApprovedBindingState[] =
	["malformed", "stale", "ambiguous", "missing-baseline"];
...
function expectedScaleUpOutcome(
	state: ScaleUpApprovedBindingState,
): ScaleUpObservedOutcome {
	if (state === "missing") {
		return "generic";
	}
	return BLOCKED_SCALE_UP_APPROVED_BINDING_STATES.includes(
		state as BlockedScaleUpApprovedBindingState,
	)
		? "blocked"
		: "approved";
}
...
async function writeReadyContextPack(
	...
	await writeContextPack(cwd, slug, prdPath, testSpecPath, [
		"scope",
		"build",
		"verify",
	]);
...
async function configureScaleUpTeamForDirectDispatch(
	teamName: string,
	cwd: string,
): Promise<void> {
	...
	if (!existsSync(manifestPath)) {
		await mkdir(join(cwd, ".omx", "state", "team", teamName), {
			recursive: true,
		});
		await writeFile(
			manifestPath,
			`${JSON.stringify({ version: 2, policy: {} }, null, 2)}\n`,
		);
	}
...
async function readScaleUpTmuxLogCommands(
	tmuxLogPath: string,
): Promise<string[]> {
...
async function readScaleUpTaskPayloads(
	...
	return await Promise.all(
		taskFiles.map((entry) => readFile(join(tasksDir, entry), "utf-8")),
	);
}
...
async function prepareScaleUpApprovedBindingState(
	...
	if (state === "incomplete" || state === "invalid" || state === "ready") {
		prdLines.push(
			buildContextPackOutcome(canonicalContextPackRelativePath(state)),
			"",
		);
	}
	...
	if (state === "invalid") {
		await writeContextPack(cwd, state, prdPath, testSpecPath, [
			"scope",
			"build",
			"verify",
		]);
...
describe("scale-down cleanup debt path containment", () => {
	...
	it("accepts same-root and descendant native Windows canonical paths", () => {
		assert.equal(
			isSameOrInsidePath("C:\\repo", "C:\\repo", windowsPathSemantics),
			true,
		);
		assert.equal(
			isSameOrInsidePath(
				"C:\\repo\\.omx\\team\\alpha",
				"C:\\repo",
	...
	it("accepts same-share UNC descendants across host and share casing", () => {
		assert.equal(
			isSameOrInsidePath(
				"\\\\SERVER\\Share\\repo/.omx\\team\\alpha",
				"\\\\server\\share\\REPO",
				windowsPathSemantics,
			),
			true,
	...
	for (const [name, candidate] of [
		...
		it(`rejects ${name} under native Windows path semantics`, () => {
			assert.equal(
				isSameOrInsidePath(candidate, "C:\\repo", windowsPathSemantics),
				false,
			);
		});
	}
	...
	it("preserves POSIX same-root and descendant containment", () => {
		assert.equal(
			isSameOrInsidePath("/repo", "/repo", posixPathSemantics),
			true,
		);
		assert.equal(
			isSameOrInsidePath("/repo/.omx/team/alpha", "/repo", posixPathSemantics),
			true,
		);
	});
	...
	it("rejects POSIX siblings, traversal, and foreign roots", () => {
		assert.equal(
			isSameOrInsidePath("/repo-other/worker", "/repo", posixPathSemantics),
			false,
		);
		assert.equal(
			isSameOrInsidePath(
		...
		assert.equal(
			isSameOrInsidePath("/foreign/worker", "/repo", posixPathSemantics),
			false,
		);
	});
});
...
describe("scaleUp", () => {
	...
	it("rejects explicit mixed worker policy before scale-up creates worker state or a pane", async () => {
		...
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-explicit-policy-bin-"),
		);
...
`,
			...
			assert.deepEqual(
				await readScaleUpTaskPayloads("scale-up-explicit-policy", cwd),
				[],
			);
			assert.equal(
				existsSync(
			...
			assert.equal(
				existsSync(
					workerStartupScriptPath(cwd, "scale-up-explicit-policy", "worker-2"),
				),
				false,
			);
	...
	it("rejects Claude and Gemini restrictive config policy before scale-up creates task payloads, worker state, a pane, or process", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-up-restrictive-noncodex-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-restrictive-noncodex-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const previousPath = process.env.PATH;
...
`,
			...
			for (const workerCli of ["claude", "gemini"] as const) {
				...
				assert.equal(
					existsSync(
						join(cwd, ".omx", "state", "team", teamName, "workers", "worker-2"),
					),
					false,
				);
				assert.equal(
					existsSync(workerStartupScriptPath(cwd, teamName, "worker-2")),
					false,
				);
				const tmuxCommands = await readScaleUpTmuxLogCommands(tmuxLogPath);
				assert.equal(
	...
	it("covers the scale-up config-policy and no-policy argv matrix", async () => {
		...
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-policy-matrix-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const capturePath = join(cwd, "worker-argv.txt");
		const emptyCodexHome = await mkdtemp(
			join(tmpdir(), "omx-scale-up-policy-matrix-codex-home-"),
		);
		const previousPath = process.env.PATH;
...
`,
				...
				const result = await scaleUp(
					...
					cwd,
					{
						...
						...(params.inheritedModel
							? { [TEAM_WORKER_INHERITED_MODEL_ENV]: params.inheritedModel }
							: {}),
					},
				);
				if (!result.ok) assert.fail(result.error);

				const startupScriptPath = workerStartupScriptPath(
					cwd,
					params.teamName,
					"worker-2",
				);
				const scriptResult = execFileSync("/bin/sh", [startupScriptPath], {
					encoding: "utf-8",
				});
			...
			process.argv = previousArgv.filter(
				(arg) =>
					arg !== "--dangerously-bypass-approvals-and-sandbox" &&
					arg !== "--madmax",
			);
			...
			assert.deepEqual(
				await runScaleUpCase({
					teamName: "scale-up-execution-default",
					role: "executor",
					launchArgs: "--model policy-model",
				}),
				[
					"-c",
					'model_reasoning_effort="medium"',
			...
			assert.deepEqual(
				await runScaleUpCase({
					teamName: "scale-up-nonexecution-default",
					role: "explore",
					launchArgs: "--model policy-model",
				}),
			...
			assert.deepEqual(
				await runScaleUpCase({
					teamName: "scale-up-absent-role-default",
					launchArgs: "--model policy-model",
				}),
				[
					"-c",
			...
			assert.deepEqual(
				await runScaleUpCase({
					teamName: "scale-up-unknown-role-default",
					role: "unknown-role",
					launchArgs: "--model policy-model",
				}),
				[
	...
	it("persists a newly created worker when rollback pane proof is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-up-owner-tag-rollback-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-owner-tag-rollback-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const previousPath = process.env.PATH;
		...
		try {
			...
			await configureScaleUpTeamForDirectDispatch(
				"scale-up-owner-tag-rollback",
				cwd,
			);
			const ownerConfig = await readTeamConfig(
				"scale-up-owner-tag-rollback",
				cwd,
			);
	...
	it("retains unpinned rollback debt without killing a pane when its first post-split proof is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-up-unpinned-rollback-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-unpinned-rollback-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		...
		try {
			await writeFile(
				...
				[
					...
					'    if [ -f "' +
						splitPath +
						'" ]; then printf "malformed pane snapshot\\n"; else',
					"      printf '%s\\t%s\\t%s\\n' '%11' '0' '42411'",
					"      printf '%s\\t%s\\t%s\\n' '%21' '0' '42421'",
					"    fi",
					"    ;;",
	...
	it("retains the original pane/PID rollback debt when the new pane ID is reused before owner tagging", async () => {
		...
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-owner-tag-reuse-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const previousPath = process.env.PATH;
	...
	it("injects persisted leader-owned Ultragoal context into scaled worker inboxes", async () => {
		...
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-up-ultragoal-context-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-ultragoal-context-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const previousPath = process.env.PATH;
		...
		try {
			...
			const result = await scaleUp(
				...
				[
					{
						subject: "Implement ultragoal follow-up",
						description: "Implement ultragoal follow-up",
						owner: "worker-2",
					},
				],
				cwd,
			...
			const inboxStateRoot =
				result.addedWorkers[0]?.team_state_root ??
	...
	it("keeps scale-up on the generic path when no approved binding is persisted", async () => {
		...
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-up-no-approved-binding-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-no-approved-binding-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const previousPath = process.env.PATH;
		...
		try {
			...
			assert.equal(
				await readExpectedScaleUpApprovedBindingError(teamName, cwd),
				null,
			);
			...
			const result = await scaleUp(
				...
				[
					{
						subject: "Implement generic follow-up",
						description: "Implement generic follow-up",
						owner: "worker-2",
					},
				],
				cwd,
	...
	it("injects approved handoff context on scale-up when the persisted binding is baseline-ready without context-pack metadata", async () => {
		...
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-plan-only-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const previousPath = process.env.PATH;
		...
		try {
			...
			const result = await scaleUp(
				...
				[
					{
						subject: "Implement plan-only follow-up",
						description: "Implement plan-only follow-up",
						owner: "worker-2",
					},
				],
				cwd,
	...
	it("injects approved handoff context into scaled worker inboxes when the persisted binding stays ready", async () => {
		...
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-approved-context-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const previousPath = process.env.PATH;
		...
		try {
			...
			await writeFile(
				...
				[
					...
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-1410"),
					),
					"",
					`Launch via omx team 1:executor "${approvedTask}"`,
				].join("\n"),
			...
			assert.match(
				inbox,
				/Approved repository context summary source: .*repo-context-issue-1410\.md/,
			);
	...
	it("proves the approved-binding scale-up model across generated state/count scenarios, including forbidden counterfactuals", async () => {
		for (const state of SCALE_UP_APPROVED_BINDING_STATES) {
			for (const count of SCALE_UP_COUNTS) {
				...
				const cwd = await mkdtemp(
					join(tmpdir(), `omx-scale-up-model-${state}-${count}-`),
				);
				const fakeBinDir = await mkdtemp(
					join(tmpdir(), `omx-scale-up-model-${state}-${count}-bin-`),
				);
				const tmuxLogPath = join(fakeBinDir, "tmux.log");
				const previousPath = process.env.PATH;
				...
				try {
					...
					const expectedError = await readExpectedScaleUpApprovedBindingError(
						teamName,
						cwd,
					);
					...
					const approvedInboxCount = inboxes.filter(
						(inbox) =>
							typeof inbox === "string" &&
							inbox.includes("## Approved Handoff Context"),
					).length;
					assert.ok(
					...
					if (expectedOutcome === "blocked") {
						...
						if (result.ok) {
							throw new Error(
								`expected blocked scale-up outcome for ${state} count=${count}`,
							);
						}
						...
						assert.equal(
							taskPayloads.some((payload) =>
								tasks.some((task) => payload.includes(task.subject)),
							),
							false,
						);
					...
					if (!result.ok) {
						throw new Error(
							`expected successful scale-up outcome for ${state} count=${count}`,
						);
					}
					...
					assert.ok(
						inboxes.every(
							(inbox): inbox is string => typeof inbox === "string",
						),
					);
					...
					for (const [index, inbox] of inboxes.entries()) {
						...
						assert.ok(
							inbox.includes(task.subject),
							`expected inbox to include task subject ${task.subject}`,
						);
					}
					assert.equal(
						taskPayloads.filter((payload) =>
							tasks.some((task) => payload.includes(task.subject)),
						).length,
						count,
					);
					if (expectedOutcome === "approved") {
						...
						assert.ok(
							inboxes.every(
								(inbox) => !inbox.includes("## Approved Handoff Context"),
							),
						);
					}
				} finally {
	...
	for (const state of BLOCKED_SCALE_UP_APPROVED_BINDING_STATES) {
		it(`fails closed before worker launch when the persisted approved binding is ${state}`, async () => {
			...
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-scale-up-approved-${state}-`),
			);
			const fakeBinDir = await mkdtemp(
				join(tmpdir(), `omx-scale-up-approved-${state}-bin-`),
			);
			const tmuxLogPath = join(fakeBinDir, "tmux.log");
			const previousPath = process.env.PATH;
			...
			try {
				...
				assert.equal(
					taskPayloads.some((payload) =>
						payload.includes("Implement approved follow-up"),
					),
					false,
				);
				assert.equal(
	...
	it("uses project-scoped CODEX_HOME for scaled worker reasoning and model defaults", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-up-project-reasoning-"),
		);
		const fakeBinDir = await mkdtemp(
		...
		try {
			...
			await writeFile(
				join(cwd, ".codex", ".omx-config.json"),
				JSON.stringify({
					env: {
			...
			await mkdir(
				join(cwd, ".omx", "state", "team", "scale-up-project-reasoning"),
				{ recursive: true },
			);
			await writeFile(
				join(
			...
			if (typeof previousStandardModel === "string")
				process.env.OMX_DEFAULT_STANDARD_MODEL = previousStandardModel;
			else delete process.env.OMX_DEFAULT_STANDARD_MODEL;
			if (typeof previousFrontierModel === "string")
				process.env.OMX_DEFAULT_FRONTIER_MODEL = previousFrontierModel;
			else delete process.env.OMX_DEFAULT_FRONTIER_MODEL;
			if (typeof previousCodeHome === "string")
	...
	it("removes generated worktree-root AGENTS when scale-up rolls back", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-up-rollback-worktree-"),
		);
		const fakeBinDir = await mkdtemp(
...
`,
			...
			await assert.rejects(readFile(workerRootAgents, "utf-8"), {
				code: "ENOENT",
			});
...
`,
			...
			await mkdir(join(cwd, ".codex", "prompts"), { recursive: true });
			await writeFile(
				join(cwd, ".codex", "prompts", "test-engineer.md"),
				"<identity>Test Engineer</identity>",
			);
			await mkdir(join(cwd, ".omx", "state", "team", "frontier-role"), {
				recursive: true,
			});
			await writeFile(
				join(cwd, ".omx", "state", "team", "frontier-role", "worker-agents.md"),
				"# Base worker instructions\n",
			);
...
`,
	...
	for (const [name, proofOutput, proofExitStatus, expectedReason] of [
		...
		[
			"malformed global pane PID",
			"%21\t0\tnot-a-pid\n",
			0,
			"malformed_snapshot",
		],
		[
			"duplicate global pane proof rows",
			"%21\t0\t42421\n%21\t0\t42422\n",
			0,
			"malformed_snapshot",
		],
		...
		it(`fails closed before split-window when exact proof has ${name}`, async () => {
			...
			const fakeBinDir = await mkdtemp(
				join(tmpdir(), "omx-scale-up-split-proof-bin-"),
			);
			const tmuxLogPath = join(fakeBinDir, "tmux.log");
			const tmuxStubPath = join(fakeBinDir, "tmux");
			const previousPath = process.env.PATH;
			...
			try {
				...
				if (!result.ok) {
					assert.equal(
						result.error,
						`scale_up_split_target_proof_unavailable:%21:${expectedReason}`,
					);
				}
	...
	it("migrates a legacy worker PID from an exact owner-bound proof before split-window", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-up-legacy-target-pid-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-legacy-target-pid-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const previousPath = process.env.PATH;
		try {
			...
			assert.equal(result.ok, true);
			assert.equal(
				(await readTeamConfig(teamName, cwd))?.workers[0]?.pid,
				42421,
			);
			const tmuxCommands = await readScaleUpTmuxLogCommands(tmuxLogPath);
			assert.ok(
	...
	for (const [name, proofOutput, owner, expectedError] of [
		[
			...
			"%21\\t0\\tnot-a-pid\\n",
			"team:scale-up",
			"scale_up_split_target_proof_unavailable:%21:malformed_snapshot",
		],
		[
			"foreign pane",
			"%210\\t0\\t424210\\n",
			"team:scale-up",
			"scale_up_split_target_proof_unavailable:%21:absent",
		],
		[
			"foreign owner tag",
			"%21\\t0\\t42421\\n",
			"team:foreign",
			"scale_up_split_target_owner_changed:%21",
		],
	] as const) {
		it(`rejects legacy PID migration with ${name} before a scale effect`, async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-scale-up-legacy-proof-reject-"),
			);
			const fakeBinDir = await mkdtemp(
				join(tmpdir(), "omx-scale-up-legacy-proof-reject-bin-"),
			);
			const tmuxLogPath = join(fakeBinDir, "tmux.log");
			const tmuxStubPath = join(fakeBinDir, "tmux");
			const previousPath = process.env.PATH;
			try {
				...
				assert.deepEqual(
					await scaleUp(teamName, 1, "executor", [], cwd, {
						OMX_TEAM_SCALING_ENABLED: "1",
						OMX_TEAM_SKIP_READY_WAIT: "1",
					}),
	...
	it("rejects a PID-reused explicit split target at the first proof before any split or owner tag", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-up-first-proof-reuse-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-first-proof-reuse-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const previousPath = process.env.PATH;
	...
	it("rolls back all preparation when the split target proof is lost immediately before split-window", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-up-second-split-proof-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-second-split-proof-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		...
		try {
				...
				[
					{
						...
						description: "proof loss rollback",
						owner: "worker-2",
					},
				],
				cwd,
			...
			assert.deepEqual(
				withoutConfigGeneration(await readFile(configPath, "utf-8")),
				withoutConfigGeneration(configBefore),
			);
			...
			assert.equal(
				existsSync(
					join(cwd, ".omx", "team", teamName, "worktrees", "worker-2"),
				),
				false,
			);
	...
	it("rejects a split target ID replacement that occurs during owner authorization", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-up-split-target-pid-reuse-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-split-target-pid-reuse-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const proofCountPath = join(fakeBinDir, "proof-count");
		const tmuxStubPath = join(fakeBinDir, "tmux");
		...
		try {
			...
			const result = await scaleUp(
				...
				[
					{
						subject: "must not persist",
						description: "PID reuse rollback",
						owner: "worker-2",
					},
				],
				cwd,
	...
	it("provisions detached worktrees for scaled-up workers from persisted team worktree mode", async () => {
		...
		try {
			...
			await mkdir(join(repo, ".omx", "state", "team", teamName), {
				recursive: true,
			});
			await writeFile(
				join(repo, ".omx", "state", "team", teamName, "worker-agents.md"),
				"# Base worker instructions\n",
			);
			await initTeamState(
				teamName,
			...
			const worker = updated?.workers.find(
				(entry) => entry.name === "worker-2",
			);
			assert.deepEqual(updated?.worktree_mode, {
				enabled: true,
				detached: true,
				name: null,
			});
			assert.ok(
				...
				"scaled worker should have detached worktree path",
			);
	...
	it("rolls back pre-split worker preparation when worktree-root instructions cannot materialize", async () => {
		...
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-pre-split-root-failure-bin-"),
		);
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const previousPath = process.env.PATH;
...
`,
			...
			assert.match(
				result.error,
				/^scale_up_worker_preparation_failed:worker-2:/,
			);
			assert.equal(
				existsSync(
					join(cwd, ".omx", "state", "team", teamName, "workers", "worker-2"),
	...
	it("retains only unresolved rollback workers and deletes never-created tasks after an explicit proof-loss phase", async () => {
		const cwd = await initRepo();
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-rollback-kill-fail-bin-"),
		);
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
...
`,
			...
			const result = await scaleUp(
				...
				[
					{
						subject: "first",
						description: "created before mixed rollback",
						owner: "worker-2",
					},
					{
						subject: "second",
					...
					{
						subject: "third",
						description: "never created after proof loss",
						owner: "worker-4",
					},
				],
				cwd,
	...
	for (const rollbackFailurePhase of [
		"rollback-membership-config-persistence",
		"rollback-membership-manifest-persistence",
	] as const) {
		it(`recovers and raw-verifies original membership after public scaleUp ${rollbackFailurePhase}`, async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-scale-up-${rollbackFailurePhase}-`),
			);
			const fakeBinDir = await mkdtemp(
				join(tmpdir(), `omx-scale-up-${rollbackFailurePhase}-bin-`),
			);
			const tmuxLogPath = join(fakeBinDir, "tmux.log");
			const previousPath = process.env.PATH;
			...
			try {
				...
				const result = await scaleUp(
					...
					[
						{
							subject: "public rollback recovery",
				...
				if (!result.ok)
					assert.match(
						result.error,
						/scale_up_rollback_membership_persistence_failed:.*rollback-persistence-failure/,
					);
				await readTeamConfig(teamName, cwd);
				assert.deepEqual(
					withoutConfigGeneration(await readFile(configPath, "utf8")),
					withoutConfigGeneration(originalConfigBytes),
				);
				assert.deepEqual(
					withoutConfigGeneration(await readFile(manifestPath, "utf8")),
					withoutConfigGeneration(originalManifestBytes),
				);
				assert.equal(
					existsSync(join(teamDir, ".membership-task-transaction.json")),
					false,
				);
	...
	for (const phase of ["identity", "inbox", "config"] as const) {
		it(`rolls back live pane and materialized state when ${phase} materialization fails`, async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-scale-up-${phase}-failure-`),
			);
			const fakeBinDir = await mkdtemp(
				join(tmpdir(), `omx-scale-up-${phase}-failure-bin-`),
			);
			const tmuxLogPath = join(fakeBinDir, "tmux.log");
			const previousPath = process.env.PATH;
			try {
				...
				if (!result.ok)
					assert.match(
						result.error,
						new RegExp(
							`scale_up_worker_materialization_failed:worker-2:.*${phase}`,
						),
					);
	...
	it("rolls back a named worktree created before injected ensure bookkeeping fails", async () => {
		const repo = await initRepo();
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-up-named-post-create-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const previousPath = process.env.PATH;
		...
		try {
			...
			await initTeamState(
				...
				process.env,
				{
					...
					worktree_mode: {
						enabled: true,
						detached: false,
						name: "feature/named-post-create",
					},
				},
			);
			...
			if (!result.ok)
				assert.match(
					result.error,
					/scale_up_worker_preparation_failed:worker-2:.*worktree-ensure-post-create/,
				);
			...
			assert.equal(
				existsSync(
					join(repo, ".omx", "state", "team", teamName, "workers", "worker-2"),
				),
				false,
			);
			assert.deepEqual(
...
describe("scaleDown", () => {
		...
		try {
			...
			const task = await createTask(
				"claim-boundary",
				{
					...
					description: "must not be claimed by a removed worker",
					status: "pending",
					owner: "worker-2",
				},
				cwd,
			);
			const down = scaleDown(
				"claim-boundary",
				cwd,
			...
			for (
				let attempt = 0;
				attempt < 50 && !existsSync(lockPath);
				attempt += 1
			) {
			...
			assert.deepEqual(await down, {
				ok: true,
				removedWorkers: ["worker-2"],
				newWorkerCount: 1,
			});
	...
	it("blocks post-snapshot task creation and null-version claims until removed membership is committed", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-down-create-claim-boundary-"),
		);
		try {
			...
			const seed = await createTask(
				...
				{
					subject: "snapshot marker",
					description: "holds the canonical task lock",
					status: "pending",
					owner: "worker-2",
				},
				cwd,
			...
			for (
				let attempt = 0;
				attempt < 50 && !existsSync(postSnapshotMarker);
				attempt += 1
			) {
				await new Promise((resolve) => setTimeout(resolve, 5));
			}
			...
			const created = createTask(
				"create-claim-boundary",
				{
					subject: "created after snapshot",
					description: "must not restore removed membership",
					status: "pending",
					owner: "worker-2",
				},
				cwd,
			...
			assert.deepEqual(
				(await readTeamConfig("create-claim-boundary", cwd))?.workers.map(
					(worker) => worker.name,
				),
				["worker-1"],
			);
			assert.equal(
				(await readTask("create-claim-boundary", task.id, cwd))?.owner,
				"worker-2",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
...
describe("scaleDown worktree AGENTS cleanup", () => {
	it("removes generated worktree-root AGENTS during scale-down", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-down-worktree-agents-"),
		);
		try {
			await initTeamState(
				"scale-down-worktree",
			...
			assert.equal(
				await readFile(join(worktree, "AGENTS.md"), "utf-8"),
				"# Tracked root instructions\n",
			);
			assert.equal(
				existsSync(
...
describe("scaleDown teardown hardening", () => {
	it("scaleDown removes a worker with a pane proven dead without targeting it", async () => {
		...
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-down-dead-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const previousPath = process.env.PATH;
...
`,
	...
	it("retains PID-less rollback debt when a recycled same-Team pane remains live", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-down-unpinned-recycled-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-down-unpinned-recycled-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const previousPath = process.env.PATH;
...
`,
			...
			assert.deepEqual(initialDebt.unresolved_panes, [
				{ name: "worker-2", index: 2, pane_id: "%405", pid: null },
			]);
			...
			if (!canonical) return;
			assert.deepEqual(
				await reconcileScaleDownCleanupDebt(
					"unpinned-recycled",
					cwd,
					canonical,
				),
			...
			const reconciledDebt = JSON.parse(await readFile(debtPath, "utf8")) as {
				reasons: string[];
			};
			assert.ok(
				reconciledDebt.reasons.includes("%405:legacy_pid_missing_live"),
			);
...
`,
			...
			if (!result.ok)
				assert.match(
					result.error,
					/pane_teardown_unresolved:%11.*pane_teardown_unresolved:%12/,
				);
			assert.deepEqual(
				(await readTeamConfig("exclusions", cwd))?.workers.map(
					(worker) => worker.name,
				),
				["worker-4"],
			);

			const tmuxCommands = (await readFile(tmuxLogPath, "utf-8"))
				.trim()
				.split("\n");
			assert.deepEqual(tmuxCommands, [
				"list-panes -a -F #{pane_id}\t#{pane_dead}\t#{pane_pid}",
	...
	it("scaleDown commits forward membership and retains artifacts/debt when pane ID is reused", async () => {
		...
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-down-kill-fail-bin-"),
		);
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const previousPath = process.env.PATH;
...
`,
			...
			await mkdir(join(cwd, ".omx", "state", "team", "kill-fail", "runtime"), {
				recursive: true,
			});
			const startupScriptPath = workerStartupScriptPath(
				cwd,
			...
			if (!result.ok)
				assert.match(
					result.error,
					/^scale_down_cleanup_debt:pane_teardown_failed:%13/,
				);
			const committed = await readTeamConfig("kill-fail", cwd);
			assert.deepEqual(
				committed?.workers.map((worker) => worker.name),
				["worker-1"],
			);
			assert.equal(
				existsSync(
			...
			assert.equal(
				await readFile(join(workerDir, "identity.json"), "utf8"),
				'{"worker":"worker-2"}',
			);
			assert.equal(
				await readFile(join(workerDir, "inbox.md"), "utf8"),
				"retryable inbox",
			);
			assert.equal(
				await readFile(join(worktreePath, "keep.txt"), "utf8"),
				"retryable worktree",
			);
			assert.equal(await readFile(startupScriptPath, "utf8"), "#!/bin/sh\n");
			assert.deepEqual(
				await readWorkerStatus("kill-fail", "worker-2", cwd),
				priorStatus,
			);
	...
	it("aborts before pane teardown and preserves exact state when task reconciliation cannot acquire a lock", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-down-task-reconcile-fail-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-down-task-reconcile-fail-bin-"),
		);
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const previousPath = process.env.PATH;
...
`,
			...
			const firstTask = await createTask(
				"task-reconcile-fail",
				{
					...
					description: "must be restored after later failure",
					status: "pending",
					owner: "worker-2",
				},
				cwd,
			);
			const firstTaskPath = join(
				cwd,
				".omx",
				"state",
				"team",
			...
			const priorConfig = structuredClone(
				await readTeamConfig("task-reconcile-fail", cwd),
			);
			await mkdir(
				join(
					cwd,
			...
			assert.deepEqual(result, {
				ok: false,
				error:
					"scale_down_task_reconciliation_failed:Error: Timed out acquiring task claim lock for task-reconcile-fail/2",
			});
			assert.deepEqual(
				await readTeamConfig("task-reconcile-fail", cwd),
				priorConfig,
	...
	it("commits forward membership and durable debt when scale-down pane proof is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-down-proof-unavailable-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-down-proof-unavailable-bin-"),
		);
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const previousPath = process.env.PATH;
		try {
			...
			const task = await createTask(
				...
				{
					subject: "preserved",
					description: "exact proof is unavailable",
					status: "pending",
					owner: "worker-2",
				},
				cwd,
			...
			const statusRaw =
				'{"state":"idle", "reason":"preserve bytes", "updated_at":"2026-07-14T00:00:00.000Z"}\n';
			...
			assert.equal(result.ok, false);
			assert.deepEqual(
				(await readTeamConfig("proof-unavailable", cwd))?.workers.map(
					(worker) => worker.name,
				),
				["worker-1"],
			);
			assert.equal(
				(await readTask("proof-unavailable", task.id, cwd))?.owner,
				undefined,
			);
	...
	it("converges entirely old or entirely new after a mid-commit scale-down interruption", async () => {
		...
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-down-atomic-bin-"),
		);
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const previousPath = process.env.PATH;
		try {
...
`,
			...
			const result = await scaleDown(
				...
				{
					OMX_TEAM_SCALING_ENABLED: "1",
					OMX_TEAM_SCALE_DOWN_INJECT_FAILURE: "after-first-task-write",
				},
			);
			assert.match(
				result.ok ? "" : result.error,
				/injected_scale_down_interruption:after-first-task-write/,
			);
	...
	it("recovers rollback-persistence failure through public config reads with config, manifest, and tasks converged", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-down-rollback-recovery-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-down-rollback-recovery-bin-"),
		);
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const previousPath = process.env.PATH;
		try {
...
`,
			...
			const failed = await scaleDown(
				"rollback-recovery",
				cwd,
				{ workerNames: ["worker-2"], force: true },
				{
					OMX_TEAM_SCALING_ENABLED: "1",
					OMX_TEAM_SCALE_DOWN_INJECT_FAILURE: "rollback-persistence-failure",
				},
			);
			assert.match(
			...
			const recoveredManifest = await readTeamManifestV2(
				"rollback-recovery",
				cwd,
			);
			const recoveredTask = await readTask("rollback-recovery", task.id, cwd);
			assert.equal(
				recoveredConfig?.workers.some((worker) => worker.name === "worker-2"),
				false,
			);
			assert.equal(
				recoveredManifest?.workers.some((worker) => worker.name === "worker-2"),
				false,
			);
	...
	for (const target of ["config", "manifest"] as const) {
		it(`publicly recovers a committed scale-up membership rollback after partial ${target} old-generation persistence`, async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-scale-up-rollback-${target}-`),
			);
			try {
				...
				const task = await createTask(
					teamName,
					{
						...
						description: "must converge with membership",
						status: "pending",
						owner: "worker-2",
					},
					cwd,
				...
				const oldConfigValue = JSON.parse(oldConfig) as {
					workers: Array<{ name: string }>;
				};
				const nextConfig = {
					...oldConfigValue,
					workers: oldConfigValue.workers.filter(
						(worker) => worker.name !== "worker-2",
					),
				};
				const nextManifest = {
					...(JSON.parse(oldManifest) as object),
					workers: nextConfig.workers,
					worker_count: nextConfig.workers.length,
				};
				const nextTask = {
					...(JSON.parse(oldTask) as object),
					owner: undefined,
					claim: undefined,
				};
				await assert.rejects(
					withTeamTaskBarrier(teamName, cwd, () =>
						commitTeamMembershipTaskTransaction(teamName, cwd, {
							...
							tasks: [
								{
									taskId: task.id,
									oldBytes: oldTask,
									newBytes: JSON.stringify(nextTask, null, 2),
								},
							],
							config: {
								oldBytes: oldConfig,
							...
							manifest: {
								oldBytes: oldManifest,
								newBytes: JSON.stringify(nextManifest, null, 2),
							},
				...
				assert.equal(
					(await readTeamConfig(teamName, cwd))?.workers.some(
						(worker) => worker.name === "worker-2",
					),
					false,
				);
				assert.equal(
					(await readTeamManifestV2(teamName, cwd))?.workers.some(
						(worker) => worker.name === "worker-2",
					),
					false,
				);
				assert.equal(
					(await readTask(teamName, task.id, cwd))?.owner,
					undefined,
				);
			} finally {
				await rm(cwd, { recursive: true, force: true });
	...
	it("discards precommit cleanup debt while removed workers remain canonical", async () => {
		...
		try {
			...
			const recovered = await reconcileScaleDownCleanupDebt(
				"precommit-debt",
				cwd,
				config,
			);
			...
			assert.equal(
				(await readTeamConfig("precommit-debt", cwd))?.workers.some(
					(worker) => worker.name === "worker-2",
				),
				true,
			);
	...
	it("commits forward state and records unresolved debt when proof is lost after a pane kill", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-down-success-proof-loss-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-down-success-proof-loss-bin-"),
		);
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
...
`,
			...
			const resolvedTask = await createTask(
				"success-proof-loss",
				{
					subject: "resolved owner task",
					description: "must be reclaimed",
					status: "pending",
					owner: "worker-2",
				},
				cwd,
			);
			const unresolvedTask = await createTask(
				"success-proof-loss",
				{
					subject: "unresolved owner task",
			...
			const resolvedClaim = await claimTask(
				"success-proof-loss",
				resolvedTask.id,
				"worker-2",
				resolvedTask.version ?? 1,
				cwd,
			);
			assert.equal(resolvedClaim.ok, true);
			const worker3StatusPath = join(
				cwd,
				".omx",
				"state",
				"team",
			...
			if (!result.ok)
				assert.match(
					result.error,
					/^scale_down_pane_proof_unavailable:%13:query_failed/,
				);
			assert.deepEqual(
				(await readTeamConfig("success-proof-loss", cwd))?.workers.map(
					(worker) => worker.name,
				),
				["worker-1"],
			);
			...
			assert.equal(
				(await readTask("success-proof-loss", resolvedTask.id, cwd))?.owner,
				undefined,
			);
			assert.equal(
				(await readTask("success-proof-loss", unresolvedTask.id, cwd))?.owner,
				undefined,
			);
			const retainedTask = await readTask(
				"success-proof-loss",
				resolvedTask.id,
				cwd,
			);
			...
			assert.deepEqual(
				debt.unresolved_panes.map((pane: { pane_id: string }) => pane.pane_id),
				["%13", "%14"],
			);
			const tmuxCommands = await readScaleUpTmuxLogCommands(tmuxLogPath);
			assert.deepEqual(
				tmuxCommands.filter((command) => command.startsWith("kill-pane ")),
				["kill-pane -t %13"],
			);
			...
			assert.deepEqual(
				recoveredCommands.filter((command) => command.startsWith("kill-pane ")),
				["kill-pane -t %13", "kill-pane -t %13", "kill-pane -t %14"],
			);
		} finally {
			if (typeof previousPath === "string") process.env.PATH = previousPath;
			else delete process.env.PATH;
	...
	it("commits forward membership and retains exact debt when a later pane teardown fails", async () => {
		...
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-down-gone-kill-fail-bin-"),
		);
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const previousPath = process.env.PATH;
...
`,
			...
			if (!result.ok)
				assert.match(
					result.error,
					/^scale_down_cleanup_debt:pane_teardown_failed:%14/,
				);
			assert.deepEqual(
				(await readTeamConfig("gone-kill-fail", cwd))?.workers.map(
					(worker) => worker.name,
				),
				["worker-1"],
			);
			assert.equal(existsSync(worker3StatusPath), true);
			assert.equal(
				await readFile(worker3StatusPath, "utf8"),
				'{\n "state" : "idle", "reason":"three", "updated_at":"2026-07-14T00:00:01.000Z"\n}\n',
			);
			assert.equal(
				(await readTask("gone-kill-fail", resolvedTask.id, cwd))?.owner,
				undefined,
			);
			assert.equal(
				(await readTask("gone-kill-fail", unresolvedTask.id, cwd))?.owner,
				undefined,
			);
			const retainedTask = await readTask(
				"gone-kill-fail",
	...
	for (const [name, priorRaw] of [
		...
		it(`scaleDown restores the exact ${name} after kill-pane failure`, async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-scale-down-status-artifact-"),
			);
			const fakeBinDir = await mkdtemp(
				join(tmpdir(), "omx-scale-down-status-artifact-bin-"),
			);
			const tmuxStubPath = join(fakeBinDir, "tmux");
			const previousPath = process.env.PATH;
			try {
...
`,
				...
				process.env.PATH = `${fakeBinDir}:${previousPath ?? ""}`;
				await initTeamState(
					`status-${name.startsWith("absent") ? "absent" : "malformed"}`,
					"task",
					"executor",
					2,
					cwd,
	...
	it("uses canonical membership rather than a stale caller config before consuming precommit debt", async () => {
		...
		try {
			...
			staleCaller.workers = staleCaller.workers.filter(
				(worker) => worker.name !== "worker-2",
			);
			...
			assert.deepEqual(
				await reconcileScaleDownCleanupDebt("canonical-debt", cwd, staleCaller),
				{ ok: true },
			);
			assert.equal(existsSync(debtPath), false);
			assert.equal(await readFile(sentinelPath, "utf8"), "preserve");
			assert.equal(
				(await readTeamConfig("canonical-debt", cwd))?.workers.some(
					(worker) => worker.name === "worker-2",
				),
				true,
			);
	...
	it("converges pane-less crash debt through worker status and directory cleanup", async () => {
		...
		try {
			...
			assert.ok(config);
			if (!config) return;
			config.workers = config.workers.filter(
				(worker) => worker.name !== "worker-2",
			);
			...
			assert.deepEqual(
				await reconcileScaleDownCleanupDebt("zero-pane-debt", cwd, config),
				{ ok: true },
			);
	...
	it("converges resource debt after a crash following successful worktree cleanup", async () => {
		...
		try {
			...
			assert.deepEqual(
				await reconcileScaleDownCleanupDebt("resource-retry", repo, config),
				{ ok: true },
			);
			assert.equal(existsSync(worktreePath), false);
			assert.equal(existsSync(workerDir), false);
	...
	it("rejects symlink-parent worktree debt without mutating the linked location", async () => {
		...
		try {
			...
			const expectedWorktreeRoot = join(
				...
				".omx",
				"team",
				"symlink-resource-debt",
				"worktrees",
			);
			await mkdir(join(repo, ".omx", "team", "symlink-resource-debt"), {
				recursive: true,
			});
			...
			await writeFile(
				...
				JSON.stringify({
					...
					resource_workers: [
						{
							name: "worker-2",
							worktree_path: join(expectedWorktreeRoot, "worker-2"),
							worktree_repo_root: repo,
							worktree_detached: true,
							worktree_created: true,
						},
	...
	it("rejects cleanup debt that collides with a surviving canonical worker before any tmux effect", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-down-debt-survivor-collision-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-down-debt-survivor-collision-bin-"),
		);
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const previousPath = process.env.PATH;
...
`,
			...
			config.workers = config.workers.filter(
				(worker) => worker.name !== "worker-2" && worker.name !== "worker-3",
			);
			config.worker_count = config.workers.length;
			config.tmux_pane_owner_id = "team:scale-down";
			await saveTeamConfig(config, cwd);
	...
	for (const [name, livePid, ownerMode, expectedReason] of [
		[
			"refuses an unrelated current pane occupant",
			42413,
			"foreign",
			"team_owner_mismatch",
		],
		[
			"refuses a PID-reused pane even with the Team owner tag",
			52413,
			"team",
			"pane_pid_changed",
		],
		[
			"fails closed when Team owner authorization is unavailable",
			42413,
			"unavailable",
		...
		it(name, async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-scale-down-debt-pane-authority-"),
			);
			const fakeBinDir = await mkdtemp(
				join(tmpdir(), "omx-scale-down-debt-pane-authority-bin-"),
			);
			const tmuxStubPath = join(fakeBinDir, "tmux");
			const tmuxLogPath = join(fakeBinDir, "tmux.log");
			const previousPath = process.env.PATH;
...
`,
				...
				await writeFile(
					...
					JSON.stringify({
						...
						removed_worker_names: ["worker-2"],
						workers: [
							{ name: "worker-2", index: 2, pane_id: "%13", pid: 42413 },
						],
						resource_workers: [{ name: "worker-2" }],
					}),
				);
				...
				assert.deepEqual(
					...
					{
						ok: false,
						error: "scale_down_cleanup_debt_unresolved:%13",
					},
				);
				const debt = JSON.parse(await readFile(debtPath, "utf8")) as {
					reasons: string[];
				};
				assert.ok(debt.reasons.includes(`%13:${expectedReason}`));
				const commands = existsSync(tmuxLogPath)
	...
	it("rejects mismatched worker pane bindings before any recovery mutation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-down-debt-mismatched-binding-"),
		);
		try {
			await initTeamState(
				"debt-mismatched-binding",
				"task",
				"executor",
				2,
				cwd,
			...
			await writeFile(
				...
				JSON.stringify({
					...
					workers: [{ name: "worker-2", index: 2, pane_id: "%13", pid: 42413 }],
					unresolved_panes: [
						{ name: "worker-2", index: 2, pane_id: "%14", pid: 42413 },
					],
					resource_workers: [{ name: "worker-2" }],
				}),
			);
	...
	it("converges a PID-less legacy pane record only after a fresh global gone proof", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-scale-down-debt-legacy-gone-"),
		);
		const fakeBinDir = await mkdtemp(
			join(tmpdir(), "omx-scale-down-debt-legacy-gone-bin-"),
		);
		const tmuxStubPath = join(fakeBinDir, "tmux");
		const tmuxLogPath = join(fakeBinDir, "tmux.log");
		const previousPath = process.env.PATH;
...
`,
			...
			await writeFile(
				...
				JSON.stringify({
					...
					unresolved_panes: [
						{ name: "worker-2", index: 2, pane_id: "%13", pid: null },
					],
					resource_workers: [{ name: "worker-2" }],
				}),
			);
			assert.deepEqual(
				await reconcileScaleDownCleanupDebt("debt-legacy-gone", cwd, config),
				{ ok: true },
			);
			assert.equal(existsSync(debtPath), false);
			assert.doesNotMatch(
				await readFile(tmuxLogPath, "utf8"),
				/kill-pane|show-option/,
			);
		} finally {
			if (typeof previousPath === "string") process.env.PATH = previousPath;
			else delete process.env.PATH;
