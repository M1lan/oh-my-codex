...
describe("buildRepoAwareTeamExecutionPlan", () => {
	...
	it("imports DAG sidecar, reduces implicit worker count, and preserves symbolic dependencies for runtime remap", () => {
		const cwd = repo();
		writeFileSync(
			join(cwd, ".omx", "plans", "team-dag-demo.json"),
			JSON.stringify({
				schema_version: 1,
				nodes: [
					{
		...
		assert.match(
			...
			/File scope: src\/team\/runtime.ts/,
		);
	});
				...
				nodes: [
					{
						...
						description: "Must not override normal startup",
					},
				],
			}),
		);
	...
	it("preserves explicit lifecycle fallback reasons when DAG handoff is disabled upstream", () => {
				...
				nodes: [
					{
						id: "stale",
						subject: "Stale sidecar",
						description: "Must not override lifecycle-rejected startup",
					},
				],
			}),
		);
		...
		assert.equal(
			plan.metadata?.fallback_reason,
			"context_pack_not_followup_ready:invalid",
		);
		assert.equal(plan.tasks[0].subject, "legacy");
	});
	...
	it("requires a matching approved test spec before importing an opted-in DAG sidecar", () => {
		...
		writeFileSync(
			join(cwd, ".omx", "plans", "test-spec-other.md"),
			"# Other tests\n",
		);
		writeFileSync(
			join(cwd, ".omx", "plans", "team-dag-demo.json"),
				...
				nodes: [
					{
						id: "impl",
						subject: "Implement",
						description: "Implement from DAG",
					},
				],
			}),
		);
	...
	it("carries approved repository context summary only when the launch gate supplies it", () => {
		...
		assert.equal(
			plan.metadata?.approved_context_summary?.content,
			"Approved context: runtime lives in src/team/runtime.ts",
		);
	});
	...
	it("honors CLI-explicit worker count beyond ready lanes", () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				schema_version: 1,
				nodes: [
					{ id: "impl", subject: "Implement one lane", description: "Do it" },
				],
