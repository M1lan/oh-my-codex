...
describe("allocation-policy", () => {
	...
	it("prefers a lighter lane for blocked work", () => {
		const decision = chooseTaskOwner(
			{
				subject: "resolve merge",
				description: "blocked on task 1",
				blocked_by: ["1"],
			},
			[
				{ name: "worker-1", role: "executor" },
	...
	it("load-balances same-role work when every worker shares the same role", () => {
		...
		assert.deepEqual(
			assignments.map((task) => task.owner),
	...
	it("keeps related file-path work on the same worker to reduce overlap", () => {
			...
			[
				{
					...
					description:
						"Implement incremental integration in src/team/runtime.ts and src/team/mcp-comm.ts",
					role: "executor",
				},
				{
					subject: "Runtime follow-up lane",
					description:
						"Add more runtime coverage for src/team/runtime.ts conflict handling",
					role: "executor",
				},
				{
		...
		assert.match(
			assignments[1].allocation_reason,
			/low-overlap file\/domain ownership/,
		);
	});
