...
function assertNoForbiddenGitStrategy(source: string, label: string): void {
	assert.doesNotMatch(
		source,
		/-X\s*(?:ours|theirs)\b/,
		`${label} must not use -X ours/theirs`,
	);
	assert.doesNotMatch(
		source,
		/--strategy-option[=\s](?:ours|theirs)\b/,
		`${label} must not use ours/theirs strategy options`,
	);
}

function assertNoFinalHistoryRewriteAutomation(
	source: string,
	label: string,
): void {
	assert.doesNotMatch(
		...
		`${label} must not automate final history rewrite`,
	);
}
...
describe("hook-primary PostToolUse E2E contract guardrails", () => {
	it("keeps buildNativePostToolUseOutput synchronous and free of async worker bridge side effects", async () => {
		...
		assert.doesNotMatch(
			source,
			/export async function buildNativePostToolUseOutput\s*\(/,
		);

		const body = functionBody(source, "buildNativePostToolUseOutput");
		assert.doesNotMatch(
			body,
			/handleTeamWorkerPostToolUseSuccess|team-worker-posttooluse/,
		);
	});
	...
	it("preserves fallback team monitoring instead of removing polling/runtime recovery", async () => {
		...
		const notifyWorkerSource = await readSource(
		...
		assert.match(
			notifyWorkerSource,
			/maybeNotifyLeaderWorkerIdle|updateWorkerHeartbeat/,
		);
	});
	...
	it("does not add conflict auto-repair or final-history rewrite automation to the hook path", async () => {
		const hookSources = [
			[
				"codex-native-hook",
				await readSource("src/scripts/codex-native-hook.ts"),
			],
			[
				"notify-hook/team-worker",
				await readSource("src/scripts/notify-hook/team-worker.ts"),
			],
			[
				"notify-hook/team-worker-posttooluse",
				await readOptionalSource(
	...
	it("keeps any worker PostToolUse bridge constrained to successful Bash events", async () => {
		const bridgeSource = await readOptionalSource(
			"src/scripts/notify-hook/team-worker-posttooluse.ts",
		);
		if (!bridgeSource) {
			assert.equal(
				bridgeSource,
				"",
				"bridge source absent in this worktree; dispatch tests cover current no-op path",
			);
			return;
		}
