...
describe("team/state/events", () => {
	...
	it("treats integration requests, merge conflicts, and stale alerts as wakeable while keeping audit-only events filtered", async () => {
		...
		try {
			...
			await appendTeamEvent(
				"wakeable-matrix",
				{
					...
					metadata: {
						...
						dedupe_key:
							"wakeable-matrix|worker-1|def456|abc123|leader_integration_attempt",
					},
				},
				cwd,
			);
			...
			assert.equal(
				wakeable[0]?.metadata?.diff_path,
				"/tmp/team/worktrees/worker-1/.omx/diff.md",
			);
			assert.deepEqual(wakeable[1]?.metadata?.conflict_files, [
				"src/team/runtime.ts",
			]);
			assert.deepEqual(wakeable[2]?.metadata?.conflict_files, [
				"src/team/runtime.ts",
			]);
			assert.equal(
				wakeable[3]?.metadata?.operation_kind,
				"leader_integration_attempt",
			);
			assert.deepEqual(wakeable[4]?.metadata?.conflict_files, [
				"src/team/runtime.ts",
			]);
	...
	it("awaits worker integration requests through the same wakeable event path leader integrations use", async () => {
		...
		try {
			...
			assert.equal(
				result.event?.metadata?.operation_kind,
				"leader_integration_attempt",
			);
		} finally {
			await cleanup();
	...
	it("preserves metadata for diff and merge events while filtering wakeable events correctly", async () => {
		...
		try {
			...
			const allEvents = await readTeamEvents("metadata-wakeable", cwd, {
				wakeableOnly: false,
			});
			const diffReport = allEvents.find(
				(event) => event.type === "worker_diff_report",
			);
			const mergeConflict = allEvents.find(
				(event) => event.type === "worker_merge_conflict",
			);
			const staleStdout = allEvents.find(
				(event) => event.type === "worker_stale_stdout",
			);

			assert.equal(diffReport?.metadata?.summary, "worker diff report");
			assert.equal(
				diffReport?.metadata?.diff_path,
				"/tmp/team/worktrees/worker-1/.omx/diff.md",
			);
			assert.equal(mergeConflict?.metadata?.summary, "merge conflict");
			assert.deepEqual(mergeConflict?.metadata?.conflict_files, [
				"src/team/runtime.ts",
			]);
			assert.equal(staleStdout?.metadata?.stale_window_ms, 60_000);

			const wakeableEvents = await readTeamEvents("metadata-wakeable", cwd, {
				wakeableOnly: true,
			});
			assert.equal(
				wakeableEvents.some((event) => event.type === "worker_diff_report"),
				false,
			);
			assert.equal(
				wakeableEvents.some((event) => event.type === "worker_merge_conflict"),
				true,
			);
			assert.equal(
				wakeableEvents.some((event) => event.type === "worker_stale_stdout"),
				true,
			);
		} finally {
			await cleanup();
	...
	it("keeps newly proposed canonical worker event names readable through appendTeamEvent", async () => {
		...
		try {
			...
			const events = await readTeamEvents("canonical-worker-events", cwd, {
				wakeableOnly: false,
			});
