...
describe("team model contract", () => {
	...
	it("maps worker roles through configured per-agent reasoning overrides and invalid fallback", async () => {
		const codexHome = await mkdtemp(
			join(tmpdir(), "omx-model-contract-reasoning-"),
		);
		try {
			await writeFile(
	...
	it("maps worker roles to configured default model lanes", () => {
		withIsolatedDefaultModelEnv(() => {
			assert.equal(
				resolveAgentDefaultModel("explore"),
				expectedLowComplexityModel(),
			);
	...
	it("honors per-agent model overrides before class and spark fallback routing", async () => {
		const codexHome = await mkdtemp(
			join(tmpdir(), "omx-model-contract-agent-models-"),
		);
		try {
			await writeFile(
				join(codexHome, ".omx-config.json"),
			...
			assert.equal(
				resolveAgentDefaultModel("architect", codexHome),
				"gpt-5.6-sol-architect",
			);
			assert.equal(
				resolveAgentDefaultModel("explore", codexHome),
				"gpt-5.6-sol-explore",
			);
		} finally {
			await rm(codexHome, { recursive: true, force: true });
	...
	it("keeps assigned worker roles as their own runtime identity", () => {
		withIsolatedDefaultModelEnv(() => {
			...
			assert.equal(resolveAgentReasoningEffort("explore"), "low");
			assert.equal(
				resolveAgentDefaultModel("style-reviewer"),
				expectedLowComplexityModel(),
			);
	...
	it("reports inherited parent model separately from role default reasoning", () => {
		withIsolatedDefaultModelEnv(() => {
			...
			assert.equal(
				diagnostics.requestedDefaultModel,
				expectedLowComplexityModel(),
			);
...
describe("resolveTeamWorkerLaunchArgs - teammate reasoning allocation", () => {
	...
	it("does not auto-inject thinking level for fallback model when no preference is provided", () => {
		...
		assert.ok(
			!joined.includes("model_reasoning_effort"),
			`Expected no auto-injected thinking level in: ${joined}`,
		);
	});
	...
	it("preserves explicit reasoning override over teammate preference", () => {
		...
		assert.ok(
			joined.includes('model_reasoning_effort="high"'),
			`Expected explicit high level in: ${joined}`,
		);
		...
		assert.equal(
			matches.length,
			1,
			"reasoning override should appear exactly once",
		);
	});
	...
	it("applies the full frozen Team reasoning precedence and diagnostics matrix", () => {
		const cases: Array<{
			...
			},
			{
				...
				existingRaw:
					"-c model_reasoning_effort=low -c model_reasoning_effort=max",
				preferredReasoning: "high",
				expectedReasoningToken: "model_reasoning_effort=max",
				expectedSource: "explicit",
				expectedActual: "max",
		...
		for (const testCase of cases) {
			...
			assert.deepEqual(
				...
				testCase.expectedReasoningToken
					? ["-c", testCase.expectedReasoningToken]
					: [],
				testCase.name,
			);
			assert.equal(
			...
			assert.equal(
				diagnostics.actualReasoning,
				testCase.expectedActual,
				testCase.name,
			);
			assert.equal(
				diagnostics.requestedDefaultReasoning,
				testCase.preferredReasoning,
				testCase.name,
			);
		}
	});
	it("does not inject thinking when model is explicit but reasoning is omitted", () => {
		...
		assert.ok(
			!joined.includes("model_reasoning_effort"),
			`Expected no reasoning in: ${joined}`,
		);
	});
});
...
describe("explicit team worker policy contract", () => {
	it("tokenizes quote-aware launch args and reversibly serializes empty and literal tokens", () => {
		...
		assert.deepEqual(
			splitWorkerLaunchArgs(serializeTeamWorkerLaunchArgs(tokens)),
			tokens,
		);
	...
	it("preserves literal Windows backslashes, including terminal backslashes, through transport", () => {
		...
		const raw = [path, terminalPath, `"${path}"`, `'${terminalPath}'`].join(
			" ",
		);
		...
		assert.deepEqual(
			splitWorkerLaunchArgs(serializeTeamWorkerLaunchArgs(expected)),
			expected,
		);
	});
	...
	it("preserves raw UNC and device backslash runs unquoted, quoted, and serialized", () => {
		...
		assert.deepEqual(
			splitWorkerLaunchArgs(`${uncPath} ${devicePath}`),
			expected,
		);
		assert.deepEqual(
			splitWorkerLaunchArgs(`'${uncPath}' "${devicePath}"`),
	...
	it("rejects only unterminated quotes while retaining literal malformed-looking escapes", () => {
		...
		assert.deepEqual(
			splitWorkerLaunchArgs(
				["one\\q", "two\\", '"three\\q"', '"four\\\\path"'].join(" "),
			),
	...
	it("normalizes every long and short direct policy split and equals form", () => {
		const cases: Array<{ raw: string; expected: string[] }> = [
			{
				raw: "--ask-for-approval on-request",
				expected: ["--ask-for-approval", "on-request"],
			},
			{
				raw: "--ask-for-approval=on-request",
				expected: ["--ask-for-approval", "on-request"],
	...
	it("normalizes approval and sandbox config policy forms without a value allowlist", () => {
		const cases: Array<{ raw: string; expected: string[] }> = [
			{
				raw: "-c approval_policy=custom-approval",
				expected: ["--ask-for-approval", "custom-approval"],
			},
			{
				raw: "--config \"sandbox_mode = 'workspace-write'\"",
				expected: ["--sandbox", "workspace-write"],
			},
			{
				raw: `-c='approval_policy = "on-request"'`,
				expected: ["--ask-for-approval", "on-request"],
			},
			{
				raw: "--config=sandbox_mode=workspace-write",
		...
		for (const { raw, expected } of cases) {
			...
			assert.equal(
				parseTeamWorkerLaunchArgs(splitWorkerLaunchArgs(raw)).policyKind,
				"direct-policy",
				raw,
			);
		}
	...
	it("preserves unrelated config overrides while extracting only canonical provider and reasoning overrides", () => {
		assert.deepEqual(
			resolveTeamWorkerLaunchArgs({
				existingRaw:
					'--config unrelated=one -c unrelated=two -c=unrelated=three -c model_provider="envRouter" --config model_reasoning_effort=high',
			}),
			[
				"--config",
				"unrelated=one",
	...
	it("rejects missing, empty, and option-shaped direct or config policy values", () => {
		for (const raw of [
			...
			"--config --ask-for-approval on-request",
			"-c=--sandbox",
			"--config=--ask-for-approval",
			"-c approval_policy=",
			"--config=sandbox_mode=",
		]) {
			assert.throws(
	...
	it("collapses matching direct and config policy values and rejects deterministic conflicts", () => {
		assert.deepEqual(
			resolveTeamWorkerLaunchArgs({
				existingRaw:
					'-a on-request --config=approval_policy="on-request" -s workspace-write -c=sandbox_mode=workspace-write',
			}),
			["--ask-for-approval", "on-request", "--sandbox", "workspace-write"],
		);
		assert.equal(
			parseTeamWorkerLaunchArgs(["-a", " on-request "]).approvalValue,
			" on-request ",
		);
		assert.throws(
			() =>
				resolveTeamWorkerLaunchArgs({
					existingRaw: '-a " on-request " --ask-for-approval=on-request',
				}),
		...
		assert.throws(
			() =>
				resolveTeamWorkerLaunchArgs({
					existingRaw: "-a on-request -c approval_policy=never",
				}),
			/conflicting duplicate approval policy/,
		);
		assert.throws(
			() =>
				resolveTeamWorkerLaunchArgs({
					existingRaw:
						"-c=sandbox_mode=workspace-write --sandbox danger-full-access",
				}),
	...
	it("keeps bypass-only input canonical and exposes direct policy metadata", () => {
		...
		assert.deepEqual(
			resolveTeamWorkerLaunchArgs({
				existingRaw: "--madmax --dangerously-bypass-approvals-and-sandbox",
			}),
			["--dangerously-bypass-approvals-and-sandbox"],
		);
	});
