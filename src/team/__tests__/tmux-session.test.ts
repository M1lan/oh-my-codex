...
async function withMockTmuxFixture<T>(
	...
	try {
		...
		const needsStandaloneGlobalProof =
			dirPrefix.includes("standalone") &&
			!fixtureScript.includes("list-panes)");
    ...
    ;;
        ...
        `;
				...
				async ({ logPath }) => {
					...
					assert.match(
						tmuxLog,
						/set-window-option -t leader:0 main-pane-width 60/,
					);
					assert.match(
						tmuxLog,
					...
					const redrawIndices = commands
						.map((command, index) =>
							command === "send-keys -t %1 C-l" ? index : -1,
						)
        ...
        `,
				async ({ logPath }) => {
					...
					const exactPaneEffects =
						/^(set-option -p -t %|split-window .* -t %|resize-pane -t %|select-pane -t %|send-keys -t %)/;
...
`,
			async () => {
				...
				assert.equal(
					isWorkerAlive("ignored-session", 2, "%88", 1, "team:liveness"),
					false,
				);
				assert.equal(
					isWorkerPaneOpen("ignored-session", 2, "%88", 1, "team:liveness"),
					false,
				);
			},
		);
