	...
	it("resolves the tmux executable twice for win32 client-attached one-shot hooks", async () => {
		...
		try {
			...
			assert.equal(
				matches.length,
				2,
				"client-attached hook should resolve tmux for both resize and unregister commands",
			);
			assert.doesNotMatch(
				args[4] ?? "",
    ...
    ;;
        ...
        `,
				async ({ logPath }) => {
					...
					assert.match(
						tmuxLog,
						/split-window -v -f -l 3 -t leader:0 -d -P -F #\{pane_id\}/,
					);
					assert.match(
						tmuxLog,
						/send-keys -t %1 C-l/,
    ...
    ;;
        ...
        `,
				async ({ logPath }) => {
					...
					assert.match(
						tmuxLog,
						/split-window -v -f -l 3 -t shared:0 -d -P -F #\{pane_id\}/,
					);
				},
			);
		} finally {
