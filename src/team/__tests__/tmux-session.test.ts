...
`
			: "";
		const standaloneGlobalPaneProofFallback =
			dirPrefix.includes("standalone") &&
...
`,
		...
		process.env.OMX_MUX_BINARY = tmuxStubPath;
			...
			async ({ logPath }) => {
				...
				const guarded = spawnSync(
					...
					[
						...
						"%1",
						"#{==:#{pane_pid},101}",
						"split-window -h -t %1 -P -F '#{pane_id}\tomx_source_recycled' \\; display-message -p 'omx_source_recycled'",
						"display-message -p ''",
					],
					{ encoding: "utf8" },
				);
...
describe("HUD resize hook command builders", () => {
	...
	it("buildRegisterResizeHookArgs uses target and numeric client-resized hook slot", () => {
		...
		assert.match(
			args[4] ?? "",
			new RegExp(`resize-pane -t %1 -y ${HUD_TMUX_TEAM_HEIGHT_LINES}`),
		);
	});
	...
	it("buildRegisterClientAttachedReconcileArgs installs one-shot client-attached reconcile hook", () => {
		...
		assert.match(
			args[4] ?? "",
			/set-hook -u -t my-session:0 client-attached\[\d+\]/,
		);
	});
	...
	it("preserves literal formats and TAB-delimited authoritative snapshots across all HUD callers", () => {
		...
		for (const command of commands) {
			...
			assert.equal(
				[...finalSnapshot].filter((character) => character === "\t").length,
				2,
			);
...
`,
			async ({ logPath }) => {
				...
				assert.match(
					commands,
					/show-option -qv -p -t %7 @omx_team_pane_owner_id/,
				);
	...
	it("buildScheduleDelayedHudResizeArgs schedules a proof-bearing tmux-side delayed reconcile", () => {
		...
		assert.match(
			args[2] ?? "",
			new RegExp(`sleep ${HUD_RESIZE_RECONCILE_DELAY_SECONDS};`),
		);
	...
	it("buildReconcileHudResizeArgs executes only after a global exact-pane proof", () => {
		...
		assert.match(
			args[1] ?? "",
			new RegExp(`resize-pane -t %7 -y ${HUD_TMUX_TEAM_HEIGHT_LINES}`),
		);
	});
...
`,
	...
	it("resolves the tmux executable for win32 hook shell snippets", async () => {
		...
		try {
			...
			const resizeArgs = buildRegisterResizeHookArgs(
				"my-session:0",
				"omx_resize_team_session_0_1",
				"%1",
			);
	...
	it("resolves the tmux executable twice for win32 client-attached one-shot hooks", async () => {
		...
		try {
			...
			const matches =
				(args[4] ?? "").match(new RegExp(escapeRegExp(tmuxPath), "g")) || [];
			assert.equal(
				matches.length,
				4,
				"client-attached hook should resolve tmux for separate resize and hook-unregister proofs",
			);
			assert.doesNotMatch(
				args[4] ?? "",
				/; tmux set-hook -u -t my-session:0 client-attached/,
			);
		} finally {
			if (origPlatform)
				Object.defineProperty(process, "platform", origPlatform);
...
describe("evaluateStartupDirectTriggerSafetyCapture", () => {
	it("classifies only the exact Codex bypass MDM rejection marker", () => {
		...
		assert.equal(
			classifyCodexBypassMdmIncompatibility(marker, "codex", bypassArgs),
			"codex_bypass_mdm_incompatible",
		);
		assert.equal(
			classifyCodexBypassMdmIncompatibility(
				`${marker} extra`,
		...
		assert.equal(
			classifyCodexBypassMdmIncompatibility(marker, "claude", bypassArgs),
			null,
		);
		assert.equal(
			classifyCodexBypassMdmIncompatibility(marker, "codex", directPolicyArgs),
			null,
			"direct policy without bypass must keep normal startup handling",
		);
		assert.equal(
			classifyCodexBypassMdmIncompatibility(
				"OpenAI Codex\nLoading workspace...",
			...
			null,
			"bypass without the exact marker must keep normal readiness/timeout handling",
		);
	});
	it("allows startup direct triggers on a ready prompt or Codex viewport", () => {
		assert.deepEqual(
			evaluateStartupDirectTriggerSafetyCapture(READY_HELPER_CAPTURE, "codex"),
			{
				safe: true,
				reason: "ready_prompt",
			},
...
`,
			async ({ logPath }) => {
				...
				const submitIndex = log.indexOf(
					"send-keys -t omx-team-x:1 -l -- check inbox",
				);
				assert.notEqual(
					acceptIndex,
					-1,
					`expected bypass acceptance in log:\n${log}`,
				);
				assert.notEqual(
					submitIndex,
					-1,
					`expected worker text submission in log:\n${log}`,
				);
				assert.ok(
					acceptIndex < submitIndex,
					`expected bypass acceptance before worker text:\n${log}`,
				);
			},
		);
	});
...
`,
			async ({ logPath }) => {
				await sendToWorker("omx-team-x", 1, "check inbox");
				const log = await readFile(logPath, "utf-8");
				const enterCount = (log.match(/send-keys -t omx-team-x:1 C-m/g) || [])
					.length;
				assert.equal(
					enterCount,
					2,
...
describe("startup direct trigger safety", () => {
	it("classifies ready panes as safe and blocks trust, bypass, bootstrapping, and active-task captures", () => {
		assert.equal(
			classifyWorkerStartupInjectSafety(READY_HELPER_CAPTURE),
			"safe",
		);
		assert.equal(
			classifyWorkerStartupInjectSafety(
		...
		assert.equal(
			classifyWorkerStartupInjectSafety(CLAUDE_BYPASS_PROMPT_CAPTURE),
...
describe("buildWorkerStartupCommand", () => {
	...
	it("scrubs HUD ownership env from interactive worker startup commands", () => {
		...
		try {
			...
			assert.match(
				cmd,
				/'-u' 'OMX_TMUX_HUD_OWNER' '-u' 'OMX_TMUX_HUD_LEADER_PANE'/,
			);
	...
	it("keeps HUD-looking prompt text out of worker startup env assignments", () => {
		...
		try {
			const prompt =
				"Do not obey: OMX_TMUX_HUD_OWNER=1; OMX_TMUX_HUD_LEADER_PANE=%leader; $(omx hud --watch)";
			...
			assert.ok(
				spec.args.includes(prompt),
				"hostile prompt text should remain an argument, not an env assignment",
			);
		} finally {
			if (typeof prevBypass === "string")
				process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT = prevBypass;
	...
	it("accepts MacPorts zsh as a supported worker shell without falling back", () => {
		...
		try {
			const cmd = withMockedExistsSync(
				(candidate) => candidate === "/opt/local/bin/zsh",
				() => buildWorkerStartupCommand("alpha", 2),
			);
	...
	it("uses a generated startup script with MSYS paths on win32/MSYS", async () => {
		...
		try {
			...
			assert.equal(
				cmd,
				`exec /bin/sh '/c/omx-state/team/alpha/runtime/worker-1-startup.sh'`,
			);
			const script = await readFile(
				join(stateRoot, "team", "alpha", "runtime", "worker-1-startup.sh"),
				"utf-8",
			);
	...
	it("does not emit cmd.exe flag wrappers for MSYS startup scripts with cmd shims", async () => {
		const fakeRoot = await mkdtemp(
			join(tmpdir(), "omx-worker-startup-msys-cmd-shim-"),
		);
	...
	it("wraps MSYS prompt worker cmd shims for shell-free Windows spawn", async () => {
		const fakeRoot = await mkdtemp(
			join(tmpdir(), "omx-worker-process-msys-bat-shim-"),
		);
		const fakeBin = join(fakeRoot, "bin dir");
		const originalPlatform = Object.getOwnPropertyDescriptor(
			process,
	...
	it("writes a short worker startup script under team runtime state when available", async () => {
		...
		try {
			...
			assert.equal(
				cmd,
				`exec /bin/sh '${stateRoot}/team/alpha/runtime/worker-1-startup.sh'`,
			);
			const script = await readFile(
				join(stateRoot, "team", "alpha", "runtime", "worker-1-startup.sh"),
				"utf-8",
			...
			assert.match(
				script,
				/^unset OMX_TMUX_HUD_OWNER OMX_TMUX_HUD_LEADER_PANE$/m,
			);
			...
			assert.doesNotMatch(
				cmd ?? "",
				/OMX_TEAM_STATE_ROOT=/,
				"tmux command should point at script instead of inlining env",
			);
		} finally {
			if (typeof prevShell === "string") process.env.SHELL = prevShell;
			else delete process.env.SHELL;
	...
	it("trusts worktree .mise.toml before worker launch when mise is available", async () => {
		...
		try {
			...
			assert.match(
				...
				new RegExp(
					`trust --yes ${wd.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&")}\\/\\.mise\\.toml`,
				),
			);
		} finally {
			if (typeof previousPath === "string") process.env.PATH = previousPath;
	...
	it("preserves explicit worker env precedence over inherited ambient proxy vars", () => {
		...
		try {
			...
			assert.doesNotMatch(
				cmd,
				/HTTPS_PROXY=https:\/\/ambient-proxy\.example:443/,
			);
		} finally {
			if (typeof prevShell === "string") process.env.SHELL = prevShell;
			else delete process.env.SHELL;
	...
	it("resolves POSIX leader paths before building fish worker startup commands", async () => {
		...
		try {
			...
			const { buildWorkerStartupCommand: buildFreshWorkerStartupCommand } =
				await import(`../tmux-session.js?posix-path=${Date.now()}`);
			...
			assert.match(
				cmd,
				new RegExp(escapeRegExp(`OMX_LEADER_NODE_PATH=${nodePath}`)),
			);
			assert.match(
				cmd,
				new RegExp(escapeRegExp(`OMX_LEADER_CLI_PATH=${codexPath}`)),
			);
			assert.match(
				cmd,
				new RegExp(
					escapeRegExp(
						`export PATH='\\''${fakeBin}'\\'':$PATH; exec '\\''${codexPath}'\\''`,
					),
				),
			);
	...
	it("recognizes every model-instructions config spelling before -- and ignores positional suffixes", () => {
		...
		try {
			for (const launchArgs of configForms) {
				...
				assert.equal(
					spec.args.filter((arg) => arg.includes("model_instructions_file="))
						.length,
					1,
				);
			...
			for (const launchArgs of configForms) {
				...
				assert.ok(
					spec.args
						.slice(0, marker)
						.some((arg) =>
							arg.includes('model_instructions_file="/tmp/project/AGENTS.md"'),
						),
				);
	...
	it("does not synthesize absent first-party OMX MCP server tables for Codex team workers", async () => {
		...
		try {
			...
			for (const server of [
				...
				assert.doesNotMatch(
					cmd,
					new RegExp(`mcp_servers\\.${server}\\.enabled=false`),
				);
			}
		} finally {
	...
	it("recognizes every MCP config spelling before -- and inserts generated overrides before positional suffixes", async () => {
		const stateRoot = await mkdtemp(
			join(tmpdir(), "omx-team-mcp-config-forms-"),
		);
		...
		try {
			...
			for (const [index, launchArgs] of configForms.entries()) {
				...
				const script = await readFile(
					join(
						...
						"alpha",
						"runtime",
						`worker-${index + 1}-startup.sh`,
					),
					"utf-8",
				);
				assert.equal(
					(script.match(/mcp_servers\.omx_state\.enabled=/g) ?? []).length,
					1,
				);
			...
			for (const [index, launchArgs] of configForms.entries()) {
				...
				const generatedIndex = script.indexOf(
					"mcp_servers.omx_state.enabled=false",
				);
				const positionalIndex = script.indexOf(
					"mcp_servers.omx_state.enabled=true",
				);
			...
			if (typeof previousCompat === "string")
				process.env.OMX_TEAM_WORKER_MCP_COMPAT = previousCompat;
			else delete process.env.OMX_TEAM_WORKER_MCP_COMPAT;
	...
	it("ignores unsupported SHELL values and resolves a supported worker shell", () => {
		...
		try {
			...
			assert.doesNotMatch(
				cmd,
				/fish/,
				"worker shell must not inherit unsupported fish SHELL",
			);
			assert.match(
				cmd,
				/\/(?:bin|usr\/bin|usr\/local\/bin|opt\/homebrew\/bin)\/(?:zsh|bash)\b|\/bin\/sh\b/,
			);
		} finally {
			if (typeof prevShell === "string") process.env.SHELL = prevShell;
			else delete process.env.SHELL;
	...
	it("uses a native PowerShell startup command on native Windows instead of /bin/sh -lc", async () => {
		...
		try {
			...
			assert.doesNotMatch(
				cmd,
				/\/bin\/sh -lc/,
				"native Windows workers must not launch through POSIX sh",
			);
			assert.match(
				cmd,
			...
			assert.match(
				decoded,
				new RegExp(escapeRegExp(`'-File' '${codexPs1Path}'`)),
			);
	...
	it("uses the resolved PowerShell executable path in native Windows startup commands", async () => {
		const fakeBin = await mkdtemp(
			join(tmpdir(), "omx-worker-startup-win32-powershell-"),
		);
		const prevPath = process.env.PATH;
		const prevPathext = process.env.PATHEXT;
		const prevBypass = process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT;
	...
	it("prefers a no-space native Windows PowerShell path when one is available", async () => {
		const fakeBin = await mkdtemp(
			join(tmpdir(), "omx-worker-startup-win32-nospace-powershell-"),
		);
		const prevPath = process.env.PATH;
		const prevPathext = process.env.PATHEXT;
		const prevBypass = process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT;
	...
	it("uses the resolved node-hosted Codex launcher in native Windows startup commands", async () => {
		const fakeRoot = await mkdtemp(
			join(tmpdir(), "omx-worker-startup-win32-node-hosted-"),
		);
		...
		try {
			...
			await mkdir(fakeBin, { recursive: true });
			await mkdir(join(fakeRoot, "node_modules", "@openai", "codex", "bin"), {
				recursive: true,
			});
			...
			const decoded = Buffer.from(cmd.slice(prefix.length), "base64").toString(
				"utf16le",
			);
			assert.match(
				decoded,
				new RegExp(escapeRegExp(`$env:OMX_LEADER_CLI_PATH = '${codexJsPath}'`)),
			);
			assert.match(
				decoded,
				new RegExp(
					escapeRegExp(
						`& '${process.execPath}' '${codexJsPath}' '--model' 'gpt-5' '--dangerously-bypass-approvals-and-sandbox'`,
					),
				),
			);
	...
	it("falls back to bash when SHELL is unsupported and zsh candidates are unavailable", () => {
		...
		try {
			const cmd = withMockedExistsSync(
				(candidate) =>
					candidate === "/opt/custom/fish" || candidate === "/bin/bash",
				() => buildWorkerStartupCommand("alpha", 1, [], process.cwd()),
			);
			assert.match(
				cmd,
				/\/bin\/bash\b/,
				"must fall back to bash when zsh is unavailable",
			);
			assert.doesNotMatch(
				cmd,
				/\.bashrc/,
				"must not source bash rc file for bash fallback by default",
			);
			assert.doesNotMatch(
				cmd,
				/fish/,
				"must not launch unsupported fish shell",
			);
		} finally {
			if (typeof prevShell === "string") process.env.SHELL = prevShell;
			else delete process.env.SHELL;
	...
	it("falls back to /bin/sh when no supported shell candidates exist", () => {
		...
		try {
			...
			assert.match(
				cmd,
				/'\/bin\/sh' -c\b/,
				"must launch workers through /bin/sh when no supported shells exist",
			);
			assert.doesNotMatch(cmd, /'\/bin\/sh' -lc\b/);
			assert.doesNotMatch(
				cmd,
				/\.zshrc|\.bashrc/,
				"must not source zsh/bash rc files for /bin/sh fallback",
			);
		} finally {
			if (typeof prevShell === "string") process.env.SHELL = prevShell;
			else delete process.env.SHELL;
...
describe("team worker launch mode helpers", () => {
	...
	it("preserves no-policy Codex bypass defaults exactly once for execution, absent, and unknown roles", () => {
		...
		try {
			...
			const readOnlySpec = buildWorkerProcessLaunchSpec(
				"policy-team",
				1,
				[],
				"/tmp/workspace",
			...
			assert.equal(
				readOnlySpec.args.includes(
					"--dangerously-bypass-approvals-and-sandbox",
				),
				false,
			);
			const bypassOnlySpec = buildWorkerProcessLaunchSpec(
				"policy-team",
				1,
				["--madmax"],
				"/tmp/workspace",
			...
			assert.deepEqual(bypassOnlySpec.args, [
				"--dangerously-bypass-approvals-and-sandbox",
			]);
		} finally {
			process.argv = previousArgv;
	...
	it("executes the generated POSIX startup script with canonical config policy and exact positional argv", async () => {
		const stateRoot = await mkdtemp(
			join(tmpdir(), "omx-worker-policy-script-"),
		);
		const codexHome = await mkdtemp(
			join(tmpdir(), "omx-worker-policy-codex-home-"),
		);
...
`,
			...
			const execution = spawnSync("/bin/sh", [scriptPath], {
				encoding: "utf-8",
			});
			...
			assert.match(
				script,
				new RegExp(escapeRegExp(`export CODEX_HOME='${codexHome}'`)),
			);
			assert.match(scriptCommand ?? "", /worker-1-startup\.sh/);
			assert.equal(execution.status, 0, execution.stderr);
			assert.deepEqual(
				JSON.parse(await readFile(capturePath, "utf-8")),
				expectedArgs,
			);
...
`,
			...
			if (typeof previousPathext === "string")
	...
	it("rejects Claude and Gemini restrictive policies before permission translation", () => {
		...
		try {
			for (const workerCli of ["claude", "gemini"] as const) {
				const incompatibility = new RegExp(
	...
	it("buildWorkerProcessLaunchSpec records the resolved node-hosted Codex launcher on native Windows", async () => {
		const fakeRoot = await mkdtemp(
			join(tmpdir(), "omx-worker-spec-win32-node-hosted-"),
		);
		const fakeBin = join(fakeRoot, "node_modules", ".bin");
		const prevPath = process.env.PATH;
		const prevPathext = process.env.PATHEXT;
	...
	it("buildWorkerProcessLaunchSpec preserves ambient CODEX_HOME so Codex workers keep provider websocket metadata", async () => {
		...
		const codexHome = await mkdtemp(
			join(tmpdir(), "omx-team-provider-websocket-"),
		);
		const sqliteHome = await mkdtemp(
			join(tmpdir(), "omx-team-provider-sqlite-"),
		);
		process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT = "0";
		process.env.CODEX_HOME = codexHome;
	...
	it("buildWorkerProcessLaunchSpec uses CLI model_provider override for Codex provider env injection", async () => {
		...
		const codexHome = await mkdtemp(
			join(tmpdir(), "omx-team-provider-cli-override-"),
		);
		process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT = "0";
		process.env.CODEX_HOME = codexHome;
	...
	it("buildWorkerProcessLaunchSpec reads provider env from worker CODEX_HOME override", async () => {
		...
		const leaderCodexHome = await mkdtemp(
			join(tmpdir(), "omx-team-provider-env-leader-"),
		);
		const workerCodexHome = await mkdtemp(
			join(tmpdir(), "omx-team-provider-env-worker-"),
		);
		process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT = "0";
		...
		try {
			await writeFile(
				join(leaderCodexHome, "config.toml"),
				[
			...
			await writeFile(
				join(workerCodexHome, "config.toml"),
				[
	...
	it("buildWorkerProcessLaunchSpec resolves relative worker CODEX_HOME against the worker cwd", async () => {
		...
		const leaderCwd = await mkdtemp(
			join(tmpdir(), "omx-team-provider-relative-leader-"),
		);
		const workerCwd = await mkdtemp(
			join(tmpdir(), "omx-team-provider-relative-worker-"),
		);
...
`,
	...
	it("classifies detached tmux incarnation evidence strictly", async () => {
		const cases = [
			{
				name: "exact",
				output: "omx-team-incarnation\t$41\t1700000001",
				expected: {
					status: "exact",
					incarnation: { sessionId: "$41", sessionCreated: "1700000001" },
				},
			},
			{
				name: "session-id replacement",
				output: "omx-team-incarnation\t$42\t1700000001",
				expected: {
					status: "replacement",
					incarnation: { sessionId: "$42", sessionCreated: "1700000001" },
				},
			},
			{
				name: "creation replacement",
				output: "omx-team-incarnation\t$41\t1700000002",
				expected: {
					status: "replacement",
					incarnation: { sessionId: "$41", sessionCreated: "1700000002" },
				},
			},
		] as const;
...
`,
				async () => {
					assert.deepEqual(
						queryDetachedTeamSession("omx-team-incarnation", {
							sessionId: "$41",
...
`,
			async ({ logPath }) => {
				...
				assert.match(
					log,
					/#\{==:#\{@omx_team_pane_owner_id\},team:incarnation\}/,
				);
...
`,
			async ({ logPath }) => {
				assert.equal(
					requestDetachedTeamSessionDestroy("omx-team-denied", {
						sessionId: "$41",
						sessionCreated: "1700000001",
						leaderPaneId: "%1",
						leaderPanePid: 4241,
		...
		const cases = [
			{
				...
				panePid: 4241,
				owner: "team:final",
				session: "$42\t1700000002",
			},
			{
				name: "pid",
				panePid: 9999,
				owner: "team:final",
				session: "$41\t1700000001",
			},
			{
				name: "owner",
				panePid: 4241,
...
`,
				async ({ logPath }) => {
					assert.equal(
						requestDetachedTeamSessionDestroy("omx-team-final", {
							sessionId: "$41",
							sessionCreated: "1700000001",
							leaderPaneId: "%1",
							leaderPanePid: 4241,
...
`,
			async () =>
				assert.deepEqual(queryDetachedTeamSession("omx-team-incarnation"), {
					status: "unavailable",
					detail: "malformed_session_incarnation",
				}),
		);
		await withMockTmuxFixture(
...
`,
			async ({ logPath }) => {
				assert.equal(
					await waitForWorkerReadyAsync("omx-team-x", 1, 1_000),
					true,
				);
				const log = await readFile(logPath, "utf-8");
				assert.match(log, /capture-pane -t omx-team-x:1 -p/);
...
`,
			async ({ logPath }) => {
				assert.equal(
					await waitForWorkerReadyAsync("omx-team-x", 1, 250),
					false,
				);
				const log = await readFile(logPath, "utf-8");
				assert.match(log, /capture-pane -t omx-team-x:1 -p/);
...
`,
				async ({ logPath }) => {
					assert.equal(
						await waitForWorkerReadyAsync("omx-team-x", 1, 5_000),
						true,
					);
					const log = await readFile(logPath, "utf-8");
					assert.match(log, /send-keys -t omx-team-x:1 C-m/);
	...
	it("fails closed on multi-row worker and HUD split-window output", async () => {
		...
		try {
			for (const scenario of [
				{
					name: "worker",
					ambiguousPane: "worker",
					reconciliationFailure: null,
					extraWorkerPane: null,
					workerOutput: "%%2\\n%%9\\n",
					hudOutput: "%%3\\n",
				},
				{
					...
					ambiguousPane: "worker",
					reconciliationFailure: null,
					extraWorkerPane: "foreign",
					workerOutput: "%%2\\n%%9\\n",
					hudOutput: "%%3\\n",
				},
				{
					name: "worker-multi-owned",
					ambiguousPane: "worker",
					reconciliationFailure: null,
					extraWorkerPane: "owned",
					workerOutput: "%%2\\n%%9\\n",
					hudOutput: "%%3\\n",
				},
				{
				...
				{
					name: "worker-reconciliation",
					ambiguousPane: "worker",
					reconciliationFailure: "worker",
					extraWorkerPane: null,
					workerOutput: "%%2\\n%%9\\n",
					hudOutput: "%%3\\n",
				},
				{
					name: "hud-reconciliation",
					ambiguousPane: "hud",
					reconciliationFailure: "hud",
					extraWorkerPane: null,
					workerOutput: "%%2\\n",
					hudOutput: "%%3\\n%%9\\n",
				},
				...
				const cwd = await mkdtemp(
					join(tmpdir(), `omx-team-${scenario.name}-split-output-`),
				);
				try {
					await withMockTmuxFixture(
...
`,
						async ({ logPath }) => {
							...
							assert.throws(
								...
								(error: unknown) => {
									...
									if (scenario.reconciliationFailure) {
										...
										assert.deepEqual(
											error.partialSession.workerPaneIdsByIndex,
											[null],
										);
										assert.deepEqual(
										...
										assert.deepEqual(
											error.partialSession.startupCleanupPanes,
											[],
										);
										assert.equal(error.partialSession.hudPaneId, null);
										assert.ok(
											error.cleanupErrors.some((message) =>
												/failed to kill tmux pane %2/.test(message),
											),
										);
										reconciledWorkerPid =
											error.partialSession.workerPanePidsByIndex?.[0] ?? null;
									} else {
										assert.deepEqual(error.partialSession.workerPaneIds, []);
										assert.deepEqual(
											error.partialSession.workerPaneIdsByIndex,
							...
							if (scenario.name === "worker-multi-owned") {
								...
								scenario.ambiguousPane === "worker" &&
								!scenario.reconciliationFailure
							) {
								...
								const teardown = await teardownWorkerPanes(["%2"], {
									...
									authorizePaneKill: (paneId, proof) =>
										paneId === "%2" && proof.pid === reconciledWorkerPid,
								});
	...
	it("rejects pane-ID reuse of the pinned ambiguous worker PID during rollback and later teardown", async () => {
		...
		const prevWorkerCli = process.env.OMX_TEAM_WORKER_CLI;
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-worker-split-recycled-pid-"),
		);
		try {
			await withMockTmuxFixture(
				...
				async ({ logPath }) => {
					...
					assert.throws(
						...
						(error: unknown) => {
							...
							assert.ok(
								error.cleanupErrors.some((message) =>
									/tmux pane identity changed: %2/.test(message),
								),
					...
					assert.equal(
						fs.existsSync(`${logPath}.kills`),
						false,
						"recycled %2 must never be killed",
					);
					assert.equal(
						fs.existsSync(`${logPath}.worker`),
						true,
						"recycled pane remains as unresolved debt",
					);
					const teardown = await teardownWorkerPanes(["%2"], {
						graceMs: 1,
						expectedPanePids: { "%2": pinnedPid },
						authorizePaneKill: (paneId, proof) =>
							paneId === "%2" && proof.pid === pinnedPid,
					});
					assert.deepEqual(teardown.killedPaneIds, []);
					assert.equal(teardown.kill.attempted, 0);
					assert.ok(
						teardown.proofUnavailable.some(
							(proof) =>
								proof.paneId === "%2" && proof.reason === "pane_pid_changed",
						),
					...
					assert.equal(
						fs.existsSync(`${logPath}.kills`),
						false,
						"later teardown must not kill recycled %2",
					);
	...
	it("covers unavailable, gone, and fully-cleaned outcomes for a uniquely reconciled ambiguous worker pane", async () => {
		...
		try {
			for (const scenario of [
				...
				const cwd = await mkdtemp(
					join(tmpdir(), `omx-team-worker-split-${scenario.name}-`),
				);
				try {
					await withMockTmuxFixture(
							...
							if (scenario.proof === "unavailable") {
								assert.throws(
									...
									(error: unknown) => {
										...
										assert.ok(
											error.originalError instanceof Error &&
												error.originalError.name ===
													"ExactPaneProofUnavailableError",
											String(error.originalError),
										);
										assert.deepEqual(error.partialSession.workerPaneIds, [
											"%2",
										...
										assert.ok(
											error.proofUnavailable.some(
												(proof) =>
													proof.paneId === "%2" &&
													proof.reason === "query_failed",
											),
								...
								assert.throws(
									...
									(error: unknown) => {
										...
										assert.ok(
											error instanceof Error &&
												error.message === "tmux_split_window_output_ambiguous",
											String(error),
										);
										return true;
									},
								...
								assert.match(
									await readFile(`${logPath}.kills`, "utf-8"),
									/kill-pane -t %2/,
								);
								assert.equal(fs.existsSync(`${logPath}.worker`), false);
							}
...
`,
				async ({ logPath }) => {
					...
					for (const workerCli of ["claude", "gemini"] as const) {
						for (const scenario of [
							...
							const tmuxCommands = (await readFile(logPath, "utf-8"))
								.trim()
								.split("\n");
							assert.deepEqual(
								tmuxCommands,
			...
			if (typeof previousBypassInstructions === "string")
				process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT =
					previousBypassInstructions;
			else delete process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT;
			await rm(cwd, { recursive: true, force: true });
		}
	});
    ...
    ;;
        ...
        `;
				...
				async ({ logPath }) => {
					...
					assert.match(
						tmuxLog,
						/split-window -v -f -l 3 -t %1 -d -P -F #\{pane_id\}/,
					);
					const redrawTransactions = commands
						.map((command, index) => ({ command, index }))
						.filter(
							({ command }) =>
								command.includes("send-keys -t %1 C-l") &&
				...
				async ({ logPath }) => {
					...
					assert.throws(
						...
						(error: unknown) => {
							...
							assert.equal(
								error.partialSession.teamPaneOwnerId,
								"team:final-window-proof",
							);
							assert.deepEqual(error.partialSession.workerPaneIds, ["%2"]);
							assert.deepEqual(
								error.partialSession.workerPanePidsByIndex,
					...
					assert.match(
						commands,
						/show-option -qv -p -t %2 @omx_team_pane_owner_id/,
					);
    ...
    ;;
        ...
        `,
				async ({ logPath }) => {
					...
					assert.match(
						tmuxLog,
						/set-option -t shared @omx_instance_id omx-pane-scope/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %1 @omx_pane_instance_id omx-pane-scope/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %2 @omx_pane_instance_id omx-pane-scope/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %3 @omx_pane_instance_id omx-pane-scope/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %1 @omx_team_pane_owner_id team:pane-tags/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %2 @omx_team_pane_owner_id team:pane-tags/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %3 @omx_team_pane_owner_id team:pane-tags/,
					);
					assert.match(
						tmuxLog,
						/exec env OMX_SESSION_ID='omx-pane-scope' OMX_TMUX_HUD_OWNER=1 OMX_TMUX_HUD_LEADER_PANE='%1' .*hud --watch/,
					);
					const commands = tmuxLog.trim().split("\n").filter(Boolean);
					const globalExactPanePidProof =
						/^list-panes -a -F #\{pane_id\}\t#\{pane_dead\}\t#\{pane_pid\}$/;
					const targetScopedExactPaneSetProof =
						/^list-panes -t shared:0 -F #\{pane_id\}\t#\{pane_current_command\}\t#\{pane_start_command\}$/;
					const exactPaneEffects =
						/^(set-option -p -t %|split-window .* -t %|resize-pane -t %|select-pane -t %|send-keys -t %|if-shell -F -t %)/;
					for (const [index, command] of commands.entries()) {
						...
						const guardedAuthorityTransaction =
							...
							(/set-option(?: [^;]*)? @omx_(?:team_pane_owner_id|instance_id|pane_instance_id)\b/.test(
								command,
							) ||
								new RegExp(
									`#\\{==:#\\{@omx_team_pane_owner_id\\},${escapeRegExp(session.teamPaneOwnerId)}\\}`,
								).test(command));
						const hasAdjacentAuthority =
							globalExactPanePidProof.test(immediatelyPrevious) ||
				...
				async ({ logPath }) => {
					...
					assert.throws(
						...
						(error: unknown) =>
							error instanceof CreateTeamSessionPartialError &&
							error.originalError instanceof Error &&
							/tmux pane team owner changed: %2/.test(
								error.originalError.message,
							),
					);
					...
					assert.doesNotMatch(
						effects,
						/split-window -v|select-layout|set-window-option|resize-pane|select-pane|send-keys|set-hook|run-shell/,
					);
				},
			);
		} finally {
    ...
    ;;
        ...
        `,
				async ({ logPath }) => {
					...
					assert.match(
						tmuxLog,
						/set-option -p -t %1 @omx_team_pane_owner_id team:explicit-owner-boundary/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %2 @omx_team_pane_owner_id team:explicit-owner-boundary/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %3 @omx_team_pane_owner_id team:explicit-owner-boundary/,
					);
					assert.match(
						tmuxLog,
						/exec env OMX_TMUX_HUD_OWNER=1 OMX_TMUX_HUD_LEADER_PANE='%1' .*hud --watch/,
					);
					assert.doesNotMatch(
						tmuxLog,
						/OMX_SESSION_ID='team:explicit-owner-boundary'/,
					);
				},
			);
		} finally {
...
case "\${1:-}" in
  ...
  *)
    ...
    ;;
        ...
        `,
				async ({ logPath }) => {
					...
					assert.doesNotMatch(
						tmuxLog,
						/set-option|kill-pane|split-window|resize-pane|select-layout|set-window-option|select-pane|send-keys/,
					);
				},
			);
		} finally {
	...
	it("preserves rollback debt when a successfully tagged pane owner changes before cleanup", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-partial-rollback-proof-"),
		);
		const previousTmux = process.env.TMUX;
		const previousTmuxPane = process.env.TMUX_PANE;
		const previousWorkerCli = process.env.OMX_TEAM_WORKER_CLI;
...
`,
				async ({ logPath }) => {
					...
					assert.throws(
						...
						(error: unknown) => {
							...
							assert.ok(
								partial.proofUnavailable.length > 0 ||
									partial.cleanupErrors.length > 0,
							);
							return true;
						},
					);
	...
	for (const firstProofFailure of ["malformed", "unavailable"] as const) {
		it(`preserves unproven split pane debt when its first proof is ${firstProofFailure}`, async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-team-first-split-${firstProofFailure}-`),
			);
			const previousTmux = process.env.TMUX;
			const previousTmuxPane = process.env.TMUX_PANE;
			const previousWorkerCli = process.env.OMX_TEAM_WORKER_CLI;
					...
					async ({ logPath }) => {
						...
						assert.throws(
							...
							(error: unknown) => {
								assert.ok(error instanceof CreateTeamSessionPartialError);
								assert.deepEqual(error.partialSession.workerPaneIds, ["%2"]);
								assert.deepEqual(error.partialSession.workerPaneIdsByIndex, [
									"%2",
								]);
								assert.deepEqual(error.partialSession.workerPanePidsByIndex, [
									null,
								]);
...
`,
				async ({ logPath }) => {
					...
					assert.doesNotMatch(
						tmuxLog,
						/split-window|resize-pane|select-pane|send-keys/,
					);
				},
			);
		} finally {
...
`,
				async ({ logPath }) => {
					...
					assert.doesNotMatch(
						tmuxLog,
						/set-option|split-window|resize-pane|select-pane|send-keys/,
					);
				},
			);
		} finally {
	...
	it("guards duplicate standalone HUD removal with a fresh global pane proof", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-duplicate-hud-global-proof-"),
		);
		try {
			await withMockTmuxFixture(
...
`;
				...
				async ({ logPath }) => {
					...
					assert.match(
						commands[duplicateHudKill - 1] ?? "",
						/^list-panes -a -F #\{pane_id\}\t#\{pane_dead\}\t#\{pane_pid\}$/,
					);
					assert.equal(
						commands.filter((command) => command === "kill-pane -t %45").length,
						1,
					);
				},
			);
	...
	it("rejects a replaced leader PID or owner before standalone HUD topology discovery", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-standalone-hud-entry-authorization-"),
		);
		try {
			await withMockTmuxFixture(
...
`,
				async ({ logPath }) => {
					...
					assert.doesNotMatch(
						commands,
						/list-panes -t %11|kill-pane|split-window|resize-pane|select-pane/,
					);
				},
			);
		} finally {
	...
	it("fails closed before standalone HUD effects when leader topology query fails", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-standalone-hud-topology-query-failure-"),
		);
		try {
			await withMockTmuxFixture(
...
`,
	...
	it("fails closed before standalone HUD effects when leader topology is malformed", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-standalone-hud-topology-malformed-"),
		);
		try {
			await withMockTmuxFixture(
...
`,
	...
	it("does not adopt a leader PID replacement before standalone HUD topology discovery", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-standalone-hud-leader-pid-reuse-"),
		);
		try {
			await withMockTmuxFixture(
...
`,
				async ({ logPath }) => {
					...
					assert.doesNotMatch(
						commands,
						/list-panes -t %11|display-message|kill-pane|split-window|resize-pane|select-pane/,
					);
				},
			);
		} finally {
	...
	it("does not resize a PID-reused existing standalone HUD on POSIX", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-standalone-existing-hud-posix-pid-reuse-"),
		);
		try {
			await withMockTmuxFixture(
...
`,
	...
	it("does not resize a PID-reused newly created standalone HUD on POSIX", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-standalone-new-hud-posix-pid-reuse-"),
		);
		try {
			await withMockTmuxFixture(
				...
				async ({ logPath }) => {
					...
					assert.throws(
						...
						(error: unknown) =>
							...
							error.cleanupErrors.some((message) =>
								message.includes("failed to kill tmux pane %2"),
							),
					);
				},
			);
...
`,
				async ({ logPath }) => {
					...
					assert.match(
						tmuxLog,
						new RegExp(
							`run-shell -b sleep ${HUD_RESIZE_RECONCILE_DELAY_SECONDS}; .*resize-pane -t %3 -y ${HUD_TMUX_TEAM_HEIGHT_LINES}`,
						),
					);
					assert.match(
						tmuxLog,
						new RegExp(
							`run-shell .*resize-pane -t %3 -y ${HUD_TMUX_TEAM_HEIGHT_LINES}`,
						),
					);
					assert.doesNotMatch(tmuxLog, /kill-pane -t %2/);
	...
	it("unregisters only the successfully registered HUD hook after partial hook registration failure", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-partial-hook-registration-"),
		);
		const previousTmux = process.env.TMUX;
		const previousTmuxPane = process.env.TMUX_PANE;
		const previousWorkerCli = process.env.OMX_TEAM_WORKER_CLI;
...
`,
				async ({ logPath }) => {
					...
					const commands = (await readFile(logPath, "utf-8"))
						...
						.split("\n")
						.filter(Boolean);
					const registeredResize = commands.filter(
						(command) =>
							command.startsWith("set-hook ") &&
							command.includes("client-resized[") &&
							!command.startsWith("set-hook -u "),
					);
					const registeredAttached = commands.filter(
						(command) =>
							command.startsWith("set-hook ") &&
							command.includes("client-attached[") &&
							!command.startsWith("set-hook -u "),
					);
					const unregisteredResize = commands.filter(
						(command) =>
							command.startsWith("set-hook -u ") &&
							command.includes("client-resized["),
					);
					const unregisteredAttached = commands.filter(
						(command) =>
							command.startsWith("set-hook -u ") &&
							command.includes("client-attached["),
					);
...
`,
				async ({ logPath }) => {
					...
					assert.doesNotMatch(
						tmuxLog,
						/set-hook -t leader:0 client-attached\[\d+\]/,
					);
					assert.doesNotMatch(
						tmuxLog,
						/run-shell -b sleep \d+; tmux resize-pane -t %3 -y \d+ >/,
					);
					assert.doesNotMatch(
						tmuxLog,
						/run-shell tmux resize-pane -t %3 -y \d+ >/,
					);
				},
			);
		} finally {
...
`,
				async ({ logPath }) => {
					...
					const listPaneCalls =
						tmuxLog.match(
							/list-panes -t leader:0 -F #\{pane_id\}\t#\{pane_current_command\}\t#\{pane_start_command\}/g,
						) || [];
...
`,
				async ({ logPath }) => {
					...
					delete process.env.WSL_DISTRO_NAME;
					delete process.env.WSL_INTEROP;
					process.env.OMX_LEADER_NODE_PATH =
						"C:\\Program Files\\nodejs\\node.exe";
					Object.defineProperty(process, "platform", {
						value: "win32",
						configurable: true,
					...
					for (const [index, command] of commands.entries()) {
						if (
							/^(split-window .* -t %11|resize-pane -t %44|select-pane -t %11)( |$)/.test(
								command,
							)
						) {
							assert.match(
					...
					assert.doesNotMatch(
						tmuxLog,
						/run-shell tmux resize-pane -t %44 -y \d+ >/,
					);
	...
	it("pins and replays restored HUD debt when successful split output is ambiguous", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-restored-hud-ambiguous-output-"),
		);
		try {
			await withMockTmuxFixture(
...
`;
				...
				async ({ logPath }) => {
					...
					const debt = JSON.parse(await readFile(debtPath, "utf-8")) as Record<
						string,
						unknown
					>;
	...
	it("rejects restored HUD debt roots outside the canonical direct Team root before splitting", async () => {
		...
		const externalRoot = await mkdtemp(
			join(tmpdir(), "omx-restored-hud-debt-external-"),
		);
...
`;
				...
				async ({ logPath }) => {
					...
					const crashWindowDebt = JSON.parse(
	...
	it("retains restored-HUD debt for a same-ID/PID non-HUD replacement after leader authorization", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-restored-hud-final-command-change-"),
		);
		try {
			await withMockTmuxFixture(
...
`,
				async ({ logPath }) => {
					...
					const commands = (await readFile(logPath, "utf-8"))
						...
						.split("\n")
						.filter(Boolean);
					const topologyIndex = commands.findIndex((command) =>
						command.startsWith("list-panes -t %11 -F "),
					);
					const ownerIndex = commands.findIndex(
						(command) =>
							command === "show-option -qv -p -t %11 @omx_team_pane_owner_id",
					);
					assert.ok(ownerIndex >= 0 && topologyIndex > ownerIndex);
					assert.equal(
						commands
							.slice(topologyIndex + 1)
							.filter((command) => command.startsWith("list-panes -a ")).length,
						0,
					);
				},
			);
...
`;
	...
	it("restores standalone HUD panes from the live leader pane cwd with explicit session ownership", async () => {
		const teamLaunchCwd = await mkdtemp(
			join(tmpdir(), "omx-standalone-team-launch-cwd-"),
		);
		const leaderPaneCwd = await mkdtemp(
			join(tmpdir(), "omx-standalone-leader-pane-cwd-"),
		);
...
`,
				async ({ logPath }) => {
					...
					assert.match(
						tmuxLog,
						/display-message -p -t %11 #\{pane_current_path\}/,
					);
					assert.match(
						tmuxLog,
						new RegExp(
					...
					assert.doesNotMatch(
						tmuxLog,
						new RegExp(`split-window .* -c ${escapeRegExp(teamLaunchCwd)} `),
					);
					assert.match(
						tmuxLog,
	...
	it("falls back to the team launch cwd when the live leader pane cwd is unusable", async () => {
		const teamLaunchCwd = await mkdtemp(
			join(tmpdir(), "omx-standalone-team-launch-cwd-fallback-"),
		);
		const deletedLeaderPaneCwd = await mkdtemp(
			join(tmpdir(), "omx-standalone-deleted-leader-pane-cwd-"),
		);
...
`,
				async ({ logPath }) => {
					...
					assert.match(
						tmuxLog,
						new RegExp(
							`split-window -v -l ${HUD_TMUX_TEAM_HEIGHT_LINES} -t %11 -d -P -F #\\{pane_id\\} -c ${escapeRegExp(teamLaunchCwd)} `,
						),
					);
					assert.doesNotMatch(
						tmuxLog,
						new RegExp(
							`split-window .* -c ${escapeRegExp(deletedLeaderPaneCwd)} `,
						),
					);
					assert.match(tmuxLog, /select-pane -t %11/);
				},
	...
	it("keeps MSYS drive-style live leader cwd candidates without raw stat prefiltering", async () => {
		...
		const fakeCygpathDir = await mkdtemp(
			join(tmpdir(), "omx-msys-live-cygpath-"),
		);
		const previousPath = process.env.PATH;
		const previousMsystem = process.env.MSYSTEM;
		const previousOstype = process.env.OSTYPE;
...
`,
				async ({ logPath }) => {
					const paneId = withMockedStatSync(
						...
						() =>
							restoreStandaloneHudPane("%11", fallbackCwd, {
								sessionId: "current-session-for-hud",
							}),
					);
					assert.equal(paneId, "%44");
					...
					assert.match(
						tmuxLog,
						new RegExp(
							`split-window -v -l ${HUD_TMUX_TEAM_HEIGHT_LINES} -t %11 -d -P -F #\\{pane_id\\} -c ${escapeRegExp(liveLeaderCwd)} `,
						),
					);
					assert.doesNotMatch(
						tmuxLog,
						new RegExp(`split-window .* -c ${escapeRegExp(fallbackCwd)} `),
					);
					assert.match(tmuxLog, /select-pane -t %11/);
				},
			);
	...
	it("falls back after an unusable MSYS drive-style live leader cwd split attempt fails", async () => {
		...
		const fallbackCwd = "C:\\fallback";
		const fakeCygpathDir = await mkdtemp(
			join(tmpdir(), "omx-msys-deleted-live-cygpath-"),
		);
		const previousPath = process.env.PATH;
		const previousMsystem = process.env.MSYSTEM;
		const previousOstype = process.env.OSTYPE;
...
`,
				async ({ logPath }) => {
					...
					assert.ok(
						liveAttemptIndex >= 0,
						"deleted MSYS live cwd should still be attempted",
					);
					assert.ok(
						fallbackAttemptIndex > liveAttemptIndex,
						"fallback should be attempted after live split failure",
					);
					assert.match(tmuxLog, /select-pane -t %11/);
				},
			);
	...
	it("restores standalone HUD panes with an absolute OMX entry path after cwd drift", async () => {
		...
		const startupCwd = await mkdtemp(
			join(tmpdir(), "omx-standalone-relative-start-"),
		);
		const previousEntryPath = process.env[OMX_ENTRY_PATH_ENV];
...
`,
				async ({ logPath }) => {
					...
					assert.match(
						tmuxLog,
						/exec env OMX_TMUX_HUD_OWNER=1 OMX_TMUX_HUD_LEADER_PANE='%11' .*hud --watch/,
					);
				},
			);
		} finally {
...
`,
			async ({ logPath }) => {
				assert.equal(
					dismissTrustPromptIfPresent("ignored-session", 1, "%9"),
					false,
				);
				const log = await readFile(logPath, "utf-8");
...
`,
			async () => {
				...
				try {
					assert.equal(
						isWorkerAlive("ignored-session", 1, "%41", 41, "team:liveness"),
						false,
					);
					assert.equal(
						isWorkerAlive("ignored-session", 2, "%42", 42, "team:liveness"),
						false,
					);
					assert.equal(
						isWorkerAlive(
...
`,
			async () => {
				...
				try {
					...
					assert.equal(
						isWorkerAlive("ignored-session", 4, "%54", 54, "team:liveness"),
						false,
					);
					assert.equal(
						isWorkerAlive(
...
`,
			async () => {
				assert.deepEqual(readExactPaneProofSync("%1"), {
					status: "gone",
					paneId: "%1",
					reason: "absent",
				});
				assert.deepEqual(readExactPaneProofSync("%9"), {
					status: "gone",
					paneId: "%9",
					reason: "dead",
				});
				assert.deepEqual(readExactPaneProofSync("%404"), {
					status: "gone",
					paneId: "%404",
					reason: "absent",
				});
				...
				assert.equal(
					getWorkerPanePid("ignored-session", 1, "%13", pane13Pid, "team:rows"),
					pane13Pid,
				);
				assert.equal(
					getWorkerPanePid(
...
`,
	...
	it("treats malformed PIDs in dead and unrelated global rows as malformed snapshots", async () => {
		const proofStatePath = join(
			tmpdir(),
			`omx-exact-pane-invalid-pid-${process.pid}-${Date.now()}`,
		);
		await withMockTmuxFixture(
...
`,
			async ({ logPath }) => {
				...
				await assert.rejects(
					() => sendToWorker("ignored-session", 1, "check inbox", "%1"),
					/not proven live/,
				);
...
`,
			async ({ logPath }) => {
				...
				const commands = (await readFile(logPath, "utf-8"))
					...
					.split("\n")
					.filter(Boolean);
				const exactGlobalPaneProof =
					/^list-panes -a -F #\{pane_id\}\t#\{pane_dead\}\t#\{pane_pid\}$/;
				const effects = commands
					.map((command, index) => ({ command, index }))
					.filter(({ command }) =>
						/^(send-keys|kill-pane) -t %9\b/.test(command),
					);
				assert.ok(
					effects.length > 0,
					`expected an explicit pane effect in log:\n${commands.join("\n")}`,
				);
				for (const { command, index } of effects) {
					assert.match(
						commands[index - 1] ?? "",
						exactGlobalPaneProof,
						`fresh proof must immediately precede ${command}`,
					);
				}
...
`,
			async ({ logPath }) => {
				...
				assert.equal(
					enableMouseScrolling("omx-team-x", () => {
						authorizations += 1;
					}),
					true,
				);
				...
				assert.ok(
					authorizations > 2,
					"copy-mode reads and mutations must not share a mouse proof",
				);
			},
		);
	});
...
case "$1" in
  ...
  kill-pane)
    ...
    ;;
			...
			async ({ logPath }) => {
				const summary = await teardownWorkerPanes(["%77"], {
					graceMs: 1,
					expectedPanePids: { "%77": 7701 },
				});
				assert.equal(summary.kill.attempted, 1);
...
`;
			...
			async ({ logPath }) => {
				...
				assert.deepEqual(
					summary.proofUnavailable.map((proof) => proof.paneId),
					["%404"],
				);
				const commands = (await readFile(logPath, "utf-8"))
					.trim()
...
`,
			async ({ logPath }) => {
				const summary = await teardownWorkerPanes(["%404", "%405"], {
					graceMs: 1,
					expectedPanePids: { "%405": process.pid },
				});
				assert.equal(summary.kill.attempted, 1);
				assert.equal(summary.kill.succeeded, 0);
				assert.equal(summary.kill.failed, 1);
...
`,
			async ({ logPath }) => {
				...
				assert.deepEqual(
					summary.proofUnavailable.map((proof) => proof.reason),
					["pane_pid_changed"],
				);
				assert.doesNotMatch(await readFile(logPath, "utf8"), /kill-pane/);
			},
...
`,
			async ({ logPath }) => {
				...
				assert.doesNotMatch(
					log,
					/set-option|split-window|resize-pane|select-layout|set-window-option|select-pane|send-keys/,
				);
			},
		);
