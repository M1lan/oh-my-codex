...
function buildReadyApprovedTeamHint(
	...
	return {
		...
		repositoryContextSummary: {
			...
			content:
				"Read the approved repository slice before broader repo exploration.",
			truncated: false,
		},
		mode: "team",
...
describe("approved execution binding", () => {
	it("buildApprovedTeamHandoffSection renders ready approved Team baseline", () => {
		const handoff = buildApprovedTeamHandoffSection(
			buildReadyApprovedTeamHint(),
		);

		assert.match(
			handoff ?? "",
			/Approved plan: \/repo\/\.omx\/plans\/prd-issue-1314\.md/,
		);
		assert.match(
			handoff ?? "",
			/Test specs: \/repo\/\.omx\/plans\/test-spec-issue-1314\.md/,
		);
		assert.match(
			handoff ?? "",
			/Approved repository context summary source: \/repo\/\.omx\/plans\/repo-context-issue-1314\.md/,
		);
		assert.match(
			handoff ?? "",
			/Read the approved repository slice before broader repo exploration\./,
		...
		assert.doesNotMatch(
			handoff ?? "",
			/query the canonical pack|Context pack index/,
		);
	});
	...
	it("buildApprovedTeamHandoffSection stays undefined outside Team handoffs", () => {
		assert.equal(
			buildApprovedTeamHandoffSection(
				buildReadyApprovedTeamHint({ mode: "ralph" }),
			),
			undefined,
		);
	});
	...
	it("writes and reads a normalized approved execution binding under the team state root", async () => {
		await withUnboxedOmxRoot(async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-approved-execution-write-"),
			);
			...
			try {
				...
				const binding = await readPersistedApprovedTeamExecutionBinding(
					"alpha-team",
					cwd,
					stateRoot,
				);
	...
	it("resolves a valid continuity state for an exact approved team binding", async () => {
		await withUnboxedOmxRoot(async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-approved-execution-valid-"),
			);
			const stateRoot = join(cwd, ".omx", "state");
			try {
				...
				await writeFile(
					join(plansDir, "test-spec-issue-1314.md"),
					"# Test spec\n",
				);
				await writePersistedApprovedTeamExecutionBinding(
					"bound-team",
				...
				assert.equal(
					state.approvedHint.task,
					"Execute approved issue 1314 plan",
				);
			} finally {
				await rm(cwd, { recursive: true, force: true });
			}
	...
	it("reports an ambiguous continuity state when a task-only binding matches multiple team launch hints", async () => {
		await withUnboxedOmxRoot(async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-approved-execution-ambiguous-"),
			);
			const stateRoot = join(cwd, ".omx", "state");
			...
			try {
				...
				await writeFile(
					join(plansDir, "test-spec-issue-1316.md"),
					"# Test spec\n",
				);
				await writePersistedApprovedTeamExecutionBinding(
					"bound-team",
	...
	it("keeps an exact-command binding valid when the task text alone would be ambiguous", async () => {
		await withUnboxedOmxRoot(async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-approved-execution-command-"),
			);
			const stateRoot = join(cwd, ".omx", "state");
			...
			try {
				...
				await writeFile(
					join(plansDir, "test-spec-issue-1317.md"),
					"# Test spec\n",
				);
				await writePersistedApprovedTeamExecutionBinding(
					"bound-team",
	...
	it("keeps an exact-command binding valid when the approved team hint is wrapped across visible lines", async () => {
		await withUnboxedOmxRoot(async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-approved-execution-wrapped-command-"),
			);
			const stateRoot = join(cwd, ".omx", "state");
			...
			try {
				...
				await writeFile(
					join(plansDir, "test-spec-issue-1317-wrapped.md"),
					"# Test spec\n",
				);
				await writePersistedApprovedTeamExecutionBinding(
					"bound-team",
	...
	it("treats bindings without a matching test-spec baseline as stale", async () => {
		await withUnboxedOmxRoot(async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-approved-execution-missing-baseline-"),
			);
			const stateRoot = join(cwd, ".omx", "state");
			try {
				const plansDir = join(cwd, ".omx", "plans");
	...
	it("reports malformed and stale binding states explicitly", async () => {
		await withUnboxedOmxRoot(async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-approved-execution-invalid-"),
			);
			const stateRoot = join(cwd, ".omx", "state");
			try {
				...
				await writeFile(
					join(teamRoot, "approved-execution.json"),
					'{"prd_path":42}',
					"utf-8",
				);
				assert.equal(
				...
				assert.equal(
					JSON.parse(
						readFileSync(join(teamRoot, "approved-execution.json"), "utf-8"),
					).task,
	...
	it("rejects unsafe team names before resolving approved binding paths", async () => {
		await withUnboxedOmxRoot(async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-approved-execution-unsafe-team-"),
			);
			try {
				...
				assert.equal(
					existsSync(
						join(cwd, ".omx", "state", "escape", "approved-execution.json"),
					),
					false,
				);
				assert.throws(
