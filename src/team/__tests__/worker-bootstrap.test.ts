...
describe("worker bootstrap", () => {
	...
	it("team and worker skills document coordination activation heuristics", async () => {
		const teamSkill = await readFile(
	...
	it("includes concise CodeGraph guidance for shared leader indexes", () => {
		...
		assert.match(
			content,
			/does not install CodeGraph, auto-index worktrees, or copy\/symlink/,
		);
	});
	...
	it("fails closed when active Ultragoal goal is not in progress", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-ultragoal-stale-active-"),
		);
		try {
			await mkdir(join(wd, ".omx", "ultragoal"), { recursive: true });
			await writeFile(
	...
	it("generateInitialInbox includes scrum/team worker goal handoff tied to task IDs and claims", () => {
		...
		assert.match(
			inbox,
			/active claim owner: worker-4 until 2026-05-04T12:32:13\.456Z/,
		);
		...
		assert.match(
			inbox,
			/update_goal\(\{status: "complete"\}\).*verification evidence/i,
		);
		assert.match(
			inbox,
			/shell\/team APIs persist only OMX artifacts and task state/,
		);
	});

	it("generateInitialInbox includes repo-aware decomposition ownership hints", () => {
		const inbox = generateInitialInbox("worker-1", "team-dag", "executor", [
			{
				id: "1",
		...
		assert.match(
			inbox,
			/Allocation reason: preserves low-overlap file\/domain ownership/,
		);
	});
		...
		const tasks: TeamTask[] = [
			{
				...
				description:
					"Fan-out read-only sweep: each worker checks a separate doc with no shared files.",
				status: "pending",
				created_at: new Date(0).toISOString(),
				coordination: {
					mode: "lightweight",
					activation_reasons: ["explicit_independent_fanout"],
				},
			},
		];
		...
		const tasks: TeamTask[] = [
			{
				...
				description:
					"Coordinate handoff across a shared runtime contract and e2e verification.",
				status: "pending",
				depends_on: ["3"],
				created_at: new Date(0).toISOString(),
				coordination: {
	...
	it("generateInitialInbox includes Native Subagent Delegation Contract for broad delegated tasks", () => {
		const tasks: TeamTask[] = [
			{
				...
				delegation: {
					...
					subtask_candidates: [
						"Map runtime assignment flow",
						"Inspect bootstrap tests",
					],
		...
		assert.match(
			inbox,
			/before doing more than 3 serial repo-search\/read commands/i,
		);
	...
	it("generateInitialInbox resolves missing child_model through OMX_TEAM_CHILD_MODEL", () => {
		...
		try {
			...
			if (typeof previousChildModel === "string")
				process.env.OMX_TEAM_CHILD_MODEL = previousChildModel;
	...
	it("buildLeaderMailboxTriggerDirective records leader mailbox-review intent separately", () => {
		const directive = buildLeaderMailboxTriggerDirective(
			"team-mail",
			"worker-2",
		);
		assert.equal(directive.intent, "pending-mailbox-review");
	...
	it("generateWorkerRootAgentsContent includes hardcoded paths and role prompt", () => {
		...
		assert.match(
			content,
			/Inbox path: \/tmp\/state\/team\/root-team\/workers\/worker-3\/inbox\.md/,
		);
	...
	it("writeWorkerRoleInstructionsFile preserves precomposed mini guidance as wrapper-only content", async () => {
		...
		try {
			...
			assert.equal(
				(content.match(/resolved_model: gpt-5\.6-terra/g) || []).length,
				1,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("generateWorkerRootAgentsContent hardcodes runtime paths and role prompt", () => {
		...
		assert.match(
			content,
			/Inbox path: \/tmp\/project\/.omx\/state\/team\/root-team\/workers\/worker-2\/inbox\.md/,
		);
		assert.match(
			content,
			/Mailbox path: \/tmp\/project\/.omx\/state\/team\/root-team\/mailbox\/worker-2\.json/,
		);
		assert.match(
			content,
			/Leader mailbox path: \/tmp\/project\/.omx\/state\/team\/root-team\/mailbox\/leader-fixed\.json/,
		);
	...
	it("writeWorkerWorktreeRootAgentsFile composes user AGENTS before project and runtime guidance", async () => {
		...
		try {
			...
			assert.ok(
				generated.indexOf("USER_SENTINEL") <
					generated.indexOf("PROJECT_SENTINEL"),
			);
			assert.ok(
				generated.indexOf("PROJECT_SENTINEL") <
					generated.indexOf("Team Worker Runtime Instructions"),
			);
		} finally {
			restoreCodexHome();
			await rm(cwd, { recursive: true, force: true });
		...
		const inbox = generateInitialInbox(
			...
			],
			{
				approvedContextSummary: {
					...
					content:
						"Key boundary: preserve approved context only for matching launches.",
					truncated: false,
				},
			},
		);
		...
		const inbox = generateInitialInbox(
			...
			],
			{
				...
				approvedContextSummary: {
					sourcePath: ".omx/plans/repo-context-issue-2039.md",
					content:
						"This legacy summary should be suppressed when the handoff section is present.",
					truncated: false,
				},
			},
		);
	...
	it("generateTaskAssignmentInbox preserves Ultragoal context for follow-up assignments", () => {
		const inbox = generateTaskAssignmentInbox(
			...
			"team-ultragoal",
			{
				...
				description:
					"Check Team evidence can feed the Ultragoal leader checkpoint",
				status: "pending",
				created_at: "2026-05-13T00:00:00.000Z",
			},
			{
