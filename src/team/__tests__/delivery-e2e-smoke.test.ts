...
const EXACT_GLOBAL_PANE_PROOF_COMMAND =
	"list-panes -a -F #{pane_id}\t#{pane_dead}\t#{pane_pid}";

function assertExactPaneProofBeforeTargetEffect(
	tmuxLog: string,
	paneId: string,
): void {
	const commands = tmuxLog.trim().split("\n").filter(Boolean);
	const effectIndex = commands.findIndex((command) =>
...
async function withFakeTmux<T>(
	cwd: string,
	fn: (tmuxLogPath: string) => Promise<T>,
): Promise<T> {
...
async function withBridgeFixture<T>(
	cwd: string,
	fn: (runtimePath: string) => Promise<T>,
): Promise<T> {
...
async function configurePaneIds(
	teamName: string,
	cwd: string,
	leaderPaneId: string,
...
describe("team message delivery end-to-end smoke tests", () => {
	it("worker -> leader: send-message API creates mailbox, dispatches, and marks leader notification", async () => {
		...
		try {
			await withFakeTmux(cwd, async () => {
				await configurePaneIds("worker-leader-api", cwd, "%95", {
					"worker-1": "%10",
					"worker-2": "%11",
				});
				...
				const workerMessages = mailbox.filter(
					(message) =>
						message.from_worker === "worker-1" &&
						message.body === "worker ack to leader",
				);
				assert.equal(workerMessages.length, 1);
				assert.ok(
					workerMessages[0]?.notified_at,
					"leader mailbox message should be notified",
				);

				const requests = await listDispatchRequests("worker-leader-api", cwd, {
					kind: "mailbox",
					to_worker: "leader-fixed",
				});
				const workerRequests = requests.filter(
					(request) => request.message_id === workerMessages[0]?.message_id,
				);
				assert.equal(workerRequests.length, 1);
				assert.equal(workerRequests[0]?.status, "notified");
				assert.match(
					workerRequests[0]?.last_reason ?? "",
					/leader_mailbox_notified/,
				);
			});
		} finally {
			await cleanup();
	...
	it("worker -> leader: fallback watcher nudges the leader when an undelivered worker message lingers", async () => {
		...
		try {
			await withFakeTmux(cwd, async (tmuxLogPath) => {
				await configurePaneIds("worker-leader-fallback", cwd, "", {
					"worker-1": "%10",
				});
				await sendWorkerMessage(
					"worker-leader-fallback",
					"worker-1",
					"leader-fixed",
					"please read mailbox",
					cwd,
				);
				await configurePaneIds("worker-leader-fallback", cwd, "%95", {
					"worker-1": "%10",
				});
				...
				assert.match(
					tmuxLog,
					/set-buffer -b omx-pane-input-.* -- Team worker-leader-fallback:/,
				);
			...
			await withFakeTmux(cwd, async () => {
				...
				await Promise.all([
					executeTeamApiOperation(
						"send-message",
						{
							team_name: "worker-leader-concurrent",
							from_worker: "worker-1",
							to_worker: "leader-fixed",
							body: "msg-1",
						},
				...
				const mailbox = await listMailboxMessages(
					"worker-leader-concurrent",
					"leader-fixed",
					cwd,
				);
				const workerMessages = mailbox.filter(
					(message) =>
						message.from_worker.startsWith("worker-") &&
						/^msg-[123]$/.test(message.body),
				);
				assert.equal(workerMessages.length, 3);
				assert.deepEqual(
					new Set(workerMessages.map((message) => message.body)),
					new Set(["msg-1", "msg-2", "msg-3"]),
				);
				assert.equal(
					new Set(workerMessages.map((message) => message.message_id)).size,
					3,
				);
				assert.equal(
					workerMessages.filter((message) => message.notified_at).length,
					3,
				);
	...
	it("leader -> worker: leader send creates mailbox and hook notification for the target worker", async () => {
		...
		try {
			await withFakeTmux(cwd, async (tmuxLogPath) => {
				await configurePaneIds("leader-worker-send", cwd, "%95", {
					"worker-1": "%10",
					"worker-2": "%11",
				});
				await sendWorkerMessage(
				...
				const requests = await listDispatchRequests("leader-worker-send", cwd, {
					kind: "mailbox",
					to_worker: "worker-1",
				});
				assert.equal(requests.length, 1);
			...
			await withFakeTmux(cwd, async (tmuxLogPath) => {
				...
				for (const worker of ["worker-1", "worker-2", "worker-3"]) {
					...
					assert.equal(
						mailbox.length,
						1,
						`${worker} should receive one broadcast`,
					);
					assert.equal(mailbox[0]?.body, "broadcast hello");
					assert.ok(
						mailbox[0]?.notified_at,
						`${worker} should have notified_at set`,
					);
				}

				const requests = await listDispatchRequests("leader-broadcast", cwd, {
					kind: "mailbox",
				});
				assert.equal(requests.length, 3);
				assert.equal(
					requests.filter((request) => request.status === "notified").length,
					3,
				);
				const tmuxLog = await readFile(tmuxLogPath, "utf-8");
		...
		try {
			await withBridgeFixture(cwd, async (runtimePath) => {
				...
				await teamCommand([
					...
					"--input",
					JSON.stringify({
						team_name: "cli-bridge-send",
						from_worker: "leader-fixed",
						to_worker: "worker-1",
						body: "cli bridge hello",
					}),
					"--json",
				]);
	...
	it("cross-seam: fallback watcher nudges for stalled workers even when the leader itself is fresh (PR #1217 guard)", async () => {
		...
		try {
			await withFakeTmux(cwd, async (tmuxLogPath) => {
				await configurePaneIds("stalled-worker-fresh-leader", cwd, "%95", {
					"worker-1": "%10",
				});
				...
				await writeFile(
					join(cwd, ".omx", "state", "team-leader-nudge.json"),
					JSON.stringify(
						{
							last_nudged_by_team: {},
	...
	it("cross-seam: hook-owned bridge success keeps mailbox notification visible in the canonical mailbox view", async () => {
		...
		try {
			await withBridgeFixture(cwd, async () => {
				...
				const request = (
					await listDispatchRequests("hook-bridge-success", cwd, {
						kind: "mailbox",
						to_worker: "worker-1",
					})
				)[0];
	...
	it("edge: duplicate same from/to/body send dedupes without adding a second mailbox record", async () => {
		...
		try {
			await withFakeTmux(cwd, async () => {
				await configurePaneIds("dedupe-smoke", cwd, "%95", {
					"worker-1": "%10",
				});
				await executeTeamApiOperation(
					"send-message",
					{
						team_name: "dedupe-smoke",
						from_worker: "worker-1",
						to_worker: "leader-fixed",
						body: "same-body",
					},
				...
				const requests = await listDispatchRequests("dedupe-smoke", cwd, {
					kind: "mailbox",
					to_worker: "leader-fixed",
				});
				const workerRequests = requests.filter(
		...
		try {
			await withBridgeFixture(cwd, async () => {
				...
				const messageId = String(
					(sendResult.data.message as { message_id?: string }).message_id ?? "",
				);
				assert.ok(messageId);
				...
				const deliveredResult = await executeTeamApiOperation(
				...
				const requestId = String(
					deliveredResult.data.dispatch_request_id ?? "",
				);
				const request = await readDispatchRequest(
					"mark-delivered-smoke",
					requestId,
					cwd,
				);
				assert.equal(request?.status, "delivered");

				const undelivered = await executeTeamApiOperation(
					"mailbox-list",
					{
						team_name: "mark-delivered-smoke",
						worker: "worker-1",
	...
	it("edge: delivery still completes through the pure TS fallback path when the bridge is disabled", async () => {
		...
		try {
			...
			await withFakeTmux(cwd, async (tmuxLogPath) => {
				await configurePaneIds("bridge-disabled-fallback", cwd, "%95", {
					"worker-1": "%10",
				});
				await sendWorkerMessage(
					"bridge-disabled-fallback",
					"leader-fixed",
					"worker-1",
					"ts fallback only",
					cwd,
				...
				assertExactPaneProofBeforeTargetEffect(tmuxLog, "%10");
				assert.equal(
					existsSync(join(cwd, ".omx", "state", "mailbox.json")),
					false,
					"bridge compat mailbox should not be created when bridge is disabled",
				);
			});
		} finally {
			if (typeof previousBridge === "string")
