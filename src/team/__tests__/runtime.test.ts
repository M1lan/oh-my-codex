...
async function attachDirtyWorkerRepo(
	teamName: string,
	cwd: string,
	repoName: string,
): Promise<void> {
...
async function markDetachedSessionAbsent(
	teamName: string,
	cwd: string,
): Promise<void> {
	const config = await readTeamConfig(teamName, cwd);
...
async function markPendingInboxDispatchesDelivered(
	...
	const requests = await listDispatchRequests(
		await resolveRuntimeTeamName(cwd, teamName),
		cwd,
		{ kind: "inbox" },
	).catch(() => []);
...
async function resolveRuntimeTeamName(
	cwd: string,
		...
		.filter(
			(entry) =>
				...
				(entry.name === requestedName ||
					entry.name.startsWith(`${requestedName}-`) ||
...
async function writeFakePromptWorkerBinary(
	...
	const bootstrap =
		options.emitStartupEvidence === false
			? ""
			: `
const fs = require('fs');
...
async function withMockTmuxFixture<T>(
	...
	try {
		...
		const originalSyntaxCheck = spawnSync("sh", ["-n", originalFixturePath], {
			encoding: "utf-8",
		});
		if (originalSyntaxCheck.status !== 0 || originalSyntaxCheck.error) {
			throw new Error(
				`invalid original mock tmux shell fixture: ${originalSyntaxCheck.error?.message ?? originalSyntaxCheck.stderr ?? "sh -n failed"}`,
			);
		}
		const fixtureDefinesGlobalExactPaneSnapshot =
			tmuxFixture.includes("-a -F #{pane_id}");
...
`,
		...
		const wrapperSyntaxCheck = spawnSync("sh", ["-n", tmuxStubPath], {
			encoding: "utf-8",
		});
		if (wrapperSyntaxCheck.status !== 0 || wrapperSyntaxCheck.error) {
			throw new Error(
				`invalid generated mock tmux shell wrapper: ${wrapperSyntaxCheck.error?.message ?? wrapperSyntaxCheck.stderr ?? "sh -n failed"}`,
			);
		}
...
const ORIGINAL_OMX_MUX_BINARY = process.env.OMX_MUX_BINARY;
...
afterEach(() => {
	...
	if (typeof ORIGINAL_OMX_MUX_BINARY === "string")
		process.env.OMX_MUX_BINARY = ORIGINAL_OMX_MUX_BINARY;
...
describe("runtime", () => {
	...
	it("rejects explicit mixed worker policy before initial team state or workers are created", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(join(cwd, ".omx", "state", "team", "explicit-policy")),
				false,
			);
		} finally {
	...
	it("startTeam launches executor workers with authoritative config policy, positional backslashes, and no bypass", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-direct-policy-start-"),
		);
...
process.on('SIGTERM', () => process.exit(0));`,
		...
		try {
			...
			const workerArgs = JSON.parse(
				await waitForFileText(capturePath, (content) => content.length > 0),
			) as string[];
	...
	it("rejects Claude and Gemini restrictive config policy before prompt worker capture and cleans state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-restrictive-noncodex-"),
		);
		const binDir = join(cwd, "bin");
		const previousPath = process.env.PATH;
		const previousTmux = process.env.TMUX;
		...
		try {
			for (const workerCli of ["claude", "gemini"] as const) {
				...
				const internalTeamName = buildInternalTeamName(
					teamName,
					resolveTeamIdentityScope({ OMX_SESSION_ID: teamSessionId }),
				);
...
`,
			...
			if (typeof previousCapture === "string")
				process.env.OMX_POLICY_ARGV_CAPTURE = previousCapture;
	...
	it("rejects mixed CLI restrictive policy before any prompt worker is spawned", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-mixed-policy-rollback-"),
		);
...
`,
			...
			for (const nonCodexCli of ["claude", "gemini"] as const) {
				...
				await assert.rejects(
					...
					new RegExp(
						`Selected team worker CLI "${nonCodexCli}" is incompatible with an explicit approval or sandbox policy\\.`,
					),
				);

				assert.equal(
					existsSync(codexCapturePath),
					false,
					"Codex must not spawn before every worker policy is compatible",
				);
				assert.equal(
					existsSync(join(cwd, ".omx", "state", "team", internalTeamName)),
					false,
				...
				assert.equal(
					existsSync(
						join(cwd, ".omx", "state", "team", internalTeamName, "tasks"),
					),
					false,
				);
				assert.equal(
					existsSync(
						join(cwd, ".omx", "state", "team", internalTeamName, "workers"),
					),
					false,
				);
				assert.equal(
					existsSync(nonCodexCapturePath),
					false,
					`${nonCodexCli} must not be spawned`,
				);
			}
		} finally {
			if (typeof previousPath === "string") process.env.PATH = previousPath;
			...
			if (typeof previousWorkerCliMap === "string")
				process.env.OMX_TEAM_WORKER_CLI_MAP = previousWorkerCliMap;
			else delete process.env.OMX_TEAM_WORKER_CLI_MAP;
			if (typeof previousLaunchArgs === "string")
				process.env.OMX_TEAM_WORKER_LAUNCH_ARGS = previousLaunchArgs;
			else delete process.env.OMX_TEAM_WORKER_LAUNCH_ARGS;
			if (typeof previousAllowNonTty === "string")
				process.env.OMX_TEST_ALLOW_NONTTY_CODEX_PROMPT = previousAllowNonTty;
			else delete process.env.OMX_TEST_ALLOW_NONTTY_CODEX_PROMPT;
			...
			if (typeof previousCodexCapture === "string")
				process.env.OMX_CODEX_PID_CAPTURE = previousCodexCapture;
			else delete process.env.OMX_CODEX_PID_CAPTURE;
			if (typeof previousNonCodexCapture === "string")
				process.env.OMX_NON_CODEX_CAPTURE = previousNonCodexCapture;
	...
	it("rejects restrictive later CLI before provisioning or composing a reused worker worktree", async () => {
		...
		try {
			...
			execFileSync("git", ["commit", "-m", "ignore team worktrees"], {
				cwd: repo,
				stdio: "ignore",
			});
			...
			execFileSync(
				"git",
				["commit", "-m", "seed reusable worker instructions"],
				{ cwd: workerWorktreePath, stdio: "ignore" },
			);
			const agentsBefore = await readFile(agentsPath);
			const indexBefore = execFileSync(
				"git",
				["ls-files", "-v", "--", "AGENTS.md"],
				{
					cwd: workerWorktreePath,
					encoding: "utf-8",
				},
			...
			const worktreeRegistryBefore = execFileSync(
				"git",
			...
			const sourceStatusBefore = execFileSync(
				"git",
				["status", "--porcelain=v1", "--untracked-files=all"],
				{
					cwd: repo,
			...
			const rejectedBranchBefore = execFileSync(
				"git",
				["branch", "--list", rejectedWorkerBranchName],
				{
					cwd: repo,
					encoding: "utf-8",
				},
			...
			assert.equal(
				existsSync(join(repo, ".omx", "state", "current-task-baseline.json")),
				false,
			);
			assert.equal(
				existsSync(join(repo, ".omx", "state", "team", internalTeamName)),
				false,
			);
		} finally {
			if (typeof previousLaunchMode === "string")
			...
			if (workerWorktreePath && existsSync(workerWorktreePath)) {
				execFileSync(
					"git",
					["worktree", "remove", "--force", workerWorktreePath],
					{ cwd: repo, stdio: "ignore" },
				);
			}
			if (
				rejectedWorkerWorktreePath &&
				existsSync(rejectedWorkerWorktreePath)
			) {
				execFileSync(
					"git",
					["worktree", "remove", "--force", rejectedWorkerWorktreePath],
					{ cwd: repo, stdio: "ignore" },
				);
			}
			if (rejectedWorkerBranchName) {
				const rejectedBranch = execFileSync(
					"git",
					["branch", "--list", rejectedWorkerBranchName],
					{
						cwd: repo,
				...
				if (rejectedBranch.trim() !== "") {
					execFileSync("git", ["branch", "-D", rejectedWorkerBranchName], {
						cwd: repo,
						stdio: "ignore",
					});
				}
	...
	it("resolveWorkerLaunchArgsFromEnv preserves explicit model in either syntax", () => {
		...
		assert.deepEqual(
			resolveWorkerLaunchArgsFromEnv(
				{ OMX_TEAM_WORKER_LAUNCH_ARGS: "--model=gpt-5.5" },
				"explore",
			),
			["--model", "gpt-5.5"],
		);
	...
	it("waitForWorkerStartupEvidence treats blocked worker status as settled progress even without a claimed task id", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-codex-blocked-startup-"),
		);
		try {
			await initTeamState(
				"codex-blocked-startup",
				"blocked startup evidence test",
				"executor",
				1,
				cwd,
			);
	...
	it("uses a production startup evidence window that can tolerate slow Codex startup", {
		...
		const prevStartupEvidenceTimeout =
			process.env.OMX_TEAM_STARTUP_EVIDENCE_TIMEOUT_MS;
		const prevStartupDispatchRetries =
			process.env.OMX_TEAM_STARTUP_DISPATCH_RETRIES;
		const prevStartupDispatchRetryDelay =
			process.env.OMX_TEAM_STARTUP_DISPATCH_RETRY_DELAY_MS;
...
`,
				...
				async () => {
					...
					const expectedTeamName = buildInternalTeamName(
						"team-startup-window",
						resolveTeamIdentityScope(process.env),
					);
	...
	it("startTeam rejects tmux fallback when worker startup evidence stays missing", {
		skip: skipSlowLifecycleUnderCoverage,
	}, async () => {
		const cwd = await mkdtemp(
...
`,
	...
	it("startTeam blocks duplicate no-session/no-tmux prompt-mode starts with stable cwd leader identity", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-prompt-duplicate-nosession-"),
		);
		const binDir = join(cwd, "bin");
		const fakeCodexPath = join(binDir, "codex");
		await mkdir(binDir, { recursive: true });
...
process.on('SIGTERM', () => process.exit(0));`,
		...
		try {
			await withPromptModeCodexEnv(
				...
				async () => {
					...
					const teamEntries = await readdir(
						join(cwd, ".omx", "state", "team"),
						{ withFileTypes: true },
					);
					assert.equal(
						teamEntries.some(
							(entry) =>
								entry.isDirectory() &&
								entry.name.startsWith("second-prompt-team-"),
						),
						false,
	...
	it("startTeam tags interactive panes with the derived tmux-pane leader identity when session id is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-pane-derived-owner-"),
		);
		const prevTmux = process.env.TMUX;
		const prevTmuxPane = process.env.TMUX_PANE;
		const prevSessionId = process.env.OMX_SESSION_ID;
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					assert.match(
						tmuxLog,
						/set-option -p -t %1 @omx_pane_instance_id '%1'/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %2 @omx_pane_instance_id '%1'/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %3 @omx_pane_instance_id '%1'/,
					);
					assert.match(
						tmuxLog,
						/exec env OMX_SESSION_ID='%1' OMX_TMUX_HUD_OWNER=1 OMX_TMUX_HUD_LEADER_PANE='%1' .*hud --watch/,
					);
	...
	it("startTeam keeps logical session id out of team shutdown ownership tags", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-pane-owner-env-isolated-"),
		);
		const prevTmux = process.env.TMUX;
		const prevTmuxPane = process.env.TMUX_PANE;
		const prevSessionId = process.env.OMX_SESSION_ID;
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					assert.match(
						tmuxLog,
						/set-option -t \$1 @omx_instance_id 'logical-session-from-env'/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %1 @omx_pane_instance_id 'logical-session-from-env'/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %2 @omx_pane_instance_id 'logical-session-from-env'/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %3 @omx_pane_instance_id 'logical-session-from-env'/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %1 @omx_team_pane_owner_id 'team:pane-owner-env-isolat-[a-f0-9]{8}'/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %2 @omx_team_pane_owner_id 'team:pane-owner-env-isolat-[a-f0-9]{8}'/,
					);
					assert.match(
						tmuxLog,
						/set-option -p -t %3 @omx_team_pane_owner_id 'team:pane-owner-env-isolat-[a-f0-9]{8}'/,
					);
					assert.doesNotMatch(
						tmuxLog,
						/set-option -p -t %1 @omx_team_pane_owner_id logical-session-from-env/,
					);
					assert.match(
						tmuxLog,
						/exec env OMX_SESSION_ID='logical-session-from-env' OMX_TMUX_HUD_OWNER=1 OMX_TMUX_HUD_LEADER_PANE='%1' .*hud --watch/,
					);
...
`,
	...
	it("applyCreatedInteractiveSessionToConfig persists worker pane ids before readiness waits", async () => {
		...
		try {
			...
			const partialWorkerPaneIds = Array.from(
				{ length: 2 },
				() => undefined as string | undefined,
			);
			applyCreatedInteractiveSessionToConfig(
			...
			await saveTeamConfig(config, cwd);
			const durablePartialConfig = await readTeamConfig(
				"team-pane-persist-race",
				cwd,
			);
			assert.ok(durablePartialConfig);
			assert.deepEqual(
				durablePartialConfig.startup_cleanup_panes,
	...
	it("replays durable startup cleanup panes by exact PID and Team owner", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-startup-cleanup-replay-"),
		);
		try {
			const config = await initTeamState(
				"startup-cleanup-replay",
				"replay startup pane cleanup debt",
				"executor",
				1,
				cwd,
			);
...
`,
	...
	it("startTeam runs worker MCP orphan cleanup before interactive tmux worker spawn", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-interactive-mcp-cleanup-"),
		);
		const prevTmux = process.env.TMUX;
		const prevTmuxPane = process.env.TMUX_PANE;
		const prevLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
...
`,
					binaries: [
						{ name: "codex", content: fakeCodexShellScript("exit 0\n") },
					],
				},
				async ({ tmuxLogPath }) => {
					delete process.env.TMUX;
...
`,
	...
	it("startTeam preserves the created worker PID when its pane is reused before startup state materializes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-startup-pane-pid-reuse-"),
		);
		const previousTmux = process.env.TMUX;
...
`,
	...
	it("startTeam saves interactive pane ids before concurrent readiness attempts", async () => {
		...
		const saveMatch = source.match(
			/await saveTeamConfig\(config, leaderCwd\);/m,
		);
	...
	it("startTeam rejects startup direct trigger success when Codex startup evidence is missing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-startup-direct-fast-"),
		);
		const previousTmux = process.env.TMUX;
		const previousTmuxPane = process.env.TMUX_PANE;
		const previousLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
		...
		const previousStartupEvidenceTimeout =
			process.env.OMX_TEAM_STARTUP_EVIDENCE_TIMEOUT_MS;
		const previousStartupDispatchRetries =
			process.env.OMX_TEAM_STARTUP_DISPATCH_RETRIES;
		const previousStartupDispatchRetryDelay =
			process.env.OMX_TEAM_STARTUP_DISPATCH_RETRY_DELAY_MS;
...
`,
				...
				async () => {
					...
					const order = (
						await readFile(join(cwd, "startup-order.log"), "utf-8")
					)
						.trim()
						.split("\n");
					assert.ok(
						order.includes("send-keys"),
						`expected direct send-keys, got ${order.join(",")}`,
					);
					assert.ok(
	...
	it("terminates attached Codex startup on the first exact MDM bypass marker without dispatch or retry", async () => {
		...
		const marker =
			"MDM_POLICY_REJECTED: approval_policy=never forbids --dangerously-bypass-approvals-and-sandbox";
		const previousTmux = process.env.TMUX;
		const previousPane = process.env.TMUX_PANE;
					...
					binaries: [
						{
							name: "codex",
							content:
								fakeCodexNodeScript(`require('fs').writeFileSync(process.env.OMX_CODEX_ARGV_CAPTURE, JSON.stringify(process.argv.slice(2)));
process.stdin.resume();`),
						},
					],
					env: {
						...
						OMX_TEAM_WORKER_CLI: "codex",
						OMX_TEAM_WORKER_LAUNCH_ARGS:
							"--dangerously-bypass-approvals-and-sandbox --model gpt-5.6-test",
				...
				async ({ tmuxLogPath }) => {
					...
					assert.ok(
						finalArgv.some((arg) => arg.startsWith("model_instructions_file=")),
					);
					assert.equal(
						finalArgv.filter(
					...
					const tmuxEvents = (await readFile(tmuxLogPath, "utf-8"))
						.trim()
						.split("\n")
						.filter(Boolean);
					const captureEvents = tmuxEvents.filter((event) =>
						event.startsWith("capture-pane "),
					);
					assert.equal(
						captureEvents.length,
						1,
						"marker must not enter a second readiness cycle",
					);
					assert.equal(
						tmuxEvents.some(
							(event) =>
								event.startsWith("send-keys ") && event.includes(" -l -- "),
						),
						false,
						`marker must stop before any literal dispatch: ${tmuxEvents.join(",")}`,
					);
					const retainedTeams = await readdir(teamStateTestPath(cwd, "team"));
					assert.equal(
						retainedTeams.length,
						1,
						"proof-loss rollback must retain one inspectable team root",
					);
					...
					const retainedWorker = await readWorkerStatus(
						retainedTeamName,
						"worker-1",
						cwd,
					);
	...
	it("terminates on an MDM marker first observed during detailed readiness before fallback dispatch", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-codex-mdm-detailed-"),
		);
			...
			await withMockTmuxFixture(
				...
				async ({ tmuxLogPath }) => {
					...
					const captureIndexes = tmuxEvents
						.map((event, index) =>
							event.startsWith("capture-pane ") ? index : -1,
						)
						.filter((index) => index >= 0);
					assert.ok(
						captureIndexes.length >= 5,
						`expected startup-direct delivery captures followed by detailed readiness marker: ${tmuxEvents.join(",")}`,
					);
					const markerCaptureIndex = captureIndexes[4]!;
					assert.equal(
						tmuxEvents
							.slice(markerCaptureIndex + 1)
							.some(
								(event) =>
									event.startsWith("send-keys ") && event.includes(" -l -- "),
							),
						false,
						`marker must stop before fallback dispatch/retry: ${tmuxEvents.join(",")}`,
					);
					const retainedTeams = await readdir(teamStateTestPath(cwd, "team"));
					assert.equal(
						retainedTeams.length,
	...
	it("startTeam treats a confirmed ready prompt as startup evidence after hook notification", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-ready-prompt-evidence-"),
		);
		const previousTmux = process.env.TMUX;
		const previousTmuxPane = process.env.TMUX_PANE;
		const previousLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
...
`,
				...
				async () => {
					...
					const requests = await listDispatchRequests(runtime.teamName, cwd, {
						kind: "inbox",
					});
					assert.ok(
						requests.some((request) => request.status === "notified") ||
							requests.some((request) =>
								/fallback_confirmed/.test(request.last_reason ?? ""),
							),
						`expected hook notification or ready-prompt fallback confirmation, got ${JSON.stringify(requests)}`,
					);

					const captureCount = Number.parseInt(
						await readFile(join(cwd, "capture-count"), "utf-8"),
						10,
					);
					assert.ok(
						captureCount >= 1,
						`expected startup capture, got ${captureCount}`,
					);
					...
					if (existsSync(timingPath)) {
						const timing = JSON.parse(await readFile(timingPath, "utf-8")) as {
							events: Array<{ phase: string; ok?: boolean }>;
						};
						assert.ok(
							timing.events.some((event) => event.phase === "ready_wait_start"),
						);
						assert.ok(
							timing.events.some(
								(event) =>
									event.phase === "ready_wait_end" && event.ok === true,
							),
						);
					}
				},
	...
	it("startTeam rejects ready-prompt timeout when dispatch never produces startup evidence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-ready-timeout-no-evidence-"),
		);
		const previousTmux = process.env.TMUX;
		const previousTmuxPane = process.env.TMUX_PANE;
		const previousLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					const tmuxLog = await readFile(tmuxLogPath, "utf-8");
					assert.match(tmuxLog, /send-keys -t %2 -l --/);
					runtimeTeamName = await resolveRuntimeTeamName(cwd, teamName).catch(
						() => null,
					);
				},
			);
...
`,
				...
				async () => {
					...
					receiptDeliverer = setInterval(() => {
						void (async () => {
							await markPendingInboxDispatchesDelivered(
								"team-parallel-ready",
								cwd,
							);
						})();
					...
					const order = (await readFile(join(cwd, "ready-order.log"), "utf-8"))
						.trim()
						.split("\n");
	...
	it("startTeam rejects no-evidence startup issues instead of treating live panes as recoverable", {
		skip: skipSlowLifecycleUnderCoverage,
	}, async () => {
		const cwd = await mkdtemp(
...
`),
				...
				async () => {
					...
					receiptFailer = setInterval(() => {
						void (async () => {
							const runtimeTeamName = await resolveRuntimeTeamName(
								cwd,
								teamName,
							);
							const requests = await listDispatchRequests(
							...
							for (const request of requests) {
								observedNoEvidenceRequest ||=
									/startup_no_evidence|fallback_attempted_but_unconfirmed/.test(
										request.last_reason ?? "",
									);
								if (request.status !== "pending") continue;
								await transitionDispatchRequest(
	...
	it("startTeam attempts worker-2 before rejecting lowest-index unrecoverable startup failure", {
		skip: skipSlowLifecycleUnderCoverage,
	}, async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-parallel-dead-pane-"),
		);
		const previousTmux = process.env.TMUX;
		const previousTmuxPane = process.env.TMUX_PANE;
		const previousLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
...
`,
				...
				async () => {
					...
					const order = (
						await readFile(join(cwd, "dead-pane-order.log"), "utf-8")
					)
						.trim()
						.split("\n");
					assert.ok(order.includes("w1-ready-start"));
	...
	it("settleStartupAttemptResults waits for sibling startup attempt to settle after worker startup throw", async () => {
		...
		const startupAttemptResults = await settleStartupAttemptResults([
			{
				...
				attempt: Promise.reject(
					new Error("test_startup_attempt_throw:worker-1"),
				),
			},
			{
				workerIndex: 2,
		...
		assert.equal(
			worker2Settled,
			true,
			"worker-2 startup attempt should settle before results return",
		);
		...
		assert.match(
			String(firstStartupError?.error),
			/test_startup_attempt_throw:worker-1/,
		);
	});
...
`,
				...
				async () => {
					...
					assert.equal(
						await readTeamConfig("team-dead-startup-pane", cwd),
						null,
					);
	...
	it("startTeam preserves the proof-unavailable contract for an ambiguous worker split whose reconciled pane proof is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-ambiguous-split-proof-"),
		);
		const previousTmux = process.env.TMUX;
		const previousTmuxPane = process.env.TMUX_PANE;
		const previousLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
...
`,
				...
				async () => {
					...
					assert.ok(
						preservedConfig,
						"proof-unavailable ambiguous split must preserve retry state",
					);
					assert.equal(preservedConfig?.workers[0]?.pane_id, "%2");
				},
			);
	...
	it("startTeam persists partial create-session metadata when rollback proof is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-partial-create-proof-"),
		);
		const previousTmux = process.env.TMUX;
		const previousTmuxPane = process.env.TMUX_PANE;
		const previousLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
...
`,
	...
	it("startTeam cleans state when create-session proof loss has no created resource", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-no-resource-create-proof-"),
		);
		const previousTmux = process.env.TMUX;
		const previousTmuxPane = process.env.TMUX_PANE;
		const previousLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
...
`,
			...
			if (typeof previousEntryPath === "string")
				process.env.OMX_ENTRY_PATH = previousEntryPath;
	...
	it("startTeam materializes all worker identity/inbox files before worker-1 startup evidence can block later workers", {
		skip: skipSlowLifecycleUnderCoverage,
	}, async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-materialize-before-evidence-"),
		);
		const previousTmux = process.env.TMUX;
		const previousTmuxPane = process.env.TMUX_PANE;
		const previousLaunchMode = process.env.OMX_TEAM_WORKER_LAUNCH_MODE;
...
`),
				...
				async () => {
					let runtimeTeamName = sanitizeTeamName(
					...
					receiptFailer = setInterval(() => {
						void (async () => {
							const activeTeamName = await resolveRuntimeTeamName(
								cwd,
								"team-materialize-before-evidence",
							);
							runtimeTeamName = activeTeamName;
							const requests = await listDispatchRequests(activeTeamName, cwd, {
								kind: "inbox",
							}).catch(() => []);
							for (const request of requests) {
								if (request.status !== "pending") continue;
					...
					for (let attempt = 0; attempt < 200; attempt += 1) {
						runtimeTeamName = await resolveRuntimeTeamName(
							cwd,
							"team-materialize-before-evidence",
						);
						const workerOneIdentity = join(
							cwd,
							".omx",
							"state",
							"team",
					...
					assert.match(
						String((outcome as { ok: false; error: Error }).error),
						/(worker_notify_failed:worker-1|startup_rollback_pane_proof_unavailable:%2:pane_proof_lost_during_process_teardown)/,
					);
	...
	it("startTeam runs worker MCP orphan cleanup before prompt worker spawn", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-prompt-mcp-cleanup-"),
		);
		const binDir = join(cwd, "bin");
		const fakeCodexPath = join(binDir, "codex");
...
setTimeout(() => {}, 5000);`,
		...
		try {
			...
			const order = await waitForFileText(capturePath, (content) =>
				/spawn/.test(content),
			);
...
`,
		...
		try {
			...
			assert.equal(
				existsSync(
					join(cwd, ".omx", "state", "team", "team-codex-explicit-launch"),
				),
				false,
			);
		} finally {
...
`,
		...
		try {
			runtime = await withIsolatedDefaultModelEnvAsync(async () => {
				...
				await writeFile(
					join(process.env.CODEX_HOME, ".omx-config.json"),
					JSON.stringify({
						agentReasoning: {
							writer: "xhigh",
						},
...
`,
		...
		try {
			...
			assert.equal(
				existsSync(join(cwd, ".omx", "state", "team", "team-prompt")),
				false,
			);
		} finally {
			if (typeof prevPath === "string") process.env.PATH = prevPath;
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					const configBeforeShutdown = await readTeamConfig(
						runtime.teamName,
						cwd,
					);
					...
					const teamHudSplitRe = new RegExp(
						`split-window -v -f -l ${HUD_TMUX_TEAM_HEIGHT_LINES} -t %1 -d -P -F #\\{pane_id\\}`,
						"g",
					);
					...
					assert.ok(
						(tmuxLog.match(
							/list-panes -a -F '#\{pane_id\}\t#\{pane_dead\}\t#\{pane_pid\}'/g,
						)?.length ?? 0) >= 6,
					);
					assert.ok(
						(tmuxLog.match(/select-layout -t @1 main-vertical/g)?.length ??
							0) >= 2,
					);
...
`,
		...
		try {
			...
			assert.match(
				startupLog,
				new RegExp(
					`\\$OMX_TEAM_STATE_ROOT/team/${runtime.teamName}/workers/worker-1/inbox\\.md`,
				),
			);
			assert.doesNotMatch(
				startupLog,
				new RegExp(
					`Read \\.omx/state/team/${runtime.teamName}/workers/worker-1/inbox\\.md`,
				),
			);
			...
			assert.match(
				rootAgents,
				new RegExp(
					`Inbox path: .*${runtime.teamName}/workers/worker-1/inbox\\.md`,
				),
			);
			...
			assert.match(
				mailboxLog,
				new RegExp(
					`\\$OMX_TEAM_STATE_ROOT/team/${runtime.teamName}/mailbox/worker-1\\.json`,
				),
			);
...
`,
	...
	it("shutdownTeam preserves generic prompt-worker state when process-group liveness is unknown", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-prompt-unknown-pid-"),
		);
		const binDir = join(cwd, "bin");
		const fakeCodexPath = join(binDir, "codex");
...
`,
		...
		let helperPid = 0;
		try {
			runtime = await withPromptModeCodexEnv(
				binDir,
				{ OMX_HELPER_PID_PATH: helperPidPath },
				() =>
					withoutTeamWorkerEnv(() =>
						startTeam(
...
`,
	...
	it("shutdownTeam clean fast path ignores worker shutdown ack files", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-clean-fast-"),
		);
		try {
			await initTeamState(
				"team-shutdown-clean-fast",
				"shutdown clean fast path test",
				"executor",
				1,
				cwd,
			);
	...
	it("follow-up created before terminal epoch blocks shutdown without starting the epoch", async () => {
		...
		try {
			...
			await createTask(
				"team-terminal-before",
				{
					subject: "valid follow-up",
					description: "must block shutdown",
					status: "pending",
				},
				cwd,
			);
			...
			assert.equal(
				(await readTask("team-terminal-before", "1", cwd))?.status,
				"pending",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("terminal epoch rejects duplicate late assignment and task creation without stale delivery", async () => {
		...
		const epochObserved = new Promise<void>((resolve) => {
			observeEpoch = resolve;
		});
		const epochRelease = new Promise<void>((resolve) => {
			releaseEpoch = resolve;
		});
		try {
			await initTeamState(
				"team-terminal-after",
				"terminal boundary after test",
				"executor",
				1,
				cwd,
			);
			const task = await createTask(
				"team-terminal-after",
				{
					subject: "forced follow-up",
					description: "must not leak after epoch",
					status: "pending",
				},
				cwd,
			);
			...
			const shutdown = shutdownWithoutTmuxSession("team-terminal-after", cwd, {
				force: true,
			});
			await epochObserved;
			...
			const attempts = await Promise.allSettled([
				...
				createTask(
					...
					{
						subject: "too late",
						description: "must require continuation",
						status: "pending",
					},
					cwd,
				),
			...
			const diagnostics = attempts.map((attempt) => {
				...
				return String(
					(attempt as PromiseRejectedResult).reason?.message ??
			...
			assert.equal(
				(await readTask("team-terminal-after", task.id, cwd))?.status,
				"pending",
			);
			assert.equal(
				(await listMailboxMessages("team-terminal-after", "worker-1", cwd))
					.length,
				0,
			);
			assert.equal(
				(
					await listDispatchRequests("team-terminal-after", cwd, {
						kind: "inbox",
					})
				).length,
				0,
	...
	it("terminal watcher phase rejects assignment with one continuation diagnostic", async () => {
		...
		try {
			...
			const task = await createTask(
				"team-terminal-watcher",
				{
					subject: "late watcher work",
					description: "worker has exited",
					status: "pending",
				},
				cwd,
			);
			...
			assert.equal(
				(await readTask("team-terminal-watcher", task.id, cwd))?.status,
				"pending",
			);
			...
			assert.equal(
				(
					await listDispatchRequests("team-terminal-watcher", cwd, {
						kind: "inbox",
					})
				).length,
				0,
	...
	it("shutdownTeam force=true bypasses shutdown gate and cleans up", async () => {
		...
		try {
			...
			await shutdownWithoutTmuxSession("team-shutdown-gate-force", cwd, {
				force: true,
			});
			const teamRoot = join(
				cwd,
	...
	it("shutdownTeam force=true emits shutdown_gate_forced audit event", async () => {
		...
		try {
			...
			await shutdownWithoutTmuxSession("team-gate-forced-event", cwd, {
				force: true,
			});
	...
	it("shutdownTeam leaves no canonical config for a stale writer to overwrite", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-stale-config-"),
		);
		try {
			await initTeamState(
				"team-shutdown-stale-config",
				"shutdown stale config test",
				"executor",
				1,
				cwd,
			);
			...
			await shutdownWithoutTmuxSession("team-shutdown-stale-config", cwd, {
				force: true,
			});
			stale.task = "must not recreate shutdown state";
			await assert.rejects(
				() => saveTeamConfig(stale, cwd),
				/team_config_missing:team-shutdown-stale-config/,
			);
			assert.equal(
				existsSync(
					join(cwd, ".omx", "state", "team", "team-shutdown-stale-config"),
				),
				false,
			);
		} finally {
...
`,
	...
	it("shutdownTeam confirmIssues=true allows failed-task shutdown without worker ack handshake", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-confirm-issues-"),
		);
		try {
			await initTeamState(
				"team-confirm-issues",
				"shutdown confirm issues test",
				"executor",
				1,
				cwd,
			);
			...
			await shutdownWithoutTmuxSession("team-confirm-issues", cwd, {
				confirmIssues: true,
			});
	...
	it("shutdownTeam retains gone-pane descendant cleanup debt until tracked descendants exit", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-gone-pane-descendant-debt-"),
		);
		...
		try {
			...
			await initTeamState(
				teamName,
				"gone pane descendant cleanup debt test",
				"executor",
				1,
				cwd,
			);
			...
			const debtPath = join(
				...
				"team",
				teamName,
				".gone-pane-descendant-cleanup-debt.json",
			);
			await writeFile(
				debtPath,
				JSON.stringify({
					...
					entries: [
						{
							...
							tracked_processes: [
								{ pid: descendant.pid, start_time: startTime! },
							],
			...
			await assert.rejects(
				...
				/gone_pane_descendant_cleanup_debt_unresolved:%404/,
			);
			...
			assert.equal(
				existsSync(join(cwd, ".omx", "state", "team", teamName)),
				false,
			);
		} finally {
			try {
	...
	it("shutdownTeam retains descendant-reuse debt without signaling a stable root or replacement PID", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-gone-pane-descendant-reuse-"),
		);
		let descendant: ReturnType<typeof spawn> | null = null;
		const originalProcessKill = process.kill;
		try {
			...
			await initTeamState(
				teamName,
				"gone pane descendant reuse test",
				"executor",
				1,
				cwd,
			);
					...
					entries: [
						{
							...
							tracked_processes: [
								{ pid: descendant.pid, start_time: `${startTime}0` },
							],
							evidence: "pane_authority_lost_during_descendant_teardown",
						},
					],
			...
			assert.doesNotThrow(
				() => originalProcessKill(process.pid, 0),
				"root remains live",
			);
			assert.deepEqual(
				signals.filter(({ pid }) => pid === descendant!.pid),
				[],
			);
			assert.equal(existsSync(debtPath), true);
		} finally {
	...
	it("shutdownTeam preserves identity-unavailable descendant debt when PID probing is unknown", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-gone-pane-unknown-probe-"),
		);
		const teamName = "team-gone-probe";
		try {
			await initTeamState(
				teamName,
				"unknown descendant probe debt test",
				"executor",
				1,
				cwd,
			);
			...
			assert.equal(existsSync(debtPath), true);
			const persistedDebt = JSON.parse(await readFile(debtPath, "utf-8")) as {
				entries: Array<{ pane_id: string }>;
			};
			assert.deepEqual(persistedDebt.entries, [
				{
					pane_id: "%405",
					authorized_pane_pid: process.pid,
					tracked_pids: [process.pid],
...
`,
	...
	it("shutdownTeam pre-kills each detached pane immediately after its exact proof", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-prekill-order-"),
		);
...
`,
			...
			await waitForFileText(
				orderPath,
				(content) =>
					content.includes("ready-pane-1") && content.includes("ready-pane-2"),
			);
...
`,
				...
				async () => {
					await initTeamState(
						"team-shutdown-prekill-order",
						"shutdown prekill order test",
						"executor",
						2,
						cwd,
					);
	...
	it("shutdownTeam tears down a detached HUD only with its persisted PID and Team owner", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-detached-hud-shutdown-"),
		);
		try {
			await withMockTmuxFixture(
				{
...
`,
	...
	it("retains an accepted detached-session kill receipt through a query failure and removes it only with final state cleanup", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-detached-session-post-kill-query-failure-"),
		);
		try {
			await withMockTmuxFixture(
				{
					dirPrefix:
						"omx-runtime-detached-session-post-kill-query-failure-bin-",
					tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					await initTeamState(
						teamName,
						"retain state after detached-session query failure",
						"executor",
						1,
						cwd,
					);
					...
					const receiptPath = join(
						...
						"team",
						teamName,
						".detached-session-destroy-receipt.json",
					);
					...
					assert.equal(
						...
						"omx-team-team-postkill-query-fail",
					);
	...
	it("does not kill after journal publication when the stable authority drifts", async () => {
		const cases: readonly (
			...
			| "generation"
			| "owner"
		)[] = ["incarnation", "config", "generation", "owner"];
		for (const drift of cases) {
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-runtime-detached-journal-${drift}-`),
			);
			try {
				await withMockTmuxFixture(
					{
...
`,
					...
					async ({ tmuxLogPath }) => {
						...
						setDetachedSessionDestroyAfterJournalHookForTest(async () => {
							assert.equal(
								...
								true,
								"journal must be durable before drift injection",
							);
							if (drift === "incarnation")
								await writeFile(`${tmuxLogPath}.replacement`, "1");
							if (drift === "owner")
								await writeFile(`${tmuxLogPath}.foreign-owner`, "1");
							if (drift === "config") {
								const manifestPath = join(
									cwd,
									".omx",
							...
							if (drift === "generation") {
								...
								const current = JSON.parse(
									await readFile(manifestPath, "utf8"),
								) as { config_generation?: number };
								current.config_generation =
									(current.config_generation ?? 0) + 1;
								await writeFile(manifestPath, JSON.stringify(current));
							}
						});
						await assert.rejects(
							() => shutdownTeam(teamName, cwd, { force: true }),
							/detached_session_destroy_(authorization_unavailable|receipt_config_mismatch):omx-team-journal-/,
						);
						assert.equal(
							existsSync(`${tmuxLogPath}.killed`),
							false,
							`${drift} drift must prevent kill-session`,
						);
					},
				);
			} finally {
	...
	it("fails closed on a malformed detached-session destruction receipt", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-detached-session-malformed-receipt-"),
		);
		try {
			const teamName = "team-malformed-receipt";
			await initTeamState(
				teamName,
				"malformed detached receipt",
				"executor",
				1,
				cwd,
			);
			...
			assert.equal(existsSync(receiptPath), true);
			assert.equal(
				(await readTeamConfig(teamName, cwd))?.tmux_session,
				"omx-team-team-malformed-receipt",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("fails closed for foreign receipt provenance and malformed receipt versions or digests", async () => {
		const cases = [
			...
			},
			{
				...
				receipt: {
					...
					operation: "detached_session_destroy",
					status: "accepted",
					team_name: "receipt-invalid",
					state_root: "/foreign/root",
				},
			},
			{
				name: "digest",
		...
		for (const entry of cases) {
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-runtime-detached-receipt-${entry.name}-`),
			);
			try {
	...
	it("replays an intent receipt only after exact leader PID and owner authorization", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-intent-receipt-replay-"),
		);
		try {
			await withMockTmuxFixture(
				{
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					const receipt = {
						...
						config_identity_digest: createHash("sha256")
							.update(
								JSON.stringify({
									version: 2,
									team_name: teamName,
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					const receipt = {
						...
						operation: "detached_session_destroy",
						status: "accepted",
						session_name: "omx-team-recycled-receipt",
						leader_pane_id: "%1",
						leader_pane_pid: 4241,
						owner_id: "team:recycled-receipt",
					};
					await writeFile(receiptPath, JSON.stringify(receipt));
					await assert.rejects(
						() => shutdownTeam(teamName, cwd, { force: true }),
						/detached_session_destroy_receipt_malformed:omx-team-recycled-receipt/,
					);
					assert.deepEqual(
					...
					assert.equal(
						(await readTeamConfig(teamName, cwd))?.tmux_session,
						"omx-team-recycled-receipt",
					);
					if (existsSync(tmuxLogPath))
						assert.doesNotMatch(
							await readFile(tmuxLogPath, "utf8"),
							/kill-session/,
						);
				},
			);
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					const receipt = {
						...
						operation: "detached_session_destroy",
						status: "accepted",
						session_name: "omx-team-foreign-receipt",
						leader_pane_id: "%1",
						leader_pane_pid: 4241,
						owner_id: "team:foreign-receipt",
					};
					await writeFile(receiptPath, JSON.stringify(receipt));
					await assert.rejects(
						() => shutdownTeam(teamName, cwd, { force: true }),
						/detached_session_destroy_receipt_malformed:omx-team-foreign-receipt/,
					);
					assert.deepEqual(
						JSON.parse(await readFile(receiptPath, "utf8")),
						receipt,
	...
	it("shutdownTeam fails closed when detached HUD PID changes before teardown", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-detached-hud-pid-takeover-"),
		);
		try {
			await withMockTmuxFixture(
				{
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					assert.doesNotMatch(
						tmuxLog,
						/kill-pane -t %2|kill-session -t omx-team-team-dethud-pid/,
					);
				},
			);
		} finally {
	...
	it("shutdownTeam fails closed when detached HUD ownership changes before teardown", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-detached-hud-owner-takeover-"),
		);
		try {
			await withMockTmuxFixture(
				{
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					assert.doesNotMatch(
						tmuxLog,
						/kill-pane -t %2|kill-session -t omx-team-team-dethud-owner/,
					);
				},
			);
		} finally {
	...
	it("shutdownTeam rejects a recycled detached session before kill-session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-detached-session-recycled-"),
		);
		try {
			await withMockTmuxFixture(
				{
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					await initTeamState(
						teamName,
						"reject recycled detached session",
						"executor",
						1,
						cwd,
					);
	...
	it("shutdownTeam rejects a detached leader owner takeover immediately before kill-session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-detached-session-owner-takeover-"),
		);
		try {
			await withMockTmuxFixture(
				{
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					await initTeamState(
						teamName,
						"reject detached leader owner takeover",
						"executor",
						1,
						cwd,
					);
					...
					assert.ok(
						tmuxLog.indexOf(
							"list-panes -a -F #{pane_id}\t#{pane_dead}\t#{pane_pid}\t#{session_name}\t#{session_id}\t#{session_created}",
						) <
							tmuxLog.indexOf(
					...
					assert.doesNotMatch(
						tmuxLog,
						/kill-session -t omx-team-detached-owner-takeover/,
					);
				},
			);
		} finally {
	...
	it("shutdownTeam stops before any OS signal when proof is lost after pane PID authorization", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-malformed-pane-proof-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						"team-shutdown-malformed-proof",
						"shutdown malformed pane proof test",
						"executor",
						1,
						cwd,
					);
					...
					config.resize_hook_name =
						"omx_resize_team_shutdown_malformed_proof_test";
					config.resize_hook_target =
						"omx-team-team-shutdown-malformed-proof:0";
					await saveTeamConfig(config, cwd);
					...
					try {
						await assert.rejects(
							() =>
								shutdownTeam("team-shutdown-malformed-proof", cwd, {
									force: true,
								}),
					...
					const preservedConfig = await readTeamConfig(
						"team-shutdown-malformed-proof",
						cwd,
					);
					assert.ok(preservedConfig);
					assert.equal(
						preservedConfig?.resize_hook_name,
						"omx_resize_team_shutdown_malformed_proof_test",
					);
					assert.equal(
						preservedConfig?.resize_hook_target,
						"omx-team-team-shutdown-malformed-proof:0",
					);
					const tmuxLog = await readFile(tmuxLogPath, "utf-8");
					const globalProofReads =
						tmuxLog.match(
							/list-panes -a -F #\{pane_id\}\t#\{pane_dead\}\t#\{pane_pid\}/g,
						) ?? [];
					...
					assert.doesNotMatch(
						tmuxLog,
						/kill-session -t omx-team-team-shutdown-malformed-proof/,
					);
					assert.doesNotMatch(
						tmuxLog,
						/set-hook -u -t omx-team-team-shutdown-malformed-proof:0 client-resized\[\d+\]/,
					);
				},
			);
		} finally {
	...
	it("shutdownTeam preserves exact-pane state when final liveness proof becomes unknown", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-exact-final-unknown-"),
		);
		const originalProcessKill = process.kill;
		const originalDateNow = Date.now;
		try {
...
`,
				...
				async () => {
					await initTeamState(
						"team-exact-final-unknown",
						"exact final unknown teardown test",
						"executor",
						1,
						cwd,
					);
					...
					await assert.rejects(() =>
						shutdownTeam("team-exact-final-unknown", cwd, { force: true }),
					);
	...
	it("scaleDown commits forward membership and durable debt when global pane proof is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-scale-down-pane-proof-"),
		);
		try {
			await withMockTmuxFixture(
				{
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					assert.match(
						result.error,
						/scale_down_cleanup_debt:pane_teardown_unresolved:%405/,
					);

					const committedConfig = await readTeamConfig(
						"team-scale-down-pane-proof",
						cwd,
					);
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					await assert.rejects(
						() =>
							shutdownTeam("team-shutdown-pane-reconcile", cwd, {
								force: true,
							}),
					...
					assert.doesNotMatch(
						tmuxLog,
						/set-hook -u|kill-pane|split-window|resize-pane|select-pane|run-shell/,
					);
				},
			);
		} finally {
	...
	it("shutdownTeam preserves shared-session state before non-force pane effects when topology cannot be queried", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-topology-unavailable-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown topology unavailable test",
						"executor",
						1,
						cwd,
					);
					...
					assert.doesNotMatch(
						tmuxLog,
						/send-keys|kill-pane|split-window|resize-pane|select-pane|show-option|kill-session/,
					);
				},
			);
		} finally {
	...
	it("shutdownTeam preserves shared-session state before non-force pane effects when topology output is malformed", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-topology-malformed-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown malformed topology test",
						"executor",
						1,
						cwd,
					);
	...
	it("shutdownTeam preserves state when an exact-proven worker pane cannot be killed", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-kill-failure-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					assert.doesNotMatch(
						tmuxLog,
						/split-window|resize-pane|select-pane|kill-session/,
					);
				},
			);
		} finally {
	...
	it("shutdownTeam preserves unrelated non-worker panes during shared-session shutdown", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-unrelated-pane-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					assert.match(
						tmuxLog,
						new RegExp(
							`split-window -v -l ${HUD_TMUX_TEAM_HEIGHT_LINES} -t %10 -d -P -F #\\{pane_id\\}`,
						),
					);
	...
	it("shutdownTeam fails closed before HUD effects when topology omits a canonical shared worker", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-stale-worker-pane-id-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown stale persisted worker pane id test",
						"executor",
						2,
						cwd,
					);
					...
					assert.doesNotMatch(
						tmuxLog,
						/split-window|resize-pane|select-pane|run-shell/,
					);
				},
			);
		} finally {
	...
	it("shutdownTeam fails closed when a canonical shared worker owner tag is missing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-worker-owner-missing-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown worker owner missing test",
						"executor",
						2,
						cwd,
					);
	...
	it("shutdownTeam fails closed when a canonical shared worker owner tag cannot be read", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-worker-owner-read-error-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown worker owner read error test",
						"executor",
						2,
						cwd,
					);
	...
	it("shutdownTeam does not use a detected worker as fallback leader when the shared-session leader is stale", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-stale-leader-worker-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown stale leader worker test",
						"executor",
						1,
						cwd,
					);
	...
	it("shutdownTeam rejects a reused persisted HUD pane after successful and partial session persistence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-reused-leader-pane-"),
		);
...
`,
	...
	it("shutdownTeam preserves an unrelated HUD when the leader is live but persisted shared-session HUD is stale", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-win32-stale-topology-"),
		);
...
`,
					...
					async ({ tmuxLogPath }) => {
						await initTeamState(
							teamName,
							"shutdown win32 stale topology test",
							"executor",
							2,
							cwd,
						);
	...
	it("shutdownTeam fails closed when a tagged shared worker loses its owner tag before the HUD gate", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-hud-prevalidation-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					await assert.rejects(
						() => shutdownTeam(teamName, cwd, { force: true }),
						/shutdown_shared_session_worker_owner_changed:%13/,
					);
					assert.equal(
						existsSync(join(cwd, ".omx", "state", "team", teamName)),
						true,
					...
					assert.doesNotMatch(
						tmuxLog,
						/set-hook -u|kill-pane|split-window|resize-pane|select-pane/,
					);
				},
			);
		} finally {
	...
	it("shutdownTeam fails closed before killing a shared HUD whose pane PID changes after authorization", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-hud-pid-continuity-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shared HUD PID continuity test",
						"executor",
						1,
						cwd,
					);
	...
	it("shutdownTeam blocks restoration when a shared leader PID and owner change after the post-HUD check", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-leader-owner-continuity-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shared leader owner continuity test",
						"executor",
						1,
						cwd,
					);
					...
					await assert.rejects(
						() => shutdownTeam(teamName, cwd, { force: true }),
						/shutdown_shared_session_restore_leader_pane_owner_changed:%11/,
					);
					assert.equal(
						existsSync(join(cwd, ".omx", "state", "team", teamName)),
						true,
	...
	it("shutdownTeam skips prekill and keeps the leader pane alive on shared-session shutdown", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-shared-session-"),
		);
		try {
			await withMockTmuxFixture(
				{
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						"team-shutdown-shared-session",
						"shutdown shared session test",
						"executor",
						2,
						cwd,
					);
					...
					await shutdownTeam("team-shutdown-shared-session", cwd, {
						force: true,
					});
					...
					assert.equal(existsSync(teamRoot), false);
					assert.equal(
						await readMonitorSnapshot("team-shutdown-shared-session", cwd),
						null,
					);
	...
	it("shutdownTeam restores a standalone HUD pane after tearing down the team HUD", async () => {
		...
		const leaderPaneCwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-restore-hud-leader-cwd-"),
		);
		try {
			await withMockTmuxFixture(
				{
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					const count = (pattern: RegExp): number =>
						[...tmuxLog.matchAll(pattern)].length;
					assert.doesNotMatch(tmuxLog, /kill-pane -t %11/);
					assert.match(tmuxLog, /kill-pane -t %12/);
					assert.match(tmuxLog, /kill-pane -t %13/);
					assert.match(
					...
					assert.match(
						tmuxLog,
						/run-shell -b sleep \d+; if snapshot=\$\(tmux list-panes -a -F/,
					);
					assert.match(
						tmuxLog,
						/run-shell if snapshot=\$\(tmux list-panes -a -F/,
					);
					assert.match(tmuxLog, /hud --watch/);
					assert.match(tmuxLog, /OMX_TMUX_HUD_LEADER_PANE='%11'/);
					assert.match(
						tmuxLog,
						/OMX_SESSION_ID='team-shutdown-restore-hud-session'/,
					);
					assert.match(
						tmuxLog,
						new RegExp(
							`split-window -v -l ${HUD_TMUX_TEAM_HEIGHT_LINES} -t %11 -d -P -F #\\{pane_id\\} -c ${escapeRegExp(leaderPaneCwd)} `,
						),
					);
					assert.doesNotMatch(
						tmuxLog,
						new RegExp(
							`split-window -v -l ${HUD_TMUX_TEAM_HEIGHT_LINES} -t %11 -d -P -F #\\{pane_id\\} -c ${escapeRegExp(cwd)} `,
						),
					);
					assert.doesNotMatch(tmuxLog, /kill-pane -t %44/);
					assert.match(tmuxLog, /select-pane -t %11/);
	...
	it("shutdownTeam replays pinned restored-HUD debt when a crash leaves canonical config on a different HUD identity", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-restored-hud-debt-retry-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					...
					const debtPath = join(
						...
						"team",
						teamName,
						".restored-hud-cleanup-debt.json",
					);
					await writeFile(
						debtPath,
						`${JSON.stringify({
	...
	it("shutdownTeam clears mismatched restored-HUD config before repeated detached recovery leaves a live non-HUD pane untouched", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-restored-hud-mismatched-detached-"),
		);
		...
		try {
			await withMockTmuxFixture(
				{
					dirPrefix:
						"omx-runtime-shutdown-restored-hud-mismatched-detached-bin-",
					tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"restored HUD mismatch detached recovery test",
						"executor",
						1,
						cwd,
					);
					...
					assert.doesNotMatch(
						tmuxLog,
						/kill-pane -t %44|kill-pane -t %45|kill-session|resize-pane|select-pane|send-keys|run-shell/,
					);
					assert.doesNotMatch(
						tmuxLog,
						/show-option -p -t %45 @omx_team_pane_owner_id/,
					);
				},
			);
		} finally {
	...
	it("shutdownTeam retires matching restored-HUD config when the pinned pane is no longer a HUD", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-restored-hud-matching-nonhud-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"matching restored HUD non-authority test",
						"executor",
						1,
						cwd,
					);
					...
					assert.doesNotMatch(
						tmuxLog,
						/kill-pane|kill-session|resize-pane|select-pane|send-keys|run-shell/,
					);
					assert.doesNotMatch(
						tmuxLog,
						/show-option -p -t %44 @omx_team_pane_owner_id/,
					);
				},
			);
		} finally {
	...
	it("shutdownTeam preserves unpersisted legacy worker-looking panes without owner tags", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-unpersisted-legacy-worker-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown unpersisted legacy worker test",
						"executor",
						2,
						cwd,
					);
	...
	it("shutdownTeam reclaims a matching-owner live HUD when the persisted HUD id is stale", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-stale-hud-live-owner-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown stale hud live owner test",
						"executor",
						1,
						cwd,
					);
	...
	it("shutdownTeam reclaims an owner-tagged shared HUD pane", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-legacy-hud-owner-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown legacy hud owner test",
						"executor",
						1,
						cwd,
					);
	...
	it("shutdownTeam restores standalone HUD only for an owner-tagged leader", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-legacy-leader-instance-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown legacy leader instance test",
						"executor",
						1,
						cwd,
					);
					...
					assert.doesNotMatch(
						tmuxLog,
						/show-option -qv -p -t %11 @omx_pane_instance_id/,
					);
					assert.match(tmuxLog, /kill-pane -t %12/);
					assert.match(tmuxLog, /kill-pane -t %13/);
					assert.match(
	...
	it("shutdownTeam preserves a persisted HUD pane when the team owner tag cannot be read", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-hud-owner-read-error-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown hud owner read error test",
						"executor",
						1,
						cwd,
					);
...
`,
	...
	it("resumeTeam fails closed when the persisted approved binding is stale", async () => {
		...
		try {
			await initTeamState(
				"team-approved-resume",
				"approved resume test",
				"executor",
				1,
				cwd,
			);
	...
	it("resumeTeam fails closed when the persisted approved binding is ambiguous", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-approved-resume-ambiguous-"),
		);
		...
		try {
			...
			await writeFile(
				join(cwd, ".omx", "plans", "test-spec-issue-2111.md"),
				"# Test spec\n",
			);
			await writePersistedApprovedTeamExecutionBinding(
				"team-approved-resume",
	...
	it("resumeTeam accepts persisted approved bindings that still resolve to a baseline-ready hint", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-approved-resume-nonready-"),
		);
		try {
			await initTeamState(
				"team-approved-resume",
			...
			await writeFile(
				join(cwd, ".omx", "plans", "test-spec-issue-2112.md"),
				"# Test spec\n",
			);
			await writePersistedApprovedTeamExecutionBinding(
				"team-approved-resume",
	...
	it("resumeTeam resolves approved binding continuity against the persisted leader cwd", async () => {
		...
		const leaderCwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-approved-leader-"),
		);
		const resumeCwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-approved-resume-alt-"),
		);
		const sharedStateRoot = await mkdtemp(
			join(tmpdir(), "omx-runtime-approved-state-"),
		);
		...
		try {
			...
			await writeFile(
				join(plansDir, "test-spec-issue-2110.md"),
				"# Test spec\n",
			);
			await writePersistedApprovedTeamExecutionBinding(
				teamName,
		...
		try {
			...
			assert.ok(
				events.some(
					(event) =>
						event.reason ===
						`prompt_resume_unavailable:missing_handle:worker-1:${sleeperPid}`,
				),
	...
	it("startTeam persists synthesized delegation plans for broad tasks", async () => {
		...
		try {
			runtime = await withPromptModeCodexEnv(binDir, {}, () =>
				withoutTeamWorkerEnv(() =>
					startTeam(
						...
						[
							{
								subject: "Investigate runtime assignment",
								description:
									"Search runtime, debug assignTask behavior, and coordinate shared handoff boundaries",
							},
						],
						cwd,
					),
			...
			assert.ok(
				task?.coordination?.activation_reasons.includes(
	...
	it("startTeam persists approved execution binding under the team state root", async () => {
		...
		try {
					...
					startTeam(
						...
						cwd,
						{
							approvedExecution: {
								...
								command:
									'omx team 1:executor "Execute approved issue 1314 plan"',
							},
						},
					),
				),
			...
			const binding = JSON.parse(
				await readFile(bindingPath, "utf-8"),
	...
	it("startTeam treats a completed Ultragoal plan without activeGoalId as no active bridge context", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-ultragoal-idle-start-"),
		);
		const binDir = join(cwd, "bin");
		const fakeCodexPath = join(binDir, "codex");
		await mkdir(binDir, { recursive: true });
		...
		try {
			...
			assert.ok(
				preflight.prohibitions.includes("workers_must_not_mutate_ultragoal"),
			);
			assert.ok(
				preflight.resume_instructions.some((line) =>
					/preflight-context\.json/.test(line),
				),
			);
			...
			assert.doesNotMatch(
				inbox,
				/omx ultragoal checkpoint --goal-id G001-completed-story/,
			);
		} finally {
			if (runtime) {
				await shutdownTeam(runtime.teamName, cwd, { force: true }).catch(
	...
	it("startTeam tolerates malformed Ultragoal artifacts for unrelated Team startup", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-ultragoal-malformed-optional-"),
		);
		const binDir = join(cwd, "bin");
		const fakeCodexPath = join(binDir, "codex");
		await mkdir(binDir, { recursive: true });
	...
	it("startTeam fails closed for malformed artifacts when explicitly linked to an Ultragoal goal", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-ultragoal-malformed-strict-"),
		);
		const binDir = join(cwd, "bin");
		const fakeCodexPath = join(binDir, "codex");
		await mkdir(binDir, { recursive: true });
		...
		try {
			...
			assert.equal(
				existsSync(
					join(cwd, ".omx", "state", "team", "team-ultragoal-malformed-strict"),
				),
				false,
			);
		} finally {
	...
	it("startTeam injects approved handoff context into ready approved worker inboxes", async () => {
		...
		const testSpecPath = join(
			...
			".omx",
			"plans",
			"test-spec-issue-1314-handoff.md",
		);
		await writeFile(
			prdPath,
			[
				...
				buildContextPackOutcome(
					canonicalContextPackRelativePath("issue-1314-handoff"),
				),
				"",
		...
		try {
			...
			assert.match(
				inbox,
				new RegExp(
					`Approved plan: ${prdPath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
				),
			);
			assert.match(
				inbox,
				new RegExp(
					`Test specs: ${testSpecPath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
				),
			);
			assert.match(
				inbox,
				/Approved repository context summary source: .*repo-context-issue-1314-handoff\.md/,
			);
	...
	it("startTeam carries explicit baseline-ready approved bindings without context-pack metadata", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-approved-binding-plan-only-"),
		);
		const binDir = join(cwd, "bin");
		const fakeCodexPath = join(binDir, "codex");
		await mkdir(binDir, { recursive: true });
		...
		await writeFile(
			join(cwd, ".omx", "plans", "test-spec-issue-1314-plan-only.md"),
			"# Test spec\n",
		);
		await writeFakePromptWorkerBinary(
			fakeCodexPath,
						...
						cwd,
						{
							approvedExecution: {
								prd_path: join(
									...
									".omx",
									"plans",
									"prd-issue-1314-plan-only.md",
								),
								task: "Execute approved issue 1314 plan-only",
								command:
									'omx team 1:executor "Execute approved issue 1314 plan-only"',
							},
						},
					),
				),
	...
	it("startTeam carries explicit baseline-ready bindings despite obsolete context-pack markers", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-approved-binding-obsolete-marker-"),
		);
		const binDir = join(cwd, "bin");
		const fakeCodexPath = join(binDir, "codex");
		await mkdir(binDir, { recursive: true });
		...
		const prdPath = join(
			...
			".omx",
			"plans",
			"prd-issue-1314-obsolete-marker.md",
		);
		await writeFile(
			prdPath,
			[
		...
		await writeFile(
			join(cwd, ".omx", "plans", "test-spec-issue-1314-obsolete-marker.md"),
			"# Test spec\n",
		);
		await writeFakePromptWorkerBinary(
			fakeCodexPath,
		...
		try {
			...
			assert.match(
				inbox,
				/Approved plan: .*prd-issue-1314-obsolete-marker\.md/,
			);
			assert.match(
				inbox,
				/Test specs: .*test-spec-issue-1314-obsolete-marker\.md/,
			);
			assert.doesNotMatch(inbox, /Approved context pack|Context pack index/);
		} finally {
			if (runtime) {
	...
	it("startTeam fails closed when an explicit approved execution binding is stale", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-approved-binding-stale-"),
		);
		const binDir = join(cwd, "bin");
		const fakeCodexPath = join(binDir, "codex");
		await mkdir(binDir, { recursive: true });
		...
		await writeFile(
			join(cwd, ".omx", "plans", "test-spec-issue-1315.md"),
			"# Test spec\n",
		);
		...
		try {
			...
			assert.equal(
				existsSync(
					join(cwd, ".omx", "state", "team", "team-approved-binding-stale"),
				),
				false,
			);
		} finally {
	...
	it("startTeam fails closed when an explicit approved execution binding is ambiguous", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-approved-binding-ambiguous-"),
		);
		const binDir = join(cwd, "bin");
		const fakeCodexPath = join(binDir, "codex");
		...
		await writeFile(
			join(cwd, ".omx", "plans", "test-spec-issue-1316.md"),
			"# Test spec\n",
		);
		await writeFakePromptWorkerBinary(
			fakeCodexPath,
		...
		try {
			...
			assert.equal(
				existsSync(
					join(cwd, ".omx", "state", "team", "team-approved-binding-ambiguous"),
				),
				false,
			);
		} finally {
		...
		try {
			...
			const task = await createTask(
				"team-assign-delegation",
				{
					...
					coordination: {
						mode: "coordinated",
						activation_reasons: ["stale_snapshot_before_assignment"],
					},
				},
				cwd,
			);
			...
			assert.ok(
				reread?.coordination?.activation_reasons.includes("shared_file_scope"),
			);
			assert.equal(
				reread?.coordination?.activation_reasons.includes(
					"stale_snapshot_before_assignment",
				),
				false,
			);
	...
	it("assignTask preserves explicitly authored coordination metadata", async () => {
		...
		try {
			await initTeamState(
				...
				"assignment explicit coordination test",
				"executor",
				1,
				cwd,
			);
			...
			const task = await createTask(
				"team-assign-explicit",
				{
					...
					coordination: {
						mode: "coordinated",
						activation_reasons: ["caller_authored_boundary"],
						source: "explicit",
					},
				},
				cwd,
			...
			assert.deepEqual(reread?.coordination?.activation_reasons, [
				"caller_authored_boundary",
			]);
			assert.equal(
				reread?.coordination?.activation_reasons.includes("shared_file_scope"),
				false,
		...
		try {
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-1320"),
					),
					"",
					`Launch via omx team 1:executor "${approvedTask}"`,
				].join("\n"),
			...
			const task = await createTask(
				"team-approved-followup",
				{
					subject: "Implement approved follow-up",
					description: "Implement approved follow-up",
					status: "pending",
				},
				cwd,
			);

			const assignPromise = assignTask(
				"team-approved-followup",
				"worker-1",
				task.id,
				cwd,
			...
			assert.ok(
				delivered,
				"expected follow-up inbox dispatch request to be queued",
			);
...
`,
	...
	it("sendWorkerMessage keeps failed hook receipts failed when fallback mailbox persistence confirms delivery", {
		concurrency: false,
	}, async () => {
		const cwd = await mkdtemp(
...
`,
	...
	it("sendWorkerMessage transport_direct fails fast for leader-fixed when leader_pane_id missing", async () => {
		...
		try {
			...
			assert.equal(
				latest?.last_reason,
				"leader_pane_missing_transport_direct_failed",
			);
			assert.ok(
				latest?.failed_at,
				"missing leader pane should fail fast with failed_at evidence",
			);
			...
			assert.equal(
				runtimeEntries.length,
				1,
				"leader direct missing-pane failure should emit exactly one runtime dispatch_result entry",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
