...
`,
				...
				async () => {
					...
					assert.ok(
						captureCount >= 2,
						`expected ready wait capture after bootstrapping, got ${captureCount}`,
					);
...
`,
	...
	it("shutdownTeam preserves worker-looking panes with a mismatched team owner tag", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-shutdown-worker-owner-mismatch-"),
		);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown worker owner mismatch test",
						"executor",
						2,
						cwd,
					);
...
`,
				...
				async ({ tmuxLogPath }) => {
					await initTeamState(
						teamName,
						"shutdown reused leader pane test",
						"executor",
						1,
						cwd,
					);
					...
					assert.match(
						tmuxLog,
						/show-option -qv -p -t %12 @omx_team_pane_owner_id/,
					);
					assert.doesNotMatch(tmuxLog, /kill-pane -t %11/);
