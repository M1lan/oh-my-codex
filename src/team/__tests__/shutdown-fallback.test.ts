...
`,
		...
		try {
			...
			await writeFile(
				join(worktreePath as string, "worker-note.txt"),
				"from worker\n",
				"utf-8",
			);
			...
			assert.equal(
				await readFile(join(repo, "worker-note.txt"), "utf-8"),
				"from worker\n",
			);
			assert.ok(
				preservedWorktreePath,
				"preserved worktree path should be captured",
			);
			assert.equal(existsSync(preservedWorktreePath as string), true);

			const reportPath = join(
				preservedWorktreePath as string,
				".omx",
				"diff.md",
			);
			...
			const commitHygieneJsonPath = join(
				repo,
				".omx",
				"reports",
				"team-commit-hygiene",
				"team-shutdown-fallback-report.context.json",
			);
			const commitHygieneMarkdownPath = join(
				repo,
				".omx",
				"reports",
				"team-commit-hygiene",
				"team-shutdown-fallback-report.md",
			);
			assert.equal(
				existsSync(commitHygieneJsonPath),
				true,
				"shutdown should preserve a structured commit hygiene context artifact",
			);
			assert.equal(
				existsSync(commitHygieneMarkdownPath),
				true,
				"shutdown should preserve a human-readable commit hygiene guide",
			);

			const commitHygieneContext = JSON.parse(
				await readFile(commitHygieneJsonPath, "utf-8"),
			) as {
			...
			assert.equal(
				commitHygieneContext.operational_entries.some(
					(entry) => entry.operation === "shutdown_checkpoint",
				),
				true,
			);
			assert.equal(
				commitHygieneContext.operational_entries.some(
					(entry) =>
						entry.operation === "shutdown_merge" && entry.status === "applied",
				),
				true,
			);
			assert.match(
				commitHygieneContext.leader_finalization_prompt,
				/Lore-format final commit/i,
			);

			const commitHygieneGuide = await readFile(
				commitHygieneMarkdownPath,
				"utf-8",
			);
