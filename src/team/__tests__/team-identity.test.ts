...
async function writePhase(
	...
	await writeFile(
		join(cwd, ".omx", "state", "team", teamName, "phase.json"),
		JSON.stringify(
			{
...
describe("team identity", () => {
	it("builds stable valid internal names for same display and distinct sessions", () => {
		const a = buildInternalTeamName(longDisplay, {
			sessionId: "session-a",
			paneId: "",
			tmuxTarget: "",
			runId: "",
		...
		const b = buildInternalTeamName(longDisplay, {
			sessionId: "session-b",
			paneId: "",
			tmuxTarget: "",
			runId: "",
		...
		const a2 = buildInternalTeamName(longDisplay, {
			sessionId: "session-a",
			paneId: "",
			tmuxTarget: "",
			runId: "",
		...
		const runA = buildInternalTeamName("demo", {
			sessionId: "",
			paneId: "",
			tmuxTarget: "",
			runId: "run-a",
		...
		const runB = buildInternalTeamName("demo", {
			sessionId: "",
			paneId: "",
			tmuxTarget: "",
			runId: "run-b",
	...
	it("does not use cwd session.json as the identity source when env is absent", () => {
		const scope = resolveTeamIdentityScope({
			TMUX: "/tmp/tmux,1,0",
			TMUX_PANE: "%42",
		});
	...
	it("resolves display names from OMX_TEAM_STATE_ROOT when cwd has no local team state", async () => {
		const leaderCwd = await mkdtemp(
			join(tmpdir(), "omx-team-identity-leader-"),
		);
		const workerCwd = await mkdtemp(
			join(tmpdir(), "omx-team-identity-worker-"),
		);
		try {
			await initTeamState(
	...
	it("prefers active display-name candidates over retained terminal states", async () => {
		...
		try {
			...
			await initTeamState(
				...
				cwd,
				undefined,
				{ OMX_SESSION_ID: "session-terminal" },
				{
					display_name: "demo",
					requested_name: "demo",
					identity_source: "env-session",
			...
			assert.equal(
				resolveTeamNameForCurrentContext("demo", cwd, {}),
				"demo-active",
			);
		} finally {
	...
	it("prefers active display-name candidates over an exact retained terminal directory", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-identity-exact-terminal-"),
		);
		try {
			await initTeamState(
				"demo",
	...
	it("uses current leader identity before latest retained terminal state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-identity-terminal-current-"),
		);
		try {
			await initTeamState(
				"demo-old-current",
				...
				cwd,
				undefined,
				{ OMX_SESSION_ID: "session-current" },
				{
					display_name: "demo",
					requested_name: "demo",
					identity_source: "env-session",
	...
	it("resolves the latest retained terminal display-name state only when unambiguous", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-identity-latest-terminal-"),
		);
		try {
			await initTeamState(
				"demo-old",
	...
	it("sanitizes unsafe lookup input instead of returning raw path-like names", async () => {
		...
		try {
			assert.equal(
				resolveTeamNameForCurrentContext("../../victim", cwd, {}),
				"victim",
			);
			assert.equal(
				resolveTeamNameForCurrentContext("Demo Team", cwd, {}),
				"demo-team",
			);
			assert.throws(
				() => resolveTeamNameForCurrentContext("---", cwd, {}),
				/invalid_team_name/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("resolves display names to the current session candidate and fails closed on ambiguity", async () => {
		...
		try {
			...
			assert.equal(
				resolveTeamNameForCurrentContext("demo-bbbbbbbb", cwd, {}),
				"demo-bbbbbbbb",
			);
			assert.throws(
				() => resolveTeamNameForCurrentContext("demo", cwd, {}),
