...
describe("team runtime boxed state path helpers", () => {
	it("routes runtime-owned team state paths through OMX_ROOT without changing source cwd semantics", () => {
		...
		try {
			...
			assert.equal(
				teamRuntimeTeamsRoot("/tmp/source"),
				"/tmp/box/.omx/state/team",
			);
			assert.equal(
				teamRuntimeTeamRoot("team-a", "/tmp/source"),
				"/tmp/box/.omx/state/team/team-a",
			);
			assert.equal(
			...
			assert.equal(
				teamRuntimeSessionPath("/tmp/source"),
				"/tmp/box/.omx/state/session.json",
			);
