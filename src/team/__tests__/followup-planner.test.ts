...
describe("followup-planner", () => {
	it("resolves available agent types from explicit prompt directories", async () => {
		...
		try {
			...
			const roles = await resolveAvailableAgentTypes(process.cwd(), {
				promptDirs: [dir],
			});
	...
	it("builds concrete team staffing guidance from the available roster", () => {
		...
		assert.ok(
			plan.allocations.every((allocation) =>
				["executor", "test-engineer", "writer"].includes(allocation.role),
			),
		);
		assert.ok(
			plan.allocations.some(
				(allocation) =>
					allocation.reason.includes("specialist") ||
					allocation.reason.includes("verification"),
			),
		);
		assert.equal(
			plan.launchHints.shellCommand,
			'omx team 3:executor "Fix flaky integration tests and update README"',
		);
		assert.equal(
			plan.launchHints.skillCommand,
			'$team 3:executor "Fix flaky integration tests and update README"',
		);
		assert.match(
			plan.verificationPlan.summary,
			/coordinated execution and verification owner/i,
		);
	...
	it("builds concrete ralph staffing guidance from the available roster", () => {
		...
		assert.ok(
			plan.allocations.some((allocation) =>
				allocation.reason.includes("sign-off"),
			),
		);
		assert.equal(
			plan.launchHints.shellCommand,
			'omx ralph "Investigate auth regression and verify the fix"',
		);
		assert.equal(
			plan.launchHints.skillCommand,
			'$ralph "Investigate auth regression and verify the fix"',
		);
		assert.match(
			plan.verificationPlan.summary,
			/persistent execution and verification owner/i,
		);
		assert.equal(plan.verificationPlan.checkpoints.length, 3);
	});
	...
	it("applies per-agent reasoning overrides to Ralph sign-off staffing guidance", async () => {
		...
		try {
			...
			assert.ok(
				plan.allocations.some(
					(allocation) =>
						allocation.role === "architect" &&
						allocation.reasoningEffort === "xhigh",
				),
			);
	...
	it("allocates explore plus researcher lanes for mixed local+official-doc follow-up", () => {
		...
		assert.ok(
			plan.allocations.some(
				(allocation) =>
					allocation.role === "explore" &&
					allocation.reason.includes("primary"),
			),
		);
		assert.ok(
			plan.allocations.some(
				(allocation) =>
					allocation.role === "researcher" &&
					allocation.reason.includes("specialist"),
			),
		);
	});
	...
	it("allocates explore plus dependency-expert lanes for mixed local+dependency evaluation follow-up", () => {
		...
		assert.ok(
			plan.allocations.some(
				(allocation) =>
					allocation.role === "dependency-expert" &&
					allocation.reason.includes("specialist"),
			),
		);
	});
	...
	it("recognizes short approved team follow-up shortcuts in English and Korean", () => {
		...
		assert.equal(
			isApprovedExecutionFollowupShortcut("team", "team으로 해줘", {
				planningComplete: true,
			}),
			true,
		);
