...
describe("role-router", () => {
	...
	describe("listAvailableRoles", () => {
		...
		it("does not expose command-specific AGENTS instruction files as roles from the repo prompts directory", async () => {
			...
			assert.equal(
				roles.some((role) => role.endsWith("-AGENTS")),
				false,
			);
		});
	});
	...
	describe("routeTaskToRole", () => {
		it("routes test-related tasks to test-engineer with high confidence", () => {
			const result = routeTaskToRole(
				"Write unit tests",
				"Add jest test coverage for the auth module",
				"team-exec",
				"executor",
			);
		...
		it("routes UI tasks to designer with high confidence", () => {
			const result = routeTaskToRole(
				"Build UI component",
				"Create a responsive layout with CSS and Tailwind",
				"team-exec",
				"executor",
			);
		...
		it("routes build error tasks to debugger", () => {
			const result = routeTaskToRole(
				"Fix build",
				"Resolve tsc type errors in the compile step",
				"team-fix",
				"executor",
			);
		...
		it("routes debug tasks to debugger", () => {
			const result = routeTaskToRole(
				...
				"Debug the root cause of the stack trace failure",
				"team-fix",
				"executor",
			);
			assert.equal(result.role, "debugger");
		...
		it("routes documentation tasks to writer", () => {
			const result = routeTaskToRole(
				"Update docs",
				"Write README and migration guide for the new API",
				"team-exec",
				"executor",
			);
		...
		it("keeps implementation-heavy auth work on the implementation fallback lane", () => {
			const result = routeTaskToRole(
				"Implement auth session refresh",
				"Add authentication refresh handling and authorization checks to the login flow",
				"team-exec",
				"executor",
			);
		...
		it("routes refactoring tasks to code-simplifier", () => {
			const result = routeTaskToRole(
				"Refactor auth",
				"Simplify and clean up the authentication module",
				"team-exec",
				"executor",
			);
		...
		it("is deterministic for the same inputs", () => {
			const r1 = routeTaskToRole(
				"Write tests",
				"Add test coverage",
				"team-exec",
				"executor",
			);
			const r2 = routeTaskToRole(
				"Write tests",
				"Add test coverage",
				"team-exec",
				"executor",
		...
		it("recognizes common Korean documentation/test signals", () => {
			const docs = routeTaskToRole(
				"문서 업데이트",
				"배포 가이드와 README 문서를 정리",
				"team-exec",
				"executor",
			);
			const tests = routeTaskToRole(
				"테스트 추가",
				"로그인 흐름 테스트와 커버리지 추가",
				"team-exec",
				"executor",
