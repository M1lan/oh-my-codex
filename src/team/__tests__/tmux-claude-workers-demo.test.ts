...
describe("tmux claude workers demo", () => {
	...
	describe("claude worker launch spec", () => {
		...
		it("claude worker drops codex-specific flags", () => {
			...
			assert.ok(
				!spec.args.includes("-c"),
				"claude worker should not have -c flag",
			);
			assert.ok(
				!spec.args.includes("--json"),
				"claude worker should not have --json flag",
			);
		});
	...
	describe("full demo scenario with 6 mixed workers", () => {
		it("creates correct launch specs for all workers in demo", () => {
			...
			for (let i = 1; i <= workerCount; i++) {
				...
				assert.ok(
					spec.env.OMX_LEADER_NODE_PATH,
					"OMX_LEADER_NODE_PATH must be set",
				);
				assert.ok(
					spec.env.OMX_LEADER_CLI_PATH,
