...
describe("team hardening e2e", () => {
	it("reopens an expired in-progress task and allows the next worker to complete the flow", async () => {
		...
		try {
			...
			const task = await createTask(
				"team-hardening-e2e",
				{ subject: "recover me", description: "d", status: "pending" },
				cwd,
			);
			...
			assert.equal(
				snapshot?.recommendations.some(
					(r) =>
						...
						r.includes("Reclaimed expired claim"),
				),
				true,
			);
			...
			const secondClaim = await claimTask(
				...
				task.id,
				"worker-2",
				reopened?.version ?? null,
				cwd,
			);
			assert.equal(secondClaim.ok, true);
			assert.equal(
				(secondClaim.ok && secondClaim.task.owner) || null,
				"worker-2",
			);
		} finally {
	...
	it("tolerates malformed worker status from an external team state root", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-hardening-worker-state-"),
		);
		...
		try {
			...
			await initTeamState(
				...
				"worker status corruption smoke",
				"executor",
				1,
				cwd,
			);
