...
describe("withTempTmuxSession", () => {
	...
	it("rejects private server logging on the ambient server", async (t) => {
		...
		await assert.rejects(
			withTempTmuxSession(
				{ useAmbientServer: true, serverLog: true },
				async () => undefined,
			),
	...
	it("rejects out-of-range client resize geometry before attaching", async (t) => {
		...
		await withTempTmuxSession(async (fixture) => {
			assert.throws(
				() => fixture.triggerClientResize(fixture.sessionName, { rows: 3 }),
				/invalid trigger rows: 3/,
			);
			assert.throws(
				() => fixture.triggerClientResize(fixture.sessionName, { rows: 501 }),
				/invalid trigger rows: 501/,
			);
			assert.throws(
				() => fixture.triggerClientResize(fixture.sessionName, { rows: 40.5 }),
				/invalid trigger rows: 40.5/,
			);
			assert.throws(
				() => fixture.triggerClientResize(fixture.sessionName, { cols: 19 }),
				/invalid trigger cols: 19/,
			);
			assert.throws(
				() => fixture.triggerClientResize(fixture.sessionName, { cols: 501 }),
				/invalid trigger cols: 501/,
			);
		});
	});
	...
	it("triggers client resize when GNU timeout is absent from PATH", async (t) => {
		...
		await withTempTmuxSession(async (fixture) => {
			const isolatedPath = await mkdtemp(
				join(tmpdir(), "omx-no-timeout-path-"),
			);
			...
			try {
				...
				fixture.triggerClientResize(fixture.sessionName, {
					rows: 41,
					cols: 121,
				});
			} finally {
