...
async function createWorktreePointerFixture(
	...
	await writeFile(
		join(commonDir, "logs", "refs", "heads", "worktree-branch"),
		"new\n",
	);
...
describe("leader runtime activity", () => {
	...
	it("uses the newest runtime signal across hud and explicit leader activity", async () => {
		...
		try {
			...
			await writeFile(
				join(stateDir, "leader-runtime-activity.json"),
				JSON.stringify({
			...
			const lastActivityMs =
				await readLatestLeaderActivityMsFromStateDir(stateDir);
	...
	it("treats recent leader-branch git movement as activity even when runtime timestamps are stale", async () => {
		...
		try {
			...
			execFileSync("git", ["add", "README.md"], { cwd, stdio: "ignore" });
			execFileSync("git", ["commit", "-m", "leader progress"], {
				cwd,
				stdio: "ignore",
			});
			...
			assert.equal(
				await isLeaderRuntimeStale(stateDir, 30_000, headMs + 5_000),
				false,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("treats worktree .git file pointers as recent branch activity on Windows", async () => {
		...
		try {
			...
			await withWindowsPlatform(async () => {
				assert.equal(
					await isLeaderRuntimeStale(stateDir, 30_000, recentMs + 5_000),
					false,
				);
			});
		} finally {
	...
	it("uses absolute git-path results for real worktree reflogs", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-leader-activity-worktree-real-"),
		);
		...
		try {
			...
			const headLogPath = execFileSync(
				"git",
			...
			const branchLogPath = execFileSync(
				"git",
				["rev-parse", "--git-path", "logs/refs/heads/feature"],
				{
					cwd: worktreePath,
					encoding: "utf-8",
					stdio: ["ignore", "pipe", "ignore"],
	...
	it("treats missing or invalid runtime evidence as not stale", async () => {
		...
		try {
			...
			assert.equal(
				await isLeaderRuntimeStale(stateDir, 30_000, Date.now()),
				false,
			);

			await writeFile(
				join(stateDir, "hud-state.json"),
				JSON.stringify({ last_turn_at: "not-a-date" }),
			);
			await writeFile(
				join(stateDir, "leader-runtime-activity.json"),
				JSON.stringify({ last_activity_at: "" }),
			);
