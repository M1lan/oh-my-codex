	...
	it("surfaces cross-rebase applied as an audit event without making it wakeable", async () => {
		...
		try {
			worker1Path = await addWorktree(
				repo,
				"wk1-cross-applied",
				"omx-cross-applied-w1-",
			);
			worker2Path = await addWorktree(
				repo,
				"wk2-cross-applied",
				"omx-cross-applied-w2-",
			);
			...
			const events = await readTeamEvents("cross-rebase-applied", repo, {
				wakeableOnly: false,
			});
			const wakeable = await readTeamEvents("cross-rebase-applied", repo, {
				wakeableOnly: true,
			});
			...
			assert.equal(
				typeof snapshot?.integrationByWorker?.["worker-2"]
					?.last_rebased_leader_head,
				"string",
			);
			assert.equal(
				events.some((event) => event.type === "worker_cross_rebase_applied"),
				true,
			);
			assert.equal(
				wakeable.some((event) => event.type === "worker_cross_rebase_applied"),
				false,
			);
		} finally {
	...
	it("surfaces cross-rebase conflicts as wakeable and preserves cleanup/report evidence", async () => {
		...
		try {
			...
			worker1Path = await addWorktree(
				repo,
				"wk1-cross-conflict",
				"omx-cross-conflict-w1-",
			);
			worker2Path = await addWorktree(
				repo,
				"wk2-cross-conflict",
				"omx-cross-conflict-w2-",
			);
			...
			const worker2StatusOutput = execFileSync("git", ["status"], {
				cwd: worker2Path,
				encoding: "utf-8",
			});
			const events = await readTeamEvents("cross-rebase-conflict", repo, {
				wakeableOnly: false,
			});
			const wakeable = await readTeamEvents("cross-rebase-conflict", repo, {
				wakeableOnly: true,
			});
			...
			assert.equal(
				events.some((event) => event.type === "worker_cross_rebase_conflict"),
				true,
			);
			assert.equal(
				wakeable.some((event) => event.type === "worker_cross_rebase_conflict"),
				true,
			);
			assert.equal(
				leaderMailbox.some((message) =>
					/rebase onto .* with -X ours failed/i.test(message.body),
				),
				true,
			);
	...
	it("keeps cross-rebase skipped as audit-only when worker state is not eligible", async () => {
		...
		try {
			worker1Path = await addWorktree(
				repo,
				"wk1-cross-skipped",
				"omx-cross-skipped-w1-",
			);
			worker2Path = await addWorktree(
				repo,
				"wk2-cross-skipped",
				"omx-cross-skipped-w2-",
			);
			...
			const events = await readTeamEvents("cross-rebase-skipped", repo, {
				wakeableOnly: false,
			});
			const wakeable = await readTeamEvents("cross-rebase-skipped", repo, {
				wakeableOnly: true,
			});
			...
			assert.equal(
				events.some((event) => event.type === "worker_cross_rebase_skipped"),
				true,
			);
			assert.equal(
				wakeable.some((event) => event.type === "worker_cross_rebase_skipped"),
				false,
			);
		} finally {
			if (worker1Path) await rm(worker1Path, { recursive: true, force: true });
