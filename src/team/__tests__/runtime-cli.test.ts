...
describe("runtime-cli helpers", () => {
	it("accepts inline JSON payloads without consuming stdin", async () => {
		...
		assert.equal(
			runtimeCli.resolveRuntimeCliInlineInput([
				"--input-json",
				'{"teamName":"alpha"}',
			]),
			'{"teamName":"alpha"}',
	...
	it("normalizes per-worker providers and validates supported values", async () => {
		...
		assert.deepEqual(runtimeCli.normalizeAgentTypes(["codex", "gemini"], 2), [
			"codex",
			"gemini",
		]);
		assert.deepEqual(runtimeCli.normalizeAgentTypes(["gemini"], 3), ["gemini"]);
		assert.equal(runtimeCli.resolveRuntimeCliProviderMap(undefined, 2), null);
		assert.equal(
			runtimeCli.resolveRuntimeCliProviderMap(["claude"], 2),
			"claude",
		);
		assert.equal(runtimeCli.resolveRuntimeCliAgentType(undefined), "executor");
		assert.equal(
	...
	it("computes dead-worker failure from live pane count, not startup snapshot", async () => {
		...
		const staleSnapshotBehavior = runtimeCli.detectDeadWorkerFailure(
			2,
			3,
			true,
			"team-exec",
		...
		const liveBehavior = runtimeCli.detectDeadWorkerFailure(
			2,
			2,
			true,
			"team-exec",
	...
	it("reads task results from explicit OMX_TEAM_STATE_ROOT during shutdown collection", async () => {
		...
		const explicitStateRoot = await mkdtemp(
			join(tmpdir(), "omx-runtime-cli-env-root-state-"),
		);
	...
	it("does not auto-force failed-task shutdown without explicit issue confirmation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-cli-confirm-issues-"),
		);
		const previousTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		delete process.env.OMX_TEAM_STATE_ROOT;
		try {
			await initTeamState(
				"shutdown-confirm-required",
				"task",
				"executor",
				1,
				cwd,
			...
			await assert.rejects(
				() =>
					runtimeCli.shutdownWithForceFallback(
						"shutdown-confirm-required",
						cwd,
					),
	...
	it("does not auto-shutdown merely because monitorTeam reaches complete", async () => {
		...
		try {
			...
			const runtimeCli = await loadRuntimeCliModule();
			const snapshot = await (await import("../runtime.js")).monitorTeam(
				"runtime-cli-complete",
				cwd,
			);
	...
	it("preserves team state when building terminal output for completed teams", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-cli-preserve-complete-"),
		);
		const previousTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		try {
			delete process.env.OMX_TEAM_STATE_ROOT;
			...
			assert.match(
				result.notice,
				/omx team shutdown runtime-cli-preserve-complete/,
			);
	...
	it("preserves team state when building terminal output for failed teams", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-cli-preserve-failed-"),
		);
		const previousTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		try {
			delete process.env.OMX_TEAM_STATE_ROOT;
			...
			assert.match(
				result.notice,
				/omx team shutdown runtime-cli-preserve-failed/,
			);
		} finally {
			if (typeof previousTeamStateRoot === "string")
				process.env.OMX_TEAM_STATE_ROOT = previousTeamStateRoot;
	...
	it("reports cancelled terminal phases without deleting team state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-cli-preserve-cancelled-"),
		);
		const previousTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		try {
			delete process.env.OMX_TEAM_STATE_ROOT;
			...
			assert.match(
				result.notice,
				/omx team shutdown runtime-cli-preserve-cancelled/,
			);
		} finally {
			if (typeof previousTeamStateRoot === "string")
				process.env.OMX_TEAM_STATE_ROOT = previousTeamStateRoot;
