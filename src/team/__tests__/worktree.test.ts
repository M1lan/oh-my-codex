...
describe("worktree parser", () => {
	...
	it("does not leak branch name into remainingArgs when --worktree <name> is used with trailing args", () => {
		const parsed = parseWorktreeMode([
			"--worktree",
			"feat/issue-203",
			"--yolo",
		]);
...
describe("worktree planning", () => {
	it("plans dedicated autoresearch branch and path naming", async () => {
		...
		try {
			...
			assert.equal(
				planned.branchName,
				"autoresearch/demo-mission/20260314t000000z",
			);
			assert.match(
				planned.worktreePath.replace(/\\/g, "/"),
				/\.omx\/worktrees\/autoresearch-demo-mission-20260314t000000z$/,
			);
		} finally {
...
describe("worktree ensure + rollback", () => {
	...
	it("rejects reusing a dirty worktree", async () => {
		...
		try {
			...
			await writeFile(
				join(created.worktreePath, "DIRTY.txt"),
				"dirty\n",
				"utf-8",
			);
