...
describe("team state", () => {
	...
	it("backfills missing or blank tmux pane owner ids in legacy manifests", async () => {
		...
		try {
			...
			assert.equal(
				loadedManifest?.tmux_pane_owner_id,
				"team:team-pane-owner-backfill",
			);
			assert.equal(
				loadedConfig?.tmux_pane_owner_id,
				"team:team-pane-owner-backfill",
			);
			...
			assert.equal(
				blankNormalized?.tmux_pane_owner_id,
				"team:team-pane-owner-backfill",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
