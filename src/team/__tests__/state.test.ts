...
import {
	withScalingLock as withScalingLeaseLock,
	withTeamLock,
} from "../state/locks.js";
...
describe("team state", () => {
	...
	it("rejects a stale full config save without overwriting a newer canonical generation", async () => {
		...
		try {
			...
			await assert.rejects(
				...
				/team_config_stale_generation:team-generation:0:1/,
			);
			assert.equal(
				(await readTeamConfig("team-generation", cwd))?.task,
	...
	it("recovers an interrupted config-first save to one old generation before exposing state", async () => {
		...
		try {
			...
			setWriteAtomicRenameForTests(async (from, to) => {
				if (to === manifestPath)
					throw new Error("injected_manifest_publish_failure");
				await rename(from, to);
			});
			await assert.rejects(
				() => saveTeamConfig(config, cwd),
				/injected_manifest_publish_failure/,
			);
			assert.equal(
				existsSync(join(teamRoot, ".membership-task-transaction.json")),
				true,
			);
	...
	it("rejects a stale membership transaction without overwriting the accepted generation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-membership-generation-"),
		);
		try {
			await initTeamState(
			...
			const staleConfig = {
				...(JSON.parse(oldConfig) as object),
				worker_count: 0,
				workers: [],
			};
			const staleManifest = {
			...
			await assert.rejects(
				withTeamTaskBarrier("membership-generation", cwd, () =>
					commitTeamMembershipTaskTransaction("membership-generation", cwd, {
						baseGeneration: 0,
						tasks: [],
						config: {
							oldBytes: oldConfig,
							newBytes: JSON.stringify(staleConfig, null, 2),
						},
						manifest: {
							oldBytes: oldManifest,
							newBytes: JSON.stringify(staleManifest, null, 2),
						},
					}),
				),
	...
	it("serializes concurrent scale and shutdown membership generations", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-membership-concurrent-generation-"),
		);
		try {
			await initTeamState(
				"membership-concurrent",
				"generation test",
				"executor",
				1,
				cwd,
			...
			const commit = (task: string) =>
				withTeamTaskBarrier("membership-concurrent", cwd, () =>
					commitTeamMembershipTaskTransaction("membership-concurrent", cwd, {
						...
						config: {
							...
							newBytes: JSON.stringify(
								{ ...(JSON.parse(oldConfig) as object), task },
								null,
								2,
							),
						},
						manifest: {
							oldBytes: oldManifest,
							newBytes: JSON.stringify(
								{ ...(JSON.parse(oldManifest) as object), task },
								null,
								2,
							),
						},
			...
			assert.equal(manifest?.config_generation, 1);
			assert.ok(
				config?.task === "scale update" || config?.task === "shutdown update",
			);
	...
	it("preserves missing or blank tmux pane owner ids as unavailable in legacy manifests", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-pane-owner-unavailable-"),
		);
		try {
			await initTeamState(
				"team-pane-owner-unavailable",
				"t",
				"executor",
				1,
				cwd,
			...
			const loadedManifest = await readTeamManifestV2(
				"team-pane-owner-unavailable",
				cwd,
			);
			const loadedConfig = await readTeamConfig(
				"team-pane-owner-unavailable",
				cwd,
			);
			...
			const blankNormalized = await readTeamManifestV2(
				"team-pane-owner-unavailable",
				cwd,
			);
	...
	it("normalizes legacy manifest policy with dispatch defaults, timeout bounds, and governance split", async () => {
		...
		try {
			...
			assert.deepEqual(loaded?.team_decomposition, {
	...
	it("dispatch bridge queue uses the same request id as the TS store", async () => {
		...
		try {
			...
			await writeFile(
				join(cwd, ".omx", "state", "dispatch.json"),
...
`,
	...
	it("coalesces mailbox wakes only for an exact target proof and releases on acknowledgement", async () => {
		...
		try {
			...
			const input = (
				...
				message_id: string,
				pane_id = "pane-1",
				intent: TeamReminderIntent = "pending-mailbox-review",
			) => ({
			...
			const first = await enqueueDispatchRequest(
				"team-dispatch",
				input("worker-1", "msg-1"),
				cwd,
			);
			const pendingSibling = await enqueueDispatchRequest(
				"team-dispatch",
				input("worker-1", "msg-2"),
				cwd,
			);
			...
			const notifiedSibling = await enqueueDispatchRequest(
				"team-dispatch",
				input("worker-1", "msg-3"),
				cwd,
			);
			assert.equal(notifiedSibling.deduped, true);
			assert.equal(
				notifiedSibling.request.request_id,
				first.request.request_id,
			);

			const differentWorker = await enqueueDispatchRequest(
				"team-dispatch",
				input("worker-2", "msg-4"),
				cwd,
			);
			const differentPane = await enqueueDispatchRequest(
				"team-dispatch",
				input("worker-1", "msg-5", "pane-2"),
				cwd,
			);
			const differentIntent = await enqueueDispatchRequest(
				"team-dispatch",
				input("worker-1", "msg-6", "pane-1", "followup-relaunch"),
				cwd,
			);
			const differentTeam = await enqueueDispatchRequest(
				"team-dispatch-other",
				input("worker-1", "msg-7"),
				cwd,
			);
			...
			const successor = await enqueueDispatchRequest(
				"team-dispatch",
				input("worker-1", "msg-8"),
				cwd,
			);
	...
	it("preserves legacy rollback evidence when authoritative dispatch absence verification is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-dispatch-authority-unavailable-"),
		);
		const previousRuntimeBinary = process.env.OMX_RUNTIME_BINARY;
		const previousRuntimeBridge = process.env.OMX_RUNTIME_BRIDGE;
		const previousSkipSnapshotWrite =
			process.env.OMX_TEST_SKIP_SNAPSHOT_COMPAT_WRITE;
		try {
			...
			const unavailableFixtures: Array<() => Promise<void>> = [
				async () => {
					await rm(dispatchPath, { recursive: true, force: true });
				},
				async () => {
					await rm(dispatchPath, { recursive: true, force: true });
					await writeFile(dispatchPath, "{");
				},
				async () => {
					await rm(dispatchPath, { recursive: true, force: true });
			...
			if (typeof previousSkipSnapshotWrite === "string")
				process.env.OMX_TEST_SKIP_SNAPSHOT_COMPAT_WRITE =
					previousSkipSnapshotWrite;
	...
	it("scopes bridge rollback IDs to the target team legacy file and preserves same-target foreign records", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-dispatch-rollback-scope-"),
		);
		const previousRuntimeBinary = process.env.OMX_RUNTIME_BINARY;
		const previousRuntimeBridge = process.env.OMX_RUNTIME_BRIDGE;
		...
		try {
			...
			const scopedRequest = await enqueueDispatchRequest(
				"team-rollback-a",
				{
					kind: "inbox",
					to_worker: "worker-1",
						...
						records: [
							{
								...
								failed_at: null,
								reason: null,
								metadata: { team_name: "team-rollback-a" },
							},
							{
								request_id: "team-b-worker-1",
								target: "worker-1",
								status: "pending",
								created_at: new Date().toISOString(),
								notified_at: null,
								...
								failed_at: null,
								reason: null,
								metadata: { team_name: "team-rollback-b" },
							},
							{
								request_id: "unscoped-worker-1",
								target: "worker-1",
								status: "pending",
								created_at: new Date().toISOString(),
								notified_at: null,
			...
			const runtimeLog = await readFile(runtimeLogPath, "utf8");
			const removalLine = runtimeLog
				.split("\n")
				.find((line) =>
					line.startsWith('exec {"command":"RemoveDispatchRecords"'),
				);
			assert.ok(removalLine);
			const removalPayload = JSON.parse(
				removalLine.slice(
					removalLine.indexOf("{"),
					removalLine.lastIndexOf(" --state-dir="),
				),
	...
	it("preserves malformed legacy rollback evidence byte-for-byte", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-dispatch-legacy-malformed-"),
		);
		const previousRuntimeBridge = process.env.OMX_RUNTIME_BRIDGE;
		try {
	...
	it("rejects an acknowledged retained dispatch ID even when its scope fields changed", async () => {
		...
		try {
			...
			if (typeof previousAckRetain === "string")
				process.env.OMX_TEST_ACK_RETAIN_DISPATCH = previousAckRetain;
	...
	it("resolves task/mailbox/approval paths under explicit OMX_TEAM_STATE_ROOT from a worker cwd (worker-env contamination regression)", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(join(explicitTeamRoot, "approvals", `task-${task.id}.json`)),
				true,
			);
			assert.equal(
				existsSync(
	...
	it("claimTask revalidates a stale member under its claim lock when expectedVersion is null", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-claim-member-boundary-"),
		);
		try {
			...
			const claim = await claimTaskWithDeps(task.id, "worker-2", null, {
				...
				taskFilePath: (_teamName, taskId, _cwd) =>
					join(
						cwd,
						".omx",
						"state",
	...
	it("claimTask rejects a removed worker for a task created after the scale-down snapshot", async () => {
		...
		try {
			...
			assert.ok(
				scaleDownSnapshot?.workers.some((worker) => worker.name === "worker-2"),
			);
			const postSnapshotTask = await createTask(
				teamName,
				{ subject: "post snapshot", description: "d", status: "pending" },
				cwd,
			);

			const claim = await claimTaskWithDeps(
				postSnapshotTask.id,
				"worker-2",
				null,
				{
	...
	it("transitionTaskStatus rejects broad delegated completion without spawn evidence or skip reason", async () => {
		...
		try {
			...
			assert.equal(
				completedWithSpawnEvidence.task.delegation_compliance?.status,
				"spawned",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("transitionTaskStatus requires evidence when optional delegation carries required parallel probe", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-optional-required-probe-"),
		);
		try {
			await initTeamState(
			...
			const t = await createTask(
				"team-optional-required-probe",
				{
					...
					description:
						"Optional mode was elevated by a required parallel probe from review enforcement",
					status: "pending",
					delegation: {
						mode: "optional",
						required_parallel_probe: true,
			...
			assert.equal(
				completed.task.delegation_compliance?.source,
				"terminal_result",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("transitionTaskStatus rejects coordinated completion without boundary evidence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-coordination-evidence-"),
		);
		try {
			await initTeamState(
			...
			assert.equal(
				completed.task.coordination_compliance?.source,
				"terminal_result",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("transitionTaskStatus accepts documented no-boundary rationale for coordinated tasks", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-team-coordination-no-boundary-"),
		);
		try {
			await initTeamState(
			...
			const t = await createTask(
				"team-coordination-no-boundary",
				{
					...
					description:
						"Original plan had a handoff, but implementation collapsed to one isolated check.",
					status: "pending",
					coordination: {
						mode: "coordinated",
						activation_reasons: ["cross_boundary_or_handoff_language"],
			...
			assert.equal(completed.ok, true);
			assert.equal(
				completed.task.coordination_compliance?.status,
				"no_boundary_handoff",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("uses bridge-authored mailbox records while shadowing legacy mailbox bodies for recovery", async () => {
		...
		try {
			...
			assert.equal(
				...
				true,
				"bridge-success path should shadow-write legacy mailbox JSON for body recovery",
			);
			const after = JSON.parse(await readFile(legacyPath, "utf8")) as {
				messages: Array<{ message_id: string; body: string }>;
			};
			...
			const compat = JSON.parse(await readFile(compatPath, "utf8")) as {
				records: Array<{ message_id: string; body: string }>;
			};
			const compatRecord = compat.records.find(
				(entry) => entry.message_id === message.message_id,
			);
			assert.ok(compatRecord);

			const runtimeLogBefore = await readFile(runtimeLogPath, "utf8");
			const deliveredCallsBefore =
				runtimeLogBefore.split("MarkMailboxDelivered").length - 1;
			const deliveredAgain = await markMessageDelivered(
				"team-mailbox-bridge-authority",
				"worker-2",
				message.message_id,
				cwd,
			...
			assert.equal(
				deliveredAgain,
				true,
				"already-delivered bridge message should be treated as delivered",
			);
			const runtimeLogAfter = await readFile(runtimeLogPath, "utf8");
			const deliveredCallsAfter =
				runtimeLogAfter.split("MarkMailboxDelivered").length - 1;
			assert.equal(
				deliveredCallsAfter,
				deliveredCallsBefore,
				"idempotent delivered calls should not invoke bridge a second time",
			);

			compatRecord!.body = "";
			await writeFile(compatPath, JSON.stringify(compat, null, 2));

			const recovered = await listMailboxMessages(
				"team-mailbox-bridge-authority",
				"worker-2",
				cwd,
			);
			...
			assert.equal(
				recovered[0]?.body,
				"hello",
				"legacy shadow mailbox should backfill blank compat bodies",
			);
		} finally {
			if (typeof previousRuntimeBinary === "string")
				process.env.OMX_RUNTIME_BINARY = previousRuntimeBinary;
	...
	it("createTask does not overwrite existing tasks when manifest/config next_task_id lags disk", async () => {
		...
		try {
			...
			const first = await createTask(
				"team-stale-next-id",
				{ subject: "first", description: "d", status: "pending" },
				cwd,
			);
			...
			const second = await createTask(
				"team-stale-next-id",
				{ subject: "second", description: "d", status: "pending" },
				cwd,
			);
			assert.equal(second.id, "2");

			const firstTask = JSON.parse(
				await readFile(join(teamRoot, "tasks", "task-1.json"), "utf8"),
			) as { subject?: string };
			const secondTask = JSON.parse(
				await readFile(join(teamRoot, "tasks", "task-2.json"), "utf8"),
			) as { subject?: string };
	...
	it("writeAtomic tolerates Windows EPERM opening the parent directory after fsyncing the file", async () => {
		...
		try {
			...
			setWriteAtomicOpenForTests(async (...args) => {
				...
				if (path === cwd) {
					const error = new Error(
						"Windows cannot open directory for sync",
					) as NodeJS.ErrnoException;
					error.code = "EPERM";
					throw error;
				}
	...
	it("sanitizes persisted integration snapshot statuses to the contract", async () => {
		...
		try {
			...
			assert.equal(
				snapshot?.integrationByWorker?.["worker-2"]?.status,
				undefined,
			);
			assert.equal(
				snapshot?.integrationByWorker?.["worker-2"]?.last_integrated_head,
				"def456",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("heartbeats live scaling and membership leases while reclaiming stale crashed locks", async () => {
		...
		const staleMs = 90;
		const deps = {
			teamDir: (name: string, root: string) =>
				join(root, ".omx", "state", "team", name),
			taskClaimLockDir: (name: string, taskId: string, root: string) =>
				join(
					root,
					".omx",
					"state",
			...
			mailboxLockDir: (name: string, worker: string, root: string) =>
				join(
					root,
					".omx",
					"state",
		...
		const locks = [
			{
				...
				acquire: (fn: () => Promise<void>) =>
					withScalingLeaseLock(teamName, cwd, staleMs, deps, fn),
			},
			{
				label: "membership",
				lockDir: join(
				...
				acquire: (fn: () => Promise<void>) =>
					withTeamLock(teamName, cwd, staleMs, deps, fn),
			},
		];
		...
		try {
			for (const lock of locks) {
				...
				const heldEnteredPromise = new Promise<void>((resolve) => {
					heldEntered = resolve;
				});
				...
				const contender = lock.acquire(async () => {
				...
				assert.equal(
					contenderEntered,
					false,
					`live ${lock.label} lock must not be reclaimed after its directory mtime becomes stale`,
				);
				releaseHeld();
				...
				assert.equal(
					reclaimed,
					true,
					`stale ${lock.label} lock must be reclaimed after its lease expires`,
				);
			}
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("serializes cleanup with concurrent scaling and claims without recreating team state", async () => {
		...
		const scaleReleased = new Promise<void>((resolve) => {
			releaseScale = resolve;
		});
		let scaleEntered!: () => void;
		const scaleEnteredPromise = new Promise<void>((resolve) => {
			scaleEntered = resolve;
		});
		...
		try {
			...
			const task = await createTask(
				teamName,
				{ subject: "racy", description: "d", status: "pending" },
				cwd,
			);
			...
			assert.equal(
				claim.ok,
				true,
				"claim must finish before the queued cleanup deletes canonical state",
			);
			...
			const afterCleanupClaim = await claimTask(
				teamName,
				task.id,
				"worker-1",
				null,
			...
			assert.deepEqual(afterCleanupClaim, {
				ok: false,
			...
			assert.equal(
				existsSync(teamRoot),
				false,
				"post-cleanup operations must not recreate a fake team root",
			);
		} finally {
			releaseScale?.();
			await rm(cwd, { recursive: true, force: true });
...
describe("membership transaction recovery path authority", () => {
	it("rejects foreign absolute paths before mutating any victim or canonical file", async () => {
		...
		try {
			...
			await writeFile(
				join(teamDir, ".membership-task-transaction.json"),
				JSON.stringify({
					schemaVersion: 1,
					phase: "committed",
					files: [
			...
			await assert.rejects(
				recoverTeamMembershipTaskTransaction(teamName, cwd),
				/escapes expected team files/,
			);
	...
	it("rejects a symlinked tasks directory before writing through it", async () => {
		...
		try {
			...
			await assert.rejects(
				recoverTeamMembershipTaskTransaction(teamName, cwd),
				/tasks root escapes canonical team root/,
			);
			assert.equal(
				await readFile(join(outsideTasksDir, "task-1.json"), "utf8"),
				"ORIGINAL",
			);
			assert.equal(await readFile(configPath, "utf8"), configBytes);
		} finally {
