...
export function readCurrentTaskBaseline(
	repoRoot: string,
): CurrentTaskBaselineFile {
...
function writeCurrentTaskBaseline(
	repoRoot: string,
	data: CurrentTaskBaselineFile,
): void {
...
export function listActiveCurrentTasks(
	repoRoot: string,
): CurrentTaskBaselineEntry[] {
	return readCurrentTaskBaseline(repoRoot).tasks.filter(
		(entry) => entry.status === "active",
	);
}

export function findActiveCurrentTaskByBranch(
	repoRoot: string,
	branchName: string,
): CurrentTaskBaselineEntry | null {
	const normalized = branchName.trim();
	if (!normalized) return null;
	return (
		listActiveCurrentTasks(repoRoot).find(
			(entry) => entry.branch_name === normalized,
		) ?? null
	);
}
...
export function upsertCurrentTaskBaseline(
	...
	const existing =
		current.tasks.find((entry) => entry.branch_name === branchName) ?? null;
	...
	const tasks = current.tasks.filter(
		(entry) => entry.branch_name !== branchName,
	);
	tasks.push(next);
