			...
			return await withTaskMembershipBarrier(sanitized, leaderCwd, async () => {
				...
				const rollbackScaleUp = async (
					...
					try {
							...
							[...authorizedRollbackPaneIds],
							{
								...
								authorizePaneKill: (paneId) => {
									...
									const expectedOwnerId =
										rollbackTaggedPaneOwnerIds.get(paneId);
						...
						for (const paneId of paneTeardown.kill.failedPaneIds)
							unresolvedPaneIds.add(paneId);
						for (const proof of paneTeardown.proofUnavailable)
							unresolvedPaneIds.add(proof.paneId);
						...
						if (
							...
							const unresolvedWithoutDirectFailure = [
								...unresolvedPaneIds,
							].filter(
								(paneId) =>
									!paneTeardown.kill.failedPaneIds.includes(paneId) &&
									!paneTeardown.proofUnavailable.some(
										(proof) => proof.paneId === paneId,
									),
							);
							if (unresolvedWithoutDirectFailure.length > 0) {
								cleanupDebt.push(
									`pane_teardown_unresolved:${unresolvedWithoutDirectFailure.join(",")}`,
								);
							}
						}
					} catch (cleanupError) {
				...
				for (const workerLaunchPlan of workerLaunchPlans) {
					...
					const currentSplitTargetOwner = readPaneTeamOwnerTagResult(
						splitTargetProof.paneId,
					);
					if (
						currentSplitTargetOwner.status !== "value" ||
						currentSplitTargetOwner.value !== expectedSplitTargetOwnerId
					) {
						return await rollbackScaleUp(
					...
					const finalSplitTargetProof = readExactPaneProofSync(
						splitTargetProof.paneId,
					);
					...
					if (paneProof.status !== "live") {
						return await rollbackScaleUp(
							`Failed to prove tmux pane for ${workerName}`,
					...
					if (config.tmux_pane_owner_id) {
						try {
							...
							rollbackTaggedPaneOwnerIds.set(
								paneProof.paneId,
								config.tmux_pane_owner_id.trim(),
							);
						} catch (error) {
