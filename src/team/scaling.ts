...
async function withTaskClaimLocks<T>(
	...
	const teamDir = (name: string, lockCwd: string) =>
		join(resolveCanonicalTeamStateRoot(lockCwd), "team", name);
	const locks = {
		teamDir,
...
const SCALE_PANE_INSTANCE_FORMAT =
	"#{pane_id}\t#{pane_pid}\t#{session_name}\t#{session_id}\t#{window_id}\t#{@omx_team_pane_owner_id}";
const SCALE_SPLIT_RECEIPT_FORMAT =
	"#{pane_id}\t#{pane_pid}\t#{session_name}\t#{session_id}\t#{window_id}";
...
function runScalePaneTransaction(
	...
	const result = spawnSync(
		...
		[
			...
			"-t",
			pane.paneId,
			buildScalePaneAuthorityCondition(pane),
	...
	return (
		result.status === 0 && hasExactScaleReceipt(result.stdout || "", receipt)
	);
}

function tagScalePaneWithAuthority(
	pane: ScalePaneBirth,
	teamOwnerId: string,
): ScalePaneInstance | null {
	const comparisons = [
		["pane_id", pane.paneId],
	...
	const condition = comparisons.reduce(
	...
	if (
		result.status !== 0 ||
		!hasExactScaleReceipt(result.stdout || "", receipt)
	)
		return null;
...
function parseScalePaneInstance(value: string): ScalePaneInstance | null {
	...
	const [paneId, pidText, sessionName, sessionId, windowId, teamOwnerId] =
		fields;
...
function readScalePaneInstance(paneId: string): ScalePaneInstance | null {
	const result = spawnSync(
		"tmux",
		["display-message", "-p", "-t", paneId, SCALE_PANE_INSTANCE_FORMAT],
		{ encoding: "utf-8" },
	);
	return result.status === 0
		? parseScalePaneInstance(result.stdout || "")
		: null;
}
...
function splitWorkerPaneWithScaleAuthority(
	...
	const splitCommand = [
		...
		"-P",
		"-F",
		`${SCALE_SPLIT_RECEIPT_FORMAT}\t${receipt}`,
		"-c",
		workerCwd,
		command,
	]
		.map(quoteTmuxCommand)
		.join(" ");
	const result = spawnSync(
		"tmux",
		[
			...
			"-t",
			source.paneId,
			buildScalePaneAuthorityCondition(source),
			splitCommand,
			"display-message -p ''",
		],
		{ encoding: "utf-8" },
...
function buildScaleUpWorkerLaunchPlans(params: {
	...
	for (let offset = 0; offset < params.count; offset += 1) {
		...
		const preferredReasoning =
			resolveAgentReasoningEffort(runtimeRole, params.codexHomeOverride) ??
...
function hasScaleUpFailureInjection(
	env: NodeJS.ProcessEnv,
	phase: string,
): boolean {
	return (
		env.OMX_TEAM_SCALE_UP_INJECT_FAILURE?.split(",")
			.map((value) => value.trim())
			.includes(phase) ?? false
	);
}

function throwIfScaleUpFailureInjected(
	env: NodeJS.ProcessEnv,
	phase: string,
): void {
...
function recoverCreatedWorktreeAfterEnsureFailure(
	...
	if (worktreePathExistedBeforeEnsure || !existsSync(plan.worktreePath))
		return null;
	...
	if (plan.branchName) {
		...
		if (
			branch.status !== 0 ||
			(branch.stdout || "").trim() !== `refs/heads/${plan.branchName}`
		)
			return null;
	} else {
...
function assertUnreachableApprovedExecutionState(state: never): never {
	throw new Error(
		`unreachable_scale_up_approved_execution_state:${JSON.stringify(state)}`,
	);
}
...
function resolveScaleUpApprovedExecutionGate(
	...
	switch (approvedExecutionState.status) {
		...
		case "malformed":
			return {
				ok: false,
				error: `approved_execution_binding_malformed:${teamName}`,
			};
		case "ambiguous":
			return {
				ok: false,
...
function resolveLegacyScaledTeamWorktreeMode(
	config: Pick<
		TeamConfig,
		"name" | "workspace_mode" | "worktree_mode" | "workers"
	>,
): WorktreeMode {
...
function resolveScaleUpWorktreeMode(config: TeamConfig): WorktreeMode {
	...
	try {
		...
		if (
			...
			error.message === `scale_up_missing_team_worktree_contract:${config.name}`
		) {
			return { enabled: true, detached: true, name: null };
		}
		throw error;
...
async function notifyWorkerPaneOutcome(
	...
	try {
		if (
			...
			throw new Error(
				`scale_up_final_send_authority_missing:${paneId ?? "unknown"}`,
			);
		}
		...
		async (): Promise<ScaleUpResult | ScaleError> => {
			return await withTaskMembershipBarrier(sanitized, leaderCwd, async () => {
				...
				const configPath = join(
					resolveCanonicalTeamStateRoot(leaderCwd),
					"team",
					sanitized,
					"config.json",
				);
				...
				const originalManifestBytes = existsSync(manifestPath)
				...
				const dispatchPolicy = normalizeTeamPolicy(manifest?.policy, {
					display_mode:
						manifest?.policy?.display_mode === "split_pane"
							? "split_pane"
							: "auto",
				...
				const persistedUltragoalContext =
					await readPersistedTeamUltragoalContext(
						sanitized,
						config.leader_cwd ?? leaderCwd,
						config.team_state_root ?? teamStateRoot,
					);
				...
				const initialSplitTargetCandidate =
					config.workers[config.workers.length - 1];
				const initialSplitTargetWorker =
					initialSplitTargetCandidate?.pane_id?.trim()
						? initialSplitTargetCandidate
						: undefined;
				const initialSplitTarget =
					initialSplitTargetWorker?.pane_id ?? config.leader_pane_id ?? "";
				const initialSplitTargetOwnerId =
				...
				if (
					...
					const migrationOwner = readPaneTeamOwnerTagResult(
						migrationProof.paneId,
					);
					if (
						migrationOwner.status !== "value" ||
						migrationOwner.value !== initialSplitTargetOwnerId
					) {
						return {
							ok: false,
					...
					const finalMigrationProof = readExactPaneProofSync(
						migrationProof.paneId,
					);
					...
					Object.assign(
						originalConfig,
						JSON.parse(JSON.stringify(config)) as TeamConfig,
					);
					initialExpectedSplitTargetPid = finalMigrationProof.pid;
				}
				const initialSplitTargetProof =
					readExactPaneProofSync(initialSplitTarget);
				...
				const effectiveWorktreeMode =
					config.worktree_mode ?? resolveScaleUpWorktreeMode(config);
				...
				const rollbackScaleUp = async (
					...
					const rollbackWorkerNames = new Set(
					...
					try {
						await withTaskMembershipBarrier(sanitized, leaderCwd, async () => {
							...
							const currentManifestBytes = existsSync(manifestPath)
								? await readFile(manifestPath, "utf8")
								: null;
							...
							const trackedByName = new Map(
								currentConfig.workers.map((worker) => [worker.name, worker]),
							);
							for (const worker of rollbackWorkers)
								trackedByName.set(worker.name, worker);
							const trackedConfig: TeamConfig = {
								...currentConfig,
								workers: [...trackedByName.values()],
								worker_count: trackedByName.size,
								next_worker_index: Math.max(
									currentConfig.next_worker_index ??
										currentConfig.workers.length + 1,
									...rollbackWorkers.map((worker) => worker.index + 1),
								),
							};
							const trackedManifestBytes =
								currentManifestBytes === null
									? null
									: JSON.stringify(
											{
												...(JSON.parse(currentManifestBytes) as Record<
													string,
													unknown
												>),
							...
							await commitTeamMembershipTaskTransaction(sanitized, leaderCwd, {
								...
								tasks: [],
								config: {
									oldBytes: currentConfigBytes,
									newBytes: JSON.stringify(trackedConfig, null, 2),
								},
								manifest: {
									oldBytes: currentManifestBytes,
									newBytes: trackedManifestBytes,
								},
							...
							if (
								!verified ||
								rollbackWorkers.some(
									(worker) =>
										!verified.workers.some(
											(entry) =>
												entry.name === worker.name &&
												entry.pane_id === worker.pane_id,
										),
								)
							) {
								throw new Error(
						...
						return {
							ok: false,
							error: `scale_up_rollback_membership_persistence_failed:${String(trackingError)}`,
						};
					}

					const cleanupDebt: string[] = [];
					try {
						await removeDispatchRequestsForWorkers(
							sanitized,
							[...rollbackWorkerNames],
							leaderCwd,
						);
						...
						cleanupDebt.push(
							`authoritative_dispatch_cleanup_failed:${String(rollbackError)}`,
						);
					}
					...
					for (const paneId of rollbackPaneIds) {
						const matchingWorkers = rollbackWorkers.filter(
							(worker) => worker.pane_id === paneId,
						);
						const worker =
							matchingWorkers.length === 1 ? matchingWorkers[0] : undefined;
						const expectedPanePid = worker?.pid;
						if (
							!worker ||
							typeof expectedPanePid !== "number" ||
							!Number.isInteger(expectedPanePid) ||
							expectedPanePid <= 0
						) {
						...
						if (
							requiredTeamOwnerId &&
							rollbackTaggedPaneOwnerIds.get(paneId) !== requiredTeamOwnerId
						) {
							unresolvedPaneIds.add(paneId);
						...
						const expectedWindowId = (
							worker as WorkerInfo & { window_id?: unknown }
						).window_id;
						const expectedSessionId = (
							worker as WorkerInfo & { session_id?: unknown }
						).session_id;
						if (
							typeof expectedWindowId !== "string" ||
							!/^@[0-9]+$/.test(expectedWindowId) ||
						...
						if (
							...
							(requiredTeamOwnerId &&
								rollbackInstance.teamOwnerId !== requiredTeamOwnerId)
						) {
							unresolvedPaneIds.add(paneId);
					...
					const unresolvedWorkerNames = new Set(
						rollbackWorkers
							.filter(
								(worker) =>
									typeof worker.pane_id === "string" &&
									unresolvedPaneIds.has(worker.pane_id),
							)
					...
					if (
						context.workerName &&
						context.paneId &&
						unresolvedPaneIds.has(context.paneId)
					) {
					...
					try {
						await withTaskMembershipBarrier(sanitized, leaderCwd, async () => {
							...
							const currentManifestBytes = existsSync(manifestPath)
								? await readFile(manifestPath, "utf8")
								: null;
							const retainedWorkers = rollbackWorkers.filter((worker) =>
								unresolvedWorkerNames.has(worker.name),
							);
							const desiredByName = new Map(
								originalConfig.workers.map((worker) => [worker.name, worker]),
							);
							for (const worker of retainedWorkers)
								desiredByName.set(worker.name, worker);
							const desiredConfig: TeamConfig = {
								...originalConfig,
								workers: [...desiredByName.values()],
								worker_count: desiredByName.size,
								next_worker_index: Math.max(
									originalConfig.next_worker_index ??
										originalConfig.workers.length + 1,
									...retainedWorkers.map((worker) => worker.index + 1),
								),
							};
							const desiredManifestBytes =
								originalManifestBytes === null
									? null
									: JSON.stringify(
											{
												...(JSON.parse(originalManifestBytes) as Record<
													string,
													unknown
												>),
							...
							for (const taskId of createdTaskIds) {
								...
								taskChanges.push({
									taskId,
									oldBytes: existsSync(taskPath)
										? await readFile(taskPath, "utf8")
										: null,
									newBytes:
										task && unresolvedWorkerNames.has(task.owner ?? "")
							...
							await commitTeamMembershipTaskTransaction(sanitized, leaderCwd, {
								...
								config: {
									oldBytes: currentConfigBytes,
									newBytes: JSON.stringify(desiredConfig, null, 2),
								},
								manifest: {
									oldBytes: currentManifestBytes,
									newBytes: desiredManifestBytes,
								},
								recoverToNewOnFailure: true,
								retainJournalOnSuccess: true,
								failRollbackPersistence: hasScaleUpFailureInjection(
									env,
									"rollback-membership-persistence",
								),
								failRollbackPersistenceAfter: hasScaleUpFailureInjection(
									env,
									"rollback-membership-config-persistence",
								)
									? "config"
									: hasScaleUpFailureInjection(
												env,
							...
							if (
								verified.workers.some(
									(worker) =>
										rollbackWorkerNames.has(worker.name) &&
										!unresolvedWorkerNames.has(worker.name),
								)
							) {
								throw new Error(
						...
						const journalPath = join(
							...
							"team",
							sanitized,
							".membership-task-transaction.json",
						);
						const suffix = existsSync(journalPath)
							? String(rollbackError)
							: `no_recoverable_journal:${String(rollbackError)}`;
						return {
							ok: false,
							error: `scale_up_rollback_membership_persistence_failed:${suffix}`,
						};
					}
					const cleanupWorkerNames = new Set([
						...rollbackWorkers
							.filter(
								(worker) =>
									typeof worker.pane_id !== "string" ||
									resolvedPaneIds.has(worker.pane_id),
							)
							.map((worker) => worker.name),
						...(context.workerName &&
						!unresolvedWorkerNames.has(context.workerName)
							? [context.workerName]
							: []),
					]);
					try {
						...
						await Promise.all(
							[...cleanupWorkerNames].map(async (workerName) => {
								await rm(
									join(teamStateRoot, "team", sanitized, "workers", workerName),
						...
						cleanupDebt.push(
							`canonical_cleanup_failed:${String(rollbackError)}`,
						);
					}

					try {
						const contextWorkerName =
							context.worker?.name ?? context.workerName;
						const contextWorktreePath =
							context.worker?.worktree_path ?? context.worktreePath;
						if (
							contextWorkerName &&
							contextWorktreePath &&
							!unresolvedWorkerNames.has(contextWorkerName)
						) {
						...
						const unresolvedWorktreePaths = new Set(
							rollbackWorkers
								.filter(
									(worker) =>
										unresolvedWorkerNames.has(worker.name) &&
						...
						await rollbackProvisionedWorktrees(
							provisionedWorktrees.filter(
								(worktree) =>
									!unresolvedWorktreePaths.has(resolve(worktree.worktreePath)),
							),
						);
						await Promise.all(
							[...preparedWorkerDirectoryOwner.entries()]
								.filter(
									([, workerName]) => !unresolvedWorkerNames.has(workerName),
								)
						...
						await Promise.all(
							[...preparedStartupScriptOwner.entries()]
								.filter(
									([, workerName]) => !unresolvedWorkerNames.has(workerName),
								)
						...
						if (!runtimeDirectoryExisted && unresolvedWorkerNames.size === 0)
					...
					if (cleanupDebt.length > 0) {
						...
						try {
							await appendTeamEvent(
								sanitized,
								{ type: "team_leader_nudge", worker: "leader-fixed", reason },
								leaderCwd,
							);
						} catch (eventError) {
							cleanupDebt.push(
								`cleanup_debt_event_failed:${String(eventError)}`,
							);
						}
						return {
							ok: false,
							error: `scale_up_rollback_cleanup_debt:${cleanupDebt.join(";")}`,
						};
					}
				...
				for (const workerLaunchPlan of workerLaunchPlans) {
					...
					const splitTargetCandidate =
						config.workers[config.workers.length - 1];
					const splitTargetWorker = splitTargetCandidate?.pane_id?.trim()
						? splitTargetCandidate
						: undefined;
					const splitTarget =
						splitTargetWorker?.pane_id ?? config.leader_pane_id ?? "";
					const expectedSplitTargetPid = splitTargetWorker
						? splitTargetWorker.pid
						: config.leader_pane_pid;
					const expectedSplitTargetOwnerId =
						typeof config.tmux_pane_owner_id === "string"
							? config.tmux_pane_owner_id.trim()
							: "";
					if (
						...
						return await rollbackScaleUp(
							`scale_up_split_target_pid_missing:${splitTarget}`,
						);
					}
					if (!expectedSplitTargetOwnerId) {
						return await rollbackScaleUp(
							`scale_up_split_target_owner_unavailable:${splitTarget}`,
						);
					}
					...
					if (workerLaunchPlan.mixedTaskRoles.length > 1) {
						console.log(
							`[omx:scaling] ${workerName}: mixed task roles [${workerLaunchPlan.mixedTaskRoles.join(", ")}], falling back to ${agentType}`,
						);
					}
					...
					try {
						...
						if (effectiveWorktreeMode.enabled) {
							...
							if (!worktreePlan.enabled)
								throw new Error(`worktree_not_planned:${workerName}`);
							const worktreePathExistedBeforeEnsure = existsSync(
							...
							const branchExistedBeforeEnsure = worktreePlan.branchName
								? spawnSync(
										...
										[
											...
											"--verify",
											"--quiet",
											`refs/heads/${worktreePlan.branchName}`,
										],
										{
							...
							try {
								...
								throwIfScaleUpFailureInjected(
									env,
									"worktree-ensure-post-create",
								);
								if (!ensuredWorkspace.enabled)
									throw new Error(`worktree_not_provisioned:${workerName}`);
								workerWorkspace = ensuredWorkspace;
							} catch (error) {
								const recoveredWorkspace =
									recoverCreatedWorktreeAfterEnsureFailure(
										worktreePlan,
						...
						const rolePromptContent = rawRolePromptContent
							...
							: null;
						const teamInstructionsPath = join(
							leaderCwd,
							".omx",
							"state",
							"team",
						...
						if (workerWorkspace) {
							...
							extraEnv.OMX_TEAM_WORKTREE_DETACHED = workerWorkspace.detached
								? "1"
								: "0";
						}
					...
					const splitTargetInstance =
						splitTargetProof.status === "live"
					...
					if (
						splitTargetInstance.sessionName !== sessionName ||
						(config.tmux_session_id &&
							splitTargetInstance.sessionId !== config.tmux_session_id) ||
					...
					const workerInfo: ScaleUpWorkerInfo = {
						...
						worktree_repo_root: workerWorkspace
							? workerWorkspace.repoRoot
							: undefined,
						worktree_path: workerWorkspace
							? workerWorkspace.worktreePath
							: undefined,
						worktree_branch: workerWorkspace
							? (workerWorkspace.branchName ?? undefined)
							: undefined,
						worktree_detached: workerWorkspace
							? workerWorkspace.detached
							: undefined,
						worktree_created: workerWorkspace
					...
					if (!taggedPane) {
						return await rollbackScaleUp(
							`scale_up_created_pane_owner_tag_authority_lost:${paneId}`,
					...
					try {
						...
						if (!skipReadyWait) {
							...
							if (!ready) {
								console.log(
									`[omx:scaling] Warning: worker ${workerName} did not become ready within timeout`,
								);
							}
						}

						// Get assigned tasks for this worker
						const workerTasks = materializedTasks.filter(
							(t) => t.owner === workerName,
						);
						...
						const queued = await queueInboxInstruction({
							...
							notify: async (_target, message) => {
								...
								return await notifyWorkerPaneOutcome(
									sessionName,
									message,
									paneId,
									workerInfo.pid,
									config.tmux_pane_owner_id ?? undefined,
									workerInfo.session_id,
									workerInfo.window_id,
								);
							},
						});
						let outcome = queued;
						if (
							dispatchPolicy.dispatch_mode === "hook_preferred_with_fallback" &&
							queued.request_id
						) {
						...
						if (!outcome.ok) {
							return await rollbackScaleUp(
								`scale_up_dispatch_failed:${workerName}:${outcome.reason}`,
								{
									paneId,
									worker: workerInfo,
								},
...
async function assertExistingPathParentContained(
	path: string,
	root: string,
): Promise<void> {
	...
	while (!existsSync(candidate)) {
		...
		if (parent === candidate)
			throw new Error(`scale_down_cleanup_debt_path_missing_parent:${path}`);
		candidate = parent;
	}
...
async function validateScaleDownCleanupResources(
	...
	for (const worker of workers) {
		...
		await assertExistingPathParentContained(
			expectedWorkerDirectory,
			canonicalTeamStateRoot,
		);
		if (
			worker.team_state_root !== undefined &&
			(!isAbsolute(worker.team_state_root) ||
				worker.team_state_root !== resolve(worker.team_state_root) ||
				resolve(worker.team_state_root) !== canonicalTeamStateRoot)
		) {
			throw new Error(
				`scale_down_cleanup_debt_invalid_team_state_root:${worker.name}`,
			);
		}
		if (worker.worktree_path === undefined) {
			if (
				worker.worktree_repo_root !== undefined ||
				worker.worktree_branch !== undefined ||
				worker.worktree_detached !== undefined ||
				worker.worktree_created !== undefined
			) {
				throw new Error(
					`scale_down_cleanup_debt_invalid_worktree_metadata:${worker.name}`,
				);
			}
			continue;
		}
		...
		if (
			!isAbsolute(worker.worktree_path) ||
			worker.worktree_path !== resolve(worker.worktree_path) ||
		...
		if (
			...
			(await realpath(expectedWorktreePath)) !== expectedWorktreePath
		) {
			throw new Error(
...
async function cleanupScaleDownResources(
	...
	const worktrees: EnsureWorktreeResult[] = workers
		.filter(
			(
				worker,
			): worker is ScaleDownCleanupDebtResource & {
				worktree_path: string;
				worktree_repo_root: string;
				worktree_detached: boolean;
	...
	await Promise.all(
		workers.map(async (worker) => {
			await rm(join(teamStateRoot, "team", teamName, "workers", worker.name), {
				recursive: true,
				force: true,
			});
		}),
...
function asScaleDownDebtResources(
	...
	if (
		...
		resources.length !== removedWorkerNames.length
	)
		return null;
	...
	for (const worker of resources) {
		if (
			...
			!expected.has(worker.name) ||
			seen.has(worker.name) ||
			(worker.worktree_path !== undefined &&
				typeof worker.worktree_path !== "string") ||
			(worker.worktree_repo_root !== undefined &&
				typeof worker.worktree_repo_root !== "string") ||
			(worker.worktree_branch !== undefined &&
				typeof worker.worktree_branch !== "string") ||
			(worker.worktree_detached !== undefined &&
				typeof worker.worktree_detached !== "boolean") ||
			(worker.worktree_created !== undefined &&
				typeof worker.worktree_created !== "boolean") ||
			(worker.team_state_root !== undefined &&
				typeof worker.team_state_root !== "string")
		)
			return null;
...
function asScaleDownDebtPanes(
	...
	const normalizedLeaderPaneId =
		typeof leaderPaneId === "string" ? leaderPaneId.trim() : "";
	const normalizedHudPaneId =
	...
	const validate = (
		...
		if (
			...
			!expectedNames.has(pane.name) ||
			!resourceNames.has(pane.name) ||
			typeof pane.index !== "number" ||
			!Number.isInteger(pane.index) ||
			pane.index <= 0 ||
			pane.index !== Number(pane.name.slice("worker-".length)) ||
			typeof pane.pane_id !== "string" ||
			!/^%\d+$/.test(pane.pane_id) ||
			pane.pane_id === normalizedLeaderPaneId ||
			pane.pane_id === normalizedHudPaneId ||
			(pane.pid !== null && (!Number.isSafeInteger(pane.pid) || pane.pid <= 0))
		)
			return false;
	...
	for (const pane of unresolved ?? []) {
		if (
			unresolvedNames.has(pane.name ?? "") ||
			unresolvedPaneIds.has(pane.pane_id) ||
			!validate(pane, true)
		)
			return null;
...
export async function reconcileScaleDownCleanupDebt(
	...
	return await withTaskMembershipBarrier(sanitized, leaderCwd, async () => {
		...
		try {
			debt = JSON.parse(
				await readFile(cleanupDebtPath, "utf8"),
			) as ScaleDownCleanupDebt;
		} catch (error) {
			return {
				ok: false,
				error: `scale_down_cleanup_debt_unreadable:${String(error)}`,
			};
		}
		...
		const removedWorkerNames = Array.isArray(debt.removed_worker_names)
			...
			: Array.isArray(debt.workers)
				? debt.workers
						.map((worker) => worker?.name)
						.filter((name): name is string => typeof name === "string")
				: [];
		if (
			removedWorkerNames.length === 0 ||
		...
		if (!resources)
			return { ok: false, error: "scale_down_cleanup_debt_malformed" };
		try {
			await validateScaleDownCleanupResources(
				sanitized,
				leaderCwd,
				teamStateRoot,
				resources,
		...
		if (!panes)
			return { ok: false, error: "scale_down_cleanup_debt_malformed" };
		const stillCanonical = removedWorkerNames.filter((name) =>
			config.workers.some((worker) => worker.name === name),
		);
		...
		if (stillCanonical.length > 0)
			return {
				ok: false,
				error: "scale_down_cleanup_debt_membership_inconsistent",
			};
		...
		const survivingWorkers = config.workers.filter(
			(worker) => !removedWorkerNames.includes(worker.name),
		);
		if (
			panes.some((pane) =>
				survivingWorkers.some(
					(worker) =>
		...
		for (const pane of panes) {
			...
			if (
				!(await hasMatchingScaleDownDebtWorkerIdentity(
					teamStateRoot,
					sanitized,
					pane,
				))
			...
			if (
				teardown.provenGonePaneIds.includes(pane.pane_id) ||
				teardown.killedPaneIds.includes(pane.pane_id)
			) {
				resolvedPaneIds.add(pane.pane_id);
				continue;
			}
			reasons.push(
				...teardown.proofUnavailable.map(
					(proof) => `${proof.paneId}:${proof.reason}`,
				),
			);
			reasons.push(
			...
			if (
				teardown.proofUnavailable.length === 0 &&
				teardown.kill.failedPaneIds.length === 0
			) {
				reasons.push(`${pane.pane_id}:teardown_unresolved`);
			}
		}
		const unresolvedPanes = panes.filter(
			(pane) => !resolvedPaneIds.has(pane.pane_id),
		);
		if (unresolvedPanes.length > 0) {
			...
			return {
				ok: false,
				error: `scale_down_cleanup_debt_unresolved:${unresolvedPanes.map((pane) => pane.pane_id).join(",")}`,
			};
		}
		try {
			...
			return {
				ok: false,
				error: `scale_down_cleanup_debt_resource_cleanup_failed:${String(error)}`,
			};
		}
		await removeDurableFile(cleanupDebtPath);
		return { ok: true };
...
export async function scaleDown(
	...
	return await withScalingLock(
		...
		async (): Promise<ScaleDownResult | ScaleError> => {
			const config = await withTaskMembershipBarrier(
				sanitized,
				leaderCwd,
				async () => {
					await recoverTeamMembershipTaskTransaction(sanitized, leaderCwd);
			...
			let teamStateRoot =
				config.team_state_root ?? resolveCanonicalTeamStateRoot(leaderCwd);
			const priorCleanup = await reconcileScaleDownCleanupDebt(
				sanitized,
				leaderCwd,
				config,
			);
			...
			const priorWorkerStatusArtifacts = new Map<
				string,
				{ exists: true; raw: Buffer } | { exists: false }
			>();
			for (const worker of targetWorkers) {
				...
				try {
					priorWorkerStatusArtifacts.set(worker.name, {
						exists: true,
			...
			const restorePriorWorkerStatuses = async (
			...
			let removableWorkerNames = new Set(
				removableWorkers.map((worker) => worker.name),
			);
			let teardownFailure: ScaleError | null = null;
			try {
				await withTaskMembershipBarrier(sanitized, leaderCwd, async () => {
					await recoverTeamMembershipTaskTransaction(sanitized, leaderCwd);
					const authoritativeConfig = await readTeamConfig(
						sanitized,
						leaderCwd,
					);
					if (!authoritativeConfig)
						throw new Error("canonical_scale_down_config_missing");
					const authoritativeWorkers = removableWorkers.map((worker) =>
						authoritativeConfig.workers.find(
							(candidate) => candidate.name === worker.name,
						),
					);
					if (
						authoritativeWorkers.some((worker): worker is undefined => !worker)
					) {
					...
					removableWorkerNames = new Set(
						removableWorkers.map((worker) => worker.name),
					);
					teamStateRoot =
						authoritativeConfig.team_state_root ??
						resolveCanonicalTeamStateRoot(leaderCwd);
					...
					await withTaskClaimLocks(
						...
						async () => {
							...
							const manifestSnapshot = existsSync(manifestPath)
								? await readFile(manifestPath)
								: null;
							const reconciledTasks = lockedTasks.filter(
								(task) =>
									task.status !== "completed" &&
									task.status !== "failed" &&
									(removableWorkerNames.has(task.owner ?? "") ||
										removableWorkerNames.has(task.claim?.owner ?? "")),
							);
							...
							const postSnapshotMarker =
								env.OMX_TEAM_SCALE_DOWN_POST_SNAPSHOT_MARKER;
							if (postSnapshotMarker)
								await writeFile(postSnapshotMarker, "snapshot-held\n");
							const boundaryHoldMs = Number.parseInt(
								env.OMX_TEAM_SCALE_DOWN_BOUNDARY_HOLD_MS ?? "",
								10,
							);
							if (Number.isFinite(boundaryHoldMs) && boundaryHoldMs > 0)
								await new Promise((resolve) =>
									setTimeout(resolve, boundaryHoldMs),
								);

							const currentConfig = JSON.parse(
								configSnapshot.toString("utf8"),
							) as TeamConfig;
							const nextConfig: TeamConfig = {
								...currentConfig,
								workers: currentConfig.workers.filter(
									(worker) => !removableWorkerNames.has(worker.name),
								),
							};
							...
							const nextManifest = manifestSnapshot
								? JSON.stringify(
										{
											...(JSON.parse(
												manifestSnapshot.toString("utf8"),
							...
							const membershipTransaction = {
								...
								tasks: reconciledTasks.map((task) => ({
									taskId: task.id,
									oldBytes:
										taskSnapshots.get(task.id)?.toString("utf8") ?? null,
								...
								interruptAfterFirstTaskWrite:
									env.OMX_TEAM_SCALE_DOWN_INJECT_FAILURE ===
									"after-first-task-write",
								failRollbackPersistence:
									env.OMX_TEAM_SCALE_DOWN_INJECT_FAILURE ===
							...
							const cleanupDebtBase = {
								...
								removed_worker_names: removableWorkers.map(
									(worker) => worker.name,
								),
								workers: removableWorkers
									.filter(
										(worker): worker is WorkerInfo & { pane_id: string } =>
											typeof worker.pane_id === "string" &&
							...
							await writeAtomic(
								cleanupDebtPath,
								JSON.stringify(cleanupDebtBase, null, 2),
							);
							await commitTeamMembershipTaskTransaction(
								sanitized,
								leaderCwd,
							...
							if (
								!committed ||
								committed.workers.some((worker) =>
									removableWorkerNames.has(worker.name),
								)
							) {
								throw new Error(
							...
							for (const task of reconciledTasks) {
								...
								if (
									reconciled &&
									(removableWorkerNames.has(reconciled.owner ?? "") ||
										removableWorkerNames.has(reconciled.claim?.owner ?? ""))
								) {
									throw new Error(
							...
							const pinnedPaneWorkers = removableWorkers.filter(
								(
									worker,
								): worker is WorkerInfo & {
									pane_id: string;
									pid: number;
								} =>
									typeof worker.pane_id === "string" &&
							...
							const unpinnedPaneWorkers = removableWorkers.filter(
								(worker) =>
									typeof worker.pane_id === "string" &&
									/^%\d+$/.test(worker.pane_id) &&
									!pinnedPaneWorkers.some(
										(pinnedWorker) => pinnedWorker.name === worker.name,
									),
							);
							...
							for (const worker of unpinnedPaneWorkers) {
								...
								if (proof.status === "gone") {
									...
									unpinnedProofDebt.push(
										`${worker.pane_id}:legacy_pid_missing_live`,
									);
								}
							}
							const targetPaneIds = pinnedPaneWorkers.map(
								(worker) => worker.pane_id,
							);
							const expectedScaleDownOwnerId =
								typeof config.tmux_pane_owner_id === "string"
									? config.tmux_pane_owner_id.trim()
									: "";
							const paneTeardown = await teardownWorkerPanes(targetPaneIds, {
								leaderPaneId: config.leader_pane_id,
								hudPaneId: config.hud_pane_id,
								expectedPanePids: Object.fromEntries(
									pinnedPaneWorkers.map((worker) => [
										worker.pane_id,
										worker.pid,
									]),
								),
								authorizePaneKill: (paneId) => {
									const currentOwner = readPaneTeamOwnerTagResult(paneId);
									return (
										Boolean(expectedScaleDownOwnerId) &&
										currentOwner.status === "value" &&
							...
							const unresolvedWorkers = removableWorkers.filter(
								(worker) =>
									typeof worker.pane_id === "string" &&
									!resolvedPaneIds.has(worker.pane_id),
							);
							if (unresolvedWorkers.length > 0) {
								...
								const proofDebt = [
									...unpinnedProofDebt,
									...paneTeardown.proofUnavailable.map(
										(proof) => `${proof.paneId}:${proof.reason}`,
									),
								];
								const killDebt = paneTeardown.kill.failedPaneIds.map(
									(paneId) => `${paneId}:kill_failed`,
								);
								await writeAtomic(
									cleanupDebtPath,
									JSON.stringify(
								...
								if (paneTeardown.proofUnavailable.length > 0) {
									teardownFailure = {
										ok: false,
										error: `scale_down_pane_proof_unavailable:${proofDebt.join(",")}`,
									};
									return;
								}
								...
								teardownFailure = {
									ok: false,
									error: `scale_down_cleanup_debt:${failedDebt}${unresolvedDebt}`,
								};
								return;
							}
							await writeAtomic(
				...
				return {
					ok: false,
					error: `scale_down_task_reconciliation_failed:${String(error)}`,
				};
			}
			...
			const resourceCleanup = await reconcileScaleDownCleanupDebt(
				sanitized,
				leaderCwd,
				config,
			);
