		...
		async (): Promise<ScaleUpResult | ScaleError> => {
			...
			for (let i = 0; i < count; i++) {
				...
				if (uniqueTaskRoles.size > 1) {
					console.log(
						`[omx:scaling] ${workerName}: mixed task roles [${[...uniqueTaskRoles].join(", ")}], falling back to ${agentType}`,
					);
				}
				...
				const workerWorkspace = workerWorkspaceResult.enabled
					? workerWorkspaceResult
					: null;
				const workerCwd = workerWorkspace
				...
				const workerTasks = persistedTasks.filter(
