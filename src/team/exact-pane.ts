...
export type ExactPaneProof =
	...
	| {
			...
			reason:
				...
				| "malformed_snapshot"
				| "pane_pid_changed"
				| "pane_proof_lost_during_process_teardown"
...
function parseExactPaneProof(paneId: string, stdout: string): ExactPaneProof {
	...
	for (const line of lines) {
		...
		if (
			!EXACT_PANE_ID_PATTERN.test(snapshotPaneId) ||
			paneIds.has(snapshotPaneId)
		) {
	...
	if (matched.paneDead !== "0")
		return { status: "gone", paneId, reason: "dead" };
...
export function readExactPaneProofSync(paneId: string): ExactPaneProof {
	...
	if (result.error)
		return unavailable(paneId, "query_failed", result.error.message);
	if (result.status !== 0) {
		const stderr =
		...
		return unavailable(
			paneId,
			"query_failed",
			stderr || `tmux exited ${result.status ?? "unknown"}`,
		);
	}

	return parseExactPaneProof(
		paneId,
 ...
 */
export function readExactPaneProofsSync(
	paneIds: readonly string[],
): ExactPaneProof[] {
	if (paneIds.length === 0) return [];
	const invalidProofs = paneIds.map((paneId) =>
		EXACT_PANE_ID_PATTERN.test(paneId)
			? null
			: unavailable(paneId, "invalid_pane_id"),
	);
	if (invalidProofs.some((proof) => proof !== null)) {
		return invalidProofs.map(
			(proof, index) => proof ?? unavailable(paneIds[index]!, "query_failed"),
		);
	}
			...
			if (code !== 0) {
				finish(
					unavailable(
						paneId,
						"query_failed",
						stderr.trim() || `tmux exited ${code ?? "unknown"}`,
					),
				);
				return;
			}
