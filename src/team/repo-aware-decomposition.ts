...
const IMPLEMENTATION_LANE_PATTERN =
	/\b(?:impl|implementation|code|build|feature|fix|refactor)\b/i;
...
function enrichNodeDescription(node: TeamDagNode, cwd: string): string {
	...
	if (files.length > 0) {
		...
		if (existing.length > 0)
			parts.push(
				`Existing paths: ${existing.map((file) => relative(cwd, `${cwd}/${file}`)).join(", ")}`,
			);
		if (missing.length > 0)
...
function topologicalSort(nodes: TeamDagNode[]): TeamDagNode[] {
	...
	while (ready.length > 0) {
		ready.sort(
			(a, b) => (inputIndex.get(a.id) ?? 0) - (inputIndex.get(b.id) ?? 0),
		);
...
function hasImplementationWork(nodes: TeamDagNode[]): boolean {
	return nodes.some((node) => {
		...
		return (
			IMPLEMENTATION_LANE_PATTERN.test(text) &&
			!VERIFICATION_LANE_PATTERN.test(text)
		);
	});
}

function usefulLaneCount(
	nodes: TeamDagNode[],
	readyLaneCount: number,
	reserveVerification: boolean,
): number {
	const laneLabels = new Set(
		nodes.map((node) => node.lane?.trim()).filter(Boolean) as string[],
	);
...
function resolveWorkerCount(params: {
	...
	const source: TeamDagWorkerCountSource = params.explicitWorkerCount
		? "cli-explicit"
		: (policy?.count_source ??
			(policy?.requested_count ? "plan-suggested" : "default-derived"));
	const requested = params.explicitWorkerCount
		? params.requested
		: (policy?.requested_count ?? params.requested);
	const hardCap = Math.min(
		DEFAULT_MAX_WORKERS,
		policy?.max_count ?? DEFAULT_MAX_WORKERS,
	);
	if (source === "cli-explicit") {
		const cap =
			policy?.strict_max_count === true ? hardCap : DEFAULT_MAX_WORKERS;
...
function buildFromDag(
	...
	const nodeDependencies = Object.fromEntries(
		sorted.map((node) => [node.id, node.depends_on ?? []]),
	);
	...
	return {
		...
		metadata: {
			decomposition_source:
				resolution.source === "sidecar" ? "dag_sidecar" : "dag_markdown",
...
export function remapRepoAwareDecompositionMetadataToCreatedTasks(
	...
	plannedTasks.forEach((task, index) => {
		...
		const symbolicDeps =
			task.symbolic_depends_on ??
			metadata.node_dependencies?.[task.symbolic_id] ??
			[];
		taskHints[taskId] = {
			...
			depends_on: symbolicDeps
				.map((dep) => nodeIdToTaskId[dep])
				.filter(Boolean),
			symbolic_depends_on: symbolicDeps,
			allocation_reason:
				task.allocation_reason ?? metadata.allocation_reasons[task.symbolic_id],
		};
	});
...
export function buildRepoAwareTeamExecutionPlan(
	input: LegacyTeamExecutionPlanInput,
): RepoAwareTeamExecutionPlan {
	const resolution =
		input.allowDagHandoff === true
			...
			: ({
					...
					source: "none",
					error:
						input.dagFallbackReason ??
						"dag_handoff_not_approved_for_invocation",
				} satisfies TeamDagResolution);
	if (resolution.dag)
		return buildFromDag(
			input,
			resolution as TeamDagResolution & { dag: TeamDagHandoff },
		);
	...
	return {
		...
		metadata: {
			...
			worker_count_source: input.explicitWorkerCount
				? "cli-explicit"
				: "default-derived",
