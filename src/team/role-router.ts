...
const REVIEW_INTENT =
	/\b(?:audit|check|inspect|review|validate|verify)\b|(?:검토|리뷰|감사|확인|검증)/i;
const PRIMARY_TEST_INTENT =
	/^(?:add|create|expand|improve|increase|write)\b.*\b(?:tests?|specs?|coverage)\b|^(?:테스트\s*(?:추가|작성)|커버리지\s*추가)/i;
const DOCS_INTENT =
	/\b(?:docs?|documentation|readme|guide|changelog)\b|(?:문서|가이드|README|변경로그)/i;
...
const DEBUG_INTENT =
	/\b(?:debug|diagnose|investigate|root cause|trace|bisect)\b|(?:디버그|조사|원인)/i;
const DESIGN_INTENT =
	/\b(?:design|layout|style)\b|\b(?:build|create)\b.*\b(?:ui|component|frontend)\b|(?:디자인|레이아웃|스타일|컴포넌트)/i;
const BUILD_FIX_INTENT =
	/\b(?:build|compile|tsc|type error|compilation)\b|(?:빌드|컴파일|타입 오류)/i;
const CLEANUP_INTENT =
	/\b(?:clean up|consolidate|reduce complexity|refactor|simplify)\b|(?:정리|단순화|리팩터)/i;
...
const RESEARCH_VERB =
	/\b(?:check|consult|investigate|look up|lookup|read|research|review|study|verify)\b/i;
...
const DOCS_DELIVERABLE_VERB =
	/\b(?:add|document|draft|edit|prepare|publish|refresh|revise|update|write)\b/i;
...
function isLocalExplorationTask(text: string): boolean {
	return (
		(LOCAL_EXPLORATION_VERB.test(text) &&
			LOCAL_EXPLORATION_SUBJECT.test(text)) ||
...
function isDocumentationDeliverableTask(text: string): boolean {
	return (
		PRIMARY_DOCS_INTENT.test(text) ||
		(DOCS_DELIVERABLE_VERB.test(text) && DOCS_DELIVERABLE_NOUN.test(text))
	);
}
...
function isImplementationHeavyDependencyTask(text: string): boolean {
	return (
		...
		/\b(?:adopt|migrate|port|replace|replacement|swap|upgrade|wire)\b/i.test(
			text,
		) &&
		!/\b(?:assess|choose|compare|evaluate|options?|recommend|select|trade-?offs?)\b/i.test(
			text,
		)
	);
}
...
function isDependencyEvaluationTask(text: string): boolean {
	if (
		...
		isImplementationHeavyDependencyTask(text)
	) {
		return false;
	}
...
function isResearchTask(text: string): boolean {
	...
	const chosenTechnologyGuidance =
		CHOSEN_TECH_RESEARCH_SIGNAL.test(text) &&
...
function inferLaneIntent(text: string): LaneIntent {
	if (
		BUILD_FIX_INTENT.test(text) &&
		/\b(?:fix|resolve|repair)\b|(?:수정|해결)/i.test(text)
	)
		return "build-fix";
