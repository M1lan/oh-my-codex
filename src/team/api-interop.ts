...
async function markLatestMailboxDispatchDelivered(
	...
	const requests = await teamListDispatchRequests(teamName, cwd, {
		kind: "mailbox",
		to_worker: worker,
	});
	...
	if (latest.status === "pending") {
		await teamMarkDispatchRequestNotified(
			teamName,
			latest.request_id,
...
function parseOptionalEventType(
	value: unknown,
): TeamEventType | "worker_idle" | null {
	if (value === undefined) return null;
...
function findLatestWorkerIdleEvent(
	events: TeamEvent[],
	workerName: string,
): TeamEvent | null {
	for (let index = events.length - 1; index >= 0; index -= 1) {
	...
	return {
		...
		intent:
			typeof event.intent === "string"
				? event.intent
				: typeof (event as Record<string, unknown>).orchestration_intent ===
						"string"
...
function buildIdleState(
	...
	const idleWorkers = workerNames.filter(
		(workerName) => snapshot?.workerStateByName[workerName] === "idle",
	);
	const nonIdleWorkers = workerNames.filter(
		(workerName) => !idleWorkers.includes(workerName),
	);
	const lastIdleTransitionByWorker = Object.fromEntries(
		workerNames.map((workerName) => [
			workerName,
			summarizeEvent(findLatestWorkerIdleEvent(recentEvents, workerName)),
		]),
	);
	const lastAllWorkersIdleEvent = findLatestEventByType(recentEvents, [
		"all_workers_idle",
	]);
	...
	return {
		...
		all_workers_idle:
			workerNames.length > 0 && idleWorkers.length === workerNames.length,
...
async function readLatestLeaderProgressEvidenceMs(
	cwd: string,
	teamName: string,
): Promise<number> {
	...
	const candidates = [leaderRuntimeMs, teamProgressMs].filter((value) =>
		Number.isFinite(value),
	);
...
function buildStallState(
	...
	const deadWorkers = workerNames.filter(
		(workerName) =>
			summary?.workers.find((worker) => worker.name === workerName)?.alive ===
			false,
	);
	const stalledWorkers = [...(summary?.nonReportingWorkers ?? [])].sort();
	const latestAllWorkersIdleEvent = findLatestEventByType(recentEvents, [
		"all_workers_idle",
	]);
	const latestLeaderNudgeEvent = findLatestEventByType(recentEvents, [
		"team_leader_nudge",
	]);
	const latestDeferredEvent = findLatestEventByType(recentEvents, [
	...
	const leaderDecisionState = terminalLeaderDecisionPending
		...
		: stuckLeaderDecisionPending
			...
			: "still_actionable";
	const teamTerminal = phaseState
		? isTerminalPhase(phaseState.current_phase)
		: false;
	const authoritativeDecisionState = teamTerminal
		? "still_actionable"
		: leaderDecisionState;
	const unreadLeaderMessageCount =
		authoritativeLeaderState.unreadLeaderMessageCount;
	const pendingLeaderDispatchCount =
		authoritativeLeaderState.pendingLeaderDispatchCount;
	...
	return {
		...
		last_leader_notification_deferred_event:
			summarizeEvent(latestDeferredEvent),
		source: {
			summary_available: summary !== null,
			snapshot_available: snapshot !== null,
...
function teamStateExists(teamName: string, candidateCwd: string): boolean {
	...
	const teamRoot = join(
		resolveCanonicalTeamStateRoot(candidateCwd),
		"team",
		teamName,
	);
	return (
...
function resolveTeamWorkingDirectoryFromMetadata(
	...
	const fromManifest = readTeamStateRootFromManifest(
	...
	for (const seed of seeds) {
		...
		while (cursor) {
			if (teamStateExists(normalizedTeamName, cursor)) {
				return (
					resolveTeamWorkingDirectoryFromMetadata(normalizedTeamName, cursor) ??
					cursor
				);
			}
...
function readLegacyMailboxMessages(
	...
	try {
		const parsed = JSON.parse(readFileSync(mailboxPath, "utf8")) as {
...
function validateCommonFields(
	...
	options: { skipTeamName?: boolean } = {},
): void {
	const teamName = String(args.team_name || "").trim();
	if (
		!options.skipTeamName &&
		teamName &&
		!TEAM_NAME_SAFE_PATTERN.test(teamName)
	) {
		throw new Error(
...
export async function executeTeamApiOperation(
	...
	try {
		...
		const resolvedTeamName = rawTeamNameForCwd
			...
			: "";
		const cwd = resolvedTeamName
			? resolveTeamWorkingDirectory(resolvedTeamName, fallbackCwd)
			: fallbackCwd;
		const opArgs = resolvedTeamName
			? { ...args, team_name: resolvedTeamName }
			: args;
		...
		switch (operation) {
			case "send-message": {
				...
				const recipient =
					toWorker === "leader-fixed"
						? null
						: (config.workers.find((worker) => worker.name === toWorker) ??
							null);
				if (toWorker !== "leader-fixed" && !recipient) {
					return {
						...
						error: {
							...
							message: `Worker ${toWorker} not found in team ${teamName}`,
						},
					};
				}
				...
				const livePaneId =
					toWorker === "leader-fixed"
						...
						: recipient?.pane_id;
				const hasLiveTmuxTarget =
					typeof livePaneId === "string" && livePaneId.trim().length > 0;

				const beforeMessages = await listMailboxMessages(
					teamName,
					toWorker,
					cwd,
				);
				const beforeIds = new Set(
					beforeMessages.map((message) => message.message_id),
				);
				...
				const message =
					persisted.body || !body ? persisted : { ...persisted, body };
			...
			case "mailbox-mark-delivered": {
				...
				const dispatch = await markLatestMailboxDispatchDelivered(
					teamName,
					worker,
					messageId,
					cwd,
			...
			case "create-task": {
				...
				const requiresCodeChange = opArgs.requires_code_change as
					| boolean
					| undefined;
			...
			case "write-worker-identity": {
				...
				await teamWriteWorkerIdentity(
					...
					worker,
					{
						...
						role,
						assigned_tasks:
							(opArgs.assigned_tasks as string[] | undefined) ?? [],
			...
			case "append-event": {
				...
				const event = await teamAppendEvent(
					teamName,
					{
						...
						worker_count:
							typeof opArgs.worker_count === "number"
								? opArgs.worker_count
								: undefined,
			...
			case "read-events": {
				...
				const wakeableOnly = parseOptionalBoolean(
					opArgs.wakeable_only,
					"wakeable_only",
				);
				const eventType = parseOptionalEventType(opArgs.type);
				const worker =
					typeof opArgs.worker === "string" ? opArgs.worker.trim() : "";
				const taskId =
					typeof opArgs.task_id === "string" ? opArgs.task_id.trim() : "";
				const events = await readTeamEvents(teamName, cwd, {
					afterEventId:
						typeof opArgs.after_event_id === "string"
							? opArgs.after_event_id.trim() || undefined
							: undefined,
			...
			case "await-event": {
				...
				const timeoutMs =
					parseOptionalNonNegativeInteger(opArgs.timeout_ms, "timeout_ms") ??
					30_000;
			...
			case "read-stall-state": {
				...
				const [
					...
					teamListDispatchRequests(teamName, cwd, {
				...
				const recentEvents = selectRecentEvents(events);
				const unreadLeaderMessageCount = leaderMailboxMessages.filter(
					(message: { delivered_at?: string }) => {
						const deliveredAt =
			...
			case "cleanup": {
				...
				const confirmIssues =
					args.confirm_issues === true || args.confirmIssues === true;
				await shutdownTeam(teamName, cwd, { force, confirmIssues });
				return {
					ok: true,
					operation,
					data: { team_name: teamName, cleanup_mode: "shutdown" },
				};
			}
			case "orphan-cleanup": {
				const teamName = String(opArgs.team_name || "").trim();
				...
				return {
					ok: true,
					operation,
					data: { team_name: teamName, cleanup_mode: "orphan_cleanup" },
				};
			}
			...
			case "read-shutdown-ack": {
				...
				const ack = await teamReadShutdownAck(
					...
					worker,
					cwd,
					opArgs.min_updated_at as string | undefined,
				);
			...
			case "write-monitor-snapshot": {
				const teamName = String(opArgs.team_name || "").trim();
				const snapshot = opArgs.snapshot as
