...
export async function loadLivePaneState(
	teamName: string,
	cwd: string,
): Promise<LivePaneState | null> {
...
export async function shutdownWithForceFallback(
	teamName: string,
	cwd: string,
	...
	try {
		...
		if (
			!message.includes("shutdown_gate_blocked") &&
			!message.includes("shutdown_rejected")
		) {
			throw error;
		}
...
export function detectDeadWorkerFailure(
	...
	const allWorkersDead =
		liveWorkerPaneCount > 0 && deadWorkerCount >= liveWorkerPaneCount;
	return {
...
export function resolveRuntimeCliStateRoot(
	cwd: string,
...
export function collectTaskResults(
	stateRoot: string,
	teamName: string,
): TaskResult[] {
...
export function normalizeAgentTypes(
	...
	const invalid = providers.filter(
		(entry) => entry !== "codex" && entry !== "claude" && entry !== "gemini",
	);
	if (invalid.length > 0) {
		throw new Error(
...
export function resolveRuntimeCliMissingFields(
	input: Partial<CliInput>,
): string[] {
	...
	const hasWorkerCount =
		Number.isInteger(input.workerCount) && Number(input.workerCount) > 0;
...
export function resolveRuntimeCliTask(
	input: Pick<CliInput, "task" | "tasks">,
): string {
...
export function resolveRuntimeCliInlineInput(
...
async function readRuntimeCliRawInput(
	argv: readonly string[],
): Promise<string> {
...
async function main(): Promise<void> {
		...
		if (runtime) {
			try {
				if (status === "failed") {
					...
					shutdownSummary = await shutdownTeam(runtime.teamName, runtime.cwd, {
						force: true,
					});
				} else {
					shutdownSummary = await shutdownWithForceFallback(
						runtime.teamName,
						runtime.cwd,
					);
				}
	...
	const handleShutdown = (signal: string): void => {
		...
		process.stderr.write(
			`[runtime-cli] Received ${signal}, shutting down...\n`,
		);
		doShutdown("failed")
			.catch((err) => {
	...
	while (pollActive) {
		...
		try {
			...
			if (livePaneState) {
				await writePanesFile(
					jobId,
					livePaneState.paneIds,
					livePaneState.leaderPaneId,
				);
			}
		} catch (err) {
			process.stderr.write(
