...
async function readTeamMetadataRootFromCandidate(
	...
	const parsed = await readJsonIfExists(
		join(resolvedStateRoot, "team", worker.teamName, filename),
	);
	if (
		!metadataTeamMatches(parsed, worker.teamName) ||
		!workerListContains(parsed, worker.workerName)
	)
		return null;
	return metadataStateRoot(parsed?.team_state_root);
}
...
async function validateWorkerNotifyStateRoot(
	...
	const identityResolved = await validateWithSource(
		stateRoot,
		source,
		cwd,
		worker,
...
export async function resolveWorkerNotifyTeamStateRoot(
	...
	const explicit =
		typeof env.OMX_TEAM_STATE_ROOT === "string"
			...
			: "";
	if (explicit) {
		const resolved = await validateWorkerNotifyStateRoot(
			resolve(cwd, explicit),
			"env",
			cwd,
			worker,
	...
	const direct = await validateWorkerNotifyStateRoot(
		leaderStateRoot,
		"leader_cwd",
		cwd,
		worker,
	...
	for (const [filename, source] of [
		...
		const metadataRoot =
			filename === "identity.json"
				? await readMetadataRootFromValidatedCandidate(
						leaderStateRoot,
						filename,
						cwd,
						worker,
					)
				: await readTeamMetadataRootFromCandidate(
						leaderStateRoot,
						filename,
						cwd,
						worker,
