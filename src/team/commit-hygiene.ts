...
function projectRootFromOmxStateRoot(
...
export function resolveTeamCommitHygieneArtifactCwd(
	config:
		| Pick<TeamConfig, "leader_cwd" | "team_state_root" | "workers">
		| null
		| undefined,
	cwd: string,
): string {
	...
	const derivedFromStateRoot = projectRootFromOmxStateRoot(
		safePath(config?.team_state_root),
		cwd,
	);
...
export function resolveTeamCommitHygieneArtifactPaths(
	teamName: string,
	cwd: string,
...
export async function readTeamCommitHygieneLedger(
	teamName: string,
	cwd: string,
...
const TEAM_COMMIT_HYGIENE_VOCABULARY: TeamCommitHygieneVocabulary = {
	operational_commit_kinds: [
		...
		},
		{
			...
			label: "cross-rebase",
			description:
				"A runtime rebase operation that moves worker work across the current leader branch baseline.",
		},
		{
			value: "worker_clean_rebase",
			label: "worker clean rebase",
			description:
				"A runtime rebase that refreshes a clean worker branch onto the current leader branch baseline.",
		},
		{
	...
	operational_commit_statuses: [
		{
			...
			label: "applied",
			description:
				"The runtime operation changed repository history or preserved worker changes as intended.",
		},
		{
			value: "noop",
			label: "no-op",
		...
		},
		{
			...
			label: "conflict",
			description:
				"The runtime operation encountered conflicts that require human or leader-side reconciliation.",
		},
		{
			value: "skipped",
			label: "skipped",
...
function buildLeaderFinalizationPrompt(
	teamName: string,
	taskSummary: TeamCommitHygieneTaskSummary[],
): string {
...
export function buildTeamCommitHygieneContext(params: {
	...
	return {
		...
		leader_finalization_prompt: buildLeaderFinalizationPrompt(
			params.teamName,
			taskSummary,
		),
	};
...
function renderVocabularyTermsMarkdown(
	title: string,
	terms: TeamCommitHygieneVocabularyTerm[],
): string {
	const lines = [`### ${title}`, ""];
...
function renderCommitHygieneVocabularyMarkdown(
	vocabulary: TeamCommitHygieneVocabulary,
): string {
	return [
		renderVocabularyTermsMarkdown(
			"Operational commit kinds",
			vocabulary.operational_commit_kinds,
		),
		"",
		renderVocabularyTermsMarkdown(
			"Operational commit statuses",
			vocabulary.operational_commit_statuses,
		),
	].join("\n");
}

function renderTaskSummaryMarkdown(
	taskSummary: TeamCommitHygieneTaskSummary[],
): string {
...
function renderOperationalEntriesMarkdown(
	entries: TeamOperationalCommitEntry[],
): string {
	if (entries.length === 0)
...
export function renderTeamCommitHygieneMarkdown(
	...
	return [
		...
		"",
		...context.recommended_next_steps.map(
...
export async function writeTeamCommitHygieneContext(
	...
	await writeAtomic(
		paths.markdownPath,
		renderTeamCommitHygieneMarkdown(context),
	);
	return paths;
}
