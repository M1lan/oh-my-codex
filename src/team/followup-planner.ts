...
export function isShortTeamFollowupRequest(text: string): boolean {
	...
	return SHORT_TEAM_FOLLOWUP_PATTERNS.some((pattern) =>
...
export function isShortRalphFollowupRequest(text: string): boolean {
	const normalized = normalizeFollowupShortcutText(text);
	return SHORT_RALPH_FOLLOWUP_PATTERNS.some((pattern) =>
		pattern.test(normalized),
	);
}
...
function chooseDistinctAvailableRole(
	...
	if (
		!disallowedRoles.includes(fallbackRole) &&
		availableRoles.includes(fallbackRole)
	)
		return fallbackRole;
	return (
		availableRoles.find((role) => !disallowedRoles.includes(role)) ??
		fallbackRole
	);
}
...
function summarizeAllocations(
	...
	return allocations
		.map((allocation) => {
			const reasoning = allocation.reasoningEffort
				? `, ${allocation.reasoningEffort} reasoning`
				: "";
...
function buildVerificationPlan(
	...
	if (mode === "team") {
		const qualityLane = allocations.find((allocation) =>
			allocation.reason.includes("verification"),
		);
		return {
			summary:
...
function pickSpecialistRole(
	...
	const wantsDependencyExpert =
		...
		/\b(?:adopt|assess|choose|compare|evaluate|recommend|replace|risk|select|swap|upgrade)\b/.test(
			normalizedTask,
		);
	const wantsResearcher =
		...
		(/\b(?:api|framework|frameworks|library|libraries|sdk|sdks|vendor)\b/.test(
			normalizedTask,
		) &&
	...
	if (
		/(security|auth|authorization|authentication|xss|injection|cve|vulnerability)/.test(
			normalizedTask,
		)
	) {
		return chooseDistinctAvailableRole(
	...
	if (
		/(debug|regression|root cause|stack trace|incident|flaky)/.test(
			normalizedTask,
		)
	) {
		return chooseDistinctAvailableRole(
...
export function buildFollowupStaffingPlan(
	...
	const workerCount = Math.max(
		1,
		options.workerCount ?? (mode === "team" ? 2 : 3),
	);
	...
	const primaryRole = chooseAvailableRole(
		availableAgentTypes,
		[primaryRoute.role],
		fallbackRole,
	);
	...
	if (mode === "team") {
		if (workerCount >= 2) {
			mergeAllocation(
				...
				qualityRole,
				1,
				"verification + regression lane",
				options.codexHomeOverride,
			);
		}
		if (workerCount >= 3) {
			const specialistRole = pickSpecialistRole(
				task,
				availableAgentTypes,
				primaryRole,
				primaryRole,
