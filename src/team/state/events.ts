...
function normalizeRawTeamEvent(raw: unknown): TeamEvent | null {
	...
	return {
		...
		prev_state: asWorkerState(value.prev_state),
		worker_count:
			typeof value.worker_count === "number" ? value.worker_count : undefined,
		to_worker:
			typeof value.to_worker === "string" ? value.to_worker : undefined,
		source_type:
			typeof value.source_type === "string" ? value.source_type : undefined,
		created_at: createdAt,
	};
}

function isDuplicateNormalizedEvent(
...
function matchesEventType(
	event: TeamEvent,
	type: TeamEventReadOptions["type"],
): boolean {
	if (!type) return true;
...
export async function getLatestTeamEventCursor(
	teamName: string,
	cwd: string,
): Promise<string> {
...
export async function waitForTeamEvent(
	...
	const baseline =
		opts.afterEventId ?? (await getLatestTeamEventCursor(teamName, cwd));
