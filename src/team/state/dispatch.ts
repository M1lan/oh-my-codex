...
function sanitizeDispatchRequestForStatus(
	record: TeamDispatchRequest,
): TeamDispatchRequest {
...
export function normalizeDispatchRequest(
	...
	const status = isTeamDispatchRequestStatus(raw.status)
		? raw.status
		: "pending";
...
function sameMailboxWakeTarget(
	...
	if (existing.status !== "pending" && existing.status !== "notified")
		return false;
	if (
		existing.to_worker !== input.to_worker ||
		existing.pane_id !== input.pane_id
	)
		return false;
...
function equivalentPendingDispatch(
	...
	if (existing.to_worker !== input.to_worker || existing.status !== "pending")
		return false;
...
function hasBridgeDispatchCompat(cwd: string): boolean {
	...
	try {
		return (
			getDefaultBridge(resolveBridgeStateDir(cwd)).readCompatFile(
				"dispatch.json",
			) !== null
		);
	} catch {
		...
		teamName,
		{
			...
			worker_index:
				typeof metadata.worker_index === "number"
					...
					: undefined,
			pane_id:
				typeof metadata.pane_id === "string" && metadata.pane_id !== ""
					? metadata.pane_id
					: undefined,
			trigger_message:
				typeof metadata.trigger_message === "string" &&
				metadata.trigger_message.trim() !== ""
					? metadata.trigger_message
					: (record.reason ?? record.request_id),
			intent: isTeamReminderIntent(metadata.intent)
				? metadata.intent
				: undefined,
			message_id:
				typeof metadata.message_id === "string" && metadata.message_id !== ""
					...
					: undefined,
			inbox_correlation_key:
				typeof metadata.inbox_correlation_key === "string" &&
			...
			attempt_count:
				typeof metadata.attempt_count === "number" ? metadata.attempt_count : 0,
...
export async function enqueueDispatchRequest(
	...
	const queued = await deps.withDispatchLock(
		deps.teamName,
		deps.cwd,
		async () => {
			...
			if (
				existing &&
				(!isBridgeEnabled() || hasBridgeDispatchCompat(deps.cwd))
			) {
			...
			if (
				...
				const bridgeRequest = await readDispatchRequest(
					request.request_id,
					deps,
				);
				if (bridgeRequest) {
					return {
						...
						deduped: false,
						queuedTransport: "bridge" as const,
					};
				}
			}
			...
			return {
				request,
				deduped: false,
				queuedTransport: "legacy-json" as const,
			};
		},
	);
	if (!queued.deduped) {
