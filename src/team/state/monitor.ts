...
function normalizeWorkerIntegrationState(
	value: unknown,
): TeamWorkerIntegrationState | null {
	if (!value || typeof value !== "object") return null;
	const raw = value as Record<string, unknown>;
	return {
		last_seen_head:
			typeof raw.last_seen_head === "string" && raw.last_seen_head !== ""
				? raw.last_seen_head
				: undefined,
		last_integrated_head:
			typeof raw.last_integrated_head === "string" &&
			raw.last_integrated_head !== ""
				? raw.last_integrated_head
				: undefined,
		last_leader_head:
			typeof raw.last_leader_head === "string" && raw.last_leader_head !== ""
				? raw.last_leader_head
				: undefined,
		last_rebased_leader_head:
			typeof raw.last_rebased_leader_head === "string" &&
			raw.last_rebased_leader_head !== ""
				? raw.last_rebased_leader_head
				: undefined,
		status: isTeamWorkerIntegrationStatus(raw.status) ? raw.status : undefined,
		conflict_commit:
			typeof raw.conflict_commit === "string" && raw.conflict_commit !== ""
				? raw.conflict_commit
				: undefined,
		conflict_files: Array.isArray(raw.conflict_files)
			? raw.conflict_files.filter(
...
function normalizeIntegrationByWorker(
	value: unknown,
): Record<string, TeamWorkerIntegrationState> {
	...
	const entries = Object.entries(value as Record<string, unknown>)
		...
		.filter(
			(entry): entry is readonly [string, TeamWorkerIntegrationState] =>
				entry !== null,
		);
...
export async function writeSummarySnapshot(
	...
	await writeAtomic(
		summarySnapshotPath(teamName, cwd),
...
export async function getTeamSummary(
	...
	const previousSnapshot = await readSummarySnapshot(
		deps.teamName,
		deps.cwd,
		deps.summarySnapshotPath,
	);
	...
	for (const { worker, hb, status } of workerSignals) {
		...
		const prevTurnCount =
			previousSnapshot?.workerTurnCountByName[worker.name] ?? 0;
...
export async function readMonitorSnapshot(
	...
	try {
		...
		return {
			...
			integrationByWorker: normalizeIntegrationByWorker(
				parsed.integrationByWorker,
			),
			monitorTimings,
		};
	} catch {
...
export async function writeMonitorSnapshot(
	...
	await writeAtomic(
		monitorSnapshotPath(teamName, cwd),
		JSON.stringify(snapshot, null, 2),
	);
}
