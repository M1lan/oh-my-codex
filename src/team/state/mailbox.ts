...
export function normalizeBridgeMailboxMessage(
	...
	await deps.withMailboxLock(deps.teamName, toWorker, deps.cwd, async () => {
		...
		if (deps.readLegacyMailbox) {
			const seenMessageIds = new Set(
				dedupeCandidates.map((candidate) => candidate.message_id),
			);
		...
		const config = await deps
			.readTeamConfig(deps.teamName, deps.cwd)
			.catch(() => null);
		const teamCreatedAt =
		...
		const isCurrentTeamMessage = (candidate: TeamMailboxMessage): boolean => {
			...
			return (
				...
				candidateCreatedAt >= teamCreatedAt
			);
		};
		...
		if (
			...
			const bridgeMailbox = await deps.readMailbox(
				deps.teamName,
				toWorker,
				deps.cwd,
			);
			const bridgeMessage = bridgeMailbox.messages.find(
				(candidate) => candidate.message_id === msgId,
			);
			if (bridgeMessage) {
				...
				const shadowIndex = shadowMailbox.messages.findIndex(
					(candidate) => candidate.message_id === msgId,
				);
...
export async function markMessageDelivered(
	...
	const existingMailbox = await deps.readMailbox(
		deps.teamName,
		workerName,
		deps.cwd,
	);
	const existingMessage = existingMailbox.messages.find(
	...
	if (
		...
		const updated = await deps.withMailboxLock(
			deps.teamName,
			workerName,
			deps.cwd,
			async () => {
				const mailbox = await deps.readMailbox(
					deps.teamName,
					workerName,
					deps.cwd,
				);
...
export async function markMessageNotified(
	...
	if (
		...
		return await deps.withMailboxLock(
			deps.teamName,
			workerName,
			deps.cwd,
			async () => {
