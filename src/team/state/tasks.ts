...
function isClaimLeaseExpired(
	claim: TeamTask["claim"] | undefined | null,
	now: Date = new Date(),
): boolean {
...
function extractCoordinationComplianceEvidence(
	...
	const result =
		typeof terminalData?.result === "string" ? terminalData.result : "";
	const checkedMatch = result.match(
		/^\s*Coordination protocol:\s*coordinated\s*[-:]\s*(.+)$/im,
	);
	if (checkedMatch?.[1]?.trim()) {
		return {
			status: "checked",
			source: "terminal_result",
			detail: checkedMatch[1].trim(),
			recorded_at: new Date().toISOString(),
		};
	}

	const noBoundaryMatch = result.match(
		/^\s*Coordination protocol:\s*no boundary handoff(?: remained)?\s*[-:]\s*(.+)$/im,
	);
	if (noBoundaryMatch?.[1]?.trim()) {
		return {
			status: "no_boundary_handoff",
			source: "terminal_result",
			detail: noBoundaryMatch[1].trim(),
			recorded_at: new Date().toISOString(),
		};
	}
...
export async function transitionTaskStatus(
	...
	const lock = await deps.withTaskClaimLock(
		...
		async () => {
			...
			if (
				to === "completed" &&
				requiresCoordinationComplianceEvidence(v) &&
				!coordinationCompliance
			) {
				return {
					ok: false as const,
					error: "missing_coordination_compliance_evidence" as const,
				};
			}
			...
			const updated: TeamTaskV2 = {
				...
				coordination_compliance:
					to === "completed"
						? (coordinationCompliance ?? v.coordination_compliance)
						: v.coordination_compliance,
				claim: undefined,
				version: v.version + 1,
			};
			await deps.writeAtomic(
	...
	const lock = await deps.withTaskClaimLock(
		...
		async () => {
			...
			if (v.status !== "in_progress" || !v.claim)
				return { ok: true as const, task: v, reclaimed: false };
			if (!isClaimLeaseExpired(v.claim))
				return { ok: false as const, error: "lease_active" as const };
