...
function startLockLeaseHeartbeat(
	lockDir: string,
	ownerPath: string,
	ownerToken: string,
	lockStaleMs: number,
...
export async function withScalingLock<T>(
	...
	while (true) {
		try {
			...
			if (Date.now() > deadline) {
				throw new Error(
					`Timed out acquiring scaling lock for team ${teamName}`,
				);
			}
			await sleep(50);
		}
	...
	const stopHeartbeat = startLockLeaseHeartbeat(
		lockDir,
		ownerPath,
		ownerToken,
		lockStaleMs,
...
export async function withMailboxLock<T>(
	...
	while (true) {
		try {
			...
			if (Date.now() > deadline) {
				throw new Error(
					`Timed out acquiring mailbox lock for ${teamName}/${workerName}`,
				);
			}
			await sleep(LOCK_OWNER_RETRY_MS);
		}
