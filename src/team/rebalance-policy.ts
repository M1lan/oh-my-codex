...
function hasCompletedDependencies(
	task: TeamTask,
...
function isWorkerAvailable(worker: RebalanceWorkerInput): boolean {
	return (
		...
		(worker.status.state === "idle" ||
			...
			worker.status.state === "unknown")
	);
}

export function buildRebalanceDecisions(
	input: RebalancePolicyInput,
): RebalanceDecision[] {
