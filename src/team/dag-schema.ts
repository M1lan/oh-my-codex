...
export type TeamDagWorkerCountSource =
	| "cli-explicit"
	| "plan-suggested"
	| "default-derived";
...
function assertDagMatchesPlan(
	...
	if (dag.plan_slug !== undefined && dag.plan_slug !== slug) {
		throw new Error(
			`Team DAG plan_slug ${dag.plan_slug} does not match latest approved plan slug ${slug}`,
		);
	}
	if (
		dag.source_prd !== undefined &&
		basename(dag.source_prd) !== basename(prdPath)
	) {
		throw new Error(
			`Team DAG source_prd ${dag.source_prd} does not match latest approved PRD ${basename(prdPath)}`,
		);
	}
}
...
function matchesSidecarSlug(file: string, slug: string): boolean {
	...
	return (
		(file === `${prefix}.json` || file.startsWith(`${prefix}-`)) &&
		file.endsWith(".json")
	);
}
...
function readSidecar(
	...
	slug: string,
	prdPath: string,
): Omit<TeamDagResolution, "planSlug"> | null {
...
export function readTeamDagHandoffForLatestPlan(
	cwd: string,
): TeamDagResolution {
	...
	if (selection.testSpecPaths.length === 0) {
		return {
			...
			source: "none",
			planSlug,
			error: "missing_matching_test_spec",
		};
	}
