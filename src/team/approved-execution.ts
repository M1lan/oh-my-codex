...
export function normalizeApprovedTeamExecutionBinding(
	...
	if (
		typeof binding.prd_path !== "string" ||
	...
	const command =
		typeof binding.command === "string" ? binding.command.trim() : "";
...
export function buildUltragoalCheckpointGuidance(
	...
	return {
		...
		checkpoint_command_template:
			"<leader must read verified .omx/ultragoal/goals.json context before constructing checkpoint command>",
		final_checkpoint_command_template:
			"<leader must read verified .omx/ultragoal/goals.json context and pass final quality gates before constructing checkpoint command>",
		evidence_requirements: [
			"team tasks are terminal",
			"verification passed",
			goalId
				? `evidence mentions ${goalId}`
				: "leader resolved the active goal ID from .omx/ultragoal/goals.json",
...
export function buildApprovedTeamHandoffSection(
	...
	if (approvedHint.repositoryContextSummary) {
		lines.push(
			...renderApprovedRepositoryContextSummary(
				approvedHint.repositoryContextSummary,
			),
		);
	}
	lines.push(
		...renderLeaderOwnedUltragoalContext(
			buildUltragoalCheckpointGuidance(approvedHint),
		),
	);
...
export async function readPersistedApprovedTeamExecutionBindingState(
	...
	try {
		...
		const binding = normalizeApprovedTeamExecutionBinding(
...
export async function readPersistedApprovedTeamExecutionBinding(
	...
	const state = await readPersistedApprovedTeamExecutionBindingState(
		teamName,
		cwd,
		teamStateRoot,
	);
...
export function readPersistedApprovedTeamExecutionBindingSync(
	...
	const state = readPersistedApprovedTeamExecutionBindingStateSync(
		teamName,
		cwd,
		teamStateRoot,
	);
...
export function readApprovedTeamExecutionHintOutcomeFromBinding(
	...
	const direct = readApprovedTeamExecutionHintOutcomeForPrdPath(
		cwd,
		normalized,
		normalized.prd_path,
	);
	...
	const matchedPrdPath = readPlanningArtifacts(cwd).prdPaths.find(
		(candidatePath) => sameFilePath(candidatePath, normalized.prd_path),
	);
	if (!matchedPrdPath || matchedPrdPath === normalized.prd_path) {
		return direct;
	}

	return readApprovedTeamExecutionHintOutcomeForPrdPath(
		cwd,
		normalized,
		matchedPrdPath,
	);
...
export async function resolvePersistedApprovedTeamExecutionContinuityState(
	...
	const approvedHintOutcome = readApprovedTeamExecutionHintOutcomeFromBinding(
		cwd,
		state.binding,
	);
	...
	return {
		...
		approvedHint: approvedHintOutcome.approvedHint,
	};
}
