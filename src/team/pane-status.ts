...
export async function readTeamPaneStatus(
	...
	snapshot?: Pick<
		TeamSnapshot,
		"teamName" | "deadWorkers" | "nonReportingWorkers" | "workers" | "tasks"
	>,
	...
	const sparkshellCommands = Object.fromEntries(
		[
			leaderPaneId
				? [
						"leader",
						`omx sparkshell --tmux-pane ${leaderPaneId} --tail-lines ${tailLines}`,
					]
				: null,
			hudPaneId
				? [
						"hud",
						`omx sparkshell --tmux-pane ${hudPaneId} --tail-lines ${tailLines}`,
					]
				: null,
	...
	const recommendedInspectTargets = [
		...
		(workerName, index, values) =>
			Object.hasOwn(workerPanes, workerName) &&
			values.indexOf(workerName) === index,
	);
	const recommendedInspectReasons = Object.fromEntries(
		recommendedInspectTargets.map((target) => [
			target,
			(snapshot?.deadWorkers ?? []).includes(target)
				? "dead_worker"
				: "non_reporting_worker",
		]),
	);
	...
	const taskStatusById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.status] as const,
		),
	);
	const taskResultById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.result ?? null] as const,
		),
	);
	const taskErrorById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.error ?? null] as const,
		),
	);
	const taskVersionById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.version ?? null] as const,
		),
	);
	const taskCreatedAtById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.created_at ?? null] as const,
		),
	);
	const taskCompletedAtById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.completed_at ?? null] as const,
		),
	);
	const taskDependsOnById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.depends_on ?? task.blocked_by ?? []] as const,
		),
	);
	const taskClaimPresentById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.claim != null] as const,
		),
	);
	const taskClaimOwnerById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.claim?.owner ?? null] as const,
		),
	);
	const taskClaimTokenById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.claim?.token ?? null] as const,
		),
	);
	const taskClaimLeaseById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.claim?.leased_until ?? null] as const,
		),
	);
	const taskRequiresCodeChangeById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.requires_code_change ?? null] as const,
		),
	);
	...
	const recommendedInspectTaskCompletedAt = Object.fromEntries(
		recommendedInspectTargets.map((target) => {
			...
			return [
				target,
				taskId ? (taskCompletedAtById.get(taskId) ?? null) : null,
			];
		}),
	);
	...
	const recommendedInspectTaskClaimPresent = Object.fromEntries(
		recommendedInspectTargets.map((target) => {
			...
			return [
				target,
				taskId ? (taskClaimPresentById.get(taskId) ?? null) : null,
			];
		}),
	);
	...
	const recommendedInspectRequiresCodeChange = Object.fromEntries(
		recommendedInspectTargets.map((target) => {
			...
			return [
				target,
				taskId ? (taskRequiresCodeChangeById.get(taskId) ?? null) : null,
			];
		}),
	);
	const taskDescriptionById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.description] as const,
		),
	);
	const recommendedInspectDescriptions = Object.fromEntries(
		recommendedInspectTargets.map((target) => {
			...
			return [
				target,
				taskId ? (taskDescriptionById.get(taskId) ?? null) : null,
			];
		}),
	);
	const taskBlockedById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.blocked_by ?? []] as const,
		),
	);
	...
	const taskRoleById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.role ?? null] as const,
		),
	);
	...
	const taskOwnerById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.owner ?? null] as const,
		),
	);
	...
	const approvalRecordByTaskId = new Map<
		string,
		Awaited<ReturnType<typeof readTaskApproval>>
	>();
	for (const taskId of new Set(
		...
		approvalRecordByTaskId.set(
			taskId,
			snapshot?.teamName
				? await readTaskApproval(snapshot.teamName, taskId, cwd)
				: null,
		);
	}
	const recommendedInspectApprovalStatuses = Object.fromEntries(
		recommendedInspectTargets.map((target) => {
			...
			return [
				target,
				taskId ? (approvalRecordByTaskId.get(taskId)?.status ?? null) : null,
			];
		}),
	);
	const recommendedInspectApprovalRequired = Object.fromEntries(
		recommendedInspectTargets.map((target) => {
			...
			return [
				target,
				taskId ? (approvalRecordByTaskId.get(taskId)?.required ?? null) : null,
			];
		}),
	);
	const recommendedInspectApprovalReviewers = Object.fromEntries(
		recommendedInspectTargets.map((target) => {
			...
			return [
				target,
				taskId ? (approvalRecordByTaskId.get(taskId)?.reviewer ?? null) : null,
			];
		}),
	);
	const recommendedInspectApprovalReasons = Object.fromEntries(
		recommendedInspectTargets.map((target) => {
			...
			return [
				target,
				taskId
					? (approvalRecordByTaskId.get(taskId)?.decision_reason ?? null)
					: null,
			];
		}),
	);
	const recommendedInspectApprovalDecidedAt = Object.fromEntries(
		recommendedInspectTargets.map((target) => {
			...
			return [
				target,
				taskId
					? (approvalRecordByTaskId.get(taskId)?.decided_at ?? null)
					: null,
			];
		}),
	);
	const recommendedInspectApprovalRecordPresent = Object.fromEntries(
		recommendedInspectTargets.map((target) => {
			...
			return [
				target,
				taskId ? approvalRecordByTaskId.get(taskId) !== null : null,
			];
		}),
	);
	const recommendedInspectPanes = Object.fromEntries(
		recommendedInspectTargets.map((target) => [
			target,
			workerPanes[target] ?? null,
		]),
	);
	const taskSubjectById = new Map(
		(snapshot?.tasks.items ?? []).map(
			(task) => [task.id, task.subject] as const,
		),
	);
	...
	const recommendedInspectTeamConfigPaths = Object.fromEntries(
		recommendedInspectTargets.map((target) => [
			target,
			snapshot?.teamName
				? join(cwd, ".omx", "state", "team", snapshot.teamName, "config.json")
				: null,
		]),
	);
	...
	const recommendedInspectTeamDirPaths = Object.fromEntries(
		recommendedInspectTargets.map((target) => [
			target,
			snapshot?.teamName
				? join(cwd, ".omx", "state", "team", snapshot.teamName)
				: null,
		]),
	);
	const recommendedInspectTeamPhasePaths = Object.fromEntries(
		recommendedInspectTargets.map((target) => [
			target,
			snapshot?.teamName
				? join(cwd, ".omx", "state", "team", snapshot.teamName, "phase.json")
				: null,
		]),
	);
		...
		recommendedInspectTargets.length > 0
			? [
					...
					recommendedInspectRoles[recommendedInspectTargets[0]!]
						...
						: "",
					typeof recommendedInspectAlive[recommendedInspectTargets[0]!] ===
					"boolean"
						? `alive=${recommendedInspectAlive[recommendedInspectTargets[0]!]}`
						: "",
					typeof recommendedInspectTurnCounts[recommendedInspectTargets[0]!] ===
					"number"
						? `turn_count=${recommendedInspectTurnCounts[recommendedInspectTargets[0]!]}`
						: "",
					typeof recommendedInspectTurnsWithoutProgress[
						recommendedInspectTargets[0]!
					] === "number"
					...
					recommendedInspectCommand
						? `command=${recommendedInspectCommand}`
						: "",
				]
					.filter(Boolean)
					.join(" ")
		...
		.map((target) => {
			...
			return {
				...
				turns_without_progress:
					recommendedInspectTurnsWithoutProgress[target] ?? null,
				...
				task_claim_leased_until:
					recommendedInspectTaskClaimLeases[target] ?? null,
				task_claim_lock_path:
					recommendedInspectTaskClaimLockPaths[target] ?? null,
				approval_required: recommendedInspectApprovalRequired[target] ?? null,
				requires_code_change:
					recommendedInspectRequiresCodeChange[target] ?? null,
				...
				approval_decided_at:
					recommendedInspectApprovalDecidedAt[target] ?? null,
				approval_record_present:
					recommendedInspectApprovalRecordPresent[target] ?? null,
				...
				worker_heartbeat_path:
					recommendedInspectWorkerHeartbeatPaths[target] ?? null,
				worker_identity_path:
					recommendedInspectWorkerIdentityPaths[target] ?? null,
				worker_inbox_path: recommendedInspectWorkerInboxPaths[target] ?? null,
				worker_mailbox_path:
					recommendedInspectWorkerMailboxPaths[target] ?? null,
				worker_shutdown_request_path:
					recommendedInspectWorkerShutdownRequestPaths[target] ?? null,
				worker_shutdown_ack_path:
					recommendedInspectWorkerShutdownAckPaths[target] ?? null,
				...
				team_monitor_snapshot_path:
					recommendedInspectTeamMonitorSnapshotPaths[target] ?? null,
				team_summary_snapshot_path:
					recommendedInspectTeamSummarySnapshotPaths[target] ?? null,
				command,
			};
		})
	...
	return {
		...
		sparkshell_hint:
			Object.keys(workerPanes).length > 0
		...
		recommended_inspect_turns_without_progress:
			recommendedInspectTurnsWithoutProgress,
		...
		recommended_inspect_worktree_repo_roots:
			recommendedInspectWorktreeRepoRoots,
		...
		recommended_inspect_task_claim_lock_paths:
			recommendedInspectTaskClaimLockPaths,
		recommended_inspect_approval_required: recommendedInspectApprovalRequired,
		recommended_inspect_requires_code_change:
			recommendedInspectRequiresCodeChange,
		...
		recommended_inspect_approval_decided_at:
			recommendedInspectApprovalDecidedAt,
		recommended_inspect_approval_record_present:
			recommendedInspectApprovalRecordPresent,
		...
		recommended_inspect_worker_status_paths:
			recommendedInspectWorkerStatusPaths,
		recommended_inspect_worker_heartbeat_paths:
			recommendedInspectWorkerHeartbeatPaths,
		recommended_inspect_worker_identity_paths:
			recommendedInspectWorkerIdentityPaths,
		recommended_inspect_worker_inbox_paths: recommendedInspectWorkerInboxPaths,
		recommended_inspect_worker_mailbox_paths:
			recommendedInspectWorkerMailboxPaths,
		recommended_inspect_worker_shutdown_request_paths:
			recommendedInspectWorkerShutdownRequestPaths,
		recommended_inspect_worker_shutdown_ack_paths:
			recommendedInspectWorkerShutdownAckPaths,
		...
		recommended_inspect_team_manifest_paths:
			recommendedInspectTeamManifestPaths,
		recommended_inspect_team_events_paths: recommendedInspectTeamEventsPaths,
		recommended_inspect_team_dispatch_paths:
			recommendedInspectTeamDispatchPaths,
		recommended_inspect_team_phase_paths: recommendedInspectTeamPhasePaths,
		recommended_inspect_team_monitor_snapshot_paths:
			recommendedInspectTeamMonitorSnapshotPaths,
		recommended_inspect_team_summary_snapshot_paths:
			recommendedInspectTeamSummarySnapshotPaths,
