...
export function teamDeliveryLogPath(
	logsDir: string,
	now: Date = new Date(),
): string {
...
export async function appendTeamDeliveryLog(
	logsDir: string,
	event: TeamDeliveryLogEvent,
): Promise<void> {
	const now = new Date();
	...
	await appendFile(
		teamDeliveryLogPath(logsDir, now),
		`${JSON.stringify(entry)}\n`,
	).catch(() => {});
}

export async function appendTeamDeliveryLogForCwd(
	cwd: string,
	event: TeamDeliveryLogEvent,
): Promise<void> {
