...
export const TEAM_WORKER_INHERITED_MODEL_ENV =
	"OMX_TEAM_WORKER_INHERITED_MODEL";
...
export type TeamWorkerLaunchPolicyClassification =
	| TeamWorkerLaunchPolicyKind
	| "mixed-policy";
...
export type TeamWorkerLaunchModelSource =
	| "env"
	| "inherited"
	| "fallback"
	| "none";
export type TeamWorkerLaunchReasoningSource =
	| "explicit"
	| "role-default"
	| "none";
...
function configSelector(arg: string): { value?: string } | null {
	...
	if (arg.startsWith(`${CONFIG_FLAG}=`))
		return { value: arg.slice(`${CONFIG_FLAG}=`.length) };
	if (arg.startsWith(`${LONG_CONFIG_FLAG}=`))
		return { value: arg.slice(`${LONG_CONFIG_FLAG}=`.length) };
	return null;
}

function extractConfigPolicyAssignment(
	value: string,
): { axis: PolicyAxis; value: string } | null {
	const match = new RegExp(
		`^(${APPROVAL_POLICY_KEY}|${SANDBOX_MODE_KEY})\\s*=\\s*([\\s\\S]*)$`,
	).exec(value.trim());
	if (!match) return null;
...
function resolveTeamWorkerLaunchDiagnosticsFromParts(params: {
	...
	const inheritedModel = normalizeOptionalModel(
		params.inheritedParsed.modelOverride,
	);
	...
	const requestedDefaultReasoning = normalizeOptionalReasoning(
		params.preferredReasoning,
	);
	const explicitReasoningOverride =
		params.inheritedParsed.reasoningOverride ??
	...
	return {
		...
		modelSource:
			...
			(!inheritedModel || envModel !== inheritedModel)
				? "env"
				: selectedModel && inheritedModel && selectedModel === inheritedModel
					...
					: selectedModel
						? "fallback"
						: "none",
		reasoningSource: explicitReasoningOverride
			? "explicit"
			: requestedDefaultReasoning
				? "role-default"
				: "none",
		inheritedParentModel:
			Boolean(inheritedModel) &&
...
function tokenizeWorkerLaunchArgs(
	raw: string | undefined,
): WorkerLaunchArgToken[] {
...
function extractExactEnvironmentReasoningOverride(
	raw: string | undefined,
): string | null {
	...
	for (let index = 0; index < tokens.length; index += 1) {
		...
		const startsWithGroupingQuote =
			configValue.source.startsWith('"') || configValue.source.startsWith("'");
		reasoningOverride = startsWithGroupingQuote
			? configValue.value
			: configValue.source;
	}
...
function restoreExactEnvironmentReasoningOverride(
	args: string[],
	raw: string | undefined,
): string[] {
	...
	for (let index = 0; index < args.length; index += 1) {
		if (args[index] === "--") break;
		if (
			args[index] !== CONFIG_FLAG ||
			!isReasoningOverride(args[index + 1] ?? "")
		)
			continue;
		reasoningValueIndex = index + 1;
		index += 1;
	}
	if (reasoningValueIndex !== null)
		args[reasoningValueIndex] = exactReasoningOverride;
	return args;
}
...
function setDirectPolicyValue(
	...
	if (existingValue !== null && existingValue !== value) {
		throw teamWorkerLaunchArgsError(
			source,
			`conflicting duplicate ${axis} policy`,
		);
	}
	return value;
}

function directPolicySelector(
	arg: string,
): { axis: "approval" | "sandbox"; flag: string; value?: string } | null {
...
export function parseTeamWorkerLaunchArgs(
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (directPolicy) {
			...
			if (directPolicy.axis === "approval") {
				approvalValue = setDirectPolicyValue(
					approvalValue,
					value,
					"approval",
					source,
				...
				sandboxValue = setDirectPolicyValue(
					sandboxValue,
					value,
					"sandbox",
					source,
		...
		if (config) {
			...
			if (configValueMissing) {
				...
				const configFlag =
					arg === LONG_CONFIG_FLAG || arg.startsWith(`${LONG_CONFIG_FLAG}=`)
						? LONG_CONFIG_FLAG
						: CONFIG_FLAG;
				throw teamWorkerLaunchArgsError(
					source,
					`missing value for ${configFlag}`,
				);
			}
			if (splitConfig) i += 1;
			...
			const configPolicy =
				typeof configValue === "string"
					? extractConfigPolicyAssignment(configValue)
					: null;
			if (configPolicy) {
				if (options.directPolicyMode === "ignore") continue;

				const flag =
					configPolicy.axis === "approval"
						? TEAM_WORKER_APPROVAL_FLAG
						: TEAM_WORKER_SANDBOX_FLAG;
			...
			if (splitConfig && typeof configValue === "string")
				passthrough.push(configValue);
			continue;
		}
	...
	const policyKind: TeamWorkerLaunchPolicyKind =
		approvalValue !== null || sandboxValue !== null
			? "direct-policy"
			: wantsBypass
				? "bypass"
				: "none";
...
export function classifyTeamWorkerLaunchPolicy(
	args: string[],
): TeamWorkerLaunchPolicyClassification {
	const parsed = parseTeamWorkerLaunchArgs(args);
	if (parsed.wantsBypass && parsed.policyKind === "direct-policy")
		return "mixed-policy";
...
export function collectInheritableTeamWorkerArgs(
	...
	const parsed = parseTeamWorkerLaunchArgs(codexArgs, "leader arguments", {
...
export function extractModelProviderOverrideValue(
	args: string[],
): string | undefined {
...
export function normalizeTeamWorkerLaunchArgs(
	...
	const parsed = parseTeamWorkerLaunchArgs(args);
	const endOfOptionsIndex =
		parsed.endOfOptionsIndex ?? parsed.passthrough.length;
	const normalized = parsed.passthrough.slice(0, endOfOptionsIndex);

	if (parsed.policyKind === "direct-policy") {
		if (parsed.approvalValue !== null)
			normalized.push(TEAM_WORKER_APPROVAL_FLAG, parsed.approvalValue);
		if (parsed.sandboxValue !== null)
			normalized.push(TEAM_WORKER_SANDBOX_FLAG, parsed.sandboxValue);
	...
	if (selectedModelProvider)
		normalized.push(
			CONFIG_FLAG,
			canonicalizeConfigStringOverride(
...
export function resolveTeamWorkerLaunchArgs(
	...
	const envParsed = parseTeamWorkerLaunchArgs(
		envArgs,
		"OMX_TEAM_WORKER_LAUNCH_ARGS",
	);
	...
	const inheritedParsed = parseTeamWorkerLaunchArgs(
		inheritedArgs,
		"inherited leader worker launch arguments",
	);
	...
	const combinedArgs =
		endOfOptionsIndex === -1
			...
			: [
					...envArgs.slice(0, endOfOptionsIndex),
					...inheritedArgs,
					...envArgs.slice(endOfOptionsIndex),
				];
...
export function resolveAgentReasoningEffort(
	...
	if (typeof agentType !== "string" || agentType.trim() === "")
		return undefined;
	return (
		normalizeOptionalReasoning(
			getAgentReasoningOverride(agentType, codexHomeOverride),
...
export function resolveTeamLowComplexityDefaultModel(
	codexHomeOverride?: string,
): string {
