...
export const TEAM_DISPATCH_REQUEST_STATUSES = [
	"pending",
	"notified",
	"delivered",
	"failed",
...
export type TeamDispatchRequestStatus =
	(typeof TEAM_DISPATCH_REQUEST_STATUSES)[number];

export const TEAM_TERMINAL_DISPATCH_REQUEST_STATUSES: ReadonlySet<TeamDispatchRequestStatus> =
	new Set(["delivered", "failed"]);
...
export function isTeamDispatchRequestStatus(
	status: unknown,
): status is TeamDispatchRequestStatus {
	return TEAM_DISPATCH_REQUEST_STATUSES.includes(
		status as TeamDispatchRequestStatus,
	);
}

export function isTerminalTeamDispatchRequestStatus(
...
export type TeamWorkerIntegrationStatus =
	(typeof TEAM_WORKER_INTEGRATION_STATUSES)[number];

export function isTeamWorkerIntegrationStatus(
	status: unknown,
): status is TeamWorkerIntegrationStatus {
	return TEAM_WORKER_INTEGRATION_STATUSES.includes(
		status as TeamWorkerIntegrationStatus,
	);
}
