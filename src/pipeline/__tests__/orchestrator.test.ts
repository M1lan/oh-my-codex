...
describe("Pipeline Orchestrator", () => {
	...
	describe("runPipeline", () => {
		...
		it("returns to ralplan rather than deep-interview after default quality-gate failures", async () => {
			...
			assert.equal(ext?.review_cycle, 1);
			assert.equal(
				(ext?.qa_verdict as { clean?: boolean } | undefined)?.clean,
				true,
			);
			assert.equal(ext?.return_to_ralplan_reason, null);
		});
		...
		it("final completion write replaces stale BLOCK verdict state with clean artifacts", async () => {
			...
			assert.equal(
				(ext?.review_verdict as { recommendation?: string } | undefined)
					?.recommendation,
				"APPROVE",
			);
			assert.equal(
				(ext?.review_verdict as { architectural_status?: string } | undefined)
			...
			assert.equal(ext?.return_to_ralplan_reason, null);
			assert.ok(
				ext?.handoff_artifacts?.code_review,
				"clean code-review artifact should be preserved in terminal state",
			);
			assert.ok(
				ext?.handoff_artifacts?.ultraqa,
				"clean ultraqa artifact should be preserved in terminal state",
			);
			assert.equal(
				(ext?.pipeline_stage_results as Record<string, unknown> | undefined)?.[
					"code-review"
				] !== undefined,
				true,
			);
		...
		it("materializes ralplan consensus handoff artifacts when ralplan is skipped", async () => {
			...
			assert.ok(
				handoffs.ralplan,
				"skipped ralplan handoff should remain visible",
			);
