...
import {
	...
	buildDeepInterviewInstruction,
} from "../stages/deep-interview.js";
import { createRalplanStage } from "../stages/ralplan.js";
import {
	createTeamExecStage,
	buildTeamInstruction,
} from "../stages/team-exec.js";
import {
	createRalphVerifyStage,
	createRalphStage,
	buildRalphInstruction,
} from "../stages/ralph-verify.js";
import {
	createCodeReviewStage,
	buildCodeReviewInstruction,
} from "../stages/code-review.js";
import {
	createUltragoalStage,
	buildUltragoalInstruction,
} from "../stages/ultragoal.js";
import {
	createUltraqaStage,
	buildUltraqaInstruction,
} from "../stages/ultraqa.js";
...
function decodeRuntimeCliInstructionPayload(
	instruction: string,
					...
					[sessionId]: {
						...
						threads: {
							"thread-leader": {
								...
								kind: "leader",
								first_seen_at: architectStart,
								last_seen_at: criticDone,
								turn_count: 1,
							},
							"thread-architect": {
								...
								mode: "architect",
								first_seen_at: architectStart,
								last_seen_at: architectDone,
								completed_at: architectDone,
								turn_count: 1,
							},
							"thread-critic": {
								...
								role: "critic",
								mode: "critic",
								first_seen_at: criticStart,
								last_seen_at: criticDone,
								completed_at: criticDone,
								turn_count: 1,
							},
						},
					},
...
describe("RALPLAN Stage", () => {
	...
	it("fails closed without planning artifacts and consensus evidence", async () => {
		...
		assert.equal(
			(result.artifacts as Record<string, unknown>).stage,
			"ralplan",
		);
	...
	it("run fails with consensus-specific artifact error when consensus exists but planning artifacts are missing", async () => {
		...
		assert.equal(
			(result.artifacts as Record<string, unknown>).planningComplete,
			false,
		);
	});
						...
						ralplan: {
							ralplanConsensusGate: {
								...
								ralplan_critic_review: {
									...
									verdict: "approve",
									sequence_index: 2,
									summary: "critic approved after architect",
								},
							},
						},
					},
			...
			stage.canSkip!(
				makeCtx({
					artifacts: {
						ralplan: {
							architectReviews: [
								{ verdict: "approve", summary: "role missing" },
							],
	...
	it("surfaces deep-interview specs in ralplan artifacts for downstream traceability", async () => {
		...
		await writeFile(
			join(specsDir, "deep-interview-my-feature.md"),
			"# Deep Interview Spec\n",
		);
		...
		const stage = createRalplanStage({
			executor: {
				async draft() {
					...
					await writeFile(
						join(plansDir, "test-spec-runtime.md"),
						"# Runtime Tests\n",
					);
					return {
						summary: "drafted",
						planPath: prdPath,
						artifacts: { runtimeDrafted: true },
					};
				},
	...
	it("fails runtime handoff when consensus approves but test spec does not match selected PRD", async () => {
		const stage = createRalplanStage({
			executor: {
				async draft() {
					...
					await writeFile(
						join(plansDir, "test-spec-old.md"),
						"# Stale runtime tests\n",
					);
		...
		const result = await stage.run(
			makeCtx({ task: "live ralplan mismatched artifacts" }),
		);
		const artifacts = result.artifacts as Record<string, unknown>;
		...
		assert.equal(artifacts.planningComplete, false);
		assert.equal(
			(artifacts.ralplanConsensusGate as { complete?: boolean }).complete,
			true,
		);
	});
	...
	it("fails runtime handoff when consensus approves but required planning artifacts are missing", async () => {
		...
		const result = await stage.run(
			makeCtx({ task: "live ralplan no artifacts" }),
		);
		const artifacts = result.artifacts as Record<string, unknown>;
	...
	it("fails runtime handoff when Critic has not approved after Architect", async () => {
		...
		assert.equal(
			result.error,
			"ralplan_consensus_not_reached_after_1_iterations",
		);
		assert.deepEqual(artifacts.ralplanConsensusGate, {
			complete: false,
			sequence: ["architect-review", "critic-review"],
...
describe("Team Exec Stage", () => {
	...
	it("derives the team-exec task from a relative latest approved PRD handoff path", async () => {
		...
		try {
			...
			assert.equal(result.status, "completed");
			const descriptor = (result.artifacts as Record<string, unknown>)
				.teamDescriptor as Record<string, unknown>;
			const instruction = (result.artifacts as Record<string, unknown>)
			...
			assert.equal(
				typeof (descriptor.staffingPlan as Record<string, unknown>)
					.staffingSummary,
				"string",
			);
			assert.equal(runtimeCliInput.task, "Execute zeta handoff");
			assert.deepEqual(
				runtimeCliInput.approvedExecution,
				descriptor.approvedExecution,
			);
	...
	it("skips newer incomplete runtime drafts when selecting the approved team handoff", async () => {
		...
		try {
			...
			assert.doesNotMatch(
				instruction,
				new RegExp(escapeRegExp(incompleteDraftPath)),
			);
			assert.deepEqual(
				runtimeCliInput.approvedExecution,
				descriptor.approvedExecution,
	...
	it("reuses baseline-only approved handoffs for team-exec", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-plan-only.md"),
			"# Plan-only test spec\n",
		);
		await writeFile(
			join(plansDir, "repo-context-plan-only.md"),
			"Baseline repo summary may reach workers.\n",
		);
		...
		try {
			...
			assert.equal(
				(
					runtimeCliInput.decompositionMetadata as
						| Record<string, unknown>
						| undefined
	...
	it("ignores obsolete context-pack markers when a matching test spec baseline exists", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-issue-nonready.md"),
			"# Nonready test spec\n",
		);
	...
	it("derives the team-exec task from the latest ralplan draft when numeric PRD slugs sort lexically out of order", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-issue-9.md"),
			"# Issue 9 test spec\n",
		);
		await writeFile(
		...
		await writeFile(
			join(plansDir, "test-spec-issue-10.md"),
			"# Issue 10 test spec\n",
		);
	...
	it("fails closed when latestPlanPath has ambiguous team launch hints", async () => {
		...
		await writeFile(
			join(plansDir, "test-spec-ambiguous-team-hint.md"),
			"# Test spec\n",
		);
	...
	describe("buildTeamInstruction", () => {
		it("builds correct runtime-cli instruction", () => {
			...
			assert.match(
				instruction,
				new RegExp(
					escapeRegExp(join(packageRoot(), "dist", "team", "runtime-cli.js")),
				),
			);
			assert.doesNotMatch(
				instruction,
				new RegExp(
					escapeRegExp(join("/tmp/test", "dist", "team", "runtime-cli.js")),
				),
			);
			...
			assert.equal(
				(runtimeCliInput.decompositionMetadata as Record<string, unknown>)
		...
		it("keeps Windows instructions free of POSIX launch comments", () => {
			...
			const encodedPayload =
				instruction.match(/--input-json-base64\s+([A-Za-z0-9_-]+)/)?.[1] ?? "";

			assert.match(
				instruction,
				/^"[^"]+"\s+"[^"]*runtime-cli\.js"\s+--input-json-base64\s+[A-Za-z0-9_-]+/,
			);
		...
		it("preserves approved DAG handoff tasks and metadata in the runtime-cli payload", async () => {
			...
			await writeFile(
				join(plansDir, "test-spec-demo.md"),
				"# Demo Test Spec\n",
			);
			await writeFile(
				join(plansDir, "team-dag-demo.json"),
				JSON.stringify({
					schema_version: 1,
					nodes: [
						{
...
describe("Ralph Verify Stage", () => {
	...
	it("includes team-exec artifacts in verification context", async () => {
		...
		const descriptor = (result.artifacts as Record<string, unknown>)
			.verifyDescriptor as Record<string, unknown>;
		const execArtifacts = descriptor.executionArtifacts as Record<
			string,
			unknown
		>;
	...
	it("preserves legacy verification context precedence over ralplan artifacts", async () => {
		...
		const descriptor = (result.artifacts as Record<string, unknown>)
			.verifyDescriptor as Record<string, unknown>;
		assert.deepEqual(descriptor.executionArtifacts, {
			teamDescriptor: { task: "completed work" },
		});
	});
	...
	describe("buildRalphInstruction", () => {
		it("includes max iterations in instruction", () => {
			const staffingPlan = buildFollowupStaffingPlan(
				"ralph",
				"verify feature",
				["architect", "executor", "test-engineer"],
			);
		...
		it("still emits a launch instruction for long task descriptions", () => {
			...
			const staffingPlan = buildFollowupStaffingPlan("ralph", longTask, [
				"architect",
				"executor",
				"test-engineer",
			]);
...
describe("Explicit Legacy Ralph Stage", () => {
	...
	it("uses ralplan artifacts as the primary explicit legacy Ralph execution input", async () => {
		const result = await createRalphStage().run(
			makeCtx({
				artifacts: {
					ralplan: { plan: "approved plan" },
...
describe("Default Autopilot Ultragoal Stage Adapters", () => {
	...
	it("creates a deep-interview descriptor and instruction", async () => {
		...
		assert.match(
			buildDeepInterviewInstruction("clarify me"),
			/^\$deep-interview /,
		);
	});
	...
	it("creates an ultragoal descriptor with explicit team condition", async () => {
		...
		const result = await stage.run(
			makeCtx({ artifacts: { ralplan: { plan: "approved" } } }),
		);
		const artifacts = result.artifacts as Record<string, unknown>;
		...
		assert.match(
			artifacts.team_condition as string,
		...
		assert.doesNotMatch(
			buildUltragoalInstruction("execute me"),
			/Native subagent support is unavailable/,
		);
		assert.match(
			buildUltragoalInstruction("execute me", {
	...
	it("emits unsupported native subagent guidance for explicit unsupported ultragoal evidence", async () => {
		...
		const instruction = buildUltragoalInstruction("execute me", {
			nativeSubagentSupport,
		});
		...
		const result = await stage.run(
			makeCtx({
				artifacts: {
					ralplan: { native_subagent_support: nativeSubagentSupport },
				},
			}),
		);
		const artifacts = result.artifacts as Record<string, unknown>;
		assert.doesNotMatch(
			artifacts.instruction as string,
			/Conductor mode contract:/,
		);
		assert.match(artifacts.instruction as string, /multi_agent_v1_unavailable/);

		const forged = await stage.run(
			makeCtx({
				artifacts: {
					ralplan: {
		...
		assert.match(
			forgedArtifacts.instruction as string,
			/Conductor mode contract:/,
		);
	});

	it("creates an ultraqa gate that fails closed without evidence and can record clean skips", async () => {
		const missingEvidence = await createUltraqaStage().run(
			makeCtx({
				artifacts: {
					ultragoal: { tests: "passed" },
					"code-review": { review_verdict: { clean: true } },
				},
			}),
		);
		const missingArtifacts = missingEvidence.artifacts as Record<
			string,
			unknown
		>;
		const missingVerdict = missingArtifacts.qa_verdict as Record<
			string,
			unknown
		>;
		assert.equal(missingVerdict.clean, false);
		assert.equal(
			missingArtifacts.return_to_ralplan_reason,
			"UltraQA evidence missing; fail closed and return to ralplan.",
		);

		const skipped = await createUltraqaStage({
			skipped: true,
			summary: "Docs-only change; QA not applicable.",
		}).run(makeCtx());
		const skippedArtifacts = skipped.artifacts as Record<string, unknown>;
		const skippedVerdict = skippedArtifacts.qa_verdict as Record<
			string,
			unknown
		>;
...
describe("Code Review Stage", () => {
	...
	it("creates a code-review stage that fails closed without review evidence", async () => {
		...
		const result = await stage.run(
			makeCtx({ artifacts: { ultragoal: { tests: "passed" } } }),
		);
		const artifacts = result.artifacts as Record<string, unknown>;
		const descriptor = artifacts.codeReviewDescriptor as Record<
			string,
			unknown
		>;
		...
		assert.equal(
			artifacts.return_to_ralplan_reason,
	...
	it("marks explicit approve and clear review evidence as clean", async () => {
		...
		const result = await stage.run(
			makeCtx({ artifacts: { ralph: { tests: "passed" } } }),
		);
		const artifacts = result.artifacts as Record<string, unknown>;
		const verdict = artifacts.review_verdict as Record<string, unknown>;
