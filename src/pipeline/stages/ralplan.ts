...
export function createRalplanStage(
	options: CreateRalplanStageOptions = {},
): PipelineStage {
	return {
		name: "ralplan",
		...
		canSkip(ctx: StageContext): boolean {
			...
			return (
				isPlanningComplete(planningArtifacts) &&
		...
		async run(ctx: StageContext): Promise<StageResult> {
			...
			try {
				if (options.executor) {
					...
					return {
						status:
							runtimeResult.status === "completed" && consensusComplete
								? "completed"
								: "failed",
						artifacts: {
...
function hasReviewLoopContext(artifacts: Record<string, unknown>): boolean {
	if (
		typeof artifacts.return_to_ralplan_reason === "string" &&
		artifacts.return_to_ralplan_reason.trim() !== ""
	) {
		return true;
	}
