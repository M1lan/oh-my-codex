...
} from "../../team/repo-aware-decomposition.js";
...
function resolveRequestedTask(
	...
	const ralplanTask =
		typeof ralplanArtifacts?.task === "string"
			? ralplanArtifacts.task.trim()
			: "";
...
function matchingTestSpecPathsForPrd(
	...
	return artifacts.testSpecPaths.filter(
		(testSpecPath) =>
			planningArtifactSlug(testSpecPath, /^test-?spec-(?<slug>.*)\.md$/i) ===
			prdSlug,
	);
}
...
function resolvePlanningPrdPath(
	...
	if (isAbsolute(normalizedRawPath)) {
		...
		const matchedPath =
			artifacts.prdPaths.find((candidatePath) =>
				sameFilePath(candidatePath, resolvedPath),
			) ?? null;
	...
	const matchedPath =
		artifacts.prdPaths.find((candidatePath) => {
			...
			const repoRelativePath = normalizePlanningRelativePath(
				relative(repoRoot, candidatePath),
			);
			const plansRelativePath = normalizePlanningRelativePath(
				relative(artifacts.plansDir, candidatePath),
			);
			return (
...
function readRuntimeLatestPlanningSelection(
	...
	const drafts = Array.isArray(ralplanArtifacts?.drafts)
		? ralplanArtifacts.drafts
		: [];
	for (let index = drafts.length - 1; index >= 0; index -= 1) {
		...
		const draftPlanPath =
			typeof draftRecord.planPath === "string"
				? draftRecord.planPath.trim()
				: "";
		if (!draftPlanPath) continue;
		const resolvedDraftPlanPath = resolvePlanningPrdPath(
			artifacts,
			cwd,
			draftPlanPath,
		).matchedPath;
		const testSpecPaths = resolvedDraftPlanPath
			? matchingTestSpecPathsForPrd(artifacts, resolvedDraftPlanPath)
			: [];
...
function readLatestApprovedPlanningSelection(artifacts: PlanningArtifacts): {
	prdPath: string | null;
...
function resolveApprovedTeamPlanPath(
	...
	const resolvedLatestPlanPath = resolvePlanningPrdPath(
		artifacts,
		cwd,
		latestPlanPath,
	);
	...
	if (!resolvedLatestPlanPath.matchedPath) {
		throw new Error(
			`team_exec_approved_handoff_missing:${resolvedLatestPlanPath.fallbackPath}`,
		);
	}
	...
	const selection =
		readRuntimeLatestPlanningSelection(artifacts, cwd, ralplanArtifacts) ??
	...
	if (selectedPrdPath !== resolvedLatestPlanPath.matchedPath) {
		throw new Error(
			`team_exec_approved_handoff_stale:${resolvedLatestPlanPath.matchedPath}:${selectedPrdPath}`,
		);
	}
...
function resolveApprovedTeamLaunchFromPlanPath(
	...
	const approvedPlanPath = resolveApprovedTeamPlanPath(
		cwd,
		latestPlanPath,
		ralplanArtifacts,
	);
	...
	return {
		...
		approvedExecution: buildApprovedTeamExecutionBinding(
...
function resolveTeamExecLaunch(
	...
	const latestPlanPath =
		typeof ralplanArtifacts?.latestPlanPath === "string"
			? ralplanArtifacts.latestPlanPath.trim()
			: "";
	if (!latestPlanPath) {
		return { task: requestedTask, approvedExecution: null };
	}
	return resolveApprovedTeamLaunchFromPlanPath(
		ctx.cwd,
		latestPlanPath,
		ralplanArtifacts,
	);
...
function quoteShellArg(value: string, platform: NodeJS.Platform): string {
	return platform === "win32"
		? quoteWindowsCmdArg(value)
		: quotePosixShellArg(value);
}

function buildTeamRuntimeCliLaunchInput(
	descriptor: TeamExecDescriptor,
): TeamRuntimeCliLaunchInput {
...
export function createTeamExecStage(
	...
	return {
		...
		async run(ctx: StageContext): Promise<StageResult> {
			...
			try {
				const ralplanArtifacts =
					typeof ctx.artifacts["ralplan"] === "object" &&
					ctx.artifacts["ralplan"] !== null
...
export function buildTeamInstruction(
	...
	const encodedInput = Buffer.from(
		JSON.stringify(runtimeCliInput),
		"utf-8",
	).toString("base64url");
