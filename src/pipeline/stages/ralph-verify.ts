 ...
 */
export function createRalphVerifyStage(
	...
	return {
		...
		async run(ctx: StageContext): Promise<StageResult> {
			...
			try {
				...
				const executionArtifactKeys = options.executionArtifactKeys ?? [
					"ralph",
					"team-exec",
				];
				const executionArtifacts = pickFirstArtifact(
					ctx.artifacts,
					executionArtifactKeys,
				);
 ...
 */
export function buildRalphInstruction(
...
export function createRalphStage(
	options: RalphVerifyStageOptions = {},
): PipelineStage {
	return createRalphVerifyStage({
		...options,
		stageName: "ralph",
		executionArtifactKeys: options.executionArtifactKeys ?? [
			"ralplan",
			"team-exec",
		],
	});
