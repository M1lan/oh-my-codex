...
export function createUltraqaStage(
	options: UltraqaStageOptions = {},
): PipelineStage {
	return {
		name: "ultraqa",
		...
		async run(ctx: StageContext): Promise<StageResult> {
			...
			const descriptor: UltraqaDescriptor = {
				...
				ultragoalArtifacts:
					(ctx.artifacts.ultragoal as Record<string, unknown> | undefined) ??
					{},
				codeReviewArtifacts:
					(ctx.artifacts["code-review"] as
						| Record<string, unknown>
						| undefined) ?? {},
				instruction: buildUltraqaInstruction(ctx.task),
			};
			const hasQaEvidence =
				options.clean !== undefined || options.skipped !== undefined;
			...
			const verdict: UltraqaVerdict = {
				...
				summary:
					...
					(hasQaEvidence
						? clean
							? "UltraQA gate clean."
							: "UltraQA found issues; return to ralplan."
				...
				...(skipped
					? { reason: options.summary ?? "UltraQA explicitly skipped." }
					: {}),
			};
