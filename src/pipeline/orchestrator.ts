...
const DOCUMENTED_HOST_CONSENSUS_RECEIPT_UNAVAILABLE =
...
export async function runPipeline(
	...
	const updatePipelineState = (
		updates: Parameters<typeof updateAutopilotPipelineState>[0],
	...
	if (
		...
		const modeState = await startMode(
			MODE_NAME,
			config.task,
	...
	for (let i = 0; i < config.stages.length; i++) {
		...
		if (stage.canSkip?.(ctx)) {
			...
			if (stage.name === "ralplan") {
				const materialized = enforceRalplanHostReceiptBlocker(
					await stage.run(ctx),
					stage.name,
				);
			...
			if (Object.keys(skippedArtifacts).length > 0) {
				...
				Object.assign(handoffArtifactsByStage, {
					[stage.name]: skippedArtifacts,
				});
			}
			...
			await updatePipelineState({
				...
				handoff_artifacts: normalizeHandoffArtifactKeys(
					handoffArtifactsByStage,
				),
			} as Partial<PipelineModeStateExtension>);
		...
		if (result.artifacts) {
			...
			Object.assign(handoffArtifactsByStage, {
				[stage.name]: result.artifacts,
			});
		}

		const resultArtifacts = result.artifacts as Record<string, unknown>;
		const reviewVerdict =
			stage.name === "code-review" ? resultArtifacts.review_verdict : undefined;
		const qaVerdict =
			stage.name === "ultraqa" ? resultArtifacts.qa_verdict : undefined;
		const returnToRalplanReason =
			stage.name === "code-review" || stage.name === "ultraqa"
				? (resultArtifacts.return_to_ralplan_reason as
						| string
						| null
						| undefined)
				: undefined;
		const reviewIsNotClean =
		...
		const handoffArtifacts = normalizeHandoffArtifactKeys(
			handoffArtifactsByStage,
		);

		// Persist stage result
		await updatePipelineState({
			current_phase: shouldReturnToRalplan
				? "ralplan"
				: result.status === "completed"
					? stage.name
			...
			...(stage.name === "ultraqa"
				...
				: {}),
			pipeline_stage_index: shouldReturnToRalplan
				? findStageIndex(config.stages, "ralplan")
				: i,
			pipeline_stage_results: { ...stageResults },
		} as Partial<PipelineModeStateExtension>);
...
export async function readPipelineState(
	...
	return {
		...
		pipeline_stage_results: state.pipeline_stage_results as Record<
			string,
			StageResult
		>,
		pipeline_max_ralph_iterations:
			state.pipeline_max_ralph_iterations as number,
		...
		return_to_ralplan_reason: state.return_to_ralplan_reason as
			| string
			| null
			| undefined,
		handoff_artifacts: state.handoff_artifacts as
...
function normalizeHandoffArtifactKeys(
	...
	for (const [key, value] of Object.entries(artifacts)) {
		...
		if (key === "ralplan" && value && typeof value === "object") {
			...
			const gate =
				ralplanArtifacts.ralplanConsensusGate ??
				ralplanArtifacts.ralplan_consensus_gate;
			if (gate) {
...
function isRalplanHostReceiptBlocked(
	stageName: string,
	artifacts: Record<string, unknown>,
): boolean {
	if (stageName !== "ralplan") return false;
	const gate =
		artifacts.ralplanConsensusGate ?? artifacts.ralplan_consensus_gate;
	if (!gate || typeof gate !== "object") return false;

	const gateRecord = gate as Record<string, unknown>;
	return (
		gateRecord.blockedReason ===
			"documented_host_consensus_receipt_unavailable" ||
		gateRecord.blocked_reason ===
			"documented_host_consensus_receipt_unavailable"
	);
}
function enforceRalplanHostReceiptBlocker(
	result: StageResult,
	stageName: string,
): StageResult {
	...
	const artifacts =
		previousError &&
		previousError !== DOCUMENTED_HOST_CONSENSUS_RECEIPT_UNAVAILABLE
			? {
					...result.artifacts,
					documented_host_consensus_receipt_diagnostic: {
						prior_error: previousError,
					},
				}
			: result.artifacts;
	return {
...
function validateConfig(config: PipelineConfig): void {
	...
	if (config.maxRalphIterations != null) {
		if (
			!Number.isInteger(config.maxRalphIterations) ||
			config.maxRalphIterations <= 0
		) {
