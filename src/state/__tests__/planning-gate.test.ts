...
omx state write --input '{"mode":"autopilot","active":true,"current_phase":"ralplan","state":{"deep_interview_gate":{"status":"complete","rationale":"done"}}}' --json`;
		...
		assert.equal(
			isImplementationToolCall({ tool_name: "Bash", tool_input: command }),
			true,
		);
	});
...
omx state write --input '{"mode":"autopilot","active":true,"current_phase":"ralplan"}' --json`;
	...
	it("does not classify Read, Glob, Grep, or safe Bash as implementation tools", () => {
		...
		assert.equal(
			isImplementationToolCall({ tool_name: "Bash", tool_input: "git status" }),
			false,
		);
		assert.equal(
			isImplementationToolCall({ tool_name: "Bash", tool_input: "npm test" }),
			false,
		);
		assert.equal(
			isImplementationToolCall({ tool_name: "Bash", tool_input: "ls -la" }),
			false,
		);
	});
...
describe("planning gate: bypass planning gate phrase + TTL", () => {
	it("detects bypass planning gate phrase case-insensitively", () => {
		assert.equal(
			containsBypassPlanningGatePhrase("please bypass planning gate for now"),
			true,
		);
		assert.equal(
			containsBypassPlanningGatePhrase("BYPASS PLANNING GATE"),
			true,
		);
		assert.equal(
			containsBypassPlanningGatePhrase("Bypass Planning Gate please"),
			true,
		);
	...
	it("allows implementation tools within TTL after bypass", () => {
		...
		const gateState: PlanningGateState = {
			downstream_authority: "plan_then_execute",
			bypass_planning_gate_until: new Date(
	...
	it("isPlanningGateBypassActive returns false for empty or invalid bypass timestamps", () => {
		assert.equal(
			isPlanningGateBypassActive({ downstream_authority: "plan_then_execute" }),
			false,
		);
		assert.equal(
			isPlanningGateBypassActive({
		...
		assert.equal(
			isPlanningGateBypassActive({
				downstream_authority: "plan_then_execute",
				bypass_planning_gate_until: "not-a-date",
			}),
			false,
		);
	});
...
describe("planning gate: execute_now downstream authority", () => {
	it("allows all tools when downstream_authority is execute_now", () => {
		...
		assert.equal(
			evaluatePreToolUseGate({ tool_name: "Edit" }, gateState, false).allowed,
			true,
		);
		assert.equal(
			evaluatePreToolUseGate({ tool_name: "Write" }, gateState, false).allowed,
			true,
		);
		assert.equal(
			evaluatePreToolUseGate(
	...
	it("allows all tools when no gate state exists", () => {
		assert.equal(
			evaluatePreToolUseGate({ tool_name: "Edit" }, null, false).allowed,
			true,
		);
		assert.equal(
			evaluatePreToolUseGate({ tool_name: "Write" }, undefined, false).allowed,
			true,
		);
	});
});
...
describe("regression: explicit $ralplan top-level entry path is unaffected", () => {
	...
	it("planning gate does not interfere with workflow transitions", () => {
		...
		const readDecision = evaluatePreToolUseGate(
			{ tool_name: "Read" },
			gateState,
			false,
		);
