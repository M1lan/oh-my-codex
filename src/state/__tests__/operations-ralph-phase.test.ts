...
describe("state operations Ralph phase contract", () => {
	...
	it("rejects terminal Ralph phase when active=true", async () => {
		...
		try {
			...
			assert.match(
				body.error || "",
				/terminal Ralph phases require active=false/i,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
			...
			const response = await executeStateOperation("state_write", {
				...
				completion_audit: {
					...
					prompt_to_artifact_checklist: [
						"requirement mapped to committed source changes",
					],
	...
	it("accepts complete Ralph state with repo-relative completion-audit artifact", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ralph-complete-audit-path-"),
		);
		try {
			await writeFile(
				join(wd, "audit.json"),
				JSON.stringify({
					...
					verification_evidence: [
						"node --test dist/state/__tests__/operations-ralph-phase.test.js exited 0",
					],
				}),
				"utf-8",
			);
