...
async function withOmxRootEnv<T>(
	root: string,
	run: () => Promise<T>,
): Promise<T> {
...
function ralplanConsensusGate(
	...
	const architectThread =
		threadOverrides.architect ??
		(provenanceKind === "native_subagent"
			? "thread-architect"
			: "exec-architect");
	const criticThread =
		threadOverrides.critic ??
		(provenanceKind === "native_subagent" ? "thread-critic" : "exec-critic");
	return {
		required: true,
		complete: true,
...
describe("state operations directory initialization", () => {
	...
	it("prefers active ultragoal artifacts over stale inactive mode state", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-ultragoal-stale-state-"),
		);
		try {
			...
			const statuses =
				(
					statusResponse.payload as {
						statuses?: Record<
							string,
							{ active?: boolean; phase?: string; source?: string }
						>;
					}
				).statuses || {};
			assert.equal(statuses.ultragoal?.active, true);
	...
	it("bootstraps tmux-hook from the current tmux pane for mutating state operations", async () => {
		...
		try {
			...
			const tmuxConfig = JSON.parse(
				await readFile(tmuxHookConfig, "utf-8"),
			) as {
	...
	it("normalizes terminal deep-interview snapshots by releasing stale locks", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-di-terminal-normalize-"),
		);
		try {
			...
			const questionEnforcement = readBody.question_enforcement as Record<
				string,
				unknown
			>;
	...
	it("lists active modes from the explicit session scope without leaking a sibling Ralph session", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-foreign-ralph-scope-"),
		);
		try {
			const currentSessionDir = join(
				wd,
				".omx",
				"state",
				"sessions",
	...
	it("isolates same workflow state across explicit session ids when starting and clearing one session", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-same-workflow-isolation-"),
		);
		try {
			...
			const sessionACanonicalPath = join(
				wd,
				".omx",
				"state",
				"sessions",
			...
			const sessionAStateBefore = JSON.parse(
				await readFile(sessionAStatePath, "utf-8"),
			) as Record<string, unknown>;
			const sessionACanonicalBefore = JSON.parse(
				await readFile(sessionACanonicalPath, "utf-8"),
			) as Record<string, unknown>;
			...
			assert.deepEqual(
				JSON.parse(await readFile(sessionAStatePath, "utf-8")),
				sessionAStateBefore,
			);
			assert.deepEqual(
				JSON.parse(await readFile(sessionACanonicalPath, "utf-8")),
				sessionACanonicalBefore,
			);
	...
	it("serializes concurrent state_write calls per mode file and preserves merged fields", async () => {
		...
		try {
			...
			const state = JSON.parse(await readFile(filePath, "utf-8")) as Record<
				string,
				unknown
			>;
			for (let i = 0; i < 16; i++) {
	...
	it("does not report a legacy root mode active after clearing the current session scope", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-clear-root-fallback-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			assert.equal(
				existsSync(join(sessionDir, "deep-interview-state.json")),
				true,
			);
			assert.equal(
				existsSync(join(stateDir, "deep-interview-state.json")),
				true,
			);
	...
	it("all_sessions clear removes session-only canonical workflow state", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-all-sessions-session-only-"),
		);
		try {
			...
			await writeFile(
				...
				JSON.stringify(
					{ active: true, mode: "ralph", current_phase: "executing" },
					null,
					2,
				),
			);
			...
			assert.equal(
				existsSync(join(sessionDir, "skill-active-state.json")),
				false,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("does not list a mode active when terminal canonical visibility contradicts an active detail state", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-terminal-canonical-wins-"),
		);
		try {
	...
	it("uses the implicit current session canonical state when filtering list-active", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-terminal-canonical-implicit-"),
		);
		try {
	...
	it("rejects a completed ralplan state transition when only local review lifecycle evidence is present", async () => {
		...
		try {
			...
			await writeFile(
				...
				JSON.stringify(staleSkillState, null, 2),
			);
			await writeFile(
				join(sessionDir, "skill-active-state.json"),
				JSON.stringify(staleSkillState, null, 2),
			...
			const rootRalplan = JSON.parse(
				await readFile(join(stateDir, "ralplan-state.json"), "utf-8"),
			) as Record<string, unknown>;
	...
	it("allows ralplan unsupported native non-clean recovery without tracker-backed consensus", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-ralplan-unsupported-recovery-"),
		);
		try {
	...
	it("rejects ralplan clean complete without tracker-backed native consensus after unsupported recovery support", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-ralplan-clean-still-strict-"),
		);
		try {
			...
			assert.match(
				String((response.payload as { error?: unknown }).error ?? ""),
				/Cannot complete ralplan cleanly while native subagent support is unavailable/,
			);
			const state = JSON.parse(
				await readFile(join(sessionDir, "ralplan-state.json"), "utf-8"),
			) as Record<string, unknown>;
	...
	it("rejects ralplan clean complete with unsupported evidence even when native consensus is valid", async () => {
		const wd = await mkdtemp(
			join(
				tmpdir(),
				"omx-state-ops-ralplan-clean-unsupported-valid-consensus-deny-",
			),
		);
		try {
	...
	it("rejects ralplan clean complete with handoff-artifact unsupported evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-ralplan-clean-handoff-unsupported-deny-"),
		);
		try {
	...
	it("rejects ralplan clean complete with nested handoff-artifact unsupported evidence", async () => {
		const wd = await mkdtemp(
			join(
				tmpdir(),
				"omx-state-ops-ralplan-clean-nested-handoff-unsupported-deny-",
			),
		);
		try {
	...
	it("rejects ralplan clean complete with handoff-root unsupported evidence", async () => {
		const wd = await mkdtemp(
			join(
				tmpdir(),
				"omx-state-ops-ralplan-clean-handoff-root-unsupported-deny-",
			),
		);
		try {
	...
	it("rejects ralplan clean status alias with unsupported evidence even when native consensus is valid", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-ralplan-status-alias-unsupported-deny-"),
		);
		try {
	...
	it("activates deep-interview when terminal Team detail outranks foreign legacy mirrors", async () => {
		await withStateRootEnv({}, async () => {
			const wd = await mkdtemp(
				join(tmpdir(), "omx-state-ops-stale-team-transition-"),
			);
			try {
				const stateDir = join(wd, ".omx", "state");
				await mkdir(stateDir, { recursive: true });
				...
				await writeFile(
					join(stateDir, "skill-active-state.json"),
					JSON.stringify(foreignSkillState, null, 2),
				);
				await writeFile(
				...
				const beforeList = await executeStateOperation("state_list_active", {
					workingDirectory: wd,
				});
				const beforeTeam = await executeStateOperation("state_read", {
					workingDirectory: wd,
					mode: "team",
				});
				...
				const payload = responsePayload<{ success: boolean; path: string }>(
					response,
				);
				...
				assert.deepEqual(
					JSON.parse(await readFile(join(stateDir, "run-state.json"), "utf-8")),
					runState,
				);
				assert.equal(
					existsSync(
				...
				const afterList = await executeStateOperation("state_list_active", {
					workingDirectory: wd,
				});
				assert.deepEqual(afterList.payload, {
					active_modes: ["deep-interview", "run"],
				});
			} finally {
				await rm(wd, { recursive: true, force: true });
			}
	...
	it("fails closed without mutating root state when ralplan terminalization sees a foreign session pointer", async () => {
		...
		try {
			...
			assert.equal(response.isError, true);
			assert.deepEqual(response.payload, {
				error: WRITABLE_STATE_SCOPE_ERRORS.unusableSession,
			});
			const rootRalplan = JSON.parse(
				await readFile(join(stateDir, "ralplan-state.json"), "utf-8"),
			) as Record<string, unknown>;
	...
	it("reconciles a stale-dead selected pointer with an explicit exact-current OMX_SESSION_ID (issue #3272)", async () => {
		...
		try {
			...
			assert.equal(
				payload.path,
				join(stateDir, "sessions", "sess-current", "ultragoal-state.json"),
			);
	...
	it("denies unsupported overlaps without writing the requested mode state", async () => {
		...
		try {
			...
			assert.match(
				String((denied.payload as { error?: string }).error || ""),
	...
	it("does not reject planning writes from stale detail-only execution state", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-stale-detail-rollback-"),
		);
		try {
	...
	it("rejects standalone ralplan writes while preserving active Autopilot supervisor state", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ralplan-child-"),
		);
		try {
	...
	it("rejects standalone ralplan writes from detail-only active Autopilot supervisor state", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-detail-only-ralplan-child-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
				...
				assert.match(
					String((denied.payload as { error?: string }).error || ""),
					/Cannot write ralplan: autopilot is already active\./,
				);
				assert.match(
					String((denied.payload as { error?: string }).error || ""),
					/Execution-to-planning rollback auto-complete is not allowed\./,
	...
	it("lets canonical ralplan authority override stale detail-only Autopilot state", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-canonical-ralplan-stale-autopilot-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("allows Autopilot itself to enter the supervised ralplan child phase", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-child-phase-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
					...
					JSON.stringify(
						{
							...
							state: {
								deep_interview_gate: {
									status: "complete",
									rationale:
										"Requirements clarified and ready for consensus planning.",
								},
								handoff_artifacts: {
									deep_interview: {
	...
	it("denies Autopilot direct deep-interview to ultragoal skip without deep-interview evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-direct-di-skip-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
				...
				assert.match(
					...
					/Cannot transition ralplan -> ultragoal|Unsupported|cannot/i,
				);
				const state = JSON.parse(
					await readFile(join(sessionDir, "autopilot-state.json"), "utf-8"),
				) as Record<string, unknown>;
	...
	it("denies Autopilot deep-interview completion before the ralplan gate", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-di-complete-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
				...
				assert.match(
					String((response.payload as { error?: string }).error || ""),
					/Cannot complete Autopilot before ralplan gate/i,
				);
				const state = JSON.parse(
					await readFile(join(sessionDir, "autopilot-state.json"), "utf-8"),
				) as Record<string, unknown>;
	...
	it("denies Autopilot direct deep-interview to ultragoal skip even with deep-interview evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-direct-di-complete-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
					...
					JSON.stringify(
						{
							...
							state: {
								deep_interview_gate: {
									status: "complete",
									rationale:
										"Deep-interview is complete, but ralplan consensus is not.",
								},
								handoff_artifacts: {
	...
	it("denies Autopilot deep-interview to ralplan self-write when only a satisfied question exists", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-child-phase-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("denies Autopilot waiting-for-user to ralplan self-write while the deep-interview question is unresolved", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-waiting-question-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
					...
					JSON.stringify(
						{
							...
							state: {
								...
								deep_interview_gate: {
									status: "complete",
									rationale:
										"Stale completion gate must not bypass an unresolved question.",
								},
							},
						},
						null,
				...
				assert.match(
					String((response.payload as { error?: string }).error || ""),
					/question obligation is still pending/i,
				);
				const state = JSON.parse(
					await readFile(join(sessionDir, "autopilot-state.json"), "utf-8"),
				) as Record<string, unknown>;
	...
	it("denies Autopilot handoff when the next state omits a still-pending deep-interview question", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-omitted-question-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
				...
				const response = await executeStateOperation("state_write", {
					...
					state: {
						deep_interview_gate: {
							status: "complete",
							rationale:
								"Replacement state must not erase an unanswered question obligation.",
						},
						handoff_artifacts: {
	...
	it("ignores stale standalone deep-interview question state for Autopilot supervisor handoff", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ignore-standalone-di-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
					...
					JSON.stringify(
						{
							...
							state: {
								...
								handoff_artifacts: {
									deep_interview: {
										summary: "Autopilot-owned handoff is ready.",
									},
								},
							},
						},
	...
	it("denies Autopilot satisfied nested question handoff without a record-backed question id", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-satisfied-question-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
					...
					JSON.stringify(
						{
							...
							state: {
								...
								deep_interview_gate: {
									status: "complete",
									rationale:
										"Question satisfaction must be backed by an answered record.",
								},
							},
						},
						null,
				...
				assert.match(
					String((response.payload as { error?: string }).error || ""),
					/lacks same-session answered omx question record/i,
				);
				const state = JSON.parse(
					await readFile(join(sessionDir, "autopilot-state.json"), "utf-8"),
				) as Record<string, unknown>;
	...
	it("allows Autopilot handoff when next state satisfies a previously pending question", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-next-question-satisfied-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
				...
				const response = await executeStateOperation("state_write", {
					...
					state: {
						...
						deep_interview_gate: {
							status: "complete",
							rationale:
								"The answered question resolves the CLI output policy.",
						},
						handoff_artifacts: {
							deep_interview: {
								summary: "Ready for ralplan after answered question.",
							},
						},
					},
				});
	...
	it("allows Autopilot deep-interview to ralplan handoff with required valid execution contract strides", async () => {
		for (const stride of ["task", "deliverable", "milestone"] as const) {
			const wd = await mkdtemp(
				join(tmpdir(), `omx-state-ops-autopilot-execution-contract-${stride}-`),
			);
			try {
				await withOmxRootEnv(wd, async () => {
	...
	it("allows partial Autopilot ralplan handoff writes when a required execution contract is already persisted", async () => {
		const wd = await mkdtemp(
			join(
				tmpdir(),
				"omx-state-ops-autopilot-execution-contract-partial-write-",
			),
		);
		try {
			await withOmxRootEnv(wd, async () => {
					...
					JSON.stringify(
						{
							...
							state: {
								deep_interview_gate: {
									status: "complete",
									rationale:
										"The persisted interview artifact already defines the milestone contract.",
								},
								handoff_artifacts: {
									deep_interview: {
										summary:
											"Ready for ralplan with a persisted milestone execution contract.",
										execution_contract_required: true,
				...
				assert.deepEqual(
					(
						(state.state as Record<string, unknown>)
							.handoff_artifacts as Record<string, unknown>
					).deep_interview,
					{
						summary:
							"Ready for ralplan with a persisted milestone execution contract.",
	...
	it("denies Autopilot deep-interview handoff when execution contract is required but missing or invalid", async () => {
		for (const [caseName, deepInterviewHandoff] of Object.entries({
			...
			legacyPhaseEnum: {
				summary:
					"Legacy phase enum must not be accepted as an execution stride.",
				execution_contract_required: true,
				execution_contract: {
			...
			const wd = await mkdtemp(
				join(
					tmpdir(),
					`omx-state-ops-autopilot-execution-contract-deny-${caseName}-`,
				),
			);
			try {
				await withOmxRootEnv(wd, async () => {
					...
					const response = await executeStateOperation("state_write", {
						...
						state: {
							deep_interview_gate: {
								status: "complete",
								rationale:
									"The interview is complete but the required contract is not valid.",
							},
							handoff_artifacts: {
	...
	it("preserves Autopilot legacy behavior when execution contract is absent or not required", async () => {
		const wd = await mkdtemp(
			join(
				tmpdir(),
				"omx-state-ops-autopilot-execution-contract-not-required-",
			),
		);
		try {
			await withOmxRootEnv(wd, async () => {
				...
				const response = await executeStateOperation("state_write", {
					...
					state: {
						deep_interview_gate: {
							status: "complete",
							rationale:
								"No execution contract was required for this legacy handoff.",
						},
						handoff_artifacts: {
							deep_interview: {
	...
	it("honors all documented execution contract required marker locations and runtime aliases", async () => {
		...
		for (const [caseName, topLevelPatch, nestedPatch, handoffPatch] of [
			[
				"gate",
				{},
				{ deep_interview_gate: { execution_contract_required: true } },
				{},
			],
			...
			const wd = await mkdtemp(
				join(
					tmpdir(),
					`omx-state-ops-autopilot-execution-contract-marker-${caseName}-`,
				),
			);
			try {
				await withOmxRootEnv(wd, async () => {
					...
					const response = await executeStateOperation("state_write", {
						...
						state: {
							...
							deep_interview_gate: {
								...
								...((
									nestedPatch as {
										deep_interview_gate?: Record<string, unknown>;
									}
								).deep_interview_gate ?? {}),
							},
							handoff_artifacts: {
	...
	it("denies stale valid execution contracts from masking an invalid next-state contract", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-execution-contract-precedence-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
					...
					JSON.stringify(
						{
							...
							state: {
								handoff_artifacts: {
									deep_interview: {
										summary:
											"Stale current-state contract must not rescue nextState.",
										execution_contract_required: true,
										execution_contract: validExecutionContract("milestone"),
									},
								},
				...
				const response = await executeStateOperation("state_write", {
					...
					state: {
						...
						deep_interview_gate: {
							status: "complete",
							rationale:
								"Resulting next state carries an invalid higher-priority contract.",
						},
						handoff_artifacts: {
							deep_interview: {
								summary:
									"Valid handoff contract must not mask invalid direct/nested contract.",
								execution_contract_required: true,
								execution_contract: validExecutionContract("milestone"),
							},
						},
	...
	it("supports direct execution contract compatibility while rejecting invalid handoff contracts", async () => {
		for (const [caseName, handoffPatch, shouldAllow] of [
			...
			const wd = await mkdtemp(
				join(
					tmpdir(),
					`omx-state-ops-autopilot-execution-contract-${caseName}-handoff-`,
				),
			);
			try {
				await withOmxRootEnv(wd, async () => {
					...
					const response = await executeStateOperation("state_write", {
						...
						state: {
							...
							deep_interview_gate: {
								status: "complete",
								rationale:
									"A compatibility direct contract may satisfy a marker, but invalid handoff data fails first.",
							},
							handoff_artifacts: {
								deep_interview: {
									summary:
										"Handoff marker requires the handoff contract to be valid too.",
									execution_contract_required: true,
									...handoffPatch,
								},
							},
					...
					assert.equal(
						state.current_phase,
						shouldAllow ? "ralplan" : "deep-interview",
					);
				});
			} finally {
				await rm(wd, { recursive: true, force: true });
	...
	it("applies required execution contract validation to explicit Autopilot deep-interview skip gates", async () => {
		for (const [caseName, deepInterviewHandoff, shouldAllow] of [
			[
				"missing",
				{
					summary: "Skip is authorized, but contract is missing.",
					execution_contract_required: true,
				},
				false,
			],
			...
			const wd = await mkdtemp(
				join(
					tmpdir(),
					`omx-state-ops-autopilot-execution-contract-skip-${caseName}-`,
				),
			);
			try {
				await withOmxRootEnv(wd, async () => {
						...
						JSON.stringify(
							{
								...
								state: {
									deep_interview_gate: {
										...
										skip_reason:
											"User explicitly authorized skipping deep-interview for this bounded follow-up.",
	...
	it("allows Autopilot deep-interview to ralplan self-write with explicit user-authorized skip evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-child-phase-skip-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("resolves Autopilot satisfied question evidence under OMX_TEAM_STATE_ROOT", async () => {
		const root = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-team-question-"),
		);
		const previousOmxRoot = process.env.OMX_ROOT;
		const previousOmxStateRoot = process.env.OMX_STATE_ROOT;
		const previousTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
				...
				JSON.stringify(
					{
						...
						answers: [
							{ question_id: "q-1", index: 0, answer: "clarified scope" },
						],
					},
					null,
					2,
				...
				JSON.stringify(
					{
						...
						state: {
							deep_interview_gate: {
								status: "complete",
								rationale:
									"The answered question resolves the execution boundary.",
							},
						},
					},
					null,
	...
	it("denies Autopilot direct ralplan to code-review skip without native consensus evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-direct-ralplan-skip-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("denies Autopilot ralplan completion before the ultragoal gate", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ralplan-complete-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("allows Autopilot ralplan unsupported native non-clean recovery", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ralplan-unsupported-recovery-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("denies Autopilot ralplan to ultragoal self-write with codex_exec consensus evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ralplan-native-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("denies Autopilot ralplan to ultragoal when unsupported native evidence is present", async () => {
		const wd = await mkdtemp(
			join(
				tmpdir(),
				"omx-state-ops-autopilot-ralplan-unsupported-ultragoal-deny-",
			),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("denies Autopilot ralplan to ultragoal with unsupported evidence even when native consensus is valid", async () => {
		const wd = await mkdtemp(
			join(
				tmpdir(),
				"omx-state-ops-autopilot-ralplan-unsupported-valid-consensus-deny-",
			),
		);
		try {
			await withOmxRootEnv(wd, async () => {
				const sessionId =
					"sess-autopilot-ralplan-unsupported-valid-consensus-deny";
				const sessionDir = join(wd, ".omx", "state", "sessions", sessionId);
				await mkdir(sessionDir, { recursive: true });
				await writeNativeSubagentTracking(wd, sessionId);
				await writeFile(
	...
	for (const { lane, architectVerdict, criticVerdict } of [
		...
		it(`denies Autopilot ralplan to ultragoal self-write when ${lane} verdict is iterate despite complete consensus flag`, async () => {
			const wd = await mkdtemp(
				join(tmpdir(), `omx-state-ops-autopilot-ralplan-${lane}-iterate-deny-`),
			);
			try {
				await withOmxRootEnv(wd, async () => {
					...
					(
						consensusGate.ralplan_architect_review as Record<string, unknown>
					).verdict = architectVerdict;
					(
						consensusGate.ralplan_critic_review as Record<string, unknown>
	...
	it("explains when native ralplan reviews are not present in subagent tracking", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ralplan-native-missing-tracker-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
				...
				assert.match(
					error,
					/official host consensus receipt verifier is unavailable/i,
				);
			});
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("denies Autopilot ralplan to ultragoal self-write when native reviews reuse one subagent thread", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ralplan-same-thread-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("denies legacy Autopilot planning to ultragoal without ralplan consensus evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-legacy-planning-gate-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("denies Autopilot implementation-phase completion before the code-review gate", async () => {
		for (const phase of ["ultragoal", "rework", "team", "ralph"]) {
			const wd = await mkdtemp(
				join(tmpdir(), `omx-state-ops-autopilot-${phase}-complete-deny-`),
			);
			try {
				await withOmxRootEnv(wd, async () => {
						...
						JSON.stringify(
							{
								...
								current_phase: phase,
								state: {
									handoff_artifacts: { ultragoal: { verification: "passed" } },
								},
							},
							null,
							2,
					...
					const response = await executeStateOperation("state_write", {
						...
						state: {
							review_verdict: {
								...
								clean: true,
								artifact_path: ".omx/reviews/code-review.json",
							},
							qa_verdict: {
								stage: "ultraqa",
								clean: true,
								skipped: false,
								url: "https://github.com/Yeachan-Heo/oh-my-codex/actions/runs/1",
							},
						},
					});
					...
					assert.match(
						String((response.payload as { error?: string }).error || ""),
						/Cannot complete Autopilot before code-review gate/i,
					);
					const state = JSON.parse(
						await readFile(join(sessionDir, "autopilot-state.json"), "utf-8"),
					) as Record<string, unknown>;
	...
	it("denies Autopilot implementation-phase skip directly to ultraqa", async () => {
		for (const phase of ["ultragoal", "rework", "team", "ralph"]) {
			const wd = await mkdtemp(
				join(tmpdir(), `omx-state-ops-autopilot-${phase}-ultraqa-skip-deny-`),
			);
			try {
				await withOmxRootEnv(wd, async () => {
					...
					assert.match(
						String((response.payload as { error?: string }).error || ""),
						/Cannot skip Autopilot code-review gate/i,
					);
					const state = JSON.parse(
						await readFile(join(sessionDir, "autopilot-state.json"), "utf-8"),
					) as Record<string, unknown>;
	...
	it("denies Autopilot code-review completion before the ultraqa gate", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-code-review-complete-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
					...
					JSON.stringify(
						{
							...
							state: {
								handoff_artifacts: {
									code_review: { source: "native-subagent" },
								},
								review_verdict: {
									recommendation: "APPROVE",
									architectural_status: "CLEAR",
				...
				assert.match(
					String((response.payload as { error?: string }).error || ""),
					/Cannot complete Autopilot before ultraqa gate/i,
				);
				const state = JSON.parse(
					await readFile(join(sessionDir, "autopilot-state.json"), "utf-8"),
				) as Record<string, unknown>;
	...
	it("allows Autopilot code-review REQUEST_CHANGES to enter implementation rework", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-review-rework-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("replaces stale blocking review state when Autopilot completes with clean latest evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-clean-clears-stale-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
					...
					JSON.stringify(
						{
							...
							return_to_ralplan_reason:
								"Earlier code-review BLOCK required fixes.",
							handoff_artifacts: {
								code_review: {
									stage: "code-review",
									recommendation: "REQUEST_CHANGES",
									...
									clean: false,
									artifact_path: ".omx/reviews/stale-block.json",
								},
								ultraqa: null,
							},
							state: {
				...
				const cleanReview = {
					...
					architectural_status: "CLEAR",
					clean: true,
					artifact_path: ".omx/reviews/code-review-cycle-2.json",
				};
				const cleanQa = {
					stage: "ultraqa",
					clean: true,
					skipped: false,
					url: "https://github.com/Yeachan-Heo/oh-my-codex/actions/runs/2864",
				};
				const response = await executeStateOperation("state_write", {
					workingDirectory: wd,
					session_id: sessionId,
				...
				const handoffArtifacts = nestedState.handoff_artifacts as Record<
					string,
					unknown
				>;
				assert.equal(state.active, false);
	...
	it("denies Autopilot ultraqa completion without clean review and QA evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ultraqa-complete-evidence-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
				...
				assert.match(
					String((response.payload as { error?: string }).error || ""),
					/without clean code-review and ultraqa verdict evidence/i,
				);
				const state = JSON.parse(
					await readFile(join(sessionDir, "autopilot-state.json"), "utf-8"),
				) as Record<string, unknown>;
	...
	it("denies Autopilot implementation and code-review terminalization via inactive ultraqa phase", async () => {
		for (const phase of [
			...
			const wd = await mkdtemp(
				join(
					tmpdir(),
					`omx-state-ops-autopilot-${phase}-inactive-ultraqa-deny-`,
				),
			);
			try {
				await withOmxRootEnv(wd, async () => {
					...
					await writeFile(
						join(sessionDir, "autopilot-state.json"),
						JSON.stringify(
							{ active: true, mode: "autopilot", current_phase: phase },
							null,
							2,
						),
					);
					...
					assert.match(
						String((response.payload as { error?: string }).error || ""),
						/Cannot (complete|skip) Autopilot/i,
					);
					const state = JSON.parse(
						await readFile(join(sessionDir, "autopilot-state.json"), "utf-8"),
					) as Record<string, unknown>;
	...
	it("denies Autopilot ultraqa completion when review and QA provenance are swapped", async () => {
		const cases = [
			{
				...
				review_verdict: {
					...
					clean: true,
					artifact_path:
						".omx/state/autopilot-state.json#pipeline_stage_results.ultraqa.artifacts.qa_verdict",
				},
				qa_verdict: {
					stage: "code-review",
					clean: true,
					skipped: false,
					artifact_path:
						".omx/state/autopilot-state.json#pipeline_stage_results.code-review.artifacts.review_verdict",
				},
			},
			{
			...
			},
			{
				...
				review_verdict: {
					...
					architectural_status: "CLEAR",
					clean: true,
					artifact_path: ".omx/ultraqa/qa-verdict.json",
				},
				qa_verdict: {
					stage: "ultraqa",
					clean: true,
					skipped: false,
					artifact_path: ".omx/qa/qa-verdict.json",
				},
			},
			{
			...
			},
			{
				...
				review_verdict: {
					...
					architectural_status: "CLEAR",
					clean: true,
					artifact_path: ".omx/evidence/shared.json",
				},
				qa_verdict: {
					stage: "ultraqa",
					clean: true,
		...
		for (const testCase of cases) {
			const wd = await mkdtemp(
				join(
					tmpdir(),
					`omx-state-ops-autopilot-ultraqa-${testCase.name}-deny-`,
				),
			);
			try {
				await withOmxRootEnv(wd, async () => {
					...
					await writeFile(
						join(sessionDir, "autopilot-state.json"),
						JSON.stringify(
							{ active: true, mode: "autopilot", current_phase: "ultraqa" },
							null,
							2,
						),
					);
	...
	it("denies Autopilot ultraqa completion with self-attested clean verdicts but no durable provenance", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ultraqa-self-attested-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("denies Autopilot ultraqa skipped completion without durable QA provenance", async () => {
		const wd = await mkdtemp(
			join(
				tmpdir(),
				"omx-state-ops-autopilot-ultraqa-skipped-no-provenance-deny-",
			),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("denies Autopilot completion from an unknown active phase", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-unknown-phase-complete-deny-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
				...
				const response = await executeStateOperation("state_write", {
					...
					state: {
						...
						qa_verdict: {
							...
							clean: true,
							skipped: false,
							url: "https://github.com/Yeachan-Heo/oh-my-codex/actions/runs/5",
						},
					},
				});
	...
	it("denies Autopilot completion from an unknown active phase when persisted state omits mode", async () => {
		const wd = await mkdtemp(
			join(
				tmpdir(),
				"omx-state-ops-autopilot-unknown-phase-no-mode-complete-deny-",
			),
		);
		try {
			await withOmxRootEnv(wd, async () => {
				...
				const response = await executeStateOperation("state_write", {
					...
					state: {
						...
						qa_verdict: {
							...
							clean: true,
							skipped: false,
							url: "https://github.com/Yeachan-Heo/oh-my-codex/actions/runs/6",
						},
					},
				});
	...
	it("does not persist user-supplied trustedPipelineProgress from Autopilot state_write", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-trusted-field-strip-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("allows Autopilot state_write cancellation from gated phases without clean review and QA evidence", async () => {
		for (const phase of [
			...
			const wd = await mkdtemp(
				join(tmpdir(), `omx-state-ops-autopilot-${phase}-cancel-allow-`),
			);
			try {
				await withOmxRootEnv(wd, async () => {
	...
	it("allows Autopilot ultraqa completion with clean review and QA evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ultraqa-complete-allow-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("allows Autopilot ultraqa skipped completion with reason and durable QA provenance", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ultraqa-skipped-allow-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("denies Autopilot ralplan to ultragoal self-write with tracker-backed native lifecycle evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-autopilot-ralplan-native-allow-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("fails closed when canonical deep-interview is active but mode state is missing", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-missing-deep-interview-state-"),
		);
		try {
			await withOmxRootEnv(wd, async () => {
	...
	it("does not auto-complete existing workflow state when tracked write validation fails", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-validate-before-transition-"),
		);
		try {
			...
			await writeFile(
				join(sessionDir, "ralplan-state.json"),
				JSON.stringify(
					{ active: true, mode: "ralplan", current_phase: "planning" },
					null,
					2,
				),
			);
	...
	it("keeps session-scoped tracked state writable after root-state parse fallback on resume", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-resume-root-fallback-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
	...
	function installScopeTakeover(
		...
		__setWritableStateScopeTestHooksForTests({
			beforeScopeRevalidation: async (event) => {
				...
				if (event.commitOrdinal === expected[0] || event.site === expected[1]) {
					throw new Error(
						`takeover drift: expected (${expected[0]}, ${expected[1]}), got (${event.commitOrdinal}, ${event.site})`,
					);
				}
			},
		});
	...
	async function seedWritableScope(
		wd: string,
		sessionId = "sess-current",
	): Promise<{ stateDir: string; sessionId: string }> {
		const stateDir = getBaseStateDir(wd);
		await mkdir(join(stateDir, "sessions", sessionId), { recursive: true });
		await writeFile(
			join(stateDir, "session.json"),
			JSON.stringify({ session_id: sessionId, cwd: wd, state_root: stateDir }),
		);
	...
	it("T5 rejects a state_write takeover at its first commit", async () => {
		...
		try {
			...
			assert.equal(response.isError, true);
			assert.deepEqual(response.payload, {
				error: WRITABLE_STATE_SCOPE_ERRORS.scopeChangedDuringWrite,
			});
			assert.equal(
				existsSync(
					join(stateDir, "sessions", sessionId, "autoresearch-state.json"),
				),
				false,
			);
		} finally {
	...
	it("T7 records state_clear commits and rejects its native-stop takeover", async () => {
		...
		try {
			...
			await writeFile(
				join(stateDir, "sessions", sessionId, "native-stop-state.json"),
				JSON.stringify({ sessions: { [sessionId]: {} } }),
			);
			await executeStateOperation("state_write", {
				workingDirectory: wd,
			...
			const rejected = await executeStateOperation("state_clear", {
				workingDirectory: wd,
				mode: "autoresearch",
			});
	...
	it("T15 fails loudly when a takeover ordinal-site pair drifts", async () => {
		...
		try {
			...
			assert.match(
				error,
				/takeover drift: expected \(2, mode.primary\), got \(1, mode.primary\)/,
			);
			assert.notEqual(
				error,
				WRITABLE_STATE_SCOPE_ERRORS.scopeChangedDuringWrite,
			);
		} finally {
			__setWritableStateScopeTestHooksForTests({});
			await rm(wd, { recursive: true, force: true });
	...
	it("T14 revalidates the transition source-mode detail commit", async () => {
		...
		try {
			...
			await updateModeState(
				"deep-interview",
				{ deep_interview_gate: { status: "complete", rationale: "ready" } },
				wd,
			);
			const events: WritableEvent[] = [];
			__setWritableStateScopeTestHooksForTests({
			...
			await writeFile(
				...
				JSON.stringify({
					active: true,
					active_skills: [
						{ skill: "deep-interview", active: true, session_id: sessionId },
					],
				}),
			);
			await writeFile(
			...
			installScopeTakeover(
				stateDir,
				[1, "transition.source-mode-detail"],
				JSON.stringify({
					session_id: "sess-replacement",
					cwd: wd,
					state_root: stateDir,
			...
			await assert.rejects(
				...
				new Error(WRITABLE_STATE_SCOPE_ERRORS.scopeChangedDuringWrite),
			);
			assert.equal(
				await readFile(
					join(stateDir, "sessions", sessionId, "deep-interview-state.json"),
	...
	it("requires capturedScope before checking either terminal or non-terminal ralplan state", async () => {
		...
		try {
			...
			await assert.rejects(
				...
				new Error(
					"completeRalplanSession requires capturedScope when beforeCommit is provided",
				),
			);
			await assert.rejects(
				() =>
	...
	it("keeps mode and run-state writes pinned to session A across transient A-B-A pointer movement", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-ops-f14-pinned-run-state-"),
		);
		try {
			...
			assert.deepEqual(
				events.find(([, site]) => site === "run-state.mode-sync"),
				[
					2,
					"run-state.mode-sync",
					"write",
					join(sessionADir, "run-state.json"),
				],
			);
			assert.deepEqual(
				events.find(([, site]) => site === "mode.primary"),
				[
					1,
					"mode.primary",
					"write",
					join(sessionADir, "autoresearch-state.json"),
				],
			);
			assert.equal(existsSync(join(sessionADir, "run-state.json")), true);
			assert.equal(
				existsSync(join(sessionADir, "autoresearch-state.json")),
				true,
			);
			assert.equal(existsSync(join(sessionBDir, "run-state.json")), false);
			assert.equal(
				existsSync(join(sessionBDir, "autoresearch-state.json")),
				false,
			);
		} finally {
			__setWritableStateScopeTestHooksForTests({});
	...
	it("revalidates the root-scoped session skill-state unlink", async () => {
		...
		try {
			...
			const writeFixture = async (): Promise<void> => {
				...
				await mkdir(sessionDir, { recursive: true });
				await writeFile(
					join(stateDir, "autoresearch-state.json"),
