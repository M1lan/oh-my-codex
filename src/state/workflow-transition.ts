...
function isScriptInterpreterName(name: string): boolean {
	return /^(?:python[0-9.]?|python3(?:\.[0-9]+)?|node|nodejs|ruby|perl|php|lua|deno|bun|tsx|go|npx|npm|pnpm|yarn)$/.test(
		name,
	);
}
...
function runtimeLoadOptionOperandMatches(
	word: string,
	args: string[],
	index: number,
	cwd: string,
	...
	if (
		...
		return Boolean(
			operand && sameShellPath(resolveCommandOperand(cwd, operand), targetPath),
		);
	}
	for (const prefix of [
		"--require=",
		...
		if (word.startsWith(prefix))
			return sameShellPath(
				resolveCommandOperand(cwd, word.slice(prefix.length)),
				targetPath,
			);
	}
	return false;
...
function stdinRedirectOperandMatches(
	args: string[],
	cwd: string,
	targetPath: string,
): boolean {
	for (let index = 0; index < args.length; index += 1) {
		...
		if (word === "<") {
			...
			if (
				operand &&
				sameShellPath(resolveCommandOperand(cwd, operand), targetPath)
			)
				return true;
			index += 1;
			continue;
...
function shellScriptOperand(
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (word === "-c" || word === "--command") {
			...
			return Boolean(
				inlineCommand &&
					hasTokenizedExecutionOfPath(inlineCommand, targetPath, cwd),
			);
		}
		...
		if (word.startsWith("--init-file=") || word.startsWith("--rcfile=")) {
			if (
				sameShellPath(
					resolveCommandOperand(cwd, word.slice(word.indexOf("=") + 1)),
					targetPath,
				)
			)
				return true;
...
function scriptInterpreterOperand(
	args: string[],
	cwd: string,
	targetPath: string,
	commandName: string,
...
function shellExecutionMatches(
	words: string[],
	cwd: string,
	targetPath: string,
): boolean {
	...
	if (isShellName(commandName)) {
		return (
			stdinRedirectOperandMatches(words.slice(1), cwd, targetPath) ||
	...
	if (rawCommandName.startsWith("./") || rawCommandName.includes("/")) {
		return sameShellPath(
			resolveCommandOperand(cwd, rawCommandName),
			targetPath,
		);
	}
...
function findDirectWrapperOperandIndex(
	words: string[],
	startIndex: number,
): number | null {
...
function findTimeWrapperOperandIndex(
	words: string[],
	startIndex: number,
): number | null {
	for (let index = startIndex; index < words.length; index += 1) {
...
function findTimeoutWrapperOperandIndex(
	words: string[],
	startIndex: number,
): number | null {
...
function unwrapExecutionCommand(
	words: string[],
	cwd: string,
): { words: string[]; cwd: string } {
...
function hasTokenizedExecutionOfPath(
	command: string,
	path: string,
	initialCwd = "",
): boolean {
	...
	for (const statement of shellStatementRecords(command)) {
		...
		if (statement.closesSubshells > 0) {
			for (
				let depth = statement.subshellDepth;
				depth > statement.subshellDepth - statement.closesSubshells;
				depth -= 1
			) {
				cwds.delete(depth);
			}
...
function stdinRedirectsPlanningTmp(args: string[], cwd: string): boolean {
	for (let index = 0; index < args.length; index += 1) {
		...
		if (word === "<") {
			...
			if (
				operand &&
				isPlanningTmpShellPath(resolveCommandOperand(cwd, operand))
			)
				return true;
			index += 1;
			continue;
...
function commandStdinRedirectsPlanningTmpIntoInterpreter(
	command: string,
	initialCwd = "",
): boolean {
	const cwds = new Map<number, string>([[0, initialCwd]]);
	for (const statement of shellStatementRecords(command)) {
		...
		if (words.length > 0) {
			...
			if (cdTarget) {
				...
				if (
					(isShellName(commandName) || isScriptInterpreterName(commandName)) &&
...
function collectProtectedArtifactWritePaths(command: string): Set<string> {
	...
	const pythonPathWritePattern =
		/\bpython3?\b[\s\S]{0,520}\bPath\s*\(\s*(["'])((?:\.\/)?\.omx\/(?:context|specs|plans|tmp)\/[^"']+)\1\s*\)\s*\.\s*(?:write_text|write_bytes)\s*\(/g;
...
export function isImplementationToolCall(input: PreToolUseGateInput): boolean {
	...
	if (input.tool_name === "Bash" && typeof input.tool_input === "string") {
		return (
			DENIED_BASH_PATTERNS.some((pattern) => pattern.test(input.tool_input!)) ||
...
export function isPlanningGateBypassActive(
	...
	const raw =
		typeof state.bypass_planning_gate_until === "string"
...
function isAutoCompleteTransition(
	a: TrackedWorkflowMode,
	b: TrackedWorkflowMode,
): boolean {
...
function isEvidenceGatedAutoCompleteTransition(
	a: TrackedWorkflowMode,
	b: TrackedWorkflowMode,
): boolean {
	return EVIDENCE_GATED_AUTO_COMPLETE_TRANSITIONS.has(
		buildAutoCompleteKey(a, b),
	);
}
...
function isRollbackTransition(
	...
	return (
		PLANNING_LIKE_MODES.has(requestedMode) &&
...
export function isTrackedWorkflowMode(
	mode: string,
): mode is TrackedWorkflowMode {
...
export function evaluateWorkflowTransition(
	...
	const autoCompleteModes = currentModes.filter(
		(mode) =>
			isAutoCompleteTransition(mode, requestedMode) ||
			isEvidenceGatedAutoCompleteTransition(mode, requestedMode),
	);
	const survivableModes = currentModes.filter(
		(mode) => !autoCompleteModes.includes(mode),
	);

	if (
		autoCompleteModes.length > 0 &&
		survivableModes.every((mode) => isAllowedOverlap(mode, requestedMode))
	) {
		return {
			allowed: true,
			kind: "auto-complete",
			currentModes,
			requestedMode,
			resultingModes: normalizeTrackedModes([
				...survivableModes,
				requestedMode,
			]),
			autoCompleteModes,
			transitionMessage: buildWorkflowTransitionMessage(
				autoCompleteModes[0],
				requestedMode,
			),
		};
	...
	return {
		...
		resultingModes: currentModes,
		autoCompleteModes: [],
		...(isRollbackTransition(currentModes, requestedMode)
			? { denialReason: "rollback" as const }
			: {}),
	};
}
...
export function assertWorkflowTransitionAllowed(
	...
	throw new Error(
		buildWorkflowTransitionError(currentActiveModes, requestedMode, action),
	);
}
...
export async function readActiveWorkflowModes(
	...
	for (const mode of TRACKED_WORKFLOW_MODES) {
		const candidatePaths = await getAuthoritativeActiveStatePaths(
			mode,
			cwd,
			sessionId,
		);
		for (const candidatePath of candidatePaths) {
			...
			try {
				...
				const overlay =
					mode === "ralplan"
						? await readNeutralizedRoutingOverlay(candidatePath, "ralplan")
						: null;
