...
function isTerminalState(state: Record<string, unknown>): boolean {
	...
	if (
		inferTerminalLifecycleOutcome(state, {
...
function releaseLockValue(
	...
	return {
		value: {
			...
			release_reason:
				stringValue(value.release_reason) || "terminal_state_normalization",
		},
		changed: true,
	};
}

function releaseTerminalLocksOnRecord(
	record: Record<string, unknown>,
	releasedAt: string,
): boolean {
	let changed = false;
	for (const key of Object.keys(record)) {
		if (!LOCK_KEYS.has(key) && !POSSIBLE_APPROVAL_MARKER_KEYS.has(key))
			continue;
...
export function normalizeTerminalWorkflowState(
	...
	if (
		options.mode === "deep-interview" &&
		isRecord(next.question_enforcement)
	) {
		...
		if (status === "pending") {
			...
			const clearReason =
				phase === "completed" || phase === "complete" ? "handoff" : "abort";
			const cleared = clearDeepInterviewQuestionObligation(
				next.question_enforcement as unknown as Parameters<
