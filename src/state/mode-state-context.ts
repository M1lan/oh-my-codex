...
export function captureTmuxPaneFromEnv(
...
export function captureTmuxWindowForPane(
	pane: string,
	env: NodeJS.ProcessEnv = process.env,
): string | null {
...
function captureRalphPaneBinding(paneId: string): RalphPaneBinding | null {
	...
	if (effectProof.status !== "live" || effectProof.pid !== initialProof.pid)
		return null;
	...
	const authority = spawnPlatformCommandSync(
		...
		[
			...
			"-t",
			effectProof.paneId,
			`#{pane_id}\x1f#{pane_pid}\x1f#{session_name}\x1f#{session_id}\x1f#{window_id}\x1f#{${OMX_RALPH_PANE_OWNER_OPTION}}\x1f#{pane_current_command}\x1f#{pane_start_command}`,
		],
		{ encoding: "utf-8", stdio: ["ignore", "pipe", "pipe"] },
	).result;
	if (
		authority.error ||
		authority.status !== 0 ||
		typeof authority.stdout !== "string"
	)
		return null;
	const [
		capturedPaneId,
		...
		paneStartCommand,
		...extra
	] = authority.stdout.trim().split("\x1f");
	const panePid = Number(capturedPid);
	if (
		extra.length > 0 ||
		capturedPaneId !== effectProof.paneId ||
		panePid !== effectProof.pid ||
		!/^%[0-9]+$/.test(capturedPaneId) ||
		!Number.isSafeInteger(panePid) ||
		panePid <= 0 ||
		!isSafeForegroundCommand(sessionName) ||
		!/^\$[0-9]+$/.test(sessionId) ||
		!/^@[0-9]+$/.test(windowId) ||
		capturedOwnerId !== paneOwnerId ||
		!isSafeForegroundCommand(paneCurrentCommand) ||
		!isSafeForegroundCommand(paneStartCommand) ||
...
export function withModeRuntimeContext<T extends ModeStateContextLike>(
	...
	const ralphPaneId =
		typeof next.tmux_pane_id === "string" ? next.tmux_pane_id.trim() : "";
