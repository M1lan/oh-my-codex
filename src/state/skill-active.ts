...
function rootMirrorEntriesForCanonicalSession(
	...
	return entries.filter((entry) => {
		...
		return (
			entrySessionId.length === 0 || entrySessionId === normalizedSessionId
		);
	});
}
...
function filterSessionOnlyEntries(
	...
	const inheritedKeys = new Set(
		rootMirrorEntriesForCanonicalSession(rootEntries, sessionId).map(entryKey),
	);
	return listActiveSkills(sessionState ?? {}).filter(
		(entry) =>
...
function normalizeSkillActiveEntry(raw: unknown): SkillActiveEntry | null {
	...
	return {
		...
		skill,
		phase:
			safeString((raw as Record<string, unknown>).phase).trim() || undefined,
		active: (raw as Record<string, unknown>).active !== false,
		activated_at:
			safeString((raw as Record<string, unknown>).activated_at).trim() ||
			undefined,
		updated_at:
			safeString((raw as Record<string, unknown>).updated_at).trim() ||
			undefined,
		session_id:
			safeString((raw as Record<string, unknown>).session_id).trim() ||
			undefined,
		thread_id:
			safeString((raw as Record<string, unknown>).thread_id).trim() ||
			undefined,
		turn_id:
			safeString((raw as Record<string, unknown>).turn_id).trim() || undefined,
		owner_codex_session_id:
			safeString(
				(raw as Record<string, unknown>).owner_codex_session_id,
			).trim() || undefined,
	};
}

export function extractSessionIdFromInitializedStatePath(
	pathValue: unknown,
): string | undefined {
	const pathText = safeString(pathValue).trim();
	if (!pathText) return undefined;
	const normalized = pathText.replace(/\\/g, "/");
	const match = /(?:^|\/)sessions\/([^/]+)\/[^/]+-state\.json$/.exec(
		normalized,
	);
	return match?.[1];
}
...
function baseInitializationMatchesTargetSession(
	...
	const initializedPathSessionId = extractSessionIdFromInitializedStatePath(
		base?.initialized_state_path,
	);
	if (
		initializedPathSessionId &&
		initializedPathSessionId !== normalizedTargetSessionId
	) {
		return false;
	}
...
export function listActiveSkills(raw: unknown): SkillActiveEntry[] {
	...
	if (deduped.size === 0 && state.active === true && topLevelSkill) {
		const topLevelEntry = {
			...
			owner_codex_session_id:
				safeString(state.owner_codex_session_id).trim() || undefined,
		};
 ...
 */
export function isTransitionCanonicalStateOwned(
	raw: unknown,
	sessionId?: string,
): boolean {
	if (!raw || typeof raw !== "object") return false;
	const outerSessionId = safeString(
		(raw as SkillActiveStateLike).session_id,
	).trim();
	const normalizedSessionId = safeString(sessionId).trim();
	return normalizedSessionId
		? !outerSessionId || outerSessionId === normalizedSessionId
		: !outerSessionId;
}

export function listTransitionActiveSkills(
	raw: unknown,
	sessionId?: string,
): SkillActiveEntry[] {
	...
	if (normalizedSessionId) {
		return entries.filter(
			(entry) => safeString(entry.session_id).trim() === normalizedSessionId,
		);
	}
	return entries.filter(
		(entry) => safeString(entry.session_id).trim().length === 0,
	);
}
...
export function listSkillActiveOwnerCodexSessionIds(raw: unknown): string[] {
	...
	const owners = [
		...
		...listActiveSkills(state).map((entry) =>
			safeString(entry.owner_codex_session_id).trim(),
		),
	].filter(Boolean);
	return [...new Set(owners)];
}

export function normalizeSkillActiveState(
	raw: unknown,
): SkillActiveStateLike | null {
	if (!raw || typeof raw !== "object") return null;
	const state = raw as SkillActiveStateLike;
	const activeSkills = listActiveSkills(state);
	const primary =
		activeSkills.find(
			(entry) => entry.skill === safeString(state.skill).trim(),
		) ?? activeSkills[0];
	...
	return {
		...
		activated_at:
			safeString(state.activated_at).trim() || primary?.activated_at || "",
		updated_at:
			safeString(state.updated_at).trim() || primary?.updated_at || "",
		source: safeString(state.source).trim() || undefined,
		session_id:
			safeString(state.session_id).trim() || primary?.session_id || undefined,
		thread_id:
			safeString(state.thread_id).trim() || primary?.thread_id || undefined,
		turn_id: safeString(state.turn_id).trim() || primary?.turn_id || undefined,
		owner_codex_session_id:
			safeString(state.owner_codex_session_id).trim() ||
			primary?.owner_codex_session_id ||
			undefined,
		active_skills: activeSkills.length > 0 ? activeSkills : undefined,
	};
}

export function getSkillActiveStatePaths(
	cwd: string,
	sessionId?: string,
): {
	rootPath: string;
...
export async function readSkillActiveState(
	path: string,
...
async function writeSkillActiveStateCopiesToPaths(
	...
	if (sessionPath) {
		...
		await beforeCommit?.({
			site: "skill-active.session-copy",
			kind: "write",
			path: sessionPath,
		});
...
export function tracksCanonicalWorkflowSkill(
	mode: string,
): mode is CanonicalWorkflowSkill {
	return (CANONICAL_WORKFLOW_SKILLS as readonly string[]).includes(mode);
}

export async function syncCanonicalSkillStateForMode(
	...
	const sessionOnlyEntries = normalizedSessionId
		...
		: [];
	const visibleEntries = listTransitionActiveSkills(
	...
	if (
		!normalizedSessionId &&
		existingRoot &&
		!isTransitionCanonicalStateOwned(existingRoot)
	)
		return;
	...
	const applyEntriesToState = (
		...
		const primaryEntry =
			entries.find((entry) => entry.skill === primarySkill) ?? entries[0];
		return {
			...inheritedBase,
			version: 1,
			...
			activated_at:
				primaryEntry?.activated_at ||
				safeString(inheritedBase.activated_at).trim() ||
				nowIso,
			updated_at: nowIso,
			source: safeString(inheritedBase.source).trim() || source,
			session_id:
				primaryEntry?.session_id ||
				safeString(inheritedBase.session_id).trim() ||
				undefined,
			thread_id:
				primaryEntry?.thread_id ||
				safeString(inheritedBase.thread_id).trim() ||
				undefined,
			turn_id:
				primaryEntry?.turn_id ||
				safeString(inheritedBase.turn_id).trim() ||
				undefined,
			owner_codex_session_id:
				primaryEntry?.owner_codex_session_id ||
				safeString(inheritedBase.owner_codex_session_id).trim() ||
				undefined,
	...
	if (normalizedSessionId) {
		const nextSessionEntries = sessionOnlyEntries.filter(
			(entry) => entry.skill !== mode,
		);
		if (active) {
			nextSessionEntries.push({
				...
				active: true,
				activated_at:
					sessionOnlyEntries.find((entry) => entry.skill === mode)
						?.activated_at || nowIso,
				updated_at: nowIso,
				...
				owner_codex_session_id:
					safeString(ownerCodexSessionId).trim() || undefined,
			});
		}

		const nextSessionRootEntries = rootEntries.filter(
			(entry) =>
				!(
					entry.skill === mode &&
					safeString(entry.session_id).trim() === normalizedSessionId
		...
		const nextRootEntries = allRootEntries.filter(
			(entry) =>
				!(
					entry.skill === mode &&
					safeString(entry.session_id).trim() === normalizedSessionId
	...
	const rootScopedEntries = rootEntries.filter(
		(entry) => safeString(entry.session_id).trim().length === 0,
	);
	...
	const nextRootScopedEntries = rootScopedEntries.filter(
		(entry) => entry.skill !== mode,
	);
	if (active) {
		nextRootScopedEntries.push({
			...
			active: true,
			activated_at:
				rootScopedEntries.find((entry) => entry.skill === mode)?.activated_at ||
				nowIso,
			updated_at: nowIso,
	...
	const nextRootState = applyEntriesToState(
		existingRoot,
		nextRootEntries,
		mode,
	);
	...
	for (const candidate of sessionIds) {
		...
		const sessionOnlyEntries = filterSessionOnlyEntries(
			existingSessionState,
			rootEntries,
			sessionId,
		).filter((entry) => !(allSessions && entry.skill === mode));
		const nextVisibleRootEntries = nextRootEntries.filter(
			(entry) => safeString(entry.session_id).trim() === sessionId,
		);
		const nextSessionEntries = [
			...nextVisibleRootEntries,
			...sessionOnlyEntries,
		];

		if (nextSessionEntries.length === 0) {
			await options.beforeCommit?.({
				site: "skill-active.session-unlink",
				kind: "unlink",
				path: sessionPath,
			});
