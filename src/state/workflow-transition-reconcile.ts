...
async function assertAuthoritativeWorkflowStateReadable(
	...
	for (const mode of TRACKED_WORKFLOW_MODES) {
		const candidatePath = modeStatePathForRoot(
			mode,
			cwd,
			sessionId,
			baseStateDir,
...
async function visibleTrackedModes(
	...
	for (const mode of TRACKED_WORKFLOW_MODES) {
		const state = await readJsonIfExists(
			modeStatePathForRoot(mode, cwd, undefined, baseStateDir),
			{
				mode,
...
async function completeSourceModeState(
	...
	const candidatePaths = [
		modeStatePathForRoot(sourceMode, cwd, sessionId, baseStateDir),
	];
	...
	for (const candidatePath of candidatePaths) {
		...
		if (sourceMode === "deep-interview") {
			const nextQuestionEnforcement = clearDeepInterviewQuestionObligation(
				existing.question_enforcement as Parameters<
					typeof clearDeepInterviewQuestionObligation
				>[0],
				"handoff",
				new Date(nowIso),
			);
		...
		const runOutcomeState = applyRunOutcomeContract(nextCandidate, { nowIso })
			.state as TransitionStateLike;
		const nextState = normalizeTerminalWorkflowState(runOutcomeState, {
			mode: sourceMode,
			nowIso,
		}).state as TransitionStateLike;
		...
		await beforeCommit?.({
			site: "transition.source-mode-detail",
			kind: "write",
			path: candidatePath,
		});
	...
	if (
		sourceMode === "deep-interview" &&
		destinationMode === "ralplan" &&
...
export async function reconcileWorkflowTransition(
	...
	if (!options.currentModes) {
		await assertAuthoritativeWorkflowStateReadable(
			cwd,
			sessionId,
			baseStateDir,
		);
	...
	if (currentModes.includes("ralplan") && requestedMode !== "ralplan") {
		throw new Error(
			`Cannot transition ralplan -> ${requestedMode}: documented_host_consensus_receipt_unavailable. Official host consensus receipt verifier is unavailable.`,
		);
	}
	...
	if (!decision.allowed) {
		throw new Error(
			buildWorkflowTransitionError(currentModes, requestedMode, action),
		);
	}
