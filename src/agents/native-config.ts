...
function resolveAgentModel(
	...
	const modelOverride = getAgentModelOverride(
		agent.name,
		options.codexHomeOverride,
	);
	if (modelOverride) {
...
export function composeRoleInstructions(
	...
	const exactModel =
		normalizedResolvedModel === EXACT_GPT_5_6_TERRA_MODEL
	...
	if (metadata) {
		...
		if (options.nativeAgent === true && metadata.nativeSubagentDelegation) {
			metadataLines.push(
				`- native_subagent_delegation: ${metadata.nativeSubagentDelegation}`,
			);
		}
	}
	if (resolvedModel) {
...
export function generateStandaloneAgentToml(
	...
	if (config.modelProvider) {
		lines.push(
			`model_provider = "${escapeTomlBasicString(config.modelProvider)}"`,
		);
	}
...
export async function installNativeAgentConfigs(
	...
	const installableAgentNames = allowUncatalogedDefinitions
		...
		: getInstallableNativeAgentNames(
				catalogManifest ?? readCatalogManifest(pkgRoot),
			);
