...
describe("agents/native-config", () => {
	...
	it("transports normalized per-agent max through exact native TOML and falls back from ultra", async () => {
		const codexHome = await mkdtemp(
			join(tmpdir(), "omx-native-config-max-reasoning-"),
		);
		try {
			await writeFile(
				join(codexHome, ".omx-config.json"),
			...
			const fallbackToml = generateAgentToml(
				AGENT_DEFINITIONS.critic,
				"Critic prompt",
				{
					codexHomeOverride: codexHome,
				},
	...
	it("applies Terra guidance when agentModels resolves exact-Sol roles to gpt-5.6-terra", async () => {
		const codexHome = await mkdtemp(
			join(tmpdir(), "omx-native-config-terra-override-"),
		);
		try {
			await writeFile(
				join(codexHome, ".omx-config.json"),
			...
			const tunedToml = generateAgentToml(
	...
	it("pins planner and architect to exact gpt-5.6-sol while keeping researcher on exact Terra", () => {
		...
		for (const role of ["planner", "architect"] as const) {
			...
			assert.match(
				toml,
				/model = "gpt-5\.6-sol"/,
				`${role} should use exact gpt-5.6-sol`,
			);
			assert.match(
				toml,
				/exact gpt-5\.6-sol model/,
				`${role} should receive exact-gpt-5.6-sol guidance`,
			);
			assert.match(
				toml,
			...
			assert.match(
				...
				`${role} should record exact gpt-5.6-sol metadata`,
			);
		}
		...
		assert.match(
			researcherToml,
			/model = "gpt-5\.6-terra"/,
			"researcher should keep exact Terra",
		);
		assert.match(
			researcherToml,
			/exact gpt-5\.6-terra model/,
			"researcher should receive exact-Terra guidance",
		);
		assert.match(
			researcherToml,
			/resolved_model: gpt-5.6-terra/,
			"researcher should record exact Terra metadata",
		);
		...
		for (const role of [
			...
			assert.match(
				toml,
				/model = "gpt-5\.6-sol"/,
				`${role} should stay on configured/root gpt-5.6-sol`,
			);
			assert.doesNotMatch(
				toml,
				/model = "gpt-5\.6-terra"/,
				`${role} must not inherit exact-Terra pins`,
			);
		}
	});
	...
	it("applies exact-model Terra guidance only for resolved gpt-5.6-terra standard roles", () => {
		...
		const tunedToml = generateAgentToml(agent, prompt, {
			env: {
				OMX_DEFAULT_STANDARD_MODEL: "gpt-5.6-terra-tuned",
	...
	it("omits inherited custom provider for default Spark-lane native agents", async () => {
		...
		try {
			...
			await writeFile(
				join(codexHome, "config.toml"),
				['model = "gpt-5.6-sol"', 'model_provider = "OpenAI"', ""].join("\n"),
			);
