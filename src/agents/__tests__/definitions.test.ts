...
describe("agents/definitions", () => {
	...
	it("requires a four-valued reasoning default on every AgentDefinition", () => {
		...
		const reasoningEffortIsRequired: undefined extends AgentDefinition["reasoningEffort"]
			? false
			: true = true;
		...
		const undefinedReasoningEffort: AgentDefinition =
			invalidUndefinedDefinition;
	...
	it("keeps every built-in reasoning default unchanged", () => {
		...
		assert.deepEqual(
			Object.fromEntries(
				Object.entries(AGENT_DEFINITIONS).map(([name, agent]) => [
					name,
	...
	it("keeps the installable agent model split aligned with the OMX subagent matrix", () => {
		...
		for (const name of [
			...
			assert.equal(
				AGENT_DEFINITIONS[name].reasoningEffort,
				name === "executor" ? "medium" : "high",
			);
		}
	});
