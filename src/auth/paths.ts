...
export function resolveLiveAuthPath(
	...
	const codexHome =
		resolveCodexHomeForLaunch(cwd, env) || resolveDefaultCodexHome(home);
...
export async function ensurePrivateDir(dir: string): Promise<void> {
	...
	if (info.isSymbolicLink())
		throw new Error(`auth directory must not be a symlink: ${dir}`);
	if (!info.isDirectory())
		throw new Error(`auth path is not a directory: ${dir}`);
