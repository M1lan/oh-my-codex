...
export async function atomicWriteFile(
	...
	try {
		...
		if (process.platform !== "win32")
			await chmod(tempPath, options.mode ?? AUTH_FILE_MODE).catch(
				() => undefined,
			);
		await options.beforeRename?.(tempPath);
		await rename(tempPath, targetPath);
		if (process.platform !== "win32")
			await chmod(targetPath, options.mode ?? AUTH_FILE_MODE).catch(
				() => undefined,
			);
	} catch (err) {
...
export async function readAuthMetadata(home?: string): Promise<AuthMetadata> {
	...
	return {
		version: 1,
		currentSlot:
			typeof parsed.currentSlot === "string" ? parsed.currentSlot : undefined,
		slots: Array.isArray(parsed.slots)
			? parsed.slots
					.filter((slot): slot is AuthSlotRecord =>
						Boolean(slot && typeof slot.slot === "string"),
					)
...
export async function listSlots(home?: string): Promise<AuthSlotRecord[]> {
	const metadata = await readAuthMetadata(home);
	return metadata.slots.filter((slot) =>
		existsSync(resolveSlotPath(slot.slot, home)),
	);
}
...
export async function clearSlotExhaustion(
	slot: string,
	home?: string,
): Promise<void> {
	const metadata = await readAuthMetadata(home);
	const record = metadata.slots.find(
		(entry) => entry.slot === validateSlotName(slot),
	);
	if (record) {
		delete record.exhaustedAt;
		await writeAuthMetadata(metadata, home);
