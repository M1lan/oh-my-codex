...
export function buildRotationPlan(
	...
	if (config.rotation === "manual")
		return {
			mode: "manual",
			order: currentSlot ? [currentSlot] : available.slice(0, 1),
		};
	...
	if (!currentSlot || !available.includes(currentSlot))
		return { mode: "round-robin", order: available };
	const index = available.indexOf(currentSlot);
	return {
		mode: "round-robin",
		order: [...available.slice(index), ...available.slice(0, index)],
	};
}
...
export function nextSlotAfter(
	...
	const start =
		current && order.includes(current) ? order.indexOf(current) + 1 : 0;
