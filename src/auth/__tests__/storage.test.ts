...
describe("auth slot storage", () => {
	it("adds, lists, and uses auth slots without exposing blob contents", async () => {
		...
		try {
			...
			assert.equal(
				await readFile(resolveSlotPath("work", home), "utf-8"),
	...
	it("uses owner-only modes for auth directory and files", async () => {
		...
		try {
			...
			const slotMode =
				(await stat(resolveSlotPath("personal", home))).mode & 0o777;
	...
	it("rejects a symlinked auth directory", async () => {
		...
		try {
			...
			await assert.rejects(
				addSlotFromAuthFile("work", live, home),
	...
	it("resolves CODEX_HOME, project-scope, and default auth paths", async () => {
		...
		try {
			assert.equal(
				resolveLiveAuthPath(wd, { CODEX_HOME: join(home, "custom") }, home),
			...
			assert.equal(
				resolveLiveAuthPath(wd, {}, home),
				join(wd, ".codex", "auth.json"),
			);
			assert.equal(
				resolveLiveAuthPath(wd, { CODEX_HOME: "" }, home),
				join(wd, ".codex", "auth.json"),
			);
		} finally {
			await rm(home, { recursive: true, force: true });
