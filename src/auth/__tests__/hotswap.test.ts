...
import { type HotswapLifecycle, runAuthHotswap } from "../hotswap.js";
...
function lifecycle(
	overrides: Partial<HotswapLifecycle> = {},
): HotswapLifecycle {
	return {
...
describe("auth hotswap pointer abort lifecycle", () => {
		...
		try {
			...
			const status = await runAuthHotswap({
				...
				lifecycle: lifecycle({
					...
					preLaunch: async () => {
						throw new Error("mandatory overlay failed");
					},
					postLaunch: async () => {
						postLaunchCalls += 1;
					},
	...
	it("runs postLaunch and runtime cleanup after a later rotation useSlot failure", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-hotswap-rotation-use-slot-failure-"),
		);
		const home = join(cwd, "home");
		const runtimeHome = join(cwd, "runtime-codex-home");
