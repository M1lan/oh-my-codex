...
export function performanceGoalEvaluatorPath(
	cwd: string,
	slug: string,
): string {
...
async function writeState(
	cwd: string,
	state: PerformanceGoalState,
): Promise<void> {
	await mkdir(workflowDir(cwd, state.slug), { recursive: true });
	await writeFile(
		performanceGoalStatePath(cwd, state.slug),
...
async function appendLedger(
	cwd: string,
	slug: string,
	entry: PerformanceGoalLedgerEntry,
): Promise<void> {
	await mkdir(workflowDir(cwd, slug), { recursive: true });
	await appendFile(
		performanceGoalLedgerPath(cwd, slug),
...
export async function createPerformanceGoal(
	cwd: string,
	options: CreatePerformanceGoalOptions,
): Promise<PerformanceGoalState> {
	const objective = requireText(options.objective, "objective");
	const evaluatorCommand = requireText(
		options.evaluatorCommand,
		"evaluator command",
	);
	const evaluatorContract = requireText(
		options.evaluatorContract,
		"evaluator contract",
	);
	...
	if (!options.force && existsSync(statePath)) {
		throw new PerformanceGoalError(
			`Refusing to overwrite existing ${repoRelative(cwd, statePath)}; pass --force to recreate it.`,
		);
	}
	...
	await writeFile(
		performanceGoalEvaluatorPath(cwd, slug),
		evaluatorMarkdown(state),
	);
...
export async function readPerformanceGoal(
	cwd: string,
	slug: string,
): Promise<PerformanceGoalState> {
	...
	try {
		...
		throw new PerformanceGoalError(
			`No performance goal found at ${repoRelative(cwd, path)}. Run \`omx performance-goal create ...\` first.`,
		);
	}
	const parsed = JSON.parse(raw) as PerformanceGoalState;
	if (
		parsed.version !== 1 ||
		parsed.workflow !== "performance-goal" ||
		!parsed.evaluator?.command
	) {
		throw new PerformanceGoalError(
			`Invalid performance goal state at ${repoRelative(cwd, path)}.`,
		);
	}
	return parsed;
}

export async function startPerformanceGoal(
	cwd: string,
	slug: string,
	nowDate = new Date(),
): Promise<{ state: PerformanceGoalState; instruction: string }> {
	...
	await appendLedger(cwd, state.slug, {
		...
		status: state.status,
		message:
			"Emitted model-facing Codex goal handoff; shell command did not mutate Codex goal state",
	});
	return { state, instruction };
}

export async function checkpointPerformanceGoal(
	cwd: string,
	options: CheckpointPerformanceGoalOptions,
): Promise<PerformanceGoalState> {
	const state = await readPerformanceGoal(cwd, options.slug);
	if (state.status === "complete") {
		throw new PerformanceGoalError(
			`Performance goal ${state.slug} is already complete; create a new goal or explicitly reopen via a future workflow before recording more checkpoints.`,
		);
	}
...
export async function completePerformanceGoal(
	cwd: string,
	options: CompletePerformanceGoalOptions,
): Promise<PerformanceGoalState> {
	const state = await readPerformanceGoal(cwd, options.slug);
	if (state.lastValidation?.status !== "pass") {
		throw new PerformanceGoalError(
			"Cannot complete performance goal until evaluator validation has a passing checkpoint. Run `omx performance-goal checkpoint --status pass ...` first.",
		);
	}
...
export function buildPerformanceGoalInstruction(
	state: PerformanceGoalState,
): string {
