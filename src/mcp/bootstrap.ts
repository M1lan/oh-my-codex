...
const DUPLICATE_SIBLING_WATCHDOG_INTERVAL_ENV =
	"OMX_MCP_DUPLICATE_SIBLING_WATCHDOG_INTERVAL_MS";
const DUPLICATE_SIBLING_PRE_TRAFFIC_GRACE_ENV =
	"OMX_MCP_DUPLICATE_SIBLING_PRE_TRAFFIC_GRACE_MS";
const DUPLICATE_SIBLING_POST_TRAFFIC_IDLE_ENV =
	"OMX_MCP_DUPLICATE_SIBLING_POST_TRAFFIC_IDLE_MS";
const DUPLICATE_SIBLING_INITIAL_DELAY_ENV =
	"OMX_MCP_DUPLICATE_SIBLING_INITIAL_DELAY_MS";
const DUPLICATE_SIBLING_INITIAL_DELAY_MAX_ENV =
	"OMX_MCP_DUPLICATE_SIBLING_INITIAL_DELAY_MAX_MS";
...
export function parseWindowsProcessTable(
	...
	for (const record of records) {
		...
		const rawCommand =
			...
			processRecord.CommandLine.trim()
...
export function analyzeDuplicateSiblingState(
	...
	const matching = processes
		...
		.filter(
			(entry) =>
				extractMcpEntrypointMarker(entry.command) === currentEntrypoint,
		)
...
export function resolveDuplicateSiblingWatchdogInitialDelayMs(
	...
	config: Pick<
		LifecycleTimingConfig,
		"duplicateSiblingInitialDelayMs" | "duplicateSiblingInitialDelayMaxMs"
	>,
): number {
	...
	if (maxMs <= 0) return 0;
	return (
		stableStringHash(`${serverName}:${entrypoint ?? "unknown"}`) % (maxMs + 1)
	);
}
...
export function shouldSelfExitForDuplicateSibling(
	...
	if (
		...
		(!Number.isFinite(lastTrafficAtMs) || lastTrafficAtMs > nowMs)
	) {
		return false;
	}
...
export function shouldSelfExitForPreTrafficSiblingHardCap(
	...
	if (
		!Number.isInteger(maxSiblingsPerEntrypoint) ||
		maxSiblingsPerEntrypoint <= 0
	)
		return false;
...
export function shouldSelfExitForPostTrafficSiblingHardCap(
	...
	if (observation.newerSiblingPids.length < maxSiblingsPerEntrypoint)
		return false;
...
export function autoStartStdioMcpServer(
	...
	let duplicateSiblingInitialDelayTimer: ReturnType<typeof setTimeout> | null =
		null;
	const stopDuplicateSiblingWatchdog = (
		reason: string,
		options: { runPendingInitialScan?: boolean } = {},
	) => {
		let stopped = false;
