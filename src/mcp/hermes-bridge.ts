...
function sessionMatchesRequested(
	state: SessionState,
	sessionId: string,
): boolean {
	return (
		state.session_id === sessionId || state.native_session_id === sessionId
	);
}
...
async function sendPromptToBoundTmuxSession(
	...
	if (!rawSession || !sessionMatchesRequested(rawSession, sessionId))
		return null;
	...
	try {
		...
		if (instanceId !== rawSession.session_id) {
			throw new Error(
				`tmux_instance_mismatch:${tmuxSessionName}:${instanceId || "missing"}`,
			);
		}
		const paneSession = String(
			execTmux([
				"display-message",
				"-p",
				"-t",
		...
		if (paneSession !== tmuxSessionName) {
			throw new Error(
				`pane_session_mismatch:${tmuxPaneId}:${paneSession || "missing"}`,
			);
		}
		...
		const reason =
			message.startsWith("tmux_instance_mismatch") ||
			message.startsWith("pane_session_mismatch")
				? message
				: "tmux_send_failed";
		throw new Error(
			`unsupported_session_kind:tmux_prompt_delivery_failed:${tmuxSessionName}:${tmuxPaneId}:${reason}`,
		);
	}
	return {
...
): HermesStatusSessionSummary | null {
	if (!session) return null;
	return {
		session_id: session.session_id,
		...(optionalString(session.native_session_id)
			? { native_session_id: optionalString(session.native_session_id) }
			: {}),
		...(optionalString(session.cwd)
			? { cwd: optionalString(session.cwd) }
			: {}),
		...(optionalString(session.started_at)
			? { started_at: optionalString(session.started_at) }
			: {}),
	};
}

function projectModeStatus(
	mode: string,
	scope: string,
	state: unknown,
): HermesModeStatusSummary {
	const raw =
	...
	return {
		mode,
		scope,
		...(optionalBoolean(raw.active) !== undefined
			? { active: optionalBoolean(raw.active) }
			: {}),
		...(optionalString(raw.current_phase)
			? { phase: optionalString(raw.current_phase) }
			: {}),
		...(optionalString(raw.phase) && !optionalString(raw.current_phase)
			? { phase: optionalString(raw.phase) }
			: {}),
		...(optionalString(raw.run_outcome)
			? { run_outcome: optionalString(raw.run_outcome) }
			: {}),
		...(optionalString(raw.lifecycle_outcome)
			? { lifecycle_outcome: optionalString(raw.lifecycle_outcome) }
			: {}),
		...(optionalString(raw.updated_at)
			? { updated_at: optionalString(raw.updated_at) }
			: {}),
		...(optionalString(raw.completed_at)
...
async function summarizeCurrentSession(
	cwd: string,
	deps: HermesBridgeDeps,
): Promise<HermesSessionSummary | null> {
	...
	return {
		...
		active: true,
		source: "current",
		modes: await listModeNamesInStateDir(
			join(getBaseStateDir(cwd), "sessions", current.session_id),
		),
	};
}
...
export async function hermesListSessions(
	...
	try {
		const cwd = resolveWorkingDirectoryForState(
			normalizeString(args.workingDirectory, "workingDirectory"),
		);
...
export async function hermesReadStatus(
	...
	HermesBridgeResult<{
		session: HermesStatusSessionSummary | null;
		modes: HermesModeStatusSummary[];
	}>
> {
	try {
		const cwd = resolveWorkingDirectoryForState(
		...
		const sessionId = validateSessionId(
			normalizeString(args.session_id, "session_id"),
		);
		const rawSession = sessionId
			? null
			: await (deps.readUsableSessionState ?? readUsableSessionState)(cwd);
		const session = projectSessionStatus(rawSession);
		if (
			sessionId &&
			!existsSync(join(getBaseStateDir(cwd), "sessions", sessionId))
		) {
			return failure(
				"no_session",
				`No OMX session state directory exists for ${sessionId}`,
			);
		}
		...
		for (const ref of refs) {
			modes.push(
				projectModeStatus(ref.mode, ref.scope, await readJsonFile(ref.path)),
			);
		}
...
export async function hermesListQuestionEvents(
	...
	HermesBridgeResult<{ events: Awaited<ReturnType<typeof readQuestionEvents>> }>
> {
	try {
		const cwd = resolveWorkingDirectoryForState(
			normalizeString(args.workingDirectory, "workingDirectory"),
...
export async function hermesListQuestions(
	...
	try {
		...
		return jsonResult({
			questions: records.map(({ record }) => projectQuestion(record)),
		});
	} catch (error) {
		return failure(
			"invalid_input",
...
export async function hermesSubmitQuestionAnswer(
	...
	HermesBridgeResult<{
		question: HermesQuestionSummary;
...
> {
	try {
		...
		const questionId = normalizeString(args.question_id, "question_id", {
			required: true,
		})!;
		const payload =
			args.answers !== undefined
				...
				: { answer: args.answer };
		const { record } = await submitQuestionAnswerById(
			cwd,
			questionId,
			payload,
			{
		...
		return jsonResult({
			question: projectQuestion(record),
			answers: record.answers ?? [],
		});
	} catch (error) {
		const message = error instanceof Error ? error.message : String(error);
		if (message.includes("allow_mutation"))
			return failure("mutation_not_allowed", message);
...
> {
	try {
		...
		const sessionId = normalizeString(args.session_id, "session_id", {
			required: true,
		})!;
		...
		try {
			...
			const tmuxResult = await sendPromptToBoundTmuxSession(
				cwd,
				sessionId,
				prompt,
				deps,
		...
		const message = error instanceof Error ? error.message : String(error);
		if (message.startsWith("unsupported_session_kind"))
			return failure("prompt_not_accepted", message);
		if (message.startsWith("job_not_input_accepting"))
			return failure("prompt_not_accepted", message);
		if (message.includes("allow_mutation"))
			return failure("mutation_not_allowed", message);
		return failure("invalid_input", message);
...
> {
	try {
		...
		if (
			worktreeName &&
			(!/^[A-Za-z0-9._/-]{1,128}$/.test(worktreeName) ||
		...
		const command =
			(deps.resolveOmxCliEntryPath ?? resolveOmxCliEntryPath)({ cwd }) ?? "omx";
		const launchArgs = [
			"--tmux",
		...
		if (!child.pid)
			return failure(
				"command_failed",
				"OMX session launcher did not report a pid",
			);
		return jsonResult({
			pid: child.pid,
			command,
...
async function resolveSafeArtifactPath(
	...
	for (const prefix of SAFE_ARTIFACT_PREFIXES) {
		...
		if (rootRealPath && isInsideDirectory(rootRealPath, artifactRealPath))
...
export async function hermesListArtifacts(
	...
	HermesBridgeResult<{ artifacts: Array<{ path: string; bytes: number }> }>
> {
	try {
		const cwd = resolveWorkingDirectoryForState(
			normalizeString(args.workingDirectory, "workingDirectory"),
...
export async function hermesReadArtifact(
	...
	HermesBridgeResult<{ path: string; content: string; truncated: boolean }>
> {
	try {
		const cwd = resolveWorkingDirectoryForState(
			normalizeString(args.workingDirectory, "workingDirectory"),
		...
		const message = error instanceof Error ? error.message : String(error);
		if (message === "artifact_missing")
			return failure("artifact_missing", message);
		if (
			message.includes("artifact path") ||
			message.includes("outside working directory")
		) {
...
export async function hermesReadTail(
	...
	try {
		...
		const lines = normalizePositiveInteger(
			args.lines,
			DEFAULT_TAIL_LINES,
			MAX_TAIL_LINES,
		);
...
export async function hermesReportStatus(
	...
	HermesBridgeResult<{ path: string; report: Record<string, unknown> }>
> {
	try {
		requireMutation(args);
		const cwd = resolveWorkingDirectoryForState(
		...
		const stateDir = sessionId
			? join(getBaseStateDir(cwd), "sessions", sessionId)
			: getBaseStateDir(cwd);
