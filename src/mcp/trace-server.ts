...
function keepLastEntries(
	...
	while (
		i + 1 < entries.length &&
		compareTraceTimestamp(entries[i], entries[i + 1]) > 0
	) {
...
export async function readModeEvents(
	...
	return [
		{
			name: "trace_timeline",
			description:
				"Show chronological agent flow trace timeline. Displays turns, mode transitions, and agent activity in time order.",
			inputSchema: {
				type: "object",
				properties: {
					...
					filter: {
						...
						description:
							"Filter: all (default), turns (agent turns only), modes (mode transitions only)",
					},
		...
		},
		{
			name: "trace_summary",
			description:
				"Show aggregate statistics for agent flow trace. Includes turn counts, mode usage, token consumption, and timing.",
			inputSchema: {
				type: "object",
				properties: {
					workingDirectory: { type: "string" },
	...
	switch (name) {
		...
		case "trace_summary": {
			...
			if (firstTurn && lastTurn) {
				durationMs =
					new Date(lastTurn).getTime() - new Date(firstTurn).getTime();
			}
