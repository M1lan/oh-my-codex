	...
	return [
		{
			name: "wiki_ingest",
			description:
				"Process knowledge into wiki pages. Creates new pages or merges into existing ones.",
			inputSchema: {
				type: "object",
				properties: {
		...
		},
		{
			name: "wiki_query",
			description:
				"Search wiki pages by keywords and tags. Returns raw matches for synthesis.",
			inputSchema: {
				type: "object",
				properties: {
					query: { type: "string" },
		...
		},
		{
			name: "wiki_add",
			description:
				"Quick-add a single wiki page. Rejects overwrites; use wiki_ingest to merge.",
			inputSchema: {
				type: "object",
				properties: {
					title: { type: "string", maxLength: 200 },
		...
		},
		{
			name: "wiki_refresh",
			description:
				"Rebuild the wiki index and refresh derived metadata surfaces.",
			inputSchema: {
				type: "object",
				properties: {
					workingDirectory: { type: "string" },
...
async function handleWikiToolCallWithOptions(
	request: {
	...
	try {
		...
		switch (name) {
			...
			case "wiki_lint":
				return text(
					lintWiki(root, undefined, {
						logLint: options.logReadOperations === true,
					}),
				);
			...
			case "wiki_add": {
				...
				if (readCanonicalPage(root, slug)) {
					return errorText(
						`Page "${slug}" already exists. Use wiki_ingest to merge into it.`,
					);
				}
				const result = ingestKnowledge(root, {
					title,
			...
			case "wiki_read": {
				const page = readPage(
					root,
					normalizeWikiPageName(String(args.page || "")),
				);
