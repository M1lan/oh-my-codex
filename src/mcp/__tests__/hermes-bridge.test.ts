...
describe("Hermes MCP bridge core", () => {
	it("lists session-scoped OMX state without exposing terminal internals", async () => {
		...
		try {
			await mkdir(join(cwd, ".omx", "state", "sessions", "sess-a"), {
				recursive: true,
			});
			await writeFile(
	...
	it("projects status without leaking raw internal mode state", async () => {
		...
		try {
			...
			assert.equal(
				JSON.stringify(result).includes("prompt_to_artifact_checklist"),
				false,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("reads a bounded session-history tail without tmux scrollback", async () => {
		...
		try {
			...
			assert.equal(result.ok, true);
			assert.deepEqual(result.data?.tail, [
	...
	it("lists question events and submits bounded answers with safe leader-pane resume injection", async () => {
		...
		try {
			const { record, recordPath } = await createQuestionRecord(
				cwd,
				{
					question: "Pick one",
			...
			const listed = await hermesListQuestions({
				workingDirectory: cwd,
				session_id: "sess-q",
			});
			...
			const answeredEvents = await hermesListQuestionEvents({
				workingDirectory: cwd,
			});
			assert.equal(
				answeredEvents.data?.events.find(
	...
	it("routes selected prompts to a bound tmux session when no active exec queue accepts input", async () => {
		...
		try {
			...
			assert.equal(
				tmuxCalls.some((args) => args.includes("attach-session")),
				false,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("returns prompt_not_accepted when tmux pane delivery fails", async () => {
		...
		try {
			...
			assert.match(
				result.error ?? "",
				/unsupported_session_kind:tmux_prompt_delivery_failed:omx-detached-demo:%42:tmux_send_failed/,
			);
	...
	it("rejects tmux prompt delivery when the stored target is not a pane id", async () => {
		...
		try {
			...
			assert.match(
				result.error ?? "",
				/unsupported_session_kind:invalid_tmux_pane_binding:omx-detached-demo/,
			);
			assert.deepEqual(tmuxCalls, []);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("rejects tmux prompt delivery when the stored pane is not in the bound session", async () => {
		...
		try {
			...
			assert.match(
				result.error ?? "",
				/unsupported_session_kind:tmux_prompt_delivery_failed:omx-detached-demo:%42:pane_session_mismatch:%42:other-session/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("rejects tmux prompt delivery when the tmux instance tag does not match session state", async () => {
		...
		try {
			...
			assert.match(
				result.error ?? "",
				/unsupported_session_kind:tmux_prompt_delivery_failed:omx-detached-demo:%42:tmux_instance_mismatch:omx-detached-demo:other-session/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("returns a clear unsupported-session-kind diagnostic when no exec or tmux binding can accept prompts", async () => {
		...
		try {
			...
			assert.match(
				result.error ?? "",
				/unsupported_session_kind:no_active_exec_session_or_tmux_binding:sess-missing/,
			);
			assert.doesNotMatch(
				result.error ?? "",
	...
	it("lists and reads only safe result artifact paths", async () => {
		...
		try {
			...
			assert.deepEqual(list.data?.artifacts, [
				{ path: ".omx/plans/prd-demo.md", bytes: 14 },
			]);

			const read = await hermesReadArtifact({
				workingDirectory: cwd,
				path: ".omx/plans/prd-demo.md",
				max_bytes: 5,
			});
			...
			const rejected = await hermesReadArtifact({
				workingDirectory: cwd,
				path: "package.json",
			});
			...
			const traversal = await hermesReadArtifact({
				workingDirectory: cwd,
				path: ".omx/plans/../../package.json",
			});
	...
	it("reads large artifacts with max_bytes truncation and reports stat sizes", async () => {
		...
		try {
			...
			assert.equal(list.ok, true);
			assert.deepEqual(list.data?.artifacts, [
				{ path: ".omx/plans/large.md", bytes: content.length },
			]);

			const read = await hermesReadArtifact({
				workingDirectory: cwd,
				path: ".omx/plans/large.md",
				max_bytes: 8,
			});
			assert.equal(read.ok, true);
			assert.deepEqual(read.data, {
	...
	it("rejects safe-root artifact symlinks that resolve outside the worktree", async () => {
		...
		const outside = await mkdtemp(
			join(tmpdir(), "omx-hermes-artifact-outside-"),
		);
		try {
			await mkdir(join(cwd, ".omx", "plans"), { recursive: true });
	...
	it("rejects workdir-root candidate symlinks that would expose outside artifacts", async () => {
		...
		try {
			...
			await writeFile(
				join(outside, ".omx", "plans", "secret.md"),
				"outside via workdir symlink",
			);
	...
	it("writes a bounded Hermes coordination report", async () => {
		...
		try {
			...
			assert.match(
				result.data?.path ?? "",
				/sessions[/\\]sess-a[/\\]hermes-coordination\.json$/,
			);
