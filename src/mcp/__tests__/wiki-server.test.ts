...
describe("mcp/wiki-server module contract", () => {
	...
	it("keeps allowlisted wiki query and lint MCP calls free of durable log writes", async () => {
		...
		try {
			for (const [name, args] of [
				[
					"wiki_query",
					{ query: "caller supplied text", workingDirectory: root },
				],
				...
				assert.equal(
					"isError" in response ? response.isError : false,
					false,
					name,
				);
				assert.equal(
					existsSync(getWikiDir(root)),
					false,
					`${name} created durable wiki state`,
				);
			}
		} finally {
	...
	it("wiki_add writes canonical pages even when the same legacy slug exists", async () => {
		...
		try {
			...
			assert.match(
				await readFile(join(getWikiDir(root), "same-title.md"), "utf8"),
	...
	it("wiki_delete does not create canonical storage when deleting from legacy-only fallback", async () => {
		...
		try {
			...
			assert.match(
				...
				/Wiki page not found or reserved: legacy\.md/,
			);
