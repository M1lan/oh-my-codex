...
function armPointerTakeover(options: {
	...
	return {
		...
		hook: async (event) => {
			...
			if (
				event.commitOrdinal === options.expectedOrdinal &&
				event.site !== options.expectedSite
			) {
				throw new Error(
					`Expected takeover at ordinal ${options.expectedOrdinal} site ${options.expectedSite}; received ordinal ${event.commitOrdinal} site ${event.site}.`,
				);
			}
			if (
				event.site === options.expectedSite &&
...
describe("validateStateFileName", () => {
	...
	it("rejects traversal and path separators", () => {
		assert.throws(
			() => validateStateFileName("../evil.json"),
			/must not contain "\.\."/,
		);
		assert.throws(
			() => validateStateFileName("foo/bar.json"),
			/path separators/,
		);
		assert.throws(
			() => validateStateFileName("foo\\bar.json"),
...
describe("state paths", () => {
	it("uses explicit OMX_TEAM_STATE_ROOT before boxed roots and workingDirectory", () => {
		...
		try {
			...
			assert.equal(
				getStateDir("/tmp/source", "sess1"),
				"/tmp/explicit-team-state/sessions/sess1",
			);
			assert.equal(
				getStatePath("ralph", "/tmp/source", "sess1"),
				"/tmp/explicit-team-state/sessions/sess1/ralph-state.json",
			);
			...
			if (typeof prevStateRoot === "string")
				process.env.OMX_STATE_ROOT = prevStateRoot;
			else delete process.env.OMX_STATE_ROOT;
			if (typeof prevTeamRoot === "string")
				process.env.OMX_TEAM_STATE_ROOT = prevTeamRoot;
	...
	it("uses OMX_ROOT as boxed workspace root before workingDirectory when no team root is explicit", () => {
		...
		try {
			...
			assert.equal(
				getStateDir("/tmp/source", "sess1"),
				"/tmp/omx-box/.omx/state/sessions/sess1",
			);
			assert.equal(
				getStatePath("ralph", "/tmp/source", "sess1"),
				"/tmp/omx-box/.omx/state/sessions/sess1/ralph-state.json",
			);
			assert.deepEqual(getBaseStateDirWithSource("/tmp/source"), {
	...
	it("uses the ancestor session pointer matching OMX_SESSION_ID as the authoritative root", async () => {
		...
		try {
			...
			await writeFile(
				join(nestedState, "session.json"),
				JSON.stringify({ session_id: "sess-nested", cwd: nested }),
			);
			...
			assert.equal(
				writable.stateDir,
				join(parentState, "sessions", "sess-parent"),
			);
		} finally {
	...
	it("uses a live legacy ancestor pointer when its inferred root matches exactly", async () => {
		...
		try {
			...
			await writeFile(
				join(parentState, "session.json"),
				JSON.stringify({ session_id: "sess-legacy", cwd: parent }),
			);
	...
	it("rejects pointers whose persisted or inferred root does not own the selected base", async () => {
		...
		try {
			...
			await writeFile(
				join(selectedState, "session.json"),
				JSON.stringify({
			...
			assert.equal(
				await readSessionMetadataFromBaseStateDir(cwd, selectedState),
				undefined,
			);

			await writeFile(
				join(selectedState, "session.json"),
				JSON.stringify({ session_id: "sess-legacy-explicit", cwd }),
			);
			assert.equal(
				await readSessionMetadataFromBaseStateDir(cwd, selectedState),
				undefined,
	...
	it("fails explicitly when multiple ancestor pointers claim the same session authority", async () => {
		...
		try {
			...
			assert.throws(
				...
				new RegExp(
					`Conflicting authoritative state roots.*${parentState}.*${nestedState}|Conflicting authoritative state roots.*${nestedState}.*${parentState}`,
				),
			);
		} finally {
			await rm(parent, { recursive: true, force: true });
	...
	it("does not discover ancestor authority outside OMX_MCP_WORKDIR_ROOTS", async () => {
		...
		try {
			...
			await writeFile(
				join(parentState, "session.json"),
				JSON.stringify({ session_id: "sess-outside", cwd: parent }),
			);
	...
	it("does not select an in-allowlist state symlink that escapes the allowed root", async () => {
		...
		try {
			...
			await writeFile(
				join(outsideState, "session.json"),
				JSON.stringify({ session_id: "sess-symlink", cwd: nested }),
			);
			await symlink(
				outsideState,
				join(nested, ".omx", "state"),
	...
	it("rejects OMX_ROOT and OMX_STATE_ROOT whose appended state directory escapes the allowlist", async () => {
		...
		try {
			...
			await symlink(
				outsideState,
				join(allowed, ".omx", "state"),
				process.platform === "win32" ? "junction" : "dir",
			);
			process.env.OMX_MCP_WORKDIR_ROOTS = allowed;
			process.env.OMX_ROOT = allowed;
			assert.throws(
				() => getBaseStateDirWithSource(allowed),
				/State root .* is outside allowed roots/,
			);
	...
	it("resolveWorkingDirectoryForState normalizes Windows path on WSL/Linux when mount exists", () => {
		...
		if (existsSync("/mnt/d")) {
			assert.equal(
				resolveWorkingDirectoryForState(raw),
				"/mnt/d/SIYUAN/external/repo",
			);
		} else {
			assert.throws(
				() => resolveWorkingDirectoryForState(raw),
	...
	it("rejects NUL bytes in workingDirectory", () => {
		assert.throws(
			() => resolveWorkingDirectoryForState("bad\0path"),
			/NUL byte/,
		);
	});
	...
	it("builds global state paths", () => {
		...
		assert.equal(
			getStatePath("team", "/repo"),
			"/repo/.omx/state/team-state.json",
		);
	});

	it("builds session state paths", () => {
		assert.equal(
			getStateDir("/repo", "sess1"),
			"/repo/.omx/state/sessions/sess1",
		);
		assert.equal(
	...
	it("reads session-sensitive runtime files from the current session without root fallback when requested", async () => {
		...
		try {
			...
			await writeFile(
				...
				JSON.stringify({ session_id: "sess-current" }),
			);

			const paths = await getReadScopedStateFilePaths(
				"hud-state.json",
				wd,
				undefined,
				{
			...
			assert.deepEqual(paths, [
				join(stateDir, "sessions", "sess-current", "hud-state.json"),
			]);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("maps native Codex session aliases to the canonical OMX session id", async () => {
		...
		try {
			...
			if (typeof previousCodexSessionId === "string")
				process.env.CODEX_SESSION_ID = previousCodexSessionId;
	...
	it("resolves current session from authoritative team state root without OMX_SESSION_ID", async () => {
		...
		try {
			...
			delete process.env.OMX_SESSION_ID;
			await mkdir(join(teamStateRoot, "sessions", "sess-team-current"), {
				recursive: true,
			});
			await writeFile(
	...
	describe("writable state scope", () => {
				...
				await assert.rejects(
					...
					(error: unknown) => {
						assert.equal(
							...
							WRITABLE_STATE_SCOPE_ERRORS.unusableSession,
						);
						return true;
					},
				);
		...
		it("fails closed for an unmatched OMX_SESSION_ID while preserving explicit fork scope", async () => {
			...
			try {
				...
				await writeFile(
					join(stateDir, "session.json"),
					JSON.stringify({ session_id: "sess-canonical", cwd: wd }),
				);
				...
				await assert.rejects(
					...
					(error: unknown) => {
						assert.equal(
							(error as Error).message,
							WRITABLE_STATE_SCOPE_ERRORS.sessionBindingMismatch,
						);
						return true;
					},
				);
		...
		it("still fails closed for a stale-dead pointer without an env binding", async () => {
			...
			try {
				...
				await writeFile(
					join(stateDir, "session.json"),
					JSON.stringify({ session_id: "sess-dead", cwd: wd, pid: 8388607 }),
				);
		...
		it("fails closed for JSON-valid non-object pointers even with an env binding", async () => {
			...
			try {
				...
				for (const body of ["null", "[]", "42", '"sess-current"']) {
					...
					assert.equal(
						existsSync(join(stateDir, "sessions", "sess-current")),
						false,
					);
				}
			} finally {
		...
		it("fails closed for an identity-indeterminate pointer even with an env binding", async () => {
			...
			try {
				...
				await writeFile(
					join(stateDir, "session.json"),
					JSON.stringify({ session_id: "sess-unknown", cwd: wd, pid: 8388607 }),
				);
				process.env.OMX_SESSION_ID = "sess-current";
		...
		it("fails closed for a stale-dead pointer without cwd or state_root authority even with an env binding", async () => {
			...
			try {
				...
				await writeFile(
					join(stateDir, "session.json"),
					JSON.stringify({ session_id: "sess-dead", pid: 8388607 }),
				);
				await assert.rejects(
					() => resolveWritableStateScope(wd),
					(error: unknown) => {
...
describe("writable state scope recovery and revalidation", () => {
	it("T10 reads the selected decision snapshot and recovery stability snapshot once", async () => {
		...
		try {
			...
			assert.deepEqual(
				decisions.map(({ ordinal }) => ordinal),
				[1],
			);
			assert.deepEqual(
				stability.map(({ ordinal }) => ordinal),
				[1],
			);
		...
		try {
			...
			__setWritableStateScopeTestHooksForTests({
				beforeRecoveryReread: async () => {
					await writeFile(
						sessionPath,
						JSON.stringify({ session_id: "sess-replaced", cwd: wd }),
					);
				},
			});
			await assert.rejects(
	...
	it("fails closed when the pointer bytes change during identity-indeterminate exact-match recovery", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(join(stateDir, "sessions", "sess-indeterminate")),
				false,
			);
		} finally {
			__setWritableStateScopeTestHooksForTests({});
	...
	it("T13 rejects stale-dead recovery without selected-root authority", async () => {
		...
		try {
			...
			const cases = [
				{
					name: "missing recorded cwd",
					pointer: { session_id: "sess-dead", pid: 8388607 },
				},
				{
					...
					pointer: {
						session_id: "sess-dead",
						cwd: wd,
						state_root: join(wd, "other-root"),
						pid: 8388607,
					},
				},
				{
					name: "recorded cwd does not contain the observed cwd",
					pointer: {
						session_id: "sess-dead",
						cwd: join(wd, "other-worktree"),
						pid: 8388607,
			...
			for (const testCase of cases) {
				await writeFile(
					join(stateDir, "session.json"),
					JSON.stringify(testCase.pointer),
				);
				await assert.rejects(
					() => resolveWritableStateScope(wd),
					(error: unknown) => {
	...
	it("fails closed for identity-indeterminate pointer without selected-root authority even with an exact-match env binding", async () => {
		...
		try {
			...
			const cases = [
				{
					name: "missing recorded cwd",
					pointer: { session_id: "sess-indeterminate", pid: 8388607 },
				},
				{
					name: "different state root",
					pointer: {
	...
	it("T1 gives each writable commit revalidator a closure-local ordinal sequence", async () => {
		...
		try {
			...
			await writeFile(
				join(stateDir, "session.json"),
				JSON.stringify({ session_id: "sess-current", cwd: wd }),
			);
			...
			const options = {
				operation: "state_write" as const,
				cwd: wd,
			...
			const attempts = [
				{
					site: "mode.primary" as const,
					kind: "write" as const,
					path: join(stateDir, "mode-state.json"),
				},
				{
					site: "skill-active.root-copy" as const,
					kind: "write" as const,
					path: join(stateDir, "skill-active.json"),
				},
				{
					site: "skill-active.session-copy" as const,
					kind: "write" as const,
					path: join(stateDir, "sessions", "sess-current", "skill-active.json"),
				},
			];
	...
	it("T2 rejects a writable commit when revalidation observes a replacement scope", async () => {
		...
		try {
			...
			const takeover = armPointerTakeover({
				...
				sessionPath,
				replacementPointer: JSON.stringify({
			...
			__setWritableStateScopeTestHooksForTests({
				beforeScopeRevalidation: takeover.hook,
			});
	...
	it("T3 clears recovery and revalidation hooks and resets snapshot ordinals", async () => {
		...
		try {
			...
			__setWritableStateScopeTestHooksForTests({
				beforeRecoveryReread: async () => {
					oldCounters.recovery += 1;
				},
				beforeScopeRevalidation: async () => {
					oldCounters.revalidation += 1;
				},
				onSelectedDecisionSnapshotRead: () => {
					oldCounters.decision += 1;
				},
				onRecoveryStabilityReread: () => {
					oldCounters.stability += 1;
				},
			});
			await resolveWritableStateScope(wd);
			__setWritableStateScopeTestHooksForTests({});
			await resolveWritableStateScope(wd);
			assert.deepEqual(oldCounters, {
				recovery: 1,
				revalidation: 0,
				decision: 1,
				stability: 1,
			...
			__setWritableStateScopeTestHooksForTests({
				onSelectedDecisionSnapshotRead: ({ ordinal }) =>
					restarted.push({ decision: ordinal, stability: 0 }),
