...
describe("mcp bootstrap auto-start guard", () => {
	it("allows auto-start by default for every OMX MCP server", () => {
		for (const server of ALL_SERVERS) {
			assert.equal(
				shouldAutoStartMcpServer(server, {}),
				true,
				`${server} should auto-start by default`,
			);
		}
	});
	...
	it("disables all servers when global disable flag is set", () => {
		...
		for (const server of ALL_SERVERS) {
			assert.equal(
				shouldAutoStartMcpServer(server, env),
				false,
				`${server} should honor global disable flag`,
			);
		}
	});
...
describe("mcp parent watchdog liveness checks", () => {
	it("treats missing or root-like parent pids as gone", () => {
		assert.equal(
			isParentProcessAlive(0, () => true),
			false,
		);
		assert.equal(
			isParentProcessAlive(1, () => true),
			false,
		);
		assert.equal(
			isParentProcessAlive(Number.NaN, () => true),
			false,
		);
	});
...
describe("mcp shared stdio lifecycle contract", () => {
	it("keeps server connection immediate and duplicate process-table scans delayed", async () => {
		const src = await readFile(
			join(process.cwd(), "src/mcp/bootstrap.ts"),
			"utf8",
		);
		const connectIndex = src.indexOf("server.connect(transport)");
		const duplicateDelayIndex = src.indexOf(
			"duplicateSiblingInitialDelayTimer = setTimeout",
		);

		assert.ok(
			connectIndex > 0,
			"bootstrap should still connect the MCP transport",
		);
		assert.ok(
			duplicateDelayIndex > 0,
			"bootstrap should delay duplicate-sibling process scans",
		);
		assert.ok(
	...
	it("keeps shared stdio lifecycle wiring in bootstrap", async () => {
		...
		assert.match(
			...
			"bootstrap should own stdio transport creation",
		);
		assert.match(
			src,
			/server\.connect\(/,
			"bootstrap should own MCP server connection",
		);
		assert.match(
			src,
			/stdin/i,
			"bootstrap should react to stdin/client disconnect",
		);
		assert.match(
			src,
			/parent_gone/,
			"bootstrap should watch for the parent process disappearing",
		);
		assert.match(
			src,
			/isParentProcessAlive\(trackedParentPid\)/,
			"bootstrap should probe parent liveness directly",
		);
		assert.match(
			src,
			/process\.exit\(0\)/,
			"bootstrap should force child exit after shutdown completes",
		);
		...
		assert.match(
			src,
			/analyzeDuplicateSiblingState/,
			"bootstrap should keep duplicate sibling detection in the shared layer",
		);
		assert.match(
			src,
			/shouldSelfExitForDuplicateSibling/,
			"bootstrap should gate self-exit conservatively in the shared layer",
		);
	});
	...
	it("keeps individual server entrypoints free of duplicated raw stdio connect snippets", async () => {
		for (const { server, file } of SERVER_ENTRYPOINTS) {
			...
			assert.match(
				src,
				new RegExp(
					`autoStartStdioMcpServer\\(['\"]${server}['\"],\\s*server\\)`,
				),
...
describe("mcp duplicate sibling detection", () => {
	it("resolves deterministic bounded initial duplicate scan delays", () => {
		const stateDelay = resolveDuplicateSiblingWatchdogInitialDelayMs(
			...
			"state-server.js",
			{
				duplicateSiblingInitialDelayMs: null,
				duplicateSiblingInitialDelayMaxMs: 1000,
			},
		);
		...
		assert.notEqual(
			stateDelay,
			memoryDelay,
			"first-party servers should be staggered by default",
		);
		assert.equal(
			resolveDuplicateSiblingWatchdogInitialDelayMs(
				"state",
				"state-server.js",
				{
					duplicateSiblingInitialDelayMs: 0,
					duplicateSiblingInitialDelayMaxMs: 1000,
				},
			),
			0,
	...
	it("extracts same-entrypoint markers from command lines", () => {
		...
		assert.equal(
			extractMcpEntrypointMarker(
				"node C:\\\\tmp\\\\oh-my-codex\\\\dist\\\\mcp\\\\trace-server.ts",
			),
			"trace-server.ts",
		);
		assert.equal(
		...
		assert.equal(
			extractMcpEntrypointMarker(
				"node /tmp/dist/cli/omx.js mcp-serve code-intel",
			),
	...
	it("stops duplicate sibling watchdog after stdio traffic only on Windows", () => {
		...
		assert.equal(
			shouldStopDuplicateSiblingWatchdogAfterTraffic("linux"),
			false,
		);
		assert.equal(
			shouldStopDuplicateSiblingWatchdogAfterTraffic("darwin"),
			false,
		);
	});
	...
	it("runs one pending startup scan before stopping Windows duplicate watchdog", () => {
		assert.equal(
			shouldRunDuplicateSiblingStartupScanBeforeStopping(true, "win32"),
			true,
		);
		assert.equal(
			shouldRunDuplicateSiblingStartupScanBeforeStopping(false, "win32"),
			false,
		);
		assert.equal(
			shouldRunDuplicateSiblingStartupScanBeforeStopping(true, "linux"),
			false,
		);
	});
			...
			[
				{
					pid: 101,
					ppid: 55,
					command: "node C:\\tmp\\dist\\mcp\\state-server.js",
				},
				{
					pid: 140,
					ppid: 55,
		...
		const processes = [
			{
				...
				ppid: 55,
				command:
					"node C:\\Users\\me\\AppData\\Local\\oh-my-codex\\dist\\mcp\\state-server.cjs",
			},
			{
				pid: 140,
				ppid: 55,
			...
			},
			{
				...
				ppid: 55,
				command:
					"node C:\\Users\\me\\AppData\\Local\\oh-my-codex\\dist\\mcp\\memory-server.cjs",
			},
		];
	...
	it("detects duplicate plugin-launched mcp-serve public-target siblings", () => {
		const processes = [
			...
			},
			{
				pid: 160,
				ppid: 55,
				command: "node /repo/dist/cli/omx.js mcp-serve memory",
			},
		];
	...
	it("hard-caps only never-owned pre-traffic older duplicates", () => {
		...
		const oldest = analyzeDuplicateSiblingState(
			processes,
			101,
			55,
			"state-server.js",
		);
		const secondOldest = analyzeDuplicateSiblingState(
			processes,
			102,
			55,
			"state-server.js",
		...
		assert.equal(
			shouldSelfExitForPreTrafficSiblingHardCap(oldest, null, 4),
			true,
		);
		assert.equal(
