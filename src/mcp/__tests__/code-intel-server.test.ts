...
describe("mcp/code-intel-server module contract", () => {
	it("declares expected MCP tools and diagnostics command shape", async () => {
		const src = await readFile(
			join(process.cwd(), "src/mcp/code-intel-server.ts"),
			"utf8",
		);
	...
	it("skips tsc diagnostics when the project has no tsconfig", async () => {
		...
		try {
			const { runTscDiagnostics } = await import(
				`../code-intel-server.js?ts=${Date.now()}`
			);
			let called = false;
	...
	it("redirects incremental build-info writes outside the workspace for tsc diagnostics", async () => {
		...
		const projectDir = await mkdtemp(
			join(tmpdir(), "omx-code-intel-incremental-"),
		);
		await writeFile(
		...
		try {
			const { runTscDiagnostics } = await import(
				`../code-intel-server.js?incremental=${Date.now()}`
			);
			let observedArgs: string[] = [];
			await runTscDiagnostics(
				"",
				projectDir,
				undefined,
