...
describe("state-server directory initialization", () => {
	...
	it("creates boxed runtime state under OMX_ROOT for mutating tools", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(join(box, ".omx", "state", "ralph-state.json")),
				true,
			);
	...
	it("keeps auto-completed transition canonical state under OMX_TEAM_STATE_ROOT", async () => {
		...
		const root = await mkdtemp(
			join(tmpdir(), "omx-state-server-team-transition-"),
		);
		const teamStateRoot = join(root, "team-state");
		const wd = join(root, "source");
		...
		try {
			...
			const completedDeepInterview = JSON.parse(
				await readFile(
					join(teamSessionDir, "deep-interview-state.json"),
					"utf-8",
				),
	...
	it("state_write accepts canonical lifecycle_outcome while preserving compatibility run_outcome", async () => {
		...
		try {
			...
			const state = JSON.parse(
				await readFile(
					join(wd, ".omx", "state", "autopilot-state.json"),
					"utf-8",
				),
			) as {
				active?: boolean;
	...
	it("state_write lets canonical lifecycle_outcome take precedence over legacy run_outcome", async () => {
		...
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-server-lifecycle-precedence-"),
		);
		try {
			const response = await handleStateToolCall({
				params: {
	...
	it("syncs canonical skill-active state for tracked mode writes and clears", async () => {
		...
		try {
			...
			const clearedCanonical = JSON.parse(
	...
	it("writes a session-scoped inactive tombstone when clearing a mode under an active session", async () => {
		...
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-server-clear-root-fallback-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			assert.deepEqual(JSON.parse(listResponse.content[0]?.text || "{}"), {
				active_modes: [],
			});
	...
	it("allows approved overlaps and preserves the remaining canonical state on clear", async () => {
		...
		try {
			...
			assert.deepEqual(
				clearedCanonical.active_skills?.map((entry) => entry.skill),
				["ralph"],
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("allows ultrawork when canonical session state is stricter than mode files", async () => {
		...
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-server-canonical-prevalidate-"),
		);
		try {
			await mkdir(
				join(wd, ".omx", "state", "sessions", "sess-canonical-deny"),
				{ recursive: true },
			);
			await writeFile(
				join(wd, ".omx", "state", "team-state.json"),
				JSON.stringify(
					{ active: true, mode: "team", current_phase: "running" },
					null,
					2,
				),
			);
	...
	it("removes tracked workflows from canonical skill-active state on all_sessions clear", async () => {
		...
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-server-canonical-clear-all-"),
		);
		try {
			await handleStateToolCall({
				params: {
	...
	it("does not propagate root clears into isolated session canonical copies", async () => {
		...
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-server-root-clear-propagate-"),
		);
		try {
			await handleStateToolCall({
				params: {
			...
			assert.deepEqual(
				sessionCanonical.active_skills?.map((entry) => entry.skill),
				["ralph"],
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("rejects standalone overlaps without mutating canonical state", async () => {
		...
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-server-standalone-overlap-"),
		);
		try {
			...
			const body = JSON.parse(invalidTeamWrite.content[0]?.text || "{}") as {
				error?: string;
			};
	...
	it("auto-completes deep-interview when starting ralplan and returns transition messaging", async () => {
		...
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-server-handoff-interview-"),
		);
		try {
			await mkdir(join(wd, ".omx", "state", "sessions", "sess-handoff"), {
				recursive: true,
			});
			await writeFile(
				join(
			...
			assert.match(
				completed.auto_completed_reason || "",
				/mode transiting: deep-interview -> ralplan/,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("rejects execution-to-planning rollback with clear-first guidance", async () => {
		...
		try {
			...
			assert.equal(denied.isError, true);
			const body = JSON.parse(denied.content[0]?.text || "{}") as {
				error?: string;
			};
			assert.match(
				body.error || "",
			...
			assert.match(
				body.error || "",
				/First clear current state first and retry if this action is intended/i,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("does not auto-complete existing workflow state when tracked write validation fails", async () => {
		...
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-server-validate-before-transition-"),
		);
		try {
			await mkdir(join(wd, ".omx", "state", "sessions", "sess-invalid"), {
				recursive: true,
			});
			await writeFile(
				join(
	...
	it("keeps session-scoped workflow states isolated across writes and clears", async () => {
		...
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-server-session-isolation-"),
		);
		try {
			...
			assert.deepEqual(
				sessionACanonical.active_skills?.map(({ skill, session_id }) => ({
					skill,
					session_id,
				})),
	...
	it("does not auto-complete session workflows from root-scoped workflow writes", async () => {
		...
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-server-root-session-isolation-"),
		);
		try {
			await handleStateToolCall({
				params: {
			...
			assert.deepEqual(
				sessionCanonical.active_skills?.map(({ skill, session_id }) => ({
					skill,
					session_id,
				})),
			...
			const rootState = JSON.parse(
				await readFile(
					join(wd, ".omx", "state", "ralplan-state.json"),
					"utf-8",
				),
			) as Record<string, unknown>;
	...
	it("keeps session canonical state when clearing the root scope without all_sessions", async () => {
		...
		const wd = await mkdtemp(
			join(tmpdir(), "omx-state-server-root-clear-isolation-"),
		);
		try {
			await handleStateToolCall({
				params: {
			...
			assert.equal(
				existsSync(join(wd, ".omx", "state", "deep-interview-state.json")),
				false,
			);
			const sessionState = JSON.parse(
				await readFile(
