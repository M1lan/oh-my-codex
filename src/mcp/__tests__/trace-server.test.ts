...
describe("trace-server session-scoped mode discovery", () => {
	it("includes mode events from session-scoped state files", async () => {
		...
		try {
			...
			assert.ok(
				events.some(
					(e: { event: string; mode: string }) =>
						e.event === "mode_start" && e.mode === "ralph",
				),
			);
			assert.ok(
				events.some(
					(e: { event: string; mode: string }) =>
						e.event === "mode_end" && e.mode === "ralph",
				),
			);
			assert.ok(
				events.every(
					(e: { details?: { scope?: string } }) =>
	...
	it("does not include unrelated session mode events when a current session is active", async () => {
		...
		try {
			...
			assert.ok(
				events.some(
					(e: { timestamp: string }) =>
						e.timestamp === "2020-01-01T00:00:00.000Z",
				),
			);
			assert.equal(
				events.some(
					(e: { timestamp: string }) =>
						e.timestamp === "2020-01-02T00:00:00.000Z",
				),
				false,
			);
		} finally {
...
describe("trace-server log readers", () => {
	...
	it("summarizes log history incrementally without materializing turn entries", async () => {
		...
		try {
			...
			await writeFile(
				join(logsDir, "turns-2020-01-01.jsonl"),
				[
					JSON.stringify({
			...
			await writeFile(
				join(logsDir, "turns-2020-01-02.jsonl"),
				[
					JSON.stringify({
...
describe("trace-server workingDirectory handling", () => {
	it("normalizes workingDirectory via state-paths resolver", async () => {
		const src = await readFile(
			join(process.cwd(), "src/mcp/trace-server.ts"),
			"utf8",
		);
