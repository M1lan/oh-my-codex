...
export function buildMemoryServerTools() {
	return [
		// Project Memory tools
		{
			...
			description:
				"Read project memory. Can read full memory or a specific section.",
			inputSchema: {
				type: "object",
				properties: {
					section: {
		...
		},
		{
			...
			description:
				"Write/update project memory. Can replace entirely or merge.",
			inputSchema: {
				type: "object",
				properties: {
			...
			inputSchema: {
				...
				properties: {
					category: {
						type: "string",
						description:
							"Note category (build, test, deploy, env, architecture)",
					},
		...
		// Notepad tools
		{
			name: "notepad_read",
			description:
				"Read notepad content. Can read full or a specific section (priority, working, manual).",
			inputSchema: {
				type: "object",
				properties: {
					section: {
		...
		},
		{
			...
			description:
				"Write to Priority Context section. Replaces existing. Keep under 500 chars.",
			inputSchema: {
				type: "object",
				properties: {
					content: {
		...
		},
		{
			name: "notepad_prune",
			description:
				"Prune Working Memory entries older than N days (default: 7).",
			inputSchema: {
				type: "object",
				properties: {
					daysOld: {
...
export async function handleMemoryToolCall(request: {
	...
	let wd: string;
	try {
		wd = resolveWorkingDirectoryForState(
			a.workingDirectory as string | undefined,
		);
	} catch (error) {
		return {
			content: [
	...
	switch (name) {
		...
		case "project_memory_write": {
			...
			if (merge && existsSync(memPath)) {
				...
				try {
					existing = JSON.parse(await readFile(memPath, "utf-8")) as Record<
						string,
						unknown
					>;
				} catch {
		...
		case "notepad_prune": {
			...
			if (!parsedDays.ok) {
				return {
					content: [
						{
							type: "text" as const,
							text: JSON.stringify({ error: parsedDays.error }),
						},
					],
					isError: true,
				};
			...
			return text({
				pruned,
				remaining: kept.filter((l) => l.match(/^\[/)).length,
			});
		}
