...
function resolveBaseStateDirWithSource(workingDirectory?: string): {
	...
	return {
		baseStateDir: join(
			resolveWorkingDirectoryForState(workingDirectory),
			".omx",
			"state",
		),
...
function normalizeSessionMetadata(
	...
	const nativeSessionId =
		typeof state.native_session_id === "string" &&
	...
	return {
		...
		...(typeof raw.owner_omx_session_id === "string" &&
		raw.owner_omx_session_id.trim()
			? { ownerOmxSessionId: raw.owner_omx_session_id.trim() }
			: {}),
		...(typeof raw.owner_codex_session_id === "string" &&
		raw.owner_codex_session_id.trim()
			? { ownerCodexSessionId: raw.owner_codex_session_id.trim() }
			: {}),
		...(typeof raw.owner_codex_thread_id === "string" &&
		raw.owner_codex_thread_id.trim()
			? { ownerCodexThreadId: raw.owner_codex_thread_id.trim() }
			: {}),
		...(typeof raw.tmux_pane_id === "string" && raw.tmux_pane_id.trim()
			? { leaderPaneId: raw.tmux_pane_id.trim() }
			: {}),
		...(typeof raw.tmux_session_name === "string" &&
		raw.tmux_session_name.trim()
			? { tmuxSessionName: raw.tmux_session_name.trim() }
			: {}),
