...
export const WRITABLE_STATE_SCOPE_ERRORS = {
	...
	scopeChangedDuringWrite:
		"Cannot commit the state write: the writable state scope changed while the write was in progress.",
} as const;

export type StateRootSource =
	| "team-env"
	| "omx-root-env"
	| "omx-state-root-env"
	| "session-authority"
...
export function resolveWorkingDirectoryForState(
	...
	if (process.platform === "win32") {
		...
		if (converted === normalized) {
			throw new Error(
				"workingDirectory Windows path is not available on this host",
			);
		}
	...
	while (true) {
		...
		try {
			...
			return suffixes.reduce(
				(acc, segment) => resolvePath(acc, segment),
				realParent,
			);
		} catch (error) {
			if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
...
function parseAllowedWorkingDirectoryRoots(
	...
	const roots = raw
		...
		.map((part) => {
			if (part.includes("\0")) {
				throw new Error(
					`${WORKDIR_ALLOWLIST_ENV} contains an invalid root with a NUL byte`,
				);
			}
...
function enforceWorkingDirectoryPolicy(
	...
	const canonicalWorkingDirectory = canonicalizeExistingPath(
		resolvedWorkingDirectory,
	);
	const allowed = roots.some((root) =>
		isWithinRoot(canonicalWorkingDirectory, root),
	);
	if (!allowed) {
		throw new Error(
...
function sessionPointerMatchesId(
...
function discoverSessionAuthorityBaseStateDir(
	...
	while (true) {
		...
		if (
			...
			!allowedRoots.some((root) => isWithinRoot(canonicalCurrent, root))
		)
			break;
		...
		if (
			allowedRoots.length > 0 &&
			!allowedRoots.some((root) => isWithinRoot(canonicalCandidate, root))
		) {
			const parent = resolvePath(current, "..");
			if (parent === current) break;
			current = parent;
...
function validateResolvedBaseStateDir(
	...
	if (allowedRoots.length > 0) {
		...
		if (
			!allowedRoots.some((root) => isWithinRoot(canonicalBaseStateDir, root))
		) {
			throw new Error(
				`State root "${canonicalBaseStateDir}" is outside allowed roots (${WORKDIR_ALLOWLIST_ENV})`,
			);
		}
	}
...
export function getBaseStateDirWithSource(
	...
	if (
		...
		return validateResolvedBaseStateDir(
			resolveWorkingDirectoryForState(teamStateRootOverride, env),
			"team-env",
			env,
		);
	}
	...
	if (typeof omxRootOverride === "string" && omxRootOverride !== "") {
		return validateResolvedBaseStateDir(
			join(
				resolveWorkingDirectoryForState(omxRootOverride, env),
				".omx",
				"state",
			),
			"omx-root-env",
	...
	if (typeof omxStateRootOverride === "string" && omxStateRootOverride !== "") {
		return validateResolvedBaseStateDir(
			join(
				resolveWorkingDirectoryForState(omxStateRootOverride, env),
				".omx",
				"state",
			),
			"omx-state-root-env",
	...
	const sessionAuthority = discoverSessionAuthorityBaseStateDir(
		workingDirectory,
		env,
	);
	if (sessionAuthority) {
	...
	return validateResolvedBaseStateDir(
		join(
			resolveWorkingDirectoryForState(workingDirectory, env),
			".omx",
			"state",
		),
		"cwd-default",
...
export function getBaseStateDir(
	workingDirectory?: string,
	env: NodeJS.ProcessEnv = process.env,
): string {
...
export function getStateFilePath(
	...
	return join(
		getStateDir(workingDirectory, sessionId),
		validateStateFileName(fileName),
	);
}
...
function readSessionIdFromEnvironment(
	env: NodeJS.ProcessEnv = process.env,
): string | undefined {
	const candidates = [
		env[OMX_SESSION_ID_ENV],
...
async function readUsableSessionStateFromBaseStateDir(
	...
	const snapshot = await readAuthoritativeSessionSnapshotFromBaseStateDir(
		cwd,
		baseStateDir,
	);
	return snapshot && isSessionStateUsable(snapshot.state, snapshot.recordedCwd)
		? snapshot.state
		: null;
}
function normalizeSessionMetadata(
	...
	sourcePath?: string,
): ResolvedSessionMetadata | undefined {
	...
	const ownerCodexThreadId =
		typeof raw.owner_codex_thread_id === "string" &&
		raw.owner_codex_thread_id.trim()
			? raw.owner_codex_thread_id.trim()
			: undefined;
	const leaderPaneId =
		typeof raw.tmux_pane_id === "string" && raw.tmux_pane_id.trim()
			? raw.tmux_pane_id.trim()
			: undefined;
	const tmuxSessionName =
		typeof raw.tmux_session_name === "string" && raw.tmux_session_name.trim()
...
export async function readSessionMetadataFromBaseStateDir(
	...
	const sessionPath = join(baseStateDir, "session.json");
	const session = await readUsableSessionStateFromBaseStateDir(
		cwd,
		baseStateDir,
	);
...
function isKnownSessionAlias(
	sessionId: string,
	metadata: ResolvedSessionMetadata,
): boolean {
	return (
		metadata.nativeSessionAliases.includes(sessionId) ||
...
export function __setWritableStateScopeTestHooksForTests(
	hooks: WritableStateScopeTestHooks,
): void {
	...
	selectedDecisionSnapshotReadHookForTests =
		hooks.onSelectedDecisionSnapshotRead;
...
export async function resolveWritableStateScope(
	...
	const liveness = snapshot
		? classifySessionStateLiveness(snapshot.state)
		: undefined;
	...
	if (validatedExplicit) {
		const sessionId =
			resolveCanonicalSessionId(validatedExplicit, metadata) ??
			validatedExplicit;
		return {
			source: "explicit",
			sessionId,
	...
	if (!metadata) {
		...
		if (existsSync(sessionPath)) {
			...
			if (snapshot && envSessionId) {
				const canonicalPointerSessionId = normalizeSessionId(
				...
				if (recoveryEligible) {
					if (selectedPointerRecoveryHookForTests)
						await selectedPointerRecoveryHookForTests();
					const reread = await readFile(sessionPath, "utf-8").catch(
...
export async function assertWritableStateScopeUnchanged(
	...
	if (writableScopeRevalidationHookForTests)
		await writableScopeRevalidationHookForTests(event!);
	const current = await resolveWritableStateScope(
		workingDirectory,
		explicitSessionId,
	);
	const currentBaseStateDir =
		getBaseStateDirWithSource(workingDirectory).baseStateDir;
	if (
...
export async function resolveRuntimeStateScope(
	...
	const stateDir = sessionId
		? join(baseStateDir, "sessions", sessionId)
		: baseStateDir;
	...
	return {
		...
		compatibilityReadDirs:
			isSessionScoped && source !== "explicit"
				? [stateDir, baseStateDir]
				: [stateDir],
	};
}
...
export async function getCompatibilityReadScopedStateDirs(
	...
	return (await resolveRuntimeStateScope(workingDirectory, explicitSessionId))
		.compatibilityReadDirs;
}
...
export async function getCompatibilityReadScopedStatePaths(
	...
	return (
		await getCompatibilityReadScopedStateDirs(
			workingDirectory,
			explicitSessionId,
		)
...
export async function getCompatibilityReadScopedStateFilePaths(
	...
	).map((dir) => join(dir, normalizedFileName));
}
...
export async function getAuthoritativeActiveStatePaths(
	...
	const dirs = await getAuthoritativeActiveStateDirs(
		workingDirectory,
		explicitSessionId,
	);
	const fileName = getStateFilename(mode);
...
export async function getReadScopedStatePaths(
	...
	const dirs = await getReadScopedStateDirs(
		workingDirectory,
		explicitSessionId,
	);
	const fileName = getStateFilename(mode);
...
export async function getAllSessionScopedStateDirs(
	workingDirectory?: string,
): Promise<string[]> {
	...
	return entries
		.filter(
			(entry) => entry.isDirectory() && SESSION_ID_PATTERN.test(entry.name),
		)
		.map((entry) => join(sessionsRoot, entry.name));
}

export async function getAllScopedStateDirs(
	workingDirectory?: string,
): Promise<string[]> {
	return [
		getBaseStateDir(workingDirectory),
		...(await getAllSessionScopedStateDirs(workingDirectory)),
	];
}
...
async function listModeStateFilesInDir(
	dir: string,
	scope: StateFileScope,
...
export async function listModeStateFilesWithScopePreference(
	...
	const readDirs = await getReadScopedStateDirs(
		workingDirectory,
		explicitSessionId,
	);
