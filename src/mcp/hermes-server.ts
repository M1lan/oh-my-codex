	...
	return [
		{
			...
			description:
				"List known OMX session state for a bounded worktree without reading terminal UI.",
			inputSchema: { type: "object", properties: { workingDirectory } },
		},
		{
			name: "hermes_start_session",
			description:
				"Start a new isolated OMX tmux session in disposable worktree mode for one bounded prompt.",
			inputSchema: {
				type: "object",
				properties: {
					workingDirectory,
		...
		},
		{
			...
			description:
				"Queue one explicit prompt for a selected OMX exec session via the audited follow-up queue.",
			inputSchema: {
				type: "object",
				properties: {
					workingDirectory,
		...
		},
		{
			...
			description:
				"Read selected session/mode status JSON from OMX state files.",
			inputSchema: {
				type: "object",
				properties: { workingDirectory, session_id: sessionId },
			},
		},
		{
			name: "hermes_read_tail",
			description:
				"Read the bounded OMX session history log tail, not tmux scrollback.",
			inputSchema: {
				type: "object",
				properties: { workingDirectory, lines: { type: "number" } },
			},
		},
		{
			name: "hermes_list_question_events",
			description:
				"Read structured question lifecycle events for coordinator bridge correlation.",
			inputSchema: {
				type: "object",
				properties: { workingDirectory, limit: { type: "number" } },
			},
		},
		{
			name: "hermes_list_questions",
			description:
				"List bounded structured question records without reading terminal UI.",
			inputSchema: {
				type: "object",
				properties: {
					workingDirectory,
		...
		},
		{
			...
			description:
				"Submit a bounded structured answer by question id; never proxies arbitrary terminal input.",
			inputSchema: {
				type: "object",
				properties: {
					workingDirectory,
		...
		},
		{
			...
			description:
				"List known safe result artifact files under .omx plans/specs/goals/context/reports.",
			inputSchema: {
				type: "object",
				properties: { workingDirectory, limit: { type: "number" } },
			},
		},
		{
			...
			description:
				"Read one safe .omx result artifact by relative path with byte truncation.",
			inputSchema: {
				type: "object",
				properties: {
					workingDirectory,
		...
		},
		{
			...
			description:
				"Write a small final/blocker status report for Hermes without owning merge policy.",
			inputSchema: {
				type: "object",
				properties: {
					workingDirectory,
					...
					status: {
						type: "string",
						enum: ["running", "blocked", "failed", "complete"],
					},
					summary: { type: "string" },
					pr_url: { type: "string" },
					blocker: { type: "string" },
