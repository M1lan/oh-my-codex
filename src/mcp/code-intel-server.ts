...
export async function runTscDiagnostics(
	...
	let stderr = "";
	try {
		({ stdout, stderr } = await runCommand("pnpm", ["exec", "tsc", ...args], {
			cwd: projectDir,
			timeout: 60000,
		}));
	} finally {
...
async function findSgBinary(): Promise<string | null> {
	...
	// Try pnpm dlx
	try {
		await execFileAsync("pnpm", ["dlx", "@ast-grep/cli", "--version"], {
			timeout: 15000,
		});
...
async function runAstGrep(
	...
	if (!sg) {
		return {
			matches: [],
			command: "ast-grep not installed. Install: pnpm add -g @ast-grep/cli",
		};
	}
			...
			if (entry.isDirectory()) {
				...
				try {
					...
					process.stderr.write(
						`[code-intel-server] operation failed: ${err}\n`,
					);
				}
			}
		}
	...
	return [
		{
			...
			description:
				"Get diagnostics (errors, warnings) for a file. Uses tsc --noEmit for TypeScript projects.",
			inputSchema: {
				type: "object",
				properties: {
		...
		},
		{
			...
			description:
				"Run project-level diagnostics on a directory using tsc --noEmit. Returns all errors across the project.",
			inputSchema: {
				type: "object",
				properties: {
					directory: {
		...
		},
		{
			...
			description:
				"Get a hierarchical outline of all symbols in a file (functions, classes, variables, etc.).",
			inputSchema: {
				type: "object",
				properties: {
					file: { type: "string", description: "Path to the source file" },
		...
		},
		{
			...
			description:
				"Search for symbols (functions, classes, etc.) across the workspace by name.",
			inputSchema: {
				type: "object",
				properties: {
					query: {
					...
					file: {
						type: "string",
						description:
							"Any file in the workspace (used to determine project root)",
					},
				},
				required: ["query", "file"],
			},
		},
		{
			name: "lsp_hover",
			description:
				"Get type information and documentation at a specific position in a file (regex-based approximation).",
			inputSchema: {
				type: "object",
				properties: {
					file: { type: "string", description: "Path to the source file" },
		...
		},
		{
			...
			description:
				"Find all references to a symbol across the codebase using grep-based search.",
			inputSchema: {
				type: "object",
				properties: {
					file: { type: "string", description: "Path to the source file" },
		...
		},
		{
			name: "lsp_servers",
			description:
				"List available diagnostic backends and their installation status.",
			inputSchema: {
				type: "object",
				properties: {},
			},
		},
		{
			...
			description:
				'Search for code patterns using AST matching. Uses meta-variables: $NAME (single node), $$$ARGS (multiple nodes). Example: "function $NAME($$$ARGS)" finds all function declarations.',
			inputSchema: {
				type: "object",
				properties: {
					pattern: {
		...
		},
		{
			...
			description:
				"Replace code patterns using AST matching. Use meta-variables in both pattern and replacement. IMPORTANT: dryRun=true (default) only previews changes. Set dryRun=false to apply.",
			inputSchema: {
				type: "object",
				properties: {
...
export async function handleCodeIntelToolCall(request: {
	...
	switch (name) {
		...
		case "lsp_diagnostics_directory": {
			...
			return text({
				directory: dir,
				totalErrors: result.diagnostics.filter((d) => d.severity === "error")
					.length,
				totalWarnings: result.diagnostics.filter(
		...
		case "lsp_find_references": {
			...
			try {
				...
				const refs = stdout
					...
					.filter(
						(entry): entry is { file: string; line: number; content: string } =>
							entry !== null,
					);
		...
		case "lsp_servers": {
			const checks: Record<
				string,
				{ available: boolean; version?: string; note?: string }
			> = {};
			// Check tsc
			try {
				const { stdout } = await exec("pnpm", ["exec", "tsc", "--version"], {
					timeout: 10000,
				});
				...
				checks["typescript"] = {
					available: false,
					note: "Install: pnpm add -D typescript",
				};
			}
			// Check ast-grep
			const sg = await findSgBinary();
			if (sg) {
				...
				checks["ast-grep"] = {
					available: false,
					note: "Install: pnpm add -g @ast-grep/cli",
				};
			}
			// Check grep
			try {
