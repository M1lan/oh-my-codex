export type CatalogSkillCategory =
	| "execution"
	| "planning"
	| "shortcut"
	| "utility";
export type CatalogAgentCategory =
	| "build"
	| "review"
	| "domain"
	| "product"
...
export function summarizeCatalogCounts(
	manifest: CatalogManifest,
): CatalogCounts {
	return {
		skillCount: manifest.skills.length,
		promptCount: manifest.agents.length,
		activeSkillCount: manifest.skills.filter((s) => s.status === "active")
			.length,
		activeAgentCount: manifest.agents.filter((a) => a.status === "active")
			.length,
	};
}
