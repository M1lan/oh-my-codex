...
describe("config generator", () => {
	it("places top-level keys before [features]", async () => {
		...
		try {
			...
			const compactIdx = toml.indexOf(
				"model_auto_compact_token_limit = 200000",
			);
			const seededEndIdx = toml.indexOf(
			...
			assert.ok(
				reasoningIdx < featuresIdx,
				"model_reasoning_effort must come before [features]",
			);
			assert.ok(
				devInstrIdx < featuresIdx,
				"developer_instructions must come before [features]",
			);
			...
			assert.ok(
				contextIdx < featuresIdx,
				"model_context_window must come before [features]",
			);
			assert.ok(
				compactIdx < featuresIdx,
				"model_auto_compact_token_limit must come before [features]",
			);
			assert.ok(
	...
	it("seeds default model and context settings on fresh config", async () => {
		...
		try {
			...
			const featuresIdx = toml.indexOf("[features]");
			assert.ok(
				modelIdx >= 0 && modelIdx < featuresIdx,
				"seeded model must come before [features]",
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("writes model_reasoning_effort and strengthened developer_instructions", async () => {
		...
		try {
			...
			assert.match(
				toml,
				/^developer_instructions = "You have oh-my-codex installed/m,
			);
			assert.match(
				toml,
				/AGENTS\.md is the orchestration brain and main control surface/,
			);
			assert.match(
				toml,
				/Follow AGENTS\.md for skill\/keyword routing, \$name workflow invocation, and role-specialized subagents/,
			);
			assert.match(toml, /Native subagents live in \.codex\/agents/);
			assert.match(
				toml,
				/set `agent_type` to an installed role and never omit it for OMX work/,
			);
			assert.match(
				toml,
				/Treat installed prompts as narrower execution surfaces under AGENTS\.md authority/,
			);
			assert.match(
				toml,
				new RegExp(
					`^developer_instructions = "${OMX_DEVELOPER_INSTRUCTIONS.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"$`,
					"m",
				),
			);
		} finally {
	...
	it("removes deprecated collab flag from [features]", async () => {
		...
		try {
			...
			// collab must be gone
			assert.ok(
				!/^\s*collab\s*=/m.test(toml),
				"deprecated collab key should be removed",
			);
	...
	it("can target the legacy codex_hooks flag when requested", async () => {
		...
		try {
			...
			await mergeConfig(configPath, wd, {
				codexHookFeatureFlag: "codex_hooks",
			});
			const merged = await readFile(configPath, "utf-8");
	...
	it("does not preserve cross-install OMX notify commands when notify is disabled", () => {
		...
		const merged = buildMergedConfig(staleConfig, pkgRoot, {
			notifyCommand: false,
		});
	...
	it("preserves real user notify commands that mention OMX paths as arguments", () => {
		...
		const merged = buildMergedConfig(userNotify, pkgRoot, {
			notifyCommand: false,
		});
