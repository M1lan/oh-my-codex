...
`,
			...
			const config = resolveDeepInterviewRuntimeConfig({
				cwd,
				homeDir,
...
`,
			...
			const config = resolveDeepInterviewRuntimeConfig({
				cwd,
				homeDir,
				text: "$deep-interview --deep clarify this",
			});
	...
	it("resolves each documented config source when higher-precedence sources are absent", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-deep-interview-config-sources-"),
		);
		const homeDir = await mkdtemp(
			join(tmpdir(), "omx-deep-interview-home-sources-"),
		);
		try {
			await mkdir(join(cwd, ".omx"), { recursive: true });
			await mkdir(join(homeDir, ".omx"), { recursive: true });
			...
			let config = resolveDeepInterviewRuntimeConfig({
				cwd,
				homeDir,
				text: "$deep-interview",
			});
			...
			config = resolveDeepInterviewRuntimeConfig({
				cwd,
				homeDir,
				text: "$deep-interview",
			});
	...
	it("resolves repository config from nested working directories", async () => {
		...
		try {
			...
			assert.equal(config?.sourcePath, join(cwd, ".omx", "config.toml"));
			const candidatePaths = getDeepInterviewConfigCandidatePaths({
				cwd: nestedCwd,
				homeDir,
			});
			assert.equal(
				candidatePaths[0]?.path,
				join(nestedCwd, ".omx", "config.toml"),
			);
...
`,
	...
	it("ignores malformed TOML without throwing", async () => {
		...
		try {
			...
			assert.match(
				warnings[0] ?? "",
				/ignoring malformed deep-interview config/,
			);
		} finally {
	...
	it("does not cascade to lower-precedence configs after a parse failure", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-deep-interview-config-malformed-precedence-"),
		);
		const homeDir = await mkdtemp(
			join(tmpdir(), "omx-deep-interview-home-malformed-precedence-"),
		);
		const originalWarn = console.warn;
		const warnings: string[] = [];
		try {
...
`,
	...
	it("does not cascade to lower-precedence configs when an existing higher-precedence file omits deepInterview", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-deep-interview-config-no-table-precedence-"),
		);
		const homeDir = await mkdtemp(
			join(tmpdir(), "omx-deep-interview-home-no-table-precedence-"),
		);
		try {
			await mkdir(join(cwd, ".omx"), { recursive: true });
			await mkdir(join(homeDir, ".omx"), { recursive: true });
...
`,
	...
	it("parses supported profile flags only", () => {
		assert.equal(
			parseDeepInterviewProfileFromText("$deep-interview --quick"),
			"quick",
		);
		assert.equal(
			parseDeepInterviewProfileFromText("$deep-interview --standard"),
			"standard",
		);
		assert.equal(
			parseDeepInterviewProfileFromText("$deep-interview --deep"),
			"deep",
		);
		assert.equal(
			parseDeepInterviewProfileFromText("$oh-my-codex:deep-interview --deep"),
			"deep",
		);
		assert.equal(
			parseDeepInterviewProfileFromText("deep interview --quick"),
		...
		assert.equal(
			parseDeepInterviewProfileFromText("$deep-interview --deeper"),
			undefined,
		);
		assert.equal(
			parseDeepInterviewProfileFromText(
				"$deep-interview clarify this and run other-tool --deep",
			),
			undefined,
		);
		assert.equal(
			parseDeepInterviewProfileFromText(
				"$deep-interview clarify the plain-text --deep option",
			),
			undefined,
		);
		assert.equal(
			parseDeepInterviewProfileFromText("explain the --deep option"),
			undefined,
		);
	});
});
