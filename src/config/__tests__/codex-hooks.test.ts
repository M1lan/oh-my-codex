...
describe("codex hooks helpers", () => {
	...
	it("parses only managed Windows shim commands and returns their validated final path", () => {
		...
		assert.equal(
			parseManagedCodexNativeHookWindowsShimCommand(managed),
			shimPath,
		);
		assert.equal(
			parseManagedCodexNativeHookWindowsShimCommand(
		...
		for (const command of [
			...
			assert.equal(
				parseManagedCodexNativeHookWindowsShimCommand(command),
				null,
			);
		}
	});
	...
	it("accepts quoted POSIX and PowerShell path operators while rejecting unquoted operators", () => {
		...
		const packageRoot =
			"D:\\Program Files\\O'Brien\\hook (fixture) $literal\\oh-my-codex";
		const codexHome = "C:\\Users\\O'Brien\\.codex (fixture) $literal";
		const shimPath =
			"C:\\Users\\O'Brien\\.codex (fixture) $literal\\hooks\\omx-native-hook-windows-shim.ps1";
		...
		assert.equal(
			parseManagedCodexNativeHookWindowsShimCommand(windowsCommand),
			shimPath,
		);
	...
	it("owns only shell-static command spellings and keeps platform ownership exact", () => {
		...
		const shimPath =
			"C:\\Users\\Ada\\.codex\\hooks\\omx-native-hook-windows-shim.ps1";
		...
		assert.equal(
			isManagedCodexHookCommand(`C:\\Node\\NODE.EXE ${windowsScript}`),
			true,
		);
		for (const command of [
		...
		const caseVariant = JSON.stringify({
			hooks: {
				SessionStart: [
					{ hooks: [{ type: "command", command: `NODE ${posixScript}` }] },
				],
			},
		});
		const posix = planManagedCodexHooksRemoval(caseVariant, "/hooks.json", {
			platform: "linux",
		});
		const windows = planManagedCodexHooksRemoval(
			caseVariant,
	...
	it("requires absolute, control-free POSIX OMX paths before ownership proof", () => {
		...
		const ownedSource = JSON.stringify({
			hooks: {
				PreToolUse: [
					{ hooks: [{ type: "command", command: historicalCommand }] },
				],
			},
		});
		const ownedMerge = planManagedCodexHooksMerge(
			ownedSource,
			"/repo",
			"/hooks.json",
			{ platform: "linux" },
		);
		const ownedRemoval = planManagedCodexHooksRemoval(
			ownedSource,
			"/hooks.json",
			{ platform: "linux" },
		);
		...
		const foreignMerge = planManagedCodexHooksMerge(
			foreignSource,
			"/repo",
			"/hooks.json",
			{ platform: "linux" },
		);
		const foreignRemoval = planManagedCodexHooksRemoval(
			foreignSource,
			"/hooks.json",
			{ platform: "linux" },
		);
		...
		const ambiguousCommands = [
			...
			...controlCodePoints.map(
				(codePoint) =>
					`node "/historical/dist/scripts/codex-native-hook.js${String.fromCodePoint(codePoint)}"`,
			),
		];
		for (const command of ambiguousCommands) {
			...
			const merge = planManagedCodexHooksMerge(source, "/repo", "/hooks.json", {
				platform: "linux",
			});
			const removal = planManagedCodexHooksRemoval(source, "/hooks.json", {
				platform: "linux",
			});
	...
	it("fails closed for decoded POSIX OMX spellings across command fields", () => {
		...
		const decodedNonExactCommands = [
			[
				"escaped filename with extra argv",
				String.raw`node /repo/dist/scripts/codex-native-ho\ok.js --extra`,
			],
			[
				"escaped filename with comment",
				String.raw`node /repo/dist/scripts/codex-native-ho\ok.js # retained comment`,
			],
			[
				"backslash-newline filename split",
				"node /repo/dist/scripts/codex-native-ho\\\nok.js --extra",
			],
		] as const;
		const commandFields = [
				...
				const source = JSON.stringify({
					hooks: {
						PreToolUse: [
							{
								hooks: [
									{ type: "command", ...commandFieldsForHandler(command) },
								],
							},
						],
					},
	...
	it("owns only the exact current drive-less Windows shim command from supplied options", () => {
		...
		assert.match(
			command,
			/-File '\\fixture codex home\\hooks\\omx-native-hook-windows-shim\.ps1'$/,
		);
	...
	it("owns only exact current host-joined Windows shim commands on non-Windows seams", () => {
		...
		for (const { label, options, hooksPath, codexHomeDir } of fixtures) {
			...
			if (!alteredMerge.ok)
				assert.equal(
					alteredMerge.error.code,
					"ambiguous_managed_handler",
					label,
				);
			if (!alteredRemoval.ok)
				assert.equal(
					alteredRemoval.error.code,
					"ambiguous_managed_handler",
					label,
				);
		}
	...
	it("fails closed for Windows direct command expansion, quoting, wrappers, and invalid cross-fields", () => {
		...
		for (const platform of ["linux", "win32"] as const) {
			...
			assert.equal(
				merge.ok,
				true,
				`${platform} must accept platform-specific managed command fields`,
			);
			assert.equal(
				removal.ok,
				true,
				`${platform} must remove platform-specific managed command fields`,
			);
		}
	});
	...
	it("fails closed for unqualified direct executables and quoted PowerShell executable paths across command fields", () => {
		...
		const unsafeCommands = [
			...
			[
				...
				`.\\node.exe "${windowsScript}"`,
				"win32",
			],
			[
				"Windows relative executable",
				`tools\\node.exe "${windowsScript}"`,
				"win32",
			],
			[
				"Windows drive-relative executable",
				`C:node.exe "${windowsScript}"`,
				"win32",
			],
			[
		...
		const assertAmbiguous = (
			...
			const hooksPath =
				platform === "win32"
					? "C:\\Users\\Ada\\.codex\\hooks.json"
					: "/hooks.json";
			const merge = planManagedCodexHooksMerge(source, pkgRoot, hooksPath, {
				platform,
			});
			const removal = planManagedCodexHooksRemoval(source, hooksPath, {
				platform,
			});
			assert.equal(merge.ok, false, context);
			assert.equal(removal.ok, false, context);
		...
		for (const [spelling, unsafeCommand, primaryPlatform] of unsafeCommands) {
			...
			assertAmbiguous(
				JSON.stringify({
					hooks: {
						PreToolUse: [
							{ hooks: [{ type: "command", command: unsafeCommand }] },
						],
					},
				}),
				primaryPlatform,
			...
			for (const [field, handler] of [
				[
					"commandWindows",
					{ command: fallbackPosixCommand, commandWindows: unsafeCommand },
				],
				[
					"command_windows",
					{ command: fallbackPosixCommand, command_windows: unsafeCommand },
				],
			] as const) {
				assertAmbiguous(
					JSON.stringify({
	...
	it("fails closed for Windows direct-node and PowerShell shim quote concatenation across command fields", () => {
		...
		const malformedCommands = [
			[
				"direct doubled",
				`node "C:\\repo\\dist\\scripts\\codex""-native-hook.js"`,
			],
			["direct adjacent", `node ""${windowsScript}""`],
			["direct triple", `node """${windowsScript}"""`],
			[
				"shim doubled",
				`${shimPrefix} "C:\\Users\\Ada\\.codex\\hooks\\omx""-native-hook-windows-shim.ps1"`,
			],
		...
		for (const [spelling, malformed] of malformedCommands) {
			for (const [field, handler] of [
				...
				[
					"commandWindows",
					{ command: `node "${posixScript}"`, commandWindows: malformed },
				],
				[
					"command_windows",
					{ command: `node "${posixScript}"`, command_windows: malformed },
				],
			] as const) {
				const source = JSON.stringify({
					hooks: {
	...
	it("fails closed for invalid Windows path syntax in direct and PowerShell commands", () => {
		...
		const powerShellExecutable =
			"C:\\base\\System32\\WindowsPowerShell\\v1.0\\powershell.exe";
		...
		const invalidWindowsPaths: Array<[string, (path: string) => string]> = [
			...
			[
				"control character",
				(path) => path.replace("\\base\\", "\\ba\u0000se\\"),
			],
			[
				"alternate data stream",
				(path) => path.replace("\\base\\", "\\base:stream\\"),
			],
			["extended namespace", (path) => `\\\\?\\${path}`],
			["device namespace", (path) => `\\\\.\\${path}`],
			[
				"NT device namespace",
				(path) => `\\Device\\HarddiskVolume2${path.slice(2)}`,
			],
			["DOS device name", (path) => path.replace("\\base\\", "\\NUL\\")],
			[
				"DOS COM device stem before extension",
				(path) => path.replace("\\base\\", "\\COM1 .cache\\"),
			],
			[
				"DOS NUL device stem before extension",
				(path) => path.replace("\\base\\", "\\NUL .dir\\"),
			],
			[
				"superscript COM device name",
				(path) => path.replace("\\base\\", "\\COM¹\\"),
			],
			[
				"superscript LPT device name",
				(path) => path.replace("\\base\\", "\\LPT²\\"),
			],
			[
				"trailing component period",
				(path) => path.replace("\\base\\", "\\base.\\"),
			],
			[
				"trailing component space",
				(path) => path.replace("\\base\\", "\\base " + String.fromCharCode(92)),
			],
		];
		const invalidCommands = invalidWindowsPaths.flatMap(
			([syntax, invalidPath]) => [
				[
					`direct executable ${syntax}`,
					`"${invalidPath(directExecutable)}" "${directScript}"`,
				],
		...
		const validUncDirect =
			'"\\\\server\\share\\node.exe" "\\\\server\\share\\repo\\dist\\scripts\\codex-native-hook.js"';
		const validShimPath =
			"C:\\Users\\O'Brien\\@scope & safe\\(fixture)\\.codex\\hooks\\omx-native-hook-windows-shim.ps1";
		const validPowerShell =
			"& 'C:\\Program Files (x86)\\WindowsPowerShell\\v1.0\\powershell.exe' -NoProfile -ExecutionPolicy Bypass -File 'C:\\Users\\O''Brien\\@scope & safe\\(fixture)\\.codex\\hooks\\omx-native-hook-windows-shim.ps1'";
		const rootedPowerShell =
			"& '\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe' -NoProfile -ExecutionPolicy Bypass -File '\\Users\\Ada\\.codex\\hooks\\omx-native-hook-windows-shim.ps1'";
		assert.equal(isManagedCodexHookCommand(validDirect), true);
		assert.equal(isManagedCodexHookCommand(validUncDirect), true);
		assert.equal(
			parseManagedCodexNativeHookWindowsShimCommand(validPowerShell),
			validShimPath,
		);
		assert.equal(
			parseManagedCodexNativeHookWindowsShimCommand(rootedPowerShell),
			null,
		);
		...
		for (const [spelling, invalid] of invalidCommands) {
			...
			for (const [field, handler] of [
				...
				[
					"command_windows",
					{ command: posixCommand, command_windows: invalid },
				],
			] as const) {
				const source = JSON.stringify({
					hooks: {
	...
	it("keeps Windows shim paths quoted when they contain spaces", () => {
		...
		const commands = merged.hooks.PreToolUse.flatMap(
			(entry) => entry.hooks ?? [],
	...
	it("builds deterministic Windows shim paths and PowerShell 5.1-compatible ProcessStartInfo content", () => {
		...
		assert.match(
			content,
			/OpenStandardInput\(\)\.CopyToAsync\(\$process\.StandardInput\.BaseStream\)/,
		);
		assert.match(
			content,
	...
	it("prepends a UTF-8 BOM to the Windows shim so PowerShell 5.1 reads non-ASCII paths as UTF-8", () => {
		...
		assert.equal(
			content.slice(1).startsWith("$ErrorActionPreference = 'Stop'"),
			true,
		);
		...
		const expected = Buffer.from(
			buildManagedCodexNativeHookWindowsShimContent(
				"C:\\Current Install\\oh-my-codex",
				{
					...
					hookScriptPath:
						"C:\\Current Install\\oh-my-codex\\dist\\scripts\\codex-native-hook.js",
				},
			),
			"utf-8",
		);
		const historical = Buffer.from(
			buildManagedCodexNativeHookWindowsShimContent(
				"C:\\Historical Install\\oh-my-codex",
				{
					...
					hookScriptPath:
						"D:\\Historical Install\\O'Malley\\oh-my-codex\\dist\\scripts\\codex-native-hook.js",
				},
			),
			"utf-8",
		);
	...
	it("requires qualified valid paths before recognizing historical Windows shim ownership", () => {
		...
		const validHookScriptPath =
			"D:\\Historical Install\\oh-my-codex\\dist\\scripts\\codex-native-hook.js";
		const invalidHistoricalPaths = [
			...
			{
				nodePath: "D:\\bad|node\\node.exe",
				hookScriptPath: validHookScriptPath,
			},
			{
				nodePath: "D:\\bad\u0000node\\node.exe",
				hookScriptPath: validHookScriptPath,
			...
			{
				nodePath: validNodePath,
				hookScriptPath: ".\\dist\\scripts\\codex-native-hook.js",
			},
			{
				nodePath: validNodePath,
				hookScriptPath: "D:historical\\dist\\scripts\\codex-native-hook.js",
			},
			{
				nodePath: validNodePath,
				hookScriptPath: "D:\\bad?script\\dist\\scripts\\codex-native-hook.js",
			},
			{
				nodePath: validNodePath,
				hookScriptPath:
					"D:\\bad\u0000script\\dist\\scripts\\codex-native-hook.js",
			},
			{
				nodePath: validNodePath,
				hookScriptPath: String.raw`\\?\D:\\Historical Install\\dist\\scripts\\codex-native-hook.js`,
			},
			{
				nodePath: validNodePath,
				hookScriptPath: String.raw`\Device\HarddiskVolume2\dist\scripts\codex-native-hook.js`,
			},
			{
				nodePath: validNodePath,
				hookScriptPath:
					"D:\\Historical Install.\\dist\\scripts\\codex-native-hook.js",
			},
			{
				nodePath: validNodePath,
				hookScriptPath:
			...
			{
				nodePath: validNodePath,
				hookScriptPath: "D:\\COM1 .cache\\dist\\scripts\\codex-native-hook.js",
			},
			{
				nodePath: validNodePath,
				hookScriptPath: "D:\\NUL .dir\\dist\\scripts\\codex-native-hook.js",
			},
		];
		const uncHistorical = Buffer.from(
			buildManagedCodexNativeHookWindowsShimContent(
				"\\\\server\\share\\Historical Install\\oh-my-codex",
				{
					nodePath: "\\\\server\\share\\Node O'Brien\\node.exe",
					hookScriptPath:
						"\\\\server\\share\\Historical Install\\O'Brien\\oh-my-codex\\dist\\scripts\\codex-native-hook.js",
				},
			),
			"utf-8",
		);
	...
	it("rejects incomplete, altered, encoding-ambiguous, and unverifiable Windows shim ownership", () => {
		...
		const modifiedFixedStatement = Buffer.from(
			historical
				.toString("utf-8")
				.replace("$process.WaitForExit()", "$process.WaitForExit(1)"),
			"utf-8",
		);
		const unverifiableNode = Buffer.from(
			historical
				.toString("utf-8")
				.replace(
					"D:\\Historical Node\\node.exe",
		...
		const unverifiableHookScript = Buffer.from(
			historical
				.toString("utf-8")
				.replace(
			...
			const result = spawnSync(
				...
				["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", shimPath],
				{
					input: JSON.stringify({
						...
						last_user_message:
							"这是 oh-my-codex PowerShell shim 回归测试，用长中文多字节 stdin JSON 验证不会触发截断。".repeat(
								600,
							),
					}),
					encoding: "utf-8",
	...
	it("distinguishes an absent final hooks artifact from fallback and scanned content", () => {
		...
		assert.deepEqual(
			buildManagedCodexHookTrustState(hooksPath, "/repo", {
				hooksContent: null,
			}),
			{},
		);
		assert.equal(
			buildManagedCodexHookTrustToml(hooksPath, "/repo", {
				hooksContent: null,
			}),
			"",
		);
		...
		assert.ok(
			normalToml.includes(`[hooks.state."${hooksPath}:pre_tool_use:1:0"]`),
		);
	});

	it("builds trust state only for generated OMX hook handlers", () => {
		const state = buildManagedCodexHookTrustState(
	...
	it("matches Codex's normalized command hook hash identity", async () => {
		...
		assert.equal(
			state["/hooks.json:pre_tool_use:0:0"]?.trusted_hash,
			expectedHash,
		);
	});
	...
	it("matches Codex's normalized command hook hash identity for Windows shim commands", async () => {
		...
		assert.equal(
			state[`${hooksPath}:pre_tool_use:0:0`]?.trusted_hash,
			expectedHash,
		);
	});
	...
	it("hashes u64 timeout literals without losing values above Number.MAX_SAFE_INTEGER", async () => {
		...
		const scanTimeout = (timeout: string) =>
			scanManagedCodexHookTrustStateFromContent(
	...
	it("drops top-level managed hook state metadata from hooks.json", () => {
		const managedState = buildManagedCodexHookTrustState(
			"/hooks.json",
			"/repo",
		);
		const managedKey =
			Object.keys(managedState).find((key) => key.includes(":stop:")) ??
	...
	it("keeps Windows shim hook merge idempotent while replacing stale direct-node wrappers", () => {
				...
				SessionStart: [
					{
						...
						hooks: [
							{
								type: "command",
								command: 'node "D:\\old\\dist\\scripts\\codex-native-hook.js"',
							},
							{ type: "command", command: "echo keep-me" },
						],
					},
						...
						hooks: [
							{
								type: "command",
								command:
									'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "C:\\Users\\Ada\\.codex\\hooks\\omx-native-hook-windows-shim.ps1"',
							},
						],
					},
				],
		...
		const commands = merged.hooks.SessionStart.flatMap(
			(entry) => entry.hooks ?? [],
		).map((hook) => hook.command ?? "");
		assert.equal(
			commands.filter((command) =>
				/omx-native-hook-windows-shim\.ps1/.test(command),
			).length,
			1,
		);
		assert.equal(
			commands.filter((command) => /codex-native-hook\.js/.test(command))
				.length,
			0,
		);
		assert.ok(commands.includes("echo keep-me"));
	...
	for (const handler of [
		...
		it(`preserves executable foreign future-event ${handler.type} handlers after managed removal`, () => {
			...
			assert.deepEqual(
				(JSON.parse(plan.finalContent) as { hooks?: Record<string, unknown> })
					.hooks?.FutureEvent,
				futureEvent,
			);
		...
		const missing = getMissingManagedCodexHookEvents(
			JSON.stringify({
				hooks: {
					...
					UserPromptSubmit: [
						{
							hooks: [{ type: "command", command: "echo custom-only" }],
						},
					],
				},
			}),
	...
	it("ignores runtime codex-home hook mirrors before hook loading", async () => {
		...
		try {
			...
			await mkdir(join(cwd, ".omx", "runtime", "codex-home", "session-1"), {
				recursive: true,
			});
			await writeFile(
				canonicalPath,
				JSON.stringify(buildManagedCodexHooksConfig("/repo")),
			);
			await symlink(canonicalPath, mirrorPath);

			assert.equal(isRuntimeCodexHomeMirrorPath(mirrorPath, cwd), true);

			const result = await dedupeCodexHookConfigPaths(
				[canonicalPath, mirrorPath],
				cwd,
			);
			assert.deepEqual(
				result.paths.map((entry) => entry.path),
				[canonicalPath],
			);
			assert.deepEqual(
				result.skipped.map((entry) => entry.reason),
				["runtime_codex_home_mirror"],
			);
		} finally {
	...
	it("models Codex empty/default/null load behavior without accepting async null", () => {
		const valid = validateCodexHooksConfigStrict(
			JSON.stringify({
				hooks: {
					PreToolUse: [
						{
		...
		const invalid = validateCodexHooksConfigStrict(
			JSON.stringify({
				hooks: {
					PreToolUse: [
						{ hooks: [{ type: "command", command: "echo", async: null }] },
					],
				},
			}),
		);
	...
	it("matches pinned serde_json strict intake for whitespace, Unicode, numbers, and nesting", () => {
		const validTimeoutPrefix =
			'{"hooks":{"PreToolUse":[{"hooks":[{"type":"command","command":"echo","timeout":';
		for (const content of [
			...
			assert.equal(
				validateCodexHooksConfigStrict(content).ok,
				false,
	...
	it("uses the platform-effective Windows command before discovery and skipped-command diagnostics", () => {
		...
		const windows = validateCodexHooksConfigStrict(withWindowsCommand, {
			platform: "win32",
		});
		assert.equal(windows.ok, true);
		if (windows.ok) {
			assert.deepEqual(
				windows.discoveredCommands.map((entry) => entry.command),
				["echo windows"],
			);
			assert.equal(
				windows.diagnostics.some((entry) => entry.code === "empty_command"),
				false,
			);
		}
		...
		const windowsAlias = validateCodexHooksConfigStrict(withWindowsAlias, {
			platform: "win32",
		});
		const posix = validateCodexHooksConfigStrict(withWindowsAlias, {
			platform: "linux",
		});
		...
		if (windowsAlias.ok) {
			...
			assert.equal(
				windowsAlias.diagnostics.filter(
					(entry) => entry.code === "empty_command",
				).length,
				1,
			);
		}
		if (posix.ok)
			assert.deepEqual(
				posix.discoveredCommands.map((entry) => entry.command),
				["echo posix"],
			);
	});

	it("accepts Codex's match-all matcher and treats whitespace-only commands as skipped", () => {
		const strict = validateCodexHooksConfigStrict(
			JSON.stringify({
				hooks: {
					PreToolUse: [
						{
		...
		if (!strict.ok) return;
		assert.equal(
			strict.diagnostics.some((entry) => entry.code === "invalid_matcher"),
			false,
		);
		assert.equal(
			strict.diagnostics.filter((entry) => entry.code === "empty_command")
				.length,
			1,
		);
		assert.deepEqual(
			strict.discoveredCommands.map((entry) => entry.command),
			["echo discovered"],
		);
	...
	it("uses Rust-regex matcher syntax rather than accepting JavaScript-only assertions", () => {
		const validateMatcher = (matcher: string) =>
			validateCodexHooksConfigStrict(
				JSON.stringify({
					hooks: {
						PreToolUse: [
	...
	it("preserves ignored nested events, nonmatching hooks.state, and unknown raw members", () => {
		...
		assert.ok(
			plan.finalContent?.includes(`"Future":{"raw":[1,{"keep":true}]}`),
		);
	...
	it("migrates only exact historical root state and fails closed for nonmatching root state", () => {
		const exact = planManagedCodexHooksMerge(
			JSON.stringify({
				state: {
					"/hooks.json:stop:0:0": {
						trusted_hash: "sha256:legacy",
		...
		const nonmatching = planManagedCodexHooksMerge(
			JSON.stringify({
				state: {
					"/hooks.json:stop:0:0": {
						trusted_hash: "sha256:legacy",
	...
	it("coalesces identical root and nested legacy trust state but rejects conflicts", () => {
		...
		const matching = planManagedCodexHooksMerge(
			JSON.stringify({
	...
	it("records every raw group and handler occurrence across all ten Codex events", () => {
		...
		assert.equal(
			strict.diagnostics.filter((entry) => entry.code === "invalid_matcher")
				.length,
			1,
		);
	});
	...
	it("appends each missing OMX event once and makes the third setup byte-identical", () => {
		...
		const second = planManagedCodexHooksMerge(
			first.finalContent,
			"/repo",
			"/hooks.json",
		);
		...
		assert.ok(finalContent);
		const merged = JSON.parse(finalContent) as {
			hooks?: Record<string, unknown[]>;
		};
	...
	it("fails closed instead of leaving an opaque managed-only group tombstone during removal", () => {
		const command =
			buildManagedCodexHooksConfig("/repo").hooks.PreToolUse[0]!.hooks[0]!
				.command;
		const source = JSON.stringify({
			hooks: {
				PreToolUse: [
	...
	it("removes a suffix duplicate only when all surviving foreign coordinates stay fixed", () => {
		const command =
			buildManagedCodexHooksConfig("/repo").hooks.SessionStart[0]?.hooks[0]
				?.command;
		...
		const unsafePlan = planManagedCodexHooksMerge(
			unsafe,
			"/repo",
			"/hooks.json",
		);
	...
	it("fails closed for unsafe mixed removal and partial-corrupt OMX commands", () => {
		const command =
			buildManagedCodexHooksConfig("/repo").hooks.SessionStart[0]?.hooks[0]
				?.command;
		const unsafeMixed = planManagedCodexHooksRemoval(
			JSON.stringify({
				hooks: {
					SessionStart: [
						{
		...
		const partial = planManagedCodexHooksMerge(
			JSON.stringify({
				hooks: {
					PreToolUse: [
						{
							hooks: [
								{
									type: "command",
									command:
										'node "/repo/dist/scripts/codex-native-hook.js" --extra',
								},
							],
						},
					],
		...
		const sharedSkipped = planManagedCodexHooksMerge(
			JSON.stringify({
				hooks: {
					PreToolUse: [
						{
							hooks: [
								{
									type: "command",
									command:
										buildManagedCodexHooksConfig("/repo").hooks.PreToolUse[0]
											?.hooks[0]?.command,
								},
