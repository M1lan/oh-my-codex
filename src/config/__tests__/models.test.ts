...
describe("getModelForMode", () => {
	...
	it("resolves different modes independently", async () => {
		await writeConfig({
			models: { team: "gpt-4.1", autopilot: "o4-mini", ralph: "gpt-5" },
		});
		assert.equal(getModelForMode("team"), "gpt-4.1");
	...
	it("uses .omx-config.json env.OMX_DEFAULT_FRONTIER_MODEL when shell env is absent", async () => {
		await writeConfig({
			env: { OMX_DEFAULT_FRONTIER_MODEL: "frontier-local" },
		});
	...
	it("uses .omx-config.json env.OMX_DEFAULT_STANDARD_MODEL when shell env is absent", async () => {
		await writeConfig({
			env: { OMX_DEFAULT_STANDARD_MODEL: "standard-local" },
		});
	...
	it("uses .omx-config.json env.OMX_DEFAULT_SPARK_MODEL when shell env is absent", async () => {
		await writeConfig({
			env: { OMX_DEFAULT_SPARK_MODEL: "spark-local" },
			models: { team: "gpt-4.1" },
		});
	...
	it("keeps legacy, per-agent, and root reasoning vocabularies distinct", () => {
		...
		const ambiguousUnsupportedEffort: AmbiguousUnsupportedReasoningEffort =
			"ultra";
		...
		assert.deepEqual(CANONICAL_REASONING_EFFORTS, [
			"low",
			"medium",
			"high",
			"xhigh",
		...
		assert.deepEqual(PER_AGENT_REASONING_EFFORTS, [
			"low",
			"medium",
			"high",
			"xhigh",
		...
		assert.deepEqual(ROOT_REASONING_EFFORTS, [
			"low",
			"medium",
			"high",
			"xhigh",
		...
		if (isAmbiguousUnsupportedReasoningEffort(ambiguousCandidate)) {
			const narrowedAmbiguousCandidate: AmbiguousUnsupportedReasoningEffort =
				ambiguousCandidate;
		...
		if (isUnsupportedRootReasoningEffort(rootCandidate)) {
			...
			const narrowedRootCandidate: RootUnsupportedReasoningEffort =
				rootCandidate;
