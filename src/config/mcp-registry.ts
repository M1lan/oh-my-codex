...
function normalizeTimeout(
	...
	if (typeof value !== "number" || !Number.isFinite(value) || value <= 0) {
		warnings.push(
			`registry entry "${name}" has invalid timeout; ignoring timeout`,
		);
		return undefined;
	}
...
function normalizeEntry(
	...
	if (
		...
		(!Array.isArray(argsValue) ||
			argsValue.some((item) => typeof item !== "string"))
	) {
...
export function getLegacyUnifiedMcpRegistryCandidate(
	homeDir = homedir(),
): string {
...
export async function loadUnifiedMcpRegistry(
	...
	try {
		...
		warnings.push(
			`failed to parse shared MCP registry at ${sourcePath}: ${String(error)}`,
		);
