...
const PROFILE_SPECS: Record<DeepInterviewProfile, DeepInterviewProfileSpec> = {
	quick: {
		...
		defaults: { threshold: 0.3, maxRounds: 5 },
	},
	standard: {
		thresholdKey: "standardThreshold",
		maxRoundsKey: "standardMaxRounds",
		defaults: { threshold: 0.2, maxRounds: 12 },
	},
	deep: {
...
const DEEP_INTERVIEW_INVOCATION_PATTERN =
	/(?:^|\s)(?:\$(?:[A-Za-z0-9_-]+:)?deep-interview|deep[-\s]interview)(?=\s|$)/i;
...
function normalizeThreshold(value: unknown): number | undefined {
	...
	return normalized !== undefined && normalized > 0 && normalized <= 1
		? normalized
		: undefined;
}
...
function warnMalformedConfig(configPath: string, error: unknown): void {
	...
	console.warn(
		`[omx] warning: ignoring malformed deep-interview config at ${configPath}: ${message}`,
	);
}

function extractDeepInterviewTable(
	parsed: unknown,
): DeepInterviewConfigTable | null {
	if (
		!isRecord(parsed) ||
		!isRecord(parsed.omx) ||
		!isRecord(parsed.omx.deepInterview)
	)
		return null;
	return parsed.omx.deepInterview as DeepInterviewConfigTable;
}

function readDeepInterviewConfigTable(
	configPath: string,
): DeepInterviewConfigReadResult {
	if (!existsSync(configPath)) return { status: "missing" };

	try {
		const table = extractDeepInterviewTable(
			parseToml(readFileSync(configPath, "utf-8")) as unknown,
		);
...
): DeepInterviewProfile {
	return (
		parseDeepInterviewProfileFromText(text) ??
...
function resolveProfileConfig(
	table: DeepInterviewConfigTable,
	profile: DeepInterviewProfile,
): Pick<DeepInterviewRuntimeConfig, "threshold" | "maxRounds"> {
	const spec = PROFILE_SPECS[profile];
	return {
		threshold:
			normalizeThreshold(table[spec.thresholdKey]) ?? spec.defaults.threshold,
		maxRounds:
			normalizeMaxRounds(table[spec.maxRoundsKey]) ?? spec.defaults.maxRounds,
	};
}
...
export function parseDeepInterviewProfileFromText(
	text: string | undefined,
): DeepInterviewProfile | undefined {
	const input = text ?? "";
	const invocationMatch = DEEP_INTERVIEW_INVOCATION_PATTERN.exec(input);
	if (!invocationMatch) return undefined;

	const afterInvocation = input
		.slice((invocationMatch.index ?? 0) + invocationMatch[0].length)
		.trimStart();
	for (const token of afterInvocation.split(/\s+/)) {
		if (!token) continue;
		if (!token.startsWith("--")) return undefined;
		const profile = normalizeProfile(
			PROFILE_FLAG_TOKEN_PATTERN.exec(token)?.[1],
		);
...
export function getDeepInterviewConfigCandidatePaths(
	options: Pick<DeepInterviewConfigOptions, "cwd" | "homeDir">,
): DeepInterviewConfigCandidate[] {
	const home = options.homeDir || homedir();
	const projectRoot = findGitLayout(options.cwd)?.worktreeRoot ?? options.cwd;
	const candidates: DeepInterviewConfigCandidate[] = [
		{
			path: join(options.cwd, ".omx", "config.toml"),
			precedence: "project-omx",
		},
	...
	if (projectRoot !== options.cwd) {
		candidates.push(
			{
				path: join(projectRoot, ".omx", "config.toml"),
				precedence: "project-omx",
			},
	...
	candidates.push({
		path: join(home, ".omx", "config.toml"),
		precedence: "user",
	});
...
export function resolveDeepInterviewRuntimeConfig(
	options: DeepInterviewConfigOptions,
): DeepInterviewRuntimeConfig | null {
