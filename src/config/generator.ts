	...
	return {
		assessments: Object.fromEntries(
			LEGACY_MULTI_AGENT_KEYS.map((key) => {
				...
				const value = isRecord(parsed[table])
					? parsed[table][entry]
					: undefined;
				if (value === undefined) {
...
function analyzeOmxSeededBehavioralDefaults(
	...
	const starts = lines
		...
		.filter(
			({ line }) =>
				line.content === OMX_SEEDED_BEHAVIORAL_DEFAULTS_START_MARKER,
		);
	const ends = lines
		.map((line, index) => ({ line, index }))
		.filter(
			({ line }) => line.content === OMX_SEEDED_BEHAVIORAL_DEFAULTS_END_MARKER,
		);
...
function projectHookTrustState(
	config: string,
...
function projectTrustSemanticView(
	config: string,
): { source: string; prefix: string } | undefined {
...
function projectTrustMarkerRange(
	config: string,
): ProjectTrustMarkerRange | undefined {
	const lexical = analyzeTomlSource(config);
	if (!lexical.isUnambiguous) return undefined;
	const lines = sourceLines(config);
	const starts = lines.filter(
		(line, index) =>
			lexical.lineStartsOutsideMultiline[index] &&
			line.content.trim() === OMX_PROJECT_TRUST_START_MARKER,
	);
	const ends = lines.filter(
		(line, index) =>
			lexical.lineStartsOutsideMultiline[index] &&
			line.content.trim() === OMX_PROJECT_TRUST_END_MARKER,
	);
	if (starts.length === 0 && ends.length === 0)
		return {
			start: config.length,
			end: config.length,
			payloadStart: config.length,
			payloadEnd: config.length,
		};
	if (
		starts.length !== 1 ||
		ends.length !== 1 ||
	...
	return {
		...
		payloadStart: start.end,
		payloadEnd: end.start,
	};
}
...
function hasInlineProjectTrustMarkerComment(config: string): boolean {
	...
	return sourceLines(config).some((line, index) => {
		...
		return (
			(trimmed.startsWith(OMX_PROJECT_TRUST_START_MARKER) &&
				trimmed !== OMX_PROJECT_TRUST_START_MARKER) ||
			(trimmed.startsWith(OMX_PROJECT_TRUST_END_MARKER) &&
				trimmed !== OMX_PROJECT_TRUST_END_MARKER)
		);
	});
}
...
function inventoryProjectTrustPayload(
	...
	| { parsed: Record<string, unknown>; headers: ProjectTrustPayloadHeader[] }
	| undefined {
	const parsed = safeParseToml(payload);
	const lexical = analyzeTomlSource(payload);
	const headers = projectTrustHeaders(payload, {
		start: 0,
		end: payload.length,
	});
	...
	for (const [index, line] of lines.entries()) {
		...
		const isBlankOrComment =
			line.content.trim() === "" || /^\s*#/.test(line.content);
	...
	for (const header of headers) {
		...
		if (
			...
			if (
				...
				value.trusted_hash.length === 0 ||
				Object.keys(value).some((key) => key !== "trusted_hash")
			)
				return undefined;
...
function projectTrustSourceNestedStructuresAreExplicit(
	...
	return Object.entries(projects).every(
		([key, value]) =>
			isPlainTomlRecord(value) &&
			hasHeader(["projects", key]) &&
			visit(value, ["projects", key]),
	);
}
...
function projectTrustPayloadNestedStructuresAreExplicit(
	...
	const hasHeader = (path: readonly string[]): boolean =>
		headers.some(
			(header) =>
				header.kind === "project" && projectTrustEqual(header.path, path),
		);
	const visit = (
		value: Record<string, unknown>,
		path: readonly string[],
...
function projectTrustPayloadTableSpan(
	source: string,
): ProjectTrustPayloadTableSpan {
...
function projectTrustPayloadTableBodyHasCommentsOrBlanks(
	source: string,
): boolean {
...
function reconcileProjectTrustPayload(
	...
	payloadInventory: {
		parsed: Record<string, unknown>;
		headers: ProjectTrustPayloadHeader[];
	},
	...
	const payloadProjectKeys = new Set(
		headers
			.filter((header) => header.kind === "project")
			.map((header) => header.path[1]!),
	);
	...
	for (const key of payloadProjectKeys) {
		const projectHeaders = headers.filter(
			(header) => header.kind === "project" && header.path[1] === key,
		);
		const parent = projectHeaders.find((header) => header.path.length === 2);
		const payloadValue = projectTrustValueAtPath(payloadInventory.parsed, [
			"projects",
			key,
		]);
		...
		if (externalValue !== undefined) {
			if (
				!projectTrustEqual(payloadValue, externalValue) ||
				(runtimeValue !== undefined &&
					!projectTrustEqual(runtimeValue, externalValue))
			)
				return undefined;
		...
		if (
			!parent ||
			projectHeaders.length !== 1 ||
			!isPlainTomlRecord(runtimeValue)
		)
			return undefined;
	...
	for (const header of headers.filter(
		...
		const payloadValue = projectTrustValueAtPath(
			payloadInventory.parsed,
			header.path,
		);
		...
		if (
			!isPlainTomlRecord(runtimeValue) ||
			typeof runtimeValue.trusted_hash !== "string" ||
			runtimeValue.trusted_hash.length === 0
		) {
			replacementByHeader.set(header.start, undefined);
			continue;
		}
		replacementByHeader.set(
			header.start,
			`[hooks.state."${escapeTomlBasicString(key)}"]${eol}trusted_hash = "${escapeTomlBasicString(runtimeValue.trusted_hash)}"`,
		);
	}
	...
	for (const [index, header] of headers.entries()) {
		...
		const span = projectTrustPayloadTableSpan(
			payload.slice(header.start, segmentEnd),
		);
	...
	for (const [key, value] of Object.entries(runtimeProjects).sort(
	...
	for (const [key, value] of Object.entries(runtimeHooksState ?? {}).sort(
		([left], [right]) => left.localeCompare(right),
	)) {
		if (
			!key.startsWith(`${projectHooksPath}:`) ||
			externalHookTrustState?.[key] !== undefined ||
			headers.some((header) => header.kind === "hook" && header.path[2] === key)
		)
			continue;
		...
		if (
			...
			continue;
		additions.push(
			`[hooks.state."${escapeTomlBasicString(key)}"]${eol}trusted_hash = "${escapeTomlBasicString(value.trusted_hash)}"${eol}`,
		);
	}
	...
	return Object.keys(safeParseToml(reconciled) ?? {}).length === 0
		? ""
		: reconciled;
}
...
function projectTrustHeaders(
	config: string,
	region: SourceSpan,
): ProjectTrustHeader[] | undefined {
	const lexical = analyzeTomlSource(config);
	if (!lexical.isUnambiguous) return undefined;
	const lines = sourceLines(config);
...
function inventoryProjectTrustSource(
	config: string,
): ProjectTrustSourceInventory | undefined {
	...
	const projectHeaders = headers.filter(
		(header) => header.path[0] === "projects",
	);
	if (projectHeaders.some((header) => header.path.length < 2)) return undefined;
	for (const key of Object.keys(projects ?? {})) {
		if (
			projectHeaders.filter(
				(header) => header.path.length === 2 && header.path[1] === key,
			).length !== 1
		) {
			return undefined;
		}
...
function projectTrustCandidateIsSafe(
	...
	const hasEquivalentExternalBytes = (actual: string): boolean => {
		...
		return (
			splitProjectTrustTrailingEols(actual).body ===
			splitProjectTrustTrailingEols(external).body
		);
	};
	if (hasMarker) {
		const candidateExternal = `${candidate.slice(0, marker.start)}${candidate.slice(marker.end)}`;
		if (
			!hasEquivalentExternalBytes(candidateExternal) &&
			!(
				candidate.startsWith(external) &&
				candidate.slice(marker.end).length === 0 &&
	...
	for (const [key, value] of Object.entries(expectedProjects)) {
		...
		if (
			inventory.headers.filter(
				(header) =>
					header.path[0] === "projects" &&
...
function projectTrustPayloadProjectFamiliesAreValid(
	...
	for (const key of projectKeys) {
		const family = headers.filter(
			(header) => header.kind === "project" && header.path[1] === key,
		);
		const parents = family.filter((header) => header.path.length === 2);
		if (
			parents.length !== 1 ||
			!projectTrustFamilyIsContiguous(headers, parents[0]!)
		)
			return false;
	}
	return true;
}
function projectTrustFamilyIsContiguous(
	headers: readonly ProjectTrustHeader[],
	parent: ProjectTrustHeader,
): boolean {
	...
	for (let index = first + 1; index < headers.length; index += 1) {
		...
		const belongsToFamily =
			path[0] === "projects" && path[1] === parent.path[1];
		if (!belongsToFamily) {
			ended = true;
			continue;
		}
...
function projectTrustEqual(left: unknown, right: unknown): boolean {
	...
	if (Array.isArray(left) || Array.isArray(right)) {
		return (
			...
			left.every((value, index) => projectTrustEqual(value, right[index]))
		);
	}
	...
	return (
		...
		leftKeys.every(
			(key, index) =>
				key === rightKeys[index] && projectTrustEqual(left[key], right[key]),
		)
	);
}
...
function renderProjectTrust(
	...
	const body = serialized
		...
		.join("\n");
	return body
		? `[projects."${escapeTomlBasicString(projectKey)}"]\n${body}`
		: undefined;
}

function splitProjectTrustTrailingEols(source: string): {
	body: string;
	trailingEols: string;
} {
	...
	return {
		body: source.slice(0, source.length - trailingEols.length),
		trailingEols,
	};
}

function appendProjectTrustMarker(
	base: string,
	payload: string,
	eol: string,
	trailingEols: string,
...
function reconcileProjectScopeTrustState(
	...
	if (
		hasInlineProjectTrustMarkerComment(durable.source) ||
		hasInlineProjectTrustMarkerComment(runtime.source)
	)
	...
	const payloadInventory = hasMarker
		...
		: {
				...
				headers: [] as ProjectTrustPayloadHeader[],
			};
	...
	const external = hasMarker
		...
		: durable.source;
	const trailingEols = splitProjectTrustTrailingEols(
		durable.source,
	).trailingEols;
	const externalInventory = inventoryProjectTrustSource(external);
	...
	const headersBeforeMarker = projectTrustHeaders(durable.source, {
		start: 0,
		end: marker.start,
	});
	const headersAfterMarker = projectTrustHeaders(durable.source, {
		start: marker.end,
		end: durable.source.length,
	});
	...
	for (const key of Object.keys(externalProjects)) {
		const family = headers.filter(
			(header) => header.path[0] === "projects" && header.path[1] === key,
		);
		const parents = family.filter((header) => header.path.length === 2);
		...
		if (
			family.some(
				(header) => header.start < marker.start !== parentIsBeforeMarker,
			)
		)
			return projectConfig;
	...
	const hooksState =
		isPlainTomlRecord(hooks) && isPlainTomlRecord(hooks.state)
			? hooks.state
			: undefined;
	...
	if (!inventoryProjectTrustPayload(nextPayload, projectHooksPath))
		return projectConfig;
	const candidate = appendProjectTrustMarker(
		external,
		nextPayload,
		eol,
		trailingEols,
...
export function repairProjectScopeTrustStateForLaunch(
	...
	const payload = durable.source.slice(marker.payloadStart, marker.payloadEnd);
	return reconcileProjectScopeTrustState(
		projectConfig,
		payload,
		projectHooksPath,
		true,
...
type ManagedHookTrustStateExpectations = ReadonlyMap<
	string,
	ReadonlySet<string>
>;
...
function isMultilineTomlString(
	mode: TomlSourceStringMode | undefined,
): boolean {
...
function lineMaySpellHooksState(line: string): boolean {
	...
	if (line[cursor] === ".") {
		...
		return (
			state === undefined || state.incomplete || isTomlSourceKey(state, "state")
		);
	}
	if (line[cursor] !== "=") return false;
	cursor = skipTomlSourceWhitespace(line, cursor + 1);
	...
	return (
		line[cursor] === "{" && inlineTomlTableMaySpellStateKey(line, cursor + 1)
	);
}
...
function parseTomlSourceArrayTableOpener(
	...
	return {
		firstKey,
		openerOnly:
			firstKey === undefined && (cursor >= line.length || line[cursor] === "#"),
	};
}
...
function hasUnprovenManagedMarkerHooksStateSyntax(
	...
	for (let index = 0; index < source.lines.length; index += 1) {
		...
		const arrayTableOpener =
			!isParsedStatementLine || isParsedStatementStartLine
				? parseTomlSourceArrayTableOpener(line)
				: undefined;
		if (
			arrayTableOpener?.firstKey &&
			isTomlSourceKey(arrayTableOpener.firstKey, "hooks")
		) {
			return true;
		}
		...
		if (
			lineMaySpellHooksState(line) ||
			(insideHooksTable && lineMaySpellStateKey(line))
		) {
			return true;
		}
		const hasIncompleteHooksPrefix =
			hasIncompleteTomlSourceHooksKeyPrefix(line);
...
function hasOnlyManagedHookTrustStateField(
	...
	if (
		!isPlainTomlRecord(hooks) ||
		Object.keys(hooks).length !== 1 ||
		!Object.hasOwn(hooks, "state")
	) {
		return false;
	}
...
function parseTomlSourceTableHeader(
	line: string,
): TomlSourceTableHeader | undefined {
...
function addManagedHookTrustStateSourceRepresentations(
	...
	if (state) {
		...
		for (const [key, value] of Object.entries(state)) {
			const hasExactSemanticScope =
				hasOnlyManagedHookTrustStateField(parsed, key) &&
				(!containingTableScope ||
					(!containingTableScope.hasComment &&
						hasOnlyManagedHookTrustStateField(
							containingTableScope.parsed,
							key,
						)));
...
function collectManagedHookTrustStateSourceRepresentations(config: string): {
	source: TomlSourceLexicalAnalysis;
	representations: Map<string, ManagedHookTrustStateSourceRepresentation[]>;
	...
	const representations = new Map<
		string,
		ManagedHookTrustStateSourceRepresentation[]
	>();
	...
	for (const { index, header } of headers) {
		const boundaryIndex = sortedBoundaries.findIndex(
...
function managedMarkerTrustStateConflicts(
	...
	representations: ReadonlyMap<
		string,
		readonly ManagedHookTrustStateSourceRepresentation[]
	>,
	managedMarkerRanges: readonly SourceSpan[],
	...
	if (
		invalidMarkerContainedHooksStateSpans.some(isInsideManagedMarkerRange) ||
	...
	for (const [key, sourceRepresentations] of representations) {
		for (const representation of sourceRepresentations) {
			...
			const isExactOwnedRepresentation =
				...
				isExactlyManagedHookTrustStateValue(
...
function stripProofManagedCodexHookTrustStateTables(
	...
	for (const [key, sourceRepresentations] of representations) {
		...
		if (
			...
			continue;
		const mayRemoveAll =
			sourceRepresentations.length === 1
...
function stripManagedCodexHookTrustStateForRefresh(
	...
	return stripProofManagedCodexHookTrustStateTables(
		config,
		managedHookTrustStateExpectations(
			priorManagedTrustState,
			finalManagedTrustState,
		),
	);
}
...
function identicalLegacyHookTrustStateEntry(
	...
	if (
		...
		Object.keys(existing).some(
			(key) => key !== "trusted_hash" && key !== "enabled",
		)
	) {
		return false;
	}
	return (
		Object.hasOwn(existing, "enabled") === Object.hasOwn(expected, "enabled") &&
		existing.enabled === expected.enabled
	);
}

function legacyHookTrustStateConflict(
...
function appendLegacyHookTrustState(
	...
	const parsed = safeParseToml(config);
	const keys = Object.keys(legacyTrustState).sort((left, right) =>
	...
	const entriesToAppend = Object.entries(legacyTrustState).filter(
		([key]) => !Object.hasOwn(existingTrustState, key),
	);
...
export function stripExistingOmxBlocks(
	...
	const removalRanges = markerRanges.map((range) => {
		...
		const start =
			previousLine && /^# =+$/.test(previousLine.content.trim())
				? previousLine.start
				: startLine.start;
...
function managedHookTrustProofRequiredForRepair(config: string): boolean {
	const inventory = collectManagedHookTrustStateSourceRepresentations(config);
	return (
		managedMarkerTrustStateConflicts(
			inventory.source,
			inventory.representations,
...
async function launchRepairOptionsWithManagedHookTrustProof(
	...
	const hooksPath =
		options.codexHooksFile ?? resolve(configPath, "..", "hooks.json");
