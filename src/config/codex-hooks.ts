...
function jsonValueFromUnknown(value: unknown): JsonValue {
	...
	if (typeof value === "number") {
		if (!Number.isFinite(value))
			throw new TypeError(
				"Managed Codex hook configuration numbers must be finite JSON values.",
			);
		return value;
	}
	...
	throw new TypeError(
		"Managed Codex hook configuration must be JSON-serializable.",
	);
}
...
function jsonObjectFromUnknown(value: unknown): JsonObject {
	...
	if (
		...
		throw new TypeError(
			"Managed Codex hook configuration root must be a JSON object.",
		);
	}
	return jsonValue;
}
...
function isProofValidNodeExecutable(nodePath: string): boolean {
	...
	return (
		isQualifiedWindowsFilesystemPath(nodePath) &&
...
function isProofValidNativeHookScriptPath(hookScriptPath: string): boolean {
	return (
		isQualifiedWindowsFilesystemPath(hookScriptPath) &&
...
export function classifyManagedCodexNativeHookWindowsShimOwnership(
	...
	if (
		!Buffer.from(decoded, "utf-8").equals(content) ||
		!decoded.startsWith("\uFEFF")
	) {
		return "modified";
	}
	...
	const nodePath = lines[2]?.startsWith("$startInfo.FileName = ")
		? decodePowerShellSingleQuotedLiteral(
				lines[2]!.slice("$startInfo.FileName = ".length),
			)
		: null;
	const encodedHookScript = lines[7]?.startsWith("$startInfo.Arguments = ")
		? decodePowerShellSingleQuotedLiteral(
				lines[7]!.slice("$startInfo.Arguments = ".length),
			)
		: null;
	const hookScriptPath =
		encodedHookScript === null
			? null
...
export function buildManagedCodexNativeHookCommand(
	...
	optionsOrPlatform:
		| HookCommandPlatform
		| ManagedCodexHookOptions = process.platform,
): string {
	const options =
...
export function buildManagedCodexHooksConfig(
	...
	return {
		hooks: {
			...
			PreToolUse: [buildCommandHook(command)],
			PostToolUse: [buildCommandHook(command)],
			UserPromptSubmit: [buildCommandHook(command)],
			PreCompact: [buildCommandHook(command)],
			PostCompact: [buildCommandHook(command)],
			Stop: [
				buildCommandHook(command, {
					timeout: 30,
				}),
...
const KNOWN_LEGACY_TRUST_STATE_ENTRY_FIELDS = new Set([
	"trusted_hash",
	"enabled",
]);
...
function containsRustUnsupportedJavaScriptMatcherSyntax(
	matcher: string,
): boolean {
	for (let index = 0; index < matcher.length; index += 1) {
		...
		if (
			marker === "<" &&
			(matcher[index + 3] === "=" || matcher[index + 3] === "!")
		) {
			return true;
		}
	}
...
function isValidRustRegexEscape(matcher: string, index: number): boolean {
	...
	if (
		RUST_SIMPLE_REGEX_ESCAPES.has(kind) ||
		RUST_ESCAPABLE_REGEX_PUNCTUATION.has(kind)
	)
		return true;
	if (kind === "x")
		return (
			matcher[index + 3] !== undefined &&
			isHexadecimal(matcher.slice(index + 2, index + 4))
		);
	if (kind === "u") {
	...
	if (kind === "p" || kind === "P") {
		...
		return (
			...
			isValidRustUnicodeProperty(matcher.slice(index + 3, end))
		);
	}
	return false;
}
...
function duplicateKnownProperty(
	node: JsonObjectNode,
...
function isU64Node(node: JsonNode): boolean {
	...
	return (
		node.raw.length < maximum.length ||
...
function validateCommandHandler(
	...
	if (duplicate) {
		return planError(
			"invalid_document",
			`Duplicate command hook field ${duplicate}.`,
			{
				eventName,
				groupIndex,
				handlerIndex,
	...
	if (stringProperty(node, "command") === undefined) {
		return planError(
			"invalid_document",
			"Command hooks require a string command.",
			{
				eventName,
				groupIndex,
				handlerIndex,
	...
	if (windows && windowsAlias) {
		return planError(
			"invalid_document",
			"Command hook Windows aliases cannot both be present.",
			{
				eventName,
				groupIndex,
				handlerIndex,
	...
	for (const property of [windows, windowsAlias]) {
		if (
			...
			property.value.kind !== "null"
		) {
			return planError(
				"invalid_document",
				"Command hook Windows command must be a string or null.",
				{
					eventName,
					groupIndex,
					handlerIndex,
	...
	if (timeout && timeout.value.kind !== "null" && !isU64Node(timeout.value)) {
		return planError(
			"invalid_document",
			"Command hook timeout must be an unsigned 64-bit integer or null.",
			{
				eventName,
				groupIndex,
				handlerIndex,
	...
	if (asynchronous && asynchronous.value.kind !== "boolean") {
		return planError(
			"invalid_document",
			"Command hook async must be a boolean.",
			{
				eventName,
				groupIndex,
				handlerIndex,
	...
	if (
		...
		status.value.kind !== "null"
	) {
		return planError(
			"invalid_document",
			"Command hook statusMessage must be a string or null.",
			{
				eventName,
				groupIndex,
				handlerIndex,
...
function validateCodexHooksDocument(
	...
	if (!root || root.kind !== "object") {
		return {
			...
			error: planError(
				"invalid_document",
				"hooks.json must contain a JSON object.",
			),
		};
	}
	...
	const rootUnknown = root.properties.find(
		(property) => !KNOWN_ROOT_FIELDS.has(property.key),
	);
	if (rootUnknown) {
		return {
			ok: false,
			error: planError(
				"invalid_document",
				`Codex does not accept unknown root field ${rootUnknown.key}.`,
			),
		};
	}
	...
	if (hooksProperty && hooksProperty.value.kind !== "object") {
		return {
			...
			error: planError(
				"invalid_document",
				"Root hooks must be an object when present.",
			),
		};
	}
	const hooksNode =
		hooksProperty?.value.kind === "object" ? hooksProperty.value : undefined;
	const duplicateEvent =
		hooksNode && duplicateKnownProperty(hooksNode, KNOWN_HOOK_EVENT_FIELDS);
	if (duplicateEvent) {
		return {
			ok: false,
			error: planError(
				"invalid_document",
				`Duplicate hooks event ${duplicateEvent}.`,
			),
		};
	}
	...
	for (const eventName of CODEX_HOOK_EVENTS) {
		const eventProperty = hooksNode
			? objectProperty(hooksNode, eventName)
			: undefined;
		...
		if (eventProperty.value.kind !== "array") {
			return {
				...
				error: planError(
					"invalid_document",
					`${eventName} must be an array of matcher groups.`,
					{ eventName },
				),
			};
		}
		for (const [
			groupIndex,
			groupNode,
		] of eventProperty.value.elements.entries()) {
			if (groupNode.kind !== "object") {
				return {
					ok: false,
					error: planError(
						"invalid_document",
						"Hook matcher groups must be objects.",
						{ eventName, groupIndex },
					),
				};
			}
			...
			if (duplicate) {
				return {
					...
					error: planError(
						"invalid_document",
						`Duplicate matcher group field ${duplicate}.`,
						{ eventName, groupIndex },
					),
				};
			}
			...
			if (
				matcher &&
				matcher.value.kind !== "string" &&
				matcher.value.kind !== "null"
			) {
				return {
					ok: false,
					error: planError(
						"invalid_document",
						"Hook matcher must be a string or null.",
						{ eventName, groupIndex },
					),
				};
			}
			...
			if (hooks && hooks.value.kind !== "array") {
				return {
					...
					error: planError(
						"invalid_document",
						"Matcher group hooks must be an array when present.",
						{ eventName, groupIndex },
					),
				};
			}
			const invalidMatcher =
				matcherIsAware(eventName) &&
				matcher?.value.kind === "string" &&
			...
			if (invalidMatcher) {
				diagnostics.push({
					...
					groupIndex,
					message:
						"Codex skips groups whose matcher is not a valid regular expression.",
				});
			}
			const handlers =
				hooks?.value.kind === "array" ? hooks.value.elements : [];
			for (const [handlerIndex, handlerNode] of handlers.entries()) {
				handlerOccurrences.push({
					eventName,
					groupIndex,
					handlerIndex,
					span: handlerNode,
				...
				const duplicateType = duplicateKnownProperty(
					handlerNode,
					KNOWN_HANDLER_TYPE_FIELD,
				);
				if (duplicateType) {
					return {
						ok: false,
						error: planError(
							"invalid_document",
							"Duplicate hook handler type field.",
							{ eventName, groupIndex, handlerIndex },
						),
					};
				}
				...
				const commandError = validateCommandHandler(
					handlerNode,
					eventName,
					groupIndex,
					handlerIndex,
				...
				const asynchronous =
					asyncProperty?.value.kind === "boolean" &&
...
function tokenizeWindowsDirectCommand(command: string): string[] | null {
	if (
		...
		containsAdjacentDoubleQuotes(command)
	)
		return null;
	...
	for (let index = 0; index < command.length; ) {
		...
		if (
			...
			(!quoted &&
				UNQUOTED_UNSAFE_WINDOWS_DIRECT_COMMAND_CHARACTERS.has(character))
		)
			return null;
		if (character === "\\") {
			const start = index;
	...
	if (tokenStarted) words.push(word);
	return words.length > 0 && words.every((value) => !value.includes('"'))
		? words
		: null;
}
...
function isValidWindowsFilesystemPath(path: string): boolean {
	if (!path || containsControlCharacter(path) || /["<>|?*]/.test(path))
		return false;
	if (
		/^(?:[\\/]{3,}|[\\/]{1,2}[?.][\\/]|[\\/]{1,2}\?\?[\\/]|[\\/](?:device|globalroot)[\\/])/i.test(
			path,
		)
	)
		return false;
	...
	return path
		...
		.every(
			(segment) =>
				...
				(!segment.endsWith(".") &&
					!segment.endsWith(" ") &&
					!isReservedWindowsDeviceComponent(segment)),
		);
}
...
function isQualifiedWindowsFilesystemPath(path: string): boolean {
	...
	return (
		...
		uncSegments[0]!.length > 0 &&
		uncSegments[1]!.length > 0
	);
}
...
function commandBasename(path: string, platform: HookCommandPlatform): string {
	const executable =
		platform === "win32" ? path.split(/[\\/]/).pop() : path.split("/").pop();
	return platform === "win32"
		? (executable?.toLowerCase() ?? "")
		: (executable ?? "");
}

function isNativeHookScriptPath(
	path: string,
	platform: HookCommandPlatform,
): boolean {
	return platform === "win32"
...
function isNativeHookShimPath(path: string): boolean {
	return (
		isQualifiedWindowsFilesystemPath(path) && hasNativeHookShimSuffix(path)
	);
}
...
function isSupportedDirectNodeExecutablePath(
	...
	const bareExecutable =
		platform === "win32" ? executablePath.toLowerCase() : executablePath;
	const supportedBareExecutable =
		platform === "win32"
			? bareExecutable === "node" || bareExecutable === "node.exe"
			: bareExecutable === "node";
	return (
		supportedBareExecutable ||
		((platform === "win32"
...
function isSupportedPowerShellExecutablePath(executablePath: string): boolean {
	return (
		executablePath.toLowerCase() === "powershell.exe" ||
		(isQualifiedWindowsFilesystemPath(executablePath) &&
			commandBasename(executablePath, "win32") === "powershell.exe")
	);
}

function isValidDirectNodeCommand(
	command: string,
	platform: HookCommandPlatform,
): boolean {
	const words =
...
const OMX_COMMAND_PATTERN =
	/(?:codex-native-hook\.js|omx-native-hook-windows-shim\.ps1)/i;
...
function decodedPosixCommandWords(command: string): string[] {
	...
	for (let index = 0; index < command.length; index += 1) {
		...
		if (
			...
			UNQUOTED_POSIX_SHELL_ACTIVE_CHARACTERS.has(character)
		) {
			flushWord();
			continue;
		}
...
function commandMentionsDecodedPosixOmx(command: string): boolean {
	return decodedPosixCommandWords(command).some((word) =>
		OMX_COMMAND_PATTERN.test(word),
	);
}

function commandMentionsOmx(command: string): boolean {
	return (
		OMX_COMMAND_PATTERN.test(command) ||
		OMX_COMMAND_PATTERN.test(command.replace(/["']/g, ""))
	);
}

function isValidManagedCommand(
	command: string,
	platform: HookCommandPlatform,
): boolean {
	return (
		isValidDirectNodeCommand(command, platform) ||
...
function buildExactCurrentWindowsHostShimCommand(
	...
	const shimPath = join(
		...
		...WINDOWS_NATIVE_HOOK_SHIM_RELATIVE_PATH,
	);
	const powerShellPath = resolveWindowsPowerShellPath(options.env);
	return `& ${quotePowerShellLiteral(powerShellPath)} -NoProfile -ExecutionPolicy Bypass -File ${quotePowerShellLiteral(shimPath)}`;
}
...
function isExactCurrentWindowsShimCommand(
	...
	if (
		...
		return false;
	return (
		command === buildManagedCodexNativeHookCommand("", options) ||
...
function isValidHistoricalManagedCommand(command: string): boolean {
	return (
		isValidDirectNodeCommand(command, "linux") ||
...
function classifyManagedCommand(
	...
	const supplied = [
		...
		...(typeof windows === "string"
			? [{ command: windows, platform: "win32" as const }]
			: []),
	];
	...
	for (const suppliedCommand of supplied) {
		...
		const mentionsDecodedPosixOmx = commandMentionsDecodedPosixOmx(
			suppliedCommand.command,
		);
		if (
			isValidManagedCommand(
				suppliedCommand.command,
				suppliedCommand.platform,
			) ||
			isExactCurrentWindowsShimCommand(
				suppliedCommand.command,
				suppliedCommand.platform,
				options,
			)
...
function matcherOwnershipError(
	...
	if (
		...
		!isValidCodexMatcher(matcher)
	) {
		return planError(
			"ambiguous_managed_group",
			"Cannot mutate an OMX group with an invalid matcher.",
			{
				eventName,
	...
	if (eventName === "SessionStart") {
		...
		if (!allowed) {
			return planError(
				"ambiguous_managed_group",
				"SessionStart matcher is not compatible with OMX ownership.",
				{
					eventName,
					groupIndex: group.groupIndex,
				},
		...
		return planError(
			"ambiguous_managed_group",
			"OMX managed groups must not carry a matcher for this event.",
			{
				eventName,
				groupIndex: group.groupIndex,
			},
...
function inspectOwnership(
	...
	for (const eventName of CODEX_HOOK_EVENTS) {
		const event = document.hooksNode
			? objectProperty(document.hooksNode, eventName)
			: undefined;
		...
		for (const [groupIndex, groupNode] of event.value.elements.entries()) {
			...
			for (const [handlerIndex, handlerNode] of handlerNodes.entries()) {
				...
				if (classification === "ambiguous") {
					return planError(
						"ambiguous_managed_handler",
						"A command mentions OMX but does not match the managed command grammar.",
						{
							eventName,
							groupIndex,
							handlerIndex,
				...
				const asyncProperty =
					type === "command" ? objectProperty(handlerNode, "async") : undefined;
				const asynchronous =
					asyncProperty?.value.kind === "boolean" &&
					asyncProperty.value.value === true;
				if (classification !== "owned" || !isManagedHookEventName(eventName)) {
					if (classification === "owned") {
						return planError(
							"ambiguous_managed_handler",
							"An OMX command is attached to an unmanaged Codex event.",
							{
								eventName,
								groupIndex,
								handlerIndex,
					...
					const foreignCommandIsSkipped =
						type === "command" &&
						(commandIsSkipped(
							effectiveCommandForPlatform(
								handlerNode,
					...
					handlers.push({
						node: handlerNode,
						foreign: true,
						discoverySkipped: foreignCommandIsSkipped,
					});
					continue;
				}
				const command = effectiveCommandForPlatform(
					handlerNode,
					options.platform ?? process.platform,
				);
				if (asynchronous || commandIsSkipped(command)) {
					return planError(
						"ambiguous_managed_handler",
						"Cannot mutate skipped OMX command handlers.",
						{
							eventName,
							groupIndex,
							handlerIndex,
			...
			group.foreign =
				handlers.length === 0 || handlers.some((handler) => handler.foreign);
			groups.push(group);
			const groupOwners = handlers.flatMap((handler) =>
				handler.owner ? [handler.owner] : [],
			);
			if (groupOwners.length > 0) {
				if (handlers.some((handler) => handler.discoverySkipped)) {
					return planError(
						"ambiguous_managed_handler",
						"Cannot mutate an OMX group that also contains a skipped command handler.",
						{
							eventName,
							groupIndex,
						},
				...
				const error = matcherOwnershipError(
					eventName as ManagedHookEventName,
					group,
				);
...
function propertyRemovalPatch(
	...
	if (index < object.properties.length - 1) {
		return {
			start: property.keyNode.start,
			end: object.properties[index + 1]!.keyNode.start,
			text: "",
		};
	}
	return {
		start: object.properties[index - 1]!.value.end,
		end: property.value.end,
		text: "",
	};
}

function appendObjectPropertyPatch(
	object: JsonObjectNode,
	key: string,
	value: string,
): SourcePatch {
	...
	return {
		...
		text: `${prefix}${JSON.stringify(key)}:${value}`,
	};
}
...
function appendArrayElementPatch(
	array: JsonArrayNode,
	value: string,
): SourcePatch {
...
function knownFieldPatches(
	...
	const removedProperties = node.properties.filter(
		(property) =>
	...
	if (removedProperties.length > 1) {
		const members = node.properties.flatMap((property) => {
			...
			return [
				`${JSON.stringify(property.key)}:${JSON.stringify(desired[property.key])}`,
			];
		});
		...
		return [
			{ start: node.start, end: node.end, text: `{${members.join(",")}}` },
		];
	}
...
function canonicalHookForEvent(
...
function canonicalGroupForEvent(
	entry: ManagedHookEntry,
): Record<string, unknown> {
	return {
...
function canonicalMatcherForEvent(
	entry: ManagedHookEntry,
): Record<string, unknown> {
...
function removesAllGroupHandlers(
	...
	return (
		group.handlers.length > 0 &&
		group.handlers.every(
			(handler) => handler.owner && ownersToRemove.has(handler.owner),
		)
	);
}
...
function removalProof(
	...
	for (const [eventName, groups] of byEvent) {
		...
		for (const group of groups) {
			...
			const remainingHandlers = group.handlers.filter(
				(handler) => !handler.owner || !ownersToRemove.has(handler.owner),
			);
...
function fullRemovalGroupKeys(
	...
	for (const group of model.groups) {
		const removable =
			!group.opaque && removesAllGroupHandlers(group, ownersToRemove);
...
function pruneRemovedManagedEventProperties(
	...
	for (;;) {
		...
		for (const eventName of deletedEvents) {
			const event = validation.hooksNode
				? objectProperty(validation.hooksNode, eventName)
				: undefined;
		...
		if (emptyEvent === undefined) {
			...
			current = applySourcePatches(current, [
				propertyRemovalPatch(validation.rootNode, hooks),
			]);
		...
		current = applySourcePatches(current, [
			propertyRemovalPatch(validation.hooksNode!, property),
		]);
	}
}
...
function applyCoordinateSafeRemovals(
	...
	for (const group of model.groups) {
		...
		const removedHandlers = new Set<JsonNode>(
			group.handlers
		...
		if (removedHandlers.size > 0)
			partialPatches.push(
				arrayWithoutElements(content, hooks, removedHandlers),
			);
	}
	let current =
		partialPatches.length > 0
			? applySourcePatches(content, partialPatches)
			: content;
	...
	for (const [eventName, groupIndices] of fullGroups) {
		...
		if (!array) continue;
		const removedGroups = new Set<JsonNode>(
			afterOwnership.groups
				.filter(
					(group) =>
						group.eventName === eventName && groupIndices.has(group.groupIndex),
				)
	...
	current =
		groupPatches.length > 0
			? applySourcePatches(current, groupPatches)
			: current;
...
function exactLegacyState(
	node: JsonNode,
): Record<string, CodexHooksJsonTrustStateEntry> | null {
	...
	for (const property of node.properties) {
		...
		if (
			entry.properties.some(
				(member) => member.key !== "trusted_hash" && member.key !== "enabled",
			)
		)
			return null;
...
function identicalLegacyTrustStateEntry(
	...
	return (
		left.trusted_hash === right.trusted_hash &&
...
function prepareLegacyState(
	content: string,
): PreparedDocument | InvalidCodexHooksConfigStrict {
	const root = parseJsonSource(content);
	if (!root || root.kind !== "object") {
		return {
			ok: false,
	...
	const rootStates = root.properties.filter(
		(property) => property.key === "state",
	);
	if (rootStates.length > 1) {
		return {
			...
			error: planError(
				"invalid_document",
				"Duplicate top-level state is not supported.",
			),
		};
	}
	if (rootStates.length === 1) {
		...
		if (!exact) {
			return {
				...
				error: planError(
					"invalid_document",
					"Top-level state is not an exact historical OMX trust map.",
				),
			};
		}
	...
	const nestedStates =
		hooks?.value.kind === "object"
			? hooks.value.properties.filter((property) => property.key === "state")
			: [];
	const exactNestedStates = nestedStates.map((state) =>
		exactLegacyState(state.value),
	);
	let nestedStatesToRemove = 0;
	if (
		nestedStates.length > 1 &&
		exactNestedStates.some((state) => state === null)
	) {
		return {
			ok: false,
			error: planError(
				"invalid_document",
				"Duplicate hooks.state entries must be exact historical OMX trust maps.",
			),
		};
	}
	if (nestedStates.length === 0 || exactNestedStates[0] !== null) {
		for (const exact of exactNestedStates) {
			if (!exact) continue;
			const conflict = mergeLegacyTrustState(
				legacyTrustState,
				exact,
				"hooks.state",
			);
	...
	for (let index = 0; index < nestedStatesToRemove; index += 1) {
		...
		if (currentHooks?.kind !== "object") {
			return {
				...
				error: planError(
					"invalid_document",
					"hooks.json changed while preparing legacy state.",
				),
			};
		}
		...
		if (!currentState) {
			return {
				...
				error: planError(
					"invalid_document",
					"hooks.state changed while preparing legacy state.",
				),
			};
		}
		preparedContent = applySourcePatches(preparedContent, [
			propertyRemovalPatch(currentHooks, currentState),
		]);
	}
...
function foreignHooksInDocument(
	document: StrictDocument,
	options: ManagedCodexHookOptions,
): boolean {
	const ownership = inspectOwnership(document, options);
	return (
		ownership instanceof ManagedCodexHooksPlanError ||
		ownership.groups.some((group) => group.foreign) ||
		document.hooksNode?.properties.some((event) =>
			hasExecutableForeignHandlerInUnknownEvent(
				event,
				options.platform ?? process.platform,
			),
		) === true
...
export function planManagedCodexHooksMerge(
	...
	if (typeof existingContent !== "string") {
		const content = serializeCodexHooksConfig(
			jsonObjectFromUnknown(
				buildManagedCodexHooksConfig(pkgRoot, resolvedOptions),
			),
		);
		return finalizePlan(
			existingContent,
	...
	const validation = validateCodexHooksDocument(
		prepared.content,
		resolvedOptions,
	);
	if (!("result" in validation)) return planFailure(validation.error);
	...
	for (const eventName of MANAGED_HOOK_EVENTS) {
		const eventOwners = ownership.owners
			.filter((owner) => owner.eventName === eventName)
			.sort(
				(left, right) =>
		...
		patches.push(
			...knownFieldPatches(
				prepared.content,
				primary.handlerNode,
				canonicalHookForEvent(canonicalEntry),
				KNOWN_COMMAND_FIELDS,
			),
		);
		const primaryGroup = ownership.groups.find(
			(group) => group.node === primary.groupNode,
		);
	...
	if (validation.hooksNode && missingEventProperties.length > 0) {
		patches.push(
			appendObjectPropertiesPatch(validation.hooksNode, missingEventProperties),
		);
	}
	if (!validation.hooksNode)
		patches.push(
			appendObjectPropertyPatch(
				validation.rootNode,
				"hooks",
				JSON.stringify(managed.hooks),
			),
		);
	...
	try {
		...
		return planFailure(
			planError(
				"invalid_document",
				"Overlapping source edits prevented a safe merge.",
				{ cause: String(error) },
			),
	...
	for (const eventName of MANAGED_HOOK_EVENTS) {
		const eventOwners = afterOwnership.owners
			.filter((owner) => owner.eventName === eventName)
			.sort(
				(left, right) =>
					left.groupIndex - right.groupIndex ||
	...
	if (!proof.safe) {
		return {
			...
			error: planError(
				"unsafe_managed_removal",
				"Removing duplicate OMX hooks would shift a foreign coordinate or discard opaque metadata.",
	...
	try {
		finalContent = applyCoordinateSafeRemovals(
			intermediate,
			afterOwnership,
			removeOwners,
			resolvedOptions,
		...
		return planFailure(
			planError(
				"invalid_document",
				"Source edits prevented a safe duplicate cleanup.",
				{ cause: String(error) },
			),
			afterMerge.result.diagnostics,
		);
	}
...
export function planManagedCodexHooksRemoval(
	...
	try {
		...
		return planFailure(
			planError("invalid_document", "Source edits prevented safe removal.", {
				cause: String(error),
			}),
			validation.result.diagnostics,
		);
...
export function getMissingManagedCodexHookEvents(
	...
	return MANAGED_HOOK_EVENTS.filter(
		(eventName) =>
			!codexHookEntries(parsed.hooks, eventName).some(
				(entry) =>
					isPlainObject(entry) &&
					isJsonArray(entry.hooks) &&
					entry.hooks.some(
						(hook) =>
							isPlainObject(hook) &&
							hook.type === "command" &&
...
export function buildManagedCodexHookTrustState(
	...
	const content =
		typeof options.hooksContent === "string"
			? options.hooksContent
			: serializeCodexHooksConfig(
					jsonObjectFromUnknown(
						buildManagedCodexHooksConfig(pkgRoot, resolvedOptions),
					),
				);
	const scan = scanManagedCodexHookTrustStateFromContent(
		content,
		hooksPath,
		resolvedOptions,
	);
...
export function hasCodexHooksJsonTopLevelState(
	content: string,
): boolean | null {
	const parsed = parseCodexHooksConfig(content);
	if (!parsed) return null;
...
export function mergeManagedCodexHooksConfig(
	...
	const hooksPath =
		typeof hooksPathOrOptions === "string" ? hooksPathOrOptions : "";
	const providedOptions =
		typeof hooksPathOrOptions === "object" && hooksPathOrOptions !== null
			? hooksPathOrOptions
			: options;
