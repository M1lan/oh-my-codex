...
function isOmxManagedHookCommand(command: string): boolean {
	return (
		/(?:^|[\\/])codex-native-hook\.js(?:["'\s]|$)/.test(command) ||
...
export function mergeManagedCodexHooksConfig(
	...
	const hooksPath =
		typeof hooksPathOrOptions === "string" ? hooksPathOrOptions : undefined;
	...
	const resolvedOptions = {
		...providedOptions,
		...(hooksPath &&
		providedOptions.platform === "win32" &&
		!providedOptions.codexHomeDir
			? { codexHomeDir: dirname(hooksPath) }
			: {}),
	};
