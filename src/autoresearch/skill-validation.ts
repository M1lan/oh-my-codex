...
export type AutoresearchValidationMode =
...
export function normalizeAutoresearchValidationMode(
	value: unknown,
): AutoresearchValidationMode | null {
...
function deriveDefaultArtifactPath(
	cwd: string,
...
function hasArchitectApproval(
	artifact: Record<string, unknown> | null,
): boolean {
	if (!artifact) return false;
	const direct = lookupBoolean(
		artifact,
		"architect_approved",
		"architectApproved",
		"approved",
	...
	const architectReview =
		safeObject(artifact.architect_review) ??
		safeObject(artifact.architectReview);
	const architectValidation =
		safeObject(artifact.architect_validation) ??
		safeObject(artifact.architectValidation);
	return (
		isPassingStatus(architectReview?.verdict) ||
		isPassingStatus(architectValidation?.verdict)
	);
}
...
function resolveOutputArtifactPath(
	...
	const explicit =
		lookupString(rawState, "output_artifact_path", "outputArtifactPath") ||
...
export async function assessAutoresearchCompletionState(
	...
	if (!validationMode) {
		return {
			...
			validationMode: null,
			artifactPath: resolveArtifactPath(cwd, rawState),
		};
	}
	...
	if (validationMode === "mission-validator-script") {
		...
		if (
			lookupBoolean(artifact, "passed", "complete", "completed", "valid") ===
				true ||
			isPassingStatus(artifact.status)
		) {
			return {
				complete: true,
	...
	const validatorPrompt =
		lookupString(rawState, "validator_prompt", "validatorPrompt") ||
...
async function readAutoresearchModeStateFromPaths(
	...
	for (const candidatePath of candidates) {
		...
		try {
			return JSON.parse(await readFile(candidatePath, "utf-8")) as Record<
				string,
				unknown
			>;
		} catch {
...
export async function readAutoresearchModeState(
	...
	const candidates = await getReadScopedStatePaths(
		"autoresearch",
		cwd,
		sessionId,
	);
...
export async function readAutoresearchModeStateForActiveDecision(
	...
	const candidates = await getAuthoritativeActiveStatePaths(
		"autoresearch",
		cwd,
		sessionId,
	);
