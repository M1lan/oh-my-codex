...
describe("autoresearch contracts", () => {
	it("slugifies mission names deterministically", () => {
		assert.equal(
			slugifyMissionName("Missions/My Demo Mission"),
...
`),
	...
	it("accepts evaluator result with pass and score", () => {
		assert.deepEqual(parseEvaluatorResult('{"pass":false,"score":61}'), {
			pass: false,
			score: 61,
		});
	});
	...
	it("rejects mission directories outside a git repository", async () => {
		const missionDir = await mkdtemp(
			join(tmpdir(), "omx-autoresearch-not-git-"),
		);
		try {
			await writeFile(
	...
	it("loads mission contract from in-repo mission directory", async () => {
		...
		try {
			...
			assert.equal(
				contract.missionRelativeDir.replace(/\\/g, "/"),
				"missions/demo",
			);
