...
describe("autoresearch skill validation", () => {
	it("normalizes the supported validation modes", () => {
		assert.equal(
			normalizeAutoresearchValidationMode("mission-validator-script"),
			"mission-validator-script",
		);
		assert.equal(
			normalizeAutoresearchValidationMode("prompt-architect-artifact"),
	...
	it("treats missing validation mode as incomplete", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autoresearch-skill-validation-"),
		);
		try {
			const result = await assessAutoresearchCompletionState(
				{ active: true },
				cwd,
			);
	...
	it("requires a validator pass artifact for mission-validator-script mode", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autoresearch-validator-script-"),
		);
		try {
			...
			await mkdir(join(cwd, ".omx", "specs", "autoresearch-demo"), {
				recursive: true,
			});
			await writeFile(
				artifactPath,
			...
			await writeFile(
				artifactPath,
				JSON.stringify({ status: "passed", passed: true }, null, 2),
			);
	...
	it("requires prompt, architect approval, and output artifact for prompt-architect-artifact mode", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-autoresearch-architect-validation-"),
		);
		try {
