...
describe("autoresearch runtime", () => {
	it("builds bootstrap instructions with mission, sandbox, and evaluator contract", async () => {
		...
		try {
			...
			const instructions = buildAutoresearchInstructions(contract, {
				...
				candidateFile:
					".omx/logs/autoresearch/missions-demo-20260314t000000z/candidate.json",
	...
	it("allows untracked .omx runtime files when checking reset safety", async () => {
		...
		try {
			...
			await writeFile(
				join(repo, ".omx", "logs", "hooks-2026-03-15.jsonl"),
				"{}\n",
				"utf-8",
			);
			await writeFile(join(repo, ".omx", "metrics.json"), "{}\n", "utf-8");
			await writeFile(
				join(repo, ".omx", "state", "hud-state.json"),
				"{}\n",
				"utf-8",
			);
	...
	it("prepares runtime artifacts and persists autoresearch mode state", async () => {
		...
		try {
			...
			assert.equal(
				...
				join(worktreePath, "missions", "demo"),
			);
			...
			assert.match(
				results,
				/^iteration	commit	pass	score	status	description$/m,
			);
			assert.match(
				results,
				/^0	.+	true	1	baseline	initial baseline evaluation$/m,
			);
			...
			assert.equal(
				state?.bootstrap_instructions_path,
