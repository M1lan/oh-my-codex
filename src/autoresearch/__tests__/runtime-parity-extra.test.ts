...
describe("autoresearch runtime parity extras", () => {
	...
	it("rejects concurrent fresh runs via the repo-root active-run lock", async () => {
		...
		try {
			...
			const runtimeA = await prepareAutoresearchRuntime(
				worktreeContractA,
				repo,
				worktreePathA,
	...
	it("resume rejects terminal manifests", async () => {
		...
		try {
			...
			await writeFile(
				join(repo, ".omx", "state", "autoresearch-state.json"),
				`${JSON.stringify(
					{
						schema_version: 1,
						active: false,
	...
	it("rejects candidate integrity mismatches and missing candidate artifacts with actionable failure state", async () => {
		...
		try {
			...
			const worktreePath = join(
				...
				".omx",
				"worktrees",
				"autoresearch-missions-demo-20260314t070000z",
			);
			execFileSync(
				"git",
				[
			...
			let failedManifest = await loadAutoresearchRunManifest(
				repo,
				runtime.runId,
			);
			assert.equal(failedManifest.status, "failed");
			assert.match(
				failedManifest.stop_reason || "",
				/non-null candidate_commit/i,
			);
			...
			const secondWorktreePath = join(
				...
				".omx",
				"worktrees",
				"autoresearch-missions-demo-20260314t071000z",
			);
			execFileSync(
				"git",
				[
			...
			const secondContract = await materializeAutoresearchMissionToWorktree(
				contract,
				secondWorktreePath,
			);
			const secondRuntime = await prepareAutoresearchRuntime(
				secondContract,
				repo,
				secondWorktreePath,
				{ runTag: "20260314T071000Z" },
			);

			manifest = await loadAutoresearchRunManifest(repo, secondRuntime.runId);
			await writeFile(
				secondRuntime.candidateFile,
				`${JSON.stringify(
					{
						status: "candidate",
			...
			assert.equal(
				await processAutoresearchCandidate(secondContract, manifest, repo),
				"error",
			);

			failedManifest = await loadAutoresearchRunManifest(
				repo,
				secondRuntime.runId,
			);
			assert.equal(failedManifest.status, "failed");
			...
			const thirdWorktreePath = join(
				repo,
				".omx",
				"worktrees",
				"autoresearch-missions-demo-20260314t072000z",
			);
			execFileSync(
				"git",
				[
			...
			const thirdContract = await materializeAutoresearchMissionToWorktree(
				contract,
				thirdWorktreePath,
			);
			const thirdRuntime = await prepareAutoresearchRuntime(
				thirdContract,
				repo,
				thirdWorktreePath,
				{ runTag: "20260314T072000Z" },
			);

			manifest = await loadAutoresearchRunManifest(repo, thirdRuntime.runId);
			await rm(thirdRuntime.candidateFile, { force: true });
			assert.equal(
				await processAutoresearchCandidate(thirdContract, manifest, repo),
				"error",
			);
			failedManifest = await loadAutoresearchRunManifest(
				repo,
			...
			assert.match(
				failedManifest.stop_reason || "",
				/autoresearch_candidate_missing/i,
			);
		} finally {
			await rm(repo, { recursive: true, force: true });
		}
	...
	it("handles interrupted, evaluator failure, and evaluator parse-error branches", async () => {
		...
		try {
			...
			const worktreePath = join(
				...
				".omx",
				"worktrees",
				"autoresearch-missions-demo-20260314t080000z",
			);
			execFileSync(
				"git",
				[
			...
			manifest = await loadAutoresearchRunManifest(repo, runtime.runId);
			await writeFile(
				join(worktreePath, "scripts", "eval.js"),
				"process.stdout.write(JSON.stringify({ pass: false, score: 0 }));\n",
				"utf-8",
			);
			await writeFile(join(worktreePath, "score.txt"), "0\n", "utf-8");
			execFileSync("git", ["add", "scripts/eval.js", "score.txt"], {
				cwd: worktreePath,
				stdio: "ignore",
			});
			execFileSync("git", ["commit", "-m", "make evaluator fail"], {
				cwd: worktreePath,
				stdio: "ignore",
			});
			const failingCommit = execFileSync("git", ["rev-parse", "HEAD"], {
				cwd: worktreePath,
				encoding: "utf-8",
			}).trim();
			await writeFile(
			...
			execFileSync("git", ["add", "scripts/eval.js"], {
				cwd: worktreePath,
				stdio: "ignore",
			});
			execFileSync("git", ["commit", "-m", "break evaluator json"], {
				cwd: worktreePath,
				stdio: "ignore",
			});
			const parseErrorCommit = execFileSync("git", ["rev-parse", "HEAD"], {
				cwd: worktreePath,
				encoding: "utf-8",
			}).trim();
			await writeFile(
			...
			assert.match(
				results,
				/^2\t.+\tfalse\t0\tdiscard\tfailing evaluator branch$/m,
			);
			...
			assert.match(
				ledger.entries[3]?.decision_reason || "",
				/evaluator error/i,
			);
		} finally {
			await rm(repo, { recursive: true, force: true });
