...
function isAllowedRuntimeDirtyLine(line: string): boolean {
	...
	return (
		trimmed.startsWith("?? ") &&
		AUTORESEARCH_WORKTREE_EXCLUDES.some((exclude) =>
			exclude.endsWith("/")
...
export function decideAutoresearchOutcome(
	...
	if (!comparableScore(manifest.last_kept_score, evaluation.score)) {
		return {
			...
			evaluator: evaluation,
			notes: [
				"candidate discarded because score_improvement policy requires comparable numeric scores",
			],
		};
	}
...
export async function materializeAutoresearchMissionToWorktree(
	...
	try {
		execFileSync("git", ["add", "--", missionFile, sandboxFile], {
			cwd: worktreePath,
			stdio: "ignore",
			windowsHide: true,
		});
		execFileSync(
			...
			[
				"commit",
				"-m",
				`autoresearch: materialize mission ${contract.missionSlug}`,
			],
			{
				cwd: worktreePath,
				stdio: "ignore",
...
export async function processAutoresearchCandidate(
	...
	if (candidate.status !== "candidate") {
		return recordNonEvaluatedCandidateStatus(
			contract,
			manifest,
			projectRoot,
			candidate,
