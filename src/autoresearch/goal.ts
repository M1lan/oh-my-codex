...
export type AutoresearchGoalStatus =
	| "created"
	| "in_progress"
	| "passed"
	| "failed"
...
export function autoresearchGoalCompletionPath(
	cwd: string,
	slug: string,
): string {
...
async function appendLedger(
	cwd: string,
	slug: string,
	entry: AutoresearchGoalLedgerEntry,
): Promise<void> {
	await mkdir(autoresearchGoalDir(cwd, slug), { recursive: true });
	await appendFile(
		autoresearchGoalLedgerPath(cwd, slug),
...
async function writeMission(
	cwd: string,
	mission: AutoresearchGoalMission,
): Promise<void> {
	await mkdir(autoresearchGoalDir(cwd, mission.slug), { recursive: true });
	await writeFile(
		autoresearchGoalMissionPath(cwd, mission.slug),
		`${JSON.stringify(mission, null, 2)}\n`,
		"utf-8",
	);
}

export async function createAutoresearchGoal(
	cwd: string,
	options: CreateAutoresearchGoalOptions,
): Promise<AutoresearchGoalMission> {
	...
	if (!options.force && existsSync(missionPath)) {
		throw new AutoresearchGoalError(
	...
	const mission: AutoresearchGoalMission = {
		...
		topic,
		rubric,
		...(options.criticCommand?.trim()
			? { critic_command: options.criticCommand.trim() }
			: {}),
		status: "created",
		created_at: now,
		updated_at: now,
		...
		completion_path: repoRelative(
			cwd,
			autoresearchGoalCompletionPath(cwd, slug),
		),
	};

	await mkdir(autoresearchGoalDir(cwd, slug), { recursive: true });
	await writeFile(
		autoresearchGoalRubricPath(cwd, slug),
		`${rubric}\n`,
		"utf-8",
	);
	...
	await appendLedger(cwd, slug, {
		...
		message: `Autoresearch goal created: ${topic}`,
	});
	return mission;
}

export async function readAutoresearchGoal(
	cwd: string,
	slug: string,
): Promise<AutoresearchGoalMission> {
...
export async function readAutoresearchGoalCompletion(
	cwd: string,
	slug: string,
): Promise<AutoresearchGoalCompletion | null> {
	const path = autoresearchGoalCompletionPath(cwd, slugifyMissionName(slug));
	if (!existsSync(path)) return null;
	const parsed = JSON.parse(
		await readFile(path, "utf-8"),
	) as AutoresearchGoalCompletion;
	return parsed;
}
...
export async function recordAutoresearchGoalVerdict(
	...
	mission: AutoresearchGoalMission;
	completion: AutoresearchGoalCompletion;
}> {
	...
	const completion: AutoresearchGoalCompletion = {
		...
		...(options.artifactPath?.trim()
			? { artifact_path: options.artifactPath.trim() }
			: {}),
		...(mission.critic_command
			? { critic_command: mission.critic_command }
			: {}),
		recorded_at: now,
	};
	...
	await writeMission(cwd, mission);
	await writeFile(
		autoresearchGoalCompletionPath(cwd, mission.slug),
...
export async function completeAutoresearchGoal(
	cwd: string,
	slug: string,
	options: CompleteAutoresearchGoalOptions = {},
): Promise<{
	mission: AutoresearchGoalMission;
	completion: AutoresearchGoalCompletion;
}> {
...
export function buildAutoresearchGoalObjective(
...
export function buildLegacyAutoresearchGoalObjective(
	mission: Pick<AutoresearchGoalMission, "topic" | "rubric" | "slug">,
): string {
	return [
		`Autoresearch goal: ${mission.topic}`,
...
export function reconcileAutoresearchCodexGoalSnapshot(
	...
	const attempts = [
		buildAutoresearchGoalObjective(mission),
		buildLegacyAutoresearchGoalObjective(mission),
	].map((expectedObjective) =>
		reconcileCodexGoalSnapshot(snapshot, {
			expectedObjective,
			allowedStatuses: options.requireComplete
				? ["complete"]
				: ["active", "complete"],
	...
	const legacyOnlyErrors = legacy.errors.filter(
		(error) => !primary.errors.includes(error),
	);
	return {
		...primary,
		errors: [
			...primary.errors,
			...(legacyOnlyErrors.length
				? [
						`Legacy autoresearch objective also failed reconciliation: ${legacyOnlyErrors.join(" ")}`,
					]
				: []),
		],
	};
...
export async function buildAutoresearchGoalHandoff(
	cwd: string,
	slug: string,
	now = new Date(),
): Promise<string> {
	...
	return [
		...
		mission.rubric,
		...(mission.critic_command
			? ["", "Professor-critic command:", mission.critic_command]
			: []),
	].join("\n");
}
