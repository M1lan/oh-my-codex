...
>(platformConfig: T | undefined, envMention: string): T | undefined {
	if (!platformConfig || platformConfig.mention !== undefined)
		return platformConfig;
	return { ...platformConfig, mention: envMention };
}

function applyEnvDiscordMention(
	...
	const discordBot = applyDiscordMentionFallback(
		config["discord-bot"],
		envMention,
	);
	...
	if (config.events) {
		for (const [event, eventConfig] of Object.entries(config.events)) {
			...
			const eventDiscord = applyDiscordMentionFallback(
				eventConfig.discord,
				envMention,
			);
			if (
...
export function getActiveProfileName(): string | null {
	...
	if (
		...
		Object.keys(notifications.profiles).length === 0
	) {
		return null;
	}
...
function applyHookConfigIfPresent(
	config: FullNotificationConfig,
): FullNotificationConfig {
...
function hasCustomTransportAlias(config: FullNotificationConfig): boolean {
	...
	const webhookEnabled = Boolean(
		webhook && webhook.enabled !== false && webhook.url,
	);
	return cliEnabled || webhookEnabled;
}

function normalizeCustomTransportGate(
	config: FullNotificationConfig,
): FullNotificationConfig {
...
const VALID_VERBOSITY_LEVELS: VerbosityLevel[] = [
	"verbose",
	"agent",
	"session",
	"minimal",
...
export function getReplyListenerPlatformConfig(
	...
	const discordEnabled = !!(
		discordBotConfig?.botToken && discordBotConfig?.channelId
	);
...
function readReplySettings(
	raw: Record<string, unknown> | null,
): ReplySettingsRaw | undefined {
...
export function getReplyConfig(): import("./types.js").ReplyConfig | null {
	...
	const enabled =
		process.env.OMX_REPLY_ENABLED === "true" || replyRaw?.enabled === true;
