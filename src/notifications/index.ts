...
export async function shouldDispatchOpenClaw(
	...
	try {
		const { getOpenClawConfig, resolveGateway } = await import(
			"../openclaw/config.js"
		);
...
export async function notifyLifecycle(
	...
	try {
		...
		const lifecycleStateDir = payload.projectPath
			? omxStateDir(payload.projectPath)
			: "";
		const normalizedIdleTmuxTail =
			event === "session-idle" ? parseTmuxTail(payload.tmuxTail || "") : "";
		const sessionIdleTmuxTailAllowed =
			event !== "session-idle" ||
			shouldIncludeSessionIdleTmuxTail(
				lifecycleStateDir,
				payload.sessionId,
				normalizedIdleTmuxTail,
			);
		...
		if (
			result.anySuccess &&
			payload.tmuxPaneId &&
			options.persistScopedReceipts !== false
		) {
			try {
