...
const DEDUPED_EVENTS = new Set<NotificationEvent>([
	"session-start",
	"session-stop",
	"session-end",
]);
...
export function shouldDedupeLifecycleNotification(
...
function shouldSendScopedLifecycleBroadcast(
	...
	const bucketState =
		state[bucket] && typeof state[bucket] === "object" ? state[bucket] : {};
