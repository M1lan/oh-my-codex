...
export function redactSensitiveTokens(text: string): string {
	const withoutKeyedSecrets = text.replace(
...
export function captureReplyAcknowledgementSummary(
	...
	const capturePaneContentImpl =
		deps.capturePaneContentImpl ?? capturePaneContent;
...
async function postDiscordReplyMessage(
	...
	try {
		...
		if (!response.ok) {
			deps.logImpl(
				`WARN: Failed to send Discord reply message: HTTP ${response.status}`,
			);
		}
	} catch (error) {
...
export async function pollDiscordOnce(
	...
	const buildSessionStatusReplyImpl =
		deps.buildSessionStatusReplyImpl ?? buildDiscordSessionStatusReply;
	const captureReplyAcknowledgementSummaryImpl =
		deps.captureReplyAcknowledgementSummaryImpl ??
	...
	const parseMentionAllowedMentionsImpl =
		deps.parseMentionAllowedMentionsImpl ?? parseMentionAllowedMentions;
	...
	try {
		...
		if (remaining !== null && parseInt(remaining, 10) < 2) {
			...
			const resetTime = Number.isFinite(parsed)
				? parsed * 1000
				: Date.now() + 10_000;
		...
		for (const msg of sorted) {
			...
			const mapping = lookupByMessageIdImpl(
				"discord-bot",
			...
			if (!rateLimiter.canProceed()) {
				logImpl(
					`WARN: Rate limit exceeded, dropping Discord message ${msg.id}`,
				);
				state.errors++;
				continue;
			}
			...
			if (success) {
				...
				const acknowledgement = formatReplyAcknowledgement(
					captureReplyAcknowledgementSummaryImpl(mapping.tmuxPaneId),
				);
