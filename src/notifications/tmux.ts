...
function isMetadataOnlyTmuxSegment(segment: string): boolean {
	return (
		OMX_METADATA_SEGMENT_RE.test(segment) ||
...
function isMetadataOnlyTmuxLine(line: string): boolean {
	...
	if (
		segments.length === 0 ||
		segments.some((segment) => !isMetadataOnlyTmuxSegment(segment))
	) {
		return false;
	}
	...
	return (
		hasExplicitStatusSegment ||
		(segments.length === 1 && BRANCH_METADATA_SEGMENT_RE.test(segments[0]))
	);
}
 ...
 */
export function sanitizeTmuxAlertText(
...
export function getCurrentTmuxSession(): string | null {
	...
	if (process.env.TMUX) {
		try {
			...
			const paneTargetSafe =
				tmuxPaneTarget && TMUX_PANE_TARGET_RE.test(tmuxPaneTarget)
					? tmuxPaneTarget
					: null;
...
function detectTmuxSessionByPid(): string | null {
	...
	try {
		...
		while (currentPid > 1 && !visited.has(currentPid)) {
			...
			// Get parent PID
			try {
				const ppidStr = execFileSync(
					"ps",
					["-o", "ppid=", "-p", String(currentPid)],
					{
						encoding: "utf-8",
						timeout: 1000,
						stdio: ["pipe", "pipe", "pipe"],
...
export function captureTmuxPaneWithLiveness(
	paneId?: string | null,
	lines: number = 12,
