...
export async function _sendJsonHttpsRequest(
	options: JsonHttpsRequestOptions,
): Promise<void> {
	...
	try {
		...
		if (!response.ok) {
			throw new Error(
				`${options.errorPrefix}_http_${response.status || "unknown"}`,
			);
		}
	} catch (error) {
		if (
			error instanceof Error &&
			/timeout|aborted/i.test(`${error.name} ${error.message}`)
		) {
