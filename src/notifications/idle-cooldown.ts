	...
	try {
		...
		if (existsSync(configPath)) {
			const raw = JSON.parse(readFileSync(configPath, "utf-8")) as Record<
				...
				unknown
			>;
			const notifications = raw?.notifications as
...
function getSessionIdleHookStatePath(
	stateDir: string,
	sessionId?: string,
): string {
...
function normalizeIdleFingerprint(
...
function readIdleNotificationState(
	cooldownPath: string,
): IdleNotificationState | null {
	try {
		...
		return {
			lastSentAt:
				typeof data?.lastSentAt === "string" ? data.lastSentAt : undefined,
			fingerprint: normalizeIdleFingerprint(
				typeof data?.fingerprint === "string" ? data.fingerprint : "",
			),
			tmuxTailFingerprint: normalizeIdleFingerprint(
				typeof data?.tmuxTailFingerprint === "string"
					? data.tmuxTailFingerprint
					: "",
			),
		};
	} catch {
...
function writeIdleNotificationState(
	cooldownPath: string,
	patch: IdleNotificationState,
): void {
	try {
 ...
 */
export function shouldSendSessionIdleHookEvent(
	stateDir: string,
	sessionId?: string,
	fingerprint?: string,
): boolean {
	...
	const state = readIdleNotificationState(
		getSessionIdleHookStatePath(stateDir, sessionId),
	);
	if (!state) return true;
 ...
 */
export function recordSessionIdleHookEventSent(
	stateDir: string,
	sessionId?: string,
	fingerprint?: string,
): void {
...
export function shouldIncludeSessionIdleTmuxTail(
	...
	const state = readIdleNotificationState(
		getCooldownStatePath(stateDir, sessionId),
	);
	if (!state) return true;
