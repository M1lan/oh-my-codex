...
describe("loadNotificationConfig", () => {
	...
	it("returns parsed config when file exists", async () => {
		...
		writeFileSync(
			join(omxDir, "notifications.json"),
...
describe("_buildDesktopArgs - command injection prevention", () => {
	...
	it("darwin: double-quotes in title/message are escaped for AppleScript context", () => {
		...
		assert.ok(
			args[1].includes('\\"hi\\"'),
			"double-quote in title should be escaped",
		);
		assert.ok(
			args[1].includes('\\"this\\"'),
			"double-quote in message should be escaped",
		);
	});

	it("darwin: shell metacharacters in title/message remain in the -e argument, not the command", () => {
		const result = _buildDesktopArgs(
			injectionTitle,
			injectionMessage,
			"darwin",
		);
		...
		assert.ok(
			...
			"injection payload present as data inside the arg",
		);
	...
	it("win32: single-quotes in title/message are doubled for PowerShell single-quoted context", () => {
		...
		assert.ok(
			args[1].includes("O''Brien"),
			"single-quote in title should be escaped as ''",
		);
		assert.ok(
			args[1].includes("It''s done"),
			"single-quote in message should be escaped as ''",
		);
	});
