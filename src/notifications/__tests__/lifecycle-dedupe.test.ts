...
describe("lifecycle notification dedupe", () => {
	...
	it("re-emits when the lifecycle fingerprint changes", async () => {
		const stateDir = await mkdtemp(
			join(tmpdir(), "omx-lifecycle-dedupe-change-"),
		);
		try {
			const payload = buildPayload();
			recordLifecycleNotificationSent(stateDir, payload);
	...
	it("stores dedupe state per session id", async () => {
		const stateDir = await mkdtemp(
			join(tmpdir(), "omx-lifecycle-dedupe-session-"),
		);
		try {
			const payload = buildPayload({
				sessionId: "session-a",
			...
			assert.equal(
				typeof state.events?.["session-start"]?.fingerprint,
				"string",
			);
			assert.equal(
				shouldSendLifecycleNotification(
					stateDir,
					buildPayload({ sessionId: "session-b", event: "session-start" }),
				),
				true,
			);
		} finally {
	...
	it("re-emits the same lifecycle fingerprint after the dedupe window elapses", async () => {
		const stateDir = await mkdtemp(
			join(tmpdir(), "omx-lifecycle-dedupe-window-"),
		);
		try {
			...
			assert.equal(
				shouldSendLifecycleNotification(stateDir, payload, startMs + 4_999),
				false,
			);
			assert.equal(
				shouldSendLifecycleNotification(stateDir, payload, startMs + 5_000),
				true,
			);
		} finally {
			await rm(stateDir, { recursive: true, force: true });
	...
	it("tracks hook dedupe separately from notification dedupe state", async () => {
		const stateDir = await mkdtemp(
			join(tmpdir(), "omx-lifecycle-dedupe-hook-"),
		);
		try {
			const payload = buildPayload({
				event: "session-start",
