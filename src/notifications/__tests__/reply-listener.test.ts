...
function createBaseConfig(
	overrides: Partial<ReplyListenerDaemonConfig> = {},
): ReplyListenerDaemonConfig {
	return {
		enabled: true,
		pollIntervalMs: 3000,
	...
	it("returns false on Windows when ps is unavailable", () => {
		const result = isReplyListenerProcess(123, {
			...
			spawnImpl: (() => ({
				...
				status: null,
				signal: null,
				error: Object.assign(new Error("spawnSync ps ENOENT"), {
					code: "ENOENT",
				}),
...
describe("captureReplyAcknowledgementSummary redaction", () => {
	it("redacts secrets from captured tmux output", () => {
		...
		assert.ok(summary);
		assert.equal(
			summary.includes("sk-proj-abc123"),
			false,
...
describe("pollDiscordOnce", () => {
	it("treats exact-match status replies as read-only Discord session lookups", async () => {
		...
		assert.deepEqual(replyBody.message_reference, {
			message_id: "discord-status-1",
		});
	...
	it("uses the latest correlated session when a Discord notification message id is reused", async () => {
		...
		await pollDiscordOnce(config, state, new RateLimiter(10), {
			...
			injectReplyImpl: () => {
				throw new Error(
					"injectReply should not run for exact-match status probes",
				);
			},
		});
	...
	it("injects authorized replies and posts a threaded acknowledgement with recent output", async () => {
		...
		await pollDiscordOnce(config, state, new RateLimiter(10), {
			...
			parseMentionAllowedMentionsImpl: (mention) => {
				...
				return { users: ["123"] } as ReturnType<
					typeof import("../config.js").parseMentionAllowedMentions
				>;
			},
		...
		const acknowledgementBody = JSON.parse(
			String(acknowledgementCall.init?.body),
		);
		assert.equal(
			acknowledgementBody.content,
			"Injected into Codex CLI session.\n\nRecent output:\nRecent pane output",
		);
		assert.deepEqual(acknowledgementBody.message_reference, {
			message_id: "discord-reply-1",
		});
	...
	it("ignores unauthorized Discord replies while still advancing the last message id", async () => {
		...
		await pollDiscordOnce(createBaseConfig(), state, new RateLimiter(10), {
			...
			injectReplyImpl: () => {
				throw new Error(
					"injectReply should not be called for unauthorized replies",
				);
			},
		});
	...
	it("does not return status data for unauthorized status replies", async () => {
		...
		await pollDiscordOnce(createBaseConfig(), state, new RateLimiter(10), {
			...
			lookupByMessageIdImpl: () => {
				throw new Error(
					"lookup should not run for unauthorized status replies",
				);
			},
			injectReplyImpl: () => {
				throw new Error(
					"injectReply should not run for unauthorized status replies",
				);
			},
		});

		assert.deepEqual(fetchCalls, [
			"https://discord.com/api/v10/channels/discord-channel/messages?limit=10",
		]);
		assert.equal(state.messagesInjected, 0);
		assert.equal(state.errors, 0);
		assert.equal(
			state.discordLastMessageId,
			"discord-reply-unauthorized-status",
		);
	});
