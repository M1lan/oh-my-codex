...
function makeTmpStateDir(): string {
	const dir = join(
		tmpdir(),
		`omx-cooldown-test-${Math.random().toString(36).slice(2)}`,
	);
...
describe("shouldSendIdleNotification", () => {
	...
	it("returns true when cooldown is 0 even for unchanged fingerprints", () => {
		...
		assert.equal(
			shouldSendIdleNotification(stateDir, sessionId, fingerprint),
			true,
		);
	});
	...
	it("allows a changed summary fingerprint immediately", () => {
		...
		recordIdleNotificationSent(
			stateDir,
			sessionId,
			'{"phase":"idle","summary":"Waiting on review"}',
		);
		...
		assert.equal(
			shouldSendIdleNotification(
				stateDir,
				sessionId,
				'{"phase":"idle","summary":"Waiting on user input"}',
			),
			true,
		);
	});
	...
	it("allows a progress transition to clear prior idle suppression", () => {
		...
		const blockedFingerprint =
			'{"phase":"idle","summary":"Blocked on dependency"}';
		const progressFingerprint =
			'{"phase":"progress","summary":"Applied fix and running tests"}';

		recordIdleNotificationSent(stateDir, sessionId, blockedFingerprint);
		assert.equal(
			shouldSendIdleNotification(stateDir, sessionId, blockedFingerprint),
			false,
		);
	...
	it("allows terminal transitions to clear prior idle suppression", () => {
		...
		const blockedFingerprint =
			'{"phase":"idle","summary":"Awaiting next step"}';

		recordIdleNotificationSent(stateDir, sessionId, blockedFingerprint);
		recordIdleNotificationSent(
			stateDir,
			sessionId,
			'{"phase":"finished","summary":"Completed and waiting for input"}',
		);
		assert.equal(
			shouldSendIdleNotification(stateDir, sessionId, blockedFingerprint),
			true,
		...
		recordIdleNotificationSent(
			stateDir,
			sessionId,
			'{"phase":"failed","summary":"Command failed"}',
		);
		assert.equal(
			shouldSendIdleNotification(stateDir, sessionId, blockedFingerprint),
			true,
...
describe("recordIdleNotificationSent", () => {
	...
	it("writes a cooldown file with lastSentAt timestamp", () => {
		...
		const content = JSON.parse(
			readFileSync(join(stateDir, "idle-notif-cooldown.json"), "utf-8"),
		) as { lastSentAt: string };
	...
	it("persists the idle fingerprint when provided", () => {
		...
		const content = JSON.parse(readFileSync(sessionFile, "utf-8")) as {
			lastSentAt: string;
			fingerprint?: string;
		};
...
describe("session-idle hook event dedupe", () => {
	...
	it("suppresses unchanged hook fingerprints even when lifecycle cooldown is disabled", () => {
		...
		assert.equal(
			shouldSendSessionIdleHookEvent(stateDir, sessionId, fingerprint),
			false,
		);
	});
...
describe("session-idle tmux tail dedupe", () => {
	...
	it("suppresses unchanged parsed tmux tails for repeated idle notifications", () => {
		...
		assert.equal(
			shouldIncludeSessionIdleTmuxTail(stateDir, sessionId, fingerprint),
			false,
		);
	});
	...
	it("treats empty tmux tails as nothing to resend", () => {
		...
		assert.equal(
			shouldIncludeSessionIdleTmuxTail(stateDir, sessionId, ""),
			false,
		);
	});
