...
function makeTmpDir(): string {
	const dir = join(
		tmpdir(),
		`omx-hook-cfg-test-${Math.random().toString(36).slice(2)}`,
	);
...
describe("resolveEventTemplate", () => {
	...
	it("returns platform-specific template when available", () => {
		assert.equal(
			resolveEventTemplate(baseConfig, "session-end", "discord"),
			"discord-specific template",
		);
	});

	it("falls back to event-level template when no platform override", () => {
		assert.equal(
			resolveEventTemplate(baseConfig, "session-end", "telegram"),
	...
	it("returns null when no template found at any level", () => {
		...
		assert.equal(
			resolveEventTemplate(minimal, "session-start", "telegram"),
			null,
		);
	});
});
