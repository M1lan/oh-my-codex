...
describe("session-idle tmux tail dedupe integration", () => {
	...
	beforeEach(async () => {
		...
		({ notifyLifecycle } = await import(
			`../index.js?session-idle-tail-dedupe=${Date.now()}`
		));
	...
	it("drops unchanged stale tmux tails from repeated idle notifications but keeps fresh tails", async () => {
		...
		const first = await notifyLifecycle("session-idle", {
			...base,
			tmuxTail: staleTail,
		});
		...
		const second = await notifyLifecycle("session-idle", {
			...base,
			tmuxTail: staleTail,
		});
		...
		assert.doesNotMatch(
			String(capturedBodies[1]?.message || ""),
			/risk summary/,
		);
		assert.doesNotMatch(String(capturedBodies[1]?.message || ""), /severity/);

		const third = await notifyLifecycle("session-idle", {
			...base,
			tmuxTail: "fresh setup error",
		});
