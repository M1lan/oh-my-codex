...
describe("proxy env resolution", () => {
	it("uses scheme-specific lowercase proxy before uppercase and ALL_PROXY", () => {
		...
		assert.equal(
			getProxyForUrl("https://discord.com/api/webhooks/1/abc", env)?.url
				.hostname,
			"lower.example",
		);
	});
	...
	it("falls back to ALL_PROXY for https when no scheme proxy exists", () => {
		...
		assert.equal(
			getProxyForUrl("https://hooks.slack.com/services/T/B/C", env)?.url
				.hostname,
			"all.example",
		);
	});
	...
	it("bypasses proxy when NO_PROXY matches exact host, suffix, port, or wildcard", () => {
		assert.equal(
			noProxyMatches(
				new URL("https://api.telegram.org/bot/sendMessage"),
				"api.telegram.org",
			),
			true,
		);
		assert.equal(
			noProxyMatches(new URL("https://sub.example.com/hook"), ".example.com"),
			true,
		);
		assert.equal(
			noProxyMatches(
		...
		assert.equal(
			noProxyMatches(new URL("https://anything.invalid/hook"), "*"),
			true,
		);
	});
	...
	it("does not bypass when NO_PROXY host port differs", () => {
		assert.equal(
			noProxyMatches(
				new URL("https://example.com:443/hook"),
				"example.com:8443",
			),
			false,
		);
