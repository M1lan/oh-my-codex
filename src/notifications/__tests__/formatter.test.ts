...
describe("formatSessionStart", () => {
	it("should include start header and session info", () => {
		const result = formatSessionStart({
			...basePayload,
			event: "session-start",
		});
...
describe("formatNotification routing", () => {
	it("should route each event type correctly", () => {
		assert.ok(
			formatNotification({ ...basePayload, event: "session-idle" }).includes(
				"# Session Idle",
			),
		);
		assert.ok(
			formatNotification({ ...basePayload, event: "session-start" }).includes(
				"# Session Started",
			),
		);
		assert.ok(
			formatNotification({ ...basePayload, event: "session-end" }).includes(
				"# Session Ended",
			),
		);
		assert.ok(
			formatNotification({ ...basePayload, event: "session-stop" }).includes(
				"# Session Continuing",
			),
		);
		assert.ok(
...
describe("parseTmuxTail", () => {
	...
	it("drops older blocks before truncating inside a wrapped block", () => {
		const makeBlock = (label: string) =>
			[`${label}: ${"가".repeat(420)}`, `  ${"나".repeat(120)}`].join("\n");
