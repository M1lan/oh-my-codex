...
describe("notifyLifecycle tmux tail auto-capture", () => {
	...
	it("does not auto-capture historical tmux tail for terminal notifications", async () => {
		...
		for (const eventName of ["session-end", "session-stop"] as const) {
			...
			const projectPath = mkdtempSync(
				join(tmpdir(), `omx-notify-index-project-${eventName}-`),
			);
	...
	it("awaits ask-user-question OpenClaw dispatch so reply routing stays on the live launch path", async () => {
		...
		const { resetOpenClawConfigCache } = await import(
			"../../openclaw/config.js"
		);
		resetOpenClawConfigCache();

		const projectPath = mkdtempSync(
			join(tmpdir(), "omx-notify-index-project-ask-"),
		);
		const { notifyLifecycle } = await import(
			`../index.js?ask-user-question-await=${Date.now()}`
		);
		...
		assert.ok(
			askElapsed >= 50,
			`ask-user-question should await OpenClaw dispatch, got ${askElapsed}ms`,
		);
		...
		assert.equal(
			openClawResolved,
			false,
			"session-start should keep fire-and-forget OpenClaw dispatch",
		);
	...
	it("keeps auto-capturing tmux tail for live session-idle notifications", async () => {
		...
		const projectPath = mkdtempSync(
			join(tmpdir(), "omx-notify-index-project-idle-"),
		);
		const { notifyLifecycle } = await import("../index.js");
