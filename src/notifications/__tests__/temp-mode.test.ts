...
describe("notification temp mode", () => {
	...
	it("temp contract bypasses persistent file/profile routing", async () => {
			...
			notifications: {
				...
				profiles: {
					"file-profile": {
						...
						discord: {
							enabled: true,
							webhookUrl: "https://discord.com/api/webhooks/file",
						},
					},
				},
			},
		...
		process.env.OMX_SLACK_WEBHOOK_URL =
			"https://hooks.slack.com/services/temp-only";
		...
		await writeCodexConfig({
			notifications: {
				...
				openclaw: {
					...
					hooks: {
						"session-end": { enabled: true, gateway: "g", instruction: "i" },
					},
				},
			},
		});
	...
	it("shouldDispatchOpenClaw enforces temp-mode explicit selection and gateway matching", async () => {
		...
		await writeCodexConfig({
			notifications: {
				...
				openclaw: {
					...
					hooks: {
						"session-end": { enabled: true, gateway: "g1", instruction: "i" },
					},
				},
			},
		});
		...
		assert.equal(
			await shouldDispatchOpenClaw("session-end", activeWithOpenClaw, {
				OMX_OPENCLAW: "0",
