...
function decodeChunked(body: Buffer): Buffer {
	...
	while (offset < body.length) {
		...
		if (lineEnd === -1) break;
		const sizeText =
			body.slice(offset, lineEnd).toString("ascii").split(";", 1)[0]?.trim() ??
			"";
...
async function requestViaProxy(
	...
	if (url.protocol === "https:") {
		...
		const connectResponse = await readUntilHeaders(
			proxySocket,
			options.timeoutMs,
		);
		const status = Number(
			connectResponse.head.match(/^HTTP\/\d(?:\.\d)?\s+(\d+)/)?.[1] ?? 0,
		);
