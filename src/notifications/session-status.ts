...
export const NO_TRACKED_SESSION_MESSAGE =
	"No tracked OMX session is associated with this message.";
export const STATUS_DATA_UNAVAILABLE_MESSAGE =
...
async function readRelevantSkillState(
	...
	deps: Pick<
		SessionStatusDeps,
		| "existsSyncImpl"
		| "getSkillActiveStatePathsImpl"
		| "readSkillActiveStateImpl"
		| "readRunStateImpl"
	>,
): Promise<SkillStateSummary | null> {
	const existsSyncImpl = deps.existsSyncImpl ?? existsSync;
	const getSkillActiveStatePathsImpl =
		deps.getSkillActiveStatePathsImpl ?? getSkillActiveStatePaths;
	const readSkillActiveStateImpl =
		deps.readSkillActiveStateImpl ?? readSkillActiveState;
	const readRunStateImpl = deps.readRunStateImpl ?? readRunState;
	const { rootPath, sessionPath } = getSkillActiveStatePathsImpl(
		projectPath,
		sessionId,
	);
	const candidatePaths = [sessionPath, rootPath].filter(
	...
	for (const candidatePath of candidatePaths) {
		...
		const skill =
			primary?.skill ||
			(typeof state.skill === "string" ? state.skill.trim() : "");
		const phase =
			primary?.phase ||
			(typeof state.phase === "string" ? state.phase.trim() : "");
		const updatedAt =
			typeof state.updated_at === "string" ? state.updated_at.trim() : "";
		const requiresDurableRuntime =
			candidatePath === sessionPath && skill.length > 0;
		const runState = requiresDurableRuntime
			? await readRunStateImpl(projectPath, sessionId)
			: null;
		const hasDurableRuntime =
			!requiresDurableRuntime ||
...
function formatSubagentSummary(summary: SubagentSessionSummary | null): string {
	...
	const visible = activeIds
		.slice(0, DISCORD_STATUS_MAX_SUBAGENTS)
		.map(shortenIdentifier);
...
function formatStateLabel(
	...
	const mode = skillState?.skill
		? skillState.phase
			? `${skillState.skill}/${skillState.phase}`
			: skillState.skill
		: "";
...
export async function buildDiscordSessionStatusReply(
	...
	const readSubagentSessionSummaryImpl =
		deps.readSubagentSessionSummaryImpl ?? readSubagentSessionSummary;
	const currentSession = await readCurrentSessionState(mapping.projectPath);
	const currentSessionMatches =
		currentSession?.session_id === mapping.sessionId ? currentSession : null;
	const historyEntry = readLatestHistoryEntry(
		mapping.projectPath,
		mapping.sessionId,
		deps,
	);
	const skillState = await readRelevantSkillState(
		mapping.projectPath,
		mapping.sessionId,
		deps,
	);
	const subagentSummary = await readSubagentSessionSummaryImpl(
		mapping.projectPath,
		mapping.sessionId,
		{
	...
	const nativeSessionId =
		currentSessionMatches?.native_session_id ||
		historyEntry?.native_session_id ||
		"unknown";
	const stateLabel = formatStateLabel(
		Boolean(currentSessionMatches),
