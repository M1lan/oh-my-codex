...
export function parseNotifyTempContractFromArgs(
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (arg === "--custom") {
			...
			if (!normalized) {
				warnings.push(
					`notify temp: ignoring invalid --custom selector "${next}"`,
				);
			} else {
		...
		if (arg.startsWith("--custom=")) {
			...
			if (!normalized) {
				warnings.push(
					`notify temp: ignoring invalid --custom selector "${raw}"`,
				);
			} else {
				selectors.push(normalized);
			}
...
export function serializeNotifyTempContract(
	contract: NotifyTempContract,
): string {
	return JSON.stringify(contract);
}

export function isNotifyTempEnvActive(
	env: NodeJS.ProcessEnv = process.env,
): boolean {
...
export function readNotifyTempContractFromEnv(
	...
	try {
		...
		return {
			...
			canonicalSelectors: parsed.canonicalSelectors.filter(
				(entry): entry is string => typeof entry === "string",
			),
...
export function isOpenClawSelectedInTempContract(
	contract: NotifyTempContract | null,
): boolean {
	if (!contract?.active) return false;
	return contract.canonicalSelectors.some(
		(selector) =>
			selector.startsWith("openclaw:") || selector.startsWith("custom:"),
	);
}

export function getTempBuiltinSelectors(
	contract: NotifyTempContract | null,
): Set<string> {
	if (!contract?.active) return new Set<string>();
	return new Set(
		contract.canonicalSelectors.filter(
			(selector) =>
				selector === "discord" ||
				selector === "slack" ||
				selector === "telegram",
...
export function getSelectedOpenClawGatewayNames(
	contract: NotifyTempContract | null,
): Set<string> {
	if (!contract?.active) return new Set<string>();
	const names: string[] = [];
