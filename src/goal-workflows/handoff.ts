...
export function buildGoalWorkflowHandoff(
	options: GoalWorkflowHandoffOptions,
): string {
	const createPayload = {
	...
	return [
		...
		options.completionCommand
			? `- Then record OMX completion evidence with the fresh snapshot: ${options.completionCommand}`
			: "- Then record OMX completion evidence in the workflow ledger/status artifacts with the fresh get_goal snapshot when the workflow supports --codex-goal-json.",
		"",
