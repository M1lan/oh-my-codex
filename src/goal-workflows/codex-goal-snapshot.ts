...
export function isCodexGoalDbSchemaContextError(
	...
	return (
		...
		(normalized.includes("no such table: thread_goals") ||
			(normalized.includes("thread_goals") &&
				/\b(?:sqlite|sql|schema|table|database|db)\b/.test(normalized)) ||
			/\b(?:codex goal|goal)\b.*\b(?:db|database|schema|context)\b.*\b(?:unavailable|missing|failed|error)\b/.test(
				normalized,
			))
	);
}
...
export function parseCodexGoalSnapshot(value: unknown): CodexGoalSnapshot {
	...
	if (!hasGoalProperty && errorMessage) {
		return {
			available: false,
			unavailableReason: isCodexGoalDbSchemaContextError(errorMessage)
	...
	const objective = safeString(
		goal.objective ?? goal.goal ?? goal.description ?? root.objective,
	);
...
export function reconcileCodexGoalSnapshot(
	...
	if (!effectiveSnapshot.available) {
		const detail = effectiveSnapshot.errorMessage
			? ` Last get_goal error: ${effectiveSnapshot.errorMessage}.`
			: "";
		const diagnostic =
	...
	if (!allowed.includes(actualStatus)) {
		errors.push(
			`Codex goal status mismatch: expected ${allowed.join(" or ")}, got ${actualStatus}.`,
		);
	}
...
export function buildCodexGoalTerminalCleanupNotice(
...
export function buildCompletedCodexGoalRemediation(
	workflowLabel: string,
): string {
