...
export interface GoalWorkflowValidationSummary {
	status: Extract<
		GoalWorkflowStatus,
		"validation_passed" | "blocked" | "failed"
	>;
	summary: string;
	artifactPath?: string;
	checkedAt: string;
...
export function normalizeGoalWorkflowSegment(
	value: string,
	fallback = "workflow",
): string {
...
export function goalWorkflowDir(
	cwd: string,
	workflow: string,
	slug: string,
): string {
...
export function goalWorkflowStatusPath(
	cwd: string,
	workflow: string,
	slug: string,
): string {
...
export function goalWorkflowLedgerPath(
	cwd: string,
	workflow: string,
	slug: string,
): string {
...
export async function createGoalWorkflowRun(
	...
	if (!options.objective.trim())
		throw new GoalWorkflowError("Missing goal workflow objective.");
	const workflow = normalizeGoalWorkflowSegment(options.workflow);
	const slug = normalizeGoalWorkflowSegment(
		options.slug ?? slugFromObjective(options.objective),
		"goal-workflow",
	);
...
export async function readGoalWorkflowRun(
	cwd: string,
	workflow: string,
	slug: string,
): Promise<GoalWorkflowRun> {
	...
	try {
		...
		throw new GoalWorkflowError(
			`No goal workflow run found at ${repoRelative(cwd, path)}.`,
		);
	}
	...
	if (
		...
		throw new GoalWorkflowError(
			`Invalid goal workflow run at ${repoRelative(cwd, path)}.`,
		);
	}
	return parsed;
}
...
function eventForStatus(
	status: TransitionGoalWorkflowOptions["status"],
): GoalWorkflowLedgerEvent {
	switch (status) {
		case "in_progress":
			return "goal_started";
...
export async function transitionGoalWorkflowRun(
	...
	if (options.status === "complete") {
		if (run.status !== "validation_passed") {
			throw new GoalWorkflowError(
				"Goal workflow completion requires a passing validation artifact first.",
			);
		}
