...
function hasPlaceholderEvidence(summary: string): boolean {
	return /\b(?:todo|tbd|placeholder|stub|not\s+implemented|fake\s+pass)\b/i.test(
		summary,
	);
}

export function normalizeGoalWorkflowValidation(
	input: GoalWorkflowValidationInput,
): GoalWorkflowValidationSummary {
	if (!input.summary.trim())
		throw new GoalWorkflowValidationError("Validation summary is required.");
	const status =
...
export function assertGoalWorkflowCanComplete(
	validation: GoalWorkflowValidationSummary | undefined,
): void {
	if (!validation)
	...
	if (validation.status !== "validation_passed") {
		throw new GoalWorkflowValidationError(
			`Completion requires validation_passed; got ${validation.status}.`,
		);
	}
	if (!validation.artifactPath?.trim())
		throw new GoalWorkflowValidationError(
			"Completion requires a validation artifact path.",
		);
