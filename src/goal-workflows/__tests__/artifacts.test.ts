...
describe("goal-workflow artifacts", () => {
	it("creates durable status and ledger artifacts under .omx/goals", async () => {
		await withTempRepo(async (cwd) => {
			...
			assert.equal(
				(await readGoalWorkflowRun(cwd, "performance-goal", "latency-pass"))
					.objective,
				"Reduce p95 latency below 100ms.",
			);

			const ledger = await readFile(
				join(cwd, ".omx/goals/performance-goal/latency-pass/ledger.jsonl"),
				"utf-8",
			);
	...
	it("requires a real validation artifact when entering validation_passed", async () => {
		await withTempRepo(async (cwd) => {
			await createGoalWorkflowRun(cwd, {
				workflow: "scrum-goal",
				slug: "strict",
				objective: "Strictly validate release.",
			});

			await assert.rejects(
				() =>
					transitionGoalWorkflowRun(cwd, "scrum-goal", "strict", {
	...
	it("blocks completion until validation passes and records ledger transitions", async () => {
		await withTempRepo(async (cwd) => {
			...
			await transitionGoalWorkflowRun(cwd, "scrum-goal", "release", {
				status: "in_progress",
			...
			await assert.rejects(
				() =>
					transitionGoalWorkflowRun(cwd, "scrum-goal", "release", {
						status: "complete",
						evidence: "looks done",
					}),
			...
			const completed = await transitionGoalWorkflowRun(
				cwd,
				"scrum-goal",
				"release",
				{ status: "complete", evidence: "audit passed" },
			);
			assert.equal(completed.status, "complete");

			const ledger = await readFile(
				join(cwd, ".omx/goals/scrum-goal/release/ledger.jsonl"),
				"utf-8",
			);
	...
	it("renders truthful Codex goal handoff instructions for degraded mode", async () => {
		await withTempRepo(async (cwd) => {
			const run = await createGoalWorkflowRun(cwd, {
				...
				objective: "Research safe rollout options.",
			});
			...
			assert.match(
				handoff,
				/update_goal\(\{status: "complete"\}\) only after/i,
			);
