...
const DEPRECATED_MODES: Record<DeprecatedModeName, string> = {
	...
	ecomode:
		'Use "ultrawork" instead. ecomode has been merged into ultrawork mode.',
};
...
function normalizeRalphModeStateOrThrow(state: ModeState): ModeState {
	...
	const validation = validateAndNormalizeRalphState(
...
function normalizeModeStateOrThrow(mode: string, state: ModeState): ModeState {
	const normalized =
		mode === "ralph" ? normalizeRalphModeStateOrThrow(state) : state;
...
export async function assertModeStartAllowed(
	...
	const activeModes = await readActiveWorkflowModes(
...
export async function startMode(
	...
	const stateBase: ModeState = {
		...
		...(mode === "ralph" && scope.sessionId
			? { owner_omx_session_id: scope.sessionId }
			: {}),
	};
	...
	await writeFile(
		getStatePath(mode, projectRoot, scope.sessionId),
...
async function readModeStateFromPaths(
	paths: string[],
): Promise<ModeState | null> {
...
export async function readModeStateForActiveDecision(
	...
	let paths: string[];
	try {
		paths = await getAuthoritativeActiveStatePaths(
			mode,
			projectRoot,
			sessionId,
		);
...
function assertRalphUpdateMatchesSession(
	state: ModeState,
	sessionId?: string,
): void {
	...
	if (ownerOmxSessionId && ownerOmxSessionId !== normalizedSessionId) {
		throw new Error(
			`Mode ralph state belongs to another session (${ownerOmxSessionId})`,
		);
	}
	...
	if (stateSessionId && stateSessionId !== normalizedSessionId) {
		throw new Error(
			`Mode ralph state belongs to another session (${stateSessionId})`,
		);
	}
}
...
export async function updateModeState(
	...
	if (
		mode === "ralph" &&
		scope.sessionId &&
		typeof updatedBase.owner_omx_session_id !== "string"
	) {
		updatedBase.owner_omx_session_id = scope.sessionId;
	}
	const normalizedBase = normalizeModeStateOrThrow(
		mode,
	...
	if (mode === "autopilot") {
		...
		const currentAutopilotChildPhase = deriveAutopilotChildPhase({
			...current,
			mode: "autopilot",
		});
		const nextAutopilotChildPhase = deriveAutopilotChildPhase({
			...normalizedBase,
			mode: "autopilot",
		});
		const completionTransitionError = validateAutopilotCompletionTransition(
			...
			normalizedBase as Record<string, unknown>,
			{
				allowUnknownActivePhaseCompletion:
					options.trustedPipelineProgress === true,
			},
		);
		...
		if (!isPipelineOrchestratorProgressWrite) {
			...
			if (
				...
				if (!gate.allowed)
					throw new Error(buildAutopilotRalplanUltragoalGateError(gate));
			}
		}
	}
 ...
 */
export async function listActiveModes(
	projectRoot?: string,
): Promise<Array<{ mode: string; state: ModeState }>> {
	const dirs = await getReadScopedStateDirs(projectRoot);
