...
describe("modes/base session-scoped persistence", () => {
	it("writes mode state into the current session scope when session.json exists", async () => {
		...
		try {
			...
			const raw = JSON.parse(await readFile(scopedPath, "utf-8")) as Record<
				string,
				unknown
			>;
	...
	it("writes session canonical skill state under the base state dir without nesting sessions twice", async () => {
		...
		try {
			...
			await updateModeState(
				"ralplan",
				{ current_phase: "planning", iteration: 1 },
				wd,
			);

			const canonicalPath = join(sessionDir, "skill-active-state.json");
			const nestedCanonicalPath = join(
				sessionDir,
				"sessions",
				sessionId,
	...
	it("prefers session-scoped reads over root fallback and writes updates back to the session scope", async () => {
		...
		try {
			...
			await writeFile(
				...
				JSON.stringify({
					active: true,
					current_phase: "root-only",
					iteration: 9,
				}),
			);
			await writeFile(
				join(sessionDir, "ralplan-state.json"),
				JSON.stringify({ active: true, current_phase: "draft", iteration: 1 }),
			);
			...
			await updateModeState(
				"ralplan",
				{ current_phase: "architect-review", iteration: 2 },
				wd,
			);
			const scoped = JSON.parse(
	...
	it("does not rebind root fallback Ralph task fields into a new session update", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-mode-session-ralph-no-rebind-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			assert.equal(
				existsSync(join(stateDir, "sessions", sessionId, "ralph-state.json")),
				false,
			);
			const root = JSON.parse(
	...
	it("preserves explicit fork updates when the implicit OMX_SESSION_ID is unmatched", async () => {
		...
		try {
			...
			const forkStatePath = join(
				stateDir,
				"sessions",
				forkSessionId,
				"ralplan-state.json",
			);
			await mkdir(join(stateDir, "sessions", forkSessionId), {
				recursive: true,
			});
			await writeFile(
				join(stateDir, "session.json"),
			...
			const updated = JSON.parse(
				await readFile(forkStatePath, "utf-8"),
			) as Record<string, unknown>;
