...
function panel(title: string, lines: string[], width: number): string[] {
	...
	return [
		bold(`╭ ${title} ${rule}`),
		...lines.map((line) => `│ ${truncate(line, safeWidth - 2)}`),
	];
}
...
function workerLabel(worker: SidecarWorkerSnapshot): string {
	...
	const turns =
		typeof worker.turns_without_progress === "number"
			? ` Δturns=${worker.turns_without_progress}`
			: "";
...
export function renderSidecar(
	...
	return withColorSetting(options.color !== false, () => {
		...
		lines.push(
			dim(
				`phase=${clean(snapshot.phase ?? "unknown")} generated=${clean(snapshot.generated_at)}`,
			),
		);
		lines.push("");
		...
		lines.push(
			...panel(
				...
				snapshot.workers.length > 0
					? snapshot.workers.map(workerLabel)
					: ["no workers found"],
				width,
			),
		);
		...
		lines.push(
			...panel(
				...
				snapshot.highlights.length > 0
					? snapshot.highlights.map(highlightLabel)
		...
		lines.push(
			...panel(
				...
				snapshot.panes.length > 0
					? snapshot.panes.map(
							(pane) => `${pane.target} [${pane.role}] ${pane.pane_id}`,
						)
					: ["no pane mapping available"],
				width,
			),
		);
		lines.push(
			...panel(
				...
				snapshot.events.length > 0
					? snapshot.events.map(
							(event) =>
								`${event.created_at} ${event.type} ${event.worker}${event.task_id ? ` task-${event.task_id}` : ""}${event.reason ? ` · ${event.reason}` : ""}`,
						)
					: ["no recent events"],
				width,
			),
		...
		if (snapshot.source_warnings.length > 0) {
			lines.push(
				...panel(
					"Warnings",
					snapshot.source_warnings.map((warning) => yellow(warning)),
					width,
				),
			);
		}
		...
		const rendered =
			maxLines > 0 && lines.length > maxLines
				? [
						...
						dim(`… ${lines.length - maxLines + 1} more lines`),
					]
				: lines;
