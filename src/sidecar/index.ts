...
export function parseSidecarArgs(args: string[]): ParsedSidecarArgs {
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (arg === "--json") flags.json = true;
			...
			flags.intervalMs = parsePositiveInt(
	...
	const renderTick = async (): Promise<void> => {
		...
		try {
			...
			const output = snapshot
				? dependencies.render(snapshot, {
						width: flags.width ?? dependencies.stdoutColumns?.(),
						height: dependencies.stdoutRows?.(),
					})
			...
			dependencies.writeStderr(
				`Sidecar watch render failed: ${error instanceof Error ? error.message : String(error)}\n`,
			);
			stop();
		} finally {
			inFlight = false;
...
export async function sidecarCommand(args: string[]): Promise<void> {
	...
	if (flags.tmux) {
		if (!process.env.TMUX) {
			console.error(
				"Not inside a tmux session. Start tmux first, then run: omx sidecar <team-name> --tmux",
			);
			process.exitCode = 1;
			return;
		}
		...
		if (!paneId) {
			console.error(
				"Failed to create sidecar tmux pane. Ensure tmux is available.",
			);
			process.exitCode = 1;
			return;
		}
	...
	if (flags.json) {
		console.log(
			JSON.stringify(
				snapshot ?? { status: "missing", team_name: teamName },
				null,
				2,
			),
		);
	...
	console.log(
		renderSidecar(snapshot, { width: flags.width ?? process.stdout.columns }),
	);
}
