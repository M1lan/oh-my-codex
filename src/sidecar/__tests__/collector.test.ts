...
async function withFixture(
	test: (cwd: string, teamRoot: string) => Promise<void>,
): Promise<void> {
...
describe("collectSidecarSnapshot", () => {
		...
		await withFixture(async (cwd, teamRoot) => {
			...
			await writeJson(join(teamRoot, "tasks", "task-1.json"), {
				...
				claim: {
					...
					leased_until: "2026-04-27T02:10:00.000Z",
				},
			});
			...
			await writeJson(join(teamRoot, "workers", "worker-1", "heartbeat.json"), {
				...
				alive: true,
				turn_count: 8,
				last_turn_at: "2026-04-27T02:00:01.000Z",
			});
			await writeJson(join(teamRoot, "workers", "worker-2", "status.json"), {
				state: "blocked",
				reason: "needs plan approval",
				current_task_id: "2",
				updated_at: "2026-04-27T02:00:02.000Z",
			});
			await writeJson(join(teamRoot, "workers", "worker-2", "heartbeat.json"), {
				pid: 456,
				alive: false,
				turn_count: 2,
				last_turn_at: "2026-04-27T01:59:00.000Z",
			});
			await writeJson(join(teamRoot, "phase.json"), {
			...
			await writeFile(
				join(teamRoot, "events", "events.ndjson"),
				[
					JSON.stringify({
						event_id: "e1",
						team: "demo",
						...
						worker: "worker-2",
						reason: "waiting",
						created_at: "2026-04-27T02:00:03.000Z",
					}),
					JSON.stringify({
						event_id: "e2",
						team: "demo",
						...
						task_id: "3",
						reason: "smoke failed",
						created_at: "2026-04-27T02:00:04.000Z",
					}),
				].join("\n"),
			);
			await writeJson(join(teamRoot, "mailbox", "worker-1.json"), {
				messages: ["must remain unchanged"],
			});

			const before = await snapshotFiles(teamRoot);
			const snapshot = await collectSidecarSnapshot("demo", {
				cwd,
				now: new Date("2026-04-27T02:01:00.000Z"),
			});
			...
			assert.deepEqual(
				after,
				before,
				"sidecar collector must be read-only over team state",
			);
			assert.ok(
				!after.has("manifest.v2.json"),
				"legacy config must not be migrated into a v2 manifest",
			);
			assert.ok(snapshot);
			assert.equal(snapshot.schema_version, "omx.sidecar/v1");
			assert.ok(
				!JSON.stringify(snapshot).includes("secret-claim-token"),
				"claim tokens must never be exposed in sidecar snapshots",
			);
			assert.deepEqual(snapshot.tasks.find((task) => task.id === "1")?.claim, {
				owner: "worker-1",
				leased_until: "2026-04-27T02:10:00.000Z",
			});
			...
			assert.ok(
				snapshot.highlights.some(
					(highlight) =>
						highlight.kind === "blocked-worker" &&
						highlight.target === "worker-2",
				),
			);
			assert.ok(
				snapshot.highlights.some(
					(highlight) =>
						highlight.kind === "dead-worker" && highlight.target === "worker-2",
				),
			);
			assert.ok(
				snapshot.highlights.some(
					(highlight) =>
						highlight.kind === "non-reporting-worker" &&
						highlight.target === "worker-1",
				),
			);
			assert.ok(
				snapshot.highlights.some(
					(highlight) =>
						highlight.kind === "blocked-task" && highlight.target === "task-2",
				),
			);
			assert.ok(
				snapshot.highlights.some(
					(highlight) =>
						highlight.kind === "failed-task" && highlight.target === "task-3",
				),
			);
	...
	it("rejects unsafe team names before reading state", async () => {
		await assert.rejects(
			() => collectSidecarSnapshot("../demo", { cwd: "/tmp" }),
			/Invalid team name/,
		);
	});
	...
	it("skips unsafe worker names before resolving worker state paths", async () => {
		await withFixture(async (cwd, teamRoot) => {
			...
			await writeJson(join(teamRoot, "escape", "status.json"), {
				state: "working",
				current_task_id: "escaped-status",
			});
			...
			assert.ok(
				snapshot.source_warnings.some((warning) =>
					warning.includes("skipped unsafe worker name: ../escape"),
				),
			);
	...
	it("returns recent events from a bounded tail window", async () => {
		await withFixture(async (cwd, teamRoot) => {
			...
			const snapshot = await collectSidecarSnapshot("demo", {
				cwd,
				eventLimit: 3,
				now: new Date("2026-04-27T02:31:00.000Z"),
			});
...
describe("collectSidecarSnapshot canonical Team state root parity", () => {
	async function withBoxedTeam(
		...
		try {
			...
			const teamRoot = join(
				resolveCanonicalTeamStateRoot(cwd, env),
				"team",
				"demo",
			);
			await mkdir(teamRoot, { recursive: true });
	...
	it("resolves sidecar team state under OMX_ROOT like the Team runtime", async () => {
		await withBoxedTeam("OMX_ROOT", async ({ cwd, env, boxedRoot }) => {
			...
			assert.ok(
				snapshot,
				"sidecar must read team state from the canonical OMX_ROOT location",
			);
	...
	it("resolves sidecar team state under OMX_STATE_ROOT like the Team runtime", async () => {
		await withBoxedTeam("OMX_STATE_ROOT", async ({ cwd, env, boxedRoot }) => {
			...
			assert.ok(
				snapshot,
				"sidecar must read team state from the canonical OMX_STATE_ROOT location",
			);
			assert.equal(snapshot.team_name, "demo");
		});
	});
	...
	it("still honors OMX_TEAM_STATE_ROOT precedence over OMX_ROOT", async () => {
		...
		try {
			...
			assert.ok(
				snapshot,
				"OMX_TEAM_STATE_ROOT must win over OMX_ROOT for sidecar lookup",
			);
			assert.equal(snapshot.team_name, "demo");
		} finally {
			await rm(cwd, { recursive: true, force: true });
