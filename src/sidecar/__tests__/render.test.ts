	...
	workers: [
		{
			...
			heartbeat: { alive: true, turn_count: 8 },
			alive: true,
			current_task: {
				id: "1",
	...
	highlights: [
		{
			...
			message: "worker-1 has 7 turns without task progress",
		},
	],
	source_warnings: [],
};
...
describe("renderSidecar", () => {
	it("renders stacked text panels with core visualization vocabulary", () => {
		...
		assert.match(
			output,
			/worker-1 \[working\/alive\] executor task-1 %3 Δturns=7/,
		);
		assert.match(
			output,
			/task-1 \[in_progress\] @worker-1 Implement collector/,
		);
	...
	it("does not leak raw ANSI fragments in default colored panel content", () => {
		const output = renderSidecar(
			{ ...SNAPSHOT, highlights: [], source_warnings: ["watch warning"] },
			{ width: 88 },
		);
	...
	it("honors height truncation for narrow right-side panes", () => {
		const output = renderSidecar(SNAPSHOT, {
			width: 44,
			height: 5,
			color: false,
		});
