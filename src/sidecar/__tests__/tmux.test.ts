...
import {
	buildSidecarTmuxSplitArgs,
	buildSidecarWatchCommand,
	launchSidecarTmuxPane,
} from "../tmux.js";

describe("sidecar tmux launcher", () => {
	it("builds a detached horizontal right-side split running watch mode", () => {
		const args = buildSidecarTmuxSplitArgs({
			cwd: "/repo",
			teamName: "demo-team",
			width: 52,
			sessionId: "sess 1",
		...
		assert.ok(
			args[10].includes(
				`'${process.execPath}' '/repo/dist/cli/omx.js' sidecar 'demo-team' --watch --width 52`,
			),
		);
	});

	it("uses a safe minimum sidecar width and parses the new pane id", () => {
		const command = buildSidecarWatchCommand({
			cwd: "/repo",
			teamName: "demo",
			width: 10,
			omxBin: "/repo/omx.js",
