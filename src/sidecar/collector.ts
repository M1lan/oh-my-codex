...
function asWorkerState(value: unknown): SidecarWorkerState {
	return typeof value === "string" &&
		(WORKER_STATES as readonly string[]).includes(value)
		? (value as SidecarWorkerState)
		: "unknown";
}

function asTaskStatus(value: unknown): SidecarTask["status"] {
	return typeof value === "string" &&
		(TEAM_TASK_STATUSES as readonly string[]).includes(value)
		? (value as SidecarTask["status"])
		: "pending";
}
...
): { worker: SidecarWorkerInfo | null; warning?: string } | null {
...
function normalizeConfig(raw: unknown): NormalizedConfig | null {
	...
	const workers = Array.isArray(value.workers)
		? value.workers
				.map((worker, index) => normalizeWorker(worker, index + 1))
				.flatMap((normalized): SidecarWorkerInfo[] => {
...
async function readSidecarConfig(root: string): Promise<{
	...
	if (manifestConfig)
		return {
			...
			source: manifestPath,
			warnings: manifestConfig.warnings,
		};
	...
	if (legacyConfig)
		return {
			...
			source: configPath,
			warnings: legacyConfig.warnings,
		};
...
function normalizeTask(raw: unknown): SidecarTask | null {
	...
	return {
		...
		claim:
			claim && safeString(claim.owner) && safeString(claim.leased_until)
				? {
						owner: safeString(claim.owner),
						leased_until: safeString(claim.leased_until),
					}
				: undefined,
...
async function readTasks(root: string): Promise<SidecarTask[]> {
	...
	const tasks = await Promise.all(
		files
			...
			.map((file) =>
				readJson<unknown>(join(tasksRoot, file)).then(normalizeTask),
			),
	);
...
async function readWorkerStatus(
	root: string,
	workerName: string,
): Promise<SidecarWorkerStatus> {
	const raw = await readJson<Record<string, unknown>>(
		join(root, "workers", workerName, "status.json"),
	);
	return {
...
async function readWorkerHeartbeat(
	root: string,
	workerName: string,
): Promise<SidecarWorkerHeartbeat | null> {
	const raw = await readJson<Record<string, unknown>>(
		join(root, "workers", workerName, "heartbeat.json"),
	);
	if (!raw) return null;
	return {
	...
	if (type === "worker_idle") {
		return {
			...
			prev_state: value.prev_state
				? asWorkerState(value.prev_state)
				: undefined,
...
export async function readTailText(
	path: string,
	maxBytes: number = DEFAULT_EVENT_TAIL_BYTES,
): Promise<string | null> {
...
function buildPanes(config: SidecarTeamConfig): SidecarPaneMapping[] {
	return [
		config.leader_pane_id
			? {
					target: "leader",
					pane_id: config.leader_pane_id,
					role: "leader" as const,
				}
			: null,
		config.hud_pane_id
			? { target: "hud", pane_id: config.hud_pane_id, role: "hud" as const }
			: null,
	...
	for (const worker of workers) {
		if (worker.status.state === "blocked") {
			highlights.push({
				...
				target: worker.name,
				kind: "blocked-worker",
				message: worker.status.reason || `${worker.name} is blocked`,
			});
		}
		if (worker.alive === false) {
			highlights.push({
				...
				target: worker.name,
				kind: "dead-worker",
				message: `${worker.name} heartbeat reports not alive`,
			});
		}
		if (
			typeof worker.turns_without_progress === "number" &&
			worker.turns_without_progress > 5
		) {
			highlights.push({
				severity: "warning",
				target: worker.name,
				kind: "non-reporting-worker",
				message: `${worker.name} has ${worker.turns_without_progress} turns without task progress`,
			});
		}
	}
	for (const task of tasks) {
		if (task.status === "blocked") {
			highlights.push({
				...
				target: `task-${task.id}`,
				kind: "blocked-task",
				message: task.subject || `task-${task.id} is blocked`,
			});
		}
		if (task.status === "failed") {
			highlights.push({
				...
				target: `task-${task.id}`,
				kind: "failed-task",
				message: task.error || task.subject || `task-${task.id} failed`,
			});
		}
	}
	return highlights;
...
function buildTopology(
	...
	const activeWorkers = workers.filter(
		(worker) => worker.status.state === "working",
	).length;
	const blockedWorkers = workers.filter(
		(worker) => worker.status.state === "blocked",
	).length;
	...
	return {
		...
		edges: workers.map((worker) => ({
			from: "leader",
			to: worker.name,
			label: worker.current_task
				? `task-${worker.current_task.id}`
				: worker.status.state,
		})),
	};
}
...
export async function collectSidecarSnapshot(
	...
	if (!TEAM_NAME_SAFE_PATTERN.test(sanitized)) {
		throw new Error(
			`Invalid team name: "${teamName}". Team name must match ${TEAM_NAME_SAFE_PATTERN}.`,
		);
	}
