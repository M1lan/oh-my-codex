...
function sidecarWidth(width: number | undefined): number {
	return Number.isFinite(width) && (width ?? 0) >= 30
		? Math.floor(width ?? 48)
		: 48;
}
...
export function buildSidecarWatchCommand(options: SidecarTmuxOptions): string {
	...
	if (!omxBin)
		throw new Error("Failed to resolve OMX launcher path for sidecar startup.");
	const prefix = options.sessionId
		? `OMX_SESSION_ID=${shellEscapeSingle(options.sessionId)} `
		: "";
...
export function buildSidecarTmuxSplitArgs(
...
export function launchSidecarTmuxPane(
	...
	try {
		return parsePaneIdFromTmuxOutput(
			execTmuxSync(buildSidecarTmuxSplitArgs(options)),
		);
	} catch {
		return null;
	}
