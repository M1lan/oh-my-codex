...
const GATEWAY_ENTRYPOINTS = ["gateway/status.py", "gateway/hooks.py"];
const DOC_ENTRYPOINTS = ["docs/acp-setup.md"];
...
function resolveHermesRoot(cwd: string): {
	path: string;
	source: "override" | "sibling-default";
} {
...
function resolveHermesHome(cwd: string): {
	path: string;
	source: "env" | "default";
} {
...
async function readJsonFile<T>(path: string): Promise<T | null> {
	try {
		const raw = await access(path, constants.R_OK).then(() =>
...
function extractConnectedPlatforms(
	runtime: HermesGatewayRuntimeFile | null,
): string[] {
...
function inferGatewayLive(
	...
	const gatewayState =
		typeof runtimeRecord?.gateway_state === "string"
...
export async function collectHermesEvidence(
	cwd = process.cwd(),
	...
	const sourceGateway = collectPathEvidence(
		hermesRoot.path,
		GATEWAY_ENTRYPOINTS,
	);
	...
	const installed =
		sourceAcp.present || sourceGateway.present || existsSync(stateStorePath);
...
export function buildHermesCapabilityOverrides(
	...
	return capabilities.map((capability) => {
		if (capability.id === "persistent-session-observation") {
			return {
				...capability,
				status: evidence.runtimeFiles.stateDbReadable
					? "ready"
					: evidence.installed
						? "stub"
						: "unsupported",
		...
		if (capability.id === "acp-envelope-bridge") {
			return {
				...capability,
				status: evidence.sourceRuntime.acp.present
					? "ready"
					: evidence.installed
						? "stub"
						: "unsupported",
...
export function buildHermesBootstrapMetadata(
	evidence: HermesEvidence,
): AdaptBootstrapMetadata {
	const commands = [...ACP_COMMANDS, ...STATUS_COMMANDS];
	...
	if (process.env[HERMES_BOOTSTRAP_ENV]?.trim()) {
		nextSteps.unshift(
			`Bootstrap override detected via ${HERMES_BOOTSTRAP_ENV}; keep Hermes-side reads pointed at OMX-owned adapter artifacts only.`,
		);
	}

	return {
		summary:
			"Hermes bootstrap metadata maps OMX lifecycle intent into ACP and gateway guidance without claiming direct control over Hermes internals.",
		eventBridge: [
...
export function buildHermesRuntimeObservation(
	evidence: HermesEvidence,
	...
	if (evidence.gateway.live) {
		return {
			state: "running",
			detail:
				evidence.gateway.connectedPlatforms.length > 0
	...
	if (
		evidence.runtimeFiles.gatewayStateReadable ||
		evidence.runtimeFiles.gatewayPidReadable ||
		evidence.runtimeFiles.stateDbReadable
	) {
		const reasons: string[] = [];
		if (evidence.runtimeFiles.gatewayStateReadable) {
			reasons.push(
				`gateway status readable (${basename(evidence.runtimeFiles.gatewayStatePath)})`,
			);
		}
		if (evidence.runtimeFiles.gatewayPidReadable) {
			reasons.push(
				`gateway pid readable (${basename(evidence.runtimeFiles.gatewayPidPath)})`,
			);
		}
		if (evidence.runtimeFiles.stateDbReadable) {
			reasons.push(
				`session store readable (${basename(evidence.runtimeFiles.stateDbPath)})`,
			);
		}
	...
	return {
		state: "installed",
		detail:
			"Hermes source surfaces were detected, but no readable runtime state files were found yet.",
		evidence: {
			hermesRoot: evidence.hermesRoot,
			hermesHome: evidence.hermesHome,
...
export function applyHermesEnvelope(
	...
	return {
		...envelope,
		capabilities: buildHermesCapabilityOverrides(
...
export function applyHermesProbe(
	...
	if (!evidence.installed) {
		nextSteps.push(
			`If Hermes lives elsewhere, set ${HERMES_ROOT_ENV} and rerun the probe.`,
		);
	} else if (!evidence.runtimeFiles.stateDbReadable) {
		nextSteps.push(
			`Ensure ${HERMES_HOME_ENV} points at the Hermes profile whose state.db OMX should inspect.`,
		);
	}
	...
	return {
		...report,
		summary:
			"Hermes probe inspected ACP, gateway, and session-store evidence from the external runtime.",
...
export function applyHermesStatus(
	...
	const summary =
		report.adapter.state === "initialized"
