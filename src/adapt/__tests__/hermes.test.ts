...
function writeHermesFixture(
	...
	if (withStateDb) {
		mkdirSync(hermesHome, { recursive: true });
		writeFileSync(
			join(hermesHome, "state.db"),
			"SQLite format 3\u0000 sessions messages",
		);
	}
}
...
beforeEach(async () => {
	...
	processCwdFixtureBase = await mkdtemp(
		join(tmpdir(), "omx-adapt-hermes-process-cwd-"),
	);
...
describe("hermes adapter integration", () => {
	it("degrades gracefully when Hermes runtime evidence is absent", async () => {
		const probe = await buildAdaptProbeReportForTarget(
			tempDir,
			"hermes",
			new Date("2026-04-14T00:00:00.000Z"),
		);
		const status = await buildAdaptStatusReportForTarget(
			tempDir,
			"hermes",
			new Date("2026-04-14T00:00:00.000Z"),
		);
		const doctor = await buildAdaptDoctorReportForTarget(
			tempDir,
			"hermes",
			new Date("2026-04-14T00:00:00.000Z"),
		);
		...
		assert.match(
			doctor.issues.map((issue) => issue.code).join(","),
	...
	it("anchors sibling-default Hermes discovery to the supplied cwd for programmatic probes", async () => {
		...
		writeFileSync(
			join(processCwdFixtureRoot, "acp_adapter", "server.py"),
			"# unrelated sibling runtime\n",
		);
		...
		assert.notEqual(
			probe.targetRuntime.evidence?.hermesRoot,
			processCwdFixtureRoot,
		);
		assert.match(
			probe.targetRuntime.detail,
			new RegExp(tempSiblingRoot.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
		);
	});
	...
	it("reports Hermes capability evidence asymmetrically in the envelope", async () => {
		...
		const envelope = await buildAdaptEnvelopeForTarget(
			tempDir,
			"hermes",
			new Date("2026-04-14T00:00:00.000Z"),
		);
		...
		assert.equal(
			envelope.capabilities.find(
				(capability) => capability.id === "persistent-session-observation",
			)?.status,
			"ready",
		);
		assert.equal(
			envelope.capabilities.find(
				(capability) => capability.id === "acp-envelope-bridge",
			)?.status,
			"ready",
		);
	});
	...
	it("keeps Hermes init writes inside OMX-owned adapter paths", async () => {
		...
		const persistedEnvelope = JSON.parse(
			readFileSync(result.envelope.adapterPaths.envelopePath, "utf-8"),
		) as {
		...
		assert.ok(
			(persistedEnvelope.bootstrap?.commands ?? []).includes("hermes acp"),
		);
	});
});
