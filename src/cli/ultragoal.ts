...
function printStatus(
	...
	if (summary.aggregateComplete) {
		...
		console.log(
			`codex goal reconciliation: not recorded in OMX aggregateCompletion; status is artifact-backed until a fresh Codex goal snapshot is available.`,
		);
		console.log(
...
function blockedDecisionHandoff(
	plan: Awaited<ReturnType<typeof readUltragoalPlan>>,
): string | null {
	...
	const [persistedSupportBlocker, persistedCapacityBlocker] = await Promise.all(
		[
			readJsonIfExists<Record<string, unknown>>(
				join(scope.baseStateDir, NATIVE_SUBAGENT_SUPPORT_BLOCKER_FILE),
			),
			readJsonIfExists<Record<string, unknown>>(
				join(scope.baseStateDir, NATIVE_SUBAGENT_CAPACITY_BLOCKER_FILE),
			),
		],
	);
	const persistedRoleRoutingMarker = scope.sessionId
		? readRoleRoutingMarker(scope.baseStateDir, {
				cwd: scope.cwd,
...
const STEERING_KINDS = new Set<UltragoalSteeringMutationKind>(
	ULTRAGOAL_STEERING_MUTATION_KINDS,
);
const STEERING_SOURCES = new Set<UltragoalSteeringSource>(
	ULTRAGOAL_STEERING_SOURCES,
);

type CliSteerResult = Awaited<ReturnType<typeof steerUltragoal>>;

function parseSteeringKind(
	raw: string | undefined,
): UltragoalSteeringMutationKind {
	if (!raw)
		throw new UltragoalError("Missing --kind for structured ultragoal steer.");
...
function normalizeTargetGoalId(
	raw: Record<string, unknown>,
): string | undefined {
	if (typeof raw.targetGoalId === "string" && raw.targetGoalId.trim())
		return raw.targetGoalId.trim();
	if (Array.isArray(raw.targetGoalIds))
		return raw.targetGoalIds
			.find(
...
function normalizeSteeringProposal(
	...
	const kind = parseSteeringKind(
		typeof raw.kind === "string" ? raw.kind : undefined,
	);
	const source = parseSteeringSource(
...
async function parseSteeringProposal(
	args: readonly string[],
): Promise<UltragoalSteeringProposal> {
	const directiveFile = readValue(args, "--directive-file");
	const directiveRaw =
		readValue(args, "--directive-json") ??
		(directiveFile ? await readFile(directiveFile, "utf-8") : undefined);
	if (directiveRaw) {
		const directive = assertPlainObject(
			await readJsonInput(directiveRaw, "--directive-json"),
			"--directive-json",
		);
		return normalizeSteeringProposal(
			directive,
			directiveRaw.trim().startsWith("{") ? directiveRaw : undefined,
		);
	}
	...
	const after = await readJsonInput(
		readValue(args, "--after-json"),
		"--after-json",
	);
	return normalizeSteeringProposal({
		...
		targetGoalId:
			readValue(args, "--target-goal-id") ?? readValue(args, "--goal-id"),
...
function printSteerResult(
	...
	if (result.rejectedReasons?.length)
		console.log(`rejected: ${result.rejectedReasons.join("; ")}`);
	if (proposal.idempotencyKey)
		console.log(`idempotency-key: ${proposal.idempotencyKey}`);
...
function readTeamWorkerIdentity(
	...
	const internalIdentity =
		typeof env.OMX_TEAM_INTERNAL_WORKER === "string"
			? env.OMX_TEAM_INTERNAL_WORKER.trim()
			: "";
	return internalIdentity || null;
}

function assertUltragoalMutationAllowedFromCurrentProcess(
	command: string,
): void {
...
export async function ultragoalCommand(
	args: string[],
	deps: UltragoalCommandDependencies = {},
): Promise<void> {
	const command = args[0] ?? "help";
	const rest = args.slice(1);
	const json = hasFlag(rest, "--json");
	const cwd = process.cwd();
	const buildGoalInstruction =
		deps.buildCodexGoalInstruction ?? buildCodexGoalInstruction;
	...
	try {
					...
					? reconcileCodexGoalSnapshot(snapshot, {
							...
							acceptedObjectives:
								plan.codexGoalMode === "aggregate"
									...
									: undefined,
							allowedStatuses:
								activeGoal && plan.codexGoalMode === "aggregate"
									? ["active"]
									: ["active", "complete"],
							requireSnapshot: false,
						})
					: null;
			const codexGoalFallback =
				reconciliation?.snapshot.unavailableReason === "db_schema_context_error"
					? {
							...
							message:
								"Codex goal DB/schema/context is unavailable; artifact-backed Ultragoal status remains available, but strict Codex goal completion reconciliation is deferred.",
						}
					: undefined;
			if (json)
				printJson({
				...
				printStatus(plan);
				if (codexGoalFallback)
					console.log(`codex goal fallback: ${codexGoalFallback.message}`);
				if (reconciliation && !reconciliation.ok)
					console.log(
						`codex goal warning: ${formatCodexGoalReconciliation(reconciliation)}`,
					);
		...
		if (command === "steer") {
			...
			if (proposalText) {
				const parsed = assertPlainObject(
					await readJsonInput(proposalText, "--proposal"),
					"--proposal",
				);
				const proposal = normalizeSteeringProposal(
					{ ...parsed, source: source ?? parsed.source },
					proposalText,
				);
				const result = await steerUltragoal(cwd, proposal, {
					directiveText: proposalText,
				});
			...
			if (!proposal.rationale?.trim?.())
				throw new UltragoalError("Missing --rationale.");
			proposal.source = parseSteeringSource(source, proposal.source);
			const result = await steerUltragoal(cwd, proposal, {
				directiveText: buildSteerDirectiveText(rest),
			});
			printSteerResult(proposal, result, json);
			if (!result.accepted) process.exitCode = 1;
			return;
		...
		if (command === "record-review-blockers") {
			...
			const codexGoal = await parseCodexGoalJson(
				readValue(rest, "--codex-goal-json"),
			);
		...
		if (
			...
			if (!result.goal) {
				...
				if (json) {
					printJson({
						...
						handoff,
						blockedGoals: result.plan.goals.filter(
			...
			const nativeSubagentSupport =
				await resolveCodexGoalNativeSubagentSupport(cwd);
		...
		if (command === "checkpoint") {
			...
			if (json)
				...
				if (
					status === "complete" &&
					(summary.aggregateComplete || summary.artifactComplete)
				) {
					console.log(
						buildCodexGoalTerminalCleanupNotice("Ultragoal completion"),
					);
				}
			}
			return;
		...
		if (
			...
			error instanceof CodexGoalSnapshotError
		) {
