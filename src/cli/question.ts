...
function parseQuestionWaitTimeoutMs(
...
function parseQuestionArgs(args: string[]): ParsedQuestionArgs {
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (arg === "--answer-question-id") {
			const next = args[index + 1];
			if (!next)
				throw new Error("Missing question id after --answer-question-id");
...
async function readOwningAutopilotDeepInterviewObligation(
	...
	const ownerObligationId = String(
		env[AUTOPILOT_DEEP_INTERVIEW_QUESTION_OWNER_ENV] ?? "",
	).trim();
...
async function finalizeDirectDeepInterviewObligation(
	...
	await updateDeepInterviewQuestionEnforcement(cwd, sessionId, (current) => {
...
export async function questionCommand(args: string[]): Promise<void> {
	...
	if (parsed.answerQuestionId) {
		if (!parsed.answer)
			throw new Error("--answer-question-id requires --answer");
	...
	let directDeepInterviewObligation: DeepInterviewQuestionEnforcementState | null =
		null;
	if (isDeepInterviewQuestionSource(input.source) && policy.sessionId) {
		directDeepInterviewObligation =
			await readOwningAutopilotDeepInterviewObligation(cwd, policy.sessionId);
		if (!directDeepInterviewObligation) {
			directDeepInterviewObligation = createDeepInterviewQuestionObligation();
			const autopilotWaitClaim =
				await claimAutopilotDeepInterviewQuestionWaiting(
					cwd,
					policy.sessionId,
					directDeepInterviewObligation,
				);
	...
	try {
		...
		finalRecord = await waitForQuestionTerminalState(recordPath, {
			timeoutMs: waitTimeoutMs,
			rendererAlive: (currentRecord) =>
				isQuestionRendererAlive(currentRecord.renderer),
