...
import { homedir, constants as osConstants } from "os";
import {
	basename,
	dirname,
	isAbsolute,
...
Usage:
  ...
  omx ask       Ask local provider CLI (claude|gemini|gjc) and write artifact output
...
function unsupportedLaunchShorthandError(flag: "--max" | "--ultra"): Error {
	...
	return new Error(
		`Unsupported OMX launch shorthand "${flag}".\n${guidance}\nRun "omx help" for usage.`,
	);
}
...
export function resolveSetupAgentsMergePolicyArg(
	args: string[],
): SetupMergeAgentsPolicyArg {
	let policy: SetupMergeAgentsPolicyArg;
	const setPolicy = (
		next: Exclude<SetupMergeAgentsPolicyArg, undefined>,
		source: string,
	): void => {
		if (
			policy &&
			(policy.kind !== next.kind ||
				(policy.kind === "set" &&
					next.kind === "set" &&
					policy.value !== next.value))
		) {
			throw new Error(
				`Conflicting setup AGENTS merge policy flags: ${source} conflicts with another merge policy selector.`,
			);
		}
		policy = next;
	};
...
export function resolveSetupMergeAgentsArg(
	args: string[],
): boolean | undefined {
...
export function resolveSetupInstallModeArg(
	args: string[],
): SetupInstallMode | undefined {
...
export function isResumeCodexLaunch(args: string[]): boolean {
	...
	for (let index = 0; index < omxArgs.length; index += 1) {
		...
		if (optionValue?.valueArity === "variadic") {
			...
			while (
				nextIndex < omxArgs.length &&
				!omxArgs[nextIndex]!.startsWith("-")
			) {
				nextIndex += 1;
			}
			index = nextIndex - 1;
...
function captureDetachedLeaderAuthority(
	leaderPaneId: string,
	expectedSessionName: string,
	ownerId: string,
): DetachedLeaderAuthority {
	if (!/^%[0-9]+$/.test(leaderPaneId) || !ownerId)
		throw new Error("invalid detached leader authority target");
	...
	if (
		...
		paneId !== leaderPaneId ||
		!/^\$[0-9]+$/.test(sessionId) ||
		!/^[0-9]+$/.test(sessionCreated) ||
		!/^[0-9]+$/.test(windowIndex) ||
		!/^@[0-9]+$/.test(windowId) ||
...
function detachedLeaderAuthorityCondition(
...
function runDetachedLeaderMutation(
	...
	const output = execTmuxFileSync(
		[
			...
			"-t",
			authority.paneId,
			detachedLeaderAuthorityCondition(authority, requireOwner),
			success,
			"display-message -p ''",
		],
		{ encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] },
	...
	if (output !== receipt)
		throw new Error("detached leader authority changed before tmux mutation");
}
...
function runDetachedHudMutation(
	...
	const output = execTmuxFileSync(
		[
			...
			"-F",
			"-t",
			leaderAuthority.paneId,
			detachedLeaderAuthorityCondition(leaderAuthority),
			hudGuard,
			"display-message -p ''",
		],
		{ encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] },
	...
	if (output !== receipt)
		throw new Error(
			"detached leader or HUD authority changed before tmux mutation",
		);
}
...
function buildDeferredDetachedHudGuard(
	...
	const guardedResize = (_match: string, sink: string): string => {
		...
		const outerFormatEscaped = (condition: string): string =>
...
export function guardDetachedHudDeferredMutation(
	...
	if (args[0] === "set-hook" && typeof args.at(-1) === "string") {
		...
		if (!quotedPayload.startsWith("'") || !quotedPayload.endsWith("'")) {
			throw new Error(
				"detached deferred HUD hook has an invalid run-shell payload",
			);
		}
		...
		return [
			...args.slice(0, -1),
			`${runShellPrefix}${quoteShellArg(guardedPayload)}`,
		];
	}
	if (args[0] === "run-shell") {
...
function runDetachedLeaderSplit(
	authority: DetachedLeaderAuthority,
	args: string[],
	...
	if (targetIndex < 0 || splitArgs[targetIndex + 1] !== authority.sessionName)
		throw new Error("invalid detached HUD split target");
	splitArgs[targetIndex + 1] = authority.paneId;
	const formatIndex = splitArgs.indexOf("-F");
	if (formatIndex < 0 || splitArgs[formatIndex + 1] !== "#{pane_id}")
		throw new Error("invalid detached HUD split receipt format");
	const commandIndex = splitArgs.length - 1;
	splitArgs[commandIndex] =
		`env OMX_DETACHED_HUD_OPERATION=${operationMarker} ${splitArgs[commandIndex]}`;
	splitArgs[formatIndex + 1] =
		`#{pane_id}\t#{pane_pid}\t#{session_id}\t#{window_id}\t${receipt}`;
	const output = execTmuxFileSync(
		[
			"if-shell",
			"-F",
			"-t",
			authority.paneId,
			detachedLeaderAuthorityCondition(authority),
			splitArgs.map(quoteShellArg).join(" "),
			"display-message -p ''",
		],
		{ encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] },
	).trim();
	const [paneId, panePid, sessionId, windowId, observedReceipt, ...extra] =
		output.split("\t");
	if (
		!/^%[0-9]+$/.test(paneId) ||
		!/^[1-9][0-9]*$/.test(panePid) ||
	...
	const parsedPid = Number(panePid);
	if (!Number.isSafeInteger(parsedPid) || parsedPid <= 0)
		throw new Error("malformed detached HUD authority");
...
function logEstablishmentCleanupEvidence(error: unknown): void {
	const cleanup = (
		error as { establishmentCleanup?: EstablishmentCleanupEvidence } | undefined
	)?.establishmentCleanup;
	if (!cleanup) return;
	process.stderr.write(
		`[omx] establishment cleanup evidence: ${JSON.stringify(cleanup)}\n`,
	);
}
...
function readMadmaxDetachedActiveRecord(
	...
	try {
		...
		) as Partial<MadmaxDetachedActiveRecord>;
		if (
			parsed.version !== 1 ||
		...
		return {
			...
			...(typeof parsed.tmux_pane_id === "string"
				...
				: {}),
			...(typeof parsed.tmux_internal_session_id === "string"
				? { tmux_internal_session_id: parsed.tmux_internal_session_id }
				: {}),
			...(typeof parsed.tmux_session_created === "string"
				? { tmux_session_created: parsed.tmux_session_created }
				: {}),
			...(typeof parsed.tmux_pane_pid === "number"
				? { tmux_pane_pid: parsed.tmux_pane_pid }
				: {}),
			...
			parsed.lifecycle_phase === "terminal"
...
function queryMadmaxDetachedRuntimeBinding(
	record: MadmaxDetachedActiveRecord,
): MadmaxDetachedRuntimeBinding | null {
	...
	try {
		...
		for (const line of output.split("\n").filter(Boolean)) {
			...
			if (
				dead !== "0" ||
				sessionName !== record.tmux_session_name ||
				omxInstanceId !== record.session_id
			)
				return null;
			binding = {
				paneId,
...
function hasMatchingMadmaxDetachedRuntimeBinding(
	record: MadmaxDetachedActiveRecord,
): boolean {
...
function isReusableMadmaxDetachedActiveRecord(
	...
	if (
		record.lifecycle_phase !== "ready" ||
		!record.leader_pid ||
		!record.base_state_root ||
		!record.launch_nonce
	)
		return false;
	try {
		process.kill(record.leader_pid, 0);
	} catch {
		return false;
	}
	try {
		if (realpathSync(record.base_state_root) !== record.base_state_root)
			return false;
	} catch {
		return false;
	}
...
function readMadmaxDetachedLockOwner(
	lockPath: string,
): MadmaxDetachedLockOwner | null {
	try {
		const parsed = JSON.parse(
			readFileSync(join(lockPath, "owner.json"), "utf-8"),
		) as Partial<MadmaxDetachedLockOwner>;
		if (
			parsed.version !== 1 ||
...
function cleanupCurrentMadmaxReuseRunRoot(
	env: NodeJS.ProcessEnv,
	runsRoot: string,
): void {
...
function detachedFailureCode(error: unknown): string {
	...
	return typeof code === "string" &&
		/^(?:EACCES|EAGAIN|EBUSY|ECANCELED|ECONNREFUSED|EEXIST|EINTR|EINVAL|EIO|EISDIR|EMFILE|ENFILE|ENOENT|ENOSPC|ENOTDIR|EPERM|EPIPE|EROFS|ESRCH|ETIMEDOUT)$/.test(
			code,
		)
		? code
		: "unknown";
...
export type DetachedLaunchTransition =
	| "D0"
	| "D1"
	| "D2"
	| "D3"
...
export interface DetachedLaunchDependencies<
	Binding = unknown,
	InertSession = unknown,
	Pane = string,
> {
	...
	publishActiveRecord(
		session: InertSession,
		pane: Pane,
	): Promise<DetachedActiveRecordOwnership>;
	 ...
	 */
	abortAndAwaitFinalization?(
		error: unknown,
	): Promise<DetachedReleaseFailureResolution>;
...
export async function executeDetachedLaunchStateMachine<
	Binding,
	InertSession,
	Pane,
>(
	...
	const transition = (name: DetachedLaunchTransition): void => {
		report.transitions.push(name);
	};
	...
	try {
		...
		if (completion.kind === "failure") {
			...
			try {
				...
				report.rollback.failures.push({
					step: "setup-finalization",
					status: "failed",
					code: detachedFailureCode(finalizationError),
				});
		...
		transition("D5");
		if (
			(await deps.updateNameMetadata(binding, inertSession)) !==
			"committed-released"
		) {
			throw new DetachedLaunchSafetyError(
				"name-metadata",
				new Error("detached name metadata did not release"),
				report,
			);
		}
		transition("D6");
		if (
			(await deps.updatePaneMetadata(binding, pane)) !== "committed-released"
		) {
			throw new DetachedLaunchSafetyError(
				"pane-metadata",
				new Error("detached pane metadata did not release"),
				report,
			);
		}
		transition("D7");
		...
		try {
			...
			try {
				...
				throw new AggregateError(
					...
					"detached release and abort publication both failed",
				);
			}
			rollbackAuthorized =
				...
				Boolean(
					resolution.nonce &&
						...
						Number.isSafeInteger(resolution.leaderPid) &&
						resolution.kind,
				);
		...
		return {
			kind: input.preflight.shouldAttach ? "attached" : "returned",
			released: true,
			report,
		};
	} catch (error) {
		...
		if (!released) {
			try {
				...
				report.rollback.failures.push({
					step: "rollback",
					status: "failed",
					code: detachedFailureCode(rollbackError),
				});
...
function preflightDetachedLaunch(): DetachedPreflightAvailable {
	...
	const report: DetachedBootstrapReport = {
		transitions: ["D0"],
		rollback: { attempted: [], failures: [] },
	};
...
function handleMadmaxDetachedReuse(
	...
	const activeRecordPath = contextKey
		? madmaxDetachedActiveRecordPath(
				resolveMadmaxRunsRoot(process.env),
				contextKey,
			)
		: null;
	...
	if (
		...
		return false;
	cleanupCurrentMadmaxReuseRunRoot(
		process.env,
		resolveMadmaxRunsRoot(process.env),
	);
...
function activateMadmaxIsolationIfNeeded(
	...
	process.stderr.write(
		`[omx] madmax isolated state: ${runDir} (source: ${cwd})\n`,
	);
}

function renderDetachedLaunchSafetyReport(
	error: DetachedLaunchSafetyError,
): string {
	const establishmentCleanup =
		error.report.establishmentCleanup?.capability.map((entry) => ({
			role: entry.role,
			phase: entry.phase,
			status: entry.status,
	...
	return JSON.stringify({
		...
		rollback: {
			...
			failures: error.report.rollback.failures.map(
				({ step, status, code }) => ({ step, status, code }),
			),
		},
	});
}
...
export async function main(args: string[]): Promise<void> {
	...
	try {
		...
		if (
			command === "resume" &&
			launchArgs.some((arg) => arg === "--help" || arg === "-h")
		) {
		...
		if (err instanceof DetachedLaunchSafetyError) {
			...
			console.error(
				`[omx] detached launch safety report: ${renderDetachedLaunchSafetyReport(err)}`,
			);
		} else {
...
async function readStaleCurrentAutopilotStatus(
	cwd: string,
): Promise<StaleCurrentAutopilotStatus | null> {
	let parsed: unknown;
	try {
		parsed = JSON.parse(
			await readFile(
	...
	const phase =
		sanitizedStatusString(state.current_phase) ??
		sanitizedStatusString(state.currentPhase);
	const sessionId =
		sanitizedStatusString(state.session_id) ??
		sanitizedStatusString(state.sessionId);
	const tmuxPaneId =
		sanitizedStatusString(state.tmux_pane_id) ??
		sanitizedStatusString(state.tmuxPaneId);
...
async function showStatus(): Promise<void> {
	...
	try {
		...
		const hasActiveWorkflowMode = async (
			candidate: ModeStateFileRef[],
		): Promise<boolean> => {
...
export async function launchWithHud(args: string[]): Promise<void> {
	...
	let { launchPolicy } = resolveTmuxAwareLaunchPolicy(
		explicitLaunchPolicy,
		isNativeWindows(),
	);
	...
	if (launchPolicy === "detached-tmux") {
		try {
			...
			console.warn(
				"[omx] warning: detached tmux transport is unavailable. Falling back to direct Codex launch.",
			);
			launchPolicy = "direct";
		}
	}
	...
	if (
		...
		detachedPreflight &&
		handleMadmaxDetachedReuse(madmaxWorktreeRuntimeContext, detachedPreflight)
	)
		return;
		...
		try {
			...
			if (isSessionPointerLaunchAbort(err)) {
				...
				reportOrdinaryLaunchRootConflict(
					...
					cwd,
					cwd === launchCwd &&
						!parsedWorktree.mode.enabled &&
						!isResumeCodexLaunch(normalizedArgs),
				);
			}
	...
	try {
		...
		if (error instanceof DetachedLaunchSafetyError)
			postLaunchHandledExternally = true;
		throw error;
	} finally {
...
export async function execWithOverlay(args: string[]): Promise<void> {
	...
	try {
		...
		if (!omxBin)
			throw new Error("Unable to resolve OMX launcher path for exec");
...
function buildDetachedWindowsLeaderCommand(
	...
	const payload = Buffer.from(
		JSON.stringify({
			...
			runtimeCodexHomeForCleanup,
			...(parentEnv
				? { parentEnv: detachedSessionParentEnvironment(parentEnv) }
				: {}),
			readyPath,
			preLaunchOptions,
		}),
...
function writeDetachedLeaderReport(
	path: string,
	report: DetachedLeaderReport,
): void {
...
function readDetachedLeaderReport(
	path: string,
): DetachedLeaderReport | undefined {
	try {
		...
		if (
			report.version !== 1 ||
			!["ready", "failed", "terminal", "release", "abort"].includes(
				String(report.kind),
			) ||
			typeof report.nonce !== "string" ||
			typeof report.sessionId !== "string" ||
			typeof report.sessionName !== "string"
		)
			return undefined;
		if (report.paneId !== undefined && typeof report.paneId !== "string")
			return undefined;
		if (
			report.leaderPid !== undefined &&
			(!Number.isSafeInteger(report.leaderPid) || Number(report.leaderPid) <= 0)
		)
			return undefined;
		if (
			report.exitStatus !== undefined &&
			(!Number.isSafeInteger(report.exitStatus) ||
				Number(report.exitStatus) < 0 ||
				Number(report.exitStatus) > 255)
		)
			return undefined;
...
export function applyDetachedTerminalExitStatus(
	...
	if (!report || report.kind !== "terminal" || report.finalized !== true)
		return false;
	if (
		report.nonce !== expected.nonce ||
		report.sessionId !== expected.sessionId ||
		report.sessionName !== expected.sessionName ||
		report.leaderPid !== expected.leaderPid
	)
		return false;
...
/** Internal detached-launch seam. Exported solely for deterministic CLI tests. */
export function probeExactDetachedSessionExists(
	authority: DetachedLeaderAuthority | null,
): boolean | undefined {
	...
	try {
		...
		const current = captureDetachedLeaderAuthority(
			authority.paneId,
			authority.sessionName,
			authority.ownerId,
		);
		return (
			current.panePid === authority.panePid &&
			current.sessionId === authority.sessionId &&
			current.sessionCreated === authority.sessionCreated &&
			current.windowId === authority.windowId &&
...
export function resolveDetachedAttachExitStatus(
	...
	if (applyDetachedTerminalExitStatus(releaseMarkerPath, expected))
		return "applied";
	return probe.exactSessionExists() === true
		? "manual-detach"
		: "protocol-failure";
}

function publishDetachedAbortMarker(
	releaseMarkerPath: string,
	nonce: string,
	sessionId: string,
	sessionName: string,
	...
	writeDetachedLeaderReport(`${releaseMarkerPath}.abort`, {
		version: 1,
		kind: "abort",
		nonce,
		sessionId,
...
export function buildDetachedSessionBootstrapSteps(
	...
	const newSessionArgs: string[] = [
		...
		...(sqliteHomeOverride
			? ["-e", `${CODEX_SQLITE_HOME_ENV}=${sqliteHomeOverride}`]
			: []),
...
export type MandatorySetupOperation =
	| "overlay"
	| "session-instructions"
	| "session-metrics";
export type CompletionResult =
	...
	| {
			...
			operations: readonly DegradedSetupOperation[];
			establishmentCleanup?: EstablishmentCleanupEvidence;
	  }
	| {
			kind: "failure";
			operation: MandatorySetupOperation;
			error: unknown;
			establishmentCleanup?: EstablishmentCleanupEvidence;
	  };
...
async function establishPreLaunchBinding(
	...
	const established = await establishLaunchSessionBinding(
		cwd,
		sessionId,
		await resolvePreLaunchSessionPointerOptions(),
	);
	if (established.kind === "committed-released")
		return { binding: established.binding, cleanup: established.cleanup };
	const error =
		established.kind === "precommit-aborted"
			? established.abort
			: established.error;
...
async function failPreLaunchSetup(
	...
	try {
		...
		try {
			...
			if (close.status === "failed")
				failures.push(close.error ?? new Error("bound session close failed"));
		} catch (error) {
			failures.push(error);
		}
	}
	if (failures.length > 0)
		throw new AggregateError(failures, "preLaunch setup finalization failed");
}
...
async function completePreLaunchSetup(
	...
	try {
		...
		degraded.push("notify-watcher");
		logCliOperationFailure(error);
	}
	try {
		await startHookDerivedWatcher(cwd);
	} catch (error) {
		degraded.push("derived-watcher");
		logCliOperationFailure(error);
	}
	try {
		if (notifyTempContract?.active) {
			...
			const startup = buildNotifyTempStartupMessages(
				notifyTempContract,
				Boolean(getNotificationConfig()?.enabled),
			);
	...
	try {
		...
		degraded.push("native-lifecycle-event");
		logCliOperationFailure(error);
	}
	return degraded.length === 0
		? { kind: "success" }
		: { kind: "degraded-success", operations: degraded };
}
...
export async function preLaunch(
	...
	if (completion.kind === "failure") {
		try {
			await failPreLaunchSetup(established.binding);
		} catch (cleanupError) {
			throw new AggregateError(
				[completion.error, cleanupError],
				`preLaunch ${completion.operation} failed`,
			);
		}
...
async function runCodex(
	...
	if (launchPolicy === "inside-tmux") {
		...
		if (!detachedPreflight) {
			throw new DetachedLaunchSafetyError(
				"establishment",
				new Error("detached launch requires a frozen available preflight"),
				{ transitions: ["D0"], rollback: { attempted: [], failures: [] } },
			);
		}
		...
		const launchDetachedSession = async (): Promise<{
			...
			const bootstrapLeader = async (): Promise<void> => {
				...
				const bootstrapSteps = buildDetachedSessionBootstrapSteps(
					sessionName,
					cwd,
					nativeWindows && detachedWindowsCodexCmd
					...
					omxBin,
					{
						...(preLaunchOptions.notifyTempContract.active
							? { notifyTempContract: preLaunchOptions.notifyTempContract }
							: {}),
						enableNotifyFallbackAuthority:
							preLaunchOptions.enableNotifyFallbackAuthority,
				...
				for (const step of bootstrapSteps) {
					try {
						if (step.name === "new-session") {
							...
							detachedLeaderAuthority = captureDetachedLeaderAuthority(
								leaderPaneId,
								sessionName,
								sessionId,
							);
						...
						if (step.name === "tag-session") {
							...
							runDetachedLeaderMutation(authority, [
								"set-option",
								"-q",
								"-t",
								authority.sessionName,
						...
						for (const finalizeStep of buildDetachedSessionFinalizeSteps(
							authority.sessionName,
							detachedHudAuthority.paneId,
							authority.windowIndex,
							process.env.OMX_MOUSE !== "0",
							nativeWindows,
							detachedPreflight.shouldAttach,
							authority.paneId,
						)) {
							if (finalizeStep.name === "attach-session") {
								attachStep = finalizeStep;
								continue;
							}
						...
						throw new DetachedLaunchSafetyError(
							step.name === "new-session" ? "inert-session" : "pane-id",
							error,
							detachedPreflight.report,
						);
					}
				}
				return undefined;
				...
				{ preflight: detachedPreflight },
				{
					...
					complete: async () => {
						...
						while (true) {
							const report = readDetachedLeaderReport(releaseMarkerPath);
							if (
								report?.nonce === detachedLaunchNonce &&
								report.sessionId === sessionId &&
								report.sessionName === sessionName &&
								report.paneId === detachedLeaderPaneId &&
								report.leaderPid &&
								report.kind === "ready"
							) {
							...
							if (
								...
								return {
									kind: "failure",
									operation: "session-instructions",
									error: new Error(
										report.error || "detached leader setup failed",
									),
								};
							}
							if (Date.now() >= readyDeadline)
								return {
									kind: "failure",
									operation: "session-instructions",
									error: new Error("detached leader readiness timed out"),
								};
							blockMs(20);
						}
					},
					createInertSession: async () => {
						...
						if (
							...
							report.sessionId !== sessionId ||
							report.sessionName !== sessionName ||
							report.leaderPid !== detachedLeaderPid
						) {
							throw new Error(
					...
					publishActiveRecord: async () => {
						const activeRecordPath = contextKey
							...
							: join(omxRoot(cwd), "state", "detached-active-record.json");
						const record = readMadmaxDetachedActiveRecord(activeRecordPath);
						if (
							!record ||
							record.launch_nonce !== detachedLaunchNonce ||
							record.leader_pid !== detachedLeaderPid ||
							record.session_id !== sessionId ||
							record.tmux_session_name !== sessionName ||
							record.tmux_pane_id !== detachedLeaderPaneId
						)
							throw new Error(
					...
					releaseBarrier: async () => {
						if (!detachedLeaderPid)
							throw new Error("detached leader PID missing before release");
						publishDetachedReleaseMarker(
							...
							sessionName,
							detachedLeaderPid,
							detachedHudAuthority ?? undefined,
						);
					},
					abortAndAwaitFinalization: async () => {
						...
						if (
							current?.nonce === detachedLaunchNonce &&
							current.sessionId === sessionId &&
							current.sessionName === sessionName &&
							current.leaderPid === detachedLeaderPid &&
							current.finalized === true &&
							(current.kind === "failed" || current.kind === "terminal")
						)
							return {
								acknowledged: true,
								nonce: current.nonce,
						...
						while (Date.now() < deadline) {
							...
							if (
								...
								(report.kind === "failed" || report.kind === "terminal")
							)
								return {
									acknowledged: true,
									nonce: report.nonce,
					...
					attachOrReturn: async () => {
						if (attachStep)
							execTmuxFileSync(attachStep.args, { stdio: "inherit" });
					},
					rollback: async (_ownedRecord, report) => {
						const attempt = async (
							...
							try {
								...
								report.rollback.failures.push({
									step,
									status: "failed",
									code: detachedFailureCode(error),
								});
							}
						};
						await attempt("parent-env", async () => {
							if (detachedParentEnvFilePath)
								await rm(detachedParentEnvFilePath, { force: true });
						});
						await attempt("runtime-codex-home", () =>
							cleanupRuntimeCodexHome(
								runtimeCodexHomeForCleanup,
								projectLocalCodexHomeForCleanup,
							),
						...
						if (createdSession) {
							for (const step of buildDetachedSessionRollbackSteps(
								sessionName,
								null,
								null,
								null,
			...
			if (launchTerminal.kind === "attached" && detachedLeaderPid) {
				...
				const attachResolution = resolveDetachedAttachExitStatus(
					...
					{
						exactSessionExists: () =>
							probeExactDetachedSessionExists(detachedLeaderAuthority),
					},
				);
				if (attachResolution === "protocol-failure") {
					process.stderr.write(
						"[omx] detached session ended without an authenticated finalized terminal status; treating the launch as failed.\n",
					);
...
function decodeDetachedLeaderPayload(
	encoded: string | undefined,
): DetachedLeaderPayload {
	if (!encoded || !/^[A-Za-z0-9_-]+$/.test(encoded))
		throw new Error("invalid detached leader payload");
	let value: unknown;
	try {
	...
	if (
		...
		typeof payload.codexCmd !== "string" ||
		!options ||
		typeof options !== "object" ||
		typeof (options as Record<string, unknown>)
			.enableNotifyFallbackAuthority !== "boolean" ||
		typeof (options as Record<string, unknown>).worktreeDirty !== "boolean"
	)
		throw new Error("invalid detached leader payload");
	const notifyTempContract = (options as Record<string, unknown>)
		.notifyTempContract;
	if (
		notifyTempContract !== undefined &&
		(!notifyTempContract || typeof notifyTempContract !== "object")
	)
		throw new Error("invalid detached leader payload");
	...
	return {
		...
		codexCmd: payload.codexCmd,
		...(typeof payload.codexHomeOverride === "string"
			? { codexHomeOverride: payload.codexHomeOverride }
			: {}),
		...(typeof payload.projectLocalCodexHomeForCleanup === "string"
			? {
					projectLocalCodexHomeForCleanup:
						payload.projectLocalCodexHomeForCleanup,
				}
			: {}),
		...(typeof payload.runtimeCodexHomeForCleanup === "string"
			? { runtimeCodexHomeForCleanup: payload.runtimeCodexHomeForCleanup }
			: {}),
		...(parentEnv ? { parentEnv: parentEnv as Record<string, string> } : {}),
		...(typeof payload.readyPath === "string"
			? { readyPath: payload.readyPath }
			: {}),
		preLaunchOptions: {
			...(notifyTempContract
				? { notifyTempContract: notifyTempContract as NotifyTempContract }
				: {}),
			enableNotifyFallbackAuthority: (options as DetachedLeaderPreLaunchOptions)
				.enableNotifyFallbackAuthority,
...
function parseDetachedHudAuthorityProof(
	value: unknown,
): DetachedHudAuthority | undefined {
	if (!value || typeof value !== "object") return undefined;
	const proof = value as Record<string, unknown>;
	if (typeof proof.paneId !== "string" || !/^%[0-9]+$/.test(proof.paneId))
		return undefined;
	if (
		typeof proof.panePid !== "number" ||
		!Number.isSafeInteger(proof.panePid) ||
		proof.panePid <= 0
	)
		return undefined;
	if (
		typeof proof.sessionId !== "string" ||
		!/^\$[0-9]+$/.test(proof.sessionId)
	)
		return undefined;
	if (typeof proof.windowId !== "string" || !/^@[0-9]+$/.test(proof.windowId))
		return undefined;
	if (
		typeof proof.operationMarker !== "string" ||
	...
	return {
		...
		operationMarker: proof.operationMarker,
	};
}

function teardownDetachedOwnedHudPane(
	leaderPaneId: string,
	payload: DetachedLeaderPayload,
	hudProof: DetachedHudAuthority | undefined,
): void {
	...
	if (!hudProof) return;
	try {
		const leaderAuthority = captureDetachedLeaderAuthority(
			leaderPaneId,
			payload.sessionName,
			payload.sessionId,
		);
		runDetachedHudMutation(leaderAuthority, hudProof, [
			"kill-pane",
			"-t",
			hudProof.paneId,
		]);
...
async function runDetachedSessionLeader(
	payload: DetachedLeaderPayload,
): Promise<void> {
	for (const [key, value] of Object.entries(payload.parentEnv ?? {}))
		process.env[key] = value;
	const established = await establishPreLaunchBinding(
		payload.cwd,
		payload.sessionId,
	);
	...
	try {
		...
		if (completion.kind === "failure") {
			const finalization = await finalizeBoundOnce(
				binding,
				"setup-failure",
				payload.cwd,
			);
		...
		const name = await updateDetachedSessionMetadata(binding, {
			tmuxSessionName: payload.sessionName,
		});
		const paneUpdate = await updateDetachedSessionMetadata(binding, {
			tmuxPaneId: pane,
		});
		if (
			name.kind !== "committed-released" ||
			paneUpdate.kind !== "committed-released"
		)
			throw new Error("detached leader metadata update denied");
		const record: MadmaxDetachedActiveRecord = {
			version: 1,
			context_key: contextKey ?? payload.sessionId!,
			created_at: new Date().toISOString(),
			source_cwd: payload.cwd,
			argv: [payload.codexCmd],
			run_dir: process.env.OMX_ROOT ?? payload.cwd,
			tmux_session_name: payload.sessionName,
			session_id: payload.sessionId,
			tmux_pane_id: pane,
			launch_nonce: nonce,
			leader_pid: process.pid,
			base_state_root: binding.context.baseStateDir,
		...
		if (payload.readyPath) {
			...
			while (true) {
				...
				if (
					...
					release.nonce === nonce &&
					release.sessionId === payload.sessionId &&
					release.sessionName === payload.sessionName &&
					release.leaderPid === process.pid
				) {
				...
				if (
					abort?.kind === "abort" &&
					abort.nonce === nonce &&
					abort.sessionId === payload.sessionId &&
					abort.sessionName === payload.sessionName &&
					abort.leaderPid === process.pid
				) {
					throw new Error("detached launch aborted before release");
				}
				if (Date.now() >= releaseDeadline)
					throw new Error("detached leader release timed out");
				blockMs(20);
			}
		...
		const child = spawn(
			...
			process.platform === "win32"
				? ["-NoProfile", "-Command", payload.codexCmd]
				: ["-c", payload.codexCmd],
			{
				cwd: payload.cwd,
				stdio: "inherit",
		...
		const forward = (signal: NodeJS.Signals): void => {
			...
			if (!escalation) {
				escalation = setTimeout(() => signalChildTree("SIGKILL", true), 5_000);
				escalation.unref();
			}
		};
		...
		process.exitCode = externalInterrupt
			...
			: (outcome.code ??
				(outcome.signal ? (signalExitCodes[outcome.signal] ?? 1) : 1));
		await postLaunch(
			payload.cwd,
			payload.sessionId,
			binding,
		...
		await cleanupRuntimeCodexHome(
			payload.runtimeCodexHomeForCleanup,
			payload.projectLocalCodexHomeForCleanup,
		);
		if (ownedRecord) {
			const bytes = readFileSync(activeRecordPath, "utf-8");
			const digest = createHash("sha256").update(bytes).digest("hex");
			if (bytes === ownedRecord.bytes && digest === ownedRecord.digest)
		...
		try {
			...
			failure = new AggregateError(
				...
				"detached leader failure finalization did not complete",
			);
		}
...
export async function postLaunch(
	...
	try {
		...
		if (!finalization.finalized)
			terminalError = new Error(
				`bound session finalization denied: ${finalization.cleanup.comparison?.reason ?? "unknown reason"}`,
			);
		const closeFailure = finalization.cleanup.capability.find(
			(evidence) => evidence.status === "failed",
		);
		if (closeFailure)
			terminalError = new Error(
				`bound session retained close failed: ${closeFailure.error?.message ?? "unknown error"}`,
			);
	} catch (error) {
		terminalError = error;
	}
	...
	if (closeEvidence.status === "failed") {
		throw new Error(
			`bound session retained close failed: ${closeEvidence.error?.message ?? "unknown error"}`,
		);
	}
}
...
export async function runDetachedSessionPostLaunch(
	...
	await strict(() =>
		emitNativeHookEvent(cwd, "session-end", {
			session_id: sessionId,
	...
	if (releaseMarkerPath)
		await strict(() => rm(releaseMarkerPath, { force: true }));
	if (failures.length > 0)
		throw new AggregateError(
			failures,
...
async function listHookVisibleRunDirStateRefs(
	cwd: string,
	...
	for (const runDir of runDirs) {
		...
		const candidateDirs = sessionId
			? [join(stateDir, "sessions", sessionId), stateDir]
			: [stateDir];
...
function isCanonicalPathWithin(
	parent: string,
	child: string,
	allowEqual = false,
): boolean {
...
async function selectAuthorizedHookVisibleRunDirState(
	cwd: string,
): Promise<AuthorizedRunStateSelection | null> {
	const runsRoot = resolveMadmaxRunsRoot(process.env);
	const activeDir = join(runsRoot, MADMAX_DETACHED_ACTIVE_DIR);
	const canonicalCwd = canonicalizePathForRunDirMatch(cwd);
	...
	for (const file of files) {
		...
		if (
			!record.session_id ||
			normalizeSessionId(record.session_id) !== record.session_id
		)
			continue;
		...
		if (
			...
			(!record.worktree_cwd ||
				canonicalizePathForRunDirMatch(record.worktree_cwd) !== canonicalCwd)
		)
			continue;
		...
		try {
			...
			if (session.session_id !== record.session_id)
				throw new Error("run session pointer changed");
			const sessionDir = realpathSync(
				join(stateDir, "sessions", record.session_id),
			);
			...
			throw new Error(
				`Refusing cancellation because detached run authority is invalid: ${record.run_dir}.`,
				{ cause: err },
			);
		}
	}
	...
	for (const file of stateFiles) {
		...
		try {
			...
			if (!isCanonicalPathWithin(sessionDir, canonicalFile)) {
				throw new Error(
					`Refusing cancellation outside authorized run session: ${path}.`,
				);
			}
			refs.push({
				mode: file.slice(0, -"-state.json".length),
			...
			throw new Error(
				`Refusing cancellation because detached run state authority is invalid: ${path}.`,
				{ cause: err },
			);
		}
	}
...
function assertCancellationAuthorityPath(
	...
	if (
		...
		realpathSync(resolvedBase) !== resolvedBase
	) {
		throw new Error(
			`Refusing cancellation through symlinked state authority root: ${baseStateDir}.`,
		);
	}
	if (!isCanonicalPathWithin(resolvedBase, resolvedAuthority, true)) {
		throw new Error(
			`Refusing cancellation outside base state authority: ${authorityRoot}.`,
		);
	}
	...
	for (const component of rel ? rel.split(/[\\/]+/) : []) {
		...
		if (
			...
			throw new Error(
				`Refusing cancellation through symlinked authority component: ${current}.`,
			);
		}
	}
...
function collectCancellationOwnerIds(
	baseStateDir: string,
	sessionId?: string,
): Set<string> {
	...
	try {
		const pointer = JSON.parse(
			readFileSync(join(baseStateDir, "session.json"), "utf-8"),
		) as Record<string, unknown>;
		for (const field of [
			"session_id",
			"native_session_id",
...
async function resolveExactSessionCancellationAuthority(
	...
	try {
		const rawPointer = JSON.parse(
			await readFile(join(baseStateDir, "session.json"), "utf-8"),
		) as Record<string, unknown>;
		if (typeof rawPointer.cwd === "string" && rawPointer.cwd.trim())
			authorityCwd = rawPointer.cwd.trim();
		pointer = await readSessionPointer(
			resolveSessionPointerContext(authorityCwd),
		);
	} catch {
		return undefined;
	}
	...
	if (
		...
		pointer.state?.session_id !== sessionId ||
		!nativeSessionId
	)
		return undefined;
	try {
		const ownerEvidence = await readNativeSessionOwnerEvidence(
			authorityCwd,
			nativeSessionId,
		);
...
function assertExactSkillOwner(
	...
	if (Object.prototype.hasOwnProperty.call(value, "owner_codex_session_id")) {
		...
		if (
			!owner ||
			(!allowStaleTopCodexOwner && owner !== authority.nativeSessionId)
		) {
			throw new Error(
				`Refusing contradictory owner_codex_session_id in ${path}.`,
			);
		}
	}
}
...
function assertCompatibleNestedSkillOwners(
	...
	for (const [index, skill] of value.active_skills.entries()) {
		...
		if (
			Object.prototype.hasOwnProperty.call(entry, "phase") &&
...
async function cancelModes(
	...
	try {
		...
		const exactAuthority =
			writableScope.source === "session"
				? await resolveExactSessionCancellationAuthority(
						cwd,
						baseStateDir,
						writableScope.sessionId,
					)
				: undefined;
		const expectedOwnerIds = collectCancellationOwnerIds(
			baseStateDir,
			writableScope.sessionId,
		);
		...
		const loadStates = async (
			...
			for (const ref of refs) {
				...
				if (
					...
					throw new Error(
						`Refusing cancellation outside authorized state root: ${ref.path}.`,
					);
				}
				...
				try {
					...
					logCliOperationFailure(err);
					throw new Error(
						`Refusing partial cancellation because ${ref.path} is malformed.`,
						{ cause: err },
					);
				}
				if (
					typeof parsedState.mode === "string" &&
					parsedState.mode !== ref.mode
				) {
					throw new Error(`Refusing contradictory mode state in ${ref.path}.`);
				}
				if (ref.mode === SKILL_ACTIVE_STATE_MODE && exactAuthority) {
					const hasTopCodexOwner = Object.prototype.hasOwnProperty.call(
						parsedState,
						"owner_codex_session_id",
					);
					...
					if (hasTopCodexOwner && !displacedOwnerId) {
						throw new Error(
							`Refusing contradictory owner_codex_session_id in ${ref.path}.`,
						);
					}
					assertExactSkillOwner(
						parsedState,
						exactAuthority,
						ref.path,
						hasTopCodexOwner,
					);
					assertCompatibleNestedSkillOwners(
						parsedState,
						exactAuthority,
						ref.path,
					);
					if (
						displacedOwnerId &&
						displacedOwnerId !== exactAuthority.nativeSessionId
					) {
						let displacedEvidence: Awaited<
							ReturnType<typeof readNativeSessionOwnerEvidence>
						>;
						try {
							displacedEvidence = await readNativeSessionOwnerEvidence(
								exactAuthority.authorityCwd,
								displacedOwnerId,
							);
						} catch (error) {
							throw new Error(
								`Refusing ambiguous displaced owner evidence for ${ref.path}.`,
								{ cause: error },
							);
						}
						if (
							displacedEvidence.status !== "absent" &&
							displacedEvidence.status !== "stale-dead"
						) {
							throw new Error(
								`Refusing non-stale displaced owner evidence for ${ref.path}.`,
							);
						}
					...
					if (
						Object.prototype.hasOwnProperty.call(parsedState, "active_skills")
					) {
						if (!Array.isArray(parsedState.active_skills)) {
							throw new Error(
								`Refusing malformed active_skills in ${ref.path}.`,
							);
						}
						for (const [index, skill] of parsedState.active_skills.entries()) {
							if (!skill || typeof skill !== "object" || Array.isArray(skill)) {
								throw new Error(
									`Refusing malformed active_skills entry ${index} in ${ref.path}.`,
								);
							}
							assertCancellationOwnerMetadata(
								skill as Record<string, unknown>,
								ownerIds,
								`${ref.path} active_skills[${index}]`,
							);
						}
					}
				}
		...
		const preferredRefs: ModeStateFileRef[] =
			writableScope.source === "root"
				? (await readdir(writableScope.stateDir).catch(() => [] as string[]))
						.filter(
							(file) => file.endsWith("-state.json") && file !== "session.json",
						)
						.map((file) => ({
							mode: file.slice(0, -"-state.json".length),
				...
				: await listModeStateFilesWithScopePreference(
						cwd,
						writableScope.sessionId,
					);
		const hasPreferredSessionStateFiles = preferredRefs.some(
			(ref) => ref.scope === "session",
		);
		const targetRefs =
			writableScope.source === "session"
		...
		const hasActiveWorkflowMode = (entries: typeof states): boolean =>
			[...entries.entries()].some(
				([mode, entry]) =>
					mode !== SKILL_ACTIVE_STATE_MODE && entry.state.active === true,
			);
		if (
			writableScope.source === "root" &&
		...
		const currentSessionId =
			writableScope.sessionId ?? runSelection?.sessionId ?? "";
		...
		const linkedRalphMode = (
			state: Record<string, unknown>,
		): "ultrawork" | "ecomode" | undefined => {
			if (state.linked_ultrawork === true || state.linked_mode === "ultrawork")
				return "ultrawork";
			if (state.linked_ecomode === true || state.linked_mode === "ecomode")
				return "ecomode";
			return undefined;
		};

		const cancelRalphSkillEntries = (
			linkedMode: "ultrawork" | "ecomode" | undefined,
		): void => {
			...
			const nextSkills = activeSkills.map((skill) => {
				...
				if (
					skillEntry.active === false ||
					typeof skillEntry.skill !== "string" ||
					!matchingSkills.has(skillEntry.skill)
				) {
					return skill;
				}
			...
			const remainingActiveSkills = nextSkills.filter(
			...
			if (!changedSkillEntries) {
				...
				if (
					typeof entry.state.skill !== "string" ||
					!matchingSkills.has(entry.state.skill)
				)
					return;
			}
		...
		const assertRunAuthority = (): void => {
			if (
				runSelection &&
				!hasMatchingMadmaxDetachedRuntimeBinding(runSelection.record)
			) {
				throw new Error(
					"Refusing cancellation because detached run authority changed.",
				);
			}
		};

		if (force && currentSessionId) {
			const stopStateEntries = [...states.entries()].filter(
				([mode]) => mode === "native-stop",
			);
			for (const [, entry] of stopStateEntries) {
				const sessions =
					entry.state.sessions &&
					typeof entry.state.sessions === "object" &&
					!Array.isArray(entry.state.sessions)
						? { ...(entry.state.sessions as Record<string, unknown>) }
						: null;
				if (
					!sessions ||
					!Object.prototype.hasOwnProperty.call(sessions, currentSessionId)
				)
					continue;
		...
		const orderedChanges = [...changed]
			...
			.map((mode) => {
				const entry = states.get(mode);
				if (!entry)
					throw new Error(`Missing frozen cancellation entry for ${mode}.`);
				return {
					mode,
					entry,
					nextContent: JSON.stringify(entry.state, null, 2),
				};
			});
		const opened: Array<{
			mode: string;
			entry: typeof states extends Map<string, infer T> ? T : never;
			nextContent: string;
		...
		try {
			for (const change of orderedChanges) {
				...
				if (
					!currentStat.isFile() ||
					currentStat.dev !== change.entry.dev ||
					currentStat.ino !== change.entry.ino
				) {
					await handle.close();
					throw new Error(
						`Refusing cancellation because state identity changed: ${change.entry.path}.`,
					);
				}
			...
			const cancellationTestWriteFailureMode =
				options.testFaults?.writeFailureMode;
			const cancellationTestRollbackFailureMode =
				options.testFaults?.rollbackFailureMode;
			try {
				for (const openedEntry of opened) {
					...
					if (cancellationTestWriteFailureMode === openedEntry.mode) {
						throw new Error(
							`Injected cancellation write failure for ${openedEntry.mode}.`,
						);
					}
				...
				for (const committedEntry of committed.reverse()) {
					try {
						if (cancellationTestRollbackFailureMode === committedEntry.mode) {
							throw new Error(
								`Injected cancellation rollback failure for ${committedEntry.mode}.`,
							);
						}
						await committedEntry.handle.truncate(0);
						await committedEntry.handle.write(
							committedEntry.entry.originalContent,
							0,
							"utf-8",
						);
						...
						if (rollbackFailureSample.length < 3) {
							rollbackFailureSample.push(
								committedEntry.mode.replace(/[^A-Za-z0-9_-]/g, "_"),
							);
						}
					}
				}
				if (rollbackFailureCount > 0) {
					const primaryMessage =
						writeError instanceof Error
							...
							: String(writeError);
					const sample =
						rollbackFailureSample.length > 0
							? `:${rollbackFailureSample.join(",")}`
							: "";
					throw new Error(
						`${primaryMessage} (cancellation_rollback_failed:${rollbackFailureCount}${sample}).`,
						{ cause: writeError },
					);
				}
				throw writeError;
