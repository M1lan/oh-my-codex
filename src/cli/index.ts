...
function tmuxPaneBelongsToSession(
	paneId: string,
	sessionName: string,
): boolean {
	try {
...
function isReusableMadmaxDetachedActiveRecord(
	...
	if (
		readTmuxSessionInstanceId(record.tmux_session_name) !== record.session_id
	) {
		return false;
	}
	return tmuxPaneBelongsToSession(
		record.tmux_pane_id,
	...
	if (launchPolicy === "inside-tmux") {
		...
		const launchDetachedSession = (): {
			...
			if (
				...
				try {
					execTmuxFileSync(
						["attach-session", "-t", activeRecord.tmux_session_name],
						{
							stdio: "inherit",
						},
					);
		...
		try {
			if (isMadmaxDetachedGuardEnabled(process.env) && contextKey) {
				return withMadmaxDetachedContextLock(
					runsRoot,
					contextKey,
			...
			if (
				err instanceof MadmaxDetachedReuseError ||
...
async function cancelModes(args: string[] = []): Promise<void> {
	...
	try {
		...
		let states = await loadStates(
			await listModeStateFilesWithScopePreference(cwd),
		);
		...
		if (!hasActiveWorkflowMode(states)) {
			const runDirStates = await loadStates(
				await listHookVisibleRunDirStateRefs(cwd),
			);
