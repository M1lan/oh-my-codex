...
export function resolveAutoUpdateMode(
	value = process.env.OMX_AUTO_UPDATE,
): AutoUpdateMode {
...
function spawnGlobalPnpmAddSync(
	installSource: string,
	options: SpawnSyncOptions,
	spawnProcess: SpawnSyncLike = spawnSync,
	platform: NodeJS.Platform = process.platform,
...
function commandFailure(
	...
	return {
		...
		stderr:
			details ||
			`${label} exited ${typeof status === "number" ? status : "without a status"}`,
	};
}
...
function runDevGlobalUpdate(
	...
	try {
		...
		if (cloneResult.error)
			return { ok: false, stderr: cloneResult.error.message };
		...
		const revisionResult = spawnProcess("git", ["rev-parse", "HEAD"], {
			cwd: checkoutDir,
			encoding: "utf-8",
			stdio: ["ignore", "pipe", "pipe"],
			timeout: 15000,
		...
		if (installResult.error)
			return { ok: false, stderr: installResult.error.message };
		...
		if (prepackResult.error)
			return { ok: false, stderr: prepackResult.error.message };
		if (prepackResult.status !== 0) {
			return commandFailure(
				prepackResult.stderr,
				prepackResult.status,
				"pnpm run prepack",
			);
		}
		...
		if (packResult.error)
			return { ok: false, stderr: packResult.error.message };
		...
		try {
			...
			if (typeof filename === "string" && filename !== "") {
				tarballPath = isAbsolute(filename)
					? filename
					: join(checkoutDir, filename);
			}
		} catch {
			tarballPath = null;
		}
		if (!tarballPath || !existsSync(tarballPath)) {
			return {
				ok: false,
				stderr: "pnpm pack did not produce an installable tarball.",
			};
		}
		...
		if (globalInstallResult.status !== 0) {
			return commandFailure(
				...
				globalInstallResult.status,
				"pnpm add -g dev tarball",
			);
		}
...
export function runGlobalUpdate(
	...
	const installSource = legacySpawnFirst
		? STABLE_INSTALL_SOURCE
	...
	const result = spawnGlobalPnpmAddSync(
		installSource,
		{
			encoding: "utf-8",
			stdio: ["ignore", "pipe", "pipe"],
	...
	if (result.status !== 0) {
		return {
			ok: false,
			stderr:
				String(result.stderr || "").trim() || `pnpm exited ${result.status}`,
		};
	}
...
export function resolveSetupRefreshArgs(cwd: string): string[] {
	...
	const mergeAgents = resolvePersistedSetupMergeAgents(
		preferences,
...
async function writeSuccessfulInstallStamp(
	...
	await writeUserInstallStamp({
		...
		...(metadata.devBaseVersion
			? { dev_base_version: stripLeadingV(metadata.devBaseVersion) }
			: {}),
...
export function isInstallVersionBump(
	...
	return (
		stripLeadingV(currentVersion) !== stripLeadingV(stamp.installed_version)
	);
}
...
function resolveUpdateCheckBaseline(
	...
	const stampVersion = stripLeadingV(
...
export async function resolveInstalledCliEntry(
...
export function spawnInstalledSetupRefresh(
	...
	const result = spawnProcess(
		process.execPath,
		[cliEntry, ...resolveSetupRefreshArgs(cwd)],
		{
			cwd,
			env: process.env,
			stdio: "inherit",
...
async function executeUpdate(options: {
	...
	const [current, latest] = await Promise.all([
		...
		channel === "stable" || !forceInstall || channel === "dev"
	...
	if (!forceInstall && (!updateCheckBaseline || !latest)) {
		if (immediate) {
			console.log(
				"[omx] Unable to determine the latest oh-my-codex version. Try again later.",
			);
		}
		return {
			status: "unavailable",
	...
	if (
		...
		updateCheckBaseline &&
		latest &&
		!isNewerVersion(updateCheckBaseline, latest)
	) {
		if (immediate) {
			if (current && !doesSetupStampMatchVersion(current, installStamp)) {
				...
				console.log(
					`[omx] Setup refresh completed for v${updateCheckBaseline}. Restart to use current code.`,
				);
				return {
					status: "up-to-date",
					currentVersion: current,
		...
		if (immediate) {
			console.log(
				`[omx] oh-my-codex is already up to date (v${updateCheckBaseline}).`,
			);
		}
		return {
			status: "up-to-date",
	...
	if (!immediate) {
		...
		if (!deferredResult.ok) {
			console.log(
				formatDeferredUpdateFailure(
					deferredResult.stderr,
					deferredResult.logPath,
				),
			);
			return {
				status: "failed",
	...
	if (channelConfig.channel === "dev") {
		console.log(
			"[omx] Running: clone dev branch, run prepack, then pnpm add -g the packed tarball",
		);
	} else {
	...
	if (!result.ok) {
		console.log(
			summarizeUpdateFailure(result.stderr, channelConfig.installSource),
		);
	...
	const versionSummary =
		channelConfig.channel === "stable" && latest ? ` to v${latest}` : "";
	console.log(
		`[omx] Updated ${channelConfig.channel} channel${versionSummary}. Restart to use new code.`,
	);
	if (channelConfig.channel === "dev") {
		console.log(
			"[omx] Dev display version may differ from the package/plugin manifest version; start a new Codex session if /skills still shows stale OMX plugin skill metadata.",
		);
	}
