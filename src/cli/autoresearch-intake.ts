...
export const AUTORESEARCH_DEEP_INTERVIEW_RESULT_KIND =
	"omx.autoresearch.deep-interview/v1";
...
async function readPersistedResult(
	...
	const parsedCompileTarget =
		parsed.compileTarget as AutoresearchDraftCompileTarget;
	const inferredRepoRoot = inferRepoRootFromResultPath(resultPath);
	const repoRoot = isUsableAbsolutePath(parsedCompileTarget.repoRoot)
		? parsedCompileTarget.repoRoot
		: inferredRepoRoot;
	const compileTarget: AutoresearchDraftCompileTarget = {
		...parsedCompileTarget,
		repoRoot,
...
export async function listAutoresearchDeepInterviewDraftPaths(
	repoRoot: string,
): Promise<string[]> {
