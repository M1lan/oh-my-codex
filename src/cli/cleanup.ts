...
const OMX_MCP_ENTRYPOINT_PATTERN =
...
const CODEX_PROCESS_PATTERN =
	/(?:^|[\\/\s])codex(?:\.js)?(?:\s|$)|@openai[\\/]codex/i;
const OMX_LAUNCH_PROCESS_PATTERN =
	/(?:^|[\\/\s])omx(?:\.js)?(?:\s|$)|(?:^|[\\/])(?:bin|dist[\\/]cli)[\\/]omx\.js(?:\s|$)|oh-my-codex[\\/]dist[\\/]cli[\\/]omx\.js/i;
...
const defaultProcessListCommandRunner: ProcessListCommandRunner = (
	file,
	args,
	options,
...
export function extractOmxMcpEntrypoint(command: string): string | null {
	...
	const directEntrypoint = normalized
		.match(OMX_MCP_ENTRYPOINT_PATTERN)?.[1]
		?.toLowerCase();
...
function isBusyBoxPsCommandFieldError(error: unknown): boolean {
	...
	return /bad -o argument ['"]command['"]|unsupported arguments:.*\bargs\b/i.test(
		error.message,
	);
}
...
export function findCleanupCandidates(
	...
	const duplicateCandidates = findDuplicateSiblingCleanupCandidates(
		processes,
	).filter((processEntry) => processEntry.pid !== currentPid);
	const duplicateCandidatePids = new Set(
		duplicateCandidates.map((candidate) => candidate.pid),
	);
	...
	return [...duplicateCandidates, ...orphanCandidates].sort(
...
export async function cleanupOmxMcpProcesses(
	...
	const selectCandidates =
		dependencies.selectCandidates ?? findCleanupCandidates;
	...
	if (dryRun) {
		writeLine(
			`Dry run: would terminate ${candidates.length} orphaned OMX MCP server process(es):`,
		);
	...
	writeLine(
		`Found ${candidates.length} orphaned OMX MCP server process(es). Sending SIGTERM...`,
	);
	for (const candidate of candidates) {
		try {
	...
	writeLine(
		`Killed ${terminatedCount} orphaned OMX MCP server process(es)${forceKilledCount > 0 ? ` (${forceKilledCount} required SIGKILL)` : ""}.`,
	);
	if (failedPids.length > 0) {
		writeLine(
			`Warning: ${failedPids.length} process(es) still appear alive: ${failedPids.join(", ")}`,
		);
	}
...
export async function cleanupStaleTmpDirectories(
	...
	const listTmpEntries =
		dependencies.listTmpEntries ??
		((root: string) => readdir(root, { withFileTypes: true }));
...
export async function cleanupCommand(
	...
	const cleanupProcesses =
		dependencies.cleanupProcesses ?? cleanupOmxMcpProcesses;
	const cleanupTmpDirectories =
		dependencies.cleanupTmpDirectories ?? cleanupStaleTmpDirectories;
