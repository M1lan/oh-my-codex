...
} from "../hooks/extensibility/loader.js";
...
async function validateHookPluginExport(
	filePath: string,
): Promise<HookPluginValidationResult> {
	try {
		...
		if (typeof mod.onHookEvent !== "function") {
			return {
				valid: false,
				reason: "missing export `onHookEvent(event, sdk)`",
			};
		}
		return { valid: true };
	} catch (error) {
		return {
			valid: false,
			reason:
				error instanceof Error ? error.message : "failed to import plugin",
		};
	}
}
...
async function testHooks(): Promise<void> {
	...
	for (const pluginResult of result.results) {
		...
		const error =
			typeof pluginResult.error === "string" ? pluginResult.error : "";
		console.log(
			error ? `${label}: ${status} (${error})` : `${label}: ${status}`,
		);
	}
	...
	const logPath = join(
		...
		".omx",
		"logs",
		`hooks-${new Date().toISOString().split("T")[0]}.jsonl`,
	);
