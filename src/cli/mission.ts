...
function readValue(
	...
	if (!value || value.startsWith("--"))
		throw new MissionCommandError(`Missing value for ${flag}.`);
	return value;
}
...
function parseMissionArgs(args: string[]): ParsedMissionArgs {
	...
	let markStatus:
		| Extract<MissionTaskStatus, "blocked" | "needs-human-review">
		| undefined;
	...
	for (let index = 0; index < commandArgs.length; index += 1) {
		...
		if (arg.startsWith("--status=")) {
			...
			if (value !== "blocked" && value !== "needs-human-review")
				throw new MissionCommandError(
		...
		if (arg.startsWith("--"))
			throw new MissionCommandError(`Unknown mission option: ${arg}`);
		if (!file) {
			file = arg;
			continue;
		}
	...
	if (action !== "rerun" && action !== "mark" && taskId)
		throw new MissionCommandError(
			`--task is only supported for mission mark/rerun.`,
		);
	if (action === "rerun" && !taskId)
		throw new MissionCommandError(`mission rerun requires --task <id>.`);
	if (action !== "mark" && markStatus)
		throw new MissionCommandError(
			`--status is only supported for mission mark.`,
		);
	if (action === "mark" && (!taskId || !markStatus))
		throw new MissionCommandError(
...
function missionCounts(tasks: MissionTask[]): MissionSummary["counts"] {
	return {
		...
		"needs-human-review": tasks.filter(
			(task) => task.status === "needs-human-review",
		).length,
	};
}
...
function missionStatus(tasks: MissionTask[]): MissionSummary["status"] {
	...
	if (tasks.some((task) => task.status === "needs-human-review"))
		return "needs-human-review";
	if (tasks.some((task) => task.status === "failed")) return "failed";
	if (tasks.length > 0 && tasks.every((task) => task.status === "passed"))
		return "passed";
	return "planned";
}
...
function resolveMissionPaths(
	...
	const looksLikePath =
		...
		parsed.file.includes("\\") ||
		parsed.file.endsWith(".md") ||
		parsed.file.endsWith(".txt");
	const inputPath = isAbsolute(parsed.file)
		? parsed.file
		: resolve(cwd, parsed.file);
	const baseSlug =
		parsed.slug ??
		((parsed.action === "status" || parsed.action === "mark") && !looksLikePath
			? parsed.file
			: slugify(basename(inputPath, extname(inputPath))));
	const slug = slugify(baseSlug);
	const missionRoot = join(omxRoot(cwd), "missions", slug);
	const summaryPath = parsed.summaryPath
		? isAbsolute(parsed.summaryPath)
			? parsed.summaryPath
			: resolve(cwd, parsed.summaryPath)
...
function syncSummary(
	...
	updates: Partial<
		Pick<
			MissionSummary,
			"status" | "dry_run" | "continue_on_error" | "codex_args"
		>
	>,
): void {
...
async function runSelectedTasks(
	...
	for (const task of summary.tasks) {
		...
		stdout(
			`[${task.status}] ${task.id}/${summary.tasks.length}: exit ${exitCode}`,
		);
	}
}
...
function printStatus(
	...
	if (json) {
		stdout(
			JSON.stringify(
				{ ok: summary.status === "passed", summary_path: summaryPath, summary },
				null,
				2,
			),
		);
	...
	stdout(
		`tasks: ${summary.counts.passed}/${summary.counts.total} passed, ${summary.counts.failed} failed, ${summary.counts.skipped} skipped, ${summary.counts.blocked} blocked, ${summary.counts["needs-human-review"]} needs-human-review, ${summary.counts.planned} planned`,
	);
	for (const task of summary.tasks)
		stdout(
			`[${task.status}] ${task.id} line ${task.source_line}: ${task.prompt}`,
		);
	...
	try {
		...
		if (parsed.action === "mark") {
			...
			if (parsed.json)
				...
				stdout(
					`mission task marked ${task.status}: ${summary.slug} ${task.id}`,
				);
		...
		if (parsed.action === "resume" || parsed.action === "rerun") {
			...
			if (
				parsed.action === "rerun" &&
				!summary.tasks.some((task) => task.id === parsed.taskId)
			) {
				throw new MissionCommandError(
					`No mission task found for --task ${parsed.taskId}.`,
				);
		...
		if (parsed.dryRun) {
			...
			if (parsed.json)
				...
				for (const task of summary.tasks)
					stdout(
						`[planned] ${task.id} line ${task.source_line}: ${task.prompt}`,
					);
				stdout(`summary: ${paths.summaryPath}`);
				stdout(`ledger: ${paths.ledgerPath}`);
			}
