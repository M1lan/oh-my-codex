...
export async function discoverProjectRuntimeCodexHomes(
	cwd: string,
...
async function discoverLocalProjectRuntimeCodexHomes(
	cwd: string,
): Promise<ProjectRuntimeCodexHome[]> {
...
async function discoverAssociatedMadmaxRunDirs(
	...
	const addMetadata = (raw: unknown): void => {
		...
		if (!metadata) return;
		if (canonicalizeComparableProjectPath(metadata.sourceCwd) !== canonicalCwd)
			return;
...
function parseMadmaxRunMetadata(
	raw: unknown,
): { sourceCwd: string; runDir: string } | null {
...
async function countImmediateSessionEntries(
	sessionsDir: string,
): Promise<number> {
