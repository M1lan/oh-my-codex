...
import {
	performanceGoalCommand,
	PERFORMANCE_GOAL_HELP,
} from "../performance-goal.js";
...
describe("cli/performance-goal", () => {
	it("prints help with evaluator gate and goal-mode constraints", () => {
		assert.match(
			PERFORMANCE_GOAL_HELP,
			/evaluator-gated performance optimization/i,
		);
		...
		assert.match(
			HELP,
			/omx performance-goal[\s\S]*evaluator-backed performance goals/,
		);
	});
		...
		await withCwd(async (cwd) => {
			const created = await capture(() =>
				performanceGoalCommand([
					"create",
					"--objective",
					"Reduce CLI startup latency by 20%",
					"--evaluator-command",
					"npm run perf:startup",
					"--evaluator-contract",
					"PASS when p95 startup latency improves by at least 20% with no regression failures.",
					"--slug",
					"startup-latency",
				]),
			);
			...
			const handoff = await capture(() =>
				performanceGoalCommand(["start", "--slug", "startup-latency"]),
			);
			...
			assert.match(
				output,
				/Do not treat this shell command as hidden Codex goal mutation/,
			);
			assert.match(output, /update_goal\(\{status: "complete"\}\)/);
			assert.ok(
				output.indexOf('update_goal({status: "complete"})') <
					output.indexOf(
						"omx performance-goal complete --slug startup-latency",
					),
			);
			...
			const state = JSON.parse(
				await readFile(
					join(cwd, ".omx/goals/performance/startup-latency/state.json"),
					"utf-8",
				),
			) as { status: string; artifactPaths: { evaluator: string } };
			assert.equal(state.status, "in_progress");
			assert.equal(
				state.artifactPaths.evaluator,
				".omx/goals/performance/startup-latency/evaluator.md",
			);
		});
	});
		...
		await withCwd(async () => {
			await capture(() =>
				performanceGoalCommand([
					"create",
					"--objective",
					"Improve hot path throughput",
					"--evaluator-command",
					"node bench.js",
					"--evaluator-contract",
					"PASS when throughput is above baseline and tests pass.",
					"--slug",
					"throughput",
				]),
			);

			const premature = await capture(() =>
				performanceGoalCommand(["complete", "--slug", "throughput"]),
			);
			assert.equal(premature.exitCode, 1);
			assert.match(
				premature.stderr.join("\n"),
			...
			const completed = await capture(() =>
				performanceGoalCommand([
					...
					"throughput",
					"--codex-goal-json",
					'{"goal":{"objective":"Improve hot path throughput","status":"complete"}}',
					"--json",
				]),
			);
		...
		await withCwd(async () => {
			await capture(() =>
				performanceGoalCommand([
					...
					"node bench.js",
					"--evaluator-contract",
					"PASS when p99 latency improves.",
					"--slug",
					"tail-latency",
				]),
			);
			...
			const completed = await capture(() =>
				performanceGoalCommand([
					...
					"tail-latency",
					"--codex-goal-json",
					'{"goal":{"objective":"Reduce tail latency","status":"complete"}}',
				]),
			);
		...
		await withCwd(async () => {
			await capture(() =>
				performanceGoalCommand([
					...
					"node bench.js",
					"--evaluator-contract",
					"PASS when allocations fall and tests pass.",
					"--slug",
					"allocations",
				]),
			);
			...
			const missing = await capture(() =>
				performanceGoalCommand(["complete", "--slug", "allocations"]),
			);
			...
			const incomplete = await capture(() =>
				performanceGoalCommand([
					...
					"allocations",
					"--codex-goal-json",
					'{"goal":{"objective":"Reduce allocations","status":"active"}}',
				]),
			);
