...
describe("omx imagegen continuation", () => {
	...
	it("writes pending imagegen metadata and queues a Stop-hook follow-up", async () => {
		...
		try {
			...
			const result = await prepareImagegenContinuation({
				...
				generatedImagesDir:
					"C:/Users/USER/.codex/generated_images/omx-test-imagegen",
			...
			const pending = JSON.parse(
				await readFile(result.pendingPath, "utf-8"),
			) as {
			...
			assert.equal(
				pending.generated_images_dir,
				"C:/Users/USER/.codex/generated_images/omx-test-imagegen",
			);
			assert.match(
				String(pending.resume_instruction),
				/Resume the interrupted Ralph visual\/imagegen workflow/,
			);
	...
	it("exposes the helper through the CLI", async () => {
		...
		try {
			...
			assert.equal(
				output.record?.resume_instruction,
				"Resume imagegen QA for fashion-sheet.png.",
			);
