...
describe("cli/autoresearch-goal", () => {
	it("prints help with goal-mode and deprecated-command boundaries", () => {
		...
		assert.match(
			AUTORESEARCH_GOAL_HELP,
			/does not revive deprecated omx autoresearch/i,
		);
		assert.match(AUTORESEARCH_GOAL_HELP, /get_goal\/create_goal\/update_goal/i);
		assert.match(
			AUTORESEARCH_GOAL_HELP,
			/blocked until professor-critic validation records verdict=pass/i,
		);
	});
		...
		await withCwd(async (cwd) => {
			const created = await capture(() =>
				autoresearchGoalCommand([
					...
					"Map migration risk",
					"--rubric",
					"Professor critic must verify source citations and reject unsupported claims.",
					"--critic-command",
					"node scripts/critic.js",
				]),
			);
			...
			const handoff = await capture(() =>
				autoresearchGoalCommand(["handoff", "--slug", "map-migration-risk"]),
			);
			const output = handoff.stdout.join("\n");
			assert.match(output, /Autoresearch-goal active-goal handoff/);
			assert.match(
				output,
				/This shell command does not mutate hidden Codex \/goal state/,
			);
			assert.match(output, /First call get_goal/);
			assert.match(output, /call create_goal with the payload below/);
			assert.match(
				output,
				/Do not call update_goal\(\{status: "complete"\}\) until the professor-critic verdict is pass/,
			);
			assert.match(output, /do not revive deprecated omx autoresearch/i);

			const mission = JSON.parse(
				await readFile(
					join(cwd, ".omx/goals/autoresearch/map-migration-risk/mission.json"),
					"utf-8",
				),
			) as { status: string; critic_command: string };
		...
		await withCwd(async (cwd) => {
			await capture(() =>
				autoresearchGoalCommand([
					"create",
					"--topic",
					"Research flaky tests",
					"--rubric",
					"Professor critic requires reproduction evidence and a cited root cause.",
				]),
			);

			const initiallyBlocked = await capture(() =>
				autoresearchGoalCommand(["complete", "--slug", "research-flaky-tests"]),
			);
			assert.equal(initiallyBlocked.exitCode, 1);
			assert.match(
				initiallyBlocked.stderr.join("\n"),
			...
			await capture(() =>
				autoresearchGoalCommand([
					...
					"fail",
					"--evidence",
					"critic rejected missing reproduction logs",
				]),
			);
			const stillBlocked = await capture(() =>
				autoresearchGoalCommand(["complete", "--slug", "research-flaky-tests"]),
			);
			...
			await capture(() =>
				autoresearchGoalCommand([
					...
					"pass",
					"--evidence",
					"critic approved report.md with reproduction logs and citations",
					"--artifact",
					".omx/specs/autoresearch-flaky-tests/report.md",
				]),
			);
			...
			const completed = await capture(() =>
				autoresearchGoalCommand([
					...
					"--codex-goal-json",
					JSON.stringify({
						goal: { objective: expectedObjective, status: "complete" },
					}),
				]),
			);
			...
			const completion = JSON.parse(
				await readFile(
					join(
						cwd,
						".omx/goals/autoresearch/research-flaky-tests/completion.json",
					),
					"utf-8",
				),
			...
			const mission = JSON.parse(
				await readFile(
					join(
						cwd,
						".omx/goals/autoresearch/research-flaky-tests/mission.json",
					),
					"utf-8",
				),
			...
			const status = await capture(() =>
				autoresearchGoalCommand(["status", "--slug", "research-flaky-tests"]),
			);
			assert.match(
				status.stdout.join("\n"),
			...
			const afterComplete = await capture(() =>
				autoresearchGoalCommand([
					"verdict",
					"--slug",
					"research-flaky-tests",
		...
		await withCwd(async () => {
			...
			await capture(() =>
				autoresearchGoalCommand([
					...
					"pass",
					"--evidence",
					"critic approved legacy in-flight mission",
				]),
			);
			...
			const completed = await capture(() =>
				autoresearchGoalCommand([
					...
					"--codex-goal-json",
					JSON.stringify({
						goal: { objective: legacyObjective, status: "complete" },
					}),
				]),
			);
		...
		await withCwd(async (cwd) => {
			await capture(() =>
				autoresearchGoalCommand([
					"create",
					"--topic",
					"Research goal snapshots",
					"--rubric",
					"Professor critic requires concrete citations.",
				]),
			);
			await capture(() =>
				autoresearchGoalCommand([
			...
			const mismatched = await capture(() =>
				autoresearchGoalCommand([
					...
					"research-goal-snapshots",
					"--codex-goal-json",
					'{"goal":{"objective":"Autoresearch goal: Different completed mission","status":"complete"}}',
				]),
			);
			...
			const completion = JSON.parse(
				await readFile(
					join(
						cwd,
						".omx/goals/autoresearch/research-goal-snapshots/completion.json",
					),
					"utf-8",
				),
			) as {
			...
			const mission = JSON.parse(
				await readFile(
					join(
						cwd,
						".omx/goals/autoresearch/research-goal-snapshots/mission.json",
					),
					"utf-8",
				),
			) as { status: string };
