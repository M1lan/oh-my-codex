...
describe("resolveSparkShellBinaryPath", () => {
	...
	it("falls back from packaged binary to repo-local build artifact", () => {
		const packaged = join(
			...
			process.platform === "win32" ? "omx-sparkshell.exe" : "omx-sparkshell",
		);
	...
	it("falls back to nested repo-local native build artifact when present", () => {
		const nestedRepoLocal =
			nestedRepoLocalSparkShellBinaryPath(fixturePackageRoot);
		...
		try {
			...
			await assert.rejects(
				...
				(error: Error) =>
					...
					error.message.includes("legacy-unverified"),
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
				...
				process.platform === "win32"
					? spawnSync(
							...
							[
								...
								"-NoProfile",
								"-Command",
								`Compress-Archive -Path '${stagedBinary.replace(/'/g, "''")}' -DestinationPath '${archivePath.replace(/'/g, "''")}' -Force`,
							],
							{ encoding: "utf-8" },
						)
					: spawnSync(
							"tar",
							["-czf", archivePath, "-C", stagingDir, "omx-sparkshell"],
							{ encoding: "utf-8" },
						);
			assert.equal(
				buildArchive.status,
				0,
				buildArchive.stderr || buildArchive.stdout,
			);
	...
	it("falls back cleanly when hydration manifest is unavailable", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-sparkshell-hydration-missing-"),
		);
		try {
...
describe("runSparkShellBinary", () => {
	it("passes argv directly to the native sidecar", () => {
		let invoked:
			| { binaryPath: string; args: string[]; stdio: unknown }
			| undefined;
	...
	it("merges .omx-config.json env overrides behind explicit shell env", async () => {
		const codexHome = await mkdtemp(
			join(tmpdir(), "omx-sparkshell-config-env-"),
		);
		await writeFile(
			...
			JSON.stringify({
				env: {
					...
					OMX_SPARKSHELL_MODEL_INSTRUCTIONS_FILE:
						"/config/sparkshell-instructions.md",
				},
			}),
		);
	...
	it("detects GLIBC symbol version failures from the native loader", () => {
		assert.equal(
			isSparkShellNativeCompatibilityFailure({
				...
				stdout: "",
				stderr:
					"omx-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.39' not found\n",
				status: 1,
				signal: null,
			}),
			true,
...
describe("parseSparkShellFallbackInvocation", () => {
	...
	it("strips native-only flags before explicit shell fallback", () => {
		assert.deepEqual(
			parseSparkShellFallbackInvocation(
				["--json", "--budget=4096", "--shell", "printf ok"],
				{ platform: "linux" },
			),
	...
	it("rejects extra argv after explicit --shell fallback", () => {
		assert.throws(
			() =>
				parseSparkShellFallbackInvocation(["--shell", "printf ok", "--help"], {
					platform: "linux",
				}),
		...
		assert.throws(
			() =>
				parseSparkShellFallbackInvocation(["--shell=printf ok", "extra"], {
					platform: "linux",
				}),
			/--shell does not accept additional arguments/,
		);
	...
	it("translates explicit shell fallback through pwsh on Windows when available", () => {
		assert.deepEqual(
			...
			{
				kind: "command",
				argv: ["pwsh", "-NoLogo", "-NoProfile", "-Command", "Write-Output ok"],
			},
		);
	});
	...
	it("matches the shared notification capture-pane argv contract", () => {
		const parsed = parseSparkShellFallbackInvocation([
			"--tmux-pane",
			"%12",
			"--tail-lines",
			"400",
		...
		assert.deepEqual(parsed, {
			...
			argv: [
				...
				...buildNotificationCapturePaneArgv("%12", 400),
			],
		});
	});
...
describe("omx sparkshell", () => {
	it("includes sparkshell in top-level help output", async () => {
		...
		try {
			...
			assert.match(
				result.stdout,
				/omx explore\s+DEPRECATED compatibility command; use normal repo inspection or omx sparkshell/,
			);
	...
	it("prints sparkshell usage when invoked with --help", async () => {
		...
		try {
			...
			assert.match(
				result.stdout,
				/OMX_SPARKSHELL_BIN overrides the native binary/,
			);
			assert.match(
				result.stdout,
				/OMX_SPARKSHELL_MODEL_INSTRUCTIONS_FILE overrides packaged summary instructions/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("preserves child stdout, stderr, and exit code through the JS bridge", async () => {
		...
		try {
			...
			await writeFile(
				stubPath,
				'process.stdout.write("spark-stdout\\n"); process.stderr.write("spark-stderr\\n"); process.exit(7);\n',
			);
	...
	it("falls back to raw execution when the packaged native binary is GLIBC-incompatible", async () => {
		...
		try {
			...
			const stubDigest = createHash("sha256")
				.update(await readFile(stubPath))
				.digest("hex");
			await writeFile(`${stubPath}.sha256`, `${stubDigest}\n`);

			const result = runOmx(
				cwd,
				["sparkshell", "node", "-e", 'process.stdout.write("raw-fallback\\n")'],
				{
					OMX_NATIVE_CACHE_DIR: cacheDir,
				},
			);
			...
			assert.match(
				result.stderr,
				new RegExp(
					`path=${canonicalStubPath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
				),
			);
	...
	it("fails clearly when the configured native binary path does not exist", async () => {
		...
		try {
			...
			assert.match(
				result.stderr,
				/failed to launch native binary: executable not found/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
