...
describe("shouldCheckForUpdates", () => {
	...
	it("returns true when last_checked_at is invalid", () => {
		assert.equal(
			shouldCheckForUpdates(Date.now(), { last_checked_at: "not-a-date" }),
			true,
		);
	});
	...
	it("returns false when checked within interval", () => {
		...
		assert.equal(
			shouldCheckForUpdates(now, { last_checked_at: recentCheck }),
			false,
		);
	});
	...
	it("returns true when check is overdue", () => {
		...
		assert.equal(
			shouldCheckForUpdates(now, { last_checked_at: oldCheck }),
			true,
		);
	});
	...
	it("returns true when exactly at interval boundary", () => {
		...
		assert.equal(
			shouldCheckForUpdates(now, { last_checked_at: exactCheck }),
			true,
		);
	});
...
describe("maybeCheckAndPromptUpdate", () => {
				...
				await maybeCheckAndPromptUpdate(cwd, {
					...
					runDeferredGlobalUpdate: (deferredCwd) => {
						...
						return {
							ok: true,
							stderr: "",
							logPath: join(deferredCwd, ".omx", "logs", "update-test.log"),
						};
					},
		...
		try {
			await withInteractiveTty(async () => {
				await maybeCheckAndPromptUpdate(cwd, {
					...
					runSetupRefresh: async () => {
						throw new Error(
							"startup setup refresh should be handled by the deferred updater",
						);
					},
				});
			});
		...
		try {
			await withInteractiveTty(async () => {
				await maybeCheckAndPromptUpdate(cwd, {
					...
					runSetupRefresh: async () => {
						throw new Error(
							"setup refresh should not run when already up to date",
						);
					},
				});
			});
	...
	it("does not infer a dev_base_version from launch-time latest alone", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-update-dev-baseline-missing-"),
		);
		...
		try {
			...
			const unchangedStamp = JSON.parse(await readFile(stampPath, "utf-8")) as {
	...
	it("uses the artifact version when it is newer than the stamped dev baseline", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-update-dev-baseline-outrun-"),
		);
		let promptCalls = 0;
		let updateAttempts = 0;
	...
	it("respects the passive launch-time cadence before checking the registry", async () => {
		const cwd = await mkdtemp(join(tmpdir(), "omx-update-"));
			...
			const result = runGlobalUpdate(
				...
				((
					...
					if (command === "pnpm" && args[0] === "pack") {
						writeFileSync(
							join(options?.cwd ?? process.cwd(), "oh-my-codex-0.18.9.tgz"),
							"packed",
						);
			...
			assert.deepEqual(
				...
				[
					...
					[
						...
						"add",
						"-g",
						join(calls[2].cwd ?? "", "oh-my-codex-0.18.9.tgz"),
					],
				],
			);
			const dependencyInstall = calls.find(
				(call) =>
					call.command === "pnpm" &&
					call.args[0] === "install" &&
					call.args.includes("--prod=false"),
			);
			assert.equal(dependencyInstall?.env?.npm_config_global, "false");
			assert.equal(dependencyInstall?.env?.npm_config_location, "project");
			assert.equal(
				calls.some((call) =>
					call.args.includes("github:Yeachan-Heo/oh-my-codex#dev"),
				),
				false,
			);
		} finally {
...
describe("runImmediateUpdate", () => {
	...
	it("force-installs stable for explicit update even when pnpm is already current", async () => {
		const cwd = await mkdtemp(join(tmpdir(), "omx-update-now-"));
		const stampPath = join(cwd, ".codex", ".omx", "install-state.json");
		const originalCodexHome = process.env.CODEX_HOME;
		const originalLog = console.log;
	...
	it("uses stable as a rollback path while preserving persisted setup preferences", async () => {
		...
		try {
			...
			const result = await runImmediateUpdate(
				cwd,
				{
					getCurrentVersion: async () => "0.14.0",
					fetchLatestVersion: async () => "0.14.0",
	...
	it("installs the upstream dev branch without implying npm latest", async () => {
		...
		try {
			...
			assert.match(
				logs.join("\n"),
				/Running: clone dev branch, run prepack, then pnpm add -g the packed tarball/,
			);
			assert.match(
				logs.join("\n"),
...
describe("runDeferredGlobalUpdate", () => {
	it("launches a detached Windows PowerShell updater that waits for the parent and runs setup after pnpm", async () => {
		...
		try {
			...
			assert.equal(
				(calls[0].options.env as NodeJS.ProcessEnv | undefined)
					?.OMX_DEFERRED_UPDATE_PARENT_PID,
				"12345",
			);
			assert.equal(
				(calls[0].options.env as NodeJS.ProcessEnv | undefined)
					?.OMX_DEFERRED_UPDATE_LOG,
				result.logPath,
			);
			assert.equal(
				(calls[0].options.env as NodeJS.ProcessEnv | undefined)
					?.OMX_SKIP_NATIVE_AGENT_REFRESH,
				"1",
			);
	...
	it("preserves plugin setup delivery mode for deferred post-update refreshes", async () => {
		...
		try {
			...
			assert.match(
				calls[0].args[1],
				/'omx' 'setup' '--scope' 'user' '--plugin' '--mcp' 'none' '--disable-team'/,
			);
			assert.equal(
				(calls[0].options as { env?: NodeJS.ProcessEnv } | undefined)?.env
					?.OMX_SKIP_NATIVE_AGENT_REFRESH,
				"1",
			);
		} finally {
	...
	it("snapshots deferred setup refresh args when scheduling the detached updater", async () => {
		...
		try {
			...
			await writeFile(
				setupScopePath,
				JSON.stringify(
					{ scope: "project", installMode: "legacy", mcpMode: "compat" },
					null,
					2,
				),
			);
...
describe("post-update setup refresh handoff", () => {
	...
	it("returns null when neither package bin nor fallback CLI entry exists", async () => {
		...
		try {
			assert.equal(
				await resolveInstalledCliEntry(join(cwd, "global-root")),
				null,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("does not impose a timeout on the interactive setup refresh handoff", () => {
		...
		const result = spawnInstalledSetupRefresh("/tmp/omx.js", "/tmp/project", ((
			_command,
			_args,
			options,
		) => {
	...
	it("passes persisted plugin setup choices to the updated CLI refresh", async () => {
		...
		try {
			...
			const result = spawnInstalledSetupRefresh("/tmp/omx.js", cwd, ((
				command,
				args,
			) => {
	...
	it("migrates legacy project-local scope when building update setup refresh args", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-update-plugin-legacy-scope-"),
		);
		...
		try {
			...
			await writeFile(
				join(cwd, ".omx", "setup-scope.json"),
				JSON.stringify(
					{ scope: "project-local", installMode: "plugin", mcpMode: "none" },
					null,
					2,
				),
			);
...
describe("persisted merge policy update replay", () => {
	...
	it("does not replay malformed, unscoped, invalid-scope, or nonboolean merge policy records", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-update-merge-policy-invalid-"),
		);
		try {
			await mkdir(join(cwd, ".omx"), { recursive: true });
			const statePath = join(cwd, ".omx", "setup-scope.json");
	...
	it("snapshots the same merge-policy argv for deferred POSIX and PowerShell commands", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-update-merge-policy-snapshot-"),
		);
		...
		try {
			...
			await writeFile(
				join(cwd, ".omx", "setup-scope.json"),
				JSON.stringify({ scope: "user", mergeAgents: false, force: true }),
			);
