...
describe("omx uninstall", () => {
	...
	it("fails closed for unsupported root metadata in hooks.json", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-invalid-hooks-root-"),
		);
		try {
			const home = join(wd, "home");
			const codexDir = join(home, ".codex");
	...
	it("removes safely positioned managed wrappers while preserving foreign hooks and the native feature flag", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-safe-foreign-hooks-"),
		);
		try {
			const home = join(wd, "home");
			const codexDir = join(home, ".codex");
	...
	it("fails closed without removing a shell-expanding foreign command that resembles an OMX hook", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-shell-expanding-hook-"),
		);
		try {
			const home = join(wd, "home");
			const codexDir = join(home, ".codex");
	...
	it("fails closed without cleaning config when managed removal would shift a foreign handler", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-unsafe-foreign-hooks-"),
		);
		try {
			const home = join(wd, "home");
			const codexDir = join(home, ".codex");
			...
			assert.match(
				res.stderr,
				/unsafe_managed_removal|shift a foreign coordinate/i,
			);
			assert.equal(await readFile(configPath, "utf-8"), config);
			assert.equal(await readFile(hooksPath, "utf-8"), hooks);
		} finally {
	...
	it("fails before uninstall writes when marker-wrapped trust state has foreign siblings", async () => {
		...
		for (const representation of representations) {
			const wd = await mkdtemp(
				join(tmpdir(), `omx-uninstall-trust-${representation.name}-`),
			);
			try {
				await withCwd(wd, async () => {
					...
					assert.deepEqual(
						(await readdir(codexDir)).filter((entry) =>
							entry.includes(".omx-"),
						),
						[],
					);
	...
	it("fails before uninstall writes when marker trust has absent or non-managed hooks expectations", async () => {
		...
		for (const fixture of fixtures) {
			const wd = await mkdtemp(
				join(tmpdir(), `omx-uninstall-marker-${fixture.name}-`),
			);
			try {
				await withCwd(wd, async () => {
					const codexDir = join(wd, ".codex");
	...
	it("preserves a drive-qualified shim referenced by a drive-less future hook event", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-drive-qualified-future-shim-"),
		);
		try {
	...
	it("preserves a shim for a distinct absolute -File target", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-quoted-inert-shim-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
	...
	it("treats exact and all ambiguous preserved shim references as read-only transaction preconditions", async () => {
		const fixtures = [
			{
				name: "normalized-alias-deletion",
				reference: "exact",
				mutation: "delete",
			},
			{
				name: "basename-ambiguity-replacement",
				reference: "basename",
				mutation: "replace",
			},
			{
				name: "environment-indirection-replacement",
				reference: "environment",
				mutation: "replace",
			},
			{
				name: "alternate-absolute-alias-deletion",
				reference: "alternate-absolute",
				mutation: "delete",
			},
			{
				name: "rooted-short-name-deletion",
				reference: "rooted-short-name",
				mutation: "delete",
			},
			{
				name: "drive-relative-short-name-deletion",
				reference: "drive-relative-short-name",
				mutation: "delete",
			},
			{
				name: "distinct-basename-absolute-deletion",
				reference: "distinct-basename-absolute",
				mutation: "delete",
			},
		] as const;
		for (const fixture of fixtures) {
			const wd = await mkdtemp(
				join(tmpdir(), `omx-uninstall-preserved-shim-${fixture.name}-`),
			);
			try {
				await withCwd(wd, async () => {
					const codexDir = join(wd, ".codex");
	...
	it("rolls back hooks when a preserved environment-indirected shim drifts under --keep-config", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-keep-config-preserved-shim-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
				...
				const config = buildOmxConfig();
				const replacement = Buffer.from(
					"concurrent preserved shim replacement\n",
					"utf-8",
				);
				await mkdir(codexDir, { recursive: true });
				await writeFile(configPath, config);
	...
	it("rolls back when a preserved Windows shim drifts after staged-cleanup finalization", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-preserved-shim-finalization-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
					...
					uninstall({
						...
						transactionFailureInjector: async (stage) => {
							...
							assert.equal(
								...
								false,
								"hooks must mutate before shim removal",
							);
							assert.equal(await readFile(configPath, "utf-8"), config);
					...
					uninstall({
						...
						transactionFailureInjector: async (stage) => {
							...
							assert.equal(
								existsSync(hooksPath),
								false,
								"hooks must mutate before config commit",
							);
							assert.equal(
								existsSync(shimPath),
								false,
								"shim must mutate before config commit",
							);
	...
	it("never replaces a concurrent symlink while rolling back a failed uninstall transaction", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-stale-rollback-link-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
	...
	it("does not truncate or follow regular and symlink replacement temporary collisions", async () => {
		for (const collisionKind of ["regular", "symlink"] as const) {
			const wd = await mkdtemp(
				join(tmpdir(), `omx-uninstall-temp-${collisionKind}-`),
			);
			try {
				await withCwd(wd, async () => {
					const codexDir = join(wd, ".codex");
					...
					assert.equal(
						await readFile(
							collisionKind === "regular" ? collisionPath : collisionTarget,
							"utf-8",
						),
						collision,
					);
	...
	it("fails closed for non-throwing regular and symlink write-temporary replacements", async () => {
		for (const replacementKind of ["regular", "symlink"] as const) {
			const wd = await mkdtemp(
				join(tmpdir(), `omx-uninstall-write-temp-${replacementKind}-`),
			);
			try {
				await withCwd(wd, async () => {
					const codexDir = join(wd, ".codex");
					...
					const replacement = Buffer.from(
						"foreign write temporary replacement\n",
					);
					await mkdir(codexDir, { recursive: true });
					await writeFile(configPath, config);
					await writeFile(hooksPath, hooks);
					...
					await assert.rejects(
						uninstall({
							...
							transactionTemporaryPath: (_path, purpose) =>
								purpose === "write"
									? temporaryPath
									: join(codexDir, ".hooks.json.staged"),
							transactionFailureInjector: async (stage) => {
						...
						(error: unknown) => {
							...
							assert.match(
								message,
								/planned artifact .* changed, was created, or was removed after planning|expected a regular file, not a symbolic link/,
							);
							assert.ok(message.includes(temporaryPath));
							assert.match(
								message,
								/preserved temporary .*manual recovery after cleanup verification failed/,
							);
							return true;
						},
					);
	...
	it("fails closed for non-throwing regular and symlink staged-tombstone replacements", async () => {
		for (const replacementKind of ["regular", "symlink"] as const) {
			const wd = await mkdtemp(
				join(tmpdir(), `omx-uninstall-staged-tombstone-${replacementKind}-`),
			);
			try {
				await withCwd(wd, async () => {
					const codexDir = join(wd, ".codex");
					...
					const config = Buffer.from(buildOmxConfig());
					const hooks = Buffer.from(
						`${JSON.stringify(buildManagedCodexHooksConfig(packageRoot()), null, 2)}\n`,
					);
					const replacement = Buffer.from(
						"foreign staged tombstone replacement\n",
					);
					await mkdir(codexDir, { recursive: true });
					await writeFile(configPath, config);
					await writeFile(hooksPath, hooks);
					...
					await assert.rejects(
						...
						(error: unknown) => {
							...
							assert.match(
								message,
								/preserved staged deletion .*manual recovery after cleanup verification failed/,
							);
							return true;
						},
					);
	...
	it("rejects stale snapshots immediately before rollback rename and staged-copy removal", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-stale-rollback-primitives-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
		...
		try {
			await withCwd(wd, async () => {
				...
				assert.equal(
					...
					false,
					"successful commit leaves no staged tombstone",
				);
	...
	it("does not roll back when preserved metadata drifts after staged-deletion cleanup finalization", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-second-staged-drift-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
	...
	it("treats applied hook, shim, and config snapshots as CAS through commit finalization", async () => {
		const fixtures = [
			{
				name: "later commit",
				stage: "before-shim-removal",
				drift: "hooks",
				committed: false,
			},
			{
				name: "staged cleanup",
				stage: "before-staged-cleanup",
				drift: "shim",
				committed: false,
			},
			{
				name: "finalization",
				stage: "after-staged-cleanup",
				drift: "config",
				committed: true,
			},
		] as const;
	...
	it("rolls back registered destinations when post-rename or post-remove verification fails", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-post-apply-recovery-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
						...
						transactionFailureInjector: async (stage) => {
							if (stage === "before-config-commit") {
								await writeFile(
									...
									JSON.stringify({
										...metadata,
										previousNotify: ["node", "/tmp/concurrent.js"],
									}),
								);
							}
						},
	...
	it("removes exact historical root trust state but preserves nonmatching nested state", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-legacy-hook-state-"),
		);
		try {
			const home = join(wd, "home");
			const codexDir = join(home, ".codex");
			...
			assert.equal(
				rootStateCleanup.status,
				0,
				rootStateCleanup.stderr || rootStateCleanup.stdout,
			);
			...
			const nestedStateCleanup = runOmx(wd, ["uninstall", "--keep-config"], {
				HOME: home,
			});
			assert.equal(
				nestedStateCleanup.status,
				0,
				nestedStateCleanup.stderr || nestedStateCleanup.stdout,
			);
	...
	it("removes only trust state at the managed hook coordinates planned from hooks.json", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-hook-trust-coordinates-"),
		);
		try {
			const home = join(wd, "home");
			const codexDir = join(home, ".codex");
	...
	it("removes unchanged OMX-seeded model/context keys during uninstall", async () => {
		...
		try {
			...
			await writeFile(
				join(codexDir, "config.toml"),
				buildConfigWithSeededModelContext(),
			);
	...
	it("preserves user-edited seeded model/context keys during uninstall", async () => {
		...
		try {
			...
			await writeFile(
				join(codexDir, "config.toml"),
				buildConfigWithEditedSeededModelContext(),
			);
	...
	it("--purge removes .omx/ cache directory", async () => {
		...
		try {
			...
			const res = runOmx(wd, ["uninstall", "--keep-config", "--purge"], {
				HOME: home,
			});
			if (shouldSkipForSpawnPermissions(res.error)) return;
			assert.equal(res.status, 0, res.stderr || res.stdout);
			...
			assert.equal(
				existsSync(omxDir),
				false,
				".omx/ directory should be removed",
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
		...
		for (const variant of [
			...
			it(`preserves ambiguous multi-agent ownership in ${scope} scope (${variant.name})`, async () => {
				...
				try {
					...
					const userConfig = buildAmbiguousMultiAgentConfig(
						"user-config",
						variant,
					);
					const projectConfig = buildAmbiguousMultiAgentConfig(
						"project-config",
						variant,
					);
					...
					const selectedPath =
						scope === "user" ? userConfigPath : projectConfigPath;
					const unselectedPath =
						scope === "user" ? projectConfigPath : userConfigPath;
	...
	it("shows summary of what was removed", async () => {
		...
		try {
			...
			assert.match(
				res.stdout,
				/MCP servers: omx_state, omx_memory, omx_code_intel, omx_trace, omx_wiki/,
			);
	...
	it("warns when overlapping legacy ~/.agents/skills remains after user-scope uninstall", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(canonicalHelp),
				false,
				"canonical OMX skill should be removed",
			);
			assert.equal(
				existsSync(join(home, ".agents", "skills")),
				true,
				"legacy skill root should remain for manual cleanup",
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("does not warn about legacy ~/.agents/skills when none exists", async () => {
		...
		try {
			...
			assert.doesNotMatch(
				res.stdout,
				/omx uninstall does not remove legacy ~\/\.agents\/skills/,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("does not warn when legacy ~/.agents/skills is just a link to the canonical skills root", async () => {
		...
		try {
			...
			const res = runOmx(wd, ["uninstall", "--keep-config"], {
				HOME: home,
				CODEX_HOME: codexDir,
			});
			if (shouldSkipForSpawnPermissions(res.error)) return;
	...
	it("--dry-run --purge does not actually remove .omx/ directory", async () => {
		...
		try {
			...
			const res = runOmx(
				wd,
				["uninstall", "--keep-config", "--purge", "--dry-run"],
				{ HOME: home },
			);
			if (shouldSkipForSpawnPermissions(res.error)) return;
			assert.equal(res.status, 0, res.stderr || res.stdout);
			...
			assert.equal(
				existsSync(omxDir),
				true,
				".omx/ should NOT be removed in dry-run",
			);
	...
	it("removes setup-scope.json and hud-config.json without --purge", async () => {
		...
		try {
			...
			await writeFile(
				join(omxDir, "hud-config.json"),
				JSON.stringify({ preset: "focused" }),
			);
		...
		const cases = [
			{
				...
				removed: ["model_context_window", "model_auto_compact_token_limit"],
				preserved: {},
			},
			{
				lines: [
				...
				preserved: { model_auto_compact_token_limit: 777 },
			},
			{
				lines: [
					"model_context_window = 123456",
				...
				preserved: { model_context_window: 123456 },
			},
		];
	...
	it("preserves unmarked and edited values, retaining only ambiguous ownership markers", async () => {
		...
		for (const fixture of cases) {
			...
			try {
				...
				fixture.markers
					? assert.match(config, /seeded behavioral defaults/)
					: assert.doesNotMatch(config, /seeded behavioral defaults/);
			} finally {
				await rm(wd, { recursive: true, force: true });
			}
	...
	it("keeps singleton and bounded cleanup dry-run-safe, differential, and idempotent", async () => {
		...
		for (const fixture of fixtures) {
			...
			try {
				...
				const dryRun = runOmx(wd, ["uninstall", "--dry-run"], {
					HOME: migratedHome,
				});
				if (shouldSkipForSpawnPermissions(dryRun.error)) return;
				assert.equal(dryRun.status, 0, dryRun.stderr || dryRun.stdout);
	...
	it("preserves existing uninstall-pipeline semantics after removing a marked pair", async () => {
		...
		try {
			...
			assert.deepEqual(
				TOML.parse(await readFile(migratedPath, "utf-8")),
				TOML.parse(await readFile(baselinePath, "utf-8")),
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("preserves a same-byte foreign replacement made immediately after an uninstall rename", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-immediate-post-rename-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
	...
	it("preflights every staged uninstall recovery copy before rollback", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-rollback-recovery-preflight-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
				...
				const hooksPath = join(codexDir, "hooks.json");
				const shimPath = buildManagedCodexNativeHookWindowsShimPath(codexDir);
				const stagedShimPath = join(
					dirname(shimPath),
					".shim-preflight-recovery",
				);
				const config = Buffer.from(buildOmxConfig(), "utf-8");
				const hooks = Buffer.from(
					`${JSON.stringify(
	...
	it("stops later uninstall rollback restores when the first restored artifact drifts", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-rollback-restored-drift-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
	...
	it("preserves a foreign inode injected after final uninstall rename validation", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-final-rename-claim-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
	...
	it("preserves a foreign inode injected after final uninstall removal validation", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-final-remove-claim-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
	...
	it("preserves a foreign inode injected after final uninstall rollback validation", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-uninstall-final-restore-claim-"),
		);
		try {
			await withCwd(wd, async () => {
				const codexDir = join(wd, ".codex");
	...
	it("warns once after a successful Windows EPERM-degraded uninstall transaction", async () => {
		...
		try {
			await withCwd(wd, async () => {
				...
				assert.equal(
					stderr.filter((line) => line.includes("Windows EPERM")).length,
					1,
				);
