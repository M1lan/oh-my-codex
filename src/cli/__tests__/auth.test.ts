...
`,
			...
			const add = runOmx(
				wd,
				["auth", "add", "secondary", "--device-auth"],
				env,
			);
			assert.equal(add.status, 0, add.stderr);
			assert.equal(
			...
			assert.equal(
				await readFile(join(home, ".omx", "auth", "secondary.json"), "utf-8"),
				'{"access_token":"new-secondary"}\n',
			);
			assert.equal(
				await readFile(loginArgsFile, "utf-8"),
				"login --device-auth\n",
			);
			assert.notEqual(
				(await readFile(loginEnvFile, "utf-8")).trim(),
				codexHome,
			);
			assert.doesNotMatch(
				add.stdout + add.stderr,
				/live-primary|new-secondary/,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("sets subscription Codex defaults when auth add sees empty config", async () => {
		...
		try {
			...
			const add = runOmx(wd, ["auth", "add", "work"], {
				HOME: home,
				CODEX_HOME: codexHome,
				PATH: `${bin}:/usr/bin:/bin`,
			});
			assert.equal(add.status, 0, add.stderr);
			assert.match(
				await readFile(join(codexHome, "config.toml"), "utf-8"),
...
`,
			...
			assert.equal(
				await readFile(join(home, ".omx", "auth", "project.json"), "utf-8"),
				'{"access_token":"project-secret"}\n',
			);
			assert.equal(
				await readFile(join(expectedLiveCodexHome, "auth.json"), "utf-8"),
				'{"access_token":"project-secret"}\n',
			);
			assert.notEqual(
				(await readFile(loginEnvFile, "utf-8")).trim(),
				expectedLiveCodexHome,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("fails soft when no slots are configured", async () => {
		...
		try {
			const result = runOmx(wd, ["--hotswap", "--direct"], {
				HOME: join(wd, "home"),
				PATH: `/usr/bin:/bin`,
			});
	...
	it("hotswaps on 429 and resumes the latest rollout with the next slot", async () => {
		...
		try {
			...
			await mkdir(codexHome, { recursive: true });
			await writeFile(
				join(authDir, "first.json"),
				'{"access_token":"first-secret"}\n',
			);
			await writeFile(
				join(authDir, "second.json"),
...
`,
			...
			assert.ok(
				modelInstructionsArg.startsWith(modelInstructionsPrefix),
				modelInstructionsArg,
			);
			assert.ok(
				modelInstructionsArg.endsWith(modelInstructionsSuffix),
				modelInstructionsArg,
			);
			...
			assert.deepEqual(
				spawnArgv[1].slice(spawnArgv[1].indexOf("--")),
				opaqueSuffix,
			);
			...
			assert.doesNotMatch(
				...
				/first-secret|second-secret|stderr-secret|abc\.def/,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
...
`,
			...
			const result = runOmx(
				wd,
				["--hotswap", "--direct", "--model", "gpt-review"],
				{ HOME: home, CODEX_HOME: codexHome, PATH: `${bin}:/usr/bin:/bin` },
			);
			assert.equal(result.status, 0, result.stderr + result.stdout);
			assert.match(
				result.stderr,
				/token invalidated for slot first; rotating to slot second/,
			);
	...
	it("emits one clean error when all hotswap slots are exhausted", async () => {
		...
		try {
			...
			assert.equal(result.status, 1);
			const matches =
				result.stderr.match(/all slots exhausted or invalid: first, second/g) ??
				[];
