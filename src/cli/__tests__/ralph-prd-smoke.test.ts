...
describe("omx ralph --prd smoke gate", () => {
	it("aborts before Codex launch when .omx/prd.json is missing", async () => {
		...
		try {
			...
			const result = runOmx(
				cwd,
				["ralph", "--prd", "ship release checklist"],
				buildEnv(home, fakeBin),
			);
			...
			assert.equal(
				...
				false,
				"expected no Codex launch log when PRD gate fails",
			);
		} finally {
	...
	it("aborts before Codex launch when only canonical PRD markdown exists", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(launchLog),
				false,
				"expected no Codex launch log when only canonical markdown exists",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("aborts before Codex launch when a completed story lacks architect approval", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(launchLog),
				false,
				"expected no Codex launch log when architect approval is missing",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("launches Codex exactly once when .omx/prd.json is valid", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(launchLog),
				true,
				"expected a Codex launch log for the valid path",
			);
			...
			assert.equal(
				launches.length,
				1,
				`expected exactly one Codex launch, got ${launches.length}`,
			);
			...
			const instructions = await readFile(
				join(cwd, ".omx", "ralph", "session-instructions.md"),
				"utf-8",
			);
