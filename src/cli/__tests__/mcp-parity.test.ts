...
async function writeOwnerEvidenceTmux(
	cwd: string,
...
describe("mcpParityCommand", () => {
	...
	it("preserves session-scoped state when used as the state fallback path", async () => {
		...
		try {
			...
			if (typeof previousDisable === "string")
				process.env.OMX_STATE_SERVER_DISABLE_AUTO_START = previousDisable;
	...
	it("rejects unmatched implicit owner environment state writes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-mcp-parity-unmatched-owner-"),
		);
		const logs = captureLogs();
		...
		try {
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", "omx-unmatched-id", "ralplan-state.json"),
				),
				false,
			);
		} finally {
	...
	it("converges owner-present env state writes on the canonical native session", async () => {
		...
		try {
			...
			const fakeBinDir = await writeOwnerEvidenceTmux(
				cwd,
				ownerSessionId,
				canonicalSessionId,
			);
	...
	it("supports notepad and project-memory parity via CLI", async () => {
		...
		try {
			await mcpParityCommand("notepad", [
				...
				"--input",
				JSON.stringify({
					content: "Investigating MCP transport death",
					workingDirectory: cwd,
				}),
				"--json",
			]);
			const notepadWrite = JSON.parse(logs.pop() || "{}") as {
				success?: boolean;
			};
			...
			const notepadRead = JSON.parse(logs.pop() || "{}") as {
				content?: string;
			};
			assert.match(
				notepadRead.content ?? "",
				/Investigating MCP transport death/,
			);
			...
			await mcpParityCommand("project-memory", [
				...
				JSON.stringify({
					...
					content: "CLI parity exists for transport fallback",
					workingDirectory: cwd,
				}),
				"--json",
			]);
			const addNote = JSON.parse(logs.pop() || "{}") as {
				success?: boolean;
				noteCount?: number;
			};
			...
			const memoryRead = JSON.parse(logs.pop() || "{}") as {
				notes?: Array<{ content?: string }>;
			};
			assert.equal(memoryRead.notes?.length, 1);
			assert.equal(
				memoryRead.notes?.[0]?.content,
				"CLI parity exists for transport fallback",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("supports wiki parity via CLI", async () => {
		...
		try {
			...
			const addResult = JSON.parse(logs.pop() || "{}") as {
			...
			const queryResult = JSON.parse(logs.pop() || "[]") as Array<{
