...
describe("nested help routing", () => {
	for (const [argv, expectedUsage] of [
		[
			["adapt", "--help"],
			/Usage:\s*omx adapt <target> <probe\|status\|init\|envelope\|doctor>/i,
		],
		[
			["ask", "--help"],
		...
		[
			["question", "--help"],
			/omx question - OMX-owned blocking user question entrypoint/i,
		],
		[
			["autoresearch", "--help"],
			/hard-deprecated legacy command surface[\s\S]*\$autoresearch/i,
		],
		[
			["explore", "--help"],
			/hard-deprecated legacy command surface[\s\S]*omx sparkshell/i,
		],
		[["hud", "--help"], /Usage:\s*\n\s*omx hud\s+Show current HUD state/i],
		[["hooks", "--help"], /Usage:\s*\n\s*omx hooks init/i],
		[
			["state", "--help"],
			/Usage:\s*omx state <read\|write\|clear\|list-active\|get-status>/i,
		],
		[
			["notepad", "--help"],
			/Usage:\s*omx notepad <tool-name>[\s\S]*Available tools:[\s\S]*notepad_read/i,
		],
		[
			["project-memory", "--help"],
			/Usage:\s*omx project-memory <tool-name>[\s\S]*Available tools:[\s\S]*project_memory_read/i,
		],
		[
			["trace", "--help"],
			/Usage:\s*omx trace <tool-name>[\s\S]*Available tools:[\s\S]*trace_timeline/i,
		],
		[
			["code-intel", "--help"],
			/Usage:\s*omx code-intel <tool-name>[\s\S]*Available tools:[\s\S]*lsp_diagnostics/i,
		],
		[["mcp-serve", "--help"], /Usage:\s*omx mcp-serve <target>/i],
		[["tmux-hook", "--help"], /Usage:\s*\n\s*omx tmux-hook init/i],
		[
			["ralph", "--help"],
			/omx ralph - Launch Codex with ralph persistence mode active/i,
		],
