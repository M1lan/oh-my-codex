...
async function makeTar(
	...
	for (const item of entries) {
		...
		const header: tar.Headers = {
			name: item.name,
			type,
			size: type === "file" ? (item.data?.length ?? 0) : 0,
...
async function writeManagedBinary(
	binaryPath: string,
...
async function writeManagedSidecar(
	binaryPath: string,
	contents: string,
): Promise<void> {
	await mkdir(dirname(binaryPath), { recursive: true });
...
async function createHydrationFixture(): Promise<{
	...
	const archive = await makeTar([
		{ name: "omx-sparkshell", data: Buffer.from("#!/bin/sh\necho hydrated\n") },
	]);
	...
	await writeFile(
		join(assetRoot, "native-release-manifest.json"),
	...
	return {
		...
		cacheDir,
		env,
		expectedPath: resolveCachedNativeBinaryPath(
			"omx-sparkshell",
			"0.8.15",
			"linux",
			"x64",
	...
	it("does not treat an installed npm package that ships src/scripts as a source checkout", async () => {
		...
		try {
			...
			await writeFile(
				...
				JSON.stringify({ name: "oh-my-codex" }),
			);
...
describe("native asset helpers", () => {
	...
	it("orders manifest assets musl-first for Linux hydration", () => {
		...
		const ordered = resolveNativeReleaseAssetCandidates(
			manifest,
			"omx-sparkshell",
			"0.8.15",
			"linux",
			...
			assets: [
				{
					...
					archive: "omx-api-x86_64-pc-windows-msvc.zip",
					binary: "omx-api",
					binary_path: "omx-api.exe",
					sha256:
						"b08ef0f6b09978755b554b1c7a7b37989723c360136f91d27cdcbf3c4abadae7",
					size: 478927,
					download_url:
						"https://github.com/Yeachan-Heo/oh-my-codex/releases/download/v0.20.3/omx-api-x86_64-pc-windows-msvc.zip",
				},
				{
					product: "omx-runtime",
					version: "0.20.3",
					...
					arch: "x64",
					target: "x86_64-pc-windows-msvc",
					archive: "omx-runtime-x86_64-pc-windows-msvc.zip",
					binary: "omx-runtime",
					binary_path: "omx-runtime.exe",
					sha256:
						"52cfc795467c2971ac19ceafb9686f83691375b6fa15ee02b5465839d98c6909",
					size: 466657,
					download_url:
						"https://github.com/Yeachan-Heo/oh-my-codex/releases/download/v0.20.3/omx-runtime-x86_64-pc-windows-msvc.zip",
				},
			],
		};
		const loadedManifest = await loadNativeReleaseManifest(
			process.cwd(),
			"0.20.3",
			{
	...
	it("derives GitHub release base url from package.json repository + version", async () => {
		...
		try {
			...
			assert.equal(
				base,
				"https://github.com/Yeachan-Heo/oh-my-codex/releases/download/v0.8.15",
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("hydrates a native binary when the archive wraps files in a top-level directory", async () => {
		...
		try {
			...
			const stagingDir = join(
				wd,
				"staging",
				"omx-sparkshell-x86_64-unknown-linux-musl",
			);
			await mkdir(stagingDir, { recursive: true });
			const binaryPath = join(stagingDir, "omx-sparkshell");
			...
			await writeFile(
				...
				await makeTar([
					{
						name: "omx-sparkshell-x86_64-unknown-linux-musl/",
						type: "directory",
					},
					{
						name: "omx-sparkshell-x86_64-unknown-linux-musl/omx-sparkshell",
						data: Buffer.from("#!/bin/sh\necho hydrated-nested\n"),
					},
				]),
			);
	...
	it("returns undefined when the native release manifest is unavailable", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-native-hydrate-missing-manifest-"),
		);
		try {
			await writeFile(
				join(wd, "package.json"),
	...
	it("converges concurrent hydrators from an absent cache tree without publication leftovers", async () => {
		...
		try {
			...
			assert.deepEqual((await readdir(dirname(fixture.expectedPath))).sort(), [
				"omx-sparkshell",
				"omx-sparkshell.sha256",
			]);
		} finally {
	...
	it("fails closed when a parent mkdir races with a symlink swap", async () => {
		...
		const canonicalCacheDir = join(
			await realpath(dirname(fixture.cacheDir)),
			"cache",
		);
		let swapped = false;
...
describe("managed native binary inspection", () => {
	it("classifies B/S/L protocol states without accepting unverified cache files", async () => {
		...
		const binaryPath = resolveCachedNativeBinaryPath(
			"omx-sparkshell",
			"0.8.15",
			"linux",
			"x64",
		...
		const cases: Array<{
			...
			},
			{
				...
				setup: async () => {
					...
					await writeManagedSidecar(
						binaryPath,
						`${sha256(Buffer.from("native-binary"))}\n`,
					);
				},
			},
			{
			...
			},
			{
				...
				setup: async () => {
					await mkdir(dirname(binaryPath), { recursive: true });
					const canonicalBinaryPath = join(
						await realpath(dirname(binaryPath)),
						"omx-sparkshell",
					);
					await writeFile(
						lockPath,
		...
		try {
			for (const testCase of cases) {
				...
				if (testCase.expected === "verified")
					assert.equal(inspected.path, await realpath(binaryPath));
				if (testCase.expectedLock) {
					assert.equal(
						inspected.lock?.classification,
						testCase.expectedLock,
						testCase.name,
					);
					assert.equal(
						inspected.lock?.owner?.binary_path,
						join(await realpath(dirname(binaryPath)), "omx-sparkshell"),
						testCase.name,
					);
				}
			}
	...
	describe("managed native binary lock diagnostics", () => {
		it("keeps a verified binary authoritative when the lock is unsafe", async () => {
			...
			try {
				...
				await symlink(
					join(wd, "missing-lock-target"),
					`${binaryPath}.hydrate.lock`,
				);
				const inspected = await inspectManagedNativeBinary(binaryPath, env);
		...
		it("treats an unsafe checksum entry as orphaned when the binary is absent", async () => {
			...
			try {
				await mkdir(dirname(binaryPath), { recursive: true });
				await symlink(
					join(wd, "missing-sidecar-target"),
					`${binaryPath}.sha256`,
				);
				assert.equal(
					(await inspectManagedNativeBinary(binaryPath, env)).state,
					"orphan-checksum",
				);
			} finally {
				await rm(wd, { recursive: true, force: true });
				...
				assets: [
					{
						...
						binary_path: "omx-api",
						sha256: "a".repeat(64),
						download_url: "https://example.invalid/one",
					},
					{
						product: "omx-api",
						version: "0.8.15",
						...
						binary_path: "omx-api",
						sha256: "b".repeat(64),
						download_url: "https://example.invalid/two",
					},
				],
			}),
		);
		...
		try {
			const archives = new Map<string, Buffer>([
				[
					...
					await makeTar(
						[
							{ name: "wrapper/", type: "directory" },
							{ name: "wrapper/omx-api", data: Buffer.from("binary") },
						],
						"tar.gz",
					),
				],
			...
			for (const [suffix, bytes] of archives) {
				...
				assert.equal(
					selectNativeArchiveBinary(entries, "omx-api").path,
					"wrapper/omx-api",
				);
			}
		} finally {
