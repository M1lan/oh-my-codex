...
describe("prompt/skill sanitization", () => {
	it("removes stale model aliases and spawn_sub_agent syntax from active prompts and skills", () => {
		const targets = [
			...walkFiles(join(repoRoot, "skills")).filter((file) =>
				file.endsWith("SKILL.md"),
			),
			...walkFiles(join(repoRoot, "prompts")).filter((file) =>
				file.endsWith(".md"),
			),
		];
		...
		const bannedPatterns: Array<{ pattern: RegExp; label: string }> = [
			...
			{
				pattern: /model="(?:haiku|sonnet|opus)"/i,
				label: "legacy model literal",
			},
			{
				pattern: /\b(?:haiku|sonnet|opus) tier\b/i,
				label: "legacy tier alias",
			},
			...
			{
				pattern: /description:\s*".*\((?:Haiku|Sonnet|Opus)\).*"/i,
				label: "legacy prompt description tier",
			},
