...
describe("omx setup skills overwrite behavior", () => {
	it("installs wiki during setup even though it is omitted from the current manifest", async () => {
		...
		try {
			...
			assert.ok(
				(await readFile(wikiSkill, "utf-8")).includes('description: "[OMX] '),
			);
		} finally {
	...
	it("removes stale web-clone installs during normal hard-deprecation refresh", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(join(wd, ".codex", "skills", "visual-ralph", "SKILL.md")),
				true,
			);
		} finally {
			process.chdir(previousCwd);
	...
	it("removes stale alias/merged skill directories on --force", async () => {
		...
		try {
			...
			for (const staleSkill of staleSkills) {
				assert.equal(
					existsSync(join(wd, ".codex", "skills", staleSkill)),
					false,
				);
			}
	...
	it("does not keep stacking the [OMX] description badge on repeated setup runs", async () => {
		...
		try {
			...
			const matches =
				content.match(
					/\[OMX\] Setup and configure oh-my-codex using current CLI behavior/g,
				) ?? [];
	...
	it("logs skip/remove decisions in verbose mode", async () => {
		...
		try {
			...
			assert.match(
				output,
				/removed stale skill swarm\/ \(status: deprecated\)/,
			);
	...
	it("prints a migration hint when legacy ~/.agents/skills overlaps canonical user skills", async () => {
		...
		try {
			...
			await mkdir(join(wd, ".omx", "state"), { recursive: true });
			await mkdir(join(home, ".agents", "skills", "omx-setup"), {
				recursive: true,
			});
			await writeFile(
				join(home, ".agents", "skills", "omx-setup", "SKILL.md"),
				"# legacy omx-setup\n",
			);
			process.chdir(wd);
			console.log = (...args: unknown[]) => {
			...
			assert.match(
				output,
				/Migration hint: Detected 1 overlapping skill names between canonical .*\.codex\/skills and legacy .*\.agents\/skills\./,
			);
			assert.match(
				output,
				/Remove or archive ~\/\.agents\/skills after confirming .*\.codex\/skills is the version you want Codex to load\./,
			);
		} finally {
			console.log = originalLog;
			process.chdir(previousCwd);
