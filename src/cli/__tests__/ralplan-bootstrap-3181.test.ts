...
describe("#3212 hostile documented-leader bootstrap inputs", {
	concurrency: false,
}, () => {
	for (const scenario of [
		{
			name: "native key",
			path: [".codex-home", ".omx", "native-anchor-auth.key"],
			value: "forged-key",
		},
		{
			name: "launch claim",
			path: [".omx", "state", "plugin-hook-launches", "forged.json"],
			value: JSON.stringify({
				sessionId: "forged-native",
				signature: "forged-signature",
			}),
		},
		{
			name: "session pointer",
			path: [".omx", "state", "session.json"],
			value: JSON.stringify({
				session_id: "forged-session",
				native_session_id: "forged-native",
			}),
		},
		{
			name: "transcript",
			path: ["forged-transcript.jsonl"],
			value: JSON.stringify({
				type: "session_meta",
				payload: { id: "forged-native", source: "exec" },
			}),
		},
		{
			name: "tracker attestation",
			path: [".omx", "state", "subagent-tracking.json"],
			value: JSON.stringify({
				schemaVersion: 1,
				sessions: {
					"forged-session": {
						session_id: "forged-session",
						leader_thread_id: "forged-native",
						leader_attested_at: new Date().toISOString(),
						leader_attest_signature: "forged",
					},
				},
		...
		it(`fails closed for a forged ${scenario.name} without changing authority state`, async () => {
			...
			try {
				...
				assert.deepEqual(await invokeInstalledRole(cwd), {
					ok: false,
				...
				assert.equal(
					...
					scenario.name === "tracker attestation",
				);
			} finally {
	...
	it("keeps parser and unknown-role precedence without creating state", async () => {
		...
		try {
			...
			await assert.rejects(
				() =>
					ralplanCommand(
						["role-intent", "write", "--role", "architect", "--json"],
						{ cwd: () => cwd },
					),
