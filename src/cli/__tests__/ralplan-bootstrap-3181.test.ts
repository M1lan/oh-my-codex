...
describe("#3194 documented PreToolUse bootstrap", () => {
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'omx ralplan role-intent write --role synthetic-unknown --parent-thread "$CODEX_THREAD_ID" --json',
					},
				},
				{ cwd, sessionOwnerPid: process.pid },
			);
	...
	it("resolves project-installed custom roles against the dispatched workspace", async () => {
		...
		try {
			...
			await writeFile(
				join(agentsDir, "custom-reviewer.toml"),
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'omx ralplan role-intent write --role custom-reviewer --parent-thread "$CODEX_THREAD_ID" --json',
					},
				},
				{ cwd, sessionOwnerPid: process.pid },
			);
			assert.equal(
				(result.outputJson?.hookSpecificOutput as Record<string, unknown>)
					?.permissionDecisionReason,
				"unsupported_documented_leader_proof: Codex 0.144.5 hooks do not expose documented root identity required for adapted Ralplan.",
			);
		} finally {
