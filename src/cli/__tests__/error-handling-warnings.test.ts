...
describe("error-handling warning guards", () => {
	...
	it("uses warning logs for watcher lifecycle best-effort failures", async () => {
		...
		const esrchGuardCount =
			source.match(/if \(!hasErrnoCode\(error, ['"]ESRCH['"]\)\)/g)?.length ??
			0;
		...
		assert.match(
			source,
			/detached:\s*shouldDetachBackgroundHelper\(options\.env,\s*process\.platform\),\s+stdio: "ignore",\s+windowsHide: true,/,
		);
		assert.match(
			source,
			/stdio: "ignore",\s+timeout: 3000,\s+windowsHide: true,/,
		);
	});
	...
	it("hides Windows child windows for prompt and notification helpers", async () => {
		...
		const replyListenerSource = await readSource(
		...
		assert.match(
			notifierSource,
			/execFileAsync\(cmd, args, \{ windowsHide: true \}\)/,
		);
		assert.match(
			replyListenerSource,
	...
	it("replaces silent log-write catches with warning logs", async () => {
		const loggingSource = await readSource(
