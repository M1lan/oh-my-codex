...
function encodeApprovedExecutionTask(
	task: string,
	quote: "single" | "double",
): string {
	return quote === "single"
...
function fixturePaneExists(
	fixture: TempTmuxSessionFixture,
	paneId: string,
): boolean {
	try {
		runFixtureTmux(fixture, [
			"display-message",
			"-p",
			"-t",
			paneId,
...
describe("parseTeamStartArgs", () => {
	...
	it("does not reuse an older approved team hint when the latest matching handoff is missing its baseline", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-followup-lineage-missing-baseline-"),
		);
		const previousCwd = process.cwd();
		...
		try {
			...
			await writeFile(
				join(plansDir, "test-spec-alpha-team-lineage.md"),
				"# Test spec\n",
			);
			await writeFile(
	...
	it("does not reuse an older ready Team handoff when the latest same-signature handoff lacks a baseline", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-followup-lineage-invalid-"),
		);
		const previousCwd = process.cwd();
		...
		try {
			...
			const alphaTestSpecPath = join(
				plansDir,
				"test-spec-alpha-team-lineage-ready.md",
			);
			await writeFile(
				alphaPrdPath,
				[
					...
					buildContextPackOutcome(
						canonicalContextPackRelativePath("alpha-team-lineage-ready"),
					),
					"",
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("zeta-team-lineage-invalid"),
					),
					"",
					`Launch via ${approvedCommand}`,
				].join("\n"),
	...
	it("does not reuse an older ready Team handoff when the latest same-signature handoff is incomplete", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-followup-lineage-incomplete-"),
		);
		const previousCwd = process.cwd();
		...
		try {
			...
			const alphaTestSpecPath = join(
				plansDir,
				"test-spec-alpha-team-lineage-complete.md",
			);
			await writeFile(
				alphaPrdPath,
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("alpha-team-lineage-complete"),
					),
					"",
					`Launch via ${approvedCommand}`,
				].join("\n"),
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("zeta-team-lineage-incomplete"),
					),
					"",
					`Launch via ${approvedCommand}`,
				].join("\n"),
	...
	it("prefers the persisted approved binding over a newer latest approved hint for a short follow-up", async () => {
		...
		try {
			...
			const boundTestSpecPath = join(
				plansDir,
				"test-spec-20260501T010203Z-issue-831.md",
			);
			await writeFile(
				boundPrdPath,
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-831"),
					),
					"",
			...
			assert.equal(result.parsed.agentType, "executor");
			assert.equal(
				result.parsed.approvedExecution?.task,
				"Execute approved issue 831 plan",
			);
			assert.equal(
			...
			assert.equal(
				sameFilePath(
					result.parsed.approvedExecution?.prd_path ?? "",
					boundPrdPath,
				),
				true,
			);
		...
		try {
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-831-quoted"),
					),
					"",
			...
			await writeFile(
				join(wd, ".omx", "state", "team-state.json"),
				JSON.stringify(
					{ active: true, team_name: "bound-team-quoted" },
					null,
					2,
				),
			);
	...
	it("round-trips double-quoted approved follow-ups from launch hint encoding through persisted binding", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-followup-bound-double-quoted-"),
		);
		const previousCwd = process.cwd();
		try {
			process.chdir(wd);
			...
			const boundTestSpecPath = join(
				plansDir,
				"test-spec-issue-831-double-quoted.md",
			);
			await writeFile(
				boundPrdPath,
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-831-double-quoted"),
					),
					"",
					`Launch via ${boundCommand}`,
				].join("\n"),
			...
			await writeFile(
				join(wd, ".omx", "state", "team-state.json"),
				JSON.stringify(
					{ active: true, team_name: "bound-team-double-quoted" },
					null,
					2,
				),
			);
	...
	it("reuses the persisted approved binding from session-scoped team state for a short follow-up", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-followup-bound-session-"),
		);
		const previousCwd = process.cwd();
		try {
			process.chdir(wd);
			...
			await writeFile(
				join(plansDir, "test-spec-issue-954.md"),
				"# Test spec\n",
			);
			await mkdir(join(wd, ".omx", "state", "sessions", sessionId), {
				recursive: true,
			});
			await writeFile(
	...
	it("carries plan-only short follow-ups when a persisted binding resolves to a baseline-ready hint", async () => {
		...
		try {
			...
			await writeFile(
				join(plansDir, "test-spec-issue-2085.md"),
				"# Test spec\n",
			);
			await mkdir(join(wd, ".omx", "state"), { recursive: true });
			await writeFile(
				join(wd, ".omx", "state", "team-state.json"),
				JSON.stringify(
					{ active: true, team_name: "bound-plan-only-team" },
					null,
					2,
				),
			);
			await writePersistedApprovedTeamExecutionBinding(
				"bound-plan-only-team",
				wd,
				{
					prd_path: prdPath,
			...
			assert.equal(
				result.parsed.approvedExecution?.task,
				"Execute approved issue 2085 plan",
			);
			assert.equal(result.parsed.approvedExecution?.command, command);
			assert.equal(
				sameFilePath(result.parsed.approvedExecution?.prd_path ?? "", prdPath),
				true,
			);
		} finally {
			process.chdir(previousCwd);
	...
	it("keeps short follow-up generic when the latest approved handoff is missing its baseline", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-followup-missing-baseline-"),
		);
		const previousCwd = process.cwd();
		try {
			process.chdir(wd);
		...
		try {
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-2086-incomplete"),
					),
					"",
	...
	it("fails closed for a short follow-up when the persisted approved binding is malformed", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-followup-malformed-binding-"),
		);
		const previousCwd = process.cwd();
		try {
			process.chdir(wd);
	...
	it("fails closed for a short follow-up when the persisted approved binding is stale even if a newer PRD is ready", async () => {
		...
		try {
			...
			await writeFile(
				join(plansDir, "test-spec-issue-955-alpha.md"),
				"# Test spec\n",
			);
			await writeFile(
			...
			await writeFile(
				join(plansDir, "test-spec-issue-955-zeta.md"),
				"# Test spec\n",
			);
	...
	it("fails closed for a short follow-up when the persisted approved binding is ambiguous", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-followup-bound-ambiguous-"),
		);
		const previousCwd = process.cwd();
		...
		try {
			...
			await writeFile(
				join(plansDir, "test-spec-issue-956.md"),
				"# Test spec\n",
			);
	...
	it("fails closed for a short team follow-up when the selected PRD lists multiple team launch hints", async () => {
		...
		try {
			...
			await writeFile(
				join(wd, ".omx", "plans", "test-spec-issue-831-ambiguous.md"),
				"# Test spec\n",
			);
		...
		try {
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-2042"),
					),
					"",
	...
	it("does not opt normal team startup into repo-aware DAG handoff even when a stale sidecar exists", async () => {
		...
		try {
			...
			await writeFile(
				join(wd, ".omx", "plans", "team-dag-issue-831.json"),
		...
		try {
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-2043"),
					),
					"",
					`Launch via omx team 2:executor "${approvedTask}"`,
					`Launch via $team ralph 5:debugger "${approvedTask}"`,
	...
	it("matches role-agnostic approved team hints for default executor launches", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-dag-role-agnostic-default-"),
		);
		const previousCwd = process.cwd();
		...
		try {
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-2045"),
					),
					"",
	...
	it("matches role-agnostic approved team hints when fallback must ignore the default executor role", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-dag-role-agnostic-fallback-"),
		);
		const previousCwd = process.cwd();
		...
		try {
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-2046"),
					),
					"",
		...
		try {
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-2044"),
					),
					"",
					`Launch via omx team 2:executor "${approvedTask}"`,
		...
		try {
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-2039"),
					),
					"",
			...
			await writeFile(
				join(plansDir, "repo-context-issue-2039.md"),
				"Key boundary: latest approved handoff only.\n",
			);

			const approved = parseTeamStartArgs([
				"3:executor",
				"Execute",
				"approved",
				"issue",
			...
			assert.equal(
				approved.parsed.approvedRepositoryContextSummary?.content,
				"Key boundary: latest approved handoff only.",
			);

			const unrelated = parseTeamStartArgs([
				"3:executor",
				"fix",
				"unrelated",
				"bug",
			...
			assert.equal(
				unrelated.parsed.approvedRepositoryContextSummary,
				undefined,
			);
		} finally {
			process.chdir(previousCwd);
	...
	it("keeps ready approved execution generic when staffing no longer matches the approved launch hint", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-context-ready-mismatch-"),
		);
		const previousCwd = process.cwd();
		try {
			process.chdir(wd);
			...
			await writeFile(
				...
				[
					...
					"",
					buildContextPackOutcome(
						canonicalContextPackRelativePath("issue-2040"),
					),
					"",
			...
			await writeFile(
				join(plansDir, "repo-context-issue-2040.md"),
				"Keep approved context off generic mismatches.\n",
			);
	...
	it("does not carry approved execution for explicit plan-only team launches", async () => {
		...
		try {
			...
			await writeFile(
				join(wd, ".omx", "plans", "repo-context-issue-2087.md"),
				"Do not widen non-baseline context.\n",
			);
	...
	it("uses generic DAG fallback reasons for explicit non-baseline approved launches", async () => {
		...
		try {
			for (const scenario of cases) {
				const wd = await mkdtemp(
					join(tmpdir(), `omx-team-dag-fallback-${scenario.name}-`),
				);
				try {
					process.chdir(wd);
					const plansDir = join(wd, ".omx", "plans");
					...
					prdSections.push(
						`Launch via omx team 3:executor "Execute approved ${scenario.name} DAG plan"`,
					);
					...
					const parsed = parseTeamStartArgs([
						"3:executor",
						"Execute",
						"approved",
						scenario.name,
					...
					const executionPlan = buildRepoAwareTeamExecutionPlan({
						...
						approvedRepositoryContextSummary:
							parsed.parsed.approvedRepositoryContextSummary,
					});
					...
					assert.equal(
						executionPlan.metadata?.decomposition_source,
						"legacy_text",
					);
					assert.equal(
						executionPlan.metadata?.fallback_reason,
						scenario.expected,
					);
				} finally {
					process.chdir(previousCwd);
...
describe("teamCommand shutdown --force parsing", () => {
	...
	it("persists cancelled session-scoped team mode state on shutdown", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-shutdown-session-mode-state-"),
		);
		const previousCwd = process.cwd();
		...
		try {
			...
			assert.ok(
				typeof scopedState.completed_at === "string" &&
					scopedState.completed_at.length > 0,
			);
		} finally {
			process.chdir(previousCwd);
			await rm(wd, { recursive: true, force: true });
	...
	it("does not create session-scoped team mode state on shutdown when only root team mode state exists", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-shutdown-root-mode-only-"),
		);
		const previousCwd = process.cwd();
	...
	it("does not create session-scoped team mode state on shutdown when a stale session.json remains", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-shutdown-stale-session-json-"),
		);
		const previousCwd = process.cwd();
...
`,
		...
		try {
			...
			const result = await runNodeCli(
				["team", "shutdown", "shared-shutdown-cli", "--force"],
				{
					cwd: wd,
					env: {
						...process.env,
			...
			assert.equal(
				result.signal,
				null,
				`shutdown CLI received signal ${result.signal ?? "none"}\n${result.stderr}`,
			);
			assert.equal(
				result.code,
				0,
				`shutdown CLI exit=${result.code}\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
			);
			assert.match(
				result.stdout,
				/Team shutdown complete: shared-shutdown-cli/,
			);
			assert.equal(
				existsSync(join(wd, ".omx", "state", "team", "shared-shutdown-cli")),
				false,
			);
			...
			assert.doesNotMatch(
				tmuxLog,
				/list-panes -t %(12|13|14)\b/,
				"explicit teardown panes must not use target-scoped fallback proof",
			);
		} finally {
	...
	it("keeps the shutdown command alive when executed inside the leader pane PTY", {
		concurrency: false,
	}, async (t) => {
		skipUnlessTmux(t);

		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-shutdown-shared-in-pane-"),
		);
		try {
			await withTempTmuxSession(async (fixture) => {
				...
				const hudPaneId = runFixtureTmux(fixture, [
					"split-window",
					"-d",
					"-P",
					"-F",
				...
				const workerPaneOne = runFixtureTmux(fixture, [
					"split-window",
					"-d",
					"-P",
					"-F",
				...
				const workerPaneTwo = runFixtureTmux(fixture, [
					"split-window",
					"-d",
					"-P",
					"-F",
				...
				assert.equal(
					fixturePaneExists(fixture, fixture.leaderPaneId),
					true,
					"leader pane should remain alive",
				);
...
describe("teamCommand api", () => {
	...
	it("executes read-stall-state via CLI api with structured JSON results", async () => {
		...
		try {
			...
			await mkdir(join(wd, ".omx", "state", "team", "api-read-stall"), {
				recursive: true,
			});
			await writeFile(
				join(
...
describe("teamCommand status", () => {
	it("prints pane ids and raw inspect hints when tmux panes are recorded", async () => {
		...
		try {
			...
			assert.match(
				output,
				/inspect_hint: raw tmux capture commands are quota-free/,
			);
			assert.match(
				output,
				/inspect_leader: tmux capture-pane -p -t %10 -S -400/,
			);
			assert.match(output, /inspect_hud: tmux capture-pane -p -t %11 -S -400/);
			assert.match(
				output,
				/inspect_worker-1: tmux capture-pane -p -t %21 -S -400/,
			);
			assert.match(
				output,
				/inspect_worker-2: tmux capture-pane -p -t %22 -S -400/,
			);
			assert.match(
				output,
				/inspect_summary: [\s\S]*command=tmux capture-pane -p -t %21 -S -400/,
			);
			assert.doesNotMatch(
				output
					.split("\n")
			...
			logs.length = 0;
			await withoutTeamTestWorkerEnv(() =>
				teamCommand(["status", "pane-team", "--model-inspect"]),
			);
	...
	it("returns pane ids and sparkshell hint in JSON mode", async () => {
		...
		try {
			...
			const expectedWd =
				process.platform === "darwin" && wd.startsWith("/var/")
					? `/private${wd}`
					: wd;
			...
			assert.deepEqual(payload.panes?.recommended_inspect_task_paths, {
				"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/tasks/task-1.json`,
			});
			assert.deepEqual(payload.panes?.recommended_inspect_approval_paths, {
				"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/approvals/task-1.json`,
			});
			assert.deepEqual(payload.panes?.recommended_inspect_worker_state_dirs, {
				"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/workers/worker-1`,
			});
			assert.deepEqual(payload.panes?.recommended_inspect_worker_status_paths, {
				"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/workers/worker-1/status.json`,
			});
			assert.deepEqual(
				payload.panes?.recommended_inspect_worker_heartbeat_paths,
				{
					"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/workers/worker-1/heartbeat.json`,
				},
			);
			assert.deepEqual(
				payload.panes?.recommended_inspect_worker_identity_paths,
				{
					"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/workers/worker-1/identity.json`,
				},
			);
			assert.deepEqual(payload.panes?.recommended_inspect_worker_inbox_paths, {
				"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/workers/worker-1/inbox.md`,
			});
			assert.deepEqual(
				payload.panes?.recommended_inspect_worker_mailbox_paths,
				{
					"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/mailbox/worker-1.json`,
				},
			);
			assert.deepEqual(
				payload.panes?.recommended_inspect_worker_shutdown_request_paths,
				{
					"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/workers/worker-1/shutdown-request.json`,
				},
			);
			assert.deepEqual(
				payload.panes?.recommended_inspect_worker_shutdown_ack_paths,
				{
					"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/workers/worker-1/shutdown-ack.json`,
				},
			);
			assert.deepEqual(payload.panes?.recommended_inspect_team_dir_paths, {
				"worker-1": `${expectedWd}/.omx/state/team/pane-json-team`,
			});
			assert.deepEqual(payload.panes?.recommended_inspect_team_config_paths, {
				"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/config.json`,
			});
			assert.deepEqual(payload.panes?.recommended_inspect_team_manifest_paths, {
				"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/manifest.v2.json`,
			});
			assert.deepEqual(payload.panes?.recommended_inspect_team_events_paths, {
				"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/events/events.ndjson`,
			});
			assert.deepEqual(payload.panes?.recommended_inspect_team_dispatch_paths, {
				"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/dispatch/requests.json`,
			});
			assert.deepEqual(payload.panes?.recommended_inspect_team_phase_paths, {
				"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/phase.json`,
			});
			assert.deepEqual(
				payload.panes?.recommended_inspect_team_monitor_snapshot_paths,
				{
					"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/monitor-snapshot.json`,
				},
			);
			assert.deepEqual(
				payload.panes?.recommended_inspect_team_summary_snapshot_paths,
				{
					"worker-1": `${expectedWd}/.omx/state/team/pane-json-team/summary-snapshot.json`,
				},
			);
	...
	it("returns Ultragoal checkpoint guidance in JSON status only when approved Team context exists", async () => {
		...
		try {
			...
			const hint = readApprovedExecutionLaunchHint(wd, "team", {
				prdPath,
				task,
			});
			assert.ok(hint);
			...
			await withoutTeamTestWorkerEnv(() =>
				teamCommand(["status", "ultragoal-json-team", "--json"]),
			);
			...
			assert.equal(
				payload.ultragoal_checkpoint_guidance?.goal_id,
				"G001-team-runtime-bridge",
			);
			assert.equal(
				payload.ultragoal_checkpoint_guidance?.goals_path,
				".omx/ultragoal/goals.json",
			);
			assert.equal(
				payload.ultragoal_checkpoint_guidance?.ledger_path,
				".omx/ultragoal/ledger.jsonl",
			);
			assert.equal(
				payload.ultragoal_checkpoint_guidance?.checkpoint_policy,
				"fresh_leader_get_goal_required",
			);
			assert.match(
				payload.ultragoal_checkpoint_guidance?.checkpoint_command_template ??
					"",
			...
			assert.ok(
				payload.ultragoal_checkpoint_guidance?.evidence_requirements?.some(
					(item) => item.includes(".omx/ultragoal artifacts"),
				),
			);
		} finally {
			console.log = originalLog;
	...
	it("omits Ultragoal checkpoint guidance in JSON status for completed plans without activeGoalId", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-team-status-ultragoal-idle-json-"),
		);
		const previousCwd = process.cwd();
		const logs: string[] = [];
		const originalLog = console.log;
		try {
			...
			await withoutTeamTestWorkerEnv(() =>
				initTeamState(
					"ultragoal-idle-json-team",
					"inspect idle ultragoal status",
					"executor",
					1,
					wd,
				),
			...
			await withoutTeamTestWorkerEnv(() =>
				teamCommand(["status", "ultragoal-idle-json-team", "--json"]),
			);
	...
	it("prints Ultragoal checkpoint guidance in text status with fresh get_goal requirement", async () => {
		...
		try {
			...
			await withoutTeamTestWorkerEnv(() =>
				initTeamState(
					"ultragoal-text-team",
					"inspect ultragoal text guidance",
					"executor",
					1,
					wd,
				),
			...
			const task =
				"Execute Team + Ultragoal G001-team-runtime-bridge text plan";
			await writeFile(
				prdPath,
				[
			...
			await withoutTeamTestWorkerEnv(() =>
				teamCommand(["status", "ultragoal-text-team"]),
			);
			...
			assert.match(
				output,
				/leader captured fresh get_goal JSON before checkpointing/i,
			);
...
describe("teamCommand await", () => {
	it("applies project-scope agentReasoning overrides when CODEX_HOME is unset", async () => {
		...
		await writeFile(
			join(wd, ".codex", "prompts", "architect.md"),
			"<identity>You are Architect.</identity>",
		);
		await writeFile(
			fakeCodexPath,
			`#!/usr/bin/env node
...
`,
		...
		try {
			...
			console.log = (...args: unknown[]) =>
				logs.push(args.map(String).join(" "));
			displayTeamName = parseTeamStartArgs(["1:architect", teamTask]).parsed
				.teamName;
			...
			runtimeTeamName = String(
				startedState?.team_name ??
					parseTeamStartArgs(["1:architect", teamTask]).parsed.teamName,
			);
			...
			for (let attempt = 0; attempt < 50; attempt += 1) {
				const capturePath = join(
					captureDir,
					`${displayTeamName}__worker-1.json`,
				);
				if (existsSync(capturePath)) {
					captured = JSON.parse(await readFile(capturePath, "utf-8")) as {
						argv: string[];
...
`,
		...
		try {
			...
			for (let attempt = 0; attempt < 50; attempt += 1) {
				...
				stderr.length = 0;
				await withoutTeamTestWorkerEnv(() =>
					teamCommand(["status", runtimeTeamName]),
				);
