...
describe("extractRalphTaskDescription", () => {
	...
	it("reuses approved launch hint task when no explicit task is supplied", () => {
		assert.equal(
			extractRalphTaskDescription([], "Execute approved issue 1072 plan"),
			"Execute approved issue 1072 plan",
		);
	});
...
describe("resolveApprovedRalphExecutionHint", () => {
	it("reuses the approved hint for follow-up launches without explicit task text", () => {
		assert.equal(
			resolveApprovedRalphExecutionHint(approvedHint, "ralph-cli-launch"),
			approvedHint,
		);
	});
	...
	it("drops the approved hint for unrelated explicit Ralph tasks", () => {
		assert.equal(
			resolveApprovedRalphExecutionHint(
				approvedHint,
				"Refactor unrelated queue handling",
			),
			null,
		);
	});
...
describe("readMatchedApprovedRalphExecutionHint", () => {
	it("selects the matching approved Ralph hint when a PRD lists multiple launch hints", async () => {
		...
		try {
			...
			await writeFile(
				join(cwd, ".omx", "plans", "test-spec-issue-909.md"),
				"# Test Spec\n",
			);
	...
	it("fails closed for bare Ralph follow-up reuse when a PRD lists multiple Ralph launch hints", async () => {
		...
		try {
			...
			await writeFile(
				join(cwd, ".omx", "plans", "test-spec-issue-909-bare.md"),
				"# Test Spec\n",
			);

			const hint = readMatchedApprovedRalphExecutionHint(
				cwd,
				"ralph-cli-launch",
			);
...
describe("RALPH_HELP", () => {
	it("clarifies that prompt-side $ralph activation is separate from CLI --prd mode", () => {
		assert.match(
			RALPH_HELP,
			/Prompt-side `\$ralph` activation is separate from this CLI entrypoint/i,
		);
		assert.match(
			RALPH_HELP,
			/does not imply `--prd` or the PRD\.json startup gate/i,
		);
	});
});
...
describe("normalizeRalphCliArgs", () => {
	...
	it("converts --prd=value into positional task text", () => {
		assert.deepEqual(normalizeRalphCliArgs(["--prd=fix the bug"]), [
			"fix the bug",
		]);
	});
	it("preserves other flags and args", () => {
		assert.deepEqual(
			normalizeRalphCliArgs(["--model", "gpt-5", "--prd", "fix it"]),
			["--model", "gpt-5", "fix it"],
		);
	});
});
...
describe("filterRalphCodexArgs", () => {
	it("consumes --prd so it is not forwarded to codex", () => {
		assert.deepEqual(filterRalphCodexArgs(["--prd", "build", "todo", "app"]), [
			"build",
			"todo",
			"app",
		]);
	...
	it("consumes --PRD case-insensitively", () => {
		assert.deepEqual(filterRalphCodexArgs(["--PRD", "--model", "gpt-5"]), [
			"--model",
			"gpt-5",
		]);
	});
	it("preserves non-omx flags", () => {
		assert.deepEqual(
			filterRalphCodexArgs(["--model", "gpt-5", "--yolo", "fix", "it"]),
			["--model", "gpt-5", "--yolo", "fix", "it"],
		);
	});
});
...
describe("assertRequiredRalphPrdJson", () => {
	...
	it("still requires legacy .omx/prd.json even when canonical PRD markdown exists", async () => {
		...
		try {
			await mkdir(join(cwd, ".omx", "plans"), { recursive: true });
			await writeFile(
				join(cwd, ".omx", "plans", "prd-existing.md"),
				"# Existing canonical PRD\n",
			);
	...
	it("does not gate non-prd Ralph runs", async () => {
		...
		try {
			assert.doesNotThrow(() =>
				assertRequiredRalphPrdJson(cwd, ["fix", "the", "bug"]),
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
...
describe("ralph deslop launch wiring", () => {
	it("consumes --no-deslop so it is not forwarded to codex", () => {
		assert.deepEqual(
			filterRalphCodexArgs(["--no-deslop", "--model", "gpt-5", "fix", "it"]),
			["--model", "gpt-5", "fix", "it"],
		);
	});
	...
	it("documents changed-files-only deslop guidance by default", () => {
		...
		assert.match(
			instructions,
			/rerun the current tests\/build\/lint verification/i,
		);
	});
	...
	it("documents the --no-deslop opt-out when enabled", () => {
		...
		assert.match(
			instructions,
			/skip the mandatory ai-slop-cleaner final pass/i,
		);
		assert.match(
			instructions,
			/latest successful pre-deslop verification evidence/i,
		);
	});
	it("requires conditional native subagent routing guidance", () => {
		...
		assert.match(
			instructions,
			/When the native surface exposes `agent_type` role routing, every Codex native subagent dispatch MUST set `agent_type` to an installed OMX role/,
		);
