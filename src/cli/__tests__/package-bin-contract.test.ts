...
describe("package bin contract", () => {
	it("declares omx with an explicit relative bin path and avoids packaging platform-specific native binaries", () => {
		...
		const binaryName =
			platform() === "win32" ? "omx-sparkshell.exe" : "omx-sparkshell";
		...
		assert.equal(
			pkg.scripts?.["build:explore"],
			"cargo build -p omx-explore-harness",
		);
		assert.equal(
			pkg.scripts?.["build:explore:release"],
			"node dist/scripts/build-explore-harness.js",
		);
		assert.equal(
			pkg.scripts?.["build:full"],
			"pnpm run build && pnpm run build:explore:release && pnpm run build:sparkshell && pnpm run build:api",
		);
		assert.equal(pkg.scripts?.["build:api"], "node dist/scripts/build-api.js");
		assert.equal(
			pkg.scripts?.["clean:native-package-assets"],
			"node dist/scripts/cleanup-explore-harness.js",
		);
		assert.equal(
			pkg.scripts?.["sync:plugin"],
			"node dist/scripts/sync-plugin-mirror.js",
		);
		assert.equal(
			pkg.scripts?.["sync:plugin:check"],
			"node dist/scripts/sync-plugin-mirror.js --check",
		);
		assert.equal(
			pkg.scripts?.["verify:plugin-bundle"],
			"node dist/scripts/sync-plugin-mirror.js --check",
		);
		assert.equal(
			pkg.scripts?.["verify:native-agents"],
			"node dist/scripts/verify-native-agents.js",
		);
		assert.equal(
			pkg.scripts?.prepack,
			"pnpm run build && pnpm run verify:native-agents && pnpm run sync:plugin && pnpm run verify:plugin-bundle && pnpm run clean:native-package-assets",
		);
		...
		assert.equal(
			pkg.scripts?.["test:explore"],
			"cargo test -p omx-explore-harness && node --test dist/cli/__tests__/explore.test.js dist/hooks/__tests__/explore-routing.test.js dist/hooks/__tests__/explore-sparkshell-guidance-contract.test.js",
		);
		assert.equal(
			pkg.scripts?.["test:team:cross-rebase-smoke:compiled"],
			"node dist/scripts/run-test-files.js dist/team/__tests__/cross-rebase-smoke.test.js",
		);
		assert.equal(
		...
		assert.equal(
			pkg.scripts?.test,
			"pnpm run build && pnpm run verify:native-agents && pnpm run verify:plugin-bundle && pnpm run test:node && node dist/scripts/generate-catalog-docs.js --check",
		);
		assert.equal(
		...
		assert.equal(
			pkg.scripts?.["test:compat:node"],
			"pnpm run build && node dist/scripts/run-test-files.js dist/compat/__tests__",
		);
		...
		for (const scriptName of [
			...
			"coverage:team-critical:compiled",
			"coverage:ts:full",
			"coverage:ts:full:compiled",
			"test:team:cross-rebase-smoke:compiled",
			"test:team:worker-runtime-identity:compiled",
			"test:recent-bug-regressions:compiled",
			"test:ralph-persistence:compiled",
			"test:plugin-boundaries:compiled",
			"test:explicit-terminal-contract:compiled",
			"test:compat:node",
		] as const) {
			...
			assert.equal(
				...
				false,
				`${scriptName} should not rely on POSIX command substitution`,
			);
			assert.equal(
				script.includes("*.test.js"),
				false,
				`${scriptName} should not rely on shell glob expansion`,
			);
		}
		...
		assert.equal(
			...
			true,
			"expected package files allowlist to include dist/",
		);
		assert.equal(
			pkg.files?.includes("bin/"),
			false,
			"did not expect broad bin/ allowlist in package files",
		);
		assert.equal(
			pkg.files?.includes("agents/"),
			false,
			"native agent TOMLs are setup output, not package input",
		);
		...
		const prepareBuildSource = readFileSync(
			join(process.cwd(), "src", "scripts", "prepare-build.js"),
			"utf-8",
		);
		...
		assert.ok(
			hasPacked("dist/cli/omx.js"),
			"expected pnpm pack output to include dist/cli/omx.js",
		);

		const packagedHarnessPath =
			process.platform === "win32"
				? "bin/omx-explore-harness.exe"
				: "bin/omx-explore-harness";

		assert.equal(
			hasPacked(packagedHarnessPath),
			false,
			`did not expect ${packagedHarnessPath} in pnpm pack output`,
		);
		assert.equal(
			hasPacked("bin/omx-explore-harness.meta.json"),
			false,
			"did not expect packaged explore harness metadata in pnpm pack output",
		);
		assert.equal(
			somePacked((path) => path.includes("bin/native/")),
			false,
			"did not expect staged native binaries in pnpm pack output",
		);
		assert.ok(
			hasPacked("Cargo.toml"),
			"expected pnpm pack output to include Cargo.toml",
		);
		assert.ok(
			hasPacked("Cargo.lock"),
			"expected pnpm pack output to include Cargo.lock",
		);
		assert.ok(
			hasPacked("crates/omx-explore/Cargo.toml"),
			"expected pnpm pack output to include crates/omx-explore/Cargo.toml",
		);
		assert.ok(
			hasPacked("crates/omx-explore/src/main.rs"),
			"expected pnpm pack output to include crates/omx-explore/src/main.rs",
		);
		assert.ok(
			hasPacked(".agents/plugins/marketplace.json"),
			"expected pnpm pack output to include .agents/plugins/marketplace.json",
		);
		assert.ok(
			hasPacked("plugins/oh-my-codex/.codex-plugin/plugin.json"),
			"expected pnpm pack output to include plugins/oh-my-codex/.codex-plugin/plugin.json",
		);
		assert.ok(
			hasPacked("plugins/oh-my-codex/.mcp.json"),
			"expected pnpm pack output to include plugins/oh-my-codex/.mcp.json",
		);
		assert.ok(
			hasPacked("plugins/oh-my-codex/.app.json"),
			"expected pnpm pack output to include plugins/oh-my-codex/.app.json",
		);
		assert.ok(
			hasPacked("plugins/oh-my-codex/hooks/hooks.json"),
			"expected pnpm pack output to include plugins/oh-my-codex/hooks/hooks.json",
		);
		assert.ok(
			hasPacked("plugins/oh-my-codex/hooks/codex-native-hook.mjs"),
			"expected pnpm pack output to include plugins/oh-my-codex/hooks/codex-native-hook.mjs",
		);
		assert.ok(
			hasPacked("dist/mcp/state-server.js"),
			"expected pnpm pack output to include dist/mcp/state-server.js for omx mcp-serve",
		);
		assert.ok(
			hasPacked("dist/mcp/memory-server.js"),
			"expected pnpm pack output to include dist/mcp/memory-server.js for omx mcp-serve",
		);
		assert.ok(
			hasPacked("dist/mcp/code-intel-server.js"),
			"expected pnpm pack output to include dist/mcp/code-intel-server.js for omx mcp-serve",
		);
		assert.ok(
			hasPacked("dist/mcp/trace-server.js"),
			"expected pnpm pack output to include dist/mcp/trace-server.js for omx mcp-serve",
		);
		assert.ok(
			hasPacked("dist/mcp/wiki-server.js"),
			"expected pnpm pack output to include dist/mcp/wiki-server.js for omx mcp-serve",
		);
		...
		const installableNativeAgentNames = [
		...
		assert.ok(
			hasPacked("skills/ralph/SKILL.md"),
			"expected pnpm pack output to keep canonical root skills",
		);
		assert.ok(
			hasPacked("skills/ultrawork/references/agent-tiers.md"),
			"expected pnpm pack output to include bundled ultrawork agent-tier reference",
		);
		assert.ok(
			hasPacked("skills/ecomode/references/agent-tiers.md"),
			"expected pnpm pack output to include bundled ecomode agent-tier reference",
		);
		assert.ok(
			hasPacked(
				"plugins/oh-my-codex/skills/ultrawork/references/agent-tiers.md",
			),
			"expected pnpm pack output to include bundled plugin ultrawork agent-tier reference",
		);
		assert.ok(
			hasPacked("prompts/executor.md"),
			"expected pnpm pack output to keep prompts",
		);
		assert.ok(
			hasPacked("templates/AGENTS.md"),
			"expected pnpm pack output to keep templates",
		);
		assert.equal(
			somePacked((path) => path === "agents" || path.startsWith("agents/")),
			false,
			"did not expect generated root native agent TOMLs in package output",
		);
		assert.equal(
			somePacked(
				(path) =>
					...
					path.startsWith("plugins/oh-my-codex/.omx/hooks/"),
			),
			false,
			"did not expect setup-owned hook assets inside the installable plugin bundle",
		);
	});
});
