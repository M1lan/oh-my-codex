...
describe("normalizeAutoresearchCodexArgs", () => {
	it("adds sandbox bypass by default for autoresearch workers", () => {
		assert.deepEqual(normalizeAutoresearchCodexArgs(["--model", "gpt-5"]), [
			"--model",
			"gpt-5",
	...
	it("normalizes --madmax to the canonical bypass flag", () => {
		assert.deepEqual(normalizeAutoresearchCodexArgs(["--madmax"]), [
			"--dangerously-bypass-approvals-and-sandbox",
		]);
	});
});
...
describe("parseAutoresearchArgs", () => {
	...
	it("parses explicit run subcommand without breaking bare mission-dir parsing", () => {
		const runParsed = parseAutoresearchArgs([
			"run",
			"missions/demo",
			"--model",
			"gpt-5",
		...
		const bareParsed = parseAutoresearchArgs([
			"missions/demo",
			"--model",
			"gpt-5",
		]);
...
describe("omx autoresearch hard deprecation", () => {
	it("documents autoresearch as deprecated in top-level help", async () => {
		...
		try {
			...
			assert.match(
				result.stdout,
				/omx autoresearch\s+\[DEPRECATED\] Use \$autoresearch; direct CLI launch removed/i,
			);
		} finally {
	...
	for (const argv of [
		...
		it(`fails legacy invocation: omx ${argv.join(" ")}`, async () => {
			...
			try {
				...
				assert.match(
					output,
					/Direct CLI launch, resume, run, bare mission-dir aliases, and tmux split-pane launch are no longer supported/i,
				);
			} finally {
				await rm(cwd, { recursive: true, force: true });
			}
	...
	it("never invokes codex or tmux on the deprecated path", async () => {
		...
		const fakeBin = await mkdtemp(
			join(tmpdir(), "omx-autoresearch-noexec-bin-"),
		);
		...
		await assert.rejects(
			...
			(error: unknown) => {
				assert.match(
					String(error),
					new RegExp(
						AUTORESEARCH_DEPRECATION_MESSAGE.replace(
							/[.*+?^${}()|[\]\\]/g,
							"\\$&",
						),
					),
