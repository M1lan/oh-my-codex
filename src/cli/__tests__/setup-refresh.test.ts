...
describe("omx setup refresh summary and dry-run behavior", () => {
	...
	it("omits Team skills and generated guidance when Team mode is disabled", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(join(wd, ".codex", "prompts", "team-executor.md")),
				false,
			);
			assert.equal(
				existsSync(join(wd, ".codex", "agents", "team-executor.toml")),
				false,
			);
			assert.equal(
	...
	it("creates .gitignore without .omx/ when global Git excludes already ignore it", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-setup-refresh-global-ignore-"),
		);
	...
	it("ignores project-local config while keeping .codex agents, skills, and prompts trackable", async () => {
		...
		try {
			...
			assert.match(
				status.stdout,
				/^\?\? \.codex\/skills\/omx-setup\/SKILL\.md$/m,
			);
			assert.match(
				status.stdout,
	...
	it("omits legacy defaults on fresh setup and removes an exact marked pair on refresh", async () => {
		...
		try {
			...
			assert.doesNotMatch(
				fresh,
				/model_(?:context_window|auto_compact_token_limit)\s*=/,
			);
			...
			assert.equal(
				(TOML.parse(refreshed) as { approval_policy?: string }).approval_policy,
				"on-failure",
			);
			await runSetupInTempDir(wd, { scope: "project" });
	...
	it("removes exact singleton markers only with one explicit opposite root sibling", async () => {
		...
		const cases = [
			...
			{
				baseline: [
					"# explicit multiline context sibling",
	...
	it("rejects malformed config.toml and hooks.json bytes before any setup mutation", async () => {
		...
		for (const fixture of fixtures) {
			...
			try {
				...
				assert.equal(
					existsSync(join(wd, ".codex", "config.toml")),
					fixture.name === "config.toml",
				);
				assert.equal(
					existsSync(join(wd, ".codex", "hooks.json")),
					fixture.name === "hooks.json",
				);
