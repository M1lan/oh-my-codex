...
describe("omx setup AGENTS refresh behavior", () => {
	it("creates user-scope AGENTS.md and leaves project AGENTS.md untouched", async () => {
		...
		try {
			...
			assert.match(
				output,
				/agents_md: updated=1, unchanged=0, backed_up=0, skipped=0, removed=0/,
			);
	...
	it("overwrites existing AGENTS.md in TTY after confirmation and moves the old file to a deterministic sibling backup", async () => {
		...
		try {
			...
			assert.match(
				normalizeDarwinTmpPath(output),
				new RegExp(
					`Backed up existing AGENTS\\.md to ${normalizeDarwinTmpPath(backupPath).replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&")}\\.`,
				),
			);
			assert.match(
				output,
	...
	it("refreshes the managed model table in non-interactive runs without requiring --force", async () => {
		...
		const template = readFileSync(
			join(process.cwd(), "templates", "AGENTS.md"),
			"utf-8",
		);
		try {
			await mkdir(join(wd, ".omx", "state"), { recursive: true });
			...
			assert.match(
				agentsContent,
				new RegExp(
					`\\| Frontier \\(leader\\) \\| \`${expectedContext.frontierModel}\` \\| high \\|`,
				),
			);
			assert.match(
				agentsContent,
				new RegExp(
					`\\| Spark \\(explorer\\/fast\\) \\| \`${expectedContext.sparkModel}\` \\| low \\|`,
				),
			);
			assert.match(
				agentsContent,
				new RegExp(
					String.raw`\| \`executor\` \| \`${expectedContext.frontierModel}\` \| medium \| Code implementation, refactoring, feature work`,
				),
			);
	...
	it("keeps --merge-agents idempotent for already generated AGENTS.md and refreshes stale model rows", async () => {
		...
		try {
			...
			assert.equal(
				countOccurrences(agentsContent, "<!-- omx:generated:agents-md -->"),
				1,
			);
			assert.equal(
				countOccurrences(agentsContent, OMX_MANAGED_AGENTS_START_MARKER),
				0,
			);
			assert.equal(
				countOccurrences(agentsContent, OMX_MANAGED_AGENTS_END_MARKER),
				0,
			);
			assert.match(
				agentsContent,
	...
	it("preserves a title-only user-authored AGENTS.md by default when no OMX markers exist", async () => {
		...
		try {
			...
			assert.match(
				output,
				/WARNING: Existing AGENTS\.md .* lacks OMX contract markers/,
			);
			assert.match(output, /omx setup --scope project --merge-agents/);
			assert.doesNotMatch(
				output,
				/Refreshed AGENTS\.md model capability table/,
			);
			assert.equal(await readFile(join(wd, "AGENTS.md"), "utf-8"), existing);
			assert.equal(existsSync(join(wd, ".omx", "backups", "setup")), false);
		} finally {
	...
	it("preserves symlinked user-scope AGENTS.md during plugin-mode cleanup", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-setup-plugin-agents-symlink-"),
		);
		const restoreTty = setMockTty(false);
		const home = join(wd, "home");
		const restoreHome = setMockHome(home);
		...
		try {
			...
			assert.match(
				await readFile(codexAgentsPath, "utf-8"),
				/Dotfiles-owned guidance\./,
			);
			assert.match(
				output,
	...
	it("merges plugin-mode OMX-managed sections into an unmarked user AGENTS.md when explicitly requested", async () => {
		...
		try {
			...
			assert.match(
				output,
				/Merged plugin-mode OMX-managed AGENTS\.md sections into/,
			);
			assert.match(
				output,
				/agents_md: updated=1, unchanged=0, backed_up=1, skipped=0, removed=0/,
	...
	it("keeps plugin-mode explicit AGENTS.md merge idempotent on repeated runs", async () => {
		...
		try {
			...
			assert.match(
				output,
				/agents_md: updated=0, unchanged=1, backed_up=0, skipped=0, removed=0/,
			);
			assert.equal(
				countOccurrences(secondContent, OMX_MANAGED_AGENTS_START_MARKER),
				1,
			);
			assert.equal(
				countOccurrences(secondContent, OMX_MANAGED_AGENTS_END_MARKER),
				1,
			);
			assert.equal(
				countOccurrences(
	...
	it("does not ask for plugin AGENTS defaults during explicit plugin-mode AGENTS.md merge", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-setup-plugin-agents-merge-prompt-"),
		);
		const restoreTty = setMockTty(true);
		const home = join(wd, "home");
		const restoreHome = setMockHome(home);
	...
	it("skips plugin-mode explicit AGENTS.md merge for symlinked user AGENTS.md", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-setup-plugin-agents-symlink-merge-"),
		);
		const restoreTty = setMockTty(false);
		const home = join(wd, "home");
		const restoreHome = setMockHome(home);
		...
		try {
			...
			assert.match(
				output,
				/Skipped plugin-mode AGENTS\.md merge for symlinked/,
			);
			assert.equal(await readlink(codexAgentsPath), dotfilesAgentsPath);
			assert.equal(
				await readFile(dotfilesAgentsPath, "utf-8"),
				dotfilesContent,
			);
			assert.match(
				output,
	...
	it("persists explicit false and clear policies when a plugin symlink skips the user AGENTS.md write", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-setup-plugin-agents-symlink-policy-"),
		);
		const restoreTty = setMockTty(false);
		const home = join(wd, "home");
		const restoreHome = setMockHome(home);
		...
		try {
			...
			for (const [mergeAgentsPolicy, expected] of [
				...
				assert.equal(
					Object.hasOwn(persisted, "mergeAgents"),
					expected !== undefined,
				);
			}
		} finally {
	...
	it("skips plugin-mode explicit AGENTS.md merge for broken symlinked user AGENTS.md", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-setup-plugin-agents-broken-symlink-merge-"),
		);
		const restoreTty = setMockTty(false);
		const home = join(wd, "home");
		const restoreHome = setMockHome(home);
	...
	it("skips plugin-mode AGENTS defaults for symlinked user AGENTS.md even when prompted overwrite would be accepted", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-setup-plugin-agents-symlink-default-"),
		);
		const restoreTty = setMockTty(true);
		const home = join(wd, "home");
		const restoreHome = setMockHome(home);
	...
	it("skips plugin-mode AGENTS defaults for broken symlinked user AGENTS.md", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-setup-plugin-agents-broken-symlink-default-"),
		);
		const restoreTty = setMockTty(false);
		const home = join(wd, "home");
		const restoreHome = setMockHome(home);
	...
	it("skips plugin-mode explicit AGENTS.md merge during an active project session", async () => {
		...
		try {
			...
			assert.match(
				output,
				/Stop the active session first, then re-run setup\./,
			);
			assert.match(
				output,
				/agents_md: updated=0, unchanged=0, backed_up=0, skipped=1, removed=0/,
	...
	it("skips plugin-mode AGENTS defaults during an active project session", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-setup-plugin-agents-active-default-"),
		);
		const restoreTty = setMockTty(true);
		const home = join(wd, "home");
		const restoreHome = setMockHome(home);
