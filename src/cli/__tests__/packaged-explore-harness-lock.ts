...
export async function withPackagedExploreHarnessLock<T>(
	fn: () => Promise<T>,
): Promise<T> {
	...
	while (true) {
		try {
			...
			if (Date.now() - startedAt > LOCK_TIMEOUT_MS) {
				throw new Error(
					`Timed out waiting for packaged explore harness test lock at ${LOCK_DIR}`,
				);
			}
...
export async function withPackagedExploreHarnessHidden<T>(
	fn: () => Promise<T>,
): Promise<T> {
	return withPackagedExploreHarnessLock(async () => {
		...
		const originalBinary = existsSync(packagedBinary)
			? await readFile(packagedBinary)
			: null;
		const originalMeta = existsSync(packagedMeta)
			? await readFile(packagedMeta)
			: null;
