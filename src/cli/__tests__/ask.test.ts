...
describe("omx ask", () => {
	...
	it("uses package-root advisor script path from non-package cwd and still writes artifact", async () => {
		...
		try {
			...
			assert.ok(
				normalizeDarwinTmpPath(artifactPath).startsWith(
					normalizeDarwinTmpPath(join(wd, ".omx", "artifacts", "claude-")),
				),
			);
	...
	it("adds Claude skip-permissions for issue-work prompts only", async () => {
		...
		try {
			...
			const issueRes = runOmx(
				wd,
				["ask", "claude", "Investigate issue #1536 and summarize it"],
				env,
			);
			...
			assert.match(
				issueArtifact,
				/CLAUDE_ARGS:--dangerously-skip-permissions -p -- Investigate issue #1536 and summarize it/,
			);

			const genericRes = runOmx(
				wd,
				["ask", "claude", "Summarize the README"],
				env,
			);
			assert.equal(
				genericRes.status,
				0,
				genericRes.stderr || genericRes.stdout,
			);
	...
	it("separates Claude frontmatter agent prompts from CLI options", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-ask-agent-prompt-frontmatter-"),
		);
		try {
			const fakeBin = join(wd, "bin");
			const codexHome = join(wd, ".codex-home");
	...
	it("keeps Claude permissions bypass before the separator for frontmatter issue prompts", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-ask-agent-prompt-frontmatter-bypass-"),
		);
		try {
			const fakeBin = join(wd, "bin");
			const codexHome = join(wd, ".codex-home");
