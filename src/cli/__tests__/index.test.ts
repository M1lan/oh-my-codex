...
function normalizeDarwinTmpPath(value: string): string {
	return process.platform === "darwin"
		? value.replaceAll("/private/var/", "/var/")
		: value;
}
...
describe("madmax state isolation", () => {
	it("auto-isolates madmax launch and exec invocations without boxing worktree-only launches", () => {
		assert.equal(
			shouldAutoIsolateMadmaxLaunch("launch", ["--madmax"], {}),
			true,
		);
		assert.equal(
			shouldAutoIsolateMadmaxLaunch("exec", ["--madmax-spark"], {}),
			true,
		);
		assert.equal(
			shouldAutoIsolateMadmaxLaunch("launch", ["--worktree"], {}),
			false,
		);
		assert.equal(
			shouldAutoIsolateMadmaxLaunch("launch", ["-wfeature"], {}),
			false,
		);
		assert.equal(
			shouldAutoIsolateMadmaxLaunch("team", ["--madmax"], {}),
			false,
		);
		assert.equal(
			shouldAutoIsolateMadmaxLaunch("launch", ["--yolo"], {}),
			false,
		);
	});
	...
	it("creates a per-run OMX_ROOT registry entry without touching source .omx", async () => {
		...
		const runs = await mkdtemp(join(tmpdir(), "omx-madmax-runs-"));
		try {
			const runDir = createMadmaxIsolatedRoot(wd, ["--madmax"], {
				OMX_RUNS_DIR: runs,
			});
			assert.equal(runDir.startsWith(runs), true);
			assert.equal(existsSync(join(wd, ".omx")), false);
			const metadata = JSON.parse(
				await readFile(join(runDir, ".omxbox-run.json"), "utf-8"),
			);
	...
	it("stamps a stable detached launch context and exposes it to boxed launch", async () => {
		...
		try {
			const env: NodeJS.ProcessEnv = { OMX_RUNS_DIR: runs };
			const runDir = createMadmaxIsolatedRoot(
				wd,
				["--madmax", "--xhigh", "--tmux"],
				env,
			);
	...
	it("gives independent madmax run roots distinct detached launch context locks", async () => {
		...
		try {
			...
			const firstRunDir = createMadmaxIsolatedRoot(
				wd,
				["--madmax", "--high"],
				firstEnv,
			);
			const secondRunDir = createMadmaxIsolatedRoot(
				wd,
				["--madmax", "--high"],
				secondEnv,
			);
			...
			await assert.rejects(
				() =>
					withMadmaxDetachedContextLock(
						...
						contextKey,
						() => "should-not-run",
						{ maxAttempts: 1, retryMs: 0 },
					),
				(err: unknown) => {
					assert.ok(err instanceof Error);
					assert.match(
						err.message,
						/timed out waiting for madmax detached launch context lock/,
					);
					assert.match(
						err.message,
						new RegExp(`holder pid ${process.pid} is still running`),
					);
					assert.match(err.message, /owner context live-context/);
					assert.match(
						err.message,
						/Another madmax detached launch is active for this directory/,
					);
					assert.match(
						err.message,
						/close the existing madmax session or use --worktree for concurrent work/,
					);
					assert.match(
						err.message,
						/Multiple madmax sessions in one directory are unsafe/,
					);
					return true;
				},
			);
...
describe("detached launch state machine", () => {
	function createDependencies(
		events: string[],
		failAt?: string,
	): DetachedLaunchDependencies<string, string, string> {
		...
		return {
			...
			finalizeSetupFailure: async () => {
				await step("finalize-setup-failure");
			},
		...
		it(`rolls back verified pre-release ownership when ${failure} fails`, async () => {
			...
			await assert.rejects(
				...
				(err: unknown) => {
					...
					assert.equal(
						(
							err as { report?: { transitions: string[] } }
						).report?.transitions.at(-1),
						failure,
					);
					return true;
				},
...
describe("resolveEnvLaunchPolicyOverride", () => {
	it("accepts direct, tmux, detached-tmux, auto, and empty policy values", () => {
		assert.equal(
			resolveEnvLaunchPolicyOverride({ OMX_LAUNCH_POLICY: "direct" }),
			"direct",
		);
		assert.equal(
		...
		assert.equal(
			resolveEnvLaunchPolicyOverride({ OMX_LAUNCH_POLICY: "auto" }),
			undefined,
		);
		assert.equal(
			resolveEnvLaunchPolicyOverride({ OMX_LAUNCH_POLICY: "" }),
			undefined,
		);
	});
...
describe("cleanupPostLaunchModeStateFiles", () => {
	it("repairs empty or truncated mode state files and still cancels valid siblings", async () => {
		...
		const sessionCanonicalPath = join(
			sessionStateDir,
	...
	it("does not preserve complete Ralph cleanup state without completion-audit evidence", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-postlaunch-ralph-complete-audit-missing-"),
		);
		...
		try {
			...
			assert.equal(
				ralph.stop_reason,
				"missing_completion_audit:missing_completion_audit",
			);
			assert.equal(ralph.completion_audit_gate, "blocked");
			assert.equal(
				ralph.completion_audit_missing_reason,
	...
	it("preserves complete Ralph cleanup state when completion-audit evidence is present", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-postlaunch-ralph-complete-audit-present-"),
		);
	...
	it("marks active Ralph state cancelled with interrupted metadata during postLaunch cleanup", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-postlaunch-ralph-interrupted-"),
		);
	...
	it("retries a transient parse failure before cancelling the rewritten mode state", async () => {
		...
		const persisted = JSON.parse(writes[0]?.content ?? "{}") as Record<
			string,
			unknown
		>;
	...
	it("warns on structurally complete malformed JSON without aborting sibling cleanup", async () => {
		...
		assert.match(
			warnings[0] ?? "",
			/skipped malformed mode state .*ralph-state\.json/,
		);
	});

	it("reconciles root skill-active entries for the finished terminal session", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-postlaunch-root-skill-active-"),
		);
	...
	it("preserves other-session active skills when session mode cleanup syncs before root scrub", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-postlaunch-mode-root-skill-active-"),
		);
		const sessionId = "sess-terminal-autopilot";
		...
		try {
			...
			await writeFile(
				join(sessionStateDir, "autopilot-state.json"),
				JSON.stringify(
					{ active: true, mode: "autopilot", current_phase: "ralph" },
					null,
					2,
				),
				"utf-8",
	...
	it("preserves review-pending Autopilot state across postLaunch compact cleanup", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-postlaunch-autopilot-review-pending-"),
		);
		...
		try {
						...
						state: {
							...
							handoff_artifacts: {
								...
								ralph: {
									...
									changed_files: ["src/cli/index.ts"],
								},
								code_review: null,
							},
							review_verdict: null,
			...
			assert.equal(
				(autopilotState.state as Record<string, unknown>)?.review_verdict,
				null,
			);
	...
	it("clears canonical skill-active entries during cleanup and hides them from HUD/overlay readers", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-postlaunch-skill-active-cleanup-"),
		);
...
describe("resolveCliInvocation", () => {
	...
	it("advertises the explicit update command in top-level help", () => {
		assert.match(
			HELP,
			/omx update\s+Install the stable channel now, then refresh setup/,
		);
		assert.match(
			HELP,
	...
	it("scopes Ralplan authority preflight in top-level help", () => {
		assert.match(
			HELP,
			/omx ralplan\s+Adapted Ralplan authority support; preflight applies only when native role routing is unavailable\s+and adapted Ralplan authority is requested/,
		);
	...
	it("advertises concise launch policy controls in top-level help", () => {
		...
		assert.match(
			HELP,
			/OMX_LAUNCH_POLICY=direct[\s\S]*Run without OMX tmux\/HUD management/,
		);
		assert.match(
			HELP,
			/OMX_LAUNCH_POLICY=tmux[\s\S]*Force OMX-managed detached tmux launch/,
		);
		assert.match(
			HELP,
			/OMX_LAUNCH_POLICY=detached-tmux[\s\S]*Force OMX-managed detached tmux launch/,
		);
		assert.match(
			HELP,
			/CLI policy flags \(--direct\/--tmux\) override OMX_LAUNCH_POLICY/,
		);
		assert.match(
			HELP,
...
describe("resolveSetupMcpModeArg", () => {
	it("maps explicit setup MCP mode flags", () => {
		...
		assert.equal(
			resolveSetupMcpModeArg(["--scope", "project", "--mcp", "compat"]),
			"compat",
		);
	});
...
describe("resolveSetupTeamModeArg", () => {
	it("maps explicit setup Team mode flags", () => {
		...
		assert.equal(
			resolveSetupTeamModeArg(["--team-mode", "disabled"]),
			"disabled",
		);
...
describe("project launch scope helpers", () => {
	...
	it("uses project CODEX_HOME when persisted scope is project even if HOME is unusable", async () => {
		...
		try {
			...
			assert.equal(
				resolveCodexHomeForLaunch(wd, { HOME: badHome }),
				join(wd, ".codex"),
			);
			assert.equal(
	...
	it("includes project Codex history artifacts in the runtime mirror for resume launches", async () => {
		...
		try {
			...
			await mkdir(join(wd, ".omx"), { recursive: true });
			await mkdir(join(projectCodexHome, "sessions", "2026", "06", "03"), {
				recursive: true,
			});
			await writeFile(
				join(wd, ".omx", "setup-scope.json"),
			...
			await writeFile(
				...
				"state db placeholder",
			);
			await writeFile(
				join(projectCodexHome, "state_5.sqlite-wal"),
				"state db wal placeholder",
			);
			await writeFile(
				join(projectCodexHome, "logs_2.sqlite-shm"),
				"logs db shm placeholder",
			);
			await writeFile(
				join(
					projectCodexHome,
			...
			assert.equal(
				existsSync(join(runtimeCodexHome, "state_5.sqlite-wal")),
				true,
			);
			assert.equal(
				existsSync(join(runtimeCodexHome, "logs_2.sqlite-shm")),
				true,
			);
			assert.equal(
				existsSync(
	...
	it("creates durable project Codex transcript links for project launches", async () => {
		...
		try {
			...
			const runtimeCodexHome = runtimeCodexHomePath(
				wd,
				"session-history-links",
			);
			...
			assert.equal(prepared.codexHomeOverride, runtimeCodexHome);
			assert.equal(
				(await lstat(join(runtimeCodexHome, "sessions"))).isSymbolicLink(),
				true,
			);
			assert.equal(
				(await lstat(join(runtimeCodexHome, "history.jsonl"))).isSymbolicLink(),
				true,
			);
			assert.equal(
				(
					await lstat(join(runtimeCodexHome, "session_index.jsonl"))
				).isSymbolicLink(),
				true,
			);
			await writeFile(
			...
			await writeFile(
				join(runtimeCodexHome, "history.jsonl"),
				'{"session_id":"linked"}\n',
			);
			await writeFile(
				join(runtimeCodexHome, "session_index.jsonl"),
				'{"id":"linked"}\n',
			);
			...
			assert.equal(
				await readFile(
					join(projectCodexHome, "sessions", "linked-rollout.jsonl"),
					"utf-8",
				),
				'{"type":"session_meta"}\n',
			);
		...
		try {
			...
			await writeFile(
				join(
					...
					"06",
					"16",
					"rollout-session-2835.jsonl",
				),
				'{"type":"session_meta","payload":{"id":"session-2835"}}\n',
			);
			await writeFile(
				join(runtimeCodexHome, "history.jsonl"),
				'{"session_id":"session-2835"}\n',
			);
			await writeFile(
				join(runtimeCodexHome, "session_index.jsonl"),
				'{"id":"session-2835"}\n',
			...
			await writeFile(
				join(runtimeCodexHome, "auth.json"),
				'{"token":"opaque"}\n',
			);
			...
			assert.equal(
				await readFile(join(projectCodexHome, "auth.json"), "utf-8"),
				'{"token":"opaque"}\n',
			);
			assert.equal(existsSync(runtimeCodexHome), false);
		} finally {
	...
	it("uses a session-scoped CODEX_HOME mirror for project launch config writes", async () => {
		...
		try {
			...
			await writeFile(
				join(projectCodexHome, "agents", "planner.toml"),
				'name = "planner"\n',
			);
			...
			assert.equal(
				await readFile(
					join(runtimeCodexHome, "agents", "planner.toml"),
					"utf-8",
				),
				'name = "planner"\n',
			);
	...
	it("project-scope launch registers native hooks exactly once and persists trust state (GH #2470)", async () => {
		...
		try {
			...
			const nextRuntimeCodexHome = runtimeCodexHomePath(
				wd,
				"session-2470-repeat",
			);
	...
	it("repairs duplicate project hook trust state before relaunching project-scope Codex home (GH #2401)", async () => {
		...
		try {
			...
			const escapedProjectHookTrustHeader = escapeRegExp(
	...
	it("keeps setup-owned hook trust state targeted at the project hooks path (GH #2470)", async () => {
		...
		try {
			...
			assert.doesNotMatch(
				runtimeConfig,
				new RegExp(
					join(runtimeCodexHome, "hooks.json").replace(
						/[.*+?^${}()|[\]\\]/g,
						"\\$&",
					),
				),
	...
	it("respects explicit CODEX_SQLITE_HOME for project-scope launches", async () => {
		...
		try {
			...
			assert.equal(
				...
				runtimeCodexHomePath(wd, "session-explicit-sqlite"),
			);
			assert.equal(prepared.sqliteHomeOverride, undefined);
		} finally {
			await rm(wd, { recursive: true, force: true });
...
describe("pointer launch aborts", () => {
	...
	it("does not exec, tag, or retain a runtime home when pointer context root resolution is rejected", async () => {
		...
		try {
			...
			assert.match(
				result.stderr,
				/Unable to establish launch directory capability/,
			);
			assert.equal(existsSync(codexLog), false);
			assert.equal(existsSync(tmuxLog), false);
			assert.equal(existsSync(join(wd, ".omx", "state", "sessions")), false);
...
describe("resolveCodexLaunchPolicy", () => {
	...
	it("uses tmux-aware launch path when already inside tmux on native Windows", () => {
		assert.equal(
			resolveCodexLaunchPolicy({ TMUX: "psmux-session" }, "win32", true, true),
			"inside-tmux",
		);
	});
	...
	it("honors explicit direct launch requests outside tmux", () => {
		assert.equal(
			resolveCodexLaunchPolicy({}, "linux", true, false, true, true, "direct"),
			"direct",
		);
	});
	...
	it("launches directly when stdin is not a tty outside tmux", () => {
		assert.equal(
			resolveCodexLaunchPolicy({}, "linux", true, false, false, true),
			"direct",
		);
	});
	...
	it("launches directly when stdout is not a tty outside tmux", () => {
		assert.equal(
			resolveCodexLaunchPolicy({}, "linux", true, false, true, false),
			"direct",
		);
	});
...
describe("resolveBackgroundHelperLaunchMode", () => {
	...
	it("spawns helpers directly on native win32", () => {
		assert.equal(
			resolveBackgroundHelperLaunchMode({}, "win32"),
			"direct-detached",
		);
	});
...
describe("tmux HUD pane helpers", () => {
	...
	it("createHudWatchPane rejects an incomplete synthetic source incarnation", () => {
		...
		const paneId = createSharedHudWatchPane(
			...
			(args) => {
				...
				if (args[0] === "if-shell") {
					...
					splitCreated = true;
					const marker = /OMX_TMUX_SPLIT_OPERATION_MARKER='([\w-]+)'/.exec(
						args[5]!,
					);
					const receipt = /display-message -p (__omx_hud_split_[\w-]+)/.exec(
						args[5]!,
					);
		...
		assert.equal(
			calls.some((args) => args[0] === "if-shell"),
			false,
		);
	});
});
...
describe("detached tmux new-session sequencing", () => {
	it("buildDetachedSessionBootstrapSteps uses shared HUD height and split-capture ordering", () => {
		...
		const splitStep = steps.find(
			(step) => step.name === "split-and-capture-hud-pane",
		);
		assert.ok(splitStep);
		...
		assert.equal(
			steps[0]?.args.includes("OMX_SESSION_ID=omx-session-test"),
			true,
		);
		assert.equal(
		...
		assert.equal(
			newSession!.args.includes("-e") &&
				newSession!.args.some(
					(arg) => arg === "OMX_SESSION_ID=sess-detached-managed",
				),
			true,
		);
		assert.equal(
			newSession!.args.some((arg) => arg === "OMX_TMUX_HUD_OWNER=1"),
			true,
		);
	...
	it("buildDetachedSessionBootstrapSteps forwards CODEX_HOME override to detached tmux session", () => {
		...
		assert.equal(
			newSession!.args.includes("-e") &&
				newSession!.args.some(
					(arg) => arg === "CODEX_HOME=/tmp/project/.codex",
				),
			true,
		);
	});
	...
	it("buildDetachedSessionBootstrapSteps forwards CODEX_SQLITE_HOME override to detached tmux session", () => {
		...
		assert.equal(
			newSession!.args.includes("-e") &&
				newSession!.args.some(
					(arg) => arg === `${CODEX_SQLITE_HOME_ENV}=/tmp/project/.codex`,
				),
			true,
		);
	});
	...
	it("buildDetachedSessionBootstrapSteps forwards boxed env to detached tmux session", () => {
		...
		assert.ok(newSession);
		assert.equal(
			newSession.args.some((arg) => arg === "OMX_ROOT=/tmp/boxed-runtime"),
			true,
		);
		assert.equal(
			newSession.args.some(
				(arg) => arg === "OMX_STATE_ROOT=/tmp/boxed-state-root",
			),
			false,
		);
		assert.equal(
		...
		assert.equal(
			newSession.args.some((arg) => arg === "OMXBOX_ACTIVE=1"),
			true,
		);
		assert.equal(
			newSession.args.some(
				(arg) => arg === "OMX_SOURCE_CWD=/tmp/source-project",
			),
			true,
		);
	});
	...
	it("buildDetachedSessionBootstrapSteps preserves OMX_STATE_ROOT identity when no root override is explicit", () => {
		...
		assert.ok(newSession);
		assert.equal(
			newSession.args.some((arg) => arg === "OMX_STATE_ROOT=/tmp/state-root"),
			true,
		);
		assert.equal(
			newSession.args.some((arg) => arg === "OMX_ROOT=/tmp/state-root"),
			false,
		);
	});
	...
	it("buildDetachedSessionBootstrapSteps preserves OMX_ROOT precedence over OMX_STATE_ROOT", () => {
		...
		assert.ok(newSession);
		assert.equal(
			newSession.args.some((arg) => arg === "OMX_ROOT=/tmp/root-from-omx-root"),
			true,
		);
		assert.equal(
			newSession.args.some(
				(arg) => arg === "OMX_STATE_ROOT=/tmp/state-root-should-not-win",
			),
			false,
		);
	});
	...
	it("buildDetachedSessionBootstrapSteps preserves OMX_TEAM_STATE_ROOT over explicit root env", () => {
		const steps = buildDetachedSessionBootstrapSteps(
			...
			"/tmp/root-from-omx-root",
			{
				...
				OMX_TEAM_STATE_ROOT: "/tmp/team-state-root",
			},
		);
		const newSession = steps.find((step) => step.name === "new-session");
		assert.ok(newSession);
		assert.equal(
			newSession.args.some(
				(arg) => arg === "OMX_TEAM_STATE_ROOT=/tmp/team-state-root",
			),
			true,
		);
		assert.equal(
	...
	it("serializes custom parent env for the interactive detached tmux leader without logging values in tmux args", () => {
		...
		assert.match(
			argsText,
			new RegExp(envFilePath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
		);
	...
	it("prepends the repo-local omx shim before global PATH entries", async () => {
		...
		try {
			...
			assert.equal(
				env.PATH,
				`${shimDir}${delimiter}/opt/homebrew/bin:/usr/bin`,
			);
...
`,
			...
			assert.deepEqual(
				(await readFile(localMarker, "utf-8")).trim().split("\n"),
				[localOmxEntry, "team", "status", "hud-check"],
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("overwrites stale runtime shim contents and permissions", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-command-shim-stale-"),
		);
		try {
			const shimPath = omxRuntimeCommandShimPath(cwd);
	...
	it("replaces a stale runtime shim symlink without following it", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-command-shim-symlink-"),
		);
		try {
			...
			assert.equal(
				await readFile(externalTarget, "utf-8"),
				"do not overwrite\n",
			);
	...
	it("preserves an inherited Windows Path key when prepending the shim dir", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-command-shim-winpath-"),
		);
		try {
			const env = prependOmxRuntimeCommandShimToEnv(
				cwd,
	...
	it("collapses duplicate Windows PATH/Path variants to a single Path key", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-command-shim-windup-"),
		);
		try {
			const env = prependOmxRuntimeCommandShimToEnv(
				cwd,
	...
	it("seeds a Windows Path entry from the shim dir when none is inherited", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-runtime-command-shim-winempty-"),
		);
		try {
			const env = prependOmxRuntimeCommandShimToEnv(
				cwd,
	...
	it("runCodex cleans only same-session same-leader HUD panes before launch", async () => {
		...
		assert.match(
			source,
			/const staleHudPaneIds = currentPaneId\s*\? listHudWatchPaneIdsInCurrentWindow\(currentPaneId, \{\s*sessionId,\s*leaderPaneId: currentPaneId,\s*\}\)\s*: \[\];/,
		);
		assert.match(
			source,
		...
		assert.match(
			source,
			/for \(const paneId of duplicateHudPaneIds\) \{\s*killTmuxPane\(paneId\);\s*\}/,
		);
		assert.match(
			source,
			/if \(keeperHudPaneId\) \{\s*hudPaneId = keeperHudPaneId;/,
		);
		assert.doesNotMatch(
			source,
	...
	it("runCodex builds inside-tmux HUD command through explicit runtime-root resolver", async () => {
		...
		assert.match(
			source,
			/const hudRuntimeEnv = \{\s*\.\.\.buildHudRuntimeEnv\(\{\s*sessionId,\s*leaderPaneId: currentPaneId,\s*\.\.\.hudRuntimeRoot,\s*\}\)\.env,\s*\.\.\.runtimeEnvOverlay,\s*\};\s*const hudEnvArgs = Object\.entries\(hudRuntimeEnv\)\.map\(\s*\(\[key, value\]\) => `\$\{key\}=\$\{value\}`,\s*\)/,
		);
		assert.match(
			source,
			/if \(env\.OMX_TEAM_STATE_ROOT\?\.trim\(\)\) return ["']team-env["'];\s*if \(env\.OMX_ROOT\?\.trim\(\) \|\| omxRootOverride\) return ["']omx-root-env["'];\s*if \(env\.OMX_STATE_ROOT\?\.trim\(\)\) return ["']omx-state-root-env["'];/,
		);
		assert.match(
			source,
			/buildTmuxPaneCommand\(["']env["'],\s*\[\s*\.\.\.hudEnvArgs,\s*["']node["'],\s*omxBin,\s*["']hud["'],\s*["']--watch["'],?\s*\]\)/,
		);
	});
	...
	describe("detached HUD deferred mutation guard", () => {
		...
		it("guards immediate and delayed resize sinks as bare tmux command-list commands", () => {
			...
			assert.doesNotMatch(
				hookCommand,
				/tmux resize-pane -t %77 -y 2 ; display-message/,
			);
		...
		it("escapes injected authority conditions so they survive the outer run-shell format pass", () => {
			...
			const hookCommand = guardDetachedHudDeferredMutation(
				leader,
				hud,
				args,
			).at(-1)!;
			...
			for (const condition of [
				...
				assert.ok(
					hookCommand.includes(condition),
					`stored payload must escape ${condition}`,
				);
			}
	...
	it("buildDetachedSessionBootstrapSteps starts native Windows with the persistent internal leader", () => {
		const steps = buildDetachedSessionBootstrapSteps(
			"omx-demo",
			"C:/project",
			buildWindowsDetachedChildCommand("codex", ["--model", "gpt-5"]),
			...
			"omx-session-123",
			"C:/project/.codex",
			"C:/project/.omx/runtime/codex-home/omx-session-123",
			undefined,
			process.env,
			undefined,
			undefined,
		...
		assert.doesNotMatch(
			leaderCmd!,
			/__detached-post-launch|Test-Path -LiteralPath/,
		);
	});

	it("buildDetachedSessionBootstrapSteps gives the POSIX leader persistent lifecycle ownership", () => {
		const releaseMarkerPath =
			"/tmp/project/.omx/runtime/detached-release/omx-session-123.release";
		const steps = buildDetachedSessionBootstrapSteps(
			"omx-demo",
			"/tmp/project",
			"'codex' '--model' 'gpt-5'",
		...
		assert.doesNotMatch(
			leaderCmd!,
			/__detached-post-launch|omx_codex_pid|while \[ ! -f/,
		);
	});
	...
	it("keeps capability data out of the detached leader command payload", () => {
		...
		assert.doesNotMatch(
			leaderCmd,
			/launch_lineage_token|directoryIdentity|FileHandle/,
		);
	});
		...
		try {
			const result = withTmuxExtendedKeys(
				...
				(_file, args) => {
					...
					if (args[0] === "show-options") {
						throw Object.assign(
							new Error("Command failed: tmux show-options -sv extended-keys"),
							{
								status: 1,
	...
	it("reapStaleNotifyFallbackWatcher skips kill when process identity does not match a watcher", async () => {
		...
		assert.equal(
			killed.length,
			0,
			"should not kill a process that is not a watcher",
		);
		await rm(cwd, { recursive: true, force: true });
	});
	...
	it("reapStaleNotifyFallbackWatcher sends SIGTERM only after confirming watcher identity", async () => {
		...
		await writeFile(
			pidPath,
			JSON.stringify({ pid: 12345, started_at: "2026-04-05T00:00:00.000Z" }),
		);
		...
		assert.deepEqual(
			killed,
			[12345],
			"should SIGTERM the verified watcher process",
		);
		await rm(cwd, { recursive: true, force: true });
	});
	...
	it("reapStaleNotifyFallbackWatcher skips recently started watcher records to avoid respawn loops", async () => {
		...
		await writeFile(
			pidPath,
			JSON.stringify({ pid: 24680, started_at: "2026-05-15T00:00:00.000Z" }),
		);
		...
		assert.equal(
			killed.length,
			0,
			"should not kill a watcher still inside the startup grace window",
		);
		await rm(cwd, { recursive: true, force: true });
	});
	...
	it("acquireTmuxExtendedKeysLease recovers from a stale lock left by a crashed process", async () => {
		...
		assert.equal(
			typeof lease,
			"string",
			"lease should succeed after stale lock recovery",
		);
	...
	it("acquireTmuxExtendedKeysLease reaps dead holders and restores before taking a new lease", async () => {
		...
		try {
			...
			const persisted = JSON.parse(await readFile(leasePath, "utf-8")) as {
				holders: Array<
					string | { id?: string; pid?: number; linuxStartTicks?: number }
				>;
			};
			...
			assert.equal(
				typeof holder === "object" ? holder.pid : process.pid,
				process.pid,
			);
	...
	it("releaseTmuxExtendedKeysLease restores when all remaining holders are dead", async () => {
		...
		try {
			...
			releaseTmuxExtendedKeysLease(
				cwd,
				"/tmp/dead-release.sock\tmissing-holder",
				execStub,
			);

			assert.ok(
				!existsSync(leasePath),
				"stale-only lease file should be removed",
			);
	...
	it("buildDetachedSessionFinalizeSteps keeps schedule after split-capture and before attach", () => {
		...
		const historyHook = steps.find(
			(step) => step.name === "register-detached-history-prune-hook",
		);
		assert.ok(historyHook);
		assert.deepEqual(historyHook.args.slice(0, 3), [
			"set-hook",
			"-t",
			"omx-demo",
		]);
	...
	it("buildDetachedSessionFinalizeSteps skips attach for Hermes MCP bridge launches", () => {
		...
		assert.equal(
			steps.some((step) => step.name === "attach-session"),
			false,
		);
		assert.equal(
			steps.some((step) => step.name === "register-resize-hook"),
			true,
		);
		assert.equal(
			steps.some((step) => step.name === "set-mouse"),
			true,
		);
	});
...
describe("buildTmuxPaneCommand", () => {
	it("wraps command with zsh without sourcing rc files by default", () => {
		...
		assert.ok(
			...
			"should not source .zshrc by default",
		);
	...
	it("prevents issue #2282 bash rc fan-out by default", () => {
		...
		assert.ok(!result.includes(" -lc "), "should not use a login shell");
		assert.ok(
			!result.includes("source ~/.bashrc"),
			"should not source .bashrc by default",
		);
		assert.ok(result.includes("exec "), "should exec the command");
	});
	...
	it("sources zsh and bash rc files only when explicitly opted in", () => {
		...
		assert.equal(
			shouldSourceTmuxPaneShellRc({ OMX_TMUX_SOURCE_SHELL_RC: "1" }),
			true,
		);
		assert.ok(
			buildTmuxPaneCommand("codex", [], "/usr/bin/zsh", {
				OMX_TMUX_SOURCE_SHELL_RC: "1",
			}).includes("source ~/.zshrc"),
			"opt-in zsh launches may source .zshrc",
		);
		assert.ok(
			buildTmuxPaneCommand("codex", [], "/bin/bash", {
				OMX_TMUX_SOURCE_SHELL_RC: "1",
			}).includes("source ~/.bashrc"),
...
describe("buildWindowsDetachedChildCommand", () => {
	it("exits with the Codex child status instead of retaining an interactive shell", () => {
		const result = buildWindowsDetachedChildCommand("codex", [
			"--model",
			"gpt-5",
		]);
		const prefix =
...
describe("Windows detached leader environment", () => {
	it("serializes inherited parent values into the detached Windows leader command", () => {
		...
		const encodedPayload = command.match(
...
describe("worktree dependency bootstrap helpers", () => {
	it("returns an explicit warning when reusable worktree dependencies are unavailable", () => {
		const result = ensureReusableNodeModules("/tmp/non-worktree", {
			gitRunner: () =>
				({ status: 1, stdout: "", stderr: "not a worktree" }) as any,
		});
		assert.equal(result.strategy, "missing");
		assert.match(
			String(result.warning || ""),
			/No reusable parent-repo node_modules was found/,
		);
	});
});
...
describe("isExistingTmuxWindowTooCrampedForLaunchHud (#2754)", () => {
	it("skips the launch-time HUD split for cramped existing tmux windows", () => {
		...
		assert.equal(
			isExistingTmuxWindowTooCrampedForLaunchHud(
				HUD_TMUX_MIN_LAUNCH_WINDOW_HEIGHT_LINES - 1,
			),
			true,
		);
	});
