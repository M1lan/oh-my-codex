...
async function writeActiveRunRecord(options: {
	...
	await writeFile(
		join(activeDir, `${options.contextKey}.json`),
		JSON.stringify(
			{
				version: 1,
...
describe("CLI session-scoped state parity", () => {
	it("status and cancel include session-scoped states", async () => {
		...
		try {
			...
			if (
				statusResult.error &&
				/(EPERM|EACCES)/i.test(statusResult.error.message)
			)
				return;
			assert.equal(
				statusResult.status,
			...
			assert.equal(
				cancelResult.status,
				0,
				cancelResult.stderr || cancelResult.stdout,
			);
			assert.match(cancelResult.stdout, /Cancelled: team/);

			const updated = JSON.parse(
				await readFile(join(scopedDir, "team-state.json"), "utf-8"),
			);
	...
	it("does not mutate an unmatched implicit OMX session, including force cleanup", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-cancel-unmatched-session-"),
		);
		try {
			...
			assert.deepEqual(JSON.parse(await readFile(ralphPath, "utf-8")), {
				active: true,
				current_phase: "executing",
			});
			assert.deepEqual(JSON.parse(await readFile(nativeStopPath, "utf-8")), {
				sessions: { [sessionId]: { last_signature: "ralph-stop|pending" } },
			});
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("ignores ambient Codex session aliases that do not own writable cancellation", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-cancel-codex-session-mismatch-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			const ownerState = JSON.stringify(
				{ active: true, mode: "ralplan", current_phase: "executing" },
				null,
				2,
			);
			const foreignState = JSON.stringify(
				{ active: true, mode: "team", current_phase: "team-exec" },
				null,
				2,
			);
			...
			for (const environmentName of ["CODEX_SESSION_ID", "SESSION_ID"]) {
				...
				const cancelResult = runOmxWithEnv(
					wd,
					{ [environmentName]: foreignSession },
					"cancel",
				);
				assert.equal(
					cancelResult.status,
				...
				assert.equal(
					JSON.parse(await readFile(ownerPath, "utf-8")).active,
					false,
				);
	...
	it("keeps root cancellation authoritative despite ambient session aliases", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-cancel-root-ambient-session-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			for (const environmentName of ["CODEX_SESSION_ID", "SESSION_ID"]) {
				...
				assert.equal(
					JSON.parse(await readFile(rootPath, "utf-8")).active,
					false,
				);
				assert.equal(await readFile(foreignPath, "utf-8"), foreignState);
			}
	...
	it("rejects symlinked ordinary-scope state targets", async () => {
		...
		const outside = await mkdtemp(
			join(tmpdir(), "omx-cli-cancel-root-symlink-outside-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
	...
	it("rejects symlinked session authority directories", async () => {
		if (process.platform === "win32") return;
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-cancel-session-root-symlink-"),
		);
		const outside = await mkdtemp(
			join(tmpdir(), "omx-cli-cancel-session-root-outside-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
	...
	it("denies wrong-typed and nested ownership contradictions transaction-wide", async () => {
		...
		try {
			...
			const modeState = JSON.stringify(
				{ active: true, mode: "autopilot", session_id: sessionId },
				null,
				2,
			);
			await mkdir(sessionDir, { recursive: true });
	...
	it("keeps root Team compatibility state read-only when current-session Ralplan owns cancellation", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-cancel-cross-mode-scope-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			assert.doesNotMatch(cancelResult.stdout, /Cancelled: team/);
			assert.equal(
				JSON.parse(await readFile(ralplanPath, "utf-8")).active,
				false,
			);
			assert.equal(await readFile(teamPath, "utf-8"), rootTeamState);
			assert.equal(
				await readFile(skillActivePath, "utf-8"),
				rootSkillActiveState,
			);
			assert.equal(
				await readFile(nativeStopPath, "utf-8"),
				rootNativeStopState,
			);
			...
			assert.equal(
				forceResult.status,
				0,
				forceResult.stderr || forceResult.stdout,
			);
	...
	it("replaces only a stale top-level skill owner under complete current native authority", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-cancel-stale-skill-owner-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			const rootSkill = JSON.stringify(
				{ active: true, skill: "team" },
				null,
				2,
			);
			await mkdir(sessionDir, { recursive: true });
			...
			await writeFile(
				join(stateDir, "sessions", nativeSessionId, "session-owner.json"),
				JSON.stringify(
					{
			...
			await writeFile(
				ralplanPath,
				JSON.stringify({ active: true, mode: "ralplan" }, null, 2),
			);
			...
			assert.equal(
				skill.active_skills[0].owner_codex_session_id,
				nativeSessionId,
			);
			...
			assert.equal(
				JSON.parse(await readFile(skillPath, "utf-8")).owner_codex_session_id,
				nativeSessionId,
			);
			...
			assert.equal(
				noActiveResult.status,
				0,
				noActiveResult.stderr || noActiveResult.stdout,
			);
			...
			await writeFile(
				join(displacedDir, "session-owner.json"),
				JSON.stringify(
					{
			...
			const activeRalplan = JSON.stringify(
				{ active: true, mode: "ralplan" },
				null,
				2,
			);
	...
	it("preserves active Team skill entries while cancelling Ralph", async () => {
		...
		try {
			...
			const teamState = JSON.stringify(
				{ active: true, mode: "team", current_phase: "team-exec" },
				null,
				2,
			);
			...
			await writeFile(
				ralphPath,
				JSON.stringify({ active: true, mode: "ralph" }, null, 2),
			);
			...
			assert.deepEqual(
				skill.active_skills.map((entry: { skill: string; active: boolean }) => [
					entry.skill,
					entry.active,
				]),
				[
			...
			assert.equal(
				JSON.parse(await readFile(ralphPath, "utf-8")).active,
				false,
			);
	...
	it("preserves a legacy top-level Team marker while cancelling Ralph", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-cancel-legacy-team-ralph-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			const teamPath = join(sessionDir, "team-state.json");
			const ralphPath = join(sessionDir, "ralph-state.json");
			const legacyTeamSkill = JSON.stringify(
				{
					active: true,
					skill: "team",
					phase: "team-exec",
					session_id: sessionId,
					active_skills: [{ skill: "code-review", active: false }],
				},
				null,
				2,
			);
			const activeTeam = JSON.stringify(
				{ active: true, mode: "team", session_id: sessionId },
				null,
				2,
			);
			...
			await writeFile(
				ralphPath,
				JSON.stringify(
					{ active: true, mode: "ralph", session_id: sessionId },
					null,
					2,
				),
			);
	...
	it("cancels valid skill-active-only workflow names without a duplicate allowlist", async () => {
		...
		try {
			...
			await writeFile(
				join(nativeOwnerDir, "session-owner.json"),
				JSON.stringify({
					session_id: nativeSessionId,
					native_session_id: nativeSessionId,
					cwd: wd,
	...
	it("preserves mode-only cancellation without creating a skill marker", async () => {
		...
		try {
			...
			await writeFile(
				modePath,
				JSON.stringify(
					{ active: true, mode: "ralplan", session_id: sessionId },
					null,
					2,
				),
			);
	...
	it("reports rollback restoration failures without false cancellation success", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-cancel-rollback-failure-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
	...
	it("fails Ralplan preflight without mutating unproven session state", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-ralplan-preflight-scope-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			assert.equal(
				...
				1,
				preflightResult.stderr || preflightResult.stdout,
			);
	...
	it("neutralizes exact-owner routing-only Ralplan before preflight denial", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-ralplan-preflight-owner-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			const rootTeam = JSON.stringify(
				{ active: true, mode: "team", current_phase: "team-exec" },
				null,
				2,
			);
			const rootSkill = JSON.stringify(
				{ active: true, skill: "team", phase: "team-exec" },
				null,
				2,
			);
			const rootStop = JSON.stringify(
				{ sessions: { foreign: { pending: true } } },
				null,
				2,
			);
			...
			const preflightResult = runOmxWithEnv(
				wd,
			...
			assert.equal(await readFile(ralplanPath, "utf-8"), ralplanState);
			assert.equal(
				await readFile(sessionSkillPath, "utf-8"),
				sessionSkillState,
			);
	...
	it("status does not report a root fallback mode as active after current-session clear", async () => {
		...
		try {
			...
			assert.equal(
				clearResult.status,
				0,
				clearResult.stderr || clearResult.stdout,
			);
			...
			assert.match(
				statusResult.stdout,
				/deep-interview: inactive \(phase: cleared\)/,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("does not cancel a foreign hook-visible run-dir from session-owned scope", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-cancel-foreign-owner-"),
		);
		const runsRoot = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-cancel-foreign-runs-"),
		);
		try {
			...
			await mkdir(join(stateDir, "sessions", ownerSessionId), {
				recursive: true,
			});
			await mkdir(foreignSessionDir, { recursive: true });
			await writeFile(
				join(stateDir, "session.json"),
				JSON.stringify({ session_id: ownerSessionId }),
			);
			await writeFile(
				join(runStateDir, "session.json"),
				JSON.stringify({ session_id: foreignSessionId }),
			);
			...
			assert.match(
				cancelResult.stderr,
				/session\.json is present but unusable/,
			);
	...
	it("keeps stale registry-only run state read-only during root cancellation", async () => {
		...
		const runsRoot = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-stale-runs-"),
		);
		try {
	...
	it("rejects a live run record whose run directory escapes through a symlink", async () => {
		...
		const runsRoot = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-symlink-runs-"),
		);
		const outsideRoot = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-symlink-outside-"),
		);
		try {
			...
			await mkdir(join(wd, ".omx", "state"), { recursive: true });
			await writeFile(
				join(outsideStateDir, "session.json"),
				JSON.stringify({ session_id: sessionId }),
			);
			await writeFile(statePath, state);
	...
	it("rejects nested session-directory and state-file symlink escapes", async () => {
		...
		for (const kind of ["session-dir", "state-file"] as const) {
			...
			const runsRoot = await mkdtemp(
				join(tmpdir(), `omx-cli-nested-${kind}-runs-`),
			);
			const outsideRoot = await mkdtemp(
				join(tmpdir(), `omx-cli-nested-${kind}-outside-`),
			);
			try {
				...
				const state = JSON.stringify(
					{ active: true, mode: "autopilot", current_phase: "executing" },
					null,
					2,
				);
				const fakeBin = await createFakeTmuxBin(wd);
				...
				assert.match(
					cancelResult.stderr,
					/detached run (?:state )?authority is invalid/,
				);
	...
	it("denies the entire run transaction when one candidate state is malformed", async () => {
		...
		const runsRoot = await mkdtemp(
			join(tmpdir(), "omx-cli-run-malformed-runs-"),
		);
		try {
	...
	it("fails closed when multiple live run records match the same root", async () => {
		if (process.platform === "win32") return;
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-duplicate-owner-"),
		);
		const runsRoot = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-duplicate-runs-"),
		);
		try {
			...
			for (const [index, mode] of ["autopilot", "ralplan"].entries()) {
				...
				const content = JSON.stringify(
					{ active: true, mode, current_phase: "executing" },
					null,
					2,
				);
				await mkdir(dirname(statePath), { recursive: true });
	...
	it("cancels hook-visible run-dir session state when worktree state list-active is empty", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-cancel-worktree-"),
		);
		const runsRoot = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-cancel-runs-"),
		);
		try {
			if (process.platform === "win32") return;
			...
			await writeFile(
				join(runSessionDir, "autopilot-state.json"),
				JSON.stringify(
					{
						active: true,
						mode: "autopilot",
			...
			await writeFile(
				join(runSessionDir, "skill-active-state.json"),
				JSON.stringify(
					{
						version: 1,
						active: true,
			...
			const autopilot = JSON.parse(
				await readFile(join(runSessionDir, "autopilot-state.json"), "utf-8"),
			);
			assert.equal(autopilot.active, false);
			assert.equal(autopilot.current_phase, "cancelled");
			assert.ok(
				typeof autopilot.completed_at === "string" &&
					autopilot.completed_at.length > 0,
			);
			assert.equal(
				await readFile(rootAutopilotPath, "utf-8"),
				rootAutopilotState,
			);

			const skillActive = JSON.parse(
				await readFile(join(runSessionDir, "skill-active-state.json"), "utf-8"),
			);
			...
			assert.deepEqual(
				skillActive.active_skills.map(
					(skill: { active: unknown; phase: unknown }) => ({
						active: skill.active,
						phase: skill.phase,
					}),
				),
	...
	it("reports hook-visible run-dir session state in status when worktree state list-active is empty", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-status-worktree-"),
		);
		const runsRoot = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-status-runs-"),
		);
		try {
			...
			assert.match(
				statusResult.stdout,
				/autopilot: ACTIVE \(phase: deep-interview\)/,
			);
	...
	it("finds hook-visible run-dir session state from an explicit worktree_cwd alias", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-worktree-alias-source-"),
		);
		const worktree = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-worktree-alias-wt-"),
		);
		const runsRoot = await mkdtemp(
			join(tmpdir(), "omx-cli-run-dir-worktree-alias-runs-"),
		);
		try {
			if (process.platform === "win32") return;
	...
	it("reports stale current-autopilot in status when no authoritative active modes exist", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-status-stale-current-autopilot-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
			await writeFile(
				join(stateDir, "session.json"),
				JSON.stringify({ session_id: "sess-stale-autopilot" }, null, 2),
			);
			...
			await writeFile(
				...
				JSON.stringify(currentAutopilot, null, 2),
			);
			...
			assert.deepEqual(JSON.parse(listResult.stdout), { active_modes: [] });
			assert.deepEqual(
				JSON.parse(await readFile(currentAutopilotPath, "utf-8")),
				currentAutopilot,
			);
			assert.equal(
				statusResult.stdout.trim(),
				"autopilot: STALE (phase: complete)",
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("reports stale current-autopilot alongside inactive authoritative modes only", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-status-stale-current-autopilot-with-inactive-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
	...
	it("prefers authoritative active autopilot over stale current-autopilot in status", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-status-active-autopilot-precedence-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			await writeFile(
				join(stateDir, "current-autopilot.json"),
				JSON.stringify(
					{
						active: true,
						current_phase: "complete",
	...
	it("ignores unreportable current-autopilot in status", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-status-unreportable-current-autopilot-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("preserves active Ultragoal mode status when durable failed artifacts coexist", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-status-active-ultragoal-failed-artifact-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			assert.match(
				statusResult.stdout,
				/ultragoal: ACTIVE \(phase: executing\)/,
			);
			assert.doesNotMatch(
				statusResult.stdout,
	...
	it("cancels linked ultrawork when Ralph is active", async () => {
		...
		try {
			...
			const ultrawork = JSON.parse(
				await readFile(join(sessionDir, "ultrawork-state.json"), "utf-8"),
			);
	...
	it("cancels linked ecomode before Ralph", async () => {
		...
		try {
			...
			await writeFile(
				join(sessionDir, "ecomode-state.json"),
				JSON.stringify({
					active: true,
					mode: "ecomode",
					session_id: sessionId,
			...
			assert.equal(
				JSON.parse(
					await readFile(join(sessionDir, "ecomode-state.json"), "utf-8"),
				).active,
				false,
			);
			assert.equal(
	...
	it("does not mutate unrelated sessions when cancelling current session mode", async () => {
		...
		try {
			...
			await writeFile(
				join(stateDir, "session.json"),
				JSON.stringify({ session_id: "sessA", cwd: wd, state_root: stateDir }),
			);

			await writeFile(
				join(sessionA, "ralph-state.json"),
				JSON.stringify({
					active: true,
					current_phase: "executing",
					started_at: "2026-02-22T00:00:00.000Z",
			...
			await writeFile(
				join(sessionB, "ralph-state.json"),
				JSON.stringify({
					active: true,
					current_phase: "executing",
					started_at: "2026-02-22T00:00:00.000Z",
			...
			const aState = JSON.parse(
				await readFile(join(sessionA, "ralph-state.json"), "utf-8"),
			);
			const bState = JSON.parse(
				await readFile(join(sessionB, "ralph-state.json"), "utf-8"),
			);
	...
	it("clears current-session autopilot and skill mirrors even when canonical root is inactive", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-cli-clear-stale-autopilot-mirror-"),
		);
		try {
			const stateDir = join(wd, ".omx", "state");
			...
			await writeFile(
				...
				JSON.stringify(
					{
						sessions: {
							[sessionId]: { last_signature: "autopilot-stop|stale" },
						},
					},
					null,
					2,
			...
			const clearAutopilot = runOmx(
				wd,
				"state",
				"clear",
				"--input",
				`{"mode":"autopilot","session_id":"${sessionId}"}`,
				"--json",
			);
			assert.equal(
				clearAutopilot.status,
				0,
				clearAutopilot.stderr || clearAutopilot.stdout,
			);
			const clearSkill = runOmx(
				wd,
				"state",
				"clear",
				"--input",
				`{"mode":"skill-active","session_id":"${sessionId}"}`,
				"--json",
			);
			assert.equal(
				clearSkill.status,
				0,
				clearSkill.stderr || clearSkill.stdout,
			);
			const cancelForce = runOmx(wd, "cancel", "--force");
			assert.equal(
				cancelForce.status,
				0,
				cancelForce.stderr || cancelForce.stdout,
			);
			...
			const nativeStop = JSON.parse(
				await readFile(join(sessionDir, "native-stop-state.json"), "utf-8"),
			);
