...
function buildRunOmxEnv(
	envOverrides: Record<string, string>,
): NodeJS.ProcessEnv {
	const env: NodeJS.ProcessEnv = { ...process.env };
	const muxBinary = env.OMX_MUX_BINARY;
	...
	return {
		...env,
		...(muxBinary ? { OMX_MUX_BINARY: muxBinary } : {}),
		...envOverrides,
	};
}
...
async function wrapFakeTmuxWithDetachedLeader(
	fakeTmuxPath: string,
	leaderCommandPrefix = "",
): Promise<void> {
...
if [ "$status" -eq 0 ] && [ "$1" = "new-session" ]; then
  ...
  ${leaderCommandPrefix ? `${leaderCommandPrefix} ` : ""}TMUX=/tmp/omx-test-tmux,1,0 TMUX_PANE="$pane" nohup /bin/sh -c "$last_arg" </dev/null >/tmp/omx-test-detached-leader.log 2>&1 &
  leader_pid=$!
...
function startHeldOmx(
	...
	return spawn(
		process.execPath,
		[join(repoRoot, "dist", "cli", "omx.js"), "--direct", "--version"],
		{
			cwd,
			env: buildRunOmxEnv(envOverrides),
			stdio: "inherit",
...
async function assertShimLogQuiescent(
	path: string,
	quietPeriodMs: number,
	message: string,
): Promise<void> {
	const deadline = Date.now() + Math.max(5_000, quietPeriodMs * 20);
	let previous = await readFile(path, "utf-8").catch(() => "");
	let unchangedSince = Date.now();
	while (Date.now() <= deadline) {
		await new Promise((resolve) =>
			setTimeout(resolve, Math.min(50, quietPeriodMs)),
		);
		...
		if (Date.now() - unchangedSince >= quietPeriodMs) return;
	}
	assert.fail(message);
}
...
function skipUnlessPrivateRealTmux(t: TestContext): boolean {
	...
	assert.equal(
		process.env.CI,
		undefined,
		"CI must provide tmux for the private-server detached launch regression",
	);
...
function skipUnlessScriptAvailable(t: TestContext): boolean {
	if (process.platform === "win32") {
		t.skip("PTY launch harness requires posix script(1)");
		return false;
	}
	...
	assert.equal(
		process.env.CI,
		undefined,
		"CI must provide script(1) for the PTY launch regression",
	);
...
`,
	...
	it("reports a missing Codex executable instead of exiting silently", async () => {
		...
		try {
			...
			assert.match(
				result.stderr,
				/failed to launch codex: executable not found in PATH/,
			);
	...
	it("launches codex directly without tmux ENOENT noise", async () => {
		...
		try {
			...
			assert.match(
				result.stdout,
				/fake-codex:.*--dangerously-bypass-approvals-and-sandbox/,
			);
			assert.match(
				result.stdout,
				/fake-codex:.*model_reasoning_effort="xhigh"/,
			);
	...
	it("passes literal max and ultra after -- with raw config args to Codex launch unchanged", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-launch-reasoning-passthrough-"),
		);
		try {
			const home = join(wd, "home");
			const fakeBin = join(wd, "bin");
			...
			const capturedArgv = JSON.parse(
				await readFile(fakeCapturePath, "utf-8"),
			) as string[];
			...
			assert.match(
				...
				/\.omx\/state\/sessions\/omx-[^"]+\/AGENTS\.md"$/,
			);
			assert.equal(
				capturedArgv.filter((arg) => arg.startsWith("model_instructions_file="))
					.length,
				1,
			);
...
`,
			...
			for (const testCase of cases) {
				const result = runOmx(wd, ["--direct", ...testCase.args], {
					HOME: home,
					PATH: `${fakeBin}:/usr/bin:/bin`,
					NODE_BINARY: process.execPath,
					OMX_AUTO_UPDATE: "0",
				...
				assert.equal(
					result.status,
					0,
					`${JSON.stringify(testCase.args)}: ${result.error || result.stderr || result.stdout}`,
				);
...
describe("ordinary launch root collision guidance", () => {
	it("keeps the cwd default for the first launch and fails the second and third launches closed with explicit-root guidance", async () => {
		...
		try {
			...
			if (
				shouldSkipForSpawnPermissions(second.error) ||
				shouldSkipForSpawnPermissions(third.error)
			)
				return;
			for (const result of [second, third]) {
				...
				assert.match(
					result.stderr,
					/concurrent conversations in this checkout require distinct user-specified OMX_ROOT values/,
				);
				assert.match(
					result.stderr,
					/POSIX: OMX_ROOT="\$HOME\/\.omx\/instances\/second-conversation" omx/,
				);
				assert.match(
					result.stderr,
					/PowerShell: \$env:OMX_ROOT = "\$HOME\/\.omx\/instances\/second-conversation"; omx/,
				);
				assert.match(
					result.stderr,
					/cmd\.exe: set "OMX_ROOT=%USERPROFILE%\\\.omx\\instances\\second-conversation" && omx/,
				);
				assert.match(
					result.stderr,
					/OMX does not reroute or allocate one automatically/,
				);
			}
	...
	it("keeps explicit roots literal: distinct roots launch independently while a shared root remains fatal without reroute guidance", async () => {
		...
		try {
			...
			assert.deepEqual(
				new Set(
					(await readFile(fixture.rootsPath, "utf-8")).trim().split("\n"),
				),
				new Set([firstRoot, secondRoot]),
			);

			const collision = runOmx(wd, ["--direct", "--version"], {
				...fixture.env,
				OMX_ROOT: firstRoot,
			});
			...
			assert.doesNotMatch(
				collision.stderr,
				/reroute or allocate one automatically/,
			);
	...
	it("keeps different checkout defaults independent and fails closed on stale default-pointer evidence", async () => {
		...
		try {
			...
			const firstEstablished = await establishLaunchSessionBinding(
				firstCheckout,
				"first-checkout-owner",
				{ pid: process.pid },
			);
			const secondEstablished = await establishLaunchSessionBinding(
				secondCheckout,
				"second-checkout-owner",
				{ pid: process.pid },
			);
			assert.equal(firstEstablished.kind, "committed-released");
			assert.equal(secondEstablished.kind, "committed-released");
			if (
				firstEstablished.kind !== "committed-released" ||
				secondEstablished.kind !== "committed-released"
			)
				return;
			...
			assert.match(
				await readFile(
					join(firstCheckout, ".omx", "state", "session.json"),
					"utf-8",
				),
				/first-checkout-owner/,
			);
			assert.match(
				await readFile(
					join(secondCheckout, ".omx", "state", "session.json"),
					"utf-8",
				),
				/second-checkout-owner/,
			);
			...
			assert.notEqual(
				relaunch.status,
				0,
				relaunch.error || relaunch.stderr || relaunch.stdout,
			);
			...
			if (firstBinding)
				await closeLaunchSessionBindingOnce(firstBinding).catch(() => {});
			if (secondBinding)
				await closeLaunchSessionBindingOnce(secondBinding).catch(() => {});
			await rm(wd, {
				recursive: true,
				force: true,
				maxRetries: 20,
...
`,
			...
			assert.match(
				...
				new RegExp(
					`fake-codex-omx-root:${escapeRegExp(normalizeDarwinTmpPath(repo))}`,
				),
			);
	...
	it("preserves explicit OMX_ROOT for launch worktree state", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-launch-worktree-explicit-root-"),
		);
		try {
			const repo = await createGitRepo(wd);
...
`,
			...
			assert.match(
				result.stdout,
				new RegExp(`fake-codex-omx-root:${escapeRegExp(explicitRoot)}`),
			);
	...
	it("keeps direct madmax worktree launches bound to the boxed run root", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-launch-madmax-worktree-root-"),
		);
		try {
			const repo = await createGitRepo(wd);
...
`,
			...
			const result = runOmx(
				repo,
				["--direct", "--madmax", "--worktree", "--version"],
				{
					HOME: home,
					PATH: `${fakeBin}:/usr/bin:/bin`,
					OMX_AUTO_UPDATE: "0",
			...
			assert.match(
				normalizedStdout,
				new RegExp(
					`fake-codex-pwd:${escapeRegExp(normalizeDarwinTmpPath(worktreePath))}`,
				),
			);
			...
			assert.match(
				boxedRoot,
				new RegExp(`^${escapeRegExp(normalizeDarwinTmpPath(runs))}/run-`),
			);
			...
			assert.match(
				normalizedStdout,
				new RegExp(
					`fake-codex-source:${escapeRegExp(normalizeDarwinTmpPath(repo))}`,
				),
			);
...
`,
			...
			assert.doesNotMatch(
				result.stderr,
				/failed to attach detached tmux session/,
			);
		} finally {
			await rm(wd, {
				recursive: true,
	...
	it("restores an enlarged detached HUD on the first larger client attach", async (t) => {
		...
		try {
			await withTempTmuxSession(async (fixture) => {
				...
				const launchedSession = fixture
					.run(["list-sessions", "-F", "#{session_name}"])
					.split("\n")
					.find(
						(name) => name.startsWith("omx-") && name !== fixture.sessionName,
					);
				assert.ok(
					launchedSession,
					"compiled launcher must create its detached session on the private server",
				);
				...
				const panes = fixture
					.run([
						...
						launchedSession,
						"-F",
						"#{pane_id}\t#{pane_height}\t#{pane_start_command}",
					])
					.split("\n")
					.map((line) => line.split("\t"));
				const hud = panes.find(([, , command]) =>
					command.includes("OMX_DETACHED_HUD_OPERATION="),
				);
				assert.ok(hud, "HUD split must carry its operation marker");
				assert.equal(
					Number(hud[1]),
					HUD_TMUX_HEIGHT_LINES,
					"authorized direct reconciliation must resize the HUD",
				);
				assert.equal(
					fixture.run([
						"show-options",
				...
				assert.equal(
					storedPaneFormat,
					"##{pane_id}\\t##{pane_dead}\\t##{pane_pid}",
					"stored hook display must retain one outer format escape and encode TAB bytes",
				);
				const executablePaneFormat = storedPaneFormat
					.replaceAll("##{", "#{")
					.replaceAll("\\t", "\t");
				assert.equal(
					executablePaneFormat,
					"#{pane_id}\t#{pane_dead}\t#{pane_pid}",
					"one outer tmux format pass must decode the stored format into the executable nested argv",
				);
				const clientAttachedHookSlots = [
					...hooks.matchAll(/client-attached\[[0-9]+\]/g),
				].map(([slot]) => slot);
				for (const hookSlot of clientAttachedHookSlots) {
					fixture.run(["set-hook", "-u", "-t", launchedSession, hookSlot]);
				}
				const installedHooks = fixture.run([
					"show-hooks",
					"-t",
					launchedSession,
				]);
				const storedResizeHookSlots = [
					...installedHooks.matchAll(/client-resized\[[0-9]+\]/g),
				].map(([slot]) => slot);
				assert.equal(
					storedResizeHookSlots.length,
					1,
					"exactly one installed client-resized hook must remain for the natural resize trigger",
				);
				assert.match(installedHooks, /client-resized\[[0-9]+\].*run-shell -b/);
				assert.doesNotMatch(
					installedHooks,
					/client-attached\[[0-9]+\]/,
					"the resize trigger must not be conflated with client-attached hook execution",
				);

				await new Promise((resolve) =>
					setTimeout(resolve, HUD_RESIZE_RECONCILE_DELAY_SECONDS * 1_000 + 250),
				);
				const launchPaneSnapshots = parseShimTmuxArgv(
					await readFile(shimLogPath, "utf-8"),
				...
				assert.equal(
					await readFile(shimLogPath, "utf-8"),
					"",
					"the shim baseline must start after launch and reconciliation activity settles",
				);
				...
				assert.equal(
					...
					7,
					"the detached HUD must be enlarged before the restore trigger",
				);
				assert.equal(
					fixture.run([
						"display-message",
					...
					"80x24",
					"enlarging the detached HUD must not attach a client",
				);
				...
				assert.equal(
					...
					HUD_TMUX_HEIGHT_LINES,
					"the first larger client attach must restore the enlarged detached HUD",
				);

				const paneSnapshots = parseShimTmuxArgv(
					await readFile(shimLogPath, "utf-8"),
				).filter((argv) => argv[0] === "list-panes" && argv[1] === "-a");
				assert.equal(
		...
		try {
			await withTempTmuxSession({ serverLog: true }, async (fixture) => {
				...
				assert.equal(
					probe.error,
					"",
					"the positive-control tmux client must start successfully",
				);
				assert.notEqual(
					probe.status,
					0,
					"tmux must reject the deliberately malformed selected command list",
				);
				assert.ok(
				...
				assert.equal(
					fixture.sessionExists(),
					true,
					"the private tmux server must remain alive after the malformed client command",
				);
				assert.equal(
					fixture.run([
						"display-message",
				...
				await waitForServerLogSubstring(
					fixture.readServerLog,
					probeOffset,
				...
				assert.equal(
					...
					7,
					"the detached HUD must be enlarged before the parser oracle trigger",
				);
				fixture.triggerClientResize(launchedSession, { cols: 121, rows: 41 });
				await new Promise((resolve) =>
					setTimeout(resolve, HUD_RESIZE_RECONCILE_DELAY_SECONDS * 1_000 + 250),
				...
				const triggerWindow = (await fixture.readServerLog()).slice(
					triggerOffset,
				);
				assert.ok(
					triggerWindow.includes(
						`resize-pane -t ${hud[0]} -y ${HUD_TMUX_HEIGHT_LINES}`,
					),
	...
	it("denies detached HUD mutation after the pane owner changes", async (t) => {
		...
		try {
			await withTempTmuxSession(async (fixture) => {
				...
				const foreignBefore = fixture.run([
					"list-panes",
					"-t",
					foreignSession,
					"-F",
					"#{pane_id}\t#{pane_pid}\t#{pane_height}",
				]);
				...
				try {
					...
					await new Promise((resolve) =>
						setTimeout(
							resolve,
							HUD_RESIZE_RECONCILE_DELAY_SECONDS * 1_000 + 250,
						),
					);
					const launchPaneSnapshots = parseShimTmuxArgv(
						await readFile(shimLogPath, "utf-8"),
					...
					assert.ok(
						originalOwner,
						"the detached session must retain its original pane owner before the denial trigger",
					);
					fixture.run(["resize-pane", "-t", hud[0]!, "-y", "7"]);
					assert.equal(
						Number(
						...
						7,
						"the detached HUD must be enlarged before the authority denial trigger",
					);
					fixture.run([
						"set-option",
						"-t",
					...
					assert.equal(
						...
						7,
						"the foreign pane owner must deny the deferred HUD mutation",
					);
					assert.equal(
						fixture.run([
							"list-panes",
						...
						foreignBefore,
						"the denial trigger must not affect a foreign session",
					);
				} finally {
					if (
						launchedSession &&
						originalOwner &&
						fixture.sessionExists(launchedSession)
					) {
						fixture.run([
							"set-option",
							"-t",
						...
						assert.equal(
							...
							originalOwner,
							"the original detached pane owner must be restored before fixture teardown",
						);
					}
				}
			});
...
`,
			...
			assert.equal(
				first.status,
				0,
				first.error || first.stderr || first.stdout,
			);
			...
			assert.equal(
				second.status,
				0,
				second.error || second.stderr || second.stdout,
			);
			assert.match(
				second.stderr,
				/madmax detached launch already active for this context/,
			);
			...
			const activeRecord = await readFile(
	...
	it("records boxed runtime identity in the leader-owned active record", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-launch-madmax-worktree-detached-"),
		);
		try {
			const repo = await createGitRepo(wd);
			const runs = join(wd, "runs");
...
`,
			...
			const activeRecord = JSON.parse(
				await readFile(join(runs, "active-detached", activeFiles[0]!), "utf-8"),
			) as {
				source_cwd: string;
				worktree_cwd?: string;
				leader_pid?: number;
			...
			assert.equal(
				normalizeDarwinTmpPath(activeRecord.source_cwd),
				normalizeDarwinTmpPath(worktreePath),
			);
			assert.equal(typeof activeRecord.leader_pid, "number");
			assert.equal(activeRecord.lifecycle_phase, "ready");
			const tmuxLog = normalizeDarwinTmpPath(
			...
			assert.match(
				tmuxLog,
				new RegExp(
					`-e OMX_SOURCE_CWD=${escapeRegExp(normalizeDarwinTmpPath(repo))}`,
				),
			);
...
`,
			...
			assert.doesNotMatch(
				tmuxLog,
				/tmux:set-option .* -t user-owned-session .*history-limit/,
			);
			assert.doesNotMatch(
				tmuxLog,
				/tmux:clear-history .*user-owned-session|tmux:clear-history .*%99/,
			);
			assert.match(tmuxLog, /tmux:new-session /);
		} finally {
			await rm(wd, {
	...
	it("does not reuse the same active-detached lock for independent --madmax --high launches", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-launch-madmax-independent-high-"),
		);
		try {
			const runs = join(wd, "runs");
...
`,
			...
			if (
				shouldSkipForSpawnPermissions(first.error) ||
				shouldSkipForSpawnPermissions(second.error)
			)
				return;
			assert.equal(
			...
			const registryEntries = (
				await readFile(join(runs, "registry.jsonl"), "utf-8")
			)
				.trim()
				.split("\n")
			...
			const activeDetachedEntries = await readdir(
				join(runs, "active-detached"),
			);
			assert.equal(
				activeDetachedEntries.filter((entry) => entry.endsWith(".lock")).length,
				0,
			);
...
`,
	...
	it("denies detached HUD finalization when the receipted pane id is recycled", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-launch-detached-hud-recycle-"),
		);
		try {
			const { env, tmuxLogPath } = await createLaunchFixture(
				wd,
...
`,
			...
			assert.match(
				tmuxLog,
				/tmux:if-shell -F -t %12 .*OMX_DETACHED_HUD_OPERATION=/,
			);
			assert.match(
				tmuxLog,
				/if-shell -F -t '%99' .*456.*\$1.*@1.*OMX_DETACHED_HUD_OPERATION=/,
			);
			assert.match(
				tmuxLog,
				/^tmux:__nested_hud_guard_denied__ %99 999 \$2 @2$/m,
			);
...
`,
			...
			assert.match(
				tmuxLog,
				new RegExp(
					`tmux:if-shell -F -t %12 .*set-option.*history-limit.*${DETACHED_TMUX_HISTORY_LIMIT}`,
				),
			);
			assert.match(
				tmuxLog,
				new RegExp(
					`tmux:if-shell -F -t %12 .*set-option.*-pq.*%12.*history-limit.*${DETACHED_TMUX_HISTORY_LIMIT}`,
				),
			);
			assert.match(
				tmuxLog,
			...
			assert.match(
				tmuxLog,
				new RegExp(
					`tmux:if-shell -F -t %12 .*split-window.*-v.*-l.*${HUD_TMUX_HEIGHT_LINES}.*-t.*%12`,
				),
			);
			assert.equal(
				result.status,
...
`,
			...
			await wrapFakeTmuxWithDetachedLeader(
				join(fakeBin, "tmux"),
				"env -u COLORTERM TERM=tmux-256color TERM_PROGRAM=tmux TERM_PROGRAM_VERSION=3.4 COLUMNS=211 LINES=77 TERMINFO=/tmp/server-terminfo TERMINFO_DIRS=/tmp/server-terminfo-dirs TERMCAP=server-termcap",
			);
...
`,
			...
			const result = runOmx(wd, ["--direct", "--madmax"], {
				...env,
				TMUX: "",
				TMUX_PANE: "",
			});
...
`,
			...
			assert.doesNotMatch(
				tmuxLog,
				/split-window|show-options|extended-keys|mouse on/,
			);
		} finally {
			await rm(wd, {
				recursive: true,
...
`,
			...
			assert.match(
				result.stderr,
				/detached launch safety failure during inert-session/,
			);
...
`,
			...
			assert.match(
				result.stderr,
				/detached launch safety failure during post-release-attach/,
			);
...
`,
			...
			assert.doesNotMatch(
				result.stderr,
				/Falling back to direct Codex launch|attach-session returned immediately/,
			);
			assert.match(tmuxLog, /tmux:attach-session -t /);
			assert.doesNotMatch(
				tmuxLog,
				/tmux:display-message -p -t .* #\{session_attached\}|tmux:kill-session -t/,
			);
		} finally {
			await rm(wd, {
				recursive: true,
...
`,
	...
	it("keeps finalization authority in the real detached leader process through child exit", async () => {
		...
		try {
			...
			const leader = spawn(
				process.execPath,
				[omxBin, "__detached-session-leader", payload],
				{
					cwd: wd,
					env: buildRunOmxEnv({
						HOME: home,
			...
			await writeFile(
				`${readyPath}.release`,
				`${JSON.stringify({ version: 1, kind: "release", nonce: `${ready.nonce}-foreign`, sessionId: ready.sessionId, sessionName: ready.sessionName, leaderPid: ready.leaderPid })}\n`,
			);
			await new Promise((resolveWait) => setTimeout(resolveWait, 100));
			assert.equal(existsSync(childReady), false);
			await writeFile(
				`${readyPath}.release`,
				`${JSON.stringify({ version: 1, kind: "release", nonce: ready.nonce, sessionId: ready.sessionId, sessionName: ready.sessionName, leaderPid: ready.leaderPid })}\n`,
			);
			...
			const descendantPid = Number.parseInt(
				(await readFile(descendantPidPath, "utf-8")).trim(),
				10,
			);
			assert.equal(
			...
			assert.equal(
				existsSync(join(wd, ".omx", "state", "detached-active-record.json")),
				true,
			);
			...
			assert.equal(
				existsSync(join(wd, ".omx", "state", "session.json")),
				false,
			);
			assert.equal(
				existsSync(join(wd, ".omx", "state", "detached-active-record.json")),
	...
	it("returns the attached client to the shell with the child status after a normal detached child exit", async (t) => {
		...
		try {
			await withTempTmuxSession(async (fixture) => {
				...
				await writeFile(
					join(home, ".omx", "state", "star-prompt.json"),
				...
				assert.equal(
					zero.status,
					0,
					`zero-status attached launch must return to the shell successfully:\n${zero.output}`,
				);
				assert.deepEqual(
					listOmxSessions(),
					[],
					"zero-status exit must destroy the owned detached session despite remain-on-exit=on",
				);
				...
				assert.equal(
					seven.status,
					7,
					`nonzero child status must propagate to the invoking shell:\n${seven.output}`,
				);
				assert.deepEqual(
					listOmxSessions(),
					[],
					"nonzero exit must destroy the owned detached session despite remain-on-exit=failed",
				);
			});
		} finally {
			await rm(wd, {
