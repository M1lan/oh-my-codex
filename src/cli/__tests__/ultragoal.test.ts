...
import { ULTRAGOAL_HELP, ultragoalCommand } from "../ultragoal.js";
...
describe("cli/ultragoal", () => {
		...
		await withCwd(async (cwd) => {
			...
			try {
				const result = await capture(() =>
					ultragoalCommand(["create-goals", "--brief", "- First milestone"]),
				);
		...
		await withCwd(async (cwd) => {
			...
			try {
				const created = await capture(() =>
					ultragoalCommand(["create-goals", "--brief", "- Original milestone"]),
				);
	...
	it("refuses mutating ultragoal commands from Team worker environments", async () => {
		const mutators: string[][] = [
			...
			[
				...
				"worker evidence",
				"--codex-goal-json",
				JSON.stringify({ goal: { objective: "x", status: "active" } }),
			],
			["complete-goals"],
			["complete"],
			["next"],
			...
			for (const args of mutators) {
				await withCwd(async (cwd) => {
					...
					try {
						...
						if (typeof previousPublic === "string")
							process.env.OMX_TEAM_WORKER = previousPublic;
						else delete process.env.OMX_TEAM_WORKER;
						if (typeof previousInternal === "string")
							process.env.OMX_TEAM_INTERNAL_WORKER = previousInternal;
	...
	it("creates and starts goals through the command surface", async () => {
		await withCwd(async (cwd) => {
			...
			assert.match(
				output,
				/omx ultragoal checkpoint --goal-id G001-first-milestone --status complete/,
			);
			...
			assert.match(
				goals.codexObjective ?? "",
				/Complete the durable ultragoal plan/,
			);
			assert.match(
				goals.codexObjective ?? "",
	...
	it("selects default, unsupported, and role-routing-unavailable conductor guidance from durable state", async () => {
		await withCwd(async (cwd) => {
			...
			const defaultGuidance = await capture(() =>
				ultragoalCommand(["complete-goals"]),
			);
			assert.equal(
				defaultGuidance.stdout.join("\n").includes(LEADER_CONDUCTOR_BLOCK),
				true,
			);
			...
			await writeFile(
				join(stateDir, NATIVE_SUBAGENT_SUPPORT_BLOCKER_FILE),
				`${JSON.stringify(unsupportedEvidence)}\n`,
				"utf-8",
			);

			const unsupportedGuidance = await capture(() =>
				ultragoalCommand(["complete-goals"]),
			);
			const unsupportedOutput = unsupportedGuidance.stdout.join("\n");
			assert.equal(
				unsupportedOutput.includes(
					buildUnsupportedNativeSubagentGuidance(unsupportedEvidence),
				),
				true,
			);
			...
			try {
				...
				assert.equal(
					existsSync(join(stateDir, NATIVE_SUBAGENT_ROLE_ROUTING_MARKER_FILE)),
					true,
				);

				const roleRoutingGuidance = await capture(() =>
					ultragoalCommand(["complete-goals"]),
				);
				const roleRoutingOutput = roleRoutingGuidance.stdout.join("\n");
				assert.equal(
					roleRoutingOutput.includes(
						buildRoleRoutingUnavailableGuidance(expectedRoleRoutingSupport),
					),
					true,
				);
				...
				assert.doesNotMatch(
					roleRoutingOutput,
					/Before Ralplan planner, reviewer, HUD, runtime, or delegation work/,
				);
			} finally {
				if (typeof previousSessionId === "string")
					process.env.OMX_SESSION_ID = previousSessionId;
		...
		await withCwd(async (cwd) => {
			...
			const checkpoint = await capture(() =>
				ultragoalCommand([
					...
					"--codex-goal-json",
					JSON.stringify({
						goal: { objective: goals.codexObjective, status: "complete" },
					}),
					"--quality-gate-json",
					cleanQualityGate(),
					"--json",
	...
	it("prints explicit terminal cleanup after final checkpoint without claiming hidden clear", async () => {
		await withCwd(async (cwd) => {
			await capture(() =>
				ultragoalCommand(["create-goals", "--brief", "- Final milestone"]),
			);
			await capture(() => ultragoalCommand(["complete-goals"]));
			const goals = JSON.parse(
				await readFile(join(cwd, ".omx/ultragoal/goals.json"), "utf-8"),
			...
			const checkpoint = await capture(() =>
				ultragoalCommand([
					"checkpoint",
					"--goal-id",
					"G001-final-milestone",
					"--status",
					"complete",
					"--evidence",
					"tests and final review passed",
					"--codex-goal-json",
					JSON.stringify({
						goal: { objective: goals.codexObjective, status: "complete" },
					}),
			...
			assert.match(
				output,
				/Terminal next step for another goal in this same Codex thread\/session: run \/goal clear/,
			);
			assert.match(
				output,
				/OMX shell commands and hooks do not call \/goal clear or hidden thread\/goal\/clear routes/,
			);
	...
	it("places completed-goal preflight remediation before create_goal guidance", async () => {
		await withCwd(async () => {
			...
			assert.match(
				output,
				/get_goal reports status complete before create_goal/,
			);
			assert.match(
				output,
				/Run \/goal clear in the Codex UI before starting another goal/,
			);
			assert.ok(
				output.indexOf("get_goal reports status complete before create_goal") <
					output.indexOf("create_goal payload"),
			);
			assert.match(
				output,
				/OMX did not and cannot clear hidden Codex goal state/,
			);
		});
	});
		...
		await withCwd(async (cwd) => {
			...
			const parsed = JSON.parse(status.stdout.join("\n")) as {
				...
				reconciliation?: {
					ok: boolean;
					warnings: string[];
					snapshot: { unavailableReason?: string };
				};
			};
			assert.equal(parsed.summary.complete, 1);
			...
			assert.equal(
				parsed.codexGoalFallback?.status,
				"codex_goal_reconciliation_unavailable",
			);
			assert.equal(parsed.codexGoalFallback?.reason, "db_schema_context_error");
			assert.match(
				parsed.codexGoalFallback?.message ?? "",
				/artifact-backed Ultragoal status remains available/,
			);
			assert.equal(parsed.reconciliation?.ok, true);
			assert.equal(
				parsed.reconciliation?.snapshot.unavailableReason,
			...
			assert.match(
				output,
				/codex goal fallback: Codex goal DB\/schema\/context is unavailable/,
			);
		...
		await withCwd(async () => {
			...
			await capture(() =>
				ultragoalCommand([
					...
					taskObjective,
					"--goal",
					"First::Synthetic slice 1.",
					"--goal",
					"Second::Synthetic slice 2.",
				]),
			);
			await capture(() => ultragoalCommand(["complete-goals"]));
	...
	it("steers ultragoal plans through structured CLI fields with audit output", async () => {
		await withCwd(async (cwd) => {
			await capture(() =>
				ultragoalCommand(["create-goals", "--brief", "- CLI bridge"]),
			);
			const steered = await capture(() =>
				ultragoalCommand([
					...
					"--objective",
					"Implement bounded prompt-submit bridge behavior.",
					"--evidence",
					".omx/ultragoal G002-cli-and-prompt-submit-bridge needs a structured CLI bridge before hook wiring.",
					"--rationale",
					"A structured CLI mutation keeps steering explicit and audited without broad natural-language mutation.",
					"--idempotency-key",
					"g002-cli-add-subgoal",
					"--json",
				]),
			);
	...
	it("dedupes structured CLI steering by idempotency key", async () => {
		await withCwd(async () => {
			...
			const args = [
				...
				"Implement bounded prompt-submit bridge behavior.",
				"--evidence",
				".omx/ultragoal G002-cli-and-prompt-submit-bridge evidence.",
				"--rationale",
				"Explicit structured steering should be idempotent.",
				"--idempotency-key",
				"same-cli-steer",
				"--json",
			];
	...
	it("rejects broad natural-language steering instead of guessing mutations", async () => {
		await withCwd(async () => {
			...
			const rejected = await capture(() =>
				ultragoalCommand(["steer", "please rewrite goals however seems best"]),
			);
			...
			const rejected = await capture(() =>
				ultragoalCommand([
					...
					JSON.stringify({
						...
						rationale:
							"Invalid mutation kinds must not bypass the structured allowlist.",
					}),
					"--json",
				]),
			);
		...
		await withCwd(async () => {
			...
			const invalid = await capture(() =>
				ultragoalCommand([
					...
					"forged",
					"--evidence",
					".omx/ultragoal G002 invalid source evidence.",
					"--rationale",
					"Invalid sources must not enter the steering audit.",
					"--json",
				]),
			);
			...
			const accepted = await capture(() =>
				ultragoalCommand([
					...
					"finding",
					"--evidence",
					".omx/ultragoal G002 reviewer finding evidence.",
					"--rationale",
					"The CLI should report the actual accepted audit source.",
					"--json",
				]),
			);
		...
		await withCwd(async () => {
			...
			const rejected = await capture(() =>
				ultragoalCommand([
					...
					"G999-missing",
					"--evidence",
					".omx/ultragoal G002-cli-and-prompt-submit-bridge invalid target evidence.",
					"--rationale",
					"This intentionally uses a missing goal to prove rejection audit output.",
					"--title",
					"New title",
					"--json",
				]),
	...
	it("steers an ultragoal mutation through structured flags", async () => {
		await withCwd(async (cwd) => {
			await capture(() =>
				ultragoalCommand(["create-goals", "--brief", "- Add and steer goal"]),
			);
			const result = await capture(() =>
				ultragoalCommand([
					"steer",
					...
					"add_subgoal",
					"--title",
					"Add steering telemetry",
					"--objective",
					"Add explicit steering bridge telemetry evidence to ultragoal.",
					"--evidence",
					"structured steer test",
					"--rationale",
					"Test should exercise bounded CLI steering path.",
					"--idempotency-key",
					"cli-steer-e2e",
					"--json",
				]),
		...
		await withCwd(async () => {
			...
			const result = await capture(() =>
				ultragoalCommand([
					...
					"No evidence",
					"--objective",
					"Missing evidence and rationale should fail.",
				]),
			);
			assert.equal(result.exitCode, 1);
			assert.match(result.stderr.join("\n"), /Missing --evidence/);
		...
		await withCwd(async (cwd) => {
			...
			const blocked = await capture(() =>
				ultragoalCommand([
					...
					"Resolve final code-review blockers",
					"--objective",
					"Fix blockers and rerun final gates.",
					"--evidence",
					"code-review REQUEST CHANGES",
					"--codex-goal-json",
					JSON.stringify({
						goal: { objective: goals.codexObjective, status: "active" },
					}),
					"--json",
				]),
			);
		...
		await withCwd(async (cwd) => {
			...
			const mismatch = await capture(() =>
				ultragoalCommand([
					...
					"tests passed",
					"--codex-goal-json",
					'{"goal":{"objective":"Different","status":"complete"}}',
				]),
			);
	...
	it("rejects null get_goal snapshots for completion without mutating OMX progress", async () => {
		await withCwd(async (cwd) => {
			...
			const plan = JSON.parse(
				await readFile(join(cwd, ".omx/ultragoal/goals.json"), "utf-8"),
			) as { activeGoalId?: string; goals: Array<{ status: string }> };
		...
		await withCwd(async (cwd) => {
			...
			const blocked = await capture(() =>
				ultragoalCommand([
					...
					"blocked",
					"--evidence",
					"completed aggregate Codex goal blocks create_goal",
					"--codex-goal-json",
					'{"goal":{"objective":"achieve all goals on this repo ultragoal status","status":"complete"}}',
					"--json",
				]),
			);
			...
			assert.equal(blocked.exitCode, undefined);
			const parsed = JSON.parse(blocked.stdout.join("\n")) as {
				summary: { inProgress: number; failed: number };
				plan: { activeGoalId?: string };
			};
		...
		await withCwd(async (cwd) => {
			...
			const blocked = await capture(() =>
				ultragoalCommand([
					...
					"blocked",
					"--evidence",
					"native matching blocked needs attention",
					"--codex-goal-json",
					blockedJson,
					"--json",
				]),
			...
			const reconParsed = JSON.parse(recon.stdout.join("\n")) as {
				reconciliation?: {
					ok?: boolean;
					snapshot?: { status?: string; raw?: { goal?: { status?: string } } };
					errors?: string[];
				};
			};
			assert.equal(reconParsed.reconciliation?.snapshot?.status, "blocked");
			assert.equal(
				reconParsed.reconciliation?.snapshot?.raw?.goal?.status,
				"blocked",
			);
			assert.equal(reconParsed.reconciliation?.ok, false);
			assert.match(
				(reconParsed.reconciliation?.errors ?? []).join("\n"),
				/got blocked/,
			);
			assert.doesNotMatch(
				(reconParsed.reconciliation?.errors ?? []).join("\n"),
		...
		await withCwd(async (cwd) => {
			...
			for (let attempt = 1; attempt <= 3; attempt += 1) {
				...
				if (attempt < 3) {
					...
					await capture(() =>
						ultragoalCommand(["complete-goals", "--retry-failed"]),
					);
				} else {
			...
			assert.match(
				output,
				/Required external decision: make the GHCR package public/,
			);
			...
			const plan = JSON.parse(
				...
				activeGoalId?: string;
				goals: Array<{ status: string; nonRetriable?: boolean }>;
			};
