...
async function makeRepo(): Promise<string> {
	...
	tempDirs.push(cwd);
	await mkdir(join(cwd, ".omx", "state", "sessions", "sess-q", "questions"), {
		recursive: true,
	});
	await writeFile(
		join(cwd, ".omx", "state", "session.json"),
		JSON.stringify({ session_id: "sess-q" }),
	);
	return cwd;
}

function makeQuestionCliEnv(
	cwd: string,
...
async function waitForQuestionRenderer(
	recordPath: string,
...
describe("omx question CLI", () => {
	...
	it("hard-fails worker contexts before UI launch", async () => {
		...
		assert.deepEqual(
			await readdir(
				join(cwd, ".omx", "state", "sessions", "sess-q", "questions"),
			),
			[],
		);
	});
	...
	it("blocks until an answer is written and returns structured payload", async () => {
		...
		const child = spawn(
			process.execPath,
			[omxBin, "question", "--input", input, "--json"],
			{
				cwd,
				env: makeQuestionCliEnv(cwd, {
					OMX_AUTO_UPDATE: "0",
		...
		assert.equal(
			record?.status,
			"prompting",
			`expected prompting question record, stderr=${stderr}`,
		);
	...
	it("bridges active autopilot deep-interview questions into waiting-for-user state until answered", async () => {
		...
		const waitingAutopilot = JSON.parse(
			...
			state?: {
				deep_interview_question?: { status?: string; previous_phase?: string };
			};
		};
		...
		assert.equal(
			waitingAutopilot.state?.deep_interview_question?.status,
			"waiting_for_user",
		);
		assert.equal(
			waitingAutopilot.state?.deep_interview_question?.previous_phase,
			"deep-interview",
		);
	...
	it("allows the owning spawned deep-interview question after Autopilot records the wait", async () => {
		...
		assert.equal(
			record?.status,
			"prompting",
			`expected owner question to prompt, stderr=${stderr}; stdout=${stdout}`,
		);
		...
		assert.equal(
			finalAutopilot.state?.deep_interview_question?.obligation_id,
			obligationId,
		);
		assert.equal(
			finalAutopilot.state?.deep_interview_question?.question_id,
	...
	it("omits legacy prompt and answer projections for batch payloads", async () => {
		...
		assert.equal(
			record?.status,
			"prompting",
			`expected prompting batch question record, stderr=${stderr}`,
		);
		await markQuestionAnswered(recordPath, [
			{
				question_id: "first",
		...
		assert.equal(payload.ok, true);
		assert.deepEqual(
			payload.answers.map((entry: any) => entry.answer.value),
			["a", "d"],
		);
		assert.equal(
...
`,
		...
		assert.match(
			...
			/pane %5 disappeared immediately after launch/i,
		);
...
`,
		...
		assert.match(
			payload.error.message,
			/renderer tmux-pane %45 exited before answering/i,
		);
	...
	it("times out unanswered test renderers instead of waiting forever", async () => {
		...
		assert.match(
			payload.error.message,
			/Timed out waiting for question answer after 50ms/i,
		);
	});
...
`,
		...
		await markQuestionAnswered(
			...
			},
			{
				injectAnswersToPane: (paneId, answers) => {
					injectedAnswers.push({
						paneId,
						answer: answers[0]?.answer.value ?? "",
					});
					return true;
				},
			},
	...
	it("runs inline in interactive Windows non-attached sessions instead of hard-failing on missing TMUX", async () => {
		...
		process.stdin.setRawMode = ((_: boolean) =>
			process.stdin) as unknown as typeof process.stdin.setRawMode;
		process.stdin.resume = (() =>
			process.stdin) as unknown as typeof process.stdin.resume;
		process.stdin.pause = (() =>
			process.stdin) as unknown as typeof process.stdin.pause;
		...
		try {
			...
			const waitForInlineFrame = async (pattern: RegExp, label: string) => {
				...
				throw new Error(
					`Timed out waiting for ${label}; stderr=${stderrWrites.join("")}`,
				);
			};

			await waitForInlineFrame(
				/↑↓ move · Enter select/,
				"single-question answering frame",
			);
			...
			assert.equal(entries.length, 1);
			const record = await readQuestionRecord(
				join(
					cwd,
					".omx",
					"state",
			...
			if (typeof originalOmxTeamStateRoot === "string")
				process.env.OMX_TEAM_STATE_ROOT = originalOmxTeamStateRoot;
