...
describe("resolveApiBinaryPath", () => {
	...
	it("checks Linux musl packaged paths before glibc and legacy paths", () => {
		assert.deepEqual(
			packagedApiBinaryCandidatePaths(fixturePackageRoot, "linux", "x64", {}, [
				"musl",
				"glibc",
			]),
			[
...
describe("omx api", () => {
	it("includes api in top-level help output", async () => {
		...
		try {
			...
			assert.match(
				result.stdout,
				/omx api\s+Run native omx-api localhost gateway commands \(serve\|status\|stop\|generate\)/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
		...
		try {
			for (const argv of [
				...
				assert.equal(
					result.status,
					0,
					`${argv.join(" ")} stderr=${result.stderr} stdout=${result.stdout}`,
				);
	...
	it("preserves child stdout, stderr, and exit code through the JS bridge", async () => {
		...
		try {
			...
			const result = runOmx(cwd, ["api", stubPath], {
				OMX_API_BIN: process.execPath,
			});
			if (shouldSkipForSpawnPermissions(result.error)) return;
	...
	it("fails clearly when the configured native binary path does not exist", async () => {
		...
		try {
			const result = runOmx(cwd, ["api", "status"], {
				OMX_API_BIN: join(cwd, "bin", "missing-api"),
			});
			if (shouldSkipForSpawnPermissions(result.error)) return;
