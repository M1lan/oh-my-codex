...
async function captureExploreCommand(
	args: string[],
): Promise<{ stdout: string; threw: boolean; error?: string }> {
...
describe("exploreCommand (hard-deprecated tombstone)", () => {
	...
	it("throws the deprecation message for bare invocation", async () => {
		...
		assert.match(
			result.error ?? "",
			/hard-deprecated and the direct command surface has been removed/,
		);
	});
	...
	it("throws the deprecation message for legacy --prompt usage", async () => {
		...
		assert.match(
			result.error ?? "",
			new RegExp(EXPLORE_DEPRECATION_MESSAGE.slice(0, 24)),
		);
	});
	...
	it("exposes deprecation help text", () => {
		...
		assert.match(
			EXPLORE_HELP,
			/normal Codex repository inspection tools\/subagents/,
		);
	});
});
...
describe("getBuiltinExploreHarnessUnsupportedReason", () => {
	...
	it("reports a reason on Windows without an override", () => {
		const reason = getBuiltinExploreHarnessUnsupportedReason(
			"win32",
			{} as NodeJS.ProcessEnv,
		);
	...
	it("returns undefined on Windows when OMX_EXPLORE_BIN is set", () => {
		assert.equal(
			getBuiltinExploreHarnessUnsupportedReason("win32", {
				OMX_EXPLORE_BIN: "/tmp/harness",
			} as NodeJS.ProcessEnv),
			undefined,
		);
	});
	...
	it("assert helper throws only on unsupported platforms", () => {
		assert.doesNotThrow(() =>
			assertBuiltinExploreHarnessSupported("linux", {} as NodeJS.ProcessEnv),
		);
		assert.throws(
			() =>
...
describe("resolvePackagedExploreHarnessCommand", () => {
	it("uses a packaged native binary when metadata matches the current platform", async () => {
		...
		try {
			...
			await writeFile(
				join(binDir, "omx-explore-harness.meta.json"),
				JSON.stringify({
	...
	it("ignores packaged binaries built for a different platform", async () => {
		...
		try {
			...
			await writeFile(
				join(binDir, packagedExploreHarnessBinaryName("linux")),
				"#!/bin/sh\nexit 0\n",
			);
