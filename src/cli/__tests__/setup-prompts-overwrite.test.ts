...
describe("omx setup prompt/native-agent overwrite behavior", () => {
	...
	it("installs setup-owned prompts separately from active/internal native agents", async () => {
		...
		try {
			...
			const installableNativeAgents = getInstallableNativeAgentNames(
				readCatalogManifest(),
			);
			...
			assert.equal(
				installedNativeAgents.has("performance-reviewer.toml"),
				false,
			);
			...
			assert.equal(
				installedNativeAgents.has("information-architect.toml"),
				false,
			);
			assert.equal(installedNativeAgents.has("product-analyst.toml"), false);

			const codeReviewerToml = await readFile(
				join(wd, ".codex", "agents", "code-reviewer.toml"),
				"utf-8",
			);
	...
	it("skips native agent TOMLs entirely during background update-check setup refreshes", async () => {
		const wd = await mkdtemp(join(tmpdir(), "omx-setup-prompts-"));
		const previousCwd = process.cwd();
		const previousSkipNativeAgentRefresh =
			process.env.OMX_SKIP_NATIVE_AGENT_REFRESH;
		try {
			await mkdir(join(wd, ".omx", "state"), { recursive: true });
			process.chdir(wd);
			...
			if (typeof previousSkipNativeAgentRefresh === "string")
				process.env.OMX_SKIP_NATIVE_AGENT_REFRESH =
					previousSkipNativeAgentRefresh;
	...
	it("preserves setup-owned prompt assets and removes unknown prompts on --force", async () => {
		...
		try {
			...
			for (const validPrompt of validPrompts) {
				assert.equal(
					existsSync(join(wd, ".codex", "prompts", validPrompt)),
					true,
				);
			}

			const unknownPromptPath = join(
				wd,
				".codex",
				"prompts",
				"unclassified-local.md",
			);
	...
	it("removes stale merged native agents on --force", async () => {
		...
		try {
			...
			for (const staleAgent of staleAgents) {
				assert.equal(
					existsSync(join(wd, ".codex", "agents", staleAgent)),
					false,
				);
			}
			assert.equal(
