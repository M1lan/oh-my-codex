...
describe("omx mission", () => {
	it("parses simple prompt lists and markdown checklists", () => {
		...
		assert.deepEqual(
			tasks.map((task) => [task.id, task.source_line, task.prompt]),
			[
	...
	it("writes a dry-run summary and ledger without executing tasks", async () => {
		await withTempDir(async (cwd) => {
			...
			const summary = JSON.parse(
				await readFile(
					join(cwd, ".omx", "missions", "demo", "summary.json"),
					"utf-8",
				),
			);
			...
			assert.deepEqual(
				summary.tasks.map((task: { status: string }) => task.status),
				["planned", "planned"],
			);
			const ledger = await readFile(
				join(cwd, ".omx", "missions", "demo", "ledger.jsonl"),
				"utf-8",
			);
	...
	it("runs tasks sequentially, records failures, and skips remaining tasks by default", async () => {
		await withTempDir(async (cwd) => {
			...
			await missionCommand(
				["run", "mission.md", "--slug", "demo", "--", "--model", "gpt-5"],
				{
					cwd,
					stdout: () => undefined,
	...
	it("reports status from an existing summary by file or slug", async () => {
		await withTempDir(async (cwd) => {
			...
			const byFile: string[] = [];
			await missionCommand(["status", "mission.md", "--slug", "demo"], {
				cwd,
				stdout: (line) => byFile.push(line),
			});
			assert.match(byFile.join("\n"), /mission status: demo \[planned\]/);
			assert.match(byFile.join("\n"), /\[planned\] task-001 line 1: First/);

			const bySlug: string[] = [];
			await missionCommand(["status", "demo"], {
				cwd,
				stdout: (line) => bySlug.push(line),
			});
			assert.match(
				bySlug.join("\n"),
	...
	it("reports invalid summary JSON as a mission error", async () => {
		await withTempDir(async (cwd) => {
			...
			assert.deepEqual(err, [
				`[mission] Invalid mission summary at ${summaryPath}.`,
			]);
	...
	it("resumes durable summaries by skipping passed tasks and retrying stale running work", async () => {
		await withTempDir(async (cwd) => {
			...
			await missionCommand(
				["run", "mission.md", "--slug", "demo", "--continue-on-error"],
				{
					cwd,
					stdout: () => undefined,
			...
			const seen: string[] = [];
			await missionCommand(
				["resume", "mission.md", "--slug", "demo", "--continue-on-error"],
				{
					cwd,
					stdout: () => undefined,
			...
			assert.deepEqual(
				resumed.tasks.map((task: { status: string }) => task.status),
	...
	it("reruns one requested task from the existing summary", async () => {
		await withTempDir(async (cwd) => {
			...
			const seen: string[] = [];
			await missionCommand(
				["rerun", "mission.md", "--slug", "demo", "--task", "task-002"],
				{
					cwd,
					stdout: () => undefined,
					runTask: async (prompt) => {
	...
	it("marks blocked and needs-human-review tasks in status counts", async () => {
		await withTempDir(async (cwd) => {
			...
			await missionCommand(
				["mark", "demo", "--task", "task-002", "--status", "blocked"],
				{ cwd, stdout: () => undefined },
			);
			await missionCommand(
				[
					"mark",
			...
			assert.match(
				status,
				/1\/3 passed, 0 failed, 0 skipped, 1 blocked, 1 needs-human-review, 0 planned/,
			);
	...
	it("leaves operator-marked tasks on resume and allows explicit rerun", async () => {
		await withTempDir(async (cwd) => {
			...
			assert.deepEqual(
				summary.tasks.map((task: { status: string }) => task.status),
				["passed", "passed", "needs-human-review"],
			);
			assert.equal(process.exitCode, 1);
			process.exitCode = undefined;
		});
