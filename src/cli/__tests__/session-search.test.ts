...
describe("omx session search", () => {
	...
	it("searches associated madmax boxed run roots without leaking raw run paths", async () => {
		...
		try {
			...
			assert.deepEqual(
				parsed.results.map((result) => result.session_id),
				["madmax-session"],
			);
			assert.ok(
				parsed.sources.some(
					(source) => source.codex_home === "madmax:omx-madmax-a",
				),
			);
			assert.equal(
				parsed.sources.some((source) => source.codex_home.includes(runsRoot)),
				false,
			);
			assert.equal(
				parsed.results.some((result) =>
					result.transcript_path.includes(runsRoot),
				),
				false,
			);
		} finally {
