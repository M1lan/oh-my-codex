...
`,
			...
			assert.match(
				result.stdout,
				new RegExp(
					`sqlite-home:${canonicalProjectCodexHome.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
				),
			);
	...
	it("persists project-scope runtime Codex transcripts after cleanup", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-resume-project-history-cleanup-"),
		);
		try {
			const home = join(wd, "home");
			const projectCodexHome = join(wd, ".codex");
	...
	it("includes generated project runtime Codex home sessions for plain resume", async () => {
		...
		try {
			...
			const runtimeCodexHome = join(
				...
				"runtime",
				"codex-home",
				"omx-existing-runtime",
			);
			const fakeBin = join(wd, "bin");
			const fakeCodexPath = join(fakeBin, "codex");
			const fakePsPath = join(fakeBin, "ps");
			const runtimeRolloutPath = join(
				runtimeCodexHome,
				"sessions",
				"2026",
				"06",
				"17",
				"rollout-runtime-session.jsonl",
			);
			...
			await writeFile(
				runtimeRolloutPath,
				'{"type":"session_meta","payload":{"id":"runtime-session"}}\n',
			);
...
`,
	...
	it("merges symlinked project runtime history during plain resume", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-resume-symlinked-runtime-history-"),
		);
		try {
			const home = join(wd, "home");
			const projectCodexHome = join(wd, ".codex");
			const previousRuntimeCodexHome = join(
				wd,
				".omx",
				"runtime",
				"codex-home",
				"omx-existing-runtime-a",
			);
			const duplicateRuntimeCodexHome = join(
				wd,
				".omx",
				"runtime",
				"codex-home",
				"omx-existing-runtime-b",
			);
			const fakeBin = join(wd, "bin");
			const fakeCodexPath = join(fakeBin, "codex");
			const fakePsPath = join(fakeBin, "ps");
			const projectRolloutPath = join(
				projectCodexHome,
				"sessions",
				"2026",
				"06",
				"18",
				"rollout-project-session.jsonl",
			);
			...
			await writeFile(
				projectRolloutPath,
				'{"type":"session_meta","payload":{"id":"project-session"}}\n',
			);
			await writeFile(
				join(projectCodexHome, "history.jsonl"),
				'{"session_id":"project-session"}\n',
			);
			await writeFile(
				join(projectCodexHome, "session_index.jsonl"),
				'{"id":"project-session"}\n',
			);
			await symlink(
				join(projectCodexHome, "sessions"),
				join(previousRuntimeCodexHome, "sessions"),
				"dir",
			);
			await symlink(
				join(projectCodexHome, "history.jsonl"),
				join(previousRuntimeCodexHome, "history.jsonl"),
			);
			await symlink(
				join(projectCodexHome, "session_index.jsonl"),
				join(previousRuntimeCodexHome, "session_index.jsonl"),
			);
			await symlink(
				join(projectCodexHome, "sessions"),
				join(duplicateRuntimeCodexHome, "sessions"),
				"dir",
			);
			await symlink(
				join(projectCodexHome, "history.jsonl"),
				join(duplicateRuntimeCodexHome, "history.jsonl"),
			);
			await symlink(
				join(projectCodexHome, "session_index.jsonl"),
				join(duplicateRuntimeCodexHome, "session_index.jsonl"),
			);
...
`,
	...
	it("does not duplicate generated runtime history across repeated plain resume cleanup", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-resume-runtime-history-dedupe-"),
		);
		try {
			const home = join(wd, "home");
			const projectCodexHome = join(wd, ".codex");
	...
	it("uses --codex-home as an explicit resume escape hatch", async () => {
		...
		try {
			...
			const rolloutPath = join(
				...
				"06",
				"17",
				"rollout-explicit-session.jsonl",
			);
			...
			await writeFile(
				rolloutPath,
				'{"type":"session_meta","payload":{"id":"explicit-session"}}\n',
			);
			await writeFile(
				fakeCodexPath,
				`#!/bin/sh
...
`,
			...
			const result = runOmx(
				wd,
				["resume", "--codex-home", explicitCodexHome, "--last"],
				{
					HOME: home,
					PATH: `${fakeBin}:/usr/bin:/bin`,
					OMX_AUTO_UPDATE: "0",
			...
			assert.match(
				result.stdout,
				new RegExp(
					`codex-home:${explicitCodexHome.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
				),
			);
...
`,
			...
			const runtimeDirs = await readdir(
				join(wd, ".omx", "runtime", "codex-home"),
			);
			const persistedNewTranscript = await Promise.all(
				runtimeDirs.map(async (dir) => {
					const transcript = join(
						...
						"06",
						"18",
						"rollout-new-project-resume.jsonl",
					);
					return readFile(transcript, "utf-8").catch(() => "");
				}),
			);
			assert.ok(
				persistedNewTranscript.some((content) =>
					content.includes("new-project-resume"),
				),
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("includes associated madmax boxed run-root sessions for plain resume", async () => {
		...
		try {
			...
			const unrelatedCodexHome = join(
				...
				"runtime",
				"codex-home",
				"omx-unrelated-runtime",
			);
			const fakeBin = join(wd, "bin");
			const fakeCodexPath = join(fakeBin, "codex");
			const fakePsPath = join(fakeBin, "ps");
			const associatedRolloutPath = join(
				madmaxCodexHome,
				"sessions",
				"2026",
				"06",
				"17",
				"rollout-madmax-session.jsonl",
			);
			const unrelatedRolloutPath = join(
				unrelatedCodexHome,
				"sessions",
				"2026",
				"06",
				"17",
				"rollout-unrelated-session.jsonl",
			);
...
`,
	...
	it("preflights madmax resume to current plugin cache after old cache deletion", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-resume-madmax-plugin-preflight-"),
		);
		try {
			const home = join(wd, "home");
			const runsRoot = join(wd, "runs");
			...
			await mkdir(join(projectCodexHome, "sessions", "2026", "06", "17"), {
				recursive: true,
			});
			...
			await writeFile(
				join(
					...
					"06",
					"17",
					"rollout-stale-session.jsonl",
				),
				'{"type":"session_meta","payload":{"id":"stale-session"}}\n',
			);
			await writeFile(
				join(projectCodexHome, "config.toml"),
				[
...
`,
	...
	it("keeps madmax runtime history deduped across repeated resume cleanup", async () => {
		...
		try {
			...
			await writeFile(
				join(madmaxCodexHome, "history.jsonl"),
				'{"session_id":"madmax-session"}\n',
			);
			await writeFile(
				join(madmaxCodexHome, "session_index.jsonl"),
				'{"id":"madmax-session"}\n',
			);
			await writeFile(
				join(runsRoot, "registry.jsonl"),
	...
	it("preserves transcript mtimes while materializing runtime history for updated sort", async () => {
		...
		try {
			...
			await writeFile(
				oldRolloutPath,
				'{"type":"session_meta","payload":{"id":"old-a"}}\n',
			);
			await writeFile(
				newerRolloutPath,
				'{"type":"session_meta","payload":{"id":"old-b"}}\n',
			);
...
`,
			...
			assert.match(
				result.stdout,
				/2024-01-02 03:04:05\.\d+ .*rollout-old-a\.jsonl/,
			);
			assert.match(
				result.stdout,
				/2024-03-04 05:06:07\.\d+ .*rollout-old-b\.jsonl/,
			);
			assert.equal(
				(await stat(oldRolloutPath)).mtime.toISOString(),
				oldMtime.toISOString(),
			);
			assert.equal(
				(await stat(newerRolloutPath)).mtime.toISOString(),
				newerMtime.toISOString(),
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("prints read-only resume help without launching through a hostile incomplete pointer lock", async () => {
		...
		try {
			...
			await writeFile(
				join(lockPath, "owner.incomplete.tmp"),
				'{"incomplete":true}\n',
			);
			await writeFile(
				fakeCodexPath,
	...
	it("preflights default user plugin cache before madmax resume while preserving stale cache metadata", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-madmax-resume-default-cache-"),
		);
		try {
			const home = join(wd, "home");
			...
			await mkdir(fakeBin, { recursive: true });
			await writeFile(
				join(staleCacheDir, ".codex-plugin", "plugin.json"),
				JSON.stringify({ name: "oh-my-codex", version: staleVersion }),
			);
			await writeFile(
...
`,
			...
			const result = runOmx(
				wd,
				["--madmax", "resume", "session-after-update"],
				{
					HOME: home,
					PATH: `${fakeBin}:/usr/bin:/bin`,
					OMX_AUTO_UPDATE: "0",
			...
			assert.doesNotMatch(
				repairedConfig,
				/^"oh-my-codex@oh-my-codex-local"\s*=/m,
			);
			assert.match(
				repairedConfig,
				/^\[plugins\."oh-my-codex@oh-my-codex-local"\]$/m,
			);
			assert.deepEqual(
				new Set(
					await readdir(
			...
			assert.equal(
				JSON.parse(
					await readFile(
						join(expectedCacheDir, ".codex-plugin", "plugin.json"),
						"utf-8",
					),
				).version,
				packageJson.version,
...
`,
			...
			const result = runOmx(
				wd,
				["--madmax", "resume", "live-session-after-update"],
				{
					HOME: home,
					PATH: `${fakeBin}:/usr/bin:/bin`,
					OMX_AUTO_UPDATE: "0",
			...
			assert.match(
				result.stdout,
				/fake-codex:resume live-session-after-update\b/,
			);
			assert.match(result.stdout, /current-cache=yes/);
...
`,
			...
			await assert.rejects(
				readFile(join(codexHome, "config.toml"), "utf-8"),
				/ENOENT/,
			);
			await assert.rejects(
				readFile(
