...
describe("findCleanupCandidates", () => {
	it("does not treat legacy team-server entrypoints as active OMX MCP processes", () => {
		assert.equal(
			isOmxMcpProcess("node /tmp/worktree/dist/mcp/team-server.js"),
			false,
		);
	});
	...
	it("extracts first-party MCP entrypoints for duplicate grouping", () => {
		assert.equal(
			extractOmxMcpEntrypoint(
				"node /repo/oh-my-codex/dist/mcp/state-server.js",
			),
			"state-server.js",
		);
		assert.equal(
			extractOmxMcpEntrypoint(
				"node C:\\repo\\oh-my-codex\\dist\\mcp\\code-intel-server.cjs",
			),
			"code-intel-server.cjs",
		);
		assert.equal(
			extractOmxMcpEntrypoint("node /tmp/worktree/dist/mcp/team-server.js"),
			null,
		);
		assert.equal(
	...
	it("selects older duplicate siblings under a reused current Codex parent", () => {
		const reusedParentProcesses: ProcessEntry[] = [
			{ pid: 700, ppid: 500, command: "codex app-server" },
			{
				pid: 701,
				ppid: 700,
				command: "node /repo/bin/omx.js cleanup --dry-run",
			},
			{
				pid: 710,
				ppid: 700,
		...
		assert.deepEqual(
			findDuplicateSiblingCleanupCandidates(reusedParentProcesses),
			[
				{
					pid: 710,
					ppid: 700,
		...
		assert.deepEqual(
			findLaunchSafeCleanupCandidates(reusedParentProcesses, 701),
			[],
		);
	});
	...
	it("reaps plugin-launched omx mcp-serve orphans during launch-safe cleanup", () => {
		const processes: ProcessEntry[] = [
			{ pid: 700, ppid: 500, command: "codex app-server" },
			{
				pid: 701,
				ppid: 700,
				command: "node /repo/bin/omx.js cleanup --launch-safe",
			},
			{
				pid: 820,
				ppid: 1,
	...
	it("keeps detached MCP candidates whose ancestor chain is live but unrelated to Codex or OMX launchers", () => {
		...
		assert.deepEqual(
			findLaunchSafeCleanupCandidates(unrelatedAncestorProcesses, 701),
			[
				{
					pid: 840,
					ppid: 841,
	...
	it("always preserves ppid=1 orphan candidates even if pid 1 matches a protected ancestor predicate", () => {
		...
		assert.deepEqual(
			findLaunchSafeCleanupCandidates(reparentedProcesses, 701),
			[
				{
					pid: 840,
					ppid: 1,
			...
			const processes = listOmxProcesses(() =>
				[
					JSON.stringify({
						pid: 800,
						ppid: 1,
						command: "node C:/tmp/oh-my-codex/dist/mcp/state-server.js",
					}),
					JSON.stringify({
						pid: 810,
						ppid: 42,
						command: "node C:/tmp/oh-my-codex/dist/mcp/trace-server.js",
					}),
				].join("\n"),
			);
	...
	it("falls back to BusyBox-compatible args field when command field is unsupported", () => {
		...
		assert.deepEqual(processes, [
			...
			},
			{
				pid: 810,
				ppid: 42,
				command: "node /tmp/oh-my-codex/dist/mcp/trace-server.js --verbose",
			},
		]);
			...
			const parsed = listOmxProcesses(() =>
				[
					...
					JSON.stringify({
						pid: 701,
						ppid: 700,
						command: "node C:/repo/bin/omx.js cleanup --dry-run",
					}),
					JSON.stringify({
						pid: 710,
						ppid: 700,
						command: "node C:/repo/dist/mcp/state-server.js",
					}),
					JSON.stringify({
						pid: 800,
						ppid: 1,
					...
					JSON.stringify({
						pid: 900,
						ppid: 1,
						command: "node C:/tmp/not-omx/other-server.js",
					}),
				].join("\n"),
			);
...
describe("cleanupOmxMcpProcesses", () => {
	...
	it("sends SIGTERM, waits, and escalates with SIGKILL when needed", async () => {
		...
		const result = await cleanupOmxMcpProcesses([], {
			currentPid: 701,
			listProcesses: () => [
				...CURRENT_SESSION_PROCESSES.filter(
					(processEntry) =>
						processEntry.pid !== 821 && processEntry.pid !== 831,
				),
			],
