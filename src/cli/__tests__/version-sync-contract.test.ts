...
describe("version sync contract", () => {
	it("keeps package.json, workspace metadata, and Rust members aligned for releases", () => {
		...
		const workspace = TOML.parse(
			readFileSync(join(process.cwd(), "Cargo.toml"), "utf-8"),
		) as {
			workspace?: { package?: WorkspacePackageMetadata; members?: string[] };
		};
		const api = TOML.parse(
			readFileSync(
				join(process.cwd(), "crates", "omx-api", "Cargo.toml"),
				"utf-8",
			),
		) as {
		...
		const explore = TOML.parse(
			readFileSync(
				join(process.cwd(), "crates", "omx-explore", "Cargo.toml"),
				"utf-8",
			),
		) as {
			package?: WorkspaceMemberPackageMetadata;
		...
		const runtimeCore = TOML.parse(
			readFileSync(
				join(process.cwd(), "crates", "omx-runtime-core", "Cargo.toml"),
				"utf-8",
			),
		) as { package?: WorkspaceMemberPackageMetadata };
		const mux = TOML.parse(
			readFileSync(
				join(process.cwd(), "crates", "omx-mux", "Cargo.toml"),
				"utf-8",
			),
		) as {
			package?: WorkspaceMemberPackageMetadata;
		...
		const runtime = TOML.parse(
			readFileSync(
				join(process.cwd(), "crates", "omx-runtime", "Cargo.toml"),
				"utf-8",
			),
		) as {
			package?: WorkspaceMemberPackageMetadata;
		...
		const sparkshell = TOML.parse(
			readFileSync(
				join(process.cwd(), "crates", "omx-sparkshell", "Cargo.toml"),
				"utf-8",
			),
		) as {
			package?: WorkspaceMemberPackageMetadata;
		...
		assert.deepEqual(runtimeCore.package?.["rust-version"], {
			workspace: true,
		});
	...
	it("keeps Cargo.lock readable by the packaged fallback Rust toolchain floor", () => {
		...
		assert.doesNotMatch(
			lockfile,
			/^version = 4$/m,
			"Cargo.lock v4 breaks cargo 1.73 fallback builds used by omx explore",
		);
	});
});
