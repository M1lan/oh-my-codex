...
async function assertNoUninstallTransactionArtifacts(
	codexDir: string,
): Promise<void> {
	const artifacts = (await readdir(codexDir)).filter((entry) =>
...
function assertUnapprovedOmxHooks(hooks: readonly CodexHookMetadata[]): void {
	for (const [event, hook] of Object.entries(managedCodexHooksByEvent(hooks))) {
		assert.notEqual(
			hook.trustStatus,
			"trusted",
			`setup-generated trust pre-approved OMX ${event}`,
		);
	}
}
...
function foreignMetadataSnapshot(
...
function isObservedCodexVersionMismatch(error: Error): boolean {
	if (!error.message.startsWith("Unsupported installed Codex version"))
		return false;
	const observations = error.message.split("\n").slice(1).filter(Boolean);
	return (
		observations.length > 0 &&
		observations.every((line) =>
			/: stdout="codex-cli \d+\.\d+\.\d+\\n" stderr=/.test(line),
		)
	);
}

function skipUnsupportedInstalledCodex(
	t: { skip: (message?: string) => void },
	error: unknown,
): boolean {
	if (error instanceof CodexExecutableNotFoundError) {
		t.skip(
			"codex executable is absent; installed-Codex boundary is unavailable",
		);
		return true;
	}
...
test("Linux installed-Codex hooks/list preserves full foreign metadata through uninstall (no macOS claim)", async (t) => {
	if (process.platform !== "linux") {
		t.skip(
			"This regression records Linux-only Codex evidence and makes no macOS claim.",
		);
		return;
	}
	...
	try {
		...
		await writeFile(
			hooksPath,
			appendDisplayOrderStableForeignHookGroups('{"hooks":{}}', foreignMarker, {
				appendGroups: true,
			}),
			"utf-8",
		);
		...
		assert.equal(
			preSetupForeignRawSnapshot.length,
			2,
			"pre-approved foreign coordinates should be recorded",
		);
		...
		const foreignApprovalServer = await CodexAppServer.start({
			cwd: projectDir,
			env,
		});
		try {
			await initializeCodexAppServer(
				foreignApprovalServer,
				"omx-hook-trust-regression",
			);
			const preSetupForeignHooks = (
				await listCodexHooks(foreignApprovalServer, projectDir, hooksPath)
			).hooks.filter((hook) => hook.command.includes(foreignMarker));
			assert.equal(
				preSetupForeignHooks.length,
				2,
				"Codex must discover both pre-seeded foreign hooks",
			);
			assert.ok(
		...
		assert.ok(
			preapprovedForeignMetadata.every(
		...
		try {
			...
			const initialHooks = (
				await listCodexHooks(approvalServer, projectDir, hooksPath)
			).hooks;
		...
		const foreignRawSnapshot = foreignHookGroupSnapshot(
			postForeignHooksContent,
			foreignMarker,
		);
		assert.equal(
			foreignRawSnapshot.length,
			4,
			"foreign group and handler coordinates should be recorded",
		);
		...
		const expectedForeignMetadata = foreignMetadataSnapshot(
			postForeignHooks,
			foreignMarker,
		);
		assert.equal(
			expectedForeignMetadata.length,
			4,
			"Codex must discover all foreign command hooks",
		);
		assert.deepEqual(
			expectedForeignMetadata.filter(
				(hook) => !hook.command.includes("-inserted.js"),
			),
			preapprovedForeignMetadata,
		...
		assert.equal(
			await readFile(hooksPath, "utf-8"),
			postForeignHooksContent,
			"rerun changed hooks.json",
		);
		assert.equal(
			await readFile(configPath, "utf-8"),
			postForeignConfig,
			"rerun changed config.toml",
		);
		assertForeignHookGroupsPreserved(
			foreignRawSnapshot,
			await readFile(hooksPath, "utf-8"),
		...
		assert.deepEqual(
			foreignMetadataSnapshot(afterRerunHooks, foreignMarker),
		...
		assert.equal(
			await readFile(hooksPath, "utf-8"),
			postForeignHooksContent,
			"third setup changed hooks.json",
		);
		assert.equal(
			await readFile(configPath, "utf-8"),
			postForeignConfig,
			"third setup changed config.toml",
		);
		assertForeignHookGroupsPreserved(
			foreignRawSnapshot,
			await readFile(hooksPath, "utf-8"),
		...
		assert.deepEqual(
			foreignMetadataSnapshot(afterNoopHooks, foreignMarker),
			expectedForeignMetadata,
		);
...
test("Linux installed-Codex preserves managed-first foreign order and fails unsafe uninstall without writes", async (t) => {
	...
	const root = await mkdtemp(
		join(tmpdir(), "omx-setup-hooks-managed-first-e2e-"),
	);
	const projectDir = resolve(root, "project");
	const home = join(root, "home");
	const codexHome = join(root, "codex-home");
	...
	try {
		...
		try {
			...
			const initialManagedFirstHooks = (
				await listCodexHooks(approvalServer, projectDir, hooksPath)
			).hooks;
		...
		assert.deepEqual(
			foreignRawSnapshot.map(
				(entry) => (entry as { groupIndex: number }).groupIndex,
			),
			[1, 2],
		);
		const beforeRerunCodexHooks = await observeHooks(
			projectDir,
			hooksPath,
			env,
		);
		...
		assert.equal(
			await readFile(hooksPath, "utf-8"),
			beforeRerunHooks,
			"rerun reordered managed-first hooks.json",
		);
		assert.equal(
			await readFile(configPath, "utf-8"),
			beforeRerunConfig,
			"rerun changed managed-first config.toml",
		);
		assertForeignHookGroupsPreserved(
			foreignRawSnapshot,
			await readFile(hooksPath, "utf-8"),
		...
		assert.equal(
			unsafeRemoval.error.message,
			expectedUnsafeManagedRemovalDiagnostic,
		);
		...
		assert.deepEqual(
			...
			beforeUninstallHooksBytes,
			"unsafe uninstall changed raw hooks.json bytes",
		);
		assert.deepEqual(
			await readFile(configPath),
			beforeUninstallConfigBytes,
			"unsafe uninstall changed raw config.toml bytes",
		);
