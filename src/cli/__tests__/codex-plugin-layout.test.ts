...
function buildExactBytePluginHookPayload(
...
function oversizedToolHookOutput(
	eventName: "PreToolUse" | "PostToolUse",
		...
		await withPluginCacheCopy(async (cachePluginRoot, cacheRoot) => {
			...
			for (const eventName of ["PreToolUse", "PostToolUse"] as const) {
				const result = runPluginNativeHook(
					cachePluginRoot,
					buildExactBytePluginHookPayload(
						eventName,
						`plain-${eventName}`,
						1024 * 1024 + 1,
					),
	...
	it("no-ops plugin hooks for nested plain Codex sessions that inherit omx launch env", async () => {
		await withPluginCacheCopy(async (cachePluginRoot, cacheRoot) => {
			...
			await assert.rejects(
				readFile(
					join(cacheRoot, "codex-home", ".omx", "native-anchor-auth.key"),
					"utf-8",
				),
				{ code: "ENOENT" },
			);
			...
			await writeFile(
				...
				`${JSON.stringify({
					...
					payload: {
						...
						source: {
							subagent: {
								thread_spawn: { parent_thread_id: "owner-codex-session" },
							},
						},
					},
				})}\n`,
		...
		await withPluginCacheCopy(async (cachePluginRoot, cacheRoot) => {
			...
			const cases: Array<[string, () => Promise<void>]> = [
				...
				[
					"oversized",
					async () => {
						await writeFile(routingPath, "x".repeat(4097), "utf-8");
					},
				],
				[
					"symlink",
					async () => {
						await symlink(targetPath, routingPath);
					},
				],
				[
					"hardlink",
					async () => {
						await link(targetPath, routingPath);
					},
				],
				[
					"replacement",
	...
	it("delegates exact UTF-8 under-limit plugin tool-hook input and locally rejects only oversized owned input", async () => {
		await withPluginCacheCopy(async (cachePluginRoot, cacheRoot) => {
			...
			const launcherPath = join(
				cacheRoot,
				process.platform === "win32"
					? "capture-tool-hook.cmd"
					: "capture-tool-hook.sh",
			);
			await writeFile(
				commandPath,
				`import { writeFileSync } from 'node:fs';
...
`,
			...
			for (const eventName of ["PreToolUse", "PostToolUse"] as const) {
				...
				for (const byteLength of [1024 * 1024 - 1, 1024 * 1024]) {
					const input = buildExactBytePluginHookPayload(
						eventName,
						sessionId,
						byteLength,
					);
				...
				assert.equal(result.stderr, "");
				assert.deepEqual(
					parseSingleJsonStdout(result.stdout),
					oversizedToolHookOutput(eventName),
				);
				await assert.rejects(readFile(sentinelPath, "utf-8"), {
					code: "ENOENT",
				});
...
`,
	...
	it("documents marketplace-aware cache semantics without replacing full setup", async () => {
		...
		assert.match(
			combined,
			/not a replacement for `pnpm add -g oh-my-codex` plus `omx setup`/,
		);
		assert.match(
			combined,
