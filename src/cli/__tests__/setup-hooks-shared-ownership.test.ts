...
function countManagedHooks(entries: HookRegistration[] | undefined): number {
	return hookCommands(entries).filter(
		(command) =>
...
describe("omx setup/uninstall shared ownership for native hooks", () => {
	it("setup merges managed wrappers into an existing user-owned hooks.json", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-setup-hooks-existing-user-file-"),
		);
		try {
	...
	it("keeps all final trust coordinates stable across a no-op rerun with interleaved foreign groups", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-setup-hooks-final-coordinates-"),
		);
		try {
			const home = join(wd, "home");
			const codexDir = join(wd, ".codex");
			...
			const firstSetup = runOmx(wd, ["setup", "--scope", "project"], {
				HOME: home,
			});
			...
			const secondSetup = runOmx(wd, ["setup", "--scope", "project"], {
				HOME: home,
			});
			...
			assert.equal(
				secondSetup.status,
				0,
				secondSetup.stderr || secondSetup.stdout,
			);
	...
	it("setup preserves user hooks while deduping stale OMX wrappers", async () => {
		...
		try {
			...
			const initial = runOmx(wd, ["setup", "--scope", "project"], {
				HOME: home,
			});
			...
			assert.ok(
				generatedSessionStart.length > 0,
				"setup should generate managed SessionStart hooks",
			);

			const staleManagedSessionStart = cloneRegistration(
				generatedSessionStart[0]!,
			);
			if (staleManagedSessionStart.hooks?.[0]) {
				staleManagedSessionStart.hooks[0].command =
					'node "/tmp/old/dist/scripts/codex-native-hook.js"';
			...
			const refreshedSetup = runOmx(wd, ["setup", "--scope", "project"], {
				HOME: home,
			});
			if (shouldSkipForSpawnPermissions(refreshedSetup.error)) return;
			assert.equal(
				refreshedSetup.status,
				0,
				refreshedSetup.stderr || refreshedSetup.stdout,
			);
			...
			assert.ok(
				hookCommands(refreshed.hooks?.UserPromptSubmit).some(
					hasManagedHookCommand,
				),
	...
	it("uninstall removes only OMX-managed wrappers and preserves user hook content", async () => {
		...
		try {
			...
			assert.equal(
				...
				true,
				"uninstall should keep hooks.json when user hooks remain",
			);
			const remaining = await readHooksJson(hooksPath);
			const allCommands = Object.values(remaining.hooks ?? {}).flatMap(
				(entries) => hookCommands(entries),
			);
	...
	it("plugin transition preserves historical multi-agent configuration and custom roles", async () => {
		...
		try {
			...
			await writeFile(
				join(codexDir, "agents", "custom-role.toml"),
				'model = "custom"\n',
			);
	...
	it("preserves malformed hooks.json bytes and creates no native setup artifacts", async () => {
		...
		try {
			...
			const result = runOmx(wd, ["setup", "--scope", "project"], {
				HOME: home,
			});
			if (shouldSkipForSpawnPermissions(result.error)) return;
