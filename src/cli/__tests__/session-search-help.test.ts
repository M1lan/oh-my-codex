...
describe("omx session help", () => {
	it("documents the session search command in help output", async () => {
		...
		try {
			...
			await writeFile(
				join(lockPath, "owner.json.tmp-stalled"),
				'{"incomplete":true}\n',
				"utf-8",
			);
			...
			assert.match(
				mainHelp.stdout,
				/omx resume\s+Resume Codex sessions \(supports --project and --codex-home <path>\)/i,
			);
			assert.match(
				mainHelp.stdout,
			...
			assert.equal(
				await readFile(join(lockPath, "owner.json.tmp-stalled"), "utf-8"),
				lockBefore,
			);

			const blockedRecovery = runOmx(cwd, [
				"session",
				"lock",
				"recover",
				"--cwd",
			...
			assert.equal(
				...
				1,
				blockedRecovery.stderr || blockedRecovery.stdout,
			);
			const recovery = JSON.parse(blockedRecovery.stdout) as {
				action: string;
				recovered: boolean;
				reason: string;
			};
