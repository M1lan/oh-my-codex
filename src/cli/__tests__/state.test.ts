...
describe("stateCommand", () => {
		...
		for (const operation of operations) {
			for (const helpForm of helpForms) {
				...
				assert.equal(
					executed,
					false,
					`${operation} ${helpForm} should not execute state operation`,
				);
				assert.match(out.join("\n"), /Usage: omx state/);
	...
	it("strips only one leading BOM from --input-file JSON", async () => {
		const dir = await mkdtemp(
			join(tmpdir(), "omx-state-input-file-double-bom-"),
		);
		const file = join(dir, "payload.json");
		...
		await assert.rejects(
			stateCommand(
				[
					"read",
					"--input-file",
					join(tmpdir(), "omx-state-missing-does-not-exist.json"),
				],
				{
					stdout: () => undefined,
					stderr: () => undefined,
	...
	it("rejects supplying both --input and --input-file", async () => {
		await assert.rejects(
			stateCommand(
				["read", "--input", '{"mode":"ralph"}', "--input-file", "payload.json"],
				{
					stdout: () => undefined,
					stderr: () => undefined,
				},
