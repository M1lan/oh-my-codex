...
describe("omx exec", () => {
	it("persists audited follow-up prompts for the active exec session without pane input", async () => {
		...
		try {
			...
			assert.equal(
				result.queued.prompt,
				"Please include the new migration note before finishing.",
			);
			assert.match(result.queuePath, /exec-followups\.json$/);

			const persisted = JSON.parse(
				await readFile(result.queuePath, "utf-8"),
			) as {
	...
	it("preserves all follow-up prompts from concurrent injections", async () => {
		...
		try {
			...
			assert.deepEqual(
				...
				Array.from(
					{ length: count },
					(_, index) => `Concurrent follow-up ${index}`,
				).sort(),
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("delivers queued follow-ups through Stop hook output and marks them delivered", async () => {
		...
		try {
			...
			assert.match(
				String(output?.systemMessage),
				/Before stopping, verify the targeted exec tests/,
			);

			const after = await readPendingExecFollowups(wd, session.session_id);
			assert.equal(after.pending.length, 0);
			const persisted = JSON.parse(
				await readFile(queued.queuePath, "utf-8"),
			) as {
	...
	it("runs codex exec with session-scoped instructions that preserve AGENTS and overlay content", async () => {
		...
		try {
			...
			assert.match(
				result.stdout,
				/instructions-path:.*\/\.omx\/state\/sessions\/omx-.*\/AGENTS\.md/,
			);
			...
			assert.equal(
				sessionFiles.includes("AGENTS.md"),
				false,
				"session-scoped AGENTS file should be cleaned up after exec exits",
			);
			assert.equal(
	...
	it("archives a native live-to-dead exec binding and leaves cancel immediately usable", async () => {
		...
		try {
			...
			assert.doesNotMatch(
				result.stderr,
				/session archive failed|stale-dead|postLaunch failed/i,
			);
			assert.equal(
				existsSync(join(wd, ".omx", "state", "session.json")),
				false,
			...
			assert.equal(
				cancel.status,
				0,
				cancel.error || cancel.stderr || cancel.stdout,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("provides interactive launcher identity parity to omx exec", async () => {
		...
		try {
			...
			assert.ok(
				captured.path
					.split(":")
					.some((entry) => entry.includes(".omx") && entry.includes("bin")),
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
		...
		try {
			...
			const assertFragmentsInOrder = (
				...
				for (const fragment of fragments) {
					...
					assert.ok(
						index > previousIndex,
						`expected ordered stderr fragment: ${fragment}`,
					);
					previousIndex = index;
				}
			};
			for (const mode of ["max", "MAX"]) {
				const result = runOmx(wd, ["reasoning", mode], {
			...
			const casingResult = runOmx(wd, ["reasoning", "High"], {
				CODEX_HOME: codexHome,
			});
			assert.equal(
				casingResult.status,
				1,
				casingResult.error || casingResult.stderr || casingResult.stdout,
			);
			...
			const absentResult = runOmx(wd, ["reasoning", "MAX"], {
				CODEX_HOME: absentCodexHome,
			});
			assert.equal(
				absentResult.status,
				1,
				absentResult.error || absentResult.stderr || absentResult.stdout,
			);
