...
describe("omx agents", () => {
	...
	it("adds a scaffolded agent TOML file with required fields and commented optional fields", async () => {
		...
		try {
			...
			const result = runOmx(
				wd,
				["agents", "add", "my-helper", "--scope", "project"],
				{
					HOME: home,
					CODEX_HOME: join(home, ".codex"),
				},
	...
	it("edits an existing agent via $EDITOR and removes it with --force", async () => {
		...
		try {
			...
			const editResult = runOmx(
				wd,
				["agents", "edit", "editor-test", "--scope", "project"],
				{
					HOME: home,
					CODEX_HOME: join(home, ".codex"),
					EDITOR: editorScript,
			...
			const removeResult = runOmx(
				wd,
				["agents", "remove", "editor-test", "--scope", "project", "--force"],
				{
					HOME: home,
					CODEX_HOME: join(home, ".codex"),
				},
	...
	it("fails with clear guidance when remove runs in non-interactive mode without --force", async () => {
		...
		try {
			...
			const result = runOmx(
				wd,
				["agents", "remove", "non-interactive", "--scope", "project"],
				{
					HOME: home,
					CODEX_HOME: join(home, ".codex"),
				},
			...
			assert.notEqual(
				result.status,
				0,
				"expected non-zero exit for non-interactive remove",
			);
			assert.equal(
				existsSync(agentPath),
				true,
				"agent file should remain when remove aborts",
			);
			assert.match(
				result.stderr,
