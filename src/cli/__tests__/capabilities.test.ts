...
async function writeObservations(
	...
	await writeFile(
		path,
		JSON.stringify(
			{ version: 1, kind: "omx_capability_observations", observations },
			null,
			2,
		),
		"utf8",
		...
		tool_calls: [
			{
				...
				arguments: { memory: { note: "capability preflight fixture" } },
			},
		],
	},
	{
...
describe("omx capabilities cli", () => {
	...
	it("exits non-zero for required missing observations", async () => {
		await withTempDir(async (dir) => {
			...
			assert.ok(
				checkJson.failures.some(
					(failure: { code: string }) =>
						failure.code === "observations_missing",
				),
			);
		});
	});
