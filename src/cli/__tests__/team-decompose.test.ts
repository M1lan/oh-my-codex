...
describe("decomposeTaskString", () => {
	it("splits conjunction-separated tasks", () => {
		const tasks = decomposeTaskString(
			"fix tests, build UI, and write docs",
			3,
			"executor",
			false,
		);
	...
	it("assigns different roles to split tasks via heuristic routing", () => {
		const tasks = decomposeTaskString(
			"fix tests, build UI component, and write documentation",
			3,
			"executor",
			false,
		);
		...
		assert.ok(
			uniqueRoles.size >= 2,
			`Expected at least 2 distinct roles, got: ${[...uniqueRoles].join(", ")}`,
		);
	});

	it("splits numbered list tasks", () => {
		const tasks = decomposeTaskString(
			"1. add auth 2. write tests 3. update docs",
			3,
			"executor",
			false,
		);
	...
	it("round-robins generated aspect subtasks when explicit same-role fanout is requested", () => {
		...
		assert.deepEqual(
			new Set(tasks.map((task) => task.role)),
			new Set(["executor"]),
		);
	});
	...
	it("clusters same-role work to preserve specialization when routing is mixed", () => {
		const tasks = decomposeTaskString(
			"write docs, build UI component, update docs, and fix tests",
			3,
			"executor",
			false,
		);
		assert.equal(tasks.length, 4);
		const docOwners = tasks
			.filter((task) => task.role === "writer")
			.map((task) => task.owner);
	...
	it("handles semicolon-separated tasks", () => {
		const tasks = decomposeTaskString(
			"analyze perf; fix bottleneck; write benchmark",
			3,
			"executor",
			false,
		);
	...
	it("keeps long analytic prose prompts in a single-worker lane by default", () => {
		...
		assert.match(
			...
			/Analyze OMX team mode reliability\/efficiency weaknesses/i,
		);
	});

	it("preserves backward compat: explicit agentType overrides routing", () => {
		const tasks = decomposeTaskString(
			"write tests and build UI",
			2,
			"debugger",
			true,
		);
	...
	it("keeps explicit worker-count small tasks conservative only when count is implicit", () => {
		const implicitPlan = buildTeamExecutionPlan(
			"fix typo in README",
			3,
			"executor",
			false,
		...
		assert.equal(
			implicitPlan.overOrchestrationNotice?.code,
			"team_over_orchestration_warning",
		);
		assert.match(
			implicitPlan.overOrchestrationNotice?.message ?? "",
			/solo execution path/i,
		);

		const explicitPlan = buildTeamExecutionPlan(
			"fix typo in README",
			3,
			"executor",
			false,
		...
		assert.equal(
			explicitPlan.overOrchestrationNotice?.code,
	...
	it("keeps medium-sized coupled implementation prompts single-lane by default", () => {
		const task =
			"Implement a staffing preview in omx team so the leader can inspect decomposition, role routing, and fanout reasons before launch.";
		const plan = buildTeamExecutionPlan(task, 3, "executor", false);
		assert.equal(plan.workerCount, 1);
		assert.equal(plan.tasks.length, 1);
		assert.equal(plan.tasks[0].owner, "worker-1");
	...
	it("still fans out atomic cross-cutting work when code-scoped parallelization signals are present", () => {
		const task =
			"Refactor cross-cutting staffing logic across src/cli/team.ts src/team/role-router.ts src/team/runtime.ts and `buildTeamExecutionPlan` to keep verification explanations aligned.";
		const plan = buildTeamExecutionPlan(task, 3, "executor", false);
	...
	it("keeps explicit numbered tasks fanned out even on implicit default team runs", () => {
		const tasks = decomposeTaskString(
			"1. add team brain overlay 2. add team-executor prompt 3. add tests",
			3,
			"executor",
			false,
		);
