...
function parseConfig(raw: unknown): TmuxHookConfig {
	...
	if (
		...
		allowedModes.some((v) => typeof v !== "string")
	) {
	...
	if (
		typeof maxInjections !== "number" ||
		maxInjections < 1 ||
		!Number.isFinite(maxInjections)
	) {
	...
	if (
		parsed.skip_if_scrolling !== undefined &&
		parsed.skip_if_scrolling !== true &&
		parsed.skip_if_scrolling !== false
	) {
...
async function loadConfigForCommand(
	...
	if (!existsSync(configPath)) {
		...
		if (initResult.created) {
			console.log(
				`No tmux-hook config found. Created ${initResult.configPath}.`,
			);
			...
			if (initResult.usedPlaceholderTarget) {
				console.log(
					"Could not auto-detect a tmux target. Edit `.omx/tmux-hook.json` when ready.",
				);
...
function runTmux(
	...
	const result = spawnSync(resolveTmuxBinaryForPlatform() || "tmux", args, {
		encoding: "utf-8",
		windowsHide: true,
	});
	if (result.error) {
...
function resolveValidateTarget(
	...
	if (config.target.type === "pane") {
		...
		if (!paneCheck.ok || paneCheck.stdout === "") {
			return {
				ok: false,
				reason: paneCheck.ok ? "pane not found" : paneCheck.stderr,
			};
		}
...
function detectInitialTarget(): InitialTargetDetection | null {
	...
	const envPane =
		process.env.TMUX && typeof process.env.TMUX_PANE === "string"
...
async function initTmuxHookConfig(opts?: {
	...
	if (!silent) {
		...
		if (detected) {
			console.log(
				`Detected target: ${detected.target.type}:${detected.target.value}`,
			);
		}
...
async function showTmuxHookStatus(): Promise<void> {
	...
	if (initResult?.usedPlaceholderTarget) {
		console.log(
			"Target Status: placeholder (set `target.value` to enable injection)",
		);
	}
	...
	if (!existsSync(statePath)) {
		...
		const legacySessions = state.session_counts
			? Object.keys(state.session_counts).length
			: 0;
...
async function testTmuxHook(args: string[]): Promise<void> {
	...
	if (initResult?.usedPlaceholderTarget) {
		console.log(
			"Proceeding with placeholder target; notify-hook may log `invalid_config` skips.",
		);
	}
	...
	const result = spawnSync(
		process.execPath,
		[notifyHook, JSON.stringify(payload)],
		{
			cwd,
			encoding: "utf-8",
			windowsHide: true,
	...
	console.log(
		"Check: .omx/logs/tmux-hook-YYYY-MM-DD.jsonl for skip/reason codes.",
	);
}
