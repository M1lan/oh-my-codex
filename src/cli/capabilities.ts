	...
	switch (parsed.subcommand) {
		case "lock": {
			...
			const result = {
				...
				warnings: built.surfaces.configured_tools.external_servers.map(
					(server) => ({
...
function parseCapabilitiesArgs(args: string[]): ParsedCapabilitiesArgs {
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		switch (arg) {
			...
			default:
				if (arg.startsWith("--"))
					throw new Error(`Unknown capabilities option: ${arg}`);
				break;
		}
	}
	return parsed;
...
function printResult(result: unknown, json: boolean): void {
	...
	const check = result as Partial<CapabilityCheckResult> & {
		lockfile?: string;
	};
	console.log(
		`${check.ok ? "OK" : "FAIL"}: capabilities ${check.ok ? "satisfied" : "check failed"}`,
	);
