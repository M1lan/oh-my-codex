...
const OMX_SPARKSHELL_INSTRUCTIONS_FILE_ENV =
...
export function sparkshellBinaryName(
...
export function packagedSparkShellBinaryCandidatePaths(
	...
	if (platform === "linux") {
		for (const libc of linuxLibcPreference ??
			resolveLinuxNativeLibcPreference({ env })) {
			candidates.push(
				packagedSparkShellBinaryPath(packageRoot, platform, arch, libc),
			);
		}
	}
...
export function resolveSparkShellBinaryPath(
	...
	for (const packaged of packagedSparkShellBinaryCandidatePaths(
		packageRoot,
		platform,
		arch,
		env,
	...
	const nestedRepoLocal = nestedRepoLocalSparkShellBinaryPath(
		packageRoot,
		platform,
	);
	if (exists(nestedRepoLocal)) return nestedRepoLocal;

	const packagedCandidates = packagedSparkShellBinaryCandidatePaths(
		packageRoot,
		platform,
		arch,
		env,
...
export function runSparkShellBinary(
	...
	const instructionsFile =
		mergedEnv[OMX_SPARKSHELL_INSTRUCTIONS_FILE_ENV]?.trim() ||
		join(
			getPackageRoot(),
			"templates",
			"model-instructions",
			"sparkshell-lightweight-AGENTS.md",
		);
...
const SPARKSHELL_GLIBC_INCOMPATIBLE_PATTERN =
	/GLIBC(?:XX)?_[0-9.]+['` ]+not found/i;

export function isSparkShellNativeCompatibilityFailure(
...
export function resolveFallbackShellArgv(
	...
	const {
		...
		commandExists = (command: string) =>
			spawnSync(command, ["--version"], { encoding: "utf-8", stdio: "ignore" })
				.error === undefined,
	} = options;
...
function stripSparkShellWrapperOptions(args: readonly string[]): string[] {
	...
	while (index < args.length) {
		...
		if (
			token.startsWith("--budget=") ||
			token.startsWith("--cache-ttl-ms=") ||
...
export function parseSparkShellFallbackInvocation(
	...
	const paneStart = args.findIndex(
		(arg) => arg === "--tmux-pane" || arg.startsWith("--tmux-pane="),
	);
	if (paneStart < 0) {
...
function runSparkShellFallback(
	args: readonly string[],
	options: RunSparkShellFallbackOptions = {},
): void {
	const {
		announce = true,
		cause = "native sidecar unavailable",
		nativePath = "native-resolution",
		state = "unavailable",
	} = options;
	const invocation = parseSparkShellFallbackInvocation(args);
	if (announce) {
		process.stderr.write(
			`[sparkshell] native sidecar unavailable; cause=${sanitizeFallbackDiagnostic(cause)}; path=${sanitizeFallbackDiagnostic(nativePath)}; state=${sanitizeFallbackDiagnostic(state)}; remediation=install a compatible native sidecar, reconnect for hydration, or set ${OMX_SPARKSHELL_BIN_ENV}. Falling back to raw command execution without summary support.\n`,
		);
	}
	...
	if (result.error) {
		...
		if (kind === "missing") {
			throw new Error(
				`[sparkshell] raw fallback failed: executable not found (${invocation.argv[0]})`,
			);
		}
		if (kind === "blocked") {
			throw new Error(
				`[sparkshell] raw fallback failed: executable is blocked (${errno.code || "blocked"})`,
			);
		}
...
export async function sparkshellCommand(args: string[]): Promise<void> {
	...
	const hasExplicitOverride =
		typeof process.env[OMX_SPARKSHELL_BIN_ENV] === "string" &&
