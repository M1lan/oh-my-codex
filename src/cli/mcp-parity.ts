...
async function runDescriptorCommand(
	...
	if (parsed.json) {
		globalThis.console.log(
			JSON.stringify("data" in result ? result.data : result.error),
		);
...
async function loadStateDescriptor(): Promise<McpCliDescriptor> {
	const { buildStateServerTools, handleStateToolCall } =
	...
	return {
		...
		tools: buildStateServerTools().map(({ name, description }) => ({
			name,
			description,
		})),
		aliases: {
...
async function loadMemoryDescriptor(
	...
	const { buildMemoryServerTools, handleMemoryToolCall } =
		await importWithAutoStartDisabled(
...
async function loadTraceDescriptor(): Promise<McpCliDescriptor> {
	const { buildTraceServerTools, handleTraceToolCall } =
		await importWithAutoStartDisabled(
	...
	return {
		...
		tools: buildTraceServerTools().map(({ name, description }) => ({
			name,
			description,
		})),
		aliases: {
...
async function loadCodeIntelDescriptor(): Promise<McpCliDescriptor> {
	...
	return {
		...
		tools: buildCodeIntelServerTools().map(({ name, description }) => ({
			name,
			description,
		})),
...
async function loadWikiDescriptor(): Promise<McpCliDescriptor> {
	...
	return {
		...
		tools: buildWikiServerTools().map(({ name, description }) => ({
			name,
			description,
		})),
		aliases: {
			...
			await runDescriptorCommand(
				...
				async () =>
					await loadMemoryDescriptor(
						"notepad",
						"notepad_",
						"JSON CLI surface for OMX notepad operations.",
					),
			);
			return;
		case "project-memory":
