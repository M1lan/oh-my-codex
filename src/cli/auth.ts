...
async function ensureSubscriptionCodexDefaults(
	...
	let updated = upsertTopLevelTomlString(
		existing,
		"model",
		DEFAULT_SUBSCRIPTION_MODEL,
	);
	updated = upsertTopLevelTomlString(
		updated,
		"model_provider",
		DEFAULT_SUBSCRIPTION_MODEL_PROVIDER,
	);
...
export async function authCommand(
	...
	if (command === "list") {
		...
		if (wantsJson(args)) {
			console.log(
				JSON.stringify(
					{
						slots,
						config: { rotation: config.rotation, priority: config.priority },
					},
					null,
					2,
				),
		...
		for (const slot of slots) {
			const quota = slot.exhaustedAt
				? ` exhausted=${slot.exhaustedAt}`
				: slot.lastQuotaAt
					? ` last-quota=${slot.lastQuotaAt}`
					: "";
