...
function persistExactTeamModeState(
	...
	try {
		const current = JSON.parse(readFileSync(statePath, "utf-8")) as Record<
			string,
			unknown
		>;
		writeFileSync(
...
function readPersistedTeamFollowupState(cwd: string): {
	...
	if (existsSync(sessionStatePath)) {
		try {
			const parsed = JSON.parse(readFileSync(sessionStatePath, "utf-8")) as {
				session_id?: unknown;
			};
...
function resolveApprovedTeamFollowupContext(
	...
	const persistedTeamName =
		typeof existingTeamState?.team_name === "string"
			? existingTeamState.team_name.trim()
			: "";
	const persistedTeamStateRoot =
		typeof existingTeamState?.team_state_root === "string" &&
	...
	const persistedAgentType =
		typeof existingTeamState?.agentType === "string" &&
		existingTeamState.agentType.trim() !== ""
			? existingTeamState.agentType.trim()
			: undefined;
	const persistedLinkedRalph =
		typeof existingTeamState?.linkedRalph === "boolean"
	...
	if (persistedTeamName !== "") {
		...
		if (continuity.status === "malformed") {
			throw new Error(
				`approved_execution_binding_malformed:${persistedTeamName}`,
			);
		}
		...
		if (continuity.status === "stale") {
			throw new Error(
				`approved_execution_binding_stale:${continuity.binding.prd_path}:${continuity.binding.task}`,
			);
		}
	...
	if (!approvedHint) {
		const approvedHintOutcome = readApprovedExecutionLaunchHintOutcome(
			...
			"team",
			{
				...
				...(persistedWorkerCount != null
					? { workerCount: persistedWorkerCount }
					: {}),
				...(persistedAgentType ? { agentType: persistedAgentType } : {}),
				...(persistedLinkedRalph != null
					? { linkedRalph: persistedLinkedRalph }
					: {}),
			},
		);
	...
	if (
		...
		persistedTask === approvedHint.task.trim()
	) {
		return {
			task: persistedTask,
			workerCount: persistedWorkerCount,
...
function logInspectEntryDescriptors(
	descriptors: InspectEntryDescriptor<unknown>[],
): void {
	for (const descriptor of descriptors) {
		logInspectEntries(
			descriptor.prefix,
			descriptor.entries,
			descriptor.shouldLog,
			descriptor.formatValue,
		);
	}
}

function inspectEntryDescriptors(
	paneStatus: TeamPaneStatus,
): InspectEntryDescriptor<unknown>[] {
	...
	const joinSpace = (value: unknown): string =>
		Array.isArray(value) ? value.join(" ") : String(value);

	return [
		{
			prefix: "inspect_reason",
			entries: paneStatus.recommended_inspect_reasons,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_cli",
			entries: paneStatus.recommended_inspect_clis,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_role",
			entries: paneStatus.recommended_inspect_roles,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_index",
			entries: paneStatus.recommended_inspect_indexes,
			shouldLog: isNumber,
		},
		{
			prefix: "inspect_alive",
			entries: paneStatus.recommended_inspect_alive,
			shouldLog: isBoolean,
		},
		{
			prefix: "inspect_turn_count",
			entries: paneStatus.recommended_inspect_turn_counts,
			shouldLog: isNumber,
		},
		{
		...
		{
			prefix: "inspect_pid",
			entries: paneStatus.recommended_inspect_pids,
			shouldLog: isNumber,
		},
		{
		...
		{
			prefix: "inspect_workdir",
			entries: paneStatus.recommended_inspect_workdirs,
			shouldLog: isPresentString,
		},
		{
		...
		{
			prefix: "inspect_task_error",
			entries: paneStatus.recommended_inspect_task_errors,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_task_version",
			entries: paneStatus.recommended_inspect_task_versions,
			shouldLog: isNumber,
		},
		{
		...
		{
			prefix: "inspect_task_role",
			entries: paneStatus.recommended_inspect_task_roles,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_task_owner",
			entries: paneStatus.recommended_inspect_task_owners,
			shouldLog: isPresentString,
		},
		{
		...
		{
			prefix: "inspect_state",
			entries: paneStatus.recommended_inspect_states,
			shouldLog: isPresentString,
		},
		{
		...
		{
			prefix: "inspect_task",
			entries: paneStatus.recommended_inspect_tasks,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_subject",
			entries: paneStatus.recommended_inspect_subjects,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_task_path",
			entries: paneStatus.recommended_inspect_task_paths,
			shouldLog: isPresentString,
		},
		{
		...
		{
			prefix: "inspect_pane",
			entries: paneStatus.recommended_inspect_panes,
			shouldLog: isPresentString,
		},
	];
}
...
function formatInspectItemLine(
	...
	const command = modelInspect
		? item.command
		: rawTmuxCaptureCommand(item.pane_id, tailLines);
	const parts = [
		...
		item.task_claim_leased_until
			? `task_claim_leased_until=${item.task_claim_leased_until}`
			: "",
		item.task_claim_lock_path
...
function renderTeamPaneStatus(
	...
	if (paneStatus.sparkshell_hint) {
		console.log(
			"inspect_hint: raw tmux capture commands are quota-free; rerun status with --model-inspect for omx sparkshell summaries",
		);
	}
	...
	if (paneStatus.recommended_inspect_command) {
		const inspectCommand = modelInspect
			...
			: rawTmuxCaptureCommand(
					paneStatus.recommended_inspect_items[0]?.pane_id ?? "",
					tailLines,
				);
		if (inspectCommand.trim().length > 0)
			console.log(`inspect_next: ${inspectCommand}`);
	}
	...
	for (const [target, paneId] of Object.entries({
		...
		const command =
			modelInspect && modelCommand
				? modelCommand
				: rawTmuxCaptureCommand(paneId, tailLines);
...
export function parseTeamArgs(
	...
	const matchesApprovedLaunchHint =
		...
		(approvedHint.workerCount == null ||
			approvedHint.workerCount === workerCount) &&
		(approvedHint.agentType == null || approvedHint.agentType === agentType) &&
		Boolean(approvedHint.linkedRalph) === false;
	const allowRepoAwareDagHandoff =
		followupContext != null || matchesApprovedLaunchHint;
...
export interface TeamExecutionPlan {
	...
	overOrchestrationNotice?: {
		code:
			| "team_over_orchestration_warning"
			| "explicit_team_override_acknowledged";
		message: string;
	};
}
...
export function buildTeamExecutionPlan(
	...
	const overOrchestrationNotice =
		plan.strategy === "atomic" && taskSize === "small"
			? explicitWorkerCount && workerCount > 1
				? {
						code: "explicit_team_override_acknowledged" as const,
						message:
							"Small atomic task is using Team fanout because the worker count was explicit.",
					}
				: workerCount > 1 && effectiveWorkerCount === 1
					? {
							code: "team_over_orchestration_warning" as const,
							message:
								"Small atomic task is capped to one Team worker; consider a solo execution path for lower overhead.",
						}
					: undefined
			: undefined;
...
export async function teamCommand(
	...
	if (subcommand === "status") {
		...
		const ultragoalReconciliation =
			await reconcilePersistedTeamUltragoalContext(
				config?.leader_cwd ?? cwd,
				ultragoalContext,
			);
		const currentUltragoalContext =
			ultragoalReconciliation.status === "valid"
		...
		if (ultragoalContextWarning) {
			console.log(
				`ultragoal_context_warning: ${ultragoalContextWarning.message}`,
			);
		}
		for (const line of renderUltragoalCheckpointGuidanceText(
			currentUltragoalContext,
		)) {
			console.log(line);
		}
			...
			: snapshot && snapshotHasDeadWorkerStall(snapshot)
				? await readTeamEvents(resolvedName, cwd, { wakeableOnly: true }).then(
						(events) => {
							...
							const fallbackEvent = buildDeadWorkerAwaitEvent(
								resolvedName,
								snapshot,
							);
	...
	if (subcommand === "shutdown") {
		...
		const summary = await shutdownTeam(resolvedName, cwd, {
			force,
			confirmIssues,
		});
	...
	if (executionPlan.overOrchestrationNotice) {
		console.log(
			`${executionPlan.overOrchestrationNotice.code}: ${executionPlan.overOrchestrationNotice.message}`,
		);
	}
	await renderStartSummary(runtime, staffingPlan);
}
