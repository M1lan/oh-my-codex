...
const COMPLETED_RALPH_STORY_STATUSES = new Set([
	"passed",
	"complete",
	"completed",
]);
...
function isStoryMarkedPassedOrCompleted(
...
function hasApprovedArchitectValidation(
	story: Record<string, unknown>,
): boolean {
	const candidates = [
		story.architect_validation,
		story.architectValidation,
		story.architect_review,
		story.architectReview,
	];
	for (const candidate of candidates) {
		...
		if (
			...
			APPROVED_RALPH_ARCHITECT_VERDICTS.has(
...
function readAndValidateRequiredRalphPrdJson(cwd: string): void {
	...
	try {
		...
		throw new Error(
			`[ralph] Invalid PRD.json at ${REQUIRED_RALPH_PRD_JSON_PATH}: ${error instanceof Error ? error.message : String(error)}`,
		);
	}

	if (!isRecord(parsed)) {
		throw new Error(
			`[ralph] Invalid PRD.json at ${REQUIRED_RALPH_PRD_JSON_PATH}: expected a JSON object.`,
		);
	}
	...
	if (!Array.isArray(parsed.userStories)) {
		throw new Error(
			`[ralph] Invalid PRD.json at ${REQUIRED_RALPH_PRD_JSON_PATH}: userStories must be an array when present.`,
		);
	}
	...
	for (const [index, story] of parsed.userStories.entries()) {
		...
		throw new Error(
			`[ralph] PRD.json ${describeStory(story, index)} is marked passed/completed without architect approval. Record architect_validation.verdict="approved" (or architect_review.verdict="approve") before running Ralph.`,
		);
	}
}
...
export function assertRequiredRalphPrdJson(
	cwd: string,
	...
	if (!existsSync(requiredPath)) {
		throw new Error(
			`[ralph] Missing required PRD.json at ${REQUIRED_RALPH_PRD_JSON_PATH}. Create the file before running \`omx ralph --prd ...\`.`,
		);
	}
...
export function extractRalphTaskDescription(
	args: readonly string[],
	fallbackTask?: string,
): string {
	const words: string[] = [];
...
function buildRalphApprovedContextLines(
	...
	if (approvedHint.deepInterviewSpecPaths.length > 0) {
		lines.push(
			`- deep-interview specs: ${approvedHint.deepInterviewSpecPaths.join(", ")}`,
		);
		lines.push(
			"- Carry forward the approved deep-interview requirements and constraints during Ralph execution and final verification.",
		);
	}
	if (approvedHint.repositoryContextSummary) {
		lines.push(
			`- approved repository context summary: ${approvedHint.repositoryContextSummary.sourcePath}${approvedHint.repositoryContextSummary.truncated ? " (bounded/truncated)" : ""}`,
		);
	...
	lines.push(
		"- Approved execution baseline is ready: use the approved plan, matching test specs, and any deep-interview artifacts as execution inputs.",
	);
	return lines;
}
...
export async function ralphCommand(args: string[]): Promise<void> {
	...
	const task =
		explicitTask === "ralph-cli-launch"
			? (approvedHint?.task ?? explicitTask)
			: explicitTask;
	const noDeslop = normalizedArgs.some(
		(arg) => arg.toLowerCase() === "--no-deslop",
	);
	...
	await updateModeState("ralph", {
		...
		native_subagent_policy:
			"Parallel Codex subagents are allowed for independent work only when each dispatch sets agent_type to an installed OMX role; phase completion must wait for active native subagent threads to finish.",
		goal_mode_integration: "codex-goal-tools",
		goal_mode_policy:
			"Use get_goal for active objective discovery and update_goal only after a prompt-to-artifact completion audit proves the objective is achieved.",
		...
		approved_deep_interview_spec_paths:
			approvedHint?.deepInterviewSpecPaths ?? [],
		...(artifacts.canonicalPrdPath
			? { canonical_prd_path: artifacts.canonicalPrdPath }
			: {}),
	});
	...
	if (artifacts.migratedProgress) {
		console.log(
			"[ralph] Migrated legacy progress -> " + artifacts.canonicalProgressPath,
		);
	}
	...
	const codexArgs =
		explicitTask === "ralph-cli-launch" && approvedHint?.task
