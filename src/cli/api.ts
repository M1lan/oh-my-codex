...
export function resolveApiBinaryPath(
	...
	const packagedCandidates = packagedApiBinaryCandidatePaths(
		packageRoot,
		platform,
		arch,
		env,
...
export async function resolveApiBinaryPathWithHydration(
	...
	const hydrated = await hydrateNativeBinary("omx-api", {
		packageRoot,
		env,
		platform,
		arch,
...
export async function apiCommand(args: string[]): Promise<void> {
	...
	if (result.error) {
		...
		if (kind === "missing")
			throw new Error(
				`[api] failed to launch native binary: executable not found (${binaryPath})`,
			);
		if (kind === "blocked")
			throw new Error(
				`[api] failed to launch native binary: executable is blocked (${errno.code || "blocked"})`,
			);
