...
import {
	inspectNativeArchive,
	selectNativeArchiveBinary,
	writeSelectedNativeArchiveMember,
} from "../native-assets/archive.js";
...
): Promise<{ version?: string; repository?: { url?: string } | string }> {
...
export function resolveCachedNativeBinaryPath(
	...
	const platformKey = libc
		? `${platform}-${arch}-${libc}`
		: `${platform}-${arch}`;
	return join(
		resolveNativeCacheRoot(env),
		version,
		platformKey,
		product,
		binary,
...
function inferRuntimeLibcFromText(
...
function detectLinuxRuntimeLibc(
	env: NodeJS.ProcessEnv = process.env,
): NativeLibc | undefined {
	if (process.platform !== "linux") return undefined;

	const lddProbe = spawnPlatformCommandSync(
		"ldd",
		["--version"],
		{ encoding: "utf-8" },
		process.platform,
	...
	const lddRuntime = inferRuntimeLibcFromText(
		`${lddProbe.result.stdout || ""}\n${lddProbe.result.stderr || ""}`,
	);
	if (lddRuntime) return lddRuntime;

	const getconfProbe = spawnPlatformCommandSync(
		"getconf",
		["GNU_LIBC_VERSION"],
		{ encoding: "utf-8" },
		process.platform,
	...
	const getconfRuntime = inferRuntimeLibcFromText(
		`${getconfProbe.result.stdout || ""}\n${getconfProbe.result.stderr || ""}`,
	);
	...
	for (const directory of MUSL_LOADER_DIRS) {
		...
		try {
			if (
				readdirSync(directory).some((entry) => MUSL_LOADER_PATTERN.test(entry))
			) {
				return "musl";
			}
		} catch {
...
export function inferNativeAssetLibc(
	asset: Pick<NativeReleaseAsset, "archive" | "target" | "libc">,
): NativeLibc | undefined {
	if (asset.libc === "musl" || asset.libc === "glibc") return asset.libc;
	const hint = [asset.target, asset.archive]
		.filter(Boolean)
		.join(" ")
		.toLowerCase();
...
export function resolveCachedNativeBinaryCandidatePaths(
	...
	if (platform === "linux") {
		for (const libc of options.linuxLibcPreference ??
			resolveLinuxNativeLibcPreference({ env })) {
			candidates.push(
	...
	candidates.push(
		resolveCachedNativeBinaryPath(product, version, platform, arch, env),
	);
...
export function resolveNativeReleaseAssetCandidates(
	...
	const preference =
		options.linuxLibcPreference ?? resolveLinuxNativeLibcPreference();
	const preferenceIndex = new Map(
		preference.map((libc, index) => [libc, index]),
	);
	return [...candidates].sort((left, right) => {
		const leftLibc = inferNativeAssetLibc(left);
		const rightLibc = inferNativeAssetLibc(right);
		const leftRank = leftLibc
			? (preferenceIndex.get(leftLibc) ?? preference.length + 1)
			: preference.length;
		const rightRank = rightLibc
			? (preferenceIndex.get(rightLibc) ?? preference.length + 1)
			: preference.length;
...
function isUnavailableManifestError(error: unknown): boolean {
	...
	return (
		/\[native-assets\] failed to fetch native release manifest/i.test(
			error.message,
...
function isUnavailableArchiveError(error: unknown): boolean {
	if (!(error instanceof Error)) return false;
	return (
		/\[native-assets\] failed to download /i.test(error.message) ||
		/fetch failed/i.test(error.message)
	);
}
...
interface OpenedFile extends FileIdentity {
	digest?: string;
	text?: string;
}
...
function sidecarPath(binaryPath: string): string {
	return `${binaryPath}${SIDECAR_SUFFIX}`;
}
function lockPath(binaryPath: string): string {
	return `${binaryPath}${LOCK_SUFFIX}`;
}
...
export function setNativeAssetsTestHooksForTests(
	hooks: NativeAssetsTestHooks | undefined,
): () => void {
	const previousHooks = nativeAssetsTestHooks;
	nativeAssetsTestHooks = hooks;
	return () => {
		nativeAssetsTestHooks = previousHooks;
	};
}
...
function canonicalDescendantPath(
	path: string,
...
async function validateDescendant(
	...
	if (
		...
		rel === ".." ||
		rel.startsWith(`..${sep}`) ||
		resolve(canonicalRoot, rel) !== candidate
	) {
		throw new Error("[native-assets] cache path escapes configured root");
	}
	...
	for (const [index, part] of parts.entries()) {
		...
		try {
			...
			if (!isLeaf && (entry.isSymbolicLink() || !entry.isDirectory()))
				throw new Error(
...
async function readOpenedFile(
	...
	try {
		...
		return {
			...
			text: hash ? undefined : Buffer.concat(chunks).toString("utf8"),
		};
	} finally {
		await handle.close();
	}
...
async function inspectLock(
	path: string,
	binaryPath: string,
): Promise<NativeCacheLockDiagnostic | undefined> {
	try {
		const file = await readOpenedFile(
			path,
			false,
			false,
		...
		if (!validLockRecord(record, text, binaryPath))
			return { path, classification: "metadata-malformed" };
		return {
			path,
		...
		return {
			path,
			classification: message.includes("race")
				? "inspection-race"
				: message.includes("unsafe")
					? "lock-unsafe"
					: error instanceof SyntaxError || message.includes("too large")
						? "metadata-malformed"
						: "metadata-unavailable",
		};
	}
...
export async function inspectManagedNativeBinary(
	binaryPath: string,
	env: NodeJS.ProcessEnv = process.env,
): Promise<ManagedNativeBinaryInspection> {
	...
	try {
		...
		effectiveBinaryPath = canonicalDescendantPath(
			binaryPath,
			configuredRoot,
			root,
		);
		...
		return {
			state:
				error instanceof Error && error.message.includes("race")
					? "inspection-race"
					: "cache-descendant-unsafe",
		};
	}
	const lockDiagnostic = await inspectLock(
		lockPath(effectiveBinaryPath!),
		effectiveBinaryPath!,
	);
	try {
		await validateDescendant(effectiveBinaryPath!, root!, false);
	...
	try {
		binary = await readOpenedFile(effectiveBinaryPath!, true, true);
	} catch (error) {
		if (!absent(error)) {
			const message = error instanceof Error ? error.message : "";
			return {
				state: message.includes("empty")
					? "binary-empty"
		...
		try {
			await lstat(sidecarPath(effectiveBinaryPath!));
			return { state: "orphan-checksum", lock: lockDiagnostic };
		} catch (sidecarError) {
			if (!absent(sidecarError))
				return { state: "inspection-race", lock: lockDiagnostic };
			return {
				state: lockDiagnostic ? "publication-in-progress" : "missing",
				lock: lockDiagnostic,
			};
		}
	}
	...
	try {
		sidecar = await readOpenedFile(
			sidecarPath(effectiveBinaryPath!),
			true,
			false,
			MAX_SIDECAR_BYTES,
		);
		...
		if (absent(error))
			return {
				state: lockDiagnostic ? "publication-in-progress" : "legacy-unverified",
				lock: lockDiagnostic,
			};
		return {
			state:
	...
	if (
		sidecar.size !== MAX_SIDECAR_BYTES ||
		!SHA256_LINE.test(sidecar.text || "")
	)
		return { state: "checksum-malformed", lock: lockDiagnostic };
	if (sidecar.text!.slice(0, 64) !== binary.digest)
		return { state: "checksum-mismatch", lock: lockDiagnostic };
	return {
		state: "verified",
		...
		try {
			...
			try {
				...
				if (bytesWritten !== recordBytes.length)
					throw new Error("[native-assets] incomplete lock write");
				...
				if (bytesRead !== readback.length || !readback.equals(recordBytes))
					throw new Error("[native-assets] lock readback mismatch");
				...
				if (!identity.isFile() || identity.nlink !== 1)
					throw new Error("[native-assets] unsafe publication lock");
			...
			if (performance.now() >= deadline) {
				const diagnostic = (await inspectLock(path, binaryPath)) ?? {
					path,
					classification: "metadata-unavailable" as const,
				};
				const owner = diagnostic.owner
					? ` owner=${JSON.stringify(diagnostic.owner)}`
					: "";
				throw new Error(
					`[native-assets] publication-lock-timeout: ${path}; elapsed=${Math.round(performance.now() - started)}ms deadline=${lockWaitMs(env)}ms; ${diagnostic.classification}${owner}. Confirm no OMX hydration process is active for this cache key, remove only this named lock manually, then retry.`,
				);
			}
...
): Promise<ManagedNativeBinaryInspection | undefined> {
	try {
		const reopened = await readOpenedFile(lock.path, false, false);
		if (!sameFile(lock.identity, reopened) || reopened.text !== lock.record)
			return { state: "cleanup-failed" };
		const current = await lstat(lock.path);
		if (
			!sameFile(lock.identity, {
				dev: current.dev,
				ino: current.ino,
				size: current.size,
			}) ||
		...
		return absent(error) ? undefined : { state: "cleanup-failed" };
	}
	return undefined;
}
...
async function publishManagedNativeBinary(
	source: string,
	destination: string,
	platform: NodeJS.Platform,
	env: NodeJS.ProcessEnv,
	...
	try {
		...
		try {
			await handle.writeFile(sidecar, "utf8");
		} finally {
			await handle.close();
		}
		if (
			(await readOpenedFile(tempSidecar, true, false, MAX_SIDECAR_BYTES))
				.text !== sidecar
		)
			throw new Error("[native-assets] temporary checksum verification failed");
		await revalidatePublicationPaths();
		...
		try {
			...
			cleanupFailures.push(
				`revalidate publication paths: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
		for (const temporary of [tempBinary, tempSidecar]) {
			try {
				...
				cleanupFailures.push(
					`remove ${temporary}: ${error instanceof Error ? error.message : String(error)}`,
				);
			}
		}
		...
		if (lockCleanup)
			cleanupFailures.push(
				`${lockCleanup.state}: publication lock was retained`,
			);
...
export async function hydrateNativeBinary(
	...
	for (const cachedBinaryPath of resolveCachedNativeBinaryCandidatePaths(
		product,
		version,
		platform,
		arch,
	...
	const assets = resolveNativeReleaseAssetCandidates(
		manifest,
		product,
		version,
		platform,
	...
	try {
		for (let index = 0; index < assets.length; index += 1) {
			...
			try {
				...
				const archiveBinary = selectNativeArchiveBinary(
					archiveEntries,
					asset.binary_path,
				);
				...
				const published = await publishManagedNativeBinary(
					extractedBinaryPath,
					cachedBinaryPath,
					platform,
					env,
