...
async function extractArchive(
	...
	if (ext === ".zip") {
		...
		if (result.status !== 0 || result.error) {
			throw new Error(
				`[native-assets] failed to extract zip archive ${archivePath}: ${(result.stderr || result.error?.message || "").trim()}`,
			);
		}
		return;
	}
	...
	if (result.status !== 0 || result.error) {
		throw new Error(
			`[native-assets] failed to extract archive ${archivePath}: ${(result.stderr || result.error?.message || "").trim()}`,
		);
	}
}

async function findExtractedBinaryPath(
	rootDir: string,
	binaryPath: string,
	...
	while (pending.length > 0) {
		...
		for (const entry of entries) {
			...
			if (
				relative === normalizedNeedle ||
				relative.endsWith(`/${normalizedNeedle}`)
			) {
				return fullPath;
			}
		}
	...
	try {
		for (let index = 0; index < assets.length; index += 1) {
			...
			try {
				...
				const extractedBinaryPath = await findExtractedBinaryPath(
					extractDir,
					asset.binary_path,
				);
				if (!extractedBinaryPath) {
					throw new Error(
						`[native-assets] extracted archive missing expected binary ${asset.binary_path}`,
					);
				}
