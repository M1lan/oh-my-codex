...
async function captureControlledTopology(
	...
	if (
		...
		throw new Error(
			`Refusing to process ${path}: it is outside controlled Codex scope ${root}.`,
		);
	}
	...
	for (const segment of [
		"",
		...relativeParent.split(/[\\/]/).filter(Boolean),
	]) {
		...
		try {
			...
			if (stat.isSymbolicLink() || !stat.isDirectory()) {
				throw new Error(
					`Refusing to process ${path}: controlled ancestor ${currentPath} must be a non-symbolic-link directory.`,
				);
			}
			ancestors.push({
				path: currentPath,
...
async function assertControlledTopologyCurrent(
	topology: FileTopology | null,
): Promise<void> {
	if (!topology) return;
	for (const ancestor of topology.ancestors) {
		try {
			...
			if (
				...
				throw new Error(
					`Refusing uninstall because controlled ancestor ${ancestor.path} changed topology after planning.`,
				);
			}
		} catch (error) {
			if (
				error instanceof Error &&
				error.message.includes("changed topology")
			) {
				throw error;
			}
...
async function readFileSnapshot(
	...
	if (before.isSymbolicLink() || !before.isFile() || before.nlink !== 1) {
		throw new Error(
			`Refusing to process ${path}: expected a regular file, not a symbolic link or other filesystem object.`,
		);
	}
	...
	if (
		...
		!after.isFile() ||
		after.nlink !== 1 ||
		!hasSameFileIdentity(fileIdentity(before), fileIdentity(after))
	) {
		throw new Error(
			`Refusing to process ${path}: it changed while uninstall was planning.`,
		);
	}
	return {
		path,
		content:
			options.strictUtf8 === false
				? bytes.toString("utf-8")
				: decodeStrictUtf8(path, bytes),
		bytes,
...
async function planHooksRemoval(
	...
	const shimPath = buildManagedCodexNativeHookWindowsShimPath(
		dirname(hooks.path),
	);
...
function invalidNotifyMetadata(
	path: string,
	reason: string,
): ManagedCodexHooksPlanError {
...
function parseNotifyMetadata(
	...
	if (snapshot.content === null) {
		throw invalidNotifyMetadata(
			snapshot.path,
			"the managed dispatcher metadata is missing",
		);
	}
	if (!currentNotify.includes(snapshot.path)) {
		throw invalidNotifyMetadata(
			snapshot.path,
			"the managed dispatcher does not reference the controlled metadata path",
		);
	}
	...
	if (metadata.managedBy !== "oh-my-codex" || metadata.version !== 1) {
		throw invalidNotifyMetadata(
			snapshot.path,
			"expected OMX ownership and version 1",
		);
	}
	...
	if (
		...
		throw invalidNotifyMetadata(
			snapshot.path,
			"dispatcherNotify does not match the managed dispatcher command",
		);
	}
	return {
		snapshot,
...
async function planNotifyMetadata(
	...
	if (
		configSnapshot.content === null ||
		!isManagedDispatcherNotify(configSnapshot.content)
	) {
		return undefined;
	}
...
function restorePreviousNotifyIfDispatcher(
	...
	if (!metadataPlan) {
		throw new Error(
			"Missing planned notification metadata for a managed dispatcher.",
		);
	}
	...
	try {
		...
		if (startIndex >= 0 && endIndex > startIndex) {
			...
			if (preserved) {
				...
				if (options.verbose)
					console.log(
						"  Removed OMX-managed AGENTS.md sections and preserved user guidance.",
					);
				return false;
			}
		}
...
function transactionTemporaryPath(
	...
	return (
		options.transactionTemporaryPath?.(path, purpose) ??
		join(
			dirname(path),
...
async function clearNativeHookClaimJournal(root: string): Promise<void> {
	return clearNativeHookClaimJournalWithDurability(
		root,
...
async function persistNativeHookClaimJournal(
	...
	return persistNativeHookClaimJournalWithDurability(
		root,
		entry,
		uninstallClaimJournalDurability(),
	);
...
async function syncNativeHookClaimParent(path: string): Promise<void> {
	return syncNativeHookClaimParentWithDurability(
		path,
		uninstallClaimJournalDurability(),
	);
}
...
function renamedDestinationOwnership(
	...
	if (
		...
		throw new Error(
			`Replacement temporary ${snapshot.path} was not fully captured before rename.`,
		);
	}
	return {
...
async function captureRenamedDestinationOwnership(
	...
	if (
		...
		throw new Error(
			`Uninstall replacement ownership changed for ${ownership.path}.`,
		);
	}
	return current;
}
...
async function assertSnapshotCurrent(snapshot: FileSnapshot): Promise<void> {
	...
	throw new Error(
		`Refusing uninstall because planned artifact ${snapshot.path} changed, was created, or was removed after planning.`,
	);
}
...
async function assertSnapshotAtPath(
	...
	if (
		...
		throw new Error(
			`Refusing uninstall because ${context} ${path} is not the planned artifact.`,
		);
	}
	return actual;
}
...
async function assertSnapshotsCurrent(
...
async function removeOwnedTemporary(
	...
	try {
		if (!snapshot) {
			throw new Error(
				`${description} path was not fully captured after writing.`,
			);
		}
		...
		try {
			await assertSnapshotAtPath(
				claimPath,
				snapshot,
				`${description} cleanup claim`,
			);
		} catch (error) {
			try {
...
async function atomicReplaceFile(
	...
	if (mutation.snapshot.mode === null) {
		throw new Error(
			`Refusing to replace ${mutation.snapshot.path}: uninstall did not plan a regular-file mode.`,
		);
	}
	const temporaryPath = transactionTemporaryPath(
	...
	try {
		...
		if (
			...
			throw new Error(
				`Read-back verification failed for replacement temporary ${temporaryPath}.`,
			);
		}
		await options.transactionFailureInjector?.(
		...
		if (!temporarySnapshot) {
			throw new Error(
				`Replacement temporary ${temporaryPath} was not fully captured before install.`,
			);
		}
		...
		try {
			await syncUninstallRegularFile(
				"installed-destination",
				installedHandle,
				options,
			);
		...
		if (
			...
			throw new Error(
				`Read-back verification failed for replacement ${mutation.snapshot.path}.`,
			);
		}
		...
		if (stage === "write" && mutation.validate) {
			mutation.validate(
				decodeStrictUtf8(mutation.snapshot.path, verified.bytes!),
			);
		}
		return verified;
	} catch (error) {
...
async function stageAndRemoveFile(
	...
	if (mutation.snapshot.bytes === null || mutation.snapshot.mode === null) {
		throw new Error(
			`Refusing to remove ${mutation.snapshot.path}: uninstall did not plan a regular-file snapshot.`,
		);
	}
	const stagedPath = transactionTemporaryPath(
		mutation.snapshot.path,
		"delete",
		options,
	);
	...
	try {
		...
		if (absent.bytes !== null) {
			throw new Error(
				`Read-back verification failed for ${mutation.snapshot.path}: expected absence.`,
			);
		}
...
function mutationStage(
	kind: UninstallArtifactKind,
): UninstallTransactionFailureStage {
	switch (kind) {
		case "hooks":
...
async function captureRollbackOwnedSnapshot(
	...
	throw new Error(
		`Refusing stale rollback for ${execution.mutation.snapshot.path}: the mutation did not produce a verifiable state.`,
	);
}
...
async function restoreFileSnapshot(
	...
	if (mutation.snapshot.bytes === null) {
		...
		if (absent.bytes !== null) {
			throw new Error(
				`Uninstall rollback mismatch for ${mutation.snapshot.path}: expected absence.`,
			);
		}
		return;
	}
...
async function assertUnmutatedTransactionPreconditions(
	...
	await assertSnapshotsCurrent(
		transaction.preconditions.filter(
			(snapshot) => !mutatedPaths.has(snapshot.path),
		),
	);
}
...
async function assertAppliedTransactionSnapshots(
	...
	for (const execution of executions) {
		...
		if (!execution.appliedSnapshot) {
			throw new Error(
				`Refusing uninstall because ${execution.mutation.snapshot.path} did not register an applied snapshot.`,
			);
		}
		await assertSnapshotCurrent(execution.appliedSnapshot);
	}
...
async function commitUninstallArtifactTransaction(
	...
	const mutations = [
		...
		(mutation): mutation is PlannedFileMutation => mutation !== undefined,
	);
...
export async function uninstall(options: UninstallOptions = {}): Promise<void> {
	...
	const artifactTransaction: UninstallArtifactTransaction = {
		...
		...(hooksRemoval.shim !== undefined && hooksRemoval.shim.bytes !== null
			...
			: {}),
		...(configCleanup?.result.configCleaned &&
		configCleanup.finalContent !== null
			? {
					config: {
						kind: "config",
	...
	summary.hooksFileRemoved =
		artifactTransaction.hooks !== undefined ||
		artifactTransaction.shim !== undefined;
	...
	if (keepConfig) {
		...
		if (configCleanup?.config.content === null) {
			...
			console.log(
				`  ${dryRun ? "Would clean" : "Cleaned"} ${scopeDirs.codexConfigFile}`,
			);
		} else {
