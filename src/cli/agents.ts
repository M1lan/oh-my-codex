...
export const RESERVED_NATIVE_AGENT_NAMES = new Set([
	"default",
	"worker",
	"explorer",
]);
...
function inferMutationScope(cwd = process.cwd()): AgentScope {
	...
	if (existsSync(persistedScopePath)) {
		try {
			const parsed = JSON.parse(readFileSync(persistedScopePath, "utf8")) as {
				scope?: string;
			};
...
async function readScopeAgents(
	scope: AgentScope,
	cwd = process.cwd(),
...
export async function listNativeAgents(
	...
	return [...projectAgents, ...userAgents].sort(
		(a, b) => a.name.localeCompare(b.name) || a.scope.localeCompare(b.scope),
	);
}
...
function resolveExistingAgentPath(
	...
	const cwd = options.cwd ?? process.cwd();
	const normalized = normalizeAgentName(name);
	const candidateScopes: AgentScope[] = options.scope
		? [options.scope]
		: ["project", "user"];
...
async function confirmRemove(path: string): Promise<boolean> {
	...
	try {
		const answer = (await rl.question(`Delete native agent ${path}? [y/N]: `))
			.trim()
			.toLowerCase();
...
async function editNativeAgent(
	...
	const editor =
		options.editor ?? process.env.EDITOR ?? process.env.VISUAL ?? "vi";
...
export async function agentsCommand(args: string[]): Promise<void> {
	...
	switch (subcommand) {
		...
		case "add": {
			...
			const path = await addNativeAgent(name, {
				cwd: process.cwd(),
				scope,
				force,
			});
		...
		case "remove": {
			...
			const path = await removeNativeAgent(name, {
				cwd: process.cwd(),
				scope,
				force,
			});
