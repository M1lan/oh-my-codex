...
function readValue(args: readonly string[], flag: string): string | undefined {
	...
	if (!value || value.startsWith("--"))
		throw new AutoresearchGoalError(`Missing value for ${flag}.`);
	return value;
}
	...
	try {
		...
		if (command === "verdict" || command === "checkpoint") {
			...
			if (json) printJson({ ok: true, ...result });
				...
				console.log(
					`autoresearch-goal verdict: ${result.mission.slug} -> ${result.completion.verdict}`,
				);
			return;
		}
		...
		if (command === "complete") {
			...
			if (json) printJson({ ok: true, ...result });
				...
				console.log(
					"Codex goal reconciliation: matched a fresh complete get_goal snapshot; OMX mission completion is now durable.",
				);
				console.log(
		...
		if (command === "status") {
			...
			if (json) printJson({ mission, completion, reconciliation });
				...
				console.log(
					`autoresearch-goal: ${mission.slug} [${mission.status}] ${mission.topic}`,
				);
				console.log(
					`completion: ${completion ? `${completion.verdict} (${completion.recorded_at})` : "missing"}`,
				);
		...
		if (
			error instanceof AutoresearchGoalError ||
