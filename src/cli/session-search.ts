...
import {
	...
	type SessionFrictionOptions,
} from "../session-history/friction.js";
import {
...
export function parseSessionFrictionArgs(
	args: string[],
): ParsedSessionFrictionArgs {
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		throw new Error(
			`Unexpected positional argument for friction report: ${token}`,
		);
	}
...
function formatFrictionReport(report: SessionFrictionReport): string {
	...
	for (const session of report.sessions) {
		...
		lines.push(
			`cwd: ${session.cwd_basename ?? "unknown"} (${session.cwd_hash ? `hash:${session.cwd_hash}` : "hash:unknown"})`,
		);
		lines.push(
			`activity: started=${session.started_at ?? "unknown"} last=${session.last_activity_at ?? "unknown"} idle=${session.idle_minutes ?? "unknown"}m age=${session.age_minutes ?? "unknown"}m`,
		);
		lines.push(
			`counts: records=${session.counters.records} user_turns=${session.counters.user_turns} assistant_turns=${session.counters.assistant_turns} tool_calls=${session.counters.tool_calls} tool_outputs=${session.counters.tool_outputs}`,
		);
		lines.push(
			`size: approx=${session.context_growth.approx_transcript_kb}KB avg_record=${session.context_growth.avg_record_bytes}B tool_ratio=${session.context_growth.tool_call_ratio}`,
		);
		lines.push(
			`idle_gaps: max=${session.idle_gaps.max_gap_minutes ?? "none"}m over_30m=${session.idle_gaps.gaps_over_30m} over_2h=${session.idle_gaps.gaps_over_2h}`,
		);
		if (session.tool_names.length > 0) {
			lines.push(
				`tools: ${session.tool_names.map((tool) => `${tool.name}(${tool.count})`).join(", ")}`,
			);
		}
		lines.push(
			`risks: ${session.risks.map((risk) => `${risk.severity}:${risk.code}`).join(", ")}`,
		);
		lines.push(
			`source: ${session.source.codex_home}:ref:${session.source.transcript_ref}`,
		);
	}
...
function formatLockResult(result: {
	...
	if (result.recovered !== undefined)
		lines.push(`recovered: ${result.recovered ? "yes" : "no"}`);
...
async function sessionLockCommand(args: string[]): Promise<void> {
	...
	if (!operation || HELP_TOKENS.has(operation)) {
		console.log(
			`Usage: omx session lock <inspect|recover> [--cwd <path>] [--json]`,
		);
		return;
	}
	...
	console.log(
		parsed.json ? JSON.stringify(result, null, 2) : formatLockResult(result),
	);

	if (
		operation === "recover" &&
		result.status !== "absent" &&
		(!("recovered" in result) || result.recovered !== true)
	) {
		process.exitCode = 1;
	}
}
