...
function defaultResumeInstruction(
	...
	const generatedDir =
		record.generated_images_dir || "$CODEX_HOME/generated_images/<session>";
...
export async function prepareImagegenContinuation(
	...
	const baseRecord: Omit<ImagegenContinuationRecord, "resume_instruction"> = {
		...
		after,
		status: "pending",
		...(options.generatedImagesDir?.trim()
			? { generated_images_dir: options.generatedImagesDir.trim() }
			: {}),
	...
	const record: ImagegenContinuationRecord = {
		...
		resume_instruction:
			...
			defaultResumeInstruction(baseRecord, pendingPath),
	};
...
export function parseImagegenContinuationArgs(
	args: string[],
): ParsedImagegenContinuationArgs {
	...
	for (let i = 0; i < rest.length; i += 1) {
		...
		if (arg === "--json") {
			...
			resumeInstruction = readFileForCli(
				arg.slice("--resume-instruction-file=".length),
			);
...
export async function imagegenCommand(
	args: string[],
	cwd = process.cwd(),
): Promise<void> {
	if (
