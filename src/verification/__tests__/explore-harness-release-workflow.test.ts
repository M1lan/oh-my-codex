...
function flattenReleaseArtifacts(sourcePaths: string[]): string[] {
	...
	for (const source of sourcePaths) {
		...
		if (destinations.has(basename)) {
			throw new Error(
				`release asset basename collision: ${source} -> release-assets/${basename}`,
			);
		}
...
describe("native release workflow", () => {
	it("defines a unified tag workflow that publishes both Rust binaries before publishing the package", () => {
		...
		assert.doesNotMatch(
			workflow,
			/scripts\/generate-native-release-manifest\.mjs/,
		);
		...
		assert.match(
			workflow,
			/verify-version-sync:[\s\S]*Verify version sync against workspace crates[\s\S]*node --input-type=module/,
		);
		assert.match(
			workflow,
			/publish-native-assets:[\s\S]*pnpm run build[\s\S]*node dist\/scripts\/generate-native-release-manifest\.js/,
		);
		assert.match(
			workflow,
			/publish-native-assets:[\s\S]*Generate release body[\s\S]*node dist\/scripts\/generate-release-body\.js --template RELEASE_BODY\.md --out RELEASE_BODY\.generated\.md/,
		);
		assert.match(workflow, /body_path:\s*RELEASE_BODY\.generated\.md/);
		assert.match(
			workflow,
			/smoke-verify-native:[\s\S]*pnpm run build[\s\S]*node dist\/scripts\/verify-native-release-assets\.js/,
		);
		assert.match(
			workflow,
			/smoke-packed-install:[\s\S]*pnpm run build[\s\S]*Smoke test packed install boot \+ core commands[\s\S]*pnpm run smoke:packed-install/,
		);
		assert.match(
			workflow,
			/publish-npm:[\s\S]*Verify version sync against workspace crates[\s\S]*pnpm pack/,
		);

		const manifestGeneration = workflow.indexOf(
			"Generate release manifest from cargo-dist plan",
		);
		const verification = workflow.indexOf(
			"Verify release archives and manifest",
		);
		const bundleUpload = workflow.indexOf(
			"Upload release asset bundle for follow-up jobs",
		);
		const releaseUpload = workflow.indexOf(
			"Attach native assets to GitHub Release",
		);
		assert.ok(
			manifestGeneration >= 0,
			"release workflow must generate the manifest",
		);
		assert.ok(
			verification > manifestGeneration,
			"release workflow must verify after manifest generation",
		);
		assert.ok(
			bundleUpload > verification,
			"release bundle upload must occur after verification",
		);
		assert.ok(
			releaseUpload > bundleUpload,
			"GitHub Release upload must occur after the verified bundle upload",
		);
		assert.match(
			workflow,
			/Verify release archives and manifest[\s\S]*node dist\/scripts\/verify-native-release-assets\.js/,
		);

		const bundleUploadBlock = workflow.slice(
			bundleUpload,
			workflow.indexOf("Generate release body", bundleUpload),
		);
		const releaseUploadBlock = workflow.slice(
			releaseUpload,
			workflow.indexOf("Write release summary", releaseUpload),
		);
		assert.doesNotMatch(
			bundleUploadBlock,
			/^\s+if:/m,
			"bundle upload must retain the default success() condition",
		);
		assert.doesNotMatch(
			releaseUploadBlock,
			/^\s+if:/m,
			"release attachment must retain the default success() condition",
		);
		for (const uploadName of [
			...
			assert.equal(
				releaseUploadRunsAfterVerifier(workflow, uploadName, "success"),
				true,
				`${uploadName} must run after successful verification`,
			);
			assert.equal(
				releaseUploadRunsAfterVerifier(workflow, uploadName, "failure"),
				false,
				`${uploadName} must not run when verification fails`,
			);
		}
	});
	...
	it("fails closed before manifest generation when downloaded artifacts share a basename", () => {
		...
		for (const fixture of [
			duplicateArchiveBasenameFixture,
			duplicateChecksumBasenameFixture,
		]) {
			assert.throws(
		...
		const copy = workflow.indexOf(
			'cp -- "$source" "$destination"',
			manifestGeneration,
		);

		assert.ok(
			collisionCheck > manifestGeneration,
			"collision detection must be in the artifact collection step",
		);
		assert.ok(
			copy > collisionCheck,
			"collision detection must run before copying an artifact basename",
		);
		assert.ok(
			verification > copy,
			"verification must not run when collision detection blocks manifest generation",
		);
		assert.ok(
			bundleUpload > verification,
			"upload must not run when collision detection blocks manifest generation or verification",
		);
		assert.match(
			workflow,
			/destination="release-assets\/\$\(basename "\$source"\)"[\s\S]*if \[\[ -e "\$destination" \|\| -L "\$destination" \]\]; then[\s\S]*exit 1[\s\S]*cp -- "\$source" "\$destination"/,
		);
	});
	...
	it("keeps cargo-dist Linux targets aligned with musl-first plus glibc fallback assets", () => {
		...
		assert.equal(
			existsSync(distWorkspacePath),
			true,
			`missing cargo-dist config: ${distWorkspacePath}`,
		);
	...
	it("retires the old explore-only release workflow", () => {
		const legacyWorkflowPath = join(
			...
			".github",
			"workflows",
			"explore-harness-artifacts.yml",
		);
		assert.equal(
			existsSync(legacyWorkflowPath),
			false,
			`legacy workflow should be removed: ${legacyWorkflowPath}`,
		);
	});
});
