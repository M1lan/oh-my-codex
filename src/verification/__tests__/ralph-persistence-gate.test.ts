...
describe("ralph persistence release gate artifacts", () => {
	it("requires baseline and parity reference docs with pinned commit/hash evidence", () => {
		...
		const baselinePath = join(
			...
			"docs",
			"reference",
			"ralph-upstream-baseline.md",
		);
		const parityPath = join(
			root,
			"docs",
			"reference",
			"ralph-parity-matrix.md",
		);
	...
	it("requires contract and QA gate docs with V1-V10 coverage", () => {
		const root = repoRoot();
		const stateContract = join(
			root,
			"docs",
			"contracts",
			"ralph-state-contract.md",
		);
		const cancelContract = join(
			root,
			"docs",
			"contracts",
			"ralph-cancel-contract.md",
		);
	...
	it("requires CI workflow gate job that runs the Ralph persistence matrix tests", () => {
		...
		assert.match(ci, /pnpm run test:ralph-persistence:compiled/);
	});
});
