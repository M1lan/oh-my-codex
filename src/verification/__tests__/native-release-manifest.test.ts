...
async function makeTarGz(
	...
	archive.entry(
		{ name: binaryName, type: "file", size: Buffer.byteLength(contents) },
...
describe("native release manifest generator", () => {
	it("annotates Linux libc variants and sorts musl assets before glibc fallbacks", async () => {
		...
		try {
			...
			await writeFile(
				join(artifactsDir, `${muslArchive}.sha256`),
				`${createHash("sha256").update(muslBytes).digest("hex")}  ${muslArchive}\n`,
			);
			await writeFile(join(artifactsDir, glibcArchive), glibcBytes);
			await writeFile(
				join(artifactsDir, `${glibcArchive}.sha256`),
				`${createHash("sha256").update(glibcBytes).digest("hex")}  ${glibcArchive}\n`,
			);
				...
				JSON.stringify(
					{
						...
						releases: [{ app_name: "omx-sparkshell", app_version: "0.10.2" }],
						artifacts: {
							linuxGlibc: {
								kind: "executable-zip",
								name: glibcArchive,
			...
			const result = spawnSync(
				...
				[
					join(
						...
						"dist",
						"scripts",
						"generate-native-release-manifest.js",
					),
					"--plan",
					planPath,
					"--artifacts-dir",
	...
	it("uses the executable path basename for Windows cargo-dist assets", async () => {
		const root = await mkdtemp(
			join(tmpdir(), "omx-native-release-manifest-windows-"),
		);
		try {
			const artifactsDir = join(root, "artifacts");
			await mkdir(artifactsDir, { recursive: true });
					...
					artifacts: {
						windows: {
							...
							assets: [
								{
									kind: "executable",
									id: "omx-api-exe-x86_64-pc-windows-msvc",
									name: "omx-api",
									path: "omx-api.exe",
								},
							],
	...
	it("generates and verifies a complete cargo-dist release manifest, including non-hydratable omx-runtime", async () => {
		const root = await mkdtemp(
			join(tmpdir(), "omx-native-release-manifest-full-"),
		);
		try {
			const artifactsDir = join(root, "artifacts");
			await mkdir(artifactsDir, { recursive: true });
			const artifacts = [
				...
				},
				{
					...
					binaryPath: "omx-explore-harness",
				...
				},
				{
					...
					product: "omx-runtime",
					target: "x86_64-unknown-linux-musl",
					archive: "omx-runtime-x86_64-unknown-linux-musl.tar.gz",
					binaryPath: "omx-runtime",
					format: "tar.gz" as const,
				},
				{
					key: "sparkshellLinux",
			...
			for (const artifact of artifacts) {
				...
				await writeFile(
					join(artifactsDir, `${artifact.archive}.sha256`),
					`${createHash("sha256").update(bytes).digest("hex")}  ${artifact.archive}\n`,
				);
			}
			const planPath = join(root, "dist-plan.json");
			await writeFile(
				...
				JSON.stringify({
					...
					releases: artifacts.map(({ product }) => ({
						app_name: product,
						app_version: "0.20.3",
					})),
			...
			const generated = spawnSync(
				...
				[
					...
					outputPath,
					"--release-base-url",
					"https://github.com/example/releases/download/v0.20.3",
					"--require-products",
					"omx-api,omx-explore-harness,omx-sparkshell",
				],
				{ cwd: process.cwd(), encoding: "utf-8" },
			);
			assert.equal(generated.status, 0, generated.stderr || generated.stdout);
			const manifest = JSON.parse(await readFile(outputPath, "utf-8")) as {
				assets: Array<{ product: string; binary: string; binary_path: string }>;
			};
			assert.deepEqual(
				new Set(manifest.assets.map((asset) => asset.product)),
				new Set(artifacts.map((artifact) => artifact.product)),
			);
			const windowsApi = manifest.assets.find(
				(asset) => asset.product === "omx-api",
			);
			...
			const verified = spawnSync(
				...
				[
					join(
						...
						"dist",
						"scripts",
						"verify-native-release-assets.js",
					),
					"--manifest",
					outputPath,
					"--artifacts-dir",
	...
	it("rejects the same closed-product, binary, path, suffix, and archive-hint violations as release verification", () => {
		const asset = {
			...
			target: "x86_64-unknown-linux-musl",
			libc: "musl",
			archive: "omx-api-x86_64-unknown-linux-musl.tar.gz",
			binary: "omx-api",
			binary_path: "omx-api",
			sha256: "a".repeat(64),
			size: 1,
			download_url:
				"https://example.invalid/omx-api-x86_64-unknown-linux-musl.tar.gz",
		};
		const assertInvalid = (
