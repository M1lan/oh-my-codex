...
function jobBlock(workflow: string, jobName: string): string {
	...
	assert.ok(
		...
		`missing CI job block for ${jobName}`,
	);

	const start = startMatch.index + startMatch[1].length;
	const afterJobHeader = start + `  ${jobName}:\n`.length;
	const nextJobOffset = workflow
		.slice(afterJobHeader)
		.search(/\n  [a-z0-9-]+:\n/);
	const end =
		nextJobOffset === -1 ? workflow.length : afterJobHeader + nextJobOffset;
	return workflow.slice(start, end);
}

function assertJobIf(
	workflow: string,
	jobName: string,
	expected: RegExp,
): void {
	assert.match(
		...
		expected,
		`${jobName} should have the expected lane predicate`,
	);
}

type GitHubJobResult = "success" | "failure" | "cancelled" | "skipped";

function satisfiesCiStatusContract(
...
function classifyChangedPaths(paths: string[]): Record<string, string> {
	...
	assert.ok(
		match?.groups?.script,
		"missing inline changed-path classifier script",
	);
...
describe("CI Rust gates", () => {
	...
	it("detects changed-file lanes once and exposes fail-closed outputs for targeted CI", () => {
		...
		for (const outputName of [
			...
			assert.equal(
				changesJob.includes(
					`${outputName}: $` + `{{ steps.classify.outputs.${outputName} }}`,
				),
				true,
			);
		}
	...
	it("classifies docs, TypeScript, Rust/native, shared config, renames, and unknown paths explicitly", () => {
		...
		assert.match(
			changesJob,
			/if \(\/\^\[RC\]\/\.test\(status\)\) return parts\.slice\(1, 3\)/,
		);
		assert.match(changesJob, /function isDocs\(path\)/);
		assert.match(changesJob, /\^docs\\\//);
		assert.match(
			changesJob,
			/README\|CHANGELOG\|CONTRIBUTING\|COVERAGE\|DEMO\|RELEASE_BODY\|RELEASE_PROTOCOL/,
		);
	...
	it("keeps unknown .gjc paths fail-closed while preserving normal TS/config behavior", () => {
		...
		const sourceChange = classifyChangedPaths([
			"src/verification/ci-status.ts",
		]);
		assert.equal(sourceChange.full_suite, "false");
		assert.equal(sourceChange.ts_changed, "true");

		const compatFixtureChange = classifyChangedPaths([
			"src/compat/fixtures/doctor/install-onboarding.stdout.txt",
		]);
	...
	it("reports lane-selection reason and actual failing gates in CI status output", () => {
		...
		assert.match(
			statusJob,
			/Lane-selection reason: \$\{REASON:-unknown\}; full_suite=\$\{FULL_SUITE:-unknown\}/,
		);
	});
	...
	it("gates expensive jobs by lane while preserving full-suite fail-closed behavior", () => {
		...
		assertJobIf(
			workflow,
			"docs-check",
			/full_suite == 'true'.*docs_changed == 'true'/s,
		);
		for (const jobName of ["rustfmt", "clippy", "rust-tests"]) {
			assertJobIf(
				workflow,
				jobName,
				/full_suite == 'true'.*rust_changed == 'true'.*native_changed == 'true'/s,
			);
		}
		for (const jobName of [
			...
			assertJobIf(
				workflow,
				jobName,
				/full_suite == 'true'.*ts_changed == 'true'.*shared_config_changed == 'true'/s,
			);
		}
		assertJobIf(
			workflow,
		...
		assertJobIf(
			workflow,
			"build",
			/full_suite == 'true'.*rust_changed == 'true'.*native_changed == 'true'.*shared_config_changed == 'true'/s,
		);
	});
	...
	it("requires rustfmt, clippy, and Rust test coverage gates plus an explicit Rust toolchain setup for the final native build lane", () => {
		...
		assert.doesNotMatch(
			rustTestsJob,
			/--lcov|output-path|Upload Rust coverage artifact/,
		);
	...
	it("reuses a prebuilt dist artifact on every compiled lane and also builds it for Rust/native changes", () => {
		...
		assert.match(
			workflow,
			/^  coverage-team-critical:\s*\n(?:.*\n)*?^\s+needs:\s*\[changes, build-dist\]/m,
		);
		assert.match(
			workflow,
			/^  ralph-persistence-gate:\s*\n(?:.*\n)*?^\s+needs:\s*\[changes, build-dist\]/m,
		);
		assert.match(
			workflow,
			/^  build:\s*\n(?:.*\n)*?^\s+needs:\s*\[changes, build-dist\]/m,
	...
	it("defines the native-cache-integrity matrix and aggregate-status contract exactly", () => {
		...
		assert.match(
			nativeCacheJob,
			/^    if: \$\{\{ needs\.changes\.outputs\.full_suite == 'true' \|\| needs\.changes\.outputs\.ts_changed == 'true' \|\| needs\.changes\.outputs\.native_changed == 'true' \|\| needs\.changes\.outputs\.shared_config_changed == 'true' \}\}$/m,
		);
		assert.match(
			nativeCacheJob,
		...
		assert.match(nativeCacheJob, /uses:\s*pnpm\/action-setup@v4/);
		assert.match(
			nativeCacheJob,
			/uses:\s*actions\/setup-node@v7\s*\n\s+with:\s*\n\s+node-version:\s*20\s*\n\s+cache:\s*pnpm/,
		);
		assert.match(
			nativeCacheJob,
			/run:\s*pnpm install --frozen-lockfile --ignore-scripts/,
		);
		assert.match(
			nativeCacheJob,
			/name:\s*Download prebuilt dist artifact\s*\n\s+uses:\s*actions\/download-artifact@v8\s*\n\s+with:\s*\n\s+name:\s*ci-dist-node20\s*\n\s+path:\s*dist/,
		);
		assert.match(
			nativeCacheJob,
		...
		assert.match(
			ciStatusJob,
			/^    needs:\s*\[changes, docs-check, rustfmt, clippy, rust-tests, native-rust-darwin, native-rust-windows, lint, typecheck, build-dist, ralplan-preflight-macos, test, coverage-team-critical, ralph-persistence-gate, native-cache-integrity, build\]$/m,
		);
		assert.match(
			ciStatusJob,
	...
	it("models CI-status active and inactive result semantics deterministically", () => {
		...
		for (const { active, result, accepted } of cases) {
			assert.equal(
				satisfiesCiStatusContract(result, active),
				accepted,
	...
	it("marks every active CI lane as required and reported in the CI status gate while accepting inactive skipped lanes", () => {
		...
		for (const jobName of requiredJobs) {
			...
			assert.equal(
				ciStatusJob.includes(
					`${envName}_RESULT: $` + `{{ needs.${jobName}.result }}`,
				),
				true,
				`${jobName} result should be captured`,
			);
			assert.match(
				ciStatusJob,
				new RegExp(`echo "  ${jobName}: \\$${envName}_RESULT"`),
