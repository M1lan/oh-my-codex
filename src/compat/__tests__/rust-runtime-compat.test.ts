	...
	it("keeps team status on the manifest-authored compatibility view", async () => {
		...
		try {
			...
			await withTempTeamStateRoot(teamStateRoot, async () => {
				...
				const result = runOmx(
					wd,
					["team", "status", "rust-compat-team", "--json"],
			...
			await withTempTeamStateRoot(teamStateRoot, async () => {
						...
						[
							{
								...
								kind: "mailbox",
								team_name: "rust-compat-bridge",
								to_worker: "worker-2",
								trigger_message: "legacy",
								status: "pending",
								attempt_count: 0,
				...
				const queued = await enqueueDispatchRequest(
					"rust-compat-bridge",
					{
						kind: "mailbox",
						to_worker: "worker-2",
						message_id: "bridge-msg",
						trigger_message: "bridge trigger",
					},
					wd,
				);
				...
				assert.equal(
					dispatch.some(
						(entry) => entry.request_id === queued.request.request_id,
					),
					true,
				);
				assert.equal(
					dispatch.some((entry) => entry.request_id === "legacy-only"),
					false,
					"bridge compat view should win over stale legacy dispatch file",
				);
				assert.equal(
					mailbox.some((entry) => entry.message_id === message.message_id),
					true,
				);
				assert.equal(
					mailbox.some((entry) => entry.message_id === "legacy-msg"),
					false,
					"bridge compat view should win over stale legacy mailbox file",
				);
			});
		} finally {
