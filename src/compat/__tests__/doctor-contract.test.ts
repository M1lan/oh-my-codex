...
function normalizeInstallDoctorOutput(
	text: string,
	home: string,
	cwd: string,
): string {
	...
	return text
		...
		.map((line) => {
			if (
				line.startsWith("  [OK] Codex CLI:") ||
				line.startsWith("  [XX] Codex CLI:")
			) {
			...
			if (
				line.startsWith("  [OK] Explore Harness:") ||
				line.startsWith("  [!!] Explore Harness:")
			) {
...
describe("compat doctor contract", () => {
	it("matches onboarding warning copy for first setup expectations", async () => {
		...
		try {
			const result = runCompatTarget(wd, ["doctor"], {
				HOME: home,
			...
			assert.equal(
				result.status,
				Number.parseInt(
					readFixture("install-onboarding.exitcode.txt").trim(),
					10,
				),
			...
			assert.equal(
				normalizeInstallDoctorOutput(result.stdout, home, wd),
				readFixture("install-onboarding.stdout.txt"),
			);
			assert.ok(
				result.stdout.includes(SAFE_DOCTOR_RECOVERY) ||
					result.stdout.includes(SAFE_DOCTOR_FAILURE),
			);
			assert.doesNotMatch(
				result.stdout,
				/^Review (?:warnings|failed checks) above\.[^\n]*--force/m,
			);
		} finally {
	...
	it("matches doctor --team resume_blocker behavior", async () => {
		...
		try {
			...
			await writeFile(
				...
				JSON.stringify({ name: "alpha", tmux_session: "omx-team-alpha" }),
			);
			...
			assert.equal(
				result.status,
				Number.parseInt(
					readFixture("team-resume-blocker.exitcode.txt").trim(),
					10,
				),
				result.stderr || result.stdout,
			);
			...
			assert.equal(
				...
				readFixture("team-resume-blocker.stdout.txt"),
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
