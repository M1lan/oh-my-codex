...
export const DOCUMENT_REFRESH_EXEMPTION_PREFIX =
...
function isTriggerOnlyExcluded(record: ChangedPathRecord): boolean {
	...
	return (
		pathMatchesAny(path, TOOLING_ONLY_GLOBS) ||
		pathMatchesAny(path, RELEASE_COLLATERAL_GLOBS)
	);
}
...
export function hasDocumentRefreshExemption(
	text: string | null | undefined,
): boolean {
	if (!text) return false;
	return text
		.split(/\r?\n/u)
		.some((line) => line.trim().startsWith(DOCUMENT_REFRESH_EXEMPTION_PREFIX));
}
...
function recordMatchesRuleSource(
	...
	return (
		candidates.some((path) => pathMatchesAny(path, rule.sourceGlobs)) &&
...
function hasRuleRefresh(
	...
	const changedRefresh = changes.some((record) => {
		if (isRenameOnly(record)) return false;
		return changedPathCandidates(record).some((path) =>
			pathMatchesAny(path, rule.refreshTargets),
		);
	});
	if (changedRefresh) return true;
	return localFreshTargets.some((path) =>
		pathMatchesAny(path, rule.refreshTargets),
	);
}
...
export function evaluateDocumentRefresh(
	...
	const localFreshTargets =
		input.scope === "final-handoff"
	...
	for (const rule of rules) {
		...
		const changedPaths = unique(
			triggering.filter((path) => pathMatchesAny(path, rule.sourceGlobs)),
		);
	...
	const triggeringPaths = unique(
		warnings.flatMap((warning) => warning.changedPaths),
	);
	const expectedTargets = unique(
		warnings.flatMap((warning) => warning.refreshTargets),
	);
	return {
		scope: input.scope,
		rules: warnings,
...
export function isFinalHandoffDocumentRefreshCandidate(
	text: string | null | undefined,
): boolean {
...
export function formatDocumentRefreshWarning(
	warning: DocumentRefreshWarning,
): string {
	const seam =
		warning.scope === "commit"
	...
	const ruleLines = warning.rules
		.map((rule) => `- ${rule.ruleId}: ${rule.description}`)
		.join("\n");
	const pathLines = warning.triggeringPaths
		.slice(0, 8)
		.map((path) => `- ${path}`)
		.join("\n");
	const targetLines = warning.expectedTargets
		.slice(0, 10)
		.map((path) => `- ${path}`)
		.join("\n");
	return [
...
function readGitNameStatus(
	cwd: string,
	args: string[],
): ChangedPathRecord[] | null {
	try {
...
export function readStagedAndUnstagedGitChanges(
	cwd: string,
): ChangedPathRecord[] | null {
