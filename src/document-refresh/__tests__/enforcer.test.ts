...
describe("document refresh evaluator", () => {
	it("triggers on mapped native-hook source without docs/spec refresh", () => {
		...
		assert.deepEqual(warning.triggeringPaths, [
	...
	it("does not suppress native-hook changes with document-refresh-only planning specs", () => {
		...
		assert.ok(warning);
		assert.deepEqual(
			warning.rules.map((rule) => rule.ruleId),
	...
	it("commit path does not suppress on ignored local-only planning spec evidence", () => {
		const warning = warningFor(
			...
			{
				scope: "commit",
				localFreshTargets: [".omx/plans/prd-document-refresh-enforcer.md"],
			},
		);
	...
	it("parses git name-status rename records", () => {
		assert.deepEqual(
			parseGitNameStatus(
				"M\tsrc/scripts/codex-native-hook.ts\nR100\tsrc/cli/old.ts\tsrc/cli/new.ts\n",
			),
			[
				{ status: "M", path: "src/scripts/codex-native-hook.ts" },
				{
	...
	it("recognizes only terminal-looking final handoff text for Stop warnings", () => {
		assert.equal(
			isFinalHandoffDocumentRefreshCandidate("Launch-ready: yes"),
			true,
		);
		assert.equal(
			isFinalHandoffDocumentRefreshCandidate(
				"All verification passed; ready to merge.",
			),
			true,
		);
		assert.equal(
			isFinalHandoffDocumentRefreshCandidate(
				"Task completed with green verification.",
			),
			true,
		);
		assert.equal(
			isFinalHandoffDocumentRefreshCandidate(
				"I am done inspecting and will edit next.",
			),
			false,
		);
		assert.equal(
	...
	it("final handoff local .omx freshness suppresses when planning spec is newer than source", async () => {
		...
		try {
			...
			assert.deepEqual(freshTargets, [
				".omx/plans/prd-native-hook-behavior.md",
			]);
			assert.equal(
				warningFor(changes, {
					scope: "final-handoff",
					localFreshTargets: freshTargets,
				}),
				null,
			);
		} finally {
