...
export const NON_TERMINAL_RUN_OUTCOMES = ["progress", "continue"] as const;
...
export type TerminalLifecycleOutcome =
	(typeof TERMINAL_LIFECYCLE_OUTCOMES)[number];
...
const TERMINAL_LIFECYCLE_OUTCOME_SET = new Set<string>(
...
const TERMINAL_LIFECYCLE_OUTCOME_ALIASES: Readonly<
	Record<string, TerminalLifecycleOutcome>
> = {
	finished: "finished",
	finish: "finished",
	complete: "finished",
...
const TERMINAL_PHASE_TO_RUN_OUTCOME: Readonly<
	Record<string, TerminalRunOutcome>
> = {
	complete: "finish",
	completed: "finish",
	blocked: "blocked_on_user",
...
const RUN_OUTCOME_FROM_LIFECYCLE: Readonly<
	Record<TerminalLifecycleOutcome, TerminalRunOutcome>
> = {
	finished: "finish",
	blocked: "blocked_on_user",
	failed: "failed",
...
function hasPendingQuestionEnforcement(
...
export function isTerminalLifecycleOutcome(
	...
	return (
		normalized !== undefined && TERMINAL_LIFECYCLE_OUTCOME_SET.has(normalized)
	);
}

export function isNonTerminalRunOutcome(
	value: unknown,
): value is NonTerminalRunOutcome {
	const normalized = normalizeRunOutcome(value).outcome;
	return (
		normalized !== undefined && NON_TERMINAL_RUN_OUTCOME_SET.has(normalized)
	);
}
...
export function inferTerminalLifecycleOutcome(
	...
	const blockedOnUserStrategy =
		options.blockedOnUserStrategy ??
		(options.includeQuestionEnforcement &&
		hasPendingQuestionEnforcement(candidate)
			? "askuserQuestion"
			: "blocked");
	...
	if (runOutcome && isTerminalRunOutcome(runOutcome)) {
		return terminalLifecycleOutcomeFromRunOutcome(runOutcome, {
			blockedOnUserStrategy,
		});
	}
	...
	if (phase) {
		const normalizedPhase = normalizeTerminalLifecycleOutcome(phase, {
			blockedOnUserStrategy,
		});
...
export function inferRunOutcome(
	...
	if (lifecycleOutcome) {
		return compatibilityRunOutcomeFromTerminalLifecycleOutcome(
			lifecycleOutcome,
		);
	}
...
export function applyRunOutcomeContract(
	...
	const normalizedLifecycle = normalizeTerminalLifecycleOutcome(
		next.lifecycle_outcome ?? next.terminal_outcome,
	);
	...
	const lifecycleOutcome =
		normalizedLifecycle.outcome ??
		inferTerminalLifecycleOutcome(next, {
			includeQuestionEnforcement: false,
		});
	...
	const terminal =
		lifecycleOutcome !== undefined || isTerminalRunOutcome(outcome);
	if (terminal) {
		if (next.active === true) {
			return {
				ok: false,
		...
		if (next.active === false) {
			return {
				ok: false,
				error: `non-terminal run outcome "${outcome}" requires active=true`,
			};
		}
		next.active = true;
		if (safeString(next.completed_at)) {
