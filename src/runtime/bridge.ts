...
export function resolveRuntimeBinaryPath(
	options: RuntimeBinaryDiscoveryOptions = {},
): string {
	...
	const workspaceDebug =
		options.debugPath ??
		resolve(__bridge_dirname, "../../target/debug/omx-runtime");
	if (exists(workspaceDebug)) return workspaceDebug;

	const workspaceRelease =
		options.releasePath ??
		resolve(__bridge_dirname, "../../target/release/omx-runtime");
...
export class RuntimeBridge {
	...
	): Extract<RuntimeEvent, { event: "DispatchRecordsRemoved" }> {
		const event = this.execCommand({
			command: "RemoveDispatchRecords",
			request_ids: [...requestIds],
		});
		if (event.event !== "DispatchRecordsRemoved") {
			throw new RuntimeBridgeError(
				"omx-runtime returned an unexpected dispatch removal event",
				{
					command: "RemoveDispatchRecords",
				},
			);
	...
	readSnapshot(): RuntimeSnapshot {
		...
		try {
			...
			throw new RuntimeBridgeError(
				"omx-runtime snapshot returned non-JSON output",
				{
					command: "snapshot",
	...
	readDispatchRecords(): DispatchRecord[] {
		const raw = this.readCompatFile<{ records: DispatchRecord[] }>(
			"dispatch.json",
		);
	...
	readDispatchRecordsStrict(): DispatchRecord[] {
		if (!this.stateDir)
			throw new RuntimeBridgeError(
				"dispatch compatibility state directory is unavailable",
		...
		try {
			...
			throw new RuntimeBridgeError(
				"dispatch compatibility output is unavailable or malformed",
				{ command: "dispatch-read", cause },
			);
		}
		if (
			!parsed ||
			...
			throw new RuntimeBridgeError(
				"dispatch compatibility output has an invalid shape",
				{ command: "dispatch-read" },
			);
		}
		...
		if (records.some((record) => !isStrictDispatchRecord(record))) {
			throw new RuntimeBridgeError(
				"dispatch compatibility output contains an invalid record",
				{ command: "dispatch-read" },
			);
		}
	...
	readMailboxRecords(): MailboxRecord[] {
		const raw = this.readCompatFile<{ records: MailboxRecord[] }>(
			"mailbox.json",
		);
		if (!raw?.records) return [];
		return raw.records;
	...
	private run(args: string[]): string {
		try {
			...
			const stderr =
				execErr.stderr?.trim() ?? execErr.message ?? "unknown error";
