...
export interface RunUntilTerminalOptions<TState> {
	...
	onIteration?: (
		result: NormalizedRunLoopIteration<TState>,
	) => Promise<void> | void;
}
...
export async function runUntilTerminal<TState>(
	...
	throw new Error(
		`run loop exceeded maxIterations=${maxIterations} without reaching a terminal outcome`,
	);
}
...
export function getRunContinuationSnapshot(
	...
	const phase =
		safeString(candidate.current_phase) || options.phaseFallback || "active";
	return {
		outcome,
		lifecycleOutcome,
