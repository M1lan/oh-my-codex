...
export type TerminalLifecycleNormalizationResult =
...
function rewriteWarning(warning: string | undefined): string | undefined {
	return warning?.replace(
		"legacy terminal lifecycle outcome",
		"legacy lifecycle outcome",
	);
}

function isCanonicalTerminalLifecycleOutcome(
	value: unknown,
): value is TerminalLifecycleOutcome {
	return (
		typeof value === "string" &&
		(TERMINAL_LIFECYCLE_OUTCOMES as readonly string[]).includes(value.trim())
	);
}
