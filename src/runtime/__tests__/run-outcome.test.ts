...
describe("run outcome contract", () => {
	...
	it("infers continue for active non-terminal state", () => {
		assert.equal(
			inferRunOutcome({ active: true, current_phase: "executing" }),
			"continue",
		);
	});
	...
	it("infers terminal outcomes from terminal phases", () => {
		assert.equal(
			inferRunOutcome({ active: false, current_phase: "complete" }),
			"finish",
		);
		assert.equal(
			inferRunOutcome({ active: false, current_phase: "blocked_on_user" }),
			"blocked_on_user",
		);
		assert.equal(
			inferRunOutcome({ active: false, current_phase: "failed" }),
			"failed",
		);
		assert.equal(
			inferRunOutcome({ active: false, current_phase: "cancelled" }),
			"cancelled",
		);
	});
	...
	it("stamps completed_at for terminal outcomes and marks them inactive", () => {
		...
		assert.equal(
			isTerminalRunOutcome(result.state?.run_outcome as never),
			true,
		);
	});
	...
	it("normalizes canonical terminal lifecycle outcomes and legacy aliases", () => {
		...
		assert.deepEqual(normalizeTerminalLifecycleOutcome("cancelled"), {
			...
			warning:
				'normalized legacy lifecycle outcome "cancelled" -> "userinterlude"',
		});
	});
	...
	it("infers terminal lifecycle outcome from legacy run_outcome when no canonical field exists", () => {
		...
		assert.deepEqual(
			inferTerminalLifecycleOutcome({ run_outcome: "blocked_on_user" }),
			{
				outcome: "blocked",
			},
		);
		assert.deepEqual(
			inferTerminalLifecycleOutcome({ run_outcome: "cancelled" }),
			{
				outcome: "userinterlude",
	...
	it("maps canonical lifecycle outcomes to compatibility run_outcome values", () => {
		...
		assert.equal(
			preferredRunOutcomeForLifecycleOutcome("blocked"),
			"blocked_on_user",
		);
		assert.equal(preferredRunOutcomeForLifecycleOutcome("failed"), "failed");
		assert.equal(
			preferredRunOutcomeForLifecycleOutcome("userinterlude"),
			"cancelled",
		);
		assert.equal(
			preferredRunOutcomeForLifecycleOutcome("askuserQuestion"),
			"blocked_on_user",
		);
	});
