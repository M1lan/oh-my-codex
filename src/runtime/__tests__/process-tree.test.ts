...
describe("runProcessTreeWithTimeout", () => {
	...
	it("terminates commands that exceed the configured output limit", async () => {
		const result = await runProcessTreeWithTimeout(
			process.execPath,
			["-e", 'while (true) process.stdout.write("x".repeat(1024));'],
	...
	it("terminates suspicious process storms before the global timeout", {
	...
	it("returns parent output promptly when inherited-stdio grandchildren outlive the parent", {
		skip: process.platform === "win32",
	}, async () => {
		const root = await mkdtemp(
			join(tmpdir(), "omx-process-tree-inherited-stdio-"),
		);
		...
		assert.ok(
			...
			"should not wait for grandchild sleep or timeout",
		);
	});

	it("sweeps process-group grandchildren when the direct child exits", {
		skip: process.platform === "win32",
	}, async () => {
