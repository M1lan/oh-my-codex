...
function terminalOutcomeFromPhase(
...
export function deriveRunOutcomeFromModeState(state: RunStateLike): RunOutcome {
	...
	const lifecycleOutcome = inferTerminalLifecycleOutcome(
	...
	const explicitOutcome = normalizeRunOutcome(
		state.outcome ?? state.run_outcome,
	).outcome;
	if (explicitOutcome) return explicitOutcome;

	const phaseOutcome = terminalOutcomeFromPhase(
		optionalString(state.current_phase),
	);
...
export function buildRunState(
	...
	const lifecycleOutcome =
		...
		inferTerminalLifecycleOutcome(state as Record<string, unknown>, {
			includeQuestionEnforcement: true,
		}) ??
	...
	const next: RunState = {
		version: 1,
		mode:
			optionalString(state.mode) ?? optionalString(existing?.mode) ?? "unknown",
		active,
		outcome,
		updated_at: nowIso,
	};
	...
	const startedAt =
		optionalString(state.started_at) ?? optionalString(existing?.started_at);
	if (startedAt) next.started_at = startedAt;

	const completedAt = active
		? undefined
		: (optionalString(state.completed_at) ??
			(isTerminalRunOutcome(outcome)
				? (optionalString(existing?.completed_at) ?? nowIso)
				: undefined));
...
export async function syncRunStateFromModeState(
	...
	const scope = options.targetPath
		? undefined
		: await resolveStateScope(workingDirectory, explicitSessionId);
	const path =
		options.targetPath ?? getRunStatePath(workingDirectory, scope!.sessionId);
	await mkdir(options.targetPath ? dirname(path) : scope!.stateDir, {
		recursive: true,
	});
