...
export type RalplanExecutionLane =
	| "ultragoal"
	| "team"
	| "ralph"
	| "conductor"
...
export interface RalplanConsensusGate {
	...
	ralplan_architect_review:
		| (RalplanReviewResult & { agent_role: "architect"; iteration: number })
		| null;
	ralplan_critic_review:
		| (RalplanReviewResult & { agent_role: "critic"; iteration: number })
		| null;
	architect_review:
		| (RalplanReviewResult & { agent_role: "architect"; iteration: number })
		| null;
...
async function recordRalplanSubagentTurn(
	...
	await recordSubagentTurnForSession(cwd, {
		...
		...(input.summary?.trim()
			? { lastHandoffSummary: input.summary.trim() }
			: {}),
		...(input.completed
			? { completed: true, completionSource: input.completionSource }
			: {}),
		...(input.preserveCompletionEvidence
...
function reviewBlocker(
	...
	if (architectReview?.verdict !== "approve")
		return "architect_review_missing_or_not_approved";
	if (criticReview?.verdict !== "approve")
		return "critic_review_missing_or_not_approved";
	if (
		!isApprovingReviewPair(
			architectReview,
			criticReview,
...
async function hasCompletedNativeReviewEvidence(
	...
	const threads = (await readSubagentTrackingState(cwd)).sessions[sessionId]
		?.threads;
	return Boolean(
		threads?.[architectReview.thread_id ?? ""]?.completed_at &&
			threads?.[criticReview.thread_id ?? ""]?.completed_at,
	);
}
...
function buildRalplanConsensusGate(
	...
	options: {
		...
		sessionId?: string;
		requireNativeSubagents?: boolean;
		nativeEvidenceComplete?: boolean;
	} = {},
	...
	return {
		...
		blocked_reason: reviewBlocker(
			latestArchitect,
			latestCritic,
			options.requireNativeSubagents === true,
			options.nativeEvidenceComplete,
		),
	};
}
...
function normalizeReviewForLane(
	...
	if (requireNativeSubagents) {
		if (!review.agent_role) {
			throw new Error(
				`ralplan_${laneRole}_review_role_missing: expected agent_role=${laneRole}`,
			);
		}
		if (review.agent_role !== laneRole) {
			throw new Error(
				`ralplan_${laneRole}_review_role_mismatch: expected agent_role=${laneRole}, received ${review.agent_role}`,
			);
		}
		if (!hasNativeSubagentEvidence(review)) {
			throw new Error(
				`ralplan_${laneRole}_review_provenance_invalid: expected provenance_kind=native_subagent`,
			);
		}
		if (!review.thread_id?.trim()) {
			throw new Error(
				`ralplan_${laneRole}_review_thread_missing: native_subagent review must declare thread_id`,
			);
		}
	} else if (
		review.provenance_kind !== undefined &&
		!hasNativeSubagentEvidence(review)
	) {
		throw new Error(
			`ralplan_${laneRole}_review_provenance_invalid: adapted provenance cannot authorize a review lane`,
		);
	}
...
function latestCompatibleRoleLane(
	...
	for (let index = reviews.length - 1; index >= 0; index -= 1) {
		...
		if (!nonEmptyString(review.thread_id) && !nonEmptyString(review.lane_id))
			continue;
		const reviewSessionId = nonEmptyString(review.session_id);
		if (sessionId && reviewSessionId && reviewSessionId !== sessionId) continue;
		return {
			agent_role: role,
			...(nonEmptyString(review.thread_id)
				? { thread_id: nonEmptyString(review.thread_id) }
				: {}),
			...(nonEmptyString(review.lane_id)
				? { lane_id: nonEmptyString(review.lane_id) }
				: {}),
			...(reviewSessionId ? { session_id: reviewSessionId } : {}),
			...(nonEmptyString(review.native_session_id)
				? { native_session_id: nonEmptyString(review.native_session_id) }
				: {}),
			...(nonEmptyString(review.tracker_path)
				? { tracker_path: nonEmptyString(review.tracker_path) }
				: {}),
		};
	}
	return undefined;
...
function assertRoleLaneReuse(
	...
	const reusedThread =
		priorThreadId && nextThreadId && priorThreadId === nextThreadId;
...
export async function runRalplanConsensus(
	...
	try {
		while (iteration <= maxIterations) {
			const reusableRoleLanes = {
				architect: latestCompatibleRoleLane(
					architectReviews,
					"architect",
					options.sessionId,
				),
				critic: latestCompatibleRoleLane(
					criticReviews,
					"critic",
					options.sessionId,
				),
			...
			await updateRalplanState(cwd, {
				...
				ralplan_consensus_gate: buildRalplanConsensusGate(
					architectReviews,
					criticReviews,
					gateOptions,
				),
				review_history: buildReviewHistory(
					drafts,
					architectReviews,
					criticReviews,
				),
			...
			const architectReview = normalizeReviewForLane(
				await executor.architectReview({
					...iterationContext,
					draft,
				}),
				"architect",
			...
			const criticReview = normalizeReviewForLane(
				await executor.criticReview({
					...iterationContext,
					draft,
					architectReview,
				}),
			...
			const consensusGate = buildRalplanConsensusGate(
				...
				criticReviews,
				{
					...
					nativeEvidenceComplete:
						options.requireNativeSubagents !== true ||
						(await hasCompletedNativeReviewEvidence(
							cwd,
							options.sessionId,
							architectReview,
							criticReview,
			...
			if (
				consensusGate.blocked_reason ===
					"documented_host_consensus_receipt_unavailable" ||
				iteration >= maxIterations
			) {
				const hostReceiptUnavailable =
					consensusGate.blocked_reason ===
					"documented_host_consensus_receipt_unavailable";
		...
		return {
			...
			architectReviews,
			criticReviews,
			ralplanConsensusGate: buildRalplanConsensusGate(
				architectReviews,
				criticReviews,
				gateOptions,
			),
	...
	await updateRalplanState(cwd, {
		...
		status_message:
			"Status: failed — ralplan reached an unexpected runtime state.",
