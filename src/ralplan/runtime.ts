...
function normalizeReviewForLane(
	...
	if (
		review.agent_role === undefined &&
		(options.requireNativeSubagents || hasNativeOrThreadEvidence(review))
	) {
		throw new Error(
...
function normalizeExecutionLane(
	lane: RalplanExecutionLane | undefined,
): "ultragoal" | "team" | "ralph" | "none" {
	if (lane === "team" || lane === "ralph" || lane === "none") return lane;
	if (lane === "ultragoal" || lane === "conductor" || lane === "execution")
		return "ultragoal";
	return "none";
}
...
function buildRalplanHandoffArtifact(
	...
	const selectedExecutionLane = normalizeExecutionLane(
		options.selectedExecutionLane,
	);
	return {
		selected_execution_lane: selectedExecutionLane,
		execution_handoff_status:
			selectedExecutionLane === "none"
			...
			if (consensusGate.complete) {
				...
				if (!planningComplete) {
					...
					await updateRalplanState(cwd, {
						...
						ralplan_consensus_gate: consensusGate,
						status_message:
							"Status: failed — ralplan consensus approved, but required PRD and test-spec planning artifacts are missing; do not hand off to execution.",
						review_history: reviewHistory,
						error,
					});
					return {
				...
				await updateRalplanState(cwd, {
					...
					latest_plan_path: latestPlanPath,
					ralplan_consensus_gate: consensusGate,
					selected_execution_lane: normalizeExecutionLane(
						options.selectedExecutionLane,
					),
					handoff_artifacts: {
					...
					status_message:
						normalizeExecutionLane(options.selectedExecutionLane) === "none"
				...
				const executionHandoffStarted = await startSelectedExecutionLane(
					cwd,
					options.task,
					options.selectedExecutionLane,
				);
