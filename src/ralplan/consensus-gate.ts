...
export function buildRalplanConsensusGateFromSources(
	...
	let firstCompleteEvidence: (ConsensusResolution & { source: string }) | null =
		null;
	...
	for (const candidate of sources) {
		...
		if (evidence?.kind === "invalid") {
			if (
				isConsensusEvidenceNewerThanSelected(evidence, firstCompleteEvidence)
			) {
		...
		if (evidence?.kind === "valid") {
			const requiresTrackerBackedLanes =
				options.requireNativeSubagents ||
				evidence.ralplan_architect_review.provenance_kind ===
					"native_subagent" ||
			...
			if (requiresTrackerBackedLanes) {
				const trackerEvaluation = evaluateTrackerBackedNativeRalplanLanes(
					evidence,
					candidateOptions,
					trackerSnapshots,
				);
	...
	if (nativeBlockedEvidence) {
		return {
			...
			blockedReason:
				RALPLAN_CONSENSUS_BLOCKED_REASONS.nativeSubagentEvidenceMissing,
			blockedDetails: [
...
export function buildRalplanConsensusGateForCwd(
	...
	const localStateCandidates = readLocalRalplanConsensusStateCandidates(
		cwd,
		options.sessionId,
	).map((candidate) => ({
		...candidate,
			...
			...(options.artifacts
				? [
						{ source: "stage-context-artifacts", value: options.artifacts },
						{
							...
							value: withParentReturnToRalplanContext(
								options.artifacts.ralplan,
								options.artifacts,
							),
						},
					]
...
export function readLocalRalplanConsensusStateCandidates(
	...
	const sessionIdList = explicitSession
		? validateLocalSessionId(sessionId)
		: readLocalCurrentSessionIds(cwd);
	...
	const stateRoots: Array<{ dir: string; sessionId?: string }> =
...
function resolveConsensusEvidence(value: unknown): ConsensusResolution | null {
	...
	const staleReturnToRalplanCycle =
		returnToRalplanCycle && advancedReviewCycle === null;
	...
	if (directGate) {
		...
		if (advancedReviewCycle !== null) {
			...
			!hasExplicitReturnToRalplanReviewCycle(record) &&
			consensusEvidenceOrder(directGate) !== null
		) {
	...
	const topLevelHandoffArtifacts = handoffArtifactsAreStale
		? null
		: asRecord(record.handoff_artifacts);
	if (topLevelHandoffArtifacts) {
		const evidence = resolveConsensusEvidence(
			withParentReturnToRalplanContext(topLevelHandoffArtifacts, record),
		);
		if (evidence) return evidence;
	}

	const stateRecord = asRecord(record.state);
	const stateHasOwnReturnLoopContext =
		stateRecord !== null && isReturnToRalplanCycle(stateRecord);
	const stateHandoffArtifacts =
		handoffArtifactsAreStale && !stateHasOwnReturnLoopContext
			? null
	...
	if (stateHandoffArtifacts) {
		...
		const evidence = resolveConsensusEvidence(
			withParentReturnToRalplanContext(stateHandoffArtifacts, stateContext),
		);
		if (evidence) return evidence;
	}
	...
	if (latestReviewEntry) {
		const architectReview = asRecord(
			latestReviewEntry.ralplan_architect_review ??
				latestReviewEntry.architect_review ??
				latestReviewEntry.architectReview,
		);
		const criticReview = asRecord(
			latestReviewEntry.ralplan_critic_review ??
				latestReviewEntry.critic_review ??
				latestReviewEntry.criticReview,
		);
		if (
	...
	const architectReviews = Array.isArray(record.architectReviews)
		? record.architectReviews
		: [];
	const criticReviews = Array.isArray(record.criticReviews)
		? record.criticReviews
		: [];
	if (
...
function resolveDirectGate(
	...
	if (gate && typeof gate === "object") {
		...
		const architectReview = asRecord(
			gateRecord.ralplan_architect_review ??
				gateRecord.architectReview ??
				gateRecord.architect_review,
		);
		const criticReview = asRecord(
			gateRecord.ralplan_critic_review ??
				gateRecord.criticReview ??
				gateRecord.critic_review,
		);
		if (
		...
		if (gateRecord.complete === true) {
			...
			if (!hasArchitectThenCriticSequence(gateRecord)) {
				blockedDetails.push(
					"consensus review sequence is not architect-review then critic-review",
				);
			}
			if (!isCriticNotBeforeArchitect(architectReview, criticReview)) {
				blockedDetails.push(
					"direct review order is not proven strictly architect-before-critic",
				);
			}
...
export function withParentReturnToRalplanContext(
	value: unknown,
	parent: Record<string, unknown>,
): unknown {
	const reason =
		parent.return_to_ralplan_reason ?? parent.returnToRalplanReason;
	if (
...
function explicitFreshnessReviewCycle(
	record: Record<string, unknown>,
): number | null {
	const parentReviewCycle = numericValue(
		record.return_to_ralplan_parent_review_cycle ??
			record.returnToRalplanParentReviewCycle,
	);
	const candidateReviewCycle = numericValue(
		record.review_cycle ?? record.reviewCycle,
	);
...
function reviewsCarryFreshnessCycle(
...
function isConsensusEvidenceNewerThanSelected(
	...
	if (evidenceOrder !== null || selectedOrder !== null) {
		...
		if (evidenceOrder.value !== selectedOrder.value)
			return evidenceOrder.value > selectedOrder.value;
	}

	return false;
}

function consensusEvidenceReviewCycle(
	evidence: ConsensusResolution,
): number | null {
	return maxKnownNumber(
		numericValue(
			evidence.ralplan_architect_review?.review_cycle ??
				evidence.ralplan_architect_review?.reviewCycle,
		),
		numericValue(
			evidence.ralplan_critic_review?.review_cycle ??
				evidence.ralplan_critic_review?.reviewCycle,
		),
	);
}

function consensusEvidenceOrder(
	evidence: ConsensusResolution,
): ReviewOrder | null {
	const architectOrder = reviewOrderValue(
		evidence.ralplan_architect_review ?? {},
	);
	...
	return architectOrder.value >= criticOrder.value
		? architectOrder
		: criticOrder;
}
...
function hasExplicitReturnToRalplanReviewCycle(
	record: Record<string, unknown>,
): boolean {
	return (
		numericValue(record.review_cycle ?? record.reviewCycle) !== null ||
		numericValue(
			record.return_to_ralplan_parent_review_cycle ??
				record.returnToRalplanParentReviewCycle,
		) !== null
...
function reviewPairCarriesFreshnessCycle(
	...
	return (
		reviewCarriesFreshnessCycle(architectReview, reviewCycle) &&
		reviewCarriesFreshnessCycle(criticReview, reviewCycle)
	);
}

function reviewCarriesFreshnessCycle(
...
function isApproveReview(
	...
	if (
		...
		!isApproveRecommendation(value.recommendation)
	) {
		return false;
	}
...
function reviewApprovalProblems(
	value: Record<string, unknown> | null,
	agentRole: "architect" | "critic",
): string[] {
	const issues: string[] = [];
	if (!value) return [`${agentRole} review is missing`];
	if (value.agent_role !== agentRole)
		issues.push(
			`${agentRole} review has agent_role=${String(value.agent_role || "missing")}`,
		);
	if (value.verdict !== undefined && value.verdict !== "approve") {
		issues.push(
			`${agentRole} review verdict=${String(value.verdict)} is not approve`,
		);
	}
	if (value.status !== undefined && !isApprovedStatus(value.status)) {
		issues.push(
			`${agentRole} review status=${String(value.status)} is not approve`,
		);
	}
	if (
		value.recommendation !== undefined &&
		...
		issues.push(
			`${agentRole} review recommendation=${String(value.recommendation)} is not approve`,
		);
	}
...
function hasPositiveReviewApprovalSignal(
...
function isApprovedStatus(value: unknown): boolean {
	return ["approve", "approved", "clear", "pass", "passed"].includes(
...
function hasArchitectThenCriticSequence(
	value: Record<string, unknown>,
): boolean {
	if (!Array.isArray(value.sequence)) return true;
	return (
		value.sequence[0] === "architect-review" &&
		value.sequence[1] === "critic-review"
	);
}
...
function isCriticNotBeforeArchitect(
	...
	if (
		isTrackerBackedReview(architectReview) ||
		isTrackerBackedReview(criticReview)
	)
		return true;
	...
	if (architectSequence !== null || criticSequence !== null) {
		if (
			architectSequence === null ||
			criticSequence === null ||
			criticSequence <= architectSequence
		)
			return false;
		const architectTimestamp = reviewTimestampValue(architectReview);
		const criticTimestamp = reviewTimestampValue(criticReview);
		return (
			architectTimestamp === null ||
			criticTimestamp === null ||
			criticTimestamp > architectTimestamp
		);
	}
...
function isTrackerBackedReview(review: Record<string, unknown>): boolean {
	return (
		review.provenance_kind === "native_subagent" ||
...
function evaluateTrackerBackedNativeRalplanLanes(
	...
	for (const trackerPath of trackerPaths) {
		...
		const evaluation = evaluateTrackerBackedNativeRalplanSnapshot(
			evidence,
			options,
			snapshot,
		);
	...
	return (
		firstEvaluation ??
		evaluateTrackerBackedNativeRalplanSnapshot(evidence, options, null)
	);
}
...
function evaluateTrackerBackedNativeRalplanSnapshot(
	...
	const pairProblem = trackerBackedNativeReviewPairProblem(
		evidence,
		options,
		snapshot,
	);
...
function buildTrackerBackedNativeConsensusDiagnostic(
	...
	const cwd = typeof options.cwd === "string" ? options.cwd.trim() : "";
	const trackerPath =
		evaluation.snapshot?.trackerPath ??
		(cwd ? subagentTrackingPath(cwd) : ".omx/state/subagent-tracking.json");
	const currentSessionId = currentTransitionSessionId(evidence, options);
	const architectThreadId = nativeReviewThreadId(
		evidence.ralplan_architect_review,
	);
...
function trackerBackedNativeReviewPairProblem(
	...
	const transitionSessionId =
		typeof options.sessionId === "string" ? options.sessionId.trim() : "";
	const architectSessionId =
		transitionSessionId ||
		nativeReviewSessionId(evidence.ralplan_architect_review);
	const criticSessionId =
		transitionSessionId ||
		nativeReviewSessionId(evidence.ralplan_critic_review);
...
function trackerBackedReviewOrderProblem(
	...
	const session = asRecord(asRecord(snapshot.tracking?.sessions)?.[sessionId]);
	const architectThread = asRecord(
		asRecord(session?.threads)?.[architectThreadId],
	);
	const criticThread = asRecord(asRecord(session?.threads)?.[criticThreadId]);
	const architectCompletedAt = trackerTimestamp(architectThread, [
		"completed_at",
	]);
	const criticStartedAt = trackerTimestamp(criticThread, [
		"first_seen_at",
		"started_at",
	]);
...
function trackerBackedReviewProblem(
	...
	if (review.agent_role !== agentRole)
		issues.push(
			`${agentRole} review has agent_role=${String(review.agent_role || "missing")}`,
		);
	if (review.provenance_kind !== provenanceKind)
		issues.push(
			`${agentRole} review has provenance_kind=${String(review.provenance_kind || "missing")}`,
		);
	const sessionId =
		typeof options.sessionId === "string" && options.sessionId.trim()
			? options.sessionId.trim()
	...
	const reviewSessionId =
		typeof review.session_id === "string" ? review.session_id.trim() : "";
	const threadId =
		typeof review.thread_id === "string" ? review.thread_id.trim() : "";
	const trackerPath =
		typeof review.tracker_path === "string" ? review.tracker_path.trim() : "";
	if (!sessionId) issues.push(`${agentRole} review cannot resolve session_id`);
	if (reviewSessionId && reviewSessionId !== sessionId)
		issues.push(
			`${agentRole} review session_id=${reviewSessionId} does not match ${sessionId || "missing"}`,
		);
	if (!threadId) issues.push(`${agentRole} review missing thread_id`);
	if (trackerPath && !trackerPath.endsWith("subagent-tracking.json"))
		issues.push(
			`${agentRole} review tracker_path=${trackerPath} is not subagent-tracking.json`,
		);
	const cwd = typeof options.cwd === "string" ? options.cwd.trim() : "";
	if (!cwd)
...
function trackerThreadProblem(
	...
	const laneLabel =
		provenanceKind === "native_subagent" ? "native" : provenanceKind;

	const session = asRecord(asRecord(tracking?.sessions)?.[sessionId]);
	const thread = asRecord(asRecord(session?.threads)?.[threadId]);
	if (!session)
		return `${agentRole} tracker session ${sessionId} is missing in ${trackerPath}; only reviews recorded in OMX subagent-tracking.json count as ${laneLabel} lanes`;
	if (!thread)
		return `${agentRole} tracker thread ${threadId} is missing in ${trackerPath}; external/collab subagent reviews are not tracker-backed ${laneLabel} lanes`;
	...
	const ledgerProvenance =
		typeof thread.provenance_kind === "string"
			? thread.provenance_kind.trim()
			: "";
	...
	if (
		...
		ledgerProvenance &&
		ledgerProvenance !== "native_subagent"
	) {
	...
	const trackerRoleIdentity =
		trackerRole || (provenanceKind === "native_subagent" ? trackerMode : "");
...
function currentSessionNativeLeaderThreadId(cwd: string | undefined): string {
	if (!cwd) return "";
	const sessionState = readJsonState(
		join(getBaseStateDir(cwd), "session.json"),
	);
	return typeof sessionState?.native_session_id === "string"
		? sessionState.native_session_id.trim()
		: "";
}
...
function hasBlockingReviewSignal(value: Record<string, unknown>): boolean {
	if (
		...
		return true;
	if (
		value.request_changes === true ||
		value.requestChanges === true ||
		value.requires_changes === true ||
		value.requiresChanges === true
	)
		return true;
...
function readLocalCurrentSessionIds(cwd: string): string[] {
	...
	const sessionId =
		typeof state?.session_id === "string" ? state.session_id : undefined;
...
function sessionIdFromStateRoot(path: string): string | undefined {
	...
	return sessionId && validateLocalSessionId(sessionId).length > 0
		? sessionId
		: undefined;
}
...
function isReturnToRalplanCycle(record: Record<string, unknown>): boolean {
	...
	const reason =
		record.return_to_ralplan_reason ?? record.returnToRalplanReason;
	return (
