...
export type RalplanHostConsensusReceiptVerifierCapability =
	| "available"
	| "unavailable";
...
export function buildRalplanConsensusGateFromSources(
	...
	let lifecycleEvidence: (ConsensusResolution & { source: string }) | null =
		null;
	for (const candidate of sources) {
		const evidence = resolveConsensusEvidence(candidate.value);
		if (
			evidence &&
			isConsensusEvidenceNewerThanSelected(evidence, lifecycleEvidence)
		) {
	...
	return {
		...
		ralplan_architect_review:
			lifecycleEvidence?.ralplan_architect_review ?? null,
		ralplan_critic_review: lifecycleEvidence?.ralplan_critic_review ?? null,
		source: lifecycleEvidence?.source ?? null,
		blockedReason:
			RALPLAN_CONSENSUS_BLOCKED_REASONS.documentedHostConsensusReceiptUnavailable,
	...
	if (gate && typeof gate === "object") {
		...
		if (gateRecord.complete === true) {
			...
			if (!hasDistinctNativeReviewThreads(architectReview, criticReview)) {
				blockedDetails.push(
					"consensus reviews must use distinct native_subagent thread_id values",
				);
			}
...
function isApproveReview(
	...
	if (
		!value ||
		value.agent_role !== agentRole ||
		value.provenance_kind !== "native_subagent"
	)
		return false;
...
function hasDistinctNativeReviewThreads(
	...
	const architectThreadId =
		typeof architectReview?.thread_id === "string"
			? architectReview.thread_id.trim()
			: "";
	const criticThreadId =
		typeof criticReview?.thread_id === "string"
			? criticReview.thread_id.trim()
			: "";
	return (
...
function reviewApprovalProblems(
	...
	if (value.provenance_kind !== "native_subagent") {
		issues.push(
			`${agentRole} review provenance_kind=${String(value.provenance_kind || "missing")} is not native_subagent`,
		);
	}
