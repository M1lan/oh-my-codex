...
export const UNSUPPORTED_DOCUMENTED_LEADER_PROOF =
	"unsupported_documented_leader_proof" as const;
...
type PreToolUseDenial =
	| typeof UNSUPPORTED_DOCUMENTED_LEADER_PRE_TOOL_USE
...
export function parseCodex01445AdaptedRoleIntentCommand(
	...
	const placeholder =
		platform === "win32" ? '"%CODEX_THREAD_ID%"' : '"$CODEX_THREAD_ID"';
	const escaped = placeholder.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
	const match = command.match(
		new RegExp(
			`^omx ralplan role-intent write --role ([A-Za-z0-9_-]{1,64}) --parent-thread ${escaped} --json$`,
		),
	);
...
export function evaluateCodex01445PreToolUse(
	...
	const parsed = parseCodex01445AdaptedRoleIntentCommand(
		command,
...
export const RALPLAN_NEUTRALIZE_TEST_SEAM: {
	fail?: (point: FaultPoint) => void | Promise<void>;
	random?: () => Buffer;
	directorySync?: () => void | Promise<void>;
	afterPin?: (directory: string) => void | Promise<void>;
	onError?: (error: unknown) => void;
	platform?: NodeJS.Platform;
	darwinHelperMode?: "failure" | "malformed";
} = {};

function regularSingleLink(
...
function validName(name: string): boolean {
	return (
		/^[A-Za-z0-9.][A-Za-z0-9._-]{0,255}$/.test(name) &&
		name !== "." &&
		name !== ".."
	);
}
interface PinnedDirectory {
	read(name: string, max?: number): Promise<Buffer | null>;
	list(): Promise<string[]>;
...
async function readPinnedPath(
	...
	try {
		...
		if (!regularSingleLink(before, max)) return null;
		...
		try {
			...
			return regularSingleLink(after, max) &&
...
function linuxPinned(
	path: string,
	handle: Awaited<ReturnType<typeof open>>,
): PinnedDirectory {
	return {
		read: (name, max = MAX_BYTES) =>
			validName(name)
				? readPinnedPath(join(path, name), max)
				: Promise.resolve(null),
		list: () => readdir(path).catch(() => []),
		async writeExclusive(name, bytes) {
			if (!validName(name) || bytes.length > MAX_GENERATION_BYTES)
				throw new Error("invalid pinned write");
...
function darwinPinned(
	...
	return new Promise((resolve) => {
		...
		const child = spawn(
			process.execPath,
			["-e", DARWIN_HELPER, String(expected.dev), String(expected.ino), mode],
		...
		const finish = (value: PinnedDirectory | null) => {
			if (!resolved) {
				resolved = true;
		...
		stdout.on("data", (chunk: Buffer) => {
			...
			if (Buffer.byteLength(buffer) > MAX_BYTES * 2) {
				child.kill();
				return;
			}
			let newline: number;
		...
		child.once("exit", () => {
			closed = true;
			while (responses.length) responses.shift()!(null);
			finish(null);
		});
		setTimeout(async () => {
			...
			if (
				!probe?.ok ||
				!Array.isArray(probe.names) ||
				!probe.names.every((name) => typeof name === "string")
			)
				return finish(null);
			finish({
				async read(name, max = MAX_BYTES) {
					if (!validName(name)) return null;
					const response = await request({ op: "read", name, max });
					if (
						!response?.ok ||
					...
					return bytes && bytes.length <= max ? bytes : null;
				},
				async list() {
					const response = await request({ op: "list" });
					return response?.ok &&
				...
				async sync() {
					const response = await request({ op: "sync" });
					if (!response?.ok) throw new Error("pinned directory sync failed");
				},
				async close() {
					if (!closed) {
						closed = true;
...
async function pinDirectory(
	...
	try {
		...
		if (platform === "darwin") return await darwinPinned(directory, before);
		const handle = await open(
			directory,
		...
		return linuxPinned(`/proc/self/fd/${handle.fd}`, handle);
	} catch {
		return null;
	}
}
...
function isRalplanProducerKeyword(value: unknown): boolean {
	...
	return (
		match?.[1] !== undefined &&
		getExplicitSkillDefinition(match[1])?.skill === "ralplan"
	);
}
const RALPLAN_KEYS = new Set([
	"active",
	"mode",
	"current_phase",
	"started_at",
...
function routingOnlyRalplan(
	...
	return (
		...
		isProducerTimestamp(state.started_at) &&
		isProducerTimestamp(state.updated_at) &&
		exactSession(state.session_id, sessionId) &&
		(!("owner_omx_session_id" in state) ||
			exactSession(state.owner_omx_session_id, sessionId)) &&
		[
			"owner_codex_session_id",
			"thread_id",
			"turn_id",
			...
			"tmux_pane_set_at",
			"tmux_window_id",
		].every((key) => !(key in state) || isNonEmptyString(state[key])) &&
		hasOnlyKeys(state, RALPLAN_KEYS)
	);
}
function routingOnlySkill(
	state: Record<string, unknown>,
	sessionId: string,
): Record<string, unknown> | null {
	const expectedPath = `.omx/state/sessions/${sessionId}/ralplan-state.json`;
	const entries = Array.isArray(state.active_skills)
		? state.active_skills
		: null;
	const topLevelValid =
		state.version === 1 &&
		...
		(!("owner_omx_session_id" in state) ||
			exactSession(state.owner_omx_session_id, sessionId)) &&
		["owner_codex_session_id", "thread_id", "turn_id"].every(
			(key) => !(key in state) || isNonEmptyString(state[key]),
		) &&
		entries !== null &&
		entries.length === 1 &&
	...
	const entryValid =
		...
		(!("owner_omx_session_id" in item) ||
			exactSession(item.owner_omx_session_id, sessionId)) &&
		["owner_codex_session_id", "thread_id", "turn_id"].every(
			(key) => !(key in item) || isNonEmptyString(item[key]),
		) &&
		hasOnlyKeys(item, ENTRY_KEYS);
	return entryValid ? item : null;
}
function routingOnlyPair(
	...
	const correlationValid =
		entry !== null &&
		["owner_codex_session_id", "thread_id", "turn_id"].every((key) =>
			correlatedOptional([ralplan, skill, entry], key),
		) &&
	...
	if (!modeValid || !entry || !correlationValid) {
		RALPLAN_NEUTRALIZE_TEST_SEAM.onError?.(
			new Error(
				`pair validation failed: mode=${modeValid} skill=${entry !== null} correlation=${correlationValid}`,
			),
		);
	}
...
function collectOwners(baseStateDir: string, sessionId: string): Set<string> {
	...
	try {
		...
		if (
			realpathSync(pointerPath) !== pointerPath ||
			!lstatSync(pointerPath).isFile()
		)
			return owners;
		...
		for (const key of [
			...
			const value = normalizeSessionId(pointer[key]);
...
interface Generation {
	...
	digest: string;
	canonical: {
		ralplan: { sha256: string; size: number };
		skill: { sha256: string; size: number };
	};
}
function validGeneration(
	...
	return (
		item.version === GENERATION_VERSION &&
		item.digest === expectedDigest &&
		item.canonical?.ralplan?.sha256 === hash(ralplan) &&
		item.canonical.ralplan.size === ralplan.length &&
		item.canonical?.skill?.sha256 === hash(skill) &&
		item.canonical.skill.size === skill.length
	);
}
interface Commit {
	version: number;
...
async function readGeneration(
	...
	for (const commitName of names
		.filter((name) => name.startsWith(prefix) && name.endsWith(COMMIT_SUFFIX))
		.sort()
		...
		const commitBytes = await directory.read(commitName, MAX_GENERATION_BYTES);
		if (
			!commitBytes ||
			!validCommit(object(commitBytes), expectedDigest, dataFile)
		)
			continue;
		const dataBytes = await directory.read(dataFile, MAX_GENERATION_BYTES);
		if (!dataBytes) continue;
		const parsed = object(dataBytes);
		if (validGeneration(parsed, expectedDigest, ralplan, skill)) return parsed;
	}
	return null;
}
export async function readNeutralizedRoutingOverlay(
	...
	const rawSessionId = basename(directory);
	const sessionId = normalizeSessionId(rawSessionId);
	if (
		!sessionId ||
		sessionId !== rawSessionId ||
		basename(join(directory, "..")) !== "sessions"
	)
		return null;
	...
	try {
		const [ralplan, skill] = await Promise.all([
			pinned.read("ralplan-state.json"),
			pinned.read("skill-active-state.json"),
		]);
		if (!ralplan || !skill) return null;
		const canonicalRalplan = object(ralplan);
		const canonicalSkill = object(skill);
		if (
			!canonicalRalplan ||
			!canonicalSkill ||
			!routingOnlyPair(canonicalRalplan, canonicalSkill, sessionId)
		)
			return null;
		const generation = await readGeneration(
			pinned,
			digest(ralplan, skill),
			ralplan,
			skill,
		);
		return generation
			? neutralized(
					kind === "ralplan" ? canonicalRalplan : canonicalSkill,
					kind,
				)
			: null;
	} finally {
...
async function syncDirectory(directory: PinnedDirectory): Promise<void> {
	await RALPLAN_NEUTRALIZE_TEST_SEAM.directorySync?.();
	await RALPLAN_NEUTRALIZE_TEST_SEAM.fail?.("directory-sync");
	await directory.sync();
}
export async function neutralizeOwnedRoutingRalplan(
	cwd: string,
): Promise<boolean> {
	try {
		...
		const pinned = await pinDirectory(scope.stateDir);
		if (!pinned) return false;
		try {
			await RALPLAN_NEUTRALIZE_TEST_SEAM.afterPin?.(scope.stateDir);
			const owners = collectOwners(base, sessionId);
			if (!owners.has(ownerSessionId)) return false;
			const [ralplanBytes, skillBytes] = await Promise.all([
				pinned.read("ralplan-state.json"),
				pinned.read("skill-active-state.json"),
			]);
			if (!ralplanBytes || !skillBytes) return false;
			const ralplan = object(ralplanBytes);
			const skill = object(skillBytes);
			if (!ralplan || !skill || !routingOnlyPair(ralplan, skill, sessionId))
				return false;
			...
			const record: Generation = {
				...
				canonical: {
					ralplan: {
						sha256: createHash("sha256").update(ralplanBytes).digest("hex"),
						size: ralplanBytes.length,
					},
					skill: {
			...
			const random =
				RALPLAN_NEUTRALIZE_TEST_SEAM.random ?? (() => randomBytes(24));
			for (let attempt = 0; attempt < 4; attempt += 1) {
				...
				try {
					...
					await RALPLAN_NEUTRALIZE_TEST_SEAM.fail?.("data-write");
					await RALPLAN_NEUTRALIZE_TEST_SEAM.fail?.("data-sync");
					await pinned.writeExclusive(dataFile, data);
					await syncDirectory(pinned);
					const committed = Buffer.from(
						`${JSON.stringify({ version: GENERATION_VERSION, digest: pairDigest, dataFile, committed: true } satisfies Commit)}\n`,
					);
					await RALPLAN_NEUTRALIZE_TEST_SEAM.fail?.("commit-create");
					await RALPLAN_NEUTRALIZE_TEST_SEAM.fail?.("commit-write");
					await RALPLAN_NEUTRALIZE_TEST_SEAM.fail?.("commit-sync");
					await RALPLAN_NEUTRALIZE_TEST_SEAM.fail?.("commit-final-write");
					await RALPLAN_NEUTRALIZE_TEST_SEAM.fail?.("commit-final-sync");
					await pinned.writeExclusive(commitFile, committed);
					await pinned.sync();
					return true;
				} catch (error) {
					RALPLAN_NEUTRALIZE_TEST_SEAM.onError?.(error);
