...
describe("ralplan runtime", () => {
			...
			const result = await runRalplanConsensus(
				{
					async draft(ctx) {
						...
						await writeFile(
							join(plansDir, "test-spec-success.md"),
							"# tests\n",
						);
						return {
							summary: `draft-${ctx.iteration}`,
							planPath: prdPath,
							artifacts: { drafted: true },
						};
					},
	...
	it("records planning-only terminal state when consensus approves without a selected execution lane", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-planning-only-"),
		);
		try {
			const result = await runRalplanConsensus(
				{
			...
			const result = await runRalplanConsensus(
				{
					async draft() {
						...
						await writeFile(prdPath, "# plan\n");
						await writeFile(
							join(plansDir, "test-spec-handoff.md"),
							"# tests\n",
						);
						return { summary: "draft", planPath: prdPath };
					},
				...
				{
					task: "approval starts ultragoal",
					cwd,
					maxIterations: 1,
	...
	it("passes and enforces reusable Architect lane on re-review iterations", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-architect-reuse-"),
		);
		try {
			...
			const result = await runRalplanConsensus(
				{
					...
					async architectReview(ctx) {
						...
						return {
							...
							agent_role: "architect",
							thread_id:
								ctx.reusableRoleLanes.architect?.thread_id ??
								"thread-architect",
						};
					},
			...
			assert.deepEqual(
				...
				["thread-architect", "thread-architect"],
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("fails closed when a re-review Architect pass spawns a fresh lane without a new-lane reason", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-architect-reuse-deny-"),
		);
		try {
			const result = await runRalplanConsensus(
				{
					async draft(ctx) {
						...
						await writeFile(prdPath, "# plan\n");
						await writeFile(
							join(plansDir, "test-spec-reuse-deny.md"),
							"# tests\n",
						);
						return { summary: `draft-${ctx.iteration}`, planPath: prdPath };
					},
	...
	it("fails Autopilot-required consensus when approvals lack native subagent provenance", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-native-required-missing-"),
		);
			...
			const result = await runRalplanConsensus(
				{
					async draft() {
						...
						await writeFile(prdPath, "# plan\n");
						await writeFile(
							join(plansDir, "test-spec-native-missing.md"),
							"# tests\n",
						);
						return { summary: "draft", planPath: prdPath };
					},
					async architectReview() {
						return {
							verdict: "approve",
							summary: "artifact-only architect ok",
						};
					},
					async criticReview() {
	...
	it("rejects planner-as-architect review role when native evidence is required", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-planner-as-architect-"),
		);
		...
		try {
			...
			const result = await runRalplanConsensus(
				{
					async draft() {
						...
						await writeFile(prdPath, "# plan\n");
						await writeFile(
							join(plansDir, "test-spec-planner-as-architect.md"),
							"# tests\n",
						);
						return { summary: "draft", planPath: prdPath };
					},
			...
			assert.match(
				result.error || "",
				/ralplan_architect_review_role_mismatch/,
			);
			assert.equal(result.architectReviews.length, 0);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("rejects native/thread-backed missing-role reviews instead of rewriting them into consensus roles", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-native-missing-role-"),
		);
			...
			const result = await runRalplanConsensus(
				{
					async draft() {
						...
						await writeFile(prdPath, "# plan\n");
						await writeFile(
							join(plansDir, "test-spec-native-missing-role.md"),
							"# tests\n",
						);
						return { summary: "draft", planPath: prdPath };
					},
	...
	it("preserves existing tracker completion for review threads without native-required mode", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-preserve-completion-"),
		);
			...
			const result = await runRalplanConsensus(
				{
					async draft() {
						...
						await writeFile(prdPath, "# plan\n");
						await writeFile(
							join(plansDir, "test-spec-preserve-completion.md"),
							"# tests\n",
						);
						return { summary: "draft", planPath: prdPath };
					},
	...
	it("does not fabricate native tracker completion from approved review bookkeeping alone", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-native-bookkeeping-only-"),
		);
			...
			const result = await runRalplanConsensus(
				{
					async draft() {
						...
						await writeFile(prdPath, "# plan\n");
						await writeFile(
							join(plansDir, "test-spec-native-bookkeeping-only.md"),
							"# tests\n",
						);
						return { summary: "draft", planPath: prdPath };
					},
	...
	it("accepts Autopilot-required consensus with tracker-backed native architect and critic lanes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-native-required-ok-"),
		);
		...
		try {
			...
			const result = await runRalplanConsensus(
				{
					async draft() {
						...
						await writeFile(prdPath, "# plan\n");
						await writeFile(
							join(plansDir, "test-spec-native-ok.md"),
							"# tests\n",
						);
						return { summary: "draft", planPath: prdPath };
					},
			...
			assert.equal(
				result.ralplanConsensusGate.ralplan_critic_review?.thread_id,
				"thread-critic",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("rejects Autopilot-required consensus with legacy OMX-adapted Architect and Critic lanes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-adapted-required-ok-"),
		);
			...
			const result = await runRalplanConsensus(
				{
					async draft() {
						...
						await writeFile(prdPath, "# plan\n");
						await writeFile(
							join(plansDir, "test-spec-adapted-ok.md"),
							"# tests\n",
						);
						return { summary: "draft", planPath: prdPath };
					},
	...
	it("fails Autopilot-required consensus when native reviews reuse one subagent thread", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-native-same-thread-"),
		);
			...
			const result = await runRalplanConsensus(
				{
					async draft() {
						...
						await writeFile(prdPath, "# plan\n");
						await writeFile(
							join(plansDir, "test-spec-native-same-thread.md"),
							"# tests\n",
						);
						return { summary: "draft", planPath: prdPath };
					},
	...
	it("does not complete or call Critic when Architect has not approved", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-architect-reject-"),
		);
		...
		try {
			...
			assert.equal(
				(
					finalState?.ralplan_consensus_gate as
						| { ralplan_architect_review?: { agent_role?: string } }
						| undefined
				)?.ralplan_architect_review?.agent_role,
				"architect",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
			...
			const result = await runRalplanConsensus(
				{
					async draft(ctx) {
						...
						await writeFile(
							join(plansDir, "test-spec-loop.md"),
							"# loop tests\n",
						);
						return { summary: `draft-${ctx.iteration}`, planPath: prdPath };
					},
			...
			const result = await runRalplanConsensus(
				{
					...
					async criticReview() {
						return {
							verdict: "approve",
							summary: "critic approves malformed flow",
						};
					},
				},
	...
	it("fails closed when consensus approves with a mismatched stale test spec", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-mismatched-artifacts-"),
		);
	...
	it("fails closed when consensus approves without required planning artifacts", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-runtime-no-artifacts-"),
		);
	...
	it("marks cancelled state cleanly", async () => {
		...
		try {
			...
			assert.ok(
				typeof finalState?.completed_at === "string" &&
					finalState.completed_at.length > 0,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
