...
async function writeTrackerBackedSubagentTracking(
	...
	const architectCompletedAt =
		options.architectCompletedAt ?? "2026-07-13T10:00:00.000Z";
	const criticFirstSeenAt =
		options.criticFirstSeenAt === undefined
			? "2026-07-13T10:05:00.000Z"
			: options.criticFirstSeenAt;
	const criticCompletedAt =
		options.criticCompletedAt ?? "2026-07-13T10:10:00.000Z";
					...
					[sessionId]: {
						...
						threads: {
							...
							"thread-critic": {
								thread_id: "thread-critic",
								kind: "subagent",
								...(criticFirstSeenAt === null
									? {}
									: { first_seen_at: criticFirstSeenAt }),
	...
	if (
		...
		options.writeRoleRoutingMarker === false
	)
		return;
...
async function writeAdaptedSubagentTracking(
	...
	await writeTrackerBackedSubagentTracking(
		cwd,
		sessionId,
		"omx_adapted",
		options,
...
describe("ralplan consensus gate state roots", () => {
	it("rejects invalid complete consensus even when it appears after a valid source", () => {
		...
		assert.match(
			gate.blockedDetails?.join(" ") ?? "",
			/architect.*verdict=iterate/i,
		);
	});
		...
		for (const form of ["explicit", "fallback"] as const) {
			...
			for (const gate of [
				...
				if (form === "explicit") {
					assert.match(
						gate.blockedDetails?.join(" ") ?? "",
						/direct review order is not proven strictly architect-before-critic/i,
					);
				}
			}
		}
	...
	it("uses sequence order authoritatively and rejects contradictory timestamps", () => {
					...
					value: {
						ralplan_consensus_gate: {
							...
							ralplan_architect_review: {
								agent_role: "architect",
								verdict: "approve",
								...(architectSequence === undefined
									? {}
									: { sequence_index: architectSequence }),
								...(architectCompletedAt
									? { completed_at: architectCompletedAt }
									: {}),
							},
							ralplan_critic_review: {
								agent_role: "critic",
								verdict: "approve",
								...(criticSequence === undefined
									? {}
									: { sequence_index: criticSequence }),
								...(criticCompletedAt
									? { completed_at: criticCompletedAt }
									: {}),
							},
		...
		assert.equal(
			buildGate(1, 2, "2026-06-12T10:05:00.000Z", "2026-06-12T10:00:00.000Z")
				.complete,
			false,
		);
		assert.equal(
			buildGate(2, 1, "2026-06-12T10:00:00.000Z", "2026-06-12T10:05:00.000Z")
				.complete,
			false,
		);
	});
	...
	it("rejects malformed complete consensus even when it appears after a valid source", () => {
		...
		assert.match(
			gate.blockedDetails?.join(" ") ?? "",
			/sequence is not architect-review then critic-review/i,
		);
	});
	...
	it("ignores ambient root consensus unless the ambient session is bound to this cwd", async () => {
		...
		const ambientRoot = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-ambient-"),
		);
	...
	it("reads tracker-backed consensus evidence from OMX_STATE_ROOT instead of cwd/.omx/state", async () => {
		...
		const boxedRoot = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-state-root-"),
		);
		const previousOmxRoot = process.env.OMX_ROOT;
		const previousOmxStateRoot = process.env.OMX_STATE_ROOT;
		const previousOmxTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		...
		try {
			...
			assert.match(
				String(gate.source),
				new RegExp(`${boxedRoot.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`),
			);
		} finally {
			if (typeof previousOmxRoot === "string")
				process.env.OMX_ROOT = previousOmxRoot;
	...
	it("accepts ordered native reviews when runtime tracker lags but workspace tracker has completion evidence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-runtime-lag-"),
		);
		const boxedRoot = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-runtime-root-"),
		);
		const previousOmxRoot = process.env.OMX_ROOT;
		const previousOmxStateRoot = process.env.OMX_STATE_ROOT;
		const previousOmxTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		...
		try {
			...
			const workspaceSessionDir = join(
				workspaceStateDir,
				"sessions",
				sessionId,
			);
			...
			await writeFile(
				subagentTrackingPath(cwd),
				JSON.stringify(laggingTracker, null, 2),
			);
			await writeFile(
				join(workspaceStateDir, "subagent-tracking.json"),
				JSON.stringify(completedTracker, null, 2),
			);
			await writeFile(
				join(runtimeSessionDir, "ralplan-state.json"),
				JSON.stringify(
					{
						active: false,
	...
	it("rejects runtime tracker lag when workspace tracker also lacks completion evidence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-runtime-lag-incomplete-"),
		);
		const boxedRoot = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-runtime-incomplete-root-"),
		);
		const previousOmxRoot = process.env.OMX_ROOT;
		const previousOmxStateRoot = process.env.OMX_STATE_ROOT;
		const previousOmxTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		...
		try {
			...
			await writeFile(
				subagentTrackingPath(cwd),
				JSON.stringify(incompleteTracker, null, 2),
			);
			await writeFile(
				join(workspaceStateDir, "subagent-tracking.json"),
				JSON.stringify(incompleteTracker, null, 2),
			...
			assert.match(
				gate.blockedDetails?.join(" ") ?? "",
				/thread-architect is not completed/,
			);
		} finally {
			if (typeof previousOmxRoot === "string")
				process.env.OMX_ROOT = previousOmxRoot;
	...
	it("accepts session-scoped tracker-backed reviews without an explicit sessionId option", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-discovered-session-"),
		);
		const boxedRoot = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-discovered-root-"),
		);
		const previousOmxRoot = process.env.OMX_ROOT;
		const previousOmxStateRoot = process.env.OMX_STATE_ROOT;
		const previousOmxTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		...
		try {
			...
			assert.match(
				String(gate.source),
				new RegExp(`${sessionId}/ralplan-state\\.json$`),
			);
		} finally {
			if (typeof previousOmxRoot === "string")
				process.env.OMX_ROOT = previousOmxRoot;
	...
	it("rejects stale local ralplan state consensus during a return-to-ralplan cycle", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-stale-local-state-"),
		);
		try {
	...
	it("ignores stale invalid local ralplan state consensus during a return-to-ralplan cycle", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-stale-invalid-local-state-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("preserves ordered invalid direct consensus in a return-to-ralplan cycle without review_cycle", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-ordered-invalid-return-"),
		);
		try {
			const gate = buildRalplanConsensusGateForCwd(cwd, {
				artifacts: {
	...
	it("rejects local nested handoff consensus when only the local container review_cycle advances", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-local-container-only-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("accepts local nested handoff consensus when both reviews carry the advanced review_cycle", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-local-review-fresh-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("rejects local state.handoff_artifacts consensus when only the local container review_cycle advances", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-local-state-container-only-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("accepts local state.handoff_artifacts consensus when nested state and both reviews carry the advanced review_cycle", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-local-state-review-fresh-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("rejects stale review history consensus during a return-to-ralplan cycle", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-stale-history-"),
		);
		try {
			const gate = buildRalplanConsensusGateForCwd(cwd, {
				artifacts: {
	...
	it("rejects stale review array consensus during a return-to-ralplan cycle", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-stale-arrays-"),
		);
		try {
			const gate = buildRalplanConsensusGateForCwd(cwd, {
				artifacts: {
	...
	it("accepts tracker-backed native Architect and Critic lanes with exact ledger role identities and strict order", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-native-ok-"),
		);
		...
		try {
			...
			const reviews = consensus.ralplan_consensus_gate as Record<
				string,
				Record<string, unknown>
			>;
	...
	it("uses native mode as the role identity and permits truly roleless legacy native lanes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-native-mode-"),
		);
		...
		try {
			...
			for (const [threadId, mode] of [
				["thread-architect", "architect"],
				["thread-critic", "critic"],
			] as const) {
			...
			assert.match(
				mismatchedModeGate.blockedDetails?.join(" ") ?? "",
				/thread-architect has mode=planner, expected architect/,
			);
	...
	it("reads a tracker candidate once before evaluating native pair and individual evidence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-native-read-once-"),
		);
		...
		try {
			...
			try {
				const gate = buildRalplanConsensusGateFromSources(
					[
						{
							source: "native-read-once-first",
							value: nativeConsensus(sessionId),
						},
						{
							source: "native-read-once-second",
							value: nativeConsensus(sessionId),
						},
					],
					{ cwd, sessionId },
	...
	it("rejects a native lane when the ledger role does not match its review role", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-native-role-"),
		);
		const sessionId = "sess-native-consensus-role";
		try {
			await writeNativeSubagentTracking(cwd, sessionId, {
				architectRole: "planner",
			});
			const gate = buildRalplanConsensusGateFromSources(
				[
					{
			...
			assert.match(
				gate.blockedDetails?.join(" ") ?? "",
				/thread-architect has role=planner, expected architect/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("rejects a native lane when strict tracker order is missing or reversed", async () => {
		...
		try {
			...
			assert.match(
				missingOrderGate.blockedDetails?.join(" ") ?? "",
				/tracker review order is missing.*critic first_seen_at/i,
			);
			...
			assert.match(
				reversedOrderGate.blockedDetails?.join(" ") ?? "",
				/tracker review order is reversed/i,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("does not combine pair and individual native evidence from different tracker snapshots", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-native-snapshot-"),
		);
		const boxedRoot = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-native-snapshot-root-"),
		);
		const previousOmxStateRoot = process.env.OMX_STATE_ROOT;
	...
	it("rejects tracker-backed OMX-adapted Architect and Critic lanes even with valid legacy marker evidence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-adapted-ok-"),
		);
		...
		try {
			...
			assert.match(
				gate.blockedDetails?.join(" ") ?? "",
				/unsupported omx_adapted provenance/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("rejects an OMX-adapted lane when tracker order evidence is missing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-adapted-no-order-"),
		);
	...
	it("rejects forged OMX-adapted artifact timestamps when tracker ledger order is reversed", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-adapted-reversed-order-"),
		);
	...
	it("rejects an OMX-adapted lane when the ledger role does not match its review role", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-adapted-role-"),
		);
		const sessionId = "sess-adapted-consensus-role";
		try {
			await writeAdaptedSubagentTracking(cwd, sessionId, {
				architectRole: "critic",
			});
			const gate = buildRalplanConsensusGateFromSources(
				[
					{
	...
	it("rejects OMX-adapted reviews that reuse one tracker thread", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-adapted-reused-thread-"),
		);
	...
	it("rejects OMX-adapted lanes without scoped role-routing-unavailable evidence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-adapted-no-marker-"),
		);
		const sessionId = "sess-adapted-consensus-no-marker";
		try {
			await writeAdaptedSubagentTracking(cwd, sessionId, {
				writeRoleRoutingMarker: false,
			});
			const gate = buildRalplanConsensusGateFromSources(
				[
					{
	...
	it("rejects a native-provenance artifact that points to OMX-adapted ledger threads", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-ralplan-consensus-adapted-native-claim-"),
		);
		...
		try {
			...
			assert.match(
				gate.blockedDetails?.join(" ") ?? "",
				/conflicting with native_subagent review provenance/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
