...
export function sameFilePath(leftPath: string, rightPath: string): boolean {
	return (
		canonicalizeComparablePath(leftPath) ===
		canonicalizeComparablePath(rightPath)
	);
}
...
export function resolveOmxEntryPath(
	...
	const hasExplicitArgv1 = Object.prototype.hasOwnProperty.call(
		options,
		"argv1",
	);
...
function isOmxCliEntryPath(value: string | null | undefined): boolean {
	...
	return (
		normalized.endsWith("/dist/cli/omx.js") || normalized.endsWith("/omx.js")
	);
}
...
export function resolveOmxCliEntryPath(
	...
	const fallback = resolveLauncherPath(
		join(packageRootDir, "dist", "cli", "omx.js"),
...
export function rememberOmxLaunchContext(
	...
	if (
		String(env[OMX_ENTRY_PATH_ENV] ?? "").trim() !== "" &&
		(!hasExplicitArgv1 || explicitArgv1 === "")
	)
		return;
...
export async function detectLegacySkillRootOverlap(
	...
	const [
		...
		canonicalExists
			? realpath(canonicalDir).catch(() => null)
			: Promise.resolve(null),
		legacyExists
			? realpath(legacyDir).catch(() => null)
			: Promise.resolve(null),
	]);
