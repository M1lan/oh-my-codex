...
function classifyWindowsCommandPath(
	path: string,
...
function resolveWindowsCommandVariants(command: string): string[] {
	...
	const aliases =
		WINDOWS_COMPATIBLE_COMMAND_ALIASES[normalizeWindowsCommandName(command)];
...
function resolveWindowsNodeHostedCommandPath(
	...
	const relativeSegments =
		WINDOWS_NODE_HOSTED_COMMANDS[normalizeWindowsCommandName(command)];
...
function resolvePowerShellExecutable(
	...
	return (
		resolveWindowsCommandPath("powershell", env, existsImpl) || "powershell.exe"
	);
}

function shouldUseWindowsVerbatimArguments(
 ...
 */
export function isRunningUnderCmux(
	env: NodeJS.ProcessEnv = process.env,
): boolean {
...
export function buildPlatformCommandSpec(
	...
	const nodeHostedPath = resolveWindowsNodeHostedCommandPath(
		command,
		resolvedPath,
		existsImpl,
	);
