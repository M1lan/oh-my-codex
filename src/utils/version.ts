...
export function readPackageVersion(
	...
	) as PackageVersionMetadata | null;
	return typeof pkg?.version === "string" && pkg.version.trim() !== ""
		? stripLeadingV(pkg.version)
		: null;
}

function readInstallVersionMetadata(
	stampPath = omxUserInstallStampPath(),
): InstallVersionMetadata | null {
...
export function resolveOmxDisplayVersionSync(
	options: {
		packageRoot?: string;
		stampPath?: string;
	...
	const explicitRevision = shortRevision(
		env.OMX_VERSION_REVISION || env.OMX_GIT_REVISION,
	);
	...
	const devBaseVersion =
		typeof stamp?.dev_base_version === "string"
			? stripLeadingV(stamp.dev_base_version)
			: "";
	const isCurrentDevInstall =
		stamp?.install_channel === "dev" && stampVersion === version;
	if (isCurrentDevInstall) {
		const displayVersion = devBaseVersion || version;
		const revision =
			shortRevision(stamp.install_revision) ??
			explicitRevision ??
			readGitRevision(packageRoot);
		return revision
			? `v${displayVersion}-dev-${revision}`
			: `v${displayVersion}-dev`;
	}
