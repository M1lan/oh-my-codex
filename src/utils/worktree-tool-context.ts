...
function normalizeRequestedCodeGraphMode(
	env: NodeJS.ProcessEnv,
): RequestedCodeGraphMode {
	const raw = String(
		env.OMX_CODEGRAPH_REQUESTED_MODE ?? env.OMX_CODEGRAPH_MODE ?? "auto",
	)
		.trim()
		.toLowerCase();
...
export function resolveWorktreeToolContext(
	...
	const worktreeRoot = resolve(
		options.worktreeRoot?.trim() || resolveGitRoot(options.cwd) || options.cwd,
	);
	const repoRoot = resolve(
		options.repoRoot?.trim() || resolveGitRoot(options.cwd) || worktreeRoot,
	);
...
export function worktreeToolContextEnv(
