...
describe("resolveWorktreeToolContext", () => {
	it("uses sibling launch worktree with shared leader CodeGraph in auto mode", async () => {
		...
		try {
			...
			assert.equal(
				worktreeToolContextEnv(context).OMX_WORKTREE_SCOPE,
				"launch",
			);
			assert.match(renderCodeGraphInstructions(context), /shared leader index/);
			assert.match(
				renderCodeGraphInstructions(context),
				/not branch-accurate for worktree-only changes/,
			);
		} finally {
	...
	it("uses nested team leader CodeGraph as shared context", async () => {
		...
		try {
			...
			assert.equal(
				worktreeToolContextEnv(context).OMX_REPO_ROOT,
				leaderWorktree,
			);
		} finally {
			await rm(repo, { recursive: true, force: true });
		...
		try {
			...
			assert.equal(
				resolveWorktreeToolContext({
					...
					worktreeRoot: worktree,
					env: {
						OMX_CODEGRAPH_MODE: "shared",
						OMX_CODEGRAPH_REQUESTED_MODE: "auto",
					},
				}).codeGraphMode,
				"shared",
			);
