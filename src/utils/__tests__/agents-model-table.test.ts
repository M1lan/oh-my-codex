...
describe("agents model table", () => {
	...
	it("builds table rows for summary roles, exact pins, and posture/modelClass-driven recommendations", () => {
		...
		assert.match(
			table,
			/\| Standard \(subagent default\) \| `gpt-standard` \| high \|/,
		);
		assert.match(
			table,
			/\| `explore` \| `gpt-spark` \| low \| Fast codebase search and file\/symbol mapping \(fast-lane, fast\) \|/,
		);
		assert.match(
			table,
		...
		assert.match(
			table,
			/\| `code-reviewer` \| `gpt-frontier` \| high \| Comprehensive review across all concerns \(frontier-orchestrator, frontier\) \|/,
		);
		assert.match(
			table,
			/\| `critic` \| `gpt-frontier` \| high \| Plan\/design critical challenge and review \(frontier-orchestrator, frontier\) \|/,
		);
		assert.match(
			table,
			/\| `writer` \| `gpt-standard` \| high \| Documentation, migration notes, user guidance \(fast-lane, standard\) \|/,
		);
		assert.match(
			table,
			/\| `executor` \| `gpt-frontier` \| medium \| Code implementation, refactoring, feature work \(deep-worker, standard\) \|/,
		);
	});
	...
	it("reflects per-agent model and reasoning overrides", async () => {
		...
		try {
			...
			assert.match(
				table,
				/\| `architect` \| `gpt-5\.6-sol-architect` \| xhigh \|/,
			);
			assert.match(
				table,
				/\| `explore` \| `gpt-5\.6-sol-explore` \| medium \|/,
			);
		} finally {
