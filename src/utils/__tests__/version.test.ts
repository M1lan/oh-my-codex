...
async function withVersionFixture(
	...
	try {
		...
		await writeFile(
			...
			JSON.stringify({ version: "0.18.8" }, null, 2),
		);
		await run({
			packageRoot: join(root, "pkg"),
			stampPath: join(root, "state", "install-state.json"),
		});
	} finally {
...
describe("resolveOmxDisplayVersionSync", () => {
	it("returns a plain release version when no current dev install stamp is present", async () => {
		await withVersionFixture(({ packageRoot, stampPath }) => {
			assert.equal(
				resolveOmxDisplayVersionSync({ packageRoot, stampPath }),
				"v0.18.8",
			);
		});
	});
