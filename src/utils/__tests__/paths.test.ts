...
describe("comparable paths", () => {
	it("leaves ordinary paths under a canonical root unchanged", async () => {
		const root = await realpath(
			await mkdtemp(join(tmpdir(), "omx-comparable-path-")),
		);
		try {
...
describe("projectSkillsDir", () => {
	it("uses provided projectRoot", () => {
		assert.equal(
			projectSkillsDir("/my/project"),
			join("/my/project", ".codex", "skills"),
		);
	});
...
describe("legacyUserSkillsDir", () => {
	...
	it("returns ~/.agents/skills under HOME", () => {
		assert.equal(
			legacyUserSkillsDir(),
			join("/tmp/test-home", ".agents", "skills"),
		);
	});
});
...
describe("omxAdaptersDir", () => {
	it("returns .omx/adapters under the project root", () => {
		assert.equal(
			omxAdaptersDir("/my/project"),
			join("/my/project", ".omx", "adapters"),
		);
	});
});
...
describe("omxStateDir", () => {
	...
	it("uses provided projectRoot", () => {
		assert.equal(
			omxStateDir("/my/project"),
			join("/my/project", ".omx", "state"),
		);
	});
	...
	it("uses OMX_ROOT as boxed workspace root for all runtime paths", () => {
		...
		assert.equal(
			omxProjectMemoryPath("/ignored/project"),
			"/tmp/omx-box/.omx/project-memory.json",
		);
		assert.equal(
			omxNotepadPath("/ignored/project"),
			"/tmp/omx-box/.omx/notepad.md",
		);
...
describe("omxNotepadPath", () => {
	it("uses provided projectRoot", () => {
		assert.equal(
			omxNotepadPath("/my/project"),
			join("/my/project", ".omx", "notepad.md"),
		);
	});
...
describe("omxPlansDir", () => {
	it("uses provided projectRoot", () => {
		assert.equal(
			omxPlansDir("/my/project"),
			join("/my/project", ".omx", "plans"),
		);
	});
...
describe("omxLogsDir", () => {
	it("uses provided projectRoot", () => {
		assert.equal(
			omxLogsDir("/my/project"),
			join("/my/project", ".omx", "logs"),
		);
	});
...
describe("OMX launcher path resolution", () => {
	...
	it("canonicalizes symlinked startup cwd launcher paths to their real path", async () => {
		...
		const laterCwd = await mkdtemp(
			join(tmpdir(), "omx-launcher-symlink-later-"),
		);
		...
		try {
			...
			assert.notEqual(
				resolved,
				join(linkedStartupCwd, "dist", "cli", "omx.js"),
			);
		} finally {
	...
	it("records launcher context once so later cwd changes keep the absolute entry path", async () => {
		...
		try {
			...
			assert.equal(
				process.env[OMX_ENTRY_PATH_ENV],
				canonicalizeComparablePath(launcherPath),
			);
		} finally {
			await rm(startupCwd, { recursive: true, force: true });
		}
	...
	it("prefers explicit argv1 over an ambient OMX_ENTRY_PATH override", async () => {
		const startupCwd = await mkdtemp(
			join(tmpdir(), "omx-launcher-explicit-start-"),
		);
		try {
			const launcherDir = join(startupCwd, "dist", "cli");
			const launcherPath = join(launcherDir, "omx.js");
	...
	it("replaces stale ambient OMX_ENTRY_PATH when recording an explicit launcher argv1", async () => {
		const startupCwd = await mkdtemp(
			join(tmpdir(), "omx-launcher-explicit-record-"),
		);
	...
	it("records the default launcher path when called without an explicit argv1", async () => {
		const startupCwd = await mkdtemp(
			join(tmpdir(), "omx-launcher-default-record-"),
		);
	...
	it("falls back to the packaged CLI entry when argv1 points at a non-CLI script", async () => {
		const startupCwd = await mkdtemp(
			join(tmpdir(), "omx-launcher-cli-fallback-start-"),
		);
		const packageRootDir = await mkdtemp(
			join(tmpdir(), "omx-launcher-cli-fallback-root-"),
		);
		try {
	...
	it("keeps the resolved path when argv1 already points at the CLI entry", async () => {
		const startupCwd = await mkdtemp(
			join(tmpdir(), "omx-launcher-cli-direct-start-"),
		);
		try {
	...
	it("falls back from a non-OMX host binary to the packaged CLI entry", async () => {
		const startupCwd = await mkdtemp(
			join(tmpdir(), "omx-launcher-cli-host-start-"),
		);
		const packageRootDir = await mkdtemp(
			join(tmpdir(), "omx-launcher-cli-host-root-"),
		);
		try {
