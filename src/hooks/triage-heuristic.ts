...
const BROAD_DESIGN_STARTERS: readonly string[] = ["redesign "];
...
function isQuestionOrExplanation(
	normalized: string,
	wordCount: number,
): boolean {
...
export function triagePrompt(prompt: string): TriageDecision {
	...
	const hasLocalResearchAnchor = LOCAL_RESEARCH_EXCLUSION_SIGNALS.some(
		(pattern) => pattern.test(normalized),
	);
	const hasImplementationAction = IMPLEMENTATION_ACTION_SIGNALS.some(
		(pattern) => pattern.test(normalized),
	);
	const hasImplementationConnector = IMPLEMENTATION_CONNECTOR_SIGNALS.some(
		(pattern) => pattern.test(normalized),
	);
	const hasResearchLookupVerb = RESEARCHER_LOOKUP_VERBS.some((pattern) =>
		pattern.test(normalized),
	);
	const hasQuestionOrExplanation = isQuestionOrExplanation(
		normalized,
		wordCount,
	);
	const hasAnchoredEdit = hasAnchoredEditPattern(normalized);
	const hasExternalResearchSignal = RESEARCHER_EXTERNAL_SIGNALS.some(
		(pattern) => pattern.test(normalized),
	);
	const hasExternalRouteOverride =
		RESEARCHER_EXTERNAL_ROUTE_OVERRIDE_SIGNALS.some((pattern) =>
			pattern.test(normalized),
		);
	...
	if (
		wordCount <= ANCHORED_EDIT_WORD_LIMIT &&
		hasAnchoredEdit &&
		!(hasResearchLookupVerb && hasExternalRouteOverride)
	) {
	...
	if (
		...
		!hasExternalRouteOverride
	) {
		return {
			lane: "LIGHT",
			destination: "explore",
			reason: "local_reference_lookup",
		};
	}
	...
	if (
		...
		return {
			lane: "HEAVY",
			destination: "autopilot",
			reason: "structural_redesign_goal",
		};
	}
	...
	for (const starter of BROAD_DESIGN_STARTERS) {
		if (
			...
			VISUAL_DESIGN_TERMS.some((pattern) => pattern.test(normalized))
		) {
			return {
				lane: "LIGHT",
				destination: "designer",
