...
function readGitIndexSignature(
	gitDir: string,
...
async function readCachedCodebaseMap(
	...
	try {
		const parsed = JSON.parse(
			await readFile(cachePathForWorktree(worktreeRoot), "utf-8"),
		) as Partial<CodebaseMapCacheFile>;
...
function getTrackedSourceFiles(cwd: string): string[] {
	try {
		...
		return out
			...
			.filter(
				(f) =>
					f && !f.split("/").includes(".omx") && SOURCE_EXTS.has(extname(f)),
			);
	} catch {
		return [];
	}
...
export async function generateCodebaseMap(cwd: string): Promise<string> {
	try {
		...
		if (layout && indexSignature) {
			const cached = await readCachedCodebaseMap(
				layout.worktreeRoot,
				layout.gitDir,
				indexSignature,
			);
		...
		for (const dir of sortedDirs.slice(0, MAX_DIRS)) {
			...
			if (dir === "src") {
				...
				const sortedSubs = [...subGrouped.keys()].sort((a, b) =>
					a.localeCompare(b),
				);
