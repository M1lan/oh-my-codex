...
export type PromptThreadClassification =
	...
	| {
			readonly kind: "target-child";
			readonly rootOwnerSessionId: string;
			readonly proof: "spawn" | "transcript" | "tracker";
	  }
	| {
			readonly kind: "foreign-child";
			readonly rootOwnerSessionId: string;
			readonly proof: "spawn" | "transcript" | "tracker";
	  }
	| {
			readonly kind: "ambiguous-child";
			readonly rootOwnerSessionIds: readonly string[];
	  }
...
export interface PromptMutationAuthorization {
	...
	readonly targetRelation:
		| "explicit-independent"
		| "pointer-alias"
		| "pointer-fallback"
		| "notify-explicit-existing-fork";
...
export type ResolvedPromptTurnContext =
	| Readonly<{
			...
			ownership: Readonly<{
				status: "compatible" | "adoptable";
				normalizedOwnerCodexSessionId: string;
			}>;
	  }>
	| Readonly<{
			status: "suppressed-target-child";
			authorization: PromptMutationAuthorization;
	  }>
	| Readonly<{
			status: "rejected";
...
export function extractSelectedTargetOwnerEvidence(
	value: unknown,
): readonly SelectedTargetOwnerEvidence[] {
...
export function classifyPromptThread(
	facts: PromptThreadFacts | undefined,
	ownerIds: readonly string[],
): PromptThreadClassification {
	const proofValues: Array<
		[string | undefined, "spawn" | "transcript" | "tracker"]
	> = [
		[normalizeSessionId(facts?.spawnRootOwnerSessionId), "spawn"],
		[normalizeSessionId(facts?.transcriptRootOwnerSessionId), "transcript"],
	];
	for (const root of facts?.trackerRootOwnerSessionIds ?? [])
		proofValues.push([normalizeSessionId(root), "tracker"]);
	const roots = orderedUnique(proofValues.map(([root]) => root));
	if (roots.length === 0)
		return Object.freeze(
			facts?.rootOrDrift ? { kind: "root-or-drift" } : { kind: "unknown" },
		);
	...
	if (roots.length > 1)
		return Object.freeze({
			kind: "ambiguous-child",
			rootOwnerSessionIds: roots,
		});
	const [root] = roots;
	const proof =
		proofValues.find(([candidate]) => candidate === root)?.[1] ?? "tracker";
...
function authorization(
	...
	const owners = orderedUnique([
		...allowedOwnerCodexSessionIds,
		ownerCodexSessionId,
	]);
	return Object.freeze({
		targetSessionId,
		...
		allowedStorageSessionIds: orderedUnique([
...
function finishAuthorization(
	...
	if (value.thread.kind === "target-child")
		return Object.freeze({
			status: "suppressed-target-child",
			authorization: value,
		});
	if (value.thread.kind === "foreign-child")
		return diagnostic(
			"foreign_child",
			input.producer,
			input.selectedPointer.status,
			value.targetRelation,
			input.nowIso,
		);
	if (value.thread.kind === "ambiguous-child")
		return diagnostic(
			"ambiguous_child",
			input.producer,
			input.selectedPointer.status,
	...
	return Object.freeze({
		...
		ownership: Object.freeze({
			status: value.legacyAdoption === "allow" ? "adoptable" : "compatible",
			normalizedOwnerCodexSessionId: value.ownerCodexSessionId,
		}),
	});
}
 ...
 */
export function evaluateResolvedPromptTurn(
	input: EvaluateResolvedPromptTurnInput,
): ResolvedPromptTurnContext {
	...
	const state =
		input.selectedPointer.status === "usable"
			? input.selectedPointer.state
			: undefined;
	...
	if (payload) {
		...
		if (!ownerEnv || ownerEnv === payload) {
			const globalSideEffects =
				input.selectedPointer.status === "absent" ? "allow" : "suppress";
			return finishAuthorization(
				input,
				authorization(
					payload,
	...
	if (!state || !canonical)
		return diagnostic(
			input.selectedPointer.status === "absent"
				? "payload_session_absent"
				: "selected_pointer_unusable",
			input.producer,
			input.selectedPointer.status,
	...
	if (ownerEnv && !aliases.includes(ownerEnv))
		return diagnostic(
			"owner_env_unbound",
			input.producer,
			input.selectedPointer.status,
...
export function preflightSelectedTargetOwner(
	...
	for (const item of evidence) {
		const target = hasRawValue(item.targetSessionId)
			? normalizeSessionId(item.targetSessionId)
			: undefined;
		const owner = hasRawValue(item.ownerCodexSessionId)
			? normalizeSessionId(item.ownerCodexSessionId)
			: undefined;
		if (hasRawValue(item.targetSessionId) && !target)
			return diagnostic(
				"target_owner_mismatch",
				producer,
				"preflight",
				authorization.targetRelation,
				nowIso,
			);
		if (target && target !== authorization.targetSessionId)
			return diagnostic(
				"target_owner_mismatch",
				producer,
				"preflight",
		...
		if (hasRawValue(item.ownerCodexSessionId) && !owner)
			return diagnostic(
				"owner_conflict",
				producer,
				"preflight",
		...
		if (owner && !authorization.allowedOwnerCodexSessionIds.includes(owner))
			return diagnostic(
				"owner_conflict",
				producer,
				"preflight",
		...
		if (
			!owner &&
			target === authorization.targetSessionId &&
			authorization.legacyAdoption === "deny"
		)
			return diagnostic(
				"legacy_adoption_denied",
				producer,
	...
	return Object.freeze({
		...
		ownership: Object.freeze({
			...context.ownership,
			status: adoptable ? "adoptable" : "compatible",
		}),
	});
}
