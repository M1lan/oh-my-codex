...
export type KeywordInertDiagnostic =
	(typeof KEYWORD_INERT_DIAGNOSTIC_ORDER)[number];
...
export interface RecordSkillActivationInput {
	...
	onProvenanceRejected?: (
		diagnostic: PromptDiagnosticDescriptor,
	) => void | Promise<void>;
}
...
export const DEEP_INTERVIEW_BLOCKED_APPROVAL_INPUTS = [
	"yes",
	"y",
	"proceed",
	"continue",
...
export const DEEP_INTERVIEW_INPUT_LOCK_MESSAGE =
...
const STATEFUL_SKILL_SEED_CONFIG: Record<
	StatefulSkillMode,
	StatefulSkillSeedConfig
> = {
...
function isSafeAutopilotContextSnapshotPath(value: unknown): value is string {
	...
	const snapshotName = path.startsWith(contextPrefix)
		? path.slice(contextPrefix.length)
		: "";
	return (
		path.startsWith(".omx/context/") &&
		path.endsWith(".md") &&
		!isAbsolute(path) &&
		!path.split("/").includes("..") &&
		!path.includes("\\") &&
...
async function isReadableAutopilotContextSnapshotPath(
	sourceCwd: string,
	value: unknown,
): Promise<boolean> {
	...
	try {
		...
		if (
			relativeToContext === "" ||
			relativeToContext.startsWith("..") ||
			isAbsolute(relativeToContext)
		)
			return false;
...
function normalizeAutopilotContextSnapshotCandidate(
	candidate: AutopilotContextSnapshotCandidate,
): AutopilotContextSnapshotDescriptor | null {
	if (isRecord(candidate.value)) {
		const path = safeString(candidate.value.path).trim();
		const kind = safeString(
			candidate.value.kind,
		).trim() as AutopilotContextSnapshotKind;
...
async function findReusableAutopilotContextSnapshotPath(
	...
	for (const candidate of candidates) {
		...
		if (
			!normalized ||
			isAutopilotRecoverySnapshotPath(normalized.path) ||
			!isSafeAutopilotContextSnapshotPath(normalized.path)
		)
			continue;
		if (
			await isReadableAutopilotContextSnapshotPath(sourceCwd, normalized.path)
		)
			return normalized;
	}
	return undefined;
...
const AUTOPILOT_CONTEXT_RECOVERY_REASON_MESSAGES: Record<
	AutopilotContextRecoveryReason,
	string
> = {
	"missing-or-unsafe-legacy-context-snapshot":
		"no safe legacy Autopilot context snapshot path was available during continuation.",
	"missing-autopilot-mode-state":
		"active Autopilot skill state existed but no matching Autopilot mode state was available during continuation.",
	"malformed-autopilot-mode-state":
		"active Autopilot mode state could not be parsed during continuation.",
	"nonpreservable-autopilot-mode-state-missing-current-phase":
		"active Autopilot mode state was missing current_phase during continuation.",
};

async function ensureSafeAutopilotContextDir(
	sourceCwd: string,
): Promise<string> {
...
async function ensureAutopilotContextSnapshot(
	...
	if (existingSnapshot) {
		if (isSafeAutopilotContextSnapshotPath(existingSnapshot.path)) {
			return {
				...
				original_task_status:
					existingSnapshot.kind === "legacy"
						? "legacy-unverified"
						: "activation-prompt",
			};
		}
		throw new Error(
			`Unsafe Autopilot context snapshot path: ${existingSnapshot.path}`,
		);
	}
	...
	if (options.allowTaskSnapshotCreation === false) {
		...
		const reason =
			options.recoveryReason ?? "missing-or-unsafe-legacy-context-snapshot";
		const body = [
...
function createDeepInterviewInputLock(
	nowIso: string,
	previous?: DeepInterviewInputLock,
...
function preserveCompletedDeepInterviewPhase(
	previousModeState: DeepInterviewModeState | null,
): string {
...
async function readExistingDeepInterviewState(
	statePath: string,
): Promise<DeepInterviewModeState | null> {
	try {
...
async function readJsonStateIfExists(
	path: string,
...
export async function persistDeepInterviewModeState(
	...
	if (nextSkill?.skill === "deep-interview" && nextSkill.active) {
		const configStateFields = buildDeepInterviewConfigStateFields(
			nextSkill.deep_interview_config,
		);
		...
		const nextState = withModeRuntimeContext<DeepInterviewModeState>(
			previousModeState ?? {},
			{
				...(previousModeState?.tmux_pane_id
					? { tmux_pane_id: previousModeState.tmux_pane_id }
					: {}),
				...(previousModeState?.tmux_pane_set_at
					? { tmux_pane_set_at: previousModeState.tmux_pane_set_at }
					: {}),
				active: true,
				mode: "deep-interview",
				current_phase: previousModeState?.active
					? previousModeState.current_phase || "intent-first"
					: "intent-first",
				started_at: previousModeState?.active
					? previousModeState.started_at || nowIso
					: nowIso,
				updated_at: nowIso,
				session_id: input.sessionId ?? previousModeState?.session_id,
				owner_codex_session_id:
					nextSkill.owner_codex_session_id ??
					previousModeState?.owner_codex_session_id,
				...
				...(nextQuestionEnforcement
					? { question_enforcement: nextQuestionEnforcement }
					: {}),
				...(previousModeState?.downstream_authority
					? { downstream_authority: previousModeState.downstream_authority }
					: {}),
				...(previousModeState?.bypass_planning_gate_until
					? {
							bypass_planning_gate_until:
								previousModeState.bypass_planning_gate_until,
						}
					: {}),
			},
	...
	const hadActiveDeepInterview =
		previousSkill?.skill === "deep-interview" && previousSkill.active === true;
	if (!previousModeState?.active && !hadActiveDeepInterview) return;

	const releasedInputLock =
		nextSkill?.skill === "deep-interview"
			? nextSkill.input_lock
			: previousSkill?.input_lock;
	const questionExitReason =
		nextSkill?.skill === "deep-interview" && nextSkill.active === false
			? "abort"
			: "handoff";
	const nextState: DeepInterviewModeState = {
		...
		mode: "deep-interview",
		current_phase:
			preserveCompletedDeepInterviewPhase(previousModeState) || "completing",
		started_at:
			previousModeState?.started_at || previousSkill?.activated_at || nowIso,
		updated_at: nowIso,
		completed_at: nowIso,
		session_id:
			input.sessionId ??
			previousModeState?.session_id ??
			previousSkill?.session_id,
		owner_codex_session_id:
			nextSkill?.owner_codex_session_id ??
			previousModeState?.owner_codex_session_id ??
			previousSkill?.owner_codex_session_id,
		thread_id:
			input.threadId ??
			previousModeState?.thread_id ??
			previousSkill?.thread_id,
		turn_id:
			input.turnId ?? previousModeState?.turn_id ?? previousSkill?.turn_id,
...
function isResettableTerminalModeState(
	state: Record<string, unknown> | null,
	expectedMode: string,
): boolean {
	if (!state || safeString(state.mode).trim() !== expectedMode) return false;

	const phase = safeString(state.current_phase)
		.trim()
		.toLowerCase()
		.replace(/_/g, "-");
...
async function persistBlockedFreshAutopilotState(
	...
	const { absolutePath } = resolveSeedStateFilePath(
		stateDir,
		"autopilot",
		sessionId,
	);
	...
	const state = withModeRuntimeContext(
		{},
		{
			...
			error,
			status_message:
				"Status: failed — Autopilot cannot start without an official host consensus receipt verifier.",
			handoff_artifacts: {
				ralplan_consensus_gate: {
					blocked_reason: error,
					blocked_details: [
						"official host consensus receipt verifier is unavailable",
					],
				},
			},
		},
...
async function persistStatefulSkillSeedState(
	...
	const config =
		STATEFUL_SKILL_SEED_CONFIG[nextSkill.skill as StatefulSkillMode];
	...
	const sameActiveSkill =
		previousSkill?.skill === nextSkill.skill && previousSkill.active;
	...
	const existingModeTerminal =
		existingModeMatches &&
		isResettableTerminalModeState(
			existingModeState as Record<string, unknown>,
			config.mode,
		);
	const preserveExistingModeState =
		existingModeMatches &&
		existingPhase !== "" &&
	...
	const startedAt =
		previousSkill?.skill === nextSkill.skill &&
		previousSkill.active &&
		!existingModeTerminal
			? safeString(existingModeState?.started_at).trim() ||
				previousSkill.activated_at ||
				nowIso
	...
	const baseState: Record<string, unknown> = withModeRuntimeContext(
		...
		{
			...(preserveExistingModeState ? existingModeState : {}),
			...(existingModeState?.tmux_pane_id
				? { tmux_pane_id: existingModeState.tmux_pane_id }
				: {}),
			...(existingModeState?.tmux_pane_set_at
				? { tmux_pane_set_at: existingModeState.tmux_pane_set_at }
				: {}),
			active: true,
			mode: config.mode,
			...
			session_id:
				nextSkill.session_id ||
				safeString(existingModeState?.session_id).trim() ||
				undefined,
			owner_codex_session_id:
				nextSkill.owner_codex_session_id ||
				safeString(existingModeState?.owner_codex_session_id).trim() ||
				undefined,
			thread_id:
				nextSkill.thread_id ||
				safeString(existingModeState?.thread_id).trim() ||
				undefined,
			turn_id:
				nextSkill.turn_id ||
				safeString(existingModeState?.turn_id).trim() ||
				undefined,
		},
		{ nowIso },
	);
	...
	if (config.includeIteration) {
		...
		const reusableModeState = preserveExistingModeState
			? existingModeState
			: null;
		baseState.iteration =
			typeof reusableModeState?.iteration === "number"
				? reusableModeState.iteration
				: defaultIteration;
		baseState.max_iterations =
			typeof reusableModeState?.max_iterations === "number"
				? reusableModeState.max_iterations
	...
	if (config.mode === "deep-interview") {
		Object.assign(
			baseState,
			buildDeepInterviewConfigStateFields(nextSkill.deep_interview_config),
		);
	}
	...
	if (config.mode === "autopilot") {
		const reusableModeState = preserveExistingModeState
			? existingModeState
			: null;
		const existingStateRaw =
			reusableModeState?.state && typeof reusableModeState.state === "object"
		...
		const existingHandoffs =
			...
			typeof existingState.handoff_artifacts === "object"
				...
				: {};
		let recoveryReason: AutopilotContextRecoveryReason =
			"missing-or-unsafe-legacy-context-snapshot";
		...
		const existingContextSnapshotPath =
			await findReusableAutopilotContextSnapshotPath(sourceCwd, [
		...
		const contextSnapshot = await ensureAutopilotContextSnapshot(
			...
			{
				allowTaskSnapshotCreation: !(
					preserveExistingModeState || options.activeContinuation === true
				),
				recoveryReason,
			},
		);
		const contextSnapshotPath = contextSnapshot.path;
		baseState.review_cycle =
			typeof reusableModeState?.review_cycle === "number"
				? reusableModeState.review_cycle
				: 0;
		delete baseState.context_snapshot_path;
		baseState.state = {
			...existingState,
			phase_cycle: Array.isArray(existingState.phase_cycle)
				? existingState.phase_cycle
			...
			handoff_artifacts: {
				...
				context_snapshot: {
					...
					...(contextSnapshot.original_task_status
						? { original_task_status: contextSnapshot.original_task_status }
						: {}),
					...(contextSnapshot.recovery
						? { recovery: contextSnapshot.recovery }
						: {}),
				},
			},
			review_verdict: Object.prototype.hasOwnProperty.call(
				existingState,
				"review_verdict",
			)
				? existingState.review_verdict
				: null,
			qa_verdict: Object.prototype.hasOwnProperty.call(
				existingState,
				"qa_verdict",
			)
				? existingState.qa_verdict
				: null,
			return_to_ralplan_reason: Object.prototype.hasOwnProperty.call(
				existingState,
				...
				: null,
			...(contextSnapshot.recovery
				? { context_snapshot_recovery: contextSnapshot.recovery }
				: {}),
			deep_interview_gate:
				existingState.deep_interview_gate &&
				typeof existingState.deep_interview_gate === "object"
					? existingState.deep_interview_gate
					: {
							status: "required",
							skip_reason: null,
			...
			planning_routing:
				...
				typeof existingState.planning_routing === "object"
					? existingState.planning_routing
...
const DEEP_INTERVIEW_MANAGEMENT_MENTION_PATTERN =
	/\b(?:clear|cleanup|clean\s+up|remove|reset|delete|fix|debug|report|reported|status|state|lock|unlock|active|inactive|session(?:-scoped)?|scope|scoped|global|legacy|root|mode|workflow)\b/i;
...
const EXPLICIT_TOKEN_START =
	/\$(?:(?:[Oo][Hh]-[Mm][Yy]-[Cc][Oo][Dd][Ee][Xx]):)?([A-Za-z][A-Za-z0-9_-]*)/guy;
...
const DIRECTIVE_COMMAND_PREFIX_AT = new RegExp(
	DIRECTIVE_COMMAND_PREFIX_SOURCE,
	"yu",
);
...
const NEGATIVE_PREFIX_PATTERN = new RegExp(
	`(?:${ASCII_NEGATIVE_PREFIX}|[Нн][Ее]\\s+(?:[Зз]апускай|[Ии]спользуй)|実行しないで|使わないで)`,
	"u",
);
const LIST_DOCUMENTATION_SUFFIX =
	/^(?:\s*(?::|：|[—–])\s*\S|\s+(?:is|are|means|refer(?:s)?\s+to|denote(?:s)?)\b.*\b(?:commands?|workflows?|skills?|modes?|files?|documentation)\b)/i;
...
const EXPLICIT_DOCUMENTATION_SUFFIX_AT = new RegExp(
	EXPLICIT_DOCUMENTATION_SUFFIX_SOURCE,
	"iyu",
);
const IMPLICIT_WORKFLOW_NOUN = "(?:modes?|workflows?|skills?|loops?)";
const POSTPOSED_NEGATIVE_PREDICATE =
	/\s+(?:(?:is|are|was|were|should|must|can|could|may|will|would)\s+(?:(?:also|still)\s+)?(?:not|never)\b|(?:isn't|aren't|wasn't|weren't|cannot|can't|shouldn't|mustn't|won't|wouldn't|couldn't)\b|(?:is|are|was|were)\s+(?:(?:also|still)\s+)?(?:inert|prohibited|forbidden|disabled|disallowed|unsupported)\b|(?:should|must|can|could|may|will|would)\s+(?:(?:also|still)\s+)?(?:not\s+)?be\s+(?:inert|avoided|prohibited|forbidden|disabled|disallowed|unsupported)\b|(?:is|are|was|were)\s+(?:(?:also|still)\s+)?to\s+be\s+(?:inert|avoided|prohibited|forbidden|disabled|disallowed|unsupported)\b)/iu;
const PROMPTS_TOKEN_PATTERN = /\/[Pp][Rr][Oo][Mm][Pp][Tt][Ss]:[\w.-]+/gu;
const COORDINATED_WORKFLOW_SUBJECT =
	"(?:(?:the|a|an)\\s+)?(?:autopilot|deep(?:[- ]+)interview|ralph|ralplan|ultrawork|ulw|ultragoal|ultraqa|autoresearch|coordinated\\s+team|team|consensus\\s+plan|code\\s+review|wiki|prometheus(?:-strict)?)";
const COORDINATED_SUBJECT_JOINER =
	"(?:and|or|nor|as\\s+well\\s+as|along\\s+with|together\\s+with|&)";
...
const COORDINATED_EXPLICIT_POSTPOSED_SEPARATOR = new RegExp(
	COORDINATED_POSTPOSED_SEPARATOR_SOURCE,
	"iyu",
);
...
const COORDINATED_SUBJECT_QUANTIFIER = new RegExp(
	asciiCaseWordPattern("both"),
	"yu",
);
const POSTPOSED_SUBJECT_NOUN = new RegExp(
	`\\s+${IMPLICIT_WORKFLOW_NOUN}`,
	"iyu",
);
...
function isDiscourseNegation(
	...
	return (
		/(?:^|\s)i'm\s+$/iu.test(text.slice(0, start)) &&
...
function isClauseBoundaryAt(text: string, index: number): boolean {
	...
	return (
		!next ||
		SAFE_TOKEN_WHITESPACE.test(next) ||
		/["'”’»›」』）)\]}]/u.test(next)
	);
}
...
function isLogicalSentenceBoundaryAt(text: string, index: number): boolean {
	...
	const normalized = normalizedPunctuation(character);
	return (
		LOGICAL_SENTENCE_BOUNDARY_PUNCTUATION.test(normalized) &&
		isClauseBoundaryAt(text, index)
	);
}

function documentationSuffixLimit(
	text: string,
	start: number,
	limit: number,
): number {
...
function hasExplicitDocumentationSuffixAt(
	text: string,
	start: number,
	limit: number,
): boolean {
	...
	return (
		EXPLICIT_DOCUMENTATION_SUFFIX_AT.exec(text.slice(start, clauseLimit)) !==
		null
	);
}
...
function isAbbreviationDotAt(text: string, index: number): boolean {
	...
	return /(?:^|[^A-Za-z0-9_])(?:e\.g|i\.e|etc|vs|mr|mrs|ms|dr|prof|sr|jr|st|no|fig|ver|v)\.$/i.test(
		context,
	);
}
	...
	for (let cursor = 0; cursor < text.length; ) {
		...
		if (isLogicalSentenceBoundaryAt(text, cursor)) {
			if (
				...
				!isAbbreviationDotAt(text, cursor)
			)
				return true;
		}
...
function hasUnicodeGrammarTokenBoundaries(
	text: string,
	start: number,
	end: number,
): boolean {
	...
	return (
		hasUnicodeGrammarTokenStart(text, start) &&
		(!next || !TOKEN_CONTINUATION.test(next))
	);
}

function firstGrammarTokenStart(
	text: string,
	start: number,
	end: number,
): number {
...
function hasAsciiGrammarTokenCharacters(
	text: string,
	start: number,
	end: number,
): boolean {
	for (let cursor = start; cursor < end; ) {
		const character = codePointAt(text, cursor);
		if (
			TOKEN_CONTINUATION.test(character) &&
...
function isExplicitTokenBoundary(text: string, index: number): boolean {
	...
	if (
		...
		EXPLICIT_TOKEN_BOUNDARY_PUNCTUATION.test(compatibility)
	)
		return true;
...
type StructuralInertDiagnostic = Exclude<
	KeywordInertDiagnostic,
	"escaped" | "not-leading-region"
>;
...
function markdownContainerPrefix(
	line: string,
	maximumLeadingSpaces = 3,
...
function markdownFenceAtStart(
	...
	return length >= 3
		? {
				...
				listItem: container.listDepth > 0,
				indent: start,
				closingEligible: /^[\t ]*$/.test(line.slice(cursor)),
			}
		: null;
}
...
function collectFencedCodeRanges(text: string): InertRange[] {
	...
	while (lineStart <= text.length) {
		...
		const line = text.slice(lineStart, end);
		if (fence) {
			const closer = markdownFenceAtStart(
				line,
				fence.definition.listItem ? fence.definition.indent + 3 : 3,
			);
			if (
				...
				!closer.listItem &&
				(!fence.definition.listItem ||
					closer.indent >= fence.definition.indent) &&
	...
	while (lineStart <= text.length) {
		...
		for (let cursor = lineStart; cursor < end; cursor += 1) {
			...
			if (
				...
				LETTER_OR_NUMBER.test(previous) &&
...
function createInertRangeIndex(
...
function collectInertRangeIndexes(
	text: string,
): Readonly<Record<StructuralInertDiagnostic, InertRangeIndex>> {
	return {
...
function inertRangeEndAt(
	index: InertRangeIndex,
	position: number,
): number | null {
	let lower = 0;
...
function isStructurallyInert(
	...
	return STRUCTURAL_INERT_DIAGNOSTICS.some((reason) =>
		isInInertRange(indexes[reason], position),
	);
}
...
function leadingDirectiveCursor(
	text: string,
	regionStart: number,
	allowLineBreaks: boolean,
): { cursor: number; listItem: boolean } {
	const whitespacePattern = allowLineBreaks
		? /^\s*/u
		: /^[^\S\r\n\u2028\u2029]*/u;
	const cursor =
		regionStart +
		(whitespacePattern.exec(text.slice(regionStart))?.[0].length ?? 0);
	const container = markdownContainerPrefix(
		text.slice(cursor, lineEnd(text, cursor)),
	);
...
function directiveCommandTargetStart(
	text: string,
	cursor: number,
	limit = text.length,
): number | null {
	...
	const targetStart =
		prefix?.index === cursor ? cursor + prefix[0].length : null;
...
function promptLeadingExplicitCandidateStart(text: string): number {
	...
	return commandTarget !== null && text[commandTarget] === "$"
		? commandTarget
		: leading.cursor;
}

function punctuationSeparatedCandidateStart(
	text: string,
	cursor: number,
): number | null {
...
function coordinatedDocumentationCandidateStart(
	text: string,
	cursor: number,
): number | null {
	const scanner =
...
function clauseDirectiveStart(
	...
	const directiveStart = boundedHorizontalCursor(
		text,
		clauseStart,
		candidateStart,
	);
	return (
		hasAdjacentPredecessorSeparator(text, predecessorEnd, directiveStart) &&
		directiveCommandTargetStart(text, directiveStart, candidateStart) ===
			candidateStart
	);
}

function hasAdjacentPredecessorSeparator(
	text: string,
	predecessorEnd: number,
	candidateStart: number,
): boolean {
	...
	for (let cursor = predecessorEnd; cursor < candidateStart; ) {
		const character = codePointAt(text, cursor);
		if (
			!SAFE_TOKEN_WHITESPACE.test(character) &&
			!isClauseBoundaryAt(text, cursor) &&
			normalizedPunctuation(character) !== ":" &&
			!/[—–-]/u.test(character)
		)
...
function collectLineMatchPositions(
	...
	while ((match = pattern.exec(text)) !== null && match.index < end) {
		if (
			...
			(!requireEndBoundary ||
				hasUnicodeGrammarTokenBoundaries(text, match.index, pattern.lastIndex))
		) {
...
function hasExclusionPrefixAt(text: string, start: number): boolean {
	const pattern = new RegExp(
		`\\s*(?:${ASCII_PLEASE}\\s+)?${EXCLUSION_PREFIX}`,
		"yu",
	);
	pattern.lastIndex = start;
	...
	return (
		match?.index === start &&
		hasUnicodeGrammarTokenBoundaries(text, start, pattern.lastIndex)
	);
}

function collectExplicitPrefixNegationIndex(
	text: string,
	candidates: readonly ExplicitCandidateScan[],
): ExplicitPrefixNegationIndex {
	...
	const directiveScanner = new RegExp(
		`(?:${ASCII_AND}|${ASCII_THEN})\\s+(?:${ASCII_PLEASE}\\s+)?(?:${ASCII_INSTEAD}\\s+)?(?=${ASCII_DIRECTIVE_COMMAND})`,
		"gu",
	);
	let firstCandidateIndex = 0;

	while (firstCandidateIndex < candidates.length) {
		const lineStart = lineStartForPosition(
			text,
			candidates[firstCandidateIndex].start,
		);
		const lineLimit = lineEnd(text, candidates[firstCandidateIndex].end);
		let afterLineCandidateIndex = firstCandidateIndex;
		while (
			afterLineCandidateIndex < candidates.length &&
			candidates[afterLineCandidateIndex].start < lineLimit
		)
			afterLineCandidateIndex += 1;

		const negativeMatches = collectLineMatchPositions(
			negativeScanner,
			normalizedText,
			lineStart,
			lineLimit,
		);
		const contrastMatches = collectLineMatchPositions(
			contrastScanner,
			text,
			lineStart,
			lineLimit,
		...
		const directiveMatches = collectLineMatchPositions(
			directiveScanner,
			text,
			lineStart,
			lineLimit,
		...
		for (
			...
			candidateIndex < afterLineCandidateIndex;
			candidateIndex += 1
		) {
			...
			while (
				negativeMatchIndex < negativeMatches.length &&
				negativeMatches[negativeMatchIndex].end <= candidate.start
			) {
				latestNegativeStart = negativeMatches[negativeMatchIndex].start;
				negativeMatchIndex += 1;
			}
			while (
				contrastMatchIndex < contrastMatches.length &&
				contrastMatches[contrastMatchIndex].end <= candidate.start
			) {
				latestContrastStart = contrastMatches[contrastMatchIndex].start;
				contrastMatchIndex += 1;
			}
			while (
				directiveMatchIndex < directiveMatches.length &&
				directiveMatches[directiveMatchIndex].end <= candidate.start
			) {
				latestCoordinatedDirectiveStart =
					directiveMatches[directiveMatchIndex].start;
				directiveMatchIndex += 1;
			}

			const punctuationDirectiveStart = boundedHorizontalCursor(
				text,
				punctuationStart,
				candidate.start,
			);
			if (
				directiveCommandTargetStart(
					text,
					punctuationDirectiveStart,
					candidate.start,
				) === candidate.start
			) {
				latestPunctuationDirectiveStart = Math.max(
					latestPunctuationDirectiveStart,
					punctuationDirectiveStart,
				);
			}
			...
			if (
				latestPunctuationDirectiveStart >= effectiveClauseStart &&
				latestPunctuationDirectiveStart > latestNegativeStart
			) {
				effectiveClauseStart = latestPunctuationDirectiveStart;
			}
			if (
				latestCoordinatedDirectiveStart >= effectiveClauseStart &&
				hasExclusionPrefixAt(text, effectiveClauseStart)
			) {
	...
	return {
		negatedStarts,
		postposedSubjectStarts,
		firstPostposedSubjectCandidates,
	};
}

function directivePrefixAfter(
	text: string,
	start: number,
	limit: number,
): { start: number; targetStart: number } | null {
	...
	if (isClauseBoundaryAt(text, cursor)) {
		...
		const coordinator = new RegExp(
			`(?:${ASCII_AND}|${ASCII_BUT}|${ASCII_INSTEAD}|${ASCII_THEN}|${ASCII_NOW})`,
			"yu",
		);
		...
		if (
			match?.index !== cursor ||
			!hasUnicodeGrammarTokenBoundaries(text, cursor, coordinator.lastIndex)
		)
			return null;
	}
...
function isExclusionPrefixDirectiveTransitionAt(
	...
	if (!punctuationTransition) {
		...
		coordinatedTransition =
			match?.index === transitionStart &&
			hasUnicodeGrammarTokenBoundaries(
				text,
				transitionStart,
				coordinator.lastIndex,
			);
	}
	...
	return (
		!punctuationTransition ||
		clauseDirectiveStart(text, transitionStart, directive.targetStart)
	);
}
...
function exclusionPrefixDirectiveTransitionStart(
	...
	for (let cursor = start; cursor < limit; ) {
		if (
			isExclusionPrefixDirectiveTransitionAt(text, prefixStart, cursor, limit)
		)
			return cursor;
		cursor += codePointAt(text, cursor).length;
	}
...
function documentationDirectiveTransitionStart(
	text: string,
	start: number,
	limit: number,
): number | null {
	const scanner = new RegExp(
		`(?:${ASCII_AND}|${ASCII_BUT}|${ASCII_THEN}|${ASCII_INSTEAD}|${ASCII_NOW})`,
		"gu",
	);
...
function postposedNegationClauseEnd(
	text: string,
	start: number,
	limit: number,
): number {
...
function stickyMatchEnd(
	...
	pattern.lastIndex = start;
	const match = pattern.exec(text);
	return match && match.index === start && pattern.lastIndex <= limit
		? pattern.lastIndex
		: null;
}
...
function coordinatedSubjectAt(
	...
	if (
		...
		candidates[explicitIndex].reasons.size === 0
	) {
		return { end: candidates[explicitIndex].end, explicitIndex };
	}
	const implicitEnd = stickyMatchEnd(
		COORDINATED_IMPLICIT_DOCUMENTATION_SUBJECT,
		text,
		start,
		limit,
	);
...
function collectMixedSubjectChain(
	...
	const quantifierEnd = stickyMatchEnd(
		COORDINATED_SUBJECT_QUANTIFIER,
		text,
		cursor,
		limit,
	);
	const quantifiedSubjectStart =
		quantifierEnd === null
			? cursor
			: boundedHorizontalCursor(text, quantifierEnd, limit);
	if (
		quantifierEnd !== null &&
	...
	const firstSubject = coordinatedSubjectAt(
		text,
		cursor,
		limit,
		candidates,
	...
	const explicitIndexes =
		firstSubject.explicitIndex === undefined
			? []
			: [firstSubject.explicitIndex];
	cursor = firstSubject.end;
	while (cursor < limit) {
		const separatorEnd = stickyMatchEnd(
			COORDINATED_EXPLICIT_POSTPOSED_SEPARATOR,
			text,
			cursor,
			limit,
		);
		if (separatorEnd === null) break;
		const subject = coordinatedSubjectAt(
			text,
			separatorEnd,
			limit,
			candidates,
...
function hasCoordinatedExplicitPostposedTail(
	text: string,
	start: number,
	end: number,
): boolean {
	let cursor =
		stickyMatchEnd(POSTPOSED_SUBJECT_NOUN, text, start, end) ?? start;
	while (cursor < end) {
		const trailingStart = boundedHorizontalCursor(text, cursor, end);
		if (trailingStart === end) return true;
		if (
			/[,，،、]/u.test(text[trailingStart] ?? "") &&
		...
		const subjectEnd = stickyMatchEnd(
			COORDINATED_EXPLICIT_POSTPOSED_SUBJECT,
			text,
			separatorEnd,
			end,
		);
		...
		cursor =
			stickyMatchEnd(POSTPOSED_SUBJECT_NOUN, text, subjectEnd, end) ??
			subjectEnd;
	}
	return true;
}
...
function coordinatedPostposedSubjectGroupStartIndex(
	...
	while (
		...
		hasCoordinatedExplicitPostposedTail(
			text,
			candidates[groupStartIndex - 1].end,
			candidates[groupStartIndex].end,
		)
	) {
		groupStartIndex -= 1;
	}
...
function mixedImplicitPostposedSubjectStart(
	...
	const start =
		prefixNegations.postposedSubjectStarts.get(candidate.start) ??
		candidate.start;
	const chain = collectMixedSubjectChain(
		text,
		start,
		predicateStart,
		candidates,
...
function collectPostposedExplicitNegationIndex(
	text: string,
	candidates: readonly ExplicitCandidateScan[],
): PostposedExplicitNegationIndex {
	...
	while (firstCandidateIndex < candidates.length) {
		const start = lineStartForPosition(
			text,
			candidates[firstCandidateIndex].start,
		);
		...
		while (
			afterLineCandidateIndex < candidates.length &&
			candidates[afterLineCandidateIndex].start < end
		)
			afterLineCandidateIndex += 1;

		const predicateScanner = new RegExp(
			POSTPOSED_NEGATIVE_PREDICATE.source,
			"giu",
		);
		...
		while ((predicate = predicateScanner.exec(normalizedLine)) !== null) {
			const predicateTokenStart = firstGrammarTokenStart(
				normalizedLine,
				predicate.index,
				predicateScanner.lastIndex,
			);
			if (
				!hasUnicodeGrammarTokenBoundaries(
					normalizedLine,
				...
				!hasAsciiGrammarTokenCharacters(
					normalizedLine,
					predicateTokenStart,
					predicateScanner.lastIndex,
				)
			...
			while (
				lastCandidateIndex + 1 < afterLineCandidateIndex &&
				candidates[lastCandidateIndex + 1].end <= predicateStart
			) {
			...
			const negationEnd =
				positiveTransition?.start ??
				postposedNegationClauseEnd(text, predicateEnd, end);
...
function postposedExplicitNegationEnd(
	index: PostposedExplicitNegationIndex,
	candidate: ExplicitCandidateScan,
): number | null {
	return index.ends.get(candidate.start) ?? null;
}

function hasPostposedExplicitNegation(
	index: PostposedExplicitNegationIndex,
	candidate: ExplicitCandidateScan,
): boolean {
...
function isNegativeExplicitMention(
	candidate: ExplicitCandidateScan,
	postposedNegations: PostposedExplicitNegationIndex,
): boolean {
	return (
		hasPostposedExplicitNegation(postposedNegations, candidate) ||
		postposedNegations.prefixNegatedStarts.has(candidate.start)
	);
}

function isInertOrNegativeMention(
	candidate: ExplicitCandidateScan,
	postposedNegations: PostposedExplicitNegationIndex,
): boolean {
	return (
		[...candidate.reasons].some((reason) => reason !== "not-leading-region") ||
		isNegativeExplicitMention(candidate, postposedNegations)
	);
}
...
function listItemDocumentationTokenRange(
	text: string,
	candidateStart: number,
	blockEnd: number,
): ListDocumentationTokenRange | null {
	...
	const tokenStart =
		...
		directiveTarget < end &&
		text[directiveTarget] === "$"
			? directiveTarget
			: leading.cursor;
	const tokenSequence =
		/^(?:(?:\$(?:oh-my-codex:)?[A-Za-z][A-Za-z0-9_-]*)|(?:\/prompts:[\w.-]+))(?:(?:\s*,\s*(?:(?:and|or)\s+)?|\s+(?:and|or)\s+|\s*\/\s*)(?:(?:\$(?:oh-my-codex:)?[A-Za-z][A-Za-z0-9_-]*)|(?:\/prompts:[\w.-]+)))*/iu.exec(
			text.slice(tokenStart, end),
		);
	if (
		!tokenSequence ||
		!LIST_DOCUMENTATION_SUFFIX.test(
			text.slice(tokenStart + tokenSequence[0].length, end),
		)
	)
		return null;
...
function isListItemDocumentation(
	text: string,
	candidateStart: number,
	blockEnd: number,
): boolean {
	return (
		listItemDocumentationTokenRange(text, candidateStart, blockEnd) !== null
	);
}

function hasActivePromptsTokenBoundary(
	text: string,
	start: number,
	end: number,
): boolean {
	...
	const validPrevious =
		!previous ||
		SAFE_TOKEN_WHITESPACE.test(previous) ||
		/[(\[{]/u.test(previous);
...
function isMarkdownTableDelimiter(line: string): boolean {
	...
	return (
		cells.length >= 2 && cells.every((cell) => /^\s*:?-{3,}:?\s*$/u.test(cell))
	);
}
...
function collectMarkdownTableRanges(text: string): InertPromptRange[] {
	...
	while (start < text.length) {
		...
		const delimiterStart =
			headerEnd < text.length ? nextLineStart(text, headerEnd) : text.length;
		const delimiterEnd =
			delimiterStart < text.length
				? lineEnd(text, delimiterStart)
				: text.length;
		if (
			text.slice(start, headerEnd).includes("|") &&
			isMarkdownTableDelimiter(text.slice(delimiterStart, delimiterEnd))
		) {
			let tableEnd = delimiterEnd;
			let rowStart =
				delimiterEnd < text.length
					? nextLineStart(text, delimiterEnd)
					: text.length;
			while (rowStart < text.length) {
				...
				tableEnd = rowEnd;
				rowStart =
					rowEnd < text.length ? nextLineStart(text, rowEnd) : text.length;
			}
...
function markdownLabelClosingIndex(
	text: string,
	start: number,
	limit: number,
): number {
...
function referenceDestinationEnd(
	text: string,
	start: number,
	end: number,
): number | null {
...
function referenceTitleStartAfterDestination(
	text: string,
	start: number,
	end: number,
): number | null {
...
function collectMarkdownReferenceIndex(text: string): MarkdownReferenceIndex {
	...
	const codeIndex = createInertRangeIndex([
			...
			if (closing > labelStart) {
				...
				if (colon) {
					...
					let destinationEnd =
						...
						!isInInertRange(codeIndex, destinationStart)
							? referenceDestinationEnd(
									text,
									destinationStart,
									destinationLineEnd,
								)
					...
					if (
						...
						const indentation =
							/^[ \t]{0,3}/u.exec(
								text.slice(continuationStart, destinationLineEnd),
							)?.[0].length ?? 0;
						destinationStart = continuationStart + indentation;
						destinationEnd =
							destinationStart < destinationLineEnd &&
							!isInInertRange(codeIndex, destinationStart)
								? referenceDestinationEnd(
										text,
										destinationStart,
					...
					if (destinationEnd !== null) {
						...
						let titleStart = referenceTitleStartAfterDestination(
							text,
							destinationStart,
							destinationLineEnd,
						);
						if (titleStart === null && destinationLineEnd < text.length) {
							...
							const titleIndent = /^[ \t]{0,3}/u.exec(
								text.slice(titleLineStart, titleLineEnd),
							);
							const candidateStart =
								titleLineStart + (titleIndent?.[0].length ?? 0);
						...
						if (titleStart !== null) {
							...
							if (
								text[titleRange.end - 1] ===
								(text[titleStart] === "(" ? ")" : text[titleStart])
							)
						...
						labels.add(
							normalizeMarkdownReferenceLabel(
								text.slice(labelStart + 1, closing),
							),
						);
					}
				}
...
function hasMarkdownReferenceDefinition(
...
function balancedMarkdownLinkRange(
	text: string,
	labelStart: number,
	destinationStart: number,
	lineLimit: number,
): InertPromptRange | null {
	let depth = 1;
	let quote: '"' | "'" | null = null;
...
function enclosingMarkdownLinkRange(
	text: string,
	start: number,
	end: number,
): InertPromptRange | null {
	...
	const range = balancedMarkdownLinkRange(
		text,
		labelStart,
		destinationStart,
		lineLimit,
	);
	return range && start >= destinationStart + 2 && end <= range.end
		? range
		: null;
}

function markdownDocumentationRange(
	text: string,
	start: number,
	end: number,
	...
	if (labelStart >= lineStart) {
		...
		if (closingLabel >= end && closingLabel < lineLimit) {
			...
			if (
				!beforeLabel.trim() &&
				(/^\s*:\s*\S/u.test(afterLabel) || !afterLabel.trim())
			) {
				return { start: lineStart, end: lineLimit };
			}
...
function isMarkdownDocumentationMention(
	text: string,
	start: number,
	end: number,
	referenceIndex: MarkdownReferenceIndex,
...
function isInactivePromptsMention(
	...
	inertRangeIndexes: Readonly<
		Record<StructuralInertDiagnostic, InertRangeIndex>
	>,
	referenceIndex: MarkdownReferenceIndex,
): boolean {
	return (
		!hasActivePromptsTokenBoundary(text, start, end) ||
...
function hasDirectPromptsInvocation(
	...
	const pattern = new RegExp(
		PROMPTS_TOKEN_PATTERN.source,
		PROMPTS_TOKEN_PATTERN.flags,
	);
	...
	return (
		(!leading.listItem ||
			!isListItemDocumentation(text, leading.cursor, matchEnd)) &&
...
function boundedPredecessorRanges(
	text: string,
	referenceIndex: MarkdownReferenceIndex,
): InertPromptRange[] {
	...
	const unboundedStructuralRanges = structuralRanges.filter(
		(range) => !range.bounded,
	);
	const unboundedStructuralIndex = createInertRangeIndex(
		unboundedStructuralRanges,
	);
	const isOutsideUnboundedStructuralParent = (
		range: InertPromptRange,
	): boolean => {
		const parentEnd = inertRangeEndAt(unboundedStructuralIndex, range.start);
		return parentEnd === null || range.end > parentEnd;
	};
	const ranges: InertPromptRange[] = [
		...referenceIndex.predecessorRanges.filter(
			isOutsideUnboundedStructuralParent,
		),
		...referenceIndex.tablePredecessorRanges.filter(
			isOutsideUnboundedStructuralParent,
		),
		...structuralRanges
			.filter(
				(range) => range.bounded && isOutsideUnboundedStructuralParent(range),
			)
	...
	while ((match = pattern.exec(text)) !== null) {
		...
		const inactive = isInactivePromptsMention(
			text,
			start,
			end,
			inertRangeIndexes,
		...
		if (
			!inactive &&
			directiveCommandTargetStart(text, leading.cursor) !== start
		)
			continue;
		const markdownRange = inactive
			? markdownDocumentationRange(text, start, end, referenceIndex)
			: null;
		const referenceEnd = inactive
			? inertRangeEndAt(referenceIndex.destinationRanges, start)
			: null;
		ranges.push({
			start: markdownRange?.start ?? start,
			end:
				inactive && isListItemDocumentation(text, start, end)
					? lineEnd(text, end)
					: Math.max(markdownRange?.end ?? end, referenceEnd ?? end),
		});
	}
	return ranges.sort(
...
function scanExplicitCandidates(text: string): ExplicitCandidateScan[] {
	...
	while ((dollar = dollarScanner.exec(text)) !== null) {
		...
		const canonicalEnd =
			canonicalMatch?.index === start ? start + canonicalMatch[0].length : null;
		const firstCharacter = codePointAt(text, start + 1);
		if (
			canonicalEnd === null &&
			(!firstCharacter ||
				SAFE_TOKEN_WHITESPACE.test(firstCharacter) ||
				isExplicitTokenBoundary(text, start + 1))
		)
			continue;
		...
		const canonicalToken =
			canonicalEnd === end ? (canonicalMatch?.[1] ?? "").toLowerCase() : "";
		const normalizedToken =
			canonicalToken ||
			rawKeyword
				.replace(/^\$(?:(?:[Oo][Hh]-[Mm][Yy]-[Cc][Oo][Dd][Ee][Xx]):)?/u, "")
				.toLowerCase();
		const definition = canonicalToken
			? getExplicitSkillDefinition(canonicalToken)
			: undefined;
		candidates.push({
			rawKeyword,
			normalizedToken,
...
function boundedHorizontalCursor(
	text: string,
	start: number,
	limit: number,
): number {
...
function documentationFollowupAt(
	...
	const followup = chain
		? {
				...
				reopenable:
					chain.explicitIndexes.length > 0
						? !hasExplicitDocumentationSuffixAt(text, chain.end, lineLimit)
						: !hasImplicitDocumentationSubjectPrefix(
								text.slice(chain.start, lineLimit),
							),
			}
		: null;
...
function isDocumentationClauseSeparatorAt(
	text: string,
	index: number,
): boolean {
	const character = codePointAt(text, index);
...
function isStrongDocumentationClauseSeparatorAt(
	text: string,
	index: number,
): boolean {
...
function explicitDocumentationClauseEnd(
	...
	return (
		documentationDirectiveTransitionStart(text, start, lineLimit) ?? lineLimit
	);
}
...
function hasStrongDocumentationClauseBoundary(
	text: string,
	start: number,
	end: number,
): boolean {
...
function isCompactSlashDocumentationCandidate(
	candidate: ExplicitCandidateScan,
): boolean {
	...
	return parts.every((part) => {
		const match =
			/^\$(?:(?:[Oo][Hh]-[Mm][Yy]-[Cc][Oo][Dd][Ee][Xx]):)?([A-Za-z][A-Za-z0-9_-]*)$/u.exec(
				part,
			);
		return (
			match !== null &&
			getExplicitSkillDefinition((match[1] ?? "").toLowerCase()) !== undefined
		);
	});
}
...
function documentationSubjectChainAtClauseStart(
	...
	const commandTarget = directiveCommandTargetStart(
		text,
		contentStart,
		lineLimit,
	);
	return collectMixedSubjectChain(
		text,
		commandTarget ?? contentStart,
		lineLimit,
		candidates,
		candidateIndexByStart,
	);
...
function independentDocumentaryClauseStart(
	...
	const directiveTarget = directiveCommandTargetStart(
		text,
		contentStart,
		lineLimit,
	);
	if (
		directiveTarget === null ||
		directiveTarget !== chain.start ||
		candidateIndexByStart.has(directiveTarget)
	)
		return null;
	...
	return coordinator
		? boundedHorizontalCursor(
				text,
				chain.start + coordinator.index + coordinator[0].length,
				lineLimit,
			)
		: null;
}
...
function collectImplicitDocumentationLineRanges(
	...
	while (lineStart < text.length) {
		...
		while (clauseStart < lineLimit) {
			const clauseLimit = documentationSuffixLimit(
				text,
				clauseStart,
				lineLimit,
			);
			const clauseText = text.slice(clauseStart, clauseLimit);
			let subjectChain = documentationSubjectChainAtClauseStart(
				text,
				clauseStart,
				clauseLimit,
				candidates,
			...
			const directDocumentation = Boolean(
				subjectChain &&
					(subjectChain.explicitIndexes.length > 0
						? hasExplicitDocumentationSuffixAt(
								text,
								subjectChain.end,
								clauseLimit,
							)
						: hasImplicitDocumentationSubjectPrefix(
								text.slice(subjectChain.start, clauseLimit),
							)),
			);
			const leadingDocumentation =
				DOCUMENTATION_LEADING_PATTERN.test(clauseText);
			if (directDocumentation || leadingDocumentation) {
				...
				const rangeStart = subjectChain
					? (independentDocumentaryClauseStart(
							text,
							clauseStart,
							subjectChain,
							lineLimit,
				...
				const rangeEnd = subjectChain
					? explicitDocumentationClauseEnd(
							text,
							subjectChain.end,
							lineLimit,
							candidates,
...
function directCandidateIndexes(
	...
	for (const [index, candidate] of candidates.entries())
		candidateIndexByStart.set(candidate.start, index);
	const embeddedPathOrUrlIndex = collectEmbeddedPathOrUrlMentionIndex(
		text,
		candidates,
	);

	const indexes = new Set<number>();
	const implicitDocumentationRanges = collectImplicitDocumentationLineRanges(
		text,
		candidates,
		candidateIndexByStart,
	);
	const documentationRanges: InertPromptRange[] = [
		...implicitDocumentationRanges,
	];
	...
	let candidateListDocumentationRange: ListDocumentationTokenRange | null =
		null;
	let candidateLineMayContainMarkdownDocumentation = false;
	const ensureCandidateDocumentationLine = (
		candidate: ExplicitCandidateScan,
	): void => {
		if (
			candidate.start >= candidateDocumentationLineStart &&
			candidate.start < candidateDocumentationLineEnd
		)
			return;
		candidateDocumentationLineStart = lineStartForPosition(
			text,
			candidate.start,
		);
		candidateDocumentationLineEnd = lineEnd(text, candidate.end);
		candidateListDocumentationRange = listItemDocumentationTokenRange(
			text,
			candidate.start,
			candidate.end,
		);
		const line = text.slice(
			candidateDocumentationLineStart,
			candidateDocumentationLineEnd,
		);
		const followingStart =
			candidateDocumentationLineEnd < text.length
				? nextLineStart(text, candidateDocumentationLineEnd)
				: text.length;
		const followingLine =
			followingStart < text.length
				? text.slice(followingStart, lineEnd(text, followingStart))
				: "";
		candidateLineMayContainMarkdownDocumentation =
			/[\[\]|]/u.test(line) ||
			/^\s*#{1,6}\s/u.test(line) ||
			/^\s*(?:={3,}|-{3,})\s*$/u.test(followingLine) ||
			(line.includes("|") && isMarkdownTableDelimiter(followingLine));
	};
	const cachedCandidateDocumentationRange = (
		candidate: ExplicitCandidateScan,
	): InertPromptRange | null => {
		ensureCandidateDocumentationLine(candidate);
		const destinationEnd = inertRangeEndAt(
			referenceIndex.destinationRanges,
			candidate.start,
		);
		if (destinationEnd !== null)
			return { start: candidate.start, end: destinationEnd };
		if (
			candidateListDocumentationRange &&
		...
		if (candidateLineMayContainMarkdownDocumentation) {
			const markdownRange = markdownDocumentationRange(
				text,
				candidate.start,
				candidate.end,
				referenceIndex,
		...
		return isEmbeddedPathOrUrlMention(embeddedPathOrUrlIndex, candidate.start)
	...
	const addBlock = (start: number): AddedDirectBlock => {
		...
		if (
			candidateIndex === undefined ||
			candidates[candidateIndex].reasons.size > 0 ||
			isNegativeExplicitMention(candidates[candidateIndex], postposedNegations)
		)
			return "none";
		const added: number[] = [];
		...
		while (
			...
			if (
				added.length > 0 &&
				hasStrongDocumentationClauseBoundary(text, blockEnd, candidate.start)
			) {
			...
			const candidateDocumentationRange =
				cachedCandidateDocumentationRange(candidate);
			...
			if (
				added.length > 0 &&
				candidate.skill &&
				!checkedPostposedSkills.has(candidate.skill) &&
				hasPostposedExplicitNegation(postposedNegations, candidate)
			)
				break;
		...
		const subjectChain = collectMixedSubjectChain(
			...
			start: candidates[added[0]].start,
			explicitIndexes: [],
			end: blockEnd,
		};
		const addedSet = new Set(added);
		const documentationAdded = [
			...new Set([...added, ...subjectChain.explicitIndexes]),
		];
		const coordinatedDocumentationAdded = subjectChain.explicitIndexes.filter(
			(index) => !addedSet.has(index),
		);
		let documentationBlockEnd = Math.max(blockEnd, subjectChain.end);
		let coordinatedStart = coordinatedDocumentationCandidateStart(
			text,
		...
		while (coordinatedStart !== null) {
			...
			if (
				coordinatedIndex === undefined ||
				candidates[coordinatedIndex].reasons.size > 0
			)
				break;
			...
			coordinatedStart = coordinatedDocumentationCandidateStart(
				text,
				documentationBlockEnd,
			);
		}
		...
		const documentationSuffix =
			documentationAdded.some(
				(index) =>
					candidates[index].skill !== null ||
					isCompactSlashDocumentationCandidate(candidates[index]),
			) &&
			hasExplicitDocumentationSuffixAt(
				text,
				documentationBlockEnd,
				documentationSuffixEnd,
			);
		...
		if (documentationSuffix) {
			...
			for (const index of coordinatedDocumentationAdded) {
				...
				documentationRange = documentationRange
					? {
							start: Math.min(documentationRange.start, markdownRange.start),
							end: Math.max(documentationRange.end, markdownRange.end),
						}
					: markdownRange;
			}
		}
	...
	for (const [candidateIndex, candidate] of candidates.entries()) {
		ensureCandidateDocumentationLine(candidate);
		while (
			inertPromptCursor < inertPromptRanges.length &&
			inertPromptRanges[inertPromptCursor].start < candidate.start
		) {
			latestInertEnd = Math.max(
				latestInertEnd,
				inertPromptRanges[inertPromptCursor].end,
			);
			inertPromptCursor += 1;
		}
		while (
			implicitDocumentationCursor < implicitDocumentationRanges.length &&
			implicitDocumentationRanges[implicitDocumentationCursor].start <
				candidate.start
		) {
			latestInertEnd = Math.max(
				latestInertEnd,
				implicitDocumentationRanges[implicitDocumentationCursor].end,
			);
			latestDocumentationEnd = Math.max(
				latestDocumentationEnd,
				implicitDocumentationRanges[implicitDocumentationCursor].end,
			);
		...
		if (isInertOrNegativeMention(candidate, postposedNegations)) {
			const structuralEnd = structuralInertRangeEnd(
				inertRangeIndexes,
				candidate.start,
			);
			let predecessorEnd = structuralEnd ?? candidate.end;
			predecessorEnd = Math.max(
				predecessorEnd,
				postposedExplicitNegationEnd(postposedNegations, candidate) ?? -1,
			);
			if (
				candidateLineMayContainMarkdownDocumentation &&
				text.lastIndexOf("[", candidate.start) >=
					candidateDocumentationLineStart &&
				text.indexOf("]", candidate.end) < candidateDocumentationLineEnd
			) {
				predecessorEnd = Math.max(
					predecessorEnd,
					markdownDocumentationRange(
						text,
						candidate.start,
						candidate.end,
						referenceIndex,
		...
		if (
			...
			const lineLeading =
				leadingDirectiveCursor(text, candidateDocumentationLineStart, false)
					.cursor === candidate.start;
			const documentationSeparated =
				latestDocumentationEnd >= 0 &&
				hasAdjacentPredecessorSeparator(
					text,
					latestDocumentationEnd,
					candidate.start,
				);
			if (
				documentationSeparated ||
				(lineLeading &&
					hasAdjacentPredecessorSeparator(
						text,
						latestInertEnd,
				...
				clauseDirectiveStart(text, latestInertEnd, candidate.start)
			) {
...
function freezeCandidates(
	candidates: ExplicitCandidateScan[],
): readonly ExplicitSkillCandidate[] {
		...
		candidates.map((candidate) =>
			Object.freeze({
				...
				reasons: Object.freeze(
					KEYWORD_INERT_DIAGNOSTIC_ORDER.filter((reason) =>
						candidate.reasons.has(reason),
					),
				),
			}),
		),
...
function implicitLineIndexAt(
	index: ImplicitClauseIndex,
	position: number,
): number {
	return lineIndexAt(index.lineStarts, position);
}

function indexedBoundaryAtOrAfter(
...
function positiveContrastStartInRange(
...
function collectImplicitClauseIndex(
	text: string,
	documentationBlocks: readonly InertPromptRange[],
): ImplicitClauseIndex {
	...
	while ((prefix = prefixScanner.exec(normalizedText)) !== null) {
		...
		const positiveContrast = positiveContrastStartInRange(
			positiveContrasts,
			prefixScanner.lastIndex,
			clauseEnd,
		);
		const exclusionTransition = exclusionPrefixDirectiveTransitionStart(
			text,
			prefix.index,
			prefixScanner.lastIndex,
			clauseEnd,
		...
		const end = Math.min(
			positiveContrast ?? clauseEnd,
			exclusionTransition ?? clauseEnd,
		);
	...
	while ((predicate = predicateScanner.exec(normalizedText)) !== null) {
		...
		const subjectStart =
			subjectStarts[
				Math.max(0, upperBound(subjectStarts, predicate.index) - 1)
			] ?? 0;
		...
		if (
			hasAsciiDirectiveCommand(subject) ||
			!IMPLICIT_POSTPOSED_SUBJECT_CHAIN.test(subject)
		)
			continue;
		const positiveContrast = positiveContrastStartInRange(
			positiveContrasts,
		...
		negativeBlocks.push({
			start: subjectStart,
			end: positiveContrast ?? clauseEnd,
		});
	}
...
function hasImplicitDocumentationFrame(text: string): boolean {
	return /(?:^|[\s([{])(?:docs?|documentation|examples?|references?|guide|manual|document(?:s|ed|ing)?|describ(?:e|es|ed|ing)|mention(?:s|ed|ing)|explain(?:s|ed|ing)?)(?:\b|\s*:)/iu.test(
		text,
	);
}
function hasIntroductoryDocumentationFrame(text: string): boolean {
	return /^\s*(?:for\s+(?:example|instance|reference)|as\s+an?\s+example|according\s+to\s+(?:the\s+)?(?:docs?|documentation|guide|manual|reference)|(?:in|per)\s+(?:the\s+)?(?:docs?|documentation|guide|manual|reference))\s*(?:[,，،、:：]|[—–-])\s*$/iu.test(
		text,
	);
}
function implicitClauseRange(
	...
	let start =
		clauses.subjectStarts[
			Math.max(0, upperBound(clauses.subjectStarts, candidateStart) - 1)
		] ?? lineStart;
	let frameStart = start;
	const introductoryPrefix =
		/^[^\r\n,:，：،、—–-]*(?:[,，،、:：]|[—–-])\s*/u.exec(
			text.slice(lineStart, candidateStart),
		)?.[0];
	...
	if (contrastStart >= 0) {
		...
		if (
			!hasGoverningIntroductoryFrame &&
			discardMatch &&
			hasUnicodeGrammarTokenBoundaries(
				text,
				start + discardMatch.index,
				start + discardMatch.index + discardMatch[0].length,
			)
		) {
	...
	const directiveTransition = new RegExp(
		`(?:${ASCII_AND}|${ASCII_THEN})\\s+(?=(?:${ASCII_PLEASE}\\s+)?(?:${ASCII_INSTEAD}\\s+)?${ASCII_DIRECTIVE_COMMAND})[^\\r\\n\\u2028\\u2029]*$`,
		"u",
	).exec(directivePrefix);
	if (
		directiveTransition &&
		hasUnicodeGrammarTokenBoundaries(
			directivePrefix,
			directiveTransition.index,
			directiveTransition.index + directiveTransition[0].length,
		)
	) {
		const transitionStart = start + directiveTransition.index;
		const hasNegativePrefix = hasUnicodeBoundedNegativePrefix(
			normalizeGrammarApostrophes(text.slice(frameStart, transitionStart)),
		);
		...
		if (
			!hasGoverningIntroductoryFrame &&
			(!hasNegativePrefix || exclusionTransition)
		) {
			frameStart = transitionStart;
			start = transitionStart;
		}
	...
	const end = Math.min(
		indexedBoundaryAtOrAfter(clauses.clauseEnds, candidateEnd, logicalLineEnd),
		logicalLineEnd,
	);
	return {
		start,
		...
		governingDocumentationFrame: hasGoverningIntroductoryFrame,
	};
}

function isImplicitListDocumentation(
	text: string,
	candidateStart: number,
): boolean {
	...
	return /(?:\s*(?::|：|[—–])\s*\S|\b(?:is|are|means|refer(?:s)?\s+to|denote(?:s)?)\b).*\b(?:commands?|workflows?|skills?|modes?|files?|documentation)\b/iu.test(
		content,
	);
}
...
function isImplicitDocumentationClause(
	...
	if (
		/^(?:(?:\s+(?:mode|workflow|skill|loop))?\s+(?:(?:is|are|means|refer(?:s)?\s+to|denote(?:s)?)\b.*\b(?:commands?|workflows?|skills?|modes?|files?|documentation)\b|(?:is|are)\s+(?:(?:(?:also|still)\s+)?(?:(?:its|an?|the)\s+)?alias(?:es)?|documented|described)\b|(?:appears?|occurs?|is\s+(?:also\s+)?mentioned)\b.*\b(?:docs?|documentation|examples?|references?|guide|manual)\b|(?:docs?|documentation|examples?|references?|guide|manual)\b)|(?:\s+(?:mode|workflow|skill|loop))?\s*\/\s*.*\b(?:is|are|means|refer(?:s)?\s+to|denote(?:s)?)\b.*\b(?:commands?|workflows?|skills?|modes?|files?|documentation)\b|(?:\s+(?:mode|workflow|skill|loop))?\s*(?::|[—–-])\s*\S.*\b(?:commands?|workflows?|skills?|modes?|files?|documentation)\b)/iu.test(
			suffix,
		)
	)
		return true;
	...
	if (
		/^(?:\s+(?:mode|workflow|skill|loop))?\s*[—–-]\s*\S.*\b(?:commands?|workflows?|skills?|modes?|files?|documentation)\b/iu.test(
			line.slice(localEnd),
		)
	)
		return true;
	if (
		isMarkdownDocumentationMention(text, matchStart, matchEnd, referenceIndex)
	)
		return true;
	return false;
}
function hasPostposedImplicitNegation(suffix: string, prefix: string): boolean {
	...
	const independentCoordinatedClause =
		/^\s+(?:modes?|workflows?|skills?|loops?)?\s*[,，،、]\s*(?:and|or|nor)\b/iu.test(
			subjectTail,
...
function collectEmbeddedPathOrUrlMentionIndex(
	...
	for (let cursor = 0; cursor < text.length; ) {
		while (
			...
			candidates[candidateIndex].start === cursor
		) {
...
function isEmbeddedPathOrUrlMention(
	index: EmbeddedPathOrUrlMentionIndex,
	start: number,
): boolean {
	return (
		index.tokenStarts.has(start) && index.embeddedCandidateStarts.has(start)
	);
}
...
function hasImplicitKeywordBoundaries(
	text: string,
	start: number,
	end: number,
): boolean {
	...
	const next = codePointAt(text, end);
	return (
		(!previous || (previous !== "$" && !TOKEN_CONTINUATION.test(previous))) &&
		(!next || !TOKEN_CONTINUATION.test(next))
	);
}
...
function isActiveImplicitMatch(
	...
	if (
		isStructurallyInert(inertRangeIndexes, matchStart) ||
		isInInertRange(referenceIndex.destinationRanges, matchStart) ||
		isInInertRange(clauses.documentationBlocks, matchStart) ||
		(!isNegativePrefixExemptImplicitKeyword(matchedKeyword) &&
			isInInertRange(clauses.negativeBlocks, matchStart)) ||
	...
	const lineStart =
		clauses.lineStarts[implicitLineIndexAt(clauses, matchStart)] ?? 0;
	const prefix = text.slice(clause.start, matchStart);
	const effectivePrefix =
		prefix.trim() ||
		text.slice(lineStart, Math.max(lineStart, clause.start - 1));
	if (
		!isNegativePrefixExemptImplicitKeyword(matchedKeyword) &&
		hasUnicodeBoundedNegativePrefix(
			normalizeGrammarApostrophes(effectivePrefix),
		)
	)
		return null;
	const suffix = text.slice(matchEnd, clause.end);
	const lineEnd =
		clauses.lineEnds[implicitLineIndexAt(clauses, matchStart)] ?? text.length;
	const lineSuffix = text.slice(matchEnd, lineEnd);
	if (hasPostposedImplicitNegation(lineSuffix, effectivePrefix)) return null;
	if (
		/^(?:\s+(?:mode|workflow|skill|loop))?(?:는|은|이|가)?\s*(?:사용하지\s*마세요|사용하지\s*마|쓰지\s*마세요|쓰지\s*마)/u.test(
			suffix,
		)
	)
		return null;
	if (
		hasActivePromptsInvocation(
			text,
			clause.start,
			clause.end,
			inertRangeIndexes,
...
function detectImplicitKeywords(
	...
	const explicitCandidateRanges = createInertRangeIndex(
	...
	for (const { keyword, pattern, skill, priority } of KEYWORD_MAP) {
		...
		while ((match = scanner.exec(normalizedText)) !== null) {
			...
			if (
				(inertRangeEndAt(explicitCandidateRanges, matchStart) ?? -1) >= matchEnd
			)
				continue;
			if (
				match[0].toLowerCase() !== "ulw" &&
				!hasImplicitKeywordBoundaries(normalizedText, matchStart, matchEnd)
			)
				continue;
			const clause = isActiveImplicitMatch(
				normalizedText,
				matchStart,
				matchEnd,
				match[0],
...
function hasIntentContextForKeyword(text: string, keyword: string): boolean {
	...
	if (k === "deep interview" || k === "interview") {
		...
		return DEEP_INTERVIEW_ACTIVATION_PATTERNS.some((pattern) =>
			pattern.test(text),
		);
	}
...
export function classifyKeywordInput(text: string): KeywordInputClassification {
	...
	const postposedNegations = collectPostposedExplicitNegationIndex(
		normalizedText,
		candidates,
	);
	const { directIndexes, documentationRanges, documentationBlocks } =
		directCandidateIndexes(
			normalizedText,
			candidates,
			referenceIndex,
			postposedNegations,
		);
	const implicitClauses = collectImplicitClauseIndex(
		normalizedText,
		documentationBlocks,
	);
	for (const [index, candidate] of candidates.entries()) {
	...
	for (const index of directIndexes) {
		...
		if (
			...
			explicitSkills.has(candidate.skill)
		)
			continue;
	...
	const directPromptsInvocation = hasDirectPromptsInvocation(
		normalizedText,
		collectInertRangeIndexes(normalizedText),
		referenceIndex,
	);
	...
	const hasActiveExplicitLike = hasActiveExplicitLikeInvocation(
		candidates,
		documentationRanges,
		postposedNegations,
	);
	...
	const implicitMatches =
		reservedInput || hasActiveExplicitLike ? [] : finalMatches;
...
function filterMatchesForTeamMode(
	matches: readonly KeywordMatch[],
	teamEnabled: boolean,
): KeywordMatch[] {
	return teamEnabled
		? [...matches]
		: matches.filter((entry) => entry.skill !== "team");
}
...
function detectPrimaryKeywordForTeamMode(
	...
	return (
		filterMatchesForTeamMode(classification.matches, teamEnabled)[0] ?? null
	);
}
...
function isActiveSkillContinuationPrompt(text: string): boolean {
	...
	return ACTIVE_SKILL_CONTINUATION_PATTERNS.some((pattern) =>
		pattern.test(normalized),
	);
}

function isNamedActiveSkillContinuationPrompt(
	text: string,
	skill: string,
): boolean {
...
function shouldReusePreviousSkillForContinuation(
	...
	if (
		...
		!isTrackedWorkflowMode(previousSkill)
	) {
		return false;
	}

	return (
		isActiveSkillContinuationPrompt(text) ||
...
async function resolveGatedSupervisedChildPhase(
	...
	const heldPhase =
		safeString(existing.current_phase).trim() || requestedChildSkill;
	...
	if (
		currentChildPhase === "deep-interview" &&
		requestedChildSkill === "ralplan"
	) {
...
function isForwardChildPhaseSkip(
	currentChildPhase: string,
	requestedChildSkill: string,
): boolean {
	const currentIndex = (AUTOPILOT_CHILD_PHASES as readonly string[]).indexOf(
		currentChildPhase,
	);
	const requestedIndex = (AUTOPILOT_CHILD_PHASES as readonly string[]).indexOf(
		requestedChildSkill,
	);
...
async function resolveAutopilotSupervisedChildPhaseState(
	...
	if (existing && existingMode !== "autopilot") {
		throw new Error(
			`Cannot advance supervised Autopilot child phase: expected autopilot detail state, found ${existingMode || "unknown"}`,
		);
	}
		...
		JSON.stringify(
			withModeRuntimeContext(
				existing ?? {},
				{
					...
					updated_at: nowIso,
					session_id:
						(sessionId ?? safeString(existing?.session_id).trim()) || undefined,
					thread_id:
						(options.threadId ?? safeString(existing?.thread_id).trim()) ||
						undefined,
					turn_id:
						(options.turnId ?? safeString(existing?.turn_id).trim()) ||
						undefined,
				},
				{ nowIso },
			),
...
async function reconcileAutopilotSupervisedChildModeStates(
	...
	if (!isTrackedWorkflowMode(childSkill)) {
		const effectivePhase = await persistAutopilotSupervisedChildPhaseState(
			cwd,
			stateDir,
			sessionId,
			childSkill,
	...
	const effectivePhase = await resolveAutopilotSupervisedChildPhaseState(
		cwd,
		stateDir,
		sessionId,
		childSkill,
	...
	for (const mode of AUTOPILOT_SUPERVISED_TRACKED_CHILD_SKILLS) {
		const candidatePaths = [
			resolveSeedStateFilePath(stateDir, mode as StatefulSkillMode, sessionId)
				.absolutePath,
		];
		for (const candidatePath of candidatePaths) {
			...
			if (
				...
				safeString(existing.mode).trim() !== mode
			)
				continue;
...
function resolveContinuationKeywordMatch(
	...
	const markedQuestionAnswerContinuation =
		...
		(previousSkill === "autopilot" || previousSkill === "deep-interview");
	if (
		classification.reservedInput ||
		(!markedQuestionAnswerContinuation &&
	...
	if (
		!shouldReusePreviousSkillForContinuation(text, previous, classification) &&
		!safeString(fallbackMatch?.keyword).trim().startsWith("$")
	) {
		return fallbackMatch;
	}
...
function initialWorkflowPhaseForMode(
...
function resolveRequestedWorkflowSkills(
	requestedWorkflowSkills: TrackedWorkflowMode[],
): {
	requestedSkills: TrackedWorkflowMode[];
	deferredSkills: TrackedWorkflowMode[];
} {
	const firstPlanningSkill = requestedWorkflowSkills.find((skill) =>
		PLANNING_LIKE_WORKFLOW_SKILLS.has(skill),
	);
	const hasExecutionSkill = requestedWorkflowSkills.some((skill) =>
		EXECUTION_LIKE_WORKFLOW_SKILLS.has(skill),
	);
	...
	return {
		...
		deferredSkills: requestedWorkflowSkills.filter(
			(skill) => skill !== firstPlanningSkill,
		),
	};
}
...
export async function recordSkillActivation(
	...
	const classification =
		input.classification ?? classifyKeywordInput(input.text);
	if (classification.originalText !== input.text) {
		throw new Error(
			"Keyword input classification text does not match activation text",
		);
	}
	...
	if (
		...
		resolvedPromptTurnContext.status !== "authorized"
	)
		return null;
	if (
		resolvedPromptTurnContext &&
		input.sessionId !== resolvedPromptTurnContext.authorization.targetSessionId
	)
		return null;
	const suppressRootMutation =
		resolvedPromptTurnContext?.authorization.globalSideEffects === "suppress";
	const provenanceOwnerCodexSessionId =
		resolvedPromptTurnContext?.authorization.ownerCodexSessionId;
	const applyProvenanceOwner = (state: SkillActiveState): SkillActiveState =>
		provenanceOwnerCodexSessionId
			? {
					...
					active_skills: state.active_skills?.map((entry) => ({
						...entry,
						owner_codex_session_id: provenanceOwnerCodexSessionId,
					})),
				}
	...
	const previousRoot = suppressRootMutation
		? null
		: await readExistingSkillState(rootStatePath);
	const previousSession = sessionStatePath
		? await readExistingSkillState(sessionStatePath)
		: null;
	...
	const hadDeepInterviewLock =
		previous?.skill === "deep-interview" &&
		previous?.input_lock?.active === true;
	...
	if (hasCancelIntent && hadDeepInterviewLock) {
		const state: SkillActiveState = {
			...
			...(previous?.input_lock
				? {
						input_lock: releaseDeepInterviewInputLock(
							previous.input_lock,
							nowIso,
							"abort",
						),
		...
		try {
			await writeSkillActiveStateCopiesForStateDir(
				...
				input.sessionId,
				selectRootSkillStateCopy(
					previousRoot,
					state,
					input.sessionId,
					suppressRootMutation,
				),
			);
			...
			console.warn(
				"[omx] warning: failed to persist keyword activation state",
				error,
			);
		}
	...
	const sameSkillContinuation =
		sameSkill &&
		shouldReusePreviousSkillForContinuation(
			input.text,
			previous,
			classification,
		);

	const matchedSeedConfig =
		STATEFUL_SKILL_SEED_CONFIG[match.skill as StatefulSkillMode];
	const matchedModeState = matchedSeedConfig
		? await readJsonStateIfExists(
	...
	const matchedModeTerminal = matchedSeedConfig
		? isResettableTerminalModeState(
				matchedModeState as Record<string, unknown> | null,
				matchedSeedConfig.mode,
			)
		: false;
	if (
		...
		const phase =
			safeString(matchedModeState.current_phase).trim() || "deep-interview";
	...
	if (
		...
		return null;
	const preserveActivatedAt =
		sameSkill && !matchedModeTerminal && (sameKeyword || sameSkillContinuation);
	const previousEntries = listActiveSkills(previous ?? {});
	const previousWorkflowEntries = previousEntries.filter(
		(entry) =>
			isTrackedWorkflowMode(entry.skill) &&
			(input.allowSecondaryAutopilot !== false ||
				entry.skill !== "autopilot" ||
				entry.skill === match.skill) &&
			(!input.sessionId ||
	...
	const markedQuestionAnswerContinuation =
		...
		classification.reservedInput === "omx-question-answered";
	const workflowMatches: TrackedWorkflowMode[] =
		isTrackedWorkflowMatch && !markedQuestionAnswerContinuation
			? classification.explicitMatches
					.map((entry) => entry.skill)
					.filter((skill) => teamMode.enabled || skill !== "team")
					.filter(
						(skill) =>
							input.allowSecondaryTeam !== false ||
							skill !== "team" ||
							skill === trackedMatchSkill,
					)
					.filter(
						(skill) =>
							input.allowSecondaryAutopilot !== false ||
	...
	const resolvedWorkflowRequest = isTrackedWorkflowMatch
		? resolveRequestedWorkflowSkills(
				workflowMatches.length > 0
					? workflowMatches
					: [trackedMatchSkill as TrackedWorkflowMode],
			)
		: null;
	const requestedWorkflowSkills =
		resolvedWorkflowRequest?.requestedSkills ?? [];
	const deferredSkills = resolvedWorkflowRequest?.deferredSkills ?? [];
	const willActivateDeepInterview =
		match.skill === "deep-interview" ||
	...
	const reusableDeepInterviewConfig =
		sameSkillContinuation &&
		isDeepInterviewRuntimeConfig(previous?.deep_interview_config)
			? previous.deep_interview_config
			: null;
	const deepInterviewConfig = willActivateDeepInterview
		? (reusableDeepInterviewConfig ??
	...
	if (
		input.allowSecondaryAutopilot !== false &&
		previous?.active === true &&
		previous.skill === "autopilot" &&
		isAutopilotSupervisedChildSkill(match.skill)
	) {
		try {
	...
	if (isTrackedWorkflowMatch) {
		let nextWorkflowEntries = previousWorkflowEntries.map((entry) => ({
			...entry,
		}));
		...
		for (const requestedMode of requestedWorkflowSkills) {
			...
			const hasStandaloneRalplanPreflightDenial =
				freshAutopilot &&
				previous?.active === true &&
			...
			if (
				...
				try {
					await persistBlockedFreshAutopilotState(
						input.stateDir,
						input.sessionId,
						nowIso,
					);
			...
			if (decision.autoCompleteModes.length > 0) {
				...
				try {
					...
					return {
						...
						active_skills: previousEntries,
						...(previous?.input_lock
							? { input_lock: previous.input_lock }
							: {}),
						transition_error:
							error instanceof Error ? error.message : String(error),
					};
			...
			nextWorkflowEntries = nextWorkflowEntries.filter(
				(entry) =>
					isTrackedWorkflowMode(entry.skill) &&
					survivingSkills.has(entry.skill),
			);

			const existingEntry = nextWorkflowEntries.find(
				(entry) => entry.skill === requestedMode,
			);
			if (existingEntry) {
				existingEntry.phase =
					requestedMode === match.skill && (!sameSkill || matchedModeTerminal)
						? initialWorkflowPhaseForMode(requestedMode)
						: existingEntry.phase;
				existingEntry.active = true;
				existingEntry.activated_at =
					requestedMode === match.skill
						? preserveActivatedAt
							? existingEntry.activated_at || previous?.activated_at || nowIso
							: nowIso
						: existingEntry.activated_at;
			...
			nextWorkflowEntries = [
				...nextWorkflowEntries,
				{
					...
					active: true,
					activated_at:
						requestedMode === match.skill && preserveActivatedAt
		...
		const primaryEntry =
			nextWorkflowEntries.find((entry) => entry.skill === match.skill) ??
			nextWorkflowEntries[0];
		const primarySkill = (primaryEntry?.skill ||
			match.skill) as TrackedWorkflowMode;
		const workflowState: SkillActiveState = {
			version: 1,
			active: true,
			skill: primarySkill,
			keyword:
				primarySkill === match.skill ? match.keyword : `$${primarySkill}`,
			...
			...(transitionMessages[0]
				? { transition_message: transitionMessages[0] }
				: {}),
			...(transitionMessages.length > 0
				? { transition_messages: [...new Set(transitionMessages)] }
				: {}),
			...(requestedWorkflowSkills.length > 1
				? { requested_skills: requestedWorkflowSkills }
				: {}),
			...(deferredSkills.length > 0 ? { deferred_skills: deferredSkills } : {}),
			...(deepInterviewInputLock ? { input_lock: deepInterviewInputLock } : {}),
			...(primarySkill === "deep-interview" && deepInterviewConfig
			...
			for (const requestedEntry of nextWorkflowEntries) {
				const seeded = await persistStatefulSkillSeedState(
					input.stateDir,
					{
						...
						keyword:
							requestedEntry.skill === workflowState.skill
								? workflowState.keyword
								: `$${requestedEntry.skill}`,
						phase: requestedEntry.phase || workflowState.phase,
						activated_at:
							requestedEntry.activated_at || workflowState.activated_at,
						updated_at: requestedEntry.updated_at || workflowState.updated_at,
						...(requestedEntry.skill === "deep-interview" && deepInterviewConfig
							? { deep_interview_config: deepInterviewConfig }
							: {}),
					},
					nowIso,
					...
					{
						activeContinuation:
							requestedEntry.skill === "autopilot" && sameSkillContinuation,
	...
	const state: SkillActiveState = {
		...
		...(deepInterviewInputLock ? { input_lock: deepInterviewInputLock } : {}),
		...(match.skill === "deep-interview" && deepInterviewConfig
			? { deep_interview_config: deepInterviewConfig }
			: {}),
	};
	...
	try {
		...
		const nextState = await persistStatefulSkillSeedState(
			...
			{
				activeContinuation:
					match.skill === "autopilot" && sameSkillContinuation,
				forceSessionScope: suppressRootMutation,
			},
		);
...
export function applyRalplanGate(
	...
	const consensusBlockedFollowup =
		planningComplete &&
		executionKeywords.some(
			(keyword) => keyword === "team" || keyword === "ralph",
		);
	const shortFollowupBypasses = executionKeywords.filter((keyword) => {
		if (keyword !== "team" && keyword !== "ralph") return false;
		return isApprovedExecutionFollowupShortcut(keyword as FollowupMode, text, {
	...
	return {
		...
		blockedReason: consensusBlockedFollowup
			? consensusEvidence.blockedReason
			: null,
	};
}
