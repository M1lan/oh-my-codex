...
export function isHookPluginsEnabled(
...
export async function validateHookPluginExport(
	pluginPath: string,
): Promise<{ valid: boolean; reason?: string }> {
	return validatePluginExport(pluginPath);
}

export async function discoverHookPlugins(
	cwd: string,
): Promise<HookPluginDescriptor[]> {
	const dir = hooksDir(cwd);
	...
	const plugins: HookPluginDescriptor[] = discovered.map((plugin) => {
		...
		const id = hasCollision
			? `${plugin.idBase}-${shortFileHash(plugin.file)}`
			: plugin.idBase;
		return {
			id,
			name: id,
...
export async function loadHookPluginDescriptors(
	cwd: string,
): Promise<HookPluginDescriptor[]> {
