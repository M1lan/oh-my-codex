...
): HookPluginOmxSessionState | null {
	const sessionId = normalizeSessionId(value.session_id);
	return sessionId
		? ({ ...value, session_id: sessionId } as HookPluginOmxSessionState)
		: null;
}
...
async function readHudState(
	...
	if (stateRoot) {
		try {
			...
			if (
				...
				lstatSync(sessionDir).isSymbolicLink()
			)
				return null;
			...
			if (
				!isContainedPath(canonicalSessionDir, canonicalStateRoot) ||
				!isContainedPath(canonicalHudStatePath, canonicalSessionDir)
			)
				return null;
	...
	const [hudStatePath] = await getReadScopedStateFilePaths(
		"hud-state.json",
		cwd,
		undefined,
		{
...
export function createHookPluginOmxApi(
	cwd: string,
	stateRoot?: string,
	requestedSessionId?: string,
): HookPluginSdk["omx"] {
	return {
		session: {
			read: () =>
				readOmxStateFile<HookPluginOmxSessionState>(
		...
		notifyFallback: {
			read: () =>
				readOmxStateFile<HookPluginOmxNotifyFallbackState>(
					omxRootStateFilePath(cwd, "notify-fallback-state.json", stateRoot),
				),
		},
		updateCheck: {
			read: () =>
				readOmxStateFile<HookPluginOmxUpdateCheckState>(
