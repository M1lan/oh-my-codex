...
async function handleNativeStopTeamLeaderAttention(
	input: HookRuntimeDispatchInput,
): Promise<void> {
	if (input.event.source !== "native" || input.event.event !== "stop") return;
	const sessionId =
		typeof input.event.session_id === "string"
			? input.event.session_id.trim()
			: "";
	if (!sessionId) return;
	const { markOwnedTeamsLeaderStopObserved } = await import(
		"../../team/state.js"
	);
	await markOwnedTeamsLeaderStopObserved(
		input.cwd,
		sessionId,
...
export async function dispatchHookEventRuntime(
	input: HookRuntimeDispatchInput,
): Promise<HookRuntimeDispatchResult> {
	const enabled =
		input.event.source === "native" || input.event.source === "derived"
			? true
