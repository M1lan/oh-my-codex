...
describe("isHookPluginsEnabled", () => {
	...
	it('returns true for "true"', () => {
		assert.equal(
			isHookPluginsEnabled({ [HOOK_PLUGIN_ENABLE_ENV]: "true" }),
			true,
		);
	});
	...
	it('returns true for "yes"', () => {
		assert.equal(
			isHookPluginsEnabled({ [HOOK_PLUGIN_ENABLE_ENV]: "yes" }),
			true,
		);
	});
	...
	it('returns false for "0"', () => {
		assert.equal(
			isHookPluginsEnabled({ [HOOK_PLUGIN_ENABLE_ENV]: "0" }),
			false,
		);
	});
	...
	it("returns true for arbitrary string", () => {
		assert.equal(
			isHookPluginsEnabled({ [HOOK_PLUGIN_ENABLE_ENV]: "enabled" }),
			true,
		);
	});
	...
	it('returns false for "false"', () => {
		assert.equal(
			isHookPluginsEnabled({ [HOOK_PLUGIN_ENABLE_ENV]: "false" }),
			false,
		);
	});
	...
	it('returns false for "no"', () => {
		assert.equal(
			isHookPluginsEnabled({ [HOOK_PLUGIN_ENABLE_ENV]: "no" }),
			false,
		);
	});
});
...
describe("resolveHookPluginTimeoutMs", () => {
	...
	it("parses valid numeric string", () => {
		assert.equal(
			resolveHookPluginTimeoutMs({ [HOOK_PLUGIN_TIMEOUT_ENV]: "5000" }),
			5000,
		);
	});
	...
	it("clamps to minimum of 100", () => {
		assert.equal(
			resolveHookPluginTimeoutMs({ [HOOK_PLUGIN_TIMEOUT_ENV]: "10" }),
			100,
		);
	});
	...
	it("clamps to maximum of 60000", () => {
		assert.equal(
			resolveHookPluginTimeoutMs({ [HOOK_PLUGIN_TIMEOUT_ENV]: "999999" }),
			60000,
		);
	});
	...
	it("floors fractional values", () => {
		assert.equal(
			resolveHookPluginTimeoutMs({ [HOOK_PLUGIN_TIMEOUT_ENV]: "2500.9" }),
			2500,
		);
	});
	...
	it("returns fallback for non-numeric string", () => {
		assert.equal(
			resolveHookPluginTimeoutMs({ [HOOK_PLUGIN_TIMEOUT_ENV]: "abc" }),
			1500,
		);
	});
	...
	it("returns fallback for empty string", () => {
		assert.equal(
			resolveHookPluginTimeoutMs({ [HOOK_PLUGIN_TIMEOUT_ENV]: "" }),
			1500,
		);
	});
});
