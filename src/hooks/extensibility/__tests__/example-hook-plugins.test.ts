...
const DERIVED_EVENTS = new Set<string>([
	"needs-input",
	"pre-tool-use",
	"post-tool-use",
]);
	...
	it("dispatches only the matching example plugin for a single event", async () => {
		...
		try {
			...
			const matched = result.results.find(
				(item) => item.plugin === "example-session-start",
			);
			assert.ok(matched);
	...
	it("covers all example plugin event types with deterministic state assertions", async () => {
		...
		try {
			...
			for (const eventName of ALL_EVENTS) {
				...
				const matched = result.results.find(
					(item) => item.plugin === `example-${eventName}`,
				);
				assert.ok(matched);
				assert.equal(matched.ok, true);
			}
			...
			for (const eventName of ALL_EVENTS) {
				const data = await readPluginData(cwd, eventName);
				const runId = `run-${eventName}`;
				const expectedSource = DERIVED_EVENTS.has(eventName)
					? "derived"
					: "native";
