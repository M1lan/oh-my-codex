...
async function writeOmxStateFile(
	cwd: string,
	fileName: string,
	value: unknown,
): Promise<void> {
	...
	describe("omx", () => {
		it("exposes only the explicit read-only omx readers", async () => {
			...
			try {
				...
				assert.deepEqual(Object.keys(sdk.omx).sort(), [
					"hud",
					"notifyFallback",
					"session",
					"updateCheck",
		...
		it("reads hud state from the current session scope instead of stale root fallback", async () => {
			...
			try {
				...
				await writeOmxStateFile(
					cwd,
					join("sessions", "sess-current", "hud-state.json"),
					{
						last_turn_at: "2026-01-01T00:00:00.000Z",
						turn_count: 3,
		...
		it("honors explicit stateRoot when reading session-scoped HUD state", async () => {
			...
			try {
				...
				await writeOmxStateFile(
					cwd,
					join("sessions", "stale-source", "hud-state.json"),
					{
						last_turn_at: "stale-source",
						turn_count: 99,
					},
				);
				await mkdir(join(stateRoot, "sessions", "sess-authority"), {
					recursive: true,
				});
				await writeFile(
				...
				await writeFile(
					join(stateRoot, "sessions", "sess-authority", "hud-state.json"),
					JSON.stringify({
		...
		it("requires a usable pointer and bound hook session for explicit HUD reads", async () => {
			...
			const stateRoot = await mkdtemp(
				join(tmpdir(), "omx-sdk-hud-binding-root-"),
			);
			try {
		...
		it("rejects invalid and symlink-escaped sessions under explicit stateRoot", async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-sdk-hud-containment-source-"),
			);
			const stateRoot = await mkdtemp(
				join(tmpdir(), "omx-sdk-hud-containment-root-"),
			);
			const outside = await mkdtemp(
				join(tmpdir(), "omx-sdk-hud-containment-outside-"),
			);
			try {
				await writeFile(
					join(stateRoot, "session.json"),
					JSON.stringify({ session_id: "../outside" }),
				);
				let sdk = createHookPluginSdk({
					cwd,
					stateRoot,
				...
				await writeFile(
					...
					JSON.stringify({ turn_count: 99 }),
				);
				await symlink(
					outside,
					join(stateRoot, "sessions", "sess-escaped"),
					process.platform === "win32" ? "junction" : "dir",
				);
				await writeFile(
					join(stateRoot, "session.json"),
					JSON.stringify({
						session_id: "sess-escaped",
						cwd,
						state_root: stateRoot,
					}),
				);
				sdk = createHookPluginSdk({
					...
					stateRoot,
					pluginName: "test",
					event: { ...makeEvent(), session_id: "sess-escaped" },
				});
				assert.equal(await sdk.omx.hud.read(), null);

				const crossSessionTarget = join(
					stateRoot,
					"sessions",
					"sess-cross-target",
				);
				...
				await writeFile(
					join(crossSessionTarget, "hud-state.json"),
					JSON.stringify({ turn_count: 77 }),
				);
				await symlink(
					crossSessionTarget,
				...
				if (process.platform !== "win32") {
					...
					sdk = createHookPluginSdk({
						...
						stateRoot,
						pluginName: "test",
						event: { ...makeEvent(), session_id: "sess-file-escaped" },
					});
					assert.equal(await sdk.omx.hud.read(), null);

					const siblingDir = join(stateRoot, "sessions", "sess-sibling");
					const siblingTargetDir = join(
						stateRoot,
						"sessions",
						"sess-sibling-target",
					);
					...
					await writeFile(
						join(siblingTargetDir, "hud-state.json"),
						JSON.stringify({ turn_count: 55 }),
					);
					await symlink(
						join(siblingTargetDir, "hud-state.json"),
						join(siblingDir, "hud-state.json"),
						"file",
					);
					await writeFile(
						join(stateRoot, "session.json"),
					...
					sdk = createHookPluginSdk({
						...
						stateRoot,
						pluginName: "test",
						event: { ...makeEvent(), session_id: "sess-sibling" },
					});
					assert.equal(await sdk.omx.hud.read(), null);
				}
			} finally {
