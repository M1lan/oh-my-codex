...
function setEuroByteOffset(
	...
	assert.ok(
		...
		`initial euro offset ${initialOffset} exceeded target ${targetOffset}`,
	);
...
describe("plugin-runner", () => {
	it("reads stdin through async stream iteration without touching the fd", async () => {
		...
		const stream = Readable.from(chunks, { encoding: "utf-8" }) as Readable & {
			fd: number;
		};
	...
	it("emits error for empty stdin", async () => {
		const { result, code } = await new Promise<{
			result: Record<string, unknown> | null;
			code: number | null;
		}>((resolve) => {
			const child = spawn(process.execPath, [getRunnerPath()], {
		...
		try {
			...
			const { result, code } = await Promise.race([
				...
				new Promise<never>((_, reject) =>
					setTimeout(() => reject(new Error("runner timed out")), 1000),
				),
			]);
	...
	it("emits runner_error when plugin throws", async () => {
		...
		try {
			...
			await writeFile(
				pluginPath,
				'export function onHookEvent() { throw new Error("boom"); }',
			);
