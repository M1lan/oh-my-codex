...
describe("dispatchHookEvent", () => {
	...
	it("forwards explicit stateRoot through the plugin runner to HUD reads", async () => {
		...
		const stateRoot = await mkdtemp(
			join(tmpdir(), "omx-dispatch-root-authority-"),
		);
		try {
			const dir = join(cwd, ".omx", "hooks");
...
export async function onHookEvent(event, sdk) {
  ...
  await writeFile(process.env.OMX_TEST_DISPATCH_OUTPUT, JSON.stringify(hud));
	...
	it("resolves plugin authority from options.env instead of ambient process.env", async () => {
		...
		const stateRoot = await mkdtemp(
			join(tmpdir(), "omx-dispatch-env-authority-"),
		);
		const ambientRoot = await mkdtemp(
			join(tmpdir(), "omx-dispatch-env-ambient-"),
		);
		...
		try {
			...
			await writeFile(
				join(stateRoot, "session.json"),
				JSON.stringify({ session_id: "sess-env", cwd, state_root: stateRoot }),
			);
			await writeFile(
				join(sessionDir, "hud-state.json"),
				JSON.stringify({ turn_count: 8 }),
			);
			await writeFile(
...
export async function onHookEvent(event, sdk) {
  await writeFile(process.env.OMX_TEST_DISPATCH_OUTPUT, JSON.stringify(await sdk.omx.hud.read()));
			...
			const result = await dispatchHookEvent(
				buildHookEvent("session-start", { session_id: "sess-env" }),
				{
					cwd,
					env: {
						...process.env,
...
`,
			...
			const pids = (
				await readFile(join(cwd, ".omx", "top-level-pids.log"), "utf-8")
			)
				.trim()
				.split("\n")
				.filter(Boolean);
          ...
          await new Promise(() => {});
			...
			const result = await dispatchHookEvent(event, {
				...
				env: {
					...process.env,
					OMX_HOOK_PLUGINS: "1",
					OMX_TEST_PLUGIN_PID_FILE: pidFile,
				},
			});
			...
			assert.ok(
				elapsedMs < 1500,
				`dispatch should timeout promptly (elapsed=${elapsedMs}ms)`,
			);
	...
	it("dedupes repeated native lifecycle hook dispatches for the same session/turn fingerprint", async () => {
		...
		const stateRoot = await mkdtemp(
			join(tmpdir(), "omx-dispatch-dedupe-root-"),
		);
		try {
			const dir = join(cwd, ".omx", "hooks");
			await mkdir(dir, { recursive: true });
			...
			assert.equal(
				existsSync(
					join(stateRoot, "sessions", "sess-1", "lifecycle-notif-state.json"),
				),
				true,
			);
			assert.equal(
