...
async function readActiveModes(
	...
	const canonicalEntries = listActiveSkills(canonicalState ?? {}).sort((a, b) =>
	...
	for (const entry of canonicalEntries) {
		try {
			const [detailPath] = await getAuthoritativeActiveStatePaths(
				entry.skill,
				cwd,
				sessionId,
			);
			const data =
				detailPath && existsSync(detailPath)
