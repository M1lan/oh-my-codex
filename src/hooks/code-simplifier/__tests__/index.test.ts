...
function makeTmpDir(): string {
	const dir = join(
		tmpdir(),
		`omx-cs-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
	);
...
function initGitRepo(dir: string): void {
	...
	execSync('git config user.email "omx-test@example.com"', {
		cwd: dir,
		stdio: "ignore",
	});
	...
	writeFileSync(
		join(dir, "rename-old.ts"),
		"export const renamed = true;\n",
		"utf-8",
	);

	execSync("git add tracked.ts deleted.ts rename-old.ts", {
		cwd: dir,
		stdio: "ignore",
	});
...
describe("getModifiedFiles", () => {
	...
	it("includes modified and untracked files, excludes deleted and rename-old paths", () => {
		...
		try {
			...
			writeFileSync(
				join(dir, "new-file.ts"),
				"export const newFile = true;\n",
				"utf-8",
			); // ??
			execSync("git mv rename-old.ts rename-new.ts", {
				cwd: dir,
				stdio: "ignore",
			}); // R old -> new
