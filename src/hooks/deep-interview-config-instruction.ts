...
export function buildDeepInterviewConfigInstruction(
	cwd: string,
	...
	if (
		...
		return null;
	const statePath = resolveInitializedStatePath(
		cwd,
		skillState.initialized_state_path,
	);
	try {
		if (!existsSync(statePath)) return null;
		const parsed = JSON.parse(readFileSync(statePath, "utf-8")) as Record<
			string,
			unknown
		>;
		const nested =
			parsed.deep_interview_config &&
			typeof parsed.deep_interview_config === "object"
		...
		const sourcePath = safeString(
			parsed.config_source ?? nested.sourcePath,
		).trim();
		const threshold =
			typeof parsed.threshold === "number"
				...
				: nested.threshold;
		const maxRounds =
			typeof parsed.max_rounds === "number"
				? parsed.max_rounds
				: nested.maxRounds;
		const enableChallengeModes =
			typeof parsed.enable_challenge_modes === "boolean"
