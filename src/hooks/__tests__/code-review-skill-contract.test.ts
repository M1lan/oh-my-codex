...
describe("code-review skill contract", () => {
	it("requires parallel code-reviewer and architect lanes", () => {
		assert.match(
			codeReviewSkill,
			/`code-reviewer` and `architect` agents in parallel/i,
		);
		...
		assert.match(
			codeReviewSkill,
			/If either lane cannot be launched or does not return evidence/i,
		);
		assert.match(
			codeReviewSkill,
			/do \*\*not\*\* substitute the current\/authoring lane/i,
		);
	});
	...
	it("uses native task agent_type examples without overriding user model or effort", () => {
		assert.match(
			codeReviewSkill,
			/Respect the user's current model and reasoning\/effort selection/i,
		);
		assert.match(
			codeReviewSkill,
			/Do not pass `model` or `reasoning_effort` overrides/i,
		);
		assert.match(
			codeReviewSkill,
			/task\(\s*agent_type="code-reviewer",\s*prompt=/s,
		);
		assert.match(
			codeReviewSkill,
			/task\(\s*agent_type="architect",\s*prompt=/s,
		);
		assert.doesNotMatch(
			codeReviewSkill,
			/task\(\s*agent_type="code-reviewer",\s*(?:model=|reasoning_effort=)/s,
		);
		assert.doesNotMatch(
			codeReviewSkill,
			/task\(\s*agent_type="architect",\s*(?:model=|reasoning_effort=)/s,
		);
	...
	it("frames architect as the devil’s-advocate lane with deterministic blocker status", () => {
		...
		assert.match(
			codeReviewSkill,
			/If architect status is \*\*BLOCK\*\*, final recommendation is \*\*REQUEST CHANGES\*\*/i,
		);
	});
	...
	it("requires final synthesis across both lanes", () => {
		...
		assert.match(
			codeReviewSkill,
			/Combine the `code-reviewer` recommendation and the architect status/i,
		);
		assert.match(
			codeReviewSkill,
			/Approval requires explicit evidence from both independent lanes/i,
		);
		assert.match(
			codeReviewSkill,
			/missing or failed delegation is a blocking unavailable-review state/i,
		);
		assert.match(
			codeReviewSkill,
			/final report must make architect blockers impossible to miss/i,
		);
	});
	...
	it("forbids self-review fallback approval when delegation is unavailable", () => {
		...
		assert.match(
			codeReviewSkill,
			/block approval until the independent lane evidence exists/i,
		);
	});
	...
	it("keeps approval criteria aligned with the deterministic synthesis contract", () => {
		assert.match(
			codeReviewSkill,
			/\*\*APPROVE\*\* - `code-reviewer` returns APPROVE, architect status is `CLEAR`, and both independent lanes returned evidence/i,
		);
		assert.match(
			codeReviewSkill,
			/\*\*REQUEST CHANGES\*\* - `code-reviewer` returns REQUEST CHANGES, architect status is `BLOCK`, or required independent review delegation is unavailable\/skipped\/failed/i,
		);
		assert.match(
			codeReviewSkill,
			/\*\*COMMENT\*\* - `code-reviewer` returns COMMENT with architect status `CLEAR`, architect status is `WATCH`, or only LOW\/MEDIUM improvements remain/i,
		);
	});
	...
	it("bounds auto-fix wording to the explicit ralph path only", () => {
		...
		assert.match(
			codeReviewSkill,
			/automatic fix follow-up without another permission prompt/i,
		);
		assert.match(
			codeReviewSkill,
			/Plain `code-review` itself remains read-only and does \*\*not\*\* promise auto-fix/i,
		);
	});
