		...
		JSON.stringify(
			{
				...
				active_skills: [
					{ skill: "autopilot", active: true, phase, session_id: sessionId },
				],
			},
			null,
			2,
...
async function readAutopilotModeState(
	stateDir: string,
	sessionId: string,
): Promise<TestAutopilotModeState> {
	return JSON.parse(
		await readFile(
...
async function assertAutopilotRecoverySnapshot(
	...
	const snapshotPath =
		modeState.state?.handoff_artifacts?.context_snapshot_path ?? "";
	if (typeof expectedPath === "string")
		assert.equal(snapshotPath, expectedPath);
	else assert.match(snapshotPath, expectedPath);
	assert.equal(
		modeState.state?.handoff_artifacts?.context_snapshot?.kind,
		"recovery",
	);
	assert.equal(
		modeState.state?.handoff_artifacts?.context_snapshot?.recovery?.reason,
		expectedReason,
	);
	assert.equal(
		(
			modeState.state?.context_snapshot_recovery as
	...
	assert.match(
		recoverySnapshot,
		new RegExp(`recovery reason: ${expectedReason}`),
	);
	assert.match(
		recoverySnapshot,
		/do not treat the continuation input as the task seed/,
	);
...
describe("keyword detector team compatibility", () => {
	...
	it("limits explicit multi-skill invocation to the first contiguous $skill block", () => {
		const matches = detectKeywords(
			"$ralplan Fix issue #1030 and ensure other directives ($ralph, $team, $deep-interview) are not affected",
		);
		assert.deepEqual(
			matches.map((m) => m.skill),
			["ralplan"],
	...
	it("does not merge implicit keyword matches when an explicit $skill is present", () => {
		const matches = detectKeywords(
			"please run $team and then analyze the result",
		);
		assert.deepEqual(
			matches.map((m) => m.skill),
			["team"],
	...
	it("does not fall back to implicit keyword detection when an unknown $token is present", () => {
		...
		const primary = detectPrimaryKeyword(
			"$maer-thinking 다시 설명해봐 keep going",
		);
	...
	it("recognizes plugin-prefixed explicit skill tokens", () => {
		const matches = detectKeywords(
			"$oh-my-codex:ralplan implement issue #1307",
		);
		assert.deepEqual(
			matches.map((m) => m.skill),
			["ralplan"],
	...
	it("supports mixed-form explicit multi-skill invocation ordering and dedupe", () => {
		const matches = detectKeywords(
			"$oh-my-codex:ralplan $ralph $oh-my-codex:ralplan ship this",
		);
		assert.deepEqual(
			matches.map((m) => m.skill),
			["ralplan", "ralph"],
	...
	it("supports plugin-prefixed hyphenated workflow tokens", () => {
		const deepInterview = detectPrimaryKeyword(
			"$oh-my-codex:deep-interview gather requirements",
		);
	...
	it("suppresses implicit detection when an unknown plugin-prefixed token is present with other keyword text", () => {
		...
		assert.deepEqual(matches, []);
		assert.equal(
			detectPrimaryKeyword("$oh-my-codex:unknown analyze this issue"),
			null,
		);
	});
	...
	it("maps explicit $ultragoal invocation to ultragoal workflow skill", () => {
		const match = detectPrimaryKeyword(
			"$ultragoal split this release into durable goals",
		);
		assert.ok(match);
	...
	it("maps explicit $best-practice-research invocation to the best-practice research wrapper", () => {
		const match = detectPrimaryKeyword(
			"$best-practice-research find current official guidance for this API",
		);
		assert.ok(match);
	...
	it("maps intentful ultragoal prose without triggering artifact path mentions", () => {
		const intentful = detectPrimaryKeyword(
			"please run ultragoal workflow for this launch",
		);
		assert.ok(intentful);
	...
	it("does not trigger autopilot from management/debug prose mentions", () => {
		assert.equal(
			detectPrimaryKeyword("inspect autopilot state before continuing"),
			null,
		);
		assert.equal(
			detectPrimaryKeyword("fix the autopilot bug in the detector"),
			null,
		);
		...
		assert.equal(
			detectPrimaryKeyword("start autopilot bug investigation"),
			null,
		);
	});
	...
	it("does not trigger team keyword from filesystem/team-state path text", () => {
		const match = detectPrimaryKeyword(
			"You have 1 new message(s). Read .omx/state/team/execute-plan/mailbox/worker-3.json, act now, reply with concrete progress, then continue assigned work or next feasible task.",
		);
	...
	it("prefers ralplan over ralph follow-up language when both implicit routes are present", () => {
		const match = detectPrimaryKeyword(
			"keep going but do consensus plan first",
		);
	...
	it("does not trigger deep-interview from cleanup or state-management mentions", () => {
		assert.equal(
			detectPrimaryKeyword("clear deep interview state before continuing"),
			null,
		);
		assert.equal(
			detectPrimaryKeyword(
				"cleanup stale deep-interview state after session clear",
			),
			null,
		);
		assert.equal(
			detectPrimaryKeyword(
				"remove the stale deep interview lock from .omx/state",
			),
			null,
		);
	});
	...
	it("does not trigger deep-interview from casual discussion mentions", () => {
		assert.equal(
			detectPrimaryKeyword(
				"the deep interview report is useful context for the next plan",
			),
			null,
		);
		assert.equal(
			detectPrimaryKeyword(
				"we already did a deep interview and should not reactivate it",
			),
			null,
		);
		assert.equal(
			detectPrimaryKeyword(
				"this interview transcript says implementation is ready",
			),
			null,
		);
	});
	...
	it("does not trigger cancel from stop inside filenames or paths", () => {
		assert.equal(
			detectPrimaryKeyword(
				"inspect .omx/context/stop-hook-invalid-json-handoff-20260629T210626Z.md",
			),
			null,
		);
		assert.equal(
			detectPrimaryKeyword(
				"Please summarize /tmp/stop-hook-invalid-json-handoff-20260629T210626Z.md",
			),
			null,
		);
	...
	it("does not trigger cancel from incidental stop/abort test-log prose", () => {
		assert.equal(
			detectPrimaryKeyword("FAIL should stop retrying after max attempts"),
			null,
		);
		assert.equal(
			detectPrimaryKeyword("PASS request aborted when upstream returns 499"),
			null,
		);
	});
	...
	it("does not trigger ultrawork from incidental parallel test-log prose", () => {
		assert.equal(
			detectPrimaryKeyword(
				"PASS runs assertions in parallel when sharding is enabled",
			),
			null,
		);
		assert.equal(
			detectPrimaryKeyword("running 8 tests in parallel across 4 workers"),
			null,
		);
	});
...
describe("keyword input classification direct grammar", () => {
	it("classifies prompt-leading direct forms, aliases, priority, and block order", () => {
		const cases = [
			...
			{
				text: "- $team $ralph ship this",
				skills: ["team", "ralph"],
				keywords: ["$team", "$ralph"],
				priorities: [8, 9],
			},
			{
				text: "12) $oh-my-codex:ralplan build this",
				skills: ["ralplan"],
				keywords: ["$oh-my-codex:ralplan"],
				priorities: [11],
			},
			{
				text: "$ulw $frontend-ui-ux build this",
				skills: ["ultrawork", "design"],
				keywords: ["$ulw", "$frontend-ui-ux"],
				priorities: [10, 6],
			},
			{
				text: "$ralplan $unknown $ralplan $ralph ship this",
				skills: ["ralplan", "ralph"],
				keywords: ["$ralplan", "$ralph"],
				priorities: [11, 9],
			},
			{
				text: "$ㅕㅣㅈ 병렬 작업",
			...
			{
				text: "use $ralplan plan this",
				skills: ["ralplan"],
				keywords: ["$ralplan"],
				priorities: [11],
			},
			{
				text: "please use $ralplan plan this",
				skills: ["ralplan"],
				keywords: ["$ralplan"],
				priorities: [11],
			},
			{
				text: "run $ralplan plan this",
				skills: ["ralplan"],
				keywords: ["$ralplan"],
				priorities: [11],
			},
			...
			{
				text: "please use $code-review",
				skills: ["code-review"],
				keywords: ["$code-review"],
				priorities: [6],
			},
			{
				text: "please run $team",
				skills: ["team"],
				keywords: ["$team"],
				priorities: [8],
			},
			{
				text: "start $deep-interview",
				skills: ["deep-interview"],
				keywords: ["$deep-interview"],
				priorities: [8],
			},
			{
				text: "enable $ultragoal",
				skills: ["ultragoal"],
				keywords: ["$ultragoal"],
				priorities: [10],
			},
		...
		for (const testCase of cases) {
			...
			assert.deepEqual(
				classification.matches.map((match) => match.skill),
				testCase.skills,
				testCase.text,
			);
			assert.deepEqual(
				classification.matches.map((match) => match.keyword),
				testCase.keywords,
				testCase.text,
			);
			assert.deepEqual(
				classification.matches.map((match) => match.priority),
				testCase.priorities,
				testCase.text,
			);
		}
	...
	it("does not extend discourse negation beyond a comma-separated directive", () => {
		for (const text of [
			...
			assert.deepEqual(
				classifyKeywordInput(text).matches.map((match) => match.skill),
				["autopilot"],
				text,
			);
		}
	...
	it("accepts every directive verb and polite prefix for every explicit token and alias", () => {
		const tokens = new Set([
			...KEYWORD_TRIGGER_DEFINITIONS.filter((definition) =>
				definition.keyword.startsWith("$"),
	...
	it("contains nested predecessors, preserves first-block and reserved dominance", () => {
		...
		const firstBlock = classifyKeywordInput(
			'$ralplan plan it\n"x"\n$autopilot build it',
		);
		assert.deepEqual(
			firstBlock.matches.map((match) => match.skill),
			["ralplan"],
		);
		assert.deepEqual(firstBlock.candidates[1]?.reasons, ["not-leading-region"]);

		const reserved = classifyKeywordInput(
			'/prompts:architect\n"x"\n$ralplan plan it',
		);
		...
		const confusablePrompts = classifyKeywordInput(
			"/promptſ:architect; use autopilot mode.",
		);
		assert.equal(confusablePrompts.reservedInput, null);
		assert.deepEqual(
			confusablePrompts.matches.map((match) => match.skill),
			["autopilot"],
		);
	});
	...
	it("tracks list fence closer identity and relative indentation", () => {
		...
		assert.deepEqual(
			relativeCloser.matches.map((match) => match.skill),
			["ralplan"],
		);
	});
		...
		const cases = [
			{
				...
				candidates: [
					{
						rawKeyword: "$ralph",
						reasons: ["fenced-code", "not-leading-region"],
					},
	...
	it("masks multiline Markdown reference titles", () => {
		...
		const closedPromptsTitle = classifyKeywordInput(
			'[docs]: /target "title\nUse /prompts:architect"\n$ralplan plan it',
		);
		assert.deepEqual(
			closedPromptsTitle.matches.map((match) => match.skill),
			["ralplan"],
		);
	});
	...
	it("fails closed for Unicode case-fold and confusable explicit-token continuations", () => {
		for (const testCase of [
			...
			assert.equal(
				classification.candidates[0]?.rawKeyword,
				testCase.rawKeyword,
				testCase.text,
			);
	...
	it("composes V11 malformed-token, documentation, directive, and Arabic-clause controls", () => {
		...
		for (const testCase of [
			{
				text: "use $autopilot is documented but use $ralplan plan it",
				skills: ["ralplan"],
			},
			{ text: "- use $ralplan： consensus-planning workflow", skills: [] },
			{
				text: "use $ralplan is the workflow command؟ run $autopilot",
				skills: ["autopilot"],
			},
			{ text: "$ralplan؛ $autopilot is prohibited", skills: ["ralplan"] },
			{ text: "$ralplan is prohibited but uſe autopilot mode.", skills: [] },
			{
				text: "$ralplan is prohibited but use autopilot mode.",
				skills: ["autopilot"],
			},
			{
			...
			{
				text: "Do not run $ralplan; use the $autopilot build it",
				skills: ["autopilot"],
			},
			{ text: '"quoted"\ncontinue with $ralplan', skills: ["ralplan"] },
			{
				text: "use $ralplan is documented; advance to $ultragoal",
				skills: ["ultragoal"],
			},
		] as const) {
			assert.deepEqual(
				classifyKeywordInput(testCase.text).matches.map((match) => match.skill),
				testCase.skills,
				testCase.text,
			);
		}
	...
	it("masks mixed postposed negation and documentary subject chains without reopening prose", () => {
		...
		assert.deepEqual(
			classifyKeywordInput(
				"use $ralplan and autopilot mode are workflow commands; use $team execute it",
		...
		for (const punctuation of ["．", ";"] as const) {
			...
			assert.deepEqual(
				classifyKeywordInput(`Do not run $ralplan${punctuation}suffix`).matches,
				[],
				`${text} attached suffix`,
			);
		}
	});
	...
	it("repairs V13 Unicode grammar, coordinated negation, documentation, and Markdown-table probes", () => {
		const cases = [
			{
				text: "Do not use deep interview яbut use autopilot mode.",
				skills: [],
			},
			{
				text: "Do not use deep interview but use autopilot mode.",
				skills: ["autopilot"],
			},
			{ text: "$ralplan, autopilot mode, $team are prohibited.", skills: [] },
			{
				text: "Autopilot mode and $ralplan are workflow commands; use $team execute it",
				skills: ["team"],
			},
			{
				text: 'Use autopilot mode; "note"; use $ralplan is the workflow command.',
				skills: ["autopilot"],
			},
			{
				text: "use $ralplan is the workflow command: use $autopilot build it",
				skills: ["autopilot"],
			},
			{
			...
			{
				text: "Do not run $ralplan but advance to $ultragoal",
				skills: ["ultragoal"],
			},
			{
				text: "Do not run $ralplan but jump straight to $ultragoal",
				skills: ["ultragoal"],
			},
			{ text: "Do not run $ralplan яbut advance to $ultragoal", skills: [] },
			{
				text: "use $ralplan is the workflow command яbut use $autopilot build it",
				skills: [],
			},
		] as const;
	...
	it("keeps V13 coordinated scans near-linear at 4096 explicit candidates", () => {
		...
		const families = [
			...
			{
				text: `Do not run ${Array.from({ length: count }, () => "$team").join(", ")}`,
				skills: [],
			},
			{
				text: `${Array.from({ length: count }, () => "$team, autopilot mode").join(", ")} are prohibited`,
				skills: [],
			},
		] as const;
		...
		for (const family of families) {
			...
			assert.ok(
				...
				"V13 coordinated scan must stay bounded",
			);
		}
	});
	...
	it("repairs V14 mixed subject chains and semicolon-local documentation", () => {
		const cases = [
			...
			{
				text: "Both autopilot mode and $ralplan are workflow commands; use $team execute it",
				skills: ["team"],
			},
			{
				text: "Use autopilot mode; use $ralplan is the workflow command.",
				skills: ["autopilot"],
			},
		] as const;
	...
	it("keeps V15 documentary chains, compact candidates, and repeated predicates bounded", () => {
		const count = 4_096;
		const implicitSubjects = Array.from(
			{ length: count },
			() => "autopilot mode",
		).join(", ");
		...
		for (const family of families) {
			...
			assert.ok(
				Date.now() - startedAt < family.maximumMilliseconds,
				`${family.name} must stay bounded`,
			);
		}
	...
	it("repairs V16 documentary clause, reference, and negation composition", () => {
		for (const testCase of [
			{
				text: "Use autopilot mode, and $ralplan is documented in the guide.",
				skills: ["autopilot"],
			},
			...
			{
				text: "$team is prohibited and is forbidden; use $ralplan plan it",
				skills: ["ralplan"],
			},
			{
				text: "Use $ralplan, autopilot mode and $team are workflow commands.",
				skills: [],
			},
		] as const) {
			assert.deepEqual(
	...
	it("keeps V16 documentary and implicit-negative families subquadratic through 8192 items", () => {
		const families = [
			{
				...
				text: (count: number) =>
					`Autopilot mode is documented, ${Array.from({ length: count }, () => "$team").join(", ")} are workflow commands`,
		...
		for (const family of families) {
			...
			for (const count of [4_096, 8_192] as const) {
				...
				assert.deepEqual(
					classification.implicitMatches,
					[],
					`${family.name}: ${count}`,
				);
			}
	...
	it("scans maximal explicit tokens and rejects documentation, paths, Unicode, compatibility, and control suffixes", () => {
		const cases = [
			{
				text: "$ralplan.md is the workflow documentation file",
				rawKeyword: "$ralplan.md",
			},
			{ text: "$autopilot/config", rawKeyword: "$autopilot/config" },
			{ text: "$ralplan한글", rawKeyword: "$ralplan한글" },
			{
				text: "$oh-my-codex:ralplan.md",
				rawKeyword: "$oh-my-codex:ralplan.md",
			},
	...
	it("accepts later directives after structurally separated inert or negative mentions", () => {
		const cases = [
			{
				text: "Do not run $ralplan; instead $autopilot build issue #3140",
				skills: ["autopilot"],
			},
			{
				text: "Do not run $ralplan, instead $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "Do not run $ralplan; use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "Do not run $ralplan but use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "Do not run $ralplan but instead use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "Do not run $ralplan — instead use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "Quoted inline-code `$ralplan`; use $autopilot build it",
				skills: ["autopilot"],
			},
	...
	it("keeps prompt reservations and list documentation structural and composable", () => {
		...
		for (const testCase of [
			...
			{
				text: "use $ralplan is the consensus-planning command; $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "use $ralplan is the consensus-planning command\nUse autopilot mode.",
			...
			{
				text: "use $ralplan is the consensus-planning command; use $autopilot build it",
				skills: ["autopilot"],
			},
			{
			...
			{
				text: "use $ralplan is the workflow command; use $autopilot update the documentation",
				skills: ["autopilot"],
			},
			{
				text: "use $ralplan is the consensus-planning command; use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "use $ralplan is the consensus-planning command; then use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "use $ralplan is the consensus-planning command\nAutopilot mode is its alias.\n$team execute it",
			...
			{
				text: "use $ralplan is the consensus-planning command; autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "use $ralplan is the consensus-planning command； use $autopilot build it",
			...
			{
				text: "use $ralplan is the workflow command; but use $autopilot build it",
				skills: ["autopilot"],
			},
			{ text: "$ralplan; $autopilot is prohibited", skills: ["ralplan"] },
			{
				text: "$ralplan; $autopilot is documented in the guide.",
				skills: ["ralplan"],
			},
			{
				text: `use $ralplan is the workflow command;${" ".repeat(161)}use $autopilot build it`,
				skills: ["autopilot"],
			},
			{
				text: `$ralplan; $autopilot${" ".repeat(193)}is prohibited`,
				skills: ["ralplan"],
			},
			{
				text: "For instance: manual mode is slower。 Use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "use $ralplan is the workflow command; autopilot mode is its alias; $team execute it",
				skills: ["team"],
			},
			{
				text: "use $ralplan is the workflow command; autopilot mode is documented in the guide; $team execute it",
				skills: ["team"],
			},
			{
				text: "use $ralplan is the workflow command; autopilot mode is workflow documentation; use $ralph execute it",
				skills: ["ralph"],
			},
			{
				text: `use $ralplan is the workflow command; use${" ".repeat(161)}$autopilot build it`,
				skills: ["autopilot"],
			},
			{
				text: "use $ralplan is the consensus-planning command! run $autopilot",
				skills: ["autopilot"],
			},
			{
				text: "use $ralplan is the consensus-planning command？ run $autopilot",
			...
			{
				text: "The docs mention autopilot mode; use $ralplan plan it",
				skills: ["ralplan"],
			},
			{
				text: "Autopilot mode is workflow documentation; use $ralplan plan it",
				skills: ["ralplan"],
			},
			{ text: "[docs]: $ralplan\n$autopilot build it", skills: ["autopilot"] },
			{
		...
		assert.deepEqual(
			detectKeywords("- $ralplan plan this").map((match) => match.skill),
			["ralplan"],
		);
	});
	...
	it("requires implicit workflow phrases to be active, unmasked, and non-negated", () => {
		...
		for (const testCase of [
			...
			{
				text: "Do not use deep interview but instead use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "Do not use deep interview — instead use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "Autopilot mode is workflow documentation.\nUse autopilot mode.",
			...
			{
				text: 'Ignore the quoted "/prompts:architect" and use autopilot mode.',
				skills: ["autopilot"],
			},
			{
				text: 'Ignore the quoted "/prompts:architect" then use autopilot mode.',
				skills: ["autopilot"],
			},
			{
				text: "Do not run $ralplan but instead use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: 'Ignore "$ralplan" and use autopilot mode.',
				skills: ["autopilot"],
			},
			{
				text: "Ignore \\/prompts:architect and use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "See https://example.com/prompts:architect and use autopilot mode.",
				skills: ["autopilot"],
			},
			...
			{
				text: "See [/prompts:architect](./docs.md) and use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "Use autopilot mode, while deep interview is prohibited.",
				skills: ["autopilot"],
			},
			{ text: "Read the docs. Use autopilot mode.", skills: ["autopilot"] },
			{
				text: "The docs are stale; use autopilot mode.",
				skills: ["autopilot"],
			},
			...
			{
				text: "See [$ralplan](./docs.md) and use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "See https://example.com/$ralplan and use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "Use autopilot mode, and deep interview is prohibited.",
				skills: ["autopilot"],
			},
			{
				text: "Ignore the docs and use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "Ignore \\/prompts:architect; use $ralplan plan it",
				skills: ["ralplan"],
			},
			{
				text: "See https://example.com/prompts:architect; use $ralplan plan it",
				skills: ["ralplan"],
			},
			{
				text: "See [/prompts:architect](./docs.md); use $ralplan plan it",
				skills: ["ralplan"],
			},
			{
				text: "See [/prompts:architect][docs]; use $ralplan plan it",
				skills: ["ralplan"],
			},
			...
			{ text: "| $ralplan |\nUse autopilot mode.", skills: ["autopilot"] },
			{
				text: "See [$ralplan][docs] and use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "See C:\\docs\\$ralplan and use autopilot mode.",
				skills: ["autopilot"],
			},
			{ text: "Users＇ request: use autopilot mode.", skills: ["autopilot"] },
			{
				text: "Use autopilot mode, and deep interview should be avoided.",
				skills: ["autopilot"],
			},
			{
				text: "Ignore the docs but use autopilot mode.",
				skills: ["autopilot"],
			},
			{
			...
			{ text: "## $ralplan\n$autopilot build it", skills: ["autopilot"] },
			{
				text: "See [$ralplan](./docs.md); use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "See C:\\docs\\$ralplan; use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "Mode | Meaning\n--- | ---\n$ralplan | planning\n$autopilot build it",
			...
			{
				text: 'Ignore "$ralplan" and use $autopilot build it',
				skills: ["autopilot"],
			},
			{
				text: "Ignore `$ralplan` and use $autopilot build it",
			...
			{
				text: "See [`$ralplan`](./docs.md) and use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "Ignore deep interview and use autopilot mode.",
				skills: ["autopilot"],
			},
			{
				text: "See [docs](https://example.com/$ralplan) and use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: 'See [docs](./docs.md "$ralplan reference") and use $autopilot build it',
				skills: ["autopilot"],
			},
			{
				text: "See [docs](https://example.com/(v1)/$ralplan) and use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "See [$ralplan](https://example.com/(v1)) and use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: 'See [docs](./docs.md "$ralplan (reference") and use $autopilot build it',
			...
			{
				text: "$ralplan is prohibited but use $autopilot build it",
				skills: ["autopilot"],
			},
			{
				text: "$ralplan is prohibited and use $autopilot build it",
				skills: ["autopilot"],
			},
	...
	it("defines punctuation-separated workflow directives as one ordered block", () => {
		assert.deepEqual(
			detectKeywords("$ralplan, $autopilot; $team build it").map(
				(match) => match.skill,
			),
			["ralplan", "autopilot", "team"],
		);
		assert.deepEqual(
			detectKeywords("$ralplan plan it, then $team execute").map(
				(match) => match.skill,
			),
			["ralplan"],
		);
	...
	it("keeps fences and escape parity inert without semantic phrase matching", () => {
		...
		for (const testCase of structuralCases) {
			...
			assert.deepEqual(
				classification.candidates[0]?.reasons,
				[testCase.reason, "not-leading-region"],
				testCase.text,
			);
		}
	...
	it("respects quote escape parity and fence container prefixes", () => {
		...
		assert.deepEqual(
			escapedQuoteClassification.candidates.map(
		...
		assert.deepEqual(
			classifyKeywordInput(evenBackslashQuote).matches.map(
				(match) => match.skill,
			),
			["autopilot"],
		);
		...
		const unquotedFenceCandidates =
			classifyKeywordInput(unquotedFence).candidates;
		assert.ok(
			unquotedFenceCandidates.every((candidate) =>
				candidate.reasons.includes("fenced-code"),
			),
		);
		...
		assert.ok(
			classifyKeywordInput(quotedFence).candidates.every(
		...
		assert.deepEqual(
			classifyKeywordInput(matchingQuotedFence).matches.map(
				(match) => match.skill,
			),
			["autopilot"],
		);
	...
	it("requires valid fence closers and directive clause prefixes after inert mentions", () => {
		...
		for (const testCase of [
			...
			{
				text: '"$ralplan"; please use $autopilot build it',
				skills: ["autopilot"],
			},
	...
	it("terminates fenced, inline, quote, and blockquote regions on every logical line ending", () => {
		...
		for (const lineTerminator of lineTerminators) {
			...
			const fenced = classifyKeywordInput(
				`~~~${lineTerminator}$ralplan${lineTerminator}~~~`,
			);
			assert.deepEqual(
				fenced.candidates[0]?.reasons,
				["fenced-code", "not-leading-region"],
				label,
			);

			const blockquoted = classifyKeywordInput(
				`> $ralplan${lineTerminator}$team`,
			);
			assert.deepEqual(
				blockquoted.candidates[0]?.reasons,
				["blockquote", "not-leading-region"],
				label,
			);
			assert.deepEqual(blockquoted.candidates[1]?.reasons, [], label);
			assert.deepEqual(
				blockquoted.matches.map((match) => match.skill),
				["team"],
				label,
			);
			...
			assert.deepEqual(
				inline.candidates[0]?.reasons,
				["inline-code", "not-leading-region"],
				label,
			);
			assert.deepEqual(inline.candidates[1]?.reasons, [], label);
			assert.deepEqual(
				inline.matches.map((match) => match.skill),
				["team"],
				label,
			);
			...
			assert.deepEqual(
				quoted.candidates[0]?.reasons,
				["quote", "not-leading-region"],
				label,
			);
			assert.deepEqual(quoted.candidates[1]?.reasons, [], label);
			assert.deepEqual(
				quoted.matches.map((match) => match.skill),
				["team"],
				label,
			);
		}
	...
	it("keeps candidate diagnostics bounded across thousands of inert ranges", () => {
		const count = 4_096;
		const classification = classifyKeywordInput(
			"`$ralplan` ".repeat(count).trimEnd(),
		);
		assert.equal(classification.candidates.length, count);
		assert.deepEqual(classification.candidates[0]?.reasons, [
			"inline-code",
			"not-leading-region",
		]);
		assert.deepEqual(classification.candidates.at(-1)?.reasons, [
			"inline-code",
			"not-leading-region",
		]);
		assert.deepEqual(classification.matches, []);
		const shortcuts = classifyKeywordInput(
			"See [$ralplan] for details.\n".repeat(count) + "\n[$ralplan]: ./docs",
		);
		assert.equal(shortcuts.candidates.length, count + 1);
		assert.deepEqual(shortcuts.matches, []);
		const nestedParents = classifyKeywordInput(
			'"`x`\n'.repeat(count) + "$ralplan plan it",
		);
		assert.deepEqual(nestedParents.matches, []);
		const unclosedTitles = classifyKeywordInput(
			"[docs]: /target (\n".repeat(count) + "$ralplan plan it",
		);
		assert.deepEqual(unclosedTitles.matches, []);
		const documentationTokens = classifyKeywordInput(
			`use $ralplan is the workflow command for ${"$team ".repeat(count)}`,
		);
		assert.equal(documentationTokens.candidates.length, count + 1);
		assert.deepEqual(documentationTokens.matches, []);
		const semicolonDocumentation = classifyKeywordInput(
			`use $ralplan is the workflow command${"; explanatory note".repeat(count)}`,
		);
		assert.deepEqual(semicolonDocumentation.matches, []);
		const candidateTailDocumentation = classifyKeywordInput(
			`use $ralplan is the workflow command${"; $team is filler".repeat(count)} documentation`,
		);
		assert.equal(candidateTailDocumentation.candidates.length, count + 1);
		assert.deepEqual(
			candidateTailDocumentation.matches.map((match) => match.skill),
			["team"],
		);
		const periodDocumentationStart = Date.now();
		const periodDocumentation = classifyKeywordInput(
			"Autopilot mode is documented. ".repeat(count) + "use $ralplan plan it",
		);
		assert.deepEqual(
			periodDocumentation.matches.map((match) => match.skill),
			["ralplan"],
		);
		assert.ok(
			Date.now() - periodDocumentationStart < 2_000,
			"period-heavy documentation scan must stay bounded",
		);
		const contiguousChainStart = Date.now();
		const contiguousChain = classifyKeywordInput(
			"$ralplan ".repeat(count).trimEnd(),
		);
		assert.equal(contiguousChain.candidates.length, count);
		assert.deepEqual(
			contiguousChain.matches.map((match) => match.skill),
			["ralplan"],
		);
		assert.ok(
			Date.now() - contiguousChainStart < 2_000,
			"contiguous explicit chains must stay bounded",
		);

		const coordinatedNegationStart = Date.now();
		const coordinatedNegation = classifyKeywordInput(
			Array.from({ length: count }, () => "$team").join(",") +
				" are prohibited",
		);
		assert.equal(coordinatedNegation.candidates.length, count);
		assert.deepEqual(coordinatedNegation.matches, []);
		assert.ok(
			Date.now() - coordinatedNegationStart < 2_000,
			"coordinated postposed negation must stay bounded",
		);
	});
	...
	it("suppresses prose, multilingual, documentation, quoted, escaped, and code candidates without a phrase classifier", () => {
		...
		for (const text of cases) {
			...
			assert.equal(classification.hasExplicitLikeInvocation, true, text);
			assert.ok(
				classification.candidates.every((candidate) =>
					candidate.reasons.includes("not-leading-region"),
				),
				text,
			);
		}
	...
	it("applies marked-answer, accepted-direct, prompts, and explicit-like precedence in order", () => {
		const cases = [
			{
				text: "[omx question answered] $ralplan",
				reservedInput: "omx-question-answered",
				skills: [],
			},
			{
				text: "$ralplan plan this; /prompts:architect review",
				reservedInput: null,
				skills: ["ralplan"],
			},
			{
				text: "$unknown /prompts:architect review",
				reservedInput: null,
				skills: [],
			},
			{
				text: "/prompts:architect, keep going",
				reservedInput: "prompts",
				skills: [],
			},
			{
				text: "/prompts:unknown $ralplan plan this",
				reservedInput: "prompts",
				skills: [],
			},
			{
	...
	it("binds G1 and G2 direct classification fields exactly", () => {
		const cases = [
			...
			},
			{
				...
				skills: [],
				candidates: [
					{ rawKeyword: "$ralplan", reasons: ["not-leading-region"] },
				],
			},
			{
				...
				skills: [],
				candidates: [
					{ rawKeyword: "$autopilot", reasons: ["not-leading-region"] },
				],
			},
		] as const;
	...
	it("freezes classifications and retains ordered inert diagnostics", () => {
		...
		assert.deepEqual(classification.candidates[1]?.reasons, [
			"quote",
			"not-leading-region",
		]);

		const quotedFence = classifyKeywordInput("> ```\n> $ralph\n> ```");
		assert.deepEqual(quotedFence.candidates[0]?.reasons, [
			"fenced-code",
			"blockquote",
			"not-leading-region",
		]);
		...
		assert.deepEqual(blockquotedInline.candidates[0]?.reasons, [
			"blockquote",
			"inline-code",
			"not-leading-region",
		]);
		...
		assert.deepEqual(
			blockquotedInline.matches.map((match) => match.skill),
			["team"],
		);
	});
	...
	it("passes a supplied classification through recording, rejects mismatched text, and leaves rejected state bytes untouched", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-classification-state-"),
		);
		...
		const rawState =
			'{"version":1,"active":true,"skill":"autopilot","keyword":"$autopilot","phase":"deep-interview","activated_at":"2026-01-01T00:00:00.000Z","updated_at":"2026-01-01T00:00:00.000Z"}';
		const rawDetail =
			'{"active":true,"mode":"autopilot","current_phase":"deep-interview"}';
		try {
	...
	it("does not create or mutate tracked workflow state for ineligible explicit candidates", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-ineligible-explicit-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		const cases = [
			{ name: "no canonical state", text: "“$ralplan”", existingSkill: null },
			{
				name: "active Ralplan same skill",
				text: "without $ralplan",
				existingSkill: "ralplan",
			},
			{
				name: "active Autopilot same skill",
				text: "don't use $autopilot",
				existingSkill: "autopilot",
			},
			{
				name: "active Autopilot cross skill",
				text: "without $ralplan",
				existingSkill: "autopilot",
			},
		] as const;
		try {
			for (const [index, testCase] of cases.entries()) {
				...
				const rawState = testCase.existingSkill
					...
					: null;
				const rawDetail =
					testCase.existingSkill === "autopilot"
		...
		try {
			for (const [index, testCase] of cases.entries()) {
				...
				if (testCase.persists) {
					...
					assert.equal(
						existsSync(join(sessionDir, "autopilot-state.json")),
						true,
					);
				} else {
					assert.equal(result, null);
					assert.equal(await readFile(statePath, "utf-8"), rawState);
					assert.equal(
						existsSync(join(sessionDir, "ralplan-state.json")),
						false,
					);
				}
			}
	...
	it("keeps terminal marked answers from restarting stale active workflows", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-terminal-marked-answer-"),
		);
		const stateDir = join(cwd, ".omx", "state");
...
describe("autoresearch keyword detection", () => {
	...
	it("does not detect bare autoresearch phrasing without explicit $ invocation", () => {
		const match = detectPrimaryKeyword(
			"please use autoresearch workflow for this mission",
		);
		assert.equal(match, null);
	});
	...
	it("does not trigger autoresearch from incidental prose", () => {
		const match = detectPrimaryKeyword(
			"Karpathy did autoresearch before native hooks existed",
		);
		assert.equal(match, null);
	});
});
...
describe("explicit skill-name invocation requirement", () => {
	...
	it("detects explicit prometheus-strict invocation only", () => {
		const match = detectPrimaryKeyword(
			"please run $prometheus-strict before implementation",
		);
		assert.ok(match);
		assert.equal(match.skill, "prometheus-strict");
		assert.equal(match.keyword.toLowerCase(), "$prometheus-strict");
		assert.equal(
			detectPrimaryKeyword("please use prometheus-strict planning here"),
			null,
		);
	});
});
...
describe("keyword registry coverage", () => {
	...
	it("resolves immutable aliases without duplicate sources or skill collisions", () => {
		...
		assert.equal(
			new Set(EXPLICIT_SKILL_ALIASES.map((alias) => alias.source.toLowerCase()))
				.size,
		...
		for (const alias of EXPLICIT_SKILL_ALIASES) {
			...
			const canonicalSource = KEYWORD_TRIGGER_DEFINITIONS.find(
				(definition) =>
					definition.keyword.toLowerCase() === `$${alias.source.toLowerCase()}`,
			);
			if (canonicalSource)
...
describe("keyword detector skill-active-state lifecycle", () => {
	...
	it("fails fresh native Autopilot before deep-interview when receipt verification is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-preflight-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			const result = await recordSkillActivation({
	...
	it("byte-preserves an active native Autopilot session when receipt verification is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-active-autopilot-preflight-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		const rawMode =
			'{"active":true,"mode":"autopilot","current_phase":"ralplan","session_id":"sess-active-autopilot-preflight","metadata":{"preserve":true},"handoff_artifacts":{"ralplan":{"path":".omx/plans/existing.md"}}}\n';
		try {
			await mkdir(sessionDir, { recursive: true });
			await writeFile(statePath, rawState);
	...
	it("writes skill-active-state.json with deep-interview phase when autopilot keyword activates", async () => {
		...
		const codexHome = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-codex-home-"),
		);
		...
		try {
			...
			assert.equal(
				result.initialized_state_path,
				".omx/state/sessions/sess-1/autopilot-state.json",
			);
			...
			const sessionScopedSkillState = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-1", SKILL_ACTIVE_STATE_FILE),
					"utf-8",
				),
			) as {
			...
			assert.deepEqual(
				sessionScopedSkillState.active_skills,
			...
			const modeState = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-1", "autopilot-state.json"),
					"utf-8",
				),
			) as {
				mode: string;
			...
			assert.deepEqual(modeState.state.handoff_artifacts, {
				context_snapshot_path:
					".omx/context/please-run-and-keep-going-20260225T000000Z.md",
				context_snapshot: {
			...
			const snapshot = await readFile(
				join(
					...
					".omx",
					"context",
					"please-run-and-keep-going-20260225T000000Z.md",
				),
				"utf-8",
			);
			assert.match(
				snapshot,
				/activation prompt \/ task seed: please run \$autopilot and keep going/,
			);
			assert.match(
				snapshot,
	...
	it("does not advance the supervised child phase past an unsatisfied deep-interview gate via keyword handoff", async () => {
		...
		try {
			...
			const beforeAdvance = JSON.parse(
				await readFile(autopilotStatePath, "utf-8"),
			...
			assert.deepEqual(
				denied?.active_skills?.map((entry) => [entry.skill, entry.phase]),
				[["autopilot", "deep-interview"]],
			);
			const canonical = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-gate", SKILL_ACTIVE_STATE_FILE),
					"utf-8",
				),
			) as {
				phase?: string;
				active_skills?: Array<{ skill?: string; phase?: string }>;
			};
			assert.equal(canonical.phase, "deep-interview");
			assert.deepEqual(
				canonical.active_skills?.map((entry) => [entry.skill, entry.phase]),
				[["autopilot", "deep-interview"]],
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("does not let a keyword jump ahead past a planning gate (forward skip)", async () => {
		...
		try {
			...
			const autopilotState = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-skip", "autopilot-state.json"),
					"utf-8",
				),
			) as { current_phase: string };
			assert.equal(
				autopilotState.current_phase,
				"deep-interview",
				"forward skip must be held",
			);
	...
	it("does not advance or drift canonical state past an unsatisfied ralplan -> ultragoal gate", async () => {
		...
		try {
			...
			assert.equal(
				afterAdvance.current_phase,
				"ralplan",
				"keyword handoff must not skip the ralplan -> ultragoal gate",
			);
			...
			assert.equal(
				ralplanState.active,
				true,
				"blocked gate must not auto-complete ralplan state",
			);
			assert.equal(
				ralplanState.current_phase,
				"planning",
				"blocked gate must not advance ralplan state",
			);
			const canonical = JSON.parse(
				await readFile(
					join(stateDir, "sessions", sessionId, SKILL_ACTIVE_STATE_FILE),
	...
	it("does not let an implementation-phase keyword skip the code-review gate to ultraqa", async () => {
		...
		try {
			...
			assert.equal(
				afterAdvance.current_phase,
				"ultragoal",
				"keyword handoff must not skip the code-review gate",
			);
	...
	it("seeds dedicated planner routing in Autopilot state when main is cheap", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-planner-routing-"),
		);
		const stateDir = join(cwd, ".omx", "state");
	...
	it("migrates legacy Autopilot context snapshot paths into handoff artifacts", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-legacy-context-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		const sessionId = "sess-autopilot-legacy-context";
		try {
			await writeActiveAutopilotSkillState(
				stateDir,
				sessionId,
				"deep-interview",
			);
				...
				JSON.stringify(
					{
						...
						context_snapshot_path:
							".omx/context/legacy-task-20260529T000000Z.md",
			...
			await writeFile(
				join(cwd, ".omx", "context", "legacy-task-20260529T000000Z.md"),
				"# legacy task",
			);
			...
			assert.equal(
				existsSync(
					join(cwd, ".omx", "context", "continue-20260530T000000Z.md"),
				),
				false,
			);
		} finally {
	...
	it("rejects unsafe legacy Autopilot context snapshot paths without writing outside .omx/context", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-unsafe-context-"),
		);
		const stateDir = join(cwd, ".omx", "state");
	...
	it("does not snapshot bare continuation text when active Autopilot mode state is corrupt", async () => {
		...
		for (const fixture of [
			...
			const cwd = await mkdtemp(
				join(
					tmpdir(),
					`omx-keyword-autopilot-corrupt-continuation-${fixture}-`,
				),
			);
			const stateDir = join(cwd, ".omx", "state");
			...
			try {
				...
				await assertAutopilotRecoverySnapshot(
					cwd,
					JSON.parse(
						await readFile(modeStatePath, "utf-8"),
					) as TestAutopilotModeState,
	...
	it("rejects nested symlink Autopilot context snapshot candidates during reuse", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-nested-symlink-context-"),
		);
		const outside = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-nested-symlink-outside-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		try {
				...
				JSON.stringify(
					{
						...
						state: {
							handoff_artifacts: {
								context_snapshot_path: ".omx/context/link/exfil.md",
							},
						},
					},
					null,
			...
			await continueAutopilotTestState(
				stateDir,
				cwd,
				sessionId,
				"nested-symlink",
	...
	it("rejects typed canonical Autopilot recovery snapshot candidates during reuse", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-typed-recovery-context-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		try {
			await mkdir(join(cwd, ".omx", "context"), { recursive: true });
			await writeFile(
				join(cwd, ".omx", "context", "autopilot-recovery-20260529T000000Z.md"),
			...
			assert.equal(
				existsSync(
					join(
						...
						".omx",
						"context",
						"autopilot-recovery-20260529T000000Z.md",
					),
				),
				true,
			);
	...
	it("rejects oversized Autopilot context snapshot candidates during reuse", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-oversized-context-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		try {
			await mkdir(join(cwd, ".omx", "context"), { recursive: true });
			await writeFile(
				join(cwd, ".omx", "context", "oversized-legacy-20260529T000000Z.md"),
				...
				JSON.stringify(
					{
						...
						state: {
							handoff_artifacts: {
								context_snapshot_path:
									".omx/context/oversized-legacy-20260529T000000Z.md",
							},
						},
					},
					null,
	...
	it("does not promote degraded recovery snapshots to canonical context on reactivation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-recovery-reactivation-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		try {
				...
				JSON.stringify(
					{
						...
						state: {
							handoff_artifacts: {
								context_snapshot_path:
									".omx/context/autopilot-recovery-20260529T000000Z.md",
								context_snapshot: {
									path: ".omx/context/autopilot-recovery-20260529T000000Z.md",
									kind: "recovery",
									recovery: {
			...
			await continueAutopilotTestState(
				...
				"$autopilot implement the real task",
			);

			const modeState = await readAutopilotModeState(stateDir, sessionId);
			assert.equal(
				modeState.state?.handoff_artifacts?.context_snapshot_path,
				".omx/context/implement-the-real-task-20260530T000000Z.md",
			);
			assert.deepEqual(modeState.state?.handoff_artifacts?.context_snapshot, {
			...
			const snapshot = await readFile(
				join(
					...
					".omx",
					"context",
					"implement-the-real-task-20260530T000000Z.md",
				),
				"utf-8",
			);
			assert.match(
				snapshot,
				/activation prompt \/ task seed: \$autopilot implement the real task/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("does not follow symlinked Autopilot context directories when writing snapshots", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-symlink-context-"),
		);
		const outside = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-symlink-outside-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			...
			assert.equal(
				existsSync(join(outside, "symlink-escape-20260530T000000Z.md")),
				false,
			);
			assert.equal(
				existsSync(
					join(
						stateDir,
						"sessions",
						"sess-autopilot-symlink-context",
						"autopilot-state.json",
					),
				),
				false,
	...
	it("allocates unique Autopilot context snapshot paths for same-second matching slugs", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-context-collision-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
			...
			assert.equal(
				first.state?.handoff_artifacts?.context_snapshot_path,
				".omx/context/same-task-20260530T000000Z.md",
			);
			assert.equal(
				second.state?.handoff_artifacts?.context_snapshot_path,
				".omx/context/same-task-20260530T000000Z-2.md",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("fully resets terminal Autopilot mode state when reactivated", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-terminal-reset-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		try {
			...
			assert.equal(
				result.active_skills?.[0]?.activated_at,
			...
			assert.equal(
				skillState.active_skills?.[0]?.activated_at,
				"2026-05-30T00:00:00.000Z",
			);
			const modeState = JSON.parse(
				await readFile(
			...
			assert.deepEqual(modeState.state?.handoff_artifacts, {
				context_snapshot_path:
					".omx/context/investigate-the-next-issue-20260530T000000Z.md",
				context_snapshot: {
	...
	it("resets stopped Autopilot mode state when reactivated", async () => {
		for (const phase of ["stopped", "user-stopped"]) {
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-keyword-autopilot-${phase}-reset-`),
			);
			const stateDir = join(cwd, ".omx", "state");
					...
					JSON.stringify(
						{
							...
							review_cycle: 3,
							state: {
								handoff_artifacts: { code_review: { verdict: "stale" } },
							},
						},
						null,
						2,
	...
	it("adds approved workflow overlaps without deleting the existing canonical state", async () => {
		...
		try {
			...
			const persisted = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-overlap", SKILL_ACTIVE_STATE_FILE),
					"utf-8",
				),
	...
	it("keeps a session-scoped Ralph activation out of the root canonical state for other sessions", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-ralph-isolation-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
			...
			const sessionScopedSkillState = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-ralph-a", SKILL_ACTIVE_STATE_FILE),
					"utf-8",
				),
	...
	it("hard-fails denied workflow overlaps without mutating current state", async () => {
		...
		try {
			...
			assert.match(
				String(denied?.transition_error),
				/Unsupported workflow overlap: team \+ autopilot\./,
			);
			assert.match(
				String(denied?.transition_error),
				/`omx state clear --input '{"mode":"<mode>"}' --json`/,
			);
			assert.match(
				String(denied?.transition_error),
			...
			const persisted = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-deny", SKILL_ACTIVE_STATE_FILE),
					"utf-8",
				),
			) as { active_skills?: Array<{ skill: string }> };
			assert.deepEqual(
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", "sess-deny", "autopilot-state.json"),
				),
				false,
			);
		} finally {
	...
	it("preserves an active Autopilot mode when canonical skill state is missing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autopilot-mode-drift-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		try {
			await mkdir(join(stateDir, "sessions", sessionId), { recursive: true });
			const rawMode =
				'{"active":true,"mode":"autopilot","current_phase":"ralplan","session_id":"sess-autopilot-mode-drift","marker":"preserve-active-mode","metadata":{"nested":{"keep":true}}}\n';
			await writeFile(modePath, rawMode);
	...
	it("preserves active standalone Ralplan state bytes when fresh Autopilot preflight is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-ralplan-autopilot-preflight-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		const rawState =
			'{"version":1,"active":true,"skill":"ralplan","keyword":"$ralplan","phase":"planning","session_id":"sess-ralplan-autopilot-preflight","active_skills":[{"skill":"ralplan","phase":"planning","active":true,"session_id":"sess-ralplan-autopilot-preflight"}]}';
		try {
			await mkdir(join(stateDir, "sessions", sessionId), { recursive: true });
			await writeFile(statePath, rawState);
	...
	it("denies prompt-submit overlaps against the current session-visible canonical state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-session-visible-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(stateDir, "sessions", "sess-visible"), {
				recursive: true,
			});
			await writeFile(
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", "sess-visible", "ultrawork-state.json"),
				),
				true,
			);
	...
	it("activates ultrawork mode from the Korean keyboard typo for ulw", async () => {
		...
		try {
			...
			assert.equal(
				result.initialized_state_path,
				".omx/state/sessions/sess-ulw-ko/ultrawork-state.json",
			);

			const modeState = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-ulw-ko", "ultrawork-state.json"),
					"utf-8",
				),
	...
	it("seeds executing state for autoresearch prompt-submit activation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-autoresearch-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
			...
			assert.equal(
				result.initialized_state_path,
				".omx/state/sessions/sess-autoresearch/autoresearch-state.json",
			);
	...
	it("preserves the planning skill when ralplan and autoresearch are invoked together", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-autoresearch-planning-precedence-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
	...
	it("captures tmux_pane_id in seeded ralplan prompt-submit state when TMUX_PANE is present", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-ralplan-pane-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		try {
			...
			const modeState = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-ralplan-pane", "ralplan-state.json"),
					"utf-8",
				),
	...
	it("captures tmux_pane_id in deep-interview prompt-submit state when TMUX_PANE is present", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-deep-interview-pane-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		const previousPane = process.env.TMUX_PANE;
		try {
	...
	it("preserves an existing deep-interview tmux_pane_id when prompt-submit re-seeds state without TMUX_PANE", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-deep-interview-preserve-pane-"),
		);
		const stateDir = join(cwd, ".omx", "state");
	...
	it("seeds first-class state for ralplan prompt-submit activation", async () => {
		...
		try {
			...
			assert.equal(
				result.initialized_state_path,
				".omx/state/sessions/sess-ralplan/ralplan-state.json",
			);

			const modeState = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-ralplan", "ralplan-state.json"),
					"utf-8",
				),
			) as { mode: string; active: boolean; current_phase: string };
	...
	it("does not reactivate a neutralized routing-only Ralplan seed on plain continuation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-neutralized-ralplan-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		const sessionId = "sess-neutralized";
		...
		try {
			...
			const skillPath = join(sessionDir, SKILL_ACTIVE_STATE_FILE);
			const [ralplanBefore, skillBefore] = await Promise.all([
				readFile(ralplanPath),
				readFile(skillPath),
			]);
			...
			assert.deepEqual(await readFile(ralplanPath), ralplanBefore);
			assert.deepEqual(await readFile(skillPath), skillBefore);
	...
	it("auto-completes deep-interview during allowlisted forward handoff", async () => {
		...
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(stateDir, "sessions", "sess-handoff"), {
				recursive: true,
			});
			await writeFile(
			...
			assert.equal(
				result?.initialized_state_path,
				".omx/state/sessions/sess-handoff/ultragoal-state.json",
			);
			assert.equal(
			...
			const ultragoal = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-handoff", "ultragoal-state.json"),
					"utf-8",
				),
	...
	it("denies ralplan handoff from deep-interview without completion or explicit skip evidence", async () => {
		...
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(stateDir, "sessions", "sess-ralplan-handoff"), {
				recursive: true,
			});
			await writeFile(
				join(
			...
			assert.match(
				...
				/missing deep-interview completion\/skip gate/i,
			);
	...
	it("allows ralplan handoff from deep-interview with a durable completion gate", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-ralplan-handoff-complete-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(stateDir, "sessions", "sess-ralplan-handoff-complete"), {
				recursive: true,
			});
			await writeFile(
				join(
	...
	it("preserves the planning skill when planning and execution workflows are invoked together", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-planning-precedence-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", "sess-multi", "ralplan-state.json"),
				),
				true,
			);
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", "sess-multi", "ralph-state.json"),
				),
				false,
			);
		} finally {
	...
	it("lets planning win even when execution appears first in the contiguous skill block", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-planning-beats-execution-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", "sess-priority", "ralplan-state.json"),
				),
				true,
			);
			assert.equal(
				existsSync(
					join(stateDir, "sessions", "sess-priority", "ralph-state.json"),
				),
				false,
			);
		} finally {
	...
	it("does not activate team state when persisted Team mode is disabled", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-team-disabled-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(cwd, ".omx"), { recursive: true });
	...
	it("ignores disabled Team when selecting the primary workflow", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-team-disabled-primary-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(cwd, ".omx"), { recursive: true });
	...
	it("filters deferred team handoffs when persisted Team mode is disabled", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-team-disabled-deferred-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(cwd, ".omx"), { recursive: true });
	...
	it("preserves active team root state when $team is re-entered from prompt routing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-team-preserve-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
	...
	it("preserves active team root state when planning follow-up defers a simultaneous $team re-entry", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-team-planning-followup-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", "sess-team-followup", "team-state.json"),
				),
				false,
			);
		} finally {
	...
	it("keeps a locally approved ralplan active when the host consensus receipt verifier is unavailable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-ralplan-ultragoal-handoff-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(stateDir, "sessions", "sess-ralplan-ultragoal"), {
				recursive: true,
			});
			await writeFile(
				join(
				...
				JSON.stringify(
					{
						...
						ralplan_consensus_gate: {
							documented_host_consensus_receipt: {
								issuer: "official-host",
								verdict: "approve",
							},
							complete: true,
							sequence: ["architect-review", "critic-review"],
			...
			assert.notEqual(
				result?.transition_message,
				"mode transiting: ralplan -> ultragoal",
			);
	...
	it("acquires a deep-interview input lock immediately on activation", async () => {
		...
		try {
			...
			const modeState = JSON.parse(
				await readFile(join(stateDir, DEEP_INTERVIEW_STATE_FILE), "utf-8"),
			) as {
				mode: string;
				active: boolean;
				current_phase: string;
	...
	it("persists repo-local deep-interview config values into activation and mode state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-deep-interview-config-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(cwd, ".omx"), { recursive: true });
...
`,
			...
			assert.equal(
				result.initialized_state_path,
				".omx/state/sessions/sess-deep-interview-config/deep-interview-state.json",
			);
			...
			assert.equal(
				modeState.deep_interview_config?.sourcePath,
				join(cwd, ".omx", "config.toml"),
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("persists deep-interview config when mixed workflow prompts defer execution modes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-deep-interview-config-mixed-"),
		);
		const stateDir = join(cwd, ".omx", "state");
...
`,
			...
			const modeState = JSON.parse(
				await readFile(
					join(stateDir, "sessions", sessionId, DEEP_INTERVIEW_STATE_FILE),
					"utf-8",
				),
			) as {
				profile?: string;
	...
	it("shows before-after state change when deep-interview config is added at runtime", async () => {
		await withIsolatedHome("deep-interview-config-before-after", async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-keyword-state-deep-interview-config-before-after-"),
			);
			const stateDir = join(cwd, ".omx", "state");
			...
			try {
				...
				const beforeModeState = JSON.parse(
					await readFile(statePath, "utf-8"),
				) as {
...
`,
				...
				const afterModeState = JSON.parse(
					await readFile(statePath, "utf-8"),
				) as {
					deep_interview_config?: {
						profile?: string;
	...
	it("preserves deep-interview config values during continuation prompts", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-deep-interview-config-continuation-"),
		);
		const stateDir = join(cwd, ".omx", "state");
...
`,
	...
	it("preserves explicit deep-interview profile flags during continuation prompts", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-keyword-state-deep-interview-config-profile-continuation-",
			),
		);
		const stateDir = join(cwd, ".omx", "state");
...
`,
	...
	it("keeps the documented deep-interview Suggested Config executable through activation state", async () => {
		...
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-deep-interview-doc-config-"),
		);
		const stateDir = join(cwd, ".omx", "state");
	...
	it("keeps deep-interview activation alive when repo config TOML is malformed", async () => {
		await withIsolatedHome("deep-interview-malformed-config", async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-keyword-state-deep-interview-malformed-config-"),
			);
			const stateDir = join(cwd, ".omx", "state");
				...
				const modeState = JSON.parse(
					await readFile(
						join(
							stateDir,
							"sessions",
							"sess-deep-interview-malformed-config",
							DEEP_INTERVIEW_STATE_FILE,
						),
						"utf-8",
					),
	...
	it("creates the session-scoped deep-interview state directory before persisting mode state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-deep-interview-session-dir-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
			...
			const modeState = JSON.parse(
				await readFile(
					join(stateDir, "sessions", "sess-sync", DEEP_INTERVIEW_STATE_FILE),
					"utf-8",
				),
	...
	it("clears stale pending deep-interview question enforcement when deep-interview is reactivated", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-deep-interview-reactivation-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(stateDir, "sessions", "sess-reactivate"), {
				recursive: true,
			});
			await writeFile(
				join(
	...
	it("releases the deep-interview input lock on abort via cancel keyword", async () => {
		...
		try {
			...
			assert.equal(
				modeState.input_lock?.released_at,
				"2026-02-25T00:05:00.000Z",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("does not seed non-stateful skill mode state on keyword activation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-non-stateful-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
	...
	it("keeps Autopilot visible and advances HUD phase when a supervised code-review child keyword appears", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-autopilot-child-code-review-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		try {
			...
			assert.deepEqual(
				persisted.active_skills?.map((entry) => [entry.skill, entry.phase]),
				[["autopilot", "code-review"]],
			);
			assert.equal(
				existsSync(
					join(stateDir, "sessions", sessionId, "code-review-state.json"),
				),
				false,
			);
	...
	it("repairs stale inactive Autopilot detail when a supervised code-review child keyword appears", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-autopilot-child-stale-detail-"),
		);
		const stateDir = join(cwd, ".omx", "state");
	...
	it("keeps tracked Autopilot child keywords supervised and completes stale child mode state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-autopilot-child-ultraqa-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		try {
			...
			assert.equal(result.transition_error, undefined);
			assert.equal(
				existsSync(join(stateDir, "sessions", sessionId, "ultraqa-state.json")),
				false,
			);
			const ultragoal = JSON.parse(
				await readFile(
			...
			assert.match(
				ultragoal.auto_completed_reason || "",
				/mode transiting: ultragoal -> ultraqa/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("denies supervised Autopilot child rollback without clearing stale execution state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-autopilot-child-rollback-"),
		);
		const stateDir = join(cwd, ".omx", "state");
	...
	it("surfaces supervised Autopilot deep-interview to ralplan gate failures", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-autopilot-child-gate-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		try {
			...
			assert.equal(result?.supervised_child_skill, undefined);
			assert.equal(
				existsSync(join(stateDir, "sessions", sessionId, "ralplan-state.json")),
				false,
			);
	...
	it("ignores stale root child mode state during session-scoped Autopilot child reconciliation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-autopilot-child-session-root-"),
		);
		const stateDir = join(cwd, ".omx", "state");
	...
	it("warns when fresh Autopilot preflight persistence fails", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-persist-fail-"),
		);
		const warnings: unknown[][] = [];
		mock.method(console, "warn", (...args: unknown[]) => {
			warnings.push(args);
	...
	it("preserves seeded mode progress for same-skill continuation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-seed-continuation-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
			await mkdir(join(stateDir, "sessions", "sess-autopilot"), {
				recursive: true,
			});
			await writeFile(
			...
			await mkdir(join(cwd, ".omx", "context"), { recursive: true });
			await writeFile(
				join(cwd, ".omx", "context", "existing.md"),
				"# existing context",
			);
			...
			const modeState = JSON.parse(
				...
				state?: {
					context_snapshot_path?: string;
					handoff_artifacts?: { context_snapshot_path?: string };
				};
			};
	...
	it("does not persist Ralph workflow state for a plain conversational mention", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-ralph-plain-text-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", "sess-plain-ralph", "ralph-state.json"),
				),
				false,
			);
		} finally {
	...
	it("preserves Ralph iteration counters for same-skill continuation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-ralph-continuation-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		const statePath = join(stateDir, SKILL_ACTIVE_STATE_FILE);
		try {
	...
	it("keeps Korean ulw typo first in mixed explicit workflow persistence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-ulw-ko-mixed-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
	...
	it("lets an explicit Korean ulw typo override an active workflow continuation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-ulw-ko-explicit-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(stateDir, "sessions", "sess-ulw-ko-explicit"), {
				recursive: true,
			});
			await writeFile(
				join(
	...
	it("routes bare keep-going continuation to the active autopilot skill instead of generic ralph continuation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-autopilot-bare-continuation-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(stateDir, "sessions", "sess-autopilot-bare"), {
				recursive: true,
			});
			await writeFile(
				join(
			...
			await mkdir(join(cwd, ".omx", "context"), { recursive: true });
			await writeFile(
				join(cwd, ".omx", "context", "autopilot.md"),
				"# autopilot context",
			);
			...
			assert.equal(
				modeState.state?.handoff_artifacts?.context_snapshot_path,
				".omx/context/autopilot.md",
			);
			assert.equal(
				existsSync(
					join(stateDir, "sessions", "sess-autopilot-bare", "ralph-state.json"),
				),
				false,
			);
		} finally {
	...
	it("preserves active Autopilot question-wait state on bare continuation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-autopilot-question-wait-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		...
		try {
			...
			const modeState = JSON.parse(
				...
				lifecycle_outcome?: string;
				state?: {
					deep_interview_question?: { obligation_id?: string; status?: string };
				};
			};
			...
			assert.equal(
				modeState.state?.deep_interview_question?.status,
				"waiting_for_user",
			);
			assert.equal(
				modeState.state?.deep_interview_question?.obligation_id,
				"obligation-question-wait",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("resets terminal Ralph blocked_on_user state when reactivated", async () => {
		...
		for (const testCase of cases) {
			const cwd = await mkdtemp(
				join(
					tmpdir(),
					`omx-keyword-state-ralph-terminal-${testCase.name}-reactivation-`,
				),
			);
			const stateDir = join(cwd, ".omx", "state");
					...
					JSON.stringify(
						{
							...
							iteration: 50,
							max_iterations: 50,
							...(testCase.run_outcome
								? { run_outcome: testCase.run_outcome }
								: {}),
						},
						null,
						2,
	...
	it("routes bare keep-going continuation to the active ralph skill instead of resetting through generic keep-going detection", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-ralph-bare-continuation-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(stateDir, "sessions", "sess-ralph-bare"), {
				recursive: true,
			});
			await writeFile(
	...
	it("does not reuse active workflow continuation when prompt contains an unknown plugin-prefixed explicit token", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-unknown-prefixed-explicit-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(join(stateDir, "sessions", "sess-unknown-prefixed"), {
				recursive: true,
			});
			await writeFile(
				join(
	...
	it("does not continue a workflow from another session root canonical entry", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-cross-session-continue-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", "sess-b", SKILL_ACTIVE_STATE_FILE),
				),
				false,
			);
	...
	it("denies switching away from a standalone workflow without explicit clear", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-skill-switch-deny-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		const statePath = join(stateDir, SKILL_ACTIVE_STATE_FILE);
		try {
...
describe("applyRalplanGate", () => {
	it("gates short team follow-up when only PRD/test-spec artifacts exist", async () => {
		...
		try {
			...
			await writeFile(
				join(plansDir, "test-spec-issue-831.md"),
				"# Test spec\n",
			);
	...
	it("keeps native-proof execution follow-ups gated when consensus is artifact-only", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-gate-native-required-"),
		);
		try {
			const plansDir = join(cwd, ".omx", "plans");
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeFile(
				join(plansDir, "test-spec-issue-833.md"),
				"# Test spec\n",
			);
			await writeFile(
				join(stateDir, "ralplan-state.json"),
			...
			const result = applyRalplanGate(["team"], "team", {
				cwd,
	...
	it("keeps short approved ralph follow-ups gated without an official host consensus receipt", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-gate-followup-ralph-"),
		);
		try {
			const plansDir = join(cwd, ".omx", "plans");
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeFile(
				join(plansDir, "test-spec-issue-832.md"),
				"# Test spec\n",
			);
			await writeFile(
				join(stateDir, "ralplan-state.json"),
	...
	it("ignores ambient OMX_ROOT consensus state for local PRD/test-spec-only follow-up gating", async () => {
		...
		const ambientRoot = await mkdtemp(
			join(tmpdir(), "omx-keyword-gate-ambient-"),
		);
		const previousOmxRoot = process.env.OMX_ROOT;
		try {
			const plansDir = join(cwd, ".omx", "plans");
			...
			await writeFile(
				join(ambientStateDir, "ralplan-state.json"),
				JSON.stringify({
					current_phase: "complete",
					planning_complete: true,
					ralplan_consensus_gate: {
...
describe("recordSkillActivation prompt provenance", () => {
				...
				active_skills: [
					{
						skill: "ralph",
						session_id: "target",
						owner_codex_session_id: "foreign",
					},
				],
			});
	...
	it("rejects malformed target state before direct activation writes", async () => {
		const stateDir = await mkdtemp(
			join(tmpdir(), "omx-keyword-malformed-owner-"),
		);
		try {
			const targetDir = join(stateDir, "sessions", "target");
			await mkdir(targetDir, { recursive: true });
	...
	it("rejects target enumeration failures before direct activation writes", async () => {
		const stateDir = await mkdtemp(
			join(tmpdir(), "omx-keyword-enumeration-owner-"),
		);
		try {
