...
describe("tmux-hook-engine declaration sync", () => {
	it("covers all runtime exports in src/types/tmux-hook-engine.d.ts", async () => {
		...
		const runtimeSource = await readFile(
			join(root, "dist", "scripts", "tmux-hook-engine.js"),
			"utf-8",
		);
		const declSource = await readFile(
			join(root, "src", "types", "tmux-hook-engine.d.ts"),
			"utf-8",
		);
		...
		const missing = [...runtimeExports].filter(
			(name) => !declaredExports.has(name),
		);
		assert.deepEqual(
			missing,
			[],
			`Missing declaration exports: ${missing.join(", ")}`,
		);
	});
});
