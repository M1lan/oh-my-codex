...
function assertFreshExactProofBeforePaneEffects(
	tmuxLog: string,
	paneId: string,
): void {
	...
	const effects = commands
		...
		.filter(
			({ command }) =>
				...
				(command.startsWith(`send-keys -t ${paneId} `) &&
					!command.includes(" -l ")),
		);

	assert.ok(
		effects.length > 0,
		`expected explicit pane effects for ${paneId}:\n${commands.join("\n")}`,
	);
	for (const { command, index } of effects) {
		assert.match(
			commands[index - 1] ?? "",
			exactGlobalPaneProof,
			`final PID proof must immediately precede ${command}`,
		);
		assert.match(
			commands[index - 2] ?? "",
			ownerProof,
			`owner proof must immediately precede the final PID proof for ${command}`,
		);
		assert.match(
			commands[index - 3] ?? "",
			exactGlobalPaneProof,
			`initial PID proof must precede owner proof for ${command}`,
		);
	}
}

function bindCanonicalTeamPaneAuthority(
	config: { tmux_pane_owner_id?: string; hud_pane_id?: string | null },
	hudPaneId = "%88",
): void {
...
async function listDispatchProcessingLeases(
	cwd: string,
	teamName: string,
): Promise<string[]> {
...
async function waitForMailboxNotifiedAt(
	teamName: string,
	workerName: string,
	messageId: string,
	cwd: string,
...
describe("notify-hook team dispatch consumer", () => {
	...
	it("marks pending request as notified and preserves mailbox notified_at semantics", async () => {
		...
		try {
			...
			const modulePath = new URL(
				"../../../dist/scripts/notify-hook/team-dispatch.js",
				import.meta.url,
			).pathname;
			...
			assert.ok(
				request?.notified_at,
				"expected dispatch state to record notified_at",
			);
			const mailbox = await listMailboxMessages("alpha", "worker-1", cwd);
			const mailboxMessage = mailbox.find(
				(entry) => entry.message_id === msg.message_id,
			);
			assert.ok(
				mailboxMessage,
				"expected the queued mailbox message to remain readable",
			);
			const notifiedAt = await waitForMailboxNotifiedAt(
				"alpha",
				"worker-1",
				msg.message_id,
				cwd,
			...
			assert.ok(
				...
				"expected dispatch state or mailbox shadow to record notified_at",
			);
		} finally {
	...
	it("leader-fixed dispatch remains pending with leader_pane_missing_deferred when pane missing", async () => {
		...
		try {
			...
			assert.ok(
				deferred,
				"expected leader_notification_deferred event for missing leader pane",
			);
			...
			const dispatchLogs = (await readFile(dispatchLogPath, "utf-8"))
				.trim()
				.split("\n")
				.filter(Boolean)
				.map((line) => JSON.parse(line));
			const deferredLog = dispatchLogs.find(
				(entry: { type?: string; request_id?: string }) =>
					entry.type === "dispatch_deferred" &&
	...
	it("does not duplicate deferred leader artifacts across repeated drain ticks", async () => {
		...
		try {
			...
			const deferredEvents = events.filter(
				(event: { type?: string; request_id?: string }) =>
					event.type === "leader_notification_deferred" &&
					event.request_id === queued.request.request_id,
			);
			assert.equal(
				...
				1,
				"should only write one deferred event per missing-pane request until state changes",
			);
			...
			const dispatchLogs = (await readFile(dispatchLogPath, "utf-8"))
				...
				.filter(Boolean)
				.map((line) => JSON.parse(line));
			const deferredLogs = dispatchLogs.filter(
				(entry: { type?: string; request_id?: string }) =>
					entry.type === "dispatch_deferred" &&
					entry.request_id === queued.request.request_id,
			);
			assert.equal(
				deferredLogs.length,
				1,
				"should only log one dispatch_deferred artifact per missing-pane request until state changes",
			);

			const deliveryLog = await readTeamDeliveryLog(cwd);
			const deferredDeliveryLogs = deliveryLog.filter(
				(entry) =>
					entry.event === "dispatch_result" &&
					entry.source === "notify-hook.team-dispatch" &&
					entry.request_id === queued.request.request_id &&
			...
			assert.equal(
				deferredDeliveryLogs.length,
				1,
				"should only log one deferred team-delivery artifact per missing-pane request until state changes",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("logs structured evidence when bridge dispatch compat cannot be parsed and legacy requests are used", async () => {
		...
		try {
			...
			assert.equal(
				result.processed,
				1,
				"legacy request fallback must still process the pending request",
			);
			const request = await readDispatchRequest(
				"alpha",
				queued.request.request_id,
			...
			const dispatchLogs = (await readFile(dispatchLogPath, "utf-8"))
				...
				.filter(Boolean)
				.map((line) => JSON.parse(line));
			const fallback = dispatchLogs.find(
				(entry: {
					type?: string;
					fallback?: boolean;
					bridge_operation?: string;
				}) =>
					entry.type === "bridge_fallback" &&
					entry.bridge_operation === "readBridgeDispatchRequests" &&
					entry.fallback === true,
			);
			assert.ok(
				fallback,
				"expected structured bridge fallback evidence in dispatch log",
			);
...
`,
			...
			assert.equal(
				result.processed,
				1,
				"JS fallback path must still finish the dispatch",
			);
			const request = await readDispatchRequest(
				"alpha",
				queued.request.request_id,
			...
			const fallback = dispatchLogs.find(
				(entry: {
					...
					entry.type === "bridge_fallback" &&
					entry.bridge_operation === "runtimeExec" &&
					entry.request_id === queued.request.request_id,
			);
			assert.ok(
				fallback,
				"expected structured runtimeExec fallback evidence in dispatch log",
			);
		...
		try {
			...
			await enqueueDispatchRequest(
				"alpha",
				{
					...
					message_id: msg.message_id,
					trigger_message:
						"Read .omx/state/team/alpha/mailbox/leader-fixed.json; worker-1 sent a new message. Review it and decide the next concrete step.",
				},
				cwd,
			);
			...
			assert.match(
				tmuxLog,
				/mailbox\/leader-fixed\.json; worker-1 sent a new message/,
			);
	...
	it("fails closed without a canonical PID for explicit worker and leader panes", async () => {
		...
		try {
			...
			const worker = await enqueueDispatchRequest(
				"alpha",
				{
					kind: "inbox",
					to_worker: "worker-1",
					worker_index: 1,
					trigger_message: "do not send",
				},
				cwd,
			);
			const leader = await enqueueDispatchRequest(
				"alpha",
				{
					kind: "nudge",
					to_worker: "leader-fixed",
					trigger_message: "leader do not send",
				},
				cwd,
			);
			const modulePath = new URL(
			...
			assert.equal(
				(await readDispatchRequest("alpha", worker.request.request_id, cwd))
					?.last_reason,
				"missing_exact_pane_pid",
			);
			assert.equal(
				(await readDispatchRequest("alpha", leader.request.request_id, cwd))
					?.last_reason,
				"missing_exact_pane_pid",
			);
		} finally {
	...
	it("does not send when a guarded worker pane is reused before input effects", async () => {
		...
		try {
			...
			assert.equal(
				(await readDispatchRequest("alpha", queued.request.request_id, cwd))
					?.last_reason,
				"exact_pane_unavailable",
			);
			const tmuxLog = await readFile(tmuxLogPath, "utf8");
			assert.doesNotMatch(
				tmuxLog,
				/(?:paste-buffer|send-keys) -t %42/,
				"replacement pane must receive no input effect",
			);
		} finally {
			if (typeof previousPath === "string") process.env.PATH = previousPath;
			else delete process.env.PATH;
...
`;
			...
			assert.doesNotMatch(
				tmuxLog,
				/display-message -p -t %91 #\{pane_id\}/,
				"configured leader pane must bypass the generic resolver",
			);

			cfg.hud_pane_id = "%91";
			await saveTeamConfig(cfg, cwd);
			const hudMsg = await sendDirectMessage(
				"alpha",
				"worker-1",
				"leader-fixed",
				"second leader message",
				cwd,
			);
			const hudQueued = await enqueueDispatchRequest(
				"alpha",
				{
					kind: "mailbox",
					to_worker: "leader-fixed",
			...
			const hudResult = await mod.drainPendingTeamDispatch({
				cwd,
				maxPerTick: 5,
			});
			assert.equal(hudResult.failed, 1);
			const hudRequest = await readDispatchRequest(
				"alpha",
				hudQueued.request.request_id,
				cwd,
			);
...
`,
			...
			assert.equal(
				mailboxMessage?.notified_at,
				undefined,
				"guard failure should not mark leader mailbox message notified",
			);
			...
			assert.doesNotMatch(
				tmuxLog,
				/send-keys -t %77/,
				"copy-mode leader pane must not receive injected keys",
			);
		} finally {
			if (typeof prevPath === "string") process.env.PATH = prevPath;
			else delete process.env.PATH;
	...
	it("reserves per-issue cooldown before releasing the dispatch lock to a concurrent drain", async () => {
		const cwd = await mkdtemp(join(tmpdir(), "omx-hook-team-dispatch-"));
		const previousIssueCooldown =
			process.env.OMX_TEAM_DISPATCH_ISSUE_COOLDOWN_MS;
		...
		try {
			...
			assert.equal(
				injectCount,
				1,
				"concurrent drain must not inject same-issue follow-up while first claim is in flight",
			);
			assert.equal(firstResult.processed, 1);
			assert.equal(concurrentDrain.processed, 0);
			assert.ok(concurrentDrain.skipped >= 1);

			const firstRequest = await readDispatchRequest(
				"alpha",
				first.request.request_id,
				cwd,
			);
			const secondRequest = await readDispatchRequest(
				"alpha",
	...
	it("releases every preclaimed dispatch lease when a claimed injector throws", async () => {
		...
		try {
			...
			assert.equal(
				retry.processed,
				2,
				"later drain should not be blocked by stale preclaimed leases",
			);
	...
	it("leaves unconfirmed injection as pending for retry (#391)", async () => {
		...
		try {
			...
			assert.equal(
				result.processed,
				0,
				"unconfirmed should not count as processed",
			);
	...
	it("marks unconfirmed as failed after max attempts (#391)", async () => {
		...
		try {
			...
			assert.equal(
				result.processed,
				1,
				"should transition to failed on 3rd attempt",
			);
			assert.equal(result.failed, 1);
			const request = await readDispatchRequest(
				"alpha",
	...
	it("keeps retry_pending derived-only and does not persist transient tags", async () => {
		...
		try {
			...
			const persisted = rawRequests.find(
				(entry: { request_id?: string }) =>
					entry?.request_id === queued.request.request_id,
			);
			assert.ok(persisted);
			assert.equal(persisted.status, "pending");
			assert.ok(
				!("retry_mode" in persisted),
				"retry_mode must not be persisted",
			);
			assert.ok(!("retry_tag" in persisted), "retry_tag must not be persisted");
			assert.ok(
				!("status_tag" in persisted),
				"status_tag must not be persisted",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("retries submit with isolated C-m and does not retype when trigger already present", async () => {
		...
		try {
			...
			assert.ok(
				!/send-keys[^\n]*-l[^\n]*C-m/.test(tmuxLog),
				"must not mix -l payload with C-m submit",
			);
	...
	it("does not confirm when narrow misses but wide tail still has unsent trigger", async () => {
		...
		try {
			...
			assert.equal(
				result.processed,
				0,
				"must not mark notified when wide tail still shows trigger",
			);
			assert.ok(result.skipped >= 1);
	...
	it("applies per-issue cooldown to avoid repeated reinjection in one drain tick", async () => {
		...
		try {
			...
			const firstReq = await readDispatchRequest(
				"alpha",
				first.request.request_id,
				cwd,
			);
			const secondReq = await readDispatchRequest(
				"alpha",
				second.request.request_id,
				cwd,
			);
	...
	it("skips repeated same-issue reinjection during per-issue cooldown window", async () => {
		...
		try {
			...
			const firstTick = await mod.drainPendingTeamDispatch({
				cwd,
				maxPerTick: 5,
				injector,
			});
			const secondTick = await mod.drainPendingTeamDispatch({
				cwd,
				maxPerTick: 5,
				injector,
			});
			...
			assert.equal(
				injectCount,
				1,
				"same issue should not be reinjected while cooldown is active",
			);
			...
			assert.equal(secondRequest?.status, "pending");
			assert.equal(
				secondRequest?.attempt_count,
				0,
				"cooldown-blocked request should remain untouched",
			);
		} finally {
	...
	it("rejects a session-only dispatch target without an exact Team pane binding", async () => {
		...
		try {
			...
			const request = requests.find(
				(entry: { to_worker?: string }) => entry.to_worker === "worker-1",
			);
			assert.equal(request?.status, "failed");
		...
		for (const [name, snapshot] of scenarios) {
			...
			try {
				...
				assert.match(
					commands[0] || "",
					/^list-panes -a(?:\s|$)/,
					`${name} must prove the global pane first`,
				);
				assert.doesNotMatch(
					tmuxLog,
					/display-message|capture-pane|send-keys|paste-buffer/,
				);
				assert.doesNotMatch(
					tmuxLog,
					/list-panes -t /,
					`${name} must not use the session resolver fallback`,
				);
				const dispatchLogPath = join(
					cwd,
					".omx",
				...
				const dispatchLog = (await readFile(dispatchLogPath, "utf8"))
					...
					.filter(Boolean)
					.map((line) => JSON.parse(line));
				const failure = dispatchLog.find(
					(entry) =>
						entry.request_id === queued.request.request_id &&
						entry.type === "dispatch_failed",
				);
				assert.equal(
					failure?.exact_pane_proof?.status,
					name === "unavailable" ? "unavailable" : "gone",
				);
			} finally {
				if (typeof previousPath === "string") process.env.PATH = previousPath;
				else delete process.env.PATH;
	...
	it("rejects a recycled configured worker pane before dispatch effects", async () => {
		...
		try {
			...
			assert.doesNotMatch(
				tmuxLog,
				/display-message|capture-pane|send-keys|paste-buffer|set-buffer/,
			);
			const dispatchLogPath = join(
				cwd,
				".omx",
	...
	it("stops explicit-pane dispatch after a later exact proof is lost", async () => {
		...
		try {
			...
			assert.match(
				tmuxLog,
				/send-keys -t %42 C-u/,
				"the live proof should permit the clear effect",
			);
			assert.doesNotMatch(tmuxLog, /send-keys -t %42 C-m/);
			const ownerProof = "show-option -qv -p -t %42 @omx_team_pane_owner_id";
			for (const [index, command] of commands.entries()) {
				if (
					!/^(send-keys|paste-buffer)\b/.test(command) ||
					!command.includes("-t %42") ||
					/^send-keys\b.*\s-l\s/.test(command)
				)
					continue;
				assert.match(
					commands[index - 1] || "",
				...
				assert.equal(
					commands[index - 2],
					ownerProof,
					`owner proof must precede final PID proof for ${command}`,
				);
				assert.match(
					commands[index - 3] || "",
					exactGlobalPaneProof,
			...
			const lastProof = commands
				.map((command, index) =>
					exactGlobalPaneProof.test(command) ? index : -1,
				)
			...
			assert.equal(
				commandsAfterLostProof.length,
				1,
				"only non-pane buffer cleanup may follow proof loss",
			);
