...
describe("prompt guidance contract", () => {
	...
	it("tracked AGENTS surfaces lock agent-owned reversible OMX/runtime actions", () => {
		for (const surface of listTrackedAgentSurfaces()) {
			...
			assert.match(
				content,
				/Do not ask or instruct humans to perform ordinary non-destructive, reversible actions/i,
			);
			assert.match(
				content,
				/Treat OMX runtime manipulation, state transitions, and ordinary command execution as agent responsibilities/i,
			);
			assert.doesNotMatch(
				content,
	...
	it("keeps native children non-writing while a Conductor workflow is active", () => {
		...
		assert.match(
			content,
			/Conductor workflow is active.*native children are verification\/advice-only/i,
		);
		assert.match(
			content,
			/child-to-leader reporting also requires separate host-authenticated caller, parent, and target proof/i,
		);
		assert.match(
			content,
			/Codex 0\.145\.0 does not expose that proof.*collaboration reporting and source\/product mutations remain denied/i,
		);
		assert.match(content, /return a bounded read-only result or blocker/i);
		assert.match(
			content,
			/Route implementation through Team only after Team's separate host-authority checks pass/i,
		);
		assert.match(
			content,
			/local state, task text, session fields, trackers, or child provenance as authority/i,
		);
	});
	...
	it("tracked AGENTS and core prompt surfaces stay action-first and avoid permission-seeking softeners", () => {
		...
		for (const surface of [
			...listTrackedAgentSurfaces(),
			...CORE_ROLE_CONTRACTS.map((contract) => contract.path),
		]) {
			const content = loadSurface(surface);
			for (const pattern of banned) {
				assert.doesNotMatch(
					content,
					pattern,
					`${surface} should not contain permission-seeking softeners matching ${pattern}`,
				);
			}
		}
	});
	...
	it("tracked AGENTS and core prompt surfaces encode AUTO-CONTINUE vs ASK autonomy steering", () => {
		...
		for (const surface of surfaces) {
			...
			assert.match(
				content,
				/AUTO-CONTINUE.*clear.*already-requested.*low-risk.*reversible.*local/i,
			);
			assert.match(
				content,
			...
			assert.match(
				content,
				/AUTO-CONTINUE branches.*permission-handoff phrasing/i,
			);
		}
	});
});
