...
describe("visual-verdict – parseVisualVerdict", () => {
	it("extracts PASS from **Status**: PASS", async () => {
		const { parseVisualVerdict } = await loadModule(
	...
	it("extracts FAIL from Verdict: FAIL", async () => {
		...
		const result = parseVisualVerdict(
			"Overall result. Verdict: FAIL due to missing tests.",
		);
...
describe("visual-verdict – maybePersistVisualVerdict (persist)", () => {
	it("writes latest-verdict.json for a matched verdict", async () => {
		const { maybePersistVisualVerdict } = await loadModule(
			"notify-hook/visual-verdict.js",
		);
	...
	it("emits info-level log entry on successful persist", async () => {
		...
		try {
			...
			assert.ok(
				logFiles.some((f) => f.startsWith("notify-hook-")),
				"Expected notify-hook log file",
			);
			const logContent = await readFile(
				join(logsDir, logFiles.find((f) => f.startsWith("notify-hook-"))!),
				"utf-8",
			);
...
describe("visual-verdict – candidate parse debug logging", () => {
	it("logs debug when verdict-like markers present but no structured match", async () => {
		...
		try {
			...
			await maybePersistVisualVerdict({
				payload: {
					"last-assistant-message": "Verdict: PENDING more analysis needed.",
				},
				stateDir,
				logsDir,
	...
	it("does not log debug when no verdict markers present at all", async () => {
		...
		try {
			await maybePersistVisualVerdict({
				payload: {
					"last-assistant-message": "Refactoring complete. All tests pass.",
				},
				stateDir,
				logsDir,
			...
			if (logFiles.length > 0) {
				const logContent = await readFile(join(logsDir, logFiles[0]), "utf-8");
				assert.equal(
					logContent.includes("visual_verdict"),
					false,
					"No verdict log expected",
				);
			}
		} finally {
			await rm(tmpDir, { recursive: true, force: true });
...
describe("visual-verdict – write failure logging", () => {
	it("logs warn-level event on persistence write failure", async () => {
		...
		try {
			await maybePersistVisualVerdict({
				payload: {
					"last-assistant-message": "**Status**: FAIL\nTests failed.",
				},
				stateDir,
				logsDir,
				sessionId: "sess-wf",
	...
	it("logs structured warn when module import fails", async () => {
		...
		try {
			...
			try {
				await import(
					pathToFileURL(
						join(SCRIPTS_DIR, "notify-hook/non-existent-visual-verdict.js"),
					).href
				);
