...
const designSkill = readFileSync(
	join(repoRoot, "skills", "design", "SKILL.md"),
	"utf-8",
);
const frontendShim = readFileSync(
	join(repoRoot, "skills", "frontend-ui-ux", "SKILL.md"),
	"utf-8",
);
const visualRalphSkill = readFileSync(
	join(repoRoot, "skills", "visual-ralph", "SKILL.md"),
	"utf-8",
);
...
describe("design skill contract", () => {
	...
	it("requires the DESIGN.md checklist sections from issue 2277", () => {
		for (const section of [
			...
			assert.match(
				designSkill,
				new RegExp(`## ${section.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`, "i"),
			);
		}
	});
	...
	it("separates design governance from Visual Ralph matching", () => {
		assert.match(
			designSkill,
			/`\$visual-ralph` owns implementation against an approved generated\/static\/live-URL visual reference/i,
		);
		assert.match(
			designSkill,
			/does not replace the `DESIGN\.md` discovery\/interview\/refresh workflow/i,
		);
	...
	it("routes explicit $design while keeping frontend-ui-ux as deprecated compatibility guidance", () => {
		...
		const deprecated = detectPrimaryKeyword(
			"$frontend-ui-ux improve this page",
		);
