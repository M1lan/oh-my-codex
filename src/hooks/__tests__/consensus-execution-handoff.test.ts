...
describe("Consensus mode execution handoff (plan/SKILL.md)", () => {
	...
	it("should use MUST language for execution handoff", () => {
		...
		assert.ok(consensusSection, "Consensus Mode section should exist");
		assert.ok(
			/MUST.*\$ultragoal/s.test(consensusSection) ||
				/\$ultragoal.*MUST/s.test(consensusSection),
	...
	it("should require RALPLAN-DR structured deliberation in consensus mode", () => {
		...
		assert.ok(consensusSection, "Consensus Mode section should exist");
		assert.ok(
			consensusSection.includes("RALPLAN-DR"),
			"Should mention RALPLAN-DR",
		);
		assert.ok(
			consensusSection.includes("**Principles** (3-5)"),
			"Should require Principles",
		);
		assert.ok(
			consensusSection.includes("**Decision Drivers** (top 3)"),
			"Should require Decision Drivers",
		);
		assert.ok(
			consensusSection.includes("**Viable Options** (>=2)"),
			"Should require Viable Options",
		);
		assert.ok(
			consensusSection.includes("**invalidation rationale**"),
			"Should require invalidation rationale",
		);
	});
	...
	it("should require ADR fields in final consensus output", () => {
		...
		assert.ok(
			consensusSection.includes("**Decision**"),
			"ADR should include Decision",
		);
		assert.ok(
			consensusSection.includes("**Drivers**"),
			"ADR should include Drivers",
		);
		assert.ok(
			consensusSection.includes("**Alternatives considered**"),
			"ADR should include Alternatives considered",
		);
		assert.ok(
			consensusSection.includes("**Why chosen**"),
			"ADR should include Why chosen",
		);
		assert.ok(
			consensusSection.includes("**Consequences**"),
			"ADR should include Consequences",
		);
		assert.ok(
			consensusSection.includes("**Follow-ups**"),
			"ADR should include Follow-ups",
		);
	});
	...
	it("should require available-agent-types roster and staffing guidance in handoff output", () => {
		...
		assert.match(
			consensusSection,
			/reasoning levels? by lane|suggested reasoning/i,
		);
	...
	it("should include contextual goal-mode follow-up suggestions in consensus handoff output", () => {
		...
		assert.match(
			consensusSection,
			/alongside the Team path and any explicit Ralph fallback/i,
		);
	});
	...
	it("should mention deliberate mode requirements in consensus mode", () => {
		...
		assert.ok(consensusSection, "Consensus Mode section should exist");
		assert.ok(
			consensusSection.includes("**Deliberate**") ||
				consensusSection.includes("deliberate mode"),
		);
		assert.ok(
			consensusSection.includes("`--deliberate`") ||
				consensusSection.includes("--deliberate"),
		...
		assert.ok(
			consensusSection.includes("unit / integration / e2e / observability"),
		);
	});
});
...
describe("User feedback step between Planner and Architect/Critic (plan/SKILL.md)", () => {
	it("should have a user feedback step after Planner and before Architect", () => {
		...
		assert.ok(
			feedbackIdx > plannerIdx,
			"User feedback should come after Planner",
		);
		assert.ok(
			architectIdx > feedbackIdx,
			"Architect should come after User feedback",
		);
	});
	...
	it("should require role-specific subsequent Architect and Critic subagents with full handoff context", () => {
		...
		assert.match(
			consensusSection,
			/dedicated subsequent `Architect` subagent/i,
		);
		assert.match(consensusSection, /dedicated subsequent `Critic` subagent/i);
		assert.match(
			consensusSection,
			/full task, current plan text\/path, RALPLAN-DR summary/i,
		);
		assert.match(consensusSection, /completed `Architect` result/i);
		assert.match(
			consensusSection,
			/Do NOT substitute a default\/improvised subagent prompt/i,
		);
		assert.match(
			consensusSection,
			/Do NOT let the `Architect` response self-approve the Critic gate/i,
		);
		assert.doesNotMatch(
			planSkill,
			/agent_role: "critic"` for plan review in consensus/i,
		);
		assert.match(planSkill, /standalone review mode/i);
		assert.match(
			planSkill,
			/dedicated sequential role-specific `Architect` and `Critic` subagents/i,
		);
	});
	...
	it("should require architect antithesis and critic rejection gates in consensus flow", () => {
		...
		assert.ok(
			consensusSection.includes("tradeoff tension"),
			"Should require tradeoff tension",
		);
		assert.ok(
			/Critic.*MUST.*explicitly reject shallow alternatives/s.test(
				consensusSection,
			) ||
				consensusSection.includes(
...
it("should require adaptive step sizing instead of a fixed five-step template", () => {
	...
	assert.match(
		planSkill,
		/do not default to exactly five steps|not a fixed five-step template/i,
	);
...
describe("RALPLAN-DR in ralplan/SKILL.md", () => {
	...
	it("should document sequential Architect then Critic execution", () => {
		assert.ok(
			/step[s]? 3 and 4 MUST run sequentially|Do NOT.*parallel/i.test(
				ralplanSkill,
	...
	it("should document roster-aware ultragoal, team, and explicit ralph fallback guidance", () => {
		...
		assert.match(
			ralplanSkill,
			/\$autoresearch-goal[\s\S]*research-project follow-up/i,
		);
		assert.match(
			ralplanSkill,
			/\$performance-goal[\s\S]*optimization\/performance follow-up/i,
		);
		assert.match(
			ralplanSkill,
			/Ultragoal as the default durable goal-mode follow-up/i,
		);
		assert.match(
			ralplanSkill,
			/Ralph only for intentionally selected persistent single-owner/i,
		);
	});
});
...
describe("Architect prompt RALPLAN-DR sections", () => {
	it("should have steelman antithesis requirement", () => {
		assert.ok(
			architectPrompt.includes("antithesis") ||
				architectPrompt.includes("steelman"),
...
describe("Planner prompt follow-up staffing guidance", () => {
	it("should require roster-aware staffing guidance for ultragoal, team, and explicit ralph fallback handoff", () => {
		...
		assert.match(
			plannerPrompt,
			/\$performance-goal[\s\S]*optimization\/performance projects/i,
		);
		assert.match(
			plannerPrompt,
			/reasoning levels? by lane|suggested reasoning/i,
	...
	it("should enforce deliberate mode requirements", () => {
		assert.ok(
			criticPrompt.includes("deliberate") &&
				(criticPrompt.includes("pre-mortem") ||
					criticPrompt.includes("expanded test plan")),
