...
describe("generateCodebaseMap", () => {
	...
	it("invalidates stale and corrupt durable cache entries safely", async () => {
		...
		try {
			...
			assert.ok(
				(await generateCodebaseMap(cacheDir)).includes("first"),
				"corrupt cache should regenerate",
			);

			await writeFile(
				join(cacheDir, "src", "second.ts"),
			...
			assert.ok(
				...
				"index change should invalidate stale cache",
			);
			assert.ok(
				!updated.includes("secret-wip"),
				"cache regeneration must not leak untracked names",
			);
		} finally {
			await rm(cacheDir, { recursive: true, force: true });
		}
	...
	it("caches empty tracked-source results without exposing .omx paths", async () => {
		...
		try {
			...
			await writeFile(
				join(emptyDir, ".omx", "cache", "secret.ts"),
			...
			const cache = JSON.parse(
				await readFile(
					join(emptyDir, ".omx", "cache", "codebase-map.json"),
					"utf-8",
				),
			);
