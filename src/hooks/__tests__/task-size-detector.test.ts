...
describe("task-size-detector", () => {
	...
	describe("detectEscapeHatch", () => {
		...
		it("detects just: prefix", () => {
			assert.equal(
				detectEscapeHatch("just: update the version number"),
				"just:",
			);
		});
	...
	describe("hasSmallTaskSignals", () => {
		...
		it('detects "this function" signal', () => {
			assert.equal(
				hasSmallTaskSignals("fix this function to return null"),
				true,
			);
		});
		...
		it("detects minor fix signal", () => {
			assert.equal(
				hasSmallTaskSignals("minor fix needed in the handler"),
				true,
			);
		});
		...
		it("returns false for regular task", () => {
			assert.equal(
				hasSmallTaskSignals("implement user authentication flow"),
				false,
			);
		});
	...
	describe("hasLargeTaskSignals", () => {
		...
		it('detects "multiple files" signal', () => {
			assert.equal(
				hasLargeTaskSignals("change imports across multiple files"),
				true,
			);
		});
		...
		it('detects "from scratch" signal', () => {
			assert.equal(
				hasLargeTaskSignals("rewrite the parser from scratch"),
				true,
			);
		});
		...
		it("detects overhaul signal", () => {
			assert.equal(
				hasLargeTaskSignals("overhaul the permissions system"),
				true,
			);
		});
	...
	describe("HEAVY_MODE_KEYWORDS set", () => {
		it("contains expected heavy modes", () => {
			...
			for (const mode of expected) {
				assert.ok(
					HEAVY_MODE_KEYWORDS.has(mode),
					`Expected HEAVY_MODE_KEYWORDS to contain "${mode}"`,
				);
			}
		});
		...
		it("does not contain lightweight modes", () => {
			...
			for (const mode of lightweight) {
				assert.ok(
					!HEAVY_MODE_KEYWORDS.has(mode),
					`Expected HEAVY_MODE_KEYWORDS NOT to contain "${mode}"`,
				);
			}
		});
	});
