...
const contractDoc = readFileSync(
	join(process.cwd(), "docs", "clawhip-event-contract.md"),
	"utf-8",
);
...
describe("clawhip event contract doc", () => {
	...
	it("documents lifecycle ownership and reduced assistant-signal noise", () => {
		assert.match(
			contractDoc,
			/native session lifecycle events are the canonical source/i,
		);
		assert.match(
			contractDoc,
			/team\/runtime operational events add canonical worker-state signals/i,
		);
		assert.match(
			contractDoc,
			/session_name.*stable across native and derived events/i,
		);
		assert.match(
			contractDoc,
			/do not duplicate session completion\/failure lifecycle events/i,
		);
	});
});
