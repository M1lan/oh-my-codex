...
describe("session lifecycle manager", () => {
	...
	it("starts a fresh native session while retaining the owner OMX launch session when native SessionStart changes", async () => {
		...
		try {
			...
			assert.match(
				reconciled.native_session_switched_at ?? "",
				/^\d{4}-\d{2}-\d{2}T/,
			);
			assert.equal(reconciled.pid, 54321);
	...
	it("carries the owner OMX launch session across chained native SessionStart replacements", async () => {
		...
		try {
			...
			assert.match(
				dailyLog,
				/"replaced_by_native_session_id":"codex-native-c"/,
			);
...
describe("session pointer transaction", () => {
	...
	it("never reaps dead, reused, indeterminate, missing, or malformed lock evidence", async () => {
		const cases: Array<{
			...
			},
			{
				...
				owner: validLockOwner(),
				probePid: "alive",
				identity: { status: "reused", startTicks: 2 },
				expected: "reused",
			},
			{
			...
			},
			{
				...
				owner: validLockOwner(),
				probePid: "alive",
				identity: { status: "indeterminate" },
				expected: "identity-indeterminate",
			},
			{
				name: "unsubstantiated-reuse",
				owner: validLockOwner(),
				probePid: "alive",
				identity: { status: "reused", startTicks: 1 },
				expected: "identity-indeterminate",
			},
			{
				...
				probePid: "alive",
				identity: matchingProcessIdentity(),
				expected: "identity-indeterminate",
			},
				...
				await withPointerDependencies(
					{
						...
						readProcessIdentity: () =>
							testCase.identity ?? matchingProcessIdentity(),
					},
					async () => {
						await assert.rejects(
	...
	it("does not recover a dead canonical owner because owner.json is not an exact acquisition claim", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-dead-canonical-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
	...
	it("fails closed for live pre-rename, PID reuse, malformed, and ambiguous lock evidence", async () => {
		const cases: Array<{
			...
			probePid: "alive" | "dead";
			identity?: ReturnType<typeof matchingProcessIdentity>;
		}> = [
			{
				name: "live-temp",
			...
			},
			{
				...
				probePid: "alive",
				identity: { status: "matching", startTicks: 2 },
			},
	...
	it("claims stale evidence before quarantine so a successor cannot be mistaken for it", async () => {
		...
		try {
					...
					fs: {
						rename: async (from, to) => {
							...
							if (from.endsWith(`owner.${TEST_TOKEN}.tmp`)) {
								await assert.rejects(mkdir(context.lockPath), {
									code: "EEXIST",
								});
			...
			assert.match(
				renames[0]![1],
				new RegExp(`owner\\.${TEST_TOKEN}\\.${SUCCESSOR_TOKEN}\\.recovery$`),
			);
			assert.match(
				renames[1]![0],
				new RegExp(`owner\\.${TEST_TOKEN}\\.${SUCCESSOR_TOKEN}\\.recovery$`),
		...
		try {
			...
			await withPointerDependencies(
				...
				async () => {
					...
					assert.match(
						recovered.reason,
						/canonical lock directory was not empty/i,
					);
	...
	it("emits one post-release warning for a degraded session start and stays silent for synced or failed transactions", async () => {
		...
		process.stderr.write = ((value: string) => {
			warnings.push({
				value,
				lockExists: existsSync(resolveSessionPointerContext(cwd).lockPath),
			});
			return true;
		}) as typeof process.stderr.write;
		const regularFileSync = async () => {
		...
		try {
			...
			await withPointerDependencies(
				...
				async () => {
					await assert.rejects(
						writeSessionStart(cwd, "sess-degraded-start", {
							platform: "win32",
							regularFileSync,
						}),
					);
	...
	it("binds an owner alias only on a verified same-native/absent transition and never during replacement", async () => {
		...
		try {
			...
			await withOwnerEnvironment("omx-owner", async () => {
				...
				const replacement = await reconcileNativeSessionStart(
					cwd,
					"native-second",
					{
						platform: "win32",
