...
async function withPointerDependencies(
	overrides: Parameters<
		typeof __setSessionPointerTransactionDependenciesForTests
	>[0],
	run: () => Promise<void>,
): Promise<void> {
	const runtimePlatform = overrides.runtimePlatform ?? "linux";
	__setSessionPointerTransactionDependenciesForTests({
...
async function withOwnerEnvironment(
...
function validLockOwner(
	overrides: Record<string, unknown> = {},
): Record<string, unknown> {
	const platform = (overrides.platform ?? "linux") as NodeJS.Platform;
	const version =
		overrides.version === 2 || overrides.process_identity !== undefined ? 2 : 1;
	const identity =
		overrides.process_identity ??
		(version === 2 ? { platform, birth: "1" } : undefined);
	return {
		version,
		token: TEST_TOKEN,
		pid: process.pid,
...
async function defaultTestAtomicRenameNoReplace(
	from: string,
	to: string,
): Promise<"moved" | "not-moved" | "unsupported"> {
...
async function assertDurableDirectoryRecovery(
	cwd: string,
	options: {
		atomicRenameNoReplace?: Parameters<
			typeof __setSessionPointerTransactionDependenciesForTests
		>[0]["atomicRenameNoReplace"];
	} = {},
): Promise<void> {
	const context = resolveSessionPointerContext(cwd);
	...
	await withPointerDependencies(
		{
			...
			atomicRenameNoReplace:
				options.atomicRenameNoReplace ?? defaultTestAtomicRenameNoReplace,
		},
		async () => {
			...
			assert.deepEqual(
				{ dev: quarantine.dev, ino: quarantine.ino },
				{ dev: parkedOwner.dev, ino: parkedOwner.ino },
			);
...
describe("session lifecycle manager", () => {
	...
	it("emits the session-end warning only after successful end finalization", async () => {
		...
		let binding: LaunchSessionBinding | undefined;
		try {
			const established = await establishLaunchSessionBinding(
				cwd,
				"sess-end-durability",
			);
			assert.equal(established.kind, "committed-released");
	...
	it("keeps session-end durability warnings silent when finalization or release fails", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-end-durability-failure-"),
		);
		const originalWrite = process.stderr.write;
		const warnings: string[] = [];
		process.stderr.write = ((value: string) => {
			...
			await withPointerDependencies(
				...
				async () => {
					await assert.rejects(
						writeSessionEnd(cwd, "sess-end-release-failure", {
	...
	it("does not delete the current session pointer when ending a different session", async () => {
		...
		try {
			...
			const beforePointer = await readFile(
				resolveSessionPointerContext(cwd).sessionPath,
				"utf-8",
			);
			const stateDir = join(cwd, ".omx", "state");
			const sessionPath = join(stateDir, "session.json");
			const currentHudPath = join(
				stateDir,
				"sessions",
				"sess-current",
				"hud-state.json",
			);
			const endingHudPath = join(
				stateDir,
				"sessions",
				"sess-ending",
				"hud-state.json",
			...
			await mkdir(join(stateDir, "sessions", "sess-ending"), {
				recursive: true,
			});
			await writeFile(
				currentHudPath,
				JSON.stringify({ turn_count: 2 }),
				"utf-8",
			);
			await writeFile(
				endingHudPath,
				JSON.stringify({ turn_count: 1 }),
				"utf-8",
			);

			await assert.rejects(
				() => writeSessionEnd(cwd, "sess-ending"),
				isSessionPointerLaunchAbort,
			);
			...
			assert.equal(
				existsSync(join(cwd, ".omx", "logs", "session-history.jsonl")),
				false,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("removes canonical and native session-scoped hud state on session end", async () => {
		...
		try {
			...
			const canonicalHudPath = join(
				stateDir,
				"sessions",
				canonicalSessionId,
				"hud-state.json",
			);
			const nativeHudPath = join(
				stateDir,
				"sessions",
				nativeSessionId,
				"hud-state.json",
			...
			await writeFile(
				rootHudPath,
				JSON.stringify({ last_turn_at: "root", turn_count: 1 }),
				"utf-8",
			);
			await writeFile(
				canonicalHudPath,
				JSON.stringify({ last_turn_at: "canonical", turn_count: 2 }),
				"utf-8",
			);
			await writeFile(
				nativeHudPath,
				JSON.stringify({ last_turn_at: "native", turn_count: 9 }),
				"utf-8",
			);
	...
	it("preserves canonical session id while reconciling native SessionStart metadata", async () => {
		...
		try {
			...
			const reconciled = await reconcileNativeSessionStart(
				cwd,
				"codex-native-1",
				{
					pid: 54321,
			...
			assert.equal(
				reconciled.launch_lineage_token,
				established.launch_lineage_token,
			);
			...
			assert.equal(
				persisted?.launch_lineage_token,
				established.launch_lineage_token,
			);
	...
	it("preserves canonical session metadata when reconciling the same native session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-native-owner-reconcile-"),
		);
		let binding: LaunchSessionBinding | undefined;
		try {
			await withPointerDependencies(
	...
	it("rejects chained native SessionStart replacements and preserves the original pointer", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-native-owner-chain-"),
		);
		let binding: LaunchSessionBinding | undefined;
		try {
			const established = await establishLaunchSessionBinding(
			...
			binding = established.binding;
			await assert.rejects(
				reconcileNativeSessionStart(cwd, "codex-native-b", {
					pid: process.pid,
					platform: "win32",
				}),
				isOwnerConflict,
			...
			await assert.rejects(
				reconcileNativeSessionStart(cwd, "codex-native-c", {
					pid: process.pid,
					platform: "win32",
				}),
				isOwnerConflict,
	...
	it("rejects replacing a live wrapper-owned native session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-native-non-omx-fresh-"),
		);
		try {
...
describe("isSessionStale", () => {
	...
	it("classifies identity-less live non-Linux pointers as identity-indeterminate", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-non-linux-identityless-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			...
			await withPointerDependencies(
				...
				async () => {
					assert.equal(
						(await readSessionPointer(context)).status,
...
describe("session pointer transaction", () => {
	...
	it("covers the process identity decision table through pointer and lock inspection", async () => {
		...
		const legacyOwner = (platform: NodeJS.Platform = "linux", startTicks = 1) =>
			validLockOwner({
				pid: 4242,
				platform,
				pid_start_ticks: startTicks,
			});
		const identitylessOwner = (platform: NodeJS.Platform) =>
			validLockOwner({
				pid: 4242,
				platform,
		...
		const classifierCases: Array<{
		...
		}> = [
			{
				...
				runtimePlatform: "linux",
				probePid: "alive",
				observations: [identity("linux", "1")],
				lockOwner: identityOwner({ platform: "linux", birth: "1" }),
				expectedPointerStatus: "usable",
				expectedLockStatus: "live",
			},
			{
				name: "darwin match",
				recordedState: {
					identity_schema_version: 2,
					process_identity: { platform: "darwin", birth: "1234.567" },
				},
				runtimePlatform: "darwin",
				probePid: "alive",
				observations: [identity("darwin", "1234.567")],
			...
			},
			{
				...
				recordedState: {
					identity_schema_version: 2,
					process_identity: { platform: "win32", birth: "132580896000000000" },
				},
				runtimePlatform: "win32",
				probePid: "alive",
				observations: [identity("win32", "132580896000000000")],
				lockOwner: identityOwner({
					platform: "win32",
					birth: "132580896000000000",
				}),
				expectedPointerStatus: "usable",
				expectedLockStatus: "live",
			},
			{
				...
				expectedPointerStatus: "stale-dead",
				expectedLockStatus: "dead",
			},
			{
				name: "birth mismatch",
				recordedState: { platform: "linux", pid_start_ticks: 1 },
				runtimePlatform: "linux",
				probePid: "alive",
				observations: [identity("linux", "2"), identity("linux", "2")],
				lockOwner: legacyOwner(),
				expectedPointerStatus: "stale-dead",
				expectedLockStatus: "reused",
			},
			{
				...
				runtimePlatform: "linux",
				probePid: "alive",
				observations: [identity("linux", "2"), identity("linux", "1")],
				lockOwner: legacyOwner(),
				expectedPointerStatus: "usable",
				expectedLockStatus: "live",
			},
			{
				...
				runtimePlatform: "linux",
				probePid: "alive",
				observations: [identity("linux", "2"), identity("linux", "3")],
				lockOwner: legacyOwner(),
				expectedPointerStatus: "stale-dead",
				expectedLockStatus: "reused",
			},
			{
				...
				observations: [],
				lockOwner: legacyOwner(),
				expectedPointerStatus: "identity-indeterminate",
				expectedLockStatus: "identity-indeterminate",
			},
			{
			...
			},
			{
				...
				recordedState: {
					identity_schema_version: 2,
					process_identity: { platform: "freebsd", birth: "1" },
				},
				runtimePlatform: "freebsd",
				probePid: "alive",
			...
			},
			{
				...
				recordedState: {
					identity_schema_version: 2,
					process_identity: { platform: "darwin", birth: "1" },
				},
				runtimePlatform: "linux",
				probePid: "alive",
				observations: [identity("linux", "1")],
			...
			},
			{
				...
				runtimePlatform: "linux",
				probePid: "alive",
				observations: [identity("linux", "1", hashA)],
				lockOwner: identityOwner({
					platform: "linux",
					birth: "1",
					cmdline_hash: hashA,
			...
			},
			{
				...
				runtimePlatform: "linux",
				probePid: "alive",
				observations: [identity("linux", "1", hashB)],
				lockOwner: identityOwner({
					platform: "linux",
					birth: "1",
					cmdline_hash: hashA,
			...
			},
			{
				...
				runtimePlatform: "linux",
				probePid: "alive",
				observations: [identity("linux", "111")],
			...
			},
			{
				...
				runtimePlatform: "linux",
				probePid: "alive",
				observations: [identity("linux", "222"), identity("linux", "222")],
				lockOwner: legacyOwner("linux", 111),
				expectedPointerStatus: "stale-dead",
				expectedLockStatus: "reused",
			},
			...
			},
			{
				...
				observations: [],
				lockOwner: "malformed",
				expectedPointerStatus: "malformed",
				expectedLockStatus: "malformed",
			},
			{
				name: "same-second Darwin collision",
				recordedState: {
					identity_schema_version: 2,
					process_identity: { platform: "darwin", birth: "1000.500" },
				},
				runtimePlatform: "darwin",
				probePid: "alive",
				observations: [
					identity("darwin", "1000.999"),
					identity("darwin", "1000.999"),
				],
			...
			},
			{
				...
				probePid: "alive",
				observations: [],
				expectedPointerStatus: "absent",
				expectedLockStatus: "absent",
			},
		];
		...
		for (const testCase of classifierCases) {
			...
			try {
				...
				if (testCase.lockOwner !== undefined) {
					...
					await writeFile(
						join(context.lockPath, "owner.json"),
						testCase.lockOwner === "malformed"
							? "{not-json"
							: JSON.stringify(testCase.lockOwner),
						"utf-8",
					);
				}
				let observerCalls = 0;
				const observer = (): ProcessObservation => {
					...
					return (
						testCase.observations[
							Math.min(index, Math.max(testCase.observations.length - 1, 0))
						] ?? { kind: "error" }
					);
				};
				await withPointerDependencies(
					...
					async () => {
						assert.equal(
							(await readSessionPointer(context)).status,
							testCase.expectedPointerStatus,
							testCase.name,
						);
					},
				);
				...
				await withPointerDependencies(
					...
					async () => {
						assert.equal(
							(await inspectSessionPointerLock(cwd)).status,
							testCase.expectedLockStatus,
							testCase.name,
						);
					},
				);
	...
	it("rejects every schema version other than v2 as identity-indeterminate", async () => {
		...
		for (const schemaVersion of schemaVersions) {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-session-identity-schema-rejection-"),
			);
			try {
				const context = resolveSessionPointerContext(cwd);
				await mkdir(context.baseStateDir, { recursive: true });
	...
	it("maps only ESRCH to a definitely dead default PID probe", () => {
		...
		const esrch = __createDefaultPidProbeForTests(() => {
			throw codedError("ESRCH");
		});
		const eperm = __createDefaultPidProbeForTests(() => {
			throw codedError("EPERM");
		});
		const unknown = __createDefaultPidProbeForTests(() => {
			throw codedError("EACCES");
		});
		const noCode = __createDefaultPidProbeForTests(() => {
			throw new Error("unknown");
		});
		const primitive = __createDefaultPidProbeForTests(() => {
			throw "unknown";
		});
	...
	it("never reaps dead, reused, indeterminate, missing, or malformed lock evidence", async () => {
		const cases: Array<{
			...
			},
			{
				...
				observation: {
					kind: "identity",
					identity: { platform: "linux", birth: "2" },
				},
				expected: "reused",
			},
			{
				...
				owner: validLockOwner(),
				probePid: "indeterminate",
				expected: "identity-indeterminate",
			},
			{
				name: "indeterminate-identity",
				owner: validLockOwner(),
				probePid: "alive",
			...
			},
			{
				...
				observation: {
					kind: "identity",
					identity: { platform: "darwin", birth: "1" },
				},
				expected: "identity-indeterminate",
			},
			{
				name: "missing-required-hash",
				owner: validLockOwner({ pid_cmdline_hash: "a".repeat(64) }),
				probePid: "alive",
		...
		for (const testCase of cases) {
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-session-lock-${testCase.name}-`),
			);
			try {
				const context = resolveSessionPointerContext(cwd);
				await mkdir(context.lockPath, { recursive: true });
	...
	it("atomically quarantines dead pre-rename temporary owner evidence and stays idempotent", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-dead-temp-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await mkdir(context.lockPath, { recursive: true });
			await writeFile(
				join(context.lockPath, `owner.${TEST_TOKEN}.tmp`),
				JSON.stringify(validLockOwner()),
				"utf-8",
			);
			await withPointerDependencies(
				{ token: () => SUCCESSOR_TOKEN, probePid: () => "dead" },
				async () => {
	...
	it("atomically quarantines dead canonical owner evidence and stays idempotent", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-preserved-"),
		);
		try {
			await assertDurableDirectoryRecovery(cwd);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	});
	...
	it("launches only after explicit recovery of a dead canonical owner lock", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recover-launch-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await writeLockOwner(cwd, validLockOwner());
			await withPointerDependencies(
				...
				async () => {
					...
					assert.equal(recovered.recovered, true);
					assert.equal(recovered.action, "quarantined");
					await writeSessionStart(cwd, "sess-after-recovery", {
						platform: "win32",
					});
					assert.equal(
						(await readSessionState(cwd))?.session_id,
						"sess-after-recovery",
					);
					assert.equal(existsSync(context.lockPath), false);
				},
	...
	it("does not claim a successor lock when the inspected dead canonical owner is displaced before the exact rename", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-canonical-race-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			const successorTemp = join(
				context.lockPath,
				`owner.${SUCCESSOR_TOKEN}.tmp`,
			);
			...
			const successorOwner = JSON.stringify(
				validLockOwner({ token: SUCCESSOR_TOKEN }),
			);
			await writeLockOwner(cwd, validLockOwner());
			let displaced = false;
			await withPointerDependencies(
				{
					...
					atomicRenameNoReplace: async (from, to) => {
						if (
							from === context.lockPath &&
							to === `${context.lockPath}.parked-lock-${SUCCESSOR_TOKEN}`
						) {
							displaced = true;
				...
				async () => {
					...
					assert.match(
						recovered.reason,
						/Atomic recovery move|recovery checkpoint/i,
					);
				},
			);
			assert.equal(displaced, true);
			assert.equal(
				await readFile(join(displacedPath, "owner.json"), "utf-8"),
				staleOwner,
			);
	...
	it("rolls back a claim that lands on a displaced successor canonical owner", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-canonical-claim-race-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			...
			const successorOwner = JSON.stringify(
				validLockOwner({ token: SUCCESSOR_TOKEN, pid: successorPid }),
			);
			await writeLockOwner(cwd, validLockOwner());
			let moved = false;
			...
			await withPointerDependencies(
				...
				async () => {
					...
					assert.match(
						recovered.reason,
						/captured object|token-bound|source pathname/i,
					);
				},
			);
			...
			assert.equal(
				await readFile(join(parkedPath, "owner.json"), "utf-8"),
				staleOwner,
			);
	...
	it("live canonical successor survives a refused recovery claim and stays recognizable, releasable, and acquirable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-live-successor-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			...
			await withPointerDependencies(
				...
				async () => {
					...
					assert.equal(await readFile(ownerPath, "utf-8"), successorOwner);
					assert.deepEqual(
						await __releasePointerLockForTests(cwd, SUCCESSOR_TOKEN),
						[],
					);
					assert.equal(existsSync(context.lockPath), false);
					const started = await writeSessionStart(cwd, "sess-after-survival", {
						platform: "win32",
					});
	...
	it("fails closed without mutating a foreign target when the lock directory is replaced by a symlink before the claim rename", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-lock-symlink-"),
		);
		const foreign = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-foreign-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			...
			await withPointerDependencies(
				{
					...
					fs: {
						link: async (from, to) => {
							if (!replaced && from === ownerPath) {
								replaced = true;
								await rm(context.lockPath, { recursive: true });
								await writeFile(foreignOwner, marker);
								await symlink(foreign, context.lockPath);
							}
							await link(from, to);
						},
					},
			...
			assert.equal(
				(await readdir(foreign)).some((entry) =>
	...
	it("does not follow an evidence symlink swapped in before the claim rename", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-evidence-symlink-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
					...
					fs: {
						link: async (from, to) => {
							if (!replaced && from === ownerPath) {
								replaced = true;
								await rm(ownerPath);
								await symlink(foreign, ownerPath);
							}
							await link(from, to);
						},
					},
	...
	it("preserves a non-file evidence replacement as exact recovery residue without clobbering", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-evidence-directory-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
					...
					fs: {
						link: async (from, to) => {
							if (!replaced && from === ownerPath) {
								...
								await writeFile(marker, "directory marker");
							}
							await link(from, to);
						},
					},
	...
	it("retries a transient token-bound park collision without allocating a second parked name", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-park-retry-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const parkedPath = `${context.lockPath}.parked-lock-${SUCCESSOR_TOKEN}`;
			...
			await withPointerDependencies(
				...
				async () => {
					...
					assert.equal(existsSync(context.lockPath), true);
					assert.equal(
						await readFile(join(parkedPath, "foreign"), "utf-8"),
						foreignMarker,
					);
			...
			assert.equal(
				existsSync(checkpointPath.replace(/\.json$/, ".completed")),
				true,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("rolls back a claim when checkpoint creation fails", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-checkpoint-write-fail-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await writeLockOwner(cwd, validLockOwner());
			...
			await withPointerDependencies(
				{
					...
					fs: {
						writeFile: async (path, data, options) => {
							if (fail && path.includes(".recovery.")) {
								fail = false;
	...
	it("rolls back a double-EBUSY evidence park failure so a normal retry succeeds", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-checkpoint-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
	...
	it("resumes an evidence-pending checkpoint without minting another recovery token", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-resume-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			const claimPath = join(
				context.lockPath,
				`owner.${TEST_TOKEN}.${SUCCESSOR_TOKEN}.recovery`,
			);
			...
			await writeFile(
				`${context.lockPath}.recovery.${TEST_TOKEN}.${SUCCESSOR_TOKEN}.json`,
				JSON.stringify({
					version: 1,
					sourcePath: ownerPath,
					parkPath,
					lockParkPath: `${context.lockPath}.parked-lock-${SUCCESSOR_TOKEN}`,
					identity: { dev: parked.dev, ino: parked.ino },
			...
			await withPointerDependencies(
				{
					token: () => {
						throw new Error("resume must reuse checkpoint token");
					},
					probePid: () => "dead",
				},
				async () => {
			...
			assert.equal(
				await readFile(
					`${context.lockPath}.quarantine.${TEST_TOKEN}.${SUCCESSOR_TOKEN}`,
					"utf-8",
				),
				owner,
			);
	...
	it("resumes the actual distinct-token evidence park path after an interruption", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-distinct-park-token-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			...
			const claimPath = join(
				context.lockPath,
				`owner.${TEST_TOKEN}.${claimToken}.recovery`,
			);
			...
			const lock = await lstat(context.lockPath);
			await writeFile(
				`${context.lockPath}.recovery.${TEST_TOKEN}.${claimToken}.json`,
				JSON.stringify({
					version: 1,
					sourcePath: ownerPath,
					parkPath,
			...
			await withPointerDependencies(
				{
					token: () => {
						throw new Error("resume must not mint a replacement token");
					},
					probePid: () => "dead",
				},
				async () =>
			...
			assert.equal(
				await readFile(
					`${context.lockPath}.quarantine.${TEST_TOKEN}.${claimToken}`,
					"utf-8",
				),
				JSON.stringify(validLockOwner()),
			);
	...
	it("resumes a checkpoint written before evidence parking", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-source-checkpoint-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			...
			await writeFile(
				...
				JSON.stringify({
					...
					parkPath,
					lockParkPath: `${context.lockPath}.parked-lock-${SUCCESSOR_TOKEN}`,
					identity: { dev: owner.dev, ino: owner.ino },
					lockIdentity: { dev: lock.dev, ino: lock.ino },
					phase: "evidence-pending",
				}),
			);
			await withPointerDependencies(
				{
					token: () => {
						throw new Error("resume must not mint token");
					},
					probePid: () => "dead",
				},
				async () =>
	...
	it("resumes after quarantine hard-link creation but before claim and park cleanup", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-dual-evidence-checkpoint-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			...
			assert.equal(
				await readFile(quarantinePath, "utf-8"),
				JSON.stringify(validLockOwner()),
			);
			assert.equal(existsSync(checkpointPath), false);
		} finally {
	...
	it("reports absent recovery on a fresh workspace without a state directory", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-fresh-"),
		);
		try {
			const recovered = await recoverSessionPointerLock(cwd);
			assert.equal(recovered.recovered, false);
	...
	it("fails closed when recovery checkpoint discovery cannot be enumerated", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-checkpoint-readdir-error-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await writeLockOwner(cwd, validLockOwner());
	...
	it("fails closed for malformed checkpoint JSON without minting a recovery token", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-malformed-checkpoint-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await writeLockOwner(cwd, validLockOwner());
			...
			await withPointerDependencies(
				{
					token: () => {
						throw new Error("must not mint token");
					},
					probePid: () => "dead",
				},
				async () => {
			...
			assert.equal(
				await readFile(join(context.lockPath, "owner.json"), "utf-8"),
				JSON.stringify(validLockOwner()),
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	for (const [label, checkpoint] of [
		...
		[
			"out-of-root path",
			{
				version: 1,
				phase: "evidence-pending",
				sourcePath: "/foreign/owner.json",
				parkPath: "/foreign/parked",
				identity: { dev: 0, ino: 0 },
			},
		],
	])
		it(`fails closed for ${label} checkpoint`, async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-session-lock-recovery-invalid-checkpoint-"),
			);
			try {
				const context = resolveSessionPointerContext(cwd);
				await writeLockOwner(cwd, validLockOwner());
				...
				await withPointerDependencies(
					...
					async () =>
						assert.equal(
							(await recoverSessionPointerLock(cwd)).recovered,
							false,
						),
				);
	...
	it("refuses multiple checkpoints without mutating either", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-multiple-checkpoints-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await writeLockOwner(cwd, validLockOwner());
			for (const token of [SUCCESSOR_TOKEN, FOREIGN_TOKEN])
				await writeFile(
					`${context.lockPath}.recovery.${TEST_TOKEN}.${token}.json`,
					"{}",
					"utf-8",
				);
			await withPointerDependencies(
				...
				async () =>
					assert.match(
						(await recoverSessionPointerLock(cwd)).reason,
	...
	it("preserves a checkpoint when its claim is missing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-missing-claim-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await mkdir(context.lockPath, { recursive: true });
			const parkPath = `${context.lockPath}.parked-${SUCCESSOR_TOKEN}`;
			await writeFile(parkPath, "stale", "utf-8");
			const stat = await lstat(parkPath);
			const checkpointPath = `${context.lockPath}.recovery.${TEST_TOKEN}.${SUCCESSOR_TOKEN}.json`;
			const lock = await lstat(context.lockPath);
			await writeFile(
				checkpointPath,
				JSON.stringify({
					version: 1,
					sourcePath: join(context.lockPath, "owner.json"),
					parkPath,
					lockParkPath: `${context.lockPath}.parked-lock-${SUCCESSOR_TOKEN}`,
			...
			assert.equal(await readFile(checkpointPath, "utf-8").then(Boolean), true);
			assert.equal(await readFile(parkPath, "utf-8"), "stale");
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	});
	...
	it("refuses an advanced checkpoint when a live successor replaces the bound lock directory", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-checkpoint-successor-swap-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			...
			const successor = JSON.stringify(
				validLockOwner({ token: FOREIGN_TOKEN, pid: successorPid }),
			);
			await mkdir(context.lockPath);
			...
			await withPointerDependencies(
				{
					token: () => {
						throw new Error("resume must not mint");
					},
					probePid: (pid) => (pid === successorPid ? "alive" : "dead"),
					observeProcess: () => matchingObservation(),
				},
				async () => {
					...
					assert.deepEqual(
						await __releasePointerLockForTests(cwd, FOREIGN_TOKEN),
						[],
					);
					assert.equal(existsSync(context.lockPath), false);
				},
	...
	it("does not remove a live successor swapped in before final checkpoint lock removal", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-final-successor-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			const checkpointPath = `${context.lockPath}.recovery.${TEST_TOKEN}.${SUCCESSOR_TOKEN}.json`;
			const lockPark = `${context.lockPath}.parked-lock-${SUCCESSOR_TOKEN}`;
			await writeLockOwner(cwd, validLockOwner());
			const owner = await lstat(ownerPath);
			const lock = await lstat(context.lockPath);
			await writeFile(
				...
				JSON.stringify({
					...
					sourcePath: ownerPath,
					identity: { dev: owner.dev, ino: owner.ino },
					evidenceIdentity: { dev: owner.dev, ino: owner.ino },
					evidenceBytes: JSON.stringify(validLockOwner()),
					lockIdentity: { dev: lock.dev, ino: lock.ino },
					lockParkPath: lockPark,
					phase: "evidence-pending",
				}),
			);
			const successor = JSON.stringify(
				validLockOwner({ token: FOREIGN_TOKEN, pid: process.pid + 100_000 }),
			);
			let moved = false;
			await withPointerDependencies(
				{
					...
					probePid: () => "dead",
					atomicRenameNoReplace: async (from, to) => {
						if (from === context.lockPath && to === lockPark) {
							await rename(from, to);
							await mkdir(context.lockPath);
							await writeFile(ownerPath, successor);
							moved = true;
							return "moved";
						}
						return "not-moved";
			...
			assert.equal(moved, true);
			assert.equal(await readFile(ownerPath, "utf8"), successor);
			assert.deepEqual(
				await __releasePointerLockForTests(cwd, FOREIGN_TOKEN),
				[],
			);
			assert.equal(existsSync(context.lockPath), false);
			assert.equal(existsSync(lockPark), true);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	});
	...
	it("refuses a quarantine path that appears after its preflight check without overwriting it", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-quarantine-race-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			const quarantine = `${context.lockPath}.quarantine.${TEST_TOKEN}.${SUCCESSOR_TOKEN}`;
			const foreign = "foreign quarantine marker";
			await writeLockOwner(cwd, validLockOwner());
			await withPointerDependencies(
				{
					token: () => SUCCESSOR_TOKEN,
					...
					fs: {
						link: async (from, to) => {
							if (to === quarantine) await writeFile(to, foreign);
							await link(from, to);
						},
					},
				},
				async () => {
					...
					assert.match(result.reason, /EEXIST|quarantine/i);
				},
			);
			assert.equal(
	...
	it("fails closed without mutation when a recovery claim destination appears before the no-clobber claim link", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-claim-exist-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			...
			await withPointerDependencies(
				{
					...
					fs: {
						writeFile: async (path, data, options) => {
							await writeFile(path, data, options);
							if (path.includes(".recovery.") && path.endsWith(".json"))
								await writeFile(claim, "foreign claim");
						},
					},
					atomicRenameNoReplace: withProbeFallback(
						context.lockPath,
						async () => {
							moved = true;
							return "moved";
	...
	it("preserves a swapped parked quarantine claim rather than unlinking it", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-parked-claim-swap-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			...
			const owner = await lstat(ownerPath);
			const lock = await lstat(context.lockPath);
			await link(ownerPath, claim);
			await link(ownerPath, quarantine);
			await rm(claim);
			await writeFile(claim, "foreign parked claim");
			await rename(context.lockPath, lockPark);
			await writeFile(
				checkpoint,
				JSON.stringify({
					version: 2,
			...
			await withPointerDependencies(
				{
					...
					atomicRenameNoReplace: async (from, to) => {
						...
						throw new Error("directory move must not occur");
					},
				},
				async () =>
					assert.equal((await recoverSessionPointerLock(cwd)).recovered, true),
			...
			assert.equal(
				await readFile(
					join(lockPark, `owner.${TEST_TOKEN}.${SUCCESSOR_TOKEN}.recovery`),
					"utf8",
				),
				"foreign parked claim",
			);
			assert.equal(
				await readFile(quarantine, "utf8"),
				JSON.stringify(validLockOwner()),
			);
			assert.equal(existsSync(lockPark), true);
		} finally {
	...
	it("fails closed without mutation when evidence lstat fails non-ENOENT before the claim", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-evidence-eacces-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
	...
	it("restores evidence and keeps the exact quarantine inspect diagnostic when quarantine lstat fails non-ENOENT", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-quarantine-eacces-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const ownerPath = join(context.lockPath, "owner.json");
			...
			await withPointerDependencies(
				{
					...
					fs: {
						...
						lstat: async (path) => {
							if (armed && path === quarantine) throw codedError("EACCES");
							return await lstat(path);
						},
					},
				},
	...
	it("preserves a swapped parked lock directory rather than removing it", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-directory-swap-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const parked = `${context.lockPath}.parked-lock-${SUCCESSOR_TOKEN}`;
	...
	for (const [label, checkpoint] of [
		...
		it(`fails closed for ${label} checkpoint`, async () => {
			...
			try {
				...
				assert.equal(await readFile(path, "utf8"), JSON.stringify(checkpoint));
				assert.equal(
					await readFile(join(context.lockPath, "owner.json"), "utf8"),
					JSON.stringify(validLockOwner()),
				);
			} finally {
				await rm(cwd, { recursive: true, force: true });
	...
	for (const fault of ["EBUSY", "ENOTEMPTY"] as const)
		it(`completes a checkpoint resume retry after one-shot atomic directory move failure ${fault}`, async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), "omx-session-lock-recovery-atomic-retry-"),
			);
			try {
				const context = resolveSessionPointerContext(cwd);
				const checkpoint = `${context.lockPath}.recovery.${TEST_TOKEN}.${SUCCESSOR_TOKEN}.json`;
				...
				await withPointerDependencies(
					{
						...
						probePid: () => "dead",
						atomicRenameNoReplace: async (from, to) => {
							if (from === context.lockPath && fail) {
								fail = false;
								throw codedError(fault);
							}
							if (existsSync(to)) return "not-moved";
							await rename(from, to);
							return "moved";
	...
	it("keeps a foreign atomic destination and the source lock when no directory move occurs", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-recovery-atomic-destination-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await writeLockOwner(cwd, validLockOwner());
			...
			await withPointerDependencies(
				{
					token: () => SUCCESSOR_TOKEN,
					probePid: () => "dead",
					atomicRenameNoReplace: async (_from, to) => {
						await mkdir(to);
						await writeFile(join(to, "foreign"), "foreign");
						return "not-moved";
					},
				},
				async () =>
	...
	it("preserves a foreign directory swapped into the atomic move and rollback residue", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-recovery-atomic-source-swap-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
	...
	it("does not overwrite a rollback destination that appears before the no-clobber rollback link", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-recovery-rollback-destination-race-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const parked = `${context.lockPath}.parked-lock-${SUCCESSOR_TOKEN}`;
			...
			assert.equal(
				await readFile(join(context.lockPath, "foreign"), "utf-8"),
				foreign,
			);
	...
	it("preserves a checkpoint replacement and foreign completed receipt collision", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-recovery-completion-collision-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await writeLockOwner(cwd, validLockOwner());
			...
			await withPointerDependencies(
				{
					...
					atomicRenameNoReplace: async (from, to) => {
						if (from === checkpoint) {
							await writeFile(to, "foreign receipt");
							return "not-moved";
						}
						await rename(from, to);
						return "moved";
			...
			assert.equal(existsSync(checkpoint), true);
			assert.equal(
				await readFile(checkpoint.replace(/\.json$/, ".completed"), "utf8"),
				"foreign receipt",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("fails closed before moving a lock directory with a foreign extra entry", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-recovery-foreign-entry-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await writeLockOwner(cwd, validLockOwner());
			...
			assert.equal(moved, false);
			assert.equal(
				await readFile(join(context.lockPath, "foreign"), "utf8"),
				"foreign",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("resumes legacy base-v1 recovery and v2 source and parked partial recovery", async () => {
		for (const state of ["v1-source", "v2-source", "v2-parked"] as const) {
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-session-recovery-legacy-${state}-`),
			);
			try {
				const context = resolveSessionPointerContext(cwd);
				const ownerPath = join(context.lockPath, "owner.json");
				...
				await withPointerDependencies(
					{
						token: () => {
							throw new Error("legacy resume must not mint");
						},
						probePid: () => "dead",
						atomicRenameNoReplace: async (from, to) => {
							if (existsSync(to)) return "not-moved";
				...
				assert.equal(
					existsSync(checkpoint.replace(/\.json$/, ".completed")),
					true,
				);
				assert.equal(existsSync(lockPark), true);
			} finally {
	...
	it("preserves recovery evidence when atomic directory quarantine is unsupported", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-recovery-unsupported-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await writeLockOwner(cwd, validLockOwner());
	...
	it("distinguishes a paused live pre-rename owner from the same owner after SIGKILL", async (t) => {
		if (process.platform !== "linux") {
			t.skip(
				"Linux process start identity is required for deterministic PID reuse protection.",
			);
			return;
		}
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-sigkill-"),
		);
		const child = spawn(
			process.execPath,
			["-e", "setInterval(() => {}, 1_000)"],
		...
		try {
			...
			await withPointerDependencies(
				...
				async () => {
					orphaned = await recoverSessionPointerLock(cwd);
				},
			);
	...
	it("fails closed for live pre-rename, PID reuse, malformed, and ambiguous lock evidence", async () => {
		const cases: Array<{
			...
			{
				name: "live-temp",
				files: [[`owner.${TEST_TOKEN}.tmp`, JSON.stringify(validLockOwner())]],
				probePid: "alive",
				observation: matchingObservation(),
			},
			{
				name: "reused",
				files: [["owner.json", JSON.stringify(validLockOwner())]],
				probePid: "alive",
				observation: {
					kind: "identity",
					identity: { platform: "linux", birth: "2" },
			...
			{ name: "malformed", files: [["owner.json", "{"]], probePid: "dead" },
			{
				name: "ambiguous",
				files: [
					["owner.json", JSON.stringify(validLockOwner())],
					[
						`owner.${SUCCESSOR_TOKEN}.tmp`,
						JSON.stringify(validLockOwner({ token: SUCCESSOR_TOKEN })),
					],
				],
				probePid: "dead",
			},
		...
		for (const testCase of cases) {
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-session-lock-recovery-${testCase.name}-`),
			);
			try {
				const context = resolveSessionPointerContext(cwd);
				await mkdir(context.lockPath, { recursive: true });
				await Promise.all(
					testCase.files.map(
						async ([name, contents]) =>
							await writeFile(join(context.lockPath, name), contents, "utf-8"),
					),
				);
				await withPointerDependencies(
					{
	...
	it("claims stale evidence before quarantine so a successor cannot be mistaken for it", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-race-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			await mkdir(context.lockPath, { recursive: true });
			...
			assert.equal(
				existsSync(`${context.lockPath}.parked-lock-${SUCCESSOR_TOKEN}`),
				true,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("does not claim a successor lock when the inspected orphan is displaced before the exact temp rename", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-successor-race-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			...
			assert.equal(
				await readFile(successorTemp, "utf-8"),
				JSON.stringify(validLockOwner({ token: SUCCESSOR_TOKEN })),
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("does not report recovery after a successor live lock appears during parked-lock removal", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-lock-recovery-post-claim-race-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const staleTemp = join(context.lockPath, `owner.${TEST_TOKEN}.tmp`);
	...
	it("retries a positively matching live owner with the bounded 25/50/100 schedule before timing out", async () => {
		...
		try {
			...
			assert.equal(
				existsSync(resolveSessionPointerContext(cwd).lockPath),
				true,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("publishes a legacy-like Linux state when process identity capture fails", async (t) => {
		...
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-identity-capture-failure-"),
		);
		try {
			await withPointerDependencies(
				{
				...
				async () => {
					const state = await writeSessionStart(
						cwd,
						"sess-identity-capture-failure",
						{ platform: "linux" },
					);
					...
					assert.ok(persisted);
					assert.equal(
						Object.hasOwn(persisted, "identity_schema_version"),
						false,
					);
	...
	it("publishes the owner and pointer on Windows when regular-file fsync reports EPERM", async () => {
		...
		try {
			...
			assert.equal(
				syncCalls,
				2,
				"owner publication and pointer publication must both attempt fsync",
			);
			assert.equal(
				existsSync(join(context.lockPath, "owner.json")),
				false,
				"lock owner is removed after commit",
			);
			assert.equal(
				JSON.parse(await readFile(context.sessionPath, "utf8")).session_id,
				"sess-windows-eperm",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("emits one post-release warning for a degraded session start and stays silent for synced or failed transactions", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-durability-warning-"),
		);
		const originalWrite = process.stderr.write;
		...
		process.stderr.write = ((value: string) => {
			warnings.push({
				value,
				lockExists: existsSync(
					resolveSessionPointerContext(warningCwd).lockPath,
				),
			});
			return true;
		}) as typeof process.stderr.write;
			...
			await withPointerDependencies(
				{
					fs: {
						rename: async (from, to) => {
							if (to === resolveSessionPointerContext(failedCwd).sessionPath)
				...
				async () => {
					await assert.rejects(
						writeSessionStart(failedCwd, "sess-failed-start", {
							platform: "win32",
							regularFileSync,
						}),
					);
		...
		const failures = [
			{
				...
				dependencies: (
					context: ReturnType<typeof resolveSessionPointerContext>,
				) => ({
					fs: {
	...
	it("preserves owned pointer residue and ordered cleanup evidence when cleanup or release cannot complete", async () => {
		...
		try {
					...
					await assert.rejects(
						...
						(error: unknown) =>
							...
							error.secondaryFailures
								...
								.join(",") === "remove-pointer-temp,remove-release-dir" &&
			...
			assert.equal(
				existsSync(`${context.lockPath}.release-${TEST_TOKEN}`),
				true,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("leaves foreign pointer temps inert and a successor commits after explicit lock recovery", async () => {
		...
		try {
			...
			await withPointerDependencies(
				...
				async () => {
					await assert.rejects(
						writeSessionStart(cwd, "sess-first", { platform: "win32" }),
						isSessionPointerLaunchAbort,
					);
				},
			);
	...
	it("binds an owner alias only on a verified same-native/absent transition and never during replacement", async () => {
		...
		try {
			await withPointerDependencies(
				...
				async () => {
					const nativeOnly = await reconcileNativeSessionStart(
						cwd,
						"native-first",
						{
							platform: "win32",
					...
					await withOwnerEnvironment("omx-owner", async () => {
						const bound = await reconcileNativeSessionStart(
							cwd,
							"native-first",
							{
								platform: "win32",
	...
	it("keeps native session owner sidecars isolated and rejects live cross-process reuse", async () => {
		...
		try {
			await withPointerDependencies(
				...
				async () => {
					const first = await writeNativeSessionOwner(cwd, "native-owner-a", {
						pid: 11,
						platform: "win32",
					});
					const second = await writeNativeSessionOwner(cwd, "native-owner-b", {
						pid: 22,
						platform: "win32",
					});
					...
					assert.equal(
						(JSON.parse(await readFile(firstPath, "utf-8")) as SessionState)
							.pid,
						11,
					);
					assert.equal(
						(JSON.parse(await readFile(secondPath, "utf-8")) as SessionState)
							.pid,
						22,
					);
					...
					await assert.rejects(
						writeNativeSessionOwner(cwd, "native-owner-a", {
							pid: 22,
							platform: "win32",
						}),
						(error: unknown) =>
	...
	it("rejects a symlinked native owner state root", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-owner-root-symlink-"),
		);
		try {
			...
			const ownerDir = join(
				externalState,
				"sessions",
				"native-owner-root-link",
			);
			await mkdir(ownerDir, { recursive: true });
			await writeFile(
				join(ownerDir, "session-owner.json"),
				JSON.stringify({
			...
			assert.equal(
				(await readNativeSessionOwnerEvidence(cwd, "native-owner-root-link"))
					.status,
				"malformed",
			);
		} finally {
	...
	it("rejects symlinked native owner directories and sidecars", async () => {
		...
		try {
			...
			await writeFile(
				join(externalDir, "session-owner.json"),
				JSON.stringify({
			...
			assert.equal(
				(await readNativeSessionOwnerEvidence(cwd, "native-owner-dir-link"))
					.status,
				"malformed",
			);
			...
			assert.equal(
				(await readNativeSessionOwnerEvidence(cwd, "native-owner-file-link"))
					.status,
				"malformed",
			);
			...
			await writeFile(
				join(swappedOwnerDir, "session-owner.json"),
				JSON.stringify({
			...
			await withPointerDependencies(
				{
					fs: {
						lstat: async (path) => {
							...
							const simulateSwap =
								path === swappedOwnerDir && ++swappedOwnerLstatCalls === 2;
							return {
				...
				async () => {
					assert.equal(
						(await readNativeSessionOwnerEvidence(cwd, swappedOwnerId)).status,
						"malformed",
					);
				},
			);
	...
	it("rejects owner sidecars recorded for another platform", async () => {
		...
		try {
			...
			const forgedPlatform: NodeJS.Platform =
			...
			assert.equal(
				(await readNativeSessionOwnerEvidence(cwd, "native-owner-platform"))
					.status,
				"identity-indeterminate",
			);
			assert.equal(
				await readNativeSessionOwner(cwd, "native-owner-platform"),
				null,
			);
			await assert.rejects(
	...
	it("replaces only stale-dead owner evidence and preserves malformed evidence", async () => {
		...
		try {
			assert.equal(
				(await readNativeSessionOwnerEvidence(cwd, "native-owner-absent"))
					.status,
				"absent",
			);
			await withPointerDependencies(
				{
					...
					observeProcess: () => ({
						kind: "identity",
						identity: { platform: "linux" as const, birth: "1" },
					}),
				},
				async () => {
					await writeNativeSessionOwner(cwd, "native-owner-recovery", {
						pid: 11,
						platform: "linux",
					});
					assert.equal(
						(await readNativeSessionOwnerEvidence(cwd, "native-owner-recovery"))
							.status,
						"stale-dead",
					);
					...
					assert.equal(
						(
							await readNativeSessionOwnerEvidence(
								cwd,
								"native-owner-malformed",
							)
						).status,
					...
					await assert.rejects(
						writeNativeSessionOwner(cwd, "native-owner-malformed", {
							pid: 22,
							platform: "win32",
						}),
						(error: unknown) =>
					...
					assert.equal(
						await readNativeSessionOwner(cwd, "native-owner-forged"),
						null,
					);
					await assert.rejects(
						writeNativeSessionOwner(cwd, "native-owner-forged", {
							pid: 22,
							platform: "linux",
						}),
						(error: unknown) =>
					...
					assert.equal(
						await readNativeSessionOwner(cwd, "native-owner-missing-cwd"),
						null,
					);
					await assert.rejects(
						writeNativeSessionOwner(cwd, "native-owner-missing-cwd", {
							pid: 22,
							platform: "linux",
						}),
						(error: unknown) =>
					...
					assert.equal(
						await readFile(missingCwdPath, "utf-8"),
						missingCwdBefore,
					);
				},
			);
	...
	it("rejects cross-process native reconciliation while preserving the live selected pointer", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-cross-process-selected-"),
		);
		try {
			await withPointerDependencies(
				{
				...
				async () => {
					...
					await assert.rejects(
						reconcileNativeSessionStart(cwd, "native-selected-b", {
							pid: 22,
							platform: "win32",
						}),
						(error: unknown) =>
		...
		try {
			await withPointerDependencies(
				...
				async () => {
					...
					await assert.rejects(
						finalizeBoundOnce(established.binding, "history-failure"),
					);
					assert.equal(
						(await readSessionState(cwd))?.session_id,
						"sess-history",
					...
					assert.equal(
						await readFile(context.sessionPath, "utf-8"),
						"{ malformed",
					);
					assert.equal(
						existsSync(join(cwd, ".omx", "logs", "session-history.jsonl")),
...
describe("bound launch authority", () => {
	it("returns one private binding and only capability-free metadata updates", async () => {
		...
		try {
			const established = await establishLaunchSessionBinding(
				cwd,
				"sess-binding-authority",
			);
			assert.equal(established.kind, "committed-released");
			if (established.kind !== "committed-released") return;
			assert.match(
				established.binding.launchLineageToken,
				/^[A-Za-z0-9_-]{16,128}$/,
			);
			...
			const metadata = await updateDetachedSessionMetadata(
				established.binding,
				{
					tmuxSessionName: "omx-authority",
					tmuxPaneId: "%42",
				},
			);
			assert.equal(metadata.kind, "committed-released");
			assert.equal(
				(await readSessionState(cwd))?.launch_lineage_token,
				established.binding.launchLineageToken,
			);
			const firstFinalization = finalizeBoundOnce(established.binding, "test");
			const secondFinalization = finalizeBoundOnce(
				established.binding,
				"duplicate",
			);
			...
			assert.equal(
				existsSync(resolveSessionPointerContext(cwd).sessionPath),
				false,
			);
			assert.equal(
				(await closeLaunchSessionBindingOnce(established.binding)).status,
				"closed",
			);
			assert.equal(
				(await closeLaunchSessionBindingOnce(established.binding)).status,
	...
	it("preserves process_identity across detached metadata updates", async (t) => {
		...
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-identity-preserve-"),
		);
		let binding: LaunchSessionBinding | undefined;
		try {
			await withPointerDependencies(
				...
				async () => {
					const established = await establishLaunchSessionBinding(
						cwd,
						"sess-binding-identity-preserve",
						{ platform: "linux" },
					);
					assert.equal(established.kind, "committed-released");
					if (established.kind !== "committed-released") return;
					...
					assert.equal(
						(await finalizeBoundOnce(establishedBinding, "identity-preserved"))
							.finalized,
						true,
					);
				},
	...
	it("anchors capability acquisition and finalization to context.baseStateDir rather than cwd", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-selected-root-"),
		);
		let binding: LaunchSessionBinding | undefined;
		try {
			...
			assert.equal(
				(await finalizeBoundOnce(binding, "selected-root")).finalized,
				true,
			);
	...
	it("rejects forged and changed-incarnation detached metadata before lifecycle mutation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-metadata-hostile-"),
		);
		let binding: LaunchSessionBinding | undefined;
		try {
			const established = await establishLaunchSessionBinding(
				cwd,
				"sess-binding-metadata-hostile",
			);
			assert.equal(established.kind, "committed-released");
			if (established.kind !== "committed-released") return;
			binding = established.binding;
			...
			await withPointerDependencies(
				...
				async () => {
					const forged = await updateDetachedSessionMetadata(
						{ ...binding } as LaunchSessionBinding,
						{ tmuxPaneId: "%forged" },
					);
			...
			const denied = await updateDetachedSessionMetadata(binding, {
				tmuxPaneId: "%changed",
			});
	...
	it("rechecks supported directory identity under the metadata lock before pointer mutation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-metadata-race-"),
		);
		...
		try {
			...
			let result:
				| Awaited<ReturnType<typeof updateDetachedSessionMetadata>>
				| undefined;
			await withPointerDependencies(
				{
				...
				async () => {
					result = await updateDetachedSessionMetadata(
						binding as LaunchSessionBinding,
						{ tmuxPaneId: "%race" },
					);
				},
			...
			assert.equal(
				await readFile(
					join(retainedCwd, ".omx", "state", "session.json"),
					"utf-8",
				),
				original,
			);
	...
	it("permits an ordinary absent launch when directory identity is unsupported", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-unsupported-"),
		);
		let binding: LaunchSessionBinding | undefined;
		try {
			const established = await establishLaunchSessionBinding(
				cwd,
				"sess-binding-unsupported",
				{ platform: "win32" },
			);
			...
			if (established.kind !== "committed-released") return;
			binding = established.binding;
			assert.deepEqual(binding.directoryIdentity, {
				kind: "unsupported",
				reason: "platform",
			});
			assert.equal(
	...
	it("establishment preflight is absent-only under the selected pointer lock and preserves every existing pointer byte", async () => {
		const root = await mkdtemp(
			join(tmpdir(), "omx-session-binding-preflight-"),
		);
		try {
			const cases: Array<{
				name: string;
				raw: string;
				probePid?: "dead" | "indeterminate";
			}> = [
				{
					name: "usable",
					raw: JSON.stringify(
						makeState({
							cwd: join(root, "usable"),
							pid: process.pid,
							platform: "linux",
							pid_start_ticks: 1,
						}),
			...
			for (const testCase of cases) {
				...
				assert.ok(
					order.indexOf(`mkdir:${context.lockPath}`) <
						order.indexOf(`read:${context.sessionPath}`),
					testCase.name,
				);
				assert.equal(
					order.filter((entry) => entry === `rename:${context.sessionPath}`)
						.length,
					0,
					testCase.name,
				);
				assert.equal(
					order.filter((entry) => entry === `unlink:${context.sessionPath}`)
						.length,
					0,
					testCase.name,
				);
	...
	it("denies an absent bound pointer without creating synthetic terminal history", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-absent-finalization-"),
		);
		let binding: LaunchSessionBinding | undefined;
		try {
			const established = await establishLaunchSessionBinding(
				cwd,
				"sess-binding-absent-finalization",
			);
			assert.equal(established.kind, "committed-released");
			if (established.kind !== "committed-released") return;
			binding = established.binding;
			...
			assert.equal(
				existsSync(join(cwd, ".omx", "logs", `omx-${todayIsoDate()}.jsonl`)),
				true,
			);
		} finally {
			if (binding) await closeLaunchSessionBindingOnce(binding).catch(() => {});
	...
	it("rejects unbound usable finalization before changing any lifecycle bytes", async () => {
		...
		try {
			...
			await assert.rejects(
				() => writeSessionEnd(cwd, "sess-binding-unbound"),
				isSessionPointerLaunchAbort,
			);
	...
	it("preserves a compatible tokenless pointer and refuses to establish cleanup authority from it", async () => {
		...
		try {
			...
			await assert.rejects(
				() =>
					writeSessionStart(cwd, "native-compatible-tokenless", {
						tmuxPaneId: "%42",
					}),
				isSessionPointerLaunchAbort,
			);
	...
	it("preserves a compatible malformed lineage token without granting cleanup authority", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-malformed-token-"),
		);
		try {
			const context = resolveSessionPointerContext(cwd);
			const tokenless = await reconcileNativeSessionStart(
				cwd,
				"native-compatible-malformed-token",
			);
			await writeFile(
				context.sessionPath,
				JSON.stringify({ ...tokenless, launch_lineage_token: "bad" }),
				"utf-8",
			);
			...
			await assert.rejects(
				() =>
					writeSessionStart(cwd, "native-compatible-malformed-token", {
						tmuxPaneId: "%42",
					}),
				isSessionPointerLaunchAbort,
			);
			...
			assert.match(
				afterUpdate,
				/"launch_lineage_token":"bad"|"launch_lineage_token": "bad"/,
			);
			assert.equal(
				(
					await reconcileNativeSessionStart(
						cwd,
						"native-compatible-malformed-token",
					)
				).launch_lineage_token,
				"bad",
			);
	...
	it("finalizes only its exact stale-dead native pointer after history is retained", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-stale-dead-"),
		);
		try {
			const established = await establishLaunchSessionBinding(
				cwd,
				"sess-binding-stale-dead",
			);
			assert.equal(established.kind, "committed-released");
			if (established.kind !== "committed-released") return;
			await reconcileNativeSessionStart(cwd, "native-binding-stale-dead", {
				pid: 424242,
			});
	...
	it("preserves a stale-dead pointer whose bound lineage token does not match", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-stale-mismatch-"),
		);
		let binding: LaunchSessionBinding | undefined;
		try {
			const established = await establishLaunchSessionBinding(
				cwd,
				"sess-binding-stale-mismatch",
			);
			assert.equal(established.kind, "committed-released");
			if (established.kind !== "committed-released") return;
			binding = established.binding;
			const context = resolveSessionPointerContext(cwd);
			const state = await reconcileNativeSessionStart(
				cwd,
				"native-binding-stale-mismatch",
				{ pid: 424242 },
			);
			await writeFile(
				context.sessionPath,
				JSON.stringify({ ...state, launch_lineage_token: FOREIGN_TOKEN }),
				"utf-8",
			);
			await withPointerDependencies({ probePid: () => "dead" }, async () => {
				const denied = await finalizeBoundOnce(
					binding as LaunchSessionBinding,
					"test",
				);
	...
	it("preserves usable and stale bound pointers without the exact valid lineage token before any mutation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-token-gate-"),
		);
		let binding: LaunchSessionBinding | undefined;
		try {
			const established = await establishLaunchSessionBinding(
				cwd,
				"sess-binding-token-gate",
			);
			assert.equal(established.kind, "committed-released");
			if (established.kind !== "committed-released") return;
			binding = established.binding;
			...
			const cases: Array<{
				...
				{
					name: "malformed usable",
					state: { ...state, launch_lineage_token: "bad" },
				},
				{
					name: "mismatched usable",
					state: { ...state, launch_lineage_token: FOREIGN_TOKEN },
				},
				{
					name: "replaced usable",
					state: { ...state, launch_lineage_token: SUCCESSOR_TOKEN },
				},
				{
					name: "same-token changed incarnation",
					state: { ...state, started_at: "1999-01-01T00:00:00.000Z" },
				},
				{
					name: "unknown identity",
					state: { ...state, pid: 424242 },
					probePid: "indeterminate",
				},
				{
					...
					state: (() => {
						...
						return {
							...tokenless,
							native_session_id: "native-tokenless-stale",
							pid: 424242,
						};
					})(),
					probePid: "dead",
			...
			for (const testCase of cases) {
				...
				await withPointerDependencies(
					{
						...
						fs: {
							...
							unlink: async (path) => {
								if (path === context.sessionPath) unlinkCalls += 1;
								await rm(path, { force: false });
							},
						},
					},
				...
				assert.equal(
					...
					lifecycleBytes.history,
					testCase.name,
				);
				assert.equal(
					await readFile(dailyPath, "utf-8"),
					lifecycleBytes.daily,
					testCase.name,
				);
				assert.equal(
					await readFile(hudPath, "utf-8"),
					lifecycleBytes.hud,
					testCase.name,
				);
				assert.equal(
					await readFile(metricsPath, "utf-8"),
					lifecycleBytes.metrics,
					testCase.name,
				);
			}
		} finally {
	...
	it("denies selected-root replacement under lock before pointer publication", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-establish-root-race-"),
		);
		...
		try {
			...
			let replaced = false;
			let established:
				| Awaited<ReturnType<typeof establishLaunchSessionBinding>>
				| undefined;
			await withPointerDependencies(
				{
					fs: {
				...
				async () => {
					established = await establishLaunchSessionBinding(
						cwd,
						"sess-establish-root-race",
					);
				},
			);
	...
	it("returns committed blocked evidence when selected root changes after pointer rename", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-establish-post-rename-race-"),
		);
		...
		try {
			...
			await withPointerDependencies(
				...
				async () => {
					established = await establishLaunchSessionBinding(
						cwd,
						"sess-establish-post-rename-race",
					);
				},
			);
			assert.ok(established);
	...
	it("denies same-path replacement before creating state, lock, history, or HUD artifacts", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-replaced-path-"),
		);
		const retainedCwd = `${cwd}-retained`;
		let binding: LaunchSessionBinding | undefined;
		try {
			...
			assert.equal(
				existsSync(join(cwd, ".omx", "state", "hud-state.json")),
				false,
			);
		} finally {
			if (binding) await closeLaunchSessionBindingOnce(binding).catch(() => {});
	...
	it("keeps native absent reconciliation tokenless without backfill", async () => {
		...
		try {
			assert.equal(
				(await reconcileNativeSessionStart(cwd, "native-tokenless"))
					.launch_lineage_token,
				undefined,
			);
			assert.equal(
	...
	it("allows authorized unsupported-capability stale-dead finalization without weakening bound lineage checks", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-binding-unsupported-lifecycle-"),
		);
		...
		try {
			await withPointerDependencies(
				...
				async () => {
					const ordinaryEstablished = await establishLaunchSessionBinding(
						cwd,
						"sess-unsupported-ordinary",
						{ platform: "win32" },
					);
					...
					assert.deepEqual(ordinary.directoryIdentity, {
						kind: "unsupported",
						reason: "platform",
					});

					const ordinaryFinalized = await finalizeBoundOnce(
						ordinary,
					...
					assert.equal(
						(await closeLaunchSessionBindingOnce(ordinary)).status,
						"closed",
					);

					const staleEstablished = await establishLaunchSessionBinding(
						cwd,
						"sess-unsupported-stale",
						{
					...
					await reconcileNativeSessionStart(cwd, "native-unsupported-stale", {
						pid: 424242,
						platform: "win32",
					});
					const context = resolveSessionPointerContext(cwd);
					...
					await withPointerDependencies(
						...
						async () => {
							const finalized = await finalizeBoundOnce(
								stale as LaunchSessionBinding,
								"stale-unsupported",
							);
							assert.equal(finalized.finalized, true);
							assert.equal(finalized.cleanup.comparison?.status, "matched");
							assert.match(
								finalized.cleanup.comparison?.reason ?? "",
								/unsupported-directory-capability:platform/,
							);
						},
					);
					assert.equal(existsSync(context.sessionPath), false);
			...
			if (ordinary)
				await closeLaunchSessionBindingOnce(ordinary).catch(() => {});
	...
	it("preserves native lineage and tmux metadata when rejecting a live native-ID replacement", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-session-native-lineage-metadata-"),
		);
		let binding: LaunchSessionBinding | undefined;
		try {
			const established = await establishLaunchSessionBinding(
				cwd,
				"sess-native-lineage-metadata",
				{
			...
			await assert.rejects(
				reconcileNativeSessionStart(cwd, "native-after", { pid: process.pid }),
				isOwnerConflict,
			);
			const persisted = await readSessionState(cwd);
			...
			await withPointerDependencies(
				{
					...
					fs: {
						unlink: async (path) => {
							if (path.endsWith(`.release-${TEST_TOKEN}/owner.json`))
								throw new Error("owner unlink failure");
							await rm(path, { force: false });
						},
						rmdir: async (path) => {
							rmdirCalls += 1;
				...
				async () => {
					const result = await establishLaunchSessionBinding(
						cwd,
						"sess-owner-unlink",
					);
					...
					if (result.kind === "committed-release-failed") {
						assert.equal(
							result.secondaryFailures[0]?.phase,
	...
	it("writes exactly one redacted record at the supplied selected root", async () => {
		...
		try {
			...
			const log = await readFile(
				join(cwd, "selected", ".omx", "logs", `omx-${todayIsoDate()}.jsonl`),
				"utf-8",
			);
