...
describe("explicit terminal stop model docs contract", () => {
	it("locks the canonical lifecycle vocabulary and legacy boundaries", () => {
		...
		assert.match(
			doc,
			/do \*\*not\*\* present as a canonical user-facing lifecycle outcome/i,
		);
	...
	it("documents lifecycle precedence in the state model", () => {
		...
		assert.match(
			doc,
			/Prefer a dedicated canonical lifecycle field over legacy `run_outcome`/i,
		);
	...
	it("keeps native hook docs aligned with lifecycle metadata precedence", () => {
		...
		assert.match(
			doc,
			/prefer explicit lifecycle metadata over assistant-text heuristics/i,
		);
		assert.match(
			doc,
			/legacy `blocked_on_user` still suppresses continuation/i,
		);
		assert.match(
			doc,
			/`cancelled` should be treated as internal legacy\/admin compatibility/i,
		);
		assert.match(
			doc,
			/no distinct native Codex `ask-user-question` hook today/i,
		);
	});
	...
	it("extends prompt guidance docs with the explicit terminal handoff rule", () => {
		...
		assert.match(
			doc,
			/name an explicit outcome such as `finished`, `blocked`, `failed`, `userinterlude`, or `askuserQuestion`/i,
		);
		...
		assert.match(
			doc,
			/dist\/hooks\/__tests__\/explicit-terminal-stop-docs-contract\.test\.js/i,
		);
	});
});
