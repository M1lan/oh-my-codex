...
import {
	...
	loadSurface,
} from "./prompt-guidance-test-helpers.js";
...
describe("prompt guidance wave two contract", () => {
	...
	it("wave-two prompts encode role-appropriate grounded-evidence wording", () => {
		...
		assert.match(
			loadSurface("prompts/test-engineer.md"),
			/recommendation is grounded/i,
		);
		assert.match(
			loadSurface("prompts/code-reviewer.md"),
			/review is grounded/i,
		);
		assert.match(
			loadSurface("prompts/quality-reviewer.md"),
			/review is grounded/i,
		);
		assert.match(
			loadSurface("prompts/code-reviewer.md"),
	...
	it("researcher encodes a docs-first technical research workflow", () => {
		...
		assert.match(
			researcher,
			/documentation structure before page-level fetches/i,
		);
		assert.match(
			researcher,
			/examples only after the docs baseline is grounded/i,
		);
	...
	it("researcher exposes cross-repo OSS research capability with structured citation format", () => {
		...
		assert.match(
			...
			"researcher must declare an explicit <repo_research> block for OSS evidence",
		);
		assert.match(
			researcher,
			/gh search code/i,
		...
		assert.match(
			...
			"researcher must allow pinned-SHA OSS file fetches via gh api or raw.githubusercontent.com",
		);
		assert.match(
			researcher,
			/Context7 MCP/i,
		...
		assert.match(
			researcher,
			/Never (?:use|cite)[\s\S]{0,30}HEAD|moving branch reference/i,
			"researcher must forbid moving-branch citations in OSS evidence",
		);
		assert.match(
			researcher,
			/already chosen technology/i,
			"researcher must keep the already-chosen-technology scope guard",
		);
		assert.match(
			researcher,
			/local repo usage[\s\S]{0,30}`explore`/i,
			"researcher must explicitly route local-repo inspection to explore while keeping external OSS repos in scope",
		);
	});
	...
	it("research specialists keep explicit output-contract fixtures for source preference and boundary discipline", () => {
		...
		assert.match(
			researcher,
			/Prefer official documentation.*over third-party summaries/i,
		);
		assert.match(
			researcher,
			/Version compatibility or version uncertainty is noted when relevant|### Version Note/i,
		);
		...
		assert.match(
			dependencyExpert,
			/Compare at least 2 candidates|multiple candidates/i,
		);
		...
		assert.match(
			dependencyExpert,
			/whether \/\s*which package, SDK, or framework to adopt, upgrade, replace, or migrate/i,
		);
		assert.match(
			dependencyExpert,
			/boundary crossing upward.*researcher|report that boundary crossing upward for `researcher`/i,
		);
		...
		assert.match(
			explore,
			/dependency recommendation.*report that handoff upward|report that handoff upward/i,
		);
	});
	...
	it("code-reviewer rejects masking fallback and workaround patches unless narrowly justified", () => {
		...
		assert.match(
			codeReviewer,
			/fallback\/workaround code when it masks failures/i,
		);
		...
		assert.match(
			codeReviewer,
			/narrow compatibility fallback can be acceptable/i,
		);
	...
	it("code-review and verifier-adjacent prompts preserve merge-if-green as downstream context", () => {
		...
		assert.match(
			loadSurface("prompts/critic.md"),
			/later workflow condition|downstream context/i,
		);
