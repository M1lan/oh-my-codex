...
const rootAgents = existsSync(repoAgentsPath)
	? readFileSync(repoAgentsPath, "utf-8")
	: null;
...
describe("analyze routing contract", () => {
	it("routes analyze through the analyze skill file in the template AGENTS surface", () => {
		...
		assert.match(
			templateRow,
			/read-only deep analysis with ranked synthesis, explicit confidence, and concrete file references/i,
		);
