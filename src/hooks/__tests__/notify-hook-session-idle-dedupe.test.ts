...
describe("notify-hook session-idle dedupe", () => {
	it("suppresses repeated unchanged post_turn_idle_notification hook events once the first hook dispatch succeeds", async () => {
		...
		try {
			...
			await writeFile(
				...
				JSON.stringify({ session_id: SESSION_ID }, null, 2),
			);
			await writeFile(
				join(hooksDir, "session-idle-counter.mjs"),
				buildSessionIdlePlugin(pluginStatePath),
				"utf-8",
			);
			...
			const pluginState = await readJson<{
				count: number;
				last_reason: string;
				last_status: string;
			}>(pluginStatePath);
	...
	it("writes session-idle hook state into the fork session scope when OMX_SESSION_ID targets a fork", async () => {
		...
		try {
			...
			await writeFile(
				join(stateDir, "session.json"),
				JSON.stringify({ session_id: canonicalSessionId }, null, 2),
			);
			await writeFile(
				join(hooksDir, "session-idle-counter.mjs"),
				buildSessionIdlePlugin(pluginStatePath),
	...
	it("keeps post_turn_idle_notification hook dedupe active even when lifecycle cooldown is disabled", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-notify-idle-dedupe-zero-cooldown-"),
		);
		const repoRoot = join(
			dirname(fileURLToPath(import.meta.url)),
			"..",
