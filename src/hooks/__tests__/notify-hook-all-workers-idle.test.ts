...
describe("notify-hook all-workers-idle notification", () => {
	it("sends notification to leader when all workers are idle", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/send-keys -t .*devsess:0/,
					"must not inject to session when leader pane is missing",
				);
			}
			...
			assert.ok(
				...
				"events.ndjson should exist after deferred notification",
			);
			...
			assert.ok(
				deferredEvent,
				"should emit leader_notification_deferred with missing-pane reason",
			);
			...
			assert.ok(
				existsSync(idleStatePath),
				"cooldown state should be written even for deferred delivery",
			);
	...
	it("writes deferred visibility once per cooldown window when leader pane is missing", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			const first = runNotifyHookAsWorker(
				cwd,
				fakeBinDir,
				`${teamName}/worker-1`,
			...
			const second = runNotifyHookAsWorker(
				cwd,
				fakeBinDir,
				`${teamName}/worker-1`,
				{ OMX_TEAM_ALL_IDLE_COOLDOWN_MS: "600000" },
			...
			const deferredEvents = events.filter(
				(event: { type?: string; reason?: string }) =>
					...
					event.reason === "leader_pane_missing_no_injection",
			);
			assert.equal(
				deferredEvents.length,
				1,
				"cooldown should bound repeated deferred all-workers-idle artifacts",
			);
		});
	});
...
`;
			...
			assert.match(
				tmuxLog,
				/display-message -p -t %181 #\{pane_current_command\}/,
			);
			assert.doesNotMatch(
				tmuxLog,
				/send-keys -t %181/,
				"must not inject into shell pane",
			);
			...
			const deferred = events.find(
				(event: { type?: string; reason?: string }) =>
					event.type === "leader_notification_deferred" &&
					event.reason === "leader_pane_shell_no_injection",
			);
			assert.ok(
				deferred,
				"should defer shell-pane all-workers-idle notification",
			);
			assert.equal(deferred.pane_current_command, "zsh");

			const idleState = JSON.parse(
				await readFile(join(teamDir, "all-workers-idle.json"), "utf-8"),
			);
...
`;
			...
			assert.match(
				tmuxLog,
				/send-keys -t %182/,
				"all-workers-idle reminder should still inject into a busy leader pane",
			);
			...
			if (existsSync(eventsPath)) {
				...
				assert.equal(
					deferred,
					undefined,
					"busy leader panes must not suppress all-workers-idle reminders",
				);
			}
		});
	});
		...
		await withTempWorkingDir(async (cwd) => {
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/\[OMX\] All .* idle/,
					"should NOT send all-idle message when some workers are busy",
				);
			}
		});
	});
		...
		await withTempWorkingDir(async (cwd) => {
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/\[OMX\] All .* idle/,
					"stale heartbeat should suppress all-workers-idle notification",
				);
			}
		});
	});
		...
		await withTempWorkingDir(async (cwd) => {
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/\[OMX\] All .* idle/,
					"cooldown should block repeated notification",
				);
			}
		});
	});
	...
	it("writes all_workers_idle event to events.ndjson", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.ok(
				existsSync(eventsPath),
				"events.ndjson should exist after notification",
			);
		...
		await withTempWorkingDir(async (cwd) => {
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/\[OMX\] All .* idle/,
					"leader context should not send all-idle notification",
				);
			}
		});
	});
	...
	it("handles single worker team correctly with singular message", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/\[OMX\] All 1 worker idle/,
				"single worker uses singular form",
			);
			assert.match(
				tmuxLog,
				/Run `omx team status solo-team` now, read unread worker messages, then assign the next concrete task, reconcile results, or shut the team down/,
				"all-workers-idle notification should tell the agent to execute the runtime status check directly",
			);
			assert.doesNotMatch(
				tmuxLog,
				/Next: run omx team status solo-team, read unread worker messages, then decide whether to assign the next concrete task, reconcile results, or shut the team down/,
				"all-workers-idle notification should not fall back to human-advisory wording",
			);
			assert.doesNotMatch(
				tmuxLog,
				/All 1 workers idle/,
				"should not use plural for single worker",
			);
		});
	});
	...
	it("uses manifest.v2.json over config.json when both present", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/-t %123/,
				"should use leader_pane_id from manifest.v2.json",
			);
			assert.doesNotMatch(
				tmuxLog,
				/wrong-session/,
				"should not use tmux_session from config.json",
			);
		});
	});
});
