...
const FORBIDDEN_PROMPT_PATTERNS: Array<[label: string, pattern: RegExp]> = [
	...
	[
		"child spawn-agent phrasing",
		/Spawn\s+the\s+`explore`\s+agent|spawn\s+explore\s+agent/i,
	],
	[
		"direct delegate-to-agent phrasing",
		/delegate to specialized agents|delegate to\s+[a-z-]+\s+agent/i,
	],
	["soft explore-agent routing", /use explore agent|via explore agent/i],
	[
		"soft next-agent chain phrasing",
		/next agent in the chain|next agent \(researcher|next agent \(analyst|next agent \(.*planner/i,
	],
	["soft delegated-checklist phrasing", /delegated to test-engineer/i],
	["legacy explore-high escalation", /explore-high/i],
	[
		"external AI routing",
		/Use an external AI assistant|Use an external long-context AI assistant/i,
	],
];
...
describe("prompt orchestration boundary", () => {
	...
	it("guidance schema documents upward-only handoff limits for role prompts", () => {
		assert.match(
			loadSurface("docs/guidance-schema.md"),
			/report upward, do not recursively orchestrate/i,
		);
		assert.match(
			loadSurface("docs/guidance-schema.md"),
			/recommend handoffs upward to the orchestrator/i,
		);
	});
});
