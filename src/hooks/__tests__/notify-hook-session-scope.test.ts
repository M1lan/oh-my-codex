...
describe("notify-hook session-scoped iteration updates", () => {
	...
	it("increments iteration for active session-scoped mode states", async () => {
		...
		try {
			...
			await writeFile(
				...
				JSON.stringify({ active: true, iteration: 0 }),
			);
	...
	it("prefers the invocation OMX session id over the persisted canonical session for notify sidefiles when a fork scope exists", async () => {
		...
		try {
			...
			const forkSkillState = JSON.parse(
				await readFile(join(forkDir, "skill-active-state.json"), "utf8"),
			);
			assert.equal(forkSkillState.owner_codex_session_id, nativeSessionId);
			const forkRalphState = JSON.parse(
				await readFile(join(forkDir, "ralph-state.json"), "utf8"),
			);
			...
			const forkRalphAfterSecond = JSON.parse(
				await readFile(join(forkDir, "ralph-state.json"), "utf8"),
			);
			assert.equal(
				forkRalphAfterSecond.owner_codex_session_id,
				nativeSessionId,
			);
			assert.equal(
				existsSync(
	...
	it("rejects a foreign-owned notify fork before receipts, activation, or Ralph mutation", async () => {
		...
		try {
			...
			const rejectionLines = diagnostics
				.split("\n")
			...
			assert.doesNotMatch(
				rejectionLines[0],
				/foreign-codex-owner|codex-payload-owner|omx-existing-fork|\$ralph/,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
		...
		try {
			...
			await writeFile(
				...
				JSON.stringify({
					...
					native_session_id: "payload-owner",
				}),
			);
