...
describe("triagePrompt — LIGHT/researcher", () => {
	...
	it("routes Korean official documentation lookup prompts to researcher", () => {
		assertLightDestination(
			"OpenAI Responses API 공식 문서 찾아줘",
			"researcher",
		);
	});
	...
	it("routes official-doc question prompts to researcher instead of explore", () => {
		assertLightDestination(
			"where can I find official docs for OpenAI Responses API?",
			"researcher",
		);
	});
	...
	it("routes endpoint-shaped official-doc lookups to researcher instead of local explore", () => {
		assertLightDestination(
			"find official docs for api/v1/responses",
			"researcher",
		);
	});
	...
	it("routes external URL-shaped official-doc lookups with repo paths to researcher", () => {
		assertLightDestination(
			"find official docs for github.com/org/repo/src/foo.ts",
			"researcher",
		);
		assertLightDestination(
			"find official docs for github.com/org/repo/src/server",
			"researcher",
		);
	});
	...
	it("does not steal implementation-shaped official-doc prompts from HEAVY", () => {
		const result = triagePrompt(
			"implement auth using official docs for the SDK",
		);
		assert.equal(
			result.lane,
			"HEAVY",
			`expected HEAVY got ${result.lane} (reason=${result.reason})`,
		);
...
describe("triagePrompt — HEAVY", () => {
	...
	it("sets destination=autopilot for HEAVY decisions", () => {
		...
		assert.equal(
			result.destination,
			"autopilot",
			`expected destination=autopilot got ${result.destination}`,
		);
	});
});
