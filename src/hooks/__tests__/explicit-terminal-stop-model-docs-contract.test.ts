...
describe("explicit terminal stop model docs contract", () => {
	...
	it("keeps cancelled internal-only and preserves blocked_on_user as a legacy compatibility value", () => {
		...
		assert.match(
			doc,
			/`blocked_on_user` \| compatibility-only user-wait signal/i,
		);
		assert.match(
			doc,
			/`cancelled`, `canceled`, `abort`, `aborted` \| internal legacy\/admin stop compatibility only/i,
		);
		assert.match(
			doc,
			/It is \*\*not\*\* a canonical user-facing terminal lifecycle outcome/i,
		);
	});
	...
	it("distinguishes userinterlude from askuserQuestion", () => {
		...
		assert.match(
			doc,
			/`userinterlude` \| The user intentionally interrupted or paused the run\./i,
		);
		assert.match(
			doc,
			/`askuserQuestion` \| OMX must ask the user a blocking question before safe progress can continue\./i,
		);
	...
	it("documents canonical-field precedence over legacy compatibility fields", () => {
		...
		assert.match(
			doc,
			/a dedicated canonical lifecycle field such as `lifecycle_outcome`/i,
		);
