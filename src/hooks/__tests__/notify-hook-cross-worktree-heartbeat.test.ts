	...
	it("logs only the latest user input preview instead of concatenating prior inputs", async () => {
		await withTempDir(async (root) => {
			...
			await writeFile(
				...
				JSON.stringify({ session_id: "thread-latest-preview" }),
			);
	...
	it("writes heartbeat under OMX_TEAM_STATE_ROOT even when payload cwd is a different worktree", async () => {
		await withTempDir(async (root) => {
			...
			assert.equal(
				...
				true,
				"heartbeat should be written to leader-owned state root",
			);
			...
			assert.equal(
				existsSync(wrongHeartbeatPath),
				false,
				"heartbeat should not be written under worker cwd state root",
			);
		});
	});
	...
	it("keeps leader-owned heartbeat state when worker cwd uses the team worktree layout", async () => {
		await withTempDir(async (root) => {
			...
			assert.equal(
				existsSync(heartbeatPath),
				true,
				"heartbeat should still resolve to leader-owned team state",
			);
			...
			assert.equal(
				existsSync(wrongHeartbeatPath),
				false,
				"team worktree cwd should not become the authoritative team state root",
			);
		});
	});
	...
	it("falls back to worker identity/config metadata when OMX_TEAM_STATE_ROOT is absent", async () => {
		await withTempDir(async (root) => {
			...
			assert.equal(
				existsSync(heartbeatPath),
				true,
				"heartbeat should resolve via metadata to leader-owned team state root",
			);
		});
	});
});
