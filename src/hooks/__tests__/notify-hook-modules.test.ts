...
describe("notify-hook/operational-events – classifyExecCommand", () => {
	it("classifies concrete test commands without matching search commands", async () => {
		const { classifyExecCommand } = await loadModule(
			"notify-hook/operational-events.js",
		);
		assert.deepEqual(classifyExecCommand("npm test"), {
			kind: "test",
			command: "npm test",
		});
...
describe("notify-hook/operational-events – parseCommandResult", () => {
	it("extracts exit code and PR metadata from command output", async () => {
		const { parseCommandResult } = await loadModule(
			"notify-hook/operational-events.js",
		);
...
describe("notify-hook/operational-events – buildOperationalContext", () => {
	it("resolves a stable session_name from cwd + session id", async () => {
		const { buildOperationalContext } = await loadModule(
			"notify-hook/operational-events.js",
		);
	...
	it("writes PR lifecycle details into the repo baseline registry as best effort state", async () => {
		...
		const { mkdtemp, rm, writeFile, readFile } = await import(
			"node:fs/promises"
		);
		const { tmpdir } = await import("node:os");
		const { join } = await import("node:path");
		...
		try {
			...
			execFileSync("git", ["checkout", "-b", "feature/opctx-pr"], {
				cwd: repo,
				stdio: "ignore",
			});
			...
			const baseline = JSON.parse(
				await readFile(
					join(repo, ".omx", "state", "current-task-baseline.json"),
					"utf-8",
				),
			);
			const entry = baseline.tasks.find(
				(item: { branch_name: string }) =>
					item.branch_name === "feature/opctx-pr",
			);
...
describe("notify-hook/operational-events – deriveAssistantSignalEvents", () => {
	it("detects handoff-needed and retry-needed from assistant text", async () => {
		const { deriveAssistantSignalEvents } = await loadModule(
			"notify-hook/operational-events.js",
		);
		const signals = deriveAssistantSignalEvents(
			"If you want, next I can do one of two things: retry the flaky step or handoff the follow-up.",
		);
		assert.equal(
			signals.some(
				(signal: { event?: string }) => signal.event === "handoff-needed",
			),
			true,
		);
		assert.equal(
			signals.some(
				(signal: { event?: string }) => signal.event === "retry-needed",
			),
			true,
		);
	});
	...
	it("avoids duplicate finished/failed assistant lifecycle signals", async () => {
		...
		assert.equal(
			deriveAssistantSignalEvents(
				"Implementation completed. Final summary ready.",
			).some((signal: { event?: string }) => signal.event === "finished"),
			false,
		);
		assert.equal(
			deriveAssistantSignalEvents(
				"The operation failed with error: unable to continue.",
			).some((signal: { event?: string }) => signal.event === "failed"),
			false,
		);
	});
});
...
describe("notify-hook/auto-nudge – detectStallPattern", () => {
	it("detects default continuation patterns case-insensitively", async () => {
		...
		assert.equal(
			detectStallPattern(
				"KEEP GOING and I will finish the patch.",
		...
		assert.equal(
			detectStallPattern(
				"Continue with the existing cleanup from here.",
				DEFAULT_STALL_PATTERNS,
			),
			true,
		);
	...
	it("detects team-worker continuation phrases like continue with and keep going", async () => {
		...
		assert.equal(
			detectStallPattern(
				"We can pick up with the cleanup after this.",
				DEFAULT_STALL_PATTERNS,
			),
			true,
		);
	...
	it("does not treat permission-seeking or planning prompts as auto-nudge stalls", async () => {
		...
		assert.equal(
			detectStallPattern("Shall I proceed?", DEFAULT_STALL_PATTERNS),
			false,
		);
		assert.equal(
			detectStallPattern(
				"If you want, I can refactor.",
				DEFAULT_STALL_PATTERNS,
			),
			false,
		);
		...
		assert.equal(
			detectStallPattern(
				"I can continue with the plan from here.",
				DEFAULT_STALL_PATTERNS,
			),
			false,
		);
	...
	it("returns false when no stall pattern present", async () => {
		...
		assert.equal(
			detectStallPattern("Refactoring complete.", DEFAULT_STALL_PATTERNS),
			false,
		);
	...
	it("works with custom patterns", async () => {
		const { detectStallPattern } = await loadModule(
			"notify-hook/auto-nudge.js",
		);
		const custom = ["awaiting approval"];
		assert.equal(
			detectStallPattern("Changes staged. Awaiting approval.", custom),
			true,
		);
		assert.equal(
			detectStallPattern("Would you like me to proceed?", custom),
			false,
		);
	});
	...
	it("keeps native Stop permission-seeking detection separate from default notify-hook detection", async () => {
		const {
			...
			DEFAULT_STALL_PATTERNS,
		} = await loadModule("notify-hook/auto-nudge.js");
		...
		assert.equal(
			detectNativeStopStallPattern(text, DEFAULT_STALL_PATTERNS),
			true,
		);
	});
	...
	it("ignores orchestration intent tags when normalizing stall text", async () => {
		const { normalizeAutoNudgeSignatureText } = await loadModule(
			"notify-hook/auto-nudge.js",
		);
		assert.equal(
			normalizeAutoNudgeSignatureText(
				"Team alpha: 1 msg(s) for leader. [OMX_INTENT:pending-mailbox-review]",
			),
...
describe("notify-hook/auto-nudge – normalizeAutoNudgeConfig", () => {
	it("returns defaults when called with null", async () => {
		const { normalizeAutoNudgeConfig, DEFAULT_STALL_PATTERNS } =
			await loadModule("notify-hook/auto-nudge.js");
	...
	it("respects enabled=false", async () => {
		const { normalizeAutoNudgeConfig } = await loadModule(
			"notify-hook/auto-nudge.js",
		);
	...
	it("accepts custom patterns array", async () => {
		...
		const cfg = normalizeAutoNudgeConfig({
			patterns: ["awaiting input", "ping me"],
		});
...
describe("notify-hook/auto-nudge – resolveEffectiveAutoNudgeResponse", () => {
	it("maps legacy approval-style responses to a non-authorizing continuation message", async () => {
		const { DEFAULT_AUTO_NUDGE_RESPONSE, resolveEffectiveAutoNudgeResponse } =
			await loadModule("notify-hook/auto-nudge.js");
		assert.equal(
			resolveEffectiveAutoNudgeResponse("yes, proceed"),
			DEFAULT_AUTO_NUDGE_RESPONSE,
		);
		assert.equal(
			resolveEffectiveAutoNudgeResponse("continue"),
			DEFAULT_AUTO_NUDGE_RESPONSE,
		);
		assert.equal(
			resolveEffectiveAutoNudgeResponse("custom follow-up"),
			"custom follow-up",
		);
	});
});
...
describe("notify-hook/auto-nudge – normalizeSkillActiveState", () => {
	it("returns null for invalid input", async () => {
		const { normalizeSkillActiveState } = await loadModule(
			"notify-hook/auto-nudge.js",
		);
...
describe("notify-hook/auto-nudge – inferSkillPhaseFromText", () => {
	it("maps planning/executing/reviewing/completing signals", async () => {
		const { inferSkillPhaseFromText } = await loadModule(
			"notify-hook/auto-nudge.js",
		);
		assert.equal(
			inferSkillPhaseFromText("Here is the plan with next steps."),
			"planning",
		);
		assert.equal(
			inferSkillPhaseFromText("I implemented the patch and updated files."),
			"executing",
		);
		assert.equal(
			inferSkillPhaseFromText("I verified with tests and typecheck."),
			"reviewing",
		);
		assert.equal(
			inferSkillPhaseFromText("All tests pass. Completed with summary."),
			"completing",
		);
	});
	...
	it("falls back to the current phase when no signal is present", async () => {
		...
		assert.equal(
			inferSkillPhaseFromText("neutral text", "reviewing"),
			"reviewing",
		);
	});
});
...
describe("notify-hook/auto-nudge – blocked deep-interview auto approvals", () => {
	it("normalizes injected approval text before matching blocked inputs", async () => {
		const { normalizeBlockedAutoApprovalInput } = await loadModule(
			"notify-hook/auto-nudge.js",
		);
		assert.equal(
			normalizeBlockedAutoApprovalInput(" yes, proceed [OMX_TMUX_INJECT] "),
			"yes proceed",
		);
	});
	...
	it("matches each blocked approval keyword or phrase", async () => {
		const {
			isBlockedAutoApprovalInput,
			DEEP_INTERVIEW_BLOCKED_APPROVAL_INPUTS,
		} = await loadModule("notify-hook/auto-nudge.js");
		for (const blocked of DEEP_INTERVIEW_BLOCKED_APPROVAL_INPUTS) {
			assert.equal(
				isBlockedAutoApprovalInput(blocked),
				true,
				`expected blocked input ${blocked} to match`,
			);
		}
	});

	it("blocks combined yes/proceed injection text", async () => {
		const { isBlockedAutoApprovalInput } = await loadModule(
			"notify-hook/auto-nudge.js",
		);
	...
	it('treats actionable "Next I should ..." replies like continuation approval', async () => {
		...
		assert.equal(
			isBlockedAutoApprovalInput("Next I should update the focused tests."),
			true,
		);
		assert.equal(
			isBlockedAutoApprovalInput(
				"Maybe next I should update the focused tests.",
			),
			false,
		);
	});
	...
	it("normalizes custom blocked inputs before exact and prefix matching", async () => {
		...
		assert.equal(
			isBlockedAutoApprovalInput("GO ahead!!!", [" go ahead "]),
			true,
		);
		assert.equal(
			isBlockedAutoApprovalInput("Next I should update the focused tests.", [
	...
	it("infers success/error/abort lock release reasons", async () => {
		const { inferDeepInterviewReleaseReason } = await loadModule(
			"notify-hook/auto-nudge.js",
		);
		const baseState = {
		...
		assert.equal(
			inferDeepInterviewReleaseReason({
...
describe("notify-hook/orchestration-intent – taxonomy helpers", () => {
	it("appends and strips orchestration intent tags", async () => {
		const { appendOrchestrationIntentTag, stripOrchestrationIntentTags } =
			await loadModule("notify-hook/orchestration-intent.js");
		const tagged = appendOrchestrationIntentTag(
			"Team alpha: 1 msg(s) for leader.",
		...
		assert.equal(
			stripOrchestrationIntentTags(tagged).trim(),
			"Team alpha: 1 msg(s) for leader.",
		);
	});

	it("maps leader reminder reasons to stable orchestration intents", async () => {
		const { resolveLeaderNudgeIntent } = await loadModule(
			"notify-hook/orchestration-intent.js",
		);
		assert.equal(
			resolveLeaderNudgeIntent({ nudgeReason: "new_mailbox_message" }),
			"pending-mailbox-review",
		);
		assert.equal(
			resolveLeaderNudgeIntent({ nudgeReason: "ack_without_start_evidence" }),
			"followup-relaunch",
		);
		assert.equal(
			resolveLeaderNudgeIntent({ nudgeReason: "stuck_waiting_on_leader" }),
			"stalled-unblock",
		);
		assert.equal(
	...
	it("maps worker-state reminders to stable orchestration intents", async () => {
		const { resolveAllWorkersIdleIntent, resolveWorkerIdleIntent } =
			await loadModule("notify-hook/orchestration-intent.js");
		assert.equal(
			resolveAllWorkersIdleIntent("done_waiting_on_leader"),
			"done-review-or-shutdown",
		);
		assert.equal(
			resolveAllWorkersIdleIntent("stuck_waiting_on_leader"),
			"stalled-unblock",
		);
		assert.equal(
			resolveAllWorkersIdleIntent("still_actionable"),
			"followup-reuse",
		);
...
describe("notify-hook/team-worker – parseTeamWorkerEnv", () => {
	it("parses valid team/worker strings", async () => {
		const { parseTeamWorkerEnv } = await loadModule(
			"notify-hook/team-worker.js",
		);
		assert.deepEqual(parseTeamWorkerEnv("fix-ts/worker-1"), {
			teamName: "fix-ts",
			workerName: "worker-1",
		});
		assert.deepEqual(parseTeamWorkerEnv("my-team/worker-99"), {
			teamName: "my-team",
			workerName: "worker-99",
		});
		assert.deepEqual(parseTeamWorkerEnv("a/worker-0"), {
			teamName: "a",
			workerName: "worker-0",
		});
	});
...
describe("notify-hook/state-io – normalizeNotifyState", () => {
	it("returns defaults for null input", async () => {
		const { normalizeNotifyState } = await loadModule(
			"notify-hook/state-io.js",
		);
	...
	it("preserves valid recent_turns", async () => {
		...
		const s = normalizeNotifyState({
			recent_turns: { key: 123 },
...
describe("notify-hook/payload-parser – language reminder injection", () => {
	it("detects non-Latin script in user input", async () => {
		const { hasNonLatinScript } = await loadModule(
			"notify-hook/payload-parser.js",
		);
	...
	it("injects language reminder for non-Latin input", async () => {
		const { injectLanguageReminder, LANGUAGE_REMINDER_MARKER } =
			await loadModule("notify-hook/payload-parser.js");
		const prompt = injectLanguageReminder(
			"Continue from current mode state. [OMX_TMUX_INJECT]",
			"帮我修复这个问题",
		);
		...
		assert.match(
			prompt,
			new RegExp(
				LANGUAGE_REMINDER_MARKER.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
			),
		);
	});

	it("does not inject reminder for Latin-only text", async () => {
		const { injectLanguageReminder } = await loadModule(
			"notify-hook/payload-parser.js",
		);
		const prompt = injectLanguageReminder(
			"Continue [OMX_TMUX_INJECT]",
			"Please fix issue 253",
		);
