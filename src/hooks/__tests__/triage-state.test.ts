...
describe("readTriageState — malformed JSON returns null", () => {
	...
	before(() => {
		...
		mkdirSync(join(tmp, ".omx", "state", "sessions", "sess-malformed"), {
			recursive: true,
		});
...
describe("readTriageState — wrong shape returns null", () => {
	...
	it("returns null for {version:2} (wrong version)", () => {
		...
		mkdirSync(join(tmp, ".omx", "state", "sessions", "v2"), {
			recursive: true,
		});
	...
	it("returns null for {} (empty object)", () => {
		...
		mkdirSync(join(tmp, ".omx", "state", "sessions", "empty"), {
			recursive: true,
		});
	...
	it("returns null for missing suppress_followup field", () => {
		...
		mkdirSync(join(tmp, ".omx", "state", "sessions", "missing-field"), {
			recursive: true,
		});
		writeFileSync(
			path,
			JSON.stringify({ version: 1, last_triage: null }),
			"utf-8",
		);
...
describe("readTriageState — valid file returns parsed object", () => {
	...
	before(() => {
		...
		mkdirSync(join(tmp, ".omx", "state", "sessions", sessionId), {
			recursive: true,
		});
	...
	it("valid state with last_triage=null parses correctly", () => {
		const nullState: TriageStateFile = {
			version: 1,
			last_triage: null,
			suppress_followup: false,
		};
		const path2 = expectedPath(tmp, "null-triage");
		mkdirSync(join(tmp, ".omx", "state", "sessions", "null-triage"), {
			recursive: true,
		});
...
describe("path selection — session-scoped vs root", () => {
	...
	it("write with malformed explicit sessionId skips persistence instead of falling back to root", () => {
		...
		try {
			...
			assert.equal(
				existsSync(
					join(isolated, ".omx", "state", "sessions", "bad", "session"),
				),
				false,
			);
		} finally {
...
describe("promptSignature — format", () => {
	...
	it("hex portion is exactly 64 lowercase hex characters", () => {
		...
		assert.match(
			hex,
			/^[0-9a-f]{64}$/,
			`hex portion "${hex}" is not 64 lowercase hex chars`,
		);
	});
});
	...
	it("returns identical result across 1000 calls for the same input", () => {
		...
		for (let i = 1; i < 1000; i++) {
			assert.equal(
				...
				first,
				`call ${i} returned different signature`,
			);
		}
	});
});
...
describe("promptSignature — collision resistance", () => {
	it("different inputs produce different signatures", () => {
		...
		assert.equal(
			unique.size,
			inputs.length,
			`expected ${inputs.length} unique signatures, got ${unique.size}`,
		);
	});
