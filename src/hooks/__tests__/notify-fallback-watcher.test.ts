		...
		JSON.stringify(
			{
				...
				hud_pane_id: null,
				hud_pane_pid: null,
				workers: [{ name: "worker-1", pane_id: "%42", pid: 4242 }],
			},
			null,
			2,
		),
...
describe("notify-fallback watcher", () => {
	...
	it("one-shot mode forwards only recent task_complete events", async () => {
		...
		try {
			...
			await writeFile(
				join(wd, ".omx", "state", "subagent-tracking.json"),
				JSON.stringify({
					schemaVersion: 1,
					sessions: {
						[sid]: {
...
`,
	...
	it("ignores invalid session_id before watcher session path joins", async () => {
		...
		try {
			...
			assert.equal(
				tmuxLog,
				"",
				"invalid session_id should not reach session-scoped or canonical follow-up joins",
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("does not keep leader mailbox nudges alive after the team becomes terminal", async () => {
		...
		try {
			...
			const logEntries = await readJsonLines(logPath);
			assert.equal(
				logEntries.some((entry) => entry.type === "leader_nudge_tick"),
				false,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("sends bounded periodic Ralph continue steer while Ralph state stays active", async () => {
		...
		try {
			...
			await writeFile(
				...
				buildFakeTmux(tmuxLogPath, { paneOwner: "ralph:owner" }),
			);
			await chmod(join(fakeBinDir, "tmux"), 0o755);
			await writeFile(
	...
	it("fails closed for recycled, incomplete, HUD, and taken-over Ralph pane bindings", async () => {
		...
		for (const [stateBinding, tmux] of cases) {
			...
			try {
				...
				await writeFile(
					join(fakeBinDir, "tmux"),
					buildFakeTmux(tmuxLogPath, tmux),
				);
				await chmod(join(fakeBinDir, "tmux"), 0o755);
				await writeFile(
					join(stateDir, "ralph-state.json"),
	...
	it("fails closed without rebinding stale, degraded, or missing Ralph pane anchors", async () => {
		...
		for (const scenario of [
			...
			try {
				...
				assert.equal(
					persistedRalph.tmux_pane_id,
					"%99",
					`${scenario} must retain its stored pane identity`,
				);
				const watcherState = JSON.parse(
				...
				assert.doesNotMatch(
					tmuxLog,
					/send-keys -t %(42|99) -l Ralph loop active continue/,
				);
				assert.doesNotMatch(
					tmuxLog,
					/list-panes -s -t/,
					"watcher must not discover a replacement pane",
				);
			} finally {
				await rm(wd, { recursive: true, force: true });
			}
