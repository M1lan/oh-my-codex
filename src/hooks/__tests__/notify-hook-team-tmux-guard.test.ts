...
describe("notify-hook team tmux guard bridge", () => {
	it("submits without typing when typePrompt=false", async () => {
		...
		try {
			...
			const moduleUrl = new URL(
				"../../../dist/scripts/notify-hook/team-tmux-guard.js",
				import.meta.url,
			).href;
			...
			assert.match(
				lines[0],
				/\[display-message\]\[-p\]\[-t\]\[%42\]\[#\{pane_start_command\}\]/,
			);
	...
	it("queue-first submits with Tab before C-m when requested", async () => {
		...
		try {
			...
			const result = runSendPaneInputInChild({
				...
				exactPaneId: "%42",
				prompt:
					"Read /tmp/team/mailbox/leader-fixed.json; new msg from worker-1. Review it; decide next step.",
				submitKeyPresses: 2,
				typePrompt: true,
			...
			const lines = (await readFile(tmuxLogPath, "utf-8"))
				.trim()
				.split("\n")
				.filter(Boolean);
	...
	it("types then submits when typePrompt=true", async () => {
		...
		try {
			...
			assert.match(
				log,
				/\[set-buffer\]\[-b\]\[omx-pane-input-.*\]\[--\]\[hello bridge\]/,
			);
...
it("rejects omitted and mismatched exact pane identities before any tmux effect", async () => {
	...
	try {
		...
		assert.equal(
			JSON.parse(omitted.stdout).exactPaneProof.reason,
			"missing_exact_pane_id",
		);
		...
		assert.equal(
			JSON.parse(mismatched.stdout).exactPaneProof.reason,
...
it("rejects a replacement Team pane after the owner proof and before readiness reads", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "omx-team-readiness-owner-replacement-"),
	);
	const fakeBinDir = join(cwd, "fake-bin");
	const tmuxLogPath = join(cwd, "tmux.log");
	const countPath = join(cwd, "proof-count");
...
it("authorizes Team ownership before reading the start command", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "omx-team-tmux-start-command-owner-"),
	);
	const fakeBinDir = join(cwd, "fake-bin");
	const tmuxLogPath = join(cwd, "tmux.log");
	try {
