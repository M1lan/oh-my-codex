...
describe("execution-heavy skill guidance contract", () => {
	...
	it("ultraqa requires adversarial dynamic e2e coverage and structured reporting", () => {
		...
		for (const [label, content] of [
			...
			assert.match(
				content,
				/adversarial dynamic e2e/i,
				`${label} skill must require dynamic e2e QA`,
			);
			assert.match(
				content,
				/not satisfied by a shallow build\/lint\/typecheck\/test checklist/i,
				`${label} skill must reject shallow static QA`,
			);
			assert.match(
				content,
				/malicious\/hostile user behavior|User\/attacker model/i,
				`${label} skill must model hostile users`,
			);
			assert.match(
				content,
				/temporary tests, scripts, fixtures, or harnesses/i,
				`${label} skill must allow generated harnesses`,
			);
			...
			for (const requiredEdgeCase of [
				...
				assert.match(
					content,
					requiredEdgeCase,
					`${label} skill is missing edge case ${requiredEdgeCase}`,
				);
			}
			...
			for (const requiredReportField of [
				...
				assert.match(
					content,
					requiredReportField,
					`${label} skill is missing report field ${requiredReportField}`,
				);
			}

			assert.match(
				content,
				/No destructive commands/i,
				`${label} skill must prohibit destructive commands`,
			);
			assert.match(
				content,
				/secret exfiltration/i,
				`${label} skill must prohibit secret exfiltration`,
			);
			assert.match(
				content,
				/bounded runtimes|No unbounded waits/i,
				`${label} skill must bound runtime`,
			);
			assert.match(
				content,
				/clean.*temporary e2e harnesses|cleanup status/i,
				`${label} skill must require cleanup evidence`,
			);
		}
	});

	it("ultraqa harness guardrail patterns require intended semantics and reject inverted phrasing", () => {
		const ultraqa = SKILL_CONTRACTS.find(
			(contract) => contract.id === "ultraqa",
		);
		assert.ok(ultraqa, "ultraqa contract must exist");

		const findPattern = (needle: string): RegExp => {
			const pattern = ultraqa.requiredPatterns.find((candidate) =>
				candidate.source.includes(needle),
			);
			assert.ok(
				pattern,
				`missing ultraqa guardrail pattern containing: ${needle}`,
			);
			return pattern;
		};
		...
		const sanitizedEnv = findPattern(
		...
		const harnessDebris = findPattern(
	...
	it("ultrawork guidance stays OMX-native and routes durability outside ultrawork", () => {
		...
		for (const [label, content] of [
			...
			assert.doesNotMatch(
				content,
				/\boracle\b|\blibrarian\b|\bartistry\b|\bPrometheus\b/i,
			);
			assert.match(
				content,
			...
			assert.match(
				content,
				/Escalate to `ultragoal` when the work needs durable goal state/i,
				`${label} skill must route durable goal state to ultragoal`,
			);
			assert.match(
				content,
				/Escalate to `team` when the work needs coordinated tmux workers/i,
				`${label} skill must route coordinated worker lifecycle to team`,
			);
			assert.match(
				content,
