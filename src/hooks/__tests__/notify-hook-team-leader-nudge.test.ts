...
async function writeJson(path: string, value: unknown): Promise<void> {
	...
	if (
		...
		if (
			!Object.prototype.hasOwnProperty.call(config, "tmux_pane_owner_id") &&
			typeof config.name === "string" &&
			config.name.trim()
		) {
			persisted = {
				...config,
				tmux_pane_owner_id: `team:${config.name.trim()}`,
			};
		}
	}
...
function runNotifyHook(
	...
	if (!existsSync(sessionPath)) {
		...
		writeFileSync(
			sessionPath,
			JSON.stringify({ session_id: extraEnv.OMX_SESSION_ID || "thread-test" }),
		);
	}
	const sessionState = JSON.parse(readFileSync(sessionPath, "utf8")) as {
		session_id?: string;
	};
	const payload = {
		cwd,
...
describe("notify-hook leader-side authority handoff", () => {
	it("does not inject leader nudge from notify-hook when team is active and stale", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			await writeFile(
				fakeTmuxPath,
				buildFakeTmux(tmuxLogPath).replace(
					'if [[ "$cmd" == "show-option" ]]; then',
					'if [[ "$cmd" == "show-option" ]]; then\n  echo "team:foreign"\n  exit 0\nfi\nif [[ "$cmd" == "show-option" ]]; then',
				),
			);
			...
			assert.doesNotMatch(
				tmuxLog,
				/send-keys/,
				"a same-ID/PID foreign owner must not receive leader input",
			);
		});
	});
		...
		for (const owner of [undefined, 17] as const) {
			await withTempWorkingDir(async (cwd) => {
				...
				assert.ok(
					delivery.some(
						(entry) =>
							entry.reason === "leader_pane_owner_missing_no_injection",
					),
					"owner absence must be durably deferred",
				);
			});
		}
	});
...
describe("notify-hook team leader nudge", () => {
	...
	it("suggests shutdown when all workers are idle and the current task set is complete", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			await writeFile(
				fakeTmuxPath,
				buildFakeTmuxWithListPanes(tmuxLogPath, [
					"%96\t0\t12096",
					"%10\t0\t12010",
					"%11\t0\t12011",
				]),
	...
	it("suggests launching a new team when follow-up tasks are pending but worker panes are no longer reusable", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			await writeFile(
				fakeTmuxPath,
				buildFakeTmuxWithListPanes(tmuxLogPath, ["%98\t0\t12098"]),
			);
			await chmod(fakeTmuxPath, 0o755);
	...
	it("ignores invalid team_name before canonical leader follow-up team path joins", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			await writeJson(join(teamRoot, "..-bad-team", "manifest.v2.json"), {
				...
				leader: {
					session_id: "sess-canonical-safe",
					worker_id: "leader-fixed",
					role: "coordinator",
				},
				tmux_session: "bad:0",
...
`,
	...
	it("rolls back team nudge bookkeeping when shutdown wins immediately before global nudge-state write", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			await writeFile(
				fakeTmuxPath,
				buildFakeTmuxWithListPanes(join(cwd, "tmux.log"), ["%11 12345"]),
			);
			await chmod(fakeTmuxPath, 0o755);
			...
			try {
				setLeaderNudgeTestHooksForTests({
					beforeGlobalNudgeStateRename: async () => {
						await rm(join(stateDir, "team", teamName), {
							recursive: true,
							force: true,
						});
					},
...
`;
			...
			assert.match(
				...
				"should retain authoritative exact-pane live proof",
			);
			assert.match(
				tmuxLog,
				/display-message -p -t %74 #\{pane_current_command\}/,
				"should query the exact pane command before capture readiness",
			);
			assert.match(tmuxLog, /capture-pane -t %74 -p -S -80/);
			assert.doesNotMatch(
				tmuxLog,
				/(?:set-buffer|paste-buffer|send-keys)/,
				"capture readiness failure must suppress all input-effect tmux commands",
			);
			...
			const deferred = events.find(
				(entry: {
					...
					entry.type === "leader_notification_deferred" &&
					entry.reason === "pane_readiness_unverified",
			);
			assert.ok(
				deferred,
				"capture readiness failure should emit a fail-closed deferred event",
			);
			...
			assert.ok(
				...
				"capture readiness failure should record a fail-closed delivery receipt",
			);
		});
	});
