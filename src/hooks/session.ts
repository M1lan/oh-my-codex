...
function defaultReadProcessIdentity(
	pid: number,
	platform: NodeJS.Platform,
): {
	status: "matching" | "reused" | "indeterminate";
...
export function isSessionStale(
	...
	const liveIdentity = (options.readLinuxIdentity ?? readLinuxProcessIdentity)(
		state.pid,
	);
...
export function isSessionStateUsable(
	...
	const hasLinuxIdentityMetadata =
		typeof state.pid_start_ticks === "number" ||
		typeof state.pid_cmdline === "string";
	return (
		(!hasPidMetadata && !hasLinuxIdentityMetadata) ||
		!isSessionStale(state, options)
	);
}
...
function classifySessionProcess(
	...
	let liveIdentity: ReturnType<
		SessionPointerTransactionDependencies["readProcessIdentity"]
	>;
	try {
...
async function inspectLockOwnerFile(
	...
	>;
	try {
		liveIdentity = transactionDependencies.readProcessIdentity(
			owner.pid,
			owner.platform,
		);
	} catch {
	...
	if (liveIdentity.status !== "matching")
		return { status: "identity-indeterminate", owner };
	if (
		owner.pid_cmdline_hash &&
		owner.pid_cmdline_hash !== liveIdentity.cmdlineHash
	) {
		return { status: "identity-indeterminate", owner };
	}
...
export async function recoverSessionPointerLock(
	...
	try {
		...
		return {
			...
			action: "none",
			recovered: false,
			reason: "Unable to re-read session pointer lock evidence.",
		};
	}
	const evidenceName =
	...
	const owner = await inspectLockOwnerFile(
		join(context.lockPath, evidenceName),
	);
	...
	try {
		...
		return {
			...
			action: "none",
			recovered: false,
			reason: "Unable to create recovery claim token.",
		};
	}
	...
	try {
		...
		return {
			...
			action: "none",
			recovered: false,
			reason: "Recovery claim path already exists.",
		};
	} catch (error) {
		if (!isNotFound(error))
			return {
				...
				action: "none",
				recovered: false,
				reason: "Unable to inspect recovery claim path.",
			};
	}
	try {
	...
	try {
		...
		return {
			...
			action: "none",
			recovered: false,
			reason: "Unable to revalidate recovery claim.",
		};
	}
	const claimOwner = await inspectLockOwnerFile(claimPath);
	if (
		claimedEntries.length !== 1 ||
		claimedEntries[0] !== claimName ||
		claimOwner.status !== "dead" ||
		claimOwner.owner?.token !== owner.owner.token ||
		claimed.status !== "unexpected"
	) {
		return {
			...claimed,
			action: "none",
			recovered: false,
			reason: "Recovery claim revalidation failed.",
		};
	}
	...
	try {
		...
		return {
			...
			action: "none",
			recovered: false,
			reason: "Recovery quarantine path already exists.",
		};
	} catch (error) {
		if (!isNotFound(error))
			return {
				...
				action: "none",
				recovered: false,
				reason: "Unable to inspect recovery quarantine path.",
			};
	}
	try {
		...
		return {
			...
			action: "none",
			recovered: false,
			reason: "Unable to quarantine the exact session pointer recovery claim.",
		};
	}
	try {
		...
		if (!isNotFound(error)) {
			return {
				...
				recovered: false,
				reason:
					"The exact recovery claim was quarantined, but the canonical lock directory was not empty.",
				quarantinePath,
			};
		}
	}
	return {
		...
		action: "quarantined",
		recovered: true,
		reason: "Dead session pointer lock was atomically claimed and quarantined.",
		quarantinePath,
	};
}
...
function buildLockOwner(token: string): SessionPointerLockOwnerV1 {
	let identity:
		| ReturnType<SessionPointerTransactionDependencies["readProcessIdentity"]>
		| undefined;
	try {
		identity = transactionDependencies.readProcessIdentity(
			process.pid,
			process.platform,
		);
	} catch {
	...
	return {
		...
		isValidStartTicks(identity.startTicks)
			? { pid_start_ticks: identity.startTicks }
			: {}),
		...(identity?.status === "matching" &&
		identity.cmdlineHash &&
		SHA256_PATTERN.test(identity.cmdlineHash)
...
async function writePointerTransaction<T>(
	...
	try {
		...
		const ownsPointerTemp =
			primary.operation === "pointer-temp-write" ||
				...
				...(ownerOmxSessionId
					? {
							replacementLog: {
								...
								...(existing.session_id !== ownerOmxSessionId
...
export async function reconcileNativeSessionStart(
	...
	if (result.value.replacementLog) {
		await appendToLogAtContext(
			result.context,
			result.value.replacementLog,
		).catch(() => {});
	}
	await appendToLogAtContext(result.context, {
		event: result.value.replacementLog
			? "session_start"
		...
		timestamp:
			result.value.state.native_session_switched_at ?? new Date().toISOString(),
	}).catch(() => {});
