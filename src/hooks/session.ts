...
type UnusableSessionPointerStatus =
	| "malformed"
	| "foreign-cwd"
	| "identity-indeterminate";
...
export type UnsupportedDirectoryCapabilityReason =
	| "platform"
	| "inadequate-identity"
	| "stat-feature";
...
export class CommittedLaunchBlockedError extends Error {
	...
	constructor(
		readonly secondaryFailures: readonly SessionPointerSecondaryFailure[],
	) {
		super(
...
export type LaunchEstablishment =
	| {
			kind: "precommit-aborted";
			abort: SessionPointerLaunchAbort;
			cleanup: EstablishmentCleanupEvidence;
	  }
	| {
	...
	| {
			kind: "committed-release-failed";
			evidence: CommittedLaunchEvidence;
			error: CommittedLaunchBlockedError;
...
function resolvedAbort(
	...
	input: Omit<
		ResolvedSessionPointerAbort,
		"name" | "committed" | "cwd" | "pointerPath" | "rootSource" | "message"
	>,
): ResolvedSessionPointerAbort {
	return new SessionPointerLaunchAbortError({
		...input,
...
class SessionPointerLaunchAbortError extends Error {
	...
	constructor(
		fields: Record<string, unknown> & { reason: string; cause?: unknown },
	) {
		super(fields.reason);
...
export function isSessionPointerLaunchAbort(
	error: unknown,
): error is SessionPointerLaunchAbort {
	...
	return (
		candidate.code === "session_pointer_context_failure" ||
...
export interface SessionPointerLockRecovery
	extends SessionPointerLockInspection {
...
export interface SessionPointerTransactionDependencies {
	...
	atomicRenameNoReplace(
		from: string,
		to: string,
	): Promise<RecoveryRenameNoReplaceResult>;
}

/** Recovery-only no-clobber rename seam. Normal lock lifecycle never uses it. */
export type RecoveryRenameNoReplaceResult =
	| "moved"
	| "not-moved"
	| "unsupported";
...
async function defaultRecoveryRenameNoReplace(
	from: string,
	to: string,
): Promise<RecoveryRenameNoReplaceResult> {
	if (!from.trim() || !to.trim() || !isAbsolute(from) || !isAbsolute(to))
		return "unsupported";
	...
	try {
		...
		if (
			parsed.outcome === "moved" ||
			parsed.outcome === "not-moved" ||
...
function defaultObserveProcess(
	pid: number,
	platform: NodeJS.Platform,
): ProcessObservation {
	if (platform === "linux") {
...
let transactionDependencies: SessionPointerTransactionDependencies =
...
function readLinuxProcessIdentity(pid: number): LinuxProcessIdentity | null {
	try {
		const startTicks = parseLinuxProcStartTicks(
...
function isNodePlatform(value: unknown): value is NodeJS.Platform {
	return (
		typeof value === "string" && NODE_PLATFORMS.has(value as NodeJS.Platform)
	);
}
...
function parseProcessIdentityOutput(stdout: string): ProcessObservation {
	try {
		...
		if (
			keys.some(
				(key) => key !== "platform" && key !== "birth" && key !== "cmdline",
			)
		) {
			return { kind: "error" };
		}
		...
		if (!isNodePlatform(platform) || !isValidBirth(birth))
			return { kind: "error" };
		if (value.cmdline !== undefined && typeof value.cmdline !== "string")
			return { kind: "error" };
...
type ProcessClassificationStatus =
	| "usable"
	| "stale-dead"
	| "identity-indeterminate";
...
function compareObservation(
	...
	if (
		...
		(observation as ProcessObservation).kind !== "identity"
	) {
		return "identity-unavailable";
	}
	const identity = (
		observation as Extract<ProcessObservation, { kind: "identity" }>
	).identity;
	if (
		!isComparableProcessIdentity(identity) ||
		identity.platform !== runtimePlatform
	) {
		return "identity-unavailable";
	}
	if (identity.birth !== recorded.birth) return "birth-mismatch-retryable";
	if (
		recorded.cmdline_hash !== undefined &&
		identity.cmdline_hash !== recorded.cmdline_hash
	) {
		return "identity-unavailable";
	}
	return "match";
...
export function classifyRecordedIdentity(
	...
	if (
		!isComparableProcessIdentity(recorded) ||
		recorded.platform !== runtimePlatform ||
	...
	const first = compareObservation(
		recorded,
		safeObserve(provider, pid, runtimePlatform),
		runtimePlatform,
	);
	if (first === "birth-mismatch-retryable") {
		const second = compareObservation(
			recorded,
			safeObserve(provider, pid, runtimePlatform),
			runtimePlatform,
		);
		if (second === "match") return { status: "match" };
		if (second === "birth-mismatch-retryable" || second === "birth-mismatch")
			return { status: "birth-mismatch" };
		return { status: "identity-unavailable" };
	}
...
function recordedIdentityForState(
	...
	if (state.process_identity !== undefined) {
		...
		if (
			...
			state.platform !== state.process_identity.platform
		)
			return "invalid";
	...
	if (
		...
		isValidStartTicks(state.pid_start_ticks)
	) {
...
export function isSessionStateAuthoritativeForCwd(
	state: SessionState,
	cwd: string,
): boolean {
...
export function isSessionStateUsable(
	...
	if (
		...
		state.cwd.trim() &&
		!isSessionStateAuthoritativeForCwd(state, cwd)
	)
		return false;
	...
	return (
		(!hasPidMetadata && !hasIdentityMetadata) || !isSessionStale(state, options)
	);
}
...
export async function readSessionPointer(
	context: SessionPointerContext,
): Promise<SessionPointerReadResult> {
	let raw: string;
	try {
		raw = await transactionDependencies.fs.readFile(
			context.sessionPath,
			"utf8",
		);
	} catch (error) {
...
export async function readSessionStateFromContext(
	context: SessionPointerContext,
): Promise<SessionState | null> {
	try {
		const raw = await transactionDependencies.fs.readFile(
			context.sessionPath,
			"utf8",
		);
...
export async function readUsableSessionStateFromContext(
	context: SessionPointerContext,
): Promise<SessionState | null> {
...
export async function readUsableSessionState(
	...
	try {
		...
		const state = await readSessionStateFromContext(context);
		return state && isSessionStateUsable(state, context.cwd, options)
			? state
			: null;
	} catch {
		return null;
...
function createSessionState(
	...
	const previousNativeSessionId = normalizeNonempty(
		options.previousNativeSessionId,
	);
	const nativeSessionSwitchedAt = normalizeNonempty(
		options.nativeSessionSwitchedAt,
	);
	...
	const processIdentity =
		identity && "birth" in identity
			? identity
			: identity
				? {
						platform: "linux" as const,
						birth: String(identity.startTicks),
						...(hashCmdline(identity.cmdline)
							? { cmdline_hash: hashCmdline(identity.cmdline) }
							: {}),
					}
				: undefined;
	const publishIdentity =
		...
		processIdentity.platform === platform
			? processIdentity
			: undefined;
	const legacyStartTicks =
		identity && "startTicks" in identity
			? identity.startTicks
			: publishIdentity?.platform === "linux" &&
					Number.isSafeInteger(Number(publishIdentity.birth))
				? Number(publishIdentity.birth)
				: undefined;
	const legacyLinuxIdentity =
		publishIdentity?.platform === "linux" &&
		publishIdentity.birth === String(legacyStartTicks)
			? readLinuxProcessIdentity(pid)
			: null;
	const legacyCmdline =
		identity && "cmdline" in identity
			? identity.cmdline
			: legacyLinuxIdentity?.cmdline;
	return {
		...
		...(previousNativeSessionId
			...
			: {}),
		...(nativeSessionSwitchedAt
			? { native_session_switched_at: nativeSessionSwitchedAt }
			: {}),
		...(ownerOmxSessionId ? { owner_omx_session_id: ownerOmxSessionId } : {}),
		started_at: options.startedAt ?? nowIso,
		...(isValidToken(options.launchLineageToken)
			? { launch_lineage_token: options.launchLineageToken }
			: {}),
		cwd,
		...
		...(publishIdentity
			? {
					...
					process_identity: publishIdentity,
				}
			: {}),
		...(legacyStartTicks !== undefined
			? { pid_start_ticks: legacyStartTicks }
			: {}),
...
function preserveExistingLaunchLineageToken(
...
function resolvePid(options: SessionStartOptions): number {
	return Number.isInteger(options.pid) && options.pid && options.pid > 0
		? options.pid
		: process.pid;
}
...
function isStartCompatible(
	existing: SessionState,
	requestedSessionId: string,
): boolean {
	return (
		existing.session_id === requestedSessionId ||
...
type SessionPointerLockOwner =
	| SessionPointerLockOwnerV1
	| SessionPointerLockOwnerV2;
...
function parseLockOwner(raw: string): SessionPointerLockOwner | null {
	try {
		...
		if (value.version === 1) {
			if (
				value.pid_start_ticks !== undefined &&
				!isValidStartTicks(value.pid_start_ticks)
			)
				return null;
			if (
				value.pid_cmdline_hash !== undefined &&
				(typeof value.pid_cmdline_hash !== "string" ||
					!SHA256_PATTERN.test(value.pid_cmdline_hash))
			) {
				return null;
			}
		...
		if (value.version === 2) {
			...
			if (
				!isValidProcessIdentity(processIdentity) ||
				processIdentity.platform !== value.platform
			)
				return null;
			if (
				value.pid_start_ticks !== undefined &&
...
function recordedIdentityForLockOwner(
...
async function inspectLockOwnerFile(
	ownerPath: string,
	missingStatus: LockOwnerStatus = "missing",
): Promise<{
...
async function inspectSessionPointerLockAtContext(
	context: SessionPointerContext,
): Promise<SessionPointerLockInspection> {
	const absent = (): SessionPointerLockInspection => ({
		status: "absent",
	...
	const tempEntries = entries.filter((entry) =>
		/^owner\.([A-Za-z0-9_-]{16,128})\.tmp$/.test(entry),
	);
	...
	if (
		(hasCanonical && entries.length !== 1) ||
		(!hasCanonical && tempEntries.length !== 1) ||
		entries.length !== 1
	) {
		return {
			status:
				entries.length > 1 && (hasCanonical || tempEntries.length > 0)
					? "ambiguous"
					: "unexpected",
			lockPath: context.lockPath,
			evidenceSource: "none",
	...
	const evidenceSource = hasCanonical
		? ("owner.json" as const)
		: ("owner-temp" as const);
	...
	if (
		evidenceSource === "owner-temp" &&
		owner.owner?.token !==
			/^owner\.([A-Za-z0-9_-]{16,128})\.tmp$/.exec(evidenceName)?.[1]
	) {
		return {
			status: "malformed",
			lockPath: context.lockPath,
...
export async function inspectSessionPointerLock(
	cwd: string,
): Promise<SessionPointerLockInspection> {
	return await inspectSessionPointerLockAtContext(
...
function sameRecoveryIdentity(
	stat: Awaited<ReturnType<SessionPointerFsDependencies["lstat"]>>,
	identity: RecoveryIdentity,
	kind: "file" | "directory",
): boolean {
	return (
...
async function lstatRecoveryPath(
	path: string,
): Promise<
	Awaited<ReturnType<SessionPointerFsDependencies["lstat"]>> | undefined
> {
	try {
		return await transactionDependencies.fs.lstat(path);
	} catch (error) {
...
async function probeRecoveryRenameCapability(): Promise<RecoveryRenameNoReplaceResult> {
	...
	try {
		...
		const outcome = await transactionDependencies.atomicRenameNoReplace(
			from,
			to,
		);
...
async function moveRecoveryPathNoReplace(
	...
	try {
		...
		if (outcome === "unsupported")
			return {
				...
				unsupported: true,
				reason:
					"Atomic no-replace recovery rename is unsupported on this platform.",
			};
		if (outcome === "not-moved")
			return {
				moved: false,
				reason: "Atomic recovery move left its source pathname in place.",
			};
	} catch (error) {
		return {
			moved: false,
			reason: `Atomic no-replace recovery rename failed (${errorCode(error) ?? "unknown"}).`,
		};
	}
	...
	if (
		destination &&
		sameRecoveryIdentity(destination, identity, kind) &&
		!source
	)
		return { moved: true };
	...
	if (destination && sameRecoveryIdentity(destination, identity, kind)) {
		try {
			await transactionDependencies.atomicRenameNoReplace(to, from);
		} catch {
			/* durable residue is safer than cleanup */
		}
	}
	return {
		moved: false,
		reason:
			"Atomic recovery move did not leave the captured object at its token-bound destination.",
	};
}
...
async function completeRecoveryCheckpoint(
	...
	if (
		!current ||
		!sameRecoveryIdentity(current, checkpointIdentity, "file") ||
		(await transactionDependencies.fs.readFile(checkpointPath, "utf8")) !==
			checkpointBytes
	) {
		return {
			completed: false,
			reason:
				"Recovery checkpoint was replaced before completion; it was preserved as residue.",
		};
	}
	...
	const completedBytes = await transactionDependencies.fs.readFile(
		completedPath,
		"utf8",
	);
	return completedBytes === checkpointBytes
		? { completed: true }
		: {
				completed: false,
				reason:
					"Completed recovery receipt bytes changed and were preserved as residue.",
			};
}
...
async function resumeRecoveryCheckpoint(
	...
	const match = new RegExp(
		`^${basename(context.lockPath).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\.recovery\\.([A-Za-z0-9_-]{16,128})\\.([A-Za-z0-9_-]{16,128})\\.json$`,
	).exec(checkpointName);
	try {
		const checkpointBytes = await transactionDependencies.fs.readFile(
			checkpointPath,
			"utf8",
		);
		const checkpointStat =
			await transactionDependencies.fs.lstat(checkpointPath);
		const checkpoint = JSON.parse(checkpointBytes) as RecoveryCheckpoint;
		const ownerToken = match?.[1];
		const recoveryToken = match?.[2];
		const expectedSource =
			ownerToken &&
			(checkpoint.sourcePath === join(context.lockPath, "owner.json") ||
				checkpoint.sourcePath ===
					join(context.lockPath, `owner.${ownerToken}.tmp`));
		const expectedPark = `${context.lockPath}.parked-lock-${recoveryToken}`;
		const isLegacyV1 =
			checkpoint.version === 1 &&
			checkpoint.lockIdentity === undefined &&
			checkpoint.lockParkPath === undefined;
		if (
			!match ||
			!expectedSource ||
			!checkpoint.identity ||
			...
			![1, 2, 3].includes(checkpoint.version) ||
			(!isLegacyV1 && checkpoint.lockParkPath !== expectedPark) ||
			!sameRecoveryIdentity(
				checkpointStat,
				{ dev: checkpointStat.dev, ino: checkpointStat.ino },
				"file",
			)
		)
		...
		const claimPath = join(
			context.lockPath,
			`owner.${ownerToken}.${recoveryToken}.recovery`,
		);
		const lock = await lstatRecoveryPath(context.lockPath);
		const parkedLock = await lstatRecoveryPath(lockParkPath);
		if (lock && parkedLock)
			throw new Error("checkpoint lock paths are both present");
		if (parkedLock) {
			if (
				!checkpoint.lockIdentity ||
				!sameRecoveryIdentity(parkedLock, checkpoint.lockIdentity, "directory")
			)
				throw new Error("checkpoint parked lock identity mismatch");
			const quarantinePath = `${context.lockPath}.quarantine.${ownerToken}.${recoveryToken}`;
			const completed = await completeRecoveryCheckpoint(
				checkpointPath,
				checkpointBytes,
				{ dev: checkpointStat.dev, ino: checkpointStat.ino },
			);
			...
			return {
				status: "dead",
				lockPath: context.lockPath,
				evidenceSource: "owner.json",
				safeToRecover: true,
				action: "quarantined",
				recovered: true,
				reason: "Dead session pointer lock recovery checkpoint resumed.",
				quarantinePath,
			};
		}
		if (!lock || lock.isSymbolicLink() || !lock.isDirectory())
			throw new Error("checkpoint lock is missing or foreign");
		const source = await lstatRecoveryPath(checkpoint.sourcePath);
		const claim = await lstatRecoveryPath(claimPath);
		const evidencePath =
			source && sameRecoveryIdentity(source, evidenceIdentity, "file")
				? checkpoint.sourcePath
				: claim && sameRecoveryIdentity(claim, evidenceIdentity, "file")
					? claimPath
					: undefined;
		if (!evidencePath)
			throw new Error("checkpoint evidence is missing or foreign");
		if (claim && !sameRecoveryIdentity(claim, evidenceIdentity, "file"))
			throw new Error("checkpoint claim is foreign");
		...
		const lockIdentity = checkpoint.lockIdentity ?? {
			dev: lock.dev,
			ino: lock.ino,
		};
		if (!sameRecoveryIdentity(lock, lockIdentity, "directory"))
			throw new Error("checkpoint lock identity mismatch");
		const ownerBytes = await transactionDependencies.fs.readFile(
			evidencePath,
			"utf8",
		);
		...
		if (
			...
			(checkpoint.evidenceBytes !== undefined &&
				checkpoint.evidenceBytes !== ownerBytes)
		)
			throw new Error("checkpoint owner evidence changed");
		const quarantinePath = `${context.lockPath}.quarantine.${ownerToken}.${recoveryToken}`;
		const quarantine = await lstatRecoveryPath(quarantinePath);
		if (quarantine) {
			if (!sameRecoveryIdentity(quarantine, evidenceIdentity, "file"))
				throw new Error("checkpoint quarantine is foreign");
		} else {
			await transactionDependencies.fs.link(evidencePath, quarantinePath);
			const linked = await lstatRecoveryPath(quarantinePath);
			if (!linked || !sameRecoveryIdentity(linked, evidenceIdentity, "file"))
				throw new Error("checkpoint quarantine link mismatch");
		}
		...
		const currentBytes =
			currentEvidence &&
			sameRecoveryIdentity(currentEvidence, evidenceIdentity, "file")
				? await transactionDependencies.fs.readFile(evidencePath, "utf8")
				: undefined;
		const currentOwner =
			currentBytes === undefined
				? undefined
				: await inspectLockOwnerFile(evidencePath);
		if (
			!currentLock ||
			!sameRecoveryIdentity(currentLock, lockIdentity, "directory") ||
			currentBytes !== ownerBytes ||
			currentOwner?.status !== "dead" ||
			currentOwner.owner?.token !== ownerToken
		)
			throw new Error(
				"checkpoint owner evidence changed before directory quarantine",
			);
		const lockEntries = await transactionDependencies.fs.readdir(
			context.lockPath,
		);
		const allowedEntries = new Set([
			basename(checkpoint.sourcePath),
			basename(claimPath),
		]);
		if (lockEntries.some((entry) => !allowedEntries.has(entry)))
			throw new Error("checkpoint lock contains foreign recovery evidence");
		const moved = await moveRecoveryPathNoReplace(
			context.lockPath,
			lockParkPath,
			lockIdentity,
		...
		return {
			...
			recovered: true,
			reason:
				"Dead session pointer lock recovered into durable quarantine residues.",
			quarantinePath,
		};
	} catch (error) {
		...
		return {
			...
			action: "none",
			recovered: false,
			reason: `Recovery checkpoint is malformed or no longer identifies its recorded recovery state (${detail}).`,
		};
	}
}

/** Explicitly quarantine only a dead lock by moving its entire directory incarnation. */
export async function recoverSessionPointerLock(
	cwd: string,
): Promise<SessionPointerLockRecovery> {
	const context = resolveSessionPointerContext(cwd);
	...
	try {
		checkpointNames = (
			await transactionDependencies.fs.readdir(dirname(context.lockPath))
		).filter(
			(name) => name.startsWith(checkpointPrefix) && name.endsWith(".json"),
		);
	} catch (error) {
		if (!isNotFound(error))
			return {
				...
				action: "none",
				recovered: false,
				reason: "Unable to enumerate session pointer recovery checkpoints.",
			};
		checkpointNames = [];
	}
	if (checkpointNames.length > 1)
		return {
			...
			action: "none",
			recovered: false,
			reason: "Multiple recovery checkpoints exist.",
		};
	if (checkpointNames.length === 1)
		return await resumeRecoveryCheckpoint(
			context,
			join(dirname(context.lockPath), checkpointNames[0]!),
			checkpointNames[0]!,
		);
		...
		return {
			...
			reason:
				inspection.status === "absent"
					? "No session pointer lock exists."
					: `Session pointer lock is not safe to recover (${inspection.status}).`,
		};
	try {
		const evidenceName =
			inspection.evidenceSource === "owner.json"
				? "owner.json"
				: (await transactionDependencies.fs.readdir(context.lockPath)).find(
						(entry) => /^owner\.([A-Za-z0-9_-]{16,128})\.tmp$/.test(entry),
					);
		if (!evidenceName)
			return {
				...inspection,
				action: "none",
				recovered: false,
				reason: "Session pointer lock evidence changed before recovery claim.",
			};
		const evidencePath = join(context.lockPath, evidenceName);
		...
		if (!isValidToken(recoveryToken))
			return {
				...
				action: "none",
				recovered: false,
				reason: "Recovery claim token is invalid.",
			};
		...
		try {
			await transactionDependencies.fs.writeFile(
				checkpointPath,
			...
			return {
				...
				action: "none",
				recovered: false,
				reason: `Unable to create recovery checkpoint (${errorCode(error) ?? "unknown"}).`,
			};
		}
		return await resumeRecoveryCheckpoint(
			context,
...
function buildLockOwner(token: string): SessionPointerLockOwner {
	...
	try {
		observation = transactionDependencies.observeProcess(
			process.pid,
			runtimePlatform,
		);
	} catch {
	...
	const legacyStartTicks =
		...
		Number.isSafeInteger(Number(identity.birth))
			? Number(identity.birth)
			: undefined;
	const legacyHash =
		identity?.cmdline_hash && SHA256_PATTERN.test(identity.cmdline_hash)
...
async function removeOwnedPath(
	path: string,
): Promise<SessionPointerSecondaryFailure | undefined> {
	try {
...
async function acquirePointerLock(
	...
	try {
		await transactionDependencies.fs.writeFile(
			ownerTempPath,
			JSON.stringify(buildLockOwner(token)),
			{ mode: 0o600 },
		);
		recordRegularFileSyncOutcome(
			tracker,
			await transactionDependencies.fs.openAndSync(
				ownerTempPath,
				platform,
				regularFileSync,
			),
...
async function releasePointerLock(
	...
	try {
		owner = parseLockOwner(
			await transactionDependencies.fs.readFile(ownerPath, "utf8"),
		);
	} catch {
		...
		return [
			{
				...
				ownership: "uncertain",
				message:
					"Unable to prove ownership of the canonical session pointer lock.",
...
export async function __releasePointerLockForTests(
	cwd: string,
	token: string,
): Promise<SessionPointerSecondaryFailure[]> {
	return await releasePointerLock({
		context: resolveSessionPointerContext(cwd),
		token,
	});
}
...
function recoveryAbort(
	...
	return resolvedAbort(context, {
		code: "session_pointer_lock_recovery_required",
		operation,
		...(primary.candidateSessionId
			? { candidateSessionId: primary.candidateSessionId }
			: {}),
		...(primary.canonicalSessionId
			? { canonicalSessionId: primary.canonicalSessionId }
			: {}),
		lockPath: context.lockPath,
		...
		cause: primary.cause,
		primaryOperation:
			primary.operation as ResolvedSessionPointerAbort["primaryOperation"],
...
function releaseFailureError<T>(
	...
	context?: SessionPointerContext,
	value?: T,
): CommittedLaunchBlockedError & {
	context?: SessionPointerContext;
	value?: T;
} {
	const error = new CommittedLaunchBlockedError(
		failures,
	) as CommittedLaunchBlockedError & {
		context?: SessionPointerContext;
		value?: T;
	};
...
async function finalizePrecommitAbort(
	...
	return recoveryAbort(
		...
		primary,
		failures,
		pointerTempPath && failures[0]?.phase === "remove-pointer-temp"
			? "precommit-cleanup"
			: "lock-release",
	);
}
...
function unusablePointerAbort(
	...
	return resolvedAbort(context, {
		...
		...(pointer.state?.session_id
			? { canonicalSessionId: pointer.state.session_id }
			: {}),
		lockPath: context.lockPath,
...
async function writePointerTransaction<T>(
	...
	options: Pick<
		SessionStartOptions,
		"context" | "platform" | "regularFileSync"
	>,
	timeoutMs: number,
	transition: (
		pointer: SessionPointerReadResult,
	...
	if (!candidateSessionId) {
		const cause = new Error(
			"A valid session ID is required for pointer mutation.",
		);
		throw resolvedAbort(context, {
			code: "session_pointer_io_failure",
			operation: "pointer-classify",
	...
	try {
		await transactionDependencies.fs.mkdir(context.baseStateDir, {
			recursive: true,
		});
	} catch (error) {
		throw resolvedAbort(context, {
	...
	try {
		...
		try {
			...
			await transactionDependencies.fs.writeFile(pointerTempPath, serialized, {
				mode: 0o600,
				flag: "wx",
			});
		} catch (error) {
		...
		try {
			await transactionDependencies.fs.rename(
				pointerTempPath,
				context.sessionPath,
			);
						...
						[
							{
								...
								cause: error,
								evidencePath: context.sessionPath,
							},
							...releaseFailures,
						],
						context,
		...
		if (releaseFailures.length > 0)
			throw releaseFailureError(releaseFailures, context, next);
		...
		throw await finalizePrecommitAbort(
			lock,
			primary,
			ownsPointerTemp ? pointerTempPath : undefined,
		);
	}
}
...
function startPointerTransition(
	...
	return (pointer, context) => {
		...
		if (
			requireLaunchLineageToken &&
			existing &&
			!isValidToken(existing.launch_lineage_token)
		) {
			throw ownerConflictAbort(context, requestedSessionId, existing);
		}
		...
		const state = createSessionState(
			...
			sessionIdentityFor(pid, platform),
			{
				...
				previousNativeSessionId:
					options.previousNativeSessionId ??
					existing?.previous_native_session_id,
				nativeSessionSwitchedAt:
					options.nativeSessionSwitchedAt ??
					existing?.native_session_switched_at,
...
export async function writeSessionStart(
	...
	if (pointer.status !== "absent") {
		...
		throw unusablePointerAbort(
			context,
			candidateSessionId ?? sessionId,
			pointer,
		);
	}
	return await writeSessionStartTransition(cwd, sessionId, {
		...options,
		context,
	});
}
...
async function writeSessionStartTransition(
	...
	await appendToLogAtContext(result.context, {
		...
		...(result.value.native_session_id
...
export class LaunchContextResolutionError extends Error {
	...
	constructor(
		...
		super(
			`Unable to establish launch directory capability: ${errorMessage(cause)}`,
			{ cause },
		);
	}
}
...
const bindingFinalizations = new WeakMap<
	LaunchSessionBinding,
	Promise<BoundFinalizationReport>
>();

function safeError(
	error: unknown,
): Readonly<{ name: string; message: string; code?: string }> {
	return {
...
function lockDisposition(
	failures: readonly SessionPointerSecondaryFailure[],
): "held" | "released-with-residue" | "uncertain" {
	if (failures.some((failure) => failure.ownership === "uncertain"))
		return "uncertain";
	return failures.some((failure) => failure.ownership === "held")
		? "held"
		: "released-with-residue";
}
...
async function acquireDirectoryCapability(
	...
	identity: LaunchSessionBinding["directoryIdentity"];
	lease: BindingLease;
	cleanup: CapabilityCloseEvidence[];
}> {
	...
	if (!DIRECTORY_CAPABILITY_PLATFORMS.has(platform)) {
		return {
			canonicalRealpath,
			identity: { kind: "unsupported", reason: "platform" },
			lease: { closed: false },
			cleanup,
		};
	}
	...
	try {
		...
		if (
			...
			try {
				...
				throw new LaunchContextResolutionError(
					{ capability: cleanup },
					closeError,
				);
			}
			return {
				canonicalRealpath,
				identity: { kind: "unsupported", reason: "inadequate-identity" },
				lease: { closed: false },
				cleanup,
			};
		}
		return {
			canonicalRealpath,
			identity: { kind: "supported", dev: stats.dev, ino: stats.ino },
			lease: { handle, closed: false },
			cleanup,
		};
	} catch (error) {
	...
	if (
		...
		return {
			canonicalRealpath,
			identity: { kind: "unsupported", reason: "stat-feature" },
			lease: { closed: false },
			cleanup,
		};
	}
...
export async function establishLaunchSessionBinding(
	...
	const acquired = await acquireDirectoryCapability(
		context.baseStateDir,
		platform,
	);
	const cleanup = (): EstablishmentCleanupEvidence => ({
		capability: acquired.cleanup,
	});
	const revalidateSelectedRoot = async (
		lockedContext: SessionPointerContext,
	): Promise<void> => {
		...
		if (
			...
			retained.dev !== acquired.identity.dev ||
			retained.ino !== acquired.identity.ino
		) {
			throw resolvedAbort(lockedContext, {
				code: "session_pointer_io_failure",
				operation: "pointer-classify",
				candidateSessionId: normalizeSessionId(requestedSessionId),
				lockPath: lockedContext.lockPath,
				reason:
					"Selected state root identity changed during pointer publication.",
			});
		}
		...
		try {
			...
			if (
				canonical !== acquired.canonicalRealpath ||
				!stats.isDirectory() ||
				stats.dev !== acquired.identity.dev ||
				stats.ino !== acquired.identity.ino
			) {
				throw resolvedAbort(lockedContext, {
					code: "session_pointer_io_failure",
					operation: "pointer-classify",
					...
					lockPath: lockedContext.lockPath,
					reason:
						"Selected state root was replaced during pointer publication.",
				});
			}
		} finally {
			await fresh.close();
	...
	try {
		...
		if (!isValidToken(token)) {
			...
			throw new LaunchContextResolutionError(
				cleanup(),
				new Error("Committed session pointer lacks a valid lineage token."),
			);
		}
		const binding: LaunchSessionBinding = Object.freeze({
			context: Object.freeze({ ...context }),
			canonicalRealpath: acquired.canonicalRealpath,
			directoryIdentity: acquired.identity,
			canonicalSessionId: state.session_id,
		...
		if (error instanceof CommittedLaunchBlockedError) {
			const state = (
				error as CommittedLaunchBlockedError & { value?: SessionState }
			).value;
			const errorContext =
				(
					error as CommittedLaunchBlockedError & {
						context?: SessionPointerContext;
					}
				).context ?? context;
			const close = await closeLease(acquired.lease, "before-authorization");
			...
			return {
				...
				evidence: {
					...
					canonicalSessionId: state?.session_id ?? requestedSessionId,
				},
				error,
				lockDisposition: lockDisposition(error.secondaryFailures),
		...
		if (isSessionPointerLaunchAbort(error))
			return { kind: "precommit-aborted", abort: error, cleanup: cleanup() };
		throw error;
	}
}
...
async function closeLease(
	lease: BindingLease,
	phase: CapabilityCloseEvidence["phase"],
): Promise<CapabilityCloseEvidence | undefined> {
	...
	try {
		...
		try {
			...
			throw resolvedAbort(binding.context, {
				code: "session_pointer_io_failure",
				operation: "pointer-read",
				candidateSessionId: binding.canonicalSessionId,
				lockPath: binding.context.lockPath,
				reason: `Unable to read selected session pointer: ${errorMessage(error)}`,
				cause: error,
			});
		}
		if (
			pointerBeforeTransaction.status !== "usable" ||
			!isAuthorizedBoundPointer(
				binding,
				binding.context,
				binding.canonicalSessionId,
				pointerBeforeTransaction.state,
			)
		) {
			throw unusablePointerAbort(
				binding.context,
		...
		const result = await writePointerTransaction(
			...
			async (pointer, context) => {
				...
				await consumeBoundDirectoryAuthorizationUnderLock(
					binding,
					authorization,
					pointer,
				);
				return {
					...
					...(patch.tmuxSessionName !== undefined
						? { tmux_session_name: patch.tmuxSessionName }
						: {}),
					...(patch.tmuxPaneId !== undefined
						? { tmux_pane_id: patch.tmuxPaneId }
						: {}),
				};
			},
			(state) => state,
		...
		if (error instanceof CommittedLaunchBlockedError) {
			...
			return {
				kind: "committed-release-failed",
				evidence: { context, canonicalSessionId: binding.canonicalSessionId },
				error,
				lockDisposition: lockDisposition(error.secondaryFailures),
				secondaryFailures: error.secondaryFailures,
				cleanup,
		...
		if (isSessionPointerLaunchAbort(error))
			return { kind: "precommit-aborted", abort: error, cleanup };
		throw error;
	}
}
...
function isAuthorizedBoundPointer(
	...
	if (
		!lease ||
		(binding.directoryIdentity.kind === "supported" && lease.closed) ||
		!state
	)
		return false;
	if (
		!isValidToken(binding.launchLineageToken) ||
		!isValidToken(state.launch_lineage_token)
	)
		return false;
	...
	const directCanonicalIdentity =
		state.session_id === binding.canonicalSessionId &&
		((state.owner_omx_session_id ?? undefined) === binding.ownerOmxSessionId ||
			(binding.ownerOmxSessionId === undefined &&
				state.owner_omx_session_id === binding.canonicalSessionId)) &&
		(binding.nativeSessionId === undefined ||
			state.native_session_id === binding.nativeSessionId);
	const replacedNativeIdentity =
		state.owner_omx_session_id === binding.canonicalSessionId &&
	...
	try {
		return (
			context.rootSource === binding.context.rootSource &&
...
function isAuthorizedBoundStaleDeadPointer(
	...
	return (
		isAuthorizedBoundPointer(binding, context, candidateSessionId, state) &&
...
class BoundDirectoryComparisonDenied extends Error {
	constructor(
		readonly comparison: NonNullable<LifecycleCleanupEvidence["comparison"]>,
		readonly capability: CapabilityCloseEvidence[],
	) {
...
async function authorizeBoundDirectoryBeforeTransaction(
	...
	if (!lease || lease.closed) {
		throw new BoundDirectoryComparisonDenied(
			{ status: "denied", reason: "binding-lease-closed" },
			capability,
		);
	}
	try {
		if (
			!sameFilePath(binding.context.cwd, postLaunchCwd) ||
			!pointer.state?.cwd ||
			!sameFilePath(binding.context.cwd, pointer.state.cwd)
		) {
			throw new BoundDirectoryComparisonDenied(
				{ status: "denied", reason: "cwd-identity-mismatch" },
				capability,
			);
		}
		if (binding.directoryIdentity.kind !== "supported") {
			return {
				comparison: {
					status: "matched",
					reason: `unsupported-directory-capability:${binding.directoryIdentity.reason}`,
				},
				capability,
		...
		if (
			!retained ||
			!retained.isDirectory() ||
			retained.dev !== binding.directoryIdentity.dev ||
			retained.ino !== binding.directoryIdentity.ino
		) {
			throw new BoundDirectoryComparisonDenied(
				{ status: "denied", reason: "retained-directory-identity-mismatch" },
				capability,
			);
		}
		...
		try {
			const stats = await handle.stat({ bigint: true });
			matched =
				canonical === binding.canonicalRealpath &&
				stats.isDirectory() &&
			...
			try {
				...
				throw new BoundDirectoryComparisonDenied(
					{ status: "denied", reason: "fresh-comparison-close-failed" },
					capability,
				);
			}
		}
		if (!matched)
			throw new BoundDirectoryComparisonDenied(
				{ status: "denied", reason: "directory-identity-mismatch" },
				capability,
			);
		return {
		...
		throw new BoundDirectoryComparisonDenied(
			{ status: "denied", reason: errorMessage(error) },
			capability,
		);
	}
}
...
async function consumeBoundDirectoryAuthorizationUnderLock(
	...
	comparison: LifecycleCleanupEvidence["comparison"];
	capability: CapabilityCloseEvidence[];
}> {
	if (
		authorization.comparison?.status !== "matched" ||
		binding.directoryIdentity.kind !== "supported"
	)
	...
	try {
		...
		try {
			...
			try {
				...
				throw new BoundDirectoryComparisonDenied(
					{
						status: "denied",
						reason: "fresh-comparison-close-failed-under-lock",
					},
					authorization.capability,
				);
			}
		...
		if (
			pointer.status !== authorization.pointerStatus ||
			pointer.raw !== authorization.pointerRaw
		) {
			throw new BoundDirectoryComparisonDenied(
				{ status: "denied", reason: "pointer-changed-before-consume" },
				authorization.capability,
			);
		}
		return authorization;
...
export function finalizeBoundOnce(
	...
	const finalization = (async (): Promise<BoundFinalizationReport> => {
		try {
			const revalidation = await writeSessionEnd(
				binding.context.cwd,
				binding.canonicalSessionId,
				{
					context: binding.context,
			...
			return {
				finalized: true,
				cleanup: {
					capability: revalidation.capability,
					comparison: revalidation.comparison,
				},
			};
		} catch (error) {
			...
			if (isSessionPointerLaunchAbort(error)) {
				return {
					...
					cleanup: {
						capability: [],
						comparison: { status: "denied", reason: error.code },
					},
				};
			}
			throw error;
...
export async function readNativeSessionOwnerEvidence(
	...
	try {
		...
		const pathStat = await transactionDependencies.fs.lstat(
			context.sessionPath,
		);
	...
	try {
		...
		const finalPathStat = await transactionDependencies.fs.lstat(
			context.sessionPath,
		);
		if (
		...
		for (const identity of directoryIdentities) {
			const finalDirectoryStat = await transactionDependencies.fs.lstat(
				identity.path,
			);
			if (
...
function reconcileNativeTransition(
	...
	return (pointer, context) => {
		...
		const existingNativeSessionId = normalizeSessionId(
		...
		return preserveExistingLaunchLineageToken(
			existing,
			createSessionState(
				context.cwd,
				context.baseStateDir,
...
export async function reconcileNativeSessionStart(
	...
	const result = await writePointerTransaction(
		...
		options,
		NATIVE_POINTER_TIMEOUT_MS,
		reconcileNativeTransition(
			normalizedNativeSessionId ?? nativeSessionId,
			options,
		),
		(state) => state,
	);
...
async function removeDeadSessionHudState(
	...
	const uniqueSessionIds = [
		...new Set(
			sessionIds.filter((value): value is string =>
				Boolean(normalizeSessionId(value)),
			),
		),
	];
	const candidatePaths = [
		...
		...uniqueSessionIds.map((sessionId) =>
			join(context.baseStateDir, "sessions", sessionId, "hud-state.json"),
		),
	];
	await Promise.all(
...
export async function writeSessionEnd(
	...
	if (!candidateSessionId) {
		const cause = new Error(
			"A valid session ID is required to end a session pointer.",
		);
		throw resolvedAbort(context, {
			code: "session_pointer_io_failure",
			operation: "pointer-classify",
	...
	if (!options.binding) {
		throw resolvedAbort(context, {
			...
			candidateSessionId,
			lockPath: context.lockPath,
			reason: "A live launch binding is required to end a session pointer.",
		});
	}
	...
	const preLockAuthorizedBoundUsable =
		preLockPointer.status === "usable" &&
		isAuthorizedBoundPointer(
			options.binding,
			context,
			candidateSessionId,
			preLockPointer.state,
		);
	const preLockAuthorizedBoundStaleDead =
		preLockPointer.status === "stale-dead" &&
		isAuthorizedBoundStaleDeadPointer(
			options.binding,
			context,
			candidateSessionId,
			preLockPointer.state,
	...
	try {
		...
		const authorizedBoundStaleDead =
			pointer.status === "stale-dead" &&
			isAuthorizedBoundStaleDeadPointer(
				options.binding,
				context,
		...
		revalidation = await consumeBoundDirectoryAuthorizationUnderLock(
			options.binding,
			authorization,
			pointer,
		);
		...
		await removeDeadSessionHudState(context, [
			...(ownsCurrentSessionFile
				? [state?.session_id, state?.native_session_id]
				: []),
			candidateSessionId,
		]);
		...
		await appendToLogAtContext(context, {
			...
			...(historyEntry.native_session_id
				? { native_session_id: historyEntry.native_session_id }
				: {}),
			...(historyEntry.active_session_id
				? { active_session_id: historyEntry.active_session_id }
				: {}),
			...(historyEntry.preserved_active_session_id
				? {
						preserved_active_session_id:
							historyEntry.preserved_active_session_id,
					}
				: {}),
			timestamp: endTime,
	...
	if (primary) {
		if (isSessionPointerLaunchAbort(primary) && releaseFailures.length > 0) {
			throw recoveryAbort(
				context,
				primary as ResolvedSessionPointerAbort,
				releaseFailures,
				"lock-release",
			);
		}
...
export async function resetSessionMetrics(
	cwd: string,
	sessionId?: string,
): Promise<void> {
	const context = resolveSessionPointerContext(cwd);
