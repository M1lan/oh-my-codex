...
async function removeDeadSessionHudState(
	...
	const candidatePaths = [
		...
		...uniqueSessionIds.map((sessionId) =>
			getStateFilePath("hud-state.json", cwd, sessionId),
		),
	];
...
function createSessionState(
	...
	const nativeSessionId =
		typeof options.nativeSessionId === "string" &&
		options.nativeSessionId.trim()
			? options.nativeSessionId.trim()
			: undefined;
	const tmuxSessionName =
		typeof options.tmuxSessionName === "string" &&
		options.tmuxSessionName.trim()
			? options.tmuxSessionName.trim()
			: undefined;
	const tmuxPaneId =
		typeof options.tmuxPaneId === "string" && options.tmuxPaneId.trim()
			? options.tmuxPaneId.trim()
			: undefined;
	const previousNativeSessionId =
		typeof options.previousNativeSessionId === "string" &&
		options.previousNativeSessionId.trim()
			? options.previousNativeSessionId.trim()
			: undefined;
	const nativeSessionSwitchedAt =
		typeof options.nativeSessionSwitchedAt === "string" &&
		options.nativeSessionSwitchedAt.trim()
			? options.nativeSessionSwitchedAt.trim()
			: undefined;
	const ownerOmxSessionId =
		typeof options.ownerOmxSessionId === "string" &&
...
export async function writeSessionStart(
	...
	const pid =
		Number.isInteger(options.pid) && options.pid && options.pid > 0
			? options.pid
			: process.pid;
	const platform = options.platform ?? process.platform;
	const linuxIdentity =
		platform === "linux" ? readLinuxProcessIdentity(pid) : null;
	const state = createSessionState(
		...
		linuxIdentity,
		{
			nativeSessionId:
				...
				(sameSession ? existing?.native_session_id : undefined),
			previousNativeSessionId:
				options.previousNativeSessionId ??
				(sameSession ? existing?.previous_native_session_id : undefined),
			nativeSessionSwitchedAt:
				options.nativeSessionSwitchedAt ??
				(sameSession ? existing?.native_session_switched_at : undefined),
			ownerOmxSessionId:
				options.ownerOmxSessionId ??
				(sameSession ? existing?.owner_omx_session_id : undefined),
			tmuxSessionName:
				options.tmuxSessionName ??
				(sameSession ? existing?.tmux_session_name : undefined),
			tmuxPaneId:
				options.tmuxPaneId ??
				(sameSession ? existing?.tmux_pane_id : undefined),
		},
	);
...
function getOmxLaunchSessionId(state: SessionState): string | undefined {
	...
	if (
		typeof state.owner_omx_session_id === "string" &&
		state.owner_omx_session_id.startsWith("omx-")
	) {
...
export async function reconcileNativeSessionStart(
	...
	const existingNativeSessionId =
		typeof existing.native_session_id === "string"
