...
const ROOT_TEMPLATE_PATTERNS = [
	...
	rx(
		"target result.*success criteria.*constraints.*available evidence.*expected output.*stop condition",
	),
	...
	rx(
		"ASK only.*destructive.*irreversible.*credential-gated.*external-production.*materially scope-changing",
	),
	rx("AUTO-CONTINUE branches.*permission-handoff phrasing"),
	rx(
		"do not ask or instruct humans.*ordinary non-destructive.*reversible actions",
	),
	...
	rx(
		"Route to `dependency-expert` when the main need is package / SDK selection",
	),
	rx(
		"whether / which package, SDK, or framework to adopt, upgrade, replace, or migrate",
	),
...
const CORE_ROLE_PATTERNS = {
	executor: [
		...
		rx(
			"target result.*constraints.*success criteria.*validation path.*stop condition",
		),
		rx("concise preamble"),
		rx("smallest useful tool loop|reflexive web/tool escalation"),
	...
	planner: [
		...
		rx(
			"desired result.*success criteria.*constraints.*evidence.*validation path.*stop condition",
		),
		rx("concise visible preamble"),
		rx("smallest useful tool loop|reflexive web/tool escalation"),
		rx("local overrides?.*non-conflicting constraints"),
		rx(
			"plan is grounded|requirements.*affected resources.*validation commands.*failure behavior",
		),
		rx("AUTO-CONTINUE.*clear.*already-requested.*low-risk.*reversible.*local"),
		rx(
			"ASK only.*destructive.*irreversible.*credential-gated.*external-production.*materially scope-changing",
	...
	verifier: [
		...
		rx(
			"Ask only when the acceptance target is materially unclear|Ask only when blocked",
		),
	],
};
...
const ULTRAQA_SKILL_PATTERNS = [
	...
	rx(
		"not satisfied by a shallow build/lint/typecheck/test checklist|build/lint/typecheck/test.*not sufficient",
	),
	rx(
		"malicious/hostile user behavior|hostile user modeling|User/attacker model",
	),
	rx("temporary tests.*harnesses|temporary harnesses"),
	rx(
		'Use absolute repo imports[^\\n]*pathToFileURL\\(join\\(repoRoot, \"dist\", \\.\\.\\.\\)\\)\\.href[^\\n]*Never rely on[^\\n]*\\./dist',
	),
	rx(
		"Use a safe file writer[^\\n]*non-interpolating file-write mechanism[^\\n]*do not use interpolating heredocs",
	),
	rx(
		"Sanitize OMX runtime env for isolated probes[^\\n]*OMX_ROOT[^\\n]*OMX_STATE_ROOT[^\\n]*unset[^\\n]*env -u OMX_ROOT -u OMX_STATE_ROOT",
	),
	rx(
		"Classify harness setup failures separately[^\\n]*record it as harness debris[^\\n]*fix the harness[^\\n]*rerun the scenario[^\\n]*before declaring a product defect",
	),
	rx("malformed input"),
...
const ULTRAWORK_SKILL_PATTERNS = [
	...
	rx(
		"Ralph owns persistence, architect verification, deslop, and the full verified-completion promise",
	),
];
...
export const ROOT_TEMPLATE_CONTRACTS: GuidanceSurfaceContract[] = [
	{
		...
		requiredPatterns: ROOT_TEMPLATE_PATTERNS,
	},
];
...
export const CORE_ROLE_CONTRACTS: GuidanceSurfaceContract[] = [
	{
		...
		requiredPatterns: CORE_ROLE_PATTERNS.executor,
	},
	{
		id: "planner",
		path: "prompts/planner.md",
		requiredPatterns: CORE_ROLE_PATTERNS.planner,
	},
	{
		id: "verifier",
		path: "prompts/verifier.md",
		requiredPatterns: CORE_ROLE_PATTERNS.verifier,
	},
];
...
export const SPECIALIZED_PROMPT_CONTRACTS: GuidanceSurfaceContract[] = [
	{
		...
		requiredPatterns: [
			...
			rx(
				"target result.*success criteria.*evidence.*output shape.*stop condition",
			),
...
export const PROMPT_REFACTOR_INVARIANT_CONTRACTS: GuidanceSurfaceContract[] = [
	...
	},
	{
		...
		requiredPatterns: [
			...
			rx(
				"Do not begin Ralph execution work|do not begin implementation|must not implement|no implementation",
			),
		],
	},
	{
	...
	},
	{
		...
		requiredPatterns: [
			rx(
				"\\$deep-interview\\s*->\\s*\\$ralplan\\s*->\\s*\\$ultragoal.*\\$code-review\\s*->\\s*\\$ultraqa",
			),
	...
	},
	{
		...
		requiredPatterns: [
			rx("read-only"),
			rx("cannot create, modify, or delete files"),
		],
	},
	{
		...
		requiredPatterns: [
			rx("source|citation|cite"),
			rx("official documentation|primary source"),
		],
	},
];
