...
#   green build · cyan run · yellow test · magenta lint/check ·
#   blue rust · red clean · dim git/util/meta
group_tint() { # <group> -> raw ANSI sequence (fzf renders with --ansi)
  case "$1" in
    build) printf '\033[32m' ;;
    run) printf '\033[36m' ;;
    test) printf '\033[33m' ;;
    lint | check) printf '\033[35m' ;;
    rust) printf '\033[34m' ;;
...
rows() {
  ...
  while IFS=$'\t' read -r name group doc params; do
    ...
    printf -v tag '%-16s %b%-9s\033[0m %s%s' \
