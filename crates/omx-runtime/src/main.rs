...
fn process_identity(pid: u32) -> Result<ProcessIdentityResult, ProcessIdentityError> {
    ...
    const PROC_PIDTBSDINFO_SIZE: c_int = std::mem::size_of::<ProcBsdInfo>() as c_int;

    // Exact struct proc_bsdinfo layout from the macOS SDK's sys/proc_info.h.
    #[repr(C)]
    struct ProcBsdInfo {
        pbi_flags: u32,
        pbi_status: u32,
    ...
    let mut buf: ProcBsdInfo = unsafe { std::mem::zeroed() };
    let ret = unsafe {
        proc_pidinfo(
            pid as c_int,
            PROC_PIDTBSDINFO,
