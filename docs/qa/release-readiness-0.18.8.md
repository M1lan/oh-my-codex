...
- [x] `pnpm run build` — PASS before and after test-script hardening, `.omx/release-0.18.8/logs/build.log` and `build-after-test-script-fix.log`.
- [x] `pnpm run lint` — PASS, `.omx/release-0.18.8/logs/lint-after-script-fix.log`.
- [x] `pnpm run check:no-unused` — PASS, `.omx/release-0.18.8/logs/no-unused-after-script-fix.log`.
- [x] `pnpm run verify:native-agents` — PASS via `.omx/release-0.18.8/logs/verify-native-agents.log` and full compiled CI.
- [x] `pnpm run sync:plugin` / `pnpm run sync:plugin:check` — PASS, `.omx/release-0.18.8/logs/sync-plugin.log`, `sync-plugin-check-targeted-final.log`.
- [x] `pnpm run verify:plugin-bundle` — PASS, `.omx/release-0.18.8/logs/verify-plugin-bundle.log` and full compiled CI.
...
- [x] `pnpm test` — PASS after unsetting `USE_OMX_EXPLORE_CMD`, `.omx/release-0.18.8/logs/npm-test-unset-explore-env.log` (`5745` tests, `5745` pass). Earlier contaminated attempts are retained in logs and were not used as final evidence.
- [x] `pnpm run test:ci:compiled` — PASS, `.omx/release-0.18.8/logs/test-ci-compiled-after-fix-2.log` (`5745` tests, `5745` pass).
- [x] `pnpm run test:compat:node` — PASS, `.omx/release-0.18.8/logs/test-compat-node.log`.
- [x] `pnpm run test:sparkshell` — PASS, `.omx/release-0.18.8/logs/test-sparkshell.log`.
- [x] `pnpm run test:explore` — PASS, `.omx/release-0.18.8/logs/test-explore.log`.
...
- [x] `pnpm run test:team:cross-rebase-smoke:compiled` — PASS after routing through the runtime-env sanitizer, `.omx/release-0.18.8/logs/team-cross-rebase-smoke-compiled-official.log`.
...
- [x] `pnpm run test:recent-bug-regressions:compiled` — PASS after routing through the runtime-env sanitizer, `.omx/release-0.18.8/logs/recent-bug-regressions-compiled-official.log`.
- [x] `pnpm run test:team:worker-runtime-identity:compiled` — PASS after routing through the runtime-env sanitizer, `.omx/release-0.18.8/logs/team-worker-runtime-identity-compiled-official.log`.
- [x] `pnpm run test:plugin-boundaries:compiled` — PASS, `.omx/release-0.18.8/logs/plugin-boundaries-compiled-official.log`.
- [x] `pnpm run test:ralph-persistence:compiled` — PASS after routing through the runtime-env sanitizer, `.omx/release-0.18.8/logs/ralph-persistence-compiled-official.log`.
- [x] `pnpm run test:explicit-terminal-contract:compiled` — PASS after routing through the runtime-env sanitizer, `.omx/release-0.18.8/logs/explicit-terminal-contract-compiled-official.log`.
...
- [x] `pnpm run test:reply-listener:live` — PASS, `.omx/release-0.18.8/logs/reply-listener-live.log`.
...
- [x] npm latest proof: fallback run `26744847619` published the exact `v0.18.8` tag artifact without provenance after the Fulcio outage; `pnpm view oh-my-codex version dist-tags --json` returned `0.18.8` / `latest: 0.18.8`.
...
- No local `pnpm publish` command was run. The normal tag-triggered release workflow attempted `pnpm publish --provenance`; after three Fulcio `ECONNRESET` failures, a temporary GitHub Actions fallback published without provenance using the repository `NPM_TOKEN` and was removed after proof.
...
- Release workflow run `26742594280` built and uploaded native assets, smoke-verified native assets, and passed packed global install smoke. The workflow failed only at `pnpm publish --provenance` because `fulcio.sigstore.dev` repeatedly reset TLS connections while npm created the Sigstore signing certificate (`CA_CREATE_SIGNING_CERTIFICATE_ERROR`, `ECONNRESET`).
- Temporary fallback workflow run `26744847619` checked out `v0.18.8`, verified package version `0.18.8`, and published to npm with `pnpm publish --access public --provenance=false` using the repository `NPM_TOKEN`. This fallback was required because Fulcio was unreachable from the self-hosted runner and local curl checks also returned TLS connection resets.
- npm proof after fallback: `pnpm view oh-my-codex version dist-tags --json` returned `{"version":"0.18.8","dist-tags":{"latest":"0.18.8"}}`.
