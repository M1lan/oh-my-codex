...
| Initial build/lint/no-unused | PASS — `pnpm run build && pnpm run lint && pnpm run check:no-unused`. |
...
| Final full release gate | PASS — `pnpm run build && pnpm run lint && pnpm run check:no-unused && node --test ... && cargo test && git diff --check`; targeted Node release suite passed 467 tests and Rust test suites passed. |
...
| pnpm pack dry-run | PASS — `pnpm pack --dry-run` produced `oh-my-codex-0.17.0.tgz` dry-run output after prepack verification, shasum `64b6cb807fab9ddb6213dde72472df137c8e81db`. |
| GitHub CI | PENDING. |
| GitHub release / pnpm publish | PENDING. |
