...
- Version bump synced across `package.json`, `pnpm-lock.yaml`, `Cargo.toml [workspace.package]`, `Cargo.lock`, and `plugins/oh-my-codex/.codex-plugin/plugin.json`; no stray `0.18.18` references remain in tracked toml/json/lock files.
- `pnpm run sync:plugin` + `node dist/scripts/sync-plugin-mirror.js --check` — verified 29 canonical skill directories and plugin metadata.
...
- `pnpm pack --dry-run` — recorded below.
...
- Publication steps (merge to `main`, tag push, tag-triggered release workflow, GitHub release, native asset attachment, `pnpm publish`) require maintainer/CI action and credentials and are intentionally not performed autonomously.
