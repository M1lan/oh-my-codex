...
Release-prep gates include version sync for `v0.19.0`, TS build, native-agent verification, plugin mirror/bundle checks, catalog docs check, the full Rust workspace test suite (rerun repeatedly to prove the flake fix), the full node test suite, and `pnpm pack --dry-run`. Branch CI, dev/main promotion, tag-triggered release workflow, GitHub release proof, and npm publication proof are appended to readiness evidence after publication.
